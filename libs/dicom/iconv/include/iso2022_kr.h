...
static int iso2022_kr_mbtowc(conv_t conv, ucs4_t* pwc, const unsigned char* s,
                             size_t n) {
...
static int iso2022_kr_wctomb(conv_t conv, unsigned char* r, ucs4_t wc,
                             size_t n) {
  ...
  if (ret != RET_ILUNI) {
    ...
    if (buf[0] < 0x80 && buf[1] < 0x80) {
      int count = (state2 == STATE2_DESIGNATED_KSC5601 ? 0 : 4) +
