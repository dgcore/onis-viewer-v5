...
static int iso2022_jp3_mbtowc(conv_t conv, ucs4_t* pwc, const unsigned char* s,
                              size_t n) {
    ...
    switch (state) {
      ...
      case STATE_JISX02132:
        if (n < count + 2)
          goto none;
        if (s[0] < 0x80 && s[1] < 0x80) {
          ucs4_t wc = jisx0213_to_ucs4(
              ((state - STATE_JISX02131 + 1) << 8) + s[0], s[1]);
          if (wc) {
            if (wc < 0x80) {
...
static int iso2022_jp3_flushwc(conv_t conv, ucs4_t* pwc) {
  ucs4_t last_wc = conv->istate >> 3;
  if (last_wc) {
    /* Output the buffered character. */
    conv->istate &= 7;
...
#define iso2022_jp3_comp_table02e9_idx \
  (iso2022_jp3_comp_table02e5_idx + iso2022_jp3_comp_table02e5_len)
#define iso2022_jp3_comp_table02e9_len 1
    {0x2b60, 0x2b66}, /* 0x12B66 = 0x12B60 U+02E9 */
#define iso2022_jp3_comp_table0300_idx \
  (iso2022_jp3_comp_table02e9_idx + iso2022_jp3_comp_table02e9_len)
...
#define iso2022_jp3_comp_table0301_idx \
  (iso2022_jp3_comp_table0300_idx + iso2022_jp3_comp_table0300_len)
...
#define iso2022_jp3_comp_table309a_idx \
  (iso2022_jp3_comp_table0301_idx + iso2022_jp3_comp_table0301_len)
...
#define SPLIT_STATE                    \
  unsigned short lasttwo = state >> 3; \
  state_t prevstate = state >> 19;     \
  state &= 7
#define COMBINE_STATE state |= (prevstate << 19) | (lasttwo << 3)
#define COMBINE_STATE_NO_LASTTWO \
  /* assume lasttwo == 0, then prevstate is ignored */

static int iso2022_jp3_wctomb(conv_t conv, unsigned char* r, ucs4_t wc,
                              size_t n) {
  int count = 0;
  unsigned char buf[2];
  unsigned short jch;
...
static int iso2022_jp3_reset(conv_t conv, unsigned char* r, size_t n) {
  state_t state = conv->ostate;
  SPLIT_STATE;
  {
    int count = (lasttwo ? (prevstate != state ? 3 : 0) + 2 : 0) +
