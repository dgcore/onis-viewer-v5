...
static int unicode_transliterate(conv_t cd, ucs4_t wc, unsigned char* outptr,
                                 size_t outleft) {
  ...
  if (plocals->l_errno == 0) {
    ...
    for (; buflen > 0; buf++, buflen--) {
      ...
      if (cd->discard_ilseq) {
        ...
        locals.l_errno = 0;
        cd->fallbacks.uc_to_mb_fallback(wc, uc_to_mb_write_replacement, &locals,
                                        cd->fallbacks.data);
        if (locals.l_errno != 0) {
...
static size_t unicode_loop_convert(iconv_t icd, const char** inbuf,
  ...
  while (inleft > 0) {
    ...
    if (incount < 0) {
      if ((unsigned int)(-1 - incount) % 2 ==
          (unsigned int)(-1 - RET_ILSEQ) % 2) {
        ...
        if (cd->discard_ilseq) {
          ...
          cd->fallbacks.mb_to_uc_fallback((const char*)inptr + incount,
                                          incount2, mb_to_uc_write_replacement,
                                          &locals, cd->fallbacks.data);
          if (locals.l_errno != 0) {
            inptr += incount;
            inleft -= incount;
...
static size_t unicode_loop_reset(iconv_t icd, char** outbuf,
                                 size_t* outbytesleft) {
  conv_t cd = (conv_t)icd;
    ...
    if (cd->ifuncs.xxx_flushwc) {
      ...
      if (cd->ifuncs.xxx_flushwc(cd, &wc)) {
        ...
        if (cd->discard_ilseq) {
          ...
          locals.l_outbytesleft = outleft;
          locals.l_errno = 0;
          cd->fallbacks.uc_to_mb_fallback(wc, uc_to_mb_write_replacement,
                                          &locals, cd->fallbacks.data);
          if (locals.l_errno != 0) {
            cd->istate = last_istate;
            errno = locals.l_errno;
