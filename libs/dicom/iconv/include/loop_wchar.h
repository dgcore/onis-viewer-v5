/*
 * Copyright (C) 2000-2002, 2005-2006, 2008-2009, 2011 Free Software Foundation,
 * Inc. This file is part of the GNU LIBICONV Library.
 *
...
static size_t wchar_from_loop_convert(iconv_t icd, const char** inbuf,
    ...
    while (inleft >= sizeof(wchar_t)) {
      ...
      if (count == (size_t)(-1)) {
        ...
        if (wcd->parent.discard_ilseq) {
          ...
          locals.l_errno = 0;
          for (fallback_inptr = (const wchar_t*)*inbuf; fallback_inptr <= inptr;
               fallback_inptr++)
            wcd->parent.fallbacks.wc_to_mb_fallback(
                *fallback_inptr, wc_to_mb_write_replacement, &locals,
      ...
      if (count == 0) {
        ...
        size_t res = unicode_loop_convert(&wcd->parent, &bufptr, &bufleft,
                                          &outptr, &outleft);
        if (res == (size_t)(-1)) {
          if (errno == EILSEQ)
            /* Invalid input. */
...
static size_t wchar_from_loop_reset(iconv_t icd, char** outbuf,
                                    size_t* outbytesleft) {
  struct wchar_conv_struct* wcd = (struct wchar_conv_struct*)icd;
  if (outbuf == NULL || *outbuf == NULL) {
    ...
    if (!mbsinit(&wcd->state)) {
      ...
      if (bufcount == (size_t)(-1) || bufcount == 0 ||
          buf[bufcount - 1] != '\0')
        abort();
      else {
        const char* bufptr = buf;
...
static size_t wchar_to_loop_convert(iconv_t icd, const char** inbuf,
                                    size_t* inbytesleft, char** outbuf,
                                    size_t* outbytesleft) {
  struct wchar_conv_struct* wcd = (struct wchar_conv_struct*)icd;
  size_t result = 0;
  while (*inbytesleft > 0) {
    ...
    for (incount = 1;;) {
      ...
      size_t res = unicode_loop_convert(&wcd->parent, &inptr, &inleft, &bufptr,
                                        &bufleft);
      if (res == (size_t)(-1)) {
        if (errno == EILSEQ)
          /* Invalid input. */
        ...
        if (res == (size_t)(-2)) {
          ...
          if (res == (size_t)(-1)) {
            ...
            if (wcd->parent.discard_ilseq) {
              ...
              locals.l_outbytesleft = *outbytesleft;
              locals.l_errno = 0;
              wcd->parent.fallbacks.mb_to_wc_fallback(
                  *inbuf, incount, mb_to_wc_write_replacement, &locals,
                  wcd->parent.fallbacks.data);
              if (locals.l_errno != 0) {
                errno = locals.l_errno;
                return -1;
...
static size_t wchar_to_loop_reset(iconv_t icd, char** outbuf,
                                  size_t* outbytesleft) {
  struct wchar_conv_struct* wcd = (struct wchar_conv_struct*)icd;
...
static size_t wchar_id_loop_convert(iconv_t icd, const char** inbuf,
                                    size_t* inbytesleft, char** outbuf,
                                    size_t* outbytesleft) {
...
static size_t wchar_id_loop_reset(iconv_t icd, char** outbuf,
                                  size_t* outbytesleft) {
  return 0;
}
