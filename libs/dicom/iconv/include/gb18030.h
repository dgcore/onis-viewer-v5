...
static int gb18030_mbtowc(conv_t conv, ucs4_t* pwc, const unsigned char* s,
                          size_t n) {
  int ret;
  ...
  /* User-defined characters range U+E000..U+E765 */
  {
    ...
    if ((c1 >= 0xaa && c1 <= 0xaf) || (c1 >= 0xf8 && c1 <= 0xfe)) {
      if (n >= 2) {
        ...
        if (c2 >= 0xa1 && c2 <= 0xfe) {
          *pwc =
              0xe000 + 94 * (c1 >= 0xf8 ? c1 - 0xf2 : c1 - 0xaa) + (c2 - 0xa1);
          return 2;
        }
      } else
        return RET_TOOFEW(0);
          ...
          if (n >= 3) {
            ...
            if (c3 >= 0x81 && c3 <= 0xfe) {
              if (n >= 4) {
                ...
                if (c4 >= 0x30 && c4 <= 0x39) {
                  unsigned int i =
                      (((c1 - 0x90) * 10 + (c2 - 0x30)) * 126 + (c3 - 0x81)) *
                          10 +
                      (c4 - 0x30);
                  if (i >= 0 && i < 0x100000) {
...
static const unsigned short gb18030_pua2charset[31 * 3] = {
    ...
    0xe7e2, 0xe7e2, 0xa958, 0xe7e3, 0xe7e3, 0xa95b,
    ...
    0xe81e, 0xe81e, 0xfe59, 0xe826, 0xe826, 0xfe61,
    0xe82b, 0xe82c, 0xfe66, /*.. 0xfe67, */
    0xe831, 0xe832, 0xfe6c, /*.. 0xfe6d, */
    0xe83b, 0xe83b, 0xfe76, 0xe843, 0xe843, 0xfe7e,
    0xe854, 0xe855, 0xfe90, /*.. 0xfe91, */
    0xe864, 0xe864, 0xfea0,
};

static int gb18030_wctomb(conv_t conv, unsigned char* r, ucs4_t wc, size_t n) {
  int ret;
      ...
      if (wc < 0xe766) {
        ...
        while (k1 < k2) {
          ...
          if (wc < gb18030_pua2charset[k * 3 + 0])
            ...
            unsigned short c = gb18030_pua2charset[k * 3 + 2] +
                               (wc - gb18030_pua2charset[k * 3 + 0]);
            r[0] = (c >> 8);
            r[1] = (c & 0xff);
            return 2;
          }
