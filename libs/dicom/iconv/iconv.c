 ...
 */
struct loop_funcs {
  size_t (*loop_convert)(iconv_t icd, const char** inbuf, size_t* inbytesleft,
...
enum {
#define DEFENCODING(xxx_names, xxx, xxx_ifuncs1, xxx_ifuncs2, xxx_ofuncs1, \
                    xxx_ofuncs2)                                           \
  ei_##xxx,
...
#define S(tag, name, encoding_index) \
  {(int)(long)&((struct stringpool2_t*)0)->stringpool_##tag, encoding_index},
#include "aliases2.h"
#undef S
};
#ifdef __GNUC__
...
inline
  ...
  unsigned int count;
  for (ptr = sysdep_aliases,
      count = sizeof(sysdep_aliases) / sizeof(sysdep_aliases[0]);
       count > 0; ptr++, count--)
...
//           char* * outbuf, size_t *outbytesleft)
size_t iconv(iconv_t icd, const char** inbuf, size_t* inbytesleft,
             char** outbuf, size_t* outbytesleft)
// DIGITALCORE
{
  ...
  if (inbuf == NULL || *inbuf == NULL)
    ...
    return cd->lfuncs.loop_convert(icd, (const char**)inbuf, inbytesleft,
                                   outbuf, outbytesleft);
}
 ...
 */
typedef int verify_size_1[2 * (sizeof(struct conv_struct) <=
                               sizeof(iconv_allocation_t)) -
                          1];
typedef int verify_size_2[2 * (sizeof(struct wchar_conv_struct) <=
                               sizeof(iconv_allocation_t)) -
                          1];
...
void iconvlist(int (*do_one)(unsigned int namescount, const char* const* names,
                             void* data),
               void* data) {
...
#define aliascount (aliascount1 + aliascount2)
  ...
  size_t num_aliases;
  {
    ...
    for (i = 0; i < aliascount1; i++) {
      ...
      if (p->name >= 0 && p->encoding_index != ei_local_char &&
...
#undef iconv
#undef iconv_close
strong_alias(libiconv_open, iconv_open) strong_alias(libiconv, iconv)
