...
/* Do computations that are needed before master selection phase */
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    /* Size in data units */
    compptr->width_in_data_units = (JDIMENSION)jdiv_round_up(
        (long)cinfo->image_width * (long)compptr->h_samp_factor,
        (long)(cinfo->max_h_samp_factor * data_unit));
    compptr->height_in_data_units = (JDIMENSION)jdiv_round_up(
  ...
  cinfo->total_iMCU_rows = (JDIMENSION)jdiv_round_up(
      (long)cinfo->image_height, (long)(cinfo->max_v_samp_factor * data_unit));
}
  ...
  for (scanno = 1; scanno <= cinfo->num_scans; scanptr++, scanno++) {
    ...
    if (cinfo->process == JPROC_LOSSLESS) {
      ...
      if (Ss < 1 || Ss > 7 || /* predictor selector */
          Se != 0 || Ah != 0 || Al < 0 ||
          Al >= cinfo->data_precision) /* point transform */
...
/* cinfo->comps_in_scan and cinfo->cur_comp_info[] are already set */
{
  ...
  if (cinfo->comps_in_scan == 1) {
    ...
    cinfo->MCUs_per_row = (JDIMENSION)jdiv_round_up(
        (long)cinfo->image_width, (long)(cinfo->max_h_samp_factor * data_unit));
