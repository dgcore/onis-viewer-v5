...
decode_mcus(j_decompress_ptr cinfo, JDIFFIMAGE diff_buf, JDIMENSION MCU_row_num,
  ...
  if (entropy->insufficient_data) {
    for (ptrn = 0; ptrn < entropy->num_output_ptrs; ptrn++)
      jzero_far(
          (void FAR*)entropy->output_ptr[ptrn],
...
jinit_lhuff_decoder(j_decompress_ptr cinfo) {
  ...
  entropy = (lhuff_entropy_ptr)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(lhuff_entropy_decoder));
