 ...
 */
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
       ci++, compptr++) {
    rgroup =
        (compptr->v_samp_factor * compptr->codec_data_unit) /
  ...
  switch (mymain->context_state) {
    case CTX_POSTPONED_ROW:
      ...
      (*cinfo->post->post_process_data)(
          cinfo, mymain->xbuffer[mymain->whichptr], &mymain->rowgroup_ctr,
          mymain->rowgroups_avail, output_buf, out_row_ctr, out_rows_avail);
...
jinit_d_main_controller(j_decompress_ptr cinfo, boolean need_full_buffer) {
  ...
  mymain = (my_main_ptr)(*cinfo->mem->alloc_small)(
