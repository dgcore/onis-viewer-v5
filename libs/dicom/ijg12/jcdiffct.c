...
compress_data(j_compress_ptr cinfo, JSAMPIMAGE input_buf) {
  ...
  for (yoffset = diff->MCU_vert_offset; yoffset < diff->MCU_rows_per_iMCU_row;
    ...
    if (MCU_col_num == 0) {
      for (comp = 0; comp < cinfo->comps_in_scan; comp++) {
        ...
        if (diff->iMCU_row_num < last_iMCU_row)
          ...
          samp_rows =
              (int)(compptr->height_in_data_units % compptr->v_samp_factor);
          if (samp_rows == 0)
            samp_rows = compptr->v_samp_factor;
          else {
        ...
        for (samp_row = 0; samp_row < samp_rows; samp_row++) {
          (*losslsc->scaler_scale)(cinfo, input_buf[ci][samp_row],
                                   diff->cur_row[ci], samps_across);
          (*losslsc->predict_difference[ci])(
              cinfo, ci, diff->cur_row[ci], diff->prev_row[ci],
              diff->diff_buf[ci][samp_row], samps_across);
    ...
    MCU_count = (*losslsc->entropy_encode_mcus)(
...
jinit_c_diff_controller(j_compress_ptr cinfo, boolean need_full_buffer) {
  ...
  diff = (c_diff_ptr)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(c_diff_controller));
