...
METHODDEF(void)
jpeg_undifference1(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                   JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  UNDIFFERENCE_1D(INITIAL_PREDICTOR2);
}

METHODDEF(void)
jpeg_undifference2(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                   JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  UNDIFFERENCE_2D(PREDICTOR2);
}

METHODDEF(void)
jpeg_undifference3(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                   JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  UNDIFFERENCE_2D(PREDICTOR3);
}

METHODDEF(void)
jpeg_undifference4(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                   JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  UNDIFFERENCE_2D(PREDICTOR4);
}

METHODDEF(void)
jpeg_undifference5(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                   JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  SHIFT_TEMPS
...
METHODDEF(void)
jpeg_undifference6(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                   JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  SHIFT_TEMPS
...
METHODDEF(void)
jpeg_undifference6a(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                    JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  SHIFT_TEMPS
...
METHODDEF(void)
jpeg_undifference7(j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                   JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width) {
  SHIFT_TEMPS
...
jpeg_undifference_first_row(j_decompress_ptr cinfo, int comp_index,
  ...
  switch (cinfo->Ss) {
    ...
    case 6:
       ...
       * Enables workaround for faulty images with integer overflow in
       * predictor 6.
       */
...
predict_start_pass(j_decompress_ptr cinfo) {
   ...
   */
  if (cinfo->Ss < 1 || cinfo->Ss > 7 || cinfo->Se != 0 || cinfo->Ah != 0 ||
      cinfo->Al > 15) /* need not check for < 0 */
    ERREXIT4(cinfo, JERR_BAD_LOSSLESS, cinfo->Ss, cinfo->Se, cinfo->Ah,
             cinfo->Al);
