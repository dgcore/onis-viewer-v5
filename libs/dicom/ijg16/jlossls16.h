...
#define PREDICTOR5 \
  (int)((IJG_INT32)Ra + RIGHT_SHIFT((IJG_INT32)Rb - (IJG_INT32)Rc, 1))
#define PREDICTOR6 \
  (int)((IJG_INT32)Rb + RIGHT_SHIFT((IJG_INT32)Ra - (IJG_INT32)Rc, 1))
#define PREDICTOR7 (int)RIGHT_SHIFT((IJG_INT32)Ra + (IJG_INT32)Rb, 1)

/* This is an incorrect predictor that causes an overflow for images with 16
 * bits/pixel. There is a known implementation of JPEG lossless that creates
 * such incorrect images, and we need this predictor to be able to correctly
 * decode such incorrect images.
 */
#define PREDICTOR6A (int)((INT16)Rb + RIGHT_SHIFT((INT16)Ra - (INT16)Rc, 1))

typedef JMETHOD(void, predict_difference_method_ptr,
                (j_compress_ptr cinfo, int ci, JSAMPROW input_buf,
                 JSAMPROW prev_row, JDIFFROW diff_buf, JDIMENSION width));

typedef JMETHOD(void, scaler_method_ptr,
                (j_compress_ptr cinfo, int ci, JSAMPROW input_buf,
                 JSAMPROW output_buf, JDIMENSION width));
...
typedef struct {
  ...
  /* Entropy encoding */
  JMETHOD(JDIMENSION, entropy_encode_mcus,
          (j_compress_ptr cinfo, JDIFFIMAGE diff_buf, JDIMENSION MCU_row_num,
  ...
  JMETHOD(void, scaler_scale,
          (j_compress_ptr cinfo, JSAMPROW input_buf, JSAMPROW output_buf,
           JDIMENSION width));
...
typedef JMETHOD(void, predict_undifference_method_ptr,
                (j_decompress_ptr cinfo, int comp_index, JDIFFROW diff_buf,
                 JDIFFROW prev_row, JDIFFROW undiff_buf, JDIMENSION width));
...
typedef struct {
  ...
  JMETHOD(JDIMENSION, entropy_decode_mcus,
          (j_decompress_ptr cinfo, JDIFFIMAGE diff_buf, JDIMENSION MCU_row_num,
           JDIMENSION MCU_col_num, JDIMENSION nMCU));
  ...
  JMETHOD(void, scaler_scale,
          (j_decompress_ptr cinfo, JDIFFROW diff_buf, JSAMPROW output_buf,
           JDIMENSION width));
