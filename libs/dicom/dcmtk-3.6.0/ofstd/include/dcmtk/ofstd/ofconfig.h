 ...
 *      The tree has a depth defined at instantiation time (by default,
 * OFConfigFile_MaxLevel), not including the (imaginary) root node.
...
public:
  ...
  OFConfigFileCursor(unsigned int maxLevel)
      : array_(NULL), maxLevel_(maxLevel) {
    clear();
  }
  ...
  const char* getKeyword(unsigned int level) const {
    if ((level <= maxLevel_) && array_ && array_[level])
   ...
   */
  void insert(unsigned int level, OFConfigFileNode*& newnode,
...
public:
   ...
   *  @param maxLevel depth of the tree maintained in this config file, default
   * 2
   *  @param commentChar character to start comment lines, default '#'
   *  @param orderedMode mode flag for processing configuration files.
   *    See documentation of member variable orderedMode_
   */
  OFConfigFile(FILE* infile, unsigned int maxLevel = OFConfigFile_MaxLevel,
   ...
   *  @param key2 level 2 section key, i.e. [[KEY]]. If omitted, section 2
   * remains unchanged.
   *  @param key2 level 3 section key, i.e. [[[KEY]]]. If omitted, section 3
   * remains unchanged.
   */
...
private:
   ...
   *  first of which will be found by search-by-keyword methods such as
   * set_section, select_section or get_entry. In ordered mode, the tree is
   * maintained in sorted order and a new entry will replace an older entry with
   * identical keyword. This permits multiple configuration files to "overlay"
   * each other but is somewhat slower when reading the configuration file.
   */
  OFBool orderedMode_;
};
