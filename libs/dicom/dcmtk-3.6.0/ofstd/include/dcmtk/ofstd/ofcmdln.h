...
struct OFCmdParam {
  ...
  OFCmdParam(const char* param, const char* descr, const E_ParamMode mode)
      : ParamName(param), ParamDescription(descr), ParamMode(mode) {}
...
public:
   ...
   */
  OFBool addOption(const char* longOpt, const char* shortOpt,
                   const int valueCount, const char* valueDescr,
                   const char* optDescr, const int flags = 0);
   ...
   ** @return OFTrue if succesfully added
   */
  OFBool addOption(const char* longOpt, const int valueCount,
   ...
   ** @return OFTrue if succesfully added
   */
  OFBool addOption(const char* longOpt, const char* optDescr,
                   const int flags = 0);
   ...
   *  @param  shortCols  minimum width of the short option column
   */
  void addGroup(const char* name, const int longCols = 0,
   ...
   *  @param  shortCols  minimum width of the short option column
   */
  void addSubGroup(const char* name, const int longCols = 0,
                   const int shortCols = 0);
   ...
   */
  OFBool addParam(
      const char* param, const char* descr,
   ...
   ** @return OFTrue if successful, OFFalse otherwise
   */
  OFBool addParam(const char* param, const OFCmdParam::E_ParamMode mode =
                                         OFCmdParam::PM_Mandatory);
   ...
   *
   ** @return status of get/conversion, PVS_Normal if successful (use
   *getStatusString for error string)
   */
  E_ParamValueStatus getParam(const int pos, OFCmdSignedInt& value);

  /** get value of specified parameter as signed integer and checks for given
   *boundary
   *
   ** @param  pos    position of parameter (1..n)
   *  @param  value  reference to variable where the value should be stored
   *  @param  low    minimum boundary for value (used for range check)
   *  @param  incl   if OFTrue 'low' value is valid (included), otherwise
   *invalid
   *
   ** @return status of get/conversion, PVS_Normal if successful (use
   *getStatusString for error string)
   */
  E_ParamValueStatus getParamAndCheckMin(const int pos, OFCmdSignedInt& value,
                                         const OFCmdSignedInt low,
                                         const OFBool incl = OFTrue);

  /** gets value of specified parameter as signed integer and checks for given
   *boundaries
   *
   ** @param  pos    position of parameter (1..n)
   *  @param  value  reference to variable where the value should be stored
   *  @param  low    minimum boundary for value (used for range check, boundary
   *included)
   ...
   *  NB: If command line argument specifies a negative value the result depends
   *on the signed->unsigned conversion implemented in sscanf() !!
   *
   ** @param  pos    position of parameter (1..n)
   *  @param  value  reference to variable where the value should be stored
   *
  ...
  /** gets value of specified parameter as unsigned integer and checks for given
   *boundary. NB: If command line argument specifies a negative value the result
   *depends on the signed->unsigned conversion implemented in sscanf() !!
   *
   ** @param  pos    position of parameter (1..n)
   *  @param  value  reference to variable where the value should be stored
   *  @param  low    minimum boundary for value (used for range check)
   ...
   *getStatusString for error string)
   */
  E_ParamValueStatus getParamAndCheckMin(const int pos, OFCmdUnsignedInt& value,
                                         const OFCmdUnsignedInt low,
                                         const OFBool incl = OFTrue);

  /** gets value of specified parameter as unsigned integer and checks for given
   *boundaries. NB: If command line argument specifies a negative value the
   *result depends on the signed->unsigned conversion implemented in sscanf() !!
   *
   ** @param  pos    position of parameter (1..n)
   *  @param  value  reference to variable where the value should be stored
   *  @param  low    minimum boundary for value (used for range check, boundary
  ...
  /** gets value of specified parameter as floating point and checks for given
   *boundary
   *
   ** @param  pos    position of parameter (1..n)
   *  @param  value  reference to variable where the value should be stored
   ...
   *getStatusString for error string)
   */
  E_ParamValueStatus getParamAndCheckMin(const int pos, OFCmdFloat& value,
   ...
   *getStatusString for error string)
   */
  E_ParamValueStatus getParamAndCheckMinMax(const int pos, OFCmdFloat& value,
                                            const OFCmdFloat low,
  ...
  // --- find/get option (option is an argument which starts with an option
  // character, see above)

  /** checks whether specified option exists in the command line.
   *  The search process starts from the last command line argument (which is a
   *direct predecessor of the optional defined reference parameter 'pos') and
   *goes to the first one.
   *
   ** @param  longOpt    name of option (in long format) to be checked
   *  @param  pos        position of reference parameter
   *                     (default: all parameters; if value is negative option
   *must be a direct predecessor of the specified reference parameter '-pos', no
   *further search processes will be performed; if search direction is left to
   *right this parameter is ignored, also option blocks are not supported in
   *this case)
   *  @param  mode       find option mode (used to support option blocks and
   *reverse search direction)
   *
   ** @return OFTrue if option exists, OFFalse otherwise
   */
  OFBool findOption(const char* longOpt, const signed int pos = 0,
  ...
  /** starts an option block which can be used to support mutually exclusive
   * options.
   */
  void beginOptionBlock();

  /** ends an option block which can be used to support mutually exclusive
   * options.
   */
  void endOptionBlock();
   ...
   *
   ** @param  value  reference to signed integer variable where the value should
   *be stored
   *
   ** @return status of get/conversion, VS_Normal if successful (use
   *getStatusString for error string)
   */
  E_ValueStatus getValue(OFCmdSignedInt& value);

  /** returns next argument as a signed integer value and checks for given
   *boundary
   *
   ** @param  value  reference to signed integer variable where the value should
   *be stored
   ...
   *
   ** @param  value  reference to unsigned integer variable where the value
   *should be stored
   *
   ** @return status of get/conversion, VS_Normal if successful (use
   *getStatusString for error string)
  ...
  /** returns next argument as an unsigned integer value and checks for given
   *boundary
   *
   ** @param  value  reference to unsigned integer variable where the value
   *should be stored
   ...
   *
   ** @param  value  reference to floating point variable where the value should
   *be stored
   *
   ** @return status of get/conversion, VS_Normal if successful (use
   *getStatusString for error string)
  ...
  /** returns next argument as a floating point value and checks for given
   *boundary
   *
   ** @param  value  reference to floating point variable where the value should
   *be stored
   ...
   *getStatusString for error string)
   */
  E_ValueStatus getValueAndCheckMin(OFCmdFloat& value, const OFCmdFloat low,
                                    const OFBool incl = OFTrue);
   ...
   *getStatusString for error string)
   */
  E_ValueStatus getValueAndCheckMinMax(OFCmdFloat& value, const OFCmdFloat low,
                                       const OFCmdFloat high);

  /** returns next argument as a C string
   *
   ** @param  value  reference to C string variable where the value should be
   *stored
   *
   ** @return status of get/conversion, VS_Normal if successful (use
   *getStatusString for error string)
   ...
   *  Additionally create internal structures for evaluation and return status
   *indicating any errors occuring during the parse process.
   *
   ...
   *  @param  flags     optional flags affecting the parse process (see PF_xxx
   *below)
   *  @param  startPos  index of first argument which should be parsed (starting
   *from 0, default: 1)
   *
   ** @return status of parse process, PS_Normal if successful (use
   *getStatusString for error string). If an exclusive option is used the status
   *code PS_ExclusiveOption is returned.
   */
  E_ParseStatus parseLine(int argCount, char* argValue[], const int flags = 0,
                          const int startPos = 1);

  // --- get usage/status strings

  /** returns command line syntax as a C++ string (single text line).
   *  Text consists of "[options]" (if defined by addOption) and all parameters
   *defined by addParam.
   *
   ...
   *  Text consists of group headings, short and long option names, option
   *descriptions and additional option values.
   *
   ** @param  optionStr  reference to C++ string where the text should be stored
   */
  void getOptionString(OFString& optionStr) const;

  /** returns description of all parameters as a C++ string (multiple text
   *lines). Text consists of heading "parameters:" and parameter names and
   *descriptions.
   *
  ...
  /// disable support for command files ("@filename") containing additional
  /// arguments
...
protected:
   ...
   */
  void unpackColumnValues(const int value, unsigned int& longCols,
                          unsigned int& shortCols) const;

#ifdef HAVE_WINDOWS_H
  /** expands wildcards in specified parameter.
   *  Very similar to Unix environments, stores each resulting parameter in the
   * argument/parameter list
   */
  void expandWildcards(const OFString& param, int directOption = 0);
#endif

  /** checks whether number of parameters in parsed command line is within the
   * range of min/max (see below)
   */
  ...
  /** returns name of parameter which is missed in the parsed command line (used
   * for error output)
   */
...
private:
  ...
  /// OFTrue if an "exclusive" option is used in the command line, OFFalse
  /// otherwise
