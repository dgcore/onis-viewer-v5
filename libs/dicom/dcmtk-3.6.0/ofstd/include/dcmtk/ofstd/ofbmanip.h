 ...
 *  This class is used to perform platform independent operations on typed
 * memory areas.
 */
template <class T>
...
public:
...
#ifdef HAVE_MEMCPY
    memcpy(OFstatic_cast(void*, dest), OFstatic_cast(const void*, src),
           OFstatic_cast(size_t, count) * sizeof(T));
#else
    unsigned long i;
    const T* p = src;
    T* q = dest;
...
#ifdef HAVE_MEMMOVE
    memmove(OFstatic_cast(void*, dest), OFstatic_cast(const void*, src),
            OFstatic_cast(size_t, count) * sizeof(T));
#else
    if (src == dest)
      return;
  ...
  static void setMem(T* dest, const T value, const unsigned long count) {
    ...
    if ((value == 0) || (sizeof(T) == sizeof(unsigned char)))
      memset(OFstatic_cast(void*, dest), OFstatic_cast(int, value),
             OFstatic_cast(size_t, count) * sizeof(T));
    else
#endif
    {
