...
/** This template class provides a data structure and operations for
 * administrating an unordered set of elements of an arbitrary type. Note the
 * following properties of this class:
 *  - an element which is inserted into the set will be copied
 *  - the datatype of the set's elements has to support operator== so that it is
 * possible to find a certain element
 *  - it is allowed to insert identical elements into the set
 *  - if a user requires to remove a certain element and if there are several
 * elements which are identical to this element, only one element will be
 * removed from the set
 *  - after removing an element of a set, the indeces of the set's elements
 * might have changed; hence, looping over all elements of a set must be
 * restarted if an element was removed from the set within the loop
 *  - the order of the elements is arbitrary, but it will not be changed unless
 * an element is removed from the set
 */
template <class T>
class OFUnorderedSet : public OFSet<T> {
protected:
public:
  /** Default constructor.
   */
  OFUnorderedSet() : OFSet<T>() {}

  /** Copy constructor.
   *  @param src Source object of which this will be a copy.
   */
  OFUnorderedSet(const OFUnorderedSet<T>& src) : OFSet<T>(src) {}
  ...
  virtual OFBool operator==(const OFUnorderedSet<T>& other) const {
    ...
    for (unsigned int i = 0; i < other.num && result == OFTrue; i++) {
      ...
      // in case s doesn't contain the current item of other the result is
      // OFFalse
      else
        result = OFFalse;
    }
  ...
  virtual void Remove(const T& item) {
    ...
    for (unsigned int i = 0; i < OFSet<T>::num && !itemDeleted; i++) {
      ...
      if (*OFSet<T>::items[i] == item) {
        ...
        // remember that an item was deleted (so that always only one item will
        // be deleted)
        itemDeleted = OFTrue;
      }
    }
  ...
  /** Tries to find a given object in the set. In case the specified object
   * could be found, a pointer to the corresponding element within the set is
   * returned; in case the specified object could not be found, NULL will be
   * returned.
  ...
  virtual OFBool IsSupersetOf(const OFUnorderedSet<T>& other) const {
    // if this contains less or the same amount of items than other, return
    // OFFalse
    if (OFSet<T>::num <= other.num)
      return (OFFalse);
    ...
    for (unsigned int i = 0; i < other.num && result == OFTrue; i++) {
      ...
      // in case s does not contain the current item of other the result is
      // OFFalse
      else
        result = OFFalse;
    }
