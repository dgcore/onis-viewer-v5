...
#include <algorithm>
#define OFForEach(InputIterator_type, Function_type, first, last, f) \
  for_each((first), (last), (f))
#define OFFind(InputIterator_type, T_type, first, last, value) \
  find((first), (last), (value))
#define OFFindIf(InputIterator_type, Predicate_type, first, last, pred) \
  find_if((first), (last), (pred))
#define OFAdjacentFind(ForwardIterator_type, first, last) \
  adjacent_find((first), (last))
#define OFAdjacentFindPred(ForwardIterator_type, BinaryPredicate_type, first, \
                           last, pred)                                        \
...
#define OFForEach(InputIterator_type, Function_type, first, last, f) \
  OF_ForEach((first), (last), (f))

#define OFFind(InputIterator_type, T_type, first, last, value) \
  OF_Find((first), (last), (value))

#define OFFindIf(InputIterator_type, Predicate_type, first, last, pred) \
  OF_FindIf((first), (last), (pred))

#define OFAdjacentFind(ForwardIterator_type, first, last) \
  OF_AdjacentFind((first), (last))

#define OFAdjacentFindPred(ForwardIterator_type, BinaryPredicate_type, first, \
                           last, pred)                                        \
  OF_AdjacentFind((first), (last), (pred))

#elif defined(HAVE_STATIC_TEMPLATE_METHOD)

#define OFForEach(InputIterator_type, Function_type, first, last, f)      \
  OF_ForEachClass<InputIterator_type, Function_type>::OF_ForEach((first), \
                                                                 (last), (f))

#define OFFind(InputIterator_type, T_type, first, last, value) \
  OF_FindClass<InputIterator_type, T_type>::OF_Find((first), (last), (value))

#define OFFindIf(InputIterator_type, Predicate_type, first, last, pred) \
  OF_FindIfClass<InputIterator_type, Predicate_type>::OF_FindIf(        \
      (first), (last), (pred))

#define OFAdjacentFind(ForwardIterator_type, first, last)                   \
  OF_AdjacentFindClass<ForwardIterator_type, int>::OF_AdjacentFind((first), \
                                                                   (last))

#define OFAdjacentFindPred(ForwardIterator_type, BinaryPredicate_type, first, \
                           last, pred)                                        \
  OF_AdjacentFindPredClass<ForwardIterator_type,                              \
                           BinaryPredicate_type>::OF_AdjacentFind((first),    \
                                                                  (last),     \
                                                                  (pred))
#else
...
#endif
      Function
      OF_ForEach(InputIterator first, InputIterator last, Function f) {
    while (first != last) {
      f(*first);
      ++first;
    }
...
#endif
      InputIterator
      OF_Find(InputIterator first, InputIterator last, const T& value) {
...
#endif
      InputIterator
      OF_FindIf(InputIterator first, InputIterator last, Predicate pred) {
...
#endif
      ForwardIterator
      OF_AdjacentFind(ForwardIterator first, ForwardIterator last) {
    if (first == last)
      return last;
