...
public:
   ...
   * hours
   */
  OFDateTime(const unsigned int year, const unsigned int month,
   ...
   * Universal Time (UTC) before they are compared. However, the special case
   * that there is an "overflow" from one day to another is currently not
   * handled.
   ...
   * otherwise
   */
  OFBool setDateTime(const unsigned int year, const unsigned int month,
                     const unsigned int day, const unsigned int hour,
                     const unsigned int minute, const double second,
                     const double timeZone = 0);
   ...
   *  This function uses operating system dependent routines. If the date or
   * time function (or both) are unavailable for some reason the corresponding
   * value is not modified.
   *  @return OFTrue if the current system date and time has been set, OFFalse
   * otherwise
   */
   ...
   *  - "YYYYMMDDHHMM[SS[.FFFFFF]][&ZZZZ]" (without delimiters, useful for DICOM
   * datetime type) where the brackets enclose optional parts.
   ...
   *  @param showDelimiter flag, indicating whether to use delimiters ("-", ":"
   * and " ") or not
   ...
   * (default: " "). Only used if 'showDelimiter' is true.
   *  @return OFTrue if result variable has been set, OFFalse otherwise
   */
  OFBool getISOFormattedDateTime(
      OFString& formattedDateTime, const OFBool showSeconds /*= OFTrue*/,
      const OFBool showFraction /*= OFFalse*/,
      const OFBool showTimeZone /*= OFFalse*/,
