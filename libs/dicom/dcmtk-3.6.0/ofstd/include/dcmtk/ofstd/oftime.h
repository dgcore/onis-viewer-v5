...
public:
   ...
   *  @param timeZone optional offset to Coordinated Universal Time (UTC) in
   * hours
   */
  OFTime(const unsigned int hour, const unsigned int minute,
         const double second, const double timeZone = 0);
   ...
   *  Please note that the time values are first transformed to the Coordinated
   * Universal Time (UTC) before they are compared.
   ...
   * Universal Time (UTC) before they are compared.
   *  @param timeVal time value compared with the current value
   *  @return OFTrue if current time is earlier than the given value, OFFalse
   * otherwise
   */
  virtual OFBool operator<(const OFTime& timeVal) const;
   ...
   * Universal Time (UTC) before they are compared.
   *  @param timeVal time value compared with the current value
   *  @return OFTrue if current time is earlier than or identical to the given
   * value, OFFalse otherwise
   */
  virtual OFBool operator<=(const OFTime& timeVal) const;
   ...
   * Universal Time (UTC) before they are compared.
   *  @param timeVal time value compared with the current value
   *  @return OFTrue if current time is later than or identical to the given
   * value, OFFalse otherwise
   */
  virtual OFBool operator>=(const OFTime& timeVal) const;
   ...
   * Universal Time (UTC) before they are compared.
   *  @param timeVal time value compared with the current value
   *  @return OFTrue if current time is later than the given value, OFFalse
   * otherwise
   */
  virtual OFBool operator>(const OFTime& timeVal) const;
   ...
   *  Valid ranges: [0,24[ for 'hour', [0,60[ for 'minute', [0.0,60.0[ for
   * 'second' and [-12.0,+12.0] for 'timeZone'
   ...
   *  @param timeZone optional offset to Coordinated Universal Time (UTC) in
   * hours
   *  @return OFTrue if the new value is valid and has been set, OFFalse
   * otherwise
   */
  OFBool setTime(const unsigned int hour, const unsigned int minute,
                 const double second, const double timeZone = 0);
   ...
   *  The time zone is given as the offset (in hours) to the Coordinated
   * Universal Time (UTC). Valid values are for instance "+1.0" (plus one hour)
   * and "-2.5" (minus two and a half hour, i.e. 2 hours and 30 minutes). Before
   * the new value is set it is checked using the "isValid()" routine.
  ...
  /** set the time zone to the specified value.
   *  The time zone is given as the offset (in hours and minutes) to the
   * Coordinated Universal Time (UTC). Before the new value is set it is checked
   * using the "isValid()" routine.
   ...
   *  @param timeZone optional offset to Coordinated Universal Time (UTC) in
   * hours
   *  @param normalize if OFTrue the 'seconds' value is normalized to the valid
   * range [0.0,86400.0[ otherwise the value is checked as is
   *  @return OFTrue if the new value is valid and has been set, OFFalse
   * otherwise
   */
  OFBool setTimeInSeconds(const double seconds, const double timeZone = 0,
   ...
   *  @param timeZone optional offset to Coordinated Universal Time (UTC) in
   * hours
   *  @param normalize if OFTrue the 'hours' value is normalized to the valid
   * range [0.0,24.0[ otherwise the value is checked as is
   *  @return OFTrue if the new value is valid and has been set, OFFalse
   * otherwise
   */
  OFBool setTimeInHours(const double hours, const double timeZone = 0,
                        const OFBool normalize = OFTrue);

  /** set the time value to the current system time.
   *  This function uses operating system dependent routines. If they are
   * unavailable for some reason the current value is not modified.
   ...
   *  The two ISO time formats supported by this function are "HH:MM[:SS]" (with
   * arbitrary delimiters) and "HHMM[SS]" (without delimiters) where the
   * brackets enclose optional parts.  The optional fractional part of a second
   * and the time zone (see getISO...) are not yet supported.
   *  @param formattedTime ISO formatted time value to be set
   *  @return OFTrue if input is valid and result variable has been set, OFFalse
   * otherwise
   */
   ...
   *  The time zone is given as the offset (in hours) to the Coordinated
   * Universal Time (UTC). Valid values are for instance "+1.0" (plus one hour)
   * and "-2.5" (minus two and a half hour, i.e. 2 hours and 30 minutes).
   ...
   *  @param formattedTime reference to string variable where the result is
   * stored
   *  @param showSeconds add optional seconds (":SS" or "SS") to the resulting
   * string if OFTrue
   *  @param showFraction add optional fractional part of a second (".FFFFFF")
   * if OFTrue. Requires parameter 'seconds' to be also OFTrue.
   *  @param showTimeZone add optional time zone ("&ZZ:ZZ" or "&ZZZZ") to the
   * resulting string if OFTrue. The time zone indicates the offset from the
   * Coordinated Universal Time (UTC) in hours and minutes. The "&" is a
   * placeholder for the sign symbol ("+" or "-").
   *  @param showDelimiter flag, indicating whether to use delimiters (":") or
   * not
   ...
   *  This function uses operating system dependent routines. If they are
   * unavailable for some reason the Coordinated Universal Time is assumed (time
   * zone offset = 0).
...
protected:
   ...
   *  @return OFTrue if the current value is valid, OFFalse otherwise
   */
  static OFBool isTimeValid(const unsigned int hour, const unsigned int minute,
                            const double second, const double timeZone);
  ...
  static double getTimeInSeconds(const unsigned int hour,
                                 const unsigned int minute, const double second,
