 ...
 *  @param arg pointer to the OFThread instance to be started, passed as void
 * pointer.
 */
extern "C" {
...
public:
   ...
   *  @param description string object into which the error description is
   * written.
 ...
 *  to coordinate access to resources. The initial and maximum value of the
 * counter is defined by the constructor. Each call to wait() decreases the
 * counter by one and each call to post() increases the count by one. If a
 * thread calls wait() while the counter is zero, the thread is blocked until
 * another thread has increased the counter using post().
 */
class OFSemaphore {
public:
  /** constructor.
   ...
   *  @return 0 upon success, OFSemaphore::busy if the semaphore is already
