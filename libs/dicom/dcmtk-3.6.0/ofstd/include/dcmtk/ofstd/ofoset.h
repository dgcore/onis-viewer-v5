 ...
 * administrating an ordered set of elements of an arbitrary type. Note the
 * following properties of this class:
 *   - an element which is inserted into the set will be copied
 *   - the datatype of the set's elements has to support operator== so that it
 * is possible to find a certain element
 ...
 *   - when removing an element, the indeces of the elements behind the removed
 * element will be reduced by one
 *   - the set will be ordered according to the point in time at which an
 * element is inserted into the set; a new element will always be inserted at
 * the end of the set
 */
template <class T>
class OFOrderedSet : public OFSet<T> {
protected:
public:
  /** Default constructor.
   */
  OFOrderedSet() : OFSet<T>() {}

  /** Copy constructor.
   *  @param src Source object of which this will be a copy.
   */
  OFOrderedSet(const OFOrderedSet<T>& src) : OFSet<T>(src) {}
