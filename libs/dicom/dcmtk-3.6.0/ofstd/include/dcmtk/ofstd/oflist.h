 ...
 *
 *  Purpose:
 *          Defines a template list class with interfaces similar to the C++
 * Standard
 *
...
#define OFListInsert(InputIterator, T, c, pos, first, last) \
...
public:
  ...
  void base_splice(OFListLinkBase* pos, OFListLinkBase* begin,
                   OFListLinkBase* end);
  void base_clear();
...
public:
   ...
   *  May only be called if iterator references a valid element or the end of a
   * list.
   ...
   *  The list is circular: the first element follows after the end of the list.
   *  May only be called if iterator references a valid element or the end of a
   * list. This is the post-increment operator.
   ...
   *  The list is circular: the end of the list follows before the first
   * element. May only be called if iterator references a valid element or the
   * end of a list.
   ...
   *  The list is circular: the end of the list follows before the first
   * element. May only be called if iterator references a valid element or the
   * end of a list. This is the post-decremented operator.
   *  @return previous iterator state, by value
   */
  OFIterator<T> operator--(int) {
    OFIterator<T> tmp(*this);
...
private:
   ...
   */
  void recalcListSize() {
    OFListBase::base_recalcListSize();
  }
...
public:
   ...
   *  @param position iterator to position before which the elements are
   * inserted
   ...
   *  @param last iterator pointing to the element after the last element to be
   * removed
  ...
  /** inserts one element from list x into the current list and removes it from
   * x
   *  @param position iterator to position before which the element is inserted
   ...
   */
  void splice(OFIterator<T> position, OFList<T>& x, OFIterator<T> first,
              OFIterator<T> last) {
...
#define OFListInsert(InputIterator, T, c, pos, first, last) \
  OF_ListInsert((c), (pos), (first), (last))
...
#define OFListInsert(InputIterator, T, c, pos, first, last)                \
  OF_ListInsertClass<InputIterator, T>::OF_ListInsert((c), (pos), (first), \
                                                      (last))

#define OFListRemoveIf(Predicate, T, c, pred) \
  OF_ListRemoveIfClass<Predicate, T>::OF_ListRemoveIf((c), (pred))
...
#endif
      void
      OF_ListInsert(OFList<T>& c, OFIterator<T> position, InputIterator first,
                    InputIterator last) {
    while (first != last) {
...
#endif
      void
      OF_ListRemoveIf(OFList<T>& c, Predicate pred) {
