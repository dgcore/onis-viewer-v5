...
public:
   ...
   *  @return pointer to string describing the error code. Please note that
   * depending on the implementation of the function used, the result may or may
   * not be a pointer to buf. The return value can also be NULL if the buffer is
   * invalid.
   */
   ...
   *  This function does not distinguish files from directories (use
   * 'fileExists()' or 'directoryExists()' if required).
   ...
   *  This function also checks that the specified path points to file and not
   * to a directory (or the like).
   ...
   *  @param result string variable in which the resulting directory name is
   * stored
   *  @param pathName path name from which the directory name should be
   * extracted
   *  @param assumeDirName assume that there always is a directory name in
   * 'pathName'
   ...
   *  @param assumeFilename assume that there always is a file name in
   * 'pathName'
   ...
   *  directory name is an empty string and the flag 'allowEmptyDirName' is
   * OFFalse the directory name is set to "." (current directory). If the
   * resulting directory name is "." and the flag 'allowEmptyDirName' is OFTrue
   * the directory name is set to an empty string.
   *  @param result string variable in which the resulting directory name is
   * stored
   *  @param dirName directory name to be normalized
   *  @param allowEmptyDirName flag indicating whether an empty directory name
   * is allowed
   *  @return reference to the resulting directory name (same as 'result')
   */
  static OFString& normalizeDirName(OFString& result, const OFString& dirName,
                                    const OFBool allowEmptyDirName = OFFalse);

  /** combine the given directory and file name.
   *  Normalizes the directory name and appends the file name (with a path
   * separator) if not empty. If both 'dirName' and 'fileName' are empty strings
   * and the flag 'allowEmptyDirName' is OFFalse the resulting path name is set
   * to "." (current directory). If 'dirName' is "." and the flag
   * 'allowEmptyDirName' is OFTrue an empty directory name is used. NB: This
   * function neither checks whether the given 'dirName' exists nor whether the
   * resulting path name points to a valid or existing file name. Furthermore,
   *      the value of 'dirName' is ignored if 'fileName' starts with a path
   * separator.
   ...
   */
  static OFString& combineDirAndFilename(
      OFString& result, const OFString& dirName, const OFString& fileName,
      const OFBool allowEmptyDirName = OFFalse);
   ...
   *  @param pathName path name from which the root directory (prefix) is
   * removed
   *  @param allowLeadingPathSeparator flag indicating whether a leading path
   * separator is allowed for the resulting path name (automatically removed
   * otherwise)
   *  @return status, EC_Normal if successful, an error code otherwise
   */
  static OFCondition removeRootDirFromPathname(
      OFString& result, const OFString& rootDir, const OFString& pathName,
   ...
   *  @param pattern optional wildcard pattern used to match the filenames
   * against. By default all files match. In order to work under Unix the system
   * function fnmatch() is required.
   *  @param dirPrefix optional prefix added to the directory name.
   *    This prefix will, however, not be part of the filenames added to the
   * list.
   *  @param recurse flag indicating whether to search recursively (default) or
   * not
   *  @return number of new files added to the list
   */
  static size_t searchDirectoryRecursively(
      const OFString& directory, OFList<OFString>& fileList,
   ...
   *  This check can be performed before convertToMarkupStream() or
   *convertToMarkupString() is called in order to speed up the process in case
   *the conversion is not required.
   ** @param sourceString source string to be checked
   *  @param convertNonASCII convert non-ASCII characters (< #32 and >= #127) to
   *numeric value (@&@#nnn;) if OFTrue
   ** @return OFTrue if markup conversion is required, OFFalse otherwise
   */
  static OFBool checkForMarkupConversion(
      const OFString& sourceString, const OFBool convertNonASCII = OFFalse);

  /** convert character string to HTML/XHTML/XML mnenonic stream.
   *  Characters with special meaning for HTML/XHTML/XML (e.g. '<' and '&') are
   *replaced by the corresponding mnenonics (e.g. "&lt;" and "&amp;").  If flag
   *'convertNonASCII' is OFTrue, all characters < #32 and >= #127 are also
   *converted (useful if only HTML 3.2 is supported which does not allow to
   *specify the character set).  In HTML 3.2 mode, the quotation mark
   *  (") is converted to "&#34;" instead of "&quot;" because the latter entity
   *is not defined. In HTML mode, the apostrophe sign (') is converted to
   ...
   *  @param convertNonASCII convert non-ASCII characters (< # 32 and >= #127)
   *to numeric value
   *    (@&@#nnn;) if OFTrue
   *  @param markupMode convert to HTML, HTML 3.2, XHTML or XML markup.
   *    LF and CR are encoded as "&#10;" and "&#13;" in XML mode, the flag
   *'newlineAllowed' has no meaning in this case.
   *  @param newlineAllowed optional flag indicating whether newlines are
   *allowed or not. If they are allowed the text "<br>" (HTML) or "<br />"
   *(XHTML) is used, "&para;" otherwise. The following combinations are
   *accepted: LF, CR, LF CR, CF LF.
   ** @return status, always returns EC_Normal
   */
  static OFCondition convertToMarkupStream(
      STD_NAMESPACE ostream& out, const OFString& sourceString,
   ...
   *"&#39;" instead of "&apos;" for the same reason.
   ** @param sourceString source string to be converted
   *  @param markupString reference to character string where the result should
   *be stored
   *  @param convertNonASCII convert non-ASCII characters (< # 32 and >= #127)
   *to numeric value
   *    (@&@#nnn;) if OFTrue
   *  @param markupMode convert to HTML, HTML 3.2, XHTML or XML markup string.
   *    LF and CR are encoded as "@&@#10;" and "@&@#13;" in XML mode, the flag
   *'newlineAllowed' has no meaning in this case.
   *  @param newlineAllowed optional flag indicating whether newlines are
   *allowed or not. If they are allowed the text "<br>" (HTML) or "<br />"
   *(XHTML) is used, "&para;" otherwise. The following combinations are
   ...
   */
  static const OFString& convertToMarkupString(
   ...
   *  Basic algorithm: groups of 3 bytes from the binary input are coded as
   *groups of 4 bytes in the textual output.  The input data is 'padded' with
   *zeros to create a length that is an even multiple of 3.  A special character
   *('=') is used to denote padding so that the output can be decoded back to
   *its exact size. If the input data is NULL an error code
   *(EC_IllegalParameter) is returned.
  ...
  static OFCondition encodeBase64(STD_NAMESPACE ostream& out,
                                  ...
                                  const size_t length, const size_t width = 0);
   ...
   *zeros to create a length that is an even multiple of 3.  A special character
   *('=') is used to denote padding so that the output can be decoded back to
   *its exact size. If the input data is NULL an empty string is returned.
   ** @param data buffer with binary data to be encoded (big endian required!)
   *  @param length length of the input data buffer (in bytes)
  ...
  static const OFString& encodeBase64(const unsigned char* data,
                                      const size_t length, OFString& result,
                                      const size_t width = 0);

  /** decode "Base64" encoded string.
   *  Any character that does not belong to the Base64 alphabet (0..9, A..Z,
   *a..z, + and /) is ignored when decoding the input string.  This is
   *especially true for line breaks which are usually contained in MIME (RFC
   *2045) encoded streams (see above).  The first occurence of a '=' character
   *is taken as evidence that the end of the data has been reached. NB: The
   *memory buffer in which the binary output is stored is allocated inside this
   *function and has to to be freed (using "delete[]") by the caller!  Do not
   *pass a pointer to an already allocated buffer to this function, the caller
   *does not know the exact size anyway.
   ** @param data Base64 encoded input data (possibly padded with '=' at the
   *end)
   *  @param result receives pointer to resulting buffer with binary data (big
   *endian encoded)
   ** @return length of the resulting binary data (0 if an error occurred, in
   *this case the buffer is deleted internally)
   */
   ...
   */
  static void ftoa(char* target, size_t targetSize, double value,
                   unsigned int flags = 0, int width = 0, int precision = -1);
   ...
   *  @return OFTrue if an overflow occurred during the addition, OFFalse
   * otherwise
   */
