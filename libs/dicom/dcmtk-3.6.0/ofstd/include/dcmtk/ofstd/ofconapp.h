...
public:
   ...
   */
  OFConsoleApplication(const char* app, const char* desc = NULL,
  ...
  /** parse command line.
   *  If the command line has no argument (in case at least one argument is
   *required) and if the command line has only one argument, namely "--help" or
   *the specified shortcut, (in all cases) the usage is printed (see
   *printUsage).
   *
   ...
   *  @param  flags     flags to be used for parsing (e.g.
   *OFCommandLine::PF_ExpandWildcards)
   *  @param  startPos  first argument to be parsed (default: 1, i.e. omit
   *program name)
   *
   ** @return status of parsing process, true if successful, false otherwise
   */
  OFBool parseCommandLine(OFCommandLine& cmd, int argCount, char* argValue[],
                          const int flags = 0, const int startPos = 1);

  /** print header of console application (consisting of identifier, name and
   *description)
   *
   ** @param  hostInfo  print host information as reported by 'config.guess' if
   *OFTrue
   *  @param  stdError  print to standard error stream if OFTrue (default:
   *standard output)
   */
   ...
   *
   ** @param  cmd   reference to command line class (default: object used for
   *parsing)
   */
  ...
  /** print error message (incl. header) to standard error stream and exit with
   *error code
   *
  ...
  /** check dependence between sub and base option and report an error if
   *required. Message format: "error: " + subOpt + " only allowed with " +
   *baseOpt
   *
   ** @param  subOpt     option to be checked
   *  @param  baseOpt    base option required for sub option
   *  @param  condition  if false error message is printed (i.e. base option is
   *absent)
   */
  void checkDependence(const char* subOpt, const char* baseOpt,
                       OFBool condition);
   ...
   *  @param  condition  if true error message is printed (i.e. a conflict
   *exists)
   */
  void checkConflict(const char* firstOpt, const char* secondOpt,
                     OFBool condition);
