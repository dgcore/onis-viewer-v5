...
/** This abstract template class provides a data structure and operations for
 * administrating a set of elements of an arbitrary type.
 */
template <class T>
class OFSet {
protected:
...
public:
  /** Default constructor.
   */
  OFSet() : items(new T*[STARTING_SIZE]), num(0), size(STARTING_SIZE) {
    init();
  }
   ...
   */
  OFSet(const OFSet<T>& src) : items(NULL), num(src.num), size(src.size) {
    init(src);
  }
