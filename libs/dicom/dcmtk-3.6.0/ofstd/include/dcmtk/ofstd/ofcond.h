...
public:
  ...
  /** this method returns a pointer to a OFConditionBase object containing a
   * clone of this object. If deletable() is true, the clone must be a deep copy
   * allocated on the heap.  If deletable() is false, the clone should be a
   * pointer to this.
  ...
  OFBool operator==(const OFConditionBase& arg) const {
    return ((status() == arg.status()) &&
            (codeAndModule() == arg.codeAndModule()));
  }
   ...
   */
  OFBool operator!=(const OFConditionBase& arg) const {
    return ((status() != arg.status()) || (code() != arg.code()) ||
            (module() != arg.module()));
  }
...
public:
   ...
   */
  OFConditionConst(unsigned short aModule, unsigned short aCode,
                   OFStatus aStatus, const char* aText)
      : OFConditionBase(),
        theCodeAndModule(OFstatic_cast(unsigned long, aCode) |
                         OFstatic_cast(unsigned long, aModule << 16)),
        theStatus(aStatus),
        theText(aText) {}
  ...
  /** this method returns a pointer to a OFConditionBase object containing a
   * clone of this object. In this case, deletable() is false and clone just
...
public:
   ...
   */
  OFConditionString(unsigned short aModule, unsigned short aCode,
                    OFStatus aStatus, const char* aText)
      : OFConditionBase(),
        theCodeAndModule(OFstatic_cast(unsigned long, aCode) |
                         OFstatic_cast(unsigned long, aModule << 16)),
  ...
  /** this method returns a pointer to a OFConditionBase object containing a
   * clone of this object. The clone is a deep copy allocated on the heap.
...
public:
   ...
   */
  OFCondition(OFConditionString* base) : theCondition(base) {
    assert(theCondition);
  }
 ...
 *   compatibility options related to the transition to OFCondition are
 * disabled: No OFCondition default constructor, no typedefs for E_Condition,
 * CONDITION, no macros for SUCCESS and condition aliases.
 *
