...
public:
   ...
   */
  OFDate(const unsigned int year, const unsigned int month,
   ...
   *  @param dateVal date value compared with the current value
   *  @return OFTrue if current date is earlier than or identical to the given
   ...
   *  @param dateVal date value compared with the current value
   *  @return OFTrue if current date is later than or identical to the given
   * value, OFFalse otherwise
   */
  virtual OFBool operator>=(const OFDate& dateVal) const;
   ...
   *  Currently, this method only checks whether the month is within the range
   * of 1 to 12 and the day is within the range of 1 to 31. In the future this
   * routine might be enhanced.
   ...
   * otherwise
   */
  OFBool setDate(const unsigned int year, const unsigned int month,
                 const unsigned int day);
   ...
   * arbitrary delimiters) and "YYYYMMDD" (without delimiters, useful for DICOM
   * date type).
   ...
   *  The two ISO date formats supported by this function are "YYYY-MM-DD" (with
   *  delimiters) and "YYYYMMDD" (without delimiters, useful for DICOM date
   * type).
...
protected:
   ...
   *  @return OFTrue if the current value is valid, OFFalse otherwise
   */
  static OFBool isDateValid(const unsigned int year, const unsigned int month,
                            const unsigned int day);
