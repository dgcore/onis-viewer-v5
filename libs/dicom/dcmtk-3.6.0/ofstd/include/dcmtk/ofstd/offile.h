...
// Mac OS X defines _LARGEFILE64_SOURCE but anyhow expects implicit 64 bit
// calls. The same is true for current Cygwin versions (tested with
// version 1.7.7-1).
...
/** this class provides a simple C++ encapsulation layer for stdio FILE
 * pointers. All stdio functions on files are directly mapped into member
 * functions. The handling of large files (64 bit file systems) is transparent.
 * Instead of type off_t, fseek() and ftell() use offile_off_t which is a 64 bit
 * type if available on the underlying platform. Similarly, getpos() and
 * setpos() use type offile_fpos_t, which is defined appropriately. This class
 * provides both fclose() and pclose(), but these are equivalent - the code
 * always closes pipes with pclose() and files with fclose(). Finally, an
 * abstraction for errno is provided. Error codes should always be retrieves
 * using methods getLastError() and getLastErrorString() which on Unix platforms
 * are based on errno and strerror/strerror_r, but may be based on other
 * mechanisms on platforms where errno does not exist.
 */
class OFFile {
public:
  /// default constructor, creates an object that is not associated with any
  /// file.
  ...
  /** opens the file whose name is the wide character string pointed to by path
   * and associates a stream with it. This function is Win32 specific and only
   * exists on WinNT and newer.
   ...
   *  @return 0 upon success, EOF otherwise, in which case the error code is
   * set.
   */
  int fclose() {
    int result = 0;
   ...
   *  them from the location given by ptr. Returns the number of items
   * successfully written (i.e., not the number of characters).  If an error
   * occurs the return value is a short item count (or zero).
   ...
   *  them at the location given by ptr. Returns the number of items
   * successfully read (i.e., not the number of characters).  If an error
   * occurs, or the end-of-file is reached, the return value is a short item
   * count (or zero). fread does not distinguish between end-of-file and error,
   ...
   */
  int fgetc() {
    return STDIO_NAMESPACE fgetc(file_);
  }
   ...
   *  @return 0 upon success, nonzero otherwise, in which case the error code
   * may be set
   *
   */
  void setlinebuf() {
   ...
   */
  void clearerr() {
    STDIO_NAMESPACE clearerr(file_);
  }
  ...
  void setbuf(char* buf) {
    STDIO_NAMESPACE setbuf(file_, buf);
  }
   ...
   *  @param modes _IONBF (unbuffered) _IOLBF (line buffered) or _IOFBF (fully
   * buffered)
   ...
   *  @param c character
   *  @return the character written as an unsigned char cast to an int or EOF on
   * error
   */
  int fputc(int c) {
    return STDIO_NAMESPACE fputc(c, file_);
  }
   ...
   *  If a newline is read, it is stored into the buffer. A '@\0' is stored
  ...
  char* fgets(char* s, int n) {
    return STDIO_NAMESPACE fgets(s, n, file_);
  }
  ...
  int fputs(const char* s) {
    return STDIO_NAMESPACE fputs(s, file_);
  }
   ...
   */
  int ungetc(int c) {
    return STDIO_NAMESPACE ungetc(c, file_);
  }
  ...
  int fseek(offile_off_t off, int whence) {
    ...
    switch (whence) {
      case SEEK_END:
        ...
        // fall through
#else
        // determine file size (using underlying file descriptor). This should
        // be safe.
#ifdef fileno
  ...
  /** return true if this object is currently associated with a stream, false
   * otherwise
   *  @return true if this object is currently associated with a stream, false
   * otherwise
   */
   ...
   *  @param s string describing last error code for this stream returned in
   * this parameter
   */
...
// wide character functions (disabled by default, since currently not used
// within DCMTK)
   ...
   *  character conversion error occurs, it sets the error code to EILSEQ and
   * returns WEOF.
  ...
  /** writes the wide character wc to stream. If ferror(stream) becomes true, it
   * returns WEOF. If a wide character conversion error occurs, it sets the
   * error code to EILSEQ and returns WEOF. Otherwise it returns wc.
 ...
 *  Fixed compile error on Cygwin where no wide-char FILE functions are
 * available
 *
