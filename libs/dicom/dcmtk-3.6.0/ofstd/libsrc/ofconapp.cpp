...
OFConsoleApplication::OFConsoleApplication(const char* app, const char* desc,
                                           const char* rcsid)
...
OFBool OFConsoleApplication::parseCommandLine(OFCommandLine& cmd, int argCount,
                                              char* argValue[], const int flags,
                                              const int startPos) {
  CmdLine = &cmd;  // store reference to cmdline object
  OFCommandLine::E_ParseStatus status =
      cmd.parseLine(argCount, argValue, flags, startPos);
...
void OFConsoleApplication::printHeader(const OFBool hostInfo,
  ...
  /* lock output stream */
  STD_NAMESPACE ostream* output =
      (stdError) ? &ofConsole.lockCerr() : &ofConsole.lockCout();
...
void OFConsoleApplication::printArguments(OFCommandLine* cmd) {
  ...
  if (CmdLine != NULL) {
    output << "expanded command line to " << CmdLine->getArgCount()
           << " arguments:" << OFendl;
    const char* arg;
...
void OFConsoleApplication::printError(const char* str, const int code) {
  if (!QuietMode) {
...
void OFConsoleApplication::printWarning(const char* str, const char* prefix) {
  if (!QuietMode) {
...
void OFConsoleApplication::checkParam(
    const OFCommandLine::E_ParamValueStatus status, OFCommandLine* cmd) {
  if (cmd == NULL)
    cmd = CmdLine;
