...
OFDate::OFDate() : Year(0), Month(0), Day(0) {}

OFDate::OFDate(const OFDate& dateVal)
    : Year(dateVal.Year), Month(dateVal.Month), Day(dateVal.Day) {}

OFDate::OFDate(const unsigned int year, const unsigned int month,
...
OFBool OFDate::operator==(const OFDate& dateVal) const {
  return (Year == dateVal.Year) && (Month == dateVal.Month) &&
         (Day == dateVal.Day);
}
...
OFBool OFDate::operator<(const OFDate& dateVal) const {
  return (Year < dateVal.Year) ||
         ((Year == dateVal.Year) &&
          ((Month < dateVal.Month) ||
           ((Month == dateVal.Month) && (Day < dateVal.Day))));
}
...
OFBool OFDate::setDate(const unsigned int year, const unsigned int month,
                       const unsigned int day) {
...
OFBool OFDate::setISOFormattedDate(const OFString& formattedDate) {
  ...
  if (length == 10) {
    ...
    if (sscanf(formattedDate.c_str(), "%04u%*c%02u%*c%02u", &year, &month,
               &day) == 3)
