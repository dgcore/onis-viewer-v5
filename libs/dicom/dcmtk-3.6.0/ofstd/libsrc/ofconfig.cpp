...
OFConfigFileNode::OFConfigFileNode(const char* keyword)
    : brother_(NULL), son_(NULL), keyword_(keyword), value_() {}
...
void OFConfigFileCursor::set_section(unsigned int level, const char* key,
                                     OFConfigFileNode* anchor) {
  int valid = (level <= maxLevel_) && array_;
  if (valid) {
    if (level < maxLevel_)
      valid = section_valid(level + 1);
    else
      valid = 1;
    if (valid) {
      first_section(level, anchor);
      while ((section_valid(level)) && array_[level] &&
             (!array_[level]->match(key)))
        next_section(level);
    } else
      clear();
  } else
...
void OFConfigFileCursor::first_section(unsigned int level,
  ...
  if (valid) {
    ...
    if (valid) {
      if (level < maxLevel_)
        array_[level] = array_[level + 1]->getSon();
      else
        array_[level] = anchor;
  ...
  if (NULL == parent) {
    ...
    if (NULL == parent->getSon()) {
      ...
      if (!leftMostBrother->less(newnode->getKeyword())) {
        ...
        // skip all list entries where the keyword is less, except for the last
        // one
        while (leftMostBrother->getBrother() &&
               leftMostBrother->getBrother()->less(newnode->getKeyword())) {
          leftMostBrother = leftMostBrother->getBrother();
        }

        // now we're either at the last entry of the list, or the next entry is
        // greater or equal.
...
void OFConfigFileCursor::insert(unsigned int level, OFConfigFileNode*& newnode,
                                OFConfigFileNode*& anchor, OFBool orderedMode) {
  // Implement option that causes entries to be sorted and identical entries to
  // be replaced
  if (level == maxLevel_) {
    if (NULL == array_[maxLevel_]) {
      ...
      if (orderedMode) {
        ...
        rootNode.setSon(
            NULL);  // destruction of root node shall not destroy whole tree
      } else {
    ...
    if (array_[level + 1]) {
      ...
      // this should never happen unless the caller passes the wrong level, in
      // which case we at least avoid a memory leak
      delete newnode;
      newnode = NULL;
    }
  }
...
OFBool OFConfigFile::get_bool_value(OFBool defaultvalue) {
  ...
  for (size_t i = 0; i < len; i++) {
    c = pstring[i];
    if ((c >= 'a') && (c <= 'z'))
      ostring += OFstatic_cast(char, toupper(c));
    else if ((c >= 'A') && (c <= 'Z'))
      ostring += c;
    else if ((c >= '0') && (c <= '9'))
      ostring += c;
...
void OFConfigFile::read_entry(FILE* infile) {
  ...
  if ((!feof(infile)) && (!ferror(infile))) {
    if (c == '[') {  // this is a higher level keyword
      level++;
      while (!done) {
        c = read_keywordchar(infile);
      ...
      if (valid) {  // Now we will read the keyword name
        ungetc(c, infile);
        done = 0;
        while (!done) {
          c = read_keywordchar(infile);
      ...
      if (level > OFstatic_cast(int, maxLevel_))
        level = maxLevel_;
    } else {  // this is a level 0 keyword
      ungetc(c, infile);
      done = 0;
      while (!done) {
        c = read_keywordchar(infile);
    ...
    if (newnode)
      cursor_.insert(level, newnode, anchor_, orderedMode_);  // new node stored
    bufptr_ = 0;
    ...
    if ((bufptr_ > 0) && (newnode)) {
      ...
      while ((bufptr_ > 0) &&
             ((buffer_[bufptr_ - 1] == 10) || (buffer_[bufptr_ - 1] == 0)))
        buffer_[--bufptr_] = 0;
...
OFConfigFile::OFConfigFile(FILE* infile, unsigned int maxLevel,
                           char commentChar, OFBool orderedMode)
    : stack_(),
      cursor_(maxLevel),
      anchor_(NULL),
      isnewline_(1),
 ...
 *  Added initialization of member variables to class OFConfigFileCursor in
 * order to avoid compiler warnings (reported by gcc with additional flags).
 *
 ...
 *    Also added function to print content of configuration in reloadable
 * format.
 *
