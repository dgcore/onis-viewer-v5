 ...
 *  Options are command line arguments to use optional functions and to specify
 * optional properties of a program. They are all starting with one or more
 * special characters and can therefore be detected.
 */
struct OFCmdOption {
  /** constructor
   *
   ...
   */
  OFCmdOption(const char* longOpt, const char* shortOpt, const int valueCount,
              const char* valueDescr, const char* optDescr, const int flags)
      : LongOption(longOpt),
  ...
  ~OFCmdOption() {
    ...
    if (!Checked &&
        !(Flags &
          (OFCommandLine::AF_Exclusive | OFCommandLine::AF_NoWarning)) &&
        (LongOption.length() > 0)) {
      ofConsole.lockCerr() << "WARNING: option " << LongOption
                           << " has possibly never been checked !" << OFendl;
 ...
 *  Parameters are all command line arguments which are no options (e.g. file
 * names).
 */
struct OFCmdParamPos {
   ...
   *  @param  optIter    iterator pointing to first option iterator in front of
   *the parameter
   *  @param  optCount   number of options in front of the parameter
   *  @param  directOpt  number of options which are direct predecessor in the
   *argument list
   */
  OFCmdParamPos(const OFListIterator(OFString) & parIter,
                const OFListIterator(OFListIterator_OFString) & optIter,
                const int optCount, const int directOpt)
      : ParamIter(parIter),
        OptionIter(optIter),
...
void OFCommandLine::setOptionColumns(const int longCols, const int shortCols) {
  LongColumn = longCols;
...
OFBool OFCommandLine::checkOption(const OFString& option,
  ...
  if (optionLen > 0)  // empty strings are allowed to support (sub)groups
  {
    result = OFFalse;
    if (optionLen >= 2) {
      if (OptionChars.find(option.at(0)) !=
          OFString_npos)  // options have to start with one of the defined chars
      {
        if (((option.at(0) != '-') &&
             (option.at(0) !=
            ...
            (option.at(1) < '0') ||
            (option.at(1) >
             '9'))  // ... don't allow a number as the following character
        {
          result = OFTrue;
        }
      }
...
OFBool OFCommandLine::addOption(const char* longOpt, const char* shortOpt,
                                const int valueCount, const char* valueDescr,
                                const char* optDescr, const int flags) {
    ...
    if (strlen(longOpt) > 0) {
      ...
      while (iter != last) {
        if ((*iter)->LongOption == longOpt) {
          ofConsole.lockCerr() << "WARNING: long option " << longOpt
                               << " already defined ... not added !" << OFendl;
          ofConsole.unlockCerr();
          return OFFalse;
        }
        if ((strlen(shortOpt) > 0) && ((*iter)->ShortOption == shortOpt)) {
          ofConsole.lockCerr()
              << "WARNING: short option " << shortOpt << " already defined for "
              << (*iter)->LongOption << " ..." << OFendl << "         option "
              << longOpt << " not added !" << OFendl;
          ofConsole.unlockCerr();
          return OFFalse;
        }
        ++iter;
...
#endif
    OFCmdOption* opt = new OFCmdOption(longOpt, shortOpt, valueCount,
                                       valueDescr, optDescr, flags);
    if (opt != NULL) {
...
#ifdef DEBUG
  ofConsole.lockCerr() << "WARNING: invalid option " << shortOpt << "/"
                       << longOpt << " ... not added !" << OFendl;
  ofConsole.unlockCerr();
#endif
  return OFFalse;
...
OFBool OFCommandLine::addOption(const char* longOpt, const int valueCount,
                                const char* valueDescr, const char* optDescr,
                                const int flags) {
  return addOption(longOpt, "", valueCount, valueDescr, optDescr, flags);
}

OFBool OFCommandLine::addOption(const char* longOpt, const char* optDescr,
                                const int flags) {
  return addOption(longOpt, "", 0, "", optDescr, flags);
}

void OFCommandLine::addGroup(const char* name, const int longCols,
                             const int shortCols) {
  addOption("", "", packColumnValues(longCols, shortCols), "", name);
}

void OFCommandLine::addSubGroup(const char* name, const int longCols,
                                const int shortCols) {
  addOption("", "", packColumnValues(longCols, shortCols), name, "");
}

OFBool OFCommandLine::addParam(const char* param, const char* descr,
    ...
    switch (LastParamMode) {
      case OFCmdParam::PM_Optional:
        if ((mode != OFCmdParam::PM_Optional) &&
            (mode != OFCmdParam::PM_MultiOptional)) {
          ofConsole.lockCerr()
              << "WARNING: " << ValidParamList.size()
              << ". parameter is optional => hides " << param << " !" << OFendl;
          ofConsole.unlockCerr();
        }
        break;
        /*
                    case OFCmdParam::PM_MultiMandatory:
                        {
                            ofConsole.lockCerr() << "WARNING: " <<
           ValidParamList.size() << ". parameter is multi-mandatory => hides "
                                                 << param << " !" << OFendl;
                            ofConsole.unlockCerr();
                        }
                        break;
      ...
      case OFCmdParam::PM_MultiOptional: {
        ofConsole.lockCerr() << "WARNING: " << ValidParamList.size()
                             << ". parameter is multi-optional => hides "
                             << param << " !" << OFendl;
        ofConsole.unlockCerr();
      } break;
      default:
...
OFCommandLine::E_ParamValueStatus OFCommandLine::getParam(
    const int pos, OFCmdSignedInt& value) {
  if (findParam(pos)) {
...
OFCommandLine::E_ParamValueStatus OFCommandLine::getParamAndCheckMin(
    const int pos, OFCmdSignedInt& value, const OFCmdSignedInt low,
    const OFBool incl) {
...
OFCommandLine::E_ParamValueStatus OFCommandLine::getParamAndCheckMinMax(
    const int pos, OFCmdSignedInt& value, const OFCmdSignedInt low,
...
OFCommandLine::E_ParamValueStatus OFCommandLine::getParam(
    const int pos, OFCmdUnsignedInt& value) {
  if (findParam(pos)) {
...
OFCommandLine::E_ParamValueStatus OFCommandLine::getParamAndCheckMin(
    const int pos, OFCmdUnsignedInt& value, const OFCmdUnsignedInt low,
    const OFBool incl) {
  E_ParamValueStatus status = getParam(pos, value);
  if (status == PVS_Normal) {
    if ((value < low) || (!incl && (value == low)))
...
OFCommandLine::E_ParamValueStatus OFCommandLine::getParamAndCheckMin(
    const int pos, OFCmdFloat& value, const OFCmdFloat low, const OFBool incl) {
  E_ParamValueStatus status = getParam(pos, value);
  if (status == PVS_Normal) {
    if ((value < low) || (!incl && (value == low)))
      return PVS_Underflow;
...
OFCommandLine::E_ParamValueStatus OFCommandLine::getParamAndCheckMinMax(
    const int pos, OFCmdFloat& value, const OFCmdFloat low,
...
OFBool OFCommandLine::findOption(const char* longOpt, const signed int pos,
  ...
  if (iter == last) {
    ofConsole.lockCerr() << "WARNING: unknown option " << longOpt
                         << " in OFCommandLine::findOption() !" << OFendl;
    ofConsole.unlockCerr();
    return OFFalse;
  }
#endif
  ...
  if ((mode == FOM_FirstFromLeft) || (mode == FOM_NextFromLeft)) {
    ...
    if (pos != 0) {
      ofConsole.lockCerr()
          << "WARNING: OFCommandLine::findOption() parameter 'pos' (" << pos
          << ") ignored !" << OFendl;
      ofConsole.unlockCerr();
    }
#endif
    ...
    OFListIterator(OFListIterator_OFString) pos_iter =
        (mode == FOM_Next) ? OptionPosIterator : OptionPosList.end();
    const OFListIterator(OFListIterator_OFString) pos_first =
        OptionPosList.begin();
    ...
    if (findParam(abs(pos), param_iter))  // go to specified parameter position
    {
      ...
      if (((*param_iter)->OptionCount ==
           0) ||  // no options in front of specified parameter or
          ((pos < 0) && (diropt == 0)))  // no 'direct' option ...
        return OFFalse;
      pos_iter =
          (*param_iter)->OptionIter;  // first option in front of parameter
      ++pos_iter;                     // goto next to facilitate loop condition
    }
    while (pos_iter != pos_first) {
      ArgumentIterator = *(--pos_iter);
      if (OptionBlockMode &&
          (pos_iter == OptionBlockIterator))  // new option is in front of
      ...
      else if (*ArgumentIterator == longOpt)  // searched option
      {
        ...
        return OFTrue;
                 ...
                 (--diropt <= 0))  // search only for the direct predecessor
        return OFFalse;
    }
  }
  return OFFalse;
...
OFCommandLine::E_ValueStatus OFCommandLine::getValueAndCheckMin(
    OFCmdSignedInt& value, const OFCmdSignedInt low, const OFBool incl) {
...
OFCommandLine::E_ValueStatus OFCommandLine::getValueAndCheckMinMax(
    OFCmdSignedInt& value, const OFCmdSignedInt low,
    const OFCmdSignedInt high) {
  E_ValueStatus status = getValue(value);
  if (status == VS_Normal) {
    if (value < low)
...
OFCommandLine::E_ValueStatus OFCommandLine::getValueAndCheckMin(
    OFCmdUnsignedInt& value, const OFCmdUnsignedInt low, const OFBool incl) {
  E_ValueStatus status = getValue(value);
  if (status == VS_Normal) {
    if ((value < low) || (!incl && (value == low)))
      return VS_Underflow;
...
OFCommandLine::E_ValueStatus OFCommandLine::getValueAndCheckMinMax(
    OFCmdUnsignedInt& value, const OFCmdUnsignedInt low,
    const OFCmdUnsignedInt high) {
  E_ValueStatus status = getValue(value);
  if (status == VS_Normal) {
    if (value < low)
...
OFCommandLine::E_ValueStatus OFCommandLine::getValueAndCheckMin(
    OFCmdFloat& value, const OFCmdFloat low, const OFBool incl) {
  E_ValueStatus status = getValue(value);
  if (status == VS_Normal) {
    if ((value < low) || (!incl && (value == low)))
      return VS_Underflow;
...
OFCommandLine::E_ValueStatus OFCommandLine::getValueAndCheckMinMax(
    OFCmdFloat& value, const OFCmdFloat low, const OFCmdFloat high) {
  E_ValueStatus status = getValue(value);
  if (status == VS_Normal) {
    if (value < low)
      return VS_Underflow;
...
void OFCommandLine::storeParameter(const OFString& param, const int directOpt) {
  ArgumentList.push_back(param);
  const OFListIterator(OFListIterator_OFString) iter =
      (OptionPosList.size() == 0) ? OptionPosList.end() : --OptionPosList.end();
  OFCmdParamPos* paramPos = new OFCmdParamPos(--ArgumentList.end(), iter,
                                              OptionPosList.size(), directOpt);
  if (paramPos != NULL)
    ParamPosList.push_back(paramPos);
}

int OFCommandLine::packColumnValues(int longCols, int shortCols) const {
  if (longCols < 0)
    longCols = 0;
  if (shortCols < 0)
    shortCols = 0;
...
void OFCommandLine::unpackColumnValues(const int value, unsigned int& longCols,
...
#ifdef HAVE_WINDOWS_H
void OFCommandLine::expandWildcards(const OFString& param, int directOpt) {
  const size_t paramLen = param.length();
  if ((paramLen >= 2) && (param.at(0) == '"') &&
      (param.at(paramLen - 1) == '"'))
    storeParameter(param.substr(1, paramLen - 2).c_str(),
                   directOpt);  // remove quotations
  else {
    size_t pos1 = param.find_first_of("*?");  // search for wildcards
    if (pos1 != OFString_npos) {
      OFString name;
      WIN32_FIND_DATA data;
      size_t pos2 = param.find_first_of(
          ":\\", pos1);  // search for next separator (":" or "\")
      HANDLE handle = FindFirstFile(param.substr(0, pos2).c_str(), &data);
      if (handle !=
          INVALID_HANDLE_VALUE)  // find first file/dir matching the wildcards
      {
        do {
          if ((strcmp(data.cFileName, ".") != 0) &&
              (strcmp(data.cFileName, "..") != 0)) {  // skip "." and ".."
            ...
            if (GetFileAttributes(name.c_str()) != 0xFFFFFFFF) {
              ...
              expandWildcards(
                  name.c_str(),
                  directOpt);  // recursively expand further wildcards
          }
        } while (FindNextFile(
            handle, &data));  // while further files/dirs exist ... add them
        FindClose(handle);
      }
    } else
      storeParameter(
          param, directOpt);  // parameter contains no wildcards, just add it
  }
}
#endif
...
OFCommandLine::E_ParseStatus OFCommandLine::parseCommandFile(
    const char* argValue, OFList<OFString>& argList) {
  E_ParseStatus result = PS_NoArguments;
  /* check for command file parameter (syntax: "@filename") */
  if ((argValue != NULL) && (strlen(argValue) > 1) &&
      (argValue[0] == COMMAND_FILE_PREFIX)) {
    /* skip '@' symbol in filename */
    const char* filename = argValue + 1;
    /* open command file */
#ifdef HAVE_IOS_NOCREATE
    STD_NAMESPACE ifstream cmdFile(
        filename, STD_NAMESPACE ios::in | STD_NAMESPACE ios::nocreate);
#else
    ...
    if (cmdFile) {
      ...
      if (block != 0) {
        ofConsole.lockCerr()
            << "WARNING: closing quotation mark (" << block
            << ") missing in command file " << filename << OFendl;
        ofConsole.unlockCerr();
      }
#endif
      result = PS_Normal;
  ...
  if (argCount > startPos)  // any command line arguments?
  {
    ...
    for (i = startPos; i < argCount; i++)  // skip program name (argValue[0])
    {
      if (flags & PF_NoCommandFiles)  // do not try to detect command files
        ...
        if (status == PS_NoArguments)
          ...
          ArgumentList.push_back(argValue[i] +
                                 1);  // store filename for error message
          return status;
        }
      }
    }
    ...
    while (argIter != argEnd) {
      if (!checkOption(*argIter, OFFalse))  // arg = parameter
      {
        ...
        if (opt != NULL) {
          ArgumentList.push_back(OFstatic_cast(
              OFString, opt->LongOption));  // convert argument to long format
          ...
          while (j-- > 0) {
            ArgumentList.push_back(
                *(++argIter));  // add values to argument list
            i--;
          }
        } else {                             // unknown
    ...
    while (iter != last) {
      ...
      if (!((*iter)->Flags & OFCommandLine::AF_Internal)) {
        ...
        if ((*iter)->LongOption.length() <= 0) {
          ...
          while (((pos = (str.find('\n', pos))) != OFString_npos) &&
                 (pos < str.length()))
            str.insert(++pos, OFString(lineIndent + shortSize + longSize +
                                           2 * columnSpace,
                                       ' '));
          optionStr += str;
          optionStr += "\n";
...
void OFCommandLine::getParamString(OFString& paramStr) const {
  ...
  if (!ValidParamList.empty()) {
    ...
    while ((iter != last) && ((*iter)->ParamDescription.length() > 0)) {
      if ((*iter)->ParamName.length() >
          columnSize)  // determine maximum column width
    ...
    while (iter != last) {
      if ((*iter)->ParamDescription.length() > 0) {
        ...
        while (((pos = (str.find('\n', pos))) != OFString_npos) &&
               (pos < str.length()))
          str.insert(++pos,
                     OFString(lineIndent + columnSize + columnSpace, ' '));
        paramStr += str;
        paramStr += "\n";
      }
      ++iter;
