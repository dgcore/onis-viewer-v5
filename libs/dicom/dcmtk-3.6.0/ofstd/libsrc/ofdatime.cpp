...
OFDateTime::OFDateTime() : Date(), Time() {}

OFDateTime::OFDateTime(const OFDateTime& dateTime)
    : Date(dateTime.Date), Time(dateTime.Time) {}

OFDateTime::OFDateTime(const OFDate& dateVal, const OFTime& timeVal)
    : Date(dateVal), Time(timeVal) {}

OFDateTime::OFDateTime(const unsigned int year, const unsigned int month,
                       const unsigned int day, const unsigned int hour,
                       const unsigned int minute, const double second,
                       const double timeZone)
    : Date(year, month, day), Time(hour, minute, second, timeZone) {}
...
OFBool OFDateTime::setDateTime(const unsigned int year,
                               const unsigned int month, const unsigned int day,
                               const unsigned int hour,
                               const unsigned int minute, const double second,
                               const double timeZone) {
  OFBool result = OFFalse;
  /* check whether new date and time are valid */
  if (OFDate::isDateValid(year, month, day) &&
      OFTime::isTimeValid(hour, minute, second, timeZone))
    result = Date.setDate(year, month, day) &&
             Time.setTime(hour, minute, second, timeZone);
  return result;
}
...
OFBool OFDateTime::setDateTime(const OFDate& dateVal, const OFTime& timeVal) {
...
OFBool OFDateTime::setCurrentDateTime() {
  /* set current system date and time, use the same "time_t" struct to avoid
   * inconsistencies */
  time_t tt = time(NULL);
...
OFBool OFDateTime::setISOFormattedDateTime(const OFString& formattedDateTime) {
  ...
  if ((length == 12) || (length == 14)) {
    if (Date.setISOFormattedDate(formattedDateTime.substr(0, 8)) &&
        Time.setISOFormattedTime(formattedDateTime.substr(8)))
      result = OFTrue;
  }
  /* YYYY-MM-DD HH:MM[:SS] */
  else if (length >= 16) {
    if (Date.setISOFormattedDate(formattedDateTime.substr(0, 10))) {
      ...
      while ((pos < length) &&
             !isdigit(OFstatic_cast(unsigned char, formattedDateTime.at(pos))))
        ++pos;
      if ((pos < length) &&
          Time.setISOFormattedTime(formattedDateTime.substr(pos)))
        result = OFTrue;
    }
  }
  return result;
...
OFBool OFDateTime::getISOFormattedDateTime(OFString& formattedDateTime,
   ...
   * file) */
  return getISOFormattedDateTime(formattedDateTime, showSeconds, showFraction,
                                 showTimeZone, showDelimiter,
                                 " " /*dateTimeSeparator*/);
}

OFBool OFDateTime::getISOFormattedDateTime(
    OFString& formattedDateTime, const OFBool showSeconds,
    const OFBool showFraction, const OFBool showTimeZone,
    const OFBool showDelimiter, const OFString& dateTimeSeparator) const {
  ...
  if (result) {
    ...
    OFString timeString;
    if (Time.getISOFormattedTime(timeString, showSeconds, showFraction,
                                 showTimeZone, showDelimiter)) {
      if (showDelimiter)
