...
OFTime::OFTime() : Hour(0), Minute(0), Second(0), TimeZone(0) {}
...
OFTime::OFTime(const unsigned int hour, const unsigned int minute,
...
OFBool OFTime::operator==(const OFTime& timeVal) const {
  return (
      getTimeInSeconds(OFTrue /*useTimeZone*/, OFFalse /*normalize*/) ==
      timeVal.getTimeInSeconds(OFTrue /*useTimeZone*/, OFFalse /*normalize*/));
}
...
OFBool OFTime::isTimeValid(const unsigned int hour, const unsigned int minute,
                           const double second, const double timeZone) {
  /* check whether given time is valid */
  return (hour < 24) && (minute < 60) && (second >= 0) && (second < 60) &&
         (timeZone >= -12) && (timeZone <= 12);
}

OFBool OFTime::setTime(const unsigned int hour, const unsigned int minute,
                       const double second, const double timeZone) {
...
OFBool OFTime::setTimeZone(const signed int hour, const unsigned int minute) {
  /* convert hour and minute values to one time zone value */
  const double timeZone =
      (hour < 0)
          ? OFstatic_cast(double, hour) - OFstatic_cast(double, minute) / 60
          : OFstatic_cast(double, hour) + OFstatic_cast(double, minute) / 60;
  /* only change the currently stored value if the new time zone is valid */
  return setTimeZone(timeZone);
}

OFBool OFTime::setTimeInSeconds(const double seconds, const double timeZone,
  ...
  if (normalize || ((seconds >= 0) && (seconds < 86400))) {
    ...
    const double normalSeconds =
        (normalize)
            ? seconds + OFstatic_cast(signed long, seconds / 86400) * 86400
            : seconds;
    /* compute time from given number of seconds since "00:00:00" */
    const unsigned int newHour =
        OFstatic_cast(unsigned int, normalSeconds / 3600);
    const unsigned int newMinute = OFstatic_cast(
        unsigned int,
        (normalSeconds - OFstatic_cast(double, newHour) * 3600) / 60);
    const double newSecond = normalSeconds -
                             OFstatic_cast(double, newHour) * 3600 -
                             OFstatic_cast(double, newMinute) * 60;
    status = setTime(newHour, newMinute, newSecond, timeZone);
  }
  return status;
}

OFBool OFTime::setTimeInHours(const double hours, const double timeZone,
                              const OFBool normalize) {
  OFBool status = OFFalse;
  /* only change if the new time is valid */
  if (normalize || ((hours >= 0) && (hours < 24))) {
    /* first normalize the value to the valid range of [0.0,24.0[ */
    const double normalHours =
        (normalize) ? hours + OFstatic_cast(signed long, hours / 24) * 24
                    : hours;
    /* compute time from given number of hours since "00:00:00" */
    const unsigned int newHour = OFstatic_cast(unsigned int, normalHours);
    const unsigned int newMinute = OFstatic_cast(
        unsigned int, (normalHours - OFstatic_cast(double, newHour)) * 60);
    const double newSecond =
        (normalHours - OFstatic_cast(double, newHour)) * 3600 -
        OFstatic_cast(double, newMinute) * 60;
    status = setTime(newHour, newMinute, newSecond, timeZone);
  }
  return status;
...
OFBool OFTime::setCurrentTime(const time_t& tt) {
  ...
  if (lt != NULL) {
    ...
    if (gt != NULL) {
      ...
      TimeZone = (lt->tm_hour - gt->tm_hour) +
                 OFstatic_cast(double, lt->tm_min - gt->tm_min) / 60;
...
OFBool OFTime::setISOFormattedTime(const OFString& formattedTime) {
  ...
  /* HHMMSS */
    ...
    /* extract components from time string */
    if (sscanf(formattedTime.c_str(), "%02u%02u%02u", &hours, &minutes,
               &seconds) == 3)
  ...
  /* HH:MM:SS */
    ...
    /* extract components from time string */
    if (sscanf(formattedTime.c_str(), "%02u%*c%02u%*c%02u", &hours, &minutes,
               &seconds) == 3)
      status = setTime(hours, minutes, seconds);
  }
  return status;
...
unsigned int OFTime::getMilliSecond() const {
  return OFstatic_cast(unsigned int,
                       (Second - OFstatic_cast(unsigned int, Second)) * 1000);
}

unsigned int OFTime::getMicroSecond() const {
  return OFstatic_cast(
      unsigned int, (Second - OFstatic_cast(unsigned int, Second)) * 1000000);
}
...
double OFTime::getTimeInSeconds(const unsigned int hour,
                                ...
                                const double timeZone, const OFBool normalize) {
  /* compute number of seconds since 00:00:00 */
  double result = ((OFstatic_cast(double, hour) - timeZone) * 60 +
                   OFstatic_cast(double, minute)) *
                      60 +
                  second;
...
double OFTime::getTimeInHours(const unsigned int hour,
  ...
  double result = OFstatic_cast(double, hour) - timeZone +
                  (OFstatic_cast(double, minute) + second / 60) / 60;
...
OFTime OFTime::getCoordinatedUniversalTime() const {
  ...
  OFTime timeVal;
  /* convert time to UTC */
  timeVal.setTimeInHours(getTimeInHours(OFTrue /*useTimeZone*/),
                         0 /*timeZone*/);
  /* return by-value */
  return timeVal;
}
...
OFTime OFTime::getLocalTime() const {
  ...
  if (TimeZone != localTimeZone)
    timeVal.setTimeInHours(
        getTimeInHours(OFTrue /*useTimeZone*/) + localTimeZone, localTimeZone);
  else {
...
OFBool OFTime::getISOFormattedTime(OFString& formattedTime,
  ...
  if (isValid()) {
    ...
    if (showTimeZone) {
      ...
      const unsigned int zoneMinute =
          OFstatic_cast(unsigned int, (zoneAbs - zoneHour) * 60);
