...
/* if WITH_THREADS is undefined, we don't even attempt to implement a thread
 * interface. */
#ifdef WITH_THREADS
...
int OFThread::start() {
  ...
  unsigned int tid = 0;
  theThreadHandle =
      _beginthreadex(NULL, 0, thread_stub, OFstatic_cast(void*, this), 0, &tid);
  if (theThreadHandle == 0)
    return errno;
  else {
  ...
  pthread_t tid = 0;
  int result =
      pthread_create(&tid, NULL, thread_stub, OFstatic_cast(void*, this));
#ifdef HAVE_POINTER_TYPE_PTHREAD_T
  if (0 == result)
...
#else
  if (0 == result)
    theThread = OFstatic_cast(unsigned long, tid);
  else
    theThread = 0;
#endif
  return result;
  ...
  thread_t tid = 0;
  int result =
      thr_create(NULL, 0, thread_stub, OFstatic_cast(void*, this), 0, &tid);
  if (0 == result)
    theThread = OFstatic_cast(unsigned long, tid);
  else
    theThread = 0;
...
int OFThread::join() {
#ifdef WINDOWS_INTERFACE
  if (WaitForSingleObject((HANDLE)theThreadHandle, INFINITE) == WAIT_OBJECT_0)
    return 0;
  else
...
#ifdef WINDOWS_INTERFACE
#ifdef HAVE_POINTER_TYPE_PTHREAD_T
  if (theThread == OFreinterpret_cast(void*, tID))
    return OFTrue;
  else
    return OFFalse;
#else
...
#elif defined(POSIX_INTERFACE)
#ifdef HAVE_POINTER_TYPE_PTHREAD_T
  // dangerous - we cast an unsigned long back to a pointer type and hope that
  // it is still valid
  if (pthread_equal(OFstatic_cast(pthread_t, theThread),
                    OFreinterpret_cast(pthread_t, tID)))
    return OFTrue;
  else
    return OFFalse;
#else
  if (pthread_equal(OFstatic_cast(pthread_t, theThread),
                    OFstatic_cast(pthread_t, tID)))
    return OFTrue;
  else
    return OFFalse;
#endif
#elif defined(SOLARIS_INTERFACE)
  if (OFstatic_cast(thread_t, theThread) == OFstatic_cast(thread_t, tID))
    return OFTrue;
  else
    return OFFalse;
#else
...
{
#ifdef WINDOWS_INTERFACE
  if (code == OFThread::busy)
    description = "another thread already waiting for join";
  else {
    LPVOID buf;
    FormatMessage(
...
OFThreadSpecificData::OFThreadSpecificData() : theKey(NULL) {
#ifdef WINDOWS_INTERFACE
  DWORD* key = new DWORD;
  if (key) {
    *key = TlsAlloc();
...
#ifdef WINDOWS_INTERFACE
  if (theKey)
    TlsFree(*OFthread_cast(DWORD*, theKey));
#elif defined(POSIX_INTERFACE)
...
OFBool OFThreadSpecificData::initialized() const {
  ...
  if (theKey)
    ...
    return OFFalse;
#else
  return OFFalse;  // thread specific data is not supported if we are working in
...
#ifdef WINDOWS_INTERFACE
  if (theKey) {
    if (0 == TlsSetValue(*((DWORD*)theKey), value))
      return (int)GetLastError();
    else
      return 0;
  } else
...
#elif defined(POSIX_INTERFACE)
  if (theKey)
    return pthread_setspecific(*OFthread_cast(pthread_key_t*, theKey), value);
  else
    return EINVAL;
#elif defined(SOLARIS_INTERFACE)
  if (theKey)
    return thr_setspecific(*OFthread_cast(thread_key_t*, theKey), value);
  else
    return EINVAL;
#else
  return -1;
...
int OFThreadSpecificData::get(void*& value) {
  ...
  value = NULL;
  if (theKey)
    return thr_getspecific(*OFthread_cast(thread_key_t*, theKey), &value);
  else
    return EINVAL;
#else
  value = NULL;
...
const int OFSemaphore::busy =
    EAGAIN;  // Posix returns EAGAIN instead of EBUSY in trywait.
#elif defined(SOLARIS_INTERFACE)
...
OFSemaphore::OFSemaphore(unsigned int /* numResources */)
    ...
    : theSemaphore(NULL) {
#ifdef WINDOWS_INTERFACE
  theSemaphore = OFstatic_cast(
      void*, CreateSemaphore(NULL, numResources, numResources, NULL));
#elif defined(POSIX_INTERFACE)
  sem_t* sem = new sem_t;
  if (sem) {
    if (sem_init(sem, 0, numResources) == -1)
      delete sem;
    else
      theSemaphore = sem;
  }
  ...
  if (sem) {
    if (sema_init(sem, numResources, USYNC_THREAD, NULL))
      delete sem;
    else
      theSemaphore = sem;
  }
...
#ifdef WINDOWS_INTERFACE
  if (WaitForSingleObject((HANDLE)theSemaphore, INFINITE) == WAIT_OBJECT_0)
    return 0;
  else
    return (int)GetLastError();
#elif defined(POSIX_INTERFACE)
  if (theSemaphore) {
    if (sem_wait(OFthread_cast(sem_t*, theSemaphore)))
      return errno;
    else
      return 0;
  } else
...
#elif defined(SOLARIS_INTERFACE)
  if (theSemaphore)
    return sema_wait(OFthread_cast(sema_t*, theSemaphore));
  else
    return EINVAL;
#else
  return -1;
...
int OFSemaphore::trywait() {
  ...
  if (result == WAIT_OBJECT_0)
    ...
    return OFSemaphore::busy;
  else
    return (int)GetLastError();
#elif defined(POSIX_INTERFACE)
  if (theSemaphore) {
    if (sem_trywait(OFthread_cast(sem_t*, theSemaphore)))
      return errno;
    else
      return 0;  // may return EAGAIN
  } else
    return EINVAL;
#elif defined(SOLARIS_INTERFACE)
  if (theSemaphore)
    return sema_trywait(OFthread_cast(sema_t*, theSemaphore));
  else
    return EINVAL;  // may return EBUSY
#else
  return -1;
#endif
}
...
#ifdef WINDOWS_INTERFACE
  if (ReleaseSemaphore((HANDLE)theSemaphore, 1, NULL))
    return 0;
  else
    return (int)GetLastError();
#elif defined(POSIX_INTERFACE)
  if (theSemaphore) {
    if (sem_post(OFthread_cast(sem_t*, theSemaphore)))
      return errno;
    else
      return 0;
  } else
...
#elif defined(SOLARIS_INTERFACE)
  if (theSemaphore)
    return sema_post(OFthread_cast(sema_t*, theSemaphore));
  else
    return EINVAL;
#else
  return -1;
...
{
#ifdef WINDOWS_INTERFACE
  if (code == OFSemaphore::busy)
    description = "semaphore is already locked";
  else {
    LPVOID buf;
    FormatMessage(
        FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL,
...
OFMutex::OFMutex() : theMutex(NULL) {
  ...
  if (mtx) {
    if (mutex_init(mtx, USYNC_THREAD, NULL))
      delete mtx;
    else
      theMutex = mtx;
  }
...
#elif defined(POSIX_INTERFACE)
  if (theMutex)
    return pthread_mutex_trylock(OFthread_cast(pthread_mutex_t*, theMutex));
  else
    return EINVAL;  // may return EBUSY.
#elif defined(SOLARIS_INTERFACE)
  if (theMutex)
    return mutex_trylock(OFthread_cast(mutex_t*, theMutex));
  else
    return EINVAL;  // may return EBUSY.
#else
  return -1;
...
#elif defined(POSIX_INTERFACE)
  if (theMutex)
    return pthread_mutex_unlock(OFthread_cast(pthread_mutex_t*, theMutex));
  else
    return EINVAL;
#elif defined(SOLARIS_INTERFACE)
  if (theMutex)
    return mutex_unlock(OFthread_cast(mutex_t*, theMutex));
  else
    return EINVAL;
#else
  return -1;
...
#ifdef WINDOWS_INTERFACE
  if (code == OFSemaphore::busy)
    description = "mutex is already locked";
  else {
    LPVOID buf;
    FormatMessage(
        FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL,
...
private:
  ...
  /* undefined */ OFReadWriteLockHelper& operator=(
      const OFReadWriteLockHelper& arg);
};
...
OFReadWriteLock::OFReadWriteLock() : theLock(NULL) {
#if defined(WINDOWS_INTERFACE) || defined(POSIX_INTERFACE_WITHOUT_RWLOCK)
  OFReadWriteLockHelper* rwl = new OFReadWriteLockHelper();
  if ((rwl->accessMutex.initialized()) && (rwl->usageSemaphore.initialized()))
    theLock = rwl;
  else
    delete rwl;
#elif defined(POSIX_INTERFACE)
  ...
  if (rwl) {
    if (rwlock_init(rwl, USYNC_THREAD, NULL))
      delete rwl;
    else
      theLock = rwl;
  }
...
int OFReadWriteLock::rdlock() {
  ...
  if (theLock) {
    ...
    int result = 0;
    while (1) {
      if (0 != (result = rwl->accessMutex.lock()))
        return result;           // lock mutex
      ...
      if (0 != (result = rwl->accessMutex.unlock()))
        return result;
      if (0 != (result = rwl->usageSemaphore.wait()))
        return result;
      if (0 != (result = rwl->usageSemaphore.post()))
        return result;
    }
  } else
    return EINVAL;
#elif defined(POSIX_INTERFACE)
  if (theLock)
    return pthread_rwlock_rdlock(OFthread_cast(pthread_rwlock_t*, theLock));
  else
    return EINVAL;
#elif defined(SOLARIS_INTERFACE)
  if (theLock)
    return rw_rdlock(OFthread_cast(rwlock_t*, theLock));
  else
    return EINVAL;
#else
  return -1;
...
#elif defined(POSIX_INTERFACE)
  if (theLock)
    return pthread_rwlock_wrlock(OFthread_cast(pthread_rwlock_t*, theLock));
  else
    return EINVAL;
#elif defined(SOLARIS_INTERFACE)
  if (theLock)
    return rw_wrlock(OFthread_cast(rwlock_t*, theLock));
  else
    return EINVAL;
#else
  return -1;
...
int OFReadWriteLock::tryrdlock() {
  ...
  if (theLock) {
    ...
    if (result)
      ...
      return OFReadWriteLock::busy;
  } else
    return EINVAL;
#elif defined(POSIX_INTERFACE)
  if (theLock)
    return pthread_rwlock_tryrdlock(OFthread_cast(pthread_rwlock_t*, theLock));
  else
    return EINVAL;  // may return EBUSY.
#elif defined(SOLARIS_INTERFACE)
  if (theLock)
    return rw_tryrdlock(OFthread_cast(rwlock_t*, theLock));
  else
    return EINVAL;  // may return EBUSY.
#else
  return -1;
...
#elif defined(POSIX_INTERFACE)
  if (theLock)
    return pthread_rwlock_trywrlock(OFthread_cast(pthread_rwlock_t*, theLock));
  else
    return EINVAL;  // may return EBUSY.
#elif defined(SOLARIS_INTERFACE)
  if (theLock)
    return rw_trywrlock(OFthread_cast(rwlock_t*, theLock));
  else
    return EINVAL;  // may return EBUSY.
#else
  return -1;
...
int OFReadWriteLock::unlock() {
  ...
  if (theLock) {
    ...
    if (rwl->numReaders == -1)
      ...
      (rwl->numReaders)--;
    if ((rwl->numReaders == 0) &&
        (0 != (result = rwl->usageSemaphore.post()))) {
      rwl->accessMutex.unlock();
      return result;
    }
    return rwl->accessMutex.unlock();
...
#elif defined(POSIX_INTERFACE)
  if (theLock)
    return pthread_rwlock_unlock(OFthread_cast(pthread_rwlock_t*, theLock));
  else
    return EINVAL;
#elif defined(SOLARIS_INTERFACE)
  if (theLock)
    return rw_unlock(OFthread_cast(rwlock_t*, theLock));
  else
    return EINVAL;
#else
  return -1;
...
#ifdef WINDOWS_INTERFACE
  if (code == OFSemaphore::busy)
    description = "read/write lock is already locked";
  else {
    LPVOID buf;
    FormatMessage(
        FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL,
...
#define lockWarn(name, locked)                                                \
  if (locked == OFTrue) {                                                     \
    ofConsole.lockCout() << "OFReadWriteLocker::" name "(): Already locked?!" \
                         << OFendl;                                           \
...
OFReadWriteLockerFunction(rdlock) OFReadWriteLockerFunction(wrlock)
    OFReadWriteLockerFunction(tryrdlock) OFReadWriteLockerFunction(trywrlock)

        int OFReadWriteLocker::unlock() {
#ifdef DEBUG
  if (locked == OFFalse) {
    ofConsole.lockCout() << "OFReadWriteLocker::unlock(): Nothing to unlock?!"
                         << OFendl;
    ofConsole.unlockCout();
  }
#endif
 ...
 *   for threads, thread specific data, semaphores, mutexes and read/write
 * locks.
 *
 *
 */
