...
/* give up after this number of unsuccessful attempts to create a unique
 * filename */
#define MAX_TRY 1024

OFFilenameCreator::OFFilenameCreator() : creation_time(0) {
...
OFBool OFFilenameCreator::makeFilename(unsigned int seed, const char* dir,
  ...
  do {
    ...
    if (stat_result == 0) {
      ...
      if (errno != ENOENT)
        result = OFFalse;  // something wrong with path
    }
  } while (!done);
  return result;
}
...
void OFFilenameCreator::addLongToString(unsigned long l, OFString& s) {
  ...
  while (l) {
    ...
    if (m > 9)
      ...
      chr_array[idx--] = OFstatic_cast(char, '0' + m);
  }
  s += chr_array;
  return;
}
...
unsigned int OFFilenameCreator::hashString(const char* str) {
  /* very simple hash function: XOR result with string character and rotate left
   * by 1 bit. */
  unsigned int result = OFstatic_cast(unsigned int, -1);
  if (str) {
    while (*str != '\0') {
      result %= *str++;
      if (OFstatic_cast(signed int, result) < 0)
        result = (result << 1) | 1;
      else
        result <<= 1;
    }
...
int OFFilenameCreator::myrand_r(unsigned int* seed) {
  ...
  *seed = OFstatic_cast(unsigned int,
                        val % (OFstatic_cast(unsigned long, 0x80000000)));
