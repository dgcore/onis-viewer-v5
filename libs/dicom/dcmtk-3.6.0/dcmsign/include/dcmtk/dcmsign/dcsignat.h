...
public:
   ...
   *    The real list of attributes signed is derived from this parameter plus
   * the requirements of the security profile. If NULL, a universal match is
   * assumed, i.e. all signable attributes in the data set are signed.
   *  @param timeStamp pointer to time stamp client used to create timestamps
   *    for the digital signature.
   *  @return status code
   */
  OFCondition createSignature(SiPrivateKey& key, SiCertificate& cert,
                              SiMAC& mac, SiSecurityProfile& profile,
  ...
  /** selects one of the digital signatures from the attached dataset for
   * reading.
   *  @param i index, must be < numberOfSignatures()
   *  @return status code
   */
   ...
   *  Current signature must be selected with selectSignature().
   *  @param str transfer syntax name or UID returned in this parameter upon
   * success
   *  @return status code
   */
  ...
  /** returns the Data Elements Signed attribute of the current signature if
   * present. Current signature must be selected with selectSignature(). If a
   * valid signature is selected but the signature does not contain the Data
   * Elements Signed element (i.e. all attributes are signed), this method
   ...
   *  @param stack search stack, must be passed to findNextSignatureItem() later
   * on.
   *  @return pointer to Item containing a DigitalSignatureSequence if found,
   * NULL otherwise.
   */
   ...
   *  a pointer to the Item in which the sequence is contained.
   *  @param item dataset to be browsed
   *  @param stack search stack as returned by findFirstSignatureItem() or the
...
private:
   ...
   *  @param newID upon successful return, new number is passed in this
   * parameter
   *  @return status code
   */
  ...
  /// pointer to mac parameters sequence of attached item, may be NULL if not
  /// attached or not yet present
  DcmSequenceOfItems* macParametersSq;

  /// pointer to digital signatures sequence of attached item, may be NULL if
  /// not attached or not yet present
