 ...
 *  Instances of derived classes are able to request timestamps from a timestamp
 * service. Timestamps are not supported (yet).
 */
class SiTimeStamp {
public:
  /// default constructor
   ...
   *  @return status code
   */
  virtual OFCondition stamp(const unsigned char* inputData,
                            unsigned long inputDataSize) = 0;

  /** writes the current timestamp into an item of the Digital Signatures
   * Sequence and creates the timestamp type information in the item.
   *  @param item item of the DigitalSignatureSQ to which the timestamp is
   * written
