...
public:
   ...
   *  @param item item of the DigitalSignatureSQ from which the certificate is
   * read
   *  @return status code
   */
  OFCondition read(DcmItem& item);

  /** writes the current X.509 certificate into an item of the Digital
   * Signatures Sequence and creates the certificate type information in the
   * item.
   *  @param item item of the DigitalSignatureSQ to which the certificate is
   * written
  ...
  /** creates an SiAlgorithm object for the public key contained in this
   * certificate. If no certificate loaded or operation fails, returns NULL. New
   * SiAlgorithm object must be deleted by caller.
  ...
  /** returns the start of validity of the current certificate in human readable
   * form. If no certificate is loaded, returns an empty string.
   *  @param str start of validity returned in this string.
   */
  void getCertValidityNotBefore(OFString& str);

  /** returns the end of validity of the current certificate in human readable
   * form. If no certificate is loaded, returns an empty string.
