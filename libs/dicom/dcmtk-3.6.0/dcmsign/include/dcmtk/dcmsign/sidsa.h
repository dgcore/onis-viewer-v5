...
public:
   ...
   * Ignored for DSA signatures.
   *  @param outputSignature pointer to array of at least getSize() which must
   * be allocated by caller.
   *  @param outputSignatureSize returns the number of bytes written to
   * outputSignature.
   *  @return SI_EC_Normal if successful, errorcode otherwise.
   */
  virtual OFCondition sign(const unsigned char* inputHash,
   ...
   *  @param inputHash array of bytes containing hash key to be verified against
   * signature
   *  @param inputHashSize length of hash key array in bytes
   *  @param inputHashAlgorithm MAC algorithm used for creation of hash key.
   * Ignored for DSA signatures.
   ...
   *  @return SI_EC_Normal if successful, errorcode otherwise.
   */
  virtual OFCondition verify(const unsigned char* inputHash,
                             unsigned long inputHashSize,
                             E_MACType inputHashAlgorithm,
   ...
   *  The result depends on the public key algorithm, key size and padding
   * scheme. In general the input to decrypt() or encrypt() must be less than or
   * equal to this block size.  The output of decrypt() or encrypt() is always
   * equal to this block size.
