...
/** a class that allows to feed selected parts of a DICOM dataset into the MAC
 * generation code
 */
...
public:
   ...
   *     The caller might wish to use DcmItem::canWriteXfer() to check
   * beforehand whether this transfer syntax can be used.
   *  @param tagListOut upon return this parameter contains the list of
   * attribute tags which were fed into the MAC codec. For sequences, only the
   * sequence attribute tag is contained in this list; the items and elements
   * within the items are not encoded.
   ...
   *     which are signable (see DcmTagKey::isSignable()) and are present in the
   * dataset are encoded.  Upon verification of a signature the caller might
   * wish to compare tagListIn and tagListOut after successful return to see
   * whether the lists of attributes are the same. If parameter is absent or
   * NULL, a global match is assumed, i.e. all elements of the dataset which are
   * present and signable are encoded.
   *  @return status code
   */
  OFCondition encodeDataset(DcmItem& item, SiMAC& mac, E_TransferSyntax oxfer,
   ...
   *  @return status code
   */
  OFCondition encodeDigitalSignatureItem(DcmItem& signatureItem, SiMAC& mac,
...
private:
   ...
   *  If the element is a sequence, all signable elements from all items are
   * added.
   ...
   *  @return status code
   */
  OFCondition encodeElement(DcmElement* element, SiMAC& mac,
                            E_TransferSyntax oxfer);

  /** checks whether the attribute tag of the given DcmElement is contained
   *  in the given list of tags.  If the list is absent (NULL), a universal
   * match is assumed, i.e. always returns true if element is nonzero. Does not
   * check whether an element is signable.
