...
public:
  ...
  /** checks whether the given MAC object can be used with this security
   * profile.
  ...
  /** checks whether the given public/private key algorithm can be used with
   * this security profile.
   *  @param keyType public key algorithm type to be checked
   *  @return true if public key algorithm is allowable for this profile, false
   * otherwise.
   */
  virtual OFBool isAllowableAlgorithmType(E_KeyType keyType) const = 0;

  /** checks whether the given public/private key object can be used with this
   * security profile.
  ...
  /** checks whether the given transfer syntax can be used with this security
   * profile
   *  @param xfer transfer syntax to be checked
   *  @return true if transfer syntax is allowable for this profile, false
   * otherwise.
   */
   ...
   *  requirements of the current security profile. For all elements present in
   * the dataset, the attribute tag is inserted or removed from the list if
   * required by the profile.
   *  @param item dataset to be handled
   *  @param tagList attribute tag list to be updated
   *  @return status code
   */
  virtual OFCondition updateAttributeList(DcmItem& item,
   ...
   *  @return true if minimum requirements for profile are fulfilled, false
   * otherwise.
   */
