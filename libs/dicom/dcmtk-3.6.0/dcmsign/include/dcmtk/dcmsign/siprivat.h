...
public:
   ...
   *  encrypted private key file in PEM format (ASN.1/DER encoded files are
   * never encrypted). Must be called prior to loadPrivateKey() in order to be
   * effective.
  ...
  /** creates an SiAlgorithm object for the private key contained in this
   * certificate. If no key is loaded or operation fails, returns NULL. New
  ...
  /** checks if the private key and the certificate set using
   * setPrivateKeyFile() and setCertificateFile() match, i.e. if they establish
