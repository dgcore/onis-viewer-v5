...
SiCertificateVerifier::SiCertificateVerifier() : x509store(NULL), errorCode(0) {
...
OFCondition SiCertificateVerifier::addTrustedCertificateFile(
  ...
  if (!X509_LOOKUP_load_file(x509_lookup, fileName, fileType))
    return SI_EC_CannotRead;
  return EC_Normal;
}

OFCondition SiCertificateVerifier::addTrustedCertificateDir(
    const char* pathName, int fileType) {
  /* fileType should be X509_FILETYPE_PEM or X509_FILETYPE_ASN1 */
  X509_LOOKUP* x509_lookup =
      X509_STORE_add_lookup(x509store, X509_LOOKUP_hash_dir());
  if (x509_lookup == NULL)
    return SI_EC_OpenSSLFailure;
...
OFCondition SiCertificateVerifier::verifyCertificate(
    SiCertificate& certificate) {
  errorCode = 0;
  ...
  if (ok)
    ...
    return SI_EC_VerificationFailed_NoTrust;
}
