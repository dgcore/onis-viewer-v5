...
OFCondition SiRSA::sign(const unsigned char* inputHash,
  ...
  int error =
      RSA_sign(openSSLmac, (unsigned char*)inputHash,
               (unsigned int)inputHashSize, outputSignature, &sigLen, rsa);
...
OFCondition SiRSA::verify(const unsigned char* inputHash,
                          unsigned long inputHashSize,
                          E_MACType inputHashAlgorithm,
                          const unsigned char* inputSignature,
                          unsigned long inputSignatureSize, OFBool& verified) {
  verified = OFFalse;
  if (rsa == NULL)
    return SI_EC_InitializationFailed;
  ...
  // we have to cast away const on inputHash yet because of OpenSSL limitations
  // we have to cast away const on inputSignature yet because of OpenSSL
  // limitations
  int error = RSA_verify(
      openSSLmac, (unsigned char*)inputHash, (unsigned int)inputHashSize,
      (unsigned char*)inputSignature, (unsigned int)inputSignatureSize, rsa);
  if (error < 0)
    return SI_EC_OpenSSLFailure;
  else if (error > 0)
    verified = OFTrue;
