...
SiCertificate::SiCertificate() : x509(NULL) {}
  ...
  if (result.good()) {
    ...
    if (result.good()) {
      if (aString == SI_DEFTERMS_X509CERT) {
        stack.clear();
        result =
            item.search(DCM_CertificateOfSigner, stack, ESM_fromHere, OFFalse);
        if (result.good()) {
  ...
  if (result.good()) {
    ...
    if (elem) {
      ...
      if (data) {
        ...
        i2d_X509(
            x509,
            &data2);  // data2 now points to the last element of the byte array
