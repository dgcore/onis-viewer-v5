...
Uint16 DcmSignature::getMACIDnumber(DcmItem& item) {
  ...
  DcmStack stack;
  if (item.search(DCM_MACIDNumber, stack, ESM_fromHere, OFFalse).good() &&
      (stack.top()->isLeaf())) {
...
void DcmSignature::currentDateTime(OFString& str) {
  DcmDateTime::getCurrentDateTime(str, OFTrue /*seconds*/, OFTrue /*fraction*/,
                                  OFTrue /*timeZone*/);
}
...
void DcmSignature::attach(DcmItem* dataset) {
  ...
  DcmStack stack;
  if (currentItem) {
    if ((currentItem->search(DCM_MACParametersSequence, stack, ESM_fromHere,
                             OFFalse))
            .good() &&
        (stack.top()->ident() == EVR_SQ)) {
      macParametersSq = (DcmSequenceOfItems*)stack.top();
    } else
      macParametersSq = NULL;
    stack.clear();
    if ((currentItem->search(DCM_DigitalSignaturesSequence, stack, ESM_fromHere,
                             OFFalse))
            .good() &&
        (stack.top()->ident() == EVR_SQ)) {
...
OFCondition DcmSignature::removeSignature(unsigned long i) {
  ...
  for (j = 0; j < seqCard; ++j) {
    ...
    if ((i != j) && tmpItem && (macIDnumber == getMACIDnumber(*tmpItem)))
      macIDunique = OFFalse;
  }
  ...
  if (macParametersSq && macIDunique) {
    ...
    while (j < macParametersSq->card()) {
      ...
      if (tmpItem && (macIDnumber == getMACIDnumber(*tmpItem)))
        delete macParametersSq->remove(j);
      else
        ++j;
    }
  }
...
OFCondition DcmSignature::allocateMACID(Uint16& newID) {
  ...
  if (signatureSq) {
    ...
    for (i = 0; i < sqCard; ++i) {
      ...
      if (tmpItem)
        isAllocated[getMACIDnumber(*tmpItem)] = 1;
    }
  }
  if (macParametersSq) {
...
OFCondition DcmSignature::createSignature(SiPrivateKey& key,
  ...
  if (!key.matchesCertificate(cert))
    return SI_EC_CertificateDoesNotMatchPrivateKey;
  if (!profile.isAllowableMAC(mac))
    return SI_EC_MacDoesNotMatchProfile;
  if (!profile.isAllowableAlgorithmType(key.getKeyType()))
    return SI_EC_AlgorithmDoesNotMatchProfile;
  if (!profile.isAllowableTransferSyntax(xfer))
    return SI_EC_TransferSyntaxDoesNotMatchProfile;
  ...
  if (tagList) {
    ...
    if (updatedTagList == NULL)
      result = EC_MemoryExhausted;
    else
      result = profile.updateAttributeList(*currentItem, *updatedTagList);
  }
    ...
    if (seqItem) {
      ...
      if (result.good()) {
        ...
        if (elemMacID) {
          ...
          if (result.good())
            result = seqItem->insert(elemMacID, OFTrue);
          if (result.bad()) {
            delete elemMacID;
          }
        } else
      ...
      if (result.good()) {
        ...
        DcmUniqueIdentifier* elemSigUID =
            new DcmUniqueIdentifier(DCM_DigitalSignatureUID);
        if (elemSigUID) {
          result = elemSigUID->putString(newUID);
          if (result.good())
            result = seqItem->insert(elemSigUID, OFTrue);
          if (result.bad()) {
            delete elemSigUID;
          }
        } else
      ...
      if (result.good()) {
        ...
        if (elemDT) {
          ...
          if (result.good())
            result = seqItem->insert(elemDT, OFTrue);
          if (result.bad()) {
            delete elemDT;
          }
        } else
  ...
  if (dumpFile)
    constructor.setDumpFile(dumpFile);
  ...
  if (result.good()) {
    ...
    if (algorithm) {
      ...
      if (signature == NULL)
        ...
        // encode main dataset
        result = constructor.encodeDataset(*currentItem, mac, xfer, *tagListOut,
                                           updatedTagList);
        // encode required attributes from the digital signatures sequence
        if (result.good())
          result = constructor.encodeDigitalSignatureItem(*seqItem, mac, xfer);
        // flush stream constructor
        if (result.good())
          result = constructor.flush(mac);
      }
    } else
      result = EC_MemoryExhausted;
  }
  ...
  if (result.good()) {
    ...
    if (digest == NULL)
      ...
      if (result.good())
        result = algorithm->sign(digest, digestLength, mac.macType(), signature,
                                 sigLength);
      delete[] digest;
    }
  }
  ...
  if (result.good()) {
    if (seqItem) {
      // Signature
      if (result.good()) {
        DcmOtherByteOtherWord* elemSig =
            new DcmOtherByteOtherWord(DCM_Signature);
        if (elemSig) {
          elemSig->setVR(EVR_OB);
          result = elemSig->putUint8Array((Uint8*)signature, sigLength);
          if (result.good())
            result = seqItem->insert(elemSig, OFTrue);
          if (result.bad()) {
            delete elemSig;
          }
        } else
    ...
    if (macItem) {
      ...
      if (result.good()) {
        ...
        if (elemMacID) {
          result = elemMacID->putUint16(macID);
          if (result.good())
            result = macItem->insert(elemMacID, OFTrue);
          if (result.bad()) {
            delete elemMacID;
          }
        } else
      ...
      if (result.good()) {
        DcmUniqueIdentifier* elemXferUID =
            new DcmUniqueIdentifier(DCM_MACCalculationTransferSyntaxUID);
        if (elemXferUID) {
          DcmXfer xid(xfer);
          result = elemXferUID->putString(xid.getXferID());
          if (result.good())
            result = macItem->insert(elemXferUID, OFTrue);
          if (result.bad()) {
            delete elemXferUID;
          }
        } else
      ...
      if (result.good()) {
        ...
        if (elemMacAlgo) {
          ...
          if (result.good())
            result = macItem->insert(elemMacAlgo, OFTrue);
          if (result.bad()) {
            delete elemMacAlgo;
          }
        } else
...
OFCondition DcmSignature::selectSignature(unsigned long i) {
  deselect();
  if ((signatureSq == NULL) || (i >= signatureSq->card()))
    return EC_IllegalCall;

  selectedSignatureItem = signatureSq->getItem(i);
  if (selectedSignatureItem == NULL)
    return EC_IllegalCall;
...
OFCondition DcmSignature::verifyCurrent() {
  if (NULL == selectedSignatureItem)
    return EC_IllegalCall;
  if (selectedMacParametersItem == NULL)
    return SI_EC_VerificationFailed_NoMAC;
  if ((selectedCertificate == NULL) ||
      (selectedCertificate->getKeyType() == EKT_none))
    return SI_EC_VerificationFailed_NoCertificate;
  ...
  if (result.good()) {
    if ((selectedMacParametersItem->search(DCM_MACCalculationTransferSyntaxUID,
      ...
      if ((((DcmElement*)(stack.top()))->getString(uid)).good()) {
        ...
        xfer = xf.getXfer();
        if (xfer == EXS_Unknown)
          result = SI_EC_WrongTransferSyntax;
      } else
        result = SI_EC_VerificationFailed_NoMAC;
    } else
      result = SI_EC_VerificationFailed_NoMAC;
  }
  ...
  if (result.good()) {
    stack.clear();
    if ((selectedMacParametersItem->search(DCM_MACAlgorithm, stack,
                                           ESM_fromHere, OFFalse))
            .good() &&
        (stack.top()->isLeaf())) {
      OFString macidentifier;
      if ((((DcmElement*)(stack.top()))->getOFString(macidentifier, 0))
              .good()) {
        if (macidentifier == SI_DEFTERMS_RIPEMD160)
          mac = new SiRIPEMD160();
        else if (macidentifier == SI_DEFTERMS_SHA1)
          mac = new SiSHA1();
        else if (macidentifier == SI_DEFTERMS_MD5)
          mac = new SiMD5();
        if (mac == NULL)
          result = SI_EC_VerificationFailed_UnsupportedMACAlgorithm;
      } else
        result = SI_EC_VerificationFailed_NoMAC;
    } else
      result = SI_EC_VerificationFailed_NoMAC;
  ...
  if (result.good()) {
    stack.clear();
    if ((selectedMacParametersItem->search(DCM_DataElementsSigned, stack,
                                           ESM_fromHere, OFFalse))
            .good() &&
        (stack.top()->ident() == EVR_AT)) {
  ...
  if (result.good()) {
    stack.clear();
    if ((selectedSignatureItem->search(DCM_Signature, stack, ESM_fromHere,
                                       OFFalse))
            .good() &&
        (stack.top()->isLeaf())) {
      signature =
          new DcmOtherByteOtherWord(*((DcmOtherByteOtherWord*)(stack.top())));
      if (signature == NULL)
        result = EC_MemoryExhausted;
    } else
      result = SI_EC_VerificationFailed_NoSignature;
  }
  ...
  if (result.good()) {
    ...
    // encode required attributes from the digital signatures sequence
    if (result.good())
      result = constructor.encodeDigitalSignatureItem(*selectedSignatureItem,
                                                      *mac, xfer);
  ...
  if (result.good()) {
    ...
    if (algorithm) {
      ...
      Uint8* sigData = NULL;
      if ((signature->getUint8Array(sigData)).bad() || (sigData == NULL))
        result = SI_EC_VerificationFailed_NoSignature;
      else {
        unsigned long digestLength = mac->getSize();
        unsigned char* digest = new unsigned char[digestLength];
        if (digest == NULL)
          ...
          result = mac->finalize(digest);
          if (result.good()) {
            result = algorithm->verify(digest, digestLength, mac->macType(),
                                       sigData, sigLength, verified);
            if ((result.good()) && (!verified))
              result = SI_EC_VerificationFailed_Corrupted;
          }
          delete[] digest;
        }
      }
...
DcmItem* DcmSignature::findNextSignatureItem(DcmItem& item, DcmStack& stack) {
  if (item.search(DCM_DigitalSignaturesSequence, stack, ESM_afterStackTop,
                  OFTrue)
          .good()) {
    DcmObject* nextItem = stack.elem(1);
    if (nextItem &&
        ((nextItem->ident() == EVR_item) || (nextItem->ident() == EVR_dataset)))
      return (DcmItem*)nextItem;
  }
  return NULL;
}
...
OFCondition DcmSignature::getCurrentMacXferSyntaxName(OFString& str) {
  str.clear();
  if ((NULL == selectedSignatureItem) || (NULL == selectedMacParametersItem))
    return EC_IllegalCall;
  OFCondition result = EC_Normal;
  DcmStack stack;
...
OFCondition DcmSignature::getCurrentMacName(OFString& str) {
  ...
  // read MAC Algorithm
  if ((selectedMacParametersItem->search(DCM_MACAlgorithm, stack, ESM_fromHere,
                                         OFFalse))
          .good() &&
      (stack.top()->isLeaf())) {
    if ((((DcmElement*)(stack.top()))->getOFString(str, 0)).good())
      result = EC_Normal;
  }
  return result;
}
...
OFCondition DcmSignature::getCurrentSignatureUID(OFString& str) {
  ...
  // read signature UID
  if ((selectedSignatureItem->search(DCM_DigitalSignatureUID, stack,
                                     ESM_fromHere, OFFalse))
          .good() &&
      (stack.top()->isLeaf())) {
    if ((((DcmElement*)(stack.top()))->getOFString(str, 0)).good())
...
OFCondition DcmSignature::getCurrentSignatureDateTime(OFString& str) {
  ...
  if ((selectedSignatureItem->search(DCM_DigitalSignatureDateTime, stack,
                                     ESM_fromHere, OFFalse))
          .good() &&
      (stack.top()->isLeaf())) {
    if ((((DcmElement*)(stack.top()))->getOFString(str, 0)).good())
