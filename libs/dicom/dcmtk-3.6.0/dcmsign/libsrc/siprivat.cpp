 ...
 */
extern "C" int SiPrivateKey_passwordCallback(char* buf, int size, int rwflag,
                                             void* userdata);

int SiPrivateKey_passwordCallback(char* buf, int size, int /* rwflag */,
                                  void* userdata) {
  if (userdata == NULL)
    return -1;
...
SiPrivateKey::SiPrivateKey()
    : privateKeyPasswd(), usePrivateKeyPassword(OFFalse), pkey(NULL) {}
        ...
        if (filetype == X509_FILETYPE_ASN1) {
          ...
          if (usePrivateKeyPassword)
            pkey = PEM_read_bio_PrivateKey(
                in, NULL, SiPrivateKey_passwordCallback, &privateKeyPasswd);
          else
            pkey = PEM_read_bio_PrivateKey(in, NULL, NULL,
                                           NULL);  // read password from console
          if (pkey)
            result = EC_Normal;
        }
      }
