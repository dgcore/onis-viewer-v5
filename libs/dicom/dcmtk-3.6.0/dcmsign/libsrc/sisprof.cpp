...
OFCondition SiSecurityProfile::updateAttributeList(DcmItem& item,
  ...
  for (i = 0; i < card; i++) {
    ...
    if (key.isSignableTag()) {
      if ((attributeRequired(key)) ||
          ((containsTag(tagList, key)) && (!attributeForbidden(key)))) {
  ...
  for (unsigned long i = 0; i < card; i++) {
    ...
    if (key.isSignableTag()) {
      if (containsTag(tagList, key)) {
        if (attributeForbidden(key))
          return OFFalse;  // attribute is signed but forbidden
      } else {
        if (attributeRequired(key))
          return OFFalse;  // attribute is required but unsigned
      }
    } else {
      if (containsTag(tagList, key))
        return OFFalse;  // unsignable tag contained in list
    }
  }
  return OFTrue;
}

OFBool SiSecurityProfile::containsTag(DcmAttributeTag& tagList,
                                      const DcmTagKey& key) {
  unsigned long vm = tagList.getVM();
  DcmTagKey current;
  for (unsigned long i = 0; i < vm; ++i) {
    if ((tagList.getTagVal(current, i)).good() && (key == current))
      return OFTrue;
  }
  return OFFalse;
}
