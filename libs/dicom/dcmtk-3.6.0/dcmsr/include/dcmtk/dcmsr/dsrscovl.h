...
public:
   ...
   *  Graphic type is set to GT_invalid.  Since an empty list of graphic data is
   ...
   *  The value is valid if the graphic type is not GT_invalid and the graphic
   ...
   *  Before setting the value the graphic type and data are checked (see
   *checkData()). If the value is invalid the current value is not replaced and
   *remains unchanged.
   ...
   *  This list contains an ordered set of (columns,rows) pairs that denote
   *positions in an image.  The allowed number of pairs is depending on the
   *graphic type.
...
protected:
   ...
   *  If 'graphicType' is valid the number of entries in the 'graphicDatalist'
   *are checked. A POINT needs exactly 1 value pair (column,row), a MULTIPOINT
   *at least 1?, a POLYLINE at least 1?, a CIRCLE exactly 2 and an ELLIPSE
   *exactly 4.
