...
public:
  ...
  /** constructor
   ** @param  node  pointer tree node used to initialize the cursor (set to this
   *node)
   */
  ...
  /** get current node ID.
   *  The node ID uniquely identifies a content item in the document tree.  Most
   *of the navigation methods above do return this ID too.
   ** @return ID of the current node if valid, 0 otherwise
   */
  size_t getNodeID() const;

  /** get current level.
   *  The level starts with 1 for the root node, then 2 for its child nodes,
   *etc.
   ...
   *  within the first level (e.g. "1"), the second one the position in the
   *second level (e.g. "2"), the third one the position in the third level (e.g.
   *"3"), etc.  A position string of "1.2.3" would, therefore, point to the
   *third child of the second child of the first node.
...
protected:
   ...
   *  Clears the internal position list/stack and sets the position counter
   *to 1.
   ** @param  node  node to which the cursor should be set
   ** @return ID of the new current node if successful, 0 otherwise
   */
 ...
 *  Added flag to iterate() method indicating whether to perform a "deep
 * search".
 *
