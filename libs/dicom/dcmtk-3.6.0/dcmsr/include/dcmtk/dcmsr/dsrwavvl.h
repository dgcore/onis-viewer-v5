 ...
 */
class DSRWaveformReferenceValue : public DSRCompositeReferenceValue {
...
public:
   ...
   *                          (VR=UI, mandatory)
   *  @param  sopInstanceUID  referenced SOP instance UID of the waveform
   ...
   ** @param  referenceValue  waveform reference value to be copied (not checked
   *!)
   ** @return reference to this waveform reference value after 'referenceValue'
   *has been copied
   */
  DSRWaveformReferenceValue& operator=(
      const DSRWaveformReferenceValue& referenceValue);

  /** clear all internal variables.
   *  Since an empty waveform reference is invalid the reference becomes invalid
   * afterwards.
   */
  virtual void clear();
   ...
   *(HemodynamicWaveform Storage,"1.2.3") If the SOP class UID is unknown the
   *UID is printed instead of the related name.
   ...
   *  The waveform reference applies to a channel if the list of referenced
   *waveform channels is empty or the group/channel pair is part of the list.
   ** @param  multiplexGroupNumber  multiplex group number of the referenced
   *channel
   *  @param  channelNumber         channel number of the referenced channel
   ** @return OFTrue if reference applies to the specified channel, OFFalse
   *otherwise
   */
...
protected:
   ...
   *  Currently all waveform SOP classes that are defined in DICOM PS 3.x 2003
   *are allowed.
...
private:
  /// list of referenced waveform channels (associated DICOM VR=US, VM=2-2n,
  /// type 1C)
