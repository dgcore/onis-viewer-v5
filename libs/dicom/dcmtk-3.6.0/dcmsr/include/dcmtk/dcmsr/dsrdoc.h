 ...
 * DICOM Structured Reporting (SR) documents. The list of supported SOP classes
 * is available in file "dsrtypes.h".
 */
class DSRDocument : protected DSRTypes {
public:
  // --- constructors and destructor ---

  /** (default) constructor.
   *  The parameter 'documentType' is optional and has a default value.
   ** @param  documentType  type of the SR document (see
   ...
   *  A new document is originally not finalized but can be finalized using the
   *method finalizeDocument().  This flag is e.g. used to indicate whether the
   *entire document is digitally signed and, therefore, each newly added
   *verifying observer would corrupt all previous signatures.
   ...
   *  @param  flags   optional flag used to customize the output (see
   *DSRTypes::PF_xxx)
   ...
   *  Please note that the current document is also deleted if the reading
   *process fails. If the log stream is set and valid the reason for any error
   *might be obtained from the error/warning output.
   ** @param  dataset  reference to DICOM dataset from which the document should
   *be read
   *  @param  flags    optional flag used to customize the reading process (see
   *DSRTypes::RF_xxx). E.g. RF_readDigitalSignatures indicates whether to read
   *the digital signatures from the dataset or not.  If set the
   *MACParametersSequence and the DigitalSignaturesSequence are read for the
   *general document header (equivilent to top-level content item) and each
   *content item of the document tree. If not removed manually (with
   *'DSRDocumentTree::removeSignatures') the signatures are written back to the
   *dataset when the method 'write' is called. Please note that the two
   *signature sequences for any other sequence (e.g. VerifyingObserver or
   *PredecessorDocuments) are never read.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition read(DcmItem& dataset, const size_t flags = 0);

  /** write current SR document to DICOM dataset.
   *  Please note that the ContentTemplateSequence for the root content item is
   *not written automatically for particular SOP Classes (e.g. Key Object
   *Selection Document). Instead, the template identification has to be set
   *manually for the root CONTAINER (see
   *DSRDocumentTreeNode::setTemplateIdentification()).  This is because the
   *template constraints cannot be checked yet.
   ** @param  dataset      reference to DICOM dataset to which the current
   *document should be written
   *  @param  markedItems  optional stack where pointers to all 'marked' content
   *items (DICOM datasets/items) are added to during the write process. Can be
   *used to digitally sign parts of the document tree.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition write(DcmItem& dataset, DcmStack* markedItems = NULL);

  /** read SR document from XML file.
   *  The format (Schema) of the XML document is expected to conform to the
   *output format of the writeXML() method.  In addition, the document can be
   *validated against an XML Schema by setting the flag XF_validateSchema.
   *  Digital signatures in the XML document are not yet supported.
   *  Please note that the current document is also deleted if the parsing
   *process fails.
   ...
   *  The output format is identical to that of the dsr2xml command line tool.
   *Digital signatures in the XML document are not yet supported.
   ...
   ** @param  stream      output stream to which the HTML/XHTML document is
   *written
   *  @param  flags       optional flag used to customize the output (see
   *DSRTypes::HF_xxx)
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition renderHTML(STD_NAMESPACE ostream& stream, const size_t flags = 0,
  ...
  /** get document completion flag description.
   *  Not applicable to Key Object Selection Documents.
   ** @param  description  reference to character string in which the value
   *should be stored
   ...
   *  A document can be verified more than once.  The verification flag should
   *be VERIFIED if any verifying observer is specified.  The details on the
   *observer can be retrieved using the getVerifyingObserver() methods. Not
   *applicable to Key Object Selection Documents.
   ...
   *  All reference variables are cleared before the information is retrieved,
   *i.e. if an error occurs (return value != EC_Normal) non-empty variables do
   *contain valid (empty) data. Not applicable to Key Object Selection
   *Documents.
   ** @param  idx           index of the verifying observer to be retrieved
   *(starting with 1). Use getNumberOfVerifyingObservers() to get the maximum
   *value.
   *  @param  dateTime      reference to variable where the date and time when
   *this document has been verified should be stored (required)
   *  @param  observerName  reference to variable where the name of the person
   *who has verified this document should be stored (required)
   *  @param  organization  reference to variable where the name of the
   *organization to which the observer belongs should be stored (required)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition getVerifyingObserver(const size_t idx, OFString& dateTime,
   ...
   *  @param  observerName  reference to variable where the name of the person
   *who has verified this document should be stored (required)
   *  @param  observerCode  reference to variable where the observer code should
   *be stored. code identifying the verifying observer (optional, see previous
   *method)
   *  @param  organization  reference to variable where the name of the
   *organization to which the observer belongs should be stored (required)
   ** @return status, EC_Normal if successful, an error code otherwise
   ...
   *  A document can have more than one (direct) predecessor document.  This is
   *e.g. the case when two or more documents have been merged to create it.  The
   *corresponding method createRevisedVersion() automatically adds a reference
   *to the current document. The DICOM standard states: "[The Predecessor
   *Documents Sequence] Shall refer to SR SOP Instances (e.g. prior or
   *provisional reports) whose content has been wholly or partially included in
   *this document with or without modification." and "[...] the use of the
   *  Predecessor Document Sequence allows tracing back to the input SR
   *Document, which in this case is the previous version." Not applicable to Key
   *Object Selection Documents.
   ** @return reference to list object
   */
  DSRSOPInstanceReferenceList& getPredecessorDocuments();

  /** get list of identical documents.
   *  Please note that currently the user is responsible for filling and
   *modifying the content of this list.  However, the list is automatically
   *cleared when a new instance is created (incl. a revised version of the
   *current document).  Possibly, there will be a createDuplicate() method or
   *the like in the future which creates an identical copy of the current
   *document in a new study/series. The DICOM standard states: "If identical
   *copies of an SR Document are to be included in multiple Studies then the
   *entire SR Document shall be duplicated with appropriate changes for
   *inclusion into the different Studies (i.e. Study Instance UID, Series
   *Instance UID, SOP Instance UID, Identical Documents Sequence etc.).  The
   *Identical Documents Sequence Attribute in each SOP Instance shall contain
   *references to all other duplicate SOP Instances."
   ** @return reference to list object
   */
  DSRSOPInstanceReferenceList& getIdenticalDocuments();

  /** get list of referenced SOP instances (Current Requested Procedure
   *Evidence). The DICOM standard states: "The intent of the Current Requested
   *Procedure Evidence Sequence is to reference all evidence created in order to
   *satisfy the current Requested Procedure(s) for this SR Document.  This shall
   *include, but is not limited to, all current evidence referenced in the
   *content tree." and "For a completed SR Document satisfying (i.e., being the
   *final report for) the current Requested Procedure(s), this sequence shall
   *list the full set of Composite SOP Instances created for the current
   *Requested Procedure(s).  For other SOP Instances that include the SR
   *Document General Module, this sequence shall contain at minimum the set of
   *Composite SOP Instances from the current Requested Procedure(s) that are
   *  referenced in the content tree." and "In the context of the Key Object
   *Selection, the current evidence is considered to be only the set of
   *instances referenced within the Key Object Selection."
   ** @return reference to list object
   */
   ...
   *  The DICOM standard states: "The Pertinent Other Evidence Sequence
   *attribute is used to reference all other evidence considered pertinent for
   *this SR Document that is not listed in the Current Requested Procedure
   *Evidence Sequence.  This requires that the same SOP Instance shall not be
   *referenced in both of these Sequences." Not applicable to Key Object
   *Selection Documents.
   ** @return reference to list object
   */
   ...
   *  The Coding Scheme Identification Sequence maps Coding Scheme Designators
   * to an external coding system registration, or to a private or local coding
   * scheme.  See DICOM standard for details.
   *  @return reference to list object
   */
  ...
  /** get modality
   ** @param  value  reference to character string in which the value should be
   *stored
  ...
  /** get software version(s). Please note that only the first component is
   *returned.
   ** @param  value  reference to character string in which the value should be
   *stored
   ** @return character string (might empty)
   ...
   *  The passed string must be a valid DICOM Code String (CS).  The internal
   *enumerated value is set accordingly.
   ...
   *  According to the DICOM standard, the concept of "completeness" is
   *independent of the concept of "preliminary" or "final".  Therefore, this
   *flag can be specified separately.
   ** @param  flag  preliminary flag to be set (use PF_invalid to omit this
   *optional value)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  The description can be removed from the DICOM dataset (type 3) by setting
   *an empty string.  Not applicable to Key Object Selection Documents.
   ** @param  value  explanation of the value set for completion flag (optional,
   *VR=LO)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   * generated.  This is a requirement of the DICOM standard.
   */
  void createNewStudy();

  /** create a new series.
   *  After generating a new series instance UID the method
   * createNewSOPInstance() is called, i.e. also a SOP instance UID is
   * generated.  This is a requirement of the DICOM standard.
   */
  void createNewSeries();

  /** create a new series within a given study.
   *  After generating a new series instance UID within the given study the
   *method createNewSOPInstance() is called, i.e. also a SOP instance UID is
   *generated. This is a requirement of the DICOM standard.
   ...
   *  Generate a new SOP instance UID, set the instance creation date/time and
   * reset the finalized flag (OFFalse). This method is used internally for
   * createNewDocument(), createRevisedVersion() and during object
   * initialization. It could also be used explicitly from the calling
   * application if a new UID should be created (this is the case if the study
   * instance UID or series instance UID has changed as well as any other
   * attribute within the SR Document General Module or SR Document Content
   * Module, see DICOM standard for details). This method also updates the other
   * DICOM header attributes (calling updateAttributes()).
   */
  void createNewSOPInstance();

  /** create a new document.
   *  A new SOP instance is only created if the current document type was
   *valid/supported. Please note that the current document is deleted (cleared).
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  A new SOP instance is only created if the current document type was
   *valid/supported. Please note that the current document is deleted by this
   *method.
   ** @param  documentType  type of the SR document (see
   *DSRTypes::E_DocumentType)
   ** @return status, EC_Normal if successful, an error code otherwise
   ...
   *  A revised version can only be created if the current document is already
   *completed (see completion flag).  If so, a reference to the current document
   *is added to the predecessor documents sequence.  If all revised versions of
   *a SR document are stored (written to datasets/files) it is possible to trace
   *back the full chain of previous versions. A new SOP instance is created and
   *the content date/time are set automatically. Furthermore, the verifying
   *observer and identical documents sequence are deleted, the verification flag
   *is set to UNVERIFIED, the completion flag is set to PARTIAL (i.e. not
   *complete), the completion flag description is deleted, all digital
   *  signatures contained in the document tree are deleted and the finalized
   *flag is reset (OFFalse).  The preliminary flag is not modified by this
   *method. Not applicable to Key Object Selection Documents.
   *  @param clearList clear list of predecessor documents before adding the
   *current document if OFTrue. Append current document to existing list
   *otherwise.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Sets the completion flag to COMPLETE if not already done (fails
   *otherwise). The completion flag description is set to an empty string (i.e.
   *absent in DICOM dataset). Not applicable to Key Object Selection Documents.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Sets the completion flag to COMPLETE if not already done (fails
   *otherwise). The completion flag description can be modified independently
   *from the flag by means of the method setCompletionFlagDescription() - see
   *above. Not applicable to Key Object Selection Documents.
   ...
   *  A document can be verified more than once.  The observer information is
   *added to a sequence stored in the dataset.  The verification flag is
   *automatically set to VERIFIED (if not already done) and the finalized flag
   *is reset (set to OFFalse). Please note that only completed documents (see
   *completion flag) can be verified and that a new SOP instance UID has to be
   *generated (manually) according to the DICOM standard when creating a
   *dataset/file from this document. Not applicable to Key Object Selection
   *Documents.
   ** @param  observerName  name of the person who has verified this document
   *(required, VR=PN)
   *  @param  organization  name of the organization to which the observer
   *belongs (required, VR=LO)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Only required since Sun CC 2.0.1 compiler does not support default
   *parameter values for "complex types" like OFString.  Reports the error
   *message: "Sorry not implemented" :-/
   ** @param  observerName  name of the person who has verified this document
   *(required, VR=PN)
   *  @param  organization  name of the organization to which the observer
   *belongs (required, VR=LO)
   *  @param  dateTime      verification date time (optional). If empty/absent
   ...
   ** @param  observerName  name of the person who has verified this document
   *(required, VR=PN)
   *  @param  observerCode  code identifying the verifying observer (optional,
   *see previous method)
   *  @param  organization  name of the organization to which the observer
   *belongs (required, VR=LO)
   ** @return status, EC_Normal if successful, an error code otherwise
   ...
   *  The list of verifying observers is cleared, the verification flag is set
   * to UNVERIFIED and the finalized flag is reset (set to OFFalse). Normally,
   * there should be no need to call this method.  On the other hand, it is
   * useful to guarantee a consistent state when processing documents which have
   * not been created with this module/toolkit.
   */
  void removeVerification();

  /** finalize the current state of the document.
   *  A new document is originally not finalized but can be finalized using this
   *method. This internal flag is e.g. used to indicate whether the entire
   *document is digitally signed and, therefore, each newly added verifying
   *observer would corrupt all previous signatures. NB: A document needs to be
   *completed first in order to be finalized.  Some of the above document
   *management functions do reset the flag (i.e. set the FinalizedFlag to
   *OFFalse), other methods (e.g. setXXX) do not change the flag though the
   *state of the document is not finalized any more after they have been called.
   *  Not applicable to Key Object Selection Documents since there's no
   *completion flag in this type of SR document.  Please note that this method
   *has nothing to do with the preliminary flag.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
...
protected:
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition readXMLDocumentHeader(DSRXMLDocument& doc, DSRXMLCursor cursor,
                                    const size_t flags);
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition readXMLPatientData(const DSRXMLDocument& doc, DSRXMLCursor cursor,
                                 const size_t flags);
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition readXMLStudyData(const DSRXMLDocument& doc, DSRXMLCursor cursor,
                               const size_t flags);
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition readXMLSeriesData(const DSRXMLDocument& doc, DSRXMLCursor cursor,
                                const size_t flags);
  ...
  OFCondition readXMLInstanceData(const DSRXMLDocument& doc,
                                  DSRXMLCursor cursor, const size_t flags);
   ...
   *  This methods checks whether the dataset contains at least the DICOM
   *attributes SOP class UID and modality.  Any incorrectness regarding these
   *two attributes is reported to the log stream (if valid).  Currently
   *unsupported SOP classes are also reported as an error.
   ...
   *  (e.g. set the modality attribute, generate a new SOP instance UID if
   *required, set date/time, etc.)
   ** @param  updateAll  flag indicating whether all DICOM attributes should be
   *updated or only the completion and verification flag. (set DICOM defined
   *terms from enum values)
   */
