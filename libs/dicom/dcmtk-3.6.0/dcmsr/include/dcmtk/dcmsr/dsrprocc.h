...
/** Class for checking the content relationship constraints of the Procedure Log
 * IOD
 */
class DSRProcedureLogConstraintChecker : public DSRIODConstraintChecker {
public:
  /** default constructor
   */
   ...
   ** @return always returns OFFalse, i.e. by-reference relationships are not
   *allowed
   */
   ...
   ** @param  sourceValueType   value type of the source content item to be
   *checked
   *  @param  relationshipType  type of relationship between source and target
   *item
   *  @param  targetValueType   value type of the target content item to be
   *checked
   *  @param  byReference       optional flag indicating whether the
   *node/relationship should be added by-value (default) or by-reference
   ** @return OFTrue if content relationship is allowed, OFFalse otherwise
   */
  virtual OFBool checkContentRelationship(
