 ...
 */
class DSRDateTimeTreeNode : public DSRDocumentTreeNode, public DSRStringValue {
public:
  /** constructor
   ...
   *  The content item is valid if the two base classes and the concept name are
   *valid.
  ...
  /** print content item.
   *  A typical output looks like this: contains
   *DATETIME:(,,"Code")="2000101012000000"
   ...
   *  The DICOM DateTime (DT) value is expected to be stored in ISO format as
   *created by writeXML().
   ** @param  doc            document containing the XML file content
   *  @param  cursor         cursor pointing to the corresponding node
   *  @param  dateTimeValue  reference to string object in which the value
   *should be stored
   *  @param  clearString    flag specifying whether to clear the
   *'dateTimeValue' or not
   ** @return reference to string object (might be empty)
   */
  static OFString& getValueFromXMLNodeContent(
      const DSRXMLDocument& doc, DSRXMLCursor cursor, OFString& dateTimeValue,
