 ...
 */
class DSRCodingSchemeIdentificationList : public DSRTypes {
public:
   ...
   *  Please note that any information previously stored under the defined
   *coding scheme designator "99_OFFIS_DCMTK" is replaced!
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition addPrivateDcmtkCodingScheme();

  /** add the specified coding scheme to the list.
   *  Internally, the item is inserted into the list of coding scheme
   *designators if not already contained in the list.  In any case the specified
   *item is selected as the current one.  See definition of 'ItemStruct' above
   *for VR, VM and Type.
   ** @param  codingSchemeDesignator  coding scheme designator of the item to be
   *added. Designators beginning with "99" and the designator "L" are defined to
   *be private or local coding schemes.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Designators beginning with "99" and the designator "L" are defined to be
   *private or local coding schemes.
...
private:
  ...
  DSRCodingSchemeIdentificationList& operator=(
      const DSRCodingSchemeIdentificationList&);
};
