...
public:
   ...
   *  @param  measurementUnit  code representing the measurement name (code
   *meaning) and unit (code value). (type 2)
   */
  DSRNumericMeasurementValue(const OFString& numericValue,
                             const DSRCodedEntryValue& measurementUnit);

  /** constructor.
   *  The two codes are only set if they passed the validity check (see
   *setValue()).
   ** @param  numericValue     numeric measurement value. (VR=DS, type 1)
   *  @param  measurementUnit  code representing the measurement name (code
   *meaning) and unit (code value). (type 2)
   *  @param  valueQualifier   code representing the numeric value qualifier.
   *(type 3)
   */
  DSRNumericMeasurementValue(const OFString& numericValue,
                             const DSRCodedEntryValue& measurementUnit,
  ...
  /** copy constructor
   ** @param  numericMeasurement  numeric measurement value to be copied (not
   *checked !)
   */
  DSRNumericMeasurementValue(
   ...
   ** @param  numericMeasurement  numeric measurement value to be copied (not
   *checked !)
   ** @return reference to this numeric value after 'numericMeasurement' has
   *been copied
   */
  DSRNumericMeasurementValue& operator=(
      const DSRNumericMeasurementValue& numericMeasurement);
   ...
   *  The value is valid if isEmpty() is true or all three values (numeric
   *value, measurement unit and value qualifier) do contain valid values (see
   *checkXXX() methods).
   ...
   *"empty" is printed instead.  The numeric value qualifier is never printed.
   ** @param  stream  output stream to which the numeric measurement value
   *should be printed
  ...
  /** read measured value sequence and numeric value qualifier code sequence
   *from dataset. The number of items within the sequences is checked.  If
   *error/warning output are enabled a warning message is printed if a sequence
   *is absent or contains more than one item.
   ** @param  dataset    DICOM dataset from which the sequences should be read
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  virtual OFCondition readSequence(DcmItem& dataset);

  /** write measured value sequence and numeric value qualifier code sequence to
   *dataset. The measured value sequence is always written (might be empty,
   *though).  The numeric value qualifier code sequence is optional and,
   *therefore, only written if non-empty.
   ...
   ** @param  numericMeasurement  reference to variable in which the value
   *should be stored
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   ** @return reference to current measurement unit code (might be invalid or
   *empty)
   */
   ...
   ** @return reference to current numeric value qualifier code (might be
   *invalid or empty)
   */
   ...
   ** @param  measurementUnit  reference to variable in which the code should be
   *stored
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Before setting the value it is checked (see checkXXX()).  If the value is
   ...
   *  Before setting the values they are checked (see checkXXX()).  If the value
   *pair is invalid the current value pair is not replaced and remains
   *unchanged.
   ...
   *  Before setting the values they are checked (see checkXXX()).  If one of
   *the three values is invalid the current numeric measurement value is not
  ...
  /** set numeric value.
   *  Before setting the value it is checked (see checkNumericValue()).  If the
   *value is invalid the current value is not replaced and remains unchanged.
   ...
   *  Before setting the code it is checked (see checkMeasurementUnit()).  If
   *the code is invalid the current code is not replaced and remains unchanged.
   ** @param  measurementUnit  measurement unit to be set
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  This optional code specifies the qualification of the Numeric Value in the
   *Measured Value Sequence, or the reason for the absence of the Measured Value
   *Sequence Item. Before setting the code it is checked (see
   *checkNumericValueQualifier()).  If the code is invalid the current code is
   *not replaced and remains unchanged.
   ** @param  valueQualifier  numeric value qualifier to be set
   ** @return status, EC_Normal if successful, an error code otherwise
   */
...
protected:
   ...
   *  Currently the only check that is performed is that the string is not
   *empty.  Later on it might be checked whether the format conforms to the
   *definition of DS.
   ...
   */
  virtual OFBool checkMeasurementUnit(
      const DSRCodedEntryValue& measurementUnit) const;

  /** check the specified numeric value qualifier for validity.
   *  The isEmpty() and isValid() methods in class DSRCodedEntryValue are used
   *for this purpose.  The conformance with the Context Group 42 (as defined in
   *the DICOM standard) is not yet checked.
   ** @param  valueQualifier  numeric value qualifier to be checked
   ** @return OFTrue if value qualifier is valid, OFFalse otherwise
   */
  virtual OFBool checkNumericValueQualifier(
      const DSRCodedEntryValue& valueQualifier) const;
