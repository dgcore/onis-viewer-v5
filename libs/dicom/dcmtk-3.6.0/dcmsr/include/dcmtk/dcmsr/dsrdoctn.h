 ...
 */
class DSRDocumentTreeNode : public DSRTreeNode {
  // allow access to getConceptNamePtr()
  friend class DSRContentItem;

public:
  /** constructor.
   *  The 'relationshipType' and 'valueType' can never be changed after the tree
   *node has been created (therefore, the corresponding member variables are
   *declared "const").
   ** @param  relationshipType  type of relationship to the parent tree node.
   *                            Should not be RT_invalid and RT_isRoot only for
   *the root node.
   ...
   *  This does not apply to the relationship and value type since they are
   * never changed.
   */
  virtual void clear();

  /** check whether the content item is valid.
   *  The content item is valid if the relationship type and the value type are
   *both not invalid.
   ...
   *  This method is used to check whether the rendered output of this content
   *item can be expanded inline or not (used for renderHTML()).  This base class
   *always returns OFTrue.
  ...
  /** print content item.
   *  The output of a content item depends on its value type.  This general
   *method prints only those parts which all derived classes (= value types) do
   *have in common, i.e. the type of relationship, the value type and the
   *(optional) concept name. A typical output looks like this: has concept mod
   *CODE: (,,"Concept")
   ...
   *  A number of readXXX() methods are called (see "protected" part) in order
   *to retrieve all possibly nested content items from the dataset.
   ** @param  dataset            DICOM dataset from which the content item
   *should be read
   *  @param  constraintChecker  checks relationship content constraints of the
   *associated IOD
   *  @param  flags              flag used to customize the reading process (see
   *DSRTypes::RF_xxx)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  A number of writeXXX() methods are called (see "protected" part) in order
   *to write all possibly nested content items to the dataset.
   ** @param  dataset      DICOM dataset to which the content item should be
   *written
   ...
   *  @param  documentType  type of the document to be read (used for debug
   *output only)
   ...
   *  @param  flags      flag used to customize the output (see
   *DSRTypes::XF_xxx)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  After rendering the current content item all child nodes (if any) are also
   *rendered (see renderHTMLChildNodes() for details).
   ** @param  docStream     output stream to which the main HTML/XHTML document
   *is written
   *  @param  annexStream   output stream to which the HTML/XHTML document annex
   *is written
   *  @param  nestingLevel  current nesting level.  Used to render section
   *headings.
   *  @param  annexNumber   reference to the variable where the current annex
   *number is stored. Value is increased automatically by 1 after a new entry
   *has been added.
   *  @param  flags         flag used to customize the output (see
   *DSRTypes::HF_xxx)
   ** @return status, EC_Normal if successful, an error code otherwise
  ...
  virtual OFCondition renderHTML(STD_NAMESPACE ostream& docStream,
                                 ...
                                 const size_t nestingLevel, size_t& annexNumber,
                                 const size_t flags) const;

  /** check whether content item is digitally signed.
   *  A content item is signed if the DigitalSignaturesSequence exists.  This
   *sequence is read from the dataset if present and the 'signature' flag for
   *the 'read' method is turned on.
   ...
   *  Pointers to the DICOM dataset/item of marked content items are added to
   *the optional stack when calling the 'write' method.  This mechanism can e.g.
   *be used to digitally sign particular content items.
  ...
  /** check whether the current content item is target of a by-reference
   *relationship
  ...
  /** specify whether the current content item is target of a by-reference
   *relationship
   ** @param  isTarget  OFTrue if the content item is target (default), OFFalse
   *otherwise
   */
   ...
   ** @return relationship type of the current content item (might be
   *RT_invalid)
   */
   ...
   *  Code describing the concept represented by this content item.  Also
   *conveys the value of document title and section headings in documents.
   ** @param  conceptName  reference to a variable where the code should be
   *stored
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Code describing the concept represented by this content item.  Also
   *conveys the value of document title and section headings in documents. If
   *the new code is invalid the current one is not replaced.  An empty code can
   *  be used to clear the current concept name.
   ** @param  conceptName  code to be set as the new concept name (checked
   *before set)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  This is the date and time on which this content item was completed.  Might
   *be empty if the date and time do not differ from the content date and time,
   *see DSRDocument.
   ** @return observation date and time of current content item (might be
   *empty/invalid)
   */
   ...
   *  This is the date and time on which this content item was completed.  Might
   *be empty if the date and time do not differ from the content date and time,
   *see DSRDocument. Please use the correct DICOM format (YYYYMMDDHHMMSS) or an
   *empty string to clear the current value.  Currently no check is performed on
   *the parameter value!
   ...
   *  This value pair identifies the template that was used to create this
   *content item (and its children).  According to the DICOM standard it is
   *"required if a template was used to define the content of this Item, and the
   *template consists of a single CONTAINER with nested content, and it is the
   *outermost invocation of a set of nested templates that start with the same
   *CONTAINER."  However, this condition is currently not checked.  The
   *identification is valid if both values are either present (non-empty) or
   *absent (empty).
   ...
   *  The identification is valid if both values are either present (non-empty)
   *or absent (empty).  See getTemplateIdentification() for details.
   ...
   *  This method clears the MACParametersSequence and the
   * DigitalSignaturesSequence for the current content item which have been
   * filled during reading.
   */
...
protected:
   ...
   ** @param  previousNode       reference to the pointer to the previous node
   *(sibling). Used to decide whether the new node is a child (value=NULL) or a
   *sibling (!=NULL).  NB: The value might be modified inside this method (to
   *store a reference to the previous node).
   *  @param  relationshipType   relationship type of the new node with regard
   *to the current one
   ...
   *  This method does nothing for this base class, but derived classes
   *overwrite it to read the contents according to their value type.
   ** @param  dataset    DICOM dataset from which the content item should be
   *read
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  This method does nothing for this base class, but derived classes
   *overwrite it to write the contents according to their value type.
   ** @param  dataset    DICOM dataset to which the content item should be
   *written
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  This method does nothing for this base class, but derived classes
   *overwrite it to render the contents according to their value type.
   ** @param  docStream     output stream to which the main HTML/XHTML document
   *is written
   *  @param  annexStream   output stream to which the HTML/XHTML document annex
   *is written
   ...
   *  @param  flags           flag used to customize the output (see
   *DSRTypes::XF_xxx)
   *  @param  closingBracket  write closing bracket of XML start tag if OFTrue,
   *otherwise the bracket has to be closed in the calling method
   */
  void writeXMLItemStart(STD_NAMESPACE ostream& stream, const size_t flags,
   ...
   ** @param  dataset            DICOM dataset from which the data should be
   *read
   *  @param  constraintChecker  checks relationship content constraints of the
   *associated IOD
   *  @param  flags              flag used to customize the reading process (see
   ...
   *  @param  markedItems  optional stack where pointers to all 'marked' content
   *items (DICOM datasets/items) are added to during the write process.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  @param  constraintChecker  checks relationship content constraints of the
   *associated IOD
   *  @param  posString          location of the current content item (e.g.
   *"1.2.3")
   *  @param  flags              flag used to customize the reading process (see
   *DSRTypes::RF_xxx)
   ** @return status, EC_Normal if successful, an error code otherwise
   ...
   ** @param  dataset       DICOM dataset to which the data should be written
   *  @param  markedItems   optional stack where pointers to all 'marked'
   *content items (DICOM datasets/items) are added to during the write process.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  If the optional observation datetime field is valid (not empty) it is also
   *rendered.
   ** @param  docStream    output stream to which the main HTML/XHTML document
   *is written
   *  @param  flags        flag used to customize the output (see
   ...
   *  @param  relationshipText  reference to string variable where the resulting
   *text should be stored.  Value is cleared if 'relationshipType' is invalid or
   *not supported.
   *  @param  flags             flag used to customize the output (see
   *DSRTypes::HF_xxx)
   ** @return reference to the 'relationshipText' string (might be empty)
   */
  static const OFString& getRelationshipText(
      const E_RelationshipType relationshipType, OFString& relationshipText,
      const size_t flags);

private:
  /// flag indicating whether the content item is marked (e.g. used for digital
  /// signatures)
  OFBool MarkFlag;
  /// flag indicating whether the content item is referenced (by-reference
  /// relationship)
