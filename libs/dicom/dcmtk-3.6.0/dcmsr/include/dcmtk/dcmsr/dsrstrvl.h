...
public:
  ...
  /** constructor
   *  The string value is only set if it passed the validity check (see
   *setValue()).
  ...
  /** print string value.
   *  The output of a typical string value looks like this: "Short text" or
   *"Very long t..." (incl. the quotation marks).
   ** @param  stream     output stream to which the string value should be
   *printed
   *  @param  maxLength  maximum number of characters to be printed.  If the
   *string value is longer the output is shortened automatically and three dots
   *"..." are added.  The value of 'maxLength' includes these three trailing
   *char's. A value of 0 turns this meachanism off (default), i.e. the full
   ...
   *  If error/warning output is enabled a warning message is printed if the
   *string value does not conform with the type (= 1) and value multiplicity (=
   *1).
   ** @param  dataset    DICOM dataset from which the string value should be
   *read
   ...
   ** @param  dataset    DICOM dataset to which the string value should be
   *written
   ...
   ** @param  docStream  output stream to which the main HTML/XHTML document is
   *written
  ...
  /** set string value.
   *  Before setting the string value it is checked (see checkValue()).  If the
   *value is invalid the current value is not replaced and remains unchanged.
   *Use clear() to empty the string value (which becomes invalid afterwards).
...
protected:
   ...
   *  This base class just checks that the string value is not empty (since all
   *corresponding DICOM attributes are type 1).  Derived classes might overwrite
   *this method to perform more sophisticated tests.
