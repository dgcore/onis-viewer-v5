...
public:
  ...
  /** constructor
   ** @param  temporalRangeType  type of temporal extent of the region of
   *interest
   */
  DSRTemporalCoordinatesValue(
  ...
  /** copy constructor
   ** @param  coordinatesValue  temporal coordinates value to be copied (not
   *checked !)
   */
  DSRTemporalCoordinatesValue(
   ...
   ** @param  coordinatesValue  Temporal coordinates value to be copied (not
   *checked !)
   ** @return reference to this Temporal coordinates value after
   *'coordinatesValue' has been copied
   */
  DSRTemporalCoordinatesValue& operator=(
      const DSRTemporalCoordinatesValue& coordinatesValue);

  /** clear all internal variables.
   *  Temporal range type is set to TRT_invalid.  Since an empty list of graphic
   * data is invalid the spatial coordinates value becomes invalid afterwards.
   */
  virtual void clear();

  /** check whether the current spatial coordinates value is valid.
   *  The value is valid if the temporal range type is not TRT_invalid and the
   *other data is valid (see checkData() for details).
   ...
   *  The output of a typical temporal coordinates value looks like this
   *(depending on the referenced data list): (SEGMENT,1,2,3) or
   *(SEGMENT,1,2.5,3.1) or (POINT,20001010120000)
   ** @param  stream  output stream to which the temporal coordinates value
   *should be printed
   ...
   ** @param  coordinatesValue  reference to variable in which the value should
   *be stored
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Before setting the value the temporal range type and other data are
   *checked (see checkData()).  If the value is invalid the current value is not
   ...
   *  This value represents the type of temporal extent of the region of
   *interest.
   ...
   *  This value represents the type of temporal extent of the region of
   *interest.
   ** @param  temporalRangeType  temporal range type to be set (TRT_invalid is
   *not allowed)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
...
protected:
   ...
   *  Please note that all three lists are (tried to) read from the dataset.  If
   *more than one list is present a warning messsage is reported.
   ...
   *  Please note that only one of the three lists is actually written to the
   *dataset.
   ...
   *  The data is valid if the 'temporalRangeType' is valid and at least one of
   *the three lists are non-empty.  If more the one list is non-empty a warning
   *is reported since they are mutually exclusive (type 1C).
   ** @param  temporalRangeType   temporal range type to be checked
   *  @param  samplePositionList  list of referenced sample positions to be
   *checked
...
private:
  ...
  /// list of referenced sample positions (associated DICOM VR=UL, VM=1-n, type
  /// 1C)
