 ...
 */
class DSRDocumentTree : public DSRTree {
public:
  /** constructor
   ...
   *  The SR document is valid if the document type is supported, the tree is
   *not empty the root item is a container and has the internal relationship
   *type RT_isRoot.
   ...
   *  Please note that the current document tree is also deleted if the reading
   *fails. If the log stream is set and valid the reason for any error might be
   *obtained from the error/warning output.
   ** @param  dataset       reference to DICOM dataset where the tree should be
   *read from
   *  @param  documentType  document type of the SR document from which the tree
   *is read
   *  @param  flags         flag used to customize the reading process (see
   *DSRTypes::RF_xxx)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition read(DcmItem& dataset, const E_DocumentType documentType,
                   const size_t flags = 0);

  /** write current SR document tree to DICOM dataset
   ** @param  dataset      reference to DICOM dataset where the current tree
   ...
   *  Please note that the document tree is deleted if the specified
   *'documentType' is supported.  Otherwise the current document remains in
   *force.
   ** @param  documentType  new document type to be set (should be !=
   *DT_invalid)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  If the tree is currently empty only a CONTAINER with the internal
   *relationship type RT_isRoot is allowed (as the new root node).  Always
   *returns true if no constraint checker is available.  This method can be used
   *to decide which type of content items can be added prior to really do so.
   ** @param  relationshipType  relationship type of node to be checked with
   *regard to the current one
   *  @param  valueType         value type of node to be checked
   *  @param  addMode           flag specifying at which position the new node
   *would be added. (AM_afterCurrent, AM_beforeCurrent, AM_belowCurrent)
  ...
  /** check whether specified by-reference relationship can be added to the
   *current content item.  Always returns true if no constraint checker is
   *available.
   ** @param  relationshipType  type of relationship between current and target
   *node
   *  @param  targetValueType   value type of the referenced target node
   ** @return OFTrue if specified by-reference relationship can be added,
   *OFFalse otherwise
   */
  OFBool canAddByReferenceRelationship(
      const E_RelationshipType relationshipType,
      const E_ValueType targetValueType);

  /** add specified content item to the current one.
   *  If possible this method creates a new node as specified and adds it to the
   *current one.  The method canAddContentItem() is called internally to check
   *parameters first.
   ** @param  relationshipType  relationship type of node to be added with
   *regard to the current one
   *  @param  valueType         value type of node to be added
   *  @param  addMode           flag specifying at which position to add the new
   *node. (AM_afterCurrent, AM_beforeCurrent, AM_belowCurrent)
   ...
   *  If possible this method creates a new pseudo-node (relationship) and adds
   *it to the current one.  The method canAddByReferenceRelationship() is called
   *internally to check parameters first.  The internal cursor is automatically
   *re-set to the current node.
   ** @param  relationshipType  relationship type between current and referenced
   *node
   ...
   *  Please note that not only the specified node but also all of its child
   *nodes are removed from the tree and then deleted.  The internal cursor is
   *set automatically to a new valid position.
   ** @return ID of the node which became the current one after deletion, 0 if
   *an error occured or the tree is now empty.
   */
   ...
   *  If more than one node exists with the given concept name the first one
   *will be selected.  Use gotoNextNamedNode() in order to go to the next
   *matching node.
   ...
   *  Starts from "next" node, i.e. either the first children of the current
   *node or the first sibling following the current node.
   ** @param  conceptName    concept name of the node to be searched for
   *  @param  searchIntoSub  flag indicating whether to search into sub-trees
   *                         ("deep search") or on the current level only
   ** @return ID of the new current node if successful, 0 otherwise
   ...
   *  Use method 'setMark' on node-level to mark and unmark a single content
   * item. Pointers to the DICOM dataset/item of marked content items are added
   * to the optional stack when calling the 'write' method.  This mechanism can
   * e.g. be used to digitally sign particular content items.
   */
   ...
   * DigitalSignaturesSequence for all content items which have been filled
   * during reading.
   */
...
protected:
   ...
   *  Please note that no copy of the given node is created.  Therefore, the
   *node should be created with new() - do not use a reference to a local
   *variable. If the node could be added successfully the cursor is set to it
   *automatically.
   ...
   *  Please note that not only the specified node but also all of his child
   *nodes are removed from the tree and deleted afterwards.  The cursor is set
   *automatically to a new valid position.
   ** @return ID of the node which became the current one after deletion, 0 if
   *an error occured or the tree is now empty.
   */
   ...
   *  This function checks whether all by-reference relationships possibly
   *contained in the document tree are valid according to the following
   *restrictions: source and target node are not identical and the target node
   *is not an ancestor of the source node (requirement from the DICOM standard
   *to prevent loops -> directed acyclic graph, though this is not 100% true -
   *see "reportlp.dcm" example). In addition, the position strings (used to
   *encode by-reference relationships according to the DICOM standard) OR the
   *node IDs (used internally to uniquely identify nodes) can be updated. Please
   *note that the modes 'CM_updatePositionString' and 'CM_updateNodeID' are
   *mutually exclusive.
   ** @param  mode   mode used to customize the checking process (see
   *DSRTypes::CM_xxx)
   *  @param  flags  flag used to customize the reading process (see
   *DSRTypes::RF_xxx)
   ** @return status, EC_Normal if successful, an error code otherwise
   ...
   * relationship. This function calls 'setReferenceTarget(OFFalse)' for all
   * content items.
   */
...
private:
  /** add new node to the current one.
   *  This method just overwrites the method from the base class DSRTree.  Use
   *the above addNode() method instead.
