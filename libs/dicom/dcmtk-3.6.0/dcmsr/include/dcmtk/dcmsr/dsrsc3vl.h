...
public:
  ...
  /** constructor
   ** @param  graphicType  graphic type specifying the geometry of the
   *coordinates
   */
  DSRSpatialCoordinates3DValue(const DSRTypes::E_GraphicType3D graphicType);

  /** copy constructor
   ** @param  coordinatesValue  spatial coordinates value to be copied (not
   *checked !)
   */
  DSRSpatialCoordinates3DValue(
      const DSRSpatialCoordinates3DValue& coordinatesValue);
   ...
   ** @param  coordinatesValue  spatial coordinates value to be copied (not
   *checked !)
   ** @return reference to this spatial coordinates value after
   *'coordinatesValue' has been copied
   */
  DSRSpatialCoordinates3DValue& operator=(
      const DSRSpatialCoordinates3DValue& coordinatesValue);

  /** clear all internal variables.
   *  Graphic type is set to GT3_invalid.  Since an empty list of graphic data
   * is invalid the spatial coordinates value becomes invalid afterwards.
   */
  virtual void clear();

  /** check whether the current spatial coordinates value is valid.
   *  The value is valid if the graphic type is not GT3_invalid and the graphic
   *data is valid (see checkData() for details).
   ...
   *  The output of a typical spatial coordinates value looks like this:
   *(POINT,,100/100/100).
   ** @param  stream  output stream to which the spatial coordinates value
   *should be printed
   ...
   *  The graphic type specifies the geometry of the coordinates stored in the
   *graphic data list.
   ...
   *  Before setting the value the graphic type, graphic data and frame of
   *reference UID are checked (see checkData()).  If the value is invalid the
   ...
   *  This list contains an ordered set of (x,y,z) triplets that denote
   *positions in an image.  The allowed number of triplets is depending on the
   *graphic type.
...
protected:
  ...
  /** check the graphic type, graphic data and frame of reference UID for
   *validity. If 'graphicType' is valid the number of entries in the
   *'graphicDatalist' are checked. A POINT needs exactly 1 value triplets
   *(x,y,z), a MULTIPOINT at least 1?, a POLYLINE at least 1?, a POLYGON at
   *least 1? where the first and last triplet are equal, an ELLIPSE exactly 4
   *and an ELLIPSOID exactly 6.
   ** @param  graphicType          graphic type to be checked
   *  @param  graphicDataList      list of graphic data to be checked
   *  @param  frameOfReferenceUID  referenced frame of reference UID to be
   *checked
