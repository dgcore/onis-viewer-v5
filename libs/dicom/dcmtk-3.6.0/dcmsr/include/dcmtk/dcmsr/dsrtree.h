 ...
 */
class DSRTreeNode : protected DSRTypes {
 ...
 */
class DSRTree : public DSRTreeNodeCursor, protected DSRTypes {
public:
  /** default constructor
   */
  DSRTree();
   ...
   *                         (the format is e.g. "1.2.3" for the third child of
   *the second child of the first node - see DSRTreeNodeCursor).
   *  @param  startFromRoot  flag indicating whether to start from the root node
   *                         or the current one
   ** @return ID of the new current node if successful, 0 otherwise
   */
   ...
   *nodes are removed from the tree and then deleted.  The cursor is set
 ...
 *  Changed order of inheritance to avoid internal compiler error on Borland
 * Builder.
 *
