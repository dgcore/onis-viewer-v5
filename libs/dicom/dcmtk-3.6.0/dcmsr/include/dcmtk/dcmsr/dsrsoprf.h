 ...
 */
class DSRSOPInstanceReferenceList : public DSRTypes {
public:
  ...
  struct StudyStruct {
    ...
    /** select the next item in the list.
     *  That means the next instance in the current series, or the first
     *instance in the next series (if available).
     ** @return status, EC_Normal if successful, an error code otherwise
     */
    OFCondition gotoNextItem();

    /** add new entry to the list of series and instances (if not already
     *existent). Finally, the specified items are selected as the current one.
     ** @param  seriesUID    series instance UID of the entry to be added
     *  @param  sopClassUID  SOP class UID of the entry to be added
     *  @param  instanceUID  SOP instance UID of the entry to be added
     ** @return status, EC_Normal if successful, an error code otherwise
     */
    OFCondition addItem(const OFString& seriesUID, const OFString& sopClassUID,
                        const OFString& instanceUID);
  ...
  /** constructor
   ** @param  sequence  DICOM tag specifying the attribute (sequence) of the
   *reference list
   */
   ...
   *  The hierarchical structure is automatically reorganized in a way that each
   *study, each series (within a study) and each instance (within a series) only
   *exist once, i.e. the structure might look different when written back to a
   *dataset.  However, the content is identical and this way of storing
   *information saves storage space.
   ...
   *  Internally the item is inserted into the hierarchical structure of
   *studies, series and instances, if not already contained in the list. In any
   *case the specified item is selected as the current one.
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition addItem(const OFString& studyUID, const OFString& seriesUID,
                      const OFString& sopClassUID, const OFString& instanceUID);

  /** add item from specified DICOM dataset to the list of references.
   *  Internally an item representing the given dataset is inserted into the
   *hierarchical structure of studies, series and instances, if not already
   *contained in the list. In any case the specified item is selected as the
   *current one.
   ** @param  dataset  reference to DICOM dataset from which the relevant UIDs
   *are retrieved
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition removeItem(const OFString& studyUID, const OFString& seriesUID,
                         const OFString& instanceUID);
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition gotoItem(const OFString& studyUID, const OFString& seriesUID,
                       const OFString& instanceUID);
   ...
   ** @param  stringValue  reference to string variable in which the result is
   *stored
  ...
  /** get the retrieve application entity title of the currently selected entry
   *(optional). The resulting string may contain multiple values separated by a
   *backslash ("\").
   ** @param  stringValue  reference to string variable in which the result is
   *stored
   ** @return reference to the resulting string (might be empty)
  ...
  /** get the storage media file set ID of the currently selected entry
   *(optional)
   ** @param  stringValue  reference to string variable in which the result is
   *stored
   ** @return reference to the resulting string (might be empty)
  ...
  /** get the storage media file set UID of the currently selected entry
   *(optional)
   ** @param  stringValue  reference to string variable in which the result is
   *stored
   ** @return reference to the resulting string (might be empty)
   ...
   ** @param  value  string value to be set (use empty string to omit optional
   *attribute)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
