...
#define OFFIS_RESPONSIBLE_ORGANIZATION \
  "OFFIS e.V., Escherweg 2, 26121 Oldenburg, Germany"

// protocol, hostname and CGI script name used for HTML hyperlinks to composite
// objects
...
/// error: the by-reference relationship between two content items is not
/// allowed
 ...
 *  All functions and constants are static and can, therefore, be accessed
...
public:
  ...
  /// do not abort on content item errors (e.g. missing value type specific
  /// attributes)
  static const size_t RF_ignoreContentItemErrors;

  /// do not abort when detecting an invalid content item, skip invalid sub-tree
  /// instead
  ...
  /// external: use code meaning for the numeric measurement unit (default: code
  /// value)
  static const size_t HF_useCodeMeaningAsUnit;

  /// external: use patient information as document title (default: document
  /// type)
  ...
  /// write: write template identification information (TID and mapping
  /// resource)
  ...
  /// write: encode code value, coding scheme designator and coding scheme
  /// version as attribute instead of element text
   ...
   *  These flags can be combined and passed to the
   * checkByReferenceRelationships() method.
   */
  //@{
  ...
  enum E_RelationshipType {
    /// internal type used to indicate an error
    RT_invalid,
    /// internal type used to indicate an unknown relationship type (defined
    /// term)
    RT_unknown,
   ...
   */
  static const char* documentTypeToSOPClassUID(
   ...
   */
  static const char* documentTypeToReadableName(
      const E_DocumentType documentType);

  /** convert SR document type to document title.
   *  This document title is used for printing/rendering.
   ** @param  documentType   SR document type to be converted
   *  @param  documentTitle  reference to variable where the resulting string is
   *stored
   ** @return document title if successful, empty string otherwise (never NULL)
   */
  static const char* documentTypeToDocumentTitle(
      const E_DocumentType documentType, OFString& documentTitle);
   ...
   ** @return OFTrue if Enhanced General Equipment Module is required, OFFalse
   *otherwise
   */
  static OFBool requiresEnhancedEquipmentModule(
      const E_DocumentType documentType);
   ...
   */
  static const char* relationshipTypeToDefinedTerm(
   ...
   ** @return readable name if successful, empty string otherwise (never NULL)
   */
  static const char* relationshipTypeToReadableName(
      const E_RelationshipType relationshipType);
   ...
   ** @return enumerated value if successful, empty string otherwise (never
   *NULL)
   */
  static const char* graphicTypeToEnumeratedValue(
   ...
   *NULL)
   */
  static const char* graphicType3DToEnumeratedValue(
      const E_GraphicType3D graphicType);
   ...
   ** @return readable name if successful, empty string otherwise (never NULL)
   */
  static const char* graphicType3DToReadableName(
      const E_GraphicType3D graphicType);
   ...
   *NULL)
   */
  static const char* temporalRangeTypeToEnumeratedValue(
   ...
   ** @return readable name if successful, empty string otherwise (never NULL)
   */
  static const char* temporalRangeTypeToReadableName(
      const E_TemporalRangeType temporalRangeType);
   ...
   *NULL)
   */
  static const char* continuityOfContentToEnumeratedValue(
   ...
   *NULL)
   */
  static const char* preliminaryFlagToEnumeratedValue(
      const E_PreliminaryFlag preliminaryFlag);
   ...
   *NULL)
   */
  static const char* completionFlagToEnumeratedValue(
   ...
   *NULL)
   */
  static const char* verificationFlagToEnumeratedValue(
   ...
   ** @return defined term if successful, empty string otherwise (never NULL)
   */
  static const char* characterSetToDefinedTerm(
      const E_CharacterSet characterSet);

  /** convert character set to HTML name.
   *  This HTML (IANA) name is used to specify the character set in an HTML
   *document.
   ** @param  characterSet  character set to be converted
   ** @return HTML name if successful, empty string or "?" otherwise (never
   *NULL)
   */
   ...
   */
  static E_RelationshipType definedTermToRelationshipType(
   ...
   */
  static E_GraphicType enumeratedValueToGraphicType(
   ...
   */
  static E_GraphicType3D enumeratedValueToGraphicType3D(
      const OFString& enumeratedValue);
   ...
   */
  static E_TemporalRangeType enumeratedValueToTemporalRangeType(
      const OFString& enumeratedValue);

  /** convert DICOM enumerated value to continuity of content flag
   ** @param  enumeratedValue  enumerated value to be converted
   ** @return continuity of content flag type if successful, COC_invalid
   *otherwise
   */
  static E_ContinuityOfContent enumeratedValueToContinuityOfContent(
      const OFString& enumeratedValue);
   ...
   */
  static E_PreliminaryFlag enumeratedValueToPreliminaryFlag(
      const OFString& enumeratedValue);
   ...
   */
  static E_CompletionFlag enumeratedValueToCompletionFlag(
      const OFString& enumeratedValue);
   ...
   */
  static E_VerificationFlag enumeratedValueToVerificationFlag(
      const OFString& enumeratedValue);
   ...
   *  Currently all three general SOP classes, the Key Object Selection
   *Document, the Mammography and Chest CAD SR class as defined in the DICOM
   *2003 standard are supported.
   ** @param  documentType  SR document type to be checked
   ...
   *  The optional UTC notation (e.g. +0100) as well as the optional fractional
   *second part are currently not supported.
   ...
   *  @param  readableDate  reference to variable where the resulting string is
   *stored
   ...
   *  The ISO format "HH:MM" or "HH:MM:SS" (if seconds are available) is used
   *for the readable format.
   ** @param  dicomTime     time in DICOM TM format (HHMM or HHMMSS...)
   *  @param  readableTime  reference to variable where the resulting string is
   *stored
   ** @return reference to resulting string (might be empty)
   */
   ...
   *  The format "YYYY-MM-DD, HH:MM" or "YYYY-MM-DD, HH:MM:SS" is used for the
   *readable format.
   ** @param  dicomDateTime     time in DICOM DT format (YYYYMMDDHHMMSS...).
   *Possible suffixes (fractional second or UTC notation) are currently ignored.
   *  @param  readableDateTime  reference to variable where the resulting string
   *is stored
   ** @return reference to resulting string (might be empty)
   */
   ...
   *  The format "<prefix> <first_name> <middle_name> <last_name>, <suffix>" is
   *used for the readable format. Please note that only the first component
   *group (characters before the first '=') of the DICOM person name is used -
   *see DcmPersonName::getNameComponents() for details.
   ** @param  dicomPersonName     person name in DICOM PN format (ln^fn^mn^p^s)
   *  @param  readablePersonName  reference to variable where the resulting
   *string is stored
   ** @return reference to resulting string (might be empty)
   */
  static const OFString& dicomToReadablePersonName(
      const OFString& dicomPersonName, OFString& readablePersonName);

  /** convert DICOM person name to XML format.
   *  The tags \<prefix\>, \<first\>, \<middle\>, \<last\> and \<suffix\> are
   *used for the XML format of a person name.  The string is automatically
   *converted to the markup notation (see convertToMarkupString()).  Two tags
   *are separated by a newline. Please note that only the first component group
   *(characters before the first '=') of the DICOM person name is used - see
   *DcmPersonName::getNameComponents() for details.
   ** @param  dicomPersonName  person name in DICOM PN format (ln^fn^mn^p^s)
   *  @param  xmlPersonName    reference to variable where the resulting string
   *is stored
   *  @param  writeEmptyValue  optional flag indicating whether an empty value
   *should be written
   ** @return reference to resulting string (might be empty)
   */
  static const OFString& dicomToXMLPersonName(
      const OFString& dicomPersonName, OFString& xmlPersonName,
   ...
   ** @return pointer to the first character of the resulting string (may be
   *NULL if 'string' was NULL)
   */
   ...
   *  This method is used to convert character strings for text "print" output.
   *Newline characters
   *  "\n" are replaced by "\\n", return characters "\r" by "\\r", etc.
   ** @param  sourceString  source string to be converted
   *  @param  printString   reference to character string where the result
   *should be stored
   ** @return reference to resulting 'printString' (might be empty if
   *'sourceString' was empty)
   */
   ...
   *  Characters with special meaning for HTML (e.g. '<' and '&') are replace by
   the
   *  corresponding mnenonics (e.g. "&lt;" and "&amp;").  If flag
   'HF_convertNonASCIICharacters'
   *  is set all characters > #127 are also converted (useful if only HTML 3.2
   is supported which
   *  does not allow to specify the character set).
   ** @param  sourceString     source string to be converted
   *  @param  markupString     reference to character string where the result
   should be stored
   *  @param  flags            optional flags, checking
   HF_convertNonASCIICharacters, HF_HTML32Compatibility and
   HF_XHTML11Compatibility only
   *  @param  newlineAllowed   optional flag indicating whether newlines are
   allowed or not.
   *                           If they are allowed the text "<br>" is used,
   "&para;" otherwise.
   *                           The following combinations are accepted: LF, CR,
   LF CR, CF LF.
   ** @return reference to resulting 'markupString' (might be empty if
   'sourceString' was empty)
   */
  static const OFString& convertToHTMLString(
      const OFString& sourceString, OFString& markupString,
      const size_t flags = 0, const OFBool newlineAllowed = OFFalse);

  /** convert character string to XML mnenonic string.
   *  Characters with special meaning for XML (e.g. '<' and '&') are replace by
   *the corresponding mnenonics (e.g. "&lt;" and "&amp;").
   ** @param  sourceString  source string to be converted
   *  @param  markupString  reference to character string where the result
   *should be stored
   ** @return reference to resulting 'markupString' (might be empty if
   ...
   *  The string should be non-empty and consist only of integer numbers
   *separated by "." where the first and the last character of the string are
   *always figures (0..9). Example: 1 or 1.2.3. Please note that this test is
   *not as strict as specified for value representation UI in the DICOM standard
   *(e.g. regarding even length padding or leading '0' for the numbers).
   ...
   ** @param  documentType  associated SR document type for which the checker
   *object is created
   ** @return pointer to new IOD checker object if successful, NULL if document
   *type is not supported
   */
  static DSRIODConstraintChecker* createIODConstraintChecker(
      const E_DocumentType documentType);

  /** create specified document tree node.
   *  This is a shortcut and the only location where document tree nodes are
   *created. It facilitates the introduction of new relationship/value types and
   *the maintenance.
   ...
   */
  static DSRDocumentTreeNode* createDocumentTreeNode(
      const E_RelationshipType relationshipType, const E_ValueType valueType);

  // --- DICOM data structure access functions ---

  /** add given element to the dataset.
   *  The element is only added if 'result' is EC_Normal and the 'delem' pointer
   *is not NULL.
   ** @param  result      reference to status variable (checked before adding
   ...
   *deleted if not inserted.
   *  @param  vm          value multiplicity (according to the data dictionary)
   *to be checked for. (valid value: "1", "1-2", "1-3", "1-8", "1-99", "1-n",
   *"2", "2-n", "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32"),
   *                      interpreted as cardinality (number of items) for
   *sequence attributes
   *  @param  type        value type (valid value: "1", "2" or something else
   *which is not checked)
   *  @param  moduleName  optional module name to be printed (default: "SR
   *document" if NULL)
   ** @return current value of 'result', EC_Normal if successful, an error code
   *otherwise
   */
  static OFCondition addElementToDataset(OFCondition& result, DcmItem& dataset,
                                         DcmElement* delem, const OFString& vm,
                                         const OFString& type,
                                         const char* moduleName = NULL);

  /** remove given attribute from the sequence.
   *  All occurrences of the attribute in all items of the sequence are removed.
   ** @param  sequence  reference to DICOM sequence from which the attribute
   *should be removed
   *  @param  tagKey    DICOM tag specifying the attribute which should be
   *removed
   */
   ...
   ** @param  dataset  reference to DICOM dataset from which the element should
   *be retrieved. (Would be 'const' if the methods from 'dcmdata' would also be
   *'const'.)
   *  @param  delem    reference to DICOM element which should be retrieved. The
   *return value is also stored in this parameter.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  static OFCondition getElementFromDataset(DcmItem& dataset, DcmElement& delem);

  /** get string value from element
   ** @param  delem  DICOM element from which the string value should be
   *retrieved
  ...
  /** get string value from element
   ** @param  delem        reference to DICOM element from which the string
   *value should be retrieved
   *  @param  stringValue  reference to character string where the result should
   *be stored
   ...
   ** @param  delem            reference to DICOM element from which the string
   *value should be retrieved
   *  @param  stringValue      reference to character string where the result
   *should be stored
   *  @param  convertNonASCII  convert non-ASCII characters (> #127) to numeric
   *value (&\#nnn;) if OFTrue
   ** @return reference character string if successful, empty string otherwise
   */
  static const OFString& getMarkupStringFromElement(
      const DcmElement& delem, OFString& stringValue,
      const OFBool convertNonASCII = OFFalse);

  /** get string value from dataset
   ** @param  dataset      reference to DICOM dataset from which the string
   *should be retrieved. (Would be 'const' if the methods from 'dcmdata' would
   *also be 'const'.)
   *  @param  tagKey       DICOM tag specifying the attribute from which the
   *string should be retrieved
   *  @param  stringValue  reference to character string in which the result
   *should be stored. (This parameter is automatically cleared if the tag could
   *not be found.)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   ** @param  dataset      reference to DICOM dataset to which the string should
   *be put.
   *  @param  tag          DICOM tag specifying the attribute to which the
   *string should be put
   ...
   *  @param  vm          value multiplicity (according to the data dictionary)
   *to be checked for. (valid value: "1", "1-2", "1-3", "1-8", "1-99", "1-n",
   *"2", "2-n", "2-2n", "3", "3-n", "3-3n", "4", "9", "6", "16", "32"),
   *                      interpreted as cardinality (number of items) for
   *sequence attributes
   *  @param  type        value type (valid value: "1", "1C", "2", something
   *else)
   *  @param  searchCond  optional flag indicating the status of a previous
   *'search' function call
   *  @param  moduleName  optional module name to be printed (default: "SR
   *document" if NULL)
   ** @return OFTrue if element value is correct, OFFalse otherwise
   */
  static OFBool checkElementValue(DcmElement& delem, const OFString& vm,
                                  const OFString& type,
                                  const OFCondition& searchCond = EC_Normal,
                                  const char* moduleName = NULL);

  /** get element from dataset and check it for correct value multipicity and
   *type.
   ** @param  dataset     reference to DICOM dataset from which the element
   *should be retrieved. (Would be 'const' if the methods from 'dcmdata' would
   *also be 'const'.)
   ...
   *sequence attributes
   *  @param  type        value type (valid value: "1", "1C", "2", something
   *else which is not checked)
   *  @param  moduleName  optional module name to be printed (default: "SR
   *document" if NULL)
   ** @return status, EC_Normal if element could be retrieved and value is
   *correct, an error code otherwise
   */
  static OFCondition getAndCheckElementFromDataset(
      DcmItem& dataset, DcmElement& delem, const OFString& vm,
      const OFString& type, const char* moduleName = NULL);

  /** get string value from dataset and check it for correct value multipicity
   *and type.
   ** @param  dataset      reference to DICOM dataset from which the element
   *should be retrieved. (Would be 'const' if the methods from 'dcmdata' would
   *also be 'const'.)
  ...
  static OFCondition getAndCheckStringValueFromDataset(
      DcmItem& dataset, const DcmTagKey& tagKey, OFString& stringValue,
      const OFString& vm, const OFString& type, const char* moduleName = NULL);

  // --- output functions ---

  /** print the warning message that the current content item is
   * invalid/incomplete. The value type (for DEBUG mode also the node ID) is
   * added if the 'node' if specified.
   *  @param  action    text describing the current action (e.g. 'Reading'),
   * 'Processing' if NULL
   *  @param  node      pointer to document tree node for which the message
   * should be printed
   *  @param  location  position of the affected content item (e.g. '1.2.3', not
   * printed if NULL)
   */
   ...
   *  The value type (for DEBUG mode also the node ID) is added if the 'node' if
   * specified.
   *  @param  action    text describing the current action (e.g. 'Reading'),
   * 'Processing' if NULL
   *  @param  result    status used to print more information on the error (no
   * message if EC_Normal)
   *  @param  node      pointer to document tree node for which the message
   * should be printed
   *  @param  location  position of the affected content item (e.g. '1.2.3', not
   * printed if NULL)
  ...
  /** print a warning message that an unknown/unsupported value has been
   * determined
   *  @param  valueName  name of the unknown/unsupported value
   *  @param  readValue  value that has been read (optional)
   *  @param  action     text describing the current action (default:
   * 'Reading'), 'Processing' if NULL
   */
   ...
   ** @param  stream           output stream to which the XML document is
   *written
   *  @param  stringValue      string value to be written
   *  @param  tagName          name of the XML tag used to surround the string
   *value
   *  @param  writeEmptyValue  optional flag indicating whether an empty value
   *should be written
   ...
   *  The output looks like this: "<" tagName ">" stringValue "</" tagName ">"
   *  For elements with DICOM VR=PN the function dicomToXMLPersonName() is used
   *internally.
   ** @param  stream           output stream to which the XML document is
   *written
   *  @param  delem            reference to DICOM element from which the value
   *is retrieved
   *  @param  tagName          name of the XML tag used to surround the string
   *value
   *  @param  writeEmptyValue  optional flag indicating whether an empty value
   ...
   ** @return OFTrue if tag/value has been written, OFFalse otherwise
   */
  static OFBool writeStringFromElementToXML(
      STD_NAMESPACE ostream& stream, DcmElement& delem, const OFString& tagName,
      const OFBool writeEmptyValue = OFFalse);

  /** create an HTML annex entry with hyperlinks.
   *  A reference text is added to the main document and a new annex entry to
   *the document annex with HTML hyperlinks between both.  Example for a
   *reference: '[for details see Annex 1]'
   ** @param  docStream      output stream used for the main document
   *  @param  annexStream    output stream used for the document annex
   *  @param  referenceText  optional text added to the main document (e.g. 'for
   *details see')
   *  @param  annexNumber    reference to the variable where the current annex
   *number is stored. Value is increased automatically by 1 after the new entry
   *has been added.
   ...
   *  @param  footnoteNumber  reference to the variable where the current
   *footnote number is stored. Value is increased automatically by 1 after the
   *new entry has been added.
   *  @param  nodeID          unique numerical identifier of the current node
   *for which this footnote is created.  Used to create a unique name for the
   *hyperlink.
   *  @param  flags           optional flag used to customize the output (see
   *DSRTypes::HF_xxx)
  ...
  static size_t createHTMLFootnote(STD_NAMESPACE ostream& docStream,
                                   ...
                                   size_t& footnoteNumber, const size_t nodeID,
                                   const size_t flags = 0);
   ...
   *  @param  heading     (optional) string which is added to the 'mainStream'
   *before the 'tempStream'. This string is only added if 'tempStream' is not
   *empty.
   ** @return status, EC_Normal if stream has been added successfully, an error
   *code otherwise
   */
 ...
 *  Enhanced method checkElementValue(), e.g. added support for type 1C
 * elements.
 *
