 ...
 *  This class allows to access the document tree nodes without using any
 * pointers.
 */
class DSRContentItem : protected DSRTypes {
...
public:
   ...
   *  Applicable to all content items.
   ** @return relationship type of current content item if valid, RT_invalid
   *otherwise
   */
   ...
   ** @return string value of current content item if valid, EmptyString
   *otherwise
   */
  const OFString& getStringValue() const;

  /** set string value.  Please use the correct format for the string value
   *depending on the corresponding content item (value type). Applicable to:
   *TEXT, DATETIME, DATE, TIME, UIDREF, PNAME
   ...
   *  Applicable to: CODE
   ** @return pointer to code value of current content item if valid, NULL
   *otherwise
   */
  ...
  /** get code value.
   *  Applicable to: CODE
   ** @return coded entry value of current content item if valid,
   *EmptyCodedEntry otherwise
   */
   ...
   *  Applicable to: NUM
   ** @return pointer to numeric value of current content item if valid, NULL
   *otherwise
   */
   ...
   *  Applicable to: NUM
   ** @param  numericValue  variable where the copy should be stored (cleared if
   *an error occurs)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   ** @return pointer to spatial coordinates value of current content item if
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition getSpatialCoordinates(
      DSRSpatialCoordinatesValue& coordinatesValue) const;
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition setSpatialCoordinates(
      const DSRSpatialCoordinatesValue& coordinatesValue);

  /** get pointer to temporal coordinates.
   *  Applicable to: TCOORD
   ** @return pointer to temporal coordinates value of current content item if
   *valid, NULL otherwise
   */
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition getTemporalCoordinates(
      DSRTemporalCoordinatesValue& coordinatesValue) const;
   ...
   *  Applicable to: COMPOSITE
   ** @return reference value of current content item if valid, EmptyReference
   *otherwise
   */
   ...
   *  Applicable to: COMPOSITE
   ** @param  referenceValue  variable where the copy should be stored (cleared
   *if an error occurs)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition getCompositeReference(
      DSRCompositeReferenceValue& referenceValue) const;
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition setCompositeReference(
   ...
   ** @return pointer to waveform reference value of current content item if
   *valid, NULL otherwise
   */
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition getWaveformReference(
      DSRWaveformReferenceValue& referenceValue) const;
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition setWaveformReference(
      const DSRWaveformReferenceValue& referenceValue);

  /** get continuity of content flag.
   *  This flag specifies whether or not its contained content items (child
   *nodes) are logically linked in a continuous textual flow, or are sparate
   *items. Applicable to: CONTAINER
   ...
   *nodes) are logically linked in a continuous textual flow, or are sparate
   *items. Applicable to: CONTAINER
   ** @param  continuityOfContent  value to be set (should be different from
   *COC_onvalid)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition setContinuityOfContent(
   ...
   ** @return pointer to comcept name value of current content item if valid,
   *NULL otherwise
   */
   ...
   *conveys the value of document title and section headings in documents.
   *  Applicable to all content items (by-value only).
   ** @return concept name value of current content item if valid,
   *EmptyCodedEntry otherwise
   */
   ...
   *conveys the value of document title and section headings in documents.
   *  Applicable to all content items (by-value only).
   ** @param  conceptName  variable where the copy should be stored (cleared if
   *an error occurs)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Code describing the concept represented by this content item.  Also
   *conveys the value of document title and section headings in documents.
   *  Applicable to all content items (by-value only, optional/conditional for
   *some value types).
   ...
   *see DSRDocument. Applicable to all content items (by-value only, optional
   *attribute).
   ** @return observation date and time of current content item if valid,
   ...
   *  This is the date and time on which this content item was completed.  Might
   *be empty if the date and time do not differ from the content date and time,
   *see DSRDocument. Please use the correct DICOM format (VR=DT). Applicable to
   *all content items (by-value only).
   ...
   *content item (and its children).  According to the DICOM standard is is
   ...
   *absent (empty). Applicable to all content items (by-value only, optional
   *attribute).
   ...
   *or absent (empty).  See getTemplateIdentification() for details. Applicable
   *to all content items (by-value only).
...
private:
  ...
  /// empty coded entry value. Used as default return value for getCodeValue()
  /// and getConceptName()
  static const DSRCodedEntryValue EmptyCodedEntry;
  /// empty numberic measurement value. Used as default return value for
  /// getNumericValue()
  static const DSRNumericMeasurementValue EmptyNumericMeasurement;
  /// empty spatial coordinates value. Used as default return value for
  /// getSpatialCoordinates()
  static const DSRSpatialCoordinatesValue EmptySpatialCoordinates;
  /// empty temporal coordinates value. Used as default return value for
  /// getTemporalCoordinates()
  static const DSRTemporalCoordinatesValue EmptyTemporalCoordinates;
  /// empty composite reference value. Used as default return value for
  /// getCompositeReference()
  static const DSRCompositeReferenceValue EmptyCompositeReference;
  /// empty image reference value. Used as default return value for
  /// getImageReference()
  static const DSRImageReferenceValue EmptyImageReference;
  /// empty waveform reference value. Used as default return value for
