...
public:
  ...
  inline OFBool operator==(const DSRWaveformChannelItem& item) const {
    return (item.MultiplexGroupNumber == MultiplexGroupNumber) &&
  ...
  /** comparison operator
   ** @param  item  item with which the (m,c) pair should be compared
   ** @return true if smaller. ChannelNumber takes precedence over
   *MultiplexGroupNumber.
   */
  inline OFBool operator<(const DSRWaveformChannelItem& item) const {
    return (ChannelNumber < item.ChannelNumber) ||
  ...
  /** comparison operator
   ** @param  item  item with which the (m,c) pair should be compared
   ** @return true if greater. ChannelNumber takes precedence over
   *MultiplexGroupNumber.
   */
  inline OFBool operator>(const DSRWaveformChannelItem& item) const {
    return (ChannelNumber > item.ChannelNumber) ||
 ...
 */
class DSRWaveformChannelList : public DSRListOfItems<DSRWaveformChannelItem> {
public:
  /** default constructor
   */
   ...
   *DSRTypes::PF_xxx)
   *  @param  pairSeparator  character specifying the separator between the
   *value pairs
   *  @param  itemSeparator  character specifying the separator between the list
   *items
   ...
   ** @param  idx                   index of the value pair to be returned
   *(starting from 1)
   *  @param  multiplexGroupNumber  multiplex group number of the specified
   *index (set to 0)
   *  @param  channelNumber         channel number of the specified index (set
   *to 0 first)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition getItem(const size_t idx, Uint16& multiplexGroupNumber,
   ...
   *  This function expects the same input format as created by print(), i.e. a
   *comma separated list of numerical value pairs.
