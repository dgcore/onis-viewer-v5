...
public:
   ...
   */
  DSRGraphicData3DItem(const Float32 x = 0, const Float32 y = 0,
                       const Float32 z = 0)
 ...
 */
class DSRGraphicData3DList : public DSRListOfItems<DSRGraphicData3DItem> {
public:
  /** default constructor
   */
   ...
   *  The output of a typical list looks like this:
   *0/0/0,127/127/127,255/255/255
   ** @param  stream            output stream to which the list should be
   *printed
   *  @param  flags             flag used to customize the output (see
   *DSRTypes::PF_xxx)
   *  @param  tripletSeparator  character specifying the separator between the
   *value triplets
   *  @param  itemSeparator     character specifying the separator between the
   *list items
   ...
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition getItem(const size_t idx, Float32& x, Float32& y,
                      Float32& z) const;
   ...
   *comma separated list of numerical value triplets.
