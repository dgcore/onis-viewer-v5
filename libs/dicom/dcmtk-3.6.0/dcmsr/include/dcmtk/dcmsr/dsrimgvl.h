 ...
 */
class DSRImageReferenceValue : public DSRCompositeReferenceValue {
...
public:
  ...
  /** constructor.
   *  The UID 4-tuple is only set if it passed the validity check (see
   *setValue()).
   ** @param  imageSOPClassUID      referenced SOP class UID of the image
   *object. (VR=UI, mandatory)
   *  @param  imageSOPInstanceUID   referenced SOP instance UID of the image
   *object. (VR=UI, mandatory)
   *  @param  pstateSOPClassUID     referenced SOP class UID of the presentation
   *state object. (VR=UI, optional)
   *  @param  pstateSOPInstanceUID  referenced SOP instance UID of the
   *presentation state object. (VR=UI, optional)
   */
  ...
  /** copy constructor
   ** @param  imageReferenceValue   imagee reference value to be copied (not
   *checked !)
   *  @param  pstateReferenceValue  presentation state reference value to be
   *copied (not checked !)
   */
  DSRImageReferenceValue(
  ...
  /** assignment operator
   ** @param  referenceValue  image reference value to be copied (not checked !)
   ** @return reference to this image reference value after 'referenceValue' has
   *been copied
   */
  DSRImageReferenceValue& operator=(
      const DSRImageReferenceValue& referenceValue);

  /** clear all internal variables.
   *  Since an empty image reference is invalid the reference becomes invalid
   * afterwards.
   */
  virtual void clear();

  /** check whether the current image reference value is valid.
   *  The reference value is valid if SOP class UID and SOP instance UID are
   *valid (see checkSOP...UID() for details) and the optional presentation state
   *is valid (see checkPresentationState()).
   ...
   *item can be expanded inline or not (used for renderHTML()).
   ...
   *  The output of a typical image reference value looks like this: (CT
   *image,"1.2.3") or (CT image,"1.2.3"),(GSPS,"1.2.3.4") if a presentation
   *state is present. If the SOP class UID is unknown the UID is printed instead
   *of the related name.
   ** @param  stream  output stream to which the image reference value should be
   *printed
   ...
   ** @param  referenceValue  reference to variable in which the value should be
   *stored
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Before setting the reference it is checked (see checkXXX()).  If the value
   ...
   *  If the value is invalid the current value is not replaced and remains
   *unchanged.
   ** @param  referenceValue  value to be set
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  The image reference applies to a frame (of multiframe images) if the list
   *of referenced frame numbers is empty or the frame number is part of the
   *list.
   ** @param  frameNumber  number of the frame to be checked
   ** @return OFTrue if reference applies to the specified frame, OFFalse
   *otherwise
   */
...
protected:
   ...
   *  The only check that is currently performed is that the UID is not empty.
   *Later on it might be checked whether the specified SOP class is really an
   ...
   *  The presentation state object is "valid" if both UIDs are empty or both
   *are not empty and SOP class UID equals to
   *"GrayscaleSoftcopyPresentationStateStorage".
   ** @param  referenceValue  value to be checked
   ** @return OFTrue if presentation state object is valid, OFFalse otherwise
   */
  OFBool checkPresentationState(
      const DSRCompositeReferenceValue& referenceValue) const;

private:
  /// composite reference value (SOP class/instance UID) to presentation state
  /// (optional)
