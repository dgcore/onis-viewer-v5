 ...
 *  effort (if required).
 */
class DSRXMLDocument : protected DSRTypes {
public:
   ...
   *  In order to enable the optional Schema validation the flag
   *XF_validateSchema has to be set.
   ...
   ** @param  charset  XML name of the character set (e.g. "ISO-8859-1" for ISO
   *Latin-1)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   ** @return cursor pointing to the root node if successful, invalid cursor
   *otherwise
   */
  DSRXMLCursor getRootNode() const;

  /** get a particular named node of the document.
   *  Please note that the search process is limited to the current node level,
   *i.e. no deep search is performed.
   ** @param  cursor    cursor pointing to the node where to start from
   *  @param  name      name of the node (XML element) to be searched for
   *  @param  required  flag specifying whether the node is required or not.  If
   *the node is required to be present an error message is reported.
   ** @return cursor pointing to the named node if successful, invalid cursor
   *otherwise
   */
  DSRXMLCursor getNamedNode(const DSRXMLCursor& cursor, const char* name,
  ...
  /** check whether particular node matches a given name and report an error if
   *not
   ** @param  cursor  cursor pointing to the particular node
   *  @param  name    name of the node (XML element) to be checked
   ** @return OFTrue if name matches, OFFalse otherwise
   ...
   *  The result variable 'stringValue' is automatically cleared at the
   *beginning.
   ** @param  cursor       cursor pointing to the particular node
   *  @param  stringValue  reference to string object in which the value should
   *be stored
   *  @param  name         name of the XML attribute to be retrieved
   *  @param  encoding     use encoding handler if OFTrue, ignore character set
   *otherwise
   *  @param  required     flag specifying whether the attribute is required or
   *not.  If the attribute is required to be present an error message is
   *reported in case it is not found.
   ** @return reference to string object (might be empty)
   */
  OFString& getStringFromAttribute(const DSRXMLCursor& cursor,
                                   OFString& stringValue, const char* name,
  ...
  OFCondition getElementFromAttribute(const DSRXMLCursor& cursor,
                                      DcmElement& delem, const char* name,
                                      const OFBool encoding = OFFalse,
                                      const OFBool required = OFTrue) const;
   ...
   *  @param  encoding     use encoding handler if OFTrue, ignore character set
   *otherwise
   *  @param  clearString  flag specifying whether to clear the 'stringValue' at
   *first or not
   ** @return reference to string object (might be empty)
   */
   ...
   *  The value type is either stored as the element name or in the attribute
   *"valType". Additionally, by-reference relationships are also supported
   *(either by attribute "ref" being present or element named "reference").
   ** @param  cursor  cursor pointing to the particular node
   ** @return value type (incl. by-reference) if successful, VT_invalid/unknown
   *otherwise
   */
  E_ValueType getValueTypeFromNode(const DSRXMLCursor& cursor) const;

  /** get relationship type from particular node.
   *  The relationship type is either stored in the element "relationship" or in
   *the attribute "relType".
   ** @param  cursor  cursor pointing to the particular node
   ** @return relationship type if successful, RT_invalid/unknown otherwise
   */
  E_RelationshipType getRelationshipTypeFromNode(
   ...
   ** @param  cursor  cursor pointing to the unexpected node
   *  @param  result  status used to print details on the error (no message if
   *EC_Normal)
   */
...
protected:
   ...
   *  @param  toString    reference to string object in which the result should
   *be stored
   ...
   ** @param  cursor       cursor pointing to the relevant node
   *  @param  stringValue  reference to string object in which the result should
   *be stored
   *  @param  omitCurrent  flag indicating whether to omit the current node or
   *not
   */
