...
public:
  ...
  /** constructor.
   *  The code triple is only set if it passed the validity check (see
   *setCode()).
   ** @param  codeValue               identifier of the code to be set that is
   *unambiguous within the coding scheme. (VR=SH, mandatory)
   *  @param  codingSchemeDesignator  textual identifier of the table where the
   *'codeValue' is linked to its 'codeMeaning'. (VR=SH, mandatory)
   *  @param  codeMeaning             human-readable translation of the
   *'codeValue'.  Used for display when code dictionary is not available.
   *                                  (VR=LO, mandatory)
   */
  ...
  /** constructor.
   *  The code 4-tuple is only set if it passed the validity check (see
   *setCode()).
   ** @param  codeValue               identifier of the code to be set that is
   *unambiguous within the coding scheme. (VR=SH, mandatory)
   *  @param  codingSchemeDesignator  textual identifier of the table where the
   *'codeValue' is linked to its 'codeMeaning'. (VR=SH, mandatory)
   *  @param  codingSchemeVersion     version of the coding scheme.  Used when a
   *coding scheme has multiple versions and the 'codingScheme Designator' does
   *not explicitly (or adequately) specify the version number. (VR=SH, optional)
   *  @param  codeMeaning             human-readable translation of the
   *'codeValue'. Used for display when (appropriate) code dictionary is not
  ...
  /** comparison operator.
   *  Two codes are equal if the code value, coding scheme designator and the
   *(optional) coding scheme version are equal.  The code meaning is not
   ...
   *(1234,99_OFFIS_DCMTK,"Code Meaning"). The optional coding scheme version is
   *printed in square brackets directly after the coding scheme designator,
   *e.g.: (cm,UCUM[1.4],"centimeter")
   ** @param  stream          output stream to which the code should be printed
   *  @param  printCodeValue  flag indicating whether the code value and coding
   *scheme designator should be printed (default) or not.  If OFFalse the output
   *looks like this: (,,"Code Meaning")
   *  @param  printInvalid    flag indicating whether the text "invalid code"
   *should be printed for invalid codes or not (default)
   */
   ...
   *  The number of items within the code sequence is checked.  If error/warning
   *output are enabled a warning message is printed if the sequence is empty or
   *contains more than one item.
   ** @param  dataset    DICOM dataset from which the code sequence should be
   *read
   *  @param  tagKey     DICOM tag specifying the attribute (= sequence) which
   *should be read
   *  @param  type       value type of the sequence (valid value: "1", "2",
   *something else). This parameter is used for checking purpose, any difference
   *is reported.
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition readSequence(DcmItem& dataset, const DcmTagKey& tagKey,
                           const OFString& type);

  /** write code sequence to dataset
   ** @param  dataset    DICOM dataset to which the code sequence should be
   *written
   *  @param  tagKey     DICOM tag specifying the attribute (= sequence) which
   *should be written
   ** @return status, EC_Normal if successful, an error code otherwise
   ...
   *  @param  flags       flag used to customize the output (see
   *DSRTypes::HF_xxx)
   *  @param  fullCode    optional flag indicating whether to render the full
   *code tuple or the code value/meaning only
   *  @param  valueFirst  optional flag indicating whether to render the code
   *value or meaning first (outside the brackets)
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition renderHTML(STD_NAMESPACE ostream& stream, const size_t flags,
   ...
   ** @param  codedEntryValue  reference to variable in which the code should be
   *stored
   ** @return status, EC_Normal if successful, an error code otherwise
   */
  ...
  /** get code value.
   *  This is a identifier of the code that is unambiguous within the coding
   *scheme.
   ...
   *  This is a textual identifier of the table where the code value is linked
   *to its code meaning.  Designators beginning with "99" and the designator "L"
   *are defined to be private or local coding schemes.  More details on the
   *coding scheme might be retrieved via the
   *DSRDocument::getCodingSchemeIdentification() method.
   ** @return current coding scheme designator (might be invalid or an empty
   *string)
   */
   ...
   *  Optional - Used when a coding scheme has multiple versions and the coding
   *scheme designator does not explicitly (or adequately) specify the version
   *number.
   ** @return current coding scheme version (might be invalid or an empty
   *string)
   */
  ...
  /** get code meaning.
   *  Human-readable translation of the code value.  Used for display when code
  ...
  /** set code value.
   *  Before setting the code it is checked (see checkCode()).  If the code is
   ...
   *information on the coding scheme can be provided via the
   *getCodingSchemeIdentification() method in the main interface class
   *DSRDocument (highly recommended for private coding schemes).
   ** @param  codeValue               identifier of the code to be set that is
   *unambiguous within the coding scheme. (VR=SH, mandatory)
   *  @param  codingSchemeDesignator  textual identifier of the table where the
   *'codeValue' is linked to its 'codeMeaning'. (VR=SH, mandatory) Designators
   *beginning with "99" and the designator "L" are defined to be private or
  ...
  /** set code value.
   *  Before setting the specified code it is checked (see checkCode()).  If the
   *code is invalid the current code is not replaced and remains unchanged.
   *Additional information on the coding scheme can be provided via the
   *getCodingSchemeIdentification() method in the main class DSRDocument (highly
   *recommended for private coding schemes).
   ** @param  codeValue               identifier of the code to be set that is
   *unambiguous within the coding scheme. (VR=SH, mandatory)
   *  @param  codingSchemeDesignator  textual identifier of the table where the
   *'codeValue' is linked to its 'codeMeaning'. (VR=SH, mandatory) Designators
...
protected:
   ...
   *  @param  moduleName  optional module name (sequence) from which the item is
   *read
   ** @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Currently the only check that is performed is that the three string values
   *are not empty. Later on it might be checked whether the specified code
   *really belongs to the specified coding scheme, etc.  This require the
   *presence of the relevant code dictionaries.
