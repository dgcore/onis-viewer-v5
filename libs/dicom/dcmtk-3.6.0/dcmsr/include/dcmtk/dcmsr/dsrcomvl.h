...
public:
   ...
   *                          (VR=UI, type 1)
   *  @param  sopInstanceUID  referenced SOP instance UID of the composite
   *object. (VR=UI, type 1)
   */
  ...
  /** assignment operator
   ** @param  referenceValue  reference value to be copied (not checked !)
   ** @return reference to this reference value after 'referenceValue' has been
   *copied
   */
  DSRCompositeReferenceValue& operator=(
      const DSRCompositeReferenceValue& referenceValue);

  /** clear all internal variables.
   *  Since an empty reference value is invalid the reference becomes invalid
   * afterwards.
   */
  virtual void clear();
   ...
   *valid (see checkSOP...UID() for details).
   ...
   *  The output of a typical composite reference value looks like this:
   *(BasicTextSR,"1.2.3"). If the SOP class UID is unknown the UID is printed
   *instead of the related name.
   ** @param  stream  output stream to which the reference value should be
   *printed
   ...
   *  The number of items within the sequence is checked.  If error/warning
   *output are enabled a warning message is printed if the sequence is absent or
   ...
   *  Before setting the value is is checked (see checkSOPClassUID()).  If the
   ...
   *  Before setting the value is is checked (see checkSOPInstanceUID()).  If
   *the value is invalid the current value is not replaced and remains
   *unchanged.
...
protected:
   ...
   *Derived classes might overwrite this method for more specific tests (e.g.
   *allowing only particular SOP classes).
   ...
   *  The only check that is currently performed is that the UID is not empty.
   *Derived classes might overwrite this method for more specific tests.
