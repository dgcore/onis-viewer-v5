...
DSRCodeTreeNode::DSRCodeTreeNode(const E_RelationshipType relationshipType)
    : DSRDocumentTreeNode(relationshipType, VT_Code), DSRCodedEntryValue() {}
...
OFBool DSRCodeTreeNode::isValid() const {
  ...
  return DSRDocumentTreeNode::isValid() && DSRCodedEntryValue::isValid() &&
...
OFCondition DSRCodeTreeNode::print(STD_NAMESPACE ostream& stream,
  ...
  if (result.good()) {
    stream << "=";
    DSRCodedEntryValue::print(stream, OFTrue /*printCodeValue*/,
                              OFTrue /*printInvalid*/);
  }
  return result;
}
...
OFCondition DSRCodeTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    const DSRXMLCursor childCursor =
        doc.getNamedNode(cursor.getChild(), "value");
...
OFCondition DSRCodeTreeNode::readContentItem(DcmItem& dataset) {
  ...
  return DSRCodedEntryValue::readSequence(dataset, DCM_ConceptCodeSequence,
                                          "1" /*type*/);
}
...
OFCondition DSRCodeTreeNode::renderHTMLContentItem(
  ...
  /* render Code */
  if (result.good()) {
    const OFBool fullCode = (flags & DSRTypes::HF_renderInlineCodes) ||
