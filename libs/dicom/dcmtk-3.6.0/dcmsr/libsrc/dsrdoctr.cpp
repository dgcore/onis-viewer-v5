...
OFBool DSRDocumentTree::isValid() const {
  ...
  if (isDocumentTypeSupported(DocumentType)) {
    ...
    const DSRDocumentTreeNode* node =
        OFstatic_cast(DSRDocumentTreeNode*, getRoot());
    if (node != NULL) {
      if ((node->getRelationshipType() == RT_isRoot) &&
    ...
    do {
      ...
      if (node != NULL) {
        ...
        if (flags & PF_printTemplateIdentification) {
          ...
          if (node->getTemplateIdentification(templateIdentifier,
                                              mappingResource)
                  .good()) {
...
OFCondition DSRDocumentTree::read(DcmItem& dataset,
  ...
  if (result.good()) {
    if (ConstraintChecker == NULL)
      DCMSR_WARN(
          "Check for relationship content constraints not yet supported");
    ...
    OFString tmpString;
    if (getAndCheckStringValueFromDataset(dataset, DCM_ValueType, tmpString,
                                          "1", "1")
            .good() ||
  ...
  if (cursor.valid()) {
    ...
    if (flags & XF_templateElementEnclosesItems) {
      ...
      const DSRXMLCursor childCursor =
          doc.getNamedNode(cursor, "template", OFFalse /*required*/);
        ...
        if (addNode(node)) {
          if (flags & XF_templateElementEnclosesItems) {
            ...
            if (node->setTemplateIdentification(templateIdentifier,
                    ...
                    .bad())
              DCMSR_WARN(
                  "Root content item has invalid/incomplete template "
                  "identification");
          }
          /* ... and let the node read the rest of the document */
...
OFCondition DSRDocumentTree::write(DcmItem& dataset, DcmStack* markedItems) {
...
OFCondition DSRDocumentTree::renderHTML(STD_NAMESPACE ostream& docStream,
  ...
  if (isValid()) {
    ...
    if (node != NULL) {
      ...
      /* start rendering from root node */
      result = node->renderHTML(docStream, annexStream, 1 /*nestingLevel*/,
                                annexNumber, flags & ~HF_internalUseOnly);
    }
  }
  return result;
}

OFCondition DSRDocumentTree::changeDocumentType(
    const E_DocumentType documentType) {
  ...
  if (node != NULL) {
    if (ConstraintChecker != NULL) {
      if ((addMode == AM_beforeCurrent) || (addMode == AM_afterCurrent)) {
        ...
        if (node != NULL)
          result = ConstraintChecker->checkContentRelationship(
              node->getValueType(), relationshipType, valueType);
      } else
        result = ConstraintChecker->checkContentRelationship(
            node->getValueType(), relationshipType, valueType);
    } else
...
OFBool DSRDocumentTree::canAddByReferenceRelationship(
  ...
  if (ConstraintChecker != NULL) {
    ...
    if (node != NULL)
      result = ConstraintChecker->checkContentRelationship(
          node->getValueType(), relationshipType, targetValueType,
          OFTrue /*byReference*/);
  } else
    result = OFTrue; /* cannot check, therefore, allow everything */
  return result;
...
size_t DSRDocumentTree::addContentItem(
    const E_RelationshipType relationshipType, const E_ValueType valueType,
    const E_AddMode addMode) {
  size_t nodeID = 0;
...
size_t DSRDocumentTree::addByReferenceRelationship(
    const E_RelationshipType relationshipType, const size_t referencedNodeID) {
  size_t nodeID = 0;
  if (referencedNodeID > 0) {
    ...
    if (cursor.isValid()) {
      ...
      if (cursor.gotoNode(referencedNodeID)) {
        ...
        /* check whether target node is an ancestor of source node (prevent
         * loops) */
        if (sourceString.substr(0, targetString.length()) != targetString) {
          const DSRDocumentTreeNode* targetNode =
              OFstatic_cast(DSRDocumentTreeNode*, cursor.getNode());
          if (targetNode != NULL) {
            /* check whether relationship is valid/allowed */
            if (canAddByReferenceRelationship(relationshipType,
                                              targetNode->getValueType())) {
              DSRDocumentTreeNode* node = new DSRByReferenceTreeNode(
                  relationshipType, referencedNodeID);
              if (node != NULL) {
...
DSRContentItem& DSRDocumentTree::getCurrentContentItem() {
  CurrentContentItem.setTreeNode(
      OFstatic_cast(DSRDocumentTreeNode*, getNode()));
...
size_t DSRDocumentTree::gotoNextNamedNode(const DSRCodedEntryValue& conceptName,
  ...
  if (nodeID > 0)
    nodeID =
        gotoNamedNode(conceptName, OFFalse /*startFromRoot*/, searchIntoSub);
  return nodeID;
}
...
OFCondition DSRDocumentTree::checkByReferenceRelationships(const size_t mode,
  ...
  if (!((mode & CM_updatePositionString) && (mode & CM_updateNodeID))) {
    ...
    if ((ConstraintChecker != NULL) &&
        ConstraintChecker->isByReferenceAllowed()) {
      /* specify for all content items not to be the target of a by-reference
       * relationship */
        ...
        do { /* for all content items */
          ...
          if (node != NULL) {
            ...
            if (node->getValueType() == VT_byReference) {
              size_t refNodeID = 0;
              /* type cast to directly access member variables of by-reference
               * class */
              DSRByReferenceTreeNode* refNode = OFconst_cast(
                  DSRByReferenceTreeNode*,
                  OFstatic_cast(const DSRByReferenceTreeNode*, node));
              if (flags & RF_showCurrentlyProcessedItem) {
                OFString posString;
                DCMSR_WARN("Updating by-reference relationship in content item "
                           << cursor.getPosition(posString));
              }
              /* start searching from root node (be careful with large trees,
              ...
              if (mode & CM_updateNodeID) {
                ...
                if (mode & CM_updatePositionString) {
                  ...
                  refNode->ValidReference =
                      checkForValidUIDFormat(refNode->ReferencedContentItem);
                } else if (refNodeID == 0)
              ...
              if (refNodeID > 0) {
                ...
                if (refNodeID != cursor.getNodeID()) {
                  OFString posString;
                  cursor.getPosition(posString);
                  /* check whether target node is an ancestor of source node
                   * (prevent loops) */
                  if (posString.substr(
                          0, refNode->ReferencedContentItem.length()) !=
                      refNode->ReferencedContentItem) {
                    /* refCursor should now point to the reference target
                     * (refNodeID > 0) */
                    const DSRDocumentTreeNode* parentNode = OFstatic_cast(
                        const DSRDocumentTreeNode*, cursor.getParentNode());
                    DSRDocumentTreeNode* targetNode = OFstatic_cast(
                        DSRDocumentTreeNode*, refCursor.getNode());
                    if ((parentNode != NULL) && (targetNode != NULL)) {
                      /* specify that this content item is target of an
                      ...
                      E_RelationshipType relationshipType =
                          refNode->getRelationshipType();
                      if (!(flags & RF_ignoreRelationshipConstraints) &&
                          (!(flags & RF_acceptUnknownRelationshipType) ||
                           (relationshipType != RT_unknown))) {
                        /* check whether relationship is valid */
                        if ((ConstraintChecker != NULL) &&
                            !ConstraintChecker->checkContentRelationship(
                                parentNode->getValueType(), relationshipType,
                                targetNode->getValueType(),
                                OFTrue /*byReference*/)) {
                          DCMSR_WARN(
                              "Invalid by-reference relationship between item "
                              "\""
                              << posString << "\" and \""
                              << refNode->ReferencedContentItem << "\"");
                        }
                      ...
                      DCMSR_WARN(
                          "Corrupted data structures while checking "
                          "by-reference relationships");
                  } else
                    DCMSR_WARN(
                        "By-reference relationship to ancestor content item "
                        "(loop check)");
                } else
                  DCMSR_WARN(
                      "Source and target content item of by-reference "
                      "relationship are identical");
              } else {
                if (mode & CM_updateNodeID) {
                  DCMSR_WARN(
                      "Target content item of by-reference relationship ("
                      << refNode->ReferencedContentItem << ") does not exist");
                } else
                  DCMSR_WARN(
                      "Target content item of by-reference relationship does "
                      "not exist");
              }
            }
          } else
 ...
 *  Enhanced warning message for invalid by-reference relationships by adding
