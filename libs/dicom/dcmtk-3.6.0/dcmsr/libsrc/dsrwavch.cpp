...
EXPLICIT_SPECIALIZATION const DSRWaveformChannelItem
    DSRListOfItems<DSRWaveformChannelItem>::EmptyItem(0, 0);

DSRWaveformChannelList::DSRWaveformChannelList()
    : DSRListOfItems<DSRWaveformChannelItem>() {}

DSRWaveformChannelList::DSRWaveformChannelList(
    const DSRWaveformChannelList& lst)
    : DSRListOfItems<DSRWaveformChannelItem>(lst) {}

DSRWaveformChannelList::~DSRWaveformChannelList() {}

DSRWaveformChannelList& DSRWaveformChannelList::operator=(
    const DSRWaveformChannelList& lst) {
...
OFCondition DSRWaveformChannelList::print(STD_NAMESPACE ostream& stream,
  ...
  while (iterator != endPos) {
    stream << (*iterator).MultiplexGroupNumber << pairSeparator
           << (*iterator).ChannelNumber;
    iterator++;
    if (iterator != endPos) {
...
OFCondition DSRWaveformChannelList::read(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckElementFromDataset(
      dataset, delem, "2-2n", "1C", "WAVEFORM content item");
  if (result.good()) {
    /* clear internal list */
    clear();
    Uint16 group = 0;
...
OFCondition DSRWaveformChannelList::write(DcmItem& dataset) const {
  ...
  /* add to dataset */
  if (result.good())
    result = DSRTypes::addElementToDataset(result, dataset,
                                           new DcmUnsignedShort(delem), "2-2n",
                                           "1", "WAVEFORM content item");
  return result;
}

OFBool DSRWaveformChannelList::isElement(const Uint16 multiplexGroupNumber,
                                         const Uint16 channelNumber) const {
  return DSRListOfItems<DSRWaveformChannelItem>::isElement(
      DSRWaveformChannelItem(multiplexGroupNumber, channelNumber));
}
...
OFCondition DSRWaveformChannelList::getItem(const size_t idx,
  ...
  OFCondition result =
      DSRListOfItems<DSRWaveformChannelItem>::getItem(idx, item);
...
void DSRWaveformChannelList::addItem(const Uint16 multiplexGroupNumber,
  ...
  DSRListOfItems<DSRWaveformChannelItem>::addItem(
      DSRWaveformChannelItem(multiplexGroupNumber, channelNumber));
}
