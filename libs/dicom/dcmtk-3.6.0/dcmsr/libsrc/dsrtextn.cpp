...
DSRTextTreeNode::DSRTextTreeNode(const E_RelationshipType relationshipType)
    : DSRDocumentTreeNode(relationshipType, VT_Text), DSRStringValue() {}
  ...
  if (result.good()) {
    ...
    if (flags & PF_shortenLongItemValues)
      DSRStringValue::print(stream,
                            30);  // text output is limited to 30 characters
    else
      DSRStringValue::print(stream);
  }
  return result;
...
OFCondition DSRTextTreeNode::writeXML(STD_NAMESPACE ostream& stream,
  ...
  writeStringValueToXML(stream, getValue(), "value",
...
OFCondition DSRTextTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  return DSRStringValue::readXML(
      doc, doc.getNamedNode(cursor.gotoChild(), "value"), OFTrue /*encoding*/);
}

OFCondition DSRTextTreeNode::renderHTMLContentItem(
  ...
  if (flags & HF_renderItemInline)
    docStream << "\"" << convertToHTMLString(getValue(), htmlString, flags)
              << "\"" << OFendl;
  else
    docStream << convertToHTMLString(getValue(), htmlString, flags,
                                     OFTrue /*newlineAllowed*/)
              << OFendl;
  return result;
}
