...
DSRDocument::DSRDocument(const E_DocumentType documentType)
    : DocumentTree(documentType),
      ...
      CurrentRequestedProcedureEvidence(
...
OFBool DSRDocument::isValid() {
  ...
  return DocumentTree.isValid() && !SOPClassUID.isEmpty() &&
  ...
  if (isValid()) {
    ...
    if (!(flags & PF_printNoDocumentHeader)) {
      ...
      if (!PatientName.isEmpty()) {
        stream << "Patient             : "
               << getPrintStringFromElement(PatientName, tmpString);
        OFString patientStr;
      ...
      if (!ReferringPhysicianName.isEmpty())
        stream << "Referring Physician : "
               << getPrintStringFromElement(ReferringPhysicianName, tmpString)
               << OFendl;
      /* study related information  */
      if (!StudyDescription.isEmpty()) {
        stream << "Study               : "
               << getPrintStringFromElement(StudyDescription, tmpString);
        if (!StudyID.isEmpty())
      ...
      if (!Manufacturer.isEmpty()) {
        stream << "Manufacturer        : "
               << getPrintStringFromElement(Manufacturer, tmpString);
        OFString deviceStr;
        if (!ManufacturerModelName.isEmpty())
          deviceStr +=
              getPrintStringFromElement(ManufacturerModelName, tmpString);
      ...
      /* Key Object Selection Documents do not contain the SR Document General
       * Module */
      if (getDocumentType() != DT_KeyObjectSelectionDocument) {
        /* preliminary flag */
        if (!PreliminaryFlag.isEmpty())
          stream << "Preliminary Flag    : "
                 << getStringValueFromElement(PreliminaryFlag, tmpString)
                 << OFendl;
        /* completion flag */
        stream << "Completion Flag     : "
               << getStringValueFromElement(CompletionFlag, tmpString)
               << OFendl;
        if (!CompletionFlagDescription.isEmpty())
          stream << "                      "
                 << getPrintStringFromElement(CompletionFlagDescription,
                                              tmpString)
                 << OFendl;
        /* predecessor documents */
        if (!PredecessorDocuments.empty())
          stream << "Predecessor Docs    : "
                 << PredecessorDocuments.getNumberOfInstances() << OFendl;
      }
      /* identical documents */
      if (!IdenticalDocuments.empty())
        stream << "Identical Docs      : "
               << IdenticalDocuments.getNumberOfInstances() << OFendl;
      if (getDocumentType() != DT_KeyObjectSelectionDocument) {
        /* verification flag */
        stream << "Verification Flag   : "
               << getStringValueFromElement(VerificationFlag, tmpString)
               << OFendl;
        /* verifying observer */
        ...
        for (size_t i = 1; i <= obsCount; i++) {
          ...
          if (getVerifyingObserver(i, dateTime, obsName, obsCode, organization)
                  .good()) {
            if (i == 1)
            ...
            if (obsCode.isValid()) {
              stream << " ";
              obsCode.print(stream,
                            (flags & PF_printAllCodes) > 0 /*printCodeValue*/);
            }
      ...
      if (!ContentDate.isEmpty() && !ContentTime.isEmpty()) {
        stream << "Content Date/Time   : "
               << getPrintStringFromElement(ContentDate, tmpString) << " ";
...
OFCondition DSRDocument::checkDatasetForReading(DcmItem& dataset,
  ...
  result = getAndCheckElementFromDataset(dataset, sopClassUID, "1", "1",
                                         "SOPCommonModule");
  if (result.good()) {
    documentType = sopClassUIDToDocumentType(
        getStringValueFromElement(sopClassUID, tmpString));
    if (documentType == DT_invalid) {
      DCMSR_ERROR(
          "SOP Class UID does not match one of the known SR document classes");
  ...
  if (result.good()) {
    ...
    if (result.good()) {
      if (getStringValueFromElement(modality, tmpString) !=
          documentTypeToModality(documentType)) {
        DCMSR_ERROR("Modality does not match '"
                    << documentTypeToModality(documentType) << "' for "
...
OFCondition DSRDocument::read(DcmItem& dataset, const size_t flags) {
  OFCondition result = EC_Normal;
  ...
  if (result.good()) {
    ...
    /* type 3 element and attributes which have already been checked are not
     * checked */
    ...
    if (SpecificCharacterSet.getVM() > 1)
      DCMSR_WARN(
          "Multiple values for Specific Character Set are not supported");
    ...
    if (documentType == DT_KeyObjectSelectionDocument) {
      ...
      getAndCheckElementFromDataset(dataset, SeriesDescription, "1", "3",
                                    "KeyObjectDocumentSeriesModule");
      /* need to check sequence in two steps (avoids additional getAndCheck...
       * method) */
      searchCond =
          getElementFromDataset(dataset, ReferencedPerformedProcedureStep);
    ...
    removeAttributeFromSequence(ReferencedPerformedProcedureStep,
    ...
    if (documentType == DT_KeyObjectSelectionDocument) {
      ...
      getAndCheckElementFromDataset(dataset, ContentTime, "1", "1",
                                    "SRDocumentGeneralModule");
      getAndCheckElementFromDataset(dataset, PreliminaryFlag, "1", "3",
                                    "SRDocumentGeneralModule");
      getAndCheckElementFromDataset(dataset, CompletionFlag, "1", "1",
                                    "SRDocumentGeneralModule");
      getAndCheckElementFromDataset(dataset, CompletionFlagDescription, "1",
                                    "3", "SRDocumentGeneralModule");
    ...
    removeAttributeFromSequence(VerifyingObserver,
                                DCM_DigitalSignaturesSequence);
    removeAttributeFromSequence(PerformedProcedureCode,
                                DCM_MACParametersSequence);
    removeAttributeFromSequence(PerformedProcedureCode,
                                DCM_DigitalSignaturesSequence);
    ...
    if (documentType != DT_KeyObjectSelectionDocument) {
      ...
      if (!PreliminaryFlag.isEmpty()) {
        PreliminaryFlagEnum = enumeratedValueToPreliminaryFlag(
            getStringValueFromElement(PreliminaryFlag, tmpString));
      ...
      CompletionFlagEnum = enumeratedValueToCompletionFlag(
          getStringValueFromElement(CompletionFlag, tmpString));
      ...
      VerificationFlagEnum = enumeratedValueToVerificationFlag(
          getStringValueFromElement(VerificationFlag, tmpString));
    ...
    SpecificCharacterSetEnum = definedTermToCharacterSet(
    ...
    if ((SpecificCharacterSetEnum == CS_invalid) && !tmpString.empty())
      printUnknownValueWarningMessage("SpecificCharacterSet",
                                      tmpString.c_str());
...
OFCondition DSRDocument::write(DcmItem& dataset, DcmStack* markedItems) {
  OFCondition result = EC_Normal;
  ...
  if (isValid()) {
    ...
    addElementToDataset(result, dataset, new DcmUniqueIdentifier(SOPClassUID),
                        "1", "1", "SOPCommonModule");
    addElementToDataset(result, dataset,
                        new DcmUniqueIdentifier(SOPInstanceUID), "1", "1",
                        "SOPCommonModule");
    addElementToDataset(result, dataset,
                        new DcmCodeString(SpecificCharacterSet), "1-n", "1C",
                        "SOPCommonModule");
    addElementToDataset(result, dataset, new DcmDate(InstanceCreationDate), "1",
                        "3", "SOPCommonModule");
    addElementToDataset(result, dataset, new DcmTime(InstanceCreationTime), "1",
                        "3", "SOPCommonModule");
    addElementToDataset(result, dataset,
                        new DcmUniqueIdentifier(InstanceCreatorUID), "1", "3",
                        "SOPCommonModule");
    CodingSchemeIdentification.write(dataset);

    // --- General Study Module ---
    addElementToDataset(result, dataset,
                        new DcmUniqueIdentifier(StudyInstanceUID), "1", "1",
                        "GeneralStudyModule");
    addElementToDataset(result, dataset, new DcmDate(StudyDate), "1", "2",
                        "GeneralStudyModule");
    addElementToDataset(result, dataset, new DcmTime(StudyTime), "1", "2",
                        "GeneralStudyModule");
    addElementToDataset(result, dataset,
                        new DcmPersonName(ReferringPhysicianName), "1", "2",
                        "GeneralStudyModule");
    addElementToDataset(result, dataset, new DcmShortString(StudyID), "1", "2",
                        "GeneralStudyModule");
    addElementToDataset(result, dataset, new DcmShortString(AccessionNumber),
                        "1", "2", "GeneralStudyModule");
    addElementToDataset(result, dataset, new DcmLongString(StudyDescription),
                        "1", "3", "GeneralStudyModule");

    // --- Patient Module ---
    addElementToDataset(result, dataset, new DcmPersonName(PatientName), "1",
                        "2", "PatientModule");
    addElementToDataset(result, dataset, new DcmLongString(PatientID), "1", "2",
                        "PatientModule");
    addElementToDataset(result, dataset, new DcmDate(PatientBirthDate), "1",
                        "2", "PatientModule");
    addElementToDataset(result, dataset, new DcmCodeString(PatientSex), "1",
                        "2", "PatientModule");

    if (requiresEnhancedEquipmentModule(getDocumentType())) {
      // --- Enhanced General Equipment Module ---
      addElementToDataset(result, dataset, new DcmLongString(Manufacturer), "1",
                          "1", "EnhancedGeneralEquipmentModule");
      addElementToDataset(result, dataset,
                          new DcmLongString(ManufacturerModelName), "1", "1",
                          "EnhancedGeneralEquipmentModule");
      addElementToDataset(result, dataset,
                          new DcmLongString(DeviceSerialNumber), "1", "1",
                          "EnhancedGeneralEquipmentModule");
      addElementToDataset(result, dataset, new DcmLongString(SoftwareVersions),
                          "1-n", "1", "EnhancedGeneralEquipmentModule");
    } else {
      // --- General Equipment Module ---
      addElementToDataset(result, dataset, new DcmLongString(Manufacturer), "1",
                          "2", "GeneralEquipmentModule");
      addElementToDataset(result, dataset,
                          new DcmLongString(ManufacturerModelName), "1", "3",
                          "GeneralEquipmentModule");
      addElementToDataset(result, dataset,
                          new DcmLongString(DeviceSerialNumber), "1", "3",
                          "GeneralEquipmentModule");
      addElementToDataset(result, dataset, new DcmLongString(SoftwareVersions),
    ...
    // --- SR Document Series Module / Key Object Document Series Module ---
    if (getDocumentType() == DT_KeyObjectSelectionDocument) {
      addElementToDataset(result, dataset, new DcmCodeString(Modality), "1",
                          "1", "KeyObjectDocumentSeriesModule");
      addElementToDataset(result, dataset,
                          new DcmUniqueIdentifier(SeriesInstanceUID), "1", "1",
                          "KeyObjectDocumentSeriesModule");
      ...
      addElementToDataset(result, dataset, new DcmLongString(SeriesDescription),
                          "1", "3", "KeyObjectDocumentSeriesModule");
      /* always write empty sequence since not yet fully supported */
      ReferencedPerformedProcedureStep.clear();
      addElementToDataset(
          result, dataset,
          new DcmSequenceOfItems(ReferencedPerformedProcedureStep), "1", "2",
          "KeyObjectDocumentSeriesModule");
    } else {
      addElementToDataset(result, dataset, new DcmCodeString(Modality), "1",
                          "1", "SRDocumentSeriesModule");
      addElementToDataset(result, dataset,
                          new DcmUniqueIdentifier(SeriesInstanceUID), "1", "1",
                          "SRDocumentSeriesModule");
      addElementToDataset(result, dataset, new DcmIntegerString(SeriesNumber),
                          "1", "1", "SRDocumentSeriesModule");
      addElementToDataset(result, dataset, new DcmLongString(SeriesDescription),
                          "1", "3", "SRDocumentSeriesModule");
      /* always write empty sequence since not yet fully supported */
      ReferencedPerformedProcedureStep.clear();
      addElementToDataset(
          result, dataset,
    ...
    if (getDocumentType() == DT_KeyObjectSelectionDocument) {
      ...
      addElementToDataset(result, dataset, new DcmDate(ContentDate), "1", "1",
                          "KeyObjectDocumentModule");
      addElementToDataset(result, dataset, new DcmTime(ContentTime), "1", "1",
                          "KeyObjectDocumentModule");
    } else {
      addElementToDataset(result, dataset, new DcmIntegerString(InstanceNumber),
                          "1", "1", "SRDocumentGeneralModule");
      addElementToDataset(result, dataset, new DcmDate(ContentDate), "1", "1",
                          "SRDocumentGeneralModule");
      addElementToDataset(result, dataset, new DcmTime(ContentTime), "1", "1",
                          "SRDocumentGeneralModule");
      addElementToDataset(result, dataset, new DcmCodeString(PreliminaryFlag),
                          "1", "3", "SRDocumentGeneralModule");
      addElementToDataset(result, dataset, new DcmCodeString(CompletionFlag),
                          "1", "1", "SRDocumentGeneralModule");
      addElementToDataset(result, dataset,
                          new DcmLongString(CompletionFlagDescription), "1",
                          "3", "SRDocumentGeneralModule");
      addElementToDataset(result, dataset, new DcmCodeString(VerificationFlag),
                          "1", "1", "SRDocumentGeneralModule");
      if (VerificationFlagEnum == VF_Verified)
        addElementToDataset(result, dataset,
                            new DcmSequenceOfItems(VerifyingObserver), "1-n",
                            "1", "SRDocumentGeneralModule");
      PredecessorDocuments.write(dataset); /* optional */
      /* always write empty sequence since not yet fully supported */
      PerformedProcedureCode.clear();
      addElementToDataset(result, dataset,
                          new DcmSequenceOfItems(PerformedProcedureCode), "1",
                          "2", "SRDocumentGeneralModule");
      if (result.good())
...
OFCondition DSRDocument::readXML(const OFString& filename, const size_t flags) {
  DSRXMLDocument doc;
    ...
    if (result.good()) {
      ...
      if (result.good()) {
        ...
        if (result.good()) {
          ...
          result =
              createNewDocument(sopClassUIDToDocumentType(getSOPClassUID()));
          if (result.good()) {
  ...
  if (doc.valid() && cursor.valid()) {
    ...
    while (cursor.valid() && result.good()) {
      ...
      if (doc.matchNode(cursor, "charset")) {
        /* use "charset" to decode special characters (has to be at the
         * beginning) */
        if (!doc.encodingHandlerValid()) {
          OFString tmpString;
          /* check for known character set */
          setSpecificCharacterSet(
              doc.getStringFromNodeContent(cursor, tmpString));
          const char* encString =
              characterSetToXMLName(SpecificCharacterSetEnum);
          if ((strcmp(encString, "?") == 0) ||
              doc.setEncodingHandler(encString).bad())
        ...
        if (doc.getStringFromNodeContent(cursor, tmpString) !=
            documentTypeToModality(getDocumentType()))
          DCMSR_WARN("Invalid value for 'modality' ... ignoring");
      } else if (doc.matchNode(cursor, "referringphysician")) {
        /* goto sub-element "name" */
        const DSRXMLCursor childNode =
            doc.getNamedNode(cursor.getChild(), "name");
        if (childNode.valid()) {
          /* Referring Physician's Name */
          OFString tmpString;
          DSRPNameTreeNode::getValueFromXMLNodeContent(
              doc, childNode.getChild(), tmpString);
        ...
        if (doc.getStringFromAttribute(cursor, typeString, "type") ==
            "Current Requested Procedure")
          result = CurrentRequestedProcedureEvidence.readXML(
              doc, cursor.getChild(), flags);
        else if (typeString == "Pertinent Other") {
          if (getDocumentType() != DT_KeyObjectSelectionDocument)
            result =
                PertinentOtherEvidence.readXML(doc, cursor.getChild(), flags);
          else
            doc.printUnexpectedNodeWarning(cursor);
        ...
        doc.getElementFromNodeContent(
            doc.getNamedNode(cursor.getChild(), "manufacturer"), Manufacturer,
            NULL, OFTrue /*encoding*/);
        doc.getElementFromNodeContent(
            doc.getNamedNode(cursor.getChild(), "model"), ManufacturerModelName,
            NULL, OFTrue /*encoding*/);
        doc.getElementFromNodeContent(
            doc.getNamedNode(cursor.getChild(), "serial", OFFalse /*required*/),
            DeviceSerialNumber, NULL, OFTrue /*encoding*/);
        doc.getElementFromNodeContent(
            doc.getNamedNode(cursor.getChild(), "version",
                             OFFalse /*required*/),
            SoftwareVersions, NULL, OFTrue /*encoding*/);
      } else if (doc.getElementFromNodeContent(cursor, Manufacturer,
                                               "manufacturer",
                                               OFTrue /*encoding*/)
                     .bad())
        doc.printUnexpectedNodeWarning(cursor);
  ...
  if (cursor.valid()) {
    ...
    while (cursor.valid()) {
      ...
      if (doc.matchNode(cursor, "name")) {
        /* Patient's Name */
        DSRPNameTreeNode::getValueFromXMLNodeContent(doc, cursor.getChild(),
                                                     tmpString);
        PatientName.putString(tmpString.c_str());
      } else if (doc.matchNode(cursor, "birthday")) {
        /* Patient's Birth Date */
        DSRDateTreeNode::getValueFromXMLNodeContent(
            doc, doc.getNamedNode(cursor.getChild(), "date"), tmpString);
        PatientBirthDate.putString(tmpString.c_str());
      } else if (doc.getElementFromNodeContent(cursor, PatientID, "id").bad() &&
                 doc.getElementFromNodeContent(cursor, PatientSex, "sex")
                     .bad()) {
        doc.printUnexpectedNodeWarning(cursor);
      }
      /* proceed with next node */
  ...
  if (cursor.valid()) {
    ...
    while (cursor.valid()) {
      ...
      if (doc.matchNode(cursor, "accession")) {
        ...
        doc.getElementFromNodeContent(
            doc.getNamedNode(cursor.getChild(), "number"), AccessionNumber);
        ...
        StudyTime.putString(tmpString.c_str());
                 ...
                 doc.getElementFromNodeContent(cursor, StudyDescription,
                                               "description",
                                               OFTrue /*encoding*/)
                     .bad()) {
        doc.printUnexpectedNodeWarning(cursor);
  ...
  if (cursor.valid()) {
    ...
    while (cursor.valid()) {
      ...
      if (doc.getElementFromNodeContent(cursor, SeriesNumber, "number").bad() &&
          doc.getElementFromNodeContent(cursor, SeriesDescription,
                                        "description", OFTrue /*encoding*/)
              .bad()) {
        doc.printUnexpectedNodeWarning(cursor);
      }
      /* proceed with next node */
  ...
  if (cursor.valid()) {
    ...
    while (cursor.valid()) {
      ...
      if (doc.matchNode(cursor, "creation")) {
        ...
        DSRTimeTreeNode::getValueFromXMLNodeContent(
            doc, doc.getNamedNode(cursor.getChild(), "time"), tmpString);
        InstanceCreationTime.putString(tmpString.c_str());
      } else if (doc.getElementFromNodeContent(cursor, InstanceNumber, "number")
                     .bad())
        doc.printUnexpectedNodeWarning(cursor);
      /* proceed with next node */
      cursor.gotoNext();
  ...
  if (cursor.valid()) {
    ...
    while (cursor.valid() && result.good()) {
      /* check for known element tags
         ...
         Module) */
      if ((documentType != DT_KeyObjectSelectionDocument) &&
          doc.matchNode(cursor, "preliminary")) {
        /* Preliminary Flag */
        PreliminaryFlagEnum = enumeratedValueToPreliminaryFlag(
            doc.getStringFromAttribute(cursor, tmpString, "flag"));
        if (PreliminaryFlagEnum == PF_invalid)
          printUnknownValueWarningMessage("PreliminaryFlag", tmpString.c_str());
      } else if ((documentType != DT_KeyObjectSelectionDocument) &&
                 doc.matchNode(cursor, "completion")) {
        /* Completion Flag */
        CompletionFlagEnum = enumeratedValueToCompletionFlag(
            doc.getStringFromAttribute(cursor, tmpString, "flag"));
        if (CompletionFlagEnum != CF_invalid) {
          /* Completion Flag Description (optional) */
          const DSRXMLCursor childCursor = doc.getNamedNode(
              cursor.getChild(), "description", OFFalse /*required*/);
          if (childCursor.valid())
            doc.getElementFromNodeContent(childCursor,
                                          CompletionFlagDescription,
                                          NULL /*name*/, OFTrue /*encoding*/);
        } else
          printUnknownValueWarningMessage("CompletionFlag", tmpString.c_str());
      } else if ((documentType != DT_KeyObjectSelectionDocument) &&
                 doc.matchNode(cursor, "verification")) {
        /* Verification Flag */
        VerificationFlagEnum = enumeratedValueToVerificationFlag(
            doc.getStringFromAttribute(cursor, tmpString, "flag"));
        if (VerificationFlagEnum != VF_invalid) {
          ...
          printUnknownValueWarningMessage("VerificationFlag",
                                          tmpString.c_str());
      } else if ((documentType != DT_KeyObjectSelectionDocument) &&
                 doc.matchNode(cursor, "predecessor")) {
        ...
        /* Content Date */
        DSRDateTreeNode::getValueFromXMLNodeContent(
            doc, doc.getNamedNode(childCursor, "date"), tmpString);
        ContentDate.putString(tmpString.c_str());
        /* Content Time */
        DSRTimeTreeNode::getValueFromXMLNodeContent(
            doc, doc.getNamedNode(childCursor, "time"), tmpString);
        ...
        if (ditem != NULL) {
          ...
          while (childCursor.valid()) {
            ...
            if (doc.matchNode(childCursor, "code")) {
              ...
              DSRPNameTreeNode::getValueFromXMLNodeContent(
                  doc, childCursor.getChild(), nameString);
            } else if (doc.matchNode(childCursor, "datetime")) {
              /* Verification Datetime */
              DSRDateTimeTreeNode::getValueFromXMLNodeContent(doc, childCursor,
                                                              datetimeString);
            } else {
              /* Verifying Observer Organization */
              doc.getStringFromNodeContent(childCursor, orgaString,
          ...
          putStringValueToDataset(*ditem, DCM_VerificationDateTime,
                                  datetimeString);
          putStringValueToDataset(*ditem, DCM_VerifyingObserverName,
                                  nameString);
          putStringValueToDataset(*ditem, DCM_VerifyingOrganization,
                                  orgaString);
          /* write code value to sequence item (might be empty, type 2) */
          codeValue.writeSequence(
              *ditem, DCM_VerifyingObserverIdentificationCodeSequence);
  ...
  if (isValid()) {
    ...
    if (!tmpString.empty()) {
      if (tmpString != "?")
        ...
        DCMSR_WARN(
            "Cannot map Specific Character Set to equivalent XML encoding");
    }
    stream << "?>" << OFendl;
    ...
    if (flags & XF_addSchemaReference) {
      ...
      stream << " xmlns:xsi=\"" XML_SCHEMA_INSTANCE_URI "\"" << OFendl
             << "       "
             << " xsi:noNamespaceSchemaLocation=\"" DCMSR_XML_XSD_FILE "\""
             << OFendl << "       ";
    }
    stream << " type=\"" << documentTypeToReadableName(getDocumentType())
           << "\">" << OFendl;

    // --- write some general document information ---

    stream << "<sopclass uid=\""
           << getMarkupStringFromElement(SOPClassUID, tmpString) << "\">";
    /* retrieve name of SOP class */
    const char* sopClass = dcmFindNameOfUID(tmpString.c_str());
    if (sopClass != NULL)
      stream << sopClass;
    stream << "</sopclass>" << OFendl;
    writeStringFromElementToXML(stream, SpecificCharacterSet, "charset",
                                (flags & XF_writeEmptyTags) > 0);
    writeStringFromElementToXML(stream, Modality, "modality",
                                (flags & XF_writeEmptyTags) > 0);
    /* check for additional device information */
    if (!ManufacturerModelName.isEmpty()) {
      stream << "<device>" << OFendl;
      writeStringFromElementToXML(stream, Manufacturer, "manufacturer",
                                  (flags & XF_writeEmptyTags) > 0);
      writeStringFromElementToXML(stream, ManufacturerModelName, "model",
                                  (flags & XF_writeEmptyTags) > 0);
      writeStringFromElementToXML(stream, DeviceSerialNumber, "serial",
                                  (flags & XF_writeEmptyTags) > 0);
      writeStringFromElementToXML(stream, SoftwareVersions, "version",
                                  (flags & XF_writeEmptyTags) > 0);
      stream << "</device>" << OFendl;
    } else
      writeStringFromElementToXML(stream, Manufacturer, "manufacturer",
    ...
    if ((flags & XF_writeEmptyTags) || !ReferringPhysicianName.isEmpty()) {
      ...
      writeStringFromElementToXML(stream, ReferringPhysicianName, "name",
                                  (flags & XF_writeEmptyTags) > 0);
      stream << "</referringphysician>" << OFendl;
    }

    stream << "<patient>" << OFendl;
    writeStringFromElementToXML(stream, PatientID, "id",
                                (flags & XF_writeEmptyTags) > 0);
    writeStringFromElementToXML(stream, PatientName, "name",
                                (flags & XF_writeEmptyTags) > 0);
    if ((flags & XF_writeEmptyTags) || !PatientBirthDate.isEmpty()) {
      stream << "<birthday>" << OFendl;
      PatientBirthDate.getISOFormattedDate(tmpString);
      writeStringValueToXML(stream, tmpString, "date",
                            (flags & XF_writeEmptyTags) > 0);
      stream << "</birthday>" << OFendl;
    }
    writeStringFromElementToXML(stream, PatientSex, "sex",
                                (flags & XF_writeEmptyTags) > 0);
    stream << "</patient>" << OFendl;

    stream << "<study uid=\""
           << getMarkupStringFromElement(StudyInstanceUID, tmpString) << "\">"
           << OFendl;
    writeStringFromElementToXML(stream, StudyID, "id",
                                (flags & XF_writeEmptyTags) > 0);
    StudyDate.getISOFormattedDate(tmpString);
    writeStringValueToXML(stream, tmpString, "date",
                          (flags & XF_writeEmptyTags) > 0);
    StudyTime.getISOFormattedTime(tmpString);
    writeStringValueToXML(stream, tmpString, "time",
                          (flags & XF_writeEmptyTags) > 0);
    if ((flags & XF_writeEmptyTags) || !AccessionNumber.isEmpty()) {
      stream << "<accession>" << OFendl;
      writeStringFromElementToXML(stream, AccessionNumber, "number",
                                  (flags & XF_writeEmptyTags) > 0);
      stream << "</accession>" << OFendl;
    }
    writeStringFromElementToXML(stream, StudyDescription, "description",
                                (flags & XF_writeEmptyTags) > 0);
    stream << "</study>" << OFendl;

    stream << "<series uid=\""
           << getMarkupStringFromElement(SeriesInstanceUID, tmpString) << "\">"
           << OFendl;
    writeStringFromElementToXML(stream, SeriesNumber, "number",
                                (flags & XF_writeEmptyTags) > 0);
    writeStringFromElementToXML(stream, SeriesDescription, "description",
                                (flags & XF_writeEmptyTags) > 0);
    stream << "</series>" << OFendl;

    stream << "<instance uid=\""
           << getMarkupStringFromElement(SOPInstanceUID, tmpString) << "\">"
           << OFendl;
    writeStringFromElementToXML(stream, InstanceNumber, "number",
                                (flags & XF_writeEmptyTags) > 0);
    if ((flags & XF_writeEmptyTags) || !InstanceCreatorUID.isEmpty() ||
      ...
      if (!InstanceCreatorUID.isEmpty())
        stream << " uid=\""
               << getMarkupStringFromElement(InstanceCreatorUID, tmpString)
               << "\"";
      stream << ">" << OFendl;
    ...
    if ((flags & XF_writeEmptyTags) ||
        !CurrentRequestedProcedureEvidence.empty()) {
    ...
    if (getDocumentType() != DT_KeyObjectSelectionDocument) {
      ...
      stream << "<completion flag=\""
             << getStringValueFromElement(CompletionFlag, tmpString) << "\">"
             << OFendl;
      writeStringFromElementToXML(stream, CompletionFlagDescription,
                                  "description",
                                  (flags & XF_writeEmptyTags) > 0);
      ...
      for (size_t i = 1; i <= obsCount; i++) {
        ...
        if (getVerifyingObserver(i, dateTime, obsName, obsCode, organization)
          ...
          DcmDateTime::getISOFormattedDateTimeFromString(
              dateTime, tmpString, OFTrue /*seconds*/, OFFalse /*fraction*/,
              OFFalse /*timeZone*/, OFFalse /*createMissingPart*/,
              "T" /*dateTimeSeparator*/);
          writeStringValueToXML(stream, tmpString, "datetime",
                                (flags & XF_writeEmptyTags) > 0);
          if (!obsName.empty() || (flags & XF_writeEmptyTags))
            stream << "<name>" << OFendl
                   << dicomToXMLPersonName(obsName, tmpString) << OFendl
                   << "</name>" << OFendl;
          if (obsCode.isValid()) {
            if (flags & DSRTypes::XF_codeComponentsAsAttribute)
              stream
                  << "<code";  // bracket ">" is closed in next writeXML() call
            else
              stream << "<code>" << OFendl;
            obsCode.writeXML(stream, flags);
            stream << "</code>" << OFendl;
          }
          writeStringValueToXML(stream, organization, "organization",
                                (flags & XF_writeEmptyTags) > 0);
        }
...
void DSRDocument::renderHTMLPatientData(STD_NAMESPACE ostream& stream,
  ...
  OFString htmlString;
  stream << convertToHTMLString(
      dicomToReadablePersonName(
          getStringValueFromElement(PatientName, tmpString), string2),
      htmlString, flags);
  OFString patientStr;
  if (!PatientSex.isEmpty()) {
  ...
  if (!PatientBirthDate.isEmpty()) {
    if (!patientStr.empty())
      patientStr += ", ";
    patientStr += '*';
    patientStr += dicomToReadableDate(
        getStringValueFromElement(PatientBirthDate, tmpString), string2);
  }
  if (!PatientID.isEmpty()) {
    if (!patientStr.empty())
      patientStr += ", ";
    patientStr += '#';
    patientStr += convertToHTMLString(
        getStringValueFromElement(PatientID, tmpString), htmlString, flags);
  }
  if (!patientStr.empty())
    stream << " (" << patientStr << ")";
}
...
void DSRDocument::renderHTMLReferenceList(STD_NAMESPACE ostream& stream,
  ...
  if (refList.gotoFirstItem().good()) {
    ...
    do {
      ...
      if (!refList.getSOPClassUID(sopClass).empty() &&
          !refList.getSOPInstanceUID(sopInstance).empty()) {
        stream << "<td><a href=\"" << HTML_HYPERLINK_PREFIX_FOR_CGI;
        stream << "?composite=" << sopClass << "+" << sopInstance << "\">";
        stream << documentTypeToDocumentTitle(
            sopClassUIDToDocumentType(sopClass), tmpString);
        stream << "</a></td>" << OFendl;
      } else
...
OFCondition DSRDocument::renderHTML(STD_NAMESPACE ostream& stream,
  ...
  if (isValid()) {
    ...
    if (flags & HF_XHTML11Compatibility)
      ...
      /* fixes for HTML 3.2 */
      newFlags =
          (flags & ~HF_useCodeDetailsTooltip) | HF_convertNonASCIICharacters;
      /* ignore CSS (if any) */
      styleSheet = NULL;
    }
    ...
    if (newFlags & HF_addDocumentTypeReference) {
      if (newFlags & HF_XHTML11Compatibility)
        ...
        if (newFlags & HF_HTML32Compatibility)
          stream << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 3.2//EN\">"
                 << OFendl;
        else
    ...
    if (!(newFlags & HF_HTML32Compatibility)) {
      ...
      if (styleSheet != NULL) {
        if (newFlags & HF_copyStyleSheetContent) {
          ...
          STD_NAMESPACE ifstream cssFile(
              styleSheet, STD_NAMESPACE ios::in | STD_NAMESPACE ios::nocreate);
#else
          ...
          if (cssFile) {
            ...
            DCMSR_WARN("Could not open CSS file \"" << styleSheet
                                                    << "\" ... ignoring");
        } else {
          /* just add a reference to the CSS file (might be an URL) */
          stream << "<link rel=\"stylesheet\" type=\"text/css\" href=\""
                 << styleSheet << "\"";
          if (newFlags & HF_XHTML11Compatibility)
            stream << " /";
          stream << ">" << OFendl;
      ...
      if (!tmpString.empty()) {
        if (tmpString != "?") {
          ...
          DCMSR_WARN(
              "Cannot map Specific Character Set to equivalent HTML charset");
      }
    }
    /* generator meta element referring to the DCMTK */
    if (!(newFlags & HF_omitGeneratorMetaElement)) {
      stream << "<meta name=\"generator\" content=\"OFFIS DCMTK "
             << OFFIS_DCMTK_VERSION << "\"";
      if (newFlags & HF_XHTML11Compatibility)
        stream << " /";
      stream << ">" << OFendl;
    ...
    if (!(newFlags & HF_renderNoDocumentHeader)) {
      ...
      if (!ReferringPhysicianName.isEmpty()) {
        ...
        stream << "<td>"
               << convertToHTMLString(
                      dicomToReadablePersonName(
                          getStringValueFromElement(ReferringPhysicianName,
                                                    tmpString),
                          string2),
                      htmlString, newFlags);
        stream << "</td>" << OFendl;
      ...
      if (!StudyDescription.isEmpty()) {
        ...
        stream << "<td>"
               << convertToHTMLString(
                      getStringValueFromElement(StudyDescription, tmpString),
                      htmlString, newFlags);
        if (!StudyID.isEmpty())
          stream << " (#"
                 << convertToHTMLString(
                        getStringValueFromElement(StudyID, tmpString),
                        htmlString, newFlags)
                 << ")";
        stream << "</td>" << OFendl;
        stream << "</tr>" << OFendl;
      ...
      if (!Manufacturer.isEmpty()) {
        ...
        stream << "<td>"
               << convertToHTMLString(
                      getStringValueFromElement(Manufacturer, tmpString),
                      htmlString, newFlags);
        OFString deviceStr;
        if (!ManufacturerModelName.isEmpty())
          deviceStr += convertToHTMLString(
              getStringValueFromElement(ManufacturerModelName, tmpString),
              htmlString, newFlags);
        if (!DeviceSerialNumber.isEmpty()) {
          if (!deviceStr.empty())
            deviceStr += ", ";
          deviceStr += '#';
          deviceStr += convertToHTMLString(
              getStringValueFromElement(DeviceSerialNumber, tmpString),
              htmlString, newFlags);
        }
        if (!deviceStr.empty())
          stream << " (" << deviceStr << ")";
      ...
      if (getDocumentType() != DT_KeyObjectSelectionDocument) {
        ...
        stream << "<td>" << getStringValueFromElement(CompletionFlag, tmpString)
               << "</td>" << OFendl;
        stream << "</tr>" << OFendl;
        ...
        if (!CompletionFlagDescription.isEmpty()) {
          ...
          stream << "<td>"
                 << convertToHTMLString(
                        getStringValueFromElement(CompletionFlagDescription,
                                                  tmpString),
                        htmlString, newFlags);
          stream << "</td>" << OFendl;
          stream << "</tr>" << OFendl;
      ...
      if (getDocumentType() != DT_KeyObjectSelectionDocument) {
        ...
        for (size_t i = 1; i <= obsCount; i++) {
          ...
          if (getVerifyingObserver(i, dateTime, obsName, obsCode, organization)
            ...
            if (obsCode.isValid() &&
                (newFlags & DSRTypes::HF_useCodeDetailsTooltip)) {
              if (newFlags & HF_XHTML11Compatibility)
                stream << "<span class=\"code\" title=\"(";
              else /* HTML 4.01 */
                stream << "<span class=\"under\" title=\"(";
              stream << convertToHTMLString(obsCode.getCodeValue(), htmlString,
                                            newFlags)
                     << ", ";
              stream << convertToHTMLString(obsCode.getCodingSchemeDesignator(),
                                            htmlString, newFlags);
              if (!obsCode.getCodingSchemeVersion().empty())
                stream << " ["
                       << convertToHTMLString(obsCode.getCodingSchemeVersion(),
                                              htmlString, newFlags)
                       << "]";
              stream << ", &quot;"
                     << convertToHTMLString(obsCode.getCodeMeaning(),
                                            htmlString, newFlags)
                     << "&quot;)\">";
            }
            stream << convertToHTMLString(
                dicomToReadablePersonName(obsName, string2), htmlString,
                newFlags);
            if (obsCode.isValid() &&
                (newFlags & DSRTypes::HF_useCodeDetailsTooltip))
              stream << "</span>";
              ...
              if (obsCode.isValid() &&
                  ((newFlags & HF_renderAllCodes) == HF_renderAllCodes)) {
                stream << " ("
                       << convertToHTMLString(obsCode.getCodeValue(),
                                              htmlString, newFlags);
                stream << ", "
                       << convertToHTMLString(
                              obsCode.getCodingSchemeDesignator(), htmlString,
                              newFlags);
                if (!obsCode.getCodingSchemeVersion().empty())
                  stream << " ["
                         << convertToHTMLString(
                                obsCode.getCodingSchemeVersion(), htmlString,
                                newFlags)
                         << "]";
                stream << ", &quot;"
                       << convertToHTMLString(obsCode.getCodeMeaning(),
            ...
            stream << ", "
                   << convertToHTMLString(organization, htmlString, newFlags);
            stream << "</td>" << OFendl;
            stream << "</tr>" << OFendl;
          }
        }
      ...
      if (!ContentDate.isEmpty() && !ContentTime.isEmpty()) {
        ...
        stream << "<td>"
               << dicomToReadableDate(
                      getStringValueFromElement(ContentDate, tmpString),
                      string2)
               << " ";
        stream << dicomToReadableTime(
    ...
    if (newFlags & HF_renderDcmtkFootnote) {
      ...
      stream << "This page was generated from a DICOM Structured Reporting "
                "document by ";
      stream << "<a href=\"" << DCMTK_INTERNET_URL << "\">OFFIS DCMTK</a> "
             << OFFIS_DCMTK_VERSION << "." << OFendl;
      if (!(newFlags & HF_XHTML11Compatibility))
        stream << "</small>" << OFendl;
      stream << "</div>" << OFendl;
    }
...
OFCondition DSRDocument::setSpecificCharacterSetType(
    const E_CharacterSet characterSet) {
  SpecificCharacterSetEnum = characterSet;
  return SpecificCharacterSet.putString(
      characterSetToDefinedTerm(SpecificCharacterSetEnum));
}
...
const OFString& DSRDocument::getCompletionFlagDescription(
...
OFCondition DSRDocument::getVerifyingObserver(const size_t idx,
  ...
  return getVerifyingObserver(idx, dateTime, observerName, dummyCode,
                              organization);
}
...
OFCondition DSRDocument::getVerifyingObserver(const size_t idx,
  ...
  if ((idx > 0) && (idx <= getNumberOfVerifyingObservers())) {
    ...
    DcmItem* ditem =
        VerifyingObserver.getItem(OFstatic_cast(unsigned long, idx - 1));
    if (ditem != NULL) {
      result =
          getStringValueFromDataset(*ditem, DCM_VerificationDateTime, dateTime);
      if (result.good())
        result = getStringValueFromDataset(*ditem, DCM_VerifyingObserverName,
                                           observerName);
      if (result.good()) {
        /* code is optional (type 2) */
        observerCode.readSequence(
            *ditem, DCM_VerifyingObserverIdentificationCodeSequence,
            "2" /*type*/);
        result = getStringValueFromDataset(*ditem, DCM_VerifyingOrganization,
                                           organization);
      }
      if (result.good()) {
...
DSRDocument::getCurrentRequestedProcedureEvidence() {
...
DSRDocument::getCodingSchemeIdentification() {
...
OFCondition DSRDocument::createRevisedVersion(const OFBool clearList) {
  ...
  if (getDocumentType() != DT_KeyObjectSelectionDocument) {
    ...
    if (CompletionFlagEnum == CF_Complete) {
      ...
      result = PredecessorDocuments.addItem(
          getStringValueFromElement(StudyInstanceUID),
      ...
      if (result.good()) {
        ...
        CompletionFlagDescription.clear();
        /* clear content date/time, will be set automatically in
         * updateAttributes() */
        ContentDate.clear();
        ContentTime.clear();
        ...
        /* create new instance UID, update creation date/time and reset
         * finalized flag */
        createNewSOPInstance();
      }
    }
...
OFCondition DSRDocument::verifyDocument(const OFString& observerName,
  ...
  return verifyDocument(observerName, DSRCodedEntryValue() /*dummy*/,
...
OFCondition DSRDocument::verifyDocument(const OFString& observerName,
  ...
  return verifyDocument(observerName, observerCode, organization,
                        "" /*dateTime*/);
}
...
void DSRDocument::updateAttributes(const OFBool updateAll) {
  ...
  if (getDocumentType() != DT_KeyObjectSelectionDocument) {
    ...
    PreliminaryFlag.putString(
        preliminaryFlagToEnumeratedValue(PreliminaryFlagEnum));
    /* check and adjust completion flag if required */
    if (CompletionFlagEnum == CF_invalid)
      CompletionFlagEnum = CF_Partial;
    CompletionFlag.putString(
        completionFlagToEnumeratedValue(CompletionFlagEnum));
    /* check and adjust verification flag if required */
    if (VerificationFlagEnum == VF_invalid)
      VerificationFlagEnum = VF_Unverified;
    VerificationFlag.putString(
        verificationFlagToEnumeratedValue(VerificationFlagEnum));
  }
}
 ...
 *  element value is empty (e.g. for checking the presence of type 3
 * attributes).
 *
 ...
 *  Moved SeriesDescription (0008,103E) from General Series to SR Document
 * Series Module (according to CP 703).
 *
 ...
 *  Fixed wrong string comparison which caused a compiler error on
 * openSUSE 11.1.
 *
 ...
 *  Changed content and layout of document header in textual dump and HTML
 * output, e.g. added StudyDescription and StudyID.
 *
 ...
 *  requirements. Made readXML() more robust with regard to expected XML
 * structure.
 *
 ...
 *  Moved declaration of loop variable 'i' into the header of the for loop to
 * get rid of warnings reported by Borland C++ 5.5.
 *
 ...
 *  Distinguish more strictly between OFBool and int (required when
 * HAVE_CXX_BOOL is defined).
 *
 ...
 *  Added further checks when reading SR documents (e.g. value of
 * VerificationFlag, CompletionsFlag, ContinuityOfContent and
 ...
 *  series UIDs. Added missing type 2 sequence to dataset. Corrected wrong
 * format of predecessor documents sequence. Changed behaviour of
 * completion/verification flags. Improved HTML and print/dump output.
 *
