  ...
  if (result.good()) {
    ...
    for (unsigned long i = 0; i < sequence.card(); i++) {
      ...
      if (ditem != NULL) {
        ...
        if (getAndCheckStringValueFromDataset(
                *ditem, DCM_CodingSchemeDesignator, codingSchemeDesignator, "1",
                "1", "CodingSchemeIdentificationSequence")
                .good()) {
          /* add new item to the list */
          if (addItem(codingSchemeDesignator, item).good()) {
            /* read additional information */
            getAndCheckStringValueFromDataset(
                *ditem, DCM_CodingSchemeRegistry, item->CodingSchemeRegistry,
                "1", "1C", "CodingSchemeIdentificationSequence");
            getAndCheckStringValueFromDataset(
                *ditem, DCM_CodingSchemeUID, item->CodingSchemeUID, "1", "1C",
                "CodingSchemeIdentificationSequence");
            getAndCheckStringValueFromDataset(
                *ditem, DCM_CodingSchemeExternalID,
                item->CodingSchemeExternalID, "1", "2C",
                "CodingSchemeIdentificationSequence");
            getAndCheckStringValueFromDataset(
                *ditem, DCM_CodingSchemeName, item->CodingSchemeName, "1", "3",
                "CodingSchemeIdentificationSequence");
            getAndCheckStringValueFromDataset(
                *ditem, DCM_CodingSchemeVersion, item->CodingSchemeVersion, "1",
                "3", "CodingSchemeIdentificationSequence");
            getAndCheckStringValueFromDataset(
                *ditem, DCM_ResponsibleOrganization,
                item->ResponsibleOrganization, "1", "3",
                "CodingSchemeIdentificationSequence");
          }
        }
      }
  ...
  while ((iter != last) && result.good()) {
    ...
    if ((item != NULL) && !item->CodingSchemeDesignator.empty()) {
      ...
      result = dataset.findOrCreateSequenceItem(
          DCM_CodingSchemeIdentificationSequence, ditem, -2 /*append new*/);
      /* write item data */
      if (result.good()) {
        putStringValueToDataset(*ditem, DCM_CodingSchemeDesignator,
                                item->CodingSchemeDesignator);
        putStringValueToDataset(*ditem, DCM_CodingSchemeRegistry,
                                item->CodingSchemeRegistry,
                                OFFalse /*allowEmpty*/);
        putStringValueToDataset(*ditem, DCM_CodingSchemeUID,
                                item->CodingSchemeUID, OFFalse /*allowEmpty*/);
        if (item->CodingSchemeUID.empty())
          putStringValueToDataset(*ditem, DCM_CodingSchemeExternalID,
                                  item->CodingSchemeExternalID,
                                  OFFalse /*allowEmpty*/);
        else if (!item->CodingSchemeExternalID.empty()) {
          DCMSR_WARN(
              "Both CodingSchemeUID and CodingSchemeExternalID present for \""
              << item->CodingSchemeDesignator
              << "\", the latter will be ignored");
        }
        putStringValueToDataset(*ditem, DCM_CodingSchemeName,
                                item->CodingSchemeName, OFFalse /*allowEmpty*/);
        putStringValueToDataset(*ditem, DCM_CodingSchemeVersion,
                                item->CodingSchemeVersion,
                                OFFalse /*allowEmpty*/);
        putStringValueToDataset(*ditem, DCM_ResponsibleOrganization,
...
OFCondition DSRCodingSchemeIdentificationList::readXML(
    const DSRXMLDocument& doc, DSRXMLCursor cursor, const size_t /*flags*/) {
  ...
  while (cursor.valid()) {
    ...
    if (doc.checkNode(cursor, "scheme").good()) {
      ...
      if (!doc.getStringFromAttribute(cursor, codingSchemeDesignator,
                                      "designator", OFTrue /*encoding*/)
               .empty()) {
        ...
        if (result.good()) {
          ...
          while (childCursor.valid()) {
            ...
            doc.getStringFromNodeContent(
                childCursor, item->CodingSchemeRegistry, "registry",
                OFTrue /*encoding*/, OFFalse /*clearString*/);
            doc.getStringFromNodeContent(childCursor, item->CodingSchemeUID,
                                         "uid", OFFalse /*encoding*/,
                                         OFFalse /*clearString*/);
            doc.getStringFromNodeContent(
                childCursor, item->CodingSchemeExternalID, "id",
                OFTrue /*encoding*/, OFFalse /*clearString*/);
            doc.getStringFromNodeContent(childCursor, item->CodingSchemeName,
                                         "name", OFTrue /*encoding*/,
                                         OFFalse /*clearString*/);
            doc.getStringFromNodeContent(childCursor, item->CodingSchemeVersion,
                                         "version", OFTrue /*encoding*/,
                                         OFFalse /*clearString*/);
            doc.getStringFromNodeContent(
                childCursor, item->ResponsibleOrganization, "organization",
                OFTrue /*encoding*/, OFFalse /*clearString*/);
...
OFCondition DSRCodingSchemeIdentificationList::writeXML(
    STD_NAMESPACE ostream& stream, const size_t flags) const {
  OFString tmpString;
  /* iterate over all list items */
  OFListConstIterator(ItemStruct*) iter = ItemList.begin();
  const OFListConstIterator(ItemStruct*) last = ItemList.end();
  while (iter != last) {
    ...
    if (item != NULL) {
      stream << "<scheme designator=\""
             << convertToXMLString(item->CodingSchemeDesignator, tmpString)
             << "\">" << OFendl;
      writeStringValueToXML(
          stream, convertToXMLString(item->CodingSchemeRegistry, tmpString),
          "registry", (flags & DSRTypes::XF_writeEmptyTags) > 0);
      writeStringValueToXML(stream, item->CodingSchemeUID, "uid",
                            (flags & DSRTypes::XF_writeEmptyTags) > 0);
      writeStringValueToXML(
          stream, convertToXMLString(item->CodingSchemeExternalID, tmpString),
          "identifier", (flags & DSRTypes::XF_writeEmptyTags) > 0);
      writeStringValueToXML(
          stream, convertToXMLString(item->CodingSchemeName, tmpString), "name",
          (flags & DSRTypes::XF_writeEmptyTags) > 0);
      writeStringValueToXML(
          stream, convertToXMLString(item->CodingSchemeVersion, tmpString),
          "version", (flags & DSRTypes::XF_writeEmptyTags) > 0);
      writeStringValueToXML(
          stream, convertToXMLString(item->ResponsibleOrganization, tmpString),
          "organization", (flags & DSRTypes::XF_writeEmptyTags) > 0);
...
OFCondition DSRCodingSchemeIdentificationList::addPrivateDcmtkCodingScheme() {
  ...
  if (result.good()) {
    ...
    item->CodingSchemeVersion
        .clear();  // there are currently no different versions
    item->ResponsibleOrganization = OFFIS_RESPONSIBLE_ORGANIZATION;
  }
  return result;
}

OFCondition DSRCodingSchemeIdentificationList::addItem(
    const OFString& codingSchemeDesignator, ItemStruct*& item) {
  ...
  if (!codingSchemeDesignator.empty()) {
    ...
    if (gotoItem(codingSchemeDesignator).bad()) {
      ...
      DCMSR_WARN("CodingSchemeDesignator \""
                 << codingSchemeDesignator
                 << "\" already exists in CodingSchemeIdentificationSequence "
                    "... overwriting");
...
OFCondition DSRCodingSchemeIdentificationList::gotoItem(
    const OFString& codingSchemeDesignator) {
  OFCondition result = EC_IllegalParameter;
  /* check parameter first */
  if (!codingSchemeDesignator.empty()) {
    ...
    while ((Iterator != last) && result.bad()) {
      if ((*Iterator != NULL) &&
          (OFstatic_cast(ItemStruct*, *Iterator)->CodingSchemeDesignator ==
           codingSchemeDesignator))
        result = EC_Normal;
      else
        ++Iterator;
...
DSRCodingSchemeIdentificationList::ItemStruct*
DSRCodingSchemeIdentificationList::getCurrentItem() const {
  ItemStruct* item = NULL;
...
const OFString& DSRCodingSchemeIdentificationList::getCodingSchemeDesignator(
...
const OFString& DSRCodingSchemeIdentificationList::getCodingSchemeRegistry(
    OFString& stringValue) const {
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
  /* get requested value or clear string if invalid */
...
const OFString& DSRCodingSchemeIdentificationList::getCodingSchemeUID(
    OFString& stringValue) const {
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
  /* get requested value or clear string if invalid */
...
const OFString& DSRCodingSchemeIdentificationList::getCodingSchemeExternalID(
    OFString& stringValue) const {
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
  /* get requested value or clear string if invalid */
...
const OFString& DSRCodingSchemeIdentificationList::getCodingSchemeName(
    OFString& stringValue) const {
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
  /* get requested value or clear string if invalid */
...
const OFString& DSRCodingSchemeIdentificationList::getCodingSchemeVersion(
    OFString& stringValue) const {
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
  /* get requested value or clear string if invalid */
...
const OFString& DSRCodingSchemeIdentificationList::getResponsibleOrganization(
    OFString& stringValue) const {
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
  /* get requested value or clear string if invalid */
...
OFCondition DSRCodingSchemeIdentificationList::setCodingSchemeRegistry(
...
OFCondition DSRCodingSchemeIdentificationList::setCodingSchemeUID(
    const OFString& value) {
  OFCondition result = EC_IllegalCall;
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
...
OFCondition DSRCodingSchemeIdentificationList::setCodingSchemeExternalID(
    const OFString& value) {
  OFCondition result = EC_IllegalCall;
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
...
OFCondition DSRCodingSchemeIdentificationList::setCodingSchemeName(
    const OFString& value) {
  OFCondition result = EC_IllegalCall;
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
...
OFCondition DSRCodingSchemeIdentificationList::setCodingSchemeVersion(
    const OFString& value) {
  OFCondition result = EC_IllegalCall;
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
...
OFCondition DSRCodingSchemeIdentificationList::setResponsibleOrganization(
    const OFString& value) {
  OFCondition result = EC_IllegalCall;
  /* check whether current item is valid */
  ItemStruct* item = getCurrentItem();
 ...
 *  Minor const iterator related changes needed to compile with VC6 with
 * HAVE_STL
 *
