...
DSRCompositeReferenceValue& DSRCompositeReferenceValue::operator=(
    const DSRCompositeReferenceValue& referenceValue) {
  /* do not check since this would unexpected to the user */
  SOPClassUID = referenceValue.SOPClassUID;
  SOPInstanceUID = referenceValue.SOPInstanceUID;
...
OFCondition DSRCompositeReferenceValue::readXML(const DSRXMLDocument& doc,
  ...
  if (cursor.gotoChild().valid()) {
    ...
    doc.getStringFromAttribute(doc.getNamedNode(cursor, "sopclass"),
                               SOPClassUID, "uid");
    doc.getStringFromAttribute(doc.getNamedNode(cursor, "instance"),
                               SOPInstanceUID, "uid");
...
OFCondition DSRCompositeReferenceValue::readItem(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckStringValueFromDataset(
      dataset, DCM_ReferencedSOPClassUID, SOPClassUID, "1", "1",
      "ReferencedSOPSequence");
  /* read ReferencedSOPInstanceUID */
  if (result.good())
    result = DSRTypes::getAndCheckStringValueFromDataset(
        dataset, DCM_ReferencedSOPInstanceUID, SOPInstanceUID, "1", "1",
        "ReferencedSOPSequence");
  return result;
}
...
OFCondition DSRCompositeReferenceValue::writeItem(DcmItem& dataset) const {
  ...
  OFCondition result = DSRTypes::putStringValueToDataset(
      dataset, DCM_ReferencedSOPClassUID, SOPClassUID);
  /* write ReferencedSOPInstanceUID */
  if (result.good())
    result = DSRTypes::putStringValueToDataset(
        dataset, DCM_ReferencedSOPInstanceUID, SOPInstanceUID);
  return result;
}
...
OFCondition DSRCompositeReferenceValue::renderHTML(
...
OFCondition DSRCompositeReferenceValue::getValue(
    DSRCompositeReferenceValue& referenceValue) const {
  referenceValue = *this;
  return EC_Normal;
}

OFCondition DSRCompositeReferenceValue::setValue(
    const DSRCompositeReferenceValue& referenceValue) {
  return setReference(referenceValue.SOPClassUID,
                      referenceValue.SOPInstanceUID);
}

OFCondition DSRCompositeReferenceValue::setReference(
    const OFString& sopClassUID, const OFString& sopInstanceUID) {
...
OFCondition DSRCompositeReferenceValue::setSOPClassUID(
...
OFCondition DSRCompositeReferenceValue::setSOPInstanceUID(
...
OFBool DSRCompositeReferenceValue::checkSOPClassUID(
    const OFString& sopClassUID) const {
  return DSRTypes::checkForValidUIDFormat(sopClassUID);
}

OFBool DSRCompositeReferenceValue::checkSOPInstanceUID(
