...
DSRSOPInstanceReferenceList::InstanceStruct::InstanceStruct(
    const OFString& sopClassUID, const OFString& instanceUID)
    : SOPClassUID(sopClassUID), InstanceUID(instanceUID) {}
...
OFCondition DSRSOPInstanceReferenceList::SeriesStruct::read(DcmItem& dataset) {
  ...
  getAndCheckStringValueFromDataset(dataset, DCM_RetrieveAETitle,
                                    RetrieveAETitle, "1-n", "3",
                                    "ReferencedSeriesSequence");
  getAndCheckStringValueFromDataset(dataset, DCM_StorageMediaFileSetID,
                                    StorageMediaFileSetID, "1", "3",
                                    "ReferencedSeriesSequence");
  getAndCheckStringValueFromDataset(dataset, DCM_StorageMediaFileSetUID,
                                    StorageMediaFileSetUID, "1", "3",
                                    "ReferencedSeriesSequence");
  ...
  if (result.good()) {
    ...
    for (unsigned long i = 0; i < sequence.card(); i++) {
      ...
      if (item != NULL) {
        ...
        if (getAndCheckStringValueFromDataset(*item, DCM_ReferencedSOPClassUID,
                                              sopClassUID, "1", "1",
                                              "ReferencedSOPSequence")
                .good() &&
            getAndCheckStringValueFromDataset(
                *item, DCM_ReferencedSOPInstanceUID, instanceUID, "1", "1",
                "ReferencedSOPSequence")
                .good()) {
          ...
          if (instance == NULL) {
            ...
            DCMSR_WARN("SOP Instance \""
                       << instanceUID
                       << "\" already exists in reference list ... ignoring");
          }
        }
      }
    }
...
OFCondition DSRSOPInstanceReferenceList::SeriesStruct::write(
  ...
  if (!StorageMediaFileSetID.empty())
    dataset.putAndInsertOFStringArray(DCM_StorageMediaFileSetID,
                                      StorageMediaFileSetID);
  if (!StorageMediaFileSetUID.empty())
    dataset.putAndInsertOFStringArray(DCM_StorageMediaFileSetUID,
                                      StorageMediaFileSetUID);
  ...
  while ((iter != last) && result.good()) {
    ...
    if (instance != NULL) {
      ...
      result = dataset.findOrCreateSequenceItem(DCM_ReferencedSOPSequence, item,
                                                -2 /*append new*/);
      if (result.good()) {
...
OFCondition DSRSOPInstanceReferenceList::SeriesStruct::readXML(
  ...
  if (cursor.valid()) {
    ...
    doc.getStringFromNodeContent(
        doc.getNamedNode(cursor, "aetitle", OFFalse /*required*/),
        RetrieveAETitle);
    const DSRXMLCursor childCursor =
        doc.getNamedNode(cursor, "fileset", OFFalse /*required*/);
    if (childCursor.valid()) {
      doc.getStringFromAttribute(childCursor, StorageMediaFileSetUID, "uid",
    ...
    do {
      ...
      if (cursor.valid()) {
        if (!doc.getStringFromAttribute(
                    doc.getNamedNode(cursor.getChild(), "sopclass"),
                    sopClassUID, "uid")
                 .empty() &&
            !doc.getStringFromAttribute(
                    doc.getNamedNode(cursor.getChild(), "instance"),
                    instanceUID, "uid")
                 .empty()) {
          /* check whether instance item already exists,
             because the internal structure is organized in a strictly
             hierarchical manner  */
          InstanceStruct* instance = gotoInstance(instanceUID);
          if (instance == NULL) {
    ...
    if (result.bad())
      DCMSR_WARN("Series \"" << SeriesUID
                             << "\" empty in reference list ... ignoring");
  }
  return result;
}
...
OFCondition DSRSOPInstanceReferenceList::SeriesStruct::writeXML(
  ...
  writeStringValueToXML(stream, RetrieveAETitle, "aetitle",
                        (flags & XF_writeEmptyTags) > 0);
  if ((flags & XF_writeEmptyTags) || !StorageMediaFileSetUID.empty() ||
      !StorageMediaFileSetID.empty()) {
    stream << "<fileset";
    if (!StorageMediaFileSetUID.empty())
...
DSRSOPInstanceReferenceList::InstanceStruct*
DSRSOPInstanceReferenceList::SeriesStruct::gotoInstance(
  ...
  if ((Iterator != InstanceList.end()) && (*Iterator != NULL) &&
      ((*Iterator)->InstanceUID == instanceUID))
    instance = *Iterator;
  else {
    ...
    while ((Iterator != last) &&
           ((*Iterator == NULL) || ((*Iterator)->InstanceUID != instanceUID)))
      Iterator++;
    /* item found */
    if (Iterator != last)
      instance = *Iterator;
...
OFCondition DSRSOPInstanceReferenceList::SeriesStruct::addItem(
    const OFString& sopClassUID, const OFString& instanceUID) {
  OFCondition result = EC_Normal;
...
DSRSOPInstanceReferenceList::StudyStruct::StudyStruct(const OFString& studyUID)
    : StudyUID(studyUID), SeriesList(), Iterator() {
  /* initialize list cursor */
  ...
  if (result.good()) {
    ...
    for (unsigned long i = 0; i < sequence.card(); i++) {
      ...
      if (item != NULL) {
        ...
        OFString seriesUID;
        if (getAndCheckStringValueFromDataset(*item, DCM_SeriesInstanceUID,
                                              seriesUID, "1", "1",
                                              "ReferencedSeriesSequence")
                .good()) {
...
OFCondition DSRSOPInstanceReferenceList::StudyStruct::write(
    DcmItem& dataset) const {
  OFCondition result = EC_Normal;
  ...
  while ((iter != last) && result.good()) {
    ...
    if (series != NULL) {
      DcmItem* item = NULL;
      /* create a new item (and a sequence if required) */
      result = dataset.findOrCreateSequenceItem(DCM_ReferencedSeriesSequence,
                                                item, -2 /*append new*/);
...
OFCondition DSRSOPInstanceReferenceList::StudyStruct::readXML(
    const DSRXMLDocument& doc, DSRXMLCursor cursor) {
  OFCondition result = SR_EC_InvalidDocument;
  if (cursor.valid()) {
    OFString seriesUID;
...
DSRSOPInstanceReferenceList::SeriesStruct*
DSRSOPInstanceReferenceList::StudyStruct::gotoSeries(
    const OFString& seriesUID) {
  SeriesStruct* series = NULL;
  /* first, check whether the current series is the one we're searching for */
  if ((Iterator != SeriesList.end()) && (*Iterator != NULL) &&
      (OFstatic_cast(SeriesStruct*, *Iterator)->SeriesUID == seriesUID))
...
DSRSOPInstanceReferenceList::InstanceStruct*
DSRSOPInstanceReferenceList::StudyStruct::gotoInstance(
    const OFString& instanceUID) {
  InstanceStruct* instance = NULL;
  /* start with the first list item */
  Iterator = SeriesList.begin();
...
OFCondition DSRSOPInstanceReferenceList::StudyStruct::addItem(
    const OFString& seriesUID, const OFString& sopClassUID,
    const OFString& instanceUID) {
  OFCondition result = EC_Normal;
  /* check whether series already exists */
  SeriesStruct* series = gotoSeries(seriesUID);
...
DSRSOPInstanceReferenceList::DSRSOPInstanceReferenceList(
    ...
    : SequenceTag(sequence), StudyList(), Iterator() {
  /* initialize list cursor */
  ...
  if (result.good()) {
    ...
    for (unsigned long i = 0; i < sequence.card(); i++) {
      ...
      if (item != NULL) {
        ...
        OFString studyUID;
        if (getAndCheckStringValueFromDataset(*item, DCM_StudyInstanceUID,
                                              studyUID, "1", "1",
                                              sequenceName.c_str())
                .good()) {
...
OFCondition DSRSOPInstanceReferenceList::write(DcmItem& dataset) const {
  ...
  while ((iter != last) && result.good()) {
    ...
    if (study != NULL) {
      DcmItem* item = NULL;
      /* create a new item (and a sequence if required) */
      result = dataset.findOrCreateSequenceItem(SequenceTag, item,
                                                -2 /*append new*/);
...
DSRSOPInstanceReferenceList::StudyStruct*
DSRSOPInstanceReferenceList::gotoStudy(const OFString& studyUID) {
  StudyStruct* study = NULL;
  /* first, check whether the current study is the one we're searching for */
  if ((Iterator != StudyList.end()) && (*Iterator != NULL) &&
      (OFstatic_cast(StudyStruct*, *Iterator)->StudyUID == studyUID))
...
OFCondition DSRSOPInstanceReferenceList::addItem(const OFString& studyUID,
  ...
  if (!studyUID.empty() && !seriesUID.empty() && !sopClassUID.empty() &&
      !instanceUID.empty()) {
    result = EC_Normal;
    StudyStruct* study = gotoStudy(studyUID);
...
OFCondition DSRSOPInstanceReferenceList::removeItem(
    const OFString& studyUID, const OFString& seriesUID,
    const OFString& instanceUID) {
  /* goto specified item ... */
...
DSRSOPInstanceReferenceList::StudyStruct*
DSRSOPInstanceReferenceList::getCurrentStudy() const {
  StudyStruct* study = NULL;
...
DSRSOPInstanceReferenceList::SeriesStruct*
DSRSOPInstanceReferenceList::getCurrentSeries() const {
  SeriesStruct* series = NULL;
...
DSRSOPInstanceReferenceList::InstanceStruct*
DSRSOPInstanceReferenceList::getCurrentInstance() const {
  InstanceStruct* instance = NULL;
...
const OFString& DSRSOPInstanceReferenceList::getStudyInstanceUID(
...
const OFString& DSRSOPInstanceReferenceList::getSeriesInstanceUID(
    OFString& stringValue) const {
  /* check whether current series is valid */
  SeriesStruct* series = getCurrentSeries();
...
const OFString& DSRSOPInstanceReferenceList::getSOPInstanceUID(
    OFString& stringValue) const {
  /* check whether current instance is valid */
...
const OFString& DSRSOPInstanceReferenceList::getSOPClassUID(
    OFString& stringValue) const {
  /* check whether current instance is valid */
  InstanceStruct* instance = getCurrentInstance();
...
const OFString& DSRSOPInstanceReferenceList::getRetrieveAETitle(
    OFString& stringValue) const {
  /* check whether current series is valid */
  SeriesStruct* series = getCurrentSeries();
...
const OFString& DSRSOPInstanceReferenceList::getStorageMediaFileSetID(
    OFString& stringValue) const {
  /* check whether current series is valid */
  SeriesStruct* series = getCurrentSeries();
...
const OFString& DSRSOPInstanceReferenceList::getStorageMediaFileSetUID(
    OFString& stringValue) const {
  /* check whether current series is valid */
  SeriesStruct* series = getCurrentSeries();
...
OFCondition DSRSOPInstanceReferenceList::setRetrieveAETitle(
...
OFCondition DSRSOPInstanceReferenceList::setStorageMediaFileSetID(
    const OFString& value) {
  OFCondition result = EC_IllegalCall;
  /* check whether current series is valid */
  SeriesStruct* series = getCurrentSeries();
...
OFCondition DSRSOPInstanceReferenceList::setStorageMediaFileSetUID(
    const OFString& value) {
  OFCondition result = EC_IllegalCall;
  /* check whether current series is valid */
  SeriesStruct* series = getCurrentSeries();
