...
DSRPNameTreeNode::DSRPNameTreeNode(const E_RelationshipType relationshipType)
    : DSRDocumentTreeNode(relationshipType, VT_PName), DSRStringValue() {}
...
OFBool DSRPNameTreeNode::isValid() const {
  ...
  return DSRDocumentTreeNode::isValid() && DSRStringValue::isValid() &&
         getConceptName().isValid();
}
...
OFCondition DSRPNameTreeNode::writeXML(STD_NAMESPACE ostream& stream,
  ...
  if (!getValue().empty() || (flags & XF_writeEmptyTags)) {
    OFString tmpString;
    stream << "<value>" << OFendl << dicomToXMLPersonName(getValue(), tmpString)
           << OFendl << "</value>" << OFendl;
  }
...
OFString& DSRPNameTreeNode::getValueFromXMLNodeContent(
    const DSRXMLDocument& doc, DSRXMLCursor cursor, OFString& nameValue) {
  nameValue.clear();
  ...
  if (cursor.valid()) {
    ...
    while (cursor.valid()) {
      ...
      doc.getStringFromNodeContent(cursor, prefix, "prefix",
                                   OFTrue /*encoding*/,
                                   OFFalse /*clearString*/);
      doc.getStringFromNodeContent(cursor, first, "first", OFTrue /*encoding*/,
                                   OFFalse /*clearString*/);
      doc.getStringFromNodeContent(cursor, middle, "middle",
                                   OFTrue /*encoding*/,
                                   OFFalse /*clearString*/);
      doc.getStringFromNodeContent(cursor, last, "last", OFTrue /*encoding*/,
                                   OFFalse /*clearString*/);
      doc.getStringFromNodeContent(cursor, suffix, "suffix",
                                   OFTrue /*encoding*/,
                                   OFFalse /*clearString*/);
    ...
    DcmPersonName::getStringFromNameComponents(last, first, middle, prefix,
                                               suffix, nameValue);
  }
  return nameValue;
}

OFCondition DSRPNameTreeNode::renderHTMLContentItem(
  ...
  if (result.good()) {
    ...
    docStream << convertToHTMLString(
        dicomToReadablePersonName(getValue(), tmpString), htmlString, flags);
    if (!(flags & DSRTypes::HF_renderItemsSeparately)) {
