...
OFBool DSRXRayRadiationDoseSRConstraintChecker::isTemplateSupportRequired()
    const {
  return OFFalse;
}

const char* DSRXRayRadiationDoseSRConstraintChecker::getRootTemplateIdentifier()
    const {
  return NULL;
}

DSRTypes::E_DocumentType
DSRXRayRadiationDoseSRConstraintChecker::getDocumentType() const {
  return DT_XRayRadiationDoseSR;
}

OFBool DSRXRayRadiationDoseSRConstraintChecker::checkContentRelationship(
    const E_ValueType sourceValueType,
    const E_RelationshipType relationshipType,
    const E_ValueType targetValueType, const OFBool byReference) const {
  /* the following code implements the constraints of table A.35.8-2 in DICOM
   * PS3.3 */
  ...
  if (!byReference) {
    ...
    if ((relationshipType == RT_contains) &&
      ...
      result =
          ...
          (targetValueType == VT_Image) || (targetValueType == VT_Composite) ||
    ...
    /* row 7 of the table */
      ...
      result =
          (targetValueType == VT_Text) || (targetValueType == VT_Code) ||
          (targetValueType == VT_Num) || (targetValueType == VT_DateTime) ||
          (targetValueType == VT_UIDRef) || (targetValueType == VT_Image) ||
          (targetValueType == VT_Composite) ||
          (targetValueType == VT_Container);
    }
  }
