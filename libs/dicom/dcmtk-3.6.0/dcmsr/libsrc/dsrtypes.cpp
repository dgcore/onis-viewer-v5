...
const size_t DSRTypes::HF_internalUseOnly =
    DSRTypes::HF_renderItemsSeparately | DSRTypes::HF_renderItemInline |
...
/* shortcuts */
const size_t DSRTypes::XF_encodeEverythingAsAttribute =
    DSRTypes::XF_codeComponentsAsAttribute |
...
// conditions
const OFConditionConst ECC_UnknownDocumentType(OFM_dcmsr, 1, OF_error,
                                               "Unknown Document Type");
const OFConditionConst ECC_InvalidDocument(OFM_dcmsr, 2, OF_error,
                                           "Invalid Document");
const OFConditionConst ECC_InvalidDocumentTree(OFM_dcmsr, 3, OF_error,
                                               "Invalid Document Tree");
const OFConditionConst ECC_MandatoryAttributeMissing(
    OFM_dcmsr, 4, OF_error, "Mandatory Attribute missing");
const OFConditionConst ECC_InvalidValue(OFM_dcmsr, 5, OF_error,
                                        "Invalid Value");
const OFConditionConst ECC_UnsupportedValue(OFM_dcmsr, 6, OF_error,
                                            "Unsupported Value");
const OFConditionConst ECC_UnknownValueType(OFM_dcmsr, 7, OF_error,
                                            "Unknown Value Type");
const OFConditionConst ECC_UnknownRelationshipType(OFM_dcmsr, 8, OF_error,
                                                   "Unknown Relationship Type");
const OFConditionConst ECC_InvalidByValueRelationship(
    OFM_dcmsr, 9, OF_error, "Invalid by-value Relationship");
const OFConditionConst ECC_InvalidByReferenceRelationship(
    OFM_dcmsr, 10, OF_error, "Invalid by-reference Relationship");
const OFConditionConst ECC_SOPInstanceNotFound(OFM_dcmsr, 11, OF_error,
                                               "SOP Instance not found");
const OFConditionConst ECC_DifferentSOPClassesForAnInstance(
    OFM_dcmsr, 12, OF_error, "Different SOP Classes for an Instance");
const OFConditionConst ECC_CodingSchemeNotFound(
    OFM_dcmsr, 13, OF_error, "Coding Scheme Designator not found");
const OFConditionConst ECC_CorruptedXMLStructure(OFM_dcmsr, 14, OF_error,
...
const OFCondition SR_EC_MandatoryAttributeMissing(
    ECC_MandatoryAttributeMissing);
...
const OFCondition SR_EC_InvalidByValueRelationship(
    ECC_InvalidByValueRelationship);
const OFCondition SR_EC_InvalidByReferenceRelationship(
    ECC_InvalidByReferenceRelationship);
const OFCondition SR_EC_SOPInstanceNotFound(ECC_SOPInstanceNotFound);
const OFCondition SR_EC_DifferentSOPClassesForAnInstance(
    ECC_DifferentSOPClassesForAnInstance);
...
static const S_DocumentTypeNameMap DocumentTypeNameMap[] = {
    ...
    {DSRTypes::DT_BasicTextSR, UID_BasicTextSRStorage, OFFalse, "SR",
     "Basic Text SR"},
    {DSRTypes::DT_EnhancedSR, UID_EnhancedSRStorage, OFFalse, "SR",
     "Enhanced SR"},
    {DSRTypes::DT_ComprehensiveSR, UID_ComprehensiveSRStorage, OFFalse, "SR",
     "Comprehensive SR"},
    {DSRTypes::DT_KeyObjectSelectionDocument,
     UID_KeyObjectSelectionDocumentStorage, OFFalse, "KO",
     "Key Object Selection Document"},
    {DSRTypes::DT_MammographyCadSR, UID_MammographyCADSRStorage, OFFalse, "SR",
     "Mammography CAD SR"},
    {DSRTypes::DT_ChestCadSR, UID_ChestCADSRStorage, OFFalse, "SR",
     "Chest CAD SR"},
    {DSRTypes::DT_ColonCadSR, UID_ColonCADSRStorage, OFTrue, "SR",
     "Colon CAD SR"},
    {DSRTypes::DT_ProcedureLog, UID_ProcedureLogStorage, OFFalse, "SR",
     "Procedure Log"},
    {DSRTypes::DT_XRayRadiationDoseSR, UID_XRayRadiationDoseSRStorage, OFTrue,
     "SR", "X-Ray Radiation Dose SR"},
    {DSRTypes::DT_SpectaclePrescriptionReport,
     UID_SpectaclePrescriptionReportStorage, OFTrue, "SR",
     "Spectacle Prescription Report"},
    {DSRTypes::DT_MacularGridThicknessAndVolumeReport,
     UID_MacularGridThicknessAndVolumeReportStorage, OFTrue, "SR",
     "Macular Grid Thickness and Volume Report"},
    {DSRTypes::DT_ImplantationPlanSRDocument,
     UID_ImplantationPlanSRDocumentStorage, OFTrue, "SR",
...
static const S_ValueTypeNameMap ValueTypeNameMap[] = {
    ...
    {DSRTypes::VT_byReference, "(by-reference)", "reference",
     "(by-reference)"}};
...
const char* DSRTypes::documentTypeToSOPClassUID(
...
const char* DSRTypes::documentTypeToModality(
    const E_DocumentType documentType) {
  const S_DocumentTypeNameMap* iterator = DocumentTypeNameMap;
  while ((iterator->Type != DT_last) && (iterator->Type != documentType))
    iterator++;
...
const char* DSRTypes::documentTypeToReadableName(
    const E_DocumentType documentType) {
  const S_DocumentTypeNameMap* iterator = DocumentTypeNameMap;
  while ((iterator->Type != DT_last) && (iterator->Type != documentType))
    iterator++;
...
const char* DSRTypes::documentTypeToDocumentTitle(
  ...
  if (!documentTitle.empty() &&
      (documentTitle.find("Document") == OFString_npos) &&
...
OFBool DSRTypes::requiresEnhancedEquipmentModule(
    const E_DocumentType documentType) {
  const S_DocumentTypeNameMap* iterator = DocumentTypeNameMap;
  while ((iterator->Type != DT_last) && (iterator->Type != documentType))
    iterator++;
...
const char* DSRTypes::relationshipTypeToDefinedTerm(
...
const char* DSRTypes::relationshipTypeToReadableName(
    const E_RelationshipType relationshipType) {
  const S_RelationshipTypeNameMap* iterator = RelationshipTypeNameMap;
  while ((iterator->Type != RT_last) && (iterator->Type != relationshipType))
    iterator++;
...
const char* DSRTypes::graphicTypeToEnumeratedValue(
...
const char* DSRTypes::graphicTypeToReadableName(
    const E_GraphicType graphicType) {
  const S_GraphicTypeNameMap* iterator = GraphicTypeNameMap;
  while ((iterator->Type != GT_last) && (iterator->Type != graphicType))
    iterator++;
...
const char* DSRTypes::graphicType3DToEnumeratedValue(
...
const char* DSRTypes::graphicType3DToReadableName(
    const E_GraphicType3D graphicType) {
  const S_GraphicType3DNameMap* iterator = GraphicType3DNameMap;
  while ((iterator->Type != GT3_last) && (iterator->Type != graphicType))
    iterator++;
...
const char* DSRTypes::temporalRangeTypeToEnumeratedValue(
...
const char* DSRTypes::temporalRangeTypeToReadableName(
    const E_TemporalRangeType temporalRangeType) {
  const S_TemporalRangeTypeNameMap* iterator = TemporalRangeTypeNameMap;
  while ((iterator->Type != TRT_last) && (iterator->Type != temporalRangeType))
    iterator++;
...
const char* DSRTypes::continuityOfContentToEnumeratedValue(
    const E_ContinuityOfContent continuityOfContent) {
  const S_ContinuityOfContentNameMap* iterator = ContinuityOfContentNameMap;
  while ((iterator->Type != COC_last) &&
         (iterator->Type != continuityOfContent))
    iterator++;
  return iterator->EnumeratedValue;
}

const char* DSRTypes::preliminaryFlagToEnumeratedValue(
...
const char* DSRTypes::completionFlagToEnumeratedValue(
...
const char* DSRTypes::verificationFlagToEnumeratedValue(
...
const char* DSRTypes::characterSetToDefinedTerm(
...
const char* DSRTypes::characterSetToHTMLName(
    const E_CharacterSet characterSet) {
  const S_CharacterSetNameMap* iterator = CharacterSetNameMap;
  while ((iterator->Type != CS_last) && (iterator->Type != characterSet))
    iterator++;
...
DSRTypes::E_DocumentType DSRTypes::sopClassUIDToDocumentType(
...
DSRTypes::E_RelationshipType DSRTypes::definedTermToRelationshipType(
...
DSRTypes::E_ValueType DSRTypes::definedTermToValueType(
    const OFString& definedTerm) {
...
DSRTypes::E_ValueType DSRTypes::xmlTagNameToValueType(
...
DSRTypes::E_GraphicType DSRTypes::enumeratedValueToGraphicType(
    const OFString& enumeratedValue) {
  E_GraphicType type = GT_invalid;
  const S_GraphicTypeNameMap* iterator = GraphicTypeNameMap;
  while ((iterator->Type != GT_last) &&
         (enumeratedValue != iterator->EnumeratedValue))
    iterator++;
...
DSRTypes::E_GraphicType3D DSRTypes::enumeratedValueToGraphicType3D(
    const OFString& enumeratedValue) {
  E_GraphicType3D type = GT3_invalid;
  const S_GraphicType3DNameMap* iterator = GraphicType3DNameMap;
  while ((iterator->Type != GT3_last) &&
         (enumeratedValue != iterator->EnumeratedValue))
    iterator++;
  if (enumeratedValue == iterator->EnumeratedValue)
    type = iterator->Type;
...
DSRTypes::E_TemporalRangeType DSRTypes::enumeratedValueToTemporalRangeType(
    const OFString& enumeratedValue) {
  E_TemporalRangeType type = TRT_invalid;
  const S_TemporalRangeTypeNameMap* iterator = TemporalRangeTypeNameMap;
  while ((iterator->Type != TRT_last) &&
         (enumeratedValue != iterator->EnumeratedValue))
    iterator++;
  if (enumeratedValue == iterator->EnumeratedValue)
    type = iterator->Type;
...
DSRTypes::E_ContinuityOfContent DSRTypes::enumeratedValueToContinuityOfContent(
    const OFString& enumeratedValue) {
...
DSRTypes::E_PreliminaryFlag DSRTypes::enumeratedValueToPreliminaryFlag(
    const OFString& enumeratedValue) {
  E_PreliminaryFlag type = PF_invalid;
  const S_PreliminaryFlagNameMap* iterator = PreliminaryFlagNameMap;
  while ((iterator->Type != PF_last) &&
         (enumeratedValue != iterator->EnumeratedValue))
    iterator++;
  if (enumeratedValue == iterator->EnumeratedValue)
    type = iterator->Type;
...
DSRTypes::E_CompletionFlag DSRTypes::enumeratedValueToCompletionFlag(
    const OFString& enumeratedValue) {
  E_CompletionFlag type = CF_invalid;
  const S_CompletionFlagNameMap* iterator = CompletionFlagNameMap;
  while ((iterator->Type != CF_last) &&
         (enumeratedValue != iterator->EnumeratedValue))
    iterator++;
  if (enumeratedValue == iterator->EnumeratedValue)
    type = iterator->Type;
...
DSRTypes::E_VerificationFlag DSRTypes::enumeratedValueToVerificationFlag(
    const OFString& enumeratedValue) {
  E_VerificationFlag type = VF_invalid;
  const S_VerificationFlagNameMap* iterator = VerificationFlagNameMap;
  while ((iterator->Type != VF_last) &&
         (enumeratedValue != iterator->EnumeratedValue))
    iterator++;
  if (enumeratedValue == iterator->EnumeratedValue)
    type = iterator->Type;
...
DSRTypes::E_CharacterSet DSRTypes::definedTermToCharacterSet(
    const OFString& definedTerm) {
...
OFCondition DSRTypes::addElementToDataset(OFCondition& result, DcmItem& dataset,
...
void DSRTypes::removeAttributeFromSequence(DcmSequenceOfItems& sequence,
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (item != NULL) {
      ...
      stack.clear();
      if (item->search(tagKey, stack, ESM_fromHere, OFTrue /*searchIntoSub*/)
              .good()) {
        while (!stack.empty())
...
OFCondition DSRTypes::getElementFromDataset(DcmItem& dataset,
                                            DcmElement& delem) {
  DcmStack stack;
  OFCondition result = dataset.search(delem.getTag(), stack, ESM_fromHere,
                                      OFFalse /*searchIntoSub*/);
  if (result.good()) {
...
const OFString& DSRTypes::getPrintStringFromElement(const DcmElement& delem,
                                                    OFString& stringValue) {
  OFString tempString;
  return convertToPrintString(getStringValueFromElement(delem, tempString),
                              stringValue);
}

const OFString& DSRTypes::getMarkupStringFromElement(
    const DcmElement& delem, OFString& stringValue,
    const OFBool convertNonASCII) {
  OFString tempString;
  return OFStandard::convertToMarkupString(
      getStringValueFromElement(delem, tempString), stringValue,
      convertNonASCII);
}
...
OFCondition DSRTypes::getStringValueFromDataset(DcmItem& dataset,
                                                const DcmTagKey& tagKey,
                                                OFString& stringValue) {
  return dataset.findAndGetOFString(tagKey, stringValue, 0,
                                    OFFalse /*searchIntoSub*/);
}
...
OFCondition DSRTypes::putStringValueToDataset(DcmItem& dataset,
  ...
  if (allowEmpty || !stringValue.empty())
    result = dataset.putAndInsertString(tag, stringValue.c_str(),
                                        OFTrue /*replaceOld*/);
  return result;
}

OFBool DSRTypes::checkElementValue(DcmElement& delem, const OFString& vm,
                                   const OFString& type,
  ...
  if (((type == "1") || (type == "2")) && searchCond.bad()) {
    DCMSR_WARN(tagName << " " << tag << " absent in " << module << " (type "
                       << type << ")");
    result = OFFalse;
    ...
    if (((type == "1") || (type == "1C")) && searchCond.good()) {
      DCMSR_WARN(tagName << " " << tag << " empty in " << module << " (type "
                         << type << ")");
      result = OFFalse;
    }
  } else {
    ...
    if (checkResult == EC_ValueRepresentationViolated) {
      DCMSR_WARN(tagName << " " << tag << " violates VR definition in "
                         << module);
      result = OFFalse;
    } else if (checkResult == EC_ValueMultiplicityViolated) {
      DCMSR_WARN(tagName << " " << tag << vmText << " != " << vm << " in "
                         << module);
      result = OFFalse;
    } else if (checkResult == EC_MaximumLengthViolated) {
      DCMSR_WARN(tagName << " " << tag << " violates maximum VR length in "
                         << module);
      result = OFFalse;
    } else if (checkResult.bad()) {
      DCMSR_DEBUG("INTERNAL ERROR while checking value of "
                  << tagName << " " << tag << " in " << module);
    }
  }
  return result;
}
...
OFCondition DSRTypes::getAndCheckStringValueFromDataset(
    DcmItem& dataset, const DcmTagKey& tagKey, OFString& stringValue,
    const OFString& vm, const OFString& type, const char* moduleName) {
  DcmStack stack;
  OFCondition result =
      dataset.search(tagKey, stack, ESM_fromHere, OFFalse /*searchIntoSub*/);
  if (result.good()) {
    ...
    if ((type == "1") || (type == "2")) {
      ...
      const OFString module = (moduleName == NULL) ? "SR document" : moduleName;
      DCMSR_WARN(tagName << " " << tagKey << " absent in " << module
                         << " (type " << type << ")");
    }
  }
  if (result.bad())
...
const OFString& DSRTypes::currentDateTime(OFString& dateTimeString) {
  DcmDateTime::getCurrentDateTime(dateTimeString, OFTrue /*seconds*/,
                                  OFFalse /*fraction*/, OFFalse /*timeZone*/);
...
const OFString& DSRTypes::dicomToReadableTime(const OFString& dicomTime,
  ...
  DcmTime::getISOFormattedTimeFromString(
      dicomTime, readableTime, OFTrue /*seconds*/, OFFalse /*fraction*/,
...
const OFString& DSRTypes::dicomToReadableDateTime(const OFString& dicomDateTime,
  ...
  DcmDateTime::getISOFormattedDateTimeFromString(
      dicomDateTime, readableDateTime, OFTrue /*seconds*/, OFFalse /*fraction*/,
      OFTrue /*timeZone*/, OFFalse /*createMissingPart*/);
  return readableDateTime;
}

const OFString& DSRTypes::dicomToReadablePersonName(
    const OFString& dicomPersonName, OFString& readablePersonName) {
  if (DcmPersonName::getFormattedNameFromString(
          dicomPersonName, readablePersonName, 0 /*componentGroup*/)
          .bad())
...
const OFString& DSRTypes::dicomToXMLPersonName(const OFString& dicomPersonName,
  ...
  if (DcmPersonName::getNameComponentsFromString(
          dicomPersonName, str1, str2, str3, str4, str5, 0 /*componentGroup*/)
          .good()) {
...
const char* DSRTypes::numberToString(const size_t number, char* stringValue) {
  if (stringValue != NULL) {
    /* unsigned long */
...
const OFString& DSRTypes::convertToHTMLString(const OFString& sourceString,
  ...
  const OFStandard::E_MarkupMode markupMode =
      (flags & HF_XHTML11Compatibility)  ? OFStandard::MM_XHTML
      : (flags & HF_HTML32Compatibility) ? OFStandard::MM_HTML32
                                         : OFStandard::MM_HTML;
  /* call the real function */
  return OFStandard::convertToMarkupString(
      sourceString, markupString, convertNonASCII, markupMode, newlineAllowed);
}
...
const OFString& DSRTypes::convertToXMLString(const OFString& sourceString,
  ...
  /* call the real function */
  return OFStandard::convertToMarkupString(
      sourceString, markupString, OFFalse /*convertNonASCII*/,
      OFStandard::MM_XML, OFFalse /*newlineAllowed*/);
}
...
OFBool DSRTypes::checkForValidUIDFormat(const OFString& stringValue) {
  ...
  if (!stringValue.empty()) {
    const unsigned char* p =
        OFreinterpret_cast(const unsigned char*, stringValue.c_str());
    if (p != NULL) {
...
DSRIODConstraintChecker* DSRTypes::createIODConstraintChecker(
    const E_DocumentType documentType) {
...
DSRDocumentTreeNode* DSRTypes::createDocumentTreeNode(
...
OFBool DSRTypes::writeStringValueToXML(STD_NAMESPACE ostream& stream,
  ...
  if (writeEmptyValue || !stringValue.empty()) {
    stream << "<" << tagName << ">";
    OFStandard::convertToMarkupStream(
        stream, stringValue, OFFalse /*convertNonASCII*/, OFStandard::MM_XML,
        OFFalse /*newlineAllowed*/);
    ...
    if (delem.getVR() == EVR_PN)  // special formatting for person names
    {
      ...
      stream << OFendl
             << dicomToXMLPersonName(
                    getStringValueFromElement(delem, tempString), xmlString,
                    writeEmptyValue)
             << OFendl;
    } else
      OFStandard::convertToMarkupStream(
          stream, getStringValueFromElement(delem, tempString),
          OFFalse /*convertNonASCII*/);
    stream << "</" << tagName << ">" << OFendl;
    result = OFTrue;
  }
  return result;
...
size_t DSRTypes::createHTMLAnnexEntry(STD_NAMESPACE ostream& docStream,
                                      ...
                                      size_t& annexNumber, const size_t flags) {
  /* hyperlink to corresponding annex */
  const char* attrName =
      (flags & DSRTypes::HF_XHTML11Compatibility) ? "id" : "name";
  docStream << "[";
  if (!referenceText.empty())
    docStream << referenceText << " ";
  docStream << "<a " << attrName << "=\"annex_src_" << annexNumber
            << "\" href=\"#annex_dst_" << annexNumber << "\">Annex "
            << annexNumber << "</a>]" << OFendl;
  /* create new annex */
  annexStream << "<h2><a " << attrName << "=\"annex_dst_" << annexNumber
              << "\" href=\"#annex_src_" << annexNumber << "\">Annex "
              << annexNumber << "</a></h2>" << OFendl;
...
size_t DSRTypes::createHTMLFootnote(STD_NAMESPACE ostream& docStream,
  ...
  if (flags & HF_XHTML11Compatibility)
    ...
    docStream << "<small><sup>";
  docStream << "<a " << attrName << "=\"footnote_src_" << nodeID << "_"
            << footnoteNumber << "\" ";
  docStream << "href=\"#footnote_dst_" << nodeID << "_" << footnoteNumber
            << "\">" << footnoteNumber << "</a>";
  if (flags & HF_XHTML11Compatibility)
  ...
  /* create new footnote */
  footnoteStream << "<b><a " << attrName << "=\"footnote_dst_" << nodeID << "_"
                 << footnoteNumber << "\" ";
  footnoteStream << "href=\"#footnote_src_" << nodeID << "_" << footnoteNumber
                 << "\">Footnote " << footnoteNumber << "</a></b>" << OFendl;
...
OFCondition DSRTypes::appendStream(STD_NAMESPACE ostream& mainStream,
  ...
  if (tempString != NULL) {
    ...
    /* very important! since we have full control we are responsible for
     * deleting the array */
 ...
 *  Use new isEmpty() method instead of length in order to determine whether the
 *  element value is empty (e.g. for checking the presence of type 1
 * attributes).
 *
