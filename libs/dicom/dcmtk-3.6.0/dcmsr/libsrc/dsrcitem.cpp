...
DSRContentItem::DSRContentItem() : TreeNode(NULL) {}
...
size_t DSRContentItem::getReferencedNodeID() const {
  ...
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_byReference)
      nodeID = OFstatic_cast(DSRByReferenceTreeNode*, TreeNode)
                   ->getReferencedNodeID();
  }
  return nodeID;
}
...
OFCondition DSRContentItem::setStringValue(const OFString& stringValue) {
  ...
  if (TreeNode != NULL) {
    switch (TreeNode->getValueType()) {
      case VT_Text:
        result =
            OFstatic_cast(DSRTextTreeNode*, TreeNode)->setValue(stringValue);
        break;
      case VT_DateTime:
        result = OFstatic_cast(DSRDateTimeTreeNode*, TreeNode)
                     ->setValue(stringValue);
        break;
      case VT_Date:
        result =
            OFstatic_cast(DSRDateTreeNode*, TreeNode)->setValue(stringValue);
        break;
      case VT_Time:
        result =
            OFstatic_cast(DSRTimeTreeNode*, TreeNode)->setValue(stringValue);
        break;
      case VT_UIDRef:
        result =
            OFstatic_cast(DSRUIDRefTreeNode*, TreeNode)->setValue(stringValue);
        break;
      case VT_PName:
        result =
            OFstatic_cast(DSRPNameTreeNode*, TreeNode)->setValue(stringValue);
        break;
      default:
        break;
    }
...
OFCondition DSRContentItem::getNumericValue(
    DSRNumericMeasurementValue& numericValue) const {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_Num)
...
OFCondition DSRContentItem::setNumericValue(
...
const DSRSpatialCoordinatesValue& DSRContentItem::getSpatialCoordinates()
    const {
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_SCoord)
...
OFCondition DSRContentItem::getSpatialCoordinates(
    DSRSpatialCoordinatesValue& coordinatesValue) const {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_SCoord)
      result = OFstatic_cast(DSRSCoordTreeNode*, TreeNode)
                   ->getValue(coordinatesValue);
    else
...
OFCondition DSRContentItem::setSpatialCoordinates(
    const DSRSpatialCoordinatesValue& coordinatesValue) {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_SCoord)
      result = OFstatic_cast(DSRSCoordTreeNode*, TreeNode)
                   ->setValue(coordinatesValue);
  }
  return result;
}
...
const DSRTemporalCoordinatesValue& DSRContentItem::getTemporalCoordinates()
    const {
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_TCoord)
...
OFCondition DSRContentItem::getTemporalCoordinates(
    DSRTemporalCoordinatesValue& coordinatesValue) const {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_TCoord)
      result = OFstatic_cast(DSRTCoordTreeNode*, TreeNode)
                   ->getValue(coordinatesValue);
    else
      coordinatesValue.clear();
  } else
...
OFCondition DSRContentItem::setTemporalCoordinates(
...
const DSRCompositeReferenceValue& DSRContentItem::getCompositeReference()
    const {
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_Composite)
...
OFCondition DSRContentItem::getCompositeReference(
    DSRCompositeReferenceValue& referenceValue) const {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_Composite)
      result = OFstatic_cast(DSRCompositeTreeNode*, TreeNode)
                   ->getValue(referenceValue);
    else
...
OFCondition DSRContentItem::setCompositeReference(
...
OFCondition DSRContentItem::getImageReference(
    DSRImageReferenceValue& referenceValue) const {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_Image)
      result =
          OFstatic_cast(DSRImageTreeNode*, TreeNode)->getValue(referenceValue);
    else
      referenceValue.clear();
  } else
    referenceValue.clear();
...
OFCondition DSRContentItem::setImageReference(
...
OFCondition DSRContentItem::getWaveformReference(
    DSRWaveformReferenceValue& referenceValue) const {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_Waveform)
      result = OFstatic_cast(DSRWaveformTreeNode*, TreeNode)
                   ->getValue(referenceValue);
    else
      referenceValue.clear();
  } else
...
OFCondition DSRContentItem::setWaveformReference(
...
DSRTypes::E_ContinuityOfContent DSRContentItem::getContinuityOfContent() const {
  ...
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_Container)
      result = OFstatic_cast(DSRContainerTreeNode*, TreeNode)
                   ->getContinuityOfContent();
  }
  return result;
}

OFCondition DSRContentItem::setContinuityOfContent(
    const E_ContinuityOfContent continuityOfContent) {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL) {
    if (TreeNode->getValueType() == VT_Container)
      result = OFstatic_cast(DSRContainerTreeNode*, TreeNode)
                   ->setContinuityOfContent(continuityOfContent);
  }
  return result;
}
...
OFCondition DSRContentItem::getConceptName(
...
OFCondition DSRContentItem::setConceptName(
...
OFCondition DSRContentItem::setObservationDateTime(
...
OFCondition DSRContentItem::getTemplateIdentification(
    OFString& templateIdentifier, OFString& mappingResource) const {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL)
    result = TreeNode->getTemplateIdentification(templateIdentifier,
                                                 mappingResource);
  return result;
}

OFCondition DSRContentItem::setTemplateIdentification(
    const OFString& templateIdentifier, const OFString& mappingResource) {
  OFCondition result = EC_IllegalCall;
  if (TreeNode != NULL)
    result = TreeNode->setTemplateIdentification(templateIdentifier,
                                                 mappingResource);
  return result;
}
