...
EXPLICIT_SPECIALIZATION const DSRGraphicData3DItem
    DSRListOfItems<DSRGraphicData3DItem>::EmptyItem(0, 0, 0);
...
DSRGraphicData3DList& DSRGraphicData3DList::operator=(
    const DSRGraphicData3DList& lst) {
...
OFCondition DSRGraphicData3DList::read(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckElementFromDataset(
      dataset, delem, "3-3n", "1", "SCOORD3D content item");
  if (result.good()) {
    /* clear internal list */
    clear();
    Float32 x = 0;
...
OFCondition DSRGraphicData3DList::write(DcmItem& dataset) const {
  ...
  /* add to dataset */
  if (result.good())
    result = DSRTypes::addElementToDataset(
        result, dataset, new DcmFloatingPointSingle(delem), "3-3n", "1",
        "SCOORD3D content item");
  return result;
}

const DSRGraphicData3DItem& DSRGraphicData3DList::getItem(
    const size_t idx) const {
  /* hidden by the following getItem() method */
  return DSRListOfItems<DSRGraphicData3DItem>::getItem(idx);
}

OFCondition DSRGraphicData3DList::getItem(const size_t idx, Float32& x,
...
void DSRGraphicData3DList::addItem(const Float32 x, const Float32 y,
                                   const Float32 z) {
