...
OFCondition DSRDocumentTreeNode::read(
    DcmItem& dataset, const DSRIODConstraintChecker* constraintChecker,
    const size_t flags) {
...
OFCondition DSRDocumentTreeNode::readXML(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    if (!doc.getStringFromAttribute(cursor, idAttr, "id", OFFalse /*encoding*/,
                                    OFFalse /*required*/)
             .empty() &&
        (stringToNumber(idAttr.c_str()) != Ident)) {
      /* create warning message */
      DCMSR_WARN("XML attribute 'id' ("
                 << idAttr << ") deviates from current node number (" << Ident
                 << ")");
    }
    ...
    if (!(flags & XF_templateElementEnclosesItems)) {
      ...
      const DSRXMLCursor childCursor =
          doc.getNamedNode(cursor.getChild(), "template", OFFalse /*required*/);
      if (childCursor.valid()) {
        ...
        if (doc.hasAttribute(childCursor, "tid")) {
          ...
          doc.getStringFromNodeContent(
              doc.getNamedNode(childCursor.getChild(), "resource"),
              mappingResource);
          doc.getStringFromNodeContent(
              doc.getNamedNode(childCursor.getChild(), "id"),
              templateIdentifier);
        }
        if (setTemplateIdentification(templateIdentifier, mappingResource)
                .bad())
          DCMSR_WARN(
              "Content item has invalid/incomplete template identification");
      }
    }
    /* read concept name (not required in some cases) */
    ConceptName.readXML(doc, doc.getNamedNode(cursor.getChild(), "concept",
                                              OFFalse /*required*/));
    /* read observation datetime (optional) */
    const DSRXMLCursor childCursor = doc.getNamedNode(
        cursor.getChild(), "observation", OFFalse /*required*/);
    if (childCursor.valid())
      DSRDateTimeTreeNode::getValueFromXMLNodeContent(
          doc, doc.getNamedNode(childCursor.getChild(), "datetime"),
          ObservationDateTime);
    ...
    while (cursor.valid() && result.good()) {
      ...
      /* get SR value type from current XML node, also supports "by-reference"
       * detection */
      ...
      if (valueType != VT_invalid) {
        ...
        E_RelationshipType relationshipType =
            doc.getRelationshipTypeFromNode(cursor);
        ...
        if (result.good()) {
          if ((flags & XF_templateElementEnclosesItems) &&
              (valueType != VT_byReference)) {
            /* set template identification (if any) */
            if (node->setTemplateIdentification(templateIdentifier,
                                                mappingResource)
                    .bad())
              DCMSR_WARN(
                  "Content item has invalid/incomplete template "
                  "identification");
          }
          ...
          DCMSR_ERROR("Cannot add \""
                      << relationshipTypeToReadableName(relationshipType) << " "
                      << valueTypeToDefinedTerm(valueType /*target item*/)
                      << "\" to "
                      << valueTypeToDefinedTerm(ValueType /*source item*/)
                      << " in " << documentTypeToReadableName(documentType));
        }
      }
...
OFCondition DSRDocumentTreeNode::readXMLContentItem(
    const DSRXMLDocument& /*doc*/, DSRXMLCursor /*cursor*/) {
...
OFCondition DSRDocumentTreeNode::writeXML(STD_NAMESPACE ostream& stream,
  ...
  if ((flags & XF_writeTemplateIdentification) &&
      !(flags & XF_templateElementEnclosesItems)) {
    if (!TemplateIdentifier.empty() && !MappingResource.empty()) {
      if (flags & XF_templateIdentifierAsAttribute)
        stream << "<template resource=\"" << MappingResource << "\" tid=\""
               << TemplateIdentifier << "\"/>" << OFendl;
      else {
  ...
  /* relationship type */
  if ((RelationshipType != RT_isRoot) &&
      !(flags & XF_relationshipTypeAsAttribute))
    writeStringValueToXML(stream,
                          relationshipTypeToDefinedTerm(RelationshipType),
                          "relationship", (flags & XF_writeEmptyTags) > 0);
  /* concept name */
  if (ConceptName.isValid()) {
    if (flags & XF_codeComponentsAsAttribute)
      stream << "<concept";  // bracket ">" is closed in the next writeXML()
                             // routine
    else
  ...
  if (!ObservationDateTime.empty()) {
    ...
    DcmDateTime::getISOFormattedDateTimeFromString(
        ObservationDateTime, tmpString, OFTrue /*seconds*/,
        OFFalse /*fraction*/, OFFalse /*timeZone*/,
        OFFalse /*createMissingPart*/, "T" /*dateTimeSeparator*/);
...
OFCondition DSRDocumentTreeNode::renderHTML(STD_NAMESPACE ostream& docStream,
  ...
  if (ReferenceTarget) {
    ...
    const char* closeElm =
        (flags & DSRTypes::HF_XHTML11Compatibility) ? " /" : "></a";
    docStream << "<a " << attrName << "=\"content_item_" << getNodeID() << "\""
              << closeElm << ">" << OFendl;
  }
  /* render content item */
  OFCondition result = renderHTMLContentItem(docStream, annexStream,
                                             nestingLevel, annexNumber, flags);
  /* render child nodes */
  if (result.good())
    result =
        renderHTMLChildNodes(docStream, annexStream, nestingLevel, annexNumber,
                             flags | HF_renderItemsSeparately);
  else
    printContentItemErrorMessage("Rendering", result, this);
  return result;
}

OFCondition DSRDocumentTreeNode::getConceptName(
    DSRCodedEntryValue& conceptName) const {
  conceptName = ConceptName;
  return EC_Normal;
}

OFCondition DSRDocumentTreeNode::setConceptName(
...
OFCondition DSRDocumentTreeNode::setObservationDateTime(
...
OFCondition DSRDocumentTreeNode::getTemplateIdentification(
    OFString& templateIdentifier, OFString& mappingResource) const {
...
OFCondition DSRDocumentTreeNode::setTemplateIdentification(
    const OFString& templateIdentifier, const OFString& mappingResource) {
  OFCondition result = EC_IllegalParameter;
  /* check for valid value pair */
  if ((templateIdentifier.empty() && mappingResource.empty()) ||
      (!templateIdentifier.empty() && !mappingResource.empty())) {
...
OFCondition DSRDocumentTreeNode::renderHTMLContentItem(
    STD_NAMESPACE ostream& /*docStream*/,
    STD_NAMESPACE ostream& /*annexStream*/, const size_t /*nestingLevel*/,
    size_t& /*annexNumber*/, const size_t /*flags*/) const {
  /* no content to render */
  return EC_Normal;
}

OFCondition DSRDocumentTreeNode::readSRDocumentContentModule(
    DcmItem& dataset, const DSRIODConstraintChecker* constraintChecker,
    const size_t flags) {
  OFCondition result = EC_Normal;
  /* read DocumentRelationshipMacro */
  result = readDocumentRelationshipMacro(dataset, constraintChecker,
                                         "1" /*posString*/, flags);
...
OFCondition DSRDocumentTreeNode::writeSRDocumentContentModule(
    DcmItem& dataset, DcmStack* markedItems) {
  OFCondition result = EC_Normal;
...
OFCondition DSRDocumentTreeNode::readDocumentRelationshipMacro(
    DcmItem& dataset, const DSRIODConstraintChecker* constraintChecker,
    const OFString& posString, const size_t flags) {
  OFCondition result = EC_Normal;
  ...
  getAndCheckStringValueFromDataset(dataset, DCM_ObservationDateTime,
                                    ObservationDateTime, "1", "1C");
  /* determine template identifier expected for this document */
  const OFString expectedTemplateIdentifier =
      (constraintChecker != NULL)
          ? OFSTRING_GUARD(constraintChecker->getRootTemplateIdentifier())
          : "";
  /* read ContentTemplateSequence (conditional) */
  DcmItem* ditem = NULL;
  if (dataset
          .findAndGetSequenceItem(DCM_ContentTemplateSequence, ditem,
                                  0 /*itemNum*/)
          .good()) {
    getAndCheckStringValueFromDataset(*ditem, DCM_MappingResource,
                                      MappingResource, "1", "1",
                                      "ContentTemplateSequence");
    getAndCheckStringValueFromDataset(*ditem, DCM_TemplateIdentifier,
                                      TemplateIdentifier, "1", "1",
                                      "ContentTemplateSequence");
    if (!expectedTemplateIdentifier.empty()) {
      ...
      if (MappingResource == "DCMR") {
        ...
        if (TemplateIdentifier != expectedTemplateIdentifier) {
          DCMSR_WARN(
              "Incorrect value for TemplateIdentifier ("
              << ((TemplateIdentifier.empty()) ? "<empty>" : TemplateIdentifier)
              << "), " << expectedTemplateIdentifier << " expected");
        }
      } else if (!MappingResource.empty())
        printUnknownValueWarningMessage("MappingResource",
                                        MappingResource.c_str());
    }
  }
  /* only check template identifier on dataset level (root node) */
  else if ((dataset.ident() == EVR_dataset) &&
           !expectedTemplateIdentifier.empty()) {
    DCMSR_WARN(
        "ContentTemplateSequence missing or empty, TemplateIdentifier "
...
OFCondition DSRDocumentTreeNode::writeDocumentRelationshipMacro(
    DcmItem& dataset, DcmStack* markedItems) {
  OFCondition result = EC_Normal;
  ...
  if (DigitalSignatures.card() > 0) {
    addElementToDataset(result, dataset,
                        new DcmSequenceOfItems(DigitalSignatures), "1-n", "3",
                        "SOPCommonModule");
    DCMSR_WARN(
        "Writing possibly incorrect digital signature - same as read from "
        "dataset");
  }
  /* add to mark stack */
  ...
  result = putStringValueToDataset(dataset, DCM_ObservationDateTime,
                                   ObservationDateTime, OFFalse /*allowEmpty*/);
  /* write ContentTemplateSequence (conditional) */
  if (result.good()) {
    if (!TemplateIdentifier.empty() && !MappingResource.empty()) {
      DcmItem* ditem = NULL;
      /* create sequence with a single item */
      result = dataset.findOrCreateSequenceItem(DCM_ContentTemplateSequence,
                                                ditem, 0 /*position*/);
      if (result.good()) {
        /* write item data */
        putStringValueToDataset(*ditem, DCM_TemplateIdentifier,
                                TemplateIdentifier);
...
OFCondition DSRDocumentTreeNode::readDocumentContentMacro(
    DcmItem& dataset, const OFString& posString, const size_t flags) {
  OFCondition result = EC_Normal;
  ...
  if (RelationshipType == RT_isRoot) {
    ...
    result = ConceptName.readSequence(dataset, DCM_ConceptNameCodeSequence,
                                      "1" /*type*/);
  } else {
    /* the concept name might be empty for all other content items */
    ConceptName.readSequence(dataset, DCM_ConceptNameCodeSequence,
                             "1C" /*type*/);
  }
...
OFCondition DSRDocumentTreeNode::writeDocumentContentMacro(
    DcmItem& dataset) const {
  OFCondition result = EC_Normal;
  /* write ValueType */
  result = putStringValueToDataset(dataset, DCM_ValueType,
                                   valueTypeToDefinedTerm(ValueType));
...
OFCondition DSRDocumentTreeNode::createAndAppendNewNode(
    DSRDocumentTreeNode*& previousNode,
    const E_RelationshipType relationshipType, const E_ValueType valueType,
    const DSRIODConstraintChecker* constraintChecker) {
  OFCondition result = EC_Normal;
  /* do not check by-reference relationships here, will be done later (after
   * complete reading) */
  if ((relationshipType == RT_unknown) ||
      ((relationshipType != RT_invalid) &&
       ((valueType == VT_byReference) || (constraintChecker == NULL) ||
        constraintChecker->checkContentRelationship(ValueType, relationshipType,
                                                    valueType)))) {
    DSRDocumentTreeNode* node =
        createDocumentTreeNode(relationshipType, valueType);
    if (node != NULL) {
      /* first child node */
...
OFCondition DSRDocumentTreeNode::readContentSequence(
    DcmItem& dataset, const DSRIODConstraintChecker* constraintChecker,
    const OFString& posString, const size_t flags) {
  OFCondition result = EC_Normal;
  DcmStack stack;
  /* read ContentSequence (might be absent or empty) */
  if (dataset.search(DCM_ContentSequence, stack, ESM_fromHere, OFFalse)
          .good()) {
    ...
    if (dseq != NULL) {
      ...
      while ((i < count) && result.good()) {
        ...
        if (ditem != NULL) {
          ...
          result = getAndCheckStringValueFromDataset(
              *ditem, DCM_RelationshipType, tmpString, "1", "1",
              "content item");
          if (result.good() || (flags & RF_acceptUnknownRelationshipType)) {
            ...
            if (relationshipType == RT_invalid) {
              printUnknownValueWarningMessage("RelationshipType",
                                              tmpString.c_str());
              if (flags & RF_acceptUnknownRelationshipType)
                relationshipType = RT_unknown;
            }
            /* check for by-reference relationship */
            DcmUnsignedLong referencedContentItemIdentifier(
                DCM_ReferencedContentItemIdentifier);
            if (getAndCheckElementFromDataset(*ditem,
                                              referencedContentItemIdentifier,
                                              "1-n", "1C", "content item")
                    .good()) {
              /* create new node (by-reference, no constraint checker required)
               */
              result = createAndAppendNewNode(node, relationshipType,
                                              VT_byReference);
              ...
              /* read ValueType (from DocumentContentMacro) - required to create
               * new node */
              result = getAndCheckStringValueFromDataset(
                  *ditem, DCM_ValueType, tmpString, "1", "1", "content item");
              if (result.good()) {
                ...
                if (valueType != VT_invalid) {
                  ...
                  result = createAndAppendNewNode(
                      node, relationshipType, valueType,
                      (flags & RF_ignoreRelationshipConstraints)
                          ? NULL
                          : constraintChecker);
                  /* read RelationshipMacro */
                  if (result.good()) {
                    newNode = node;
                    result = node->readDocumentRelationshipMacro(
                        *ditem, constraintChecker, location, flags);
                    /* read DocumentContentMacro */
                    if (result.good())
                      result = node->readDocumentContentMacro(
                          *ditem, location.c_str(), flags);
                  } else {
                    /* create new node failed */
                    ...
                    const E_DocumentType documentType =
                        (constraintChecker != NULL)
                            ? constraintChecker->getDocumentType()
                            : DT_invalid;
                    DCMSR_ERROR(
                        "Cannot add \""
                        << relationshipTypeToReadableName(relationshipType)
                  ...
                  printUnknownValueWarningMessage("ValueType",
                                                  tmpString.c_str());
                  result = SR_EC_UnknownValueType;
                }
              }
          ...
          if (result.bad()) {
            printContentItemErrorMessage("Reading", result, newNode,
                                         location.c_str());
            /* print current data set (item) that caused the error */
            DCMSR_DEBUG(OFString(31, '-')
                        << " DICOM DATA SET " << OFString(31, '-') << OFendl
                        << DcmObject::PrintHelper(*ditem, 0, 1)
                        << OFString(78, '-'));
          }
        } else
          result = SR_EC_InvalidDocumentTree;
...
OFCondition DSRDocumentTreeNode::writeContentSequence(
    DcmItem& dataset, DcmStack* markedItems) const {
  OFCondition result = EC_Normal;
      ...
      do { /* for all child nodes */
        ...
        if (node != NULL) {
          ...
          if (ditem != NULL) {
            ...
            result = putStringValueToDataset(
                *ditem, DCM_RelationshipType,
                relationshipTypeToDefinedTerm(node->getRelationshipType()));
            /* check for by-reference relationship */
            if (node->getValueType() == VT_byReference) {
              ...
              if (result.good())
                result =
                    node->writeDocumentRelationshipMacro(*ditem, markedItems);
              /* write DocumentContentMacro */
              if (result.good())
...
OFCondition DSRDocumentTreeNode::renderHTMLConceptName(
    STD_NAMESPACE ostream& docStream, const size_t flags) const {
  if (!(flags & HF_renderItemInline) && (flags & HF_renderItemsSeparately)) {
    const char* lineBreak =
        (flags & DSRTypes::HF_renderSectionTitlesInline) ? " "
        : (flags & DSRTypes::HF_XHTML11Compatibility)    ? "<br />"
                                                         : "<br>";
    ...
    if (!ConceptName.getCodeMeaning().empty()) {
      ...
      ConceptName.renderHTML(docStream, flags,
                             (flags & HF_renderConceptNameCodes) &&
                                 ConceptName.isValid() /*fullCode*/);
      docStream << ":</b>";
      writeLine = OFTrue;
    ...
    if (!ObservationDateTime.empty()) {
      ...
      if (flags & HF_XHTML11Compatibility)
        ...
        docStream << "<small>";
      docStream << "(observed: "
                << dicomToReadableDateTime(ObservationDateTime, tmpString)
                << ")";
      if (flags & HF_XHTML11Compatibility)
        docStream << "</span>";
      else
...
OFCondition DSRDocumentTreeNode::renderHTMLChildNodes(
    STD_NAMESPACE ostream& docStream, STD_NAMESPACE ostream& annexStream,
    const size_t nestingLevel, size_t& annexNumber, const size_t flags) const {
  OFCondition result = EC_Normal;
  /* goto first child of current node */
  DSRTreeNodeCursor cursor(Down);
  if (cursor.isValid()) {
    ...
    do { /* for all child nodes */
      ...
      if (node != NULL) {
        ...
        if (!(flags & HF_renderItemsSeparately) && node->hasChildNodes() &&
            (node->getValueType() != VT_Container))
          newFlags |= HF_createFootnoteReferences;
        /* render (optional) reference to annex */
        OFString relationshipText;
        if (!getRelationshipText(node->getRelationshipType(), relationshipText,
                                 flags)
                 .empty()) {
          if (paragraphFlag) {
          ...
          if (newFlags & HF_XHTML11Compatibility)
            docStream << "<span class=\"relation\">" << relationshipText
                      << "</span>: ";
          else if (flags & DSRTypes::HF_HTML32Compatibility)
            docStream << "<u>" << relationshipText << "</u>: ";
          else /* HTML 4.01 */
            docStream << "<span class=\"under\">" << relationshipText
                      << "</span>: ";
          /* expand short nodes with no children inline (or depending on 'flags'
           * all nodes) */
          if ((flags & HF_alwaysExpandChildrenInline) ||
              (!(flags & HF_neverExpandChildrenInline) &&
               !node->hasChildNodes() && node->isShort(flags))) {
            if (node->getValueType() != VT_byReference) {
              ...
              if (node->getConceptName().getCodeMeaning().empty())
                ...
                node->getConceptName().renderHTML(
                    docStream, flags,
                    (flags & HF_renderConceptNameCodes) &&
                        ConceptName.isValid() /*fullCode*/);
              docStream << " = ";
            ...
            result =
                node->renderHTML(docStream, annexStream, 0 /*nesting level*/,
                                 annexNumber, newFlags | HF_renderItemInline);
          } else {
            ...
            createHTMLAnnexEntry(docStream, annexStream, "" /*referenceText*/,
                                 annexNumber, newFlags);
            if (flags & HF_XHTML11Compatibility)
            ...
            result = node->renderHTML(annexStream, tempAnnexStream,
                                      0 /*nesting level*/, annexNumber,
                                      newFlags | HF_currentlyInsideAnnex);
            annexStream << "</div>" << OFendl;
          ...
          if (newFlags & HF_createFootnoteReferences) {
            ...
            result = node->renderHTMLContentItem(docStream, annexStream,
                                                 0 /*nestingLevel*/,
                                                 annexNumber, newFlags);
            ...
            if (result.good()) {
              ...
              createHTMLFootnote(docStream, tempDocStream, footnoteNumber,
                                 node->getNodeID(), flags);
              /* render child nodes to temporary stream */
              result = node->renderHTMLChildNodes(tempDocStream, annexStream,
                                                  0 /*nestingLevel*/,
                                                  annexNumber, newFlags);
            }
          } else {
            /* render HTML code (incl. child nodes)*/
            result = node->renderHTML(docStream, annexStream, nestingLevel + 1,
                                      annexNumber, newFlags);
          }
          /* end paragraph */
          if (flags & HF_renderItemsSeparately) {
...
const OFString& DSRDocumentTreeNode::getRelationshipText(
    const E_RelationshipType relationshipType, OFString& relationshipText,
    const size_t flags) {
