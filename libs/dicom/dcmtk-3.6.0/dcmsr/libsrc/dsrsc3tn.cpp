...
OFBool DSRSCoord3DTreeNode::isValid() const {
  return DSRDocumentTreeNode::isValid() &&
         DSRSpatialCoordinates3DValue::isValid();
}
...
OFCondition DSRSCoord3DTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    result = setGraphicType(enumeratedValueToGraphicType3D(
...
OFCondition DSRSCoord3DTreeNode::renderHTMLContentItem(
  ...
  if (result.good()) {
    result = DSRSpatialCoordinates3DValue::renderHTML(docStream, annexStream,
                                                      annexNumber, flags);
    docStream << OFendl;
  }
  return result;
