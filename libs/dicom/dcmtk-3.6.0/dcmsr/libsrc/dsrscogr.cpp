...
EXPLICIT_SPECIALIZATION const DSRGraphicDataItem
    DSRListOfItems<DSRGraphicDataItem>::EmptyItem(0, 0);
...
DSRGraphicDataList& DSRGraphicDataList::operator=(
    const DSRGraphicDataList& lst) {
...
OFCondition DSRGraphicDataList::read(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckElementFromDataset(
      dataset, delem, "2-2n", "1", "SCOORD content item");
  if (result.good()) {
    /* clear internal list */
    clear();
    Float32 column = 0;
...
OFCondition DSRGraphicDataList::write(DcmItem& dataset) const {
  ...
  if (result.good())
    result = DSRTypes::addElementToDataset(result, dataset,
                                           new DcmFloatingPointSingle(delem),
                                           "2-2n", "1", "SCOORD content item");
  return result;
}
...
OFCondition DSRGraphicDataList::getItem(const size_t idx, Float32& column,
                                        Float32& row) const {
...
void DSRGraphicDataList::addItem(const Float32 column, const Float32 row) {
