...
OFBool DSRSCoordTreeNode::isValid() const {
  return DSRDocumentTreeNode::isValid() &&
         DSRSpatialCoordinatesValue::isValid();
}
...
OFCondition DSRSCoordTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    result = setGraphicType(enumeratedValueToGraphicType(
...
OFCondition DSRSCoordTreeNode::renderHTMLContentItem(
  ...
  if (result.good()) {
    result = DSRSpatialCoordinatesValue::renderHTML(docStream, annexStream,
                                                    annexNumber, flags);
    docStream << OFendl;
  }
  return result;
