...
DSRStringValue::DSRStringValue() : Value() {}

DSRStringValue::DSRStringValue(const OFString& stringValue) : Value() {
...
void DSRStringValue::print(STD_NAMESPACE ostream& stream,
  ...
  if ((maxLength > 3) && (Value.length() > maxLength))
    stream << "\""
           << DSRTypes::convertToPrintString(Value.substr(0, maxLength - 3),
                                             printString)
           << "...\"";
  else
...
OFCondition DSRStringValue::read(DcmItem& dataset, const DcmTagKey& tagKey) {
  /* tbd: check value */
  return DSRTypes::getAndCheckStringValueFromDataset(dataset, tagKey, Value,
                                                     "1", "1", "content item");
}
