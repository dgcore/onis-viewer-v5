...
DSRSpatialCoordinates3DValue::DSRSpatialCoordinates3DValue(
    const DSRTypes::E_GraphicType3D graphicType)
    : GraphicType(graphicType), GraphicDataList(), FrameOfReferenceUID() {}
...
DSRSpatialCoordinates3DValue& DSRSpatialCoordinates3DValue::operator=(
    const DSRSpatialCoordinates3DValue& coordinatesValue) {
  GraphicType = coordinatesValue.GraphicType;
  GraphicDataList = coordinatesValue.GraphicDataList;
  FrameOfReferenceUID = coordinatesValue.FrameOfReferenceUID;
...
OFBool DSRSpatialCoordinates3DValue::isShort(const size_t flags) const {
  return GraphicDataList.isEmpty() ||
         ((flags & DSRTypes::HF_renderFullData) == 0);
}
...
OFCondition DSRSpatialCoordinates3DValue::readXML(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    if (cursor.valid()) {
      ...
      result = GraphicDataList.putString(
          doc.getStringFromNodeContent(cursor, tmpString).c_str());
    }
  }
  return result;
}

OFCondition DSRSpatialCoordinates3DValue::writeXML(
...
OFCondition DSRSpatialCoordinates3DValue::read(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckStringValueFromDataset(
      dataset, DCM_ReferencedFrameOfReferenceUID, FrameOfReferenceUID, "1", "1",
      "SCOORD3D content item");
  /* read GraphicType */
  if (result.good())
    result = DSRTypes::getAndCheckStringValueFromDataset(
        dataset, DCM_GraphicType, tmpString, "1", "1", "SCOORD3D content item");
  if (result.good()) {
    GraphicType = DSRTypes::enumeratedValueToGraphicType3D(tmpString);
    /* check GraphicType */
    if (GraphicType == DSRTypes::GT3_invalid)
      DSRTypes::printUnknownValueWarningMessage("GraphicType",
                                                tmpString.c_str());
    /* read GraphicData */
...
OFCondition DSRSpatialCoordinates3DValue::write(DcmItem& dataset) const {
  ...
  OFCondition result = DSRTypes::putStringValueToDataset(
      dataset, DCM_ReferencedFrameOfReferenceUID, FrameOfReferenceUID);
  /* write GraphicType */
  if (result.good())
    DSRTypes::putStringValueToDataset(
        dataset, DCM_GraphicType,
        DSRTypes::graphicType3DToEnumeratedValue(GraphicType));
  /* write GraphicData */
  if (result.good()) {
    if (!GraphicDataList.isEmpty())
...
OFCondition DSRSpatialCoordinates3DValue::renderHTML(
  ...
  if (!isShort(flags)) {
    ...
    if (flags & DSRTypes::HF_currentlyInsideAnnex) {
      ...
      DSRTypes::createHTMLAnnexEntry(
          docStream, annexStream, "for more details see", annexNumber, flags);
...
OFCondition DSRSpatialCoordinates3DValue::setGraphicType(
    const DSRTypes::E_GraphicType3D graphicType) {
...
OFCondition DSRSpatialCoordinates3DValue::getValue(
    DSRSpatialCoordinates3DValue& coordinatesValue) const {
  coordinatesValue = *this;
  return EC_Normal;
}

OFCondition DSRSpatialCoordinates3DValue::setValue(
    const DSRSpatialCoordinates3DValue& coordinatesValue) {
  OFCondition result = EC_IllegalParameter;
  if (checkData(coordinatesValue.GraphicType, coordinatesValue.GraphicDataList,
                coordinatesValue.FrameOfReferenceUID)) {
    GraphicType = coordinatesValue.GraphicType;
    GraphicDataList = coordinatesValue.GraphicDataList;
    FrameOfReferenceUID = coordinatesValue.FrameOfReferenceUID;
    result = EC_Normal;
...
OFBool DSRSpatialCoordinates3DValue::checkData(
    const DSRTypes::E_GraphicType3D graphicType,
  ...
  if (graphicType == DSRTypes::GT3_invalid)
    ...
    switch (graphicType) {
      case DSRTypes::GT3_Point:
        if (count > 1)
          DCMSR_WARN(
              "GraphicData has too many entries, only a single entry expected");
        result = OFTrue;
        break;
      case DSRTypes::GT3_Multipoint:
        if (count < 1)
          DCMSR_WARN(
              "GraphicData has too few entries, at least one entry expected");
        result = OFTrue;
        break;
      ...
      case DSRTypes::GT3_Polygon:
        if (count < 1)
          ...
          DCMSR_WARN(
              "First and last entry in GraphicData are not equal (POLYGON)");
          result = OFTrue;
        }
        break;
      case DSRTypes::GT3_Ellipse:
        if (count < 4)
          DCMSR_WARN(
              "GraphicData has too few entries, exactly four entries expected");
        else {
          if (count > 4)
            DCMSR_WARN(
                "GraphicData has too many entries, exactly four entries "
                "expected");
          result = OFTrue;
        }
        break;
      case DSRTypes::GT3_Ellipsoid:
        if (count < 6)
          DCMSR_WARN(
              "GraphicData has too few entries, exactly six entries expected");
        else {
          if (count > 6)
            DCMSR_WARN(
                "GraphicData has too many entries, exactly six entries "
                "expected");
          result = OFTrue;
        }
        break;
