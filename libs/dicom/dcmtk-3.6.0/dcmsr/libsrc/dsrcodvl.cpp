...
DSRCodedEntryValue& DSRCodedEntryValue::operator=(
    const DSRCodedEntryValue& codedEntryValue) {
  /* do not check since this would unexpected to the user */
  CodeValue = codedEntryValue.CodeValue;
  CodingSchemeDesignator = codedEntryValue.CodingSchemeDesignator;
...
OFBool DSRCodedEntryValue::operator==(
    const DSRCodedEntryValue& codedEntryValue) const {
...
OFBool DSRCodedEntryValue::isEmpty() const {
  return CodeValue.empty() && CodingSchemeDesignator.empty() &&
         CodingSchemeVersion.empty() && CodeMeaning.empty();
}
...
void DSRCodedEntryValue::print(STD_NAMESPACE ostream& stream,
  ...
  if (isValid()) {
    ...
    if (printCodeValue) {
      ...
      stream << DSRTypes::convertToPrintString(CodingSchemeDesignator,
                                               printString);
      if (!CodingSchemeVersion.empty())
        stream << "["
               << DSRTypes::convertToPrintString(CodingSchemeVersion,
                                                 printString)
               << "]";
    } else
      stream << ",";
...
OFCondition DSRCodedEntryValue::readItem(DcmItem& dataset,
  ...
  OFCondition result = DSRTypes::getAndCheckStringValueFromDataset(
      dataset, DCM_CodeValue, CodeValue, "1", "1", moduleName);
  if (result.good())
    result = DSRTypes::getAndCheckStringValueFromDataset(
        dataset, DCM_CodingSchemeDesignator, CodingSchemeDesignator, "1", "1",
        moduleName);
  if (result.good()) /* conditional (type 1C) */
    DSRTypes::getAndCheckStringValueFromDataset(
        dataset, DCM_CodingSchemeVersion, CodingSchemeVersion, "1", "1C",
        moduleName);
  if (result.good())
    result = DSRTypes::getAndCheckStringValueFromDataset(
        dataset, DCM_CodeMeaning, CodeMeaning, "1", "1", moduleName);
...
OFCondition DSRCodedEntryValue::writeItem(DcmItem& dataset) const {
  ...
  OFCondition result =
      DSRTypes::putStringValueToDataset(dataset, DCM_CodeValue, CodeValue);
  if (result.good())
    result = DSRTypes::putStringValueToDataset(
        dataset, DCM_CodingSchemeDesignator, CodingSchemeDesignator);
  if (result.good() && !CodingSchemeVersion.empty()) /* conditional (type 1C) */
    result = DSRTypes::putStringValueToDataset(dataset, DCM_CodingSchemeVersion,
                                               CodingSchemeVersion);
  if (result.good())
    result = DSRTypes::putStringValueToDataset(dataset, DCM_CodeMeaning,
                                               CodeMeaning);
  return result;
}
...
OFCondition DSRCodedEntryValue::readXML(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    if (doc.hasAttribute(cursor, "codValue")) {
      doc.getStringFromAttribute(cursor, CodeValue, "codValue",
                                 OFTrue /*encoding*/);
      doc.getStringFromAttribute(cursor, CodingSchemeDesignator, "codScheme",
                                 OFTrue /*encoding*/);
      doc.getStringFromAttribute(cursor, CodingSchemeVersion, "codVersion",
                                 OFTrue /*encoding*/, OFFalse /*required*/);
      doc.getStringFromNodeContent(cursor, CodeMeaning, NULL /*name*/,
                                   OFTrue /*encoding*/);
    } else {
      ...
      while (cursor.valid()) {
        ...
        if (doc.matchNode(cursor, "scheme")) {
          doc.getStringFromNodeContent(
              doc.getNamedNode(cursor.getChild(), "designator"),
              CodingSchemeDesignator, NULL /*name*/, OFTrue /*encoding*/,
              OFFalse /*clearString*/);
          doc.getStringFromNodeContent(
              doc.getNamedNode(cursor.getChild(), "version",
                               OFFalse /*required*/),
              CodingSchemeVersion, NULL /*name*/, OFTrue /*encoding*/,
              OFFalse /*clearString*/);
        } else {
          doc.getStringFromNodeContent(cursor, CodeValue, "value",
                                       OFTrue /*encoding*/,
                                       OFFalse /*clearString*/);
          doc.getStringFromNodeContent(cursor, CodeMeaning, "meaning",
                                       OFTrue /*encoding*/,
                                       OFFalse /*clearString*/);
        }
...
OFCondition DSRCodedEntryValue::writeXML(STD_NAMESPACE ostream& stream,
  ...
  if (flags & DSRTypes::XF_codeComponentsAsAttribute) {
    stream << " codValue=\""
           << DSRTypes::convertToXMLString(CodeValue, tmpString) << "\"";
    stream << " codScheme=\""
           << DSRTypes::convertToXMLString(CodingSchemeDesignator, tmpString)
           << "\"";
    if (!CodingSchemeVersion.empty() || (flags & DSRTypes::XF_writeEmptyTags))
      stream << " codVersion=\""
             << DSRTypes::convertToXMLString(CodingSchemeVersion, tmpString)
             << "\"";
    stream << ">";  // close open bracket from calling routine
    stream << DSRTypes::convertToXMLString(CodeMeaning, tmpString);
  } else {
    DSRTypes::writeStringValueToXML(stream, CodeValue, "value",
                                    (flags & DSRTypes::XF_writeEmptyTags) > 0);
    stream << "<scheme>" << OFendl;
    DSRTypes::writeStringValueToXML(stream, CodingSchemeDesignator,
                                    "designator",
                                    (flags & DSRTypes::XF_writeEmptyTags) > 0);
    DSRTypes::writeStringValueToXML(stream, CodingSchemeVersion, "version",
                                    (flags & DSRTypes::XF_writeEmptyTags) > 0);
    stream << "</scheme>" << OFendl;
    DSRTypes::writeStringValueToXML(stream, CodeMeaning, "meaning",
                                    (flags & DSRTypes::XF_writeEmptyTags) > 0);
  }
  return EC_Normal;
}
...
OFCondition DSRCodedEntryValue::renderHTML(STD_NAMESPACE ostream& stream,
  ...
  if (flags & DSRTypes::HF_useCodeDetailsTooltip) {
    ...
    stream << DSRTypes::convertToHTMLString(CodeValue, htmlString, flags)
           << ", ";
    stream << DSRTypes::convertToHTMLString(CodingSchemeDesignator, htmlString,
                                            flags);
    if (!CodingSchemeVersion.empty())
      stream << " ["
             << DSRTypes::convertToHTMLString(CodingSchemeVersion, htmlString,
                                              flags)
             << "]";
    stream << ", &quot;"
           << DSRTypes::convertToHTMLString(CodeMeaning, htmlString, flags)
           << "&quot;)\">";
    /* render value */
    if (valueFirst)
      stream << DSRTypes::convertToHTMLString(CodeValue, htmlString, flags);
    ...
    if (fullCode) {
      ...
      if (!valueFirst)
        stream << DSRTypes::convertToHTMLString(CodeValue, htmlString, flags)
               << ", ";
      stream << DSRTypes::convertToHTMLString(CodingSchemeDesignator,
                                              htmlString, flags);
      if (!CodingSchemeVersion.empty())
        stream << " ["
               << DSRTypes::convertToHTMLString(CodingSchemeVersion, htmlString,
...
OFCondition DSRCodedEntryValue::getValue(
    DSRCodedEntryValue& codedEntryValue) const {
  codedEntryValue = *this;
  return EC_Normal;
}

OFCondition DSRCodedEntryValue::setValue(
    const DSRCodedEntryValue& codedEntryValue) {
  return setCode(
      codedEntryValue.CodeValue, codedEntryValue.CodingSchemeDesignator,
