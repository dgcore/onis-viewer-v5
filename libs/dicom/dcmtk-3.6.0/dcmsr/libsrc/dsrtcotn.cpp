...
OFBool DSRTCoordTreeNode::isValid() const {
  return DSRDocumentTreeNode::isValid() &&
         DSRTemporalCoordinatesValue::isValid();
}
...
OFCondition DSRTCoordTreeNode::writeXML(STD_NAMESPACE ostream& stream,
  ...
  stream << " type=\""
         << temporalRangeTypeToEnumeratedValue(getTemporalRangeType()) << "\"";
  stream << ">" << OFendl;
...
OFCondition DSRTCoordTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    result = setTemporalRangeType(enumeratedValueToTemporalRangeType(
        doc.getStringFromAttribute(cursor, tmpString, "type")));
    if (result.good()) {
...
OFCondition DSRTCoordTreeNode::renderHTMLContentItem(
  ...
  if (result.good()) {
    result = DSRTemporalCoordinatesValue::renderHTML(docStream, annexStream,
                                                     annexNumber, flags);
    docStream << OFendl;
  }
  return result;
