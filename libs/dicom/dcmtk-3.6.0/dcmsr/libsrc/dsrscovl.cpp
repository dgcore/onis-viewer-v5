...
DSRSpatialCoordinatesValue::DSRSpatialCoordinatesValue()
    : GraphicType(DSRTypes::GT_invalid), GraphicDataList() {}

DSRSpatialCoordinatesValue::DSRSpatialCoordinatesValue(
    const DSRTypes::E_GraphicType graphicType)
    : GraphicType(graphicType), GraphicDataList() {}
...
DSRSpatialCoordinatesValue& DSRSpatialCoordinatesValue::operator=(
    const DSRSpatialCoordinatesValue& coordinatesValue) {
  GraphicType = coordinatesValue.GraphicType;
  GraphicDataList = coordinatesValue.GraphicDataList;
  return *this;
...
OFCondition DSRSpatialCoordinatesValue::read(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckStringValueFromDataset(
      dataset, DCM_GraphicType, tmpString, "1", "1", "SCOORD content item");
  if (result.good()) {
...
OFCondition DSRSpatialCoordinatesValue::write(DcmItem& dataset) const {
  ...
  OFCondition result = DSRTypes::putStringValueToDataset(
      ...
      DSRTypes::graphicTypeToEnumeratedValue(GraphicType));
  /* write GraphicData */
  if (result.good()) {
    if (!GraphicDataList.isEmpty())
...
OFCondition DSRSpatialCoordinatesValue::renderHTML(
...
OFCondition DSRSpatialCoordinatesValue::setGraphicType(
    const DSRTypes::E_GraphicType graphicType) {
...
OFCondition DSRSpatialCoordinatesValue::getValue(
    DSRSpatialCoordinatesValue& coordinatesValue) const {
  coordinatesValue = *this;
  return EC_Normal;
}

OFCondition DSRSpatialCoordinatesValue::setValue(
    const DSRSpatialCoordinatesValue& coordinatesValue) {
  OFCondition result = EC_IllegalParameter;
  if (checkData(coordinatesValue.GraphicType,
                coordinatesValue.GraphicDataList)) {
    GraphicType = coordinatesValue.GraphicType;
    GraphicDataList = coordinatesValue.GraphicDataList;
    result = EC_Normal;
  }
...
OFBool DSRSpatialCoordinatesValue::checkData(
    const DSRTypes::E_GraphicType graphicType,
    ...
    switch (graphicType) {
      ...
      case DSRTypes::GT_Polyline:
        /*              // not required any more according to CP-233
                        if (graphicDataList.getItem(1) !=
           graphicDataList.getItem(count)) DCMSR_WARN("First and last entry in
           GraphicData are not equal (POLYLINE)");
        */
        result = OFTrue;
        break;
      case DSRTypes::GT_Circle:
        if (count < 2)
          DCMSR_WARN(
              "GraphicData has too few entries, exactly two entries expected");
        else {
          if (count > 2)
            DCMSR_WARN(
                "GraphicData has too many entries, exactly two entries "
                "expected");
          result = OFTrue;
        }
        break;
