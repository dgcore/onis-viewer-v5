...
DSRReferencedDatetimeList::DSRReferencedDatetimeList(
    const DSRReferencedDatetimeList& lst)
    : DSRListOfItems<OFString>(lst) {}

DSRReferencedDatetimeList::~DSRReferencedDatetimeList() {}

DSRReferencedDatetimeList& DSRReferencedDatetimeList::operator=(
    const DSRReferencedDatetimeList& lst) {
...
OFCondition DSRReferencedDatetimeList::write(DcmItem& dataset) const {
  ...
  if (result.good())
    result =
        DSRTypes::addElementToDataset(result, dataset, new DcmDateTime(delem),
                                      "1-n", "1", "TCOORD content item");
  return result;
}
