...
DSRXMLDocument::DSRXMLDocument() : Document(NULL), EncodingHandler(NULL) {}
...
#ifdef WITH_LIBXML
OFCondition DSRXMLDocument::read(const OFString& filename, const size_t flags) {
  ...
  if (Document != NULL) {
    ...
    if (flags & XF_validateSchema) {
      ...
      xmlSchemaParserCtxtPtr context =
          xmlSchemaNewParserCtxt(DCMSR_XML_XSD_FILE);
      xmlSchemaSetParserErrors(context, errorFunction, errorFunction,
                               &tmpErrorString);
      /* parse Schema file */
      xmlSchemaPtr schema = xmlSchemaParse(context);
      if (schema != NULL) {
        xmlSchemaValidCtxtPtr validCtx = xmlSchemaNewValidCtxt(schema);
        xmlSchemaSetValidErrors(validCtx, errorFunction, errorFunction,
                                &tmpErrorString);
        ...
        xmlGenericError(xmlGenericErrorContext,
                        "error: failed to compile schema \"%s\"\n",
                        DCMSR_XML_XSD_FILE);
      ...
      xmlSchemaSetValidErrors(context, errorFunction, errorFunction,
                              &tmpErrorString);
      /* validate the document */
    ...
    if ((current != NULL) && isValid) {
      ...
      if (!(flags & XF_useDcmsrNamespace) ||
          (xmlSearchNsByHref(
               Document, current,
               OFreinterpret_cast(const xmlChar*, DCMSR_XML_NAMESPACE_URI)) !=
           NULL)) {
        result = EC_Normal;
      } else
...
#else /* WITH_LIBXML */
OFCondition DSRXMLDocument::read(const OFString&, const size_t) {
  ...
  if ((name != NULL) && (strlen(name) > 0)) {
    ...
    while (current != NULL) {
      ...
      if (current != NULL) {
        ...
        if (xmlStrcmp(current->name,
                      OFreinterpret_cast(const xmlChar*, name)) == 0)
          break;
    ...
    if (current == NULL) {
      ...
      if (required) {
        ...
        DCMSR_ERROR(
            ...
            << name << "' expected at "
            << getFullNodePath(cursor, tmpString, OFFalse /*omitCurrent*/));
      }
    } else {
...
#else /* WITH_LIBXML */
DSRXMLCursor DSRXMLDocument::getNamedNode(const DSRXMLCursor&, const char*,
                                          const OFBool) const {
  DSRXMLCursor result;
  return result;
}
...
OFBool DSRXMLDocument::matchNode(const DSRXMLCursor& cursor,
  ...
  if (cursor.Node != NULL) {
    ...
    if ((name != NULL) && (strlen(name) > 0))
      result = (xmlStrcmp(cursor.Node->name,
                          OFreinterpret_cast(const xmlChar*, name)) == 0);
  }
  return result;
}
#else /* WITH_LIBXML */
OFBool DSRXMLDocument::matchNode(const DSRXMLCursor&, const char*) const {
  return OFFalse;
}
#endif
...
OFCondition DSRXMLDocument::checkNode(const DSRXMLCursor& cursor,
  ...
  if ((name != NULL) && (strlen(name) > 0)) {
    ...
    if (cursor.Node != NULL) {
      /* check whether node has expected name */
      if (xmlStrcmp(cursor.Node->name,
                    OFreinterpret_cast(const xmlChar*, name)) != 0) {
        DCMSR_ERROR("Document of the wrong type, was '"
                    << OFreinterpret_cast(const char*, cursor.Node->name)
                    << "', '" << name << "' expected");
...
#else /* WITH_LIBXML */
OFCondition DSRXMLDocument::checkNode(const DSRXMLCursor&, const char*) const {
  return EC_IllegalCall;
}
#endif
...
#else /* WITH_LIBXML */
OFBool DSRXMLDocument::convertUtf8ToCharset(const xmlChar*, OFString&) const {
  return OFFalse;
}
#endif
...
OFBool DSRXMLDocument::hasAttribute(const DSRXMLCursor& cursor,
  ...
  if (cursor.Node != NULL) {
    ...
    if ((name != NULL) && (strlen(name) > 0))
      result = (xmlHasProp(cursor.Node,
                           OFreinterpret_cast(const xmlChar*, name)) != NULL);
  }
  return result;
}
#else /* WITH_LIBXML */
OFBool DSRXMLDocument::hasAttribute(const DSRXMLCursor&, const char*) const {
  return OFFalse;
}
#endif
...
OFString& DSRXMLDocument::getStringFromAttribute(const DSRXMLCursor& cursor,
  ...
  if ((cursor.Node != NULL) && (name != NULL) && (strlen(name) > 0)) {
    ...
    xmlChar* attrVal =
        xmlGetProp(cursor.Node, OFreinterpret_cast(const xmlChar*, name));
...
#ifdef WITH_LIBXML
OFCondition DSRXMLDocument::getElementFromAttribute(
    const DSRXMLCursor& cursor, DcmElement& delem, const char* name,
    const OFBool encoding, const OFBool required) const {
  OFCondition result = SR_EC_InvalidDocument;
  /* check whether parameters are valid */
  if ((cursor.Node != NULL) && (name != NULL) && (strlen(name) > 0)) {
    /* get the XML attribute value */
...
#ifdef WITH_LIBXML
OFString& DSRXMLDocument::getStringFromNodeContent(
    const DSRXMLCursor& cursor, OFString& stringValue, const char* name,
    const OFBool encoding, const OFBool clearString) const {
  if (clearString)
    stringValue.clear();
  if (cursor.Node != NULL) {
    /* compare element name if required */
    if ((name == NULL) ||
        (xmlStrcmp(cursor.Node->name,
                   OFreinterpret_cast(const xmlChar*, name)) == 0)) {
...
#ifdef WITH_LIBXML
OFCondition DSRXMLDocument::getElementFromNodeContent(
    const DSRXMLCursor& cursor, DcmElement& delem, const char* name,
...
OFString& DSRXMLDocument::getFullNodePath(const DSRXMLCursor& cursor,
  ...
  if (cursor.Node != NULL) {
    ...
    if (!omitCurrent)
      stringValue =
          OFSTRING_GUARD(OFreinterpret_cast(const char*, current->name));
...
OFString& DSRXMLDocument::getFullNodePath(const DSRXMLCursor&,
                                          OFString& stringValue, const OFBool) {
  stringValue.clear();
  return stringValue;
}
#endif
...
#ifdef WITH_LIBXML
DSRTypes::E_ValueType DSRXMLDocument::getValueTypeFromNode(
    const DSRXMLCursor& cursor) const {
  E_ValueType valueType = VT_invalid;
  if (cursor.valid()) {
    if (xmlStrcmp(cursor.Node->name,
                  OFreinterpret_cast(const xmlChar*, "item")) == 0) {
      /* check for "ref" attribute */
      if (xmlHasProp(cursor.Node, OFreinterpret_cast(const xmlChar*, "ref")))
        valueType = DSRTypes::VT_byReference;
      else {
        /* get the XML attribute value */
        xmlChar* attrVal = xmlGetProp(
            cursor.Node, OFreinterpret_cast(const xmlChar*, "valType"));
        /* try to convert attribute value to SR value type */
        valueType =
            definedTermToValueType(OFreinterpret_cast(const char*, attrVal));
        /* free allocated memory */
        xmlFree(attrVal);
      }
    } else {
      /* try to convert tag name to SR value type */
      valueType = xmlTagNameToValueType(
          OFreinterpret_cast(const char*, cursor.Node->name));
    }
  }
  return valueType;
...
#else /* WITH_LIBXML */
DSRTypes::E_ValueType DSRXMLDocument::getValueTypeFromNode(
    const DSRXMLCursor&) const {
  return VT_invalid;
}
#endif

DSRTypes::E_RelationshipType DSRXMLDocument::getRelationshipTypeFromNode(
    const DSRXMLCursor& cursor) const {
  ...
  if (cursor.valid()) {
    ...
    if (hasAttribute(cursor, "relType")) {
      ...
      relationshipType = definedTermToRelationshipType(
          getStringFromAttribute(cursor, tmpString, "relType"));
    } else {
      const DSRXMLCursor childCursor =
          getNamedNode(cursor.getChild(), "relationship");
      /* try to convert content of "relationship" tag to SR relationship type */
      if (childCursor.valid())
        relationshipType = definedTermToRelationshipType(
            getStringFromNodeContent(childCursor, tmpString));
    }
  }
  return relationshipType;
}

void DSRXMLDocument::printUnexpectedNodeWarning(
    const DSRXMLCursor& cursor) const {
  OFString tmpString;
  DCMSR_WARN("Unexpected node '" << getFullNodePath(cursor, tmpString)
                                 << "', skipping");
}
...
void DSRXMLDocument::printMissingAttributeError(const DSRXMLCursor& cursor,
  ...
  if (name != NULL) {
    OFString tmpString;
    DCMSR_ERROR("XML attribute '" << name << "' missing/empty in "
                                  << getFullNodePath(cursor, tmpString));
  }
}
...
void DSRXMLDocument::printGeneralNodeError(const DSRXMLCursor& cursor,
  ...
  if (result.bad()) {
    OFString tmpString;
    DCMSR_ERROR("Parsing node " << getFullNodePath(cursor, tmpString) << " ("
                                << result.text() << ")");
  }
}
