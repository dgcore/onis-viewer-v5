...
DSRDateTimeTreeNode::DSRDateTimeTreeNode(
    ...
    : DSRDocumentTreeNode(relationshipType, VT_DateTime), DSRStringValue() {}

DSRDateTimeTreeNode::DSRDateTimeTreeNode(
    const E_RelationshipType relationshipType, const OFString& stringValue)
...
OFCondition DSRDateTimeTreeNode::writeXML(STD_NAMESPACE ostream& stream,
  ...
  DcmDateTime::getISOFormattedDateTimeFromString(
      getValue(), tmpString, OFTrue /*seconds*/, OFFalse /*fraction*/,
      OFFalse /*timeZone*/, OFFalse /*createMissingPart*/,
      "T" /*dateTimeSeparator*/);
  writeStringValueToXML(stream, tmpString, "value",
...
OFCondition DSRDateTimeTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  OFCondition result = setValue(getValueFromXMLNodeContent(
      doc, doc.getNamedNode(cursor.gotoChild(), "value"), tmpString));
  if (result == EC_IllegalParameter)
    result = SR_EC_InvalidValue;
  return result;
}

OFString& DSRDateTimeTreeNode::getValueFromXMLNodeContent(
...
OFCondition DSRDateTimeTreeNode::renderHTMLContentItem(
