...
OFBool DSRByReferenceTreeNode::isValid() const {
  ...
  return DSRDocumentTreeNode::isValid() && getConceptName().isEmpty() &&
         ValidReference;
}
...
OFCondition DSRByReferenceTreeNode::readContentItem(DcmItem& dataset) {
  ...
  OFCondition result = getAndCheckElementFromDataset(
      dataset, delem, "1-n", "1C", "by-reference relationship");
  if (result.good()) {
    ...
    for (unsigned long i = 0; i < count; i++) {
      ...
      if (delem.getUint32(value, i).good())
        ReferencedContentItem +=
            numberToString(OFstatic_cast(size_t, value), buffer);
    }
  }
  return result;
}
  ...
  if (checkForValidUIDFormat(ReferencedContentItem)) {
    ...
    do {
      ...
      if (posEnd == OFString_npos)
        delem.putUint32(DSRTypes::stringToNumber(
                            ReferencedContentItem.substr(posStart).c_str()),
                        i);
      else {
        delem.putUint32(
            DSRTypes::stringToNumber(
                ReferencedContentItem.substr(posStart, posEnd - posStart)
                    .c_str()),
            i);
        posStart = posEnd + 1;
      }
    ...
    addElementToDataset(result, dataset, new DcmUnsignedLong(delem), "1-n", "1",
                        "by-reference relationship");
  }
  return result;
}

OFCondition DSRByReferenceTreeNode::readXMLContentItem(
  ...
  if (cursor.valid()) {
    ...
    if (!doc.getStringFromAttribute(cursor, refID, "ref").empty()) {
      ...
      /* this does not mean that the reference is really correct, this will be
       * checked later */
      result = EC_Normal;
    } else
...
OFCondition DSRByReferenceTreeNode::renderHTMLContentItem(
    STD_NAMESPACE ostream& docStream, STD_NAMESPACE ostream& /*annexStream*/,
    const size_t /*nestingLevel*/, size_t& /*annexNumber*/,
    const size_t /*flags*/) const {
  /* render reference string */
  docStream << "Content Item <a href=\"#content_item_" << ReferencedNodeID
            << "\">by-reference</a>" << OFendl;
  return EC_Normal;
}

OFCondition DSRByReferenceTreeNode::setConceptName(
    const DSRCodedEntryValue& /*conceptName*/) {
  /* invalid: no concept name allowed */
  return EC_IllegalCall;
}

OFCondition DSRByReferenceTreeNode::setObservationDateTime(
    const OFString& /*observationDateTime*/) {
  /* invalid: no observation date and time allowed */
  return EC_IllegalCall;
}

OFCondition DSRByReferenceTreeNode::setTemplateIdentification(
