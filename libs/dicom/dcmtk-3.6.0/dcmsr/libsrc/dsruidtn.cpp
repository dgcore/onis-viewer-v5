...
DSRUIDRefTreeNode::DSRUIDRefTreeNode(const E_RelationshipType relationshipType)
    : DSRDocumentTreeNode(relationshipType, VT_UIDRef), DSRStringValue() {}
...
OFCondition DSRUIDRefTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  return DSRStringValue::readXML(doc,
                                 doc.getNamedNode(cursor.gotoChild(), "value"));
}

OFCondition DSRUIDRefTreeNode::renderHTMLContentItem(
