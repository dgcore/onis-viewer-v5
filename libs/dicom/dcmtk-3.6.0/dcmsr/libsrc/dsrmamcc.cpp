...
const char* DSRMammographyCadSRConstraintChecker::getRootTemplateIdentifier()
    const {
  return "4000";
}

DSRTypes::E_DocumentType DSRMammographyCadSRConstraintChecker::getDocumentType()
    const {
  return DT_MammographyCadSR;
}

OFBool DSRMammographyCadSRConstraintChecker::checkContentRelationship(
    const E_ValueType sourceValueType,
    const E_RelationshipType relationshipType,
    const E_ValueType targetValueType, const OFBool byReference) const {
  /* the following code implements the constraints of table A.35.5-2 in DICOM
   * PS3.3 */
  OFBool result = OFFalse;
  /* row 1 of the table */
  if ((relationshipType == RT_contains) && !byReference &&
      (sourceValueType == VT_Container)) {
    result = (targetValueType == VT_Code) || (targetValueType == VT_Num) ||
             (targetValueType == VT_SCoord) || (targetValueType == VT_Image) ||
  ...
  /* row 2 of the table */
           ...
           ((sourceValueType == VT_Container) || (sourceValueType == VT_Text) ||
            (sourceValueType == VT_Code) || (sourceValueType == VT_Num))) {
    result = (targetValueType == VT_Text) || (targetValueType == VT_Code) ||
             (targetValueType == VT_Num) || (targetValueType == VT_Date) ||
             (targetValueType == VT_Time) || (targetValueType == VT_PName) ||
             (targetValueType == VT_UIDRef) ||
             (targetValueType == VT_Composite);
  }
  /* row 3 of the table */
  else if ((relationshipType == RT_hasAcqContext) && !byReference &&
           (sourceValueType == VT_Image)) {
    result = (targetValueType == VT_Text) || (targetValueType == VT_Code) ||
             (targetValueType == VT_Date) || (targetValueType == VT_Time) ||
             (targetValueType == VT_Num);
  }
  /* row 4 of the table */
  ...
  /* row 5 the table */
    ...
    result = (targetValueType == VT_Container) ||
             ...
             (targetValueType == VT_Num) || (targetValueType == VT_Date) ||
             (targetValueType == VT_Image) || (targetValueType == VT_SCoord) ||
             (targetValueType == VT_UIDRef);
  }
  /* row 6 of the table */
    ...
    /* by-reference allowed */
    result = (targetValueType == VT_Code) || (targetValueType == VT_Num) ||
             (targetValueType == VT_SCoord) ||
             (targetValueType == VT_Container) ||
             (targetValueType == VT_Text) || (targetValueType == VT_Image);
  }
  /* row 7 of the table */
  else if ((relationshipType == RT_selectedFrom) &&
           (sourceValueType == VT_SCoord)) {
    /* by-reference allowed */
 ...
 *  modification required for CP 492 (has already been added by the last
 * commit).
 *
