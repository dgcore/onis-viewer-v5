...
DSRTemporalCoordinatesValue& DSRTemporalCoordinatesValue::operator=(
    const DSRTemporalCoordinatesValue& coordinatesValue) {
  TemporalRangeType = coordinatesValue.TemporalRangeType;
  SamplePositionList = coordinatesValue.SamplePositionList;
  TimeOffsetList = coordinatesValue.TimeOffsetList;
...
OFBool DSRTemporalCoordinatesValue::isValid() const {
  return checkData(TemporalRangeType, SamplePositionList, TimeOffsetList,
                   DatetimeList);
}

OFBool DSRTemporalCoordinatesValue::isShort(const size_t flags) const {
  return (SamplePositionList.isEmpty() && TimeOffsetList.isEmpty() &&
...
OFCondition DSRTemporalCoordinatesValue::print(STD_NAMESPACE ostream& stream,
  ...
  /* TemporalRangeType */
  stream << "("
         << DSRTypes::temporalRangeTypeToEnumeratedValue(TemporalRangeType);
  /* print data */
  stream << ",";
  /* print only one list */
...
OFCondition DSRTemporalCoordinatesValue::read(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckStringValueFromDataset(
      dataset, DCM_TemporalRangeType, tmpString, "1", "1",
      "TCOORD content item");
  if (result.good()) {
    ...
    if (TemporalRangeType == DSRTypes::TRT_invalid)
      DSRTypes::printUnknownValueWarningMessage("TemporalRangeType",
                                                tmpString.c_str());
    ...
    if (!checkData(TemporalRangeType, SamplePositionList, TimeOffsetList,
                   DatetimeList))
      result = SR_EC_InvalidValue;
  }
  return result;
...
OFCondition DSRTemporalCoordinatesValue::write(DcmItem& dataset) const {
  ...
  checkData(TemporalRangeType, SamplePositionList, TimeOffsetList,
            DatetimeList);
  return result;
}

OFCondition DSRTemporalCoordinatesValue::renderHTML(
...
OFCondition DSRTemporalCoordinatesValue::setValue(
    const DSRTemporalCoordinatesValue& coordinatesValue) {
  OFCondition result = EC_IllegalParameter;
  if (checkData(coordinatesValue.TemporalRangeType,
                coordinatesValue.SamplePositionList,
                coordinatesValue.TimeOffsetList,
                coordinatesValue.DatetimeList)) {
    TemporalRangeType = coordinatesValue.TemporalRangeType;
    SamplePositionList = coordinatesValue.SamplePositionList;
    TimeOffsetList = coordinatesValue.TimeOffsetList;
    DatetimeList = coordinatesValue.DatetimeList;
...
OFCondition DSRTemporalCoordinatesValue::getValue(
    DSRTemporalCoordinatesValue& coordinatesValue) const {
  coordinatesValue = *this;
  return EC_Normal;
}

OFCondition DSRTemporalCoordinatesValue::setTemporalRangeType(
    const DSRTypes::E_TemporalRangeType temporalRangeType) {
  OFCondition result = EC_IllegalParameter;
  if (temporalRangeType != DSRTypes::TRT_invalid) {
    TemporalRangeType = temporalRangeType;
...
OFBool DSRTemporalCoordinatesValue::checkData(
    const DSRTypes::E_TemporalRangeType temporalRangeType,
  ...
  if (list1 && list2 && list3)
    DCMSR_WARN(
        "ReferencedSamplePositions/TimeOffsets/Datetime present in TCOORD "
        "content item");
  else if (list1 && list2)
    DCMSR_WARN(
        "ReferencedSamplePositions/TimeOffsets present in TCOORD content item");
  else if (list1 && list3)
    DCMSR_WARN(
        "ReferencedSamplePositions/Datetime present in TCOORD content item");
  else if (list2 && list3)
    DCMSR_WARN("ReferencedTimeOffsets/Datetime present in TCOORD content item");
  else if (!list1 && !list2 && !list3) {
    DCMSR_WARN(
        "ReferencedSamplePositions/TimeOffsets/Datetime empty in TCOORD "
        "content item");
