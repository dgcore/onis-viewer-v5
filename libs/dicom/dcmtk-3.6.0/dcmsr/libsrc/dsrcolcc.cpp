...
DSRTypes::E_DocumentType DSRColonCadSRConstraintChecker::getDocumentType()
    const {
  return DT_ColonCadSR;
}

OFBool DSRColonCadSRConstraintChecker::checkContentRelationship(
    const E_ValueType sourceValueType,
    const E_RelationshipType relationshipType,
    const E_ValueType targetValueType, const OFBool byReference) const {
  /* the following code implements the constraints of table A.35.10-2 in DICOM
   * PS3.3 */
  ...
  if ((relationshipType == RT_contains) && !byReference &&
    ...
    result = (targetValueType == VT_Code) || (targetValueType == VT_Num) ||
             ...
             (targetValueType == VT_UIDRef) || (targetValueType == VT_Date) ||
  ...
  /* row 3 of the table */
    ...
    result = (targetValueType == VT_Text) || (targetValueType == VT_Code) ||
             ...
             (targetValueType == VT_Num) || (targetValueType == VT_Container);
  }
  /* row 4 of the table */
  ...
  /* row 5 the table */
    ...
    result = (targetValueType == VT_Container) ||
             ...
             (targetValueType == VT_SCoord3D) || (targetValueType == VT_UIDRef);
  }
  /* row 6 of the table */
    ...
    result = (targetValueType == VT_Code) || (targetValueType == VT_Num) ||
             (targetValueType == VT_Image) || (targetValueType == VT_SCoord) ||
             (targetValueType == VT_SCoord3D) ||
             (targetValueType == VT_Container) || (targetValueType == VT_Text);
  }
  /* row 7 of the table */
  else if ((relationshipType == RT_selectedFrom) && !byReference &&
