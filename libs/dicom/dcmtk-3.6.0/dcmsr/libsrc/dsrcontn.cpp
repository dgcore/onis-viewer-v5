...
OFBool DSRContainerTreeNode::isValid() const {
  ...
  return DSRDocumentTreeNode::isValid() &&
         (ContinuityOfContent != COC_invalid) &&
...
OFCondition DSRContainerTreeNode::readContentItem(DcmItem& dataset) {
  ...
  OFCondition result = getAndCheckStringValueFromDataset(
      dataset, DCM_ContinuityOfContent, tmpString, "1", "1",
  ...
  if (result.good()) {
    ...
    if (ContinuityOfContent == COC_invalid) {
      printUnknownValueWarningMessage("ContinuityOfContent value",
                                      tmpString.c_str());
...
OFCondition DSRContainerTreeNode::writeContentItem(DcmItem& dataset) const {
  ...
  return putStringValueToDataset(
      dataset, DCM_ContinuityOfContent,
      continuityOfContentToEnumeratedValue(ContinuityOfContent));
}
...
OFCondition DSRContainerTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    ...
    ContinuityOfContent = enumeratedValueToContinuityOfContent(
...
OFCondition DSRContainerTreeNode::renderHTMLContentItem(
  ...
  if (nestingLevel > 0) {
    ...
    if (!getConceptName().getCodeMeaning().empty()) {
      ...
      docStream << "<h" << section << ">";
      getConceptName().renderHTML(docStream, flags,
                                  (flags & HF_renderConceptNameCodes) &&
                                      getConceptName().isValid() /*fullCode*/);
    ...
    if (!getObservationDateTime().empty()) {
      ...
      if (flags & HF_XHTML11Compatibility)
        ...
        docStream << "<small>";
      docStream << "(observed: "
                << dicomToReadableDateTime(getObservationDateTime(), tmpString)
                << ")";
      if (flags & HF_XHTML11Compatibility)
...
OFCondition DSRContainerTreeNode::renderHTML(STD_NAMESPACE ostream& docStream,
  ...
  if (result.good()) {
    ...
    if (ContinuityOfContent == COC_Continuous)
      result =
          renderHTMLChildNodes(docStream, annexStream, nestingLevel,
                               annexNumber, flags & ~HF_renderItemsSeparately);
...
OFCondition DSRContainerTreeNode::setContinuityOfContent(
    const E_ContinuityOfContent continuityOfContent) {
