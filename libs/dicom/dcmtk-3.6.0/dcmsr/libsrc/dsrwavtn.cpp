...
OFCondition DSRWaveformTreeNode::readXMLContentItem(const DSRXMLDocument& doc,
  ...
  return DSRWaveformReferenceValue::readXML(
      doc, doc.getNamedNode(cursor.gotoChild(), "value"));
}

OFCondition DSRWaveformTreeNode::renderHTMLContentItem(
  ...
  /* render Reference */
  if (result.good()) {
    result = DSRWaveformReferenceValue::renderHTML(docStream, annexStream,
                                                   annexNumber, flags);
    docStream << OFendl;
  }
  return result;
