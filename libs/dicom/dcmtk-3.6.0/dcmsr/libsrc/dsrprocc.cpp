...
const char* DSRProcedureLogConstraintChecker::getRootTemplateIdentifier()
    const {
  return "3001";
}

DSRTypes::E_DocumentType DSRProcedureLogConstraintChecker::getDocumentType()
    const {
  return DT_ProcedureLog;
}

OFBool DSRProcedureLogConstraintChecker::checkContentRelationship(
    const E_ValueType sourceValueType,
    const E_RelationshipType relationshipType,
    const E_ValueType targetValueType, const OFBool byReference) const {
  /* the following code implements the constraints of table A.35.7-2 in DICOM
   * PS3.3 */
  ...
  if (!byReference) {
    ...
    if ((relationshipType == RT_contains) &&
      ...
      result = (targetValueType == VT_Text) || (targetValueType == VT_Code) ||
               (targetValueType == VT_Num) || (targetValueType == VT_PName) ||
