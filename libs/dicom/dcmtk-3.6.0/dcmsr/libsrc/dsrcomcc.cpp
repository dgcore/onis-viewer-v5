...
const char* DSRComprehensiveSRConstraintChecker::getRootTemplateIdentifier()
    const {
  return NULL;
}

DSRTypes::E_DocumentType DSRComprehensiveSRConstraintChecker::getDocumentType()
    const {
  return DT_ComprehensiveSR;
}

OFBool DSRComprehensiveSRConstraintChecker::checkContentRelationship(
    const E_ValueType sourceValueType,
    const E_RelationshipType relationshipType,
    const E_ValueType targetValueType, const OFBool byReference) const {
  /* the following code implements the contraints of table A.35.3-2 in DICOM
   * PS3.3 */
  ...
  if ((relationshipType == RT_contains) && (sourceValueType == VT_Container)) {
    result =
        ...
        (targetValueType == VT_Num) || (targetValueType == VT_DateTime) ||
        ...
        (targetValueType == VT_Composite) || (targetValueType == VT_Image) ||
  ...
  /* row 5 and 6 of the table */
    ...
    result =
        ...
        (targetValueType == VT_UIDRef) || (targetValueType == VT_PName) ||
        (targetValueType == VT_Composite) || (targetValueType == VT_Image) ||
        (targetValueType == VT_Waveform) || (targetValueType == VT_SCoord) ||
  ...
  /* row 8 of the table */
           ...
           (sourceValueType == VT_TCoord)) {
    result = (targetValueType == VT_SCoord) || (targetValueType == VT_Image) ||
             (targetValueType == VT_Waveform);
  }
  return result;
}
 ...
 *  Removed outdated comment copied from a previous edition of the DICOM
 * standard and removed references to CP 359 and 571.
 *
