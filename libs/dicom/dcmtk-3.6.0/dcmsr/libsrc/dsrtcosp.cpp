...
DSRReferencedSamplePositionList::DSRReferencedSamplePositionList(
    const DSRReferencedSamplePositionList& lst)
    : DSRListOfItems<Uint32>(lst) {}

DSRReferencedSamplePositionList::~DSRReferencedSamplePositionList() {}

DSRReferencedSamplePositionList& DSRReferencedSamplePositionList::operator=(
    const DSRReferencedSamplePositionList& lst) {
  DSRListOfItems<Uint32>::operator=(lst);
  return *this;
}

OFCondition DSRReferencedSamplePositionList::print(
    STD_NAMESPACE ostream& stream, const size_t flags,
...
OFCondition DSRReferencedSamplePositionList::read(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckElementFromDataset(
      dataset, delem, "1-n", "1C", "TCOORD content item");
  if (result.good()) {
    /* clear internal list */
    clear();
    Uint32 value = 0;
...
OFCondition DSRReferencedSamplePositionList::write(DcmItem& dataset) const {
  ...
  if (result.good())
    result = DSRTypes::addElementToDataset(result, dataset,
                                           new DcmUnsignedLong(delem), "1-n",
                                           "1", "TCOORD content item");
  return result;
}

OFCondition DSRReferencedSamplePositionList::putString(
