...
DSRNumericMeasurementValue::DSRNumericMeasurementValue()
    : NumericValue(), MeasurementUnit(), ValueQualifier() {}

DSRNumericMeasurementValue::DSRNumericMeasurementValue(
    const OFString& numericValue, const DSRCodedEntryValue& measurementUnit)
    : NumericValue(), MeasurementUnit(), ValueQualifier() {
...
DSRNumericMeasurementValue& DSRNumericMeasurementValue::operator=(
    const DSRNumericMeasurementValue& numericMeasurement) {
  /* do not check since this would unexpected to the user */
  NumericValue = numericMeasurement.NumericValue;
  MeasurementUnit = numericMeasurement.MeasurementUnit;
...
OFCondition DSRNumericMeasurementValue::print(STD_NAMESPACE ostream& stream,
  ...
  if (isEmpty()) {
    ...
    stream << "\"" << DSRTypes::convertToPrintString(NumericValue, printString)
           << "\" ";
    MeasurementUnit.print(stream, OFTrue /*printCodeValue*/,
                          OFTrue /*printInvalid*/);
  }
  return EC_Normal;
}
...
OFCondition DSRNumericMeasurementValue::readXML(const DSRXMLDocument& doc,
  ...
  if (cursor.valid()) {
    cursor.gotoChild();
    /* get "value" element (might be absent since "Measured Value Sequence" is
     * type 2) */
    if (!doc.getStringFromNodeContent(
                doc.getNamedNode(cursor, "value", OFFalse /*required*/),
                NumericValue)
             .empty()) {
      ...
      result = EC_Normal;
    if (result.good()) {
      /* get "qualifier" element (optional, do not report if absent or
       * erroneous) */
      ValueQualifier.readXML(
          doc, doc.getNamedNode(cursor, "qualifier", OFFalse /*required*/));
    }
    if (!isValid())
...
OFCondition DSRNumericMeasurementValue::writeXML(STD_NAMESPACE ostream& stream,
                                                 const size_t flags) const {
  DSRTypes::writeStringValueToXML(stream, NumericValue, "value",
  ...
  if (!MeasurementUnit.isEmpty() || (flags & DSRTypes::XF_writeEmptyTags)) {
    ...
    if (flags & DSRTypes::XF_codeComponentsAsAttribute)
      stream
          << "<unit";  // bracket ">" is closed in the next writeXML() routine
    else
      stream << "<unit>" << OFendl;
  ...
  if (!ValueQualifier.isEmpty() || (flags & DSRTypes::XF_writeEmptyTags)) {
    ...
    if (flags & DSRTypes::XF_codeComponentsAsAttribute)
      stream << "<qualifier";  // bracket ">" is closed in the next writeXML()
                               // routine
    else
...
OFCondition DSRNumericMeasurementValue::readItem(DcmItem& dataset) {
  ...
  OFCondition result = DSRTypes::getAndCheckStringValueFromDataset(
      dataset, DCM_NumericValue, NumericValue, "1", "1",
      "MeasuredValueSequence");
  if (result.good()) {
    /* read MeasurementUnitsCodeSequence */
    result = MeasurementUnit.readSequence(
        dataset, DCM_MeasurementUnitsCodeSequence, "1" /*type*/);
  }
  return result;
}
...
OFCondition DSRNumericMeasurementValue::writeItem(DcmItem& dataset) const {
  /* write NumericValue */
  OFCondition result = DSRTypes::putStringValueToDataset(
      dataset, DCM_NumericValue, NumericValue);
  /* write MeasurementUnitsCodeSequence */
  if (result.good())
    result = MeasurementUnit.writeSequence(dataset,
...
OFCondition DSRNumericMeasurementValue::readSequence(DcmItem& dataset) {
  ...
  if (result.good()) {
    ...
    ValueQualifier.readSequence(dataset, DCM_NumericValueQualifierCodeSequence,
                                "3" /*type*/);
  }
  return result;
}
...
OFCondition DSRNumericMeasurementValue::writeSequence(DcmItem& dataset) const {
  ...
  if (result.good()) {
    ...
    if (!ValueQualifier.isEmpty())
      ValueQualifier.writeSequence(dataset,
                                   DCM_NumericValueQualifierCodeSequence);
  }
  return result;
}

OFCondition DSRNumericMeasurementValue::renderHTML(
  ...
  if (isEmpty()) {
    ...
    const OFBool fullCode = (flags & DSRTypes::HF_renderNumericUnitCodes) &&
                            ((flags & DSRTypes::HF_renderInlineCodes) ||
                             (flags & DSRTypes::HF_renderItemsSeparately));
    ...
    docStream << DSRTypes::convertToHTMLString(NumericValue, htmlString, flags)
              << " ";
    /* render full code of the measurement unit (value first?) or code value
     * only */
    MeasurementUnit.renderHTML(
        docStream, flags, fullCode,
        (flags & DSRTypes::HF_useCodeMeaningAsUnit) == 0 /*valueFirst*/);
    if (!fullCode || (flags & DSRTypes::HF_useCodeDetailsTooltip)) {
  ...
  if (!ValueQualifier.isEmpty()) {
    ...
    docStream << " [";
    ValueQualifier.renderHTML(
        docStream, flags,
        (flags & DSRTypes::HF_renderInlineCodes) > 0 /*fullCode*/);
    docStream << "]";
  }
  return EC_Normal;
}

OFCondition DSRNumericMeasurementValue::getValue(
    DSRNumericMeasurementValue& numericMeasurement) const {
  numericMeasurement = *this;
  return EC_Normal;
}

OFCondition DSRNumericMeasurementValue::getMeasurementUnit(
    DSRCodedEntryValue& measurementUnit) const {
  measurementUnit = MeasurementUnit;
  return EC_Normal;
}

OFCondition DSRNumericMeasurementValue::setValue(
    const DSRNumericMeasurementValue& numericMeasurement) {
  return setValue(numericMeasurement.NumericValue,
...
OFCondition DSRNumericMeasurementValue::setNumericValue(
...
OFCondition DSRNumericMeasurementValue::setMeasurementUnit(
...
OFCondition DSRNumericMeasurementValue::setNumericValueQualifier(
    const DSRCodedEntryValue& valueQualifier) {
  OFCondition result = EC_IllegalParameter;
...
OFBool DSRNumericMeasurementValue::checkNumericValue(
    const OFString& numericValue) const {
  return !numericValue.empty();
}

OFBool DSRNumericMeasurementValue::checkMeasurementUnit(
    const DSRCodedEntryValue& measurementUnit) const {
  return measurementUnit.isValid();
}

OFBool DSRNumericMeasurementValue::checkNumericValueQualifier(
