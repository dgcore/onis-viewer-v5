...
SocketAppender::ConnectorThread::ConnectorThread(
    ...
    : sa(socket_appender), exit_flag(false) {}

SocketAppender::ConnectorThread::~ConnectorThread() {}

void SocketAppender::ConnectorThread::run() {
  while (true) {
    trigger_ev.timed_wait(30 * 1000);

    getLogLog().debug(LOG4CPLUS_TEXT("SocketAppender::ConnectorThread::run()")
    ...
    if (!socket.isOpen()) {
      getLogLog().error(LOG4CPLUS_TEXT("SocketAppender::ConnectorThread::run()")
...
void SocketAppender::ConnectorThread::terminate() {
  {
    thread::Guard guard(access_mutex);
    exit_flag = true;
    trigger_ev.signal();
...
void SocketAppender::ConnectorThread::trigger() {
  trigger_ev.signal();
}
...
SocketAppender::SocketAppender(const tstring& host_, int port_,
    ...
    : host(host_), port(port_), serverName(serverName_) {
  openSocket();
  initConnector();
}

SocketAppender::SocketAppender(const helpers::Properties& properties, tstring&)
    : Appender(properties), port(9998) {
...
void SocketAppender::openSocket() {
  if (!socket.isOpen()) {
    socket = helpers::Socket(host, port);
  }
}

void SocketAppender::initConnector() {
#if !defined(LOG4CPLUS_SINGLE_THREADED)
  connected = true;
...
void SocketAppender::append(const spi::InternalLoggingEvent& event) {
#if !defined(LOG4CPLUS_SINGLE_THREADED)
  if (!connected) {
    connector->trigger();
    return;
  ...
  if (!socket.isOpen()) {
    ...
    if (!socket.isOpen()) {
      getLogLog().error(
          LOG4CPLUS_TEXT("SocketAppender::append()- Cannot connect to server"));
      return;
    }
  }
...
spi::InternalLoggingEvent readFromBuffer(SocketBuffer& buffer) {
  ...
  if (msgVersion != LOG4CPLUS_MESSAGE_VERSION) {
    ...
    loglog->warn(LOG4CPLUS_TEXT(
        "readFromBuffer() received socket message with an invalid version"));
  }
  ...
  return spi::InternalLoggingEvent(loggerName, ll, ndc, message, thread,
