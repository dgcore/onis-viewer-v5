...
LogLog::LogLog()
    : mutex(LOG4CPLUS_MUTEX_CREATE), debugEnabled(false), quietMode(false) {}
...
void LogLog::debug(const log4cplus::tstring& msg) {
...
void LogLog::warn(const log4cplus::tstring& msg) {
  LOG4CPLUS_BEGIN_SYNCHRONIZE_ON_MUTEX(mutex)
  if (quietMode)
    return;
...
void LogLog::error(const log4cplus::tstring& msg) {
  LOG4CPLUS_BEGIN_SYNCHRONIZE_ON_MUTEX(mutex)
  if (quietMode)
    return;
