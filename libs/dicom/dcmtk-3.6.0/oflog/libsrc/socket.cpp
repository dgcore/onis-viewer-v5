...
log4cplus::helpers::AbstractSocket::AbstractSocket()
    : sock(INVALID_SOCKET), state(not_opened), err(0) {}

log4cplus::helpers::AbstractSocket::AbstractSocket(SOCKET_TYPE sock_,
                                                   SocketState state_, int err_)
    : sock(sock_), state(state_), err(err_) {}

log4cplus::helpers::AbstractSocket::AbstractSocket(
    const log4cplus::helpers::AbstractSocket& rhs) {
  copy(rhs);
}
...
void log4cplus::helpers::AbstractSocket::close() {
...
bool log4cplus::helpers::AbstractSocket::isOpen() const {
  return sock != INVALID_SOCKET;
}

log4cplus::helpers::AbstractSocket&
log4cplus::helpers::AbstractSocket::operator=(
    const log4cplus::helpers::AbstractSocket& rhs) {
  if (&rhs != this) {
    close();
    copy(rhs);
...
void log4cplus::helpers::AbstractSocket::copy(
    const log4cplus::helpers::AbstractSocket& r) {
...
log4cplus::helpers::Socket::Socket() : AbstractSocket() {}
...
bool log4cplus::helpers::Socket::read(SocketBuffer& buffer) {
#ifdef WIN32
...
bool log4cplus::helpers::Socket::write(const SocketBuffer& buffer) {
#ifdef WIN32
...
log4cplus::helpers::Socket log4cplus::helpers::ServerSocket::accept() {
#if defined(_WIN32)
  SocketState state_;
  return Socket(INVALID_SOCKET, state_, 0);
