...
void LoggerImpl::callAppenders(const InternalLoggingEvent& event) {
  int writes = 0;
  ...
  if (!hierarchy.emittedNoAppenderWarning && writes == 0) {
    getLogLog().error(
        LOG4CPLUS_TEXT("No appenders could be found for logger (") + getName() +
        LOG4CPLUS_TEXT(")."));
    getLogLog().error(
...
void LoggerImpl::closeNestedAppenders() {
  ...
  for (SharedAppenderPtrListIterator it = appenders.begin();
       it != appenders.end(); ++it) {
    (*it)->close();
  }
}

bool LoggerImpl::isEnabledFor(LogLevel ll_) const {
...
void LoggerImpl::log(LogLevel ll_, const log4cplus::tstring& message,
...
LogLevel LoggerImpl::getChainedLogLevel() const {
  ...
  getLogLog().error(LOG4CPLUS_TEXT(
      "LoggerImpl::getChainedLogLevel()- No valid LogLevel found"));
...
bool LoggerImpl::getAdditivity() const {
  return additive;
}

void LoggerImpl::setAdditivity(bool additive_) {
  this->additive = additive_;
}

void LoggerImpl::forcedLog(LogLevel ll_, const log4cplus::tstring& message,
                           const char* file, int line, const char* function) {
  callAppenders(spi::InternalLoggingEvent(this->getName(), ll_, message, file,
                                          line, function));
}
