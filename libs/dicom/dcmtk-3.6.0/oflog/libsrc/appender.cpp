...
void OnlyOnceErrorHandler::error(const log4cplus::tstring& err) {
  if (firstTime) {
    getLogLog().error(err);
    firstTime = false;
  }
...
Appender::Appender(const log4cplus::helpers::Properties properties)
  ...
  if (properties.exists(LOG4CPLUS_TEXT("layout"))) {
    ...
    try {
      tstring error;
      OFauto_ptr<Layout> newLayout(
          factory->createObject(layoutProperties, error));
      if (newLayout.get() == 0) {
      ...
      getLogLog().error(
          LOG4CPLUS_TEXT("Exception caught while creating Layout"));
      return;
    }
  }
  ...
  while (
      filterProps.exists(filterName = convertIntegerToString(++filterCount))) {
...
void Appender::destructorImpl() {
  getLogLog().debug(LOG4CPLUS_TEXT("Destroying appender named [") + name +
                    LOG4CPLUS_TEXT("]."));
...
void Appender::doAppend(const log4cplus::spi::InternalLoggingEvent& event) {
...
log4cplus::tstring Appender::getName() {
  return name;
}

void Appender::setName(const log4cplus::tstring& name_) {
  this->name = name_;
}
...
void Appender::setErrorHandler(OFauto_ptr<ErrorHandler> eh) {
  if (eh.get() == NULL) {
...
void Appender::setLayout(OFauto_ptr<Layout> lo) {
  LOG4CPLUS_BEGIN_SYNCHRONIZE_ON_MUTEX(access_mutex)
  this->layout = lo;
  LOG4CPLUS_END_SYNCHRONIZE_ON_MUTEX;
}
