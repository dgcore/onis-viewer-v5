...
static int is_space(tchar ch) {
  return isspace(OFstatic_cast(unsigned char, ch));
}

static void trim_leading_ws(tstring& str) {
...
static void trim_trailing_ws(tstring& str) {
...
static void trim_ws(tstring& str) {
...
void Properties::init(tistream& input) {
  if (!input)
    return;
...
OFList<tstring> Properties::propertyNames() const {
  OFList<tstring> tmp;
...
bool Properties::removeProperty(const tstring& key) {
  return (data.erase(key) > 0);
}

Properties Properties::getPropertySubset(const tstring& prefix) const {
  Properties ret;
  OFList<tstring> const keys = propertyNames();
  size_t const prefix_len = prefix.size();
  for (OFListConstIterator(tstring) it = keys.begin(); it != keys.end(); ++it) {
