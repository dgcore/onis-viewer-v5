...
bool CopySid(SID** ppDstSid, SID* pSrcSid) {
  ...
  *ppDstSid = (SID*)::HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, dwLength);
...
HKEY regGetKey(const log4cplus::tstring& subkey, DWORD* disposition) {
  HKEY hkey = 0;
...
void regSetString(HKEY hkey, const log4cplus::tstring& name,
                  const log4cplus::tstring& value) {
  RegSetValueEx(hkey, name.c_str(), 0, REG_SZ, (LPBYTE)value.c_str(),
                OFstatic_cast(DWORD, value.length() * sizeof(tchar)));
}

void regSetDword(HKEY hkey, const log4cplus::tstring& name, DWORD value) {
  RegSetValueEx(hkey, name.c_str(), 0, REG_DWORD, (LPBYTE)&value,
                sizeof(DWORD));
}
...
NTEventLogAppender::NTEventLogAppender(const Properties properties,
                                       tstring& error)
    : Appender(properties), hEventLog(NULL), pCurrentUserSID(NULL) {
...
void NTEventLogAppender::append(const InternalLoggingEvent& event) {
  BOOL bSuccess;
  ...
  bSuccess =
      ::ReportEvent(hEventLog, getEventType(event), getEventCategory(event),
                    0x1000, pCurrentUserSID, 1, 0, &s, NULL);
...
WORD NTEventLogAppender::getEventType(const InternalLoggingEvent& event) {
  WORD ret_val;
...
WORD NTEventLogAppender::getEventCategory(const InternalLoggingEvent& event) {
  WORD ret_val;
...
void NTEventLogAppender::addRegistryInfo() {
  ...
  HKEY hkey = 0;
  tstring subkey =
      LOG4CPLUS_TEXT("SYSTEM\\CurrentControlSet\\Services\\EventLog\\") + log +
      LOG4CPLUS_TEXT("\\") + source;

  hkey = regGetKey(subkey, &disposition);
  if (disposition == REG_CREATED_NEW_KEY) {
    regSetString(hkey, LOG4CPLUS_TEXT("EventMessageFile"),
                 LOG4CPLUS_TEXT("NTEventLogAppender.dll"));
    regSetString(hkey, LOG4CPLUS_TEXT("CategoryMessageFile"),
                 LOG4CPLUS_TEXT("NTEventLogAppender.dll"));
