...
const log4cplus::tstring& InternalLoggingEvent::getMessage() const {
  return message;
}
...
OFauto_ptr<InternalLoggingEvent> InternalLoggingEvent::clone() const {
  OFauto_ptr<InternalLoggingEvent> tmp(new InternalLoggingEvent(*this));
  return tmp;
}

log4cplus::spi::InternalLoggingEvent& InternalLoggingEvent::operator=(
