...
HierarchyLocker::HierarchyLocker(Hierarchy& _h)
    : h(_h), hierarchyLocker(h.hashtable_mutex), loggerList() {
  ...
  try {
    for (LoggerListIterator it = loggerList.begin(); it != loggerList.end();
         ++it) {
      LOG4CPLUS_MUTEX_LOCK((*it).value->appender_list_mutex);
    }
  } catch (...) {
    h.getLogLog().error(LOG4CPLUS_TEXT(
        "HierarchyLocker::ctor()- An error occurred while locking"));
...
HierarchyLocker::~HierarchyLocker() {
  try {
    ...
    h.getLogLog().error(LOG4CPLUS_TEXT(
        "HierarchyLocker::dtor()- An error occurred while unlocking"));
    throw;
  }
}

void HierarchyLocker::resetConfiguration() {
...
Logger HierarchyLocker::getInstance(const log4cplus::tstring& name) {
...
void HierarchyLocker::addAppender(Logger& logger,
