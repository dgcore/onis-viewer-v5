 ...
 */
static bool substVars(tstring& dest, const tstring& val,
                      Properties const& props, helpers::LogLog& loglog,
                      unsigned flags) {
  ...
  tstring replacement;
  bool changed = false;
  bool const empty_vars = !!(flags & PropertyConfigurator::fAllowEmptyVars);
  bool const shadow_env = !!(flags & PropertyConfigurator::fShadowEnvironment);
  bool const rec_exp = !!(flags & PropertyConfigurator::fRecursiveExpansion);
  ...
  while (true) {
    ...
    if (var_end == OFString_npos) {
      ...
      buffer << '"' << pattern << "\" has no closing brace. "
    ...
    if (!shadow_env || (!empty_vars && replacement.empty())) {
      char const* env_var = getenv(LOG4CPLUS_TSTRING_TO_STRING(key).c_str());
      if (env_var)
...
void PropertyConfigurator::doConfigure(const tstring& file, Hierarchy& h,
                                       unsigned flags) {
...
void PropertyConfigurator::configure() {
  ...
  log4cplus::tstring val = properties.getProperty(LOG4CPLUS_TEXT("configDebug"),
                                                  LOG4CPLUS_TEXT("false"));
  getLogLog().setInternalDebugging(helpers::toLower(val) ==
                                   LOG4CPLUS_TEXT("true"));
...
log4cplus::tstring const& PropertyConfigurator::getPropertyFilename() const {
...
void PropertyConfigurator::replaceEnvironVariables() {
  ...
  do {
    ...
    for (OFListConstIterator(tstring) it = keys.begin(); it != keys.end();
         ++it) {
...
void PropertyConfigurator::configureLoggers() {
  if (properties.exists(LOG4CPLUS_TEXT("rootLogger"))) {
    ...
    configureLogger(root, properties.getProperty(LOG4CPLUS_TEXT("rootLogger")));
  }
...
void PropertyConfigurator::configureLogger(Logger logger,
  ...
  if (tokens.size() == 0) {
    getLogLog().error(LOG4CPLUS_TEXT("PropertyConfigurator::configureLogger()")
                      ...
                      logger.getName() + LOG4CPLUS_TEXT("): \"") + config +
                      LOG4CPLUS_TEXT("\""));
    return;
  }
    ...
    if (it->find(LOG4CPLUS_TEXT('.')) == OFString_npos) {
      ...
      try {
        ...
        SharedAppenderPtr appender =
            factory->createObject(my_properties, error);
        if (appender.get() == 0) {
          tstring err = LOG4CPLUS_TEXT("PropertyConfigurator::")
...
void PropertyConfigurator::configureAdditivity() {
  ...
  for (OFListConstIterator(tstring) it = additivitysProps.begin();
    ...
    if (value == LOG4CPLUS_TEXT("true"))
      ...
      getLogLog().warn(LOG4CPLUS_TEXT("Invalid Additivity value: \"") +
                       actualValue + LOG4CPLUS_TEXT("\""));
  }
}

Logger PropertyConfigurator::getLogger(const tstring& name) {
  return h.getInstance(name);
}

void PropertyConfigurator::addAppender(Logger& logger,
                                       SharedAppenderPtr& appender) {
...
void BasicConfigurator::doConfigure(Hierarchy& h) {
...
bool ConfigurationWatchDogThread::checkForFileModification(Time& mtime) {
...
void ConfigurationWatchDogThread::updateLastModTime(Time const& mtime) {
  lastModTime = mtime;
}
