...
static bool startsWith(tstring const& teststr, tstring const& substr) {
  bool val = false;
...
Hierarchy::Hierarchy()
    : hashtable_mutex(LOG4CPLUS_MUTEX_CREATE),
      ...
      root(NULL),
      disableValue(
          DISABLE_OFF),  // Don't disable any LogLevel level by default.
...
bool Hierarchy::exists(const log4cplus::tstring& name) {
  LOG4CPLUS_BEGIN_SYNCHRONIZE_ON_MUTEX(hashtable_mutex)
...
Logger Hierarchy::getInstance(const log4cplus::tstring& name) {
...
void Hierarchy::setLoggerFactory(OFauto_ptr<spi::LoggerFactory> factory) {
...
Logger Hierarchy::getInstanceImpl(const log4cplus::tstring& name,
                                  spi::LoggerFactory& factory) {
  LoggerMap::iterator it = loggerPtrs.find(name);
  if (it != loggerPtrs.end()) {
    ...
    if (!inserted) {
      ...
      // throw STD_NAMESPACE runtime_error("Hierarchy::getInstanceImpl()- Insert
      // failed");
      abort();
    }
    ...
    if (it2 != provisionNodes.end()) {
      ...
      if (!deleted) {
        ...
        // throw STD_NAMESPACE runtime_error("Hierarchy::getInstanceImpl()-
        // Delete failed");
        abort();
      }
    }
...
void Hierarchy::initializeLoggerList(LoggerList& list) const {
  for (LoggerMap::const_iterator it = loggerPtrs.begin();
       it != loggerPtrs.end(); ++it) {
    list.push_back((*it).second);
  }
}

void Hierarchy::updateParents(Logger logger) {
  ...
  for (size_t i = name.find_last_of(LOG4CPLUS_TEXT('.'), length - 1);
       i != OFString_npos; i = name.find_last_of(LOG4CPLUS_TEXT('.'), i - 1)) {
    ...
    if (it != loggerPtrs.end()) {
      ...
      if (it2 != provisionNodes.end()) {
        ...
        // bool inserted = provisionNodes.insert(STD_NAMESPACE make_pair(substr,
        // node)).second;
        if (!tmp.second) {
          getLogLog().error(
              LOG4CPLUS_TEXT("Hierarchy::updateParents()- Insert failed"));
          // throw STD_NAMESPACE runtime_error("Hierarchy::updateParents()-
          // Insert failed");
          abort();
        }
      }
...
void Hierarchy::updateChildren(ProvisionNode& pn, Logger logger) {
