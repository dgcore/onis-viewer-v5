...
public:
  DatePatternConverter(const FormattingInfo& info,
                       const log4cplus::tstring& pattern, bool use_gmtime);
  virtual log4cplus::tstring convert(const InternalLoggingEvent& event);
...
typedef OFListIterator(log4cplus::pattern::PatternConverter*)
    PatternConverterListIterator;

////////////////////////////////////////////////
// PatternConverter methods:
////////////////////////////////////////////////

void log4cplus::pattern::FormattingInfo::reset() {
  minLen = -1;
  maxLen = 0x7FFFFFFF;
  leftAlign = false;
}

void log4cplus::pattern::FormattingInfo::dump(
...
log4cplus::pattern::PatternConverter::PatternConverter(
    const FormattingInfo& i) {
  minLen = i.minLen;
  maxLen = i.maxLen;
  leftAlign = i.leftAlign;
}

void log4cplus::pattern::PatternConverter::formatAndAppend(
...
log4cplus::pattern::BasicPatternConverter::BasicPatternConverter(
    ...
    : PatternConverter(info), llmCache(getLogLevelManager()), type(type_) {}

log4cplus::tstring log4cplus::pattern::BasicPatternConverter::convert(
    const InternalLoggingEvent& event) {
  switch (type) {
    case LOGLEVEL_CONVERTER:
      return llmCache.toString(event.getLogLevel());
    case LOGLEVEL_PREFIX_CONVERTER:
      return llmCache.toString(event.getLogLevel()).substr(0, 1);
    case NDC_CONVERTER:
      return event.getNDC();
    ...
    case FULL_LOCATION_CONVERTER: {
      ...
      if (!filename.empty()) {
        return filename + LOG4CPLUS_TEXT(":") +
...
log4cplus::tstring log4cplus::pattern::LoggerPatternConverter::convert(
    const InternalLoggingEvent& event) {
...
log4cplus::pattern::DatePatternConverter::DatePatternConverter(
    ...
    bool use_gmtime_)
    : PatternConverter(info), use_gmtime(use_gmtime_), format(pattern) {}

log4cplus::tstring log4cplus::pattern::DatePatternConverter::convert(
    const InternalLoggingEvent& event) {
...
log4cplus::pattern::HostnamePatternConverter::HostnamePatternConverter(
    ...
    : PatternConverter(info), hostname_(helpers::getHostname(fqdn)) {}
#else
log4cplus::pattern::HostnamePatternConverter::HostnamePatternConverter(
    const FormattingInfo& info, bool fqdn)
    : PatternConverter(info), hostname_("") {}
#endif
// END MODIFY DIGITALCORE

log4cplus::tstring log4cplus::pattern::HostnamePatternConverter::convert(
...
log4cplus::tstring log4cplus::pattern::PatternParser::extractOption() {
...
int log4cplus::pattern::PatternParser::extractPrecisionOption() {
...
OFauto_ptr<PatternConverterList> log4cplus::pattern::PatternParser::parse() {
  tchar c;
  pos = 0;
  while (pos < pattern.length()) {
    ...
    switch (state) {
      ...
      case MIN_STATE:
        ...
        if (c >= LOG4CPLUS_TEXT('0') && c <= LOG4CPLUS_TEXT('9')) {
          formattingInfo.minLen =
              formattingInfo.minLen * 10 + (c - LOG4CPLUS_TEXT('0'));
      ...
      case MAX_STATE:
        currentLiteral += c;
        if (c >= LOG4CPLUS_TEXT('0') && c <= LOG4CPLUS_TEXT('9'))
          formattingInfo.maxLen =
              formattingInfo.maxLen * 10 + (c - LOG4CPLUS_TEXT('0'));
        else {
          finalizeConverter(c);
          state = LITERAL_STATE;
        }
...
void log4cplus::pattern::PatternParser::finalizeConverter(log4cplus::tchar c) {
  ...
  switch (c) {
    ...
    case LOG4CPLUS_TEXT('l'):
      pc = new BasicPatternConverter(
          formattingInfo, BasicPatternConverter::FULL_LOCATION_CONVERTER);
    ...
    case LOG4CPLUS_TEXT('P'):
      pc = new BasicPatternConverter(
          formattingInfo, BasicPatternConverter::LOGLEVEL_PREFIX_CONVERTER);
...
PatternLayout::PatternLayout(const log4cplus::helpers::Properties& properties,
  ...
  bool hasConversionPattern =
      properties.exists(LOG4CPLUS_TEXT("ConversionPattern"));
  log4cplus::tstring eachLine =
      properties.getProperty(LOG4CPLUS_TEXT("FormatEachLine"), "yes");
  ...
  if (eachLine == LOG4CPLUS_TEXT("yes"))
    ...
    getLogLog().warn(
        LOG4CPLUS_TEXT("PatternLayout- the \"FormatEachLine\" property has an "
                       "invalid value, assuming \"yes\""));
  ...
  if (hasConversionPattern) {
    ...
    // throw STD_NAMESPACE runtime_error("ConversionPattern not specified in
    // properties");
    return;
  }
  error.clear();
...
void PatternLayout::init(const log4cplus::tstring& pattern_,
                         bool formatEachLine_) {
  ...
  if (parsedPattern->size() == 0) {
    ...
    parsedPattern->push_back(new BasicPatternConverter(
...
void PatternLayout::formatAndAppend(log4cplus::tostream& output,
                                    const InternalLoggingEvent& event) {
  if (formatEachLine && event.getMessage().find('\n') != OFString_npos) {
    ...
    while (pos != OFString_npos) {
      ...
      log4cplus::tstring tmp_message(event.getMessage().substr(
          last_pos, (pos == OFString_npos) ? pos : pos - last_pos));

      // Then create a temporary InternalLoggingEvent for this one line
      InternalLoggingEvent tmp_event(
          event.getLoggerName(), event.getLogLevel(), event.getNDC(),
          tmp_message, event.getThread(), event.getTimestamp(), event.getFile(),
          event.getLine(), event.getFunction());
