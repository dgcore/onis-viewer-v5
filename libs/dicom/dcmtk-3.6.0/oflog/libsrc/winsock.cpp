...
// stdexcept header not included by default on mingw (needed for
// std::runtime_error)
#ifdef __MINGW32__
#include <stdexcept>
#endif
...
enum WSInitStates { WS_UNINITIALIZED, WS_INITIALIZING, WS_INITIALIZED };
...
static LONG DoInterlockedCompareExchange(LPLONG p, LONG comp, LONG set) {
  ...
  void* tmp = ::InterlockedCompareExchange(OFreinterpret_cast(void**, p),
                                           OFreinterpret_cast(void*, comp),
                                           OFreinterpret_cast(void*, set));
...
static void init_winsock() {
  ...
  LONG val = DoInterlockedCompareExchange(OFconst_cast(LPLONG, &winsock_state),
                                          WS_INITIALIZING, WS_UNINITIALIZED);
  switch (val) {
    case WS_UNINITIALIZED: {
      ...
      if (ret != 0) {
        ...
        val = DoInterlockedCompareExchange(OFconst_cast(LPLONG, &winsock_state),
      ...
      val = DoInterlockedCompareExchange(OFconst_cast(LPLONG, &winsock_state),
                                         WS_INITIALIZED, WS_INITIALIZING);
      assert(val == WS_INITIALIZING);
      return;
    }
...
log4cplus::helpers::connectSocket(const log4cplus::tstring& hostn,
  ...
  struct hostent* hp =
      ::gethostbyname(LOG4CPLUS_TSTRING_TO_STRING(hostn).c_str());
  ...
  int retval;
  while ((retval = ::connect(sock, (struct sockaddr*)&insock,
                             sizeof(insock))) == -1 &&
...
int log4cplus::helpers::closeSocket(SOCKET_TYPE sock) {
  return ::closesocket(sock);
}

long log4cplus::helpers::read(SOCKET_TYPE sock, SocketBuffer& buffer) {
  long read = 0;
...
long log4cplus::helpers::write(SOCKET_TYPE sock, const SocketBuffer& buffer) {
  return ::send(sock, buffer.getBuffer(), OFstatic_cast(int, buffer.getSize()),
                0);
}

tstring log4cplus::helpers::getHostname(bool fqdn) {
