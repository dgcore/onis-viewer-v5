...
Time::Time() : tv_sec(0), tv_usec(0) {}

Time::Time(time_t tv_sec_, long tv_usec_) : tv_sec(tv_sec_), tv_usec(tv_usec_) {
...
#else
#warning \
    "Time::gettimeofday()- low resolution timer: gettimeofday and ftime unavailable"
...
time_t Time::setTime(struct tm* t) {
...
void Time::gmtime(struct tm* t) const {
  time_t clock = tv_sec;
...
void Time::localtime(struct tm* t) const {
  time_t clock = tv_sec;
...
void Time::build_uc_q_value(log4cplus::tstring& uc_q_str) const {
  build_q_value(uc_q_str);

#if defined(LOG4CPLUS_HAVE_GETTIMEOFDAY)
  log4cplus::tstring usecs(convertIntegerToString(tv_usec % 1000));
  size_t usecs_len = usecs.length();
  usecs.insert(0, usecs_len <= 3 ? uc_q_padding_zeros[usecs_len]
                                 : uc_q_padding_zeros[3]);
...
log4cplus::tstring Time::getFormattedTime(const log4cplus::tstring& fmt_orig,
                                          bool use_gmtime) const {
...
const Time operator/(const Time& lhs, long rhs) {
  return Time(lhs) /= rhs;
}
...
bool operator<(const Time& lhs, const Time& rhs) {
  return ((lhs.sec() < rhs.sec()) ||
          ((lhs.sec() == rhs.sec()) && (lhs.usec() < rhs.usec())));
}
