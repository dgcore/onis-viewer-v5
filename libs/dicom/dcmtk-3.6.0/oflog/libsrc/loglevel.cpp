...
#define GET_FROM_STRING_NODE \
  OFstatic_cast(FromStringNode*, this->fromStringMethods)
...
static log4cplus::tstring defaultLogLevelToStringMethod(LogLevel ll) {
  switch (ll) {
    case OFF_LOG_LEVEL:
      return _OFF_STRING;
...
static LogLevel defaultStringToLogLevelMethod(const log4cplus::tstring& arg) {
...
log4cplus::tstring LogLevelManager::toString(LogLevel ll) const {
  ToStringNode* toStringTmp = GET_TO_STRING_NODE;
  while (toStringTmp) {
...
LogLevel LogLevelManager::fromString(const log4cplus::tstring& s) const {
  FromStringNode* fromStringTmp = GET_FROM_STRING_NODE;
  while (fromStringTmp) {
...
void LogLevelManager::pushToStringMethod(LogLevelToStringMethod newToString) {
  ToStringNode* toStringTmp = GET_TO_STRING_NODE;
  while (1) {
    if (toStringTmp->next) {
      toStringTmp = toStringTmp->next;
...
void LogLevelManager::pushFromStringMethod(
