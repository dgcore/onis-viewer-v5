...
static int file_rename(tstring const& src, tstring const& target) {
...
static int file_remove(tstring const& src) {
  return remove(LOG4CPLUS_TSTRING_TO_STRING(src).c_str()) == 0 ? 0 : -1;
}

static void loglog_renaming_result(helpers::LogLog& loglog, tstring const& src,
                                   tstring const& target, int ret) {
  if (ret == 0) {
    loglog.debug(LOG4CPLUS_TEXT("Renamed file ") + src +
                 LOG4CPLUS_TEXT(" to ") + target);
  } else if (ret == -1 && errno != ENOENT) {
    loglog.error(LOG4CPLUS_TEXT("Failed to rename file from ") + target +
                 LOG4CPLUS_TEXT(" to ") + target);
  }
}

static void loglog_opening_result(helpers::LogLog& loglog,
                                  log4cplus::tostream const& os,
...
FileAppender::FileAppender(const tstring& filename_,
    ...
    : immediateFlush(immediateFlush_), reopenDelay(1) {
  init(filename_, mode);
}

FileAppender::FileAppender(const Properties& properties, log4cplus::tstring&,
                           LOG4CPLUS_OPEN_MODE_TYPE mode)
    : Appender(properties), immediateFlush(true), reopenDelay(1) {
  ...
  init(filename_,
       (append_ ? STD_NAMESPACE ios::app : STD_NAMESPACE ios::trunc));
}

void FileAppender::init(const tstring& filename_,
                        LOG4CPLUS_OPEN_MODE_TYPE mode) {
...
void FileAppender::open(STD_NAMESPACE ios::openmode mode) {
...
bool FileAppender::reopen() {
  ...
  if (reopen_time == log4cplus::helpers::Time() && reopenDelay != 0)
    ...
    // Otherwise, check for end of the delay (or absence of delay) to re-open
    // the file.
...
RollingFileAppender::RollingFileAppender(const tstring& filename_,
                                         long maxFileSize_, int maxBackupIndex_,
...
// This method does not need to be locked since it is called by
// doAppend() which performs the locking
void RollingFileAppender::append(const spi::InternalLoggingEvent& event) {
...
void RollingFileAppender::rollover() {
  ...
  if (maxBackupIndex > 0) {
    ...
    loglog.debug(LOG4CPLUS_TEXT("Renaming file ") + filename +
                 LOG4CPLUS_TEXT(" to ") + target);
...
DailyRollingFileAppender::DailyRollingFileAppender(const Properties& properties,
  ...
  if (scheduleStr == LOG4CPLUS_TEXT("MONTHLY"))
    ...
    getLogLog().warn(
        LOG4CPLUS_TEXT(
            "DailyRollingFileAppender::ctor()- \"Schedule\" not valid: ") +
...
void DailyRollingFileAppender::init(DailyRollingFileSchedule schedule_) {
...
// This method does not need to be locked since it is called by
// doAppend() which performs the locking
void DailyRollingFileAppender::append(const spi::InternalLoggingEvent& event) {
  ...
  switch (schedule) {
    case MONTHLY: {
      ...
      if (ret.setTime(&nextMonthTime) == -1) {
        getLogLog().error(LOG4CPLUS_TEXT(
...
tstring DailyRollingFileAppender::getFilename(const Time& t) const {
