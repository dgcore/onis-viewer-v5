...
void deleteMutex(LOG4CPLUS_MUTEX_PTR_DECLARE m) {
#if defined(LOG4CPLUS_USE_PTHREADS)
...
#if defined(LOG4CPLUS_USE_PTHREADS)
pthread_key_t* createPthreadKey(void (*cleanupfunc)(void*)) {
...
#if defined(LOG4CPLUS_USE_PTHREADS)
void* ThreadStart::threadStartFuncWorker(void* arg)
#elif defined(LOG4CPLUS_USE_WIN32_THREADS)
unsigned ThreadStart::threadStartFuncWorker(void* arg)
#endif
{
  blockAllSignals();
  ...
  if (!arg)
    ...
    try {
      ...
      loglog->warn(LOG4CPLUS_TEXT(
          "threadStartFunc()- run() terminated with an exception."));
    }
...
#else
  h = OFreinterpret_cast(
      HANDLE, ::_beginthreadex(0, 0, threadStartFunc, this, 0, &thread_id));
#endif
  if (!h) {
    removeReference();
    helpers::SharedObjectPtr<helpers::LogLog> loglog =
...
void AbstractThread::join() const {
#if defined(LOG4CPLUS_USE_PTHREADS)
