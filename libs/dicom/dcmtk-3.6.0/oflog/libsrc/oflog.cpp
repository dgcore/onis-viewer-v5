...
OFLogger::OFLogger(const log4cplus::Logger& base) : log4cplus::Logger(base) {}
...
static void OFLog_init() {
  ...
  log4cplus::SharedAppenderPtr console(new log4cplus::ConsoleAppender(
      OFTrue /* logToStrErr */, OFTrue /* immediateFlush */));
...
OFLogger OFLog::getLogger(const char* loggerName) {
  OFLog_init();
  // logger objects have a reference counting copy-constructor, so returning
  // by-value is cheap
 ...
 *             may be NULL
 */
static void addVariables(log4cplus::helpers::Properties& props,
                         OFCommandLine* cmd) {
  OFString date;
  OFString time;
  ...
  OFTime::getCurrentTime().getISOFormattedTime(time, OFTrue, OFFalse, OFFalse,
                                               OFFalse);
  ...
  props.setProperty("pid", log4cplus::helpers::convertIntegerToString(
                               OFStandard::getProcessID()));
...
void OFLog::reconfigure(OFCommandLine* cmd) {
  ...
  log4cplus::PropertyConfigurator conf(
      *props, log4cplus::Logger::getDefaultHierarchy(), flags);
  conf.configure();
}
...
void OFLog::configureFromCommandLine(OFCommandLine& cmd,
  ...
  if (cmd.findOption("--log-level")) {
    app.checkConflict("--log-level", "--verbose, --debug or --quiet",
                      level != log4cplus::NOT_SET_LOG_LEVEL);
  ...
  if (cmd.findOption("--log-config")) {
    ...
    app.checkConflict("--log-config", "--verbose, --debug or --quiet",
                      level != log4cplus::NOT_SET_LOG_LEVEL);
    ...
    if (configProperties_->size() == 0)
      app.printError(
          "Specified --log-config file does not contain any settings");
    if (configProperties_->getPropertySubset("log4cplus.").size() == 0)
      app.printError(
          "Specified --log-config file does not contain any valid settings");
    if (!configProperties_->exists("log4cplus.rootLogger"))
      app.printError(
          "Specified --log-config file does not set up log4cplus.rootLogger");
  ...
  if (cmd.findOption("--arguments")) {
    ...
    stream << "calling '" << cmd.getProgramName() << "' with "
           << cmd.getArgCount() << " arguments: ";
    const char* arg;
...
void OFLog::addOptions(OFCommandLine& cmd) {
  ...
  cmd.addOption(
      "--log-level", "-ll", 1, "[l]evel: string constant",
