...
struct ConvertIntegerToStringHelper {
  static inline void step1(tchar*& it, intType& value) {
    ...
    const OFBool isUnsigned =
        (OFstatic_cast(intType, -1) < 0) ? OFFalse : OFTrue;

    // If intType is a signed type, halfMaxSigned is intType_MAX / 2
    const intType halfMaxSigned = OFstatic_cast(intType, 1)
                                  << (sizeof(intType) * 8 - 2);
...
inline void convertIntegerToString(tstring& str, intType value) {
  ...
  const size_t buffer_size = 30;  // More than enough space, even for 64 bit
                                  // integers = intTypeLimits::digits10 + 2;
