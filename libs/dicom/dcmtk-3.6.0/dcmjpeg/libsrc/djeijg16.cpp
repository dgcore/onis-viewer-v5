 ...
 *
 *  Purpose: compression routines of the IJG JPEG library configured for 16
 * bits/sample.
 *
...
DJCompressIJG16Bit::DJCompressIJG16Bit(const DJCodecParameter& cp, EJ_Mode mode,
...
OFCondition DJCompressIJG16Bit::encode(Uint16 /* columns */, Uint16 /* rows */,
...
OFCondition DJCompressIJG16Bit::encode(
  ...
  if (setjmp(jerr.setjmp_buffer)) {
    ...
    return makeOFCondition(OFM_dcmjpeg, EJCode_IJG16_Compression, OF_error,
                           buffer);
  }
  ...
  switch (modeofOperation) {
    ...
    default:
      return makeOFCondition(
          OFM_dcmjpeg, EJCode_IJG16_Compression, OF_error,
          "JPEG with 16 bits/sample only allowed with lossless compression");
      /* break; */
  }
  ...
  if (pixelDataList.size() > 1)
    length += (pixelDataList.size() - 1) * IJGE16_BLOCKSIZE;
  if (length % 2)
