 ...
 *
 *  Purpose: singleton class that registers decoders for all supported JPEG
 * processes.
 *
...
DJDecoder2kLossLess* DJDecoderRegistration::dec2kLossLess = NULL;
...
void DJDecoderRegistration::registerCodecs(
  ...
  if (!registered) {
    cp = new DJCodecParameter(ECC_lossyYCbCr,  // ignored, compression only
                              pDecompressionCSConversion, pCreateSOPInstanceUID,
                              pPlanarConfiguration, predictor6WorkaroundEnable);
    if (cp) {
      // baseline JPEG
      decbas = new DJDecoderBaseline();
      if (decbas)
        DcmCodecList::registerCodec(decbas, NULL, cp);

      // extended JPEG
      decext = new DJDecoderExtended();
      if (decext)
        DcmCodecList::registerCodec(decext, NULL, cp);

      // spectral selection JPEG
      decsps = new DJDecoderSpectralSelection();
      if (decsps)
        DcmCodecList::registerCodec(decsps, NULL, cp);

      // progressive JPEG
      decpro = new DJDecoderProgressive();
      if (decpro)
        DcmCodecList::registerCodec(decpro, NULL, cp);

      // lossless SV1 JPEG
      decsv1 = new DJDecoderP14SV1();
      if (decsv1)
        DcmCodecList::registerCodec(decsv1, NULL, cp);

      // lossless JPEG
      declol = new DJDecoderLossless();
      if (declol)
        DcmCodecList::registerCodec(declol, NULL, cp);

      // JPEG 2000
      dec2k = new DJDecoder2k();
      if (dec2k)
        DcmCodecList::registerCodec(dec2k, NULL, cp);

      // JPEG 2000 LossLess
      dec2kLossLess = new DJDecoder2kLossLess();
      if (dec2kLossLess)
        DcmCodecList::registerCodec(dec2kLossLess, NULL, cp);
