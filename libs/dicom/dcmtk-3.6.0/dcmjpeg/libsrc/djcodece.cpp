...
DJCodecEncoder::DJCodecEncoder() : DcmCodec() {}
...
OFBool DJCodecEncoder::canChangeCoding(
  ...
  if (oldRep.isNotEncapsulated() && (newRepType == myXfer))
...
OFCondition DJCodecEncoder::decodeFrame(
    ...
    DcmPixelSequence* /* fromPixSeq */, const DcmCodecParameter* /* cp */,
...
OFCondition DJCodecEncoder::encode(
    ...
    DcmPixelSequence*& /* toPixSeq */, const DcmCodecParameter* /* cp */,
    DcmStack& /* objStack */) const {
  // we don't support re-coding for now
  return EC_IllegalCall;
}

OFCondition DJCodecEncoder::encode(const Uint16* /* pixelData */,
                                   const Uint32 /* length */,
                                   const DcmRepresentationParameter* toRepParam,
                                   DcmPixelSequence*& pixSeq,
  ...
  // if true lossless mode is enabled, and we're supposed to do lossless
  // compression, call the "true lossless encoding"-engine
  ...
  if ((!dataset) ||
      ((dataset->ident() != EVR_dataset) && (dataset->ident() != EVR_item)))
    result = EC_InvalidTag;
  else {
    double compressionRatio = 0.0;
    EP_Interpretation photometricInterpretation =
        DcmJpegHelper::getPhotometricInterpretation((DcmItem*)dataset);
    switch (photometricInterpretation) {
      case EPI_Monochrome1:
      case EPI_Monochrome2:
        // monochrome image
        result = encodeMonochromeImage((DcmItem*)dataset, toRepParam, pixSeq,
                                       djcp, compressionRatio);
        break;
      ...
      case EPI_YBR_Partial_422:
        ...
        result = encodeColorImage(OFFalse, (DcmItem*)dataset, toRepParam,
                                  pixSeq, djcp, compressionRatio);
        break;
      case EPI_YBR_Full:
      case EPI_YBR_Full_422:
        // YCbCr color image
        result = encodeColorImage(OFTrue, (DcmItem*)dataset, toRepParam, pixSeq,
                                  djcp, compressionRatio);
        break;
      case EPI_Unknown:
      case EPI_Missing:
    ...
    if (dataset->ident() == EVR_dataset) {
      ...
      if (result.good()) {
        if (compressedBits == 0) {
          result = ((DcmItem*)dataset)
                       ->findAndGetUint16(DCM_BitsStored, compressedBits);
        }
      }

      // update derivation description
      if (result.good())
        result = updateDerivationDescription((DcmItem*)dataset, toRepParam,
                                             djcp, (Uint8)compressedBits,
                                             compressionRatio);

      if (result.good()) {
        // lossy process - create new UID unless mode is EUC_never and we're not
        // converting to Secondary Capture (pseudo-lossless mode may also result
        // in lossy compression, so treat it the same way)
        if (djcp->getConvertToSC() || (djcp->getUIDCreation() != EUC_never))
          result = DcmCodec::newInstance((DcmItem*)dataset, "DCM", "121320",
                                         "Uncompressed predecessor");

        // update lossy compression ratio
        if (result.good())
          result =
              updateLossyCompressionRatio((DcmItem*)dataset, compressionRatio);
      }
...
OFCondition DJCodecEncoder::encodeColorImage(
    OFBool YBRmode, DcmItem* dataset,
    const DcmRepresentationParameter* toRepParam, DcmPixelSequence*& pixSeq,
    const DJCodecParameter* cp, double& compressionRatio) const {
  OFCondition result = EC_Normal;
  ...
  if ((cp->getCompressionColorSpaceConversion() == ECC_lossyYCbCr) &&
      (!isLosslessProcess())) {
    if (cp->getWriteYBR422())
  ...
  // integrate DicomImage flags transported by DJCodecParameter into
  // "flags"-variable
  ...
  // create dcmimage object. Will fail if dcmimage has not been activated in
  // main(). transfer syntax can be any uncompressed one.
  DicomImage* dimage = NULL;
  if (monochromeMode) {
    DicomImage colorimage(dataset, EXS_LittleEndianImplicit,
                          flags);  // read all frames
    if (colorimage.getStatus() == EIS_Normal)
      dimage = colorimage.createMonochromeImage();
    if (dimage)
      dimage->setNoVoiTransformation();
  } else {
    dimage = new DicomImage(dataset, EXS_LittleEndianImplicit,
                            flags);  // read all frames
  }
  ...
  if (dimage == NULL)
    ...
    result = EC_IllegalCall;  // should return dimage->getStatus()
  ...
  if (result.good()) {
    if (compressedBits == 0) {
      result =
          ((DcmItem*)dataset)->findAndGetUint16(DCM_BitsStored, compressedBits);
    }
  }
  ...
  if (result.good()) {
    DJEncoder* jpeg =
        createEncoderInstance(toRepParam, cp, (Uint8)compressedBits);
    if (jpeg) {
      ...
      uncompressedSize = columns * rows * dimage->getDepth() * frameCount *
                         samplesPerPixel / 8.0;
      for (unsigned long i = 0; (i < frameCount) && (result.good()); i++) {
        ...
        if (frame == NULL)
          ...
          jpegData = NULL;
          if (bytesPerSample == 1) {
            result = jpeg->encode(columns, rows, interpr, samplesPerPixel,
                                  (Uint8*)frame, jpegData, jpegLen,
                                  pixelRepresentation, 0.0, 0.0);
          } else {
            result = jpeg->encode(columns, rows, interpr, samplesPerPixel,
                                  (Uint16*)frame, jpegData, jpegLen,
                                  pixelRepresentation, 0.0, 0.0);
          }
          ...
          if (result.good()) {
            result = pixelSequence->storeCompressedFrame(
                offsetList, jpegData, jpegLen, cp->getFragmentSize());
          }
  ...
  if (result.good()) {
    ...
    if (result.good())
      result = dataset->putAndInsertString(DCM_PhotometricInterpretation,
    ...
    if (result.good())
      result = dataset->putAndInsertUint16(DCM_PixelRepresentation, 0);
    if (result.good()) {
      if (monochromeMode)
...
OFCondition DJCodecEncoder::encodeTrueLossless(
  ...
  if ((!dataset) ||
    ...
    OFBool planConfSwitched =
        OFFalse;  // true if planar configuration was toggled
    DcmOffsetList offsetList;
    ...
    if (result.good())
      result = datsetItem->findAndGetUint16(DCM_BitsAllocated, bitsAllocated);
    if (result.good())
      result =
          datsetItem->findAndGetUint16(DCM_SamplesPerPixel, samplesPerPixel);
    if (result.good())
      result = datsetItem->findAndGetUint16(DCM_Columns, columns);
    if (result.good())
      result = datsetItem->findAndGetUint16(DCM_Rows, rows);
    if (result.good())
      result = datsetItem->findAndGetOFString(DCM_PhotometricInterpretation,
                                              photometricInterpretation);
    if (result.good())
      result = datsetItem->findAndGetUint16Array(DCM_PixelData, pixelData, NULL,
                                                 OFFalse);
    if (result.good())
    ...
    if (result.bad()) {
      DCMJPEG_ERROR(
          "True lossless encoder: Unable to get relevant attributes from "
          "dataset");
      return result;
    }
    ...
    if (bitsAllocated == 8)
      ...
      DCMJPEG_ERROR(
          "True lossless encoder: Only 8 or 16 bits allocated supported");
    ...
    if ((columns < 1) || (rows < 1) || (samplesPerPixel < 1)) {
      DCMJPEG_ERROR(
          "True lossless encoder: Invalid attribute values in pixel module");
     ...
     * IJG lib are set. For all others "unknown" is set. Some are even rejected
     * here.
     */
    ...
    // some photometric interpretations are not supported:
      ...
      DCMJPEG_ERROR(
          "True lossless encoder: Photometric interpretation not supported: "
          << photometricInterpretation);
      return EC_IllegalParameter;
    ...
    if (result.good() && (samplesPerPixel > 1)) {
      result = datsetItem->findAndGetUint16(DCM_PlanarConfiguration,
                                            planarConfiguration);
      if (result.good() && (planarConfiguration == 1)) {
        if (bytesAllocated == 1)
          result = togglePlanarConfiguration8(
              (Uint8*)pixelData, length, samplesPerPixel,
              (Uint16)1 /* switch to "by pixel"*/);
        else
          result = togglePlanarConfiguration16(
              (Uint16*)pixelData, length / 2 /*16 bit*/, samplesPerPixel,
              (Uint16)1 /* switch to "by pixel"*/);
    ...
    if (result.bad()) {
      DCMJPEG_ERROR(
          "True lossless encoder: Unable to change Planar Configuration from "
          "'by plane' to 'by pixel' for encoding");
      return result;
    }
    ...
    if (bytesAllocated * samplesPerPixel * columns * rows *
        ...
        length) {
      DCMJPEG_ERROR(
          "True lossless encoder: Cannot change representation, not enough "
          "data");
      return EC_CannotChangeRepresentation;
    }
    ...
    if ((gLocalByteOrder == EBO_BigEndian) && (bitsAllocated == 8)) {
      result = swapIfNecessary(
          EBO_LittleEndian, gLocalByteOrder,
          OFstatic_cast(void*, OFconst_cast(Uint16*, pixelData)), length,
          sizeof(Uint16));
      if (result.bad()) {
        DCMJPEG_ERROR(
            "True lossless encoder: Unable to swap bytes to respect local byte "
            "ordering");
        return EC_CannotChangeRepresentation;
      }
      byteSwapped = OFTrue;
    ...
    DJEncoder* jpeg = createEncoderInstance(
        toRepParam, djcp, OFstatic_cast(Uint8, bitsAllocated));
    if (jpeg) {
      // main loop for compression: compress each frame
      for (unsigned int i = 0; i < frameCount && result.good(); i++) {
        if (bitsAllocated == 8) {
          jpeg->encode(columns, rows, interpr, samplesPerPixel,
                       (Uint8*)framePointer, jpegData, jpegLen,
                       pixelRepresentation, 0.0, 0.0);
        } else if (bitsAllocated == 16) {
          jpeg->encode(columns, rows, interpr, samplesPerPixel,
                       (Uint16*)framePointer, jpegData, jpegLen,
                       pixelRepresentation, 0.0, 0.0);
        }
        // update variables
        compressedSize += jpegLen;
        ...
        if (jpegLen == 0) {
          ...
          result = pixelSequence->storeCompressedFrame(
              offsetList, jpegData, jpegLen, djcp->getFragmentSize());
        }
        // free memory
        delete[] jpegData;
      }
    ...
    if (result.good()) {
      compressionRatio = ((double)bytesAllocated * samplesPerPixel * columns *
                          rows * OFstatic_cast(unsigned long, numberOfFrames)) /
                         compressedSize;
      pixSeq = pixelSequence;
    } else
      delete pixelSequence;
    ...
    result = updateDerivationDescription(datsetItem, toRepParam, djcp,
                                         OFstatic_cast(Uint8, bitsAllocated),
                                         compressionRatio);
    ...
    if ((datsetItem->ident() == EVR_dataset) && result.good()) {
      ...
      if (djcp->getConvertToSC() || (djcp->getUIDCreation() == EUC_always)) {
        ...
        if (result.good())
          result = DcmCodec::newInstance((DcmItem*)datsetItem, "DCM", "121320",
                                         "Uncompressed predecessor");
      }
    }
    ...
    if (planConfSwitched) {
      if (bytesAllocated == 1)
        result = togglePlanarConfiguration8((Uint8*)pixelData, length,
                                            samplesPerPixel,
                                            (Uint16)0 /*switch to "by plane"*/);
      else
        result = togglePlanarConfiguration16(
            (Uint16*)pixelData, length / 2, samplesPerPixel,
            (Uint16)0 /*switch to "by plane"*/);
      if (result.good()) {
        // update Planar Configuration in dataset
        result =
            updatePlanarConfiguration(datsetItem, 0 /* update to "by pixel" */);
      } else {
        DCMJPEG_ERROR(
            "True lossless encoder: Cannot switch back to original planar "
            "configuration of the pixel data");
        result = EC_CannotChangeRepresentation;
      }
    }
...
OFCondition DJCodecEncoder::updateLossyCompressionRatio(DcmItem* dataset,
                                                        double ratio) const {
  if (dataset == NULL)
    return EC_IllegalCall;
...
OFCondition DJCodecEncoder::updateDerivationDescription(
    DcmItem* dataset, const DcmRepresentationParameter* toRepParam,
    const DJCodecParameter* cp, Uint8 bitsPerSample, double ratio) const {
  OFString derivationDescription;

  // create new Derivation Description
  createDerivationDescription(toRepParam, cp, bitsPerSample, ratio,
  ...
  if (result.good()) {
    ...
    if (djcp->getTrueLosslessMode())
      result =
          DcmCodec::insertCodeSequence(dataset, DCM_DerivationCodeSequence,
...
OFCondition DJCodecEncoder::adjustOverlays(DcmItem* dataset,
...
OFCondition DJCodecEncoder::encodeMonochromeImage(
    DcmItem* dataset, const DcmRepresentationParameter* toRepParam,
    DcmPixelSequence*& pixSeq, const DJCodecParameter* cp,
  ...
  OFBool mode_XA =
      OFFalse;  // true if the current SOP class uses the X-Ray Image Module
  OFBool mode_CT =
      OFFalse;  // true if the current SOP class uses the CT Image Module
  ...
  // create DicomImage object. Will fail if dcmimage has not been activated in
  // main(). transfer syntax can be any uncompressed one.
  DicomImage dimage(dataset, EXS_LittleEndianImplicit,
                    flags);  // read all frames
  ...
  if (result.good()) {
    switch (windowType) {
      ...
      {
        // disable correction of polarity or any other presentation LUT
        // transformation
        ...
        if (classUID && !cp->getConvertToSC()) {
          // these three SOP classes use the X-Ray Image Module in which the
          // meaning of the Modality LUT transformation is "inversersed" and,
          // therefore, needs special handling.  This is not an issue if we're
          // converting to secondary capture anyway.
          if ((0 == strcmp(classUID, UID_XRayAngiographicImageStorage)) ||
            ...
            mode_XA = OFTrue;
            mode_useModalityRescale =
                OFFalse;  // inverse definition of Modality LUT Module
          }

          // CT is also a special case because the Modality LUT is required here
          // to convert to Hounsfield units. Again, this is not an issue if
          // we're converting to SC anyway.
        ...
        if (result.good()) {
          ...
          if (!dimage.getMinMaxValues(minRange, maxRange, 1))
            result = EC_IllegalCall;
        ...
        if (result.good()) {
          ...
          if (!dimage.getMinMaxValues(minUsed, maxUsed, 0))
            result = EC_IllegalCall;
          if (maxUsed < minUsed)
            result = EC_IllegalCall;
        }
      ...
      case 1:  // use the n-th VOI window from the image file
      {
        ...
        if ((windowParameter < 1) ||
            (windowParameter > dimage.getWindowCount()))
          result = EC_IllegalCall;
        if (!dimage.setWindow(windowParameter - 1))
          result = EC_IllegalCall;
      } break;
      case 2:  // use the n-th VOI look up table from the image file
      {
        unsigned long windowParameter = cp->getWindowParameter();
        if ((windowParameter < 1) ||
            (windowParameter > dimage.getVoiLutCount()))
          result = EC_IllegalCall;
        if (!dimage.setVoiLut(windowParameter - 1))
          result = EC_IllegalCall;
      } break;
      case 3:  // Compute VOI window using min-max algorithm
        if (!dimage.setMinMaxWindow(0))
          result = EC_IllegalCall;
        break;
      case 4:  // Compute VOI window using Histogram algorithm, ignoring n
               // percent
      {
        unsigned long windowParameter = cp->getWindowParameter();
        if (!dimage.setHistogramWindow(((double)windowParameter) / 100.0))
          result = EC_IllegalCall;
      } break;
      ...
      case 7:  // Compute region of interest VOI window
      {
        ...
        if (!dimage.setRoiWindow(left_pos, top_pos, width, height))
          result = EC_IllegalCall;
      } break;
  ...
  if (result.good()) {
    ...
    if (jpeg) {
      ...
      if (windowType == 0) {
        ...
        if (mode_usePixelValues) {
          ...
          if (z >= 0)
            ...
            z = ceil(z - 1023.0);
          double rangeUsed =
              maxUsed - minUsed + 1;  // range that must be covered by window

          // compute optimum window width
          if ((unsigned long)pixelDepth < (unsigned long)bitsPerSample)
            windowWidth = 1 << pixelDepth;
          else
            windowWidth = 1 << bitsPerSample;
          while (windowWidth < rangeUsed)
            windowWidth *= 2;

          windowCenter = windowWidth * 0.5;
          if ((z + windowWidth) > maxUsed)
            windowCenter += z;
          else
            windowCenter += minUsed;
        ...
        if (mode_useModalityRescale) {
          ...
          // back to the original value range, keeping all VOI transformations
          // valid.
          if (mode_usePixelValues) {
          ...
          // Don't use Rescale Slope/Intercept (remove or encode as identity
          // transformation). Modify window centers/widths encoded in image,
          // remove VOI LUTs.
          deleteVOILUT = OFTrue;
          if (mode_usePixelValues) {
            voiFactor = ((1 << bitsPerSample) - 1) /
                        (windowWidth - 1);  // 1/rescaleSlope
            voiOffset =
                (windowWidth * 0.5) - windowCenter;  // - rescaleIntercept
          } else {
            voiFactor = ((1 << bitsPerSample) - 1) /
                        (maxRange - minRange);  // 1/rescaleSlope
      ...
      // compute original image size in bytes, ignoring any padding bits.
      Uint16 samplesPerPixel = 0;
      if ((dataset->findAndGetUint16(DCM_SamplesPerPixel, samplesPerPixel))
              .bad())
        samplesPerPixel = 1;
      ...
      uncompressedSize =
          columns * rows * pixelDepth * frameCount * samplesPerPixel / 8.0;
      for (unsigned long i = 0; (i < frameCount) && (result.good()); i++) {
        frame = dimage.getOutputData(bitsPerSample, i, 0);
        if (frame == NULL)
          result = EC_MemoryExhausted;
          ...
          jpegData = NULL;
          if (bytesPerSample == 1) {
            result = jpeg->encode(columns, rows, EPI_Monochrome2, 1,
                                  (Uint8*)frame, jpegData, jpegLen,
                                  pixelRepresentation, minUsed, maxUsed);
          } else {
            result = jpeg->encode(columns, rows, EPI_Monochrome2, 1,
                                  (Uint16*)frame, jpegData, jpegLen,
                                  pixelRepresentation, minUsed, maxUsed);
  ...
  if (result.good()) {
    // adapt attributes in image pixel module
    if (result.good())
      result = dataset->putAndInsertUint16(DCM_SamplesPerPixel, 1);
    if (result.good() && (windowType != 0))
      result = dataset->putAndInsertString(DCM_PhotometricInterpretation,
                                           "MONOCHROME2");
    if (result.good()) {
      if (bitsPerSample > 8)
    ...
    if ((dataset->search(DCM_ModalityLUTSequence, stack, ESM_fromHere, OFFalse))
            .good())
      foundModalityLUT = OFTrue;
    if (!foundModalityLUT) {
      stack.clear();
      if ((dataset->search(DCM_RescaleSlope, stack, ESM_fromHere, OFFalse))
              .good())
        foundModalityLUT = OFTrue;
    }
    ...
    if (windowType == 0) {
      if (mode_XA)  // XA needs special handling
      {
        ...
        if (result.good())
          result = dataset->putAndInsertString(DCM_PixelIntensityRelationship,
                                               "DISP");
      }
      /* else if we had a modality LUT before, a LUT is inserted again.
         or if specific rescale slope/intercept has been computed, use that in
         image
       */
      else if (foundModalityLUT || rescaleSlope != 1.0 ||
               rescaleIntercept != 0.0) {
        char buf[64];
        OFStandard::ftoa(buf, sizeof(buf), rescaleIntercept,
                         OFStandard::ftoa_uppercase, 0, 6);
        if (result.good())
          result = dataset->putAndInsertString(DCM_RescaleIntercept, buf);
        OFStandard::ftoa(buf, sizeof(buf), rescaleSlope,
                         OFStandard::ftoa_uppercase, 0, 6);
        if (result.good())
          result = dataset->putAndInsertString(DCM_RescaleSlope, buf);
        if (result.good()) {
          if (mode_CT)
            result = dataset->putAndInsertString(DCM_RescaleType,
                                                 "HU");  // Hounsfield units
          else
            result = dataset->putAndInsertString(DCM_RescaleType,
                                                 "US");  // unspecified
      ...
      // if we had found a Modality LUT Transformation, create a identity LUT
      // transformation
      if (foundModalityLUT) {
        if (result.good())
          result = dataset->putAndInsertString(DCM_RescaleIntercept, "0");
        if (result.good())
          result = dataset->putAndInsertString(DCM_RescaleSlope, "1");
        if (result.good())
          result = dataset->putAndInsertString(DCM_RescaleType,
                                               "US");  // unspecified
      }

      // Adjust Pixel Intensity Relationship (needed for XA). If present, set
      // value to "DISP".
      stack.clear();
      if ((dataset->search(DCM_PixelIntensityRelationship, stack, ESM_fromHere,
                           OFFalse))
              .good()) {
        if (result.good())
          result = dataset->putAndInsertString(DCM_PixelIntensityRelationship,
    ...
    if (windowType == 0) {
      ...
      if (result.good())
        result = correctVOIWindows(dataset, voiOffset, voiFactor);
      ...
      if ((dataset->search(DCM_PresentationLUTSequence, stack, ESM_fromHere,
                           OFFalse))
              .good())
        foundPresentationLUT = OFTrue;
      if (!foundPresentationLUT) {
        stack.clear();
        if ((dataset->search(DCM_PresentationLUTShape, stack, ESM_fromHere,
                             OFFalse))
                .good())
          foundPresentationLUT = OFTrue;
      }
      ...
      // if we had found a Presentation LUT Transformation, create a new
...
OFCondition DJCodecEncoder::correctVOIWindows(DcmItem* dataset,
                                              double voiOffset,
                                              double voiFactor) {
  if (voiOffset == 0.0 && voiFactor == 1.0)
    return EC_Normal;
  ...
  DcmStack stack;
  if ((dataset->search(DCM_WindowCenter, stack, ESM_fromHere, OFFalse))
          .good()) {
  ...
  stack.clear();
  if ((dataset->search(DCM_WindowCenterWidthExplanation, stack, ESM_fromHere,
                       OFFalse))
          .good()) {
  ...
  if (center && width) {
    ...
    for (unsigned long i = 0; i < numWindows; i++) {
      if (((center->getFloat64(currentCenter, i)).good()) &&
          ((width->getFloat64(currentWidth, i)).good())) {
        // found one pair of values, adapt them to value range shifted pixel
        // data
        tempCenter = (currentCenter + voiOffset) * voiFactor;
        tempWidth = currentWidth * voiFactor;
        // add this window to the attribute values that are later replacing old
        // windows
        OFStandard::ftoa(buf, sizeof(buf), tempCenter,
                         OFStandard::ftoa_uppercase, 0, 6);
        if (newCenter.length() > 0)
          newCenter += "\\";
        newCenter += buf;
        OFStandard::ftoa(buf, sizeof(buf), tempWidth,
                         OFStandard::ftoa_uppercase, 0, 6);
        if (newWidth.length() > 0)
          newWidth += "\\";
        newWidth += buf;

        if (newExplanation.length() > 0)
          newExplanation += "\\";
        if (explanation &&
            ((explanation->getOFString(currentExplanation, i)).good()))
  ...
  if (newCenter.length() > 0) {
    if (result.good())
      result = dataset->putAndInsertString(DCM_WindowCenter, newCenter.c_str());
    if (result.good())
      result = dataset->putAndInsertString(DCM_WindowWidth, newWidth.c_str());
    if (result.good())
      result = dataset->putAndInsertString(DCM_WindowCenterWidthExplanation,
                                           newExplanation.c_str());
  }
...
OFCondition DJCodecEncoder::togglePlanarConfiguration8(
    Uint8* pixelData, const unsigned long numValues,
...
OFCondition DJCodecEncoder::updatePlanarConfiguration(
    DcmItem* item, const Uint16 newPlanConf) const {
 ...
 *   For this new method to work with compressed images, all classes derived
 * from DcmCodec need to implement a new method decodeFrame(). For now, only
 ...
 *   that prevented "windowed" compression of images containing an Icon Image
 * SQ.
 *
