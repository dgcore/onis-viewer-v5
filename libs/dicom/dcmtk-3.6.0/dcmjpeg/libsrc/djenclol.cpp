...
DJEncoderLossless::DJEncoderLossless() : DJCodecEncoder() {}
...
void DJEncoderLossless::createDerivationDescription(
  ...
  if (cp->getTrueLosslessMode())
    ...
    derivationDescription =
        "Pseudo-Lossless JPEG compression, selection value ";
...
DJEncoder* DJEncoderLossless::createEncoderInstance(
  ...
  if (bitsPerSample > 12)
    result = new DJCompressIJG16Bit(*cp, EJM_lossless, rp->getPrediction(),
                                    rp->getPointTransformation());
  else if (bitsPerSample > 8)
    result = new DJCompressIJG12Bit(*cp, EJM_lossless, rp->getPrediction(),
                                    rp->getPointTransformation());
  else
    result = new DJCompressIJG8Bit(*cp, EJM_lossless, rp->getPrediction(),
                                   rp->getPointTransformation());
  return result;
}
