...
DiJPEGPlugin::DiJPEGPlugin()
    : DiPluginFormat(), Quality(75), Sampling(ESS_444) {}
...
int DiJPEGPlugin::write(DiImage* image, FILE* stream,
  ...
  if ((image != NULL) && (stream != NULL)) {
    ...
    if (data != NULL) {
      const OFBool isMono =
          (image->getInternalColorModel() == EPI_Monochrome1) ||
          (image->getInternalColorModel() == EPI_Monochrome2);

      /* taking the address of the variable prevents allocation
      ...
      cinfo.in_color_space =
          (isMono)
              ? JCS_GRAYSCALE
              : ((image->getInternalColorModel() == EPI_YBR_Full) ? JCS_YCbCr
                                                                  : JCS_RGB);
      ...
      if (cinfo.jpeg_color_space == JCS_YCbCr) {
        switch (Sampling) {
          ...
          case ESS_422: /* 4:2:2 sampling (horizontal subsampling of chroma
            ...
            cinfo.comp_info[0].v_samp_factor = 1;
            break;
          case ESS_411: /* 4:1:1 sampling (horizontal and vertical subsampling
      ...
      const unsigned long row_stride =
 ...
 *   allocation issued by gcc 3.2 -Wuninitialized
 *
