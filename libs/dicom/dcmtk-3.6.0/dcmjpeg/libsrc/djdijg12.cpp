 ...
 *
 *  Purpose: decompression routines of the IJG JPEG library configured for 12
 * bits/sample.
 *
...
ijg_boolean DJDIJG12fillInputBuffer(j_decompress_ptr cinfo) {
  ...
  if (src->next_buffer) {
    ...
    // The suspension was caused by DJDIJG12skipInputData iff src->skip_bytes >
...
DJDecompressIJG12Bit::DJDecompressIJG12Bit(const DJCodecParameter& cp,
                                           OFBool isYBR)
    : DJDecoder(),
      cparam(&cp),
      cinfo(NULL),
...
OFCondition DJDecompressIJG12Bit::init() {
  ...
  if (cinfo) {
    ...
    if (jerr) {
      ...
      cinfo->err =
          jpeg_std_error(&OFconst_cast(DJDIJG12ErrorStruct*, jerr)->pub);
      jerr->instance = this;
      ...
      if (setjmp(OFconst_cast(DJDIJG12ErrorStruct*, jerr)->setjmp_buffer)) {
        ...
        cleanup();
        return makeOFCondition(OFM_dcmjpeg, EJCode_IJG12_Decompression,
                               OF_error, buffer);
      }
    } else {
      delete cinfo;
...
OFCondition DJDecompressIJG12Bit::decode(Uint8* compressedFrameBuffer,
  ...
  if (setjmp(((DJDIJG12ErrorStruct*)(cinfo->err))->setjmp_buffer)) {
    ...
    (*cinfo->err->format_message)((jpeg_common_struct*)cinfo,
                                  buffer); /* Create the message */
    cleanup();
    return makeOFCondition(OFM_dcmjpeg, EJCode_IJG12_Decompression, OF_error,
                           buffer);
  }
