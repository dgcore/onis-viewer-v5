 ...
 *
 *  Purpose: compression routines of the IJG JPEG library configured for 12
 * bits/sample.
 *
...
static void jpeg_simple_spectral_selection(j_compress_ptr cinfo) {
  ...
  if (ncomps == 3 && cinfo->jpeg_color_space == JCS_YCbCr)
    ...
    nscans = 1 + 2 * ncomps; /* 1 DC scan; 2 AC scans per component */

  /* Allocate space for script.
   * We need to put it in the permanent pool in case the application performs
   * multiple compressions without changing the settings.  To avoid a memory
   * leak if jpeg_simple_spectral_selection is called repeatedly for the same
   * JPEG object, we try to re-use previously allocated space, and we allocate
...
DJCompressIJG12Bit::DJCompressIJG12Bit(const DJCodecParameter& cp, EJ_Mode mode,
                                       Uint8 theQuality)
    : DJEncoder(),
      cparam(&cp),
      quality(theQuality),
...
OFCondition DJCompressIJG12Bit::encode(Uint16 /* columns */, Uint16 /* rows */,
                                       EP_Interpretation /* interpr */,
                                       Uint16 /* samplesPerPixel */,
                                       Uint8* /* image_buffer */,
                                       Uint8*& /* to */, Uint32& /* length */,
...
OFCondition DJCompressIJG12Bit::encode(
  ...
  switch (modeofOperation) {
    case EJM_baseline:  // baseline only supports 8 bits/sample. Assume
                        // sequential.
    case EJM_sequential:
  ...
  if (cinfo.jpeg_color_space == JCS_YCbCr) {
    switch (cparam->getSampleFactors()) {
      case ESS_444: /* 4:4:4 sampling (no subsampling) */
        ...
        break;
      case ESS_422: /* 4:2:2 sampling (horizontal subsampling of chroma
                       components) */
        cinfo.comp_info[0].h_samp_factor = 2;
        cinfo.comp_info[0].v_samp_factor = 1;
        break;
      case ESS_411: /* 4:1:1 sampling (horizontal and vertical subsampling of
                       chroma components) */
        cinfo.comp_info[0].h_samp_factor = 2;
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    ...
    row_pointer[0] =
        (JSAMPLE*)(&image_buffer[cinfo.next_scanline * row_stride]);
  ...
  if (pixelDataList.size() > 1)
    length += (pixelDataList.size() - 1) * IJGE12_BLOCKSIZE;
  if (length % 2)
