 ...
 *
 *  Purpose: decompression routines of the IJG JPEG library configured for 16
 * bits/sample.
 *
...
// disable any preprocessor magic the IJG library might be doing with the
// "const" keyword
#ifdef const
#undef const
#endif
...
struct DJDIJG16SourceManagerStruct {
  ...
  // buffer from which reading will continue as soon as the current buffer is
  // empty
  Uint8* next_buffer;
...
ijg_boolean DJDIJG16fillInputBuffer(j_decompress_ptr cinfo) {
  ...
  if (src->next_buffer) {
    ...
    // The suspension was caused by DJDIJG16skipInputData iff src->skip_bytes >
    // 0. In this case we must skip the remaining number of bytes here.
    if (src->skip_bytes > 0) {
...
DJDecompressIJG16Bit::DJDecompressIJG16Bit(const DJCodecParameter& cp,
                                           OFBool isYBR)
    : DJDecoder(),
      cparam(&cp),
      cinfo(NULL),
...
OFCondition DJDecompressIJG16Bit::init() {
  ...
  if (cinfo) {
    ...
    if (jerr) {
      ...
      cinfo->err =
          jpeg_std_error(&OFconst_cast(DJDIJG16ErrorStruct*, jerr)->pub);
      jerr->instance = this;
      ...
      if (setjmp(OFconst_cast(DJDIJG16ErrorStruct*, jerr)->setjmp_buffer)) {
        ...
        (*cinfo->err->format_message)((jpeg_common_struct*)cinfo,
                                      buffer); /* Create the message */
        cleanup();
        return makeOFCondition(OFM_dcmjpeg, EJCode_IJG16_Decompression,
                               OF_error, buffer);
      }
    } else {
      delete cinfo;
    ...
    if (cparam->predictor6WorkaroundEnabled())
      cinfo->workaround_options |= WORKAROUND_PREDICTOR6OVERFLOW;
  } else
    return EC_MemoryExhausted;
...
OFCondition DJDecompressIJG16Bit::decode(Uint8* compressedFrameBuffer,
                                         Uint32 compressedFrameBufferSize,
                                         Uint8* uncompressedFrameBuffer,
                                         Uint32 uncompressedFrameBufferSize,
                                         OFBool isSigned) {
  if (cinfo == NULL || compressedFrameBuffer == NULL ||
      uncompressedFrameBuffer == NULL)
  ...
  if (setjmp(((DJDIJG16ErrorStruct*)(cinfo->err))->setjmp_buffer)) {
    ...
    (*cinfo->err->format_message)((jpeg_common_struct*)cinfo,
                                  buffer); /* Create the message */
    cleanup();
    return makeOFCondition(OFM_dcmjpeg, EJCode_IJG16_Decompression, OF_error,
                           buffer);
  }
  ...
  if (suspension < 2) {
    ...
    switch (cparam->getDecompressionColorSpaceConversion()) {
      case EDC_photometricInterpretation:  // color space conversion if DICOM
      ...
      case EDC_never:  // never do color space conversion
        break;
      case EDC_guessLossyOnly:  // use color space guess by IJG library if lossy
                                // JPEG
        if (cinfo->process != JPROC_LOSSLESS) {
  ...
  if (suspension < 3) {
    ...
    bufsize = cinfo->output_width *
              cinfo->output_components;   // number of JSAMPLEs per row
  ...
  if (uncompressedFrameBufferSize < rowsize * cinfo->output_height)
  ...
  while (cinfo->output_scanline < cinfo->output_height) {
    ...
    memcpy(uncompressedFrameBuffer + (cinfo->output_scanline - 1) * rowsize,
           *buffer, rowsize);
  }
