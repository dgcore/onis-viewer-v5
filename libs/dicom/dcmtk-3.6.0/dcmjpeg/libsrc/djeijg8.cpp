 ...
 *
 *  Purpose: compression routines of the IJG JPEG library configured for 8
 * bits/sample.
 *
...
DJCompressIJG8Bit::DJCompressIJG8Bit(const DJCodecParameter& cp, EJ_Mode mode,
                                     Uint8 theQuality)
    : DJEncoder(),
      cparam(&cp),
      quality(theQuality),
...
OFCondition DJCompressIJG8Bit::encode(Uint16 /* columns */, Uint16 /* rows */,
...
OFCondition DJCompressIJG8Bit::encode(Uint16 columns, Uint16 rows,
                                      ...
                                      Uint32& length, Uint8 pixelRepresentation,
  ...
  if (setjmp(jerr.setjmp_buffer)) {
    ...
    return makeOFCondition(OFM_dcmjpeg, EJCode_IJG8_Compression, OF_error,
                           buffer);
  }
  ...
  if (pixelDataList.size() > 1)
    length += (pixelDataList.size() - 1) * IJGE8_BLOCKSIZE;
  if (length % 2)
