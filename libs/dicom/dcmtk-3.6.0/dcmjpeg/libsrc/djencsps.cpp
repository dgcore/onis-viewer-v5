...
void DJEncoderSpectralSelection::createDerivationDescription(
  ...
  if (bitsPerSample > 8)
    derivationDescription += "12 bit";
  else
    derivationDescription += "8 bit";
...
DJEncoder* DJEncoderSpectralSelection::createEncoderInstance(
  ...
  if (bitsPerSample > 8)
    result =
        new DJCompressIJG12Bit(*cp, EJM_spectralSelection, rp->getQuality());
  else
    result =
        new DJCompressIJG8Bit(*cp, EJM_spectralSelection, rp->getQuality());
  return result;
}
