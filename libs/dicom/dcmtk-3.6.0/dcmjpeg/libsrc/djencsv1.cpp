 ...
 *
 *  Purpose: Codec class for encoding JPEG Lossless Selection Value 1
 * (8/12/16-bit)
 *
...
DJEncoderP14SV1::DJEncoderP14SV1() : DJCodecEncoder() {}
...
void DJEncoderP14SV1::createDerivationDescription(
  ...
  const DJ_RPLossless* rp =
      toRepParam ? (const DJ_RPLossless*)toRepParam : &defaultRP;
  char buf[64];

  if (cp->getTrueLosslessMode())
    derivationDescription =
        "Lossless JPEG compression, selection value 1, point transform ";
  else
    derivationDescription =
        "Pseudo-Lossless JPEG compression, selection value 1, point transform ";
...
DJEncoder* DJEncoderP14SV1::createEncoderInstance(
  ...
  if (bitsPerSample > 12)
    result = new DJCompressIJG16Bit(*cp, EJM_lossless, 1,
                                    rp->getPointTransformation());
  else if (bitsPerSample > 8)
    result = new DJCompressIJG12Bit(*cp, EJM_lossless, 1,
                                    rp->getPointTransformation());
  else
    result = new DJCompressIJG8Bit(*cp, EJM_lossless, 1,
                                   rp->getPointTransformation());
  return result;
}
