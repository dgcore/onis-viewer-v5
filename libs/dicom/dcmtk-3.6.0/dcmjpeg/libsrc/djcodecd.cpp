...
DJCodecDecoder::DJCodecDecoder() : DcmCodec() {}
...
OFCondition DJCodecDecoder::decode(
    ...
    DcmPolymorphOBOW& uncompressedPixelData, const DcmCodecParameter* cp,
  ...
  if ((!dataset) ||
    ...
    Uint16 pixelRep =
        0;  // needed to decline color conversion of signed pixel data to RGB
    OFBool numberOfFramesPresent = OFFalse;

    if (result.good())
      result =
          ((DcmItem*)dataset)
              ->findAndGetUint16(DCM_SamplesPerPixel, imageSamplesPerPixel);
    if (result.good())
      result = ((DcmItem*)dataset)->findAndGetUint16(DCM_Rows, imageRows);
    if (result.good())
      result = ((DcmItem*)dataset)->findAndGetUint16(DCM_Columns, imageColumns);
    if (result.good())
      result = ((DcmItem*)dataset)
                   ->findAndGetUint16(DCM_BitsAllocated, imageBitsAllocated);
    if (result.good())
      result = ((DcmItem*)dataset)
                   ->findAndGetUint16(DCM_BitsStored, imageBitsStored);
    if (result.good())
      result = ((DcmItem*)dataset)->findAndGetUint16(DCM_HighBit, imageHighBit);
    if (result.good())
      result = ((DcmItem*)dataset)
                   ->findAndGetUint16(DCM_PixelRepresentation, pixelRep);
    ...
    if (result.good()) {
      if (((DcmItem*)dataset)
              ->findAndGetSint32(DCM_NumberOfFrames, imageFrames)
              .good())
        numberOfFramesPresent = OFTrue;
    }

    // we consider SOP Class UID as optional since we only need it to determine
    // SOP Class specific encoding rules for planar configuration.
    if (result.good())
      (void)((DcmItem*)dataset)->findAndGetString(DCM_SOPClassUID, sopClassUID);

    EP_Interpretation dicomPI =
        DcmJpegHelper::getPhotometricInterpretation((DcmItem*)dataset);

    OFBool isYBR = OFFalse;
    if ((dicomPI == EPI_YBR_Full) || (dicomPI == EPI_YBR_Full_422) ||
        (dicomPI == EPI_YBR_Partial_422))
      isYBR = OFTrue;
    ...
    if (imageFrames < 1)
      imageFrames =
          1;  // default in case the number of frames attribute contains garbage

    if (result.good()) {
      DcmPixelItem* pixItem = NULL;
      Uint8* jpegData = NULL;
      result = pixSeq->getItem(
          pixItem, 1);  // first item is offset table, use second item
      if (result.good()) {
        ...
        if (result.good()) {
          if (jpegData == NULL)
            ...
            if (jp2k)  // JPEG2000 support
              precision = (Uint8)imageBitsAllocated;

            if (precision == 0)
              result = EC_CannotChangeRepresentation;  // something has gone
                                                       // wrong, bail out
            else {
              DJDecoder* jpeg =
                  createDecoderInstance(fromRepParam, djcp, precision, isYBR);
              if (jpeg == NULL)
                result = EC_MemoryExhausted;
              else {
                Uint32 frameSize =
                    ((precision > 8) ? sizeof(Uint16) : sizeof(Uint8)) *
                    imageRows * imageColumns * imageSamplesPerPixel;
                ...
                if (isYBR &&
                    (imageBitsStored <
                     imageBitsAllocated))  // check for a special case that is
                                           // currently not handled properly
                {
                  if (djcp->getDecompressionColorSpaceConversion() !=
                      EDC_never) {
                    DCMJPEG_WARN(
                        "BitsStored < BitsAllocated for JPEG compressed image "
                        "with YCbCr color model, color space conversion will "
                        "probably not work properly");
                    DCMJPEG_DEBUG(
                        "workaround: use option --conv-never (for command line "
                        "tools) or EDC_never (for the "
                        "DJDecoderRegistration::registerCodecs() call)");
                  }
                }

                result = uncompressedPixelData.createUint16Array(
                ...
                if (result.good()) {
                    ...
                    if (result.good()) {
                      ...
                      while (EJ_Suspension == result) {
                        ...
                        if (result.good()) {
                          ...
                          if (result.good()) {
                            result =
                                jpeg->decode(jpegData, fragmentLength,
                                             imageData8, frameSize, isSigned);
                          }
                        }
                      }
                      if (result.good()) {
                        if (!createPlanarConfigurationInitialized) {
                          // we need to know the decompressed photometric
                          // interpretation in order to determine the final
                          // planar configuration.  However, this is only known
                          // after the first call to jpeg->decode(), i.e. here.
                          colorModel = jpeg->getDecompressedColorModel();
                          if (colorModel == EPI_Unknown) {
                            // derive color model from DICOM photometric
                            // interpretation
                            if ((dicomPI == EPI_YBR_Full_422) ||
                                (dicomPI == EPI_YBR_Partial_422))
                          ...
                          switch (djcp->getPlanarConfiguration()) {
                            case EPC_default:
                              createPlanarConfiguration =
                                  requiresPlanarConfiguration(sopClassUID,
                                                              colorModel);
                              break;
                        ...
                        if ((imageSamplesPerPixel == 3) &&
                            createPlanarConfiguration) {
                          if (precision > 8)
                            result = createPlanarConfigurationWord(
                                (Uint16*)imageData8, imageColumns, imageRows);
                          else
                            result = createPlanarConfigurationByte(
                                imageData8, imageColumns, imageRows);
                        }
                        currentFrame++;
                  ...
                  if (result.good()) {
                    // decompression is complete, finally adjust byte order if
                    // necessary
                    if (jpeg->bytesPerSample() ==
                        1)  // we're writing bytes into words
                    {
                      result = swapIfNecessary(gLocalByteOrder,
                                               EBO_LittleEndian, imageData16,
                                               totalSize, sizeof(Uint16));
                    }
                  }

                  // adjust photometric interpretation depending on what
                    ...
                    switch (colorModel) {
                      case EPI_Monochrome2:
                        ...
                        if (result.good()) {
                          ...
                          result =
                              ((DcmItem*)dataset)
                                  ->putAndInsertUint16(DCM_SamplesPerPixel,
                      ...
                      case EPI_YBR_Full:
                        result =
                            ((DcmItem*)dataset)
                                ->putAndInsertString(
                                    DCM_PhotometricInterpretation, "YBR_FULL");
                        if (result.good()) {
                      ...
                      case EPI_RGB:
                        result = ((DcmItem*)dataset)
                                     ->putAndInsertString(
                                         DCM_PhotometricInterpretation, "RGB");
                        if (result.good()) {
                          imageSamplesPerPixel = 3;
                          result =
                              ((DcmItem*)dataset)
                        ...
                        break;
                      default:
                        /* leave photometric interpretation untouched unless it
                         * is YBR_FULL_422 or YBR_PARTIAL_422. In this case,
                         * replace by YBR_FULL since decompression eliminates
                         * the subsampling.
                         */
                        if ((dicomPI == EPI_YBR_Full_422) ||
                            (dicomPI == EPI_YBR_Partial_422)) {
                  ...
                  if (result.good()) {
                    if (precision > 8)
                      result = ((DcmItem*)dataset)
                                   ->putAndInsertUint16(DCM_BitsAllocated, 16);
                    else
                      result = ((DcmItem*)dataset)
                                   ->putAndInsertUint16(DCM_BitsAllocated, 8);
                  }

                  // Planar Configuration depends on the
                  // createPlanarConfiguration flag
                  if ((result.good()) && (imageSamplesPerPixel > 1)) {
                    result = ((DcmItem*)dataset)
                                 ->putAndInsertUint16(
                                     DCM_PlanarConfiguration,
                                     (createPlanarConfiguration ? 1 : 0));
                  }
                  ...
                  if ((result.good()) && (imageBitsStored > precision)) {
                    result =
                        ((DcmItem*)dataset)
                            ->putAndInsertUint16(DCM_BitsStored, precision);
                  }

                  // High Bit cannot be larger than precision - 1
                  if ((result.good()) && ((unsigned long)(imageHighBit + 1) >
                                          (unsigned long)precision)) {
                    result =
                        ((DcmItem*)dataset)
                            ->putAndInsertUint16(DCM_HighBit, precision - 1);
                  }

                  // Number of Frames might have changed in case the previous
                  // value was wrong
                  if (result.good() &&
                      (numberOfFramesPresent || (imageFrames > 1))) {
                    char numBuf[20];
                    sprintf(numBuf, "%ld", OFstatic_cast(long, imageFrames));
                    result =
                        ((DcmItem*)dataset)
                            ->putAndInsertString(DCM_NumberOfFrames, numBuf);
                  }

                  // Pixel Representation could be signed if lossless JPEG. For
                  // now, we just believe what we get.
                }
                delete jpeg;
              }
            }
...
OFCondition DJCodecDecoder::decodeFrame(
  ...
  if ((!dataset) ||
    ...
    if (result.good())
      result =
          dataset->findAndGetUint16(DCM_SamplesPerPixel, imageSamplesPerPixel);
    if (result.good())
      result = dataset->findAndGetUint16(DCM_Rows, imageRows);
    if (result.good())
      result = dataset->findAndGetUint16(DCM_Columns, imageColumns);
    if (result.good())
      result = dataset->findAndGetUint16(DCM_BitsAllocated, imageBitsAllocated);
    if (result.good())
      result = dataset->findAndGetUint16(DCM_BitsStored, imageBitsStored);
    if (result.good())
      result = dataset->findAndGetUint16(DCM_HighBit, imageHighBit);
    if (result.good())
      result = dataset->findAndGetUint16(DCM_PixelRepresentation, pixelRep);
    if (result.good())
    ...
    if (imageSamplesPerPixel > 1) {
      if (result.good())
        result =
            dataset->findAndGetUint16(DCM_PlanarConfiguration, planarConfig);
    }
    ...
    EP_Interpretation dicomPI =
        DcmJpegHelper::getPhotometricInterpretation(dataset);
    ...
    // now access and decompress the frame starting at the item we have
    // identified
    if (result.good()) {
      DcmPixelItem* pixItem = NULL;
      Uint8* jpegData = NULL;
        ...
        if (result.good()) {
          if (jpegData == NULL)
            ...
            if (precision == 0)
              ...
              DJDecoder* jpeg =
                  createDecoderInstance(fromParam, djcp, precision, isYBR);
              if (jpeg == NULL)
                result = EC_MemoryExhausted;
              else {
                if (isYBR &&
                ...
                if (result.good()) {
                  ...
                  while (EJ_Suspension == result) {
                    ...
                    if (result.good()) {
                      ...
                      if (result.good()) {
                        result =
                            jpeg->decode(jpegData, fragmentLength,
                                         (Uint8*)buffer, frameSize, isSigned);
                        pastLastFragmentUsed = currentItem;
                      }
                    }
                  }
                  if (result.good()) {
                    // convert planar configuration to color by plane if
                    // necessary
                    if ((imageSamplesPerPixel == 3) && (planarConfig == 1)) {
                      if (precision > 8)
                        result = createPlanarConfigurationWord(
                            (Uint16*)buffer, imageColumns, imageRows);
                      else
                        result = createPlanarConfigurationByte(
                  ...
                  if (result.good()) {
                        ...
                        1)  // we're writing bytes into words
                    {
                      result = swapIfNecessary(
                          gLocalByteOrder, EBO_LittleEndian, (Uint16*)buffer,
                  ...
                  if (result.good()) {
                    ...
                    decompressedColorModel =
                        photometricInterpretation;  // this is the default

                    // now see if we have to change the photometric
                    // interpretation because the decompression has changed
                    // something
...
OFCondition DJCodecDecoder::encode(
    const Uint16* /* pixelData */, const Uint32 /* length */,
    const DcmRepresentationParameter* /* toRepParam */,
    DcmPixelSequence*& /* pixSeq */, const DcmCodecParameter* /* cp */,
...
OFCondition DJCodecDecoder::determineDecompressedColorModel(
  ...
  if ((dataset != NULL) && (fromPixSeq != NULL)) {
    ...
    if ((fromPixSeq->getUncompressedFrameSize(dataset, bufSize).good()) &&
        (bufSize > 0)) {
      // allocate temporary buffer for a single frame
      Uint8* buffer = new Uint8[bufSize];
      if (buffer != NULL) {
        DCMJPEG_DEBUG(
            "decompressing first frame to determine the decompressed color "
            "model");
        // simple approach: decode first frame in order to determine the
        // uncompressed color model
        result = decodeFrame(
            fromParam, fromPixSeq, cp, dataset, 0 /* frameNo */, startFragment,
...
Uint8 DJCodecDecoder::scanJpegDataForBitDepth(const Uint8* data,
  ...
  while (offset + 4 < fragmentLength) {
    switch (readUint16(data + offset)) {
      ...
      case 0xffc3:  // SOF_3: JPEG lossless sequential
        return data[offset + 4];
        /* break; */
      case 0xffc5:  // SOF_5: differential (hierarchical) extended sequential,
                    // Huffman
        return data[offset + 4];
        /* break; */
      ...
      case 0xffcb:  // SOF_11: lossless, arithmetic
        return data[offset + 4];
        /* break; */
      case 0xffcd:  // SOF_13: differential (hierarchical) extended sequential,
                    // arithmetic
        return data[offset + 4];
        /* break; */
      case 0xffce:  // SOF_14: differential (hierarchical) progressive,
                    // arithmetic
        return data[offset + 4];
        /* break; */
...
OFCondition DJCodecDecoder::createPlanarConfigurationByte(Uint8* imageFrame,
                                                          Uint16 columns,
                                                          Uint16 rows) {
  if (imageFrame == NULL)
    return EC_IllegalCall;
...
OFCondition DJCodecDecoder::createPlanarConfigurationWord(Uint16* imageFrame,
                                                          Uint16 columns,
                                                          Uint16 rows) {
  if (imageFrame == NULL)
    return EC_IllegalCall;
...
/* This method examines if a given image requires color-by-plane planar
 * configuration depending on SOP Class UID (DICOM IOD) and photometric
 * interpretation. All SOP classes defined in the 2001 edition of the DICOM
 * standard or earlier are handled correctly.
 */

OFBool DJCodecDecoder::requiresPlanarConfiguration(
    const char* sopClassUID, EP_Interpretation photometricInterpretation) {
  if (sopClassUID) {
    OFString sopClass(sopClassUID);

    // Hardcopy Color Image always requires color-by-plane
    if (sopClass == UID_RETIRED_HardcopyColorImageStorage)
      return OFTrue;
    ...
    if (photometricInterpretation == EPI_YBR_Full) {
      if ((sopClass == UID_UltrasoundMultiframeImageStorage) ||
          (sopClass == UID_UltrasoundImageStorage))
        return OFTrue;
    }
  }
  return OFFalse;
 ...
 *   NumberOfFrames is larger than the number of compressed fragments, if and
 * only if there is just a single fragment per frame.
 *
