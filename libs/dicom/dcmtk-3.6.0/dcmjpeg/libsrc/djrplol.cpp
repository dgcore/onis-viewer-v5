...
DJ_RPLossless::DJ_RPLossless(int aPrediction, int aPt)
    : DcmRepresentationParameter(), prediction(aPrediction), pt(aPt) {}

DJ_RPLossless::DJ_RPLossless(const DJ_RPLossless& arg)
    : DcmRepresentationParameter(arg), prediction(arg.prediction), pt(arg.pt) {}
...
OFBool DJ_RPLossless::operator==(const DcmRepresentationParameter& arg) const {
  ...
  if (argname) {
    ...
    if (argstring == className()) {
      ...
      if ((prediction == argll.prediction) && (pt == argll.pt))
        return OFTrue;
    }
  }
  return OFFalse;
