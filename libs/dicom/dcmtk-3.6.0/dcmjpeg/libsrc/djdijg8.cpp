 ...
 *
 *  Purpose: decompression routines of the IJG JPEG library configured for 8
 * bits/sample.
 *
...
ijg_boolean DJDIJG8fillInputBuffer(j_decompress_ptr cinfo) {
  ...
  if (src->next_buffer) {
    ...
    // The suspension was caused by DJDIJG8skipInputData iff src->skip_bytes >
  ...
  if (cinfo) {
    ...
    if (jerr) {
      ...
      if (setjmp(OFconst_cast(DJDIJG8ErrorStruct*, jerr)->setjmp_buffer)) {
        ...
        cleanup();
        return makeOFCondition(OFM_dcmjpeg, EJCode_IJG8_Decompression, OF_error,
                               buffer);
      }
    } else {
      delete cinfo;
...
OFCondition DJDecompressIJG8Bit::decode(Uint8* compressedFrameBuffer,
