...
public:
   ...
   *  @param compressedFrameBuffer pointer to compressed input data, must not be
   * NULL
   *  @param compressedFrameBufferSize size of buffer, in bytes
   *  @param uncompressedFrameBuffer pointer to uncompressed output data, must
   * not be NULL. This buffer must not change between multiple decode() calls
   * for a single frame.
   *  @param uncompressedFrameBufferSize size of buffer, in bytes (!)
   *     Buffer must be large enough to contain a complete frame.
   *  @param isSigned OFTrue, if uncompressed pixel data is signed, OFFalse
   * otherwise
   *  @return EC_Normal if successful, EC_Suspend if more data is needed, an
