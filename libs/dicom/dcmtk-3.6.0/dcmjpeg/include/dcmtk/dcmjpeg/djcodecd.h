...
private:
   ...
   *  @param isYBR flag indicating whether DICOM photometric interpretation is
   * YCbCr
  ...
  virtual DJDecoder* createDecoderInstance(
      ...
      Uint8 bitsPerSample, OFBool isYBR) const = 0;
   ...
   */
  static Uint8 scanJpegDataForBitDepth(const Uint8* data,
   ...
   *  @param data pointer to array, must not be NULL, must at least 2 bytes
   * large
   ...
   *  @return EC_Normal if successful, an error code otherwise
   */
  static OFCondition createPlanarConfigurationByte(Uint8* imageFrame,
   ...
   *  @return EC_Normal if successful, an error code otherwise
   */
  static OFCondition createPlanarConfigurationWord(Uint16* imageFrame,
                                                   Uint16 columns, Uint16 rows);
