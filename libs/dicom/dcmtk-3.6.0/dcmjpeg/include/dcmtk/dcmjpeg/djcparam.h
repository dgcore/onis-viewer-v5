...
public:
   ...
   *  @param predictor6WorkaroundEnable enable workaround for buggy lossless
   * compressed images with overflow in predictor 6 for images with 16
   * bits/pixel
   *  @param pOptimizeHuffman perform huffman table optimization for 8
   * bits/pixel compression?
   *  @param pSmoothingFactor smoothing factor for image compression, 0..100
   *  @param pForcedBitDepth forced bit depth for image compression, 0 (auto) or
   * 8/12/16
   *  @param pFragmentSize maximum fragment size (in kbytes) for compression, 0
   * for unlimited.
   ...
   *  @param pWriteYBR422 flag indicating whether a compressed YBR color stream
   * should be marked as YBR_FULL or YBR_FULL_422 on DICOM level
   ...
   * used in modes 1, 2, 4, 6
   *  @param pVoiCenter VOI window center for mode 5
   *  @param pVoiWidth  VOI window width for mode 5
   *  @param pRoiLeft   Region of Interest left corner for for VOI transform of
   * monochrome images, mode 7
   *  @param pRoiTop    Region of Interest upper corner for for VOI transform of
   * monochrome images, mode 7
   *  @param pRoiWidth  Region of Interest width for for VOI transform of
   * monochrome images, mode 7
   *  @param pRoiHeight Region of Interest height for for VOI transform of
   * monochrome images, mode 7
   *  @param pUsePixelValues check smallest and largest pixel value and optimize
   * compression, mode 0 only
   *  @param pUseModalityRescale create Rescale Slope/Intercept to scale back to
   * original pixel range, mode 0 only
   *  @param pAcceptWrongPaletteTags accept wrong palette attribute tags (only
   * "pseudo lossless" encoder)
   *  @param pAcrNemaCompatibility accept old ACR-NEMA images without
   * photometric interpretation (only "pseudo" lossless encoder)
   *  @param pTrueLosslessMode Enables true lossless compression (replaces old
   * "pseudo lossless" encoder)
   */
  DJCodecParameter(
      ...
      OFBool pOptimizeHuffman = OFFalse, int pSmoothingFactor = 0,
      int pForcedBitDepth = 0, Uint32 pFragmentSize = 0,
      OFBool pCreateOffsetTable = OFTrue,
      E_SubSampling pSampleFactors = ESS_444, OFBool pWriteYBR422 = OFFalse,
      OFBool pConvertToSC = OFFalse, unsigned long pWindowType = 0,
      unsigned long pWindowParameter = 0, double pVoiCenter = 0.0,
      double pVoiWidth = 0.0, unsigned long pRoiLeft = 0,
      unsigned long pRoiTop = 0, unsigned long pRoiWidth = 0,
      unsigned long pRoiHeight = 0, OFBool pUsePixelValues = OFTrue,
  ...
  /** returns flag indicating whether a compressed YBR color stream should be
   * marked as YBR_FULL or YBR_FULL_422 on DICOM level
   *  @return YBR422 flag
   */
  ...
  /** returns parameter for VOI transform of monochrome images, used in VOI
   * modes 1, 2, 4, 6
   *  @return parameter for VOI transform of monochrome images, used in VOI
   * modes 1, 2, 4, 6
   */
   ...
   */
  void getROI(unsigned long& left_pos, unsigned long& top_pos,
   ...
   *  @return color conversion mode for compression
   */
  E_DecompressionColorSpaceConversion getDecompressionColorSpaceConversion()
      const {
    return decompressionCSConversion;
  }

  /** returns flag indicating if compression to be optimized for real pixel
   * values
   *  @return flag indicating if compression to be optimized for real pixel
   * values
   */
  ...
  /** returns flag indicating if compression should compute Rescale
   * Slope/Intercept
   *  @return flag indicating if compression should compute Rescale
   * Slope/Intercept
   */
  ...
  /** Returns flag indicating if incorrect palette attribute tags should be
   * accepted
   *  @return flag indicating if incorrect palette attribute tags should be
   * accepted
   */
  ...
  /** returns flag indicating whether the workaround for buggy JPEG lossless
   * images with incorrect predictor 6 is enabled
   *  @return flag indicating whether the workaround for buggy JPEG lossless
   * images with incorrect predictor 6 is enabled
   */
...
private:
  ...
  /** mode for VOI transformation of monochrome images. possible values:
      ...
      2: use the n-th VOI look up table from the image file (see variable
     windowType) 3: compute VOI window using min-max algorithm 4: compute VOI
     window using Histogram algorithm, ignoring n percent 5: compute VOI window
     using center r and width s 6: compute VOI window using min-max algorithm
     ignoring extremes 7: compute region of interest VOI window
   */
  ...
  /// Region of Interest left corner for for VOI transform of monochrome images,
  /// mode 7
  unsigned long roiLeft;

  /// Region of Interest upper corner for for VOI transform of monochrome
  /// images, mode 7
  unsigned long roiTop;

  /// Region of Interest width for for VOI transform of monochrome images, mode
  /// 7
  unsigned long roiWidth;

  /// Region of Interest height for for VOI transform of monochrome images, mode
  /// 7
  unsigned long roiHeight;

  /// Check smallest and largest pixel value and optimize compression, mode 0
  /// only
  OFBool usePixelValues;

  /// Create Rescale Slope/Intercept to scale back to original pixel range, mode
  /// 0 only
  ...
  /// True losless mode, replaces old "pseudo" lossless encoders, when true
  /// (default)
  OFBool trueLosslessMode;

  /// flag indicating that the workaround for buggy JPEG lossless images with
  /// incorrect predictor 6 is enabled
 ...
 *   a faulty lossless JPEG encoder that produces integer overflows in
 * predictor 6.
 *
