...
int DiPNGPlugin::write(DiImage* image, FILE* stream,
  ...
  if ((image != NULL) && (stream != NULL)) {
    ...
    if (data != NULL) {
      ...
      for (row = 0,
          pix_ptr = OFstatic_cast(png_byte*, OFconst_cast(void*, data));
           row < height; row++, pix_ptr += width * bpp) {
        row_ptr[row] = pix_ptr;
      }
...
OFString DiPNGPlugin::getLibraryVersionString() {
  ...
  if (ver < 999999) {
    sprintf(cver, "%li.%li.%li", OFstatic_cast(long int, (ver / 10000) % 100),
