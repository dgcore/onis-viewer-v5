...
DiColorPixel::DiColorPixel(const DiDocument* docu, const DiInputPixel* pixel,
                           const Uint16 samples, EI_Status& status,
                           const Uint16 sample_rate)
    : DiPixel(0), PlanarConfiguration(0) {
  if (docu != NULL) {
    Uint16 us = 0;
    if (docu->getValue(DCM_SamplesPerPixel, us)) {
      if (us != samples) {
        DCMIMAGE_WARN("invalid value for 'SamplesPerPixel' ("
                      << us << ") ... assuming " << samples);
      }
      if (docu->getValue(DCM_PlanarConfiguration, us)) {
        ...
        if (samples > 1) {
          ...
          if ((us != 0) && (us != 1)) {
            DCMIMAGE_WARN("invalid value for 'PlanarConfiguration' ("
                          << us << ") ... assuming 'color-by-pixel' (0)");
          }
        } else {
          DCMIMAGE_WARN("unexpected attribute 'PlanarConfiguration' ("
                        << us << ") ... ignoring");
        }
      } else if (samples > 1) {
      ...
      if (pixel != NULL) {
        // number of pixels (per plane) computed from the length of the
        // PixelData attribute
        InputCount = pixel->getPixelCount() /
                     ((sample_rate == 0) ? samples : sample_rate);
        // number of pixels allocated for the intermediate buffer
        Count = pixel->getComputedCount() /
                ((sample_rate == 0) ? samples : sample_rate);
      }
    } else {
      status = EIS_MissingAttribute;
