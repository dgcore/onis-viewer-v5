...
int DiTIFFPlugin::write(DiImage* image, FILE* stream,
...
#error TIFF library versions prior to 3.7.4 are not supported by DCMTK on Win32 - critical API change!
    ...
    /* Older versions of libtiff expected a Win32 HANDLE when compiled on
     * Windows instead of a file descriptor. The code below was needed to make
     * that work. Libtiff version 3.7.4 and newer are known to use a file
     * descriptor instead, but it is not completely clear at which libtiff
     * release the API change happened.
     *
     * #ifdef __CYGWIN__
...
#error TIFF library versions prior to 3.7.0 are not supported by DCMTK - TIFFCleanup is missing!
    ...
    void* data = OFconst_cast(
        void*, image->getOutputData(frame, 8 /*bits*/, 0 /*planar*/));
    if (data != NULL) {
      OFBool isMono = (image->getInternalColorModel() == EPI_Monochrome1) ||
      ...
      if (bytesperrow > 0) {
        ...
        if (opt_rowsperstrip <= 0)
          opt_rowsperstrip = 8192 / bytesperrow;
        if (opt_rowsperstrip == 0)
          opt_rowsperstrip++;
        ...
        if (tif) {
          ...
          /* TIFFSetField(tif, TIFFTAG_STRIPBYTECOUNTS, rows /
           * opt_rowsperstrip); */
