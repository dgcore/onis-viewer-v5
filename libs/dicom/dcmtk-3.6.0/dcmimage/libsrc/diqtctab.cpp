...
static int redcompare(const void* x1, const void* x2) {
  return OFstatic_cast(int,
                       (*OFstatic_cast(const DcmQuantHistogramItemPointer*, x1))
                           ->getRed()) -
         OFstatic_cast(int,
                       (*OFstatic_cast(const DcmQuantHistogramItemPointer*, x2))
                           ->getRed());
}
...
DcmQuantColorTable::DcmQuantColorTable()
    : array(NULL), numColors(0), maxval(0) {}
...
OFCondition DcmQuantColorTable::medianCut(DcmQuantColorTable& histogram,
  ...
  for (unsigned int xx = 0; xx < numberOfColors; xx++)
    array[xx] = new DcmQuantHistogramItem();
    ...
    */
    if (largeType == DcmLargestDimensionType_default) {
      if (maxr - minr >= maxg - ming && maxr - minr >= maxb - minb)
        qsort(OFreinterpret_cast(char*, &(histogram.array[indx])), clrs,
              sizeof(DcmQuantHistogramItemPointer), redcompare);
      else if (maxg - ming >= maxb - minb)
        qsort(OFreinterpret_cast(char*, &(histogram.array[indx])), clrs,
              sizeof(DcmQuantHistogramItemPointer), greencompare);
      else
        qsort(OFreinterpret_cast(char*, &(histogram.array[indx])), clrs,
              sizeof(DcmQuantHistogramItemPointer), bluecompare);
  ...
  */
  if (repType == DcmRepresentativeColorType_centerOfBox) {
    for (bi = 0; bi < boxes; ++bi) {
      ...
      array[bi]->assign((minr + maxr) / 2, (ming + maxg) / 2,
                        (minb + maxb) / 2);
    }
    ...
    for (bi = 0; bi < boxes; ++bi) {
      ...
      g = g / clrs;
      b = b / clrs;
      array[bi]->assign(OFstatic_cast(DcmQuantComponent, r),
                        OFstatic_cast(DcmQuantComponent, g),
                        OFstatic_cast(DcmQuantComponent, b));
    }
  } else  // DcmRepresentativeColorType_averagePixels
  {
    for (bi = 0; bi < boxes; ++bi) {
      ...
      for (i = 0; i < clrs; ++i) {
        r += histogram.array[indx + i]->getRed() *
             histogram.array[indx + i]->getValue();
        g += histogram.array[indx + i]->getGreen() *
             histogram.array[indx + i]->getValue();
        b += histogram.array[indx + i]->getBlue() *
             histogram.array[indx + i]->getValue();
...
OFCondition DcmQuantColorTable::write(DcmItem& target, OFBool writeAsOW,
  ...
  if (array) {
    ...
    descriptor[0] =
        (numColors > 65535)
            ? 0
            : OFstatic_cast(Uint16, numColors);  // number of entries
    descriptor[1] = 0;                           // first pixel value mapped
    descriptor[2] =
        write16BitEntries ? 16 : 8;  // bits per entry, must be 8 or 16.

    // if we're writing a 16-bit LUT with 64K entries, we must write as OW
    // because otherwise the US length field will overflow when writing in
    // explicit VR!
    ...
    if (result.good()) {
      ...
      if (write16BitEntries) {
        ...
        if (rLUT && gLUT && bLUT) {
          for (unsigned long i = 0; i < numColors; i++) {
            rLUT[i] = OFstatic_cast(
                Uint16, OFstatic_cast(double, array[i]->getRed()) * factor);
            gLUT[i] = OFstatic_cast(
                Uint16, OFstatic_cast(double, array[i]->getGreen()) * factor);
            bLUT[i] = OFstatic_cast(
                Uint16, OFstatic_cast(double, array[i]->getBlue()) * factor);

            // if the source data is only 8 bits per entry, replicate high and
            // low bytes
            if (sizeof(DcmQuantComponent) == 1) {
              rLUT[i] |= rLUT[i] << 8;
              gLUT[i] |= gLUT[i] << 8;
        ...
        // number of Uint16 words needed to store numColors Uint8 values plus
        // padding
        ...
        if (rLUT && gLUT && bLUT) {
          ...
          for (unsigned long i = 0; i < numColors; i++) {
            rLUT8[i] = OFstatic_cast(
                Uint8, OFstatic_cast(double, array[i]->getRed()) * factor);
            gLUT8[i] = OFstatic_cast(
                Uint8, OFstatic_cast(double, array[i]->getGreen()) * factor);
            bLUT8[i] = OFstatic_cast(
                Uint8, OFstatic_cast(double, array[i]->getBlue()) * factor);
          }
          // we have written the byte array in little endian order, now swap to
          // US/OW if neccessary
          swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, rLUT,
                          numWords * sizeof(Uint16), sizeof(Uint16));
          swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, gLUT,
                          numWords * sizeof(Uint16), sizeof(Uint16));
          swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, bLUT,
                          numWords * sizeof(Uint16), sizeof(Uint16));
        } else
          result = EC_MemoryExhausted;
      }
      ...
      if (result.good()) {
        if (writeAsOW)
          elem = new DcmOtherByteOtherWord(
              DcmTag(DCM_RedPaletteColorLookupTableData, EVR_OW));
        else
          elem = new DcmUnsignedShort(
              DcmTag(DCM_RedPaletteColorLookupTableData, EVR_US));
        if (elem) {
      ...
      if (result.good()) {
        if (writeAsOW)
          elem = new DcmOtherByteOtherWord(
              DcmTag(DCM_GreenPaletteColorLookupTableData, EVR_OW));
        else
          elem = new DcmUnsignedShort(
              DcmTag(DCM_GreenPaletteColorLookupTableData, EVR_US));
        if (elem) {
      ...
      if (result.good()) {
        if (writeAsOW)
          elem = new DcmOtherByteOtherWord(
              DcmTag(DCM_BluePaletteColorLookupTableData, EVR_OW));
        else
          elem = new DcmUnsignedShort(
              DcmTag(DCM_BluePaletteColorLookupTableData, EVR_US));
        if (elem) {
...
void DcmQuantColorTable::setDescriptionString(OFString& str) const {
  ...
  sprintf(buf, "Converted to PALETTE COLOR %lu/0/%u with DCMTK %s",
          (numColors > 65535) ? 0 : numColors,
          (sizeof(DcmQuantComponent) == 1) ? 8 : 16, OFFIS_DCMTK_VERSION);
