...
public:
   ...
   *  @param  rgb     specifies whether internal image representation is in RGB
   *format
   */
  DiColorImage(const DiDocument* docu, const EI_Status status, const int spp,
   ...
   *
   ** @return returns EPI_RGB or EPI_YBR_Full depending on the 'rgb' flag in the
   *constructor
   */
   ...
   *  The standard color model of the pixel data is RGB, but if the flag "rgb"
   *is OFFalse and the original color model was YCbCr_Full or YCbCr_Full_422
   *YCbCr_Full is used instead.
   *
   ** @param  frame   number of frame to be rendered
   *  @param  bits    number of bits per sample for the output pixel data
   *(depth)
   ...
   */
  const void* getOutputData(const unsigned long frame, const int bits,
                            const int planar = 0);
   ...
   */
  int getOutputData(void* buffer, const unsigned long size,
   ...
   */
  DiImage* createScale(const signed long left_pos, const signed long top_pos,
                       const unsigned long src_cols,
                       const unsigned long src_rows,
                       const unsigned long dest_cols,
                       const unsigned long dest_rows, const int interpolate,
                       const int aspect, const Uint16 pvalue) const;
   ...
   ** @return pointer to new DiImage object (NULL if an error occurred)
   */
  DiImage* createMono(const double red, const double green,
   ...
   *  @param  bits    number of bits used for output of pixel data (24 or 32,
   *default (0) = 24)
   *
   ** @return true if successful, false otherwise
   */
...
protected:
   ...
   *  @param  fcount  number of frames
   */
  DiColorImage(const DiColorImage* image, const unsigned long fstart,
   ...
   *component will be calculated automatically)
   */
  DiColorImage(const DiColorImage* image, const signed long left_pos,
               const signed long top_pos, const Uint16 src_cols,
               const Uint16 src_rows, const Uint16 dest_cols,
               const Uint16 dest_rows, const int interpolate = 0,
               const int aspect = 0);
   ...
   *  Removes color palette lookup tables.  Used in writeXXXToDataset()
   *routines.
   *
  ...
  /// flag, indicating whether the intermediate representation uses the RGB
  /// color model
