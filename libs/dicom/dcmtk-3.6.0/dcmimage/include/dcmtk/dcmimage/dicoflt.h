 ...
 */
template <class T>
class DiColorFlipTemplate : public DiColorPixelTemplate<T>,
...
public:
   ...
   */
  DiColorFlipTemplate(const DiColorPixel* pixel, const Uint16 columns,
                      const Uint16 rows, const Uint32 frames, const int horz,
                      const int vert)
      : DiColorPixelTemplate<T>(pixel, OFstatic_cast(unsigned long, columns) *
    ...
    if ((pixel != NULL) && (pixel->getCount() > 0)) {
      if (pixel->getCount() == OFstatic_cast(unsigned long, columns) *
        ...
        flip(OFstatic_cast(const T**, OFconst_cast(void*, pixel->getData())),
             horz, vert);
      else {
...
private:
   ...
   *
   ** @param  pixel  pointer to pixel data (3 components) which should be
   *flipped
   *  @param  horz   flip horizontally if true
   *  @param  vert   flip vertically if true
   */
