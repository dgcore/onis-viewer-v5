...
public:
  ...
  /** uses the Floyd-Steinberg error vectors to adjust the color of the current
   * image pixel.
   *  @param px the original image pixel is passed in this parameter. Upon
   * return, the pixel value contains the new value after error diffusion.
   *  @param col column in which the current pixel is located, must be
   * [0..columns-1]
  ...
  inline void adjust(DcmQuantPixel& px, long col, long maxval) {
    ...
    if (sr < 0)
      sr = 0;
    else if (sr > OFstatic_cast(long, maxval))
      sr = maxval;
    if (sg < 0)
      sg = 0;
    else if (sg > OFstatic_cast(long, maxval))
      sg = maxval;
    if (sb < 0)
      sb = 0;
    else if (sb > OFstatic_cast(long, maxval))
      sb = maxval;
    px.assign(OFstatic_cast(DcmQuantComponent, sr),
              OFstatic_cast(DcmQuantComponent, sg),
              OFstatic_cast(DcmQuantComponent, sb));
  }

  /** propagates the Floyd-Steinberg error terms for one pixel.
   *  @param px color value the current image pixel should have (after
   * adjustment)
   *  @param mapped color value (selected from the color LUT) the current image
   * pixel really uses
   *  @param col column in which the current pixel is located, must be
   * [0..columns-1]
   */
  inline void propagate(const DcmQuantPixel& px, const DcmQuantPixel& mapped,
                        long col) {
    long err;

    /* Propagate Floyd-Steinberg error terms. */
    if (fs_direction) {
      err = (OFstatic_cast(long, px.getRed()) -
             OFstatic_cast(long, mapped.getRed())) *
            DcmQuantFloydSteinbergScale;
      thisrerr[col + 2] += (err * 7) / 16;
      nextrerr[col] += (err * 3) / 16;
      nextrerr[col + 1] += (err * 5) / 16;
      nextrerr[col + 2] += (err) / 16;
      err = (OFstatic_cast(long, px.getGreen()) -
             OFstatic_cast(long, mapped.getGreen())) *
            DcmQuantFloydSteinbergScale;
      thisgerr[col + 2] += (err * 7) / 16;
      nextgerr[col] += (err * 3) / 16;
      nextgerr[col + 1] += (err * 5) / 16;
      nextgerr[col + 2] += (err) / 16;
      err = (OFstatic_cast(long, px.getBlue()) -
             OFstatic_cast(long, mapped.getBlue())) *
            DcmQuantFloydSteinbergScale;
...
private:
  ...
  /// red error vector for next row. Points to an array of (columns + 2)
  /// entries.
  long* nextrerr;
  ...
  /// green error vector for next row. Points to an array of (columns + 2)
  /// entries.
  long* nextgerr;
  ...
  /// blue error vector for next row. Points to an array of (columns + 2)
  /// entries.
  long* nextberr;
