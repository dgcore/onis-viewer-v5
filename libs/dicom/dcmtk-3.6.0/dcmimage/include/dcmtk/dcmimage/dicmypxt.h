...
public:
   ...
   */
  DiCMYKPixelTemplate(const DiDocument* docu, const DiInputPixel* pixel,
    ...
    if (this->Init(pixel)) {
      ...
      if (this->PlanarConfiguration) {
        /*
                        ...
                        {
                            q = this->Data[j];
                            k = pixel + 3 * this->InputCount; // beginning of
           'black' plane for (i = count; i != 0; --i)
                                *(q++) = maxvalue - removeSign(*(p++), offset) -
           removeSign(*(k++), offset);
                        }
        */
        const T1* k;
        ...
        for (i = 0; i < count; ++i) {
          ...
          for (j = 0; j < 3; ++j)
            this->Data[j][i] =
                maxvalue - removeSign(*(p++), offset) - removeSign(k, offset);
          ++p;  // skip 'black'
        }
      }
    }
