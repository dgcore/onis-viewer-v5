...
inline Uint8 removeSign(const Sint8 value, const Sint8 offset) {
  return OFstatic_cast(
      Uint8, OFstatic_cast(Sint16, value) + OFstatic_cast(Sint16, offset) + 1);
}

inline Uint16 removeSign(const Sint16 value, const Sint16 offset) {
  return OFstatic_cast(
      Uint16, OFstatic_cast(Sint32, value) + OFstatic_cast(Sint32, offset) + 1);
}

/*
inline Uint32 removeSign(const Sint32 value, const Sint32 offset)
{
    return (value < 0) ? OFstatic_cast(Uint32, value + offset + 1) :
OFstatic_cast(Uint32, value) + OFstatic_cast(Uint32, offset) + 1;
}
 ...
 */
template <class T>
class DiColorPixelTemplate : public DiColorPixel,
...
public:
   ...
   *  @param  samples      number of expected samples per pixel (for checking
   *purposes)
   ...
   */
  DiColorPixelTemplate(const DiDocument* docu, const DiInputPixel* pixel,
                       const Uint16 samples, EI_Status& status,
   ...
   *
   ** @param  data    pointer to memory block (array of 8 or 16 bit values,
   *OB/OW)
   ...
   *representation
   *  @param  planar  flag indicating whether data shall be stored
   *color-by-pixel or color-by-plane
   *
   ** @return OFTrue if successful, OFFalse otherwise
   */
  OFBool getPixelData(void* data, const unsigned long count,
                      const unsigned long fcount, const unsigned long frames,
                      const int planar) const {
    OFBool result = OFFalse;
    /* check parameters and internal data */
    if ((data != NULL) && (count >= Count * 3) && (frames > 0) &&
        (frames * fcount <= Count) && (Data[0] != NULL) && (Data[1] != NULL) &&
        (Data[2] != NULL)) {
   ...
   *  @param  fromBits    number of bits per sample used for internal
   *representation of the image
   *  @param  toBits      number of bits per sample used for the output bitmap
   *(<= 8)
   *  @param  mode        color output mode (24 or 32 bits, see
   *dcmimgle/dcmimage.h for details)
  ...
  unsigned long createDIB(void*& data, const unsigned long size,
                          ...
                          const unsigned long frame, const int fromBits,
                          const int toBits, const int mode,
                          const int upsideDown, const int padding) const {
    ...
    if ((Data[0] != NULL) && (Data[1] != NULL) && (Data[2] != NULL) &&
      ...
      const unsigned long start =
          count * frame + ((upsideDown)
                               ? OFstatic_cast(unsigned long, height - 1) *
                                     OFstatic_cast(unsigned long, width)
                               : 0);
      const signed long nextRow =
          (upsideDown) ? -2 * OFstatic_cast(signed long, width) : 0;
      ...
      if (mode == 24)  // 24 bits per pixel
      {
        ...
        const int gap =
            (padding) ? OFstatic_cast(int, (4 - (wid3 & 0x3)) & 0x3) : 0;
        unsigned long fsize =
            (wid3 + gap) * OFstatic_cast(unsigned long, height);
                  ...
                  OFstatic_cast(double, gradient2))  // integer multiplication?
              {
                ...
                for (y = height; y != 0; y--) {
                  for (x = width; x != 0; x--) {
                    /* reverse sample order: B-G-R */
                    *(q++) = OFstatic_cast(
                        Uint8, OFstatic_cast(double, *(b++)) * gradient1);
                    *(q++) = OFstatic_cast(
                        Uint8, OFstatic_cast(double, *(g++)) * gradient1);
                    *(q++) = OFstatic_cast(
                        Uint8, OFstatic_cast(double, *(r++)) * gradient1);
                  }
                  r += nextRow;
                  g += nextRow;
                  b += nextRow;  // go backwards if 'upsideDown'
                  ...
                  OFstatic_cast(double, gradient2))  // integer multiplication?
              {
                ...
                for (y = height; y != 0; y--) {
                  for (x = width; x != 0; x--) {
                    ...
                    *(q++) =
                        (OFstatic_cast(
                             Uint32, OFstatic_cast(double, *(r++)) * gradient1)
                         << 16) |
                        (OFstatic_cast(
                             Uint32, OFstatic_cast(double, *(g++)) * gradient1)
                         << 8) |
                        OFstatic_cast(
                            Uint32, OFstatic_cast(double, *(b++)) * gradient1);
                  }
                  r += nextRow;
                  g += nextRow;
                  b += nextRow;  // go backwards if 'upsideDown'
   ...
   *
   ** @param  data      resulting pointer to bitmap data (set to NULL if an
   *error occurred)
   *  @param  width     number of columns of the image
   *  @param  height    number of rows of the image
   *  @param  frame     index of frame to be converted (starting from 0)
   ...
   ** @return number of bytes allocated by the bitmap, or 0 if an error occured
   */
  unsigned long createAWTBitmap(void*& data, const Uint16 width,
                                const Uint16 height, const unsigned long frame,
      ...
      if (data != NULL) {
        ...
        if (fromBits == toBits) {
              ...
              OFstatic_cast(double, gradient2))  // integer multiplication?
          {
            ...
            for (i = count; i != 0; --i) {
              /* sample order: R-G-B */
              *(q++) = (OFstatic_cast(Uint32,
                                      OFstatic_cast(double, *(r++)) * gradient1)
                        << 24) |
                       (OFstatic_cast(Uint32,
                                      OFstatic_cast(double, *(g++)) * gradient1)
                        << 16) |
                       (OFstatic_cast(Uint32,
                                      OFstatic_cast(double, *(b++)) * gradient1)
                        << 8);
            }
          }
        } else /* fromBits > toBits */
