 ...
 */
template <class T>
class DiColorRotateTemplate : public DiColorPixelTemplate<T>,
...
public:
   ...
   */
  DiColorRotateTemplate(const DiColorPixel* pixel, const Uint16 src_cols,
      ...
      : DiColorPixelTemplate<T>(
        ...
        DiRotateTemplate<T>(3, src_cols, src_rows, dest_cols, dest_rows,
                            frames) {
    if ((pixel != NULL) && (pixel->getCount() > 0)) {
      if (pixel->getCount() == OFstatic_cast(unsigned long, src_cols) *
                                   OFstatic_cast(unsigned long, src_rows) *
                                   frames)
        rotate(OFstatic_cast(const T**, OFconst_cast(void*, pixel->getData())),
               degree);
      else {
...
private:
   ...
   *  @param  degree  angle by which the pixel data should be rotated
   */
  inline void rotate(const T* pixel[3], const int degree) {
    if (this->Init(pixel)) {
      if (degree == 90)
