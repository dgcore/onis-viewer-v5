...
public:
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition medianCut(DcmQuantColorTable& histogram, unsigned long sum,
                        unsigned long theMaxval, unsigned long numberOfColors,
  ...
  inline int computeIndex(const DcmQuantPixel& px) const {
    ...
    for (unsigned long i = 0; i < numColors; ++i) {
      ...
      if (newdist < dist) {
        ...
        dist = newdist;
        if (dist < array[i]->getValue())
          i = numColors;  // break out of for loop
      }
    }
    return result;
  }
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition write(DcmItem& target, OFBool writeAsOW,
