...
public:
   ...
   *  @param maxcolors maximum number of colors allowed.  If more colors are
   * found, the method immediately returns with a return value of zero.
   *  @return number of colors found, 0 if too many colors.
   */
  unsigned long addToHashTable(DicomImage& image, unsigned long newmaxval,
