...
class DiARGBPixelTemplate : public DiColorPixelTemplate<T3> {
public:
  /** constructor
   *
   ...
   */
  DiARGBPixelTemplate(const DiDocument* docu, const DiInputPixel* pixel,
                      DiLookupTable* palette[3], EI_Status& status,
                      const unsigned long planeSize, const int bits)
...
private:
  ...
  void convert(const T1* pixel, DiLookupTable* palette[3],
               ...
               const int bits) {  // not very much optimized, but no one really
                                  // uses ARGB !!
    if (this->Init(pixel)) {
      T2 value;
      ...
      if (this->PlanarConfiguration) {
        /*
                        const T1 *a = pixel; // points to alpha plane const T1
           *rgb[3]; rgb[0] = a + this->InputCount; // points to red plane rgb[1]
           = rgb[0] + this->InputCount;                                   //
           points to green plane rgb[2] = rgb[1] + this->InputCount; // points
           to blue plane for (i = 0; i < count; ++i)
                        {
                            value = OFstatic_cast(T2, *(a++)); // get alpha
           value if (value > 0)
                            {
                                for (int j = 0; j < 3; ++j) // set palette color
                                {
                                    if (value <=
           palette[j]->getFirstEntry(value)) this->Data[j][i] =
           OFstatic_cast(T3, palette[j]->getFirstValue()); else if (value >=
           palette[j]->getLastEntry(value)) this->Data[j][i] = OFstatic_cast(T3,
           palette[j]->getLastValue()); else this->Data[j][i] =
           OFstatic_cast(T3, palette[j]->getValue(value));
            ...
            if (value > 0) {
              for (int j = 0; j < 3; ++j)  // set palette color
              {
                if (value <= palette[j]->getFirstEntry(value))
                  this->Data[j][i] =
                      OFstatic_cast(T3, palette[j]->getFirstValue());
                else if (value >= palette[j]->getLastEntry(value))
                  this->Data[j][i] =
                      OFstatic_cast(T3, palette[j]->getLastValue());
                else
                  this->Data[j][i] =
                      OFstatic_cast(T3, palette[j]->getValue(value));
                ++rgb[j];  // skip RGB values
              ...
              for (int j = 0; j < 3; ++j)  // copy RGB values
                this->Data[j][i] =
                    OFstatic_cast(T3, removeSign(*(rgb[j]++), offset));
            }
          }
