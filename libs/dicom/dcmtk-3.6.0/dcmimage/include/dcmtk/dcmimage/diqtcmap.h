...
public:
   ...
   *    Initially (i.e. when called for the first frame) the hash table is
   * empty.
   ...
   *    or class DcmQuantFloydSteinberg, depending on the template
   * instantiation.
   *  @param tp pointer to an array to which the palette color image data
   *    is written.  The array must be large enough to store
   * sourceImage.getWidth() times sourceImage.getHeight() values of type T2.
   */
  static void create(DicomImage& sourceImage, unsigned long frameNumber,
                     unsigned long maxval, DcmQuantColorHashTable& cht,
                     DcmQuantColorTable& colormap, T1& fs, T2* tp) {
    ...
    if (data) {
      const DcmQuantComponent* cp =
          OFstatic_cast(const DcmQuantComponent*, data);
