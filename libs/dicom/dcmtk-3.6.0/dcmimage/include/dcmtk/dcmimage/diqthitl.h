...
public:
   ...
   *  The list becomes empty if the array is large enough to contain all list
   * members.
   *  @param array array of pointers to DcmQuantHistogramItem
   *  @param counter When called, contains the index of the array element
   *    into which the first member of the list will be moved.  Must be <
   * numcolors. Upon return, contains the array index of the last element moved
   * + 1.
   *  @param numcolors number of elements in array
   */
  void moveto(DcmQuantHistogramItemPointer* array, unsigned long& counter,
              unsigned long numcolors);

  /** searches the list for an entry that equals the given pixel value.
   *  If found, the integer value assigned to that pixel is returned, otherwise
   * returns -1.
...
private:
  ...
  /// temporary iterator used in various methods; declared here for efficiency
  /// reasons only.
  OFListIterator(DcmQuantHistogramItem*) first;

  /// constant iterator which always contains list_.end(); declared here for
