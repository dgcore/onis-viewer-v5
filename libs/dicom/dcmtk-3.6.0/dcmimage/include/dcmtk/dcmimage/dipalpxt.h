...
class DiPalettePixelTemplate : public DiColorPixelTemplate<T3> {
public:
  /** constructor
   *
   ...
   */
  DiPalettePixelTemplate(const DiDocument* docu, const DiInputPixel* pixel,
...
private:
  ...
  void convert(const T1* pixel,
               DiLookupTable* palette[3]) {  // can be optimized if necessary !
    if (this->Init(pixel)) {
      const T1* p = pixel;
      T2 value = 0;
      unsigned long i;
