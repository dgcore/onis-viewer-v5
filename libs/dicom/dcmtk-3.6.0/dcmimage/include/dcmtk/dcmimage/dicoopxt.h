...
class DiColorOutputPixelTemplate : public DiColorOutputPixel,
...
public:
   ...
   */
  DiColorOutputPixelTemplate(void* buffer, const DiColorPixel* pixel,
                             const unsigned long count,
                             const unsigned long frame,
                             const int bits1, /* input depth */
                             const int bits2, /* output depth */
                             const int planar, const int inverse)
    ...
    if ((pixel != NULL) && (Count > 0) && (FrameSize >= Count)) {
      ...
      convert(OFstatic_cast(const T1**, OFconst_cast(void*, pixel->getData())),
              frame * FrameSize, bits1, bits2, planar, inverse);
    }
  }
   ...
   *color-by-pixel or color-by-plane
   */
  DiColorOutputPixelTemplate(void* buffer, const DiPixel* pixel,
                             const unsigned long count,
                             const unsigned long frame,
                             const unsigned long /*frames*/, const int planar)
      : DiColorOutputPixel(pixel, count, frame),
        Data(NULL),
        DeleteData(buffer == NULL),
        isPlanar(planar) {
  ...
  inline const void* getPlane(const int plane) const {
    ...
    if (Data != NULL) {
      if (plane <= 0)
        ...
        if (isPlanar)
          result =
              OFstatic_cast(void*, Data + ((plane == 1) ? 1 : 2) * FrameSize);
        else
  ...
  int writePPM(STD_NAMESPACE ostream& stream) const {
    if (Data != NULL) {
      ...
      for (i = FrameSize; i != 0; --i)
        for (j = 3; j != 0; --j)
          stream << OFstatic_cast(unsigned long, *(p++))
...
private:
   ...
   *  @param  inverse  invert pixel data if true (0/0/0 = white)
   */
  void convert(const T1* pixel[3], const unsigned long start, const int bits1,
               const int bits2, const int planar, const int inverse) {
        ...
        if (planar) {
          ...
          if (bits1 == bits2) {
            for (int j = 0; j < 3; ++j) {
              ...
              if (Count < FrameSize) {
                OFBitmanipTemplate<T2>::zeroMem(
                    q, FrameSize -
                           Count);  // set remaining pixels of frame to zero
                q += (FrameSize - Count);
              }
            }
          } else if (bits1 < bits2)  // optimization possible using LUT
          {
            const double gradient1 =
                OFstatic_cast(double, DicomImageClass::maxval(bits2)) /
                  ...
                  OFstatic_cast(double, gradient2))  // integer multiplication?
              {
                ...
                if (inverse) {
                  for (i = Count; i != 0; --i)  // expand depth & invert
                    *(q++) = max2 -
                             OFstatic_cast(
                                 T2, OFstatic_cast(double, *(p++)) * gradient1);
                } else {
                  for (i = Count; i != 0; --i)  // expand depth
                    *(q++) = OFstatic_cast(
                        T2, OFstatic_cast(double, *(p++)) * gradient1);
              ...
              if (inverse) {
                for (i = start; i < start + Count; ++i)
                  for (j = 0; j < 3; ++j)  // expand depth & invert
                    *(q++) = max2 - OFstatic_cast(
                                        T2, OFstatic_cast(double, pixel[j][i]) *
                                                gradient1);
              } else {
                for (i = start; i < start + Count; ++i)
                  for (j = 0; j < 3; ++j)  // expand depth
                    *(q++) = OFstatic_cast(
                        T2, OFstatic_cast(double, pixel[j][i]) * gradient1);
              }
            }
          } else /* bits1 > bits2 */
          {
  ...
  /// destructor
  int DeleteData;
  /// flag indicating whether pixel data is stored color-by-pixel or
  /// color-by-plane
  int isPlanar;
