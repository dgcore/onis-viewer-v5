...
/** the list of displayed area selections contained in a presentation state
 * (internal use only). This class manages the data structures comprising one
 * complete displayed area selection sequence contained in a presentation state
 * object.
 */
...
public:
  ...
  DVPSDisplayedArea_PList* clone() {
    return new DVPSDisplayedArea_PList(*this);
  }

  /// destructor
  virtual ~DVPSDisplayedArea_PList();

  /** reads a list of displayed area selections (DisplayedAreaSelectionSequence)
   * from a DICOM dataset. The DICOM elements of the displayed area selection
   * item are copied from the dataset to this object. The completeness of all
   * items (presence of all required elements, value multiplicity) is checked.
  ...
  /** writes the list of displayed area selections managed by this object to a
   * DICOM dataset. Copies of the DICOM elements managed by this object are
   * inserted into the DICOM dataset.
   *  @param dset the DICOM dataset to which the DisplayedAreaSelectionSequence
   * is written
   *  @return EC_Normal if successful, an error code otherwise.
   */
  ...
  /** checks if an displayed area selection exists for the given image and
   * frame.
   ...
   */
  DVPSDisplayedArea* findDisplayedArea(const char* instanceUID,
                                       unsigned long frame);

  /** finds or creates a displayed area selection SQ item
   *  with an applicability controlled by the applicability, instanceUID and
   * frame parameters. The displayed area selection sequence is rearranged such
   * that all other referenced images/frames keep their old displayed area
   * settings.
  ...
  DVPSDisplayedArea* createDisplayedArea(
      DVPSReferencedSeries_PList& allReferences, const char* sopclassUID,
      const char* instanceUID, unsigned long frame,
      unsigned long numberOfFrames, DVPSObjectApplicability applicability);
