...
/** an item of the graphic layer sequence in a presentation state (internal use
 * only). This class manages the data structures comprising one item of the
...
public:
  ...
  DVPSGraphicLayer* clone() {
    return new DVPSGraphicLayer(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the GraphicLayerSequence from which the data is to
   * be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the GraphicLayerSequence to which the data is
   * written
   *  @return EC_Normal if successful, an error code otherwise.
   */
  ...
  /** checks whether a recommended display value (grayscale, color or both)
   * exists.
   ...
   *  @param gl a pointer to the graphic layer name, which is copied into this
   * object.
   */
   ...
   *  This method does not affect (set or modify) the recommended RGB display
   * value which should be set separately.
   ...
   *  This method does not affect (set or modify) the recommended grayscale
   * display value which should be set separately.
   *  @param r the red component of the recommended display value in P-values
   * 0..0xffff.
   *  @param g the green component of the recommended display value in P-values
   * 0..0xffff.
   *  @param b the blue component of the recommended display value in P-values
   * 0..0xffff.
   */
  void setGLRecommendedDisplayValueRGB(Uint16 r, Uint16 g, Uint16 b);

  /** set graphic layer description of this layer.
   *  @param glDescription a pointer to the graphic layer description, which is
