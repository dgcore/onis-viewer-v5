 ...
 *  This class manages the data structures comprising a Presentation State
 * object. Presentation states can be created, read, written, and modified.
 */
class DVPresentationState : public DcmPresentationState {
public:
  /** default constructor
   *  @param dispFunction list of objects describing the display
   *    characteristics of the monitor. Used to implement the standard display
   * function. The parameter should be an array with DVPSD_max entries (see
   * DVInterface). If absent, no display transform is performed.
   ...
   *  If this method returns an error code, the object is in undefined state
   * afterwards.
   ...
   *  @param voiActivation flag defining how VOI LUTs or VOI window width/center
   * should be handled. Default: Use VOI and prefer VOI LUT from VOI window.
   *  @param curveActivation flag defining whether curves in the image should
   *    be activated. Default: Activate curves.
   *  @param shutterActivation flag defining whether a shutter (not bitmap
   * shutter) should be copied to the presentation state when found in the
   * image. Default: Copy shutter.
   *  @param presentationActivation flag defining whether a presentation LUT
   * shape should be copied to the presentation state when found in the image.
   *    Default: Copy presentation LUT shape.
   *  @param layering flag defining how graphic layers should be created for
   *    activated overlays and curves. Default: Create one layer for all
   * overlays and another layer for all curves.
   *  @param aetitle the series retrieveAETitle. Must be a valid DICOM 'AE'
   * value. Default: value absent.
   *  @param filesetID the series storageMediaFileSetID. Must be a valid DICOM
   * 'SH' value. Default: value absent.
   *  @param filesetUID the series storageMediaFileSetUID. Must be a valid DICOM
   * UID. Default: value absent.
  ...
  OFCondition createFromImage(
      ...
      OFBool curveActivation = OFTrue, OFBool shutterActivation = OFTrue,
      OFBool presentationActivation = OFTrue,
      DVPSGraphicLayering layering = DVPSG_twoLayers,
      const char* aetitle = NULL, const char* filesetID = NULL,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageReferenceAttached(const char* aetitle = NULL,
                                        const char* filesetID = NULL,
                                        const char* filesetUID = NULL);

  /** removes a reference to the currently attached image from this presentation
   * state.
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  contains a lookup table, i.e. if havePresentationLookupTable() returns
   * OFTrue.
   ...
   *  which is DVPSP_identity for MONOCHROME2 images and DVPSP_inverse for
   * MONOCHROME1. DVPSP_table can only be used if the presentation state
   *  contains a lookup table, i.e. if havePresentationLookupTable() returns
   * OFTrue.
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setPresentationLookupTable(DcmUnsignedShort& lutDescriptor,
   ...
   *  The applicability of the activation is controlled by the applicability
   * parameter.
   *  @param idx index of the VOI transform, must be <
   * getNumberOfVOILUTsInImage().
   *  @param applicability defines the applicability of the new VOI transform.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition setVOILUTFromImage(
      size_t idx, DVPSObjectApplicability applicability = DVPSB_currentImage);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition setVOIWindowFromImage(
      size_t idx, DVPSObjectApplicability applicability = DVPSB_currentImage);

  /** sets a user defined VOI window center and width.
   *  The applicability of the VOI window is controlled by the applicability
   * parameter.
  ...
  OFCondition setVOIWindow(
      double wCenter, double wWidth, const char* description = NULL,
   ...
   *  The applicability of the VOI LUT is controlled by the applicability
   * parameter.
   *  @param lutDescriptor the LUT Descriptor in DICOM format (VM=3)
   *  @param lutData the LUT Data in DICOM format
   *  @param lutExplanation the LUT Explanation in DICOM format, may be empty.
  ...
  OFCondition setVOILUT(
      DcmUnsignedShort& lutDescriptor, DcmUnsignedShort& lutData,
      DcmLongString& lutExplanation,
      DVPSObjectApplicability applicability = DVPSB_currentImage);

  /** deactivates the current VOI transformation.
   *  The applicability of the deactivation is controlled by the applicability
   * parameter. After a call to this method, no VOI transform is active for the
   * current image and frame.
   *  @param applicability defines the applicability of the new VOI transform.
   */
  void deactivateVOI(
      DVPSObjectApplicability applicability = DVPSB_currentImage);

  /** stores VOI lookup table with a gamma curve shape in the presentation
   * state. If a VOI window is currently active the center and width values are
   * used to specify the number of LUT entries and the first value mapped,
   * otherwise the full pixel range is used. The output range of the LUT is
   * always 16 bit (data is stored as OW). This method stores a VOI lookup table
   * in the presentation state and activates it. The LUT is copied to the
   * presentation state. If the method returns an error code, an old LUT is left
   * unchanged. The applicability of the VOI LUT is controlled by the
  ...
  /** gets the presentation pixel aspect ratio for for the current image and
   * frame. Pixel aspect ratio is defined here as the width of a pixel divided
   ...
   *  bottom righthand corner for the current potentially rotated and flipped
   * image and frame. This method may only be called when an image is attached
   * to the presentation state.
   *  @param tlhcX the displayed area top lefthand corner X value is returned in
   * this parameter
   *  @param tlhcY the displayed area top lefthand corner Y value is returned in
   * this parameter
   *  @param brhcX the displayed area bottom righthand corner X value is
   * returned in this parameter
   *  @param brhcY the displayed area bottom righthand corner Y value is
   * returned in this parameter
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getStandardDisplayedArea(Sint32& tlhcX, Sint32& tlhcY,
                                       Sint32& brhcX, Sint32& brhcY);

  /** gets the displayed area top lefthand corner and
   *  bottom righthand corner for the current image and frame, as if the image
   * was unrotated This method may only be called when an image is attached to
   * the presentation state.
   *  @param tlhcX the displayed area top lefthand corner X value is returned in
   * this parameter
   *  @param tlhcY the displayed area top lefthand corner Y value is returned in
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getImageRelativeDisplayedArea(Sint32& tlhcX, Sint32& tlhcY,
                                            Sint32& brhcX, Sint32& brhcY);

  /** gets the presentation pixel spacing for the current image and frame if it
   * is known.
   *  @param x the horizontal pixel spacing (mm) is returned in this parameter
   * upon success
   *  @param y the vertical pixel spacing (mm) is returned in this parameter
   * upon success
   *  @return EC_Normal if successful, an error code if no presentation pixel
   * spacing is available.
   */
  OFCondition getDisplayedAreaPresentationPixelSpacing(double& x, double& y);

  /** gets the presentation pixel magnification ratio for the current image and
   * frame if it is present.
   *  @param magnification the magnification ratio is returned in this parameter
   * upon success
   *  @return EC_Normal if successful, an error code if no magnification ratio
   * is available.
   */
  OFCondition getDisplayedAreaPresentationPixelMagnificationRatio(
      double& magnification);

  /** checks if "TRUE SIZE" can be used as presentation size mode for the
   * current image and frame (i.e. pixel spacing is known).
   *  @return OFTrue if TRUE SIZE mode is available, OFFalse otherwise.
   */
  OFBool canUseDisplayedAreaTrueSize();

  /** sets the displayed area and size mode (for the current frame, the current
   * image or all images referenced by the presentation state object).
   ...
   *  @param applicability defines the applicability of the new displayed area
   * definition. Possible choices are: DVPSB_currentFrame - current frame only,
   ...
   *    because it will also cause the pixel spacing or pixel aspect ratio of
   * the current image to be applied to all images referenced by the
   * presentation state.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setStandardDisplayedArea(
      DVPSPresentationSizeMode sizeMode, Sint32 tlhcX, Sint32 tlhcY,
      Sint32 brhcX, Sint32 brhcY, double magnification = 1.0,
      DVPSObjectApplicability applicability = DVPSB_currentImage);

  /** sets the displayed area and size mode (for the current frame, the current
   * image or all images referenced by the presentation state object). Treats
   * the image as if it was neither rotated nor flipped.
   *  @param sizeMode presentation size mode.
   *  @param tlhcX displayed area top lefthand corner X
   *  @param tlhcY displayed area top lefthand corner Y
   *  @param brhcX displayed area bottom righthand corner X
   ...
   *  a possible bitmap shutter is deactivated. If no shutter display value
   ...
   *  and overlay objects on this graphic layer are also deleted or deactivated,
   * respectively.
   *  @param idx index of the graphic layer, must be <
   ...
   *  @param layer index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   ...
   *  @param layer index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param idx index of the text object, must be <
   * getNumberOfTextObjects(layer)
   ...
   *  @param layer index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param applicability defines to which images/frames the new object
   * applies. Default: all images referenced by the presentation state.
   *  @return a pointer to the new text object
   */
  DVPSTextObject* addTextObject(
      size_t layer, DVPSObjectApplicability applicability = DVPSB_allImages);
   ...
   *  @param idx index of the text object, must be <
   * getNumberOfTextObjects(layer)
   *  @param new_layer index of the graphic layer to which the text object is
   * moved, must be < getNumberOfGraphicLayers()
   *  @param applicability defines to which images/frames the new object applies
   * from now on. Default: all images referenced by the presentation state.
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition moveTextObject(
      size_t old_layer, size_t idx, size_t new_layer,
   ...
   *  @param layer index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param idx index of the graphic object, must be <
   * getNumberOfGraphicObjects(layer)
   ...
   *  Only the objects that are applicable to the current (attached) image
   *  and the selected frame number are used by this method.
   *  @param old_layer index of the graphic layer on which the graphic object
   * is, must be < getNumberOfGraphicLayers()
   *  @param idx index of the graphic object, must be <
   * getNumberOfGraphicObjects(layer)
   *  @param new_layer index of the graphic layer to which the graphic object is
   * moved, must be < getNumberOfGraphicLayers()
   *  @param applicability defines to which images/frames the new object applies
   * from now on. Default: all images referenced by the presentation state.
   *  @return EC_Normal upon success, an error code otherwise
   ...
   *  @param overlayData upon success a pointer to the overlay plane is passed
   * back in this parameter. The overlay plane is organized as one byte or one
   * word per pixel. The byte/word values are already transformed from pvalues
   * to DDLs.
   *  @param width upon success the width of the overlay bitmap in pixels is
   * returned in this parameter.
   *  @param height upon success the height of the overlay bitmap in pixels is
   * returned in this parameter.
   *  @param left_pos upon success the horizontal position of the overlay
   * relative to the image is returned. 0 means that the overlay is left aligned
   * with the image. Since the overlay is cropped at the borders of the image,
   * values < 0 are impossible.
   *  @param top_pos upon success the vertical position of the overlay relative
   * to the image is returned.
   *  @param isROI returns OFTrue if the overlay is ROI, OFFalse if the overlay
   * is Graphic.
   *  @param fore returns value of foreground color (pvalue), all other values
   * are transparent
   *  @param bits number of bits used for overlayData (valid: 8 or 12, default:
   * 8). If bits is less than or equal to 8 the resulting overlayData is an
   * array of 8 bit values, an array 16 bit values otherwise.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition getOverlayData(size_t layer, size_t idx, const void*& overlayData,
                             unsigned int& width, unsigned int& height,
                             unsigned int& left_pos, unsigned int& top_pos,
                             OFBool& isROI, Uint16& fore,
                             unsigned int bits = 8);

  /** gets the number of overlays which are embedded in the
   *  image currently attached to the presentation state. Overlays in the image
   * are counted only if they are not shadowed by overlays that are embedded in
   * the presentation state and use the same repeating group number.
   *  @return number of overlays in attached image
   */
  size_t getNumberOfOverlaysInImage();

  /** gets the repeating group number of the given overlay in the attached
   * image.
   ...
   *  @param idx index of the overlay, must be < getNumberOfOverlaysInImage().
   *  @return layer index (which is < getNumberOfGraphicLayers()) if overlay
   * exists and is activated, DVPS_IDX_NONE otherwise.
   */
   ...
   *  Since overlays in the presentation state can shadow overlays in the
   * attached image, execution of this method may change the number of overlays
   * reported in the attached image.
   *  @param idx index of the overlay, must be <
   ...
   *  @param idx index of the overlay, must be <
   * getNumberOfOverlaysInPresentationState().
   *  @param newGroup new repeating group number 0x6000-0x601F (even). If this
   * optional parameter is omitted, the method attemps to automatically
   * determine a new group number so that no overlay in the attached image is
   * shadowed any more. If this is impossible, the method fails and leaves the
   * overlay repeating group unchanged.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition changeOverlayGroupInPresentationState(size_t idx,
                                                    Uint16 newGroup = 0);
   ...
   *  The overlay data is copied into the presentation state, i.e. the DICOM
   * dataset can be deleted after execution of this method.
   *  @param overlayIOD the DICOM dataset from which the overlay is to be read
   *  @param groupInItem the repeating group 0x6000..0x61F (even) of the overlay
   * to be read
   *  @param newGroup repeating group number 0x6000-0x601F (even) to be used for
   *    the overlay in the presentation state. If this optional parameter is
   * omitted, the method attemps to automatically determine a new group number
   * so that no overlay in the attached image is shadowed any more. If this is
   * impossible, a group number so far unused in the presentation state is
   * taken.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition addOverlayToPresentationState(DcmItem& overlayIOD,
                                            Uint16 groupInItem,
                                            Uint16 newGroup = 0);

  /** checks if an overlay from the presentation state is suitable
   *  for use as a bitmap shutter. An overlay is suitable if it is a graphic
   * overlay with the same size as the attached image and with the origin 1\1.
   *  This method does not check wether the overlay is already activated as
   * overlay or bitmap shutter.
   *  @param idx index of the overlay, must be <
   * getNumberOfOverlaysInPresentationState().
   ...
   *  the old image is detached (and freed if necessary) and the new image is
   * attached. A preview image is created automatically (if values for preview
   ...
   *    of the passed DcmDataset, which is freed when a different image is
   * attached or the presentation state is deleted.
   *  @return EC_Normal if successful, an error code otherwise. If an error code
   *    is returned, the presentation state has the same state as before the
   * call to this method.
   */
   ...
   *  @param transferOwnership if true, the presentation state assumes ownership
   *    of the passed DcmFileFormat, which is freed when a different image is
   * attached or the presentation state is deleted.
   *  @return EC_Normal if successful, an error code otherwise. If an error code
   *    is returned, the presentation state has the same state as before the
   * call to this method.
  ...
  /** detaches and frees the image (incl. preview) attached to the presentation
   * state.
   */
  void detachImage();
   ...
   *  of the none-grayscale transformations of the presentation state "burned
   * in" into the pixel data. The pixel data returned is already corrected by a
   *  display transform for the current display device and can be mapped
   * directly to digital driving levels of the graphics board. The pointer to
   * the pixel data remains valid until the next call to this function, or until
   * the image is detached or the presentation state is deleted.
   *  @param pixelData in this parameter a pointer to the pixel data is returned
   *    (array of 8 bit values). The storage area allocated for this pointer
   * must not be freed by the caller. If the return value is an error code, no
   * valid pixel data pointer may be assumed.
   *  @param width returns the width of the bitmap in pixels
   *  @param height returns the height of the bitmap in pixels
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition getPixelData(const void*& pixelData, unsigned long& width,
   ...
   *  This method may only be called when an image is attached to the
   *  presentation state.
   *  @param width upon success, the image width (pixels) is returned in this
   * parameter.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  This method may only be called when an image is attached to the
   *  presentation state.
   *  @param height upon success, the image height (pixels) is returned in this
   * parameter.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  Smaller images are scaled up by an appropriate integer factor. Both
   * maximum values need to be twice greater than the maximum of the minimum
   * values.
   ...
   *  Larger images are scaled down by an appropriate integer factor. Both
   * maximum values need to be twice greater than the maximum of the minimum
   * values.
   ...
   *  Bitmap size depends on implicit scaling, a heuristic is used for very
   * small images. The return values depend on the current minimum/maximum print
   ...
   *  Bitmap size depends on implicit scaling, a heuristic is used for very
   * small images. The return value depends on the current minimum/maximum print
   * bitmaps width/height values!
   *  @param width upon success, the image width (in pixels) is returned in this
   * parameter
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   * small images The return value depends on the current minimum/maximum print
   * bitmaps width/height values!
   *  @param height upon success, the image height (in pixels) is returned in
   * this parameter
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  by the height of a pixel (x/y). The related image is already rotated and
   * flipped!
   *  @return pixel aspect ratio
   */
   ...
   *  the true physical width (in mm) of the print image (under consideration of
   * the rotation status) and writes it to the requestedImageSize string.
   *  @param requestedImageSize requested image size is written to this
   * parameter upon successful return. Otherwise string is empty upon return.
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getPrintBitmapRequestedImageSize(OFString& requestedImageSize);

  /** writes the bitmap data into the given buffer.
   *  The bitmap has the format: 12 bits stored and 16 bits allocated. This
   * method is used to create the preformatted bitmap where the annotations are
   * later burned in. Implicit scaling is performed if the bitmap is too small
   * (see minimum bitmap size). The storage area must be allocated and deleted
   * from the calling method.
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getPrintBitmap(void* bitmap, unsigned long size,
                             OFBool inversePLUT = OFFalse);

  /** creates a new preview image based on the current image and pstate.
   *  The maximum size of this image is specified by the two parameters maxWidth
   * and maxHeight. The actual size should be determined using one of the
   * following appropriate methods (e.g. getPreviewImageWidthHeight) since the
   * original pixel aspect ratio is alsways considered. The preview image
   * includes all grayscale and spatial transformations performed on the current
   * image so far. The method renderPixelData also renders the preview image (if
   * existing). Therefore the preview image is always held consistent with the
   * current image. Overlays, bitmapped shutters and any other annotations are
   * not rendered into the preview image.
   ...
   *  @param clipMode specifies whether to clip the preview image to the
   * displayed area (not implemented!)
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  could possibly be contained in the image after application of the Modality
   * transform, but before any VOI, Presentation or display transform. This
   * method may only be called when an image is attached to the presentation
   * state.
   *  @param minValue upon success, the smallest value is returned in this
   * parameter.
   *  @param maxValue upon success, the biggest value is returned in this
   * parameter.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  These values are defined as the smallest and biggest number that
   *  are actually contained in the image after application of the Modality
   * transform, but before any VOI, Presentation or display transform. This
   * method may only be called when an image is attached to the presentation
   * state.
   *  @param minValue upon success, the smallest value is returned in this
   ...
   *  This method may only be called when an image is attached to the
   *  presentation state.
   *  @param frames upon success, the number of frames is returned in this
   * parameter.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  that is rendered, the overlay bitmaps and the visibility of graphic and
   * text objects. This method may only be called when an image is attached to
   * the presentation state.
   ...
   *  @param bits number of bits used for the output DDL value (8 = softcopy, 12
   * = hardcopy)
   ...
   *  @return softcopy VOI LUT item for current image and frame if found, NULL
   * otherwise
   */
...
private:
   ...
   *  @return EC_Normal if successful, an error code otherwise
   */
  static OFCondition activateOverlayHelper(DVPSOverlay& ovl, DicomImage& image,
  ...
  /** contains the width of the attached image without consideration of
   * rotation.
   */
  unsigned long currentImageWidth;
  /** contains the height of the attached image without consideration of
   * rotation.
   */
  unsigned long currentImageHeight;
  /** contains the width of the attached image after pixel data have been
   * rendered (w/o clipping).
   */
  unsigned long renderedImageWidth;
  /** contains the height of the attached image after pixel data have been
   * rendered (w/o clipping).
   */
  unsigned long renderedImageHeight;
  /** contains the top hand corner of the attached image after pixel data have
   * been rendered.
   */
  signed long renderedImageTop;
  /** contains the left hand corner of the attached image after pixel data have
   * been rendered. (the following equation is always true: renderedImageTop <=
   * renderedImageBottom)
   */
  signed long renderedImageLeft;
  /** contains the bottom hand corner of the attached image after pixel data
   * have been rendered. (the following equation is always true:
   * renderedImageLeft <= renderedImageRight)
   */
  signed long renderedImageBottom;
  /** contains the right hand corner of the attached image after pixel data have
   * been rendered.
   */
