...
/** the list of referenced images contained in a presentation state (internal
 * use only). This class manages the data structures comprising one complete
...
public:
  ...
  DVPSReferencedImage_PList* clone() {
    return new DVPSReferencedImage_PList(*this);
  }

  /// destructor
  virtual ~DVPSReferencedImage_PList();

  /** reads a list of image references (ReferencedImageSequence) from a DICOM
  ...
  /** writes the list of image references managed by this object to a DICOM
   * dataset. Copies of the DICOM element managed by this object are inserted
   * into the DICOM dataset.
   *  @param dset the DICOM dataset to which the ReferencedImageSequence is
   * written
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   */
  void removeFrameReference(const char* sopinstanceuid, unsigned long frame,
                            unsigned long numberOfFrames);

  /** checks if an image reference with the given SOP instance UID exists
   *  in this ReferencedImageSequence and deletes it.
   *  @param sopinstanceuid the SOP instance UID of the image reference to be
   * removed.
   */
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getImageReference(size_t idx, OFString& sopclassUID,
