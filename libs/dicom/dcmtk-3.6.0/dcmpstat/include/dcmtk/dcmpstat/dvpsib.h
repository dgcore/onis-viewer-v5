...
public:
  ...
  DVPSImageBoxContent* clone() {
    return new DVPSImageBoxContent(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the ImageBoxContentSequence from which the data is
   * to be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the ImageBoxContentSequence to which the data
   * is written
   *  @param writeRequestedImageSize if false, the Requested Image Size
   * attribute is not written, e.g. because it is not supported by the target
   * printer.
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition createDefaultValues(OFBool renumber, unsigned long number,
   ...
   *  @param presentationlutreference referenced SOP instance UID for the
   * referenced Presentation LUT,
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setContent(const char* instanceuid, const char* retrieveaetitle,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getImageReference(const char*& studyUID, const char*& seriesUID,
  ...
  OFBool printSCPSet(DVConfiguration& cfg, const char* cfgname,
                     ...
                     DcmDataset*& rspDataset, DcmDataset& imageDataset,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setUIDsAndAETitle(DcmUniqueIdentifier& studyUID,
...
private:
  ...
  // we do not support the ReferencedImageOverlayBoxSequence which is retired in
  // Supplement 35 anyway.
