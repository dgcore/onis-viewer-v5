...
/** the list of referenced series contained in a presentation state (internal
 * use only). This class manages the data structures comprising the complete
...
public:
  ...
  DVPSReferencedSeries_PList* clone() {
    return new DVPSReferencedSeries_PList(*this);
  }

  /// destructor
  virtual ~DVPSReferencedSeries_PList();

  /** reads a list of series references (ReferencedSeriesSequence) from a DICOM
   * dataset. The DICOM elements of the image references item are copied from
   * the dataset to this object. The completeness of all items (presence of all
   * required elements, value multiplicity) is checked. If this method returns
   * an error code, the object is in undefined state afterwards.
   *  @param dset the DICOM dataset from which the sequence is to be read
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition read(DcmItem& dset);

  /** writes the list of series references managed by this object to a DICOM
   * dataset. Copies of the DICOM element managed by this object are inserted
   * into the DICOM dataset.
   *  @param dset the DICOM dataset to which the ReferencedSeriesSequence is
   * written
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   * series contain image references and whether all references images use the
   * same SOP class.
   */
  OFBool isValid();

  /** checks whether all images referenced in this series use the passed SOP
   * class UID.
   ...
   *  This method checks if a series reference with the given Series Instance
   * UID exists in this ReferencedSeriesSequence.
   *  @param seriesUID the Series Instance UID of the searched series reference
   *  @return a pointer to the matching DVPSReferencedSeries if found, NULL
   * otherwise.
   */
  DVPSReferencedSeries* findSeriesReference(const char* seriesUID);

  /** find image reference in this list.
   *  This method checks if an image reference with the given Series Instance
   * UID and SOP instance UID exists in this ReferencedImageSequence.
   ...
   *  @return a pointer to the matching DVPSReferencedImage if found, NULL
   * otherwise.
   */
  DVPSReferencedImage* findImageReference(const char* seriesUID,
                                          const char* instanceUID);

  /** removes series reference.
   *  This method checks if an series reference with the given Series Instance
   * UID exists in this ReferencedSeriesSequence and deletes it (including its
   * image references).
   *  @param seriesUID the Series Instance UID of the series reference to be
   * removed.
   */
  void removeSeriesReference(const char* seriesUID);

  /** removes image reference.
   *  This method checks if an image reference with the given SOP instance UID
   * exists in the series with the given Series Instance UID and deletes it.
   ...
   *  @param filesetUID the series storageMediaFileSetUID. Default: value
   * absent.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageReference(const char* seriesUID, const char* sopclassUID,
                                const char* instanceUID,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getImageReference(size_t idx, OFString& seriesUID,
                                OFString& sopclassUID, OFString& instanceUID,
                                OFString& frames, OFString& aetitle,
