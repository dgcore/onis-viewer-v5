...
/** the list of graphic layers contained in a presentation state (internal use
 * only). This class manages the data structures comprising the complete Graphic
 * Layer Sequence in a Presentation State object.
 */
...
public:
  ...
  DVPSGraphicLayer_PList* clone() {
    return new DVPSGraphicLayer_PList(*this);
  }
  ...
  /** writes the Graphic Layer Sequence managed by this object to a DICOM
   ...
   *  @param gLayer name of the graphic layer. Must be unique within the
   * sequence, i.e. no other graphic layer with the same name may exist,
   * otherwise an error code is returned.
   *  @param gLayerOrder graphic layer order of the new layer.
   *  @param gLayerDescription (optional) description of the new layer. Default:
   * description absent.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addGraphicLayer(const char* gLayer, const Sint32 gLayerOrder,
   ...
   */
  void cleanupLayers(DVPSOverlayCurveActivationLayer_PList& activations,
