...
/** an item of the graphic object sequence in a presentation state (internal use
 * only). This class manages the data structures comprising one item of the
 * Graphic Object Sequence which is contained in the Graphic Annotation Sequence
...
public:
  ...
  DVPSGraphicObject* clone() {
    return new DVPSGraphicObject(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the GraphicObjectSequence from which the data is
   * to be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the GraphicObjectSequence to which the data is
   * written
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  @param x upon success the x value of the point is returned in this
   * parameter
   *  @param y upon success the y value of the point is returned in this
   * parameter
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  @param data pointer to an array of Float32 values with a size of (at
   * least) 2*number. The values are copied into the graphic object.
   *  @param unit the graphic annotation units for this data.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setData(size_t number, const Float32* data,
