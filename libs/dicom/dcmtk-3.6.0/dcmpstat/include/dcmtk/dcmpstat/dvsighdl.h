...
public:
   ...
   *  @param onRead true if the dataset is just being read, false if it is being
   * written
   */
  void updateDigitalSignatureInformation(DcmItem& dataset,
                                         DVPSObjectType objtype, OFBool onRead);
   ...
   *  @param tagList list of attributes (tags) to be looked up inside the
   * dataset
   *  @return true if any of the given attributes is affected by a digital
   ...
   *  @param mainDataset reference to main dataset in which signature(s) are to
   * be added
   *  @param itemStack stack of items within the main dataset that are to be
   * signed separately.  If main dataset is to be signed, it must be included in
   * this stack.
   *  @param attributesNotToSignInMainDataset list of attribute tags that should
   * be omitted from the signature on the main dataset level
   ...
   */
  OFCondition createSignature(DcmItem& mainDataset, const DcmStack& itemStack,
...
private:
   ...
   * It is assumed that the stack top is a DigitalSignatureSequence which is not
   * printed and that the stack bottom is the main dataset, which is also not
   * printed.
   * @param stack search stack, as returned by
   * DcmSignature::findFirstSignatureItem() etc.
   * @param os output stream
   */
  static void printSignatureItemPosition(DcmStack& stack,
                                         STD_NAMESPACE ostream& os);

  /** replaces the current HTML page for the given object type by the new
   * string.
   *  @param objtype object type
