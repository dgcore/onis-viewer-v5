...
public:
  ...
  DVPSImageBoxContent_PList* clone() {
    return new DVPSImageBoxContent_PList(*this);
  }

  /// destructor
  virtual ~DVPSImageBoxContent_PList();

  /** reads a list of image boxes (ImageBoxContentSequence) from a DICOM
   * dataset. The DICOM elements of the image boxes are copied from the dataset
   * to this object. The completeness of all items (presence of all required
   * elements, value multiplicity) is checked. If this method returns an error
   * code, the object is in undefined state afterwards.
   *  @param dset the DICOM dataset from which the sequence is to be read
   *  @param presentationLUTList list of presentation LUTs which may be
   * referenced
   ...
   *  the DICOM dataset.
   *  @param dset the DICOM dataset to which the ImageBoxContentSequence is
   * written
   ...
   * target printer.
   *  @param numItems the number of items (from the beginning of the list) to be
   * written. Default: all items are written.
   *  @param ignoreEmptyImages if true, all image boxes without image box
   * position are ignored when writing.
   *  @param writeReferencedPLUTSQ if false, the Referenced Presentation LUT
   * Sequence is not written, e.g. because a "general" presentation LUT is used
   * instead of those referenced by the image boxes.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition write(DcmItem& dset, OFBool writeRequestedImageSize,
                    size_t numItems, OFBool ignoreEmptyImages,
   ...
   *  @param ignoreEmptyImages if true, an empty image box position does not
   * cause an error.
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  @param seq sequence to be added to, should be a
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageBox(const char* instanceuid, const char* retrieveaetitle,
  ...
  /** sets magnification type, smoothing type and configuration information back
   * to default for all registered images.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  ...
  /** gets the referenced Presentation LUT SOP Instance UID for the given
   * registered image box..
   *  @param idx index, must be < getNumberOfImages()
   *  @return SOP Instance UID, may be NULL.
   */
   ...
   *  @param studyUID study instance UID under which Hardcopy Grayscale images
   * will be saved
   *  @param seriesUID series instance UID under which Hardcopy Grayscale images
   * will be saved
   *  @param aetitle retrieve AE title for Hardcopy Grayscale images
   *  @return OFTrue if successful, OFFalse otherwise.
   */
  OFBool printSCPCreate(unsigned long numBoxes, DcmUniqueIdentifier& studyUID,
                        DcmUniqueIdentifier& seriesUID, const char* aetitle);
