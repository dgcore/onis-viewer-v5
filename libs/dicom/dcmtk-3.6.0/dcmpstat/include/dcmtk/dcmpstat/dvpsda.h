...
public:
  ...
  DVPSDisplayedArea* clone() {
    return new DVPSDisplayedArea(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the DisplayedAreaSelectionSequence from which the
   * data is to be read
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition read(DcmItem& dset);

  /** writes the displayed area selection managed by this object to a DICOM
   * dataset. Copies of the DICOM element managed by this object are inserted
   * into the DICOM dataset.
   *  @param dset the the item of the DisplayedAreaSelectionSequence to which
   * the data is written
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   */
  OFBool matchesApplicability(const char* instanceUID, unsigned long frame,
  ...
  /** gets the presentation pixel aspect ratio for this displayed area
   * selection. Pixel aspect ratio is defined here as the width of a pixel
   * divided by the height of a pixel (x/y).
   ...
   * returned in this parameter
   */
  void getDisplayedArea(Sint32& tlhcX, Sint32& tlhcY, Sint32& brhcX,
                        Sint32& brhcY);
  ...
  /** gets the presentation pixel magnification ratio for this displayed area if
   * present.
   ...
   * added.
   *  @param frame the frame number of the image reference (current image) to be
   * added.
   *  @param applicability the applicability of the image reference
   * (DVPSB_currentFrame or DVPSB_currentImage)
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageReference(const char* sopclassUID,
                                const char* instanceUID, unsigned long frame,
                                DVPSObjectApplicability applicability);

  /** removes a reference to an image or frame. If the current reference is
   * empty ("global"), an explicit list of references is constructed from the
   * list of series/instance references. The image or frame reference is removed
   * from the total list of references in this object. If the only reference
   * contained in this object is removed, the reference list becomes empty which
   * means that the current reference becomes "global". This case must be
   ...
   *  @param applicability the applicability of the image reference to be
   * removed (DVPSB_currentFrame or DVPSB_currentImage)
   */
  void removeImageReference(DVPSReferencedSeries_PList& allReferences,
                            const char* instanceUID, unsigned long frame,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setDisplayedArea(DVPSPresentationSizeMode sizeMode, Sint32 tlhcX,
                               Sint32 tlhcY, Sint32 brhcX, Sint32 brhcY,
   ...
   */
  void rotateAndFlip(DVPSRotationType rotationFrom, OFBool isFlippedFrom,
                     DVPSRotationType rotationTo, OFBool isFlippedTo);
