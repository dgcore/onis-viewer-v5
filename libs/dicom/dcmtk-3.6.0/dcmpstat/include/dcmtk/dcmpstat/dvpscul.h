...
/** the list of curves contained in image which is attached to a presentation
 * state. This class manages the data structures comprising the list of curves
...
public:
  ...
  DVPSCurve_PList* clone() {
    return new DVPSCurve_PList(*this);
  }
   ...
   *  (i.e. 2-dimensional POLY or ROI curves without curve descriptor) are
   * copied into the "list of curves" structure managed by this object. The
   * completeness of the curves (presence of all required elements, value
   * multiplicity) is checked. If this method returns an error code, the object
