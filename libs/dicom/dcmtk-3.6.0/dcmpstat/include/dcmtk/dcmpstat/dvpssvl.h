...
/** the list of softcopy VOI LUT items contained in a presentation state
 * (internal use only). This class manages the data structures comprising one
 * complete Softcopy VOI LUT sequence contained in a presentation state object.
 */
...
public:
  ...
  DVPSSoftcopyVOI_PList* clone() {
    return new DVPSSoftcopyVOI_PList(*this);
  }

  /// destructor
  virtual ~DVPSSoftcopyVOI_PList();

  /** reads a list of softcopy VOI LUTs (SoftcopyVOILUTSequence) from a DICOM
   * dataset. The DICOM elements of the softcopy VOI LUT item are copied from
  ...
  /** writes the list of softcopy VOI LUTs managed by this object to a DICOM
   * dataset. Copies of the DICOM elements managed by this object are inserted
   * into the DICOM dataset.
   *  @param dset the DICOM dataset to which the SoftcopyVOILUTSequence is
   * written
   *  @return EC_Normal if successful, an error code otherwise.
   */
  ...
  /** creates a default softcopy VOI LUT sequence for a presentation state from
   * a DICOM image. If this method returns an error code, the object is in
  ...
  OFCondition createFromImage(DcmItem& dset,
                              ...
                              const char* sopclassUID, const char* instanceUID,
   ...
   */
  DVPSSoftcopyVOI* findSoftcopyVOI(const char* instanceUID,
                                   unsigned long frame);

  /** finds or creates a softcopy VOI LUT SQ item
   *  with an applicability controlled by the applicability, instanceUID and
   * frame parameters. The softcopy VOI LUT sequence is rearranged such that all
   * other referenced images/frames keep their old settings.
   *  @param allReferences list of series/instance references registered for the
   *    presentation state.
   *  @param sopclassUID SOP class UID of the current image
   *  @param instanceUID SOP instance UID of the current image
   ...
   *    out of memory.
   */
  DVPSSoftcopyVOI* createSoftcopyVOI(DVPSReferencedSeries_PList& allReferences,
   ...
   */
  void removeSoftcopyVOI(DVPSReferencedSeries_PList& allReferences,
