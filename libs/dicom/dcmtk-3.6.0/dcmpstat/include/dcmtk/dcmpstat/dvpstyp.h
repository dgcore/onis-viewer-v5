...
enum DVPSoverlayActivation {
   ...
   *  created in the presentation state object. The overlays remain in the
   * image.
   */
...
/** describes how to handle VOI transformations when creating a default
 * presentation state for an image.
 */
...
enum DVPSGraphicLayering {
   ...
   */
  DVPSG_oneLayer,
  /** create one graphic layer for overlays and one graphic layer for curves on
   * top of that. If overlays are present, one layer containing all overlays is
   * created. If curves are present, a different layer containing all curves is
   * created. If both curves and overlays are present, the curves are assigned
   * the higher layer order.
   */
  DVPSG_twoLayers,
  /** create individual layers for each referenced element, curves on top of
   * overlays. For each overlay and curve present, a separate graphic layer is
   * created. The curve layers have higher order than the overlay layers.
   */
...
enum DVPSObjectApplicability {
  /** the object only applies to the current (selected) frame of the current
   * (attached) image
   */
  DVPSB_currentFrame,
...
/** describes the trim mode (printing of borders around image boxes) for a basic
 * film box
 */
enum DVPSTrimMode {
...
/** describes the certificate verification policy for TLS association
 * negotiation
 */
...
enum DVPSSignatureStatus {
  ...
  /** one or more digital signatures are present and have been successfully
   * verified
   */
  DVPSW_signed_OK,
...
enum DVPSVerifyAndSignMode {
  ...
  /** verify and digitally sign the document (apart from VerifyingObserver and
   * SOPInstanceUID)
   */
  DVPSY_verifyAndSign,
