...
public:
  ...
  DVPSStoredPrint* clone() {
    return new DVPSStoredPrint(*this);
  }
   ...
   *    that are needed for the current image display format (film layout) are
   * written. If false, all image references are written.
   *  @param updateDecimateCrop if true, the decimate/crop attribute on image
   * box level is replaced by the global stored print level setting in all image
   * boxes prior to writing the dataset.
   *  @param ignoreEmptyImages if true, all image boxes without image box
   * position are ignored when writing.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition write(DcmItem& dset, OFBool writeRequestedImageSize,
                    OFBool limitImages, OFBool updateDecimateCrop,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setRequestedDecimateCropBehaviour(
   ...
   *  are reset to default. For all registered images, magnification, smoothing
   * type and configuration information are also set back to default.
   *  @param name name of the new printer (optional)
   *  @param aetitle of the new printer (optional)
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition newPrinter(const char* name = NULL,
                         const char* destinationAE =
                             NULL);  // short cut, delete all optional settings
  ...
  /** checks if one of the registered images has additional settings that are
   * not default values on the image box level.
   *  @param idx index, must be < getNumberOfImages()
   *  @return EC_Normal if successful, an error code otherwise.
   */
  ...
  /** sets the (optional) configuration information for the given registered
   * image box.
   *  @param idx index, must be < getNumberOfImages()
   *  @param value new attribute value, may be NULL.
   *    The caller is responsible for making sure
  ...
  /** gets the (optional) configuration information for the given registered
   * image box.
   *  @param idx index, must be < getNumberOfImages()
   *  @return configuration information, may be NULL.
   */
  ...
  /** gets the "global" presentation LUT which overrides the settings for the
   * image boxes. If not available the presentation LUT of the image box is
   * used.
   *  @return pointer to presentation LUT, may be NULL.
   */
  DVPSPresentationLUT* getPresentationLUT();

  /** resets the Presentation LUT to the default setting. i.e. the presentation
   * LUT which is specified separately for each image box is used.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setDefaultPresentationLUT();

  /** sets the current Presentation LUT shape (overrides the image box
   * settings). Only DVPSP_identity and DVPSP_lin_od are allowed.
  ...
  /** converts an optical density (OD) value to an 8/12/16-bit P-value which is
   * linear to luminance. The output is not calibrated according to the GSDF.
   * This can be done by convertPValueToDDL() in class DVPSPresentationState.
   * The attributes illumination, reflected ambient light and min/max density
   * (default 20/300) from this stored print object are used for the
   * calculation.
  ...
  /** writes the general study and series module attributes for a grayscale
   * hardcopy image that is related to this stored print object to a DICOM
   * dataset. Copies of the DICOM elements managed by this object are inserted
   * into the DICOM dataset.
   *  @param dset the dataset to which the data is written
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition writeHardcopyImageAttributes(DcmItem& dset);

  /** creates a new image box object and sets the content of this image box
   * object.
   ...
   *  @param requestedimagesize requested images size for this image, may be
   * NULL (absent)
   ...
   *  @param inversePLUT true if presentation LUT is for Monochrome1 and must be
   * inversed.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageBox(const char* retrieveaetitle, const char* refstudyuid,
  ...
  /** creates a new image box object and sets the content of this image box
   * object. This is a specialized version of the method with the same name and
   * more parameters. SOP Class is assumed to be Grayscale Hardcopy, Study and
   * Series are derived from the Stored Print internal defaults.
   *  @param retrieveaetitle retrieve AETITLE of the referenced image
   *  @param refsopinstanceuid SOP instance UID of the referenced image
   *  @param requestedimagesize requested images size for this image, default:
   * absent
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageBox(const char* retrieveaetitle,
                          const char* refsopinstanceuid,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setSingleAnnotation(const char* displayformat, const char* text,
                                  Uint16 position);
  ...
  /** returns the image UIDs that are required to look up the referenced image
   * in the database
   *  @param idx index, must be < getNumberOfImages()
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getImageReference(size_t idx, const char*& studyUID,
                                const char*& seriesUID,
                                const char*& instanceUID) {
    return imageBoxContentList.getImageReference(idx, studyUID, seriesUID,
                                                 instanceUID);
  }
   ...
   *  The properties are not returned, but if the message handler is switched to
   * "dump mode", the DIMSE communication will be printed.
  ...
  /** checks whether a presentation LUT or LUT shape is active in this stored
   * print object. In this case, if the printer supports the Presentation LUT
   * SOP class, a Presentation LUT SOP Instance is created in the printer.
   *  @param printHandler print communication handler, association must be open.
   *  @param printerRequiresMatchingLUT true if printer requires presentation
   * LUTs matching the image depth
   *  @param printerLUTRenderingPreferred true if SCP side presentation LUTs
   * should be preferred even if printer supports 12-bit image transmission.
   ...
   *  @param printHandler print communication handler, association must be open.
   *  @param dset DICOM dataset containing all Basic Film Session attributes
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition printSCUcreateBasicFilmBox(DVPSPrintMessageHandler& printHandler,
                                         OFBool plutInSession);

  /** Transmits a DICOM image to the printer (Basic Grayscale Image Box N-SET).
   *  @param printHandler print communication handler, association must be open.
   *  @param idx index of the image reference from which the Image Box settings
   * are taken, must be < getNumberOfImages().
   *  @param image DICOM image to be printed
   *  @param useMonochrome1 if true, the image is transmitted in MONOCHROME1
   * photometric interpretation. Default is false, image is transmitted in
   * MONOCHROME2 in this case.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition printSCUsetBasicImageBox(DVPSPrintMessageHandler& printHandler,
                                       size_t idx, DicomImage& image,
   ...
   *  @param study study UID to be used when storing Stored Print or image
   * objects
   *  @param psSeries series UID to be used when storing Stored Print objects
   *  @param imgSeries series UID to be used when storing image objects
   * (Hardcopy Grayscale)
   *  @return OFTrue if N-CREATE was successful, OFFalse otherwise.
   */
  OFBool printSCPCreate(DVConfiguration& cfg, const char* cfgname,
   ...
   */
  OFBool printSCPSet(DVConfiguration& cfg, const char* cfgname,
                     DcmDataset* rqDataset, T_DIMSE_Message& rsp,
                     DcmDataset*& rspDataset, OFBool presentationLUTnegotiated,
                     DVPSPresentationLUT_PList& globalPresentationLUTList);
  ...
  /** looks up the image box with the given SOP instance UID in the image box
   * list and returns a pointer to a new object containing a copy of this image
   * box. If the object is not found, NULL is returned.
  ...
  DVPSImageBoxContent* duplicateImageBox(const char* uid) {
    return imageBoxContentList.duplicateImageBox(uid);
  }
   ...
   *  Any other object existing in the list with the same SOP instance UID is
   * removed. Used during a Print SCP basic grayscale image box N-SET operation.
   *  @param newImageBox new image box object to be added to the list.
   */
  void replaceImageBox(DVPSImageBoxContent* newImageBox) {
    imageBoxContentList.replace(newImageBox);
  }

  /** updates the list of Presentation LUTs managed by the Stored Print object
   *  from a global list. If a Presentation LUT is active on Film Box level, the
   * corresponding LUT is copied from the global presentation LUT list.
   *  Presentation LUT references on Image Box level are ignored.
   *  Used during a Print SCP N-ACTION operation.
   *  @param globalPresentationLUTList list of presentation LUTs managed by the
   * Print SCP
   */
   ...
   *  with all image boxes in this film box on a Print SCP that requires a
   * matching alignment between a Presentation LUT and the image pixel data.
   ...
   *  @param newReferencedPLUT new value for referenced presentation LUT
   * instance UID
   ...
   *  @return OFTrue if empty page (no image box position assigned), OFFalse
   * otherwise.
   */
  OFBool emptyPageWarning() {
    return imageBoxContentList.emptyPageWarning();
  }
...
private:
  ...
  /// Module=Film_Box_Module, VR=US, VM=1, Type 2c required if presentation SOP
  /// class present
...
private:
   ...
   *  content of this string can differ from
   * referencedPresentationLUTInstanceUID which contains the UID from the Stored
   * Print object wheras this string contains the UID assigned by the remote
   * Print SCP. If used as Print SCP, these strings should always be equal.
   */
  OFString presentationLUTInstanceUID;

  /// stores the "global" presentation LUT that overrides the image box LUTs
  /// (optional)
  DVPSPresentationLUT globalPresentationLUT;

  /// flag indicating whether the globalPresentationLUT is currently valid or
  /// not
