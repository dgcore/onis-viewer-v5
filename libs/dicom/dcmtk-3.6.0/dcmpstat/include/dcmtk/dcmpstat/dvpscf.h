...
public:
   ...
   *  @param peerType defines a filter for the peer service type. Default:
   * handle only storage peers.
   ...
   *  with the given index. The target identifier is unique within the
   * configuration file
   ...
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID().
   *  @return entry if present and parsable in the config file, DVPSE_storage
   * otherwise.
   */
  ...
  /** returns the IMPLICITONLY entry for the communication partner with the
   * given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID().
  ...
  /** returns the DISABLENEWVRS entry for the communication partner with the
   * given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID().
   *  @return entry if present in the config file, OFFalse otherwise.
   ...
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID().
   *  @param idx index of the value, must be <
   * getTargetNumberOfCipherSuites(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetCipherSuite(const char* targetID, Uint32 idx,
                                   OFString& value);

  /** returns the PEERAUTHENTICATION entry for the communication partner with
   * the given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID().
   *  @return entry if present and parsable in the config file, DVPSQ_require
   * otherwise.
   */
  DVPSCertificateVerificationType getTargetPeerAuthentication(
      const char* targetID);

  /** returns the DIFFIEHELLMANPARAMETERS entry for the storage peer with the
   * given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID().
   *  @return entry if present in the config file, NULL otherwise.
  ...
  /** returns the PRESENTATIONLUTMATCHREQUIRED entry for the printer with the
   * given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
  ...
  /** returns the PRESENTATIONLUTPREFERSCPRENDERING entry for the printer with
   * the given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @return entry if present in the config file, OFFalse otherwise.
   */
  OFBool getTargetPrinterPresentationLUTPreferSCPRendering(
      const char* targetID);

  /** returns the PRESENTATIONLUTINFILMSESSION entry for the printer with the
   * given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @return entry if present in the config file, OFFalse otherwise.
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfBorderDensities(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterBorderDensity(const char* targetID, Uint32 idx,
                                            OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfMaxDensities(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterMaxDensity(const char* targetID, Uint32 idx,
                                         OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfMinDensities(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterMinDensity(const char* targetID, Uint32 idx,
                                         OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfPortraitDisplayFormats(targetID)
   *  @return number of rows for this display format if present, 0 otherwise
   */
  Uint32 getTargetPrinterPortraitDisplayFormatRows(const char* targetID,
                                                   Uint32 idx);
   ...
   */
  Uint32 getTargetPrinterPortraitDisplayFormatColumns(const char* targetID,
                                                      Uint32 idx);
   ...
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @return OFTrue if printer supports annotations in film session label,
   * OFFalse otherwise.
   */
   ...
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterAnnotationDisplayFormatID(const char* targetID,
                                                        OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfFilmSizeIDs(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterFilmSizeID(const char* targetID, Uint32 idx,
                                         OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfMediumTypes(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterMediumType(const char* targetID, Uint32 idx,
                                         OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfFilmDestinations(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterFilmDestination(const char* targetID, Uint32 idx,
                                              OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfPrinterResolutionIDs(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterResolutionID(const char* targetID, Uint32 idx,
                                           OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfMagnificationTypes(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterMagnificationType(const char* targetID,
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfSmoothingTypes(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterSmoothingType(const char* targetID, Uint32 idx,
                                            OFString& value);
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfConfigurationSettings(targetID)
   *  @return value if present, NULL otherwise.
   */
   ...
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @param idx index of the value, must be <
   * getTargetPrinterNumberOfEmptyImageDensities(targetID)
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getTargetPrinterEmptyImageDensity(const char* targetID,
                                                Uint32 idx, OFString& value);

  /** returns the OMITSOPCLASSUIDFROMCREATERESPONSE entry for the printer with
   * the given target ID from the configuration file.
   *  @param targetID communication target ID, must be one of the target
   *    identifiers returned by getTargetID() for peer type DVPSE_printerAny.
   *  @return entry if present in the config file, OFFalse otherwise.
   */
  OFBool getTargetPrintSCPOmitSOPClassUIDFromCreateResponse(
      const char* targetID);
  ...
  /** returns the name of the log file to be used for general application
   * messages. Value is taken from the section GENERAL/APPLICATION/LOGFILE in
   * the config file.
   *  @return name of the log file, NULL if absent.
   */
  const char* getLogFile();

  /** returns the log level / filter to be used for general application
   * messages. Value is taken from the section GENERAL/APPLICATION/LOGLEVEL in
   * the config file.
   ...
   *  with the given index. The identifier is unique within the configuration
   * file
   ...
   *  @param lutID LUT ID, must be one of the identifiers returned by
   * getLUTID().
   *  @return entry if present in the config file, NULL otherwise.
   */
   ...
   *  ID from the configuration file.
   *  @param reportID SR "template" ID, must be one of the identifiers returned
   * by getReportID().
   *  @return entry if present in the config file, NULL otherwise.
   */
  ...
  /** returns the coding scheme designator of the user code for the given user
   * if present.
   *  @param userID user ID as returned by getUserID()
   *  @param value the result is both stored in this object and returned as
   * return value.
   *  @return value if present, NULL otherwise.
   */
  const char* getUserCodingSchemeDesignator(const char* userID,
                                            OFString& value);

  /** returns the coding scheme version of the user code for the given user if
   * present.
   *  @param userID user ID as returned by getUserID()
   *  @param value the result is both stored in this object and returned as
   * return value.
...
private:
   ...
   */
  const char* getConfigEntry(const char* l2_key, const char* l1_key,
                             const char* l0_key);
   ...
   *  @param l1_key level 1 key
   *  @param l0_key level 0 key (entry name)
   *  @param deflt default to be returned if entry is not present or syntax
   * error.
   *  @return value assigned to the key if present, default otherwise.
   */
  OFBool getConfigBoolEntry(const char* l2_key, const char* l1_key,
                            const char* l0_key, OFBool deflt);

  /* member variables */

  /** pointer to the configuration file data if the configuration file was
