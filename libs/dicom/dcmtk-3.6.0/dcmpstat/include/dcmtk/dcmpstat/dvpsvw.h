...
public:
   ...
   * afterwards.
   *  @param idx the index of the VOI window to be read, must be <
   * wcenter.getVM()
   *  @param wcenter the window center(s)
   *  @param wwidth the window width(s). wwidth.getVM() must be ==
   * wcenter.getVM().
   *  @param expl the window center/window width explanation. If omitted,
   * explanation remains empty.
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition read(size_t idx, DcmDecimalString& wcenter,
                   DcmDecimalString& wwidth, DcmLongString* expl = NULL);
