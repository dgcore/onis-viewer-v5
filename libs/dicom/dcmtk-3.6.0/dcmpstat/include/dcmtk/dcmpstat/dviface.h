...
public:
   ...
   *     by the interface object. The caller should make sure that the config
   * file really exists because the constructor cannot return an error status.
   *     If a non-existing filename (or NULL) is passed, an empty configuration
   * file is assumed.
   *  @param useLog (optional) flag specifying whether a general log file should
   * be used (config entry GENERAL\APPLICATION\LOGFILE).
   */
   ...
   *  This method acquires a database lock which must be explicitly freed by the
   * user.
   ...
   */
  OFCondition loadImage(const char* studyUID, const char* seriesUID,
   ...
   *  @param changeStatus if true the pstate and (first) image file is marked
   * 'reviewed' (not new)
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition loadPState(const char* studyUID, const char* seriesUID,
                         const char* instanceUID,
                         OFBool changeStatus = OFFalse);

  /** loads a presentation state and an image (which need not be contained in
   * the database) and attaches the image to the presentation state (if
   * specified, otherwise the current image will be used). This method does not
   ...
   *  @param changeStatus if true the structured report file is marked
   * 'reviewed' (not new)
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition loadStructuredReport(const char* studyUID, const char* seriesUID,
                                   const char* instanceUID,
                                   OFBool changeStatus = OFFalse);
   ...
   *  This "template" is just a DICOM Structured Reporting file which resides in
   * a special sub-folder and is referenced by the configuration file.  This
   * meachanism should facilitate the creation of new reports since one does not
   * have to start with a completely empty report. Please note that the current
   * structured report is replaced by the specified "template".  New
   * study/series/instance UIDs are generated automatically for the new report.
   *  This method does not acquire a database lock.
   ...
   *  in which the database index file resides. The filename is generated
   * automatically. A new SOP Instance UID is assigned whenever a presentation
   * state is saved. After successfully storing the presentation state, the
   * database index is updated to include the new object. This method releases
   * under any circumstances the database lock if it exists.
  ...
  /** saves the current presentation state in a file with the given path and
   * filename. A new SOP Instance UID is assigned whenever a presentation state
   * is saved. This method does not acquire a database lock and does not
   * register the saved presentation state in the database.
   *  @param filename path and filename under which the presentation state is to
   * be saved
   *  @param replaceSOPInstanceUID flag indicating whether the
   *    SOP Instance UID should be replaced by a new UID.
   *    If true, a new UID is always generated. If false, a new
   *    UID is generated only if no UID has been assigned before.
   *  @param explicitVR selects the transfer syntax to be written. True (the
   * default) selects Explicit VR Little Endian, False selects Implicit VR
   * Little Endian.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition savePState(const char* filename, OFBool replaceSOPInstanceUID,
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition saveCurrentImage(const char* filename,
                               OFBool explicitVR = OFTrue);

  /** saves the current structured report in the same directory in which the
   * database index file resides.  The filename is generated automatically.  A
   * new SOP Instance UID is not assigned automatically unless the method
   * getStructuredReport().createRevisedVersion() is called (see documentation
   * in file dcmsr/dsrdoc.h).  This is not required for the first version of a
   * document (i.e. directly after program start or calling the method
   *  getStructuredReport().createNewDocument()).
   *  After successfully storing the structured report, the database index is
  ...
  /** saves the current structured report in a file with the given path and
   * filename. A new SOP Instance UID is not assigned automatically unless the
   * method getStructuredReport().createRevisedVersion() is called (see
   * documentation in file dcmsr/dsrdoc.h).  This is not required for the first
   * version of a document (i.e. directly after program start or calling the
   * method getStructuredReport().createNewDocument()). This method does not
   * acquire a database lock and does not the saved structured report in the
   * database.
   *  @param filename path and filename under which the structured report is to
   * be saved
   *  @param explicitVR selects the transfer syntax to be written. True (the
   * default) selects Explicit VR Little Endian, False selects Implicit VR
   * Little Endian.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition saveStructuredReport(const char* filename,
                                   OFBool explicitVR = OFTrue);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition addImageReferenceToPState(const char* studyUID,
                                        const char* seriesUID,
                                        const char* instanceUID);

  /** gets the number of image references contained in the current presentation
   * state.
   ...
   *  (using loadPState or loadImage) or when resetPresentationState() is
   * called.
   ...
   *  This reference will become invalid when the DVInterface object is deleted
   * or a different structured report is loaded (using loadStructuredReport).
   ...
   * after the last successful operation of "loadImage" or "loadPState". A state
   * can also explicitly specified as such a "reset state" by using the method
   * saveCurrentPStateForReset(). Attention: The last reference returned by
   * getCurrentPState() becomes invalid when this method is called.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition resetPresentationState();

  /** saves the current state of the presentation state object to be used for
   *  resetPresentationState(). This is e.g. useful after registration of
   * additional images directly after a new images has been loaded.
   *  @return EC_Normal upon success, an error code otherwise.
   */
   ...
   */
  const char* getFilename(const char* studyUID, const char* seriesUID,
                          const char* instanceUID);

  /** returns the number of studies currently contained in the database.
   *  This method acquires a database lock which must be explicitly freed by the
   * user. The number reported (returned) by this method remains valid as long
   *  as the database lock remains active and no function modifying the database
   * is called. Functions that modify the database are: Storing new presentation
   * states, deleting data, modifying the 'reviewed' status flag of IODs.
   ...
   *  This method acquires a database lock which must be explicitly freed by the
   * user. The selection remains valid until the database lock is removed or the
   * database is modified (see comments for getNumberOfStudies). Implicitly the
   * first series and first instance within this study is selected, too.
   ...
   *  This method acquires a database lock which must be explicitly freed by the
   * user. Implicitly the first instance within this series is selected, too.
  ...
  /** returns the Name Of Physicians Reading Study of the currently selected
   * study. May be called only if a valid study selection exists - see
   * selectStudy(). This method acquires a database lock which must be
   * explicitly freed by the user.
   *  @return Name Of Physicians Reading Study or NULL if absent or not
   * selected.
   */
   ...
   *  May be called only if a valid study selection exists - see selectStudy().
   *  This method acquires a database lock which must be explicitly freed by the
   * user. See the comments for getNumberOfStudies() about the validity period
   *  of the returned number.
   *  @return number of series in the current study.
   */
  Uint32 getNumberOfSeries();

  /** selects the series with the given index within the currently selected
   * study. This method acquires a database lock which must be explicitly freed
   * by the user. The selection remains valid until the database lock is removed
   * or the database is modified (see comments for getNumberOfStudies).
   ...
   *  This method acquires a database lock which must be explicitly freed by the
   * user. The selection remains valid until the database lock is removed or the
   * database is modified (see comments for getNumberOfStudies).
   *  @param seriesUID series instance UID of the image
   *  @return EC_Normal upon success, an error code otherwise.
   */
   ...
   *  May be called only if a valid series selection exists - see
   * selectSeries(). This method acquires a database lock which must be
   * explicitly freed by the user.
   ...
   *  the first examined instance (which is no image) is used (e.g.
   * PresentationState). May be called only if a valid series selection exists -
   * see selectSeries(). This method acquires a database lock which must be
   * explicitly freed by the user.
   ...
   * selectInstance(). This method acquires a database lock which must be
   * explicitly freed by the user.
   *  @return instance type
   */
  ...
  /** returns the Series Performing Physicians Name of the currently selected
   * series. May be called only if a valid series selection exists - see
   * selectSeries(). This method acquires a database lock which must be
   * explicitly freed by the user.
   *  @return Series Performing Physicians Name or NULL if absent or not
   * selected.
   */
  ...
  /** returns the number of instances (IODs) within the currently selected
   * series. May be called only if a valid series selection exists - see
   * selectSeries(). This method acquires a database lock which must be
   * explicitly freed by the user. See the comments for getNumberOfStudies()
   * about the validity period of the returned number.
   *  @return number of instances in the current series.
   */
  Uint32 getNumberOfInstances();

  /** selects the instance with the given index within the currently selected
   * series. This method acquires a database lock which must be explicitly freed
   * by the user. The selection remains valid until the database lock is removed
   * or the database is modified (see comments for getNumberOfStudies).
  ...
  /** selects the instance with the given UID within the currently selected
   * series. This method acquires a database lock which must be explicitly freed
   * by the user. The selection remains valid until the database lock is removed
   * or the database is modified (see comments for getNumberOfStudies).
  ...
  /** selects the instance with the given UID and SOP class over all studies and
   * series. Please note that in worst case all studies, series and instances
   * are examined. This method acquires a database lock which must be explicitly
   * freed by the user. The selection remains valid until the database lock is
   * removed or the database is modified (see comments for getNumberOfStudies).
   *  @param instanceUID SOP instance UID of the instance
   *  @param sopClassUID SOP class UID of the instance (might be NULL to be not
   * compared)
   *  @return EC_Normal upon success, an error code otherwise.
   */
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition selectInstance(const char* studyUID, const char* seriesUID,
                             const char* instanceUID);
   ...
   *  'reviewed' state (DVIF_objectIsNotNew). The status of the corresponding
   * series and study is updated automatically. This method acquires a database
   * lock which must be explicitly freed by the user. The database is modified -
   * any study, series or instance selection and the number of studies, series
   * and instances reported will become invalid since other processes may modify
   * the database before the exclusive lock is granted to this method.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition instanceReviewed(const char* studyUID, const char* seriesUID,
                               const char* instanceUID);

  /** deletes the given instance from the database. If the corresponding DICOM
   * file resides in the same directory as the index file, it is also removed.
   *  This method acquires a database lock which must be explicitly freed by the
   * user. The database is modified - any study, series or instance selection
   *  and the number of studies, series and instances reported will become
   * invalid since other processes may modify the database before the exclusive
   * lock is granted to this method.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition deleteInstance(const char* studyUID, const char* seriesUID,
                             const char* instanceUID);

  /** deletes the given series from the database. Any of the corresponding DICOM
   * files residing in the same directory as the index file are also removed.
   *  This method acquires a database lock which must be explicitly freed by the
   * user. The database is modified - any study, series or instance selection
   *  and the number of studies, series and instances reported will become
   * invalid since other processes may modify the database before the exclusive
  ...
  /** deletes the given study from the database. Any of the corresponding DICOM
   * files residing in the same directory as the index file are also removed.
   *  This method acquires a database lock which must be explicitly freed by the
   * user. The database is modified - any study, series or instance selection
   *  and the number of studies, series and instances reported will become
   ...
   *  The receiver processes will wait for incoming DICOM associations, receive
   * images and presentation states. Data will be stored in file and registered
   * in the database index file. Attention: Successful return of this method is
   * no guarantee that the network receiver has successfully started, because
   * certain errors (i.e. incorrect settings in the config file) will only be
   * noted in the network receiver process when running. On Unix platform,
   * successful return of this method means that the fork() used to start the
   * receiver was successful. On Win32 platforms, it means that the
   * CreateProcess() call was successful.
   *  @return EC_Normal if the receiver processes could be started, an error
   * code otherwise.
   */
   ...
   *  requesting a DICOM association with it and delivering a special "shutdown"
   * command. If for some reason the network receiver cannot be found (i.e.
   * because it has terminated abnormally), a TCP/IP timeout (several seconds)
   * may occur before this method returns.
   *  @return EC_Normal if the receiver process could be terminated, an error
   * code otherwise.
   */
  OFCondition terminateReceiver();

  /** starts the query/retrieve server process (Query/Retrieve Service Class
   * SCP). The query/retrieve process will wait for incoming DICOM associations,
   * serve queries and send the requested instances to the specified
   * destination.  Data will be taken from the same local database used to store
   * received instances and created presentation states, stored print objects
   * and hardcopy grayscale images. The configuration file which is required for
   * the query/retrieve process will be created automatically from the 'global'
   * configuration file each time this method is called, unless this automatical
   * creation is disabled (entry: [QUERYRETIUEVE] AutoCreateConfigFile = false).
   *  Attention: Successful return of this method is no guarantee that the
   * query/retrieve server has successfully started, because certain errors
   * (i.e. incorrect settings in the config file) will only be noted in the
   * query/retrieve process when running. On Unix platform, successful return of
   * this method means that the fork() used to start the receiver was
   * successful. On Win32 platforms, it means that the CreateProcess() call was
   * successful.
   *  @return EC_Normal if the query/retrieve process could be started, an error
   * code otherwise.
   */
  OFCondition startQueryRetrieveServer();

  /** terminates the query/retrieve server process (Query/Retrieve Service Class
   * SCP). This method attempts to terminate the query/retrieve process by
   * requesting a DICOM association with it and delivering a special "shutdown"
   * command. If for some reason the query/retrieve server cannot be found (i.e.
   * because it has terminated abnormally), a TCP/IP timeout (several seconds)
   * may occur before this method returns.
   ...
   *  call to this method. Any write access to the database (adding, deleting,
   * changing) is reported. This method works by modifying and checking the
   * "modification date/time" of the database index file. This method is not
   * affected by database locks and can be called at any time. Always returns
   * OFTrue for the first call after construction of the interface. Also returns
   * OFTrue if something goes wrong (i.e. if the method cannot make sure that
   * nothing has changed).
   *  @return OFTrue if the database has been modified since the last call to
   * this method.
   */
  OFBool newInstancesReceived();

  /** sends a complete study over network to a different DICOM peer.
   *  A separate application or process is launched to handle the send
   * operation. This call returns when the send operation has successfully been
   * launched. No information about the status or success of the transfer itself
   * is being made available.
   *  @param targetID symbolic identifier of the send target, must be one of the
   *     strings returned by getTargetID().
   *  @param studyUID Study Instance UID of the study to be sent. Must be a
   * study contained in the database.
   ...
   *  @param targetID symbolic identifier of the send target, must be one of the
   *     strings returned by getTargetID().
   *  @param studyUID Study Instance UID of the series to be sent. Must be a
   * series contained in the database.
   *  @param seriesUID Series Instance UID of the series to be sent. Must be a
   * series contained in the database.
   *  @return EC_Normal when the send process has successfully been launched,
   *     an error condition otherwise.
   */
  OFCondition sendSeries(const char* targetID, const char* studyUID,
                         const char* seriesUID) {
   ...
   *     an error condition otherwise.
   */
  OFCondition sendIOD(const char* targetID, const char* studyUID,
                      const char* seriesUID, const char* instanceUID);

  /** creates a dump of the contents of a DICOM file and displays it on-screen.
   *  A separate application or process is launched to handle the dump and
   * display. This call returns when the dump operation has successfully been
   * launched. No information about the status or success of the process itself
   * is being made available. This method does not acquire a database lock.
   ...
   * display. This call returns when the dump operation has successfully been
   * launched. No information about the status or success of the process itself
   * is being made available. This method acquires a database lock which must be
   * explicitly freed by the user.
   *  @param studyUID Study Instance UID of the IOD to be dumped. Must be an IOD
   *     contained in the database.
   *  @param seriesUID Series Instance UID of the IOD to be dumped. Must be an
   * IOD contained in the database.
   *  @param instanceUID SOP Instance UID of the IOD to be dumped. Must be an
   * IOD contained in the database.
   *  @return EC_Normal when the process has successfully been launched,
   *     an error condition otherwise.
   */
  OFCondition dumpIOD(const char* studyUID, const char* seriesUID,
                      const char* instanceUID);

  /** checks the contents of a DICOM file and displays an evaluation report on
   * the screen. A separate application or process is launched to handle the
   * evaluation and display. This call returns when the check operation has
   * successfully been launched. No information about the status or success of
   * the process itself is being made available. This method does not acquire a
   * database lock.
   ...
   * evaluation and display. This call returns when the check operation has
   * successfully been launched. No information about the status or success of
   * the process itself is being made available. This method acquires a database
   * lock which must be explicitly freed by the user.
   *  @param studyUID Study Instance UID of the IOD to be checked. Must be an
   * IOD contained in the database.
   *  @param seriesUID Series Instance UID of the IOD to be checked. Must be an
   * IOD contained in the database.
   *  @param instanceUID SOP Instance UID of the IOD to be checked. Must be an
   * IOD contained in the database.
   *  @return EC_Normal when the process has successfully been launched,
   *     an error condition otherwise.
   */
  OFCondition checkIOD(const char* studyUID, const char* seriesUID,
                       const char* instanceUID);
   ...
   *  @param aspectRatio the pixel aspect ratio as width/height. If omitted, a
   * pixel aspect ratio of 1/1 is assumed.
   *  @param explicitVR selects the transfer syntax to be written.
   *    True selects Explicit VR Little Endian, False selects Implicit VR Little
   * Endian.
   *  @param instanceUID optional parameter containing the SOP Instance UID to
   * be written. This parameter should be omitted unless the SOP Instance UID
   * needs to be controlled externally.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition saveDICOMImage(const char* filename, const void* pixelData,
   ...
   *  This method releases under any circumstances the database lock if it
   * exists.
   *  @param pixelData a pointer to the image data. Must contain at least
   *    width*height bytes of data.
   *  @param width the width of the image, must be <= 0xFFFF
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition saveDICOMImage(const void* pixelData, unsigned long width,
                             unsigned long height, double aspectRatio = 1.0);
  ...
  OFCondition saveHardcopyGrayscaleImage(
      ...
      unsigned long height, double aspectRatio = 1.0,
      OFBool explicitVR = OFTrue, const char* instanceUID = NULL);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition saveHardcopyGrayscaleImage(const void* pixelData,
                                         unsigned long width,
  ...
  /** loads a Stored Print object which is contained in the database into
   * memory. Attention: The current print job (Stored Print object) will be
   * deleted by doing this. This method acquires a database lock which must be
   * explicitly freed by the user.
   ...
   *  @param changeStatus if true the stored print object is marked 'reviewed'
   * (not new)
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition loadStoredPrint(const char* studyUID, const char* seriesUID,
                              const char* instanceUID,
                              OFBool changeStatus = OFFalse);

  /** loads a Stored Print object (which need not be contained in the database)
   * into memory. Attention: The current print job (Stored Print object) will be
   * deleted by doing this. This method does not acquire a database lock.
   ...
   *  @param filename the file name or path under which the image is saved.
   *  @param writeRequestedImageSize if false, the Requested Image Size
   * attributes are not written, e. g. because they are not supported by the
   * target printer.
   *  @param explicitVR selects the transfer syntax to be written.
   *    True selects Explicit VR Little Endian, False selects Implicit VR Little
   * Endian.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition saveStoredPrint(const char* filename,
  ...
  /** gets the number of Hardcopy Grayscaleimages currently registered by the
   * stored print object.
   *  @return number of images.
   */
  size_t getNumberOfPrintPreviews();

  /** loads a Hardcopy Grayscale image registered by the stored print object and
   * creates a preview. The preview bitmap is implicitly scaled to fit into the
   * rectangle specified by setMaxPrintPreviewWidthHeight().
   *  @param idx index of the image, must be < getNumberOfPrintPreviews()
   *  @param printLUT OFTrue if presentation LUT should be interpreted as a
   * print presentation LUT (default, in this case there is no implicit scaling
   * of the input width of the LUT and, therefore, the VOI transformation -
   * which is absent for print - is used), OFFalse otherwise (softcopy
   * interpretation of a presentation LUT)
   *  @param changeStatus if true the hardcopy grayscale image file is marked
   * 'reviewed' (not new)
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition loadPrintPreview(size_t idx, OFBool printLUT = OFTrue,
                               OFBool changeStatus = OFFalse);
   ...
   *  Larger images are scaled down (according to the pixel aspect ratio) to fit
   * into the specified rectangle. Attention: If the values differ from the the
   * previous ones the currently loaded hardcopy grayscale image (preview) is
   * automatically detroyed and has to be re-loaded.
   ...
   *  The return values depend on the current maximum preview bitmap
   * width/height values!
   *  @param width upon success, the bitmap width (in pixels) is returned in
   * this parameter
   *  @param height upon success, the bitmap height (in pixels) is returned in
   * this parameter
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getPrintPreviewWidthHeight(unsigned long& width,
                                         unsigned long& height);
  ...
  /** returns number of presentation states referencing the currently selected
   * image. If no instance is currently selected or the selected instance is a
   * presentation state, returns an error.
   *  @return number of presentation states, 0 if none available or an error
   * occurred
   */
  Uint32 getNumberOfPStates();

  /** selects and loads specified presentation state referencing the currently
   * selected image.
   *  @param idx index to be selected, must be < getNumberOfPStates()
   *  @param changeStatus if true the presentation state is marked 'reviewed'
   * (not new)
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition selectPState(Uint32 idx, OFBool changeStatus = OFFalse);

  /** returns description of specified presentation state referencing the
   ...
   */
  OFBool isDisplayTransformPossible(
   ...
   *  and communicate them to the printer using the DICOM Print Management
   * Service Class. Attention: Successful return of this method is no guarantee
   *  that the spooler has successfully started, because certain errors
   *  (i.e. incorrect settings in the config file) will only be noted in the
   * spooler process when running. On Unix platform, successful return of this
   * method means that the fork() used to start the spooler was successful. On
   * Win32 platforms, it means that the CreateProcess() call was successful.
   *  @return EC_Normal if the spooler process could be started, an error code
   * otherwise.
   */
  OFCondition startPrintSpooler();

  /** terminates the print spooler process. This method creates a "dummy"
   *  print job that request the print spooler to shutdown as soon as all other
   * pending print jobs are finished.
   ...
   *  The print server process will wait for incoming DICOM associations, handle
   * the DICOM print protcol, store data in file and stored print and grayscale
   *  image objects in the database index file.
   *  Attention: Successful return of this method is no guarantee that the print
   *  server has successfully started, because certain errors (i.e. incorrect
   * settings in the config file) will only be noted in the print server process
   * when running. On Unix platform, successful return of this method means that
   * the fork() used to start the server was successful. On Win32 platforms, it
   * means that the CreateProcess() call was successful.
   *  @return EC_Normal if the server process could be started, an error code
   * otherwise.
   */
  OFCondition startPrintServer();

  /** terminates the print server process (Basic Grayscale Print Management
   * SCP). This method attempts to terminate the print server process by
   * requesting a DICOM association with it and delivering a special "shutdown"
   * command. If for some reason the print server cannot be found (i.e. because
   * it has terminated abnormally), a TCP/IP timeout (several seconds) may occur
   * before this method returns.
   *  @return EC_Normal if the server process could be terminated, an error code
   * otherwise.
   */
  OFCondition terminatePrintServer();

  /** adds an existing DICOM image (should be Hardcopy Grayscale)
   *  that is already present in the image database to the current print image
   * queue without rendering it again. The "requested image size" option is not
   * used - the bitmap is treated as if the presentation mode was "SCALE TO
   * FIT".
   *  @param studyUID study instance UID of the image, as reported by
   * getStudyUID()
   *  @param seriesUID series instance UID of the image, as reported by
   * getSeriesUID()
   *  @param instanceUID SOP instance UID of the image, as reported by
   * getInstanceUID()
   *  @return EC_Normal upon success, an error code otherwise.
   */
   ...
   *  in the database as a "stored print" object. The Stored Print that is
   * printed does not contain all parameters of a print job. The following
   * parameters are taken from the current settings in this object: Target
   * printer, medium type, illumination and reflected ambient light.
   *  @param studyUID study instance UID of the Stored Print, as reported by
   * getStudyUID()
   *  @param seriesUID series instance UID of the Stored Print, as reported by
   * getSeriesUID()
   *  @param instanceUID SOP instance UID of the Stored Print, as reported by
   * getInstanceUID()
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition spoolStoredPrintFromDB(const char* studyUID,
                                     const char* seriesUID,
                                     const char* instanceUID);

  /** Initiates the creation of a DICOM Basic Film Session SOP Instance in the
   * printer. This method stores all Basic Film Session related attributes that
   * are managed by this object in a DICOM dataset and passes the result to the
   * embedded Stored Print object which manages the further communication.
   *  @param printHandler print communication handler, association must be open.
   *  @param plutInSession true if printer expects referenced presentation LUT
   * sequence, illumination and reflected ambient light in basic film session,
   * false if it expects them in basic film box.
   *  @return EC_Normal upon success, an error code otherwise.
   */
   ...
   *  @param value OFTrue if Prepend Date/Time is switched on, OFFalse
   * otherwise.
   */
   ...
   *  @param value OFTrue if Prepend Printer Name is switched on, OFFalse
   * otherwise.
   */
   ...
   *  If the user ID is known (i.e. specified in the configuration file) the
   * current structured report is verified (a verifying observer is added). If
   * the 'mode' parameter is set accordingly and the password is correct (see
   * verifyUserPassword) the report is also digitally signed. Please note that a
   * document can be verified/signed more than once, but only completed
   *  documents can be verified. After signing the report no modifications
   * should be performed before the object is stored in the database or a file.
   * Otherwise the digital signature would be corrupted. Therefore, the SOP
   * instance UID should be generated before calling this method. To digitally
   * sign the report it is required that DCMTK is configured and compiled with
   * the WITH_OPENSSL flag, otherwise only verification is available (returns an
   *  error code if mode differs from DVPSY_verify).
   *  @param userID symbolic user ID for given user, as returned by
   *    DVConfiguration::getUserID()
   *  @param passwd for user as entered in some GUI control
   *  @param mode flag specifying whether to verify only, verify and sign or
   * verify and sign and finalize the document. The difference between the
   * second and the third mode is that "finalize" always signs the entire
   * document whereas the other mode only signs the marked items (if any, all
   * items otherwise) and leaves out certain header attributes (e.g. the SOP
   * instance UID and the verifying observer sequence).
   *  @return status, EC_Normal if successful, an error code otherwise.
   */
  OFCondition verifyAndSignStructuredReport(const char* userID,
                                            const char* passwd,
  ...
  /** returns the combined status flag for the current image and presentation
   * state.
   ...
   *  @return number of digital signatures
   */
  unsigned long getNumberOfUntrustworthySignatures(
...
private:
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition exchangeImageAndPState(DVPresentationState* newState,
                                     DcmFileFormat* image,
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition createPrintJobFilenames(const char* printer, OFString& tempname,
                                      OFString& jobname);
   ...
   *    condition otherwise.
   */
  OFCondition startExternalApplication(const char* application,
                                       const char* filename);
  ...
  /** pointer to the current DICOM dataset containing the loaded presentation
   * state. Is NULL when the presentation state has been created "on the fly"
   * from image.
   */
  ...
  /** updates (hierarchical) status cache (propagates status information from
   * instances to series and from series to studies)
   */
  void updateStatusCache();

  /** returns pointer to study struct specified by given UIDs or to current
   * study
   */
  DVStudyCache::ItemStruct* getStudyStruct(const char* studyUID = NULL,
                                           const char* seriesUID = NULL);

  /** returns pointer to series struct specified by given UIDs or to current
   * series
   */
  ...
  /** returns pointer to instance struct specified by given UIDs or to current
   * instance
   */
  DVInstanceCache::ItemStruct* getInstanceStruct(
      const char* studyUID = NULL, const char* seriesUID = NULL,
      const char* instanceUID = NULL);
  ...
  /** conditionally deletes given image file (only if file resides in index.dat
   * directory)
   */
  ...
  /** modifies the review flag for one instance in the database (see method with
   * three UID parameters)
   */
  ...
  /** config file identifier of LUT currently selected as Display Presentation
   * LUT
   */
  ...
  /** true if reflected ambient light and illumination should be prepended to
   * annotation text
   */
