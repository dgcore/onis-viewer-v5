...
public:
  ...
  DVPSPresentationLUT* clone() {
    return new DVPSPresentationLUT(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the PresentationLUTContentSequence from which the
   * data is to be read
   *  @param withSOPInstance true if SOPinstanceUID should be read (when used
   * with Stored Print).
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the PresentationLUTContentSequence to which
   * the data is written
   *  @param withSOPInstance true if SOPinstanceUID should be written (when used
   * with Stored Print).
   *  @return EC_Normal if successful, an error code otherwise.
   */
  ...
  /** returns the LUT explanation of the presentation LUT if it exists and is
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setLUT(DcmUnsignedShort& lutDescriptor, DcmUnsignedShort& lutData,
   ...
   *  @param image the DicomImage for which the presentation transform is to be
   * set.
   *  @param printLUT OFTrue if presentation LUT is activated for print bitmap
   * rendering (in this case there is no implicit scaling of the input width of
   * the LUT and, therefore, the VOI transformation - which is absent for print
   * - is used), OFFalse otherwise (softcopy rendering, default)
   *  @return OFTrue if successful, OFFalse otherwise.
   */
  OFBool activate(DicomImage* image, OFBool printLUT = OFFalse);

  /** activates the inverse LUT of the current presentation LUT (if any) in the
   * given DicomImage. Presentation LUT shape is not supported by this method.
   *  @param image the DicomImage for which the inverse presentation LUT is to
   * be set.
   *  @return OFTrue if successful, OFFalse otherwise.
   */
   ...
   *  matches the current image bit depth in number of entries and first value
   * mapped.
   ...
   */
  OFBool printSCPCreate(DcmDataset* rqDataset, T_DIMSE_Message& rsp,
                        DcmDataset*& rspDataset, OFBool matchRequired,
