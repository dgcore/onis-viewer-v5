...
/** the list of graphic objects contained in a presentation state (internal use
 * only). This class manages the data structures comprising one complete Graphic
 * Object Sequence which is contained in one item of the Graphic Annotation
 * Sequence in a Presentation State object.
 */
...
public:
  ...
  DVPSGraphicObject_PList* clone() {
    return new DVPSGraphicObject_PList(*this);
  }
  ...
  /** writes the list of graphic objects managed by this object to a DICOM
