...
/** an item of the referenced series sequence in a presentation state (internal
 * use only). This class manages the data structures comprising one item of the
...
public:
  ...
  DVPSReferencedSeries* clone() {
    return new DVPSReferencedSeries(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the ReferencedSeriesSequence from which the data
   * is to be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the ReferencedSeriesSequence to which the data
   * is written
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition write(DcmItem& dset);

  /** check if the passed SOP Class UID is equal to the ones stored in this
   * object. This method checks whether this object contains at least one image
   * reference and whether all image references use the same SOP class UID as
   * the one passed. If sopclassuid is empty, the referencedSOPClassUID of the
   * first image reference is assigned to it.
   ...
   *  @param aetitle the retrieveAETitle, must be a valid DICOM 'AE' value.
   * Default: value absent.
   *  @param filesetID the storageMediaFileSetID, must be a valid DICOM 'SH'
   * value. Default: value absent.
   *  @param filesetUID the storageMediaFileSetUID, must be a valid DICOM UID.
   * Default: value absent.
   */
  void setRetrieveLocation(const char* aetitle = NULL,
   ...
   *  @param sopclassUID a pointer to the Image SOP Class UID, which is copied
   * into the new image reference
   *  @param instanceUID a pointer to the Image SOP Instance UID, which is
   * copied into the new image reference
  ...
  size_t numberOfImageReferences() const {
    return referencedImageList.size();
  }
