...
/** an item of the referenced image sequence in a presentation state (internal
 * use only). This class manages the data structures comprising one item of the
 * Reference Image Sequence which is contained in the Referenced Series Sequence
 * in a Presentation State object.
 */
...
public:
  ...
  DVPSReferencedImage* clone() {
    return new DVPSReferencedImage(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the ReferencedImageSequence from which the data is
   * to be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the ReferencedImageSequence to which the data
   * is written
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition write(DcmItem& dset);

  /** check if the passed SOP Class UID is equal to the one stored in this
   * object. If sopclassuid is empty, the referencedSOPClassUID is assigned to
   * it and the method returns OFTrue. Otherwise the passed UID is compared with
   * the referencedSOPClassUID. OFTrue is returned if they are equal, OFFalse
   * otherwise. This method is used to check whether all image references in a
   * presentation state share the same SOP Class UID.
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getImageReference(OFString& sopclassUID, OFString& instanceUID,
                                OFString& frames);
   ...
   *  listed in the referencedFrameNumber attribute or if the
   * referencedFrameNumber is empty.
   *  @param frame frame number to be checked
   *  @return OFTrue if the image reference applies to the given frame number,
   * OFFalse otherwise.
   */
  OFBool appliesToFrame(unsigned long frame);

  /** checks whether this image reference applies exactly to the given frame
   * number. This is the case if the referencedFrameNumber only contains the
   * given frame number.
   *  @param frame frame number to be checked
   *  @return OFTrue if the image reference applies only to the given frame
   * number, OFFalse otherwise.
   */
   ...
   *  @return OFTrue if the image reference applies to all frames, OFFalse
   * otherwise.
   */
