...
/** a curve or overlay activation layer in a presentation state (internal use
 * only). This class manages the data structures comprising a single curve
...
public:
  ...
  /** clone method.
   *  @return a pointer to a new DVPSOverlayCurveActivationLayer object
   * containing a copy of this object.
   */
  DVPSOverlayCurveActivationLayer* clone() {
    return new DVPSOverlayCurveActivationLayer(*this);
  }

  /// destructor
  virtual ~DVPSOverlayCurveActivationLayer();

  /** reads the activation layer for the specified repeating group from a DICOM
   * dataset. The DICOM elements of the Overlay/Curve Activation Layer module
   * are copied from the dataset to this object. The completeness of the module
   * is checked. If this method returns an error code, the object is in
   * undefined state afterwards.
   *  @param dset the DICOM dataset from which the activation layer is to be
   * read
   ...
   *  @param aLayer a pointer to the activation layer name, which is copied into
   * this object.
   */
   ...
   *  @return OFTrue if the activation matches the passed repeating group,
   * OFFalse otherwise.
   */
...
private:
  ...
  DVPSOverlayCurveActivationLayer& operator=(
