...
public:
  ...
  DVPSPresentationLUT_PList* clone() {
    return new DVPSPresentationLUT_PList(*this);
  }

  /// destructor
  virtual ~DVPSPresentationLUT_PList();

  /** reads a list of Presentation LUTs (Presentation LUT Content Sequence) from
   * a DICOM dataset. The DICOM elements of the image references item are copied
   * from the dataset to this object. The completeness of all items (presence of
   * all required elements, value multiplicity) is checked. If this method
   * returns an error code, the object is in undefined state afterwards.
   *  @param dset the DICOM dataset from which the sequence is to be read
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition read(DcmItem& dset);

  /** writes the list of Presentation LUTs managed by this object to a DICOM
   ...
   *  @param filmBox Presentation LUT UID reference on film box level, may be
   * NULL.
   ...
   * inversed.
   *  @return UID of referenced Presentation LUT. May be NULL (if input was
   * NULL).
   */
   ...
   *  The results of the N-DELETE operation are stored in the object passed as
   * rsp.
