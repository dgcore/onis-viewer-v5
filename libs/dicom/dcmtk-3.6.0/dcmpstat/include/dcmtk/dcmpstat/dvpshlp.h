...
public:
   ...
   */
  static OFCondition loadFileFormat(const char* filename,
   ...
   *  @param value the value to be inserted. If omitted, an empty element is
   * created.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  static OFCondition putStringValue(DcmItem* item, DcmTagKey tag,
   ...
   */
  static void setDefault(OFCondition& result, DcmElement& a_name,
                         const char* a_value);

  /** static helper method that checks whether the given sequence contains an
   *  item with a ReferencedSOPClassUID element that matches the given UID
   * string.
   *  @param seq sequence to be searched, should be a
   * PrintManagementCapabilitiesSequence.
   *  @param uid UID string
   *  @return OFTrue if found, OFFalse otherwise. Returns OFFalse if uid is
   * NULL.
   */
   ...
   *  contains a ReferencedSOPClassUID element with the given UID string as
   * value.
   ...
   */
  static OFCondition addReferencedUIDItem(DcmSequenceOfItems& seq,
                                          const char* uid);
};
