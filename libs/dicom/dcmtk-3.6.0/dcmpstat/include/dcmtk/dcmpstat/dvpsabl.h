...
public:
  ...
  DVPSAnnotationContent_PList* clone() {
    return new DVPSAnnotationContent_PList(*this);
  }

  /// destructor
  virtual ~DVPSAnnotationContent_PList();

  /** reads a list of annotations (AnnotationContentSequence) from a DICOM
   * dataset. The DICOM elements of the annotations are copied from the dataset
   ...
   *  the DICOM dataset.
   *  @param dset the DICOM dataset to which the AnnotationContentSequence is
   * written
   *  @return EC_Normal if successful, an error code otherwise.
   */
  ...
  /** creates a new annotation object and sets the content of this annotation
   * object.
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addAnnotationBox(const char* instanceuid, const char* text,
                               Uint16 position);
  ...
  /** deletes multiple of the registered annotations, starting with the first
   * one.
