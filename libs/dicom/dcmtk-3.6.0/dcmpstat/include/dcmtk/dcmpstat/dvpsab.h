...
public:
  ...
  DVPSAnnotationContent* clone() {
    return new DVPSAnnotationContent(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the AnnotationContentSequence from which the data
   * is to be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the AnnotationContentSequence to which the
   * data is written
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setContent(const char* instanceuid, const char* text,
                         Uint16 position);
