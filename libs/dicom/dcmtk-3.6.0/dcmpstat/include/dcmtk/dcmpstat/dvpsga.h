...
/** an item of the graphic annotation sequence in a presentation state (internal
...
public:
  ...
  DVPSGraphicAnnotation* clone() {
    return new DVPSGraphicAnnotation(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the GraphicAnnotationSequence from which the data
   * is to be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the GraphicAnnotationSequence to which the
   * data is written
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  @param aLayer a pointer to the annotation layer name, which is copied into
   * this object.
   */
   ...
   * (DVPSB_currentFrame or DVPSX_currentImage)
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  An annotation layer is empty when it contains no text object and no
   * graphic object.
   *  @return OFTrue if empty.
   */
  OFBool isEmpty();

  /** checks if this annotation layer is applicable to the given image and
   * frame.
   *  @param instanceUID SOP instance UID of the current image
   *  @param frame number of the current frame
   *  @param applicability the required (minimum) applicability of the
   * reference. Default: annotation layer applies to the current frame of the
   * current image.
