...
/* SOP instance UID used by the Print SCP when creating a default IDENTITY
 * Presentation LUT */
#define WELLKNOWN_IDENTITY_PLUT_UID \
  "1.2.276.0.7230010.3.4.1915765545.18030.917282194.1"

/* SOP class UID used by the Print SCP when storing a DIMSE log as a DICOM file
 */
#define PSTAT_DIMSE_LOG_STORAGE_UID \
  "1.2.276.0.7230010.3.4.1915765545.18030.917282194.2"
...
#define ADD_TO_PDATASET(a_type, a_name)  \
  if (writeresult == EC_Normal) {        \
    delem = new a_type(a_name);          \
    if (delem)                           \
      rspDataset->insert(delem, OFTrue); \
    else                                 \
...
#define READ_FROM_DATASET(a_type, a_name)                          \
  stack.clear();                                                   \
  if (EC_Normal == dset.search((DcmTagKey&)a_name.getTag(), stack, \
...
#define READ_FROM_DATASET2(a_type, a_name)                          \
  stack.clear();                                                    \
  if (EC_Normal == item->search((DcmTagKey&)a_name.getTag(), stack, \
                                ESM_fromHere, OFFalse)) {           \
    a_name = *((a_type*)(stack.top()));                             \
  }
...
#define READ_FROM_PDATASET(a_type, a_name)                                \
  stack.clear();                                                          \
  if (rqDataset &&                                                        \
      (EC_Normal == rqDataset->search((DcmTagKey&)a_name.getTag(), stack, \
                                      ESM_fromHere, OFFalse))) {          \
    a_name = *((a_type*)(stack.top()));                                   \
  }
...
#define SET_UID(a_name)                                            \
  if (result == EC_Normal) {                                       \
    if (a_name.getLength() == 0)                                   \
      result = a_name.putString(dcmGenerateUniqueIdentifier(uid)); \
  }
