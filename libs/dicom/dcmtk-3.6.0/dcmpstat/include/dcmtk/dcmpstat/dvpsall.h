...
/** the list of curve and overlay activation layers contained in a presentation
 * state (internal use only). This class manages the data structures comprising
 * the list of curve activation layers and overlay activation layers (all
 * instances of the Curve Activation Layer Module and Overlay Activation Layer
 * Module repeating elements) contained in a Presentation State object.
 */
...
public:
  ...
  /// copy constructor
  DVPSOverlayCurveActivationLayer_PList(
      const DVPSOverlayCurveActivationLayer_PList& copy);

  /** clone method.
   *  @return a pointer to a new DVPSOverlayCurveActivationLayer_PList object
   * containing a deep copy of this object.
   */
  DVPSOverlayCurveActivationLayer_PList* clone() {
    return new DVPSOverlayCurveActivationLayer_PList(*this);
  }
   ...
   *  The DICOM elements of the activations are copied from the dataset to this
   * object. If this method returns an error code, the object is in undefined
   * state afterwards.
  ...
  /** writes the curve and overlay activations managed by this object to a DICOM
   ...
   */
  OFCondition createFromImage(DcmItem& dset, DVPSGraphicLayer_PList& gLayerList,
   ...
   *  @param isCurve if OFTrue, curves are counted, otherwise overlays are
   * counted.
   *  @return number of curves or overlays on the given layer.
   */
  size_t getNumberOfActivations(const char* layer, OFBool isCurve);

  /** gets the repeating group of the overlay/curve activation with the given
   * index on the given layer. If the activation or the graphic layer does not
   * exist, 0 is returned.
   *  @param layer name of the graphic layer
   *  @param idx index of the object, must be < getNumberOfActivations(layer,
   * isCurve)
   *  @param isCurve if OFTrue, curves are searched, otherwise overlays are
   * searched.
...
private:
  ...
  DVPSOverlayCurveActivationLayer_PList& operator=(
      const DVPSOverlayCurveActivationLayer_PList&);
