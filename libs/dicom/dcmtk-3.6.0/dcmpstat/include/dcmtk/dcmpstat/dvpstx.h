...
/** an item of the text object sequence in a presentation state (internal use
 * only). This class manages the data structures comprising one item of the Text
 * Object Sequence which is contained in the Graphic Annotation Sequence in a
...
public:
  ...
  DVPSTextObject* clone() {
    return new DVPSTextObject(*this);
  }
   ...
   * afterwards.
   *  @param dset the item of the TextObjectSequence from which the data is to
   * be read
   ...
   *  the DICOM dataset.
   *  @param dset the the item of the TextObjectSequence to which the data is
   * written
   *  @return EC_Normal if successful, an error code otherwise.
   */
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setAnchorPoint(double x, double y, DVPSannotationUnit unit,
                             OFBool isVisible);
   ...
   *  @param justification bounding box horizontal justification
   * (left/right/center)
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition setBoundingBox(double TLHC_x, double TLHC_y, double BRHC_x,
                             double BRHC_y, DVPSannotationUnit unit,
   ...
   *  Attention: A text object must always contain either anchor point, bounding
   * box or both. This property is not asserted by the text object itself.
   */
   ...
   *  May only be called when a bounding box is present
   * (haveBoundingBox()==OFTrue)
   ...
   *  May only be called when an anchor point is present
   * (haveAnchorPoint()==OFTrue)
