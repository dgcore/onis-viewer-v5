...
public:
  ...
  /** returns the (currently hard-coded) SOP Class UID of the Presentation
   * State.
   ...
   *  @param seriesUID the Series Instance UID of the image reference to be
   * added.
   *  @param sopclassUID the SOP class UID of the image reference to be added.
   *  @param instanceUID the SOP instance UID of the image reference to be
   * added.
   *  @param frames a list of frame numbers in DICOM IS format
   *    (integer numbers separated by '\' characters). Default: frame numbers
   * absent. The frame numbers are required if the referenced image is a
   * multiframe image.
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageReference(const char* studyUID, const char* seriesUID,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition addImageReference(DcmItem& dset, const char* aetitle = NULL,
                                const char* filesetID = NULL,
                                const char* filesetUID = NULL);

  /** removes a reference to an image from this presentation state.
   *  @param studyUID the Study Instance UID of the image reference to be
   * removed.
   *  @param seriesUID the Series Instance UID of the image reference to be
   * removed.
   *  @param instanceUID the SOP instance UID of the image reference to be
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition removeImageReference(const char* studyUID, const char* seriesUID,
                                   const char* instanceUID);
   ...
   *  @param filesetID the series storageMediaFileSetID is returned in this
   * string
   *  @param filesetUID the series storageMediaFileSetUID is returned in this
   * string
   *  @return EC_Normal if successful, an error code otherwise.
   */
  OFCondition getImageReference(size_t idx, OFString& studyUID,
                                OFString& seriesUID, OFString& sopclassUID,
                                OFString& instanceUID, OFString& frames,
                                OFString& aetitle, OFString& filesetID,
                                OFString& filesetUID);
  ...
  DVPSPresentationLUTType getPresentationLUT() {
    return presentationLUT.getType();
  }
  ...
  OFBool havePresentationLookupTable() {
    return presentationLUT.haveTable();
  }
  ...
  const char* getCurrentPresentationLUTExplanation() {
    return presentationLUT.getCurrentExplanation();
  }
  ...
  const char* getPresentationLUTExplanation() {
    return presentationLUT.getLUTExplanation();
  }
   ...
   *  @param idx the index of the shutter point, must be <
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getGraphicLayerRecommendedDisplayValueGray(size_t idx,
                                                         Uint16& gray);
   ...
   *  @param idx index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param r returns the R component of the recommended display value as
   * unsigned 16-bit P-value
   *  @param g returns the G component of the recommended display value as
   * unsigned 16-bit P-value
   *  @param b returns the B component of the recommended display value as
   * unsigned 16-bit P-value
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getGraphicLayerRecommendedDisplayValueRGB(size_t idx, Uint16& r,
                                                        Uint16& g, Uint16& b);

  /** set graphic layer recommended grayscale display value for the given
   * graphic layer. This method does not affect (set or modify) the recommended
   * RGB display value which should be set separately.
   *  @param idx index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param gray the recommended display value as an unsigned 16-bit P-value
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition setGraphicLayerRecommendedDisplayValueGray(size_t idx,
                                                         Uint16 gray);

  /** set graphic layer recommended RGB display value for the given graphic
   * layer. This method does not affect (set or modify) the recommended
   * grayscale display value which should be set separately.
   *  @param idx index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param r the R component of the recommended display value as unsigned
   * 16-bit P-value
   *  @param g the G component of the recommended display value as unsigned
   * 16-bit P-value
   *  @param b the B component of the recommended display value as unsigned
   * 16-bit P-value
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition setGraphicLayerRecommendedDisplayValueRGB(size_t idx, Uint16 r,
                                                        Uint16 g, Uint16 b);
   ...
   *  @param monochrome if true the monochrome recommended display value is
   * removed
   */
  void removeGraphicLayerRecommendedDisplayValue(size_t idx, OFBool rgb,
                                                 OFBool monochrome);

  /** assigns a new unique name to the given graphic layer.
   *  The new name must be unique, otherwise an error code is returned.
   *  Upon success, all references (for graphic annotations, curves and
   * overlays) to the given graphic layer are also renamed so that the
   * presentation state remains consistent.
   *  @param idx index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param name the new name of the graphic layer. Must be a valid DICOM Code
   * String.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  @param idx index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param descr description of the graphic layer. Must be a valid DICOM Long
   * String.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  the graphic layers.
   *  @param idx1 index of the first graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param idx2 index of the second graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  @param gLayer the name of the graphic layer. Must be a valid DICOM Code
   * String.
   ...
   *  @param layer index of the graphic layer, must be <
   * getNumberOfGraphicLayers()
   *  @param idx index of the curve activation, must be <
   * getNumberOfCurves(layer)
   *  @return EC_Normal upon success, an error code otherwise
   */
  ...
  /** gets the repeating group number of the given overlay in the presentation
   * state.
  ...
  /** gets the overlay description of the given overlay in the presentation
   * state.
   *  @param idx index of the overlay, must be <
   * getNumberOfOverlaysInPresentationState().
   ...
   *  @param idx index of the overlay, must be <
   * getNumberOfOverlaysInPresentationState().
   *  @return OFTrue if overlay exists and is activated as bitmap shutter,
   * OFFalse otherwise.
   */
   ...
   *  @param old_layer index of the graphic layer on which the curve is,
   *    must be < getNumberOfGraphicLayers()
   *  @param idx index of the overlay activation, must be <
   * getNumberOfActiveOverlays(layer)
   *  @param new_layer index of the graphic layer to which the curve is moved,
   *    must be < getNumberOfGraphicLayers()
   *  @return EC_Normal upon success, an error code otherwise
   */
...
protected:
   ...
   * There must never be more that one Presentation LUT for one Presentation
   * State, therefore we need not save a list of LUTs.
   */
   ...
   * "required if display shutter to be applied and BitmapDispShutter not
   * present"
   */
   ...
   */
  /// Overlay/Curve Activation Layer(s),
  /// Module=Overlay_Activation/Curve_Activation
