...
public:
  ...
  DVPSOverlay_PList* clone() {
    return new DVPSOverlay_PList(*this);
  }
   ...
   *  @groupInItem the repeating group 0x6000..0x61F (even) of the overlay to be
   * read
   *  @param newGroup repeating group number 0x6000-0x601F (even) to be used for
   *    the overlay in the presentation state.
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition addOverlay(DcmItem& overlayIOD, Uint16 groupInItem,
                         Uint16 newGroup);
