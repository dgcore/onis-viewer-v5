...
public:
  ...
  struct ItemStruct {
     ...
     */
    ItemStruct(const OFString& uid, const int pos,
               const DVIFhierarchyStatus status, const DVPSInstanceType type,
               const int size, const OFString& filename)
        : UID(uid),
          Pos(pos),
          Status(status),
          Type(type),
  ...
  /** Constructor
   */
  DVInstanceCache() : List(), Iterator(), OldIterator() {
   ...
   *          OFFalse if last visited position was invalid or the last one in
   *the list
   */
  inline OFBool reset() {
    OFBool result = OFFalse;
   ...
   *  @param  filename  filename of instance
   */
  inline void addItem(const OFString& uid, const int pos,
                      const DVIFhierarchyStatus status,
                      const DVPSInstanceType type, const int size,
...
public:
  ...
  struct ItemStruct {
    ...
    ItemStruct(const OFString& uid,
        ...
        : UID(uid), Status(status), Type(type), List() {}
  ...
  /** Constructor.
   */
  DVSeriesCache() : List(), Iterator(), OldIterator() {
    Iterator = OldIterator = List.end();
  }
