...
DVPSIPCMessage::DVPSIPCMessage(const DVPSIPCMessage& copy)
    : messageType(copy.messageType),
      ...
      payload(NULL) {
  payload = new unsigned char[payloadAllocated];
  OFBitmanipTemplate<unsigned char>::copyMem(copy.payload, payload,
                                             payloadUsed);
}
...
void DVPSIPCMessage::resizePayload(Uint32 i) {
  ...
  while (payloadAllocated < requiredSize)
    payloadAllocated += PAYLOAD_ALLOCATION_UNIT;
...
void DVPSIPCMessage::addStringToPayload(const char* str) {
  ...
  // write pad bytes
  for (Uint32 i = 0; i < padBytes; i++)
    *(payload + payloadUsed++) = 0;
  return;
}
...
void DVPSIPCMessage::addIntToPayload(Uint32 i) {
  ...
  *(Uint32*)target = i;
  ...
  swapIfNecessary(EBO_BigEndian, gLocalByteOrder, target, sizeof(Uint32),
                  sizeof(Uint32));
...
OFBool DVPSIPCMessage::extractStringFromPayload(OFString& str) {
  Uint32 length = 0;
  if (!extractIntFromPayload(length))
    return OFFalse;
  // check if we have sufficient data available
  if (payloadReadOffset + length > payloadUsed)
    return OFFalse;

  str = (const char*)(payload + payloadReadOffset);  // guaranteed to be zero
                                                     // terminated string
...
OFBool DVPSIPCMessage::extractIntFromPayload(Uint32& i) {
  // check if we have sufficient data available
  if (payloadReadOffset + sizeof(Uint32) > payloadUsed)
    return OFFalse;

  // copy integer into temporary buffer and adjust byte order there
  unsigned char* temp =
      new unsigned char[sizeof(Uint32) +
                        8];  // allocate a bit more than needed to be safe
  OFBitmanipTemplate<unsigned char>::copyMem(payload + payloadReadOffset, temp,
                                             sizeof(Uint32));
  swapIfNecessary(gLocalByteOrder, EBO_BigEndian, temp, sizeof(Uint32),
                  sizeof(Uint32));
...
OFBool DVPSIPCMessage::send(DcmTransportConnection& connection) {
  ...
  *(Uint32*)(payload + sizeof(Uint32)) = (payloadUsed - PAYLOAD_OFFSET);
  swapIfNecessary(EBO_BigEndian, gLocalByteOrder, payload, 2 * sizeof(Uint32),
                  sizeof(Uint32));

  // send
  if (connection.write(payload, (size_t)payloadUsed) <= 0)
    return OFFalse;
  return OFTrue;
}
...
OFBool DVPSIPCMessage::receive(DcmTransportConnection& connection) {
  ...
  if (connection.read(payload, 2 * sizeof(Uint32)) <= 0)
    return OFFalse;
  swapIfNecessary(gLocalByteOrder, EBO_BigEndian, payload, 2 * sizeof(Uint32),
                  sizeof(Uint32));
...
DVPSIPCClient::DVPSIPCClient(Uint32 clientType, const char* txt, int thePort,
                             OFBool keepOpen)
    : port(thePort),
      serverActive(OFTrue),
      applicationID(0),
  ...
  if (performTransaction(msg)) {
    if ((msg.getMessageType() != DVPSIPCMessage::assignApplicationID) ||
        (!msg.extractIntFromPayload(applicationID))) {
      // protocol violation
