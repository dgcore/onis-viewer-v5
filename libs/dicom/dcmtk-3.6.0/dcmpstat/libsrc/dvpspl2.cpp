...
OFBool DVPSPresentationLUT::compareDiLookupTable(DiLookupTable* lut) {
  if ((presentationLUT == DVPSP_table) && lut &&
      (0 == lut->compareLUT(presentationLUTData, presentationLUTDescriptor)))
    return OFTrue;
  return OFFalse;
}
...
DiLookupTable* DVPSPresentationLUT::createDiLookupTable() {
  ...
  if (presentationLUT == DVPSP_table)
    result = new DiLookupTable(presentationLUTData, presentationLUTDescriptor);
  return result;
}
...
OFCondition DVPSPresentationLUT::invert() {
  ...
  switch (presentationLUT) {
    ...
    case DVPSP_table:
      ...
      if (haveTable()) {
        DiLookupTable* lut =
            new DiLookupTable(presentationLUTData, presentationLUTDescriptor);
        if (lut && (lut->mirrorTable(0x2)))
          status = EC_Normal;  // flag = 0x2: mirror only original LUT data
        delete lut;
      }
      break;
    case DVPSP_lin_od:
...
OFBool DVPSPresentationLUT::activate(DicomImage* image, OFBool printLUT) {
  ...
  switch (presentationLUT) {
    case DVPSP_identity:
      ...
      if (!result)
        DCMPSTAT_INFO(
            "unable to set identity presentation LUT shape, ignoring.");
      break;
    case DVPSP_inverse:
      if (!printLUT)
      ...
      if (!result)
        DCMPSTAT_INFO(
            "unable to set inverse presentation LUT shape, ignoring.");
      break;
    case DVPSP_lin_od:
      result = image->setPresentationLutShape(ESP_LinOD);
      if (!result)
        DCMPSTAT_INFO(
            "unable to set linear optical density presentation LUT shape, "
            "ignoring.");
      break;
    case DVPSP_table:
      if (printLUT)
        result =
            image->setVoiLut(presentationLUTData, presentationLUTDescriptor,
                             &presentationLUTExplanation);
      else
        result = image->setPresentationLut(presentationLUTData,
...
OFBool DVPSPresentationLUT::activateInverseLUT(DicomImage* image) {
  ...
  if ((image != NULL) && (presentationLUT == DVPSP_table)) {
    result = image->setInversePresentationLut(presentationLUTData,
                                              presentationLUTDescriptor);
    if (!result)
  ...
  switch (presentationLUT) {
    ...
    case DVPSP_table:
      if ((presentationLUTDescriptor.getVM() == 3) &&
        ...
        DiLookupTable* lut =
            new DiLookupTable(presentationLUTData, presentationLUTDescriptor);
        if (lut && (lut->getFirstValue() > lut->getLastValue()))
          result = OFTrue;
        delete lut;
      }
      break;
