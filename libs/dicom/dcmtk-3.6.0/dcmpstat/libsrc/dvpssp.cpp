...
DVPSStoredPrint::DVPSStoredPrint(const DVPSStoredPrint& copy)
    : patientName(copy.patientName),
      ...
      referencedPresentationLUTInstanceUID(
          copy.referencedPresentationLUTInstanceUID),
      referencedPresentationLUTAlignment(
          copy.referencedPresentationLUTAlignment),
...
OFBool DVPSStoredPrint::isImageStorageSOPClass(OFString& sopclassuid) {
  for (int i = 0; i < numberOfDcmImageSOPClassUIDs; i++) {
    if (dcmImageSOPClassUIDs[i] && (sopclassuid == dcmImageSOPClassUIDs[i]))
      return OFTrue;
  }
  return OFFalse;
}
...
OFCondition DVPSStoredPrint::read(DcmItem& dset) {
  ...
  if (EC_Normal == result)
    result = presentationLUTList.read(dset);

  if (result == EC_Normal) {
    stack.clear();
    if (EC_Normal == dset.search(DCM_RETIRED_FilmBoxContentSequence, stack,
      ...
      if (seq->card() == 1) {
        ...
        // if there is any reference, it must refer to one of the presentation
        // LUTs we are managing.
        stack.clear();
        if (EC_Normal == item->search(DCM_ReferencedPresentationLUTSequence,
                                      stack, ESM_fromHere, OFFalse)) {
          seq = (DcmSequenceOfItems*)stack.top();
          if (seq->card() == 1) {
            item = seq->getItem(0);
            stack.clear();
            READ_FROM_DATASET2(DcmUniqueIdentifier,
                               referencedPresentationLUTInstanceUID)
            if (referencedPresentationLUTInstanceUID.getLength() > 0) {
              referencedPresentationLUTInstanceUID.getOFString(aString, 0);
              if (NULL ==
                  presentationLUTList.findPresentationLUT(aString.c_str())) {
                result = EC_IllegalCall;
                DCMPSTAT_INFO(
                    "FilmBoxContentSequence presentation LUT reference cannot "
                    "be resolved");
              }
            }
          } else {
            ...
            DCMPSTAT_INFO(
                "found FilmBoxContentSequence in Stored Print with "
                "ReferencedPresentationLUTSequence number of items != 1");
          }
        }
      } else {
        result = EC_TagNotFound;
        DCMPSTAT_INFO(
            "found FilmBoxContentSequence in Stored Print with number of items "
            "!= 1");
      }
    }
  }
  ...
  if (EC_Normal == result)
    result = imageBoxContentList.read(dset, presentationLUTList);

  if (EC_Normal == result)
    result = annotationContentList.read(dset);

  /* Now perform basic sanity checks */

  if (result == EC_Normal) {
    if ((studyInstanceUID.getLength() == 0) ||
        (studyInstanceUID.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO("StudyInstanceUID missing or incorrect in Stored Print");
    }
    if ((seriesInstanceUID.getLength() == 0) ||
        (seriesInstanceUID.getVM() != 1)) {
      result = EC_TagNotFound;
  ...
  if (result == EC_Normal) {
    stack.clear();
    if (EC_Normal ==
        dset.search(DCM_RETIRED_PrintManagementCapabilitiesSequence, stack,
                    ESM_fromHere, OFFalse)) {
      ...
      if (EC_Normal == result) {
        if (!haveFilmBox) {
          result = EC_IllegalCall;
          DCMPSTAT_INFO(
              "Film Box SOP Class not referenced in "
              "PrintManagementCapabilitiesSequence");
        }
        if (!haveGrayscaleImageBox) {
          result = EC_IllegalCall;
          DCMPSTAT_INFO(
              "Basic Grayscale Image Box SOP Class not referenced in "
              "PrintManagementCapabilitiesSequence");
        }
        if (!haveImageStorage) {
          result = EC_IllegalCall;
          DCMPSTAT_INFO(
              "No Image Storage SOP Class referenced in "
              "PrintManagementCapabilitiesSequence");
        }
      }
    } else {
  ...
  if (result == EC_Normal) {
    ...
    printerName.clear();
    stack.clear();
    if (EC_Normal == dset.search(DCM_RETIRED_PrinterCharacteristicsSequence,
                                 stack, ESM_fromHere, OFFalse)) {
      seq = (DcmSequenceOfItems*)stack.top();
      if (seq->card() > 0) {
        item = seq->getItem(0);
    ...
    if (printerName.getLength() == 0)
      DCMPSTAT_INFO(
          "PrinterName missing or incorrect in PrinterCharacteristicsSequence");
  }
...
OFCondition DVPSStoredPrint::createDefaultValues() {
  ...
  if ((result == EC_Normal) && (sOPInstanceUID.getLength() == 0)) {
    ...
    if (result == EC_Normal)
      result = instanceCreationDate.putString(aString.c_str());
    DVPSHelper::currentTime(aString);
    if (result == EC_Normal)
      result = instanceCreationTime.putString(aString.c_str());
  }

  if ((result == EC_Normal) && (studyInstanceUID.getLength() == 0)) {
    result = studyInstanceUID.putString(
        dcmGenerateUniqueIdentifier(uid, SITE_STUDY_UID_ROOT));
    DVPSHelper::currentDate(aString);
    if (result == EC_Normal)
      result = studyDate.putString(aString.c_str());
    DVPSHelper::currentTime(aString);
    if (result == EC_Normal)
      result = studyTime.putString(aString.c_str());
  }

  if ((result == EC_Normal) && (imageDisplayFormat.getLength() == 0))
    result = imageDisplayFormat.putString(DEFAULT_imageDisplayFormat);
  return result;
}
...
OFCondition DVPSStoredPrint::write(DcmItem& dset,
  ...
  if ((EC_Normal == result) && updateDecimateCrop)
    result = imageBoxContentList.setRequestedDecimateCropBehaviour(
        decimateCropBehaviour);  // set in all image boxes
  if (EC_Normal == result)
    result = imageBoxContentList.createDefaultValues(
        limitImages, ignoreEmptyImages);  // renumber if limitImages is true
  ...
  if (instanceCreationDate.getLength() > 0) {
    ADD_TO_DATASET(DcmDate, instanceCreationDate)
  }
  if (instanceCreationTime.getLength() > 0) {
    ADD_TO_DATASET(DcmTime, instanceCreationTime)
  }
  ...
  if (result == EC_Normal) {
    ...
    if (ditem) {
      ...
      if (dseq) {
        ...
        if (annotationDisplayFormatID.getLength() > 0) {
          ADD_TO_DATASET2(DcmCodeString, annotationDisplayFormatID)
        }
        if (smoothingType.getLength() > 0) {
          ADD_TO_DATASET2(DcmCodeString, smoothingType)
        }
        if (borderDensity.getLength() > 0) {
          ADD_TO_DATASET2(DcmCodeString, borderDensity)
        }
        if (emptyImageDensity.getLength() > 0) {
          ADD_TO_DATASET2(DcmCodeString, emptyImageDensity)
        }
        if (minDensity.getLength() > 0) {
          ADD_TO_DATASET2(DcmUnsignedShort, minDensity)
        }
        if (trim.getLength() > 0) {
          ADD_TO_DATASET2(DcmCodeString, trim)
        }
        if (requestedResolutionID.getLength() > 0) {
          ADD_TO_DATASET2(DcmCodeString, requestedResolutionID)
        }
        if ((presentationLUTList.size() > 0) || globalPresentationLUTValid) {
          ...
          if (EC_Normal == result)
            result = addReferencedPLUTSQ(*ditem);
        }
        if (result == EC_Normal) {
          dseq->insert(ditem);
  ...
  if (limitImages && currentValuesValid)
    writeImageBoxes = currentNumCols * currentNumRows;
  ...
  if (EC_Normal == result)
    result = imageBoxContentList.write(
        dset, writeRequestedImageSize, (size_t)writeImageBoxes,
        ignoreEmptyImages, !globalPresentationLUTValid);

  // write annotationContentList
  if (EC_Normal == result)
    result = annotationContentList.write(dset);

  // write PrintManagementCapabilitiesSequence
  dseq =
      new DcmSequenceOfItems(DCM_RETIRED_PrintManagementCapabilitiesSequence);
  if (dseq) {
    if (EC_Normal == result)
      result =
          DVPSHelper::addReferencedUIDItem(*dseq, UID_BasicFilmSessionSOPClass);
    if (EC_Normal == result)
      result =
          DVPSHelper::addReferencedUIDItem(*dseq, UID_BasicFilmBoxSOPClass);
    if (EC_Normal == result)
      result = DVPSHelper::addReferencedUIDItem(
          *dseq, UID_BasicGrayscaleImageBoxSOPClass);
    if (EC_Normal == result)
      result = imageBoxContentList.addImageSOPClasses(*dseq,
                                                      (size_t)writeImageBoxes);
    if ((result == EC_Normal) && (presentationLUTList.size() > 0)) {
      result =
          DVPSHelper::addReferencedUIDItem(*dseq, UID_PresentationLUTSOPClass);
    }
    if ((result == EC_Normal) && (annotationContentList.size() > 0)) {
      result = DVPSHelper::addReferencedUIDItem(*dseq,
...
DVPSPresentationLUT* DVPSStoredPrint::getImagePresentationLUT(size_t idx) {
  ...
  const char* plutuid =
      imageBoxContentList.getReferencedPresentationLUTInstanceUID(idx);
...
OFCondition DVPSStoredPrint::setPresentationLUTShape(
    DVPSPresentationLUTType shape) {
...
Sint32 DVPSStoredPrint::convertODtoPValue(Uint16 density, unsigned int bits) {
  ...
  if ((min < max) && ((bits == 8) || (bits == 12) || (bits == 16))) {
    if (density >= max)
      ...
      const double factor =
          (double)DicomImageClass::maxval(bits) / (jmax - jmin);
      return (
          Sint32)((DiGSDFunction::getJNDIndex(la + l0 * pow((double)10, -d0)) -
                   jmin) *
                  factor);
    }
  }
...
OFCondition DVPSStoredPrint::addImageBox(
    const char* retrieveaetitle, const char* refstudyuid,
    const char* refseriesuid, const char* refsopclassuid,
    const char* refsopinstanceuid, const char* requestedimagesize,
    const char* patientid, DVPSPresentationLUT* presentationlut,
    OFBool inversePLUT) {
  char instanceuid[100];
  const char* lutUID =
      presentationLUTList.addPresentationLUT(presentationlut, inversePLUT);
  return imageBoxContentList.addImageBox(
      dcmGenerateUniqueIdentifier(instanceuid), retrieveaetitle, refstudyuid,
      refseriesuid, refsopclassuid, refsopinstanceuid, requestedimagesize,
      patientid, lutUID);
}
...
OFCondition DVPSStoredPrint::addImageBox(const char* retrieveaetitle,
  ...
  studyInstanceUID.getString(
      refstudyuid);  // same study UID for stored print and hardcopy image
  imageSeriesInstanceUID.getString(
      refseriesuid);  // but separate series for the hardcopy images

  return addImageBox(retrieveaetitle, refstudyuid, refseriesuid,
                     UID_RETIRED_HardcopyGrayscaleImageStorage,
                     refsopinstanceuid, requestedimagesize, patientid,
                     presentationlut, inversePLUT);
}
...
OFCondition DVPSStoredPrint::setRequestedDecimateCropBehaviour(
    DVPSDecimateCropBehaviour value) {
  decimateCropBehaviour = value;
  return EC_Normal;
}

OFCondition DVPSStoredPrint::newPrinter(const char* name,
                                        const char* destinationAE) {
  filmSizeID.clear();
  magnificationType.clear();
  smoothingType.clear();
  configurationInformation.clear();
  ...
  if (EC_Normal == result)
    result = imageBoxContentList.setAllImagesToDefault();
  return result;
}

const char* DVPSStoredPrint::getOriginator() {
  char* c = NULL;
  if (EC_Normal == originator.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getDestination() {
  char* c = NULL;
  if (EC_Normal == destination.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getPrinterName() {
  char* c = NULL;
  if (EC_Normal == printerName.getString(c))
    return c;
  else
    return NULL;
}
...
DVPSFilmOrientation DVPSStoredPrint::getFilmOrientation() {
  ...
  if ((EC_Normal == filmOrientation.getString(c)) && c) {
    OFString aString(c);
    if (aString == "PORTRAIT")
      result = DVPSF_portrait;
    else if (aString == "LANDSCAPE")
      result = DVPSF_landscape;
  }
  return result;
}
...
DVPSTrimMode DVPSStoredPrint::getTrim() {
  ...
  if ((EC_Normal == trim.getString(c)) && c) {
    OFString aString(c);
    if (aString == "YES")
      result = DVPSH_trim_on;
    else if (aString == "NO")
      result = DVPSH_trim_off;
  }
  return result;
}
...
const char* DVPSStoredPrint::getStudyInstanceUID() {
  char* c = NULL;
  if (EC_Normal == studyInstanceUID.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getSeriesInstanceUID() {
  char* c = NULL;
  if (EC_Normal == seriesInstanceUID.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getSOPInstanceUID() {
  char* c = NULL;
  if (EC_Normal == sOPInstanceUID.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getFilmSizeID() {
  char* c = NULL;
  if (EC_Normal == filmSizeID.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getMagnificationType() {
  char* c = NULL;
  if (EC_Normal == magnificationType.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getSmoothingType() {
  char* c = NULL;
  if (EC_Normal == smoothingType.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getConfigurationInformation() {
  char* c = NULL;
  if (EC_Normal == configurationInformation.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getResolutionID() {
  char* c = NULL;
  if (EC_Normal == requestedResolutionID.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getBorderDensity() {
  char* c = NULL;
  if (EC_Normal == borderDensity.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSStoredPrint::getEmtpyImageDensity() {
  char* c = NULL;
  if (EC_Normal == emptyImageDensity.getString(c))
    return c;
  else
    return NULL;
}
...
Uint16 DVPSStoredPrint::getPrintIllumination() {
  Uint16 result = 0;
  if (EC_Normal == illumination.getUint16(result, 0))
    return result;
  else
    return 0;
}
...
Uint16 DVPSStoredPrint::getPrintReflectedAmbientLight() {
  Uint16 result = 0;
  if (EC_Normal == reflectedAmbientLight.getUint16(result, 0))
    return result;
  else
    return 0;
}
...
OFCondition DVPSStoredPrint::deleteSpooledImages() {
  ...
  if (currentValuesValid) {
    ...
    if (deleteImageBoxes > imageBoxContentList.size())
      deleteImageBoxes = imageBoxContentList.size();
...
OFCondition DVPSStoredPrint::printSCUgetPrinterInstance(
    DVPSPrintMessageHandler& printHandler) {
  DcmDataset* attributeListOut = NULL;
  Uint16 status = 0;
  OFCondition cond =
      printHandler.getRQ(UID_PrinterSOPClass, UID_PrinterSOPInstance, NULL, 0,
                         status, attributeListOut);

  /* the N-GET response has been dumped somewhere else, we only need to delete
   * it */
...
OFCondition DVPSStoredPrint::printSCUpreparePresentationLUT(
    DVPSPrintMessageHandler& printHandler, OFBool printerRequiresMatchingLUT,
    OFBool printerLUTRenderingPreferred, OFBool printerSupports12Bit) {
  /* first of all we determine whether we can let the print SCP render
   * Presentation LUT for us. */
  renderPresentationLUTinSCP = OFFalse;  // set to true if we can create a
                                         // presentation LUT for all images
  transmitImagesIn12Bit = OFTrue;  // set to false later if images should be
                                   // transmitted in 8-bit depth

  OFBool printerSupportsPresentationLUT =
      printHandler.printerSupportsPresentationLUT();
  DVPSPresentationLUT* plut = NULL;
  if (printerSupportsPresentationLUT) {
    char* filmBox = NULL;
    if (EC_Normal != referencedPresentationLUTInstanceUID.getString(filmBox))
      filmBox = NULL;
    const char* plutuid =
        imageBoxContentList.haveSinglePresentationLUTUsed(filmBox);
    if (plutuid)
      plut = presentationLUTList.findPresentationLUT(plutuid);
    if (plut && (plut->isLegalPrintPresentationLUT())) {
      /* there is a single Presentation LUT that can be used for the complete
       * film, and it is a valid Supplement 22 Presentation LUT.
       */
      if (printerSupports12Bit) {
        /* 12-bit printer, we use the LUT if the printer can handle it and if
         * the user wants it */
        if (printerLUTRenderingPreferred) {
          if (printerRequiresMatchingLUT) {
            if (plut->matchesImageDepth(OFTrue))
    ...
    transmitImagesIn12Bit = printerSupports12Bit;
    DCMPSTAT_INFO(
        "spooler: printer does not support Presentation LUT SOP Class,"
  ...
  if (printerSupportsPresentationLUT) {
    ...
    if (result == EC_Normal) {
      OFCondition cond = printHandler.createRQ(UID_PresentationLUTSOPClass,
                                               presentationLUTInstanceUID,
                                               &dset, status, attributeListOut);
...
OFCondition DVPSStoredPrint::addReferencedPLUTSQ(DcmItem& dset) {
  ...
  DcmSequenceOfItems* dseq =
      new DcmSequenceOfItems(DCM_ReferencedPresentationLUTSequence);
...
OFCondition DVPSStoredPrint::addPresentationLUTReference(DcmItem& dset) {
  ...
  if (presentationLUTInstanceUID.size() > 0) {
    ...
    if (result == EC_Normal)
      result = refsopclassuid.putString(UID_PresentationLUTSOPClass);
    if (result == EC_Normal)
      result = refsopinstanceuid.putString(presentationLUTInstanceUID.c_str());
    DcmSequenceOfItems* dseq =
        new DcmSequenceOfItems(DCM_ReferencedPresentationLUTSequence);
    DcmItem* ditem = new DcmItem();
...
OFCondition DVPSStoredPrint::printSCUcreateBasicFilmSession(
    DVPSPrintMessageHandler& printHandler, DcmDataset& dset,
  ...
  // we expect 'number of copies', 'print priority', 'medium type' and 'film
  // destination' in dset add illumination and reflection, and presentation LUT
  // reference if necessary.
  if ((printHandler.printerSupportsPresentationLUT()) && plutInSession)
    result = addPresentationLUTReference(dset);

  if (result == EC_Normal) {
    OFCondition cond = printHandler.createRQ(UID_BasicFilmSessionSOPClass,
                                             filmSessionInstanceUID, &dset,
                                             status, attributeListOut);
...
OFCondition DVPSStoredPrint::printSCUcreateBasicFilmBox(
    DVPSPrintMessageHandler& printHandler, OFBool plutInSession) {
  if ((filmSessionInstanceUID.size() == 0) || (filmBoxInstanceUID.size() > 0))
    return EC_IllegalCall;
  ...
  if (filmOrientation.getLength() > 0) {
  ...
  if (smoothingType.getLength() > 0) {
    ADD_TO_DATASET(DcmCodeString, smoothingType)
  }
  if (borderDensity.getLength() > 0) {
    ADD_TO_DATASET(DcmCodeString, borderDensity)
  }
  if (emptyImageDensity.getLength() > 0) {
    ADD_TO_DATASET(DcmCodeString, emptyImageDensity)
  }
  if (minDensity.getLength() > 0) {
    ADD_TO_DATASET(DcmUnsignedShort, minDensity)
  }
  if (trim.getLength() > 0) {
    ADD_TO_DATASET(DcmCodeString, trim)
  }
  if (requestedResolutionID.getLength() > 0) {
    ADD_TO_DATASET(DcmCodeString, requestedResolutionID)
  }
  if ((printHandler.printerSupportsAnnotationBox()) &&
      (annotationDisplayFormatID.getLength() > 0)) {
  ...
  DcmUniqueIdentifier refsopinstanceuid(DCM_ReferencedSOPInstanceUID);
  if (result == EC_Normal)
    result = refsopclassuid.putString(UID_BasicFilmSessionSOPClass);
  if (result == EC_Normal)
    result = refsopinstanceuid.putString(filmSessionInstanceUID.c_str());
  if (result == EC_Normal) {
    ditem = new DcmItem();
    if (ditem) {
  ...
  // add illumination and reflection, and presentation LUT reference if
  // necessary.
  if ((result == EC_Normal) &&
      (printHandler.printerSupportsPresentationLUT()) && (!plutInSession))
    result = addPresentationLUTReference(dset);
  ...
  if (result == EC_Normal) {
    ...
    size_t i;
    OFCondition cond =
        printHandler.createRQ(UID_BasicFilmBoxSOPClass, filmBoxInstanceUID,
                              &dset, status, attributeListOut);
    if ((cond.good()) && ((status == 0) || ((status & 0xf000) == 0xb000)) &&
        attributeListOut) {
      // N-CREATE was successful, now evaluate Referenced Image Box SQ
      stack.clear();
      if (EC_Normal == attributeListOut->search(DCM_ReferencedImageBoxSequence,
                                                stack, ESM_fromHere, OFFalse)) {
        seq = (DcmSequenceOfItems*)stack.top();
        numItems = (size_t)seq->card();
        if (numItems > imageBoxContentList.size())
          numItems = imageBoxContentList.size();
        for (i = 0; i < numItems; i++) {
          ...
          if (EC_Normal == result)
            result = refsopclassuid.getString(c);
          if ((EC_Normal == result) && c && (grayscaleIB == c)) {
            result = refsopinstanceuid.getString(c);
            if (EC_Normal == result)
              result = imageBoxContentList.setImageSOPInstanceUID(i, c);
          } else
            result = EC_IllegalCall; /* wrong SOP class or unable to read UID */
        }
      } else
        result = EC_TagNotFound;
      ...
      if (EC_Normal ==
          attributeListOut->search(DCM_ReferencedBasicAnnotationBoxSequence,
                                   stack, ESM_fromHere, OFFalse)) {
        seq = (DcmSequenceOfItems*)stack.top();
        numItems = (size_t)seq->card();
        if (numItems > annotationContentList.size())
          numItems = annotationContentList.size();
        for (i = 0; i < numItems; i++) {
          item = seq->getItem(i);
          stack.clear();
          READ_FROM_DATASET2(DcmUniqueIdentifier, refsopclassuid)
          ...
          if ((EC_Normal == result) && c && (annotationB == c)) {
            result = refsopinstanceuid.getString(c);
            if (EC_Normal == result)
              result = annotationContentList.setAnnotationSOPInstanceUID(i, c);
          } else
            result = EC_IllegalCall; /* wrong SOP class or unable to read UID */
        }
      }
...
OFCondition DVPSStoredPrint::printSCUprintBasicFilmBox(
    DVPSPrintMessageHandler& printHandler) {
  ...
  OFCondition cond = printHandler.actionRQ(
      UID_BasicFilmBoxSOPClass, filmBoxInstanceUID.c_str(),
      1 /* action type ID 1 = print */, NULL /* no action information */,
      status, attributeListOut);
...
OFCondition DVPSStoredPrint::printSCUprintBasicFilmSession(
    DVPSPrintMessageHandler& printHandler) {
  DcmDataset* attributeListOut = NULL;
  Uint16 status = 0;

  OFCondition cond = printHandler.actionRQ(
      UID_BasicFilmSessionSOPClass, filmSessionInstanceUID.c_str(),
      1 /* action type ID 1 = print */, NULL /* no action information */,
      status, attributeListOut);
  delete attributeListOut;  // should be empty anyway
...
OFCondition DVPSStoredPrint::printSCUdelete(
    DVPSPrintMessageHandler& printHandler) {
  ...
  if (filmBoxInstanceUID.size() > 0) {
    cond = printHandler.deleteRQ(UID_BasicFilmBoxSOPClass,
  ...
  if (filmSessionInstanceUID.size() > 0) {
    cond = printHandler.deleteRQ(UID_BasicFilmSessionSOPClass,
  ...
  if ((presentationLUTInstanceUID.size() > 0) &&
      (printHandler.printerSupportsPresentationLUT())) {
    cond = printHandler.deleteRQ(UID_PresentationLUTSOPClass,
                                 presentationLUTInstanceUID.c_str(), status);
    if ((cond.bad()) || ((status != 0) && ((status & 0xf000) != 0xb000)))
      result = EC_IllegalCall;
    presentationLUTInstanceUID.clear();
  }
...
OFCondition DVPSStoredPrint::printSCUsetBasicImageBox(
    DVPSPrintMessageHandler& printHandler, size_t idx, DicomImage& image,
  ...
  if (imageSopInstanceUID == NULL)
    return EC_IllegalCall;

  /* any presentation LUT to render on SCU side? */
  if (!renderPresentationLUTinSCP) {
    /* look for referenced Presentation LUT in image box */
    const char* imageplutuid =
        imageBoxContentList.getReferencedPresentationLUTInstanceUID(idx);
    char* filmplutuid = NULL;
    if (EC_Normal !=
        referencedPresentationLUTInstanceUID.getString(filmplutuid))
      filmplutuid = NULL;
    /* if absent, look for referenced Presentation LUT in film box */
    if ((imageplutuid == NULL) || (strlen(imageplutuid) == 0))
      imageplutuid = filmplutuid;
    DVPSPresentationLUT* pLUT = NULL;
    if (imageplutuid && (strlen(imageplutuid) > 0))
      pLUT = presentationLUTList.findPresentationLUT(imageplutuid);
    if (pLUT) {
      /* found presentation LUT, activate */
      if (pLUT->activate(&image)) {
        if ((pLUT->getType() == DVPSP_table) && (!transmitImagesIn12Bit)) {
          DCMPSTAT_INFO(
              "rendering Presentation LUT into 8-bit bitmap, image quality "
              "loss possible.");
        }
      } else {
        DCMPSTAT_INFO(
            "unable to activate Presentation LUT, using IDENTITY instead.");
        image.setPresentationLutShape(ESP_Identity);
      }
    } /* else image.setPresentationLutShape(ESP_Identity); -- this does not make
         sense for MONO1 HG images */
  } /* else image.setPresentationLutShape(ESP_Identity); -- this does not make
       sense for MONO1 HG images */
  ...
  if (EC_Normal == result) {
    ...
    if (ditem) {
      ...
      if (dseq) {
        if (EC_Normal == result)
          result = DVPSHelper::putUint16Value(ditem, DCM_SamplesPerPixel, 1);
        if (useMonochrome1) {
          image.setPolarity(EPP_Reverse);
          if (EC_Normal == result)
            result = DVPSHelper::putStringValue(
                ditem, DCM_PhotometricInterpretation, "MONOCHROME1");
        } else {
          if (EC_Normal == result)
            result = DVPSHelper::putStringValue(
                ditem, DCM_PhotometricInterpretation, "MONOCHROME2");
        }
        if (EC_Normal == result)
          result =
              DVPSHelper::putUint16Value(ditem, DCM_PixelRepresentation, 0);
        if (EC_Normal == result)
          result = DVPSHelper::putUint16Value(ditem, DCM_Rows, (Uint16)height);
        if (EC_Normal == result)
          result =
              DVPSHelper::putUint16Value(ditem, DCM_Columns, (Uint16)width);
        ...
        if ((aspectRatio != 1.0) && (aspectRatio != 0)) {
          ...
          if (EC_Normal == result)
            result =
                DVPSHelper::putStringValue(ditem, DCM_PixelAspectRatio, str);
        }
        if (transmitImagesIn12Bit) {
          if (EC_Normal == result)
            result = DVPSHelper::putUint16Value(ditem, DCM_BitsAllocated, 16);
          if (EC_Normal == result)
            result = DVPSHelper::putUint16Value(ditem, DCM_BitsStored, 12);
          if (EC_Normal == result)
            result = DVPSHelper::putUint16Value(ditem, DCM_HighBit, 11);
          if (EC_Normal == result) {
            pxDataVoid = image.getOutputData(12);
            pxData = new DcmPolymorphOBOW(DCM_PixelData);
            if (pxData && pxDataVoid) {
              /*FILE *fp = fopen("C:\\Users\\Cedo\\Documents\\pixdata.ppp",
              "wb"); for (int j=0; j<height; j++) for (int i = 0; i < width;
              i++) {

                  Uint16 val = ((Uint16 *)pxDataVoid)[j*width + i];
                  fwrite(&val, 2, 1, fp);
              ...
              result =
                  pxData->putUint16Array((Uint16*)pxDataVoid, (width * height));
              if (EC_Normal == result)
                result = ditem->insert(pxData, OFTrue /*replaceOld*/);
              else
                delete pxData;
            } else
              result = EC_IllegalCall;
          ...
          if (EC_Normal == result)
            result = DVPSHelper::putUint16Value(ditem, DCM_BitsAllocated, 8);
          if (EC_Normal == result)
            result = DVPSHelper::putUint16Value(ditem, DCM_BitsStored, 8);
          if (EC_Normal == result)
            result = DVPSHelper::putUint16Value(ditem, DCM_HighBit, 7);
          if (EC_Normal == result) {
            pxDataVoid = image.getOutputData(8);
            pxData = new DcmPolymorphOBOW(DCM_PixelData);
            if (pxData && pxDataVoid) {
              result =
                  pxData->putUint8Array((Uint8*)pxDataVoid, (width * height));
              if (EC_Normal == result)
                result = ditem->insert(pxData, OFTrue /*replaceOld*/);
              else
                delete pxData;
  ...
  if (EC_Normal == result) {
    OFCondition cond = printHandler.setRQ(UID_BasicGrayscaleImageBoxSOPClass,
                                          imageSopInstanceUID, &dataset, status,
                                          attributeListOut);
    if ((cond.bad()) || ((status != 0) && ((status & 0xf000) != 0xb000)))
      result = EC_IllegalCall;
  }
...
OFCondition DVPSStoredPrint::printSCUsetBasicAnnotationBox(
    DVPSPrintMessageHandler& printHandler, size_t idx) {
  DcmDataset dataset;
  DcmDataset* attributeListOut = NULL;
  Uint16 status = 0;
  OFCondition result = EC_Normal;
  ...
  if (printHandler.printerSupportsAnnotationBox()) {
    const char* annotationSopInstanceUID =
        annotationContentList.getSOPInstanceUID(idx);
    if ((annotationSopInstanceUID == NULL) ||
        (strlen(annotationSopInstanceUID) == 0)) {
      DCMPSTAT_INFO(
          "not enough Annotation Boxes created by printer, ignoring "
          "annotation.");
      return EC_Normal;
    }
    ...
    if (EC_Normal == result) {
      OFCondition cond = printHandler.setRQ(UID_BasicAnnotationBoxSOPClass,
                                            annotationSopInstanceUID, &dataset,
                                            status, attributeListOut);
      if ((cond.bad()) || ((status != 0) && ((status & 0xf000) != 0xb000)))
        result = EC_IllegalCall;
    }
    ...
    DCMPSTAT_INFO(
        "printer does not support Annotation Box, ignoring annotation.");
  }
...
OFCondition DVPSStoredPrint::setSingleAnnotation(const char* displayformat,
  ...
  if (displayformat && text) {
    ...
    if (EC_Normal == result)
      result = annotationDisplayFormatID.putString(displayformat);
  }
  return result;
}
...
OFBool DVPSStoredPrint::printSCPCreate(
    DVConfiguration& cfg, const char* cfgname, DcmDataset* rqDataset,
    T_DIMSE_Message& rsp, DcmDataset*& rspDataset,
    OFBool presentationLUTnegotiated,
    DVPSPresentationLUT_PList& globalPresentationLUTList,
    const char* filmSessionUID, DcmUniqueIdentifier& study,
    DcmUniqueIdentifier& psSeries, DcmUniqueIdentifier& imgSeries) {
  ...
  if (result) {
    ...
    if (filmOrientation.getLength() == 0)
      filmOrientation.putString(DEFAULT_filmOrientation);
    else {
      OFString aString;
      filmOrientation.getOFString(aString, 0, OFTrue);
      if ((aString != "PORTRAIT") && (aString != "LANDSCAPE")) {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: illegal film orientation: '"
            << aString.c_str() << "'");
  ...
  if (result) {
    ...
    if (imageDisplayFormat.getLength() == 0) {
      DCMPSTAT_INFO(
          "cannot create Basic Film Box: image display format missing or "
          "empty");
      rsp.msg.NCreateRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    } else {
      currentValuesValid = OFFalse;
      updateCache();  // evaluates image display format and computes number of
                      // columns and rows
      if (currentValuesValid) {
        // now we check whether this is a supported layout as per the config
        // file
        Uint32 numPortraitDisplayFormats =
            cfg.getTargetPrinterNumberOfPortraitDisplayFormats(cfgname);
        if (numPortraitDisplayFormats > 0) {
          ...
          if (!found) {
            ...
            DCMPSTAT_INFO(
                "cannot create Basic Film Box: unsupported image display "
                "format: '"
                << aString.c_str() << "'");
            rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
            result = OFFalse;
        ...
        imageDisplayFormat.getOFStringArray(aString);
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: illegal image display format: '"
            << aString.c_str() << "'");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
  ...
  if (result) {
    ...
    if (filmSizeID.getLength() == 0) {
      ...
      if (!found) {
        DCMPSTAT_INFO("cannot create Basic Film Box: illegal film size ID: '"
                      << theSizeID.c_str() << "'");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
  ...
  // magnificationType
  if (result) {
    Uint32 numMagnifications =
        cfg.getTargetPrinterNumberOfMagnificationTypes(cfgname);
    ...
    if (magnificationType.getLength() == 0) {
      ...
      if (!found) {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: illegal magnification type: '"
            << theMagnification.c_str() << "'");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
    }
  ...
  if (result) {
    ...
    if (smoothingType.getLength() == 0) {
      ...
      if (numSmoothings == 0) {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: smoothing type requested but not "
            "supported.");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else if (!found) {
        DCMPSTAT_INFO("cannot create Basic Film Box: illegal smoothing type: '"
                      << theSmoothing.c_str() << "'");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
  ...
  // borderDensity
  if (result) {
    Uint32 numBorderDensities =
        cfg.getTargetPrinterNumberOfBorderDensities(cfgname);
    ...
    if (borderDensity.getLength() == 0) {
      ...
      {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: border density requested but not "
            "supported.");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        for (Uint32 i = 0; i < numBorderDensities; i++) {
          ...
          if (theBorderDensity == aString) {
            ...
            if (1 == sscanf(aString.c_str(), "%lu", &l))
              supportsNumericDensity = OFTrue;
          }
        }
        if ((!found) && supportsNumericDensity) {
          // the density was not found in the list; check whether it is
          // numerical
          if (1 == sscanf(theBorderDensity.c_str(), "%lu", &l))
            found = OFTrue;
        }

        if (!found) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: illegal border density: '"
              << theBorderDensity.c_str() << "'");
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
      }
  ...
  // emptyImageDensity
  if (result) {
    Uint32 numEmptyImageDensities =
        cfg.getTargetPrinterNumberOfEmptyImageDensities(cfgname);
    ...
    if (emptyImageDensity.getLength() == 0) {
      ...
      {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: empty image density requested but "
            "not supported.");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        if ((!found) && supportsNumericDensity) {
          // the density was not found in the list; check whether it is
          // numerical
          if (1 == sscanf(theEIDensity.c_str(), "%lu", &l))
            found = OFTrue;
        }

        if (!found) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: illegal empty image density: '"
              << theEIDensity.c_str() << "'");
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
  ...
  if (result) {
    ...
    if (maxDensity.getLength() == 0) {
      if (numMaxDensities > 0) {
        ...
        if (EC_Normal != setMaxDensity(aString.c_str()))
          maxDensity.putUint16(DEFAULT_maxDensity, 0);
      }
  ...
  if (result) {
    ...
    if (minDensity.getLength() == 0) {
      if (numMinDensities > 0) {
        ...
        if (EC_Normal != setMinDensity(aString.c_str()))
          minDensity.putUint16(DEFAULT_minDensity, 0);
      }
    } else {
      if (numMinDensities == 0)  // we don't support min density
      {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: min density requested but not "
            "supported.");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      }
  ...
  if (result) {
    ...
    if (trim.getLength() == 0) {
      if (cfg.getTargetPrinterSupportsTrim(cfgname))
        trim.putString(DEFAULT_trim);
    } else {
      if (cfg.getTargetPrinterSupportsTrim(cfgname)) {
        OFString aString;
        trim.getOFString(aString, 0, OFTrue);
        if ((aString != "YES") && (aString != "NO")) {
          DCMPSTAT_INFO("cannot create Basic Film Box: illegal trim: '"
                        << aString.c_str() << "'");
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
        ...
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: trim requested but not supported.");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      }
    }
  ...
  if (result) {
    ...
    if (configurationInformation.getLength() > 0) {
      ...
      Uint32 numConfigurationInformation =
          cfg.getTargetPrinterNumberOfConfigurationSettings(cfgname);
      if (numConfigurationInformation ==
          0)  // we don't support configuration information
      {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: configuration information requested "
            "but not supported.");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        for (Uint32 i = 0; i < numConfigurationInformation; i++) {
          if (theConfiguration ==
              cfg.getTargetPrinterConfigurationSetting(cfgname, i)) {
            found = OFTrue;
            break;
          }
        }
        if (!found) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: illegal configuration "
              "information: '"
              << theConfiguration.c_str() << "'");
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
      }
  ...
  if (result) {
    Uint32 numResolutionIDs =
        cfg.getTargetPrinterNumberOfPrinterResolutionIDs(cfgname);
    ...
    if (requestedResolutionID.getLength() == 0) {
      ...
      {
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: requested resolution ID present but "
            "not supported.");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        if (!found) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: illegal requested resolution ID: '"
              << theResolutionID.c_str() << "'");
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
      }
  ...
  if (presentationLUTnegotiated) {
    ...
    if (result) {
      ...
      if (illumination.getLength() == 0)
        illumination.putUint16(DEFAULT_illumination, 0);
    ...
    if (result) {
      ...
      if (reflectedAmbientLight.getLength() == 0)
        illumination.putUint16(DEFAULT_reflectedAmbientLight, 0);
    ...
    if (result) {
      ...
      if (rqDataset &&
          (EC_Normal == rqDataset->search(DCM_ReferencedPresentationLUTSequence,
                                          stack, ESM_fromHere, OFFalse))) {
        ...
        if (seq->card() == 1) {
          ...
          if (referencedPresentationLUTInstanceUID.getLength() > 0) {
            referencedPresentationLUTInstanceUID.getOFString(aString, 0);
            DVPSPresentationLUT* currentPLUT =
                globalPresentationLUTList.findPresentationLUT(aString.c_str());
            if (NULL == currentPLUT) {
              DCMPSTAT_INFO(
                  "cannot create Basic Film Box: presentation LUT reference "
                  "cannot be resolved");
              rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
              result = OFFalse;
            } else {
              ...
              if (refClassUID.getLength() > 0) {
                ...
                if (aString != UID_PresentationLUTSOPClass) {
                  DCMPSTAT_INFO(
                      "cannot create Basic Film Box: referenced SOP class UID "
                      "in referenced presentation LUT sequence incorrect:"
                      << DcmObject::PrintHelper(
                             *stack.top(), DCMTypes::PF_shortenLongTagValues));
                  rsp.msg.NCreateRSP.DimseStatus =
                      STATUS_N_InvalidAttributeValue;
                  result = OFFalse;
                } else {
                  // referenced presentation LUT sequence is OK
                  // synchronize presentationLUTInstanceUID and
                  // referencedPresentationLUTInstanceUID
                  presentationLUTInstanceUID.clear();
                  referencedPresentationLUTInstanceUID.getOFString(
                      presentationLUTInstanceUID, 0);
                  referencedPresentationLUTAlignment =
                      currentPLUT->getAlignment();
                }
              } else {
                DCMPSTAT_INFO(
                    "cannot create Basic Film Box: no referenced SOP class UID "
                    "in referenced presentation LUT sequence");
                rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
                result = OFFalse;
              }
            }
            ...
            DCMPSTAT_INFO(
                "cannot create Basic Film Box: no referenced SOP instance UID "
                "in referenced presentation LUT sequence");
            rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
            result = OFFalse;
          }
          ...
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: referenced presentation LUT "
              "sequence number of items != 1");
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
      }
  ...
  if (result) {
    ...
    if (rqDataset &&
        (EC_Normal == rqDataset->search(DCM_ReferencedFilmSessionSequence,
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      if (refFilmSessionSequence->card() == 1) {
        ...
        if (instanceUID.getLength() > 0) {
          ...
          if (aString != filmSessionUID) {
            DCMPSTAT_INFO(
                "cannot create Basic Film Box: referenced film session "
                "instance UID incorrect");
            rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
            result = OFFalse;
          } else {
            // check referenced SOP class UID
            ...
            if (classUID.getLength() > 0) {
              ...
              if (aString != UID_BasicFilmSessionSOPClass) {
                DCMPSTAT_INFO(
                    "cannot create Basic Film Box: referenced SOP class UID in "
                    "referenced film session sequence incorrect:"
                    << DcmObject::PrintHelper(
                           *stack.top(), DCMTypes::PF_shortenLongTagValues));
                rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
                result = OFFalse;
              ...
              DCMPSTAT_INFO(
                  "cannot create Basic Film Box: no referenced SOP class UID "
                  "in referenced film session sequence");
              rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
              result = OFFalse;
            }
          }
          ...
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: no referenced SOP instance UID in "
              "referenced film session sequence");
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
        ...
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: referenced film session sequence "
            "number of items != 1");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
  ...
  if (result && rqDataset) {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub)) {
      ...
      if (currentTag.getElement() == 0x0000) /* group length */
        ...
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: illumination received:\n"
              << DcmObject::PrintHelper(*stack.top(),
        ...
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Box: reflected ambient light "
              "received:\n"
              << DcmObject::PrintHelper(*stack.top(),
                                        DCMTypes::PF_shortenLongTagValues));
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        ...
        DCMPSTAT_INFO(
            "cannot create Basic Film Box: unsupported attribute received:\n"
            << DcmObject::PrintHelper(*stack.top(),
                                      DCMTypes::PF_shortenLongTagValues));
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
    ...
    if (rspDataset) {
      ...
      if (refFilmSessionSequence)  // should never be NULL if we get this far
      {
        ...
        if (newRefFilmSessionSequence)
          rspDataset->insert(newRefFilmSessionSequence, OFTrue /*replaceOld*/);
        else
      ...
      if (requestedResolutionID.getLength() > 0) {
        ADD_TO_PDATASET(DcmCodeString, requestedResolutionID)
      }

      if (presentationLUTnegotiated) {
        if (referencedPresentationLUTInstanceUID.getLength() == 0) {
          referencedPresentationLUTInstanceUID.putString(
              WELLKNOWN_IDENTITY_PLUT_UID);
          if (NULL == globalPresentationLUTList.findPresentationLUT(
                          WELLKNOWN_IDENTITY_PLUT_UID)) {
            ...
            if (wellknownlut) {
              ...
              if (EC_Normal == writeresult)
                writeresult = wellknownlut->setSOPInstanceUID(
                    WELLKNOWN_IDENTITY_PLUT_UID);
              if (EC_Normal == writeresult)
                globalPresentationLUTList.insert(wellknownlut);
            } else
              writeresult = EC_MemoryExhausted;
          }
        }
        if (EC_Normal == writeresult)
          writeresult = addPresentationLUTReference(*rspDataset);
      }

      // create image boxes and referenced image box sequence
      if (imageBoxContentList.printSCPCreate(
              currentNumRows * currentNumCols, studyInstanceUID,
              imageSeriesInstanceUID, cfg.getNetworkAETitle())) {
        if (EC_Normal == writeresult)
          writeresult =
              imageBoxContentList.writeReferencedImageBoxSQ(*rspDataset);
      } else
        writeresult = EC_MemoryExhausted;
...
OFBool DVPSStoredPrint::printSCPSet(
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)magnificationType.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      if (!found) {
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: illegal magnification type: '"
            << theMagnification.c_str() << "'");
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)smoothingType.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      smoothingType = *((DcmCodeString*)(stack.top()));
      Uint32 numSmoothings =
          cfg.getTargetPrinterNumberOfSmoothingTypes(cfgname);
      OFString theSmoothing;
      OFString aString;
      OFBool found = OFFalse;
      smoothingType.getOFString(theSmoothing, 0, OFTrue);
      ...
      if (numSmoothings == 0) {
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: smoothing type requested but not "
            "supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else if (!found) {
        DCMPSTAT_INFO("cannot update Basic Film Box: illegal smoothing type: '"
                      << theSmoothing.c_str() << "'");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      } else {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)borderDensity.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      {
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: border density requested but not "
            "supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        if (!found) {
          DCMPSTAT_INFO(
              "cannot update Basic Film Box: illegal border density: '"
              << theBorderDensity.c_str() << "'");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        } else {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)emptyImageDensity.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      {
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: empty image density requested but "
            "not supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        if (!found) {
          DCMPSTAT_INFO(
              "cannot update Basic Film Box: illegal empty image density: '"
              << theEIDensity.c_str() << "'");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        } else {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)maxDensity.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)minDensity.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      minDensity = *((DcmUnsignedShort*)(stack.top()));
      Uint32 numMinDensities =
          cfg.getTargetPrinterNumberOfMinDensities(cfgname);
      if (numMinDensities == 0)  // we don't support min density
      {
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: min density requested but not "
            "supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)trim.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      ...
      if (cfg.getTargetPrinterSupportsTrim(cfgname)) {
        ...
        trim.getOFString(aString, 0, OFTrue);
        if ((aString != "YES") && (aString != "NO")) {
          DCMPSTAT_INFO("cannot update Basic Film Box: illegal trim: '"
                        << aString.c_str() << "'");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        } else {
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: trim requested but not supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      }
    }
  ...
  if (result) {
    ...
    if (rqDataset &&
        (EC_Normal ==
         rqDataset->search((DcmTagKey&)configurationInformation.getTag(), stack,
                           ESM_fromHere, OFFalse))) {
      ...
      {
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: configuration information requested "
            "but not supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        if (!found) {
          DCMPSTAT_INFO(
              "cannot update Basic Film Box: illegal configuration "
              "information: '"
              << theConfiguration.c_str() << "'");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
  ...
  if (presentationLUTnegotiated) {
    ...
    if (result) {
      stack.clear();
      if (rqDataset &&
          (EC_Normal == rqDataset->search((DcmTagKey&)illumination.getTag(),
                                          stack, ESM_fromHere, OFFalse))) {
    ...
    if (result) {
      ...
      if (rqDataset &&
          (EC_Normal ==
           rqDataset->search((DcmTagKey&)reflectedAmbientLight.getTag(), stack,
                             ESM_fromHere, OFFalse))) {
      ...
      if (rqDataset &&
        ...
        if (seq->card() == 1) {
          ...
          if (referencedPresentationLUTInstanceUID.getLength() > 0) {
            ...
            if (NULL == currentPLUT) {
              DCMPSTAT_INFO(
                  "cannot update Basic Film Box: presentation LUT reference "
                  "cannot be resolved");
              rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
              result = OFFalse;
            } else {
              ...
              if (refClassUID.getLength() > 0) {
                ...
                if (aString != UID_PresentationLUTSOPClass) {
                  DCMPSTAT_INFO(
                      "cannot update Basic Film Box: referenced SOP class UID "
                      "in referenced presentation LUT sequence incorrect:\n"
                      << DcmObject::PrintHelper(
                             *stack.top(), DCMTypes::PF_shortenLongTagValues));
                  rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
                  ...
                  referencedPresentationLUTAlignment =
                      currentPLUT->getAlignment();
                  if ((cfg.getTargetPrinterPresentationLUTMatchRequired(
                          cfgname)) &&
                      (!imageBoxContentList.matchesPresentationLUT(
                          referencedPresentationLUTAlignment))) {
                    DCMPSTAT_INFO(
                        "cannot update Basic Film Box: referenced presentation "
                        "LUT number of entries does not match image bit "
                        "depth.");
                    rsp.msg.NSetRSP.DimseStatus =
                        STATUS_N_InvalidAttributeValue;
                    result = OFFalse;
                  } else {
                    // referenced presentation LUT sequence is OK
                    ...
                    if (newSeq)
                      rspDataset->insert(newSeq, OFTrue /*replaceOld*/);
                    else {
                      writeresult = EC_MemoryExhausted;
                    }
                  }
                ...
                DCMPSTAT_INFO(
                    "cannot update Basic Film Box: no referenced SOP class UID "
                    "in referenced presentation LUT sequence");
                rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
                result = OFFalse;
              }
            ...
            DCMPSTAT_INFO(
                "cannot update Basic Film Box: no referenced SOP instance UID "
                "in referenced presentation LUT sequence");
            rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
            result = OFFalse;
          }
          ...
          DCMPSTAT_INFO(
              "cannot update Basic Film Box: referenced presentation LUT "
              "sequence number of items != 1");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
  ...
  if (result && rqDataset) {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub)) {
      ...
      if (currentTag.getElement() == 0x0000) /* group length */
        ...
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot update Basic Film Box: illumination received:\n"
              << DcmObject::PrintHelper(*stack.top(),
                                        DCMTypes::PF_shortenLongTagValues));
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
          result = OFFalse;
        ...
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot update Basic Film Box: reflected ambient light "
              "received:\n"
              << DcmObject::PrintHelper(*stack.top(),
                                        DCMTypes::PF_shortenLongTagValues));
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Film Box: unsupported attribute received:"
            << DcmObject::PrintHelper(*stack.top(),
                                      DCMTypes::PF_shortenLongTagValues));
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
  ...
  if (result && (EC_Normal == writeresult)) {
    ...
    delete rspDataset;
    rspDataset = NULL;
    if (rsp.msg.NSetRSP.DimseStatus == 0)
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_ProcessingFailure;
    result = OFFalse;
  }
  return result;
...
OFBool DVPSStoredPrint::usesPresentationLUT(const char* c) {
  char* plut = NULL;
  if (c &&
      (EC_Normal == referencedPresentationLUTInstanceUID.getString(plut)) &&
      plut) {
...
void DVPSStoredPrint::updatePresentationLUTList(
    DVPSPresentationLUT_PList& globalPresentationLUTList) {
  presentationLUTList.clear();
  if (referencedPresentationLUTInstanceUID.getLength() > 0) {
    OFString aString;
    ...
    DVPSPresentationLUT* currentPLUT =
        globalPresentationLUTList.findPresentationLUT(aString.c_str());
    if (currentPLUT)
      presentationLUTList.insert(currentPLUT->clone());
    else {
      referencedPresentationLUTInstanceUID.clear();
      presentationLUTInstanceUID.clear();
    }
...
void DVPSStoredPrint::overridePresentationLUTSettings(
  ...
  presentationLUTInstanceUID.clear();
  referencedPresentationLUTInstanceUID.getOFString(presentationLUTInstanceUID,
                                                   0);
 ...
 *  Print SCP now writes PrinterCharacteristicsSequence when saving Stored
 * Prints.
 *
 ...
 *  Implemented Referenced Presentation LUT Sequence on Basic Film Session
 * level. Empty film boxes (pages) are not written to file anymore.
 *
