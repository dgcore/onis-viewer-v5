...
OFCondition DVPSAnnotationContent::setContent(const char* instanceuid,
  ...
  if (instanceuid && text) {
    ...
    if (EC_Normal == result)
      result = textString.putString(text);
    if (EC_Normal == result)
      result = annotationPosition.putUint16(position, 0);
  } else
...
OFCondition DVPSAnnotationContent::read(DcmItem& dset) {
  ...
  if (result == EC_Normal) {
    if ((sOPInstanceUID.getLength() == 0) || (sOPInstanceUID.getVM() != 1)) {
      ...
      DCMPSTAT_INFO(
          "SOPInstanceUID missing or incorrect in Stored Print Annotation");
    }
    if ((annotationPosition.getLength() == 0) ||
        (annotationPosition.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "AnnotationPosition missing or incorrect in Stored Print Annotation");
    }
    if ((textString.getLength() == 0) || (textString.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "TextString missing or incorrect in Stored Print Annotation");
    }
  }
...
OFCondition DVPSAnnotationContent::write(DcmItem& dset) {
  ...
  if (annotationPosition.getLength() == 0) {
    result = EC_TagNotFound;
    DCMPSTAT_INFO(
        "cannot write Stored Print Annotation: AnnotationPosition empty");
  }
