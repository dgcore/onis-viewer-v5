...
DVInterface::DVInterface(const char* config_file, OFBool useLog)
    : DVConfiguration(config_file),
      ...
      annotationText() {
#ifdef WITH_OPENSSL
  DcmSignature::initializeLibrary();  // initializes OpenSSL for dcmsign and
                                      // dcmtls
#endif
  ...
  if (!getGUIConfigEntryBool(L2_HIGHRESOLUTIONGRAPHICS, OFFalse)) {
    ...
    if (displayFunctionFile && (strlen(displayFunctionFile) > 0)) {
      ...
      if (df && (df->isValid())) {
        ...
        if (df)
          delete df;
        DCMPSTAT_WARN("Unable to load monitor characterics file '"
                      << displayFunctionFile << "', ignoring");
      }
    }
  }
  ...
  pPrint =
      new DVPSStoredPrint(getDefaultPrintIllumination(),
                          getDefaultPrintReflection(), getNetworkAETitle());
  ...
  if (referenceTime >= 86400)
    referenceTime -= 86400;  // subtract one day
  ...
  if (useLog) {
    ...
    if (filename != NULL) {
      ...
      OFauto_ptr<log4cplus::Layout> layout(
          new log4cplus::PatternLayout(pattern));
      log4cplus::SharedAppenderPtr logfile(
          new log4cplus::FileAppender(filepath, STD_NAMESPACE ios::app));
...
OFCondition DVInterface::loadImage(const char* studyUID, const char* seriesUID,
  ...
  if (studyUID && seriesUID && instanceUID) {
    if (lockDatabase() == EC_Normal) {
      ...
      if (filename) {
        ...
        DCMPSTAT_LOGFILE(
            "Load image from database failed: UIDs not in index file");
    } else
      DCMPSTAT_LOGFILE(
          "Load image from database failed: could not lock index file");
  } else
...
OFCondition DVInterface::loadReferencedImage(size_t idx, OFBool changeStatus) {
  ...
  if ((pState != NULL) && (idx < pState->numberOfImageReferences())) {
    OFString ofstudyUID, ofseriesUID, ofsopclassUID, ofinstanceUID, offrames,
        aetitle, mediaID, mediaUID;
    if ((status = pState->getImageReference(
             idx, ofstudyUID, ofseriesUID, ofsopclassUID, ofinstanceUID,
             offrames, aetitle, mediaID, mediaUID)) == EC_Normal) {
      if (lockDatabase() == EC_Normal) {
        const char* filename = getFilename(
            ofstudyUID.c_str(), ofseriesUID.c_str(), ofinstanceUID.c_str());
        if (filename != NULL) {
          DcmFileFormat* image = NULL;
          if ((status = DVPSHelper::loadFileFormat(filename, image)) ==
              EC_Normal) {
            status = EC_IllegalCall;
            if (image != NULL) {
              DcmDataset* dataset = image->getDataset();
              if (dataset != NULL) {
                if ((status = pState->attachImage(image, OFFalse)) ==
                    EC_Normal) {
                  exchangeImageAndPState(pState,
                                         image);  // do not exchange pState
                  imageInDatabase = OFTrue;
                  if (changeStatus)
                    instanceReviewed(ofstudyUID.c_str(), ofseriesUID.c_str(),
                                     ofinstanceUID.c_str());
                }
              }
            }
            if (status != EC_Normal)
              DCMPSTAT_LOGFILE(
                  "Load referenced image from file failed: invalid data "
                  "structures");
          } else {
            status = EC_CorruptedData;
            DCMPSTAT_LOGFILE(
                "Load referenced image from file failed: could not read "
                "fileformat");
          }
          if (status != EC_Normal)
            delete image;
          ...
          status = EC_IllegalCall;
          DCMPSTAT_LOGFILE(
              "Load referenced image from database failed: UIDs not in index "
              "file");
        }
      } else
        DCMPSTAT_LOGFILE(
            "Load referenced image from database failed: could not lock index "
            "file");
    } else
      DCMPSTAT_LOGFILE(
          "Load referenced image from database failed: internal error");
  } else
    DCMPSTAT_LOGFILE(
        "Load referenced image from database failed: internal error");
  return status;
...
OFCondition DVInterface::loadPState(const char* studyUID, const char* seriesUID,
                                    const char* instanceUID,
                                    OFBool changeStatus) {
  ...
  if (status != EC_Normal) {
    DCMPSTAT_LOGFILE(
        "Load presentation state from database failed: could not lock index "
        "file");
    return status;
  }
  const char* filename = getFilename(studyUID, seriesUID, instanceUID);
  if (filename == NULL) {
    DCMPSTAT_LOGFILE(
        "Load presentation state from database failed: UIDs not in index file");
  ...
  if (newState == NULL) {
    DCMPSTAT_LOGFILE(
        "Load presentation state from database failed: memory exhausted");
    return EC_MemoryExhausted;
  }

  if ((EC_Normal == (status = DVPSHelper::loadFileFormat(filename, pstate))) &&
      (pstate)) {
    DcmDataset* dataset = pstate->getDataset();
    if (dataset)
      status = newState->read(*dataset);
    else
      status = EC_CorruptedData;
  }
  if (status == EC_Normal) {
    ...
    OFString ofstudyUID, ofseriesUID, ofsopclassUID, ofinstanceUID, offrames,
        aetitle, mediaID, mediaUID;
    status = newState->getImageReference(0, ofstudyUID, ofseriesUID,
                                         ofsopclassUID, ofinstanceUID, offrames,
                                         aetitle, mediaID, mediaUID);

    // we could do something fancy with the retrieve AE title and the storage
    // media ID here, but we just assume that the referenced image is in our
    // local database.
    if (EC_Normal == status) {
      // determine the filename of the referenced image
      const char* imagefilename = NULL;
      imagefilename = getFilename(ofstudyUID.c_str(), ofseriesUID.c_str(),
                                  ofinstanceUID.c_str());
      ...
      if (imagefilename) {
        ...
        if ((EC_Normal ==
             (status = DVPSHelper::loadFileFormat(imagefilename, fimage))) &&
            (fimage)) {
          DcmDataset* dataset = pstate->getDataset();
          if (dataset) {
        ...
        if (status != EC_Normal) {
          delete fimage;
          DCMPSTAT_LOGFILE(
              "Load presentation state from database failed: could not read "
              "image data");
        }
      } else {
        status = EC_IllegalCall;  // no valid image filename
        DCMPSTAT_LOGFILE(
            "Load presentation state from database failed: referenced image "
            "not in index file");
      }
    }
  } else
...
OFCondition DVInterface::loadPState(const char* pstName, const char* imgName) {
  OFCondition status = EC_IllegalCall;
  DcmFileFormat* pstate = NULL;
  DcmFileFormat* image =
      pDicomImage;  // default: do not replace image if image filename is NULL
  DVPresentationState* newState = new DVPresentationState(
      OFstatic_cast(DiDisplayFunction**, displayFunction),
      minimumPrintBitmapWidth, minimumPrintBitmapHeight,
      maximumPrintBitmapWidth, maximumPrintBitmapHeight,
  ...
  if (newState == NULL) {
    DCMPSTAT_LOGFILE(
        "Load presentation state from file failed: memory exhausted");
    return EC_MemoryExhausted;
  }

  if ((status = DVPSHelper::loadFileFormat(pstName, pstate)) == EC_Normal) {
    if ((imgName == NULL) ||
        ((status = DVPSHelper::loadFileFormat(imgName, image)) == EC_Normal)) {
      if ((pstate) && (image)) {
        DcmDataset* dataset = pstate->getDataset();
        if (dataset) {
      ...
      if (status != EC_Normal)
        DCMPSTAT_LOGFILE(
            "Load presentation state from file failed: invalid data "
            "structures");
    } else
      DCMPSTAT_LOGFILE(
          "Load presentation state from file failed: could not load image");
  } else
    DCMPSTAT_LOGFILE(
        "Load presentation state from file failed: could not read fileformat");
  if (status != EC_Normal) {
    delete newState;
  ...
  if (studyUID && seriesUID && instanceUID) {
    if (lockDatabase() == EC_Normal) {
      ...
      if (filename) {
        ...
        DCMPSTAT_LOGFILE(
            "Load structured report from database failed: UIDs not in index "
            "file");
    } else
      DCMPSTAT_LOGFILE(
          "Load structured report from database failed: could not lock index "
          "file");
  } else
    DCMPSTAT_LOGFILE(
        "Load structured report from database failed: invalid UIDs");
  return status;
}
...
OFCondition DVInterface::loadStructuredReport(const char* filename) {
  ...
  if (newReport == NULL) {
    DCMPSTAT_LOGFILE(
        "Load structured report from file failed: memory exhausted");
    return EC_MemoryExhausted;
  }

  if ((status = DVPSHelper::loadFileFormat(filename, fileformat)) ==
      EC_Normal) {
    if (fileformat) {
      DcmDataset* dataset = fileformat->getDataset();
      if (dataset) {
        if ((status = newReport->read(
                 *dataset, DSRTypes::RF_readDigitalSignatures)) == EC_Normal) {
          delete pReport;
          pReport = newReport;
          if (pSignatureHandler) {
            pSignatureHandler->updateDigitalSignatureInformation(
                *dataset, DVPSS_structuredReport, OFTrue);
            /* check whether loaded report is 'finalized' (certain attributes
    ...
    if (status != EC_Normal)
      DCMPSTAT_LOGFILE(
          "Load structured report from file failed: invalid data structures");
  } else
    DCMPSTAT_LOGFILE(
        "Load structured report from file failed: could not read fileformat");
  if (status != EC_Normal)
    delete newReport;
  delete fileformat;
  return status;
  ...
  if (reportID) {
    ...
    if (srfile) {
      ...
      if (result == EC_Normal) {
        ...
        DCMPSTAT_LOGFILE(
            "Load structured reporting 'template' from file failed");
    }
  }
  return result;
}
...
OFCondition DVInterface::savePState(OFBool replaceSOPInstanceUID) {
  ...
  if (replaceSOPInstanceUID)
    instanceUID = pState->createInstanceUID();
  else
    instanceUID = pState->getInstanceUID();
  if (instanceUID == NULL)
    return EC_IllegalCall;
  ...
  DcmQueryRetrieveIndexDatabaseHandle dbhandle(
      getDatabaseFolder(), PSTAT_MAXSTUDYCOUNT, PSTAT_STUDYSIZE, result);
  if (result.bad()) {
    DCMPSTAT_LOGFILE(
        "Save presentation state to database failed: could not lock index "
        "file");
    return EC_IllegalCall;
  }

  if (dbhandle
          .makeNewStoreFileName(UID_GrayscaleSoftcopyPresentationStateStorage,
                                instanceUID, imageFileName)
          .good()) {
    // now store presentation state as filename
    result = savePState(imageFileName, OFFalse);
    if (EC_Normal == result) {
      if (dbhandle
              .storeRequest(UID_GrayscaleSoftcopyPresentationStateStorage,
                            instanceUID, imageFileName, &dbStatus)
              .bad()) {
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Save presentation state to database failed: could not in index "
            "file");
        DCMPSTAT_DEBUG("Unable to presentation state '" << imageFileName
                                                        << "' in database");
      }
    }
  }
  if (pDicomImage != NULL) {
    ...
    if (dset != NULL) {
      ...
      if (DU_getStringDOElement(dset, DCM_SOPClassUID, sopClass) &&
          ...
          ((!imageInDatabase) ||
           (getSeriesStruct(studyUID, seriesUID, instanceUID2) == NULL))) {
        releaseDatabase(); /* avoid deadlocks */
        if (dbhandle.makeNewStoreFileName(sopClass, instanceUID2, imageFileName)
                .good()) {
          // now store presentation state as filename
          result = saveCurrentImage(imageFileName);
          if (EC_Normal == result) {
            if (dbhandle
                    .storeRequest(sopClass, instanceUID2, imageFileName,
                                  &dbStatus)
                    .bad()) {
              result = EC_IllegalCall;
              DCMPSTAT_LOGFILE(
                  "Save presentation state to database failed: could not image "
                  "in index file");
              DCMPSTAT_DEBUG("Unable to image '" << imageFileName
                                                 << "' in database");
            } else {
              imageInDatabase = OFTrue;
            }
...
OFCondition DVInterface::savePState(const char* filename,
  ...
  if (dataset) {
    if ((status = pState->write(*dataset, replaceSOPInstanceUID)) ==
        EC_Normal) {
      ...
      if (pSignatureHandler) {
        pSignatureHandler->updateDigitalSignatureInformation(
            *pDicomPState->getDataset(), DVPSS_presentationState, OFFalse);
      }
    }
    if (status != EC_Normal)
      DCMPSTAT_LOGFILE(
          "Save presentation state to file failed: could not write fileformat");
  } else {
    DCMPSTAT_LOGFILE(
        "Save presentation state to file failed: memory exhausted");
...
OFCondition DVInterface::saveCurrentImage(const char* filename,
  ...
  if (pDicomImage == NULL)
    return EC_IllegalCall;
  OFCondition result =
      DVPSHelper::saveFileFormat(filename, pDicomImage, explicitVR);
  if (result != EC_Normal)
...
OFCondition DVInterface::saveStructuredReport() {
  ...
  if (result.bad()) {
    DCMPSTAT_LOGFILE(
        "Save structured report to database failed: could not lock index file");
    return EC_IllegalCall;
  }

  if (dbhandle
          .makeNewStoreFileName(sopClassUID.c_str(), instanceUID.c_str(),
                                filename)
          .good()) {
    // now store presentation state as filename
    ...
    if (EC_Normal == result) {
      if (dbhandle
              .storeRequest(sopClassUID.c_str(), instanceUID.c_str(), filename,
                            &dbStatus)
              .bad()) {
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Save structured report to database failed: could not in index "
            "file");
        DCMPSTAT_DEBUG("Unable to structured report '" << filename
                                                       << "' in database");
      }
    }
  }
...
OFCondition DVInterface::saveStructuredReport(const char* filename,
  ...
  if (dataset) {
    ...
    if ((status = pReport->write(*dataset)) == EC_Normal) {
      ...
      if (pSignatureHandler) {
        pSignatureHandler->updateDigitalSignatureInformation(
            *dataset, DVPSS_structuredReport, OFFalse);
      }
    }
    if (status != EC_Normal)
      DCMPSTAT_LOGFILE(
          "Save structured report to file failed: could not write fileformat");
  } else {
      ...
      if (lockDatabase() == EC_Normal) {
        ...
        if (filename) {
          ...
          if ((status = DVPSHelper::loadFileFormat(filename, image)) ==
            ...
            if (status != EC_Normal)
              DCMPSTAT_LOGFILE(
                  "Add image reference to presentation state failed: invalid "
                  "data structures");
          } else
            DCMPSTAT_LOGFILE(
                "Add image reference to presentation state failed: could not "
                "read fileformat");
          delete image;
        } else
          DCMPSTAT_LOGFILE(
              "Add image reference to presentation state failed: UIDs not in "
              "index file");
      } else
        DCMPSTAT_LOGFILE(
            "Add image reference to presentation state failed: could not lock "
            "index file");
    } else
      DCMPSTAT_LOGFILE(
          "Add image reference to presentation state failed: not the same "
          "study UID");
  } else
    DCMPSTAT_LOGFILE(
        "Add image reference to presentation state failed: invalid UIDs");
  return status;
}
...
OFCondition DVInterface::exchangeImageAndPState(DVPresentationState* newState,
  ...
  if (pState != newState) {
    ...
    if (pSignatureHandler) {
      if (pDicomPState)
        pSignatureHandler->updateDigitalSignatureInformation(
            *pDicomPState->getDataset(), DVPSS_presentationState, OFTrue);
      else
        pSignatureHandler->disableDigitalSignatureInformation(
  ...
  if (pDicomImage != image) {
    ...
    if (pSignatureHandler) {
      pSignatureHandler->updateDigitalSignatureInformation(
          *pDicomImage->getDataset(), DVPSS_image, OFTrue);
    }
  }
  return EC_Normal;
}
...
OFCondition DVInterface::resetPresentationState() {
  DVPresentationState* newState = new DVPresentationState(
      displayFunction, minimumPrintBitmapWidth, minimumPrintBitmapHeight,
      maximumPrintBitmapWidth, maximumPrintBitmapHeight,
      maximumPreviewImageWidth, maximumPreviewImageHeight);
  if (newState == NULL)
    return EC_MemoryExhausted;
  ...
  if ((pDicomImage) && (pDicomPState)) {
    ...
    if (dataset) {
      if (EC_Normal == (status = newState->read(*dataset)))
        status = newState->attachImage(pDicomImage, OFFalse);
      if (EC_Normal == status) {
        if (pState)
          delete pState;
        if (pStoredPState)
...
OFCondition DVInterface::saveCurrentPStateForReset() {
  ...
  if (pState != NULL) {
    ...
    if (fileformat != NULL) {
      ...
      if (dataset) {
        status = pState->write(
            *dataset, OFFalse);  // write current state to 'reset' dataset
        if (status == EC_Normal) {
          delete pDicomPState;
          pDicomPState = fileformat;
          fileformat =
              NULL;  // avoid deletion of pDicomPState a few lines later
        }
      } else
        status = EC_MemoryExhausted;
    } else
...
Uint32 DVInterface::getNumberOfPStates() {
  if (createPStateCache()) {
    ...
    if ((instance != NULL) && ((instance->Type == DVPSI_image) ||
                               (instance->Type == DVPSI_hardcopyGrayscale)))
...
OFCondition DVInterface::selectPState(Uint32 idx, OFBool changeStatus) {
  if (createPStateCache()) {
    ...
    if ((instance != NULL) && ((instance->Type == DVPSI_image) ||
                               (instance->Type == DVPSI_hardcopyGrayscale))) {
      OFListIterator(DVInstanceCache::ItemStruct*) iter =
          instance->List.begin();
      ...
      if (dataset != NULL) {
        ...
        if (newState != NULL) {
          if ((status = newState->createFromImage(*dataset)) == EC_Normal) {
            if ((status = newState->attachImage(pDicomImage, OFFalse)) ==
                EC_Normal) {
...
DVSeriesCache::ItemStruct* DVInterface::getSeriesStruct(
    const char* studyUID, const char* seriesUID, const char* instanceUID) {
...
DVInstanceCache::ItemStruct* DVInterface::getInstanceStruct(
    const char* studyUID, const char* seriesUID, const char* instanceUID) {
  if ((studyUID && seriesUID && instanceUID) ||
      (!studyUID && !seriesUID && !instanceUID)) {
    DVSeriesCache::ItemStruct* series =
        getSeriesStruct(studyUID, seriesUID, instanceUID);
    if (series != NULL)
...
OFCondition DVInterface::lockDatabase() {
  if (pHandle)
    return EC_Normal;  // may be called multiple times

  OFCondition result;
  pHandle = new DcmQueryRetrieveIndexDatabaseHandle(
      getDatabaseFolder(), PSTAT_MAXSTUDYCOUNT, PSTAT_STUDYSIZE, result);
  if (result.good()) {
    lockingMode = OFFalse;
...
void DVInterface::resetDatabaseReferenceTime() {
  ...
  // Modern macOS uses struct utimbuf instead
  struct utimbuf utime_buf;
  utime_buf.actime = OFstatic_cast(time_t, referenceTime);
  utime_buf.modtime = OFstatic_cast(time_t, referenceTime);
...
OFBool DVInterface::newInstancesReceived() {
  ...
  if (databaseIndexFile.length() > 0) {
    struct stat stat_buf;
    if (0 == stat(databaseIndexFile.c_str(), &stat_buf)) {
      if (OFstatic_cast(unsigned long, stat_buf.st_mtime) == referenceTime)
        return OFFalse;
    }
        ...
        if (study != NULL) {
          ...
          if (series != NULL) {
            if (!series->List.isElem(record.SOPInstanceUID)) {
              ...
              series->List.addItem(record.SOPInstanceUID, counter, record.hstat,
                                   type, record.ImageSize, record.filename);
              if (series->Type == DVPSI_image)
                series->Type =
                    type;  // series contains only one type of instances
            }
          }
        }
      }
                    ...
                    if (series->List.gotoFirst()) {
                      do { /* for all instances */
                        if (series->List.getType() == DVPSI_presentationState) {
                          DcmFileFormat* pstate = NULL;
                          if ((DVPSHelper::loadFileFormat(
                                   series->List.getFilename(), pstate) ==
                               EC_Normal) &&
                              pstate) {
                            DcmDataset* dataset = pstate->getDataset();
                            DVPSReferencedSeries_PList plist;
                            if (dataset &&
                                (plist.read(*dataset) == EC_Normal) &&
                                plist.isValid()) {
                              if (plist.findImageReference(
                                      seriesUID.c_str(), instanceUID.c_str())) {
                                DVInstanceCache::ItemStruct* reference =
                                    series->List.getItem();
                                if (reference != NULL) {
                                  DcmStack stack;
                                  if (dataset->search(DCM_ContentDescription,
                                                      ...
                                                      OFFalse) == EC_Normal) {
                                    char* value = NULL;
                                    if ((*OFstatic_cast(DcmLongString*,
                                                        stack.top()))
                                            .getString(value) == EC_Normal)
                                      reference->Description = value;
                                  }
                                  stack.clear();
                                  if (dataset->search(DCM_ContentLabel, stack,
                                                      ESM_fromHere,
                                                      OFFalse) == EC_Normal) {
                                    char* value = NULL;
                                    if ((*OFstatic_cast(DcmLongString*,
...
void DVInterface::clearIndexRecord(IdxRecord& record, int& recpos) {
  OFBitmanipTemplate<Uint8>::zeroMem(OFreinterpret_cast(Uint8*, &record),
                                     sizeof(idxRec));
  recpos = -1;
}

OFBool DVInterface::readIndexRecord(const int pos, IdxRecord& record,
                                    int* oldpos) {
  if (lockDatabase() == EC_Normal) {
          ...
          do { /* for all series */
            ...
            if ((series != NULL) && series->List.isElem(instanceUID)) {
              if (readIndexRecord(series->List.getPos(), idxRec, &idxRecPos)) {
                if (sopClassUID == NULL)
                  return EC_Normal;
                else if ((idxRec.SOPClassUID != NULL) &&
                         (strcmp(sopClassUID, idxRec.SOPClassUID) == 0))
                  return EC_Normal;
              }
            }
          } while (study->List.gotoNext());
...
OFCondition DVInterface::instanceReviewed(const char* studyUID,
                                          ...
                                          const char* instanceUID) {
  OFCondition result = EC_IllegalCall;
  DVInstanceCache::ItemStruct* instance =
      getInstanceStruct(studyUID, seriesUID, instanceUID);
  if (instance != NULL) {
...
int DVInterface::findStudyIdx(StudyDescRecord* study, const char* uid) {
...
int DVInterface::deleteImageFile(const char* filename) {
  if ((filename != NULL) && (pHandle != NULL)) {
    const char* pos;
    if (((pos = strrchr(filename, OFstatic_cast(int, PATH_SEPARATOR))) ==
         NULL) ||  // check whether image file resides in index.dat directory
...
OFCondition DVInterface::deleteStudy(const char* studyUID) {
  ...
  if (study != NULL) {
    ...
    if (lockExclusive() == EC_Normal) {
      ...
      if (study->List.gotoFirst()) {
        StudyDescRecord* study_desc =
            OFstatic_cast(StudyDescRecord*, malloc(SIZEOF_STUDYDESC));
        if (study_desc != NULL) {
...
OFCondition DVInterface::setAmbientLightValue(double value) {
  OFCondition result = EC_IllegalCall;
  for (int i = DVPSD_first; i < DVPSD_max; i++) {
    if ((displayFunction[i] != NULL) &&
        (displayFunction[i]->setAmbientLightValue(value)))
...
OFCondition DVInterface::sendIOD(const char* targetID, const char* studyUID,
                                 const char* seriesUID,
                                 const char* instanceUID) {
  if ((targetID == NULL) || (studyUID == NULL))
    return EC_IllegalCall;
  const char* sender_application = getSenderName();
  if (sender_application == NULL)
    return EC_IllegalCall;
  if (configPath.length() == 0)
    return EC_IllegalCall;

  DVPSHelper::cleanChildren();  // clean up old child processes before creating
                                // new ones
  ...
  if (seriesUID && instanceUID)
    sprintf(commandline, "%s %s %s %s %s %s", sender_application,
            configPath.c_str(), targetID, studyUID, seriesUID, instanceUID);
  else if (seriesUID)
    sprintf(commandline, "%s %s %s %s %s", sender_application,
            configPath.c_str(), targetID, studyUID, seriesUID);
  else
    sprintf(commandline, "%s %s %s %s", sender_application, configPath.c_str(),
            targetID, studyUID);
#ifdef DEBUG
  if (CreateProcess(NULL, commandline, NULL, NULL, 0, 0, NULL, NULL, &sinfo,
                    &procinfo))
#else
  if (CreateProcess(NULL, commandline, NULL, NULL, 0, DETACHED_PROCESS, NULL,
                    NULL, &sinfo, &procinfo))
#endif
  {
    return EC_Normal;
...
OFCondition DVInterface::startReceiver() {
  ...
  for (Uint32 i = 0; i < numberOfReceivers; i++) {
    DVPSHelper::cleanChildren();  // clean up old child processes before
                                  // creating new ones
#ifdef HAVE_FORK
    // Unix version - call fork() and execl()
    pid_t pid = fork();
    if (pid < 0) {
      ...
      // we are the child process
      if (execl(receiver_application, receiver_application, configPath.c_str(),
                getTargetID(i, DVPSE_receiver), NULL) < 0) {
    ...
    sinfo.cb = sizeof(sinfo);
    char commandline[4096];
    sprintf(commandline, "%s %s %s", receiver_application, configPath.c_str(),
            getTargetID(i, DVPSE_receiver));
#ifdef DEBUG
    if (CreateProcess(NULL, commandline, NULL, NULL, 0, 0, NULL, NULL, &sinfo,
                      &procinfo))
#else
...
OFCondition DVInterface::startQueryRetrieveServer() {
  ...
  if (pid < 0) {
    ...
    if (timeout > 0) {
      ...
      execl(server_application, server_application, "-c",
            config_filename.c_str(), "--allow-shutdown", "--timeout",
            str_timeout, NULL);
    } else {
      execl(server_application, server_application, "-c",
            config_filename.c_str(), "--allow-shutdown", NULL);
    }
  ...
  if (timeout > 0) {
    sprintf(commandline, "%s -c %s --allow-shutdown --timeout %lu",
            server_application, config_filename.c_str(),
            (unsigned long)timeout);
  } else {
    sprintf(commandline, "%s -c %s --allow-shutdown", server_application,
...
OFCondition DVInterface::terminateQueryRetrieveServer() {
  if (getQueryRetrieveServerName() == NULL)
    return EC_IllegalCall;
  if (configPath.length() == 0)
    return EC_IllegalCall;
  ...
  if (cond.good()) {
    ...
    if (cond.good()) {
      ASC_setAPTitles(params, getNetworkAETitle(), getQueryRetrieveAETitle(),
                      NULL);
      gethostname(localHost, sizeof(localHost) - 1);
      sprintf(peerHost, "localhost:%d",
              OFstatic_cast(int, getQueryRetrievePort()));
      ASC_setPresentationAddresses(params, localHost, peerHost);

      const char* transferSyntaxes[] = {UID_LittleEndianImplicitTransferSyntax};
      cond = ASC_addPresentationContext(params, 1, UID_PrivateShutdownSOPClass,
                                        transferSyntaxes, 1);
      if (cond.good()) {
        cond = ASC_requestAssociation(net, params, &assoc);
        if (cond.good())
          ASC_abortAssociation(assoc);  // tear down association if necessary
...
OFCondition DVInterface::createQueryRetrieveServerConfigFile(
    const char* filename) {
  STD_NAMESPACE ofstream output(filename);
  if (output) {
    DCMPSTAT_LOGFILE("Creating configuration file for query/retrieve server");
    output << "# ATTENTION: This file has been created automatically and will"
           << OFendl;
    output << "#            be re-created each time the query/retrieve server"
           << OFendl;
    output << "#            is started.  To avoid that manual changes to this"
           << OFendl;
    output << "#            file are destroyed, the flag AutoCreateConfigFile"
           << OFendl;
    output << "#            in the configuration file '" << configPath
           << "' has to be" << OFendl;
    ...
    output << "MaxAssociations = " << getQueryRetrieveMaxAssociations()
           << OFendl;
    ...
    for (Uint32 i = 0; i < count; i++) {
      ...
      if (id != NULL) {
        ...
        if ((aet != NULL) && (name != NULL))
          output << id << " = (" << aet << ", " << name << ", "
                 << getTargetPort(id) << ")" << OFendl;
      }
    }
    output << "HostTable END" << OFendl;
    output << OFendl;
    output << "AETable BEGIN" << OFendl;
    output << getQueryRetrieveAETitle() << "\t" << getDatabaseFolder()
           << "\tR\t(";
    output << PSTAT_MAXSTUDYCOUNT << ", " << PSTAT_STUDYSIZE / 1024 / 1024
           << "mb)\tANY" << OFendl;
    output << "AETable END" << OFendl;
    return EC_Normal;
  }
  DCMPSTAT_LOGFILE(
      "Could not create configuration file for query/retrieve server");
  return EC_IllegalCall;
}
...
OFCondition DVInterface::saveDICOMImage(const char* filename,
                                        ...
                                        double aspectRatio, OFBool explicitVR,
                                        const char* instanceUID) {
  ...
  if (dataset) {
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_PatientName);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_PatientID);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_PatientBirthDate);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_PatientSex);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_StudyDate);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_StudyTime);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_ReferringPhysicianName);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_StudyID);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_AccessionNumber);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_SeriesNumber);
    if (EC_Normal == status)
    ...
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_SOPClassUID,
                                          UID_SecondaryCaptureImageStorage);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_Modality, "OT");
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_ConversionType, "WSD");
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(
          dataset, DCM_PhotometricInterpretation, "MONOCHROME2");
    dcmGenerateUniqueIdentifier(newuid);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_SOPInstanceUID,
                                          (instanceUID ? instanceUID : newuid));
    dcmGenerateUniqueIdentifier(newuid, SITE_SERIES_UID_ROOT);
    if (EC_Normal == status)
      status =
          DVPSHelper::putStringValue(dataset, DCM_SeriesInstanceUID, newuid);
    dcmGenerateUniqueIdentifier(newuid, SITE_STUDY_UID_ROOT);
    if (EC_Normal == status)
      status =
          DVPSHelper::putStringValue(dataset, DCM_StudyInstanceUID, newuid);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_SamplesPerPixel, 1);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_Rows, rows);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_Columns, columns);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_BitsAllocated, 8);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_BitsStored, 8);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_HighBit, 7);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_PixelRepresentation, 0);
    if ((EC_Normal == status) && (aspectRatio != 1.0)) {
      sprintf(newuid, "%ld\\%ld", 1000L,
              OFstatic_cast(long, aspectRatio * 1000.0));
      status =
          DVPSHelper::putStringValue(dataset, DCM_PixelAspectRatio, newuid);
    }
    DcmPolymorphOBOW* pxData = new DcmPolymorphOBOW(DCM_PixelData);
    if (pxData) {
      status = pxData->putUint8Array(
          OFstatic_cast(Uint8*, OFconst_cast(void*, pixelData)),
          OFstatic_cast(unsigned long, width* height));
      if (EC_Normal == status)
        status = dataset->insert(pxData, OFTrue /*replaceOld*/);
      else
        delete pxData;
    } else
      status = EC_MemoryExhausted;
    ...
    if (EC_Normal == status) {
      ...
      if (status != EC_Normal)
        DCMPSTAT_LOGFILE(
            "Save image to file failed: could not write fileformat");
    }
  } else {
    status = EC_MemoryExhausted;
...
OFCondition DVInterface::saveDICOMImage(const void* pixelData,
  ...
  if (result.bad()) {
    DCMPSTAT_LOGFILE(
        "Save image to database failed: could not lock index file");
    return result;
  }

  if (handle
          .makeNewStoreFileName(UID_SecondaryCaptureImageStorage, uid,
                                imageFileName)
          .good()) {
    // now store presentation state as filename
    result = saveDICOMImage(imageFileName, pixelData, width, height,
                            aspectRatio, OFTrue, uid);
    if (EC_Normal == result) {
      if (handle
              .storeRequest(UID_SecondaryCaptureImageStorage, uid,
                            imageFileName, &dbStatus)
              .bad()) {
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Save image to database failed: could not in index file");
        DCMPSTAT_DEBUG("Unable to secondary capture image '"
                       << imageFileName << "' in database");
      }
    }
  }
  return result;
...
OFCondition DVInterface::saveHardcopyGrayscaleImage(
    const char* filename, const void* pixelData, unsigned long width,
    unsigned long height, double aspectRatio, OFBool explicitVR,
    const char* instanceUID) {
  if (pState == NULL)
    return EC_IllegalCall;
  if (pPrint == NULL)
  ...
  if (dataset) {
    ...
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(
          dataset, DCM_RETIRED_HardcopyDeviceManufacturer, "OFFIS");
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(
          dataset, DCM_RETIRED_HardcopyDeviceSoftwareVersion,
    ...
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_PatientOrientation);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_ImageType,
                                          "DERIVED\\SECONDARY");
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(
          dataset, DCM_DerivationDescription,
          "Hardcopy rendered using Presentation State");
    // source image sequence is written in
    // pState->writeHardcopyImageAttributes().
    ...
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_InstanceCreationDate,
                                          aString.c_str());
    DVPSHelper::currentTime(aString);
    if (EC_Normal == status)
      status = DVPSHelper::putStringValue(dataset, DCM_InstanceCreationTime,
                                          aString.c_str());
    ...
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_Columns, columns);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_BitsAllocated, 16);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_BitsStored, 12);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_HighBit, 11);
    if (EC_Normal == status)
      status = DVPSHelper::putUint16Value(dataset, DCM_PixelRepresentation, 0);
    if ((EC_Normal == status) && (aspectRatio != 1.0)) {
      sprintf(newuid, "%ld\\%ld", 1000L,
    ...
    DcmPolymorphOBOW* pxData = new DcmPolymorphOBOW(DCM_PixelData);
    if (pxData) {
      status = pxData->putUint16Array(
          OFstatic_cast(Uint16*, OFconst_cast(void*, pixelData)),
          OFstatic_cast(unsigned long, width* height));
      if (EC_Normal == status)
        status = dataset->insert(pxData, OFTrue /*replaceOld*/);
      else
    ...
    // add Presentation LUT to hardcopy file if present, making it a Standard
    // Extended SOP Class
    if ((EC_Normal == status) &&
        (pState->getPresentationLUT() == DVPSP_table)) {
      status = pState->writePresentationLUTforPrint(*dataset);
    }

    if (status != EC_Normal)
      DCMPSTAT_LOGFILE(
          "Save hardcopy grayscale image to file failed: invalid data "
          "structures");
    ...
    if (EC_Normal == status) {
      ...
      if (status != EC_Normal)
        DCMPSTAT_LOGFILE(
            "Save hardcopy grayscale image to file failed: could not write "
            "fileformat");
    }
  } else {
    status = EC_MemoryExhausted;
    DCMPSTAT_LOGFILE(
        "Save hardcopy grayscale image to file failed: memory exhausted");
  }
  ...
  if (EC_Normal == status) {
    ...
    /* we don't pass the patient ID (available as pState->getPatientID()) here
     * because then we could end up with multiple images being part of one study
     * and one series, but having different patient IDs. This might confuse
     * archives using the patient root query model.
     */
    status = pPrint->addImageBox(getNetworkAETitle(), theInstanceUID.c_str(),
                                 reqImageSize, NULL, presLUT,
...
OFCondition DVInterface::saveHardcopyGrayscaleImage(const void* pixelData,
  ...
  if (result.bad()) {
    DCMPSTAT_LOGFILE(
        "Save hardcopy grayscale image to database failed: could not lock "
        "index file");
    return result;
  }

  if (handle
          .makeNewStoreFileName(UID_RETIRED_HardcopyGrayscaleImageStorage, uid,
                                imageFileName)
          .good()) {
    result = saveHardcopyGrayscaleImage(imageFileName, pixelData, width, height,
                                        aspectRatio, OFTrue, uid);
    if (EC_Normal == result) {
      if (handle
              .storeRequest(UID_RETIRED_HardcopyGrayscaleImageStorage, uid,
                            imageFileName, &dbStatus)
              .bad()) {
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Save hardcopy grayscale image to database failed: could not in "
            "index file");
        DCMPSTAT_DEBUG("Unable to hardcopy grayscale image '"
                       << imageFileName << "' in database");
      }
    }
  }
...
OFCondition DVInterface::saveFileFormatToDB(DcmFileFormat& fileformat) {
  ...
  if (result.bad()) {
    DCMPSTAT_LOGFILE(
        "Save fileformat to database failed: could not lock index file");
    return result;
  }

  if (handle.makeNewStoreFileName(classUID, instanceUID, imageFileName)
          .good()) {
    // save image file
    result = DVPSHelper::saveFileFormat(imageFileName, &fileformat, OFTrue);
    if (EC_Normal == result) {
      if (handle.storeRequest(classUID, instanceUID, imageFileName, &dbStatus)
              .bad()) {
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Save fileformat to database failed: could not in index file");
        DCMPSTAT_DEBUG("Unable to file '" << imageFileName << "' in database");
      }
    }
  }
  return result;
  ...
  if (studyUID && seriesUID && instanceUID) {
    if (lockDatabase() == EC_Normal) {
      ...
      if (filename) {
        ...
        DCMPSTAT_LOGFILE(
            "Load stored print from database failed: UIDs not in index file");
    } else
      DCMPSTAT_LOGFILE(
          "Load stored print from database failed: could not lock index file");
  } else
...
OFCondition DVInterface::loadStoredPrint(const char* filename) {
  OFCondition status = EC_IllegalCall;
  DcmFileFormat* fileformat = NULL;
  DVPSStoredPrint* print = new DVPSStoredPrint(getDefaultPrintIllumination(),
  ...
  if ((status = DVPSHelper::loadFileFormat(filename, fileformat)) ==
    ...
    if (status != EC_Normal)
      DCMPSTAT_LOGFILE(
          "Load stored print from file failed: invalid data structures");
  } else
    DCMPSTAT_LOGFILE(
        "Load stored print from file failed: could not read fileformat");
  if (status != EC_Normal) {
    delete print;
  }
  return status;
...
OFCondition DVInterface::saveStoredPrint(const char* filename,
  ...
  if (activateAnnotation) {
    ...
    OFString dummy;
    if (prependDateTime) {
      OFDateTime::getCurrentDateTime().getISOFormattedDateTime(
          text, OFFalse /*showSeconds*/);
    }
    ...
    if (prependLighting) {
      sprintf(buf, "%d/%d ", pPrint->getPrintIllumination(),
              pPrint->getPrintReflectedAmbientLight());
      text += buf;
    }
    text += annotationText;
    if (text.size() > 64)
      text.erase(64);  // limit to max annotation length

    if (getTargetPrinterSupportsAnnotationBoxSOPClass(currentPrinter.c_str())) {
      const char* displayformat = getTargetPrinterAnnotationDisplayFormatID(
          currentPrinter.c_str(), dummy);
      Uint16 position =
          getTargetPrinterAnnotationPosition(currentPrinter.c_str());
  ...
  if (dataset) {
    if (instanceUID)
      status = pPrint->setInstanceUID(instanceUID);
    else {
      dcmGenerateUniqueIdentifier(newuid);
      status = pPrint->setInstanceUID(newuid);
    }
    if (EC_Normal == status)
      status = pPrint->write(*dataset, writeRequestedImageSize, OFTrue, OFTrue,
                             OFFalse);
    ...
    if (status != EC_Normal)
      DCMPSTAT_LOGFILE(
          "Save stored print to file failed: could not write fileformat");
  } else {
...
OFCondition DVInterface::saveStoredPrint(OFBool writeRequestedImageSize) {
  ...
  if (result.bad()) {
    DCMPSTAT_LOGFILE(
        "Save stored print to database failed: could not lock index file");
    return result;
  }

  if (handle
          .makeNewStoreFileName(UID_RETIRED_StoredPrintStorage, uid,
                                imageFileName)
          .good()) {
    // now store stored print object as filename
    result =
        saveStoredPrint(imageFileName, writeRequestedImageSize, OFTrue, uid);
    if (EC_Normal == result) {
      if (handle
              .storeRequest(UID_RETIRED_StoredPrintStorage, uid, imageFileName,
                            &dbStatus)
              .bad()) {
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Save stored print to database failed: could not in index file");
        DCMPSTAT_DEBUG("Unable to stored print object '" << imageFileName
                                                         << "' in database");
      }
    }
  }
...
OFCondition DVInterface::loadPrintPreview(size_t idx, OFBool printLUT,
                                          OFBool changeStatus) {
  OFCondition status = EC_IllegalCall;
  if ((pPrint != NULL) && (maximumPrintPreviewWidth > 0) &&
      (maximumPrintPreviewHeight > 0)) {
    const char* studyUID;
    const char* seriesUID;
    const char* instanceUID;
    if ((status = pPrint->getImageReference(idx, studyUID, seriesUID,
                                            instanceUID)) == EC_Normal) {
      status = EC_IllegalCall;
      const char* filename = getFilename(studyUID, seriesUID, instanceUID);
      if (filename) {
        ...
        if (image != NULL) {
          if (image->getStatus() == EIS_Normal) {
            ...
            if (ratio == 0.0)
              ratio = 1.0;
            if (OFstatic_cast(double, image->getWidth()) /
                    OFstatic_cast(double, width* ratio) <
                OFstatic_cast(double, image->getHeight()) /
                    OFstatic_cast(double, height)) {
              width = 0;
            } else
              height = 0;
            image->setWidthHeightRatio(ratio);
            pHardcopyImage = image->createScaledImage(
                width, height, 0 /*interpolate*/, 1 /*aspect ratio*/);
            if (pHardcopyImage != NULL) {
              if (pHardcopyImage->getStatus() == EIS_Normal) {
                /* set display function for calibrated output */
                if (displayFunction != NULL)
                  pHardcopyImage->setDisplayFunction(
                      displayFunction[DVPSD_GSDF]);
                ...
                DVPSPresentationLUT* plut =
                    pPrint->getPresentationLUT();  // first check whether
                                                   // there's a global one
                if (plut == NULL)
                  plut = pPrint->getImagePresentationLUT(
                      idx);  // ... then check for an image box specific
                if (plut != NULL) {
                  pHardcopyImage->setHardcopyParameters(
                      pPrint->getMinDensityValue(),
                      pPrint->getMaxDensityValue(),
                      pPrint->getPrintReflectedAmbientLight(),
              ...
              DCMPSTAT_LOGFILE(
                  "Load hardcopy grayscale image for print preview failed: "
                  "memory exhausted");
          } else
            DCMPSTAT_LOGFILE(
                "Load hardcopy grayscale image for print preview failed: could "
                "not read image");
          delete image;
        } else
          DCMPSTAT_LOGFILE(
              "Load hardcopy grayscale image for print preview failed: memory "
              "exhausted");
      } else
        DCMPSTAT_LOGFILE(
            "Load hardcopy grayscale image for print preview failed: UIDs not "
            "in index file");
    }
  }
  return status;
...
void DVInterface::setMaxPrintPreviewWidthHeight(unsigned long width,
                                                unsigned long height) {
  if ((width != maximumPrintPreviewWidth) ||
      (height != maximumPrintPreviewHeight)) {
    unloadPrintPreview();
...
OFCondition DVInterface::setCurrentPrinter(const char* targetID) {
  if (targetID == NULL)
    return EC_IllegalCall;
  if (getTargetHostname(targetID) == NULL)
...
OFCondition DVInterface::selectDisplayPresentationLUT(const char* lutID) {
  ...
  if (lutID && pState) {
    ...
    if (lutfile) {
      ...
      filename += lutfile;
      DcmFileFormat* fileformat = NULL;
      if ((result = DVPSHelper::loadFileFormat(filename.c_str(), fileformat)) ==
          EC_Normal) {
        if (fileformat) {
          DcmDataset* dataset = fileformat->getDataset();
          if (dataset)
        ...
        if (result != EC_Normal)
          DCMPSTAT_LOGFILE(
              "Load display presentation LUT from file: invalid data "
              "structures");
      } else
        DCMPSTAT_LOGFILE(
            "Load display presentation LUT from file: could not read "
            "fileformat");
      delete fileformat;
    } else
      DCMPSTAT_LOGFILE(
          "Load display presentation LUT from file: not specified in config "
          "file");
  }
  return result;
}
  ...
  if (lutID && pPrint) {
    ...
    if (lutfile) {
      ...
      if ((result = DVPSHelper::loadFileFormat(filename.c_str(), fileformat)) ==
        ...
        if (result != EC_Normal)
          DCMPSTAT_LOGFILE(
              "Load print presentation LUT from file: invalid data structures");
      } else
        DCMPSTAT_LOGFILE(
            "Load print presentation LUT from file: could not read fileformat");
      delete fileformat;
    } else
      DCMPSTAT_LOGFILE(
          "Load print presentation LUT from file: not specified in config "
          "file");
  }
  return result;
}
...
OFCondition DVInterface::spoolPrintJob(OFBool deletePrintedImages) {
  if (pPrint == NULL)
    return EC_IllegalCall;
  if (currentPrinter.size() == 0)
    return EC_IllegalCall;

  OFCondition result = saveStoredPrint(
      getTargetPrinterSupportsRequestedImageSize(currentPrinter.c_str()));
  if (EC_Normal == result) {
    result = spoolStoredPrintFromDB(pPrint->getStudyInstanceUID(),
                                    pPrint->getSeriesInstanceUID(),
                                    pPrint->getSOPInstanceUID());
  }
  if ((EC_Normal == result) && deletePrintedImages)
    result = pPrint->deleteSpooledImages();
  return result;
}

OFCondition DVInterface::startPrintSpooler() {
  const char* spooler_application = getSpoolerName();
  if (spooler_application == NULL)
    return EC_IllegalCall;
  if (configPath.length() == 0)
    return EC_IllegalCall;
  ...
  if (numberOfPrinters > 0)
    for (Uint32 i = 0; i < numberOfPrinters; i++) {
      ...
      pid_t pid = fork();
      if (pid < 0)
        result = EC_IllegalCall;  // fork failed - return error code
      else if (pid == 0) {
        // we are the child process

        if (detailedLog) {
          if (execl(spooler_application, spooler_application, "--verbose",
                    "--dump", "--spool", printJobIdentifier.c_str(),
                    "--printer", printer, "--config", configPath.c_str(),
                    "--sleep", sleepStr, NULL) < 0) {
            DCMPSTAT_ERROR("Unable to execute '" << spooler_application << "'");
          }
        } else {
          if (execl(spooler_application, spooler_application, "--spool",
                    printJobIdentifier.c_str(), "--printer", printer,
                    "--config", configPath.c_str(), "--sleep", sleepStr,
                    NULL) < 0) {
            DCMPSTAT_ERROR("Unable to execute '" << spooler_application << "'");
          }
        }
      ...
      if (detailedLog) {
        sprintf(commandline,
                "%s --verbose --dump --spool %s --printer %s --config %s "
                "--sleep %s",
                spooler_application, printJobIdentifier.c_str(), printer,
                configPath.c_str(), sleepStr);
      } else {
        sprintf(commandline,
                "%s --spool %s --printer %s --config %s --sleep %s",
                spooler_application, printJobIdentifier.c_str(), printer,
                configPath.c_str(), sleepStr);
      }
#ifdef DEBUG
      if (0 == CreateProcess(NULL, commandline, NULL, NULL, 0, 0, NULL, NULL,
                             &sinfo, &procinfo))
#else
      if (0 == CreateProcess(NULL, commandline, NULL, NULL, 0, DETACHED_PROCESS,
...
OFCondition DVInterface::terminatePrintSpooler() {
  ...
  if (numberOfPrinters > 0)
    for (Uint32 i = 0; i < numberOfPrinters; i++) {
      ...
      if (EC_Normal !=
          createPrintJobFilenames(prt, tempFilename, spoolFilename))
        return EC_IllegalCall;
      ...
      if (outf) {
        ...
        if (0 != rename(tempFilename.c_str(), spoolFilename.c_str())) {
          DCMPSTAT_ERROR("Unable to activate spooler termination request '"
                         << spoolFilename.c_str() << "'");
          return EC_IllegalCall;
        }
      } else {
        DCMPSTAT_ERROR("Unable to create spooler termination request '"
                       << tempFilename.c_str() << "'");
        return EC_IllegalCall;
      }
    }
  ...
  if (numberOfPrinters > 0)
    for (Uint32 i = 0; i < numberOfPrinters; i++) {
      ...
      if (pid < 0)
        ...
        if (detailedLog) {
          if (execl(application, application, "--logfile", "--verbose",
                    "--dump", "--printer", printer, "--config",
                    configPath.c_str(), NULL) < 0) {
            DCMPSTAT_ERROR("Unable to execute '" << application << "'");
          }
        } else {
          if (execl(application, application, "--logfile", "--printer", printer,
                    "--config", configPath.c_str(), NULL) < 0) {
            DCMPSTAT_ERROR("Unable to execute '" << application << "'");
          }
        }
      ...
      if (detailedLog) {
        sprintf(commandline,
                "%s --logfile --verbose --dump --printer %s --config %s",
                application, printer, configPath.c_str());
      } else {
        sprintf(commandline, "%s --logfile --printer %s --config %s",
                application, printer, configPath.c_str());
      }
#ifdef DEBUG
      if (0 == CreateProcess(NULL, commandline, NULL, NULL, 0, 0, NULL, NULL,
...
OFCondition DVInterface::terminatePrintServer() {
  ...
  if (!getTLSPEMFormat())
    keyFileFormat = SSL_FILETYPE_ASN1;
#endif
      ...
      if (cond.good()) {
        ...
        if (cond.good()) {
          if (useTLS) {
            ...
            if (current)
              tlsCertificateFile += current;
            else
              tlsCertificateFile += "sitecert.pem";
            ...
            if (current)
              tlsPrivateKeyFile += current;
            else
              tlsPrivateKeyFile += "sitekey.pem";

            /* private key password */
            const char* tlsPrivateKeyPassword =
                getTargetPrivateKeyPassword(target);
            ...
            if (current)
              tlsRandomSeedFile += current;
            else
              tlsRandomSeedFile += "siteseed.bin";

            /* CA certificate directory */
            const char* tlsCACertificateFolder = getTLSCACertificateFolder();
            if (tlsCACertificateFolder == NULL)
              tlsCACertificateFolder = ".";

          /* ciphersuites */
#if OPENSSL_VERSION_NUMBER >= 0x0090700fL
            OFString tlsCiphersuites(TLS1_TXT_RSA_WITH_AES_128_SHA
                                     ":" SSL3_TXT_RSA_DES_192_CBC3_SHA);
#else
            OFString tlsCiphersuites(SSL3_TXT_RSA_DES_192_CBC3_SHA);
#endif
            Uint32 tlsNumberOfCiphersuites =
                getTargetNumberOfCipherSuites(target);
            if (tlsNumberOfCiphersuites > 0) {
              ...
              for (Uint32 ui = 0; ui < tlsNumberOfCiphersuites; ui++) {
                ...
                if (NULL !=
                    (currentOpenSSL =
                         DcmTLSTransportLayer::findOpenSSLCipherSuiteName(
                             currentSuite.c_str()))) {
                  if (tlsCiphersuites.length() > 0)
                    tlsCiphersuites += ":";
            ...
            DcmTLSTransportLayer* tLayer = new DcmTLSTransportLayer(
                DICOM_APPLICATION_REQUESTOR, tlsRandomSeedFile.c_str());
            if (tLayer) {
              if (tlsCACertificateFolder)
                tLayer->addTrustedCertificateDir(tlsCACertificateFolder,
                                                 keyFileFormat);
              if (tlsDHParametersFile.size() > 0)
                tLayer->setTempDHParameters(tlsDHParametersFile.c_str());
              tLayer->setPrivateKeyPasswd(
                  tlsPrivateKeyPassword);  // never prompt on console
              tLayer->setPrivateKeyFile(tlsPrivateKeyFile.c_str(),
                                        keyFileFormat);
              tLayer->setCertificateFile(tlsCertificateFile.c_str(),
                                         keyFileFormat);
...
#else
            // we cannot shutdown a TLS process since we're compiled without
            // OpenSSL support
            cond = EC_IllegalCall;
#endif
          }

          ASC_setAPTitles(params, getNetworkAETitle(), getTargetAETitle(target),
                          NULL);
          gethostname(localHost, sizeof(localHost) - 1);
          sprintf(peerHost, "%s:%d", getTargetHostname(target),
                  OFstatic_cast(int, getTargetPort(target)));
          ASC_setPresentationAddresses(params, localHost, peerHost);
          ...
          if (cond.good())
            cond = ASC_addPresentationContext(
                params, 1, UID_PrivateShutdownSOPClass, transferSyntaxes, 1);
          if (cond.good()) {
            cond = ASC_requestAssociation(net, params, &assoc);
            if (cond.good())
              ASC_abortAssociation(
                  assoc);  // tear down association if necessary
            ASC_dropAssociation(assoc);
            ASC_destroyAssociation(&assoc);
          }
        } else
    ...
    if (studyUID && seriesUID && instanceUID) {
      if (EC_Normal == (result = lockDatabase())) {
        ...
        if (filename) {
          ...
          if ((result = DVPSHelper::loadFileFormat(filename, ff)) ==
              EC_Normal) {
            if (ff) {
              ...
              if (dataset) {
                ...
                if (EC_Normal != presentationLUT.read(*dataset, OFFalse))
                  presentationLUT.setType(DVPSP_identity);
                result = dataset->search(sopclassuid.getTag(), stack,
                                         ESM_fromHere, OFFalse);
                if (EC_Normal == result) {
                  char* sopclass = NULL;
                  sopclassuid =
                      *OFstatic_cast(DcmUniqueIdentifier*, stack.top());
                  if (EC_Normal == sopclassuid.getString(sopclass))
                    result = pPrint->addImageBox(
                        getNetworkAETitle(), studyUID, seriesUID, sopclass,
                        instanceUID, NULL, NULL, &presentationLUT, OFFalse);
                  else
                    result = EC_IllegalCall;
                }
              } else
            ...
            if (result != EC_Normal)
              DCMPSTAT_LOGFILE(
                  "Load hardcopy grayscale image from file failed: invalid "
                  "data structures");
          } else
            DCMPSTAT_LOGFILE(
                "Load hardcopy grayscale image from file failed: could not "
                "read fileformat");
          if (ff)
            delete ff;
        } else {
          result = EC_IllegalCall;
          DCMPSTAT_LOGFILE(
              "Load hardcopy grayscale image from database failed: UIDs not in "
              "index file");
        }
      } else
        DCMPSTAT_LOGFILE(
            "Load hardcopy grayscale image from database failed: could not "
            "lock index file");
    } else
      DCMPSTAT_LOGFILE(
          "Load hardcopy grayscale image from database failed: invalid UIDs");
  }
...
OFCondition DVInterface::spoolStoredPrintFromDB(const char* studyUID,
  ...
  if (EC_Normal != createPrintJobFilenames(prt, tempFilename, spoolFilename))
    return EC_IllegalCall;
  ...
  if (outf) {
    ...
    fprintf(outf, "study        %s\nseries       %s\ninstance     %s\n",
            studyUID, seriesUID, instanceUID);
    if (printerMediumType.size() > 0)
      fprintf(outf, "mediumtype   %s\n", printerMediumType.c_str());
    if (printerFilmDestination.size() > 0)
      fprintf(outf, "destination  %s\n", printerFilmDestination.c_str());
    if (printerFilmSessionLabel.size() > 0)
      fprintf(outf, "label        %s\n", printerFilmSessionLabel.c_str());
    if (printerPriority.size() > 0)
      fprintf(outf, "priority     %s\n", printerPriority.c_str());
    if (printerOwnerID.size() > 0)
      fprintf(outf, "owner_id     %s\n", printerOwnerID.c_str());
    if (printerNumberOfCopies > 0)
      fprintf(outf, "copies       %lu\n", printerNumberOfCopies);
...
OFCondition DVInterface::printSCUcreateBasicFilmSession(
  ...
  if ((EC_Normal == result) && (printerMediumType.size() > 0)) {
    ...
    if (delem)
      result = delem->putString(printerMediumType.c_str());
    else
      result = EC_IllegalCall;
    if (EC_Normal == result)
      result = dset.insert(delem, OFTrue /*replaceOld*/);
  }

  if ((EC_Normal == result) && (printerFilmDestination.size() > 0)) {
    delem = new DcmCodeString(DCM_FilmDestination);
    if (delem)
      result = delem->putString(printerFilmDestination.c_str());
    else
      result = EC_IllegalCall;
    if (EC_Normal == result)
      result = dset.insert(delem, OFTrue /*replaceOld*/);
  ...
  if ((EC_Normal == result) && (printerFilmSessionLabel.size() > 0)) {
    ...
    if (delem)
      result = delem->putString(printerFilmSessionLabel.c_str());
    else
      result = EC_IllegalCall;
    if (EC_Normal == result)
      result = dset.insert(delem, OFTrue /*replaceOld*/);
  ...
  if ((EC_Normal == result) && (printerPriority.size() > 0)) {
    ...
    if (delem)
      result = delem->putString(printerPriority.c_str());
    else
      result = EC_IllegalCall;
    if (EC_Normal == result)
      result = dset.insert(delem, OFTrue /*replaceOld*/);
  ...
  if ((EC_Normal == result) && (printerOwnerID.size() > 0)) {
    ...
    if (delem)
      result = delem->putString(printerOwnerID.c_str());
    else
      result = EC_IllegalCall;
    if (EC_Normal == result)
      result = dset.insert(delem, OFTrue /*replaceOld*/);
  ...
  if (EC_Normal == result)
    result = pPrint->printSCUcreateBasicFilmSession(printHandler, dset,
                                                    plutInSession);
  return result;
}
...
OFCondition DVInterface::startExternalApplication(const char* application,
  ...
  pid_t pid = fork();
  if (pid < 0)
    return EC_IllegalCall;  // fork failed - return error code
  else if (pid > 0)
    return EC_Normal;  // we are the parent process
  else {
    // we are the child process
...
OFCondition DVInterface::dumpIOD(const char* studyUID, const char* seriesUID,
                                 const char* instanceUID) {
  OFCondition result = EC_IllegalCall;
  if (studyUID && seriesUID && instanceUID) {
    if (EC_Normal == (result = lockDatabase())) {
      ...
      if (filename)
        ...
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Dump IOD from database failed: could not lock index file");
      }
    } else
...
OFCondition DVInterface::checkIOD(const char* studyUID, const char* seriesUID,
                                  const char* instanceUID) {
  OFCondition result = EC_IllegalCall;
  if (studyUID && seriesUID && instanceUID) {
    if (EC_Normal == (result = lockDatabase())) {
      ...
      if (filename)
        ...
        result = EC_IllegalCall;
        DCMPSTAT_LOGFILE(
            "Check IOD from database failed: could not lock index file");
      }
    } else
      DCMPSTAT_LOGFILE(
          "Check IOD from database failed: UIDs not in index file");
  } else
 ...
 * rwflag  : nonzero if the password will be used as a new password, i.e. user
 * should be asked to repeat the password userdata: arbitrary pointer that can
 * be set with SSL_CTX_set_default_passwd_cb_userdata() returns : number of
 * bytes written to password buffer, -1 upon error
 */
extern "C" int DVInterfacePasswordCallback(char* buf, int size, int rwflag,
                                           void* userdata);

int DVInterfacePasswordCallback(char* buf, int size, int /* rwflag */,
                                void* userdata) {
  if (userdata == NULL)
    return -1;
...
#endif
{
  ...
  if (passwd)
    privateKeyPasswd = passwd;
  OFBool isPEMFormat = getTLSPEMFormat();
  const char* userKey = getUserPrivateKey(userID);
  if (userKey == NULL)
    DCMPSTAT_LOGFILE(
        "Cannot verify user password: unknown user or undefined private key "
        "file");
  else {
    ...
    if (in) {
      if (BIO_read_filename(in, filename.c_str()) > 0) {
        if (isPEMFormat) {
          pkey = PEM_read_bio_PrivateKey(in, NULL, DVInterfacePasswordCallback,
                                         &privateKeyPasswd);
          if (pkey)
            result = OFTrue;
        } else {
        ...
        DCMPSTAT_LOGFILE(
            "Cannot verify user password: private key file not found");
      BIO_free(in);
    }
    if (pkey)
      EVP_PKEY_free(pkey);
...
#else
  DCMPSTAT_LOGFILE(
      "Cannot verify user password: not compiled with OpenSSL support");
#endif
  return result;
}

#ifdef WITH_OPENSSL
OFCondition DVInterface::verifyAndSignStructuredReport(
    const char* userID, const char* passwd, DVPSVerifyAndSignMode mode)
#else
OFCondition DVInterface::verifyAndSignStructuredReport(
    const char* userID, const char* /*passwd*/, DVPSVerifyAndSignMode mode)
#endif
{
  ...
  if ((pReport != NULL) && (userID != NULL)) {
    ...
    DSRCodedEntryValue userCode(getUserCodeValue(userID, userCV),
                                getUserCodingSchemeDesignator(userID, userCSD),
                                getUserCodingSchemeVersion(userID, userCSV),
                                getUserCodeMeaning(userID, userCM));
    /* verify document */
    if (pReport->verifyDocument(userName, userCode, userOrg) == EC_Normal) {
      if ((mode == DVPSY_verifyAndSign) ||
          (mode == DVPSY_verifyAndSign_finalize)) {
#ifdef WITH_OPENSSL
        if (pSignatureHandler) {
          DcmStack stack;
          DcmItem dataset;
          if (pReport->write(dataset, &stack) == EC_Normal) {
            ...
            if (pSignatureHandler->createSignature(
                    dataset, stack, tagList, userID, passwd) == EC_Normal) {
              DSRDocument* newReport = new DSRDocument();
              if (newReport != NULL) {
                if (newReport->read(dataset,
                                    DSRTypes::RF_readDigitalSignatures) ==
                    EC_Normal) {
                  /* replace report in memory */
                  delete pReport;
                  pReport = newReport;
                  pSignatureHandler->updateDigitalSignatureInformation(
                      dataset, DVPSS_structuredReport, OFFalse /* onRead? */);
...
#else
        DCMPSTAT_LOGFILE(
            "Cannot sign structured report: not compiled with OpenSSL support");
#endif
      } else
        result = EC_Normal;
    }
...
const char* DVInterface::getCurrentSignatureValidationHTML(
    DVPSObjectType objtype) const {
...
DVPSSignatureStatus DVInterface::getCurrentSignatureStatus(
    DVPSObjectType objtype) const {
...
unsigned long DVInterface::getNumberOfCorrectSignatures(
    DVPSObjectType objtype) const {
  return pSignatureHandler->getNumberOfCorrectSignatures(objtype);
}

unsigned long DVInterface::getNumberOfUntrustworthySignatures(
    DVPSObjectType objtype) const {
  return pSignatureHandler->getNumberOfUntrustworthySignatures(objtype);
}

unsigned long DVInterface::getNumberOfCorruptSignatures(
    DVPSObjectType objtype) const {
 ...
 *  Set association timeout in ASC_initializeNetwork to 30 seconds. This
 * improves the responsiveness of the tools if the peer blocks during assoc
 * negotiation.
 *
 ...
 *    class to OFConfigFile. Cleaned up implementation (no more friend
 * declarations).
 *
 ...
 *    that are operating with TLS encryption by adding a special shutdown mode
 * to dcmpsrcv.
 *
 ...
 *  Adapted dcmpstat data structures and API to supplement 33 letter ballot
 * text.
 *
 ...
 *  Added methods to get and set ambient light value (re: Barten
 * transformation). Moved method 'isBartenTransformPossible()' from presentation
 * state class to interface class.
 *
 ...
 *  under Window NT).
 *  Removed bug in createPStateCache (cache was reported invalid on second
 * call). Modified method selectPState (image file is now implicitly loaded if
 *  necessary).
 *
 ...
 *  record deletion bug removed. Errorhandling for indexfiles with length zero
 * added.
 *
 ...
 *  Bug in getaninstance removed. getNumberOfSeries and getNumberOfInstances
 * results are correct now. getStudyStatus bug removed. Error handling routines
 * added.
 *
 ...
 *  getNumberOfSeries bug corrected. Error handling in deletion and reviewed
 * methods added.
 *
