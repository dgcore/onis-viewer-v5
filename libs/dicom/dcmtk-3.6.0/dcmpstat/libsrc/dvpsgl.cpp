...
DVPSGraphicLayer::DVPSGraphicLayer()
    : graphicLayer(DCM_GraphicLayer),
      ...
      graphicLayerRecommendedDisplayRGBValue(
          DCM_RETIRED_GraphicLayerRecommendedDisplayRGBValue),
...
DVPSGraphicLayer::DVPSGraphicLayer(const DVPSGraphicLayer& copy)
    : graphicLayer(copy.graphicLayer),
      ...
      graphicLayerRecommendedDisplayRGBValue(
          copy.graphicLayerRecommendedDisplayRGBValue),
...
OFCondition DVPSGraphicLayer::read(DcmItem& dset) {
  ...
  READ_FROM_DATASET(DcmUnsignedShort,
                    graphicLayerRecommendedDisplayGrayscaleValue)
  ...
  if (graphicLayer.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a graphic layer SQ item with graphicLayer "
        "absent or empty");
  ...
  if (graphicLayerOrder.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic layer SQ item with "
        "graphicLayerOrder absent or empty");
  } else if (graphicLayerOrder.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic layer SQ item with "
        "graphicLayerOrder VM != 1");
  }
  ...
  if (graphicLayerRecommendedDisplayGrayscaleValue.getVM() > 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a graphic layer SQ item with "
        "graphicLayerRecommendedDisplayGrayscaleValue VM != 1");
  }

  if ((graphicLayerRecommendedDisplayRGBValue.getVM() > 0) &&
      (graphicLayerRecommendedDisplayRGBValue.getVM() != 3)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic layer SQ item with "
        "graphicLayerRecommendedDisplayRGBValue VM != 3");
  }
...
OFCondition DVPSGraphicLayer::write(DcmItem& dset) {
  ...
  if (graphicLayerRecommendedDisplayGrayscaleValue.getLength() > 0) {
    ADD_TO_DATASET(DcmUnsignedShort,
                   graphicLayerRecommendedDisplayGrayscaleValue)
  }
  if (graphicLayerRecommendedDisplayRGBValue.getLength() > 0) {
    ADD_TO_DATASET(DcmUnsignedShort, graphicLayerRecommendedDisplayRGBValue)
  }
  if (graphicLayerDescription.getLength() > 0) {
    ADD_TO_DATASET(DcmLongString, graphicLayerDescription)
  }
...
const char* DVPSGraphicLayer::getGL() {
  char* c = NULL;
  if (EC_Normal == graphicLayer.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSGraphicLayer::getGLDescription() {
  char* c = NULL;
  if (EC_Normal == graphicLayerDescription.getString(c))
    return c;
  else
    return NULL;
}
...
void DVPSGraphicLayer::setGLRecommendedDisplayValueRGB(Uint16 r, Uint16 g,
                                                       Uint16 b) {
...
void DVPSGraphicLayer::setGLDescription(const char* glDescription) {
  if (glDescription)
    graphicLayerDescription.putString(glDescription);
  else
    graphicLayerDescription.clear();
  return;
}

Sint32 DVPSGraphicLayer::getGLOrder() {
  Sint32 result = 0;
  if (EC_Normal == graphicLayerOrder.getSint32(result, 0))
    return result;
  else
    return 0;
}
...
OFCondition DVPSGraphicLayer::getGLRecommendedDisplayValueGray(Uint16& gray) {
  ...
  if (graphicLayerRecommendedDisplayGrayscaleValue.getVM() == 1) {
    ...
    if (EC_Normal == result)
      result = graphicLayerRecommendedDisplayRGBValue.getUint16(g, 1);
    if (EC_Normal == result)
      result = graphicLayerRecommendedDisplayRGBValue.getUint16(b, 2);
    if (result == EC_Normal) {
...
OFCondition DVPSGraphicLayer::getGLRecommendedDisplayValueRGB(Uint16& r,
  ...
  if (graphicLayerRecommendedDisplayRGBValue.getVM() == 3) {
    ...
    if (EC_Normal == result)
      result = graphicLayerRecommendedDisplayRGBValue.getUint16(gg, 1);
    if (EC_Normal == result)
      result = graphicLayerRecommendedDisplayRGBValue.getUint16(bb, 2);
    if (result == EC_Normal) {
      r = rr;
      g = gg;
      b = bb;
