...
DVPSAssociationNegotiationResult DVPSPrintSCP::negotiateAssociation(
  ...
  if (aetitle == NULL)
    aetitle = dviface.getNetworkAETitle();  // default if AETITLE is missing
  ...
  if (maxPDU == 0)
    ...
    DCMPSTAT_INFO("max PDU size "
                  << maxPDU << " too big, using default: " << DEFAULT_MAXPDU);
    maxPDU = DEFAULT_MAXPDU;
  } else if (maxPDU < ASC_MINIMUMPDUSIZE) {
    DCMPSTAT_INFO("max PDU size "
                  << maxPDU << " too small, using default: " << DEFAULT_MAXPDU);
    maxPDU = DEFAULT_MAXPDU;
  }

  // check whether we want to support Presentation LUT
  OFBool supportPresentationLUT =
      dviface.getTargetPrinterSupportsPresentationLUT(cfgname);
  ...
  OFCondition cond = ASC_receiveAssociation(&net, &assoc, maxPDU, &associatePDU,
                                            &associatePDUlength, useTLS);
  if (errorCond(cond, "Failed to receive association:")) {
    ...
    if (cond.bad() || strcmp(buf, DICOM_STDAPPLICATIONCONTEXT) != 0) {
      ...
      const char* abstractSyntaxes[] = {
          ...
          UID_PresentationLUTSOPClass, UID_PrivateShutdownSOPClass};

      int numAbstractSyntaxes = 3;
      if (supportPresentationLUT)
        numAbstractSyntaxes = 4;
      ...
      cond = ASC_acceptContextsWithPreferredTransferSyntaxes(
          assoc->params, (const char**)abstractSyntaxes, numAbstractSyntaxes,
    ...
    if (0 != ASC_findAcceptedPresentationContextID(
                 assoc, UID_PrivateShutdownSOPClass)) {
  ...
  if (acseSequence && associatePDU) {
    ...
    if (newItem) {
      DcmElement* assocData =
          new DcmOtherByteOtherWord(PSTAT_DCM_AssociateData);
      if (assocData) {
...
OFCondition DVPSPrintSCP::refuseAssociation(OFBool isBadContext) {
  ...
  OFCondition cond =
      ASC_rejectAssociation(assoc, &rej, &associatePDU, &associatePDUlength);
...
void DVPSPrintSCP::handleClient() {
  ...
  OFCondition cond =
      ASC_acknowledgeAssociation(assoc, &associatePDU, &associatePDUlength);
  ...
  if (!errorCond(cond, "Cannot acknowledge association:")) {
    DCMPSTAT_INFO(
        "Association Acknowledged (Max Send PDV: "
        << assoc->sendPDVLength << ")"
    ...
    while (cond.good()) {
      ...
      if (cond.good()) {
        ...
        if (rawCommandSet) {
          if (logSequence)
            logSequence->insert(new DcmItem(*rawCommandSet));
      ...
      if (cond.good()) {
        ...
        switch (msg.CommandField) {
          ...
          default:
            ...
            DCMPSTAT_INFO("Cannot handle command: 0x"
...
OFCondition DVPSPrintSCP::handleCEcho(T_DIMSE_Message& rq,
                                      T_ASC_PresentationContextID presID) {
  OFCondition cond = DIMSE_sendEchoResponse(assoc, presID, &rq.msg.CEchoRQ,
                                            STATUS_Success, NULL);
  return cond;
}
...
OFCondition DVPSPrintSCP::handleNGet(T_DIMSE_Message& rq,
  ...
  if (rq.msg.NGetRQ.DataSetType == DIMSE_DATASET_PRESENT) {
    ...
    if (cond.good()) {
      if (logSequence)
        logSequence->insert(new DcmItem(*dataset));
  ...
  cond = DIMSE_sendMessageUsingMemoryData(assoc, presID, &rsp, NULL, rspDataset,
                                          NULL, NULL, &rspCommand);
  if (logSequence) {
    if (rspCommand)
      logSequence->insert(new DcmItem(*rspCommand));
    if (rspDataset)
      logSequence->insert(new DcmItem(*rspDataset));
    else
      logSequence->insert(new DcmItem());
  }
  delete rspCommand;
...
OFCondition DVPSPrintSCP::handleNSet(T_DIMSE_Message& rq,
  ...
  if (rq.msg.NSetRQ.DataSetType == DIMSE_DATASET_PRESENT) {
    ...
    if (cond.bad())
      return cond;
    if (logSequence && rqDataset)
      logSequence->insert(new DcmItem(*rqDataset));
  } else if (logSequence)
    logSequence->insert(new DcmItem());
...
OFCondition DVPSPrintSCP::handleNAction(T_DIMSE_Message& rq,
  ...
  cond = DIMSE_sendMessageUsingMemoryData(assoc, presID, &rsp, NULL, NULL, NULL,
                                          NULL, &rspCommand);
  if (logSequence) {
    if (rspCommand)
      logSequence->insert(new DcmItem(*rspCommand));
...
OFCondition DVPSPrintSCP::handleNCreate(T_DIMSE_Message& rq,
  ...
  if (rq.msg.NCreateRQ.opts & O_NCREATE_AFFECTEDSOPINSTANCEUID) {
    ...
    strncpy(rsp.msg.NCreateRSP.AffectedSOPInstanceUID,
  ...
  OFBool omitFlag =
      dviface.getTargetPrintSCPOmitSOPClassUIDFromCreateResponse(cfgname);
  if (omitFlag) {
    ...
    rsp.msg.NCreateRSP.opts =
        O_NCREATE_AFFECTEDSOPINSTANCEUID | O_NCREATE_AFFECTEDSOPCLASSUID;
  }
...
void DVPSPrintSCP::printerNGet(T_DIMSE_Message& rq, T_DIMSE_Message& rsp,
  ...
  if (printerInstance == rq.msg.NGetRQ.RequestedSOPInstanceUID) {
    ...
    while (i + 1 < rq.msg.NGetRQ.ListCount) {
      ...
      if ((group == 0x2110) && (element == 0x0010)) {
        if (EC_Normal != DVPSHelper::putStringValue(rspDataset,
                                                    DCM_PrinterStatus,
                                                    DEFAULT_printerStatus))
          result = OFFalse;
        ...
        DCMPSTAT_INFO(
            "cannot retrieve printer information: unsupported attribute ("
            << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
            << STD_NAMESPACE setw(4) << (int)group << "," << STD_NAMESPACE hex
            << STD_NAMESPACE setfill('0') << STD_NAMESPACE setw(4)
            << (int)element << ") in attribute list.");
    ...
    if (rq.msg.NGetRQ.ListCount == 0) {
      if (EC_Normal != DVPSHelper::putStringValue(rspDataset, DCM_PrinterStatus,
                                                  DEFAULT_printerStatus))
        result = OFFalse;
      if (EC_Normal != DVPSHelper::putStringValue(rspDataset,
                                                  DCM_PrinterStatusInfo,
    ...
    if (result) {
      ...
      delete rspDataset;
      rspDataset = NULL;
      if (rsp.msg.NGetRSP.DimseStatus == 0)
        rsp.msg.NGetRSP.DimseStatus = STATUS_N_ProcessingFailure;
      result = OFFalse;
    }
  } else {
    DCMPSTAT_INFO(
        "cannot retrieve printer information, instance UID is not well-known "
        "printer SOP instance UID.");
    rsp.msg.NGetRSP.DimseStatus = STATUS_N_NoSuchObjectInstance;
  }
}

void DVPSPrintSCP::filmSessionNSet(T_DIMSE_Message& rq, DcmDataset* rqDataset,
                                   T_DIMSE_Message& rsp,
                                   DcmDataset*& rspDataset) {
  if (filmSession &&
      (filmSession->isInstance(rq.msg.NSetRQ.RequestedSOPInstanceUID))) {
    OFBool usePLUTinFilmSession = OFFalse;
    if (assoc && (0 != ASC_findAcceptedPresentationContextID(
                           assoc, UID_PresentationLUTSOPClass))) {
      if (dviface.getTargetPrinterPresentationLUTinFilmSession(cfgname))
        usePLUTinFilmSession = OFTrue;
    }
    ...
    if (newSession) {
      if (newSession->printSCPSet(dviface, cfgname, rqDataset, rsp, rspDataset,
                                  usePLUTinFilmSession, presentationLUTList,
                                  storedPrintList)) {
        delete filmSession;
...
void DVPSPrintSCP::filmBoxNSet(T_DIMSE_Message& rq, DcmDataset* rqDataset,
                               T_DIMSE_Message& rsp, DcmDataset*& rspDataset) {
  OFBool usePLUTinFilmBox = OFFalse;
  if (assoc && (0 != ASC_findAcceptedPresentationContextID(
                         assoc, UID_PresentationLUTSOPClass))) {
    if (!dviface.getTargetPrinterPresentationLUTinFilmSession(cfgname))
      usePLUTinFilmBox = OFTrue;
  }
  storedPrintList.printSCPBasicFilmBoxSet(dviface, cfgname, rq, rqDataset, rsp,
                                          rspDataset, usePLUTinFilmBox,
                                          presentationLUTList);
}
...
void DVPSPrintSCP::filmSessionNAction(T_DIMSE_Message& rq,
                                      T_DIMSE_Message& rsp) {
  if (filmSession &&
      (filmSession->isInstance(rq.msg.NActionRQ.RequestedSOPInstanceUID))) {
    storedPrintList.printSCPBasicFilmSessionAction(dviface, cfgname, rsp,
                                                   presentationLUTList);
  } else {
    // film session does not exist or wrong instance UID
...
void DVPSPrintSCP::filmBoxNAction(T_DIMSE_Message& rq, T_DIMSE_Message& rsp) {
  storedPrintList.printSCPBasicFilmBoxAction(dviface, cfgname, rq, rsp,
                                             presentationLUTList);
}
...
void DVPSPrintSCP::filmSessionNCreate(DcmDataset* rqDataset,
  ...
  if (filmSession) {
    ...
    DVPSFilmSession* newSession = new DVPSFilmSession(
        DEFAULT_illumination, DEFAULT_reflectedAmbientLight);
    if (newSession) {
      DIC_AE peerTitle;
      peerTitle[0] = 0;
      ASC_getAPTitles(assoc->params, peerTitle, NULL, NULL);
      if (newSession->printSCPCreate(dviface, cfgname, rqDataset, rsp,
                                     rspDataset, peerTitle,
                                     usePLUTinFilmSession, presentationLUTList))
        filmSession = newSession;
      char uid[100];
      studyInstanceUID.putString(
          dcmGenerateUniqueIdentifier(uid, SITE_STUDY_UID_ROOT));
      psSeriesInstanceUID.putString(
...
void DVPSPrintSCP::filmBoxNCreate(DcmDataset* rqDataset, T_DIMSE_Message& rsp,
                                  DcmDataset*& rspDataset) {
  if (filmSession) {
    if (storedPrintList.haveFilmBoxInstance(
            rsp.msg.NCreateRSP.AffectedSOPInstanceUID)) {
      DCMPSTAT_INFO(
          "cannot create film box, requested SOP instance UID already in use.");
      rsp.msg.NCreateRSP.DimseStatus = STATUS_N_DuplicateSOPInstance;
      rsp.msg.NCreateRSP.opts = 0;  // don't include affected SOP instance UID
    } else {
      DVPSStoredPrint* newSPrint = new DVPSStoredPrint(
          DEFAULT_illumination, DEFAULT_reflectedAmbientLight);
      if (newSPrint) {
        if (assoc)
          newSPrint->setOriginator(assoc->params->DULparams.callingAPTitle);

        // get AETITLE from config file
        const char* aetitle = dviface.getTargetAETitle(cfgname);
        if (aetitle == NULL)
          aetitle =
              dviface.getNetworkAETitle();  // default if AETITLE is missing
        ...
        if (newSPrint->printSCPCreate(
                dviface, cfgname, rqDataset, rsp, rspDataset, usePLUTinFilmBox,
                presentationLUTList, filmSession->getUID(), studyInstanceUID,
                psSeriesInstanceUID, imageSeriesInstanceUID)) {
          if (usePLUTinFilmSession) {
            filmSession->copyPresentationLUTSettings(
                *newSPrint);  // update P-LUT settings from film session
          }
...
void DVPSPrintSCP::presentationLUTNCreate(DcmDataset* rqDataset,
                                          T_DIMSE_Message& rsp,
                                          DcmDataset*& rspDataset) {
  if ((assoc == NULL) || (0 == ASC_findAcceptedPresentationContextID(
                                   assoc, UID_PresentationLUTSOPClass))) {
    DCMPSTAT_INFO("cannot create presentation LUT, not negotiated.");
    rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchSOPClass;
    rsp.msg.NCreateRSP.opts = 0;  // don't include affected SOP instance UID
  } else if (presentationLUTList.findPresentationLUT(
                 rsp.msg.NCreateRSP.AffectedSOPInstanceUID)) {
    DCMPSTAT_INFO(
        "cannot create presentation LUT, requested SOP instance UID already in "
        "use.");
    rsp.msg.NCreateRSP.DimseStatus = STATUS_N_DuplicateSOPInstance;
    rsp.msg.NCreateRSP.opts = 0;  // don't include affected SOP instance UID
  } else {
    ...
    if (newPLut) {
      OFBool matchRequired =
          dviface.getTargetPrinterPresentationLUTMatchRequired(cfgname);
      OFBool supports12Bit =
          dviface.getTargetPrinterSupports12BitTransmission(cfgname);
      if (newPLut->printSCPCreate(rqDataset, rsp, rspDataset, matchRequired,
                                  supports12Bit)) {
...
void DVPSPrintSCP::filmSessionNDelete(T_DIMSE_Message& rq,
                                      T_DIMSE_Message& rsp) {
  if (filmSession &&
      (filmSession->isInstance(rq.msg.NDeleteRQ.RequestedSOPInstanceUID))) {
    ...
    // film session does not exist or wrong instance UID
    DCMPSTAT_INFO("cannot delete film session with instance UID '"
                  << rq.msg.NDeleteRQ.RequestedSOPInstanceUID
...
void DVPSPrintSCP::presentationLUTNDelete(T_DIMSE_Message& rq,
  ...
  if (storedPrintList.usesPresentationLUT(
          rq.msg.NDeleteRQ.RequestedSOPInstanceUID)) {
    DCMPSTAT_INFO("cannot delete presentation LUT '"
                  << rq.msg.NDeleteRQ.RequestedSOPInstanceUID
                  << "': object still in use.");
...
void DVPSPrintSCP::imageBoxNSet(T_DIMSE_Message& rq, DcmDataset* rqDataset,
                                T_DIMSE_Message& rsp, DcmDataset*& rspDataset) {
  OFBool usePLUT = OFFalse;
  if (assoc && (0 != ASC_findAcceptedPresentationContextID(
                         assoc, UID_PresentationLUTSOPClass)))
    usePLUT = OFTrue;
  storedPrintList.printSCPBasicGrayscaleImageBoxSet(
      dviface, cfgname, rq, rqDataset, rsp, rspDataset, usePLUT);
}
...
void DVPSPrintSCP::setDimseLogPath(const char* fname) {
  if (fname) {
    logPath = fname;
    if (logSequence == NULL)
      logSequence = new DcmSequenceOfItems(PSTAT_DCM_LogSequence);
    if (acseSequence == NULL)
      acseSequence = new DcmSequenceOfItems(PSTAT_DCM_AcseSequence);
  } else {
    logPath.clear();
    delete logSequence;
    logSequence = NULL;
...
void DVPSPrintSCP::saveDimseLog() {
  ...
  logSequence = NULL;
  if (acseSequence)
    dset->insert(acseSequence, OFTrue /*replaceOld*/);
  acseSequence = NULL;
  ...
  DVPSHelper::putStringValue(dset, DCM_SOPClassUID,
                             PSTAT_DIMSE_LOG_STORAGE_UID);
  DVPSHelper::putStringValue(dset, DCM_SOPInstanceUID,
  ...
  OFCondition cond =
      DVPSHelper::saveFileFormat(logPath.c_str(), &fformat, OFTrue);
  if (cond == EC_Normal) {
...
void DVPSPrintSCP::dumpNMessage(T_DIMSE_Message& msg, DcmItem* dataset,
                                OFBool outgoing) {
  OFString str;
  if (outgoing) {
