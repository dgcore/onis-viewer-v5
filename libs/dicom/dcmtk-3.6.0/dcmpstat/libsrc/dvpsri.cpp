...
OFCondition DVPSReferencedImage::read(DcmItem& dset) {
  ...
  if (referencedSOPClassUID.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a referenced image SQ item with "
        "referencedSOPClassUID absent or empty");
  } else if (referencedSOPClassUID.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a referenced image SQ item with "
        "referencedSOPClassUID VM != 1");
  }
  ...
  if (referencedSOPInstanceUID.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a referenced image SQ item with "
        "referencedSOPInstanceUID absent or empty");
  } else if (referencedSOPInstanceUID.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a referenced image SQ item with "
        "referencedSOPInstanceUID VM != 1");
  }
...
OFBool DVPSReferencedImage::validateSOPClassUID(OFString& sopclassuid) {
  ...
  if (sopclassuid.length() == 0)
    referencedSOPClassUID.getOFString(sopclassuid, 0);
  else {
    OFString currentUID;
    ...
    if (currentUID != sopclassuid) {
      result = OFFalse;
      DCMPSTAT_INFO(
          "images of different SOP classes referenced in presentation state");
    }
  }
  return result;
}
...
void DVPSReferencedImage::setSOPClassUID(const char* uid) {
  if (uid)
    referencedSOPClassUID.putString(uid);
  else
...
void DVPSReferencedImage::setSOPInstanceUID(const char* uid) {
  if (uid)
    referencedSOPInstanceUID.putString(uid);
  else
...
void DVPSReferencedImage::setFrameNumbers(const char* frames) {
  if (frames)
    referencedFrameNumber.putString(frames);
  else
...
OFCondition DVPSReferencedImage::getImageReference(OFString& sopclassUID,
  ...
  if (EC_Normal == result)
    result = referencedSOPInstanceUID.getOFString(instanceUID, 0);
  if (EC_Normal == result)
    result = referencedFrameNumber.getOFStringArray(frames);
  return result;
}
...
OFBool DVPSReferencedImage::appliesToAllFrames() {
  if (referencedFrameNumber.getLength() == 0)
    return OFTrue;
  if (referencedFrameNumber.getVM() == 0)
    return OFTrue;
  return OFFalse;
}
    ...
    if (frameCacheEntries > 0) {
      ...
      if (frameCache) {
        for (i = 0; i < frameCacheEntries; i++) {
          if (EC_Normal == referencedFrameNumber.getSint32(val, i))
            frameCache[i] = val;
          else
            frameCache[i] = 0;
        }
      } else
        frameCacheEntries = 0;  // out of memory
    }
  }
}
  ...
  if (frameCache) {
    for (i = 0; i < frameCacheEntries; i++) {
      if (frameCache[i] != (Sint32)frame) {
        if (aString.size() == 0)
          sprintf(str, "%ld", (long)(frameCache[i]));
        else
          sprintf(str, "\\%ld", (long)(frameCache[i]));
        aString += str;
      }
