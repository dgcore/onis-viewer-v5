...
OFCondition DVPSImageBoxContent::setContent(
    const char* instanceuid, const char* retrieveaetitle,
    const char* refstudyuid, const char* refseriesuid,
    const char* refsopclassuid, const char* refsopinstanceuid,
    const char* requestedimagesize, const char* patientid,
    const char* presentationlutreference) {
  OFCondition result = EC_Normal;
  if (refstudyuid && refseriesuid && instanceuid && retrieveaetitle &&
      refsopclassuid && refsopinstanceuid) {
    clear();
    result = sOPInstanceUID.putString(instanceuid);
    if (EC_Normal == result)
      result = retrieveAETitle.putString(retrieveaetitle);
    if (EC_Normal == result)
      result = referencedSOPClassUID.putString(refsopclassuid);
    if (EC_Normal == result)
      result = referencedSOPInstanceUID.putString(refsopinstanceuid);
    if (EC_Normal == result)
      result = studyInstanceUID.putString(refstudyuid);
    if (EC_Normal == result)
      result = seriesInstanceUID.putString(refseriesuid);
    if (requestedimagesize && (EC_Normal == result))
      result = requestedImageSize.putString(requestedimagesize);
    if (patientid && (EC_Normal == result))
      result = patientID.putString(patientid);
    if (presentationlutreference && (EC_Normal == result))
      result = referencedPresentationLUTInstanceUID.putString(
          presentationlutreference);
  } else
    result = EC_IllegalCall;
  return result;
}

OFCondition DVPSImageBoxContent::read(
    DcmItem& dset, DVPSPresentationLUT_PList& presentationLUTList) {
  ...
  if (result == EC_Normal) {
    stack.clear();
    if (EC_Normal == dset.search(DCM_ReferencedImageSequence, stack,
      ...
      if (seq->card() == 1) {
        ...
        DCMPSTAT_INFO(
            "found ReferencedImageSequence in Stored Print Image Box with "
            "number of items != 1");
      }
    }
  }
  ...
  if (result == EC_Normal) {
    ...
    // if there is any reference, it must refer to one of the presentation LUTs
    // we are managing.
    stack.clear();
    if (EC_Normal == dset.search(DCM_ReferencedPresentationLUTSequence, stack,
                                 ESM_fromHere, OFFalse)) {
      seq = (DcmSequenceOfItems*)stack.top();
      if (seq->card() == 1) {
        item = seq->getItem(0);
        ...
        if (referencedPresentationLUTInstanceUID.getLength() > 0) {
          ...
          if (NULL ==
            ...
            result = EC_IllegalCall;
            DCMPSTAT_INFO(
                "ImageBoxContentSequence presentation LUT reference cannot be "
                "resolved");
          }
        }
      } else {
        result = EC_TagNotFound;
        DCMPSTAT_INFO(
            "found ImageBoxContentSequence in Stored Print with "
  ...
  if (result == EC_Normal) {
    stack.clear();
    if (EC_Normal == dset.search(DCM_RETIRED_ReferencedImageOverlayBoxSequence,
                                 stack, ESM_fromHere, OFFalse)) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "Stored Print: retired ReferencedImageOverlayBoxSequence not "
          "supported");
    }
  }
  ...
  if (result == EC_Normal) {
    if ((sOPInstanceUID.getLength() == 0) || (sOPInstanceUID.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "SOPInstanceUID missing or incorrect in Stored Print Image Box");
    }
    if ((imageBoxPosition.getLength() == 0) ||
        (imageBoxPosition.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "ImageBoxPosition missing or incorrect in Stored Print Image Box");
    }
    ...
    if ((referencedSOPClassUID.getLength() == 0) ||
        (referencedSOPClassUID.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "ReferencedSOPClassUID missing or incorrect in Stored Print Image "
          "Box");
    }
    if ((referencedSOPInstanceUID.getLength() == 0) ||
        (referencedSOPInstanceUID.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "ReferencedSOPInstanceUID missing or incorrect in Stored Print Image "
          "Box");
    }
    if ((studyInstanceUID.getLength() == 0) ||
      ...
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "StudyInstanceUID missing or incorrect in Stored Print Image Box");
    }
    if ((seriesInstanceUID.getLength() == 0) ||
        (seriesInstanceUID.getVM() != 1)) {
      result = EC_TagNotFound;
      DCMPSTAT_INFO(
          "SeriesInstanceUID missing or incorrect in Stored Print Image Box");
    }
...
OFCondition DVPSImageBoxContent::write(DcmItem& dset,
  ...
  if (imageBoxPosition.getLength() == 0) {
    result = EC_TagNotFound;
    DCMPSTAT_INFO(
        "cannot write Stored Print Image Box: ImageBoxPosition empty");
  }
  if (retrieveAETitle.getLength() == 0) {
    result = EC_TagNotFound;
  ...
  if (referencedSOPClassUID.getLength() == 0) {
    result = EC_TagNotFound;
    DCMPSTAT_INFO(
        "cannot write Stored Print Image Box: ReferencedSOPClassUID empty");
  }
  if (referencedSOPInstanceUID.getLength() == 0) {
    result = EC_TagNotFound;
    DCMPSTAT_INFO(
        "cannot write Stored Print Image Box: ReferencedSOPInstanceUID empty");
  }
  if (studyInstanceUID.getLength() == 0) {
    result = EC_TagNotFound;
    DCMPSTAT_INFO(
        "cannot write Stored Print Image Box: StudyInstanceUID empty");
  }
  if (seriesInstanceUID.getLength() == 0) {
    result = EC_TagNotFound;
    DCMPSTAT_INFO(
        "cannot write Stored Print Image Box: SeriesInstanceUID empty");
  }
  ...
  if ((writeRequestedImageSize) && (requestedImageSize.getLength() > 0)) {
    ADD_TO_DATASET(DcmDecimalString, requestedImageSize)
  }
  if (requestedDecimateCropBehavior.getLength() > 0) {
    ADD_TO_DATASET(DcmCodeString, requestedDecimateCropBehavior)
  }
...
OFCondition DVPSImageBoxContent::prepareBasicImageBox(DcmItem& dset) {
  ...
  if (requestedImageSize.getLength() > 0) {
    ADD_TO_DATASET(DcmDecimalString, requestedImageSize)
  }
  if (requestedDecimateCropBehavior.getLength() > 0) {
    ADD_TO_DATASET(DcmCodeString, requestedDecimateCropBehavior)
...
OFCondition DVPSImageBoxContent::createDefaultValues(OFBool renumber,
  ...
  if (renumber) {
    ...
    if (imageBoxPosition.getVM() != 1)
      isEmpty = OFTrue;
    if ((!ignoreEmptyImages) && isEmpty)
      result = EC_TagNotFound;
  }
...
const char* DVPSImageBoxContent::getSOPClassUID() {
  char* c = NULL;
  if (EC_Normal == referencedSOPClassUID.getString(c))
    return c;
  else
    return NULL;
}

OFCondition DVPSImageBoxContent::setRequestedDecimateCropBehaviour(
...
OFCondition DVPSImageBoxContent::getImageReference(const char*& studyUID,
  ...
  if (EC_Normal == result)
    result = seriesInstanceUID.getString(aseriesUID);
  if (EC_Normal == result)
    result = referencedSOPInstanceUID.getString(aninstanceUID);
  studyUID = astudyUID;
...
DVPSImageBoxContent::getRequestedDecimateCropBehaviour() {
  DVPSDecimateCropBehaviour result = DVPSI_default;
  char* c = NULL;
  if ((EC_Normal == requestedDecimateCropBehavior.getString(c)) && c) {
    OFString aString(c);
    if (aString == "DECIMATE")
      result = DVPSI_decimate;
    else if (aString == "CROP")
      result = DVPSI_crop;
    else if (aString == "FAIL")
      result = DVPSI_fail;
  }
  return result;
}
...
const char* DVPSImageBoxContent::getPolarity() {
  char* c = NULL;
  if (EC_Normal == polarity.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSImageBoxContent::getRequestedImageSize() {
  char* c = NULL;
  if (EC_Normal == requestedImageSize.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSImageBoxContent::getReferencedPresentationLUTInstanceUID() {
  char* c = NULL;
  if (EC_Normal == referencedPresentationLUTInstanceUID.getString(c))
    return c;
  else
    return NULL;
}
...
OFCondition DVPSImageBoxContent::setConfigurationInformation(
    const char* value) {
  if ((value == NULL) || (strlen(value) == 0)) {
    configurationInformation.clear();
    return EC_Normal;
...
OFBool DVPSImageBoxContent::matchesPresentationLUT(
    DVPSPrintPresentationLUTAlignment align) const {
  ...
  switch (align) {
    case DVPSK_shape:  // always matches
      ...
      break;
    case DVPSK_table8:
      if (imageDepth != DVPSN_12bit)
        result = OFTrue;
      break;
    case DVPSK_table12:
      if (imageDepth != DVPSN_8bit)
        result = OFTrue;
      break;
...
OFBool DVPSImageBoxContent::printSCPSet(
    ...
    T_DIMSE_Message& rsp, DcmDataset*& rspDataset, DcmDataset& imageDataset,
    DVPSPrintPresentationLUTAlignment align, OFBool presentationLUTnegotiated) {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)imageBoxPosition.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: attribute (2020,0010) "
  ...
  if (result) {
    ...
    if (rqDataset &&
      ...
      if (!found) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal magnification "
            "type: '"
  ...
  if (result) {
    ...
    if (rqDataset &&
      ...
      if (numSmoothings == 0) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: smoothing type requested "
            "but not supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else if (!found) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal smoothing type: '"
            << theSmoothing.c_str() << "'");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      } else {
  ...
  if (result) {
    ...
    if (rqDataset &&
      ...
      {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: configuration "
            "information requested but not supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
        ...
        if (!found) {
          DCMPSTAT_INFO(
              "cannot update Basic Grayscale Image Box: illegal configuration "
              "information: '"
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)polarity.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      ...
      if ((thePolarity != "NORMAL") && (thePolarity != "REVERSE")) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal polarity: '"
            << thePolarity.c_str() << "'");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      } else {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)requestedImageSize.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      if (!cfg.getTargetPrinterSupportsRequestedImageSize(cfgname)) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: requested image size not "
            "supported.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        result = OFFalse;
      } else {
  ...
  if (result) {
    ...
    if (rqDataset &&
        (EC_Normal ==
         rqDataset->search((DcmTagKey&)requestedDecimateCropBehavior.getTag(),
                           stack, ESM_fromHere, OFFalse))) {
      if (!cfg.getTargetPrinterSupportsDecimateCrop(cfgname)) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: requested decimate/crop "
        ...
        if ((theBehaviour != "DECIMATE") && (theBehaviour != "CROP") &&
            (theBehaviour != "FAIL")) {
          DCMPSTAT_INFO(
              "cannot update Basic Grayscale Image Box: illegal decimate/crop "
              "behaviour: '"
              << theBehaviour.c_str() << "'");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search(DCM_BasicGrayscaleImageSequence, stack,
                                        ESM_fromHere, OFFalse))) {
      DcmSequenceOfItems* seq = (DcmSequenceOfItems*)stack.top();
      if (seq->card() == 1) {
        DcmItem* item = seq->getItem(0);
        result = printSCPEvaluateBasicGrayscaleImageSequence(
            cfg, cfgname, item, rsp, imageDataset, align,
            presentationLUTnegotiated);
      } else {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: basic grayscale image "
  ...
  if (result && rqDataset) {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub)) {
      ...
      if (currentTag.getElement() == 0x0000) /* group length */
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: unsupported attribute "
            "received:\n"
...
OFBool DVPSImageBoxContent::printSCPEvaluateBasicGrayscaleImageSequence(
    DVConfiguration& cfg, const char* cfgname, DcmItem* rqDataset,
    T_DIMSE_Message& rsp, DcmDataset& imageDataset,
    DVPSPrintPresentationLUTAlignment align, OFBool presentationLUTnegotiated) {
  OFBool result = OFTrue;
  DcmStack stack;
  OFCondition writeresult = EC_Normal;
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)samplesPerPixel.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      if ((EC_Normal == samplesPerPixel.getUint16(val, 0)) && (val == 1)) {
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal samples per "
            "pixel value: "
            << val);
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: samples per pixel missing "
          "in basic grayscale image sequence");
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    }
  }
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)rows.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      ...
      if ((EC_Normal == rows.getUint16(val, 0)) && (val > 0)) {
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal rows value: "
            << val);
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: rows missing in basic "
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)columns.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      ...
      if ((EC_Normal == columns.getUint16(val, 0)) && (val > 0)) {
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal columns value: "
            << val);
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: columns missing in basic "
          "grayscale image sequence");
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    }
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)bitsStored.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      bitsStored = *((DcmUnsignedShort*)(stack.top()));
      val = 0;
      if ((EC_Normal == bitsStored.getUint16(val, 0)) &&
          ((val == 8) || (val == 12))) {
        bitsStoredValue = val;
        if (bitsStoredValue == 8)
          imageDepth = DVPSN_8bit;
        else
          imageDepth = DVPSN_12bit;
        OFBool supports12Bit =
            cfg.getTargetPrinterSupports12BitTransmission(cfgname);
        if ((bitsStoredValue == 12) && (!supports12Bit)) {
          DCMPSTAT_INFO(
              "cannot update Basic Grayscale Image Box: image transmission "
              "with 12 bits/pixel not supported.");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        } else {
          if (presentationLUTnegotiated &&
              (cfg.getTargetPrinterPresentationLUTMatchRequired(cfgname)) &&
              (!matchesPresentationLUT(align))) {
            DCMPSTAT_INFO(
                "cannot update Basic Grayscale Image Box: image data with "
                << bitsStoredValue
                << " bits/pixel does not match characteristics of active "
                   "Presentation LUT.");
            rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
            result = OFFalse;
          } else {
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal bits stored "
            "value: "
            << val);
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: bits stored missing in "
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)bitsAllocated.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      bitsAllocated = *((DcmUnsignedShort*)(stack.top()));
      val = 0;
      if ((EC_Normal == bitsAllocated.getUint16(val, 0)) &&
          (((val == 8) && (bitsStoredValue == 8)) ||
           ((val == 16) && (bitsStoredValue == 12)))) {
        ADD_TO_PDATASET(DcmUnsignedShort, bitsAllocated)
      } else {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal bits allocated "
            "value: "
            << val);
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: bits allocated missing in "
          "basic grayscale image sequence");
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    }
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)highBit.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      highBit = *((DcmUnsignedShort*)(stack.top()));
      val = 0;
      if ((EC_Normal == highBit.getUint16(val, 0)) &&
          (((val == 7) && (bitsStoredValue == 8)) ||
           ((val == 11) && (bitsStoredValue == 12)))) {
        ADD_TO_PDATASET(DcmUnsignedShort, highBit)
      } else {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal high bit value: "
            << val);
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: high bit missing in basic "
          "grayscale image sequence");
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    }
  ...
  if (result) {
    stack.clear();
    if (rqDataset && (EC_Normal == rqDataset->search(
                                       (DcmTagKey&)pixelRepresentation.getTag(),
                                       stack, ESM_fromHere, OFFalse))) {
      ...
      if ((EC_Normal == pixelRepresentation.getUint16(val, 0)) && (val == 0)) {
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal pixel "
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: pixel representation "
          "missing in basic grayscale image sequence");
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    }
  }
  ...
  if (result) {
    ...
    if (rqDataset &&
        (EC_Normal ==
         rqDataset->search((DcmTagKey&)photometricInterpretation.getTag(),
                           stack, ESM_fromHere, OFFalse))) {
      ...
      if ((theColorModel != "MONOCHROME1") &&
          (theColorModel != "MONOCHROME2")) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal photometric "
            "interpretation: '"
            << theColorModel.c_str() << "'");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: photometric interpretation "
          "missing in basic grayscale image sequence");
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    }
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)pixelAspectRatio.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      pixelAspectRatio = *((DcmIntegerString*)(stack.top()));
      if (pixelAspectRatio.getVM() != 2) {
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: illegal pixel aspect "
            "ratio, VM=: '"
            << pixelAspectRatio.getVM());
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      } else {
  ...
  if (result) {
    stack.clear();
    if (rqDataset && (EC_Normal == rqDataset->search(DCM_PixelData, stack,
                                                     ESM_fromHere, OFFalse))) {
      ...
      if (pixelData) {
        ...
        if ((EC_Normal == oldPxData->getUint16Array(pxdata16)) && pxdata16) {
          pixelData->putUint16Array(pxdata16,
                                    oldPxData->getLength() / sizeof(Uint16));
          rspDataset->insert(pixelData, OFTrue /*replaceOld*/);
        } else if ((EC_Normal == oldPxData->getUint8Array(pxdata8)) &&
                   pxdata8) {
          pixelData->putUint8Array(pxdata8,
                                   oldPxData->getLength() / sizeof(Uint8));
          rspDataset->insert(pixelData, OFTrue /*replaceOld*/);
        } else {
          DCMPSTAT_INFO(
              "cannot update Basic Grayscale Image Box: cannot access pixel "
              "data");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_ProcessingFailure;
          result = OFFalse;
        }
      ...
      DCMPSTAT_INFO(
          "cannot update Basic Grayscale Image Box: pixel data missing in "
          "basic grayscale image sequence");
      rsp.msg.NSetRSP.DimseStatus = STATUS_N_MissingAttribute;
      result = OFFalse;
    }
  ...
  if (result && rqDataset) {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub)) {
      ...
      if (currentTag.getElement() == 0x0000) /* group length */
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Grayscale Image Box: unsupported attribute in "
            "basic grayscale image sequence:\n"
            << DcmObject::PrintHelper(*stack.top(),
                                      DCMTypes::PF_shortenLongTagValues));
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
  ...
  if (result && (EC_Normal == writeresult)) {
    ...
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(rspDataset, DCM_PatientName,
                                               DEFAULT_patientName);
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(rspDataset, DCM_PatientID);
    if (EC_Normal == writeresult)
      writeresult =
          DVPSHelper::putStringValue(rspDataset, DCM_PatientBirthDate);
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(rspDataset, DCM_PatientSex);

    // general study and general series modules are written somewhere else

    // Hardcopy Equipment Module
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(
          rspDataset, DCM_RETIRED_HardcopyDeviceManufacturer, "OFFIS");
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(
    ...
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(rspDataset, DCM_InstanceNumber);
    if (EC_Normal == writeresult)
      writeresult =
          DVPSHelper::putStringValue(rspDataset, DCM_PatientOrientation);
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(rspDataset, DCM_ImageType,
                                               "DERIVED\\SECONDARY");
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(
          rspDataset, DCM_DerivationDescription, "Hardcopy");

    // SOP Common Module
    if (EC_Normal == writeresult)
      writeresult =
          DVPSHelper::putStringValue(rspDataset, DCM_SOPClassUID,
                                     UID_RETIRED_HardcopyGrayscaleImageStorage);
    if (EC_Normal == writeresult)
      writeresult = referencedSOPClassUID.putString(
          UID_RETIRED_HardcopyGrayscaleImageStorage);
    dcmGenerateUniqueIdentifier(newuid);
    if (EC_Normal == writeresult)
      writeresult =
          DVPSHelper::putStringValue(rspDataset, DCM_SOPInstanceUID, newuid);
    if (EC_Normal == writeresult)
      writeresult = referencedSOPInstanceUID.putString(newuid);
    DVPSHelper::currentDate(aString);
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(
          rspDataset, DCM_InstanceCreationDate, aString.c_str());
    DVPSHelper::currentTime(aString);
    if (EC_Normal == writeresult)
      writeresult = DVPSHelper::putStringValue(
          rspDataset, DCM_InstanceCreationTime, aString.c_str());
...
Uint16 DVPSImageBoxContent::getImageBoxPosition() {
  Uint16 result = 0;
  if (EC_Normal == imageBoxPosition.getUint16(result, 0))
    return result;
  else
    return 0;
}

OFCondition DVPSImageBoxContent::setUIDsAndAETitle(
    DcmUniqueIdentifier& studyUID, DcmUniqueIdentifier& seriesUID,
    const char* aetitle) {
  if (aetitle == NULL)
    return EC_IllegalCall;
 ...
 *  Fixed sequence tag used for writing image box references in Stored Print
 * objects.
 *
