...
DVPSVOIWindow::DVPSVOIWindow()
    : windowCenter(0.0), windowWidth(0.0), windowCenterWidthExplanation() {}
...
OFCondition DVPSVOIWindow::read(size_t idx, DcmDecimalString& wcenter,
                                DcmDecimalString& wwidth, DcmLongString* expl) {
  if (wcenter.getVM() <= idx)
  ...
  if (EC_Normal == result)
    result = wwidth.getFloat64(ww, idx);
  if (EC_Normal == result) {
    windowCenter = wc;
    windowWidth = ww;
    if (expl && (expl->getVM() > idx))
      expl->getOFString(windowCenterWidthExplanation, idx);
    else {
...
const char* DVPSVOIWindow::getExplanation() {
  if (windowCenterWidthExplanation.length() == 0)
    return NULL;
