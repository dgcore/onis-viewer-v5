...
DVPSVOIWindow_PList::DVPSVOIWindow_PList() : list_() {}
...
OFCondition DVPSVOIWindow_PList::read(DcmItem& dset) {
  ...
  if (windowCenterWidthExplanation.getVM() > 0)
    expl = &windowCenterWidthExplanation;

  size_t vm = (size_t)windowCenter.getVM();
  for (size_t i = 0; i < vm; i++) {
    newObject = new DVPSVOIWindow();
    if (newObject) {
      if (EC_Normal == newObject->read(i, windowCenter, windowWidth, expl))
