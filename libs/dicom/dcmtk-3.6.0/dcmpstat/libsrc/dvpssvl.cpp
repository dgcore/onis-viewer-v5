...
DVPSSoftcopyVOI_PList::DVPSSoftcopyVOI_PList() : list_() {}
...
OFCondition DVPSSoftcopyVOI_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal ==
      dset.search(DCM_SoftcopyVOILUTSequence, stack, ESM_fromHere, OFFalse)) {
...
DVPSSoftcopyVOI* DVPSSoftcopyVOI_PList::findSoftcopyVOI(const char* instanceUID,
  ...
  OFListIterator(DVPSSoftcopyVOI*) last = list_.end();
  while (first != last) {
    if ((*first)->isApplicable(instanceUID, frame))
      return (*first);
    ++first;
  }
  return NULL;
...
DVPSSoftcopyVOI* DVPSSoftcopyVOI_PList::createSoftcopyVOI(
  ...
  if (oldArea == NULL)
    ...
    switch (applicability) {
      ...
      case DVPSB_currentImage:
        while (first != last) {
          (*first)->removeImageReference(allReferences, instanceUID, frame,
  ...
  if (newArea) {
    if (applicability != DVPSB_allImages)
      newArea->addImageReference(sopclassUID, instanceUID, frame,
                                 applicability);
...
OFCondition DVPSSoftcopyVOI_PList::createFromImage(
    DcmItem& dset, DVPSReferencedSeries_PList& allReferences,
    const char* sopclassUID, const char* instanceUID,
    DVPSVOIActivation voiActivation) {
  if (voiActivation == DVPSV_ignoreVOI)
    return EC_Normal;
  ...
  if (result == EC_Normal) {
    stack.clear();
    if (EC_Normal ==
        dset.search(DCM_VOILUTSequence, stack, ESM_fromHere, OFFalse)) {
      ...
      if (seq->card() > 0) {
        ...
        stack.clear();
        if (EC_Normal == item->search((DcmTagKey&)voiLUTData.getTag(), stack,
  ...
  OFBool haveLUT =
      ((voiLUTDescriptor.getVM() == 3) && (voiLUTData.getLength() > 0));

  if (result == EC_Normal) {
    DVPSSoftcopyVOI* voi = NULL;
    if (haveWindow &&
        ((voiActivation == DVPSV_preferVOIWindow) || (!haveLUT))) {
      // create VOI window
      Float64 wc, ww;
      char* wexp = NULL;
      result = windowCenter.getFloat64(wc, 0);
      if (EC_Normal == result)
        result = windowWidth.getFloat64(ww, 0);
      if (windowCenterWidthExplanation.getLength() > 0) {
        if (EC_Normal != windowCenterWidthExplanation.getString(wexp))
          wexp = NULL;
      }
      if (EC_Normal == result) {
        voi = createSoftcopyVOI(allReferences, sopclassUID, instanceUID, 1, 1,
                                DVPSB_currentImage);
        if (voi)
          result = voi->setVOIWindow(wc, ww, wexp);
        else
          result = EC_MemoryExhausted;
      }
    } else if (haveLUT &&
               ((voiActivation == DVPSV_preferVOILUT) || (!haveWindow))) {
      // create VOI LUT
      voi = createSoftcopyVOI(allReferences, sopclassUID, instanceUID, 1, 1,
                              DVPSB_currentImage);
      if (voi)
        result =
            voi->setVOILUT(voiLUTDescriptor, voiLUTData, voiLUTExplanation);
      else
        result = EC_MemoryExhausted;
    }
  }
