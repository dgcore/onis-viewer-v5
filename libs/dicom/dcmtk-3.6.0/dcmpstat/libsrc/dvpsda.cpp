...
DVPSDisplayedArea::DVPSDisplayedArea()
    : referencedImageList(),
      ...
      displayedAreaBottomRightHandCorner(
          DCM_DisplayedAreaBottomRightHandCorner),
      ...
      presentationPixelMagnificationRatio(
          DCM_PresentationPixelMagnificationRatio) {}
...
DVPSDisplayedArea::DVPSDisplayedArea(const DVPSDisplayedArea& copy)
    : referencedImageList(copy.referencedImageList),
      ...
      presentationPixelMagnificationRatio(
          copy.presentationPixelMagnificationRatio) {}
...
OFCondition DVPSDisplayedArea::read(DcmItem& dset) {
  ...
  if (result == EC_Normal)
    result = referencedImageList.read(dset);
  ...
  if (displayedAreaTopLeftHandCorner.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "displayedAreaTopLeftHandCorner absent or empty");
  } else if (displayedAreaTopLeftHandCorner.getVM() != 2) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "displayedAreaTopLeftHandCorner VM != 2");
  }
  ...
  if (displayedAreaBottomRightHandCorner.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "displayedAreaBottomRightHandCorner absent or empty");
  } else if (displayedAreaBottomRightHandCorner.getVM() != 2) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "displayedAreaBottomRightHandCorner VM != 2");
  }
  ...
  if (presentationSizeMode.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "presentationSizeMode absent or empty");
  } else if (presentationSizeMode.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "presentationSizeMode VM != 1");
  } else {
    aString.clear();
    ...
    if (aString == "TRUE SIZE") {
      if (presentationPixelSpacing.getVM() != 2) {
        ...
        DCMPSTAT_INFO(
            "presentation state contains a display area selection SQ item with "
            "mode 'TRUE SIZE' but presentationPixelSpacing VM != 2");
      }
      ...
      if (presentationPixelMagnificationRatio.getVM() != 1) {
        ...
        DCMPSTAT_INFO(
            "presentation state contains a display area selection SQ item with "
            "mode 'MAGNIFY' but presentationPixelSpacing VM != 1");
      }
    } else if (aString != "SCALE TO FIT") {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "presentation state contains a display area selection SQ item with "
          "unknown presentation size mode: "
          << aString);
    }
  }

  if ((presentationPixelSpacing.getLength() > 0) &&
      (presentationPixelSpacing.getVM() != 2)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "presentationPixelSpacing VM != 2");
  }

  if ((presentationPixelAspectRatio.getLength() > 0) &&
      (presentationPixelAspectRatio.getVM() != 2)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "presentationPixelAspectRatio VM != 2");
  }

  if ((presentationPixelMagnificationRatio.getLength() > 0) &&
      (presentationPixelMagnificationRatio.getVM() != 1)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "presentationPixelMagnificationRatio VM != 1");
  }
  ...
  if ((presentationPixelSpacing.getLength() == 0) &&
    ...
    DCMPSTAT_INFO(
        "presentation state contains a display area selection SQ item with "
        "both presentationPixelSpacing and presentationPixelAspectRatio "
        "missing/incorrect");
  }
...
OFCondition DVPSDisplayedArea::write(DcmItem& dset) {
  ...
  if ((presentationPixelSpacing.getLength() == 0) &&
      (presentationPixelAspectRatio.getLength() == 0))
  ...
  if (presentationPixelSpacing.getLength() > 0) {
    ADD_TO_DATASET(DcmDecimalString, presentationPixelSpacing)
  }
  if (presentationPixelAspectRatio.getLength() > 0) {
    ADD_TO_DATASET(DcmIntegerString, presentationPixelAspectRatio)
  }
  if (presentationPixelMagnificationRatio.getLength() > 0) {
    ADD_TO_DATASET(DcmFloatingPointSingle, presentationPixelMagnificationRatio)
  }
  if ((result == EC_Normal) && (referencedImageList.size() > 0))
    result = referencedImageList.write(dset);
...
OFBool DVPSDisplayedArea::matchesApplicability(
    const char* instanceUID, unsigned long frame,
    DVPSObjectApplicability applicability) {
  return referencedImageList.matchesApplicability(instanceUID, frame,
                                                  applicability);
}

void DVPSDisplayedArea::removeImageReference(
    DVPSReferencedSeries_PList& allReferences, const char* instanceUID,
    unsigned long frame, unsigned long numberOfFrames,
    DVPSObjectApplicability applicability) {
  referencedImageList.removeImageReference(allReferences, instanceUID, frame,
                                           numberOfFrames, applicability);
  return;
}
...
DVPSPresentationSizeMode DVPSDisplayedArea::getPresentationSizeMode() {
  ...
  presentationSizeMode.getOFString(aString, 0);
  if (aString == "TRUE SIZE")
    return DVPSD_trueSize;
  else if (aString == "MAGNIFY")
    return DVPSD_magnify;
...
double DVPSDisplayedArea::getPresentationPixelAspectRatio() {
  ...
  if (presentationPixelSpacing.getVM() == 2) {
    ...
    if (siY != 0)
      result = ((double)siX) / ((double)siY);
  }
  return result;
}

void DVPSDisplayedArea::getDisplayedArea(Sint32& tlhcX, Sint32& tlhcY,
                                         Sint32& brhcX, Sint32& brhcY) {
  if (EC_Normal != displayedAreaTopLeftHandCorner.getSint32(tlhcX, 0))
    tlhcX = 0;
  if (EC_Normal != displayedAreaTopLeftHandCorner.getSint32(tlhcY, 1))
    tlhcY = 0;
  if (EC_Normal != displayedAreaBottomRightHandCorner.getSint32(brhcX, 0))
    brhcX = 0;
  if (EC_Normal != displayedAreaBottomRightHandCorner.getSint32(brhcY, 1))
    brhcY = 0;
  return;
}
...
OFCondition DVPSDisplayedArea::getPresentationPixelMagnificationRatio(
...
OFCondition DVPSDisplayedArea::setDisplayedAreaPixelSpacing(
    const char* spacing) {
  if (spacing == NULL)
    return EC_IllegalCall;
  ...
  if (EC_Normal == result) {
    ...
    if (fl == 0.0)
      ...
      presentationPixelSpacing.putFloat64(
          -fl, 0);  // silently correct negative values
    fl = 0.0;
    ...
    if (fl == 0.0)
      ...
      presentationPixelSpacing.putFloat64(
          -fl, 1);  // silently correct negative values
  }
  if (EC_Normal != result)
...
OFCondition DVPSDisplayedArea::setDisplayedAreaPixelAspectRatio(
    const char* ratio) {
  if (ratio == NULL)
    return EC_IllegalCall;
  presentationPixelSpacing.clear();
  ...
  if (EC_Normal == result) {
    ...
    if (si == 0)
      ...
      presentationPixelAspectRatio.putSint32(
          -si, 0);  // silently correct negative values
    si = 0;
    ...
    if (si == 0)
      ...
      presentationPixelAspectRatio.putSint32(
          -si, 1);  // silently correct negative values
  }
  if (EC_Normal != result)
    presentationPixelAspectRatio.clear();
  return result;
...
OFCondition DVPSDisplayedArea::setDisplayedArea(
    ...
    Sint32 brhcY, double magnification) {
  OFCondition result = EC_Normal;
  switch (sizeMode) {
    case DVPSD_scaleToFit:
      result = presentationSizeMode.putString("SCALE TO FIT");
      if (EC_Normal == result)
        presentationPixelMagnificationRatio.clear();
      break;
    case DVPSD_trueSize:
      if (!canUseTrueSize())
        return EC_IllegalCall;
    ...
    case DVPSD_magnify:
      ...
      if (EC_Normal == result)
        presentationPixelMagnificationRatio.clear();
      if (EC_Normal == result)
        result = presentationPixelMagnificationRatio.putFloat32(
            (Float32)magnification, 0);
      break;
  }
  if (EC_Normal == result)
    result = displayedAreaTopLeftHandCorner.putSint32(tlhcX, 0);
  if (EC_Normal == result)
    result = displayedAreaTopLeftHandCorner.putSint32(tlhcY, 1);
  if (EC_Normal == result)
    result = displayedAreaBottomRightHandCorner.putSint32(brhcX, 0);
  if (EC_Normal == result)
    result = displayedAreaBottomRightHandCorner.putSint32(brhcY, 1);
  return result;
}
