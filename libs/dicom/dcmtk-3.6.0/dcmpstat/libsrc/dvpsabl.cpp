...
DVPSAnnotationContent_PList::DVPSAnnotationContent_PList() : list_() {}

DVPSAnnotationContent_PList::DVPSAnnotationContent_PList(
    const DVPSAnnotationContent_PList& arg)
    : list_() {
...
OFCondition DVPSAnnotationContent_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal == dset.search(DCM_RETIRED_AnnotationContentSequence, stack,
...
OFCondition DVPSAnnotationContent_PList::deleteMultipleAnnotations(
    size_t number) {
  OFListIterator(DVPSAnnotationContent*) first = list_.begin();
  OFListIterator(DVPSAnnotationContent*) last = list_.end();
...
DVPSAnnotationContent* DVPSAnnotationContent_PList::getAnnotationBox(
    size_t idx) {
  OFListIterator(DVPSAnnotationContent*) first = list_.begin();
  OFListIterator(DVPSAnnotationContent*) last = list_.end();
  while (first != last) {
...
OFCondition DVPSAnnotationContent_PList::setAnnotationSOPInstanceUID(
...
OFCondition DVPSAnnotationContent_PList::prepareBasicAnnotationBox(
    size_t idx, DcmItem& dset) {
  DVPSAnnotationContent* box = getAnnotationBox(idx);
  if (box)
    return box->prepareBasicAnnotationBox(dset);
  return EC_IllegalCall;
}
