...
OFCondition DVPSGraphicObject::read(DcmItem& dset) {
  ...
  DcmUnsignedShort graphicDimensions(
      DCM_GraphicDimensions);  // VR=US, VM=1, Type 1
  ...
  if (graphicAnnotationUnits.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with "
        "graphicAnnotationUnits absent or empty");
  } else if (graphicAnnotationUnits.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with "
        "graphicAnnotationUnits VM != 1");
  }
  ...
  if (graphicDimensions.getVM() != 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with "
        "graphicDimensions VM != 1");
  } else {
    Uint16 dimension = 0;
    if ((EC_Normal != graphicDimensions.getUint16(dimension, 0)) ||
        (dimension != 2)) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "presentation state contains a graphic object SQ item with "
  ...
  if (numberOfGraphicPoints.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with "
        "numberOfGraphicPoints absent or empty");
  } else if (numberOfGraphicPoints.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with "
        "numberOfGraphicPoints VM != 1");
  }

  if (graphicData.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with graphicData "
        "absent or empty");
  ...
  if (graphicType.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with graphicType "
        "absent or empty");
  ...
  if ((graphicFilled.getLength() > 0) && (graphicFilled.getVM() != 1)) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a graphic object SQ item with "
        "graphicFilled present but VM != 1");
  }
...
size_t DVPSGraphicObject::getNumberOfPoints() {
  Uint16 result = 0;
  if (EC_Normal == numberOfGraphicPoints.getUint16(result, 0))
    return result;
  else
    return 0;
}
...
OFCondition DVPSGraphicObject::getPoint(size_t idx, Float32& x, Float32& y) {
  ...
  if ((idx * 2 + 1) < graphicData.getVM()) {
    ...
    if (result == EC_Normal)
      result = graphicData.getFloat32(y, 2 * idx + 1);
  }
  return result;
}
...
DVPSGraphicType DVPSGraphicObject::getGraphicType() {
  ...
  if (aString == "INTERPOLATED")
    return DVPST_interpolated;
  else if (aString == "CIRCLE")
    return DVPST_circle;
  else if (aString == "ELLIPSE")
    return DVPST_ellipse;
  else if (aString == "POINT")
    return DVPST_point;
  /* default */
...
OFCondition DVPSGraphicObject::setData(size_t number, const Float32* data,
  ...
  if (EC_Normal == result)
    result = numberOfGraphicPoints.putUint16(npoints, 0);
  if (EC_Normal == result) {
    if (unit == DVPSA_display)
      result = graphicAnnotationUnits.putString("DISPLAY");
    else
      result = graphicAnnotationUnits.putString("PIXEL");
  }
  return result;
}
...
OFCondition DVPSGraphicObject::setFilled(OFBool filled) {
  OFCondition result = EC_Normal;
  if (filled)
    result = graphicFilled.putString("Y");
  else
    result = graphicFilled.putString("N");
  return result;
}
