...
DVPSImageBoxContent_PList::DVPSImageBoxContent_PList() : list_() {}

DVPSImageBoxContent_PList::DVPSImageBoxContent_PList(
    const DVPSImageBoxContent_PList& arg)
    : list_() {
...
OFCondition DVPSImageBoxContent_PList::read(
  ...
  if (EC_Normal == dset.search(DCM_RETIRED_ImageBoxContentSequence, stack,
...
OFCondition DVPSImageBoxContent_PList::write(DcmItem& dset,
  ...
  if (size() == 0)
    return EC_IllegalCall;  // can't write if sequence is empty
  ...
  if (dseq) {
    ...
    while ((first != last) && working) {
      if ((result == EC_Normal) &&
          ((!ignoreEmptyImages) || ((*first)->getImageBoxPosition() > 0))) {
        ditem = new DcmItem();
        if (ditem) {
          result = (*first)->write(*ditem, writeRequestedImageSize,
                                   writeReferencedPLUTSQ);
          if (result == EC_Normal) {
            dseq->insert(ditem);
            numWritten++;
          } else
    ...
    if ((result == EC_Normal) && (numWritten > 0))
...
OFCondition DVPSImageBoxContent_PList::createDefaultValues(
    OFBool renumber, OFBool ignoreEmptyImages) {
  if (size() == 0)
    return EC_IllegalCall;  // can't write if sequence is empty
  OFCondition result = EC_Normal;
  ...
  while ((first != last) && (EC_Normal == result)) {
    result =
        (*first)->createDefaultValues(renumber, counter++, ignoreEmptyImages);
    ++first;
  }
  return result;
}

OFCondition DVPSImageBoxContent_PList::addImageSOPClasses(
    DcmSequenceOfItems& seq, size_t numItems) {
  OFCondition result = EC_Normal;
  OFBool working = OFTrue;
  const char* c = NULL;
  OFListIterator(DVPSImageBoxContent*) first = list_.begin();
  ...
  while ((first != last) && working) {
    if (EC_Normal == result) {
      ...
      if (c && (!DVPSHelper::haveReferencedUIDItem(seq, c)))
        result = DVPSHelper::addReferencedUIDItem(seq, c);
    }
    ++first;
    if (numItems && (--numItems == 0))
      working = OFFalse;
...
OFCondition DVPSImageBoxContent_PList::addImageBox(
  ...
  if (newImage) {
    result = newImage->setContent(
        instanceuid, retrieveaetitle, refstudyuid, refseriesuid, refsopclassuid,
        refsopinstanceuid, requestedimagesize, patientid, presentationlutuid);
    if (EC_Normal == result)
      list_.push_back(newImage);
    else
      delete newImage;
...
OFCondition DVPSImageBoxContent_PList::setRequestedDecimateCropBehaviour(
...
OFBool DVPSImageBoxContent_PList::imageHasAdditionalSettings(size_t idx) {
  ...
  if (box)
    return box->hasAdditionalSettings();
  return OFFalse;
}
...
OFCondition DVPSImageBoxContent_PList::setImagePolarity(size_t idx,
  ...
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->setPolarity(value);
  return EC_IllegalCall;
}

OFCondition DVPSImageBoxContent_PList::setImageRequestedSize(
    size_t idx, const char* value) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->setRequestedImageSize(value);
  return EC_IllegalCall;
}

OFCondition DVPSImageBoxContent_PList::setImageMagnificationType(
    size_t idx, const char* value) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->setMagnificationType(value);
  return EC_IllegalCall;
}

OFCondition DVPSImageBoxContent_PList::setImageSmoothingType(
    size_t idx, const char* value) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->setSmoothingType(value);
  return EC_IllegalCall;
}

OFCondition DVPSImageBoxContent_PList::setImageConfigurationInformation(
    size_t idx, const char* value) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->setConfigurationInformation(value);
  return EC_IllegalCall;
}

OFCondition DVPSImageBoxContent_PList::setImageSOPInstanceUID(
    size_t idx, const char* value) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->setSOPInstanceUID(value);
  return EC_IllegalCall;
}
...
const char* DVPSImageBoxContent_PList::getImageRequestedSize(size_t idx) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->getRequestedImageSize();
  return NULL;
}
...
const char* DVPSImageBoxContent_PList::getImageMagnificationType(size_t idx) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->getMagnificationType();
  return NULL;
}
...
const char* DVPSImageBoxContent_PList::getImageSmoothingType(size_t idx) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->getSmoothingType();
  return NULL;
}

const char* DVPSImageBoxContent_PList::getImageConfigurationInformation(
    size_t idx) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->getConfigurationInformation();
  return NULL;
}
...
const char* DVPSImageBoxContent_PList::getSOPInstanceUID(size_t idx) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->getSOPInstanceUID();
  return NULL;
}

const char* DVPSImageBoxContent_PList::getReferencedPresentationLUTInstanceUID(
    size_t idx) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->getReferencedPresentationLUTInstanceUID();
  return NULL;
}
...
OFCondition DVPSImageBoxContent_PList::getImageReference(
    size_t idx, const char*& studyUID, const char*& seriesUID,
    const char*& instanceUID) {
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->getImageReference(studyUID, seriesUID, instanceUID);
  return EC_IllegalCall;
}
...
OFCondition DVPSImageBoxContent_PList::prepareBasicImageBox(size_t idx,
  ...
  DVPSImageBoxContent* box = getImageBox(idx);
  if (box)
    return box->prepareBasicImageBox(dset);
  return EC_IllegalCall;
}

OFBool DVPSImageBoxContent_PList::presentationLUTInstanceUIDisUsed(
    const char* uid) {
  OFString uidS;
  if (uid)
    uidS = uid;
...
const char* DVPSImageBoxContent_PList::haveSinglePresentationLUTUsed(
    const char* filmBox) {
  OFList<char*> uidList;
  if (filmBox == NULL)
    filmBox = "";
  ...
  while (first != last) {
    ...
    if ((c == NULL) || (strlen(c) == 0))
      c = filmBox;
    aString = c;  // aString now contains the UID of the P-LUT to be used for
                  // this image, if any.
    ...
    if (!found)
      uidList.push_back((char*)c);
    ++first;
  }

  if (uidList.size() == 1)
    return *(uidList.begin());  // if there is only one LUT, return it
  return NULL;
}
...
OFBool DVPSImageBoxContent_PList::printSCPCreate(unsigned long numBoxes,
  ...
  for (unsigned long i = 0; i < numBoxes; i++) {
    ...
    if (box) {
      if ((EC_Normal ==
           box->setSOPInstanceUID(dcmGenerateUniqueIdentifier(uid))) &&
...
OFCondition DVPSImageBoxContent_PList::writeReferencedImageBoxSQ(
    DcmItem& dset) {
  if (size() == 0)
    return EC_IllegalCall;  // can't write if sequence is empty
    ...
    while (first != last) {
      if (result == EC_Normal) {
        ...
        if (ditem) {
          ...
          if (uid)
            result = uid->putString(UID_BasicGrayscaleImageBoxSOPClass);
          else
            result = EC_MemoryExhausted;
          if (EC_Normal == result)
            result = ditem->insert(uid, OFTrue /*replaceOld*/);
          else
            delete uid;
          ...
          if (uid && instanceUID)
            result = uid->putString(instanceUID);
          else
            result = EC_MemoryExhausted;
          if (EC_Normal == result)
            result = ditem->insert(uid, OFTrue /*replaceOld*/);
...
OFBool DVPSImageBoxContent_PList::matchesPresentationLUT(
...
DVPSImageBoxContent* DVPSImageBoxContent_PList::duplicateImageBox(
    const char* uid) {
  if (uid == NULL)
    return NULL;
...
OFBool DVPSImageBoxContent_PList::haveImagePositionClash(const char* uid,
  ...
  OFListIterator(DVPSImageBoxContent*) last = list_.end();
  while (first != last) {
    if (((*first)->getImageBoxPosition() == position) &&
        (aString != (*first)->getSOPInstanceUID()))
      return OFTrue;  // clash
    ++first;
  }
  return OFFalse;
...
OFBool DVPSImageBoxContent_PList::emptyPageWarning() {
  ...
  OFListIterator(DVPSImageBoxContent*) last = list_.end();
  while (first != last) {
    if ((*first)->getImageBoxPosition() > 0)
      return OFFalse;
    ++first;
  }
  return OFTrue;
