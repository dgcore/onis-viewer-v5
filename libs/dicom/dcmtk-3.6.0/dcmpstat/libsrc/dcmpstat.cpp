...
OFLogger DCM_dcmpstatLogfileGetLogger() {
  static OFLogger DCM_dcmpstatLogger =
      OFLog::getLogger("dcmtk.dcmpstat.logfile");
  return DCM_dcmpstatLogger;
}
...
const char* DcmPresentationState::createInstanceUID() {
  ...
  if (EC_Normal == result) {
    if (EC_Normal != sOPInstanceUID.getString(puid))
      puid = NULL;
  }
  return puid;
}
...
const char* DcmPresentationState::getPatientID() {
  char* c = NULL;
  if (EC_Normal == patientID.getString(c))
    return c;
  else
    return NULL;
}
...
OFCondition DcmPresentationState::createDummyValues(
  ...
  if ((result == EC_Normal) &&
      (replaceSOPInstanceUID || (sOPInstanceUID.getLength() == 0))) {
    sOPInstanceUID.putString(dcmGenerateUniqueIdentifier(uid));
    DVPSHelper::currentDate(aString);
    DVPSHelper::setDefault(result, instanceCreationDate, aString.c_str());
    DVPSHelper::currentTime(aString);
  ...
  // DVPSHelper::setDefault(result, specificCharacterSet,
  // DEFAULT_specificCharacterSet );

  // create some dummy defaults for shutters
  if (shutterPresentationValue.getVM() != 1)
    shutterPresentationValue.putUint16(0);
  ...
  DVPSHelper::setDefault(result, verticesOfThePolygonalShutter,
                         "1\\1\\1\\1024\\1024\\1024\\1024\\1\\1\\1");
  if (shutterOverlayGroup.getVM() != 1)
    shutterOverlayGroup.putUint16(0x6000);
...
OFCondition DcmPresentationState::read(DcmItem& dset) {
  ...
  if (result == EC_Normal) {
    stack.clear();
    if (EC_Normal ==
        dset.search(DCM_ModalityLUTSequence, stack, ESM_fromHere, OFFalse)) {
      seq = (DcmSequenceOfItems*)stack.top();
      if (seq->card() == 1) {
        item = seq->getItem(0);
        stack.clear();
        if (EC_Normal ==
            item->search((DcmTagKey&)modalityLUTDescriptor.getTag(), stack,
                         ESM_fromHere, OFFalse)) {
          modalityLUTDescriptor = *((DcmUnsignedShort*)(stack.top()));
        }
        stack.clear();
        if (EC_Normal ==
            item->search((DcmTagKey&)modalityLUTExplanation.getTag(), stack,
                         ESM_fromHere, OFFalse)) {
        ...
        DCMPSTAT_INFO(
            "Modality LUT SQ does not have exactly one item in presentation "
            "state");
      }
    }
  }
  ...
  if (result == EC_Normal)
    result = overlayList.read(dset);
  if (result == EC_Normal)
    result = activationLayerList.read(dset);
  if (result == EC_Normal)
    result = graphicLayerList.read(dset);
  if (result == EC_Normal)
    result = referencedSeriesList.read(dset);
  if (result == EC_Normal)
    result = graphicAnnotationList.read(dset);
  if (result == EC_Normal)
    result = displayedAreaSelectionList.read(dset);
  if (result == EC_Normal)
    result = softcopyVOIList.read(dset);
  if (result == EC_Normal)
    result = presentationLUT.read(dset, OFFalse);
  ...
  if (result == EC_Normal) {
    ...
    if (displayedAreaSelectionList.size() == 0) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "displayedAreaSelectionSQ absent or empty in presentation state");
    }
    ...
    if (presentationCreationDate.getLength() == 0) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "presentationCreationDate absent or empty in presentation state");
    ...
    if (presentationCreationTime.getLength() == 0) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "presentationCreationTime absent or empty in presentation state");
    ...
    /* if imageRotation or imageHorizontalFlip are present, then both must be
     * present. */
    if ((imageRotation.getLength() > 0) &&
        (imageHorizontalFlip.getLength() == 0)) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "imageRotation present but imageHorizontalFlip absent or empty in "
          "presentation state");
    }
    ...
    if ((imageRotation.getLength() == 0) &&
      ...
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "imageHorizontalFlip present but imageRotation absent or empty in "
          "presentation state");
    }
    ...
    if (rescaleIntercept.getLength() > 0) {
      ...
      if (rescaleSlope.getLength() == 0) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rescaleIntercept present but rescaleSlope absent or empty in "
            "presentation state");
      } else if (rescaleSlope.getVM() != 1) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rescaleIntercept present but rescaleSlope VM != 1 in presentation "
            "state");
      }
      if (rescaleType.getLength() == 0) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rescaleIntercept present but rescaleType absent or empty in "
            "presentation state");
      } else if (rescaleType.getVM() != 1) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rescaleIntercept present but rescaleType VM != 1 in presentation "
            "state");
      }
      if (rescaleIntercept.getVM() != 1) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rescaleIntercept present but VM != 1 in presentation state");
      }
    } else
      useModalityRescale = OFFalse;
    if (modalityLUTData.getLength() > 0) {
      ...
      if (modalityLUTDescriptor.getLength() == 0) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "modalityLUTData present but modalityLUTDescriptor absent or empty "
            "in presentation state");
      } else if (modalityLUTDescriptor.getVM() != 3) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "modalityLUTData present but modalityLUTDescriptor VM != 3 in "
            "presentation state");
      }
    ...
    if (useModalityRescale && useModalityLUT) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "both modality rescale and LUT present in presentation state");
    }
  ...
  if (result == EC_Normal) {
    ...
    for (i = 0; i < shutterShape.getVM(); i++) {
      ...
      if (aString == "RECTANGULAR")
        useShutterRectangular = OFTrue;
      else if (aString == "CIRCULAR")
        useShutterCircular = OFTrue;
      else if (aString == "POLYGONAL")
        useShutterPolygonal = OFTrue;
      else if (aString == "BITMAP")
    ...
    if (useShutterRectangular) {
      if ((shutterLeftVerticalEdge.getLength() == 0) ||
          (shutterLeftVerticalEdge.getVM() != 1)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rectangular shutter used but shutterLeftVerticalEdge absent or VM "
            "!= 1 in presentation state");
      }
      if ((shutterRightVerticalEdge.getLength() == 0) ||
          (shutterRightVerticalEdge.getVM() != 1)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rectangular shutter used but shutterRightVerticalEdge absent or "
            "VM != 1 in presentation state");
      }
      if ((shutterUpperHorizontalEdge.getLength() == 0) ||
          (shutterUpperHorizontalEdge.getVM() != 1)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rectangular shutter used but shutterUpperHorizontalEdge absent or "
            "VM != 1 in presentation state");
      }
      if ((shutterLowerHorizontalEdge.getLength() == 0) ||
          (shutterLowerHorizontalEdge.getVM() != 1)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "rectangular shutter used but shutterLowerHorizontalEdge absent or "
            "VM != 1 in presentation state");
      }
    }
    if (useShutterCircular) {
      if ((centerOfCircularShutter.getLength() == 0) ||
          (centerOfCircularShutter.getVM() != 2)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "circular shutter used but centerOfCircularShutter absent or VM != "
            "2 in presentation state");
      }
      if ((radiusOfCircularShutter.getLength() == 0) ||
          (radiusOfCircularShutter.getVM() != 1)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "circular shutter used but radiusOfCircularShutter absent or VM != "
            "1 in presentation state");
      }
    }
    if (useShutterPolygonal) {
      ...
      if ((verticesOfThePolygonalShutter.getLength() == 0) ||
        ...
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "polygonal shutter used but verticesOfThePolygonalShutter absent "
            "or VM != 2-2n in presentation state");
      }
    }
    if (useShutterBitmap) {
      if ((shutterOverlayGroup.getLength() == 0) ||
          (shutterOverlayGroup.getVM() != 1)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "bitmap shutter used but shutterOverlayGroup absent or VM != 1 in "
            "presentation state");
      }
    }
    if (useShutterRectangular || useShutterCircular || useShutterPolygonal ||
      ...
      if ((shutterPresentationValue.getLength() == 0) ||
          (shutterPresentationValue.getVM() != 1)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "shutter used but shutterPresentationValue absent or VM != 1 in "
            "presentation state");
      }
    }
     ...
     */
    if (useShutterBitmap &&
        (useShutterRectangular || useShutterCircular || useShutterPolygonal)) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "both bitmap and rect/circ/poly shutter specified in presentation "
          "state");
    }
  }
  ...
  if ((result == EC_Normal) && useShutterBitmap) {
    ...
    if (shuttergroup >= 0x6000) {
      if (!overlayList.haveOverlayGroup(shuttergroup)) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "bitmap shutter specified but referenced overlay group missing in "
            "presentation state");
      }
    } else {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "wrong overlay group specified for bitmap shutter in presentation "
          "state");
    }
  }

  /* check that referenced series list is non-empty and that every series
     contains images and that all referenced images share the same SOP class
     UID.
   */
  if (result.good() && (!referencedSeriesList.isValid())) {
    // referencedSeriesList.isValid() has already displayed an error message in
    // verbose mode
    result = EC_IllegalCall;
  }

  /* check that there is no mask module present. We don't support Mask and
     therefore are obliged to reject any presentation state containing a mask!
   */

  stack.clear();
  if (EC_Normal ==
      dset.search(DCM_MaskSubtractionSequence, stack, ESM_fromHere, OFFalse)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO("mask module present in presentation state, but unsupported");
  }
  stack.clear();
  if (EC_Normal ==
      dset.search(DCM_RecommendedViewingMode, stack, ESM_fromHere, OFFalse)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO("mask module present in presentation state, but unsupported");
  }
...
OFCondition DcmPresentationState::createDefaultDisplayedArea(DcmItem& dset) {
  ...
  if (result == EC_Normal)
    result = columns.getUint16(cols_uint, 0);
  if (result == EC_Normal)
    result = rows.getUint16(rows_uint, 0);
  if (result == EC_Normal)
    result = sopclassUID.getString(c_sopclassUID);
  if (result == EC_Normal)
    result = imageUID.getString(c_imageUID);

  /* create displayed area selection item for this image */
  if (EC_Normal == result) {
    DVPSDisplayedArea* area = displayedAreaSelectionList.createDisplayedArea(
        referencedSeriesList, c_sopclassUID, c_imageUID, 1, 1,
        DVPSB_currentImage);
    if (area) {
      if (EC_Normal == result)
        result = area->setDisplayedArea(DVPSD_scaleToFit, (Sint32)1, (Sint32)1,
                                        (Sint32)cols_uint, (Sint32)rows_uint);
      if (EC_Normal == result) {
        OFBool foundRatio = OFFalse;
        if ((imagerPixelSpacing.getVM() == 2) &&
            (EC_Normal == imagerPixelSpacing.getString(pixelspacing))) {
          result = area->setDisplayedAreaPixelSpacing(pixelspacing);
          if (EC_Normal == result)
            foundRatio = OFTrue;
        }
        if ((!foundRatio) && (pixelSpacing.getVM() == 2) &&
            (EC_Normal == pixelSpacing.getString(pixelspacing))) {
          result = area->setDisplayedAreaPixelSpacing(pixelspacing);
          if (EC_Normal == result)
            foundRatio = OFTrue;
        }
        if ((!foundRatio) && (pixelAspectRatio.getVM() == 2) &&
            (EC_Normal == pixelAspectRatio.getString(pixelspacing))) {
          result = area->setDisplayedAreaPixelAspectRatio(pixelspacing);
          if (EC_Normal == result)
            foundRatio = OFTrue;
        }
        if (!foundRatio)
          result = area->setDisplayedAreaPixelAspectRatio(1.0);
      }
    } else
...
OFCondition DcmPresentationState::createFromImage(
  ...
  if ((result == EC_Normal) && (overlayActivation == DVPSO_copyOverlays))
    result = overlayList.read(dset);
  ...
  if (photometricInterpretation.getVM() == 1) {
    ...
    if ((aString == "MONOCHROME1") || (aString == "MONOCHROME 1"))
      isMonochrome1 = OFTrue;
    else if ((aString != "MONOCHROME2") && (aString != "MONOCHROME 2")) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "Wrong image photometric interpretation - not MONOCHROME1/2");
    }
  }
  ...
  if (result == EC_Normal)
    result = seriesInstanceUID.putString(
        dcmGenerateUniqueIdentifier(uid, SITE_SERIES_UID_ROOT));
  if (result == EC_Normal)
    result = sOPInstanceUID.putString(dcmGenerateUniqueIdentifier(uid));
  if (result == EC_Normal)
    result = seriesNumber.putString(DEFAULT_seriesNumber);
  if (result == EC_Normal)
    result = specificCharacterSet.putString(DEFAULT_specificCharacterSet);

  if (result == EC_Normal) {
    /* Presentation LUT Shape */
    if (isMonochrome1)
      presentationLUT.setType(DVPSP_inverse);
    else
  ...
  if (result == EC_Normal) {
    ...
    if ((rescaleIntercept.getVM() == 1) && (rescaleSlope.getVM() == 1)) {
      ...
      modalityLUTExplanation.clear();
      modalityLUTType.clear();
      // be lenient with Modality rescale type. If missing or wrong, just invent
      // a value.
      if (rescaleType.getVM() != 1)
        rescaleType.putString("US");
    } else
      useModalityRescale = OFFalse;
    ...
    if ((modalityLUTDescriptor.getVM() == 3) &&
        (modalityLUTData.getLength() > 0)) {
      useModalityLUT = OFTrue;
      rescaleIntercept.clear();
      rescaleSlope.clear();
      rescaleType.clear();
   ...
   */
  if (result == EC_Normal)
    result = activationLayerList.createFromImage(dset, graphicLayerList,
                                                 overlayList, overlayActivation,
                                                 curveActivation, layering);

  /* create reference for this image */
  if (EC_Normal == result)
    result = addImageReference(dset, aetitle, filesetID, filesetUID);

  /* create displayed area selection item for this image */
  if (EC_Normal == result)
    result = createDefaultDisplayedArea(dset);

  /* create softcopy VOI LUT sequence item for this image if necessary */
  if (EC_Normal == result)
    result = softcopyVOIList.createFromImage(dset, referencedSeriesList,
                                             ofsopclassUID.c_str(),
                                             ofimageUID.c_str(), voiActivation);
...
OFCondition DcmPresentationState::write(DcmItem& dset,
  ...
  if (result == EC_Normal) {
    result =
        sopclassuid.putString(UID_GrayscaleSoftcopyPresentationStateStorage);
  }
  ...
  if (instanceCreatorUID.getLength() > 0) {
    ADD_TO_DATASET(DcmUniqueIdentifier, instanceCreatorUID)
  }
  ...
  if (EC_Normal == result)
    result = referencedSeriesList.write(dset);
  if (EC_Normal == result)
    result = overlayList.write(dset);
  if (EC_Normal == result)
    result = activationLayerList.write(dset);
  if (EC_Normal == result)
    result = graphicAnnotationList.write(dset);
  if (EC_Normal == result)
    result = displayedAreaSelectionList.write(dset);
  if (EC_Normal == result)
    result = softcopyVOIList.write(dset);
  if (EC_Normal == result)
    result = graphicLayerList.write(dset);
  if (EC_Normal == result)
...
OFCondition DcmPresentationState::addImageReference(
    const char* studyUID, const char* seriesUID, const char* sopclassUID,
    const char* instanceUID, const char* frames, const char* aetitle,
    const char* filesetID, const char* filesetUID) {
  if ((studyUID == NULL) || (seriesUID == NULL) || (sopclassUID == NULL) ||
      (instanceUID == NULL))
  ...
  if (study != studyUID) {
    DCMPSTAT_INFO(
        "cannot add reference to image with different Study Instance UID.");
    return EC_IllegalCall;
  }
  return referencedSeriesList.addImageReference(seriesUID, sopclassUID,
                                                instanceUID, frames, aetitle,
...
OFCondition DcmPresentationState::addImageReference(DcmItem& dset,
  ...
  if (ofnumberOfFrames > 1) {
    ...
    result = addImageReference(ofstudyUID.c_str(), ofseriesUID.c_str(),
                               ofsopclassUID.c_str(), ofimageUID.c_str(),
                               aString.c_str(), aetitle, filesetID, filesetUID);
  } else {
    result = addImageReference(ofstudyUID.c_str(), ofseriesUID.c_str(),
                               ofsopclassUID.c_str(), ofimageUID.c_str(), NULL,
...
OFCondition DcmPresentationState::removeImageReference(
  ...
  if ((studyUID == NULL) || (seriesUID) || (instanceUID == NULL))
    return EC_IllegalCall;
...
OFCondition DcmPresentationState::removeImageReference(DcmItem& dset) {
  ...
  imageUID.getOFString(ofimageUID, 0);
  studyUID.getOFString(ofstudyUID, 0);
  referencedSeriesList.removeImageReference(ofseriesUID.c_str(),
                                            ofimageUID.c_str());
...
OFCondition DcmPresentationState::getImageReference(
    size_t idx, OFString& studyUID, OFString& seriesUID, OFString& sopclassUID,
    OFString& instanceUID, OFString& frames, OFString& aetitle,
    OFString& filesetID, OFString& filesetUID) {
  OFCondition result = referencedSeriesList.getImageReference(
      idx, seriesUID, sopclassUID, instanceUID, frames, aetitle, filesetID,
      filesetUID);
  if (EC_Normal == result)
    result = studyInstanceUID.getOFString(studyUID, 0);
  return result;
}
...
DVPSRotationType DcmPresentationState::getRotation() {
  ...
  if (rotation == 90)
    ...
    result = DVPSR_180_deg;
  else if (rotation == 270)
    result = DVPSR_270_deg;
  return result;
}
...
OFCondition DcmPresentationState::setFlip(OFBool isFlipped) {
  ...
  displayedAreaSelectionList.rotateAndFlip(rotation, oldFlip, rotation,
                                           isFlipped);

  if (isFlipped)
    return imageHorizontalFlip.putString("Y");
  else
    return imageHorizontalFlip.putString("N");
}
...
OFCondition DcmPresentationState::getPolyShutterVertex(size_t idx, Sint32& x,
                                                       Sint32& y) {
  x = 0;
  y = 0;
  OFCondition result = verticesOfThePolygonalShutter.getSint32(y, 2 * idx);
  if (EC_Normal == result)
    result = verticesOfThePolygonalShutter.getSint32(x, 2 * idx + 1);
  return result;
}
...
const char* DcmPresentationState::getPresentationLabel() {
  char* value = NULL;
  if (EC_Normal != presentationLabel.getString(value))
    return NULL;
  return value;
}
...
const char* DcmPresentationState::getPresentationDescription() {
  char* value = NULL;
  if (EC_Normal != presentationDescription.getString(value))
    return NULL;
  return value;
}
...
const char* DcmPresentationState::getPresentationCreatorsName() {
  char* value = NULL;
  if (EC_Normal != presentationCreatorsName.getString(value))
    return NULL;
  return value;
}
...
OFCondition DcmPresentationState::setPresentationDescription(
    const char* descr) {
  return presentationDescription.putString(descr);
}

OFCondition DcmPresentationState::setPresentationCreatorsName(
    const char* name) {
...
OFBool DcmPresentationState::haveGraphicLayerRecommendedDisplayValue(
    size_t idx) {
  return graphicLayerList.haveGraphicLayerRecommendedDisplayValue(idx);
}

OFCondition DcmPresentationState::getGraphicLayerRecommendedDisplayValueGray(
    size_t idx, Uint16& gray) {
  return graphicLayerList.getGraphicLayerRecommendedDisplayValueGray(idx, gray);
}

OFCondition DcmPresentationState::getGraphicLayerRecommendedDisplayValueRGB(
    size_t idx, Uint16& r, Uint16& g, Uint16& b) {
  return graphicLayerList.getGraphicLayerRecommendedDisplayValueRGB(idx, r, g,
                                                                    b);
}

OFCondition DcmPresentationState::setGraphicLayerRecommendedDisplayValueGray(
    size_t idx, Uint16 gray) {
  return graphicLayerList.setGraphicLayerRecommendedDisplayValueGray(idx, gray);
}

OFCondition DcmPresentationState::setGraphicLayerRecommendedDisplayValueRGB(
    size_t idx, Uint16 r, Uint16 g, Uint16 b) {
  return graphicLayerList.setGraphicLayerRecommendedDisplayValueRGB(idx, r, g,
                                                                    b);
}

void DcmPresentationState::removeGraphicLayerRecommendedDisplayValue(
    size_t idx, OFBool rgb, OFBool monochrome) {
  graphicLayerList.removeGraphicLayerRecommendedDisplayValue(idx, rgb,
                                                             monochrome);
  return;
}
...
OFCondition DcmPresentationState::setGraphicLayerName(size_t idx,
  ...
  if (name == NULL)
    return EC_IllegalCall;
  OFString oldName(
      oname);  // make a copy of the pointer which will become invalid
...
OFCondition DcmPresentationState::setGraphicLayerDescription(
    size_t idx, const char* descr) {
...
OFCondition DcmPresentationState::addGraphicLayer(
    const char* gLayer, const char* gLayerDescription) {
...
DVPScharacterSet DcmPresentationState::getCharset() {
  ...
  if (aString == "ISO_IR 6")
    return DVPSC_ascii;
  else if (aString == "ISO_IR 100")
    return DVPSC_latin1;
  else if (aString == "ISO_IR 101")
    return DVPSC_latin2;
  else if (aString == "ISO_IR 109")
    return DVPSC_latin3;
  else if (aString == "ISO_IR 110")
    return DVPSC_latin4;
  else if (aString == "ISO_IR 148")
    return DVPSC_latin5;
  else if (aString == "ISO_IR 144")
    return DVPSC_cyrillic;
  else if (aString == "ISO_IR 127")
    return DVPSC_arabic;
  else if (aString == "ISO_IR 126")
    return DVPSC_greek;
  else if (aString == "ISO_IR 138")
    return DVPSC_hebrew;
  else if (aString == "ISO_IR 13")
    return DVPSC_japanese;
  /* default */
  return DVPSC_other;
}
...
const char* DcmPresentationState::getCharsetString() {
  char* c = NULL;
  if (EC_Normal == specificCharacterSet.getString(c))
    return c;
  else
    return NULL;
}
...
OFCondition DcmPresentationState::moveCurve(size_t old_layer, size_t idx,
                                            size_t new_layer) {
...
Uint16 DcmPresentationState::getActiveOverlayGroup(size_t layer, size_t idx) {
  return activationLayerList.getActivationGroup(
...
Uint16 DcmPresentationState::getOverlayInPresentationStateGroup(size_t idx) {
  ...
  if (overlay)
    return (0x6000 + overlay->getOverlayGroup());
  return 0;
}

const char* DcmPresentationState::getOverlayInPresentationStateLabel(
    size_t idx) {
  DVPSOverlay* overlay = overlayList.getOverlay(idx);
  if (overlay)
    return overlay->getOverlayLabel();
  return NULL;
}

const char* DcmPresentationState::getOverlayInPresentationStateDescription(
    size_t idx) {
  DVPSOverlay* overlay = overlayList.getOverlay(idx);
  if (overlay)
    return overlay->getOverlayDescription();
  return NULL;
}

size_t DcmPresentationState::getOverlayInPresentationStateActivationLayer(
    size_t idx) {
  DVPSOverlay* overlay = overlayList.getOverlay(idx);
  if (overlay) {
    const char* layerName = activationLayerList.getActivationLayer(
        0x6000 + overlay->getOverlayGroup());
    if (layerName)
...
OFBool DcmPresentationState::overlayIsBitmapShutter(size_t idx) {
  ...
  if (EC_Normal == shutterOverlayGroup.getUint16(shutterGroup, 0)) {
    DVPSOverlay* overlay = overlayList.getOverlay(idx);
    if (overlay)
      return (shutterGroup == 0x6000 + overlay->getOverlayGroup());
  }
  return OFFalse;
}
...
OFBool DcmPresentationState::overlayInPresentationStateIsROI(size_t idx) {
  DVPSOverlay* overlay = overlayList.getOverlay(idx);
  if (overlay)
    return overlay->isROI();
  return OFFalse;
}

OFCondition DcmPresentationState::moveOverlay(size_t old_layer, size_t idx,
                                              size_t new_layer) {
  const char* lname = graphicLayerList.getGraphicLayerName(new_layer);
  if (lname == NULL)
    return EC_IllegalCall;
