...
DVPSReferencedImage_PList::DVPSReferencedImage_PList() : list_() {}

DVPSReferencedImage_PList::DVPSReferencedImage_PList(
    const DVPSReferencedImage_PList& arg)
    : list_() {
...
OFCondition DVPSReferencedImage_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal ==
      dset.search(DCM_ReferencedImageSequence, stack, ESM_fromHere, OFFalse)) {
...
OFBool DVPSReferencedImage_PList::isValid(OFString& sopclassuid) {
  if (size() == 0) {
    DCMPSTAT_INFO(
        "referenced image SQ contains empty item in presentation state");
    return OFFalse;
  }
...
DVPSReferencedImage* DVPSReferencedImage_PList::findImageReference(
...
void DVPSReferencedImage_PList::removeFrameReference(
    const char* sopinstanceuid, unsigned long frame,
    unsigned long numberOfFrames) {
  if ((frame < 1) || (numberOfFrames < frame))
    return;
  OFListIterator(DVPSReferencedImage*) first = list_.begin();
  OFListIterator(DVPSReferencedImage*) last = list_.end();
  while (first != last) {
...
OFCondition DVPSReferencedImage_PList::addImageReference(
    const char* sopclassUID, const char* instanceUID, const char* frames) {
  OFCondition result = EC_Normal;

  /* make sure that we don't create two references to the same image */
  if (findImageReference(instanceUID))
...
OFCondition DVPSReferencedImage_PList::addImageReference(
  ...
  if ((sopclassUID == NULL) || (instanceUID == NULL) ||
      (applicability == DVPSB_allImages))
...
OFCondition DVPSReferencedImage_PList::getImageReference(size_t idx,
  ...
  while (first != last) {
    if (idx == 0)
      return (*first)->getImageReference(sopclassUID, instanceUID, frames);
    else {
      idx--;
      ++first;
    }
...
void DVPSReferencedImage_PList::removeImageReference(
  ...
  if (size() == 0) {
    ...
    for (size_t i = 0; i < numberOfReferences; i++) {
      ...
      frames.clear();
      if (EC_Normal == allReferences.getImageReference(
                           i, seriesUID, sopclassUID, sopinstanceUID, frames,
                           aetitle, filesetID, filesetUID)) {
        if (frames.size() > 0)
          cframes = frames.c_str();
        else
          cframes = NULL;
  ...
  if (applicability == DVPSB_currentImage)
    removeImageReference(instanceUID);
  else
    removeFrameReference(instanceUID, frame, numberOfFrames);
  return;
}
...
OFBool DVPSReferencedImage_PList::isApplicable(const char* instanceUID,
                                               unsigned long frame) {
  if (size() == 0)
    return OFTrue;  // if no image references exist, the object is valid
                    // "globally".
  DVPSReferencedImage* imageRef = findImageReference(instanceUID);
  if (imageRef)
    return imageRef->appliesToFrame(frame);
  return OFFalse;
}

OFBool DVPSReferencedImage_PList::matchesApplicability(
  ...
  switch (applicability) {
    case DVPSB_currentFrame:
      ...
      if (size() == 1) {
        ...
        if (imageRef)
          return imageRef->appliesOnlyToFrame(frame);
      }
      break;
    case DVPSB_currentImage:
      ...
      if (size() == 1) {
        imageRef = findImageReference(instanceUID);
        if (imageRef)
          return imageRef->appliesToAllFrames();
      }
      break;
