...
OFBool DVPSFilmSession::printSCPCreate(
    ...
    T_DIMSE_Message& rsp, DcmDataset*& rspDataset, const char* peerae,
  ...
  if (result) {
    ...
    if (numberOfCopies.getLength() == 0)
      numberOfCopies.putString(DEFAULT_numberOfCopies);
    else {
      Sint32 numCopies = 0;
      char* numCopiesString = NULL;
      if ((EC_Normal != numberOfCopies.getSint32(numCopies)) ||
          (numCopies < 1)) {
        numberOfCopies.getString(numCopiesString);
        if (numCopiesString == NULL)
          numCopiesString = (char*)"";
        DCMPSTAT_INFO(
            "cannot create Basic Film Session: illegal number of copies: '"
            << numCopiesString << "'");
  ...
  if (result) {
    ...
    if (printPriority.getLength() == 0)
      printPriority.putString(DEFAULT_priority);
    else {
      OFString aString;
      printPriority.getOFString(aString, 0, OFTrue);
      if ((aString != "HIGH") && (aString != "MED") && (aString != "LOW")) {
        DCMPSTAT_INFO(
            "cannot create Basic Film Session: illegal print priority: '"
            << aString.c_str() << "'");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
  ...
  if (result) {
    ...
    if (mediumType.getLength() == 0) {
      ...
      if (!found) {
        DCMPSTAT_INFO("cannot create Basic Film Session: illegal medium type: '"
                      << theMedium.c_str() << "'");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
  ...
  // filmDestination
  if (result) {
    Uint32 numFilmDestination =
        cfg.getTargetPrinterNumberOfFilmDestinations(cfgname);
    ...
    if (filmDestination.getLength() == 0) {
      ...
      if (!found) {
        DCMPSTAT_INFO(
            "cannot create Basic Film Session: illegal film destination: '"
            << theDestination.c_str() << "'");
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      }
    }
  ...
  if (result) {
    ...
    if (ownerID.getLength() == 0) {
      if (peerae)
        ownerID.putString(peerae);
      else
        ownerID.putString(DEFAULT_ownerID);
    }
  }
      ...
      if (rqDataset &&
        ...
        if (seq->card() == 1) {
          ...
          if (referencedPresentationLUTInstanceUID.getLength() > 0) {
            ...
            if (NULL == currentPLUT) {
              DCMPSTAT_INFO(
                  "cannot create Basic Film Session: presentation LUT "
              ...
              if (refClassUID.getLength() > 0) {
                ...
                if (aString != UID_PresentationLUTSOPClass) {
                  DCMPSTAT_INFO(
                      "cannot create Basic Film Session: referenced SOP class "
                      "UID in referenced presentation LUT sequence incorrect:\n"
                ...
                DCMPSTAT_INFO(
                    "cannot create Basic Film Session: no referenced SOP class "
                    "UID in referenced presentation LUT sequence");
                rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
                result = OFFalse;
              }
            }
            ...
            DCMPSTAT_INFO(
                "cannot create Basic Film Session: no referenced SOP instance "
                "UID in referenced presentation LUT sequence");
            rsp.msg.NCreateRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
            result = OFFalse;
          }
          ...
          DCMPSTAT_INFO(
              "cannot create Basic Film Session: referenced presentation LUT "
  ...
  if (result && rqDataset) {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub)) {
      ...
      if (currentTag.getElement() == 0x0000) /* group length */
        ...
        DCMPSTAT_INFO(
            "warning while creating Basic Film Session: memory allocation not "
            "supported:\n"
            << DcmObject::PrintHelper(*stack.top(),
                                      DCMTypes::PF_shortenLongTagValues));
        if (rsp.msg.NCreateRSP.DimseStatus == 0)
          rsp.msg.NCreateRSP.DimseStatus =
              STATUS_N_PRINT_BFS_Warn_MemoryAllocation;
      } else if (currentTag == DCM_Illumination) {
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Session: illumination received:\n"
              << DcmObject::PrintHelper(*stack.top(),
                                        DCMTypes::PF_shortenLongTagValues));
        ...
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot create Basic Film Session: reflected ambient light "
              "received:\n"
              << DcmObject::PrintHelper(*stack.top(),
                                        DCMTypes::PF_shortenLongTagValues));
          rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        ...
        DCMPSTAT_INFO(
            "cannot create Basic Film Session: unsupported attribute "
            "received:\n"
            << DcmObject::PrintHelper(*stack.top(),
                                      DCMTypes::PF_shortenLongTagValues));
        rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
...
OFBool DVPSFilmSession::printSCPSet(
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)numberOfCopies.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      if ((EC_Normal != numberOfCopies.getSint32(numCopies)) ||
        ...
        if (numCopiesString == NULL)
          numCopiesString = (char*)"";
        DCMPSTAT_INFO(
            "cannot update Basic Film Session: illegal number of copies: '"
            << numCopiesString << "'");
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)printPriority.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      if ((aString != "HIGH") && (aString != "MED") && (aString != "LOW")) {
        DCMPSTAT_INFO(
            "cannot update Basic Film Session: illegal print priority: '"
            << aString.c_str() << "'");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      } else {
  ...
  if (result) {
    ...
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)mediumType.getTag(), stack,
      ...
      if (!found) {
        DCMPSTAT_INFO("cannot update Basic Film Session: illegal medium type: '"
                      << theMedium.c_str() << "'");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      } else {
  ...
  if (result) {
    ...
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)filmDestination.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
      ...
      if (!found) {
        DCMPSTAT_INFO(
            "cannot update Basic Film Session: illegal film destination: '"
            << theDestination.c_str() << "'");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
        result = OFFalse;
      } else {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)filmSessionLabel.getTag(),
                                        stack, ESM_fromHere, OFFalse))) {
  ...
  if (result) {
    stack.clear();
    if (rqDataset &&
        (EC_Normal == rqDataset->search((DcmTagKey&)ownerID.getTag(), stack,
                                        ESM_fromHere, OFFalse))) {
      ...
      if (rqDataset &&
        ...
        if (seq->card() == 1) {
          ...
          if (referencedPresentationLUTInstanceUID.getLength() > 0) {
            ...
            if (NULL == currentPLUT) {
              DCMPSTAT_INFO(
                  "cannot update Basic Film Session: presentation LUT "
                  "reference cannot be resolved");
              rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
              result = OFFalse;
            } else {
              ...
              if (refClassUID.getLength() > 0) {
                ...
                if (aString != UID_PresentationLUTSOPClass) {
                  DCMPSTAT_INFO(
                      "cannot update Basic Film Session: referenced SOP class "
                      "UID in referenced presentation LUT sequence incorrect:\n"
                      << DcmObject::PrintHelper(
                             *stack.top(), DCMTypes::PF_shortenLongTagValues));
                  rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
                  ...
                  if ((cfg.getTargetPrinterPresentationLUTMatchRequired(
                          cfgname)) &&
                      (!basicFilmBoxList.matchesPresentationLUT(
                          referencedPresentationLUTAlignment))) {
                    DCMPSTAT_INFO(
                        "cannot update Basic Film Session: referenced "
                        "presentation LUT number of entries does not match "
                        "image bit depth.");
                ...
                DCMPSTAT_INFO(
                    "cannot update Basic Film Session: no referenced SOP class "
                    "UID in referenced presentation LUT sequence");
                rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
                result = OFFalse;
              }
            ...
            DCMPSTAT_INFO(
                "cannot update Basic Film Session: no referenced SOP instance "
                "UID in referenced presentation LUT sequence");
            rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
            result = OFFalse;
          }
          ...
          DCMPSTAT_INFO(
              "cannot update Basic Film Session: referenced presentation LUT "
              "sequence number of items != 1");
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_InvalidAttributeValue;
          result = OFFalse;
        }
  ...
  if (result && rqDataset) {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub)) {
      ...
      if (currentTag.getElement() == 0x0000) /* group length */
        ...
        DCMPSTAT_INFO(
            "warning while updating Basic Film Session: memory allocation not "
            "supported\n"
            << DcmObject::PrintHelper(*stack.top(),
                                      DCMTypes::PF_shortenLongTagValues));
        ...
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot update Basic Film Session: illumination received:\n"
              << DcmObject::PrintHelper(*stack.top(),
                                        DCMTypes::PF_shortenLongTagValues));
        ...
        if (!presentationLUTnegotiated) {
          DCMPSTAT_INFO(
              "cannot update Basic Film Session: reflected ambient light "
              "received:\n"
              << DcmObject::PrintHelper(*stack.top(),
                                        DCMTypes::PF_shortenLongTagValues));
          rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
        ...
        DCMPSTAT_INFO(
            "cannot update Basic Film Session: unsupported attribute "
            "received:\n"
            << DcmObject::PrintHelper(*stack.top(),
                                      DCMTypes::PF_shortenLongTagValues));
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_NoSuchAttribute;
  ...
  if (result && (EC_Normal == writeresult)) {
    ...
    if (overrideFilmBoxPLUTSettings) {
      basicFilmBoxList.overridePresentationLUTSettings(
