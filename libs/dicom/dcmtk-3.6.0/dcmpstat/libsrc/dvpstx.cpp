...
DVPSTextObject::DVPSTextObject()
    : boundingBoxAnnotationUnits(DCM_BoundingBoxAnnotationUnits),
      ...
      boundingBoxTextHorizontalJustification(
          DCM_BoundingBoxTextHorizontalJustification),
...
DVPSTextObject::DVPSTextObject(const DVPSTextObject& copy)
    : boundingBoxAnnotationUnits(copy.boundingBoxAnnotationUnits),
      ...
      boundingBoxTextHorizontalJustification(
          copy.boundingBoxTextHorizontalJustification),
...
OFCondition DVPSTextObject::read(DcmItem& dset) {
  ...
  if (unformattedTextValue.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "unformattedTextValue absent or empty");
  } else if (unformattedTextValue.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "unformattedTextValue VM != 1");
  }
  ...
  if (boundingBoxAnnotationUnits.getVM() > 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "boundingBoxAnnotationUnits VM > 1");
  }
  ...
  if (anchorPointAnnotationUnits.getVM() > 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "anchorPointAnnotationUnits VM > 1");
  }
  ...
  if (boundingBoxTLHC.getVM() == 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "boundingBoxTLHC VM == 1");
  } else if (boundingBoxTLHC.getVM() > 2) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "boundingBoxTLHC VM > 2");
  }
  ...
  if (boundingBoxBRHC.getVM() == 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "boundingBoxBRHC VM == 1");
  } else if (boundingBoxBRHC.getVM() > 2) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "boundingBoxBRHC VM > 2");
  }
  ...
  if (boundingBoxTextHorizontalJustification.getVM() > 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with "
        "boundingBoxTextHorizontalJustification VM > 1");
  }

  if (anchorPoint.getVM() == 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with anchorPoint VM "
        "== 1");
  ...
  if ((boundingBoxAnnotationUnits.getVM() == 0) &&
    ...
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with bounding box "
        "but boundingBoxAnnotationUnits absent or empty");
  }
  ...
  if ((boundingBoxTextHorizontalJustification.getVM() == 0) &&
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with bounding box "
        "but boundingBoxTextHorizontalJustification absent or empty");
  }

  if ((anchorPointAnnotationUnits.getVM() == 0) && (anchorPoint.getVM() > 0)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with anchor point "
        "but anchorPointAnnotationUnits absent or empty");
  }
  ...
  if ((anchorPointVisibility.getVM() == 0) && (anchorPoint.getVM() > 0)) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item with anchor point "
        "but anchorPointVisibility absent or empty");
  }

  if ((boundingBoxTLHC.getVM() == 0) && (anchorPoint.getVM() == 0)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a text object SQ item without bounding "
        "box and anchor point");
  }
...
OFCondition DVPSTextObject::setAnchorPoint(double x, double y,
  ...
  if (result == EC_Normal) {
    if (isVisible)
      result = anchorPointVisibility.putString("Y");
    else
      result = anchorPointVisibility.putString("N");
  }
  if (result == EC_Normal) {
    if (unit == DVPSA_display)
      result = anchorPointAnnotationUnits.putString("DISPLAY");
    else
      result = anchorPointAnnotationUnits.putString("PIXEL");
  }
  return result;
}

OFCondition DVPSTextObject::setBoundingBox(
    double TLHC_x, double TLHC_y, double BRHC_x, double BRHC_y,
    DVPSannotationUnit unit, DVPSTextJustification justification) {
  Float32 aPoint[2];
  ...
  if (result == EC_Normal) {
    if (unit == DVPSA_display)
      result = boundingBoxAnnotationUnits.putString("DISPLAY");
    else
      result = boundingBoxAnnotationUnits.putString("PIXEL");
...
const char* DVPSTextObject::getText() {
  char* c = NULL;
  if (EC_Normal == unformattedTextValue.getString(c))
    return c;
  else
    return NULL;
}
...
DVPSannotationUnit DVPSTextObject::getBoundingBoxAnnotationUnits() {
  ...
  if ((result == EC_Normal) && (aString == "DISPLAY"))
    aresult = DVPSA_display;
  return aresult;
}
...
DVPSTextJustification DVPSTextObject::getBoundingBoxHorizontalJustification() {
  ...
  OFString aString;
  OFCondition result =
      boundingBoxTextHorizontalJustification.getOFString(aString, 0);
  if ((result == EC_Normal) && (aString == "RIGHT"))
    aresult = DVPSX_right;
  if ((result == EC_Normal) && (aString == "CENTER"))
    aresult = DVPSX_center;
  return aresult;
}
