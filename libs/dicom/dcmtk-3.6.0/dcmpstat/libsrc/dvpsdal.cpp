...
DVPSDisplayedArea_PList::DVPSDisplayedArea_PList() : list_() {}

DVPSDisplayedArea_PList::DVPSDisplayedArea_PList(
    const DVPSDisplayedArea_PList& arg)
    : list_() {
...
OFCondition DVPSDisplayedArea_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal == dset.search(DCM_DisplayedAreaSelectionSequence, stack,
...
DVPSDisplayedArea* DVPSDisplayedArea_PList::findDisplayedArea(
...
void DVPSDisplayedArea_PList::rotateAndFlip(DVPSRotationType rotationFrom,
  ...
  OFListIterator(DVPSDisplayedArea*) last = list_.end();
  while (first != last) {
    (*first)->rotateAndFlip(rotationFrom, isFlippedFrom, rotationTo,
                            isFlippedTo);
    ++first;
  }
}
