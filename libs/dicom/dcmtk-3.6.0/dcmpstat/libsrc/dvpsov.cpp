...
OFCondition DVPSOverlay::read(DcmItem& dset, Uint8 ovGroup, Uint8 asGroup) {
  ...
  if (overlayRows.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayRows absent or "
        "empty");
  } else if (overlayRows.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayRows VM != 1");
  }

  if (overlayColumns.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayColumns absent or "
        "empty");
  } else if (overlayColumns.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayColumns VM != 1");
  }

  if (overlayType.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayType absent or "
        "empty");
  } else if (overlayType.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayType VM != 1");
  }

  if (overlayOrigin.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayOrigin absent or "
        "empty");
  } else if (overlayOrigin.getVM() != 2) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayOrigin VM != 2");
  }

  if (overlayBitsAllocated.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayBitsAllocated "
        "absent or empty");
  } else if (overlayBitsAllocated.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayBitsAllocated VM "
        "!= 1");
  }
  ...
  if (overlayBitPosition.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayBitPosition absent "
        "or empty");
  } else if (overlayBitPosition.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayBitPosition VM != "
        "1");
  }

  /* in a presentation state object, the overlay data must always be present in
   * this group */
  if (overlayData.getLength() == 0) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains an overlay with overlayData absent or "
        "empty");
  }
...
OFCondition DVPSOverlay::write(DcmItem& dset) {
  ...
  ADD_REPEATING_ELEMENT_TO_DATASET(DcmUnsignedShort, overlayRows,
                                   repeatingGroup)
  ADD_REPEATING_ELEMENT_TO_DATASET(DcmUnsignedShort, overlayColumns,
                                   repeatingGroup)
  ADD_REPEATING_ELEMENT_TO_DATASET(DcmCodeString, overlayType, repeatingGroup)
  ADD_REPEATING_ELEMENT_TO_DATASET(DcmSignedShort, overlayOrigin,
                                   repeatingGroup)
  ADD_REPEATING_ELEMENT_TO_DATASET(DcmUnsignedShort, overlayBitsAllocated,
                                   repeatingGroup)
  ADD_REPEATING_ELEMENT_TO_DATASET(DcmUnsignedShort, overlayBitPosition,
                                   repeatingGroup)
  if (overlayData.getLength() > 0) {
    ADD_REPEATING_ELEMENT_TO_DATASET(DcmOverlayData, overlayData,
                                     repeatingGroup)
  }
  if (overlayDescription.getLength() > 0) {
    ADD_REPEATING_ELEMENT_TO_DATASET(DcmLongString, overlayDescription,
                                     repeatingGroup)
  }
  if (overlayLabel.getLength() > 0) {
    ADD_REPEATING_ELEMENT_TO_DATASET(DcmLongString, overlayLabel,
                                     repeatingGroup)
  }
...
OFBool DVPSOverlay::isSuitableAsShutter(unsigned long x, unsigned long y) {
  ...
  if (result == EC_Normal)
    result = overlayOrigin.getSint16(originY, 1);
  if ((result != EC_Normal) || (originX != 1) || (originY != 1))
    return OFFalse;
  ...
  if (result == EC_Normal)
    result = overlayColumns.getUint16(columns, 0);
...
const char* DVPSOverlay::getOverlayLabel() {
  char* c = NULL;
  if (EC_Normal == overlayLabel.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSOverlay::getOverlayDescription() {
  char* c = NULL;
  if (EC_Normal == overlayDescription.getString(c))
    return c;
  else
    return NULL;
}
...
OFCondition DVPSOverlay::getValues(Sint16& originX, Sint16& originY,
                                   Uint16& sizeX, Uint16& sizeY) {
  OFCondition result = overlayOrigin.getSint16(originX, 1);
  if (result.good())
    result = overlayOrigin.getSint16(originY, 0);
  if (result.good())
    result = overlayColumns.getUint16(sizeX, 0);
  if (result.good())
    result = overlayRows.getUint16(sizeY, 0);
  return result;
}
