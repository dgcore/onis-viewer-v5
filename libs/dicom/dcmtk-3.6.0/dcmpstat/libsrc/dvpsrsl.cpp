...
DVPSReferencedSeries_PList::DVPSReferencedSeries_PList() : list_() {}

DVPSReferencedSeries_PList::DVPSReferencedSeries_PList(
    const DVPSReferencedSeries_PList& arg)
    : list_() {
...
OFCondition DVPSReferencedSeries_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal ==
      dset.search(DCM_ReferencedSeriesSequence, stack, ESM_fromHere, OFFalse)) {
...
void DVPSReferencedSeries_PList::removeImageReference(const char* seriesUID,
  ...
  if (series)
    series->removeImageReference(instanceUID);
  return;
}

DVPSReferencedSeries* DVPSReferencedSeries_PList::findSeriesReference(
...
DVPSReferencedImage* DVPSReferencedSeries_PList::findImageReference(
    const char* seriesUID, const char* instanceUID) {
  DVPSReferencedSeries* series = findSeriesReference(seriesUID);
  if (series)
    return series->findImageReference(instanceUID);
  else
    return NULL;
}
...
OFCondition DVPSReferencedSeries_PList::addImageReference(
    const char* seriesUID, const char* sopclassUID, const char* instanceUID,
    const char* frames, const char* aetitle, const char* filesetID,
    const char* filesetUID)

{
  if ((seriesUID == NULL) || (sopclassUID == NULL) || (instanceUID == NULL))
  ...
  if (checkSOPClass(sopclassUID)) {
    ...
    if (series) {
      ...
      if (EC_Normal == result)
        series->setRetrieveLocation(aetitle, filesetID, filesetUID);
    } else
      result = EC_MemoryExhausted;
  } else
...
OFCondition DVPSReferencedSeries_PList::getImageReference(
  ...
  if (found)
    return (*first)->getImageReference(idx, seriesUID, sopclassUID, instanceUID,
                                       frames, aetitle, filesetID, filesetUID);
  return EC_IllegalCall;
}
