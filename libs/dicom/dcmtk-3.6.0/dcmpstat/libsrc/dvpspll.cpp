...
DVPSPresentationLUT_PList::DVPSPresentationLUT_PList() : list_() {}

DVPSPresentationLUT_PList::DVPSPresentationLUT_PList(
    const DVPSPresentationLUT_PList& arg)
    : list_() {
...
OFCondition DVPSPresentationLUT_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal == dset.search(DCM_RETIRED_PresentationLUTContentSequence,
...
OFCondition DVPSPresentationLUT_PList::write(DcmItem& dset) {
  if (size() == 0)
    return EC_Normal;  // don't write if sequence is empty
...
void DVPSPresentationLUT_PList::cleanup(const char* filmBox,
  ...
  while (first != last) {
    ...
    if (uid && ((aFilmbox == uid) ||
                (imageBoxes.presentationLUTInstanceUIDisUsed(uid))))
      ++first;
    else {
      delete (*first);
      first = list_.erase(first);
...
DVPSPresentationLUT* DVPSPresentationLUT_PList::findPresentationLUT(
...
const char* DVPSPresentationLUT_PList::addPresentationLUT(
    DVPSPresentationLUT* newLUT, OFBool inversePLUT) {
  if (newLUT == NULL)
    return NULL;
  ...
  if (lutType == DVPSP_inverse)
    lutType = DVPSP_identity;

  DVPSPresentationLUT* myLUT = newLUT->clone();
  if (myLUT) {
    // make sure that we don't copy an inverse LUT shape
    if (myLUT->getType() == DVPSP_inverse)
      myLUT->setType(DVPSP_identity);
...
void DVPSPresentationLUT_PList::printSCPDelete(T_DIMSE_Message& rq,
  ...
  while ((first != last) && (!found)) {
    if (theUID == (*first)->getSOPInstanceUID())
      found = OFTrue;
    else
      ++first;
  }
  ...
  if (found) {
    ...
    DCMPSTAT_INFO("cannot delete presentation LUT with instance UID '"
