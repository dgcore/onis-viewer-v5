...
DVPSCurve::DVPSCurve(const DVPSCurve& copy)
  ...
  if (copy.curveData) {
    ...
    if (curveData) {
      OFBitmanipTemplate<double>::copyMem(copy.curveData, curveData,
                                          2 * numberOfPoints);
    }
  }
}
...
OFCondition DVPSCurve::read(DcmItem& dset, Uint8 group) {
  ...
  stack.clear();
  if (EC_Normal == dset.search(key, stack, ESM_fromHere, OFFalse)) {
    return EC_IllegalCall;  // we don't support curves with Curve Data
                            // Descriptor!
  }
  ...
  if (EC_Normal != d_typeOfData->getOFString(aString, 0)) {
    ...
    if (aString == "ROI")
      typeOfData = DVPSL_roiCurve;
    else if (aString == "POLY")
      typeOfData = DVPSL_polylineCurve;
    else
      return EC_IllegalCall;  // unsupported type of data, bail out
  }

  if ((d_curveDescription) &&
      (EC_Normal == d_curveDescription->getOFString(aString, 0))) {
  ...
  switch (dataVR) {
    case 0:  // VR=US. align can be 1 or 2
      if ((align == 1) || (align == sizeof(Uint16))) {
        ...
        if (maxidx * sizeof(Uint16) <= byteLength) {
          for (i = 0; i < maxidx; i++)
            curveData[i] = (double)(((Uint16*)rawData)[i]);
        } else
          result = EC_IllegalCall;
      } else
        result = EC_IllegalCall;
    ...
    case 1:  // VR=SS. align can be 1 or 2
      if ((align == 1) || (align == sizeof(Sint16))) {
        if (align == 1)
          swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, rawData,
                          byteLength, sizeof(Sint16));
        if (maxidx * sizeof(Sint16) <= byteLength) {
          for (i = 0; i < maxidx; i++)
            curveData[i] = (double)(((Sint16*)rawData)[i]);
        } else
          result = EC_IllegalCall;
      } else
        result = EC_IllegalCall;
    ...
    case 2:  // VR=FL. align can be 1, 2, 4
      if ((align == 1) || (align == sizeof(Uint16)) ||
        ...
        if (align == sizeof(Uint16))
          swapIfNecessary(EBO_LittleEndian, gLocalByteOrder, rawData,
                          byteLength, sizeof(Uint16));
        if ((align == 1) || (align == sizeof(Uint16)))
          swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, rawData,
                          byteLength, sizeof(Float32));
        if (maxidx * sizeof(Float32) <= byteLength) {
          for (i = 0; i < maxidx; i++)
            curveData[i] = (double)(((Float32*)rawData)[i]);
        } else
          result = EC_IllegalCall;
      } else
        result = EC_IllegalCall;
    ...
    case 3:  // VR=FD. align can be 1, 2, 8
      if ((align == 1) || (align == sizeof(Uint16)) ||
        ...
        if ((align == 1) || (align == sizeof(Uint16)))
          swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, rawData,
                          byteLength, sizeof(Float64));
        if (maxidx * sizeof(Float64) <= byteLength) {
          for (i = 0; i < maxidx; i++)
            curveData[i] = (double)(((Float64*)rawData)[i]);
        } else
          result = EC_IllegalCall;
      } else
        result = EC_IllegalCall;
    ...
    case 4:  // VR=SL. align can be 1, 2, 4
      if ((align == 1) || (align == sizeof(Uint16)) ||
        ...
        if ((align == 1) || (align == sizeof(Uint16)))
          swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, rawData,
                          byteLength, sizeof(Sint32));
        if (maxidx * sizeof(Sint32) <= byteLength) {
          for (i = 0; i < maxidx; i++)
            curveData[i] = (double)(((Sint32*)rawData)[i]);
        } else
          result = EC_IllegalCall;
      } else
        result = EC_IllegalCall;
