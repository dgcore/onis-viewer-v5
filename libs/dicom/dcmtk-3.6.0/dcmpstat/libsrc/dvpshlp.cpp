...
OFCondition DVPSHelper::saveFileFormat(const char* filename,
  ...
  return fileformat->saveFile(filename, xfer, EET_ExplicitLength, EGL_recalcGL,
                              EPD_withoutPadding);
}

OFCondition DVPSHelper::putStringValue(DcmItem* item, DcmTagKey tag,
                                       const char* value) {
  ...
  if (item) {
    ...
    if (EC_Normal == result)
      item->insert(elem, OFTrue);
  } else
    result = EC_IllegalCall;
  return result;
}

OFCondition DVPSHelper::putUint16Value(DcmItem* item, DcmTagKey tag,
                                       Uint16 value) {
  OFCondition result = EC_Normal;
  DcmTag localTag(tag);
  if (item) {
  ...
  while (child > 0) {
    ...
    if (child < 0) {
      if ((errno != ECHILD) && (errno != 0)) {
        char buf[256];
        DCMPSTAT_ERROR("wait for child failed: " << OFStandard::strerror(
...
void DVPSHelper::setDefault(OFCondition& result, DcmElement& a_name,
                            const char* a_value) {
  if ((result == EC_Normal) && (a_name.getLength() == 0))
    result = a_name.putString(a_value);
  return;
}
...
OFBool DVPSHelper::haveReferencedUIDItem(DcmSequenceOfItems& seq,
  ...
  for (unsigned long i = 0; i < numItems; i++) {
    ...
    stack.clear();
    if (EC_Normal ==
        item->search(DCM_ReferencedSOPClassUID, stack, ESM_fromHere, OFFalse)) {
      aString.clear();
      refuid = (DcmUniqueIdentifier*)(stack.top());
      if (refuid)
        refuid->getOFString(aString, 0);
      if (aString == uid)
        return OFTrue;
    }
  }
