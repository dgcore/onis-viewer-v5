...
OFCondition DVPSPresentationLUT::read(DcmItem& dset, OFBool withSOPInstance) {
  ...
  if (result == EC_Normal) {
    ...
    if (EC_Normal == dset.search(DCM_PresentationLUTSequence, stack,
      ...
      if (seq->card() == 1) {
        ...
        stack.clear();
        if (EC_Normal ==
            item->search((DcmTagKey&)presentationLUTDescriptor.getTag(), stack,
                         ESM_fromHere, OFFalse)) {
          presentationLUTDescriptor = *((DcmUnsignedShort*)(stack.top()));
        }
        stack.clear();
        if (EC_Normal ==
            item->search((DcmTagKey&)presentationLUTExplanation.getTag(), stack,
                         ESM_fromHere, OFFalse)) {
  ...
  if (presentationLUTShape.getLength() == 0) {
    ...
    if (presentationLUTDescriptor.getLength() == 0) {
      ...
      DCMPSTAT_INFO(
          "presentationLUTShape and presentationLUTDescriptor absent or empty");
    } else if (presentationLUTDescriptor.getVM() != 3) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "presentationLUTDescriptor present but VM != 3 in presentation "
          "state");
    }
    if (presentationLUTData.getLength() == 0) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "presentationLUTShape and presentationLUTData absent or empty");
    }
  } else {
    if (presentationLUTShape.getVM() != 1) {
      ...
      if (aString == "IDENTITY")
        presentationLUT = DVPSP_identity;
      else if (aString == "INVERSE")
        presentationLUT = DVPSP_inverse;
      else if (aString == "LIN OD")
        presentationLUT = DVPSP_lin_od;
      else {
        result = EC_IllegalCall;
  ...
  if (withSOPInstance) {
    if (sOPInstanceUID.getLength() == 0) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "sOPInstanceUID absent in Presentation LUT Content Sequence");
    } else if (sOPInstanceUID.getVM() != 1) {
      result = EC_IllegalCall;
      DCMPSTAT_INFO(
          "sOPInstanceUID VM != 1 in Presentation LUT Content Sequence");
    }
  }
...
OFCondition DVPSPresentationLUT::write(DcmItem& dset, OFBool withSOPInstance) {
  ...
  if (presentationLUT == DVPSP_table) {
    ...
    if (presentationLUT == DVPSP_inverse)
      presentationLUTShape.putString("INVERSE");
    else if (presentationLUT == DVPSP_lin_od)
      presentationLUTShape.putString("LIN OD");
    else
      presentationLUTShape.putString("IDENTITY");
...
OFBool DVPSPresentationLUT::haveTable() {
  if ((presentationLUTDescriptor.getVM() == 3) &&
      (presentationLUTData.getLength() > 0))
    return OFTrue;
  else
    return OFFalse;
}
...
const char* DVPSPresentationLUT::getCurrentExplanation() {
  ...
  switch (presentationLUT) {
    ...
    case DVPSP_table:
      ...
      if (value == NULL)
        value = "Unnamed Presentation LUT";
      break;
  }
  return value;
}
...
const char* DVPSPresentationLUT::getLUTExplanation() {
  char* value = NULL;
  if (EC_Normal != presentationLUTExplanation.getString(value))
    return NULL;
  return value;
}
...
OFCondition DVPSPresentationLUT::setType(DVPSPresentationLUTType newType) {
  if ((newType == DVPSP_table) && (!haveTable()))
    return EC_IllegalCall;
  ...
  switch (presentationLUT) {
    case DVPSP_table:
      ...
      if ((EC_Normal == presentationLUTDescriptor.getUint16(numEntries, 0)) &&
        ...
        if ((firstMapped == 0) && ((is12bit && (numEntries == 4096)) ||
                                   ((!is12bit) && (numEntries == 256))))
          result = OFTrue;
      }
      break;
    case DVPSP_inverse:
...
DVPSPrintPresentationLUTAlignment DVPSPresentationLUT::getAlignment() {
  if (presentationLUT == DVPSP_table) {
    ...
    if (EC_Normal != presentationLUTDescriptor.getUint16(numberOfEntries, 0))
      numberOfEntries = 0;
    if (EC_Normal != presentationLUTDescriptor.getUint16(firstEntryMapped, 1))
      firstEntryMapped = 0xFFFF;
    if ((numberOfEntries == 256) && (firstEntryMapped == 0))
      return DVPSK_table8;
    if ((numberOfEntries == 4096) && (firstEntryMapped == 0))
      return DVPSK_table12;
    return DVPSK_other;
  }
  return DVPSK_shape;
}
...
OFBool DVPSPresentationLUT::printSCPCreate(DcmDataset* rqDataset,
  ...
  if (EC_Normal !=
      setSOPInstanceUID(rsp.msg.NCreateRSP.AffectedSOPInstanceUID)) {
  ...
  if (result && rqDataset) {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub)) {
      ...
      if (currentTag.getElement() == 0x0000) /* group length */
        ...
        DCMPSTAT_INFO(
            "cannot create Presentation LUT: unsupported attribute received:"
            << OFendl
  ...
  if (result && matchRequired) {
    ...
    if (!matches) {
      DCMPSTAT_INFO(
          "cannot create Presentation LUT: Mismatch between LUT entries and "
          "image pixel depth.");
      rsp.msg.NCreateRSP.DimseStatus = STATUS_N_NoSuchAttribute;
      result = OFFalse;
    }
 ...
 *  Moved all methods of class DVPSPresentationLUT that depend on module
 * dcmimgle into a separate implementation file
 *
