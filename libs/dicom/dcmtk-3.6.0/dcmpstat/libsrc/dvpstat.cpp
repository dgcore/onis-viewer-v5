...
void DVPresentationState::detachImage() {
  ...
  if (currentImageOwned) {
    if (currentImageFileformat)
      delete currentImageFileformat;
    else if (currentImageDataset)
      delete currentImageDataset;
  }
  currentImage = NULL;
  currentImageFileformat = NULL;
  currentImageDataset = NULL;
  ...
  if (result == EC_Normal) {
    ...
    if (dseq) {
      ...
      if (ditem) {
        ...
        if (delem) {
          result =
              delem->putString(UID_GrayscaleSoftcopyPresentationStateStorage);
          ditem->insert(delem, OFTrue /*replaceOld*/);
        } else
          result = EC_MemoryExhausted;
        if (EC_Normal == result)
...
unsigned long DVPresentationState::getPrintBitmapSize() {
  ...
  if (getPrintBitmapWidthHeight(width, height) == EC_Normal)
    result =
        width * height *
        2;  // print bitmap: 12 bit stored, 16 bit allocated (2 bytes per pixel)
  return result;
}

OFCondition DVPresentationState::setMinimumPrintBitmapWidthHeight(
    unsigned long width, unsigned long height) {
  OFCondition result = EC_IllegalCall;
  const unsigned long max = (width > height) ? width : height;
  if (((maximumPrintBitmapWidth == 0) ||
       (maximumPrintBitmapWidth >= 2 * max)) &&
      ((maximumPrintBitmapHeight == 0) ||
       (maximumPrintBitmapHeight >= 2 * max))) {
...
OFCondition DVPresentationState::setMaximumPrintBitmapWidthHeight(
    unsigned long width, unsigned long height) {
  OFCondition result = EC_IllegalCall;
  const unsigned long min = (width < height) ? width : height;
  if (((minimumPrintBitmapWidth == 0) ||
       (min >= 2 * minimumPrintBitmapWidth)) &&
      ((minimumPrintBitmapHeight == 0) ||
       (min >= 2 * minimumPrintBitmapHeight))) {
...
OFCondition DVPresentationState::getPrintBitmapWidthHeight(
    unsigned long& width, unsigned long& height) {
  OFCondition result = EC_Normal;
  if (currentImage) {
    ...
    if ((width > 0) && (height > 0)) {
      ...
      if ((minimumPrintBitmapWidth > 0) && (width < minimumPrintBitmapWidth)) {
        if ((minimumPrintBitmapHeight > 0) &&
            (height < minimumPrintBitmapHeight)) {
          const unsigned long xfactor =
              (unsigned long)((double)(minimumPrintBitmapWidth - 1) /
                              (double)width) +
              1;
          const unsigned long yfactor =
              (unsigned long)((double)(minimumPrintBitmapHeight - 1) /
                              (double)height) +
              1;
          if (xfactor > yfactor) {
            width *= xfactor;
      ...
      } else if ((minimumPrintBitmapHeight > 0) &&
                 (height < minimumPrintBitmapHeight)) {
        const unsigned long factor =
            (unsigned long)((double)(minimumPrintBitmapHeight - 1) /
                            (double)height) +
      ...
      if ((maximumPrintBitmapWidth > 0) && (width > maximumPrintBitmapWidth)) {
        if ((maximumPrintBitmapHeight > 0) &&
            (height > maximumPrintBitmapHeight)) {
          const unsigned long xdivisor =
              (unsigned long)((double)(width - 1) /
                              (double)maximumPrintBitmapWidth) +
              1;
          const unsigned long ydivisor =
              (unsigned long)((double)(height - 1) /
                              (double)maximumPrintBitmapHeight) +
              1;
      ...
      } else if ((maximumPrintBitmapHeight > 0) &&
                 (height > maximumPrintBitmapHeight)) {
        const unsigned long divisor =
            (unsigned long)((double)(height - 1) /
                            (double)maximumPrintBitmapHeight) +
...
double DVPresentationState::getPrintBitmapPixelAspectRatio() {
  ...
  if (result == 1.0)
    return result;  // handle most frequent case quickly
  if (result == 0.0)
    result = 1.0;  // should never happen
        ...
        if (presentationLUT.getType() == DVPSP_table) {
          if (inversePLUT) {
            if (currentImageMonochrome1)
              currentImage->setPolarity(EPP_Reverse);
            presentationLUT.activateInverseLUT(currentImage);
          } else {
            // we never render a presentation LUT into the print bitmap at this
            // stage.
            if (currentImageMonochrome1)
              currentImage->setPresentationLutShape(ESP_Inverse);
            else
              currentImage->setPresentationLutShape(ESP_Identity);
          }
          // make sure the presentation LUT is re-activated for on-screen
          // display
          currentImagePLUTValid = OFFalse;
        }

        /* clip to displayed area if necessary */
        if ((renderedImageLeft != 1) ||
            (renderedImageRight != (signed long)renderedImageWidth) ||
            (renderedImageTop != 1) ||
            (renderedImageBottom != (signed long)renderedImageHeight)) {
          DicomImage* img = currentImage->createMonoOutputImage(
              currentImageSelectedFrame - 1, 12 /*bits*/);
          if (img == NULL)
            img = currentImage;  // fall-back solution
          image = img->createClippedImage(
              renderedImageLeft - 1, renderedImageTop - 1,
              renderedImageRight - renderedImageLeft + 1,
              renderedImageBottom - renderedImageTop + 1,
              getShutterPresentationValue());
          if (img != currentImage)
            delete img;
        }
        /* scale up to minimum size or down to maximum size if necessary */
        if (((signed long)width !=
             renderedImageRight - renderedImageLeft + 1) ||
            ((signed long)height !=
             renderedImageBottom - renderedImageTop + 1)) {
          DicomImage* img = image;
          image = img->createScaledImage(width, height, 0 /*no interpolation*/,
                                         0 /*ignore aspect ratio*/);
          if (img != currentImage)
            delete img;
        }
        if (image != NULL) {
          if (image->getOutputData(bitmap, size, 12 /*bits*/,
                                   currentImageSelectedFrame - 1))
            result = EC_Normal;
        }
...
OFCondition DVPresentationState::createPreviewImage(unsigned long maxWidth,
  ...
  if ((currentImage != NULL) && (maxWidth > 0) && (maxHeight > 0)) {
    ...
    if ((double)renderedImageWidth / (double)maxWidth * ratio <
        (double)renderedImageHeight / (double)maxHeight)
      width = 0;
    else
      height = 0;
    if (clipMode) {
    ...
    previewImage = currentImage->createScaledImage(
        width, height, 1 /*interpolate*/, 1 /*aspect ratio*/);
...
OFCondition DVPresentationState::getPreviewImageWidthHeight(
    unsigned long& width, unsigned long& height) {
  OFCondition result = EC_IllegalCall;
  if (previewImage != NULL) {
...
OFCondition DVPresentationState::getPreviewImageBitmap(void* bitmap,
  ...
  if ((previewImage != NULL) && (bitmap != NULL) && (size > 0)) {
    renderPixelData();
    if (previewImage->getOutputData(bitmap, size, 8 /*bits*/,
                                    currentImageSelectedFrame - 1))
      result = EC_Normal;
  }
  return result;
...
OFCondition DVPresentationState::attachImage(DcmDataset* dataset,
  ...
  if (useModalityRescale) {
    ...
    image = new DicomImage(dataset, dataset->getOriginalXfer(), slope,
                           intercept, CIF_UsePresentationState);
  } else if (useModalityLUT) {
    image = new DicomImage(dataset, dataset->getOriginalXfer(), modalityLUTData,
                           modalityLUTDescriptor, &modalityLUTExplanation,
                           CIF_UsePresentationState);
  } else {
  ...
  if (image) {
    if (EIS_Normal == image->getStatus()) {
      ...
      // get Modality
      if (EC_Normal ==
          dataset->search(DCM_Modality, stack, ESM_fromHere, OFFalse)) {
      ...
      if (EC_Normal == dataset->search(DCM_PhotometricInterpretation, stack,
        ...
        DcmCodeString* photometricInterpretation =
            (DcmCodeString*)(stack.top());
        if (photometricInterpretation->getVM() == 1) {
          aString.clear();
          photometricInterpretation->getOFString(aString, 0);
          if ((aString == "MONOCHROME1") || (aString == "MONOCHROME 1"))
            currentImageMonochrome1 = OFTrue;
        }
      }
      stack.clear();
      ...
      if ((EC_Normal == result) &&
        ...
        result = ((DcmUniqueIdentifier*)(stack.top()))
                     ->getString(currentImageSOPClassUID);
      }
      stack.clear();
      if ((EC_Normal == result) &&
          (EC_Normal ==
        ...
        result = ((DcmUniqueIdentifier*)(stack.top()))
                     ->getString(currentImageSOPInstanceUID);
      }
      if (EC_Normal == result)
        result = currentImageCurveList.read(*dataset);
      if (EC_Normal == result)
        result = currentImageVOILUTList.read(*dataset);
      if (EC_Normal == result)
        result = currentImageVOIWindowList.read(*dataset);
...
OFCondition DVPresentationState::attachImage(DcmFileFormat* fileformat,
  ...
  if (EC_Normal == result)
    currentImageFileformat = fileformat;
  return result;
}

OFCondition DVPresentationState::addImageReferenceAttached(
    const char* aetitle, const char* filesetID, const char* filesetUID) {
  if (currentImageDataset)
    return addImageReference(*currentImageDataset, aetitle, filesetID,
                             filesetUID);
  else
    return EC_IllegalCall;
}
...
OFCondition DVPresentationState::removeImageReferenceAttached() {
  if (currentImageDataset)
    return removeImageReference(*currentImageDataset);
  else
    return EC_IllegalCall;
}

OFCondition DVPresentationState::setCurrentPresentationLUT(
...
OFCondition DVPresentationState::setDefaultPresentationLUTShape() {
  OFCondition result = EC_Normal;
  if (currentImageMonochrome1)
    result = presentationLUT.setType(DVPSP_inverse);
  else
...
OFCondition DVPresentationState::setPresentationLookupTable(
  ...
  OFCondition result =
      presentationLUT.setLUT(lutDescriptor, lutData, lutExplanation);
  currentImagePLUTValid = OFFalse;  // PLUT has changed
  OFBool wasInverse = imageInverse;
  imageInverse = presentationLUT.isInverse();

  // keep inverse/normal status as before
  if ((wasInverse && (!imageInverse)) || (imageInverse && (!wasInverse)))
    result = invertImage();
  return result;
}
...
OFCondition DVPresentationState::setPresentationLookupTable(DcmItem& dset) {
  ...
  if (EC_Normal != result)
    presentationLUT.setType(
        DVPSP_identity);  // set to well-defined default in case of error
  currentImagePLUTValid = OFFalse;  // PLUT has changed
  OFBool wasInverse = imageInverse;
  imageInverse = presentationLUT.isInverse();
...
OFCondition DVPresentationState::setRectShutter(Sint32 lv, Sint32 rv, Sint32 uh,
                                                Sint32 lh) {
  OFCondition result = EC_Normal;
  char buf[80];
  ...
  if (EC_Normal == result)
    result = shutterRightVerticalEdge.putString(buf);
  sprintf(buf, "%ld", (long)uh);
  if (EC_Normal == result)
    result = shutterUpperHorizontalEdge.putString(buf);
  sprintf(buf, "%ld", (long)lh);
  if (EC_Normal == result)
    result = shutterLowerHorizontalEdge.putString(buf);
...
OFCondition DVPresentationState::setCircularShutter(Sint32 centerX,
  ...
  if (EC_Normal == result)
    result = radiusOfCircularShutter.putString(buf);
  if ((EC_Normal == result) && (shutterPresentationValue.getLength() == 0))
    result = shutterPresentationValue.putUint16(0, 0);
  if (EC_Normal == result) {
...
OFCondition DVPresentationState::addPolyShutterVertex(Sint32 x, Sint32 y) {
  ...
  OFString aString;
  OFCondition result =
      verticesOfThePolygonalShutter.getOFStringArray(aString, OFTrue);
  if (result == EC_Normal) {
    char buf[80];
...
size_t DVPresentationState::getNumberOfTextObjects(size_t layer) {
  if (!currentImage)
    return 0;
  return graphicAnnotationList.getNumberOfTextObjects(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPInstanceUID,
...
DVPSTextObject* DVPresentationState::getTextObject(size_t layer, size_t idx) {
  if (!currentImage)
    return NULL;
  return graphicAnnotationList.getTextObject(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPInstanceUID,
      currentImageSelectedFrame, idx);
}

DVPSTextObject* DVPresentationState::addTextObject(
    size_t layer, DVPSObjectApplicability applicability) {
  if (!currentImage)
    return NULL;
  return graphicAnnotationList.addTextObject(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPClassUID,
...
OFCondition DVPresentationState::removeTextObject(size_t layer, size_t idx) {
  if (!currentImage)
    return EC_IllegalCall;
  return graphicAnnotationList.removeTextObject(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPInstanceUID,
      currentImageSelectedFrame, idx);
}

OFCondition DVPresentationState::moveTextObject(
  ...
  return graphicAnnotationList.moveTextObject(
      graphicLayerList.getGraphicLayerName(old_layer), currentImageSOPClassUID,
      currentImageSOPInstanceUID, currentImageSelectedFrame, idx, applicability,
...
size_t DVPresentationState::getNumberOfGraphicObjects(size_t layer) {
  if (!currentImage)
    return 0;
  return graphicAnnotationList.getNumberOfGraphicObjects(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPInstanceUID,
      currentImageSelectedFrame);
}
...
DVPSGraphicObject* DVPresentationState::getGraphicObject(size_t layer,
  ...
  if (!currentImage)
    return NULL;
  return graphicAnnotationList.getGraphicObject(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPInstanceUID,
      currentImageSelectedFrame, idx);
}

DVPSGraphicObject* DVPresentationState::addGraphicObject(
    size_t layer, DVPSObjectApplicability applicability) {
  if (!currentImage)
    return NULL;
  return graphicAnnotationList.addGraphicObject(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPClassUID,
      currentImageSOPInstanceUID, currentImageSelectedFrame, applicability);
}
...
OFCondition DVPresentationState::removeGraphicObject(size_t layer, size_t idx) {
  if (!currentImage)
    return EC_IllegalCall;
  return graphicAnnotationList.removeGraphicObject(
      graphicLayerList.getGraphicLayerName(layer), currentImageSOPInstanceUID,
      currentImageSelectedFrame, idx);
}

OFCondition DVPresentationState::moveGraphicObject(
    size_t old_layer, size_t idx, size_t new_layer,
    DVPSObjectApplicability applicability) {
  if (!currentImage)
    return EC_IllegalCall;
...
DVPSCurve* DVPresentationState::getCurve(size_t layer, size_t idx) {
  ...
  if (rgroup == 0)
    ...
    return currentImageCurveList.getCurveGroup(rgroup);
}
...
OFCondition DVPresentationState::addCurve(size_t layer,
  ...
  if ((curve == NULL) || (lname == NULL))
    return EC_IllegalCall;
  return activationLayerList.setActivation(0x5000 + curve->getCurveGroup(),
                                           lname);
}
...
const char* DVPresentationState::getCurrentVOIDescription() {
  DVPSSoftcopyVOI* voi = getCurrentSoftcopyVOI();
  if (voi)
    return voi->getCurrentVOIDescription();
  return NULL;
}
...
OFCondition DVPresentationState::getCurrentWindowWidth(double& w) {
  DVPSSoftcopyVOI* voi = getCurrentSoftcopyVOI();
  if (voi)
    return voi->getCurrentWindowWidth(w);
  return EC_IllegalCall;
}
...
OFCondition DVPresentationState::getCurrentWindowCenter(double& c) {
  DVPSSoftcopyVOI* voi = getCurrentSoftcopyVOI();
  if (voi)
    return voi->getCurrentWindowCenter(c);
  return EC_IllegalCall;
}
...
OFCondition DVPresentationState::setVOILUTFromImage(
    size_t idx, DVPSObjectApplicability applicability) {
  ...
  if (lut) {
    ...
    DVPSSoftcopyVOI* voi = softcopyVOIList.createSoftcopyVOI(
        referencedSeriesList, currentImageSOPClassUID,
        currentImageSOPInstanceUID, currentImageSelectedFrame,
        currentImage->getFrameCount(), applicability);
    if (voi)
      return lut->assign(*voi);
  }
  return EC_IllegalCall;
}

OFCondition DVPresentationState::setVOIWindowFromImage(
    size_t idx, DVPSObjectApplicability applicability) {
  currentImageVOIValid = OFFalse;  // VOI has changed
  DVPSVOIWindow* window = currentImageVOIWindowList.getVOIWindow(idx);
  if (window) {
    return setVOIWindow(window->getWindowCenter(), window->getWindowWidth(),
                        window->getExplanation(), applicability);
  }
  return EC_IllegalCall;
}

OFCondition DVPresentationState::setVOIWindow(
    double wCenter, double wWidth, const char* description,
    DVPSObjectApplicability applicability) {
  if (currentImage == NULL)
    return EC_IllegalCall;
  currentImageVOIValid = OFFalse;  // VOI has changed
  ...
  if (voi)
    return voi->setVOIWindow(wCenter, wWidth, description);
  return EC_IllegalCall;
}

OFCondition DVPresentationState::setVOILUT(
    DcmUnsignedShort& lutDescriptor, DcmUnsignedShort& lutData,
    DcmLongString& lutExplanation, DVPSObjectApplicability applicability) {
  if (lutData.getLength() == 0)
    return EC_IllegalCall;
  if (lutDescriptor.getVM() != 3)
    return EC_IllegalCall;
  if (currentImage == NULL)
    return EC_IllegalCall;
  ...
  DVPSSoftcopyVOI* voi = softcopyVOIList.createSoftcopyVOI(
      ...
      currentImageSelectedFrame, currentImage->getFrameCount(), applicability);
  if (voi)
    return voi->setVOILUT(lutDescriptor, lutData, lutExplanation);
  return EC_IllegalCall;
}
...
OFCondition DVPresentationState::setGammaVOILUT(
    double gammaValue, DVPSObjectApplicability applicability) {
  if (currentImage == NULL)
    return EC_IllegalCall;
  ...
  if (haveActiveVOIWindow())  // use active VOI window to specify the LUT
                              // descriptor
  {
    double ww, wc;
    if ((getCurrentWindowWidth(ww) == EC_Normal) &&
        (getCurrentWindowCenter(wc) == EC_Normal)) {
      if (ww <= 65536) {
  ...
  if ((numberOfEntries > 0) && (numberOfEntries <= 65536) &&
      (((firstMapped >= -32768) && (firstMapped <= 32767)) ||
       ((firstMapped >= 0) && (firstMapped <= 65535)))) {
    ...
    if (data != NULL) {
      ...
      if (status == EC_Normal) {
        ...
        char gammabuf[16];
        OFStandard::ftoa(gammabuf, sizeof(gammabuf), gammaValue,
                         OFStandard::ftoa_format_f, 3, 1);

        sprintf(explanation, "LUT with gamma %s, descriptor %u/%ld/%u",
                gammabuf,
                (numberOfEntries < 65536) ? (Uint16)numberOfEntries : 0,
                firstMapped, numberOfBits);
      ...
      /* set VOI LUT */
      if (status == EC_Normal) {
        if ((lutDescriptor != NULL) && (lutData != NULL) &&
            (lutExplanation != NULL))
          status = setVOILUT(*(DcmUnsignedShort*)lutDescriptor,
                             *(DcmUnsignedShort*)lutData, *lutExplanation,
                             applicability);
      }
...
const char* DVPresentationState::getActiveOverlayLabel(size_t layer,
  ...
  if (internalOverlay)
    return internalOverlay->getOverlayLabel();
  if (currentImage)
    return currentImage->getOverlayLabel(group);
  return NULL;
}
...
const char* DVPresentationState::getActiveOverlayDescription(size_t layer,
  ...
  DVPSOverlay* internalOverlay = overlayList.getOverlayGroup(group);
  if (internalOverlay)
    return internalOverlay->getOverlayDescription();
  if (currentImage)
    return currentImage->getOverlayDescription(group);
  return NULL;
}
...
OFBool DVPresentationState::activeOverlayIsROI(size_t layer, size_t idx) {
  ...
  DVPSOverlay* internalOverlay = overlayList.getOverlayGroup(group);
  if (internalOverlay)
    return internalOverlay->isROI();
  if ((currentImage) &&
      (EMO_RegionOfInterest == currentImage->getOverlayMode(group)))
    return OFTrue;
  return OFFalse;
}
...
size_t DVPresentationState::getNumberOfOverlaysInImage() {
  ...
  for (unsigned int i = 0; i < numOverlays; i++) {
    ...
    if ((group == 0) || (overlayList.haveOverlayGroup(group)))
      result--;
  }
  return result;
}
...
const char* DVPresentationState::getOverlayInImageLabel(size_t idx) {
  Uint16 group =
      getOverlayInImageGroup(idx);  // returns 0 if currentImage==NULL
  if (group == 0)
    return NULL;
  else
    return currentImage->getOverlayLabel(group);
...
size_t DVPresentationState::getOverlayInImageActivationLayer(size_t idx) {
  ...
  if (layerName)
    return graphicLayerList.getGraphicLayerIndex(layerName);
  else
    return DVPS_IDX_NONE;
}
...
OFBool DVPresentationState::overlayInImageIsROI(size_t idx) {
  ...
  if (group == 0)
    return OFFalse;
  if (EMO_RegionOfInterest == currentImage->getOverlayMode(group))
    return OFTrue;
  else
    return OFFalse;
}

OFCondition DVPresentationState::removeOverlayFromPresentationState(
    size_t idx) {
...
Uint16 DVPresentationState::findOverlayGroup(Uint16 currentGroup) {
  ...
  for (i = 0; i < max;
       i++) {  // mark all group numbers used in presentation state
    group = getOverlayInPresentationStateGroup(i);
    if ((group >= 0x6000) && (group <= 0x601F))
      allocated[(Uint16)(group - 0x6000) >> 1] = 2;
  }
  max = getNumberOfOverlaysInImage();
  for (i = 0; i < max; i++) {  // mark all group numbers used in image
    group = getOverlayInImageGroup(i);
    if ((group >= 0x6000) && (group <= 0x601F))
      allocated[(Uint16)(group - 0x6000) >> 1] += 1;
  }
  ...
  if ((currentGroup >= 0x6000) && (currentGroup <= 0x601F)) {
    if (allocated[(Uint16)(group - 0x6000) >> 1] == 2)
      return currentGroup;
  }
  // find a free group
  for (i = 0; i < 16; i++)
...
OFCondition DVPresentationState::changeOverlayGroupInPresentationState(
    size_t idx, Uint16 newGroup) {
  Uint16 group = getOverlayInPresentationStateGroup(idx);
  if (group != 0) {
    if (newGroup == 0)
      newGroup = findOverlayGroup(group);
    if (newGroup == group)
      return EC_Normal;  // shortcut - no change needed
...
OFCondition DVPresentationState::addOverlayToPresentationState(
    DcmItem& overlayIOD, Uint16 groupInItem, Uint16 newGroup) {
  if (newGroup == 0)
    newGroup = findOverlayGroup();
  if (newGroup == 0)
    return EC_IllegalCall;  // no group number available
...
OFBool DVPresentationState::overlayIsSuitableAsBitmapShutter(size_t idx) {
  if (currentImage) {
    ...
    if (overlay) {
      return overlay->isSuitableAsShutter(currentImage->getWidth(),
...
OFCondition DVPresentationState::activateOverlayInImage(size_t layer,
  ...
  if (group == 0)
    return EC_IllegalCall;
  if (activationLayerList.getActivationLayer(group) != NULL)
    return EC_IllegalCall;  // already activated
...
OFCondition DVPresentationState::activateOverlayInPresentationState(
    size_t layer, size_t idx) {
  Uint16 group = getOverlayInPresentationStateGroup(idx);
  if (group == 0)
    return EC_IllegalCall;
  if (activationLayerList.getActivationLayer(group) != NULL)
    return EC_IllegalCall;          // already activated
  if (haveShutter(DVPSU_bitmap)) {  // check if the overlay is used as the
                                    // bitmap shutter
    Uint16 shutterGroup = 0;
    shutterOverlayGroup.getUint16(shutterGroup, 0);
    if (shutterGroup == group)
      return EC_IllegalCall;  // used as bitmap shutter
  }
  const char* layerName = getGraphicLayerName(layer);
  if (layerName == NULL)
    return EC_IllegalCall;
...
OFCondition DVPresentationState::activateOverlayAsBitmapShutter(size_t idx) {
  ...
  if (group == 0)
    return EC_IllegalCall;
  if (activationLayerList.getActivationLayer(group) != NULL)
    return EC_IllegalCall;  // activated as overlay
...
void DVPresentationState::renderPixelData(OFBool display) {
  ...
  if (!currentImageVOIValid) {
    ...
    if (voi) {
      if (voi->haveLUT()) {
        result =
            currentImage->setVoiLut(voi->getLUTData(), voi->getLUTDescriptor());
        if (previewImage != NULL)
  ...
  if (useShutterBitmap) {
    if (EC_Normal != shutterOverlayGroup.getUint16(bitmapShutterGroup, 0))
      bitmapShutterGroup = 0;
    if (EC_Normal != shutterPresentationValue.getUint16(bitmapShutterPValue, 0))
      bitmapShutterPValue = 0;
  }

  if (currentImageOverlaysValid == 1) {
    /* overlays are invalid but no external overlays have been added */
    /* remove all external overlays that are not active as overlay or bitmap
     * shutter */
    for (unsigned int remgroup = 0x6000; remgroup <= 0x601F; remgroup += 2) {
      if ((remgroup != bitmapShutterGroup) &&
          ((!overlayList.haveOverlayGroup(remgroup)) ||
    ...
    if (srot != 0) {
      ...
      if (!result)
        DCMPSTAT_INFO("unable to rotate image by " << srot
                                                   << " degrees, ignoring.");
    }
    currentImageRotation = DVPSR_0_deg;
          ...
          if (activateOverlayHelper(*overlay, *currentImage).bad()) {
            if (!result)
              DCMPSTAT_INFO("unable to set external overlay group 0x"
                            << STD_NAMESPACE hex << ovgroup << STD_NAMESPACE dec
                            << ", ignoring.");
          }
        } else if ((useShutterBitmap) && (ovgroup == bitmapShutterGroup)) {
          // activate bitmap overlay
          if (activateOverlayHelper(*overlay, *currentImage, OFTrue,
                                    bitmapShutterPValue)
                  .bad()) {
            if (!result)
  ...
  if (tlhcX > brhcX) {  // swap 'left' and 'right' if necessary
    Sint32 tmp = tlhcX;
    tlhcX = brhcX;
    brhcX = tmp;
  }
  if (tlhcY > brhcY) {  // swap 'top' and 'bottom' if necessary
    Sint32 tmp = tlhcY;
    tlhcY = brhcY;
    brhcY = tmp;
  }
  ...
  if (pstateFlip) {
    ...
    renderedImageLeft =
        (signed long)renderedImageWidth - renderedImageRight + 1;
  ...
  OFBool flp = OFFalse;
  if ((pstateFlip && !currentImageFlip) || (!pstateFlip && currentImageFlip))
    flp = OFTrue;
  else
    flp = OFFalse;
  ...
  if (rot != 0) {
    ...
    if (!result)
      DCMPSTAT_INFO("unable to rotate image by " << rot
                                                 << " degrees, ignoring.");
  }
...
OFCondition DVPresentationState::getOverlayData(
    size_t layer, size_t idx, const void*& overlayData, unsigned int& width,
    unsigned int& height, unsigned int& left_pos, unsigned int& top_pos,
    OFBool& isROI, Uint16& fore, unsigned int bits) {
  ...
  if (currentImage && ((bits == 8) || (bits == 12))) {
    ...
    Uint16 pvalue = 65535;
    if (graphicLayerList.getGraphicLayerRecommendedDisplayValueGray(
            layer, pvalue) == EC_Normal)
      currentImage->convertPValueToDDL(pvalue, fore, bits);
    if (fore == 0)
      back = (Uint16)DicomImageClass::maxval(bits);
    const void* data = currentImage->getOverlayData(
        (unsigned int)group, left_pos, top_pos, width, height, mode,
        currentImageSelectedFrame - 1, bits, fore, back);
    if (EMO_RegionOfInterest == mode)
      isROI = OFTrue;
    else
      isROI = OFFalse;
    if (data)
...
OFCondition DVPresentationState::getPixelData(void* pixelData,
  ...
  if (currentImage) {
    renderPixelData();
    if (currentImage->getOutputData(pixelData, size, 8,
                                    currentImageSelectedFrame - 1))
      return EC_Normal;
  }
  return EC_IllegalCall;
...
OFCondition DVPresentationState::getImageMinMaxPixelRange(double& minValue,
  ...
  OFCondition result = EC_IllegalCall;
  if (currentImage) {
    if (currentImage->getMinMaxValues(minValue, maxValue, 1))
      result = EC_Normal;
  }
  return result;
}
...
OFCondition DVPresentationState::getImageMinMaxPixelValue(double& minValue,
  ...
  OFCondition result = EC_IllegalCall;
  if (currentImage) {
    if (currentImage->getMinMaxValues(minValue, maxValue, 0))
      result = EC_Normal;
  }
  return result;
}
...
DVPSDisplayedArea* DVPresentationState::getDisplayedAreaSelection() {
  if (currentImage == NULL)
    return NULL;
  DVPSDisplayedArea* area = displayedAreaSelectionList.findDisplayedArea(
      currentImageSOPInstanceUID, currentImageSelectedFrame);
  if (area == NULL) {
    DCMPSTAT_INFO(
        "no displayed area selection item for current image found, creating "
        "default.");
    if ((currentImageDataset) &&
        (EC_Normal == createDefaultDisplayedArea(*currentImageDataset))) {
      area = displayedAreaSelectionList.findDisplayedArea(
          currentImageSOPInstanceUID, currentImageSelectedFrame);
    }
  }
  return area;
...
DVPresentationState::getDisplayedAreaPresentationSizeMode() {
  DVPSDisplayedArea* area = getDisplayedAreaSelection();
  if (area)
    return area->getPresentationSizeMode();
  else
...
double DVPresentationState::getDisplayedAreaPresentationPixelAspectRatio() {
  DVPSDisplayedArea* area = getDisplayedAreaSelection();
  if (area)
    return area->getPresentationPixelAspectRatio();
  else
    return 1.0;
}
...
OFCondition DVPresentationState::getDisplayedAreaPresentationPixelSpacing(
    double& x, double& y) {
  DVPSDisplayedArea* area = getDisplayedAreaSelection();
  if (area)
    return area->getPresentationPixelSpacing(x, y);
  else
    return EC_IllegalCall;
}

OFCondition
DVPresentationState::getDisplayedAreaPresentationPixelMagnificationRatio(
    double& magnification) {
  DVPSDisplayedArea* area = getDisplayedAreaSelection();
  if (area)
    return area->getPresentationPixelMagnificationRatio(magnification);
  else
    return EC_IllegalCall;
}
...
OFBool DVPresentationState::canUseDisplayedAreaTrueSize() {
  DVPSDisplayedArea* area = getDisplayedAreaSelection();
  if (area)
    return area->canUseTrueSize();
  else
    return OFFalse;
}

OFCondition DVPresentationState::setStandardDisplayedArea(
    DVPSPresentationSizeMode sizeMode, Sint32 tlhcX, Sint32 tlhcY, Sint32 brhcX,
    Sint32 brhcY, double magnification, DVPSObjectApplicability applicability) {
  if (currentImage == NULL)
    return EC_IllegalCall;

  // make sure that we have an old displayed area item that "knows" about pixel
  // spacing/aspect ratio, because we will only copy this data into the new
  // item.
  DVPSDisplayedArea* area = getDisplayedAreaSelection();
  ...
  if (area)
    return area->setDisplayedArea(sizeMode, tlhcX, tlhcY, brhcX, brhcY,
                                  magnification);
  return EC_IllegalCall;
}
...
OFCondition DVPresentationState::setImageRelativeDisplayedArea(
  ...
  if (area) {
    ...
    // because in this case standard displayed area and image relative displayed
    // area are the same
    area->rotateAndFlip(rotation, flip, DVPSR_0_deg, OFFalse);

    // set displayed area
    OFCondition result = area->setDisplayedArea(sizeMode, tlhcX, tlhcY, brhcX,
                                                brhcY, magnification);
...
DVPSSoftcopyVOI* DVPresentationState::getCurrentSoftcopyVOI() {
  if (currentImage == NULL)
    return NULL;
  return softcopyVOIList.findSoftcopyVOI(currentImageSOPInstanceUID,
                                         currentImageSelectedFrame);
}

OFCondition DVPresentationState::getPrintBitmapRequestedImageSize(
    OFString& requestedImageSize) {
  requestedImageSize.clear();
  if ((currentImage) &&
      (getDisplayedAreaPresentationSizeMode() == DVPSD_trueSize)) {
    double x = 0.0, y = 0.0;
...
OFCondition DVPresentationState::writePresentationLUTforPrint(DcmItem& dset) {
  ...
  if (currentImageMonochrome1) {
    ...
    if (EC_Normal == result)
      result = presentationLUT.write(dset, OFFalse);
    presentationLUT.invert();
  } else
    result = presentationLUT.write(dset, OFFalse);
  return result;
...
const char* DVPresentationState::getCurrentImageModality() {
  char* c = NULL;
  if (EC_Normal == currentImageModality.getString(c))
    return c;
  else
    return NULL;
}
...
OFCondition DVPresentationState::activateOverlayHelper(DVPSOverlay& ovl,
                                                       DicomImage& image,
                                                       OFBool asShutter,
                                                       Uint16 pvalue) {
  Sint16 originX = 0;
  ...
  if (result.good()) {
    ...
    if (0 == image.addOverlay(group, left_pos, top_pos, columns, rows,
                              ovl.getData(), ovl.getLabel(),
                              ovl.getDescription(), mode))
      result = EC_IllegalCall;
    if ((asShutter) && (EC_Normal == result)) {
      if (0 == image.showOverlay(group, pvalue))
        result = EC_IllegalCall;
    }
  }
  return result;
...
OFCondition DVPresentationState::createFromImage(
    DcmItem& dset, DVPSoverlayActivation overlayActivation,
    DVPSVOIActivation voiActivation, OFBool curveActivation,
    OFBool shutterActivation, OFBool presentationActivation,
    DVPSGraphicLayering layering, const char* aetitle, const char* filesetID,
    const char* filesetUID) {
  OFCondition result = DcmPresentationState::createFromImage(
      dset, overlayActivation, voiActivation, curveActivation,
      shutterActivation, presentationActivation, layering, aetitle, filesetID,
      filesetUID);
  imageInverse = presentationLUT.isInverse();
  return result;
}
 ...
 *  Moved static helper functions to new class DVPSHelper, removed some unused
 * code.
 *
 ...
 *  P-value to DDL is implictly performed and the correct P-value for the
 * related layer is used.
 *
