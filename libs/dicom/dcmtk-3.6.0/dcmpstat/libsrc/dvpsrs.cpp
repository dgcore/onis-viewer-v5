...
OFCondition DVPSReferencedSeries::read(DcmItem& dset) {
  ...
  if (seriesInstanceUID.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a referenced series SQ item with "
        "seriesInstanceUID absent or empty");
  } else if (seriesInstanceUID.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a referenced series SQ item with "
        "seriesInstanceUID VM != 1");
  } else if (retrieveAETitle.getVM() > 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a referenced series SQ item with "
        "retrieveAETitle VM > 1");
  } else if (storageMediaFileSetID.getVM() > 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a referenced series SQ item with "
        "storageMediaFileSetID VM > 1");
  } else if (storageMediaFileSetUID.getVM() > 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a referenced series SQ item with "
        "storageMediaFileSetUID VM > 1");
  }
...
DVPSReferencedImage* DVPSReferencedSeries::findImageReference(
...
OFCondition DVPSReferencedSeries::addImageReference(const char* sopclassUID,
                                                    ...
                                                    const char* frames) {
  return referencedImageList.addImageReference(sopclassUID, instanceUID,
                                               frames);
}

void DVPSReferencedSeries::setSeriesInstanceUID(const char* uid) {
  if (uid)
    seriesInstanceUID.putString(uid);
  else
    seriesInstanceUID.clear();
  return;
}

void DVPSReferencedSeries::setRetrieveLocation(const char* aetitle,
                                               const char* filesetID,
                                               const char* filesetUID) {
  if (aetitle)
    retrieveAETitle.putString(aetitle);
  else
    retrieveAETitle.clear();
  if (filesetID)
    storageMediaFileSetID.putString(filesetID);
  else
    storageMediaFileSetID.clear();
  if (filesetUID)
    storageMediaFileSetUID.putString(filesetUID);
  else
...
const char* DVPSReferencedSeries::getRetrieveAETitle() {
  char* c = NULL;
  if (EC_Normal == retrieveAETitle.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSReferencedSeries::getStorageMediaFileSetID() {
  char* c = NULL;
  if (EC_Normal == storageMediaFileSetID.getString(c))
    return c;
  else
    return NULL;
}
...
const char* DVPSReferencedSeries::getStorageMediaFileSetUID() {
  char* c = NULL;
  if (EC_Normal == storageMediaFileSetUID.getString(c))
    return c;
  else
    return NULL;
}
...
OFCondition DVPSReferencedSeries::getImageReference(
    size_t idx, OFString& seriesUID, OFString& sopclassUID,
    OFString& instanceUID, OFString& frames, OFString& aetitle,
    OFString& filesetID, OFString& filesetUID) {
  OFCondition result = referencedImageList.getImageReference(
      idx, sopclassUID, instanceUID, frames);
  if (EC_Normal == result)
    result = seriesInstanceUID.getOFString(seriesUID,
                                           0);  // must not be empty string
  if (EC_Normal == result) {
    if (retrieveAETitle.getLength() == 0)
      aetitle.clear();
    else
