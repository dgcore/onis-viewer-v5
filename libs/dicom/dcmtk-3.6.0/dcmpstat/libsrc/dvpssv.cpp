  ...
  if (result == EC_Normal) {
    ...
    if (EC_Normal ==
      ...
      if (seq->card() == 1) {
        ...
        DCMPSTAT_INFO(
            "VOI LUT SQ does not have exactly one item in presentation state");
      }
    }
  }
  ...
  if (result == EC_Normal) {
    if (windowCenter.getLength() > 0) {
      ...
      if (windowWidth.getLength() == 0) {
        ...
        DCMPSTAT_INFO(
            "windowCenter present but windowWidth absent or empty in "
            "presentation state");
      } else if (windowWidth.getVM() != 1) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "windowCenter present but windowWidth VM != 1 in presentation "
            "state");
      }
    ...
    if (voiLUTData.getLength() > 0) {
      ...
      if (voiLUTDescriptor.getLength() == 0) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "voiLUTData present but voiLUTDescriptor absent or empty in "
            "presentation state");
      } else if (voiLUTDescriptor.getVM() != 3) {
        result = EC_IllegalCall;
        DCMPSTAT_INFO(
            "voiLUTData present but voiLUTDescriptor VM != 3 in presentation "
            "state");
      }
    } else if (useLUT) {
      result = EC_IllegalCall;
...
OFCondition DVPSSoftcopyVOI::write(DcmItem& dset) {
  ...
  if (useLUT) {
    ...
    if (windowCenterWidthExplanation.getLength() > 0) {
      ADD_TO_DATASET(DcmLongString, windowCenterWidthExplanation)
    }
  }
...
const char* DVPSSoftcopyVOI::getCurrentVOIDescription() {
  char* c = NULL;
  if (useLUT) {
    if (EC_Normal == voiLUTExplanation.getString(c))
      return c;
  } else {
    if (EC_Normal == windowCenterWidthExplanation.getString(c))
      return c;
  }
  return NULL;
}
...
OFCondition DVPSSoftcopyVOI::setVOIWindow(double wCenter, double wWidth,
  ...
  if (EC_Normal == result)
    result = ww.putString(buf);
  if ((EC_Normal == result) && (description))
    result = expl.putString(description);
  if (EC_Normal == result) {
