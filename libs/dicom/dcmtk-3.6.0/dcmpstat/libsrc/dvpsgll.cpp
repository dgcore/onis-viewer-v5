...
DVPSGraphicLayer_PList::DVPSGraphicLayer_PList() : list_() {}

DVPSGraphicLayer_PList::DVPSGraphicLayer_PList(
    const DVPSGraphicLayer_PList& arg)
    : list_() {
...
OFCondition DVPSGraphicLayer_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal ==
      dset.search(DCM_GraphicLayerSequence, stack, ESM_fromHere, OFFalse)) {
...
OFCondition DVPSGraphicLayer_PList::write(DcmItem& dset) {
  if (size() == 0)
    return EC_Normal;  // don't write empty Sequence
...
OFCondition DVPSGraphicLayer_PList::addGraphicLayer(
    const char* gLayer, const Sint32 gLayerOrder,
  ...
  if (newLayer) {
    ...
    if (gLayerDescription)
      newLayer->setGLDescription(gLayerDescription);
    list_.push_back(newLayer);
  } else
...
const char* DVPSGraphicLayer_PList::getGraphicLayerDescription(size_t idx) {
  DVPSGraphicLayer* layer = getGraphicLayer(idx);
  if (layer)
    return layer->getGLDescription();
  else
    return NULL;
}

OFBool DVPSGraphicLayer_PList::haveGraphicLayerRecommendedDisplayValue(
    size_t idx) {
  DVPSGraphicLayer* layer = getGraphicLayer(idx);
  if (layer)
    return layer->haveGLRecommendedDisplayValue();
  else
    return OFFalse;
}

OFCondition DVPSGraphicLayer_PList::getGraphicLayerRecommendedDisplayValueGray(
    size_t idx, Uint16& gray) {
  DVPSGraphicLayer* layer = getGraphicLayer(idx);
  if (layer)
    return layer->getGLRecommendedDisplayValueGray(gray);
  else
    return EC_IllegalCall;
}

OFCondition DVPSGraphicLayer_PList::getGraphicLayerRecommendedDisplayValueRGB(
    size_t idx, Uint16& r, Uint16& g, Uint16& b) {
  DVPSGraphicLayer* layer = getGraphicLayer(idx);
  if (layer)
    return layer->getGLRecommendedDisplayValueRGB(r, g, b);
  else
    return EC_IllegalCall;
}

OFCondition DVPSGraphicLayer_PList::setGraphicLayerRecommendedDisplayValueGray(
    size_t idx, Uint16 gray) {
  DVPSGraphicLayer* layer = getGraphicLayer(idx);
  if (layer) {
...
OFCondition DVPSGraphicLayer_PList::setGraphicLayerRecommendedDisplayValueRGB(
    size_t idx, Uint16 r, Uint16 g, Uint16 b) {
  DVPSGraphicLayer* layer = getGraphicLayer(idx);
  if (layer) {
...
void DVPSGraphicLayer_PList::removeGraphicLayerRecommendedDisplayValue(
    size_t idx, OFBool rgb, OFBool monochrome) {
  DVPSGraphicLayer* layer = getGraphicLayer(idx);
  if (layer)
    layer->removeRecommendedDisplayValue(rgb, monochrome);
  return;
}
...
OFCondition DVPSGraphicLayer_PList::setGraphicLayerDescription(
...
void DVPSGraphicLayer_PList::cleanupLayers(
  ...
  while (first != last) {
    name = (*first)->getGL();
    if ((!activations.usesLayerName(name)) &&
        (!annotations.usesLayerName(name))) {
      delete (*first);
      first = list_.erase(first);
    } else
      ++first;
