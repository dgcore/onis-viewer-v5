...
DVPSOverlayCurveActivationLayer::DVPSOverlayCurveActivationLayer()
    : repeatingGroup(0),
      activationLayer(
          DCM_OverlayActivationLayer)  // default is Overlay not Curve
{}

DVPSOverlayCurveActivationLayer::DVPSOverlayCurveActivationLayer(
    const DVPSOverlayCurveActivationLayer& copy)
...
OFCondition DVPSOverlayCurveActivationLayer::read(DcmItem& dset,
  ...
  if (activationLayer.getVM() > 1) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a curve or overlay activation layer with "
        "VM > 1");
  }
...
OFCondition DVPSOverlayCurveActivationLayer::write(DcmItem& dset) {
  ...
  ADD_REPEATING_ELEMENT_TO_DATASET(DcmCodeString, activationLayer,
                                   repeatingGroup)
...
void DVPSOverlayCurveActivationLayer::setActivationLayer(const char* aLayer) {
  if (aLayer)
    activationLayer.putString(aLayer);
  else
    activationLayer.clear();
  return;
}
...
const char* DVPSOverlayCurveActivationLayer::getActivationLayer() {
  char* c = NULL;
  if (EC_Normal == activationLayer.getString(c))
    return c;
  else
    return NULL;
}
