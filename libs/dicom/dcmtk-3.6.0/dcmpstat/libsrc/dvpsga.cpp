...
OFCondition DVPSGraphicAnnotation::read(DcmItem& dset) {
  ...
  if (result == EC_Normal)
    result = textObjectList.read(dset);
  if (result == EC_Normal)
    result = graphicObjectList.read(dset);
  if (result == EC_Normal)
  ...
  if (graphicAnnotationLayer.getLength() == 0) {
    ...
    DCMPSTAT_INFO(
        "presentation state contains a graphic annotation SQ item with "
        "graphicAnnotationLayer absent or empty");
  } else if (graphicAnnotationLayer.getVM() != 1) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic annotation SQ item with "
        "graphicAnnotationLayer VM != 1");
  }

  if ((textObjectList.size() == 0) && (graphicObjectList.size() == 0)) {
    result = EC_IllegalCall;
    DCMPSTAT_INFO(
        "presentation state contains a graphic annotation SQ item without any "
...
OFCondition DVPSGraphicAnnotation::write(DcmItem& dset) {
  ...
  if (result == EC_Normal)
    result = textObjectList.write(dset);
  if (result == EC_Normal)
    result = graphicObjectList.write(dset);
...
void DVPSGraphicAnnotation::setAnnotationLayer(const char* aLayer) {
  if (aLayer)
    graphicAnnotationLayer.putString(aLayer);
  else
    graphicAnnotationLayer.clear();
  return;
}

const char* DVPSGraphicAnnotation::getAnnotationLayer() {
  char* c = NULL;
  if (EC_Normal == graphicAnnotationLayer.getString(c))
    return c;
  else
    return NULL;
}
...
OFBool DVPSGraphicAnnotation::isApplicable(
  ...
  if (referencedImageList.size() == 0)
    return OFTrue;  // if no image references exist, the annotation is valid
                    // "globally".
  ...
  switch (applicability) {
    case DVPSB_currentFrame:  // check if annotation applies to current frame
      ...
      break;
    case DVPSB_currentImage:  // check if annotation applies to all frames of
                              // current image
      imageRef = referencedImageList.findImageReference(instanceUID);
      if (imageRef)
