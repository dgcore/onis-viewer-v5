...
DVPSStoredPrint_PList::DVPSStoredPrint_PList() : list_() {}
...
void DVPSStoredPrint_PList::printSCPBasicFilmBoxSet(
    DVConfiguration& cfg, const char* cfgname, T_DIMSE_Message& rq,
    DcmDataset* rqDataset, T_DIMSE_Message& rsp, DcmDataset*& rspDataset,
  ...
  while ((first != last) && (!found)) {
    if ((*first)->isFilmBoxInstance(rq.msg.NSetRQ.RequestedSOPInstanceUID))
      found = OFTrue;
    else
      ++first;
  }
  ...
  if (found) {
    ...
    if (newSP) {
      if (newSP->printSCPSet(cfg, cfgname, rqDataset, rsp, rspDataset,
...
void DVPSStoredPrint_PList::printSCPBasicGrayscaleImageBoxSet(
    DVInterface& cfg, const char* cfgname, T_DIMSE_Message& rq,
    DcmDataset* rqDataset, T_DIMSE_Message& rsp, DcmDataset*& rspDataset,
    OFBool presentationLUTnegotiated) {
  OFListIterator(DVPSStoredPrint*) first = list_.begin();
  OFListIterator(DVPSStoredPrint*) last = list_.end();
  ...
  if (newib && sp) {
    ...
    if (newib->printSCPSet(cfg, cfgname, rqDataset, rsp, rspDataset,
                           *imageDataset,
                           sp->getReferencedPresentationLUTAlignment(),
                           presentationLUTnegotiated)) {
      if (EC_Normal == sp->writeHardcopyImageAttributes(*imageDataset)) {
        // check for image position clash
        if (sp->haveImagePositionClash(rq.msg.NSetRQ.RequestedSOPInstanceUID,
                                       newib->getImageBoxPosition())) {
          delete rspDataset;
          rspDataset = NULL;
          DCMPSTAT_INFO(
              "cannot update basic grayscale image box, image position "
              "collision.");
        ...
        rspDataset = NULL;
        DCMPSTAT_INFO(
            "cannot update basic grayscale image box, out of memory.");
        rsp.msg.NSetRSP.DimseStatus = STATUS_N_ProcessingFailure;
      }
    }
  } else {
...
void DVPSStoredPrint_PList::printSCPBasicFilmBoxAction(
  ...
  OFBool found = OFFalse;
  while ((first != last) && (!found)) {
    if ((*first)->isFilmBoxInstance(rq.msg.NActionRQ.RequestedSOPInstanceUID))
      found = OFTrue;
    else
      ++first;
  }
  if (found) {
    ...
    OFBool writeRequestedImageSize =
        cfg.getTargetPrinterSupportsRequestedImageSize(cfgname);
    ...
    if ((*first)->emptyPageWarning()) {
      ...
      if (STATUS_Success == rsp.msg.NActionRSP.DimseStatus)
        rsp.msg.NActionRSP.DimseStatus = STATUS_N_PRINT_BFB_Warn_EmptyPage;
    } else {
      if (EC_Normal == sp->write(*spDataset, writeRequestedImageSize, OFFalse,
                                 OFFalse, OFTrue)) {
...
void DVPSStoredPrint_PList::printSCPBasicFilmSessionAction(
    DVInterface& cfg, const char* cfgname, T_DIMSE_Message& rsp,
    DVPSPresentationLUT_PList& globalPresentationLUTList) {
  if (size() > 0) {
    OFBool writeRequestedImageSize =
        cfg.getTargetPrinterSupportsRequestedImageSize(cfgname);
    ...
    while (first != last) {
      ...
      if ((*first)->emptyPageWarning()) {
        DCMPSTAT_INFO("empty page, will not be stored in database");
        if (STATUS_Success == rsp.msg.NActionRSP.DimseStatus)
          rsp.msg.NActionRSP.DimseStatus = STATUS_N_PRINT_BFS_Warn_EmptyPage;
      } else {
        if (EC_Normal == (*first)->write(*spDataset, writeRequestedImageSize,
                                         OFFalse, OFFalse, OFTrue)) {
          if (EC_Normal == cfg.saveFileFormatToDB(spFile)) {
...
void DVPSStoredPrint_PList::printSCPBasicFilmBoxDelete(T_DIMSE_Message& rq,
  ...
  OFBool found = OFFalse;
  while ((first != last) && (!found)) {
    if ((*first)->isFilmBoxInstance(rq.msg.NDeleteRQ.RequestedSOPInstanceUID))
      found = OFTrue;
    else
      ++first;
  }
  ...
  if (found) {
    ...
    // film box does not exist or wrong instance UID
    DCMPSTAT_INFO("cannot delete film box with instance UID '"
...
OFBool DVPSStoredPrint_PList::haveFilmBoxInstance(const char* uid) {
  ...
  OFListIterator(DVPSStoredPrint*) last = list_.end();
  while (first != last) {
    if ((*first)->isFilmBoxInstance(uid))
      return OFTrue;
    else
      ++first;
  }
...
OFBool DVPSStoredPrint_PList::usesPresentationLUT(const char* uid) {
  ...
  OFListIterator(DVPSStoredPrint*) last = list_.end();
  while (first != last) {
    if ((*first)->usesPresentationLUT(uid))
      return OFTrue;
    else
      ++first;
  }
...
OFBool DVPSStoredPrint_PList::matchesPresentationLUT(
...
void DVPSStoredPrint_PList::overridePresentationLUTSettings(
  ...
  OFListIterator(DVPSStoredPrint*) last = list_.end();
  while (first != last) {
    (*first)->overridePresentationLUTSettings(newIllumination,
                                              newReflectedAmbientLight,
                                              newReferencedPLUT, newAlignment);
    ++first;
  }
}
