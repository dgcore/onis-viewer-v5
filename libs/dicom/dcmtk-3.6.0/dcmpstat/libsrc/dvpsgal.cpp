...
DVPSGraphicAnnotation_PList::DVPSGraphicAnnotation_PList() : list_() {}

DVPSGraphicAnnotation_PList::DVPSGraphicAnnotation_PList(
    const DVPSGraphicAnnotation_PList& arg)
    : list_() {
...
OFCondition DVPSGraphicAnnotation_PList::read(DcmItem& dset) {
  ...
  if (EC_Normal == dset.search(DCM_GraphicAnnotationSequence, stack,
...
size_t DVPSGraphicAnnotation_PList::getNumberOfTextObjects(
  ...
  while (first != last) {
    if ((aString == (*first)->getAnnotationLayer()) &&
        ((*first)->isApplicable(instanceUID, frame))) {
...
DVPSTextObject* DVPSGraphicAnnotation_PList::getTextObject(
    const char* layer, const char* instanceUID, unsigned long frame,
    size_t idx) {
  if (layer == NULL)
    return NULL;
  ...
  while (first != last) {
    if ((aString == (*first)->getAnnotationLayer()) &&
      ...
      if (idx < tmp)
        return (*first)->getTextObject(idx);
      else
        idx -= tmp;
    }
    ++first;
...
DVPSTextObject* DVPSGraphicAnnotation_PList::addTextObject(
    const char* layer, const char* sopclassUID, const char* instanceUID,
  ...
  while (first != last) {
    if ((aString == (*first)->getAnnotationLayer()) &&
        ((*first)->isApplicable(instanceUID, frame, applicability))) {
      annotation = *first;
      first = last;
    } else
      ++first;
  ...
  if (annotation == NULL) {
    ...
    if (annotation) {
      ...
      if (applicability != DVPSB_allImages)
        annotation->addImageReference(sopclassUID, instanceUID, frame,
                                      applicability);
...
OFCondition DVPSGraphicAnnotation_PList::removeTextObject(
    const char* layer, const char* instanceUID, unsigned long frame,
    size_t idx) {
  if (layer == NULL)
...
OFCondition DVPSGraphicAnnotation_PList::moveTextObject(
    const char* old_layer, const char* sopclassUID, const char* instanceUID,
    unsigned long frame, size_t idx, DVPSObjectApplicability applicability,
  ...
  while (first != last) {
    if ((aString == (*first)->getAnnotationLayer()) &&
      ...
      if (idx < tmp) {
        DVPSTextObject* textObject = (*first)->removeTextObject(idx);
        if (textObject) {
          textObject = addTextObject(new_layer, sopclassUID, instanceUID, frame,
                                     applicability, textObject);
          if (textObject)
            return EC_Normal;
          else
...
size_t DVPSGraphicAnnotation_PList::getNumberOfGraphicObjects(
    const char* layer, const char* instanceUID, unsigned long frame) {
  if (layer == NULL)
    return 0;
...
DVPSGraphicObject* DVPSGraphicAnnotation_PList::getGraphicObject(
    const char* layer, const char* instanceUID, unsigned long frame,
    size_t idx) {
  if (layer == NULL)
    return NULL;
  ...
  while (first != last) {
    if ((aString == (*first)->getAnnotationLayer()) &&
      ...
      if (idx < tmp)
        return (*first)->getGraphicObject(idx);
      else
        idx -= tmp;
    }
    ++first;
...
OFCondition DVPSGraphicAnnotation_PList::removeGraphicObject(
    const char* layer, const char* instanceUID, unsigned long frame,
    size_t idx) {
  if (layer == NULL)
    return EC_IllegalCall;
  ...
  while (first != last) {
    if ((aString == (*first)->getAnnotationLayer()) &&
      ...
      if (idx < tmp) {
        DVPSGraphicObject* graphicObject = (*first)->removeGraphicObject(idx);
        if (graphicObject) {
          graphicObject = addGraphicObject(new_layer, sopclassUID, instanceUID,
                                           frame, applicability, graphicObject);
          if (graphicObject)
            return EC_Normal;
          else
            return EC_MemoryExhausted;
