...
public:
  ...
  DVSignatureHandlerSignatureProfile(DcmAttributeTag& at)
      : SiNullProfile(), notToSign(at), vmNotToSign(notToSign.getVM()) {}
  ...
  OFBool tagInList(const DcmTagKey& tag) const {
    DcmAttributeTag tempList(
        notToSign);  // required because getTagVal() is not const
    DcmTagKey tagkey;
    for (unsigned long n = 0; n < vmNotToSign; n++) {
      if ((EC_Normal == tempList.getTagVal(tagkey, n)) && (tag == tagkey))
        return OFTrue;
    }
    return OFFalse;
  }
...
DVSignatureHandler::DVSignatureHandler(DVConfiguration& cfg)
    : htmlSR(
          "<html><head><title>Structured Report</title></head><body>No "
          "structured report is currently active.</body></html>\n"),
      htmlImage(
          "<html><head><title>Image</title></head><body>No image is currently "
          "active.</body></html>\n"),
      htmlPState(
          "<html><head><title>Presentation State</title></head><body>No "
          "presentation state is currently active.</body></html>\n"),
      htmlOverview(),
...
#ifdef WITH_OPENSSL
  int fileFormat =
      config.getTLSPEMFormat() ? X509_FILETYPE_PEM : X509_FILETYPE_ASN1;
  const char* tlsCACertificateFolder = config.getTLSCACertificateFolder();
  if (tlsCACertificateFolder)
    certVerifier.addTrustedCertificateDir(tlsCACertificateFolder, fileFormat);
#endif
...
void DVSignatureHandler::printSignatureItemPosition(DcmStack& stack,
  ...
  if (stack.card() > 2) {
    ...
    for (unsigned long l = stack.card() - 2; l > 0;
         --l)  // loop over all elements except the stack top and bottom
    {
      elem = stack.elem(l);
      if (elem) {
        if ((elem->ident() == EVR_item) && sq) {
          ...
          if (tagname)
            ...
            sprintf(buf, "(%04x,%04x)", elem->getTag().getGroup(),
                    elem->getTag().getElement());
            os << buf;
            printed = OFTrue;
          }
...
void DVSignatureHandler::updateDigitalSignatureInformation(
    DcmItem& dataset, DVPSObjectType objtype, OFBool /* onRead */)
#else
void DVSignatureHandler::updateDigitalSignatureInformation(
    DcmItem& /*dataset*/, DVPSObjectType objtype, OFBool /* onRead */)
#endif
{
  ...
  const char* htmlLine1 =
      "<tr><td width=\"20\" nowrap>&nbsp;</td><td colspan=\"2\" nowrap>";
  const char* htmlLine2 = "<tr><td colspan=\"3\" nowrap>&nbsp;</td><td>";
  const char* htmlLine3 = "<tr><td colspan=\"2\" nowrap>&nbsp;</td><td nowrap>";
  const char* htmlLine4 =
      "<tr><td width=\"20\" nowrap>&nbsp;</td><td width=\"20\" "
      "nowrap>&nbsp;</td><td>";
    ...
    for (l = 0; l < numSignatures; l++) {
      if (EC_Normal == signer.selectSignature(l)) {
        ...
        if (EC_Normal == signer.getCurrentSignatureDateTime(aString))
          ...
          os << "(unknown)";
        os << htmlEndl << htmlLine1 << "Certificate of signer" << htmlNext;
        if ((cert == NULL) || (cert->getKeyType() == EKT_none))
          os << "none" << htmlEndl;
        else {
          os << "X.509v" << cert->getX509Version() << htmlEndl;
          cert->getCertSubjectName(aString);
          os << htmlLine3 << "Subject" << htmlNext << aString.c_str()
             << htmlEndl;
          cert->getCertIssuerName(aString);
          os << htmlLine3 << "Issued by" << htmlNext << aString.c_str()
             << htmlEndl << htmlLine3 << "Serial no." << htmlNext
             << cert->getCertSerialNo() << htmlEndl << htmlLine3 << "Validity"
             << htmlNext << "not before ";
          cert->getCertValidityNotBefore(aString);
          os << aString.c_str() << ", not after ";
          cert->getCertValidityNotAfter(aString);
          os << aString.c_str() << htmlEndl << htmlLine4 << "Public key"
             << htmlNext;
        ...
        if (sicond.good()) {
          ...
          os << htmlVfyCA
             << "<b>Verification: Signature is valid but certificate could not "
                "be verified: "
  ...
  switch (objtype) {
    case DVPSS_structuredReport:
      if (counter == 0)
        os << "The current structured report does not contain any digital "
              "signature."
           << OFendl;
    ...
    case DVPSS_image:
      if (counter == 0)
        os << "The current image does not contain any digital signature."
           << OFendl;
      corruptSignaturesImage = corrupt_counter;
      untrustSignaturesImage = untrustworthy_counter;
      correctSignaturesImage =
          counter - corrupt_counter - untrustworthy_counter;
      break;
    case DVPSS_presentationState:
      if (counter == 0)
        os << "The current presentation state does not contain any digital "
              "signature."
           << OFendl;
...
void DVSignatureHandler::disableDigitalSignatureInformation(
  ...
  switch (objtype) {
    case DVPSS_structuredReport:
      htmlSR =
          "<html><head><title>Structured Report</title></head><body>The "
          "current structured report does not contain any digital "
          "signature.</body></html>\n";
      corruptSignaturesSR = 0;
      untrustSignaturesSR = 0;
      correctSignaturesSR = 0;
    ...
    case DVPSS_image:
      ...
      correctSignaturesImage = 0;
      htmlImage =
          "<html><head><title>Image</title></head><body>The current image does "
          "not contain any digital signature.</body></html>\n";
      break;
    case DVPSS_presentationState:
      corruptSignaturesPState = 0;
      untrustSignaturesPState = 0;
      correctSignaturesPState = 0;
      htmlPState =
          "<html><head><title>Presentation State</title></head><body>The "
          "current presentation state does not contain any digital "
          "signature.</body></html>\n";
      break;
  }
  updateSignatureValidationOverview();
...
DVPSSignatureStatus DVSignatureHandler::getCurrentSignatureStatus(
  ...
  switch (objtype) {
    case DVPSS_structuredReport:
      if ((correctSignaturesSR + corruptSignaturesSR + untrustSignaturesSR) ==
          0)
        return DVPSW_unsigned;
      if ((corruptSignaturesSR + untrustSignaturesSR) == 0)
        return DVPSW_signed_OK;
      if (corruptSignaturesSR == 0)
        return DVPSW_signed_unknownCA;
      break;
    case DVPSS_image:
      if ((correctSignaturesImage + corruptSignaturesImage +
           untrustSignaturesImage) == 0)
        return DVPSW_unsigned;
      if ((corruptSignaturesImage + untrustSignaturesImage) == 0)
        return DVPSW_signed_OK;
      if (corruptSignaturesImage == 0)
        return DVPSW_signed_unknownCA;
      break;
    case DVPSS_presentationState:
      if ((correctSignaturesPState + corruptSignaturesPState +
           untrustSignaturesPState) == 0)
        return DVPSW_unsigned;
      if ((corruptSignaturesPState + untrustSignaturesPState) == 0)
        return DVPSW_signed_OK;
      if (corruptSignaturesPState == 0)
        return DVPSW_signed_unknownCA;
      break;
  }
...
DVPSSignatureStatus DVSignatureHandler::getCombinedImagePStateSignatureStatus()
  ...
  DVPSSignatureStatus statPState =
      getCurrentSignatureStatus(DVPSS_presentationState);
  if ((statImage == DVPSW_signed_corrupt) ||
      (statPState == DVPSW_signed_corrupt))
    return DVPSW_signed_corrupt;
  if ((statImage == DVPSW_signed_unknownCA) ||
      (statPState == DVPSW_signed_unknownCA))
    return DVPSW_signed_unknownCA;
  if ((statImage == DVPSW_signed_OK) && (statPState == DVPSW_signed_OK))
    return DVPSW_signed_OK;
  return DVPSW_unsigned;
}
...
void DVSignatureHandler::updateSignatureValidationOverview() {
  ...
  const char* htmlNext = "</td><td>";
  const char* htmlTableUns =
      "<p><table cellspacing=\"0\" bgcolor=\"#E0E0E0\">\n";
  const char* htmlTableSig =
      "<p><table cellspacing=\"0\" bgcolor=\"#FFD0D0\">\n";
  const char* htmlTableCA =
      "<p><table cellspacing=\"0\" bgcolor=\"#FFF8DC\">\n";
  ...
  os << htmlLine1 << "Number of correct signatures" << htmlNext
     << correctSignaturesSR << htmlEndl;
  os << htmlLine1 << "Number of corrupt signatures" << htmlNext
     << corruptSignaturesSR << htmlEndl;
  os << htmlLine1 << "Number of untrusted signatures" << htmlNext
     << untrustSignaturesSR << htmlEndl;
  switch (status) {
    case DVPSW_unsigned:
    ...
    case DVPSW_signed_unknownCA:
      os << htmlVfyCA
         << "<b>Status: signed but untrustworthy: certificate could not be "
            "verified</b>"
         << htmlEndl;
      break;
    case DVPSW_signed_corrupt:
      os << htmlVfyErr << "<b>Status: contains corrupt signatures</b>"
         << htmlEndl;
      break;
  }
  os << htmlTableE;
  ...
  os << htmlLine1 << "Number of correct signatures" << htmlNext
     << correctSignaturesImage << htmlEndl;
  os << htmlLine1 << "Number of corrupt signatures" << htmlNext
     << corruptSignaturesImage << htmlEndl;
  os << htmlLine1 << "Number of untrusted signatures" << htmlNext
     << untrustSignaturesImage << htmlEndl;
  switch (status) {
    case DVPSW_unsigned:
      os << htmlVfyUns << "<b>Status: unsigned</b>" << htmlEndl;
      break;
  ...
  os << htmlLine1 << "Number of correct signatures" << htmlNext
     << correctSignaturesPState << htmlEndl;
  os << htmlLine1 << "Number of corrupt signatures" << htmlNext
     << corruptSignaturesPState << htmlEndl;
  os << htmlLine1 << "Number of untrusted signatures" << htmlNext
     << untrustSignaturesPState << htmlEndl;
  switch (status) {
    case DVPSW_unsigned:
      os << htmlVfyUns << "<b>Status: unsigned</b>" << htmlEndl;
      break;
  ...
  while (sigItem) {
    if (sigItem == &item) {
      ...
      for (l = 0; l < numSignatures; l++) {
        if (EC_Normal == signer.selectSignature(l)) {
          ...
          if (EC_Normal == signer.getCurrentDataElementsSigned(at)) {
            ...
            for (unsigned long n = 0; n < atVM; n++) {
              if (EC_Normal == at.getTagVal(tagkey, n)) {
                if (sigProfile.tagInList(tagkey))
                  return OFTrue;  // one of the elements in tagList is signed
              }
            }
          } else
            return OFTrue;  // all elements are signed
        }
      }
      signer.detach();
    } else {
      // signatures on lower level - check whether attribute on main level is in
      // list
      ...
      if (scard > 1)  // should always be true
      {
        ...
        {
          if (sigProfile.tagInList(obj->getTag()))
            return OFTrue;  // one of the elements in tagList contains a
                            // signature
        }
      }
    }
...
OFBool DVSignatureHandler::attributesSigned(
    DcmItem& /* item */, DcmAttributeTag& /* tagList */) const {
  return OFFalse;
}
...
OFCondition DVSignatureHandler::createSignature(
    DcmItem& mainDataset, const DcmStack& itemStack,
    DcmAttributeTag& attributesNotToSignInMainDataset, const char* userID,
    const char* passwd) {
  if (userID == NULL)
  ...
  filename += userKey;
  if (passwd)
    key.setPrivateKeyPasswd(passwd);
  OFCondition sicond = key.loadPrivateKey(filename.c_str(), fileformat);
  if (sicond != EC_Normal)
    return EC_IllegalCall;  // unable to load private key
  ...
  if (sicond != EC_Normal)
    return EC_IllegalCall;  // unable to load certificate
  if (!key.matchesCertificate(cert))
    return EC_IllegalCall;  // private key does not match certificate

  DcmSignature signer;
  SiRIPEMD160 mac;
  SiNullProfile nullProfile;
  DVSignatureHandlerSignatureProfile mainProfile(
  ...
  while (!workStack.empty()) {
    ...
    if ((current->ident() != EVR_dataset) && (current->ident() != EVR_item))
      return EC_IllegalCall;  // wrong type on stack
    ...
    if (currentItem == &mainDataset) {
      ...
      // we have to establish an explicit tag list, otherwise the profile does
      // not work!
      ...
      sicond = signer.createSignature(key, cert, mac, mainProfile,
                                      EXS_LittleEndianExplicit, &tagList);
      if (sicond != EC_Normal)
        return EC_IllegalCall;  // error while creating signature
    } else {
      // we're creating a signature in a sequence item
      sicond = signer.createSignature(key, cert, mac, nullProfile,
