...
static void moveSubOpProgressCallback(void* /* callbackData */,
  ...
  //  output, if it's set to "DEBUG" then we'll assume that there is debug
  //  output generated for each PDU elsewhere.
...
void DcmQueryRetrieveMoveContext::callbackHandler(
    /* in */
    OFBool cancelled, T_DIMSE_C_MoveRQ* request, DcmDataset* requestIdentifiers,
    int responseCount,
    /* out */
  ...
  if (responseCount == 1) {
    ...
    DCMQRDB_INFO("Move SCP Request Identifiers:"
                 << OFendl << DcmObject::PrintHelper(*requestIdentifiers));
    dbcond = dbHandle.startMoveRequest(request->AffectedSOPClassUID,
                                       requestIdentifiers, &dbStatus);
    if (dbcond.bad()) {
  ...
  if (dbStatus.status() != STATUS_Pending) {
    ...
    /*
     * if all the sub-operations failed then we need to generate a failed or
     * refused status. cf. DICOM part 4, C.4.2.3.1 we choose to generate a
     * "Refused - Out of Resources - Unable to perform suboperations" status.
     */
  ...
  *stDetail = dbStatus.extractStatusDetail();
  ...
  if (DICOM_PENDING_STATUS(dbStatus.status()) &&
    ...
    DCMQRDB_DEBUG("Move SCP Response Identifiers:"
                  << OFendl << DcmObject::PrintHelper(**responseIdentifiers));
  }
  if (*stDetail) {
    DCMQRDB_DEBUG("  Status detail:" << OFendl
                                     << DcmObject::PrintHelper(**stDetail));
  }
}

void DcmQueryRetrieveMoveContext::addFailedUIDInstance(
  ...
  if (failedUIDs == NULL) {
    ...
    if ((failedUIDs = (char*)realloc(
             failedUIDs, (len + strlen(sopInstance) + 2))) == NULL) {
...
OFCondition DcmQueryRetrieveMoveContext::performMoveSubOp(DIC_UI sopClass,
                                                          DIC_UI sopInstance,
                                                          char* fname) {
  OFCondition cond = EC_Normal;
  T_DIMSE_C_StoreRQ req;
  ...
  if (lockfd < 0) {
    ...
    DCMQRDB_ERROR("Move SCP: storeSCU: [file: "
                  << fname
  ...
  if (presId == 0) {
    ...
    addFailedUIDInstance(sopInstance);
    DCMQRDB_ERROR("Move SCP: storeSCU: [file: "
                  << fname << "] No presentation context for: ("
  ...
  DCMQRDB_INFO("Store SCU RQ: MsgID "
               << msgId << ", (" << dcmSOPClassUIDToModality(sopClass, "OT")
               << ")");

  cond = DIMSE_storeUser(subAssoc, presId, &req, fname, NULL,
                         moveSubOpProgressCallback, this, options_.blockMode_,
                         options_.dimse_timeout_, &rsp, &stDetail);
  ...
  if (stDetail != NULL) {
    DCMQRDB_INFO("  Status Detail:" << OFendl
                                    << DcmObject::PrintHelper(*stDetail));
    delete stDetail;
  }
  return cond;
...
OFCondition DcmQueryRetrieveMoveContext::buildSubAssociation(
  ...
  if (!mapMoveDestination(origHostName, origAETitle, request->MoveDestination,
                          dstHostName, &dstPortNumber)) {
...
void DcmQueryRetrieveMoveContext::moveNextImage(
    DcmQueryRetrieveDatabaseStatus* dbStatus) {
  OFCondition cond = EC_Normal;
  OFCondition dbcond = EC_Normal;
  ...
  /* get DB response */
  dbcond = dbHandle.nextMoveResponse(subImgSOPClass, subImgSOPInstance,
                                     subImgFileName, &nRemaining, dbStatus);
  if (dbcond.bad()) {
  ...
  if (dbStatus->status() == STATUS_Pending) {
    ...
    if (cond != EC_Normal) {
      OFString temp_str;
      DCMQRDB_ERROR("moveSCP: Move Sub-Op Failed: " << DimseCondition::dump(
                        temp_str, cond));
...
void DcmQueryRetrieveMoveContext::failAllSubOperations(
    DcmQueryRetrieveDatabaseStatus* dbStatus) {
  OFCondition dbcond = EC_Normal;
  DIC_UI subImgSOPClass;               /* sub-operation image SOP Class */
  DIC_UI subImgSOPInstance;            /* sub-operation image SOP Instance */
...
OFBool DcmQueryRetrieveMoveContext::mapMoveDestination(const char* origPeer,
                                                       const char* origAE,
                                                       const char* dstAE,
                                                       char* dstPeer,
                                                       int* dstPort) {
  ...
  if (options_.restrictMoveToSameAE_) {
    ...
    if (!ok) {
      DCMQRDB_INFO("mapMoveDestination: strictMove Reqs: '"
                   << origAE << "' != '" << dstAE << "'");
      return OFFalse;
    }
  }
  ...
  if (options_.restrictMoveToSameHost_) {
    ...
    if (!ok) {
      DCMQRDB_INFO("mapMoveDestination: different hosts: '"
                   << origPeer << "', '" << dstPeer << "'");
      return OFFalse;
    }
  }
  ...
  if (options_.restrictMoveToSameVendor_) {
    ...
    if (!ok) {
      DCMQRDB_INFO("mapMoveDestination: different vendors: '"
                   << origAE << "', '" << dstAE << "'");
      return OFFalse;
    }
  }
...
OFCondition DcmQueryRetrieveMoveContext::addAllStoragePresentationContexts(
    T_ASC_Parameters* params) {
