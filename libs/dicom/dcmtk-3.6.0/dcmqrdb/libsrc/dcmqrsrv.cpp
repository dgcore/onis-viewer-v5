...
static void findCallback(
    ...
    DcmDataset** stDetail) {
  DcmQueryRetrieveFindContext* context =
      OFstatic_cast(DcmQueryRetrieveFindContext*, callbackData);
...
static void getCallback(
    /* in */
    void* callbackData, OFBool cancelled, T_DIMSE_C_GetRQ* request,
    DcmDataset* requestIdentifiers, int responseCount,
    /* out */
    T_DIMSE_C_GetRSP* response, DcmDataset** stDetail,
    DcmDataset** responseIdentifiers) {
  DcmQueryRetrieveGetContext* context =
      OFstatic_cast(DcmQueryRetrieveGetContext*, callbackData);
  context->callbackHandler(cancelled, request, requestIdentifiers,
...
static void moveCallback(
    /* in */
    void* callbackData, OFBool cancelled, T_DIMSE_C_MoveRQ* request,
    DcmDataset* requestIdentifiers, int responseCount,
    /* out */
    T_DIMSE_C_MoveRSP* response, DcmDataset** stDetail,
    DcmDataset** responseIdentifiers) {
  DcmQueryRetrieveMoveContext* context =
      OFstatic_cast(DcmQueryRetrieveMoveContext*, callbackData);
  context->callbackHandler(cancelled, request, requestIdentifiers,
                           responseCount, response, stDetail,
                           responseIdentifiers);
}
...
static void storeCallback(
    /* in */
    void* callbackData, T_DIMSE_StoreProgress* progress, /* progress state */
    ...
    DcmDataset** stDetail) {
  DcmQueryRetrieveStoreContext* context =
      OFstatic_cast(DcmQueryRetrieveStoreContext*, callbackData);
  context->callbackHandler(progress, req, imageFileName, imageDataSet, rsp,
                           stDetail);
}
...
OFCondition DcmQueryRetrieveSCP::dispatch(T_ASC_Association* assoc,
  ...
  // this while loop is executed exactly once unless the
  // "keepDBHandleDuringAssociation_" flag is not set, in which case the inner
  // loop is executed only once and this loop repeats for each incoming DIMSE
  // command. In this case, the DB handle is created and released for each DIMSE
  // command.
  while (cond.good()) {
    /* Create a database handle for this association */
    DcmQueryRetrieveDatabaseHandle* dbHandle =
        factory_.createDBHandle(assoc->params->DULparams.callingAPTitle,
    ...
    dbHandle->setIdentifierChecking(dbCheckFindIdentifier_,
                                    dbCheckMoveIdentifier_);
    firstLoop = OFTrue;

    // this while loop is executed exactly once unless the
    // "keepDBHandleDuringAssociation_" flag is set, in which case the DB handle
    // remains open until something goes wrong or the remote peer closes the
    // association
    while (cond.good() &&
           (firstLoop || options_.keepDBHandleDuringAssociation_)) {
      firstLoop = OFFalse;
      cond =
          DIMSE_receiveCommand(assoc, DIMSE_BLOCKING, 0, &presID, &msg, NULL);
      ...
      if (cond.good()) {
        ...
        switch (msg.CommandField) {
          ...
          case DIMSE_C_STORE_RQ:
            cond = storeSCP(assoc, &msg.msg.CStoreRQ, presID, *dbHandle,
                            correctUIDPadding);
            break;
          ...
          default:
            ...
            DCMQRDB_ERROR("Cannot handle command: 0x"
                          << STD_NAMESPACE hex << (unsigned)msg.CommandField);
            /* the condition will be returned, the caller will abort the
             * association. */
        }
...
OFCondition DcmQueryRetrieveSCP::handleAssociation(T_ASC_Association* assoc,
                                                   OFBool correctUIDPadding) {
  OFCondition cond = EC_Normal;
  ...
  if (cond.bad()) {
    DCMQRDB_ERROR(
        "Cannot Drop Association: " << DimseCondition::dump(temp_str, cond));
  }
  cond = ASC_destroyAssociation(&assoc);
  if (cond.bad()) {
    DCMQRDB_ERROR(
        "Cannot Destroy Association: " << DimseCondition::dump(temp_str, cond));
  }

  return cond;
}

OFCondition DcmQueryRetrieveSCP::echoSCP(T_ASC_Association* assoc,
  ...
  if (cond.bad()) {
    OFString temp_str;
    DCMQRDB_ERROR("echoSCP: Echo Response Failed: " << DimseCondition::dump(
                      temp_str, cond));
  }
  return cond;
}

OFCondition DcmQueryRetrieveSCP::findSCP(
    T_ASC_Association* assoc, T_DIMSE_C_FindRQ* request,
    T_ASC_PresentationContextID presID,
    DcmQueryRetrieveDatabaseHandle& dbHandle)

{
  ...
  cond = DIMSE_findProvider(assoc, presID, request, findCallback, &context,
...
OFCondition DcmQueryRetrieveSCP::getSCP(
    T_ASC_Association* assoc, T_DIMSE_C_GetRQ* request,
    T_ASC_PresentationContextID presID,
    DcmQueryRetrieveDatabaseHandle& dbHandle) {
  OFCondition cond = EC_Normal;
  DcmQueryRetrieveGetContext context(dbHandle, options_, STATUS_Pending, assoc,
                                     request->MessageID, request->Priority,
                                     presID);
  ...
  cond = DIMSE_getProvider(assoc, presID, request, getCallback, &context,
                           options_.blockMode_, options_.dimse_timeout_);
  if (cond.bad()) {
...
OFCondition DcmQueryRetrieveSCP::moveSCP(
    T_ASC_Association* assoc, T_DIMSE_C_MoveRQ* request,
    T_ASC_PresentationContextID presID,
    DcmQueryRetrieveDatabaseHandle& dbHandle) {
  OFCondition cond = EC_Normal;
  DcmQueryRetrieveMoveContext context(dbHandle, options_, config_,
                                      STATUS_Pending, assoc, request->MessageID,
                                      request->Priority);
  ...
  cond = DIMSE_moveProvider(assoc, presID, request, moveCallback, &context,
                            options_.blockMode_, options_.dimse_timeout_);
  if (cond.bad()) {
...
OFCondition DcmQueryRetrieveSCP::storeSCP(
    T_ASC_Association* assoc, T_DIMSE_C_StoreRQ* request,
    T_ASC_PresentationContextID presId,
    DcmQueryRetrieveDatabaseHandle& dbHandle, OFBool correctUIDPadding) {
  OFCondition cond = EC_Normal;
  OFCondition dbcond = EC_Normal;
  char imageFileName[MAXPATHLEN + 1];
  DcmFileFormat dcmff;

  DcmQueryRetrieveStoreContext context(dbHandle, options_, STATUS_Success,
                                       &dcmff, correctUIDPadding);
...
#ifdef O_BINARY
  int lockfd =
      open(imageFileName, (O_WRONLY | O_CREAT | O_TRUNC | O_BINARY), 0666);
#else
  ...
  if (options_.bitPreserving_) { /* the bypass option can be set on the command
                                    line */
    cond = DIMSE_storeProvider(assoc, presId, request, imageFileName,
                               (int)options_.useMetaheader_, NULL,
                               storeCallback, (void*)&context,
                               options_.blockMode_, options_.dimse_timeout_);
  } else {
    cond = DIMSE_storeProvider(assoc, presId, request, (char*)NULL,
                               (int)options_.useMetaheader_, &dset,
                               storeCallback, (void*)&context,
                               options_.blockMode_, options_.dimse_timeout_);
  }
  ...
  if (!options_.ignoreStoreData_ &&
    ...
    /* remove file */
    if (strcmp(imageFileName, NULL_DEVICE_NAME) !=
        0)  // don't try to delete /dev/null
    {
  ...
  for (i = 0; i < numberOfAllDcmStorageSOPClassUIDs; i++) {
    do {
      pid = ASC_findAcceptedPresentationContextID(assoc,
                                                  dcmAllStorageSOPClassUIDs[i]);
      if (pid != 0)
        ASC_refusePresentationContext(assoc->params, pid, ASC_P_USERREJECTION);
    } while (pid != 0);  // repeat as long as we find presentation contexts for
                         // this SOP class - there might be multiple ones.
  }
}

OFCondition DcmQueryRetrieveSCP::refuseAssociation(T_ASC_Association** assoc,
  ...
  if (cond.bad()) {
    DCMQRDB_ERROR(
        "Association Reject Failed: " << DimseCondition::dump(temp_str, cond));
  }
...
OFCondition DcmQueryRetrieveSCP::negotiateAssociation(
  ...
  switch (options_.networkTransferSyntax_) {
    ...
    case EXS_JPEGProcess14SV1TransferSyntax:
      /* we prefer JPEGLossless:Hierarchical-1stOrderPrediction (default
       * lossless) */
  ...
  for (i = 0; i < numberOfNonStorageSyntaxes; i++) {
    if (0 == strcmp(nonStorageSyntaxes[i],
                    UID_FINDPatientRootQueryRetrieveInformationModel)) {
      if (options_.supportPatientRoot_)
        selectedNonStorageSyntaxes[numberOfSelectedNonStorageSyntaxes++] =
            nonStorageSyntaxes[i];
    } else if (0 == strcmp(nonStorageSyntaxes[i],
                           UID_MOVEPatientRootQueryRetrieveInformationModel)) {
      if (options_.supportPatientRoot_)
        selectedNonStorageSyntaxes[numberOfSelectedNonStorageSyntaxes++] =
            nonStorageSyntaxes[i];
      ...
      if (options_.supportStudyRoot_ && (!options_.disableGetSupport_))
        selectedNonStorageSyntaxes[numberOfSelectedNonStorageSyntaxes++] =
            nonStorageSyntaxes[i];
               ...
               strcmp(nonStorageSyntaxes[i], UID_PrivateShutdownSOPClass)) {
  ...
  cond = ASC_acceptContextsWithPreferredTransferSyntaxes(
      assoc->params, (const char**)selectedNonStorageSyntaxes,
      numberOfSelectedNonStorageSyntaxes, (const char**)transferSyntaxes,
      numTransferSyntaxes);
  if (cond.bad()) {
  ...
  if (options_.disableGetSupport_) {
    ...
    cond = ASC_acceptContextsWithPreferredTransferSyntaxes(
        ...
        numberOfAllDcmStorageSOPClassUIDs, (const char**)transferSyntaxes,
    ...
    for (i = 0; i < npc; i++) {
      ...
      if (dcmIsaStorageSOPClassUID(pc.abstractSyntax)) {
        ...
        ** When processing the C-GET operation we can be the SCU of the Storage
        *Service Class.
        */
        ...
        */
        for (int k = numTransferSyntaxes - 1; k >= 0; k--) {
          for (int j = 0; j < (int)pc.transferSyntaxCount; j++) {
             ...
             */
            if (strcmp(pc.proposedTransferSyntaxes[j], transferSyntaxes[k]) ==
                0) {
              cond = ASC_acceptPresentationContext(assoc->params,
  ...
  if (0 != ASC_findAcceptedPresentationContextID(assoc,
                                                 UID_PrivateShutdownSOPClass)) {
    DCMQRDB_INFO(
        "Shutting down server ... (negotiated private \"shut down\" SOP "
        "class)");
  ...
  for (i = 0; i < (int)DIM_OF(queryRetrievePairs); i++) {
    movepid = ASC_findAcceptedPresentationContextID(
        assoc, queryRetrievePairs[i].moveSyntax);
    if (movepid != 0) {
      findpid = ASC_findAcceptedPresentationContextID(
          assoc, queryRetrievePairs[i].findSyntax);
      if (findpid == 0) {
        if (options_.requireFindForMove_) {
          /* refuse the move */
          ASC_refusePresentationContext(assoc->params, movepid,
                                        ASC_P_USERREJECTION);
        } else {
          DCMQRDB_ERROR(
...
OFCondition DcmQueryRetrieveSCP::waitForAssociation(T_ASC_Network* theNet) {
  ...
  if (ASC_associationWaiting(theNet, timeout)) {
    ...
    if (cond.bad()) {
      DCMQRDB_INFO("Failed to receive association: " << DimseCondition::dump(
                       temp_str, cond));
      go_cleanup = OFTrue;
    }
  } else
  ...
  if (!go_cleanup) {
    time_t t = time(NULL);
    DCMQRDB_INFO("Association Received ("
  ...
  if (!go_cleanup) {
    ...
    if (processtable_.countChildProcesses() >=
        OFstatic_cast(size_t, options_.maxAssociations_)) {
  ...
  if (!go_cleanup) {
    DCMQRDB_INFO("Association Acknowledged (Max Send PDV: "
                 << assoc->sendPDVLength << ")");
  ...
  if (!options_.singleProcess_ && (cond != ASC_SHUTDOWNAPPLICATION)) {
    ...
    cond = ASC_destroyAssociation(&assoc);
    if (cond.bad()) {
      DCMQRDB_ERROR("Cannot Destroy Association: " << DimseCondition::dump(
                        temp_str, cond));
    }
  }
