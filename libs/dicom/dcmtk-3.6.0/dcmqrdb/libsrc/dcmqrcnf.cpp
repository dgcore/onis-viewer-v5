...
int DcmQueryRetrieveConfig::aeTitlesForPeer(const char* hostName,
                                            const char*** aeTitleList) const {
  int n = 0;
  int i, j, k;
...
#else
      if (strcmp(hname, hostName) ==
          0) { /* fallback if case insensitive compare is unavailable */
#endif
        ...
        if (!found) {
          if (n >= maxAlloc) {
            *aeTitleList = (const char**)realloc(
                *aeTitleList, (maxAlloc + chunkSize) * sizeof(const char*));
            maxAlloc += chunkSize;
          }
...
int DcmQueryRetrieveConfig::aeTitlesForSymbolicName(
    const char* symbolicName, const char*** aeTitleList) const {
  int i = 0;
  int j = 0;
  int n = 0;
...
const char* DcmQueryRetrieveConfig::vendorForPeerAETitle(
    const char* peerAETitle) const {
  int i = 0;
  int j = 0;
  ...
  while (!feof(cnffp)) {
    ...
    if (!strcmp("ApplicationTitle", mnemonic)) {
      ...
      if (!strcmp("BEGIN", value)) {
        ...
        panic(
            "No \"HostTable BEGIN\" before END in configuration file, line %d",
            lineno);
        error = 1;
      } else {
        panic("Unknown HostTable status \"%s\" in configuartion file, line %d",
              value, lineno);
        error = 1;
      }
      ...
      if (!strcmp("BEGIN", value)) {
        ...
        panic(
            "No \"VendorTable BEGIN\" before END in configuration file, line "
            "%d",
            lineno);
        error = 1;
      } else {
        panic(
            "Unknown VendorTable status \"%s\" in configuartion file, line %d",
            value, lineno);
        error = 1;
      }
      ...
      if (!strcmp("BEGIN", value)) {
        ...
        panic("No \"AETable BEGIN\" before END in configuration file, line %d",
              lineno);
        error = 1;
      } else {
        panic("Unknown AETable status \"%s\" in configuartion file, line %d",
              value, lineno);
        error = 1;
      }
    } else {
      panic("Unknown mnemonic \"%s\" in configuration file, line %d", mnemonic,
            lineno);
      error = 1;
    }
  }
  ...
  while (!feof(cnffp)) {
    ...
    if (!strcmp("HostTable", mnemonic)) {
      if (!strcmp("END", value)) {
        ...
        panic("Illegal HostTable status \"%s\" in configuration file, line %d",
              value, *lineno);
        error = 1;
        break;
      }
    ...
    if ((helpentry = (DcmQueryRetrieveConfigHostEntry*)malloc(
             CNF_HETable.noOfHostEntries *
             sizeof(DcmQueryRetrieveConfigHostEntry))) == NULL)
      panic("Memory allocation 1 (%d)", CNF_HETable.noOfHostEntries);
    if (CNF_HETable.noOfHostEntries - 1) {
      memcpy((char*)helpentry, (char*)CNF_HETable.HostEntries,
             (CNF_HETable.noOfHostEntries - 1) *
                 sizeof(DcmQueryRetrieveConfigHostEntry));
      free(CNF_HETable.HostEntries);
    }
    CNF_HETable.HostEntries = helpentry;

    CNF_HETable.HostEntries[CNF_HETable.noOfHostEntries - 1].SymbolicName =
        parsevalues(&lineptr);
    CNF_HETable.HostEntries[CNF_HETable.noOfHostEntries - 1].Peers =
        readPeerList(&lineptr, &noOfPeers);
    CNF_HETable.HostEntries[CNF_HETable.noOfHostEntries - 1].noOfPeers =
        noOfPeers;
    if (!noOfPeers)
      error = 1;
  }
  ...
  while (!feof(cnffp)) {
    ...
    if (!strcmp("VendorTable", mnemonic)) {
      if (!strcmp("END", value)) {
        ...
        panic(
            "Illegal VendorTable status \"%s\" in configuration file, line %d",
            value, *lineno);
        error = 1;
        break;
      }
    ...
    if (CNF_VendorTable.noOfHostEntries - 1) {
      memcpy((char*)helpentry, (char*)CNF_VendorTable.HostEntries,
             (CNF_VendorTable.noOfHostEntries - 1) *
                 sizeof(DcmQueryRetrieveConfigHostEntry));
      free(CNF_VendorTable.HostEntries);
    }
    CNF_VendorTable.HostEntries = helpentry;

    CNF_VendorTable.HostEntries[CNF_VendorTable.noOfHostEntries - 1]
        .SymbolicName = parsevalues(&lineptr);
    CNF_VendorTable.HostEntries[CNF_VendorTable.noOfHostEntries - 1].Peers =
        readPeerList(&lineptr, &noOfPeers);
    CNF_VendorTable.HostEntries[CNF_VendorTable.noOfHostEntries - 1].noOfPeers =
        noOfPeers;
    if (!noOfPeers)
      error = 1;
  }
  ...
  while (!feof(cnffp)) {
    ...
    if (!strcmp("AETable", mnemonic)) {
      if (!strcmp("END", value)) {
        ...
        panic("Illegal AETable status \"%s\" in configuration file, line %d",
              value, *lineno);
        error = 1;
        break;
      }
    ...
    lineptr = rcline;
    noOfAEEntries++;
    if ((helpentry = (DcmQueryRetrieveConfigAEEntry*)malloc(
             noOfAEEntries * sizeof(DcmQueryRetrieveConfigAEEntry))) == NULL)
      panic("Memory allocation 3 (%d)", noOfAEEntries);
    if (noOfAEEntries - 1) {
      memcpy((char*)helpentry, (char*)CNF_Config.AEEntries,
             (noOfAEEntries - 1) * sizeof(DcmQueryRetrieveConfigAEEntry));
      free(CNF_Config.AEEntries);
    }
    CNF_Config.AEEntries = helpentry;

    CNF_Config.AEEntries[noOfAEEntries - 1].ApplicationTitle =
        parsevalues(&lineptr);
    ...
    CNF_Config.AEEntries[noOfAEEntries - 1].Peers = parsePeers(
        &lineptr, &CNF_Config.AEEntries[noOfAEEntries - 1].noOfPeers);
...
DcmQueryRetrieveConfigQuota* DcmQueryRetrieveConfig::parseQuota(
    char** valuehandle) {
  int studies;
  char *helpvalue, helpval[20];
  DcmQueryRetrieveConfigQuota* helpquota;

  if ((helpquota = (DcmQueryRetrieveConfigQuota*)malloc(
           sizeof(DcmQueryRetrieveConfigQuota))) == NULL)
...
DcmQueryRetrieveConfigPeer* DcmQueryRetrieveConfig::parsePeers(
    char** valuehandle, int* peers) {
  char* helpvalue;
...
DcmQueryRetrieveConfigPeer* DcmQueryRetrieveConfig::readPeerList(
    char** valuehandle, int* peers) {
  int i, found, noOfPeers = 0;
  char* helpvalue;
  DcmQueryRetrieveConfigPeer *helppeer, *peerlist = NULL;
  ...
  while ((helpvalue = parsevalues(valuehandle)) != NULL) {
    ...
    if (strchr(helpvalue, ',') == NULL) { /* symbolic name */
      ...
      if ((helppeer = (DcmQueryRetrieveConfigPeer*)malloc(
               noOfPeers * sizeof(DcmQueryRetrieveConfigPeer))) == NULL)
        panic("Memory allocation 5 (%d)", noOfPeers);
      if (noOfPeers - CNF_HETable.HostEntries[i].noOfPeers) {
        memcpy((char*)helppeer, (char*)peerlist,
               (noOfPeers - CNF_HETable.HostEntries[i].noOfPeers) *
                   sizeof(DcmQueryRetrieveConfigPeer));
        free(peerlist);
      }
      peerlist = helppeer;
      memcpy((char*)(peerlist +
                     (noOfPeers - CNF_HETable.HostEntries[i].noOfPeers)),
             (char*)CNF_HETable.HostEntries[i].Peers,
             CNF_HETable.HostEntries[i].noOfPeers *
                 sizeof(DcmQueryRetrieveConfigPeer));
    }
      ...
      if (noOfPeers - 1) {
        memcpy((char*)helppeer, (char*)peerlist,
               (noOfPeers - 1) * sizeof(DcmQueryRetrieveConfigPeer));
        free(peerlist);
      }
      peerlist = helppeer;
...
void DcmQueryRetrieveConfig::printConfig() {
  ...
  for (i = 0; i < CNF_HETable.noOfHostEntries; i++) {
    DCMQRDB_INFO(CNF_HETable.HostEntries[i].SymbolicName
                 << " " << CNF_HETable.HostEntries[i].noOfPeers);
    for (j = 0; j < CNF_HETable.HostEntries[i].noOfPeers; j++) {
      DCMQRDB_INFO(CNF_HETable.HostEntries[i].Peers[j].ApplicationTitle
                   << " " << CNF_HETable.HostEntries[i].Peers[j].HostName << " "
                   << CNF_HETable.HostEntries[i].Peers[j].PortNumber);
    }
  }
  DCMQRDB_INFO("\nVendorTable: " << CNF_VendorTable.noOfHostEntries);
  for (i = 0; i < CNF_VendorTable.noOfHostEntries; i++) {
    DCMQRDB_INFO(CNF_VendorTable.HostEntries[i].SymbolicName
                 << " " << CNF_VendorTable.HostEntries[i].noOfPeers);
    for (j = 0; j < CNF_VendorTable.HostEntries[i].noOfPeers; j++) {
      DCMQRDB_INFO(CNF_VendorTable.HostEntries[i].Peers[j].ApplicationTitle
                   << " " << CNF_VendorTable.HostEntries[i].Peers[j].HostName
                   << " "
                   << CNF_VendorTable.HostEntries[i].Peers[j].PortNumber);
    }
  }
  DCMQRDB_INFO("\nGlobal Parameters:\n"
               << networkTCPPort_ << "\n"
    ...
    if (CNF_Config.AEEntries[i].noOfPeers == -1)
      ...
      for (j = 0; j < CNF_Config.AEEntries[i].noOfPeers; j++) {
        DCMQRDB_INFO(CNF_Config.AEEntries[i].Peers[j].ApplicationTitle
                     << " " << CNF_Config.AEEntries[i].Peers[j].HostName << " "
                     << CNF_Config.AEEntries[i].Peers[j].PortNumber);
      }
    }
    DCMQRDB_INFO("----------------------------------\n");
  }
...
int DcmQueryRetrieveConfig::peerInAETitle(const char* calledAETitle,
...
int DcmQueryRetrieveConfig::peerForAETitle(const char* AETitle,
                                           const char** HostName,
...
int DcmQueryRetrieveConfig::checkForSameVendor(const char* AETitle1,
                                               const char* AETitle2) const {
  int i, j, k, found = 0;
    ...
    for (j = 0; j < CNF_VendorTable.HostEntries[i].noOfPeers; j++) {
      if (!strcmp(AETitle1,
        ...
        for (k = 0; k < CNF_VendorTable.HostEntries[i].noOfPeers; k++) {
          if (!strcmp(AETitle2,
                      CNF_VendorTable.HostEntries[i].Peers[k].ApplicationTitle))
            found = 1;
        }
      }
    }
...
int DcmQueryRetrieveConfig::HostNamesForVendor(
    const char* Vendor, const char*** HostNameArray) const {
  int i, j, found = 0;
  ...
  if ((*HostNameArray = (const char**)malloc(
           CNF_VendorTable.HostEntries[i].noOfPeers * sizeof(const char*))) ==
      NULL) {
