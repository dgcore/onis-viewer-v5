...
void DcmQueryRetrieveStoreContext::updateDisplay(
...
void DcmQueryRetrieveStoreContext::saveImageToDB(
  ...
  if (status == STATUS_Success) {
    dbcond = dbHandle.storeRequest(req->AffectedSOPClassUID,
                                   req->AffectedSOPInstanceUID, imageFileName,
                                   &dbStatus);
    if (dbcond.bad()) {
      OFString temp_str;
...
void DcmQueryRetrieveStoreContext::writeToFile(DcmFileFormat* ff,
                                               const char* fname,
  ...
  OFCondition cond =
      ff->saveFile(fname, xfer, options_.sequenceType_, options_.groupLength_,
                   options_.paddingType_, (Uint32)options_.filepad_,
                   (Uint32)options_.itempad_,
                   (options_.useMetaheader_) ? EWM_fileformat : EWM_dataset);
  ...
  if (progress->state == DIMSE_StoreEnd) {
    ...
    if (rsp->DimseStatus != STATUS_Success)
      DCMQRDB_WARN("NOTICE: StoreSCP:"
                   << OFendl << DIMSE_dumpMessage(str, *rsp, DIMSE_OUTGOING));
    else
      DCMQRDB_INFO("Sending:" << OFendl
