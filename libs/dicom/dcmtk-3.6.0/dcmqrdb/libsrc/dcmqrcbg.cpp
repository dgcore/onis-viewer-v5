...
void DcmQueryRetrieveGetContext::callbackHandler(
    /* in */
    OFBool cancelled, T_DIMSE_C_GetRQ* request, DcmDataset* requestIdentifiers,
    int responseCount,
    /* out */
  ...
  if (dbStatus.status() != STATUS_Pending) {
     ...
     * refused status. cf. DICOM part 4, C.4.3.3.1 we choose to generate a
...
OFCondition DcmQueryRetrieveGetContext::performGetSubOp(DIC_UI sopClass,
                                                        DIC_UI sopInstance,
                                                        char* fname) {
  ...
  if (lockfd < 0) {
    ...
    DCMQRDB_ERROR("Get SCP: storeSCU: [file: "
                  << fname
  ...
  presId = ASC_findAcceptedPresentationContextID(origAssoc, sopClass);
  if (presId == 0) {
    nFailed++;
    addFailedUIDInstance(sopInstance);
    DCMQRDB_ERROR("Get SCP: storeSCU: [file: "
    ...
    if ((pc.acceptedRole != ASC_SC_ROLE_SCP) &&
      ...
      addFailedUIDInstance(sopInstance);
      DCMQRDB_ERROR(
          "Get SCP: storeSCU: [file: "
          << fname << "] No presentation context with requestor SCP role for: ("
  ...
  cond = DIMSE_storeUser(origAssoc, presId, &req, fname, NULL,
                         getSubOpProgressCallback, this, options_.blockMode_,
  ...
  if (cond.good()) {
    if (cancelParameters.cancelEncountered) {
      if (origPresId == cancelParameters.presId &&
        ...
        DCMQRDB_ERROR("Get SCP: Unexpected C-Cancel-RQ encountered: pid="
                      << (int)cancelParameters.presId << ", mid="
                      << (int)cancelParameters.req.MessageIDBeingRespondedTo);
      }
    }
    ...
    addFailedUIDInstance(sopInstance);
    OFString temp_str;
    DCMQRDB_ERROR("Get SCP: storeSCU: Store Request Failed: "
...
void DcmQueryRetrieveGetContext::getNextImage(
  ...
  if (dbStatus->status() == STATUS_Pending) {
    ...
    if (cond != EC_Normal) {
      OFString temp_str;
      DCMQRDB_ERROR("getSCP: Get Sub-Op Failed: " << DimseCondition::dump(
                        temp_str, cond));
