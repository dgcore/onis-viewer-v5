...
/** helper class that describes entries in the process slot table. Internal use
 * only.
 */
...
public:
   ...
   */
  DcmQueryRetrieveProcessSlot(const char* peerName, const char* callingAETitle,
                              const char* calledAETitle, int processId,
                              time_t startTime, OFBool hasStorageAbility);
   ...
   */
  OFBool isProcessWithWriteAccess(const char* calledAETitle) const;
...
DcmQueryRetrieveProcessSlot::DcmQueryRetrieveProcessSlot(
    const char* peerName, const char* callingAETitle, const char* calledAETitle,
    int processId, time_t startTime, OFBool hasStorageAbility)
    : peerName_(),
      callingAETitle_(),
      calledAETitle_(),
  ...
  if (calledAETitle)
    calledAETitle_ = calledAETitle;
}

OFBool DcmQueryRetrieveProcessSlot::isProcessWithWriteAccess(
    const char* calledAETitle) const {
  return (hasStorageAbility_ && calledAETitle &&
          calledAETitle_ == calledAETitle);
}
...
void DcmQueryRetrieveProcessTable::addProcessToTable(int pid,
  ...
  for (int i = 0; i < numberOfAllDcmStorageSOPClassUIDs; i++) {
    if (ASC_findAcceptedPresentationContextID(assoc,
  ...
  DcmQueryRetrieveProcessSlot* slot =
      new DcmQueryRetrieveProcessSlot(peerName, callingAETitle, calledAETitle,
                                      pid, time(NULL), hasStorageAbility);
...
OFBool DcmQueryRetrieveProcessTable::haveProcessWithWriteAccess(
    const char* calledAETitle) const {
  ...
  while (first != last) {
    if ((*first)->isProcessWithWriteAccess(calledAETitle))
      return OFTrue;
    ++first;
  }
  return OFFalse;
  ...
  while (child > 0) {
    ...
    if (child < 0) {
      if (errno == ECHILD) {
        ...
        char buf[256];
        DCMQRDB_ERROR("wait for child process failed: " << OFStandard::strerror(
