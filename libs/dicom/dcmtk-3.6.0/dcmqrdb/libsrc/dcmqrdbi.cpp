...
const OFConditionConst DcmQRIndexDatabaseErrorC(OFM_imagectn, 0x001, OF_error,
                                                "DcmQR Index Database Error");
...
static const DB_FindAttr TbFindAttr[] = {
    ...
    DB_FindAttr(DCM_NumberOfPatientRelatedSeries, PATIENT_LEVEL, OPTIONAL_KEY,
                STRING_CLASS),
    DB_FindAttr(DCM_NumberOfPatientRelatedInstances, PATIENT_LEVEL,
                OPTIONAL_KEY, STRING_CLASS),
...
static void DB_UIDAddFound(DB_Private_Handle* phandle, IdxRecord* idxRec) {
  DB_UidList* plist;
...
static int DB_UIDAlreadyFound(DB_Private_Handle* phandle, IdxRecord* idxRec) {
  DB_UidList* plist;
...
static void DB_IdxInitRecord(IdxRecord* idx, int linksOnly) {
  if (!linksOnly) {
    ...
    idx->EthnicGroup[0] = '\0';
    idx->param[RECORDIDX_NumberofPatientRelatedStudies].XTag =
        DCM_NumberOfPatientRelatedStudies;
    idx->param[RECORDIDX_NumberofPatientRelatedStudies].ValueLength =
        IS_MAX_LENGTH;
    idx->NumberofPatientRelatedStudies[0] = '\0';
    idx->param[RECORDIDX_NumberofPatientRelatedSeries].XTag =
        DCM_NumberOfPatientRelatedSeries;
    idx->param[RECORDIDX_NumberofPatientRelatedSeries].ValueLength =
        IS_MAX_LENGTH;
    idx->NumberofPatientRelatedSeries[0] = '\0';
    idx->param[RECORDIDX_NumberofPatientRelatedInstances].XTag =
        DCM_NumberOfPatientRelatedInstances;
    idx->param[RECORDIDX_NumberofPatientRelatedInstances].ValueLength =
        IS_MAX_LENGTH;
    ...
    idx->param[RECORDIDX_NameOfPhysiciansReadingStudy].XTag =
        DCM_NameOfPhysiciansReadingStudy;
    idx->param[RECORDIDX_NameOfPhysiciansReadingStudy].ValueLength =
        PN_MAX_LENGTH;
    ...
    idx->param[RECORDIDX_ReferringPhysicianName].XTag =
    ...
    idx->param[RECORDIDX_AttendingPhysiciansName].XTag =
        DCM_NameOfPhysiciansReadingStudy;
    ...
    idx->param[RECORDIDX_OtherStudyNumbers].XTag =
        DCM_RETIRED_OtherStudyNumbers;
    idx->param[RECORDIDX_OtherStudyNumbers].ValueLength = IS_MAX_LENGTH;
    idx->OtherStudyNumbers[0] = '\0';
    idx->param[RECORDIDX_AdmittingDiagnosesDescription].XTag =
        DCM_AdmittingDiagnosesDescription;
    idx->param[RECORDIDX_AdmittingDiagnosesDescription].ValueLength =
        LO_MAX_LENGTH;
    ...
    idx->Occupation[0] = '\0';
    idx->param[RECORDIDX_NumberofStudyRelatedSeries].XTag =
        DCM_NumberOfStudyRelatedSeries;
    idx->param[RECORDIDX_NumberofStudyRelatedSeries].ValueLength =
        IS_MAX_LENGTH;
    idx->NumberofStudyRelatedSeries[0] = '\0';
    idx->param[RECORDIDX_NumberofStudyRelatedInstances].XTag =
        DCM_NumberOfStudyRelatedInstances;
    idx->param[RECORDIDX_NumberofStudyRelatedInstances].ValueLength =
        IS_MAX_LENGTH;
    ...
    idx->param[RECORDIDX_PerformingPhysicianName].XTag =
  ...
  idx->param[RECORDIDX_PatientBirthDate].PValueField =
      (char*)idx->PatientBirthDate;
  ...
  idx->param[RECORDIDX_PatientBirthTime].PValueField =
      (char*)idx->PatientBirthTime;
  idx->param[RECORDIDX_OtherPatientIDs].PValueField =
      (char*)idx->OtherPatientIDs;
  idx->param[RECORDIDX_OtherPatientNames].PValueField =
      (char*)idx->OtherPatientNames;
  idx->param[RECORDIDX_EthnicGroup].PValueField = (char*)idx->EthnicGroup;
  idx->param[RECORDIDX_NumberofPatientRelatedStudies].PValueField =
      (char*)idx->NumberofPatientRelatedStudies;
  idx->param[RECORDIDX_NumberofPatientRelatedSeries].PValueField =
      (char*)idx->NumberofPatientRelatedSeries;
  idx->param[RECORDIDX_NumberofPatientRelatedInstances].PValueField =
      (char*)idx->NumberofPatientRelatedInstances;
  ...
  idx->param[RECORDIDX_StudyDescription].PValueField =
      (char*)idx->StudyDescription;
  idx->param[RECORDIDX_NameOfPhysiciansReadingStudy].PValueField =
      (char*)idx->NameOfPhysiciansReadingStudy;
  idx->param[RECORDIDX_AccessionNumber].PValueField =
      (char*)idx->AccessionNumber;
  idx->param[RECORDIDX_ReferringPhysicianName].PValueField =
      (char*)idx->ReferringPhysicianName;
  idx->param[RECORDIDX_ProcedureDescription].PValueField =
      (char*)idx->ProcedureDescription;
  idx->param[RECORDIDX_AttendingPhysiciansName].PValueField =
      (char*)idx->AttendingPhysiciansName;
  idx->param[RECORDIDX_StudyInstanceUID].PValueField =
      (char*)idx->StudyInstanceUID;
  idx->param[RECORDIDX_OtherStudyNumbers].PValueField =
      (char*)idx->OtherStudyNumbers;
  idx->param[RECORDIDX_AdmittingDiagnosesDescription].PValueField =
      (char*)idx->AdmittingDiagnosesDescription;
  ...
  idx->param[RECORDIDX_NumberofStudyRelatedSeries].PValueField =
      (char*)idx->NumberofStudyRelatedSeries;
  idx->param[RECORDIDX_NumberofStudyRelatedInstances].PValueField =
      (char*)idx->NumberofStudyRelatedInstances;
  idx->param[RECORDIDX_SeriesNumber].PValueField = (char*)idx->SeriesNumber;
  idx->param[RECORDIDX_SeriesInstanceUID].PValueField =
      (char*)idx->SeriesInstanceUID;
  ...
  idx->param[RECORDIDX_SeriesDescription].PValueField =
      (char*)idx->SeriesDescription;
  ...
  idx->param[RECORDIDX_PerformingPhysicianName].PValueField =
      (char*)idx->PerformingPhysicianName;
  idx->param[RECORDIDX_PresentationLabel].PValueField =
      (char*)idx->PresentationLabel;
}
...
static long DB_lseek(int fildes, long offset, int whence) {
  ...
  */
  ...
  if (curpos < 0) {
    DCMQRDB_ERROR("DB_lseek: cannot get current position: "
  ...
  if (endpos < 0) {
    DCMQRDB_ERROR("DB_lseek: cannot get end of file position: "
                  << OFStandard::strerror(errno, buf, sizeof(buf)));
    return endpos;
  }
  ...
  if (curpos < 0) {
    DCMQRDB_ERROR("DB_lseek: cannot reset current position: "
                  << OFStandard::strerror(errno, buf, sizeof(buf)));
    return curpos;
  }
  ...
  if (pos < 0) {
    DCMQRDB_ERROR("DB_lseek: cannot seek to "
                  << offset << ": "
                  << OFStandard::strerror(errno, buf, sizeof(buf)));
    return pos;
  }
  ...
  */
  ...
  if (pos > maxFileSize) {
    DCMQRDB_ERROR("*** DB ALERT: attempt to seek beyond " << maxFileSize
                                                          << " bytes");
  }
  ...
  if ((endpos > 0) && (pos > endpos)) {
    DCMQRDB_ERROR("*** DB ALERT: attempt to seek beyond end of file"
                  << OFendl << "              offset=" << offset
                  << " filesize=" << endpos);
  }
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::DB_IdxRead(int idx,
  ...
  DB_lseek(handle_->pidx, (long)(SIZEOF_STUDYDESC + idx * SIZEOF_IDXRECORD),
           SEEK_SET);
...
static OFCondition DB_IdxAdd(DB_Private_Handle* phandle, int* idx,
                             IdxRecord* idxRec) {
  IdxRecord rec;
  ...
  *idx = 0;
  ...
  while (read(phandle->pidx, (char*)&rec, SIZEOF_IDXRECORD) ==
         SIZEOF_IDXRECORD) {
    if (rec.filename[0] == '\0')
      break;
    (*idx)++;
  ...
  DB_lseek(phandle->pidx, (long)(SIZEOF_STUDYDESC + (*idx) * SIZEOF_IDXRECORD),
           SEEK_SET);
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::DB_StudyDescChange(
    StudyDescRecord* pStudyDesc) {
  OFCondition cond = EC_Normal;
  DB_lseek(handle_->pidx, 0L, SEEK_SET);
  if (write(handle_->pidx, (char*)pStudyDesc, SIZEOF_STUDYDESC) !=
      SIZEOF_STUDYDESC)
    cond = DcmQRIndexDatabaseError;
  DB_lseek(handle_->pidx, 0L, SEEK_SET);
  return cond;
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::DB_IdxGetNext(
    int* idx, IdxRecord* idxRec) {
  (*idx)++;
  DB_lseek(handle_->pidx, SIZEOF_STUDYDESC + (long)(*idx) * SIZEOF_IDXRECORD,
           SEEK_SET);
  while (read(handle_->pidx, (char*)idxRec, SIZEOF_IDXRECORD) ==
         SIZEOF_IDXRECORD) {
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::DB_GetStudyDesc(
    StudyDescRecord* pStudyDesc) {
  DB_lseek(handle_->pidx, 0L, SEEK_SET);
  if (read(handle_->pidx, (char*)pStudyDesc, SIZEOF_STUDYDESC) ==
      SIZEOF_STUDYDESC)
    return EC_Normal;
...
static double DB_TimeToDouble(char* thetime) {
  ...
  if ((pc = strchr(t, '.')) != NULL) {
    ...
    *pc = '\0';
    for (pc++, f = 1.; (*pc) && (isdigit(OFstatic_cast(unsigned char, *pc)));
         pc++) {
      f /= 10.;
      result += (*pc - '0') * f;
    }
...
static void DB_DuplicateElement(DB_SmallDcmElmt* src, DB_SmallDcmElmt* dst) {
  ...
  if (src->ValueLength == 0)
    ...
    if (dst->PValueField != NULL) {
      memcpy(dst->PValueField, src->PValueField, (size_t)src->ValueLength);
    } else {
...
extern "C" int DB_Compare(const void* ve1, const void* ve2) {
  ...
  if (e1->RecordedDate > e2->RecordedDate)
    return (1);
  else if (e1->RecordedDate == e2->RecordedDate)
    return (0);
  else
    return (-1);
}
...
int DcmQueryRetrieveIndexDatabaseHandle::matchDate(DB_SmallDcmElmt* mod,
                                                   DB_SmallDcmElmt* elt) {
...
int DcmQueryRetrieveIndexDatabaseHandle::matchTime(DB_SmallDcmElmt* mod,
                                                   DB_SmallDcmElmt* elt) {
...
int DcmQueryRetrieveIndexDatabaseHandle::matchUID(DB_SmallDcmElmt* mod,
                                                  DB_SmallDcmElmt* elt) {
  int match;
  char* uid;
  char* modl;
...
int DcmQueryRetrieveIndexDatabaseHandle::matchStrings(DB_SmallDcmElmt* mod,
                                                      DB_SmallDcmElmt* elt) {
  int match;
  char* string;
  char* modl;
...
int DcmQueryRetrieveIndexDatabaseHandle::matchOther(DB_SmallDcmElmt* mod,
                                                    DB_SmallDcmElmt* elt) {
  if (mod->ValueLength != elt->ValueLength)
    return OFFalse;

  return (memcmp(mod->PValueField, elt->PValueField,
                 (size_t)(elt->ValueLength)) == 0);
}
...
int DcmQueryRetrieveIndexDatabaseHandle::dbmatch(DB_SmallDcmElmt* mod,
                                                 DB_SmallDcmElmt* elt) {
...
void DcmQueryRetrieveIndexDatabaseHandle::makeResponseList(
    DB_Private_Handle* phandle, IdxRecord* idxRec) {
  int i;
  ...
  for (pRequestList = phandle->findRequestList; pRequestList;
       pRequestList = pRequestList->next) {
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::testFindRequestList(
    DB_ElementList* findRequestList, DB_LEVEL queryLevel, DB_LEVEL infLevel,
    DB_LEVEL lowestLevel) {
  DB_ElementList* plist;
  DB_LEVEL XTagLevel =
      PATIENT_LEVEL;  // DB_GetTagLevel() will set this correctly
  ...
  ***/
  ...
  if (queryLevel < infLevel) {
    DCMQRDB_INFO("Level incompatible with Information Model (level "
                 << queryLevel << ")");
    return DcmQRIndexDatabaseError;
  }

  if (queryLevel > lowestLevel) {
    DCMQRDB_DEBUG("Level incompatible with Information Model (level "
                  << queryLevel << ")");
    return DcmQRIndexDatabaseError;
  }
    ...
    if ((level == PATIENT_LEVEL) && (infLevel == STUDY_LEVEL)) {
      ...
      */
      ...
      if (atLeastOneKeyFound && (queryLevel != STUDY_LEVEL)) {
        DCMQRDB_DEBUG("Key found in Study Root Information Model (level "
                      << level << ")");
        return DcmQRIndexDatabaseError;
      }
    }
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::hierarchicalCompare(
    DB_Private_Handle* phandle, IdxRecord* idxRec, DB_LEVEL level,
    DB_LEVEL infLevel, int* match) {
  int i;
  DcmTagKey XTag;
  DB_ElementList* plist;
  DB_LEVEL XTagLevel =
  ...
  ***/
  ...
  if (level < phandle->queryLevel) {
    ...
    if (plist == NULL) {
      *match = OFFalse;
      DCMQRDB_WARN("hierarchicalCompare : No UID Key found at level "
                   << (int)level);
      return DcmQRIndexDatabaseError;
    }
    ...
    */
    ...
    return hierarchicalCompare(phandle, idxRec, (DB_LEVEL)(level + 1), infLevel,
                               match);
  }
    ...
    for (plist = phandle->findRequestList; plist; plist = plist->next) {
      ...
      */
      ...
      if ((XTagLevel == PATIENT_LEVEL) &&
          (phandle->queryLevel == STUDY_LEVEL) && (infLevel == STUDY_LEVEL))
        ;
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::startFindRequest(
  ...
  IdxRecord idxRec;
  DB_LEVEL qLevel =
      PATIENT_LEVEL;  // highest legal level for a query in the current model
  DB_LEVEL lLevel =
      IMAGE_LEVEL;  // lowest legal level for a query in the current model
  ...
  ***/
  ...
  if (strcmp(SOPClassUID, UID_FINDPatientRootQueryRetrieveInformationModel) ==
      0)
    handle_->rootLevel = PATIENT_ROOT;
  else if (strcmp(SOPClassUID,
                  UID_FINDStudyRootQueryRetrieveInformationModel) == 0)
  ...
  ***/
  ...
  for (int elemIndex = 0; elemIndex < elemCount; elemIndex++) {
    ...
    if (elem.XTag == DCM_QueryRetrieveLevel || DB_TagSupported(elem.XTag)) {
      ...
      } else if ((elem.PValueField =
                      (char*)malloc((size_t)(elem.ValueLength + 1))) == NULL) {
...
#ifdef DEBUG
          DCMQRDB_DEBUG("DB_startFindRequest () : Illegal query level ("
                        << level << ")");
#endif
  ...
  ***/
  ...
  if (doCheckFindIdentifier) {
    cond = testFindRequestList(handle_->findRequestList, handle_->queryLevel,
                               qLevel, lLevel);
    if (cond != EC_Normal) {
      handle_->idxCounter = -1;
      DB_FreeElementList(handle_->findRequestList);
      ...
      DCMQRDB_DEBUG(
          "DB_startFindRequest () : "
          "STATUS_FIND_Failed_IdentifierDoesNotMatchSOPClass - Invalid "
          "RequestList");
#endif
      status->setStatus(STATUS_FIND_Failed_IdentifierDoesNotMatchSOPClass);
      return (cond);
...
#ifdef DEBUG
    DCMQRDB_DEBUG(
        "DB_startFindRequest () : STATUS_FIND_Failed_UnableToProcess");
#endif
    status->setStatus(STATUS_FIND_Failed_UnableToProcess);
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::nextFindResponse(
  ...
  *findResponseIdentifiers = new DcmDataset;
  if (*findResponseIdentifiers != NULL) {
    ...
    for (plist = handle_->findResponseList; plist != NULL;
         plist = plist->next) {
      ...
      OFCondition ec =
          (*findResponseIdentifiers)->insert(dce, OFTrue /*replaceOld*/);
      if (ec != EC_Normal) {
    ...
    DU_putStringDOElement(*findResponseIdentifiers, DCM_QueryRetrieveLevel,
                          queryLevelString);
#ifdef DEBUG
    DCMQRDB_DEBUG("DB: findResponseIdentifiers:"
                  << OFendl
...
#ifdef DEBUG
    DCMQRDB_DEBUG(
        "DB_nextFindResponse () : STATUS_FIND_Failed_UnableToProcess");
#endif
    status->setStatus(STATUS_FIND_Failed_UnableToProcess);
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::cancelFindRequest(
    DcmQueryRetrieveDatabaseStatus* status) {
  handle_->idxCounter = -1;
  DB_FreeElementList(handle_->findRequestList);
  handle_->findRequestList = NULL;
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::startMoveRequest(
    const char* SOPClassUID, DcmDataset* moveRequestIdentifiers,
    DcmQueryRetrieveDatabaseStatus* status) {
  DB_SmallDcmElmt elem;
  DB_ElementList* plist = NULL;
  DB_ElementList* last = NULL;
  ...
  ***/
  ...
  if (strcmp(SOPClassUID, UID_MOVEPatientRootQueryRetrieveInformationModel) ==
      0)
    handle_->rootLevel = PATIENT_ROOT;
  else if (strcmp(SOPClassUID,
                  UID_MOVEStudyRootQueryRetrieveInformationModel) == 0)
    handle_->rootLevel = STUDY_ROOT;
#ifndef NO_PATIENTSTUDYONLY_SUPPORT
  else if (strcmp(
               SOPClassUID,
  ...
  /* experimental support for GET */
                  ...
                  UID_GETPatientRootQueryRetrieveInformationModel) == 0)
    handle_->rootLevel = PATIENT_ROOT;
  else if (strcmp(SOPClassUID, UID_GETStudyRootQueryRetrieveInformationModel) ==
           0)
    handle_->rootLevel = STUDY_ROOT;
#ifndef NO_PATIENTSTUDYONLY_SUPPORT
  else if (strcmp(
...
#ifdef DEBUG
          DCMQRDB_DEBUG(
              "DB_startMoveRequest : STATUS_MOVE_Failed_UnableToProcess");
#endif
          status->setStatus(STATUS_MOVE_Failed_UnableToProcess);
          return (DcmQRIndexDatabaseError);
        }
  ...
  ***/
  ...
  if (doCheckMoveIdentifier) {
    cond = testMoveRequestList(handle_->findRequestList, handle_->queryLevel,
                               qLevel, lLevel);
    if (cond != EC_Normal) {
      handle_->idxCounter = -1;
      DB_FreeElementList(handle_->findRequestList);
      ...
      DCMQRDB_DEBUG(
          "DB_startMoveRequest () : "
          "STATUS_MOVE_Failed_IdentifierDoesNotMatchSOPClass - Invalid "
          "RequestList");
#endif
      status->setStatus(STATUS_MOVE_Failed_IdentifierDoesNotMatchSOPClass);
      return (cond);
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::nextMoveResponse(
    char* SOPClassUID, char* SOPInstanceUID, char* imageFileName,
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::cancelMoveRequest(
    DcmQueryRetrieveDatabaseStatus* status) {
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::deleteImageFile(
    char* imgFile) {
  if (!quotaSystemEnabled) {
    DCMQRDB_WARN("file delete operations are disabled, keeping file: "
                 << imgFile << " despite duplicate SOP Instance UID");
    return EC_Normal;
  } else {
    DCMQRDB_WARN("Deleting file: "
                 << imgFile << " due to quota or duplicate SOP instance UID");
  }
  ...
  if (unlink(imgFile) < 0) {
    char buf[256];
    /* delete file */
    DCMQRDB_ERROR("DB ERROR: cannot delete image file: "
                  << imgFile << OFendl << "DcmQRIndexDatabaseError: "
                  << OFStandard::strerror(errno, buf, sizeof(buf)));
  }
  ...
  if (dcmtk_flock(lockfd, LOCK_UN) < 0) { /* unlock */
    DCMQRDB_WARN(
        "DB ERROR: cannot unlock image file  for deleting: " << imgFile);
    dcmtk_plockerr("DB ERROR");
  }
...
int DcmQueryRetrieveIndexDatabaseHandle::deleteOldestStudy(
    StudyDescRecord* pStudyDesc) {
  int oldestStudy;
  double OldestDate;
  int s;
  ...
  for (s = 0; s < handle_->maxStudiesAllowed; s++) {
    if ((pStudyDesc[s].NumberofRegistratedImages != 0) &&
        ((OldestDate == 0.0) ||
         (pStudyDesc[s].LastRecordedDate < OldestDate))) {
  ...
  while (DB_IdxRead(idx, &idxRec) == EC_Normal) {
    if (!(strncmp(idxRec.StudyInstanceUID,
                  pStudyDesc[oldestStudy].StudyInstanceUID, n))) {
      DB_IdxRemove(idx);
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::deleteOldestImages(
    StudyDescRecord* pStudyDesc, int StudyNum, char* StudyUID,
    long RequiredSize) {
...
#endif
  n = strlen(StudyUID);
  StudyArray = (ImagesofStudyArray*)malloc(MAX_NUMBER_OF_IMAGES *
                                           sizeof(ImagesofStudyArray));
...
#ifdef DEBUG
  {
    ...
    for (i = 0; i < nbimages; i++)
      DCMQRDB_DEBUG(
          "[" << STD_NAMESPACE setw(2) << i << "] :   Size "
              << StudyArray[i].ImageSize << "   Date " << STD_NAMESPACE setw(20)
              << STD_NAMESPACE setprecision(3) << StudyArray[i].RecordedDate
 ...
 *   If the study UID does not exist, the index of the first unused descriptor
 * entry is returned. If no entries are free, maxStudiesAllowed is returned.
 */

int DcmQueryRetrieveIndexDatabaseHandle::matchStudyUIDInStudyDesc(
    StudyDescRecord* pStudyDesc, char* StudyUID, int maxStudiesAllowed) {
  int s = 0;
  while (s < maxStudiesAllowed) {
    if ((pStudyDesc[s].NumberofRegistratedImages > 0) &&
        (0 == strcmp(pStudyDesc[s].StudyInstanceUID, StudyUID)))
      break;
    s++;
  }
  if (s ==
      maxStudiesAllowed)  // study uid does not exist, look for free descriptor
  {
    s = 0;
    while (s < maxStudiesAllowed) {
      if (pStudyDesc[s].NumberofRegistratedImages == 0)
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::checkupinStudyDesc(
    StudyDescRecord* pStudyDesc, char* StudyUID, long imageSize) {
  int s;
  long RequiredSize;
...
#endif
    if ((pStudyDesc[s].StudySize + imageSize) > handle_->maxBytesPerStudy) {
      if (imageSize > handle_->maxBytesPerStudy) {
#ifdef DEBUG
        DCMQRDB_DEBUG("checkupinStudyDesc: imageSize = " << imageSize
                                                         << " too large");
#endif
        return (DcmQRIndexDatabaseError);
      }
      ...
      RequiredSize =
          imageSize - (handle_->maxBytesPerStudy - pStudyDesc[s].StudySize);
...
#ifdef DEBUG
  DCMQRDB_DEBUG(
      "checkupinStudyDesc: ~~~~~~~~ StudySize = " << pStudyDesc[s].StudySize);
#endif
...
#ifdef DEBUG
      DCMQRDB_DEBUG(
          "--- Removing Existing DB Image Record: " << idxRec.filename);
#endif
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::storeRequest(
    const char* SOPClassUID, const char* /*SOPInstanceUID*/,
    const char* imageFileName, DcmQueryRetrieveDatabaseStatus* status,
    OFBool isNew) {
  IdxRecord idxRec;
  StudyDescRecord* pStudyDesc;
  int i;
...
#ifdef DEBUG
  DCMQRDB_DEBUG(
      "DB_storeRequest () : storage request of file : " << idxRec.filename);
#endif
  ...
  ***/
  ...
  if (dcmff.loadFile(imageFileName).bad()) {
    char buf[256];
    DCMQRDB_WARN("DB: Cannot open file: "
                 << imageFileName << ": "
                 << OFStandard::strerror(errno, buf, sizeof(buf)));
  ...
  for (i = 0; i < NBPARAMETERS; i++) {
    ...
    if ((ec != EC_Normal) || (strPtr == NULL)) {
      ...
      /* important: do not change the ValueLength field before the string is
       * copied! */
  ...
  if (SOPClassUID != NULL) {
    ...
    if (strcmp(SOPClassUID, UID_GrayscaleSoftcopyPresentationStateStorage) ==
        0) {
      descrTag = DCM_ContentDescription;
    } else if (strcmp(SOPClassUID, UID_RETIRED_HardcopyGrayscaleImageStorage) ==
               0) {
      ...
      useDescrTag = OFFalse;
               ...
               (strcmp(SOPClassUID, UID_KeyObjectSelectionDocumentStorage) ==
                0) ||
               ...
               (strcmp(SOPClassUID, UID_SpectaclePrescriptionReportStorage) ==
                0) ||
               (strcmp(SOPClassUID,
                       UID_MacularGridThicknessAndVolumeReportStorage) == 0) ||
               (strcmp(SOPClassUID, UID_ImplantationPlanSRDocumentStorage) ==
                0)) {
      OFString string;
      ...
      if (dset->findAndGetOFString(DCM_CompletionFlagDescription, string) ==
          EC_Normal) {
        description += ", ";
        description += string;
      }
      strncpy(idxRec.InstanceDescription, description.c_str(),
  ...
  if (strlen(idxRec.InstanceDescription) + 9 < DESCRIPTION_MAX_LENGTH) {
    DcmStack stack;
    if (dset->search(DCM_DigitalSignaturesSequence, stack, ESM_fromHere,
                     OFTrue /* searchIntoSub */) == EC_Normal) {
      /* in principle it should be checked whether there is _any_ non-empty
       * digital signatures sequence, but ... */
  ...
  ***/
  ...
  for (i = 0; i < NBPARAMETERS; i++) { /* new definition */
    DB_SmallDcmElmt* se = idxRec.param + i;
    const char* value = "";
    if (se->PValueField != NULL)
      value = se->PValueField;
    DcmTag tag(se->XTag);
  ...
  ***/
  ...
  removeDuplicateImage(idxRec.SOPInstanceUID, idxRec.StudyInstanceUID,
                       pStudyDesc, imageFileName);

  if (checkupinStudyDesc(pStudyDesc, idxRec.StudyInstanceUID,
                         idxRec.ImageSize) != EC_Normal) {
    free(pStudyDesc);
    status->setStatus(STATUS_STORE_Refused_OutOfResources);
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::pruneInvalidRecords() {
  ...
  while (DB_IdxRead(idx, &idxRec) == EC_Normal) {
    if (access(idxRec.filename, R_OK) < 0) {
      ...
      /* update the study info */
      int studyIdx =
          matchStudyUIDInStudyDesc(pStudyDesc, idxRec.StudyInstanceUID,
                                   (int)(handle_->maxStudiesAllowed));
...
void DcmQueryRetrieveIndexDatabaseHandle::printIndexFile(char* storeArea) {
  ...
  for (i = 0; i < handle.handle_->maxStudiesAllowed; i++) {
    if (pStudyDesc[i].NumberofRegistratedImages != 0) {
      COUT << "******************************************************" << OFendl
           ...
           << "  NumOfImages: " << pStudyDesc[i].NumberofRegistratedImages
           << OFendl;
    }
  }
  ...
  while (1) {
    ...
    COUT << "  InstanceDescription: \"" << idxRec.InstanceDescription << "\""
         << OFendl;
  }
  COUT << "*******************************************************" << OFendl
...
void DcmQueryRetrieveIndexDatabaseHandle::setIdentifierChecking(
    OFBool checkFind, OFBool checkMove) {
...
DcmQueryRetrieveIndexDatabaseHandle::DcmQueryRetrieveIndexDatabaseHandle(
    const char* storageArea, long maxStudiesPerStorageArea,
  ...
  if (maxStudiesPerStorageArea > DB_UpperMaxStudies) {
    DCMQRDB_WARN("maxStudiesPerStorageArea too large"
                 << OFendl << "        setting to " << DB_UpperMaxStudies);
  ...
  if (handle_) {
    ...
    sprintf(handle_->indexFilename, "%s%c%s", storageArea, PATH_SEPARATOR,
            DBINDEXFILE);
    ...
    if (f == NULL) {
      char buf[256];
      DCMQRDB_ERROR(handle_->indexFilename
                    << ": " << OFStandard::strerror(errno, buf, sizeof(buf)));
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::makeNewStoreFileName(
  ...
  if (!fnamecreator.makeFilename(seed, handle_->storageArea, prefix, ".dcm",
                                 filename))
    return DcmQRIndexDatabaseError;
...
OFCondition DcmQueryRetrieveIndexDatabaseHandle::instanceReviewed(int idx) {
  ...
  if (result.good() && (record.hstat != DVIF_objectIsNotNew)) {
    ...
    DB_lseek(handle_->pidx,
             OFstatic_cast(long, SIZEOF_STUDYDESC + idx * SIZEOF_IDXRECORD),
             SEEK_SET);
...
IdxRecord::IdxRecord()
    : RecordedDate(0.0), ImageSize(0), hstat(DVIF_objectIsNotNew) {}

DB_SmallDcmElmt::DB_SmallDcmElmt()
    : PValueField(NULL), ValueLength(0), XTag() {}

DcmQueryRetrieveIndexDatabaseHandleFactory::
    DcmQueryRetrieveIndexDatabaseHandleFactory(
        const DcmQueryRetrieveConfig* config)
    : DcmQueryRetrieveDatabaseHandleFactory(), config_(config) {}

DcmQueryRetrieveIndexDatabaseHandleFactory::
    ~DcmQueryRetrieveIndexDatabaseHandleFactory() {}

DcmQueryRetrieveDatabaseHandle*
DcmQueryRetrieveIndexDatabaseHandleFactory::createDBHandle(
    const char* /* callingAETitle */, const char* calledAETitle,
 ...
 * Passing calling aetitle to
 * DcmQueryRetrieveDatabaseHandleFactory::createDBHandle to allow configuration
 * retrieval based on calling aetitle.
 *
