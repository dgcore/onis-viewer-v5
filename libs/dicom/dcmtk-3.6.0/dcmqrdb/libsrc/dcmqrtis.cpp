...
void TI_getInfoFromDataset(DcmDataset* dset, DIC_PN patientName, DIC_CS studyId,
                           DIC_IS seriesNumber, DIC_CS modality,
                           DIC_IS imageNumber) {
...
void TI_getInfoFromImage(char* imgFile, DIC_PN patientName, DIC_CS studyId,
                         DIC_IS seriesNumber, DIC_CS modality,
                         DIC_IS imageNumber) {
  DcmFileFormat dcmff;
  ...
  TI_getInfoFromDataset(obj, patientName, studyId, seriesNumber, modality,
                        imageNumber);
}

void TI_destroyImageEntries(TI_SeriesEntry* series) {
  int i;
...
void TI_destroyStudyEntries(TI_DBEntry* db) {
  int i;
...
static void findCallback(
  ...
  OFString res;
  DCMQRDB_INFO("Find Response "
               << responseCount << ":" << OFendl
...
static OFBool TI_welcome() {
  ...
  printf("\n");
  printf(
      "This program allows you to list the contents of the CTN databases, "
      "send\n");
  printf(
      "images to peer Application Entities (AEs), and to verify connectivity "
      "with\n");
  printf("peer AEs.\n");
  ...
  printf("\n");
  printf(
      "The prompt shows the current database title and the current peer AE "
      "title.\n");
  printf("\n");
...
static void printStudyEntry(TI_StudyEntry* study) {
  printf(STUDYFORMAT, study->patientName, study->patientID, study->studyID);
}

#define SERIESFORMAT "%-6s %-8s %-s\n"

static void printSeriesEntry(TI_SeriesEntry* series) {
  printf(SERIESFORMAT, series->seriesNumber, series->modality,
...
OFBool TI_genericEntryCallback(TI_GenericCallbackStruct* cbs,
                               DcmDataset* reply) {
  if (cbs->db)
    return TI_addStudyEntry(cbs->db, reply);
  if (cbs->study)
    return TI_addSeriesEntry(cbs->study, reply);
  if (cbs->series)
    return TI_addImageEntry(cbs->series, reply);
  return OFFalse;
}
...
OFBool TI_addSeriesEntry(TI_StudyEntry* study, DcmDataset* reply) {
  ...
  ok = DU_getStringDOElement(reply, DCM_SeriesInstanceUID,
                             series->seriesInstanceUID);
  if (ok)
    ok = DU_getStringDOElement(reply, DCM_SeriesNumber, series->seriesNumber);
  if (ok)
    ok = DU_getStringDOElement(reply, DCM_Modality, series->modality);
...
OFBool TI_addImageEntry(TI_SeriesEntry* series, DcmDataset* reply) {
  ...
  /* extract info from reply */
  ok = DU_getStringDOElement(reply, DCM_SOPInstanceUID, image->sopInstanceUID);
  if (ok)
    ok = DU_getStringDOElement(reply, DCM_InstanceNumber, image->imageNumber);
...
OFBool TI_addStudyEntry(TI_DBEntry* db, DcmDataset* reply) {
  ...
  if (ok)
    ok = DU_getStringDOElement(reply, DCM_StudyID, se->studyID);
  if (ok)
    ok = DU_getStringDOElement(reply, DCM_PatientName, se->patientName);
  if (ok)
    ok = DU_getStringDOElement(reply, DCM_PatientID, se->patientID);
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_detachAssociation(OFBool abortFlag) {
  ...
  ASC_getPresentationAddresses(assoc->params, NULL, presentationAddress);
  ...
  if (abortFlag) {
    ...
    if (cond.bad()) {
      DCMQRDB_ERROR(
          "Association Abort Failed: " << DimseCondition::dump(temp_str, cond));
    }
  } else {
    /* release association */
    ...
    if (cond.bad()) {
      DCMQRDB_ERROR("Association Release Failed: " << DimseCondition::dump(
                        temp_str, cond));
    }
  }
...
OFCondition DcmQueryRetrieveTelnetInitiator::addPresentationContexts(
    T_ASC_Parameters* params) {
  OFCondition cond = EC_Normal;

  /* abstract syntaxes for storage SOP classes are taken from dcmdata */
  const char* abstractSyntaxes[] = {
      UID_VerificationSOPClass, UID_FINDStudyRootQueryRetrieveInformationModel};
  ...
  */
  ...
  for (i = 0; i < (int)DIM_OF(abstractSyntaxes) && cond.good(); i++) {
    cond = ASC_addPresentationContext(params, pid, abstractSyntaxes[i],
  ...
  for (i = 0; i < numberOfDcmLongSCUStorageSOPClassUIDs && cond.good(); i++) {
    cond = ASC_addPresentationContext(params, pid,
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_attachAssociation() {
  ...
  if (!config.peerForAETitle(currentPeerTitle, &peer, &port)) {
    DCMQRDB_ERROR("Help, AE title (" << currentPeerTitle
                                     << ") no longer in config");
  ...
  if (cond.bad()) {
    if (cond == DUL_ASSOCIATIONREJECTED) {
      ...
      DCMQRDB_ERROR("Association Rejected:"
                    << OFendl << ASC_printRejectParameters(temp_str, &rej));
      ASC_dropAssociation(assoc);
      ASC_destroyAssociation(&assoc);

      return OFFalse;
    } else {
      DCMQRDB_ERROR("Association Request Failed: Peer ("
  ...
  if (ASC_countAcceptedPresentationContexts(params) == 0) {
    DCMQRDB_ERROR("All Presentation Contexts Refused: Peer ("
                  << presentationAddress << "," << currentPeerTitle << ")");
  ...
  DCMQRDB_INFO("Association Accepted (Max Send PDV: " << assoc->sendPDVLength
                                                      << ")");
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_sendEcho() {
  ...
  cond = DIMSE_echoUser(assoc, msgId, blockMode_, dimse_timeout_, &status,
                        &stDetail);
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_storeImage(char* sopClass,
                                                      char* sopInstance,
                                                      char* imgFile) {
  OFCondition cond = EC_Normal;
  DIC_US msgId;
  ...
  TI_getInfoFromImage(imgFile, patientName, studyId, seriesNumber, modality,
                      imageNumber);
  ...
  cond =
      DIMSE_storeUser(assoc, presId, &req, imgFile, NULL, storeProgressCallback,
                      NULL, blockMode_, dimse_timeout_, &rsp, &stDetail);
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_remoteFindQuery(
  ...
  DCMQRDB_INFO("Sending Find SCU RQ: MsgID " << msgId << ":" << OFendl
  ...
  cond = DIMSE_findUser(assoc, presId, &req, query, findCallback, &cbd,
                        blockMode_, dimse_timeout_, &rsp, &stDetail);
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_shortHelp(int /*arg*/,
                                                     const char* /*cmdbuf*/) {
  printf(
      "h)elp, t)itle, da)tabase, st)udy, ser)ies i)mage, di)splay, sen)d, "
      "e)cho, q)uit\n");
  return OFTrue;
}

OFBool DcmQueryRetrieveTelnetInitiator::TI_help(int arg,
                                                const char* /*cmdbuf*/) {
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_attachDB(TI_DBEntry* db) {
  ...
  if (!db->isRemoteDB && db->dbHandle == NULL) {
    ...
    db->dbHandle = new DcmQueryRetrieveIndexDatabaseHandle(
        config.getStorageArea(db->title), config.getMaxStudies(db->title),
  ...
  if (arg < 0) {
    ...
    if (arg >= dbCount) {
      ...
      for (i = 0; !found && i < db->peerTitleCount; i++) {
        found = (strcmp(currentPeerTitle, db->peerTitles[i]) == 0);
      }
      if (!found) {
        printf(
            "WARNING: Actual Peer AE Title (%s) has no access to database: "
            "%s\n",
            currentPeerTitle, db->title);
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_quit(int arg,
                                                const char* /*cmdbuf*/) {
  if (DCM_dcmqrdbGetLogger().isEnabledFor(OFLogger::INFO_LOG_LEVEL)) {
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_sendStudy(int arg,
  ...
  */
  ...
  if (strcmp(db->title, currentPeerTitle) == 0) {
    printf(
        "Sorry, cannot send images from a DB to itself, possible deadlock\n");
    return OFFalse;
  }
  ...
  while (ok && dbStatus.status() == STATUS_Pending) {
    dbcond = db->dbHandle->nextMoveResponse(sopClass, sopInstance, imgFile,
                                            &nRemaining, &dbStatus);
    if (dbcond.bad()) {
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_sendImage(int arg,
  ...
  DU_putStringDOElement(query, DCM_SeriesInstanceUID,
                        series->seriesInstanceUID);
  DU_putStringDOElement(query, DCM_SOPInstanceUID, image->sopInstanceUID);
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_send(int /*arg*/,
                                                const char* cmdbuf) {
  OFBool ok = OFTrue;
  char cmdarg[128];
  int iarg;
  ...
  if (strncmp("st", cmdarg, strlen("st")) == 0)
    TI_sendStudy(iarg, cmdbuf);
  else if (strncmp("se", cmdarg, strlen("se")) == 0)
    TI_sendSeries(iarg, cmdbuf);
  else if (strncmp("i", cmdarg, strlen("i")) == 0)
    TI_sendImage(iarg, cmdbuf);
  else {
...
void DcmQueryRetrieveTelnetInitiator::TI_userInput() {
  ...
  while (1) {
    printf("%s->%s> ", dbEntries[currentdb]->title, currentPeerTitle);
    ...
    /* find command parser */
    if (strncmp("h", cmdBuf, strlen("h")) == 0)
      TI_help(arg, cmdBuf);
    else if (strncmp("?", cmdBuf, strlen("?")) == 0)
      TI_shortHelp(arg, cmdBuf);
    else if (strncmp("t", cmdBuf, strlen("t")) == 0)
      TI_title(arg, cmdBuf);
    else if (strncmp("da", cmdBuf, strlen("da")) == 0)
      TI_database(arg, cmdBuf);
    else if (strncmp("st", cmdBuf, strlen("st")) == 0)
      TI_study(arg, cmdBuf);
    else if (strncmp("ser", cmdBuf, strlen("ser")) == 0)
      TI_series(arg, cmdBuf);
    else if (strncmp("i", cmdBuf, strlen("i")) == 0)
      TI_image(arg, cmdBuf);
    else if (strncmp("send", cmdBuf, strlen("send")) == 0)
      TI_send(arg, cmdBuf);
    else if (strncmp("ec", cmdBuf, strlen("ec")) == 0)
      TI_echo(arg, cmdBuf);
    else if (strncmp("q", cmdBuf, strlen("q")) == 0)
      TI_quit(arg, cmdBuf);
    else if (strncmp("exit", cmdBuf, strlen("exit")) == 0)
      TI_quit(arg, cmdBuf);
    else {
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_dbReadable(const char* dbTitle) {
  ...
  sprintf(path, "%s%c%s", config.getStorageArea(dbTitle), PATH_SEPARATOR,
          DBINDEXFILE);
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_buildStudies(TI_DBEntry* db) {
  ...
  if (db->studyCount != 0 && TI_dbModifyTime(db->title) < db->lastQueryTime) {
    /* nothing has changed */
    return OFTrue;
  }
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_buildRemoteSeries(
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_buildSeries(TI_DBEntry* db,
                                                       TI_StudyEntry* study) {
  OFCondition dbcond = EC_Normal;
  DcmQueryRetrieveDatabaseStatus dbStatus(STATUS_Pending);
  DcmDataset* query = NULL;
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_buildRemoteImages(
    TI_DBEntry* db, TI_StudyEntry* study, TI_SeriesEntry* series) {
  TI_GenericCallbackStruct cbs;
  DcmDataset* query = NULL;
  OFBool ok = OFTrue;
...
OFBool DcmQueryRetrieveTelnetInitiator::TI_buildImages(TI_DBEntry* db,
                                                       TI_StudyEntry* study,
                                                       TI_SeriesEntry* series) {
  OFCondition dbcond = EC_Normal;
  DcmQueryRetrieveDatabaseStatus dbStatus(STATUS_Pending);
...
OFBool DcmQueryRetrieveTelnetInitiator::addPeerName(
    const char* peerName, const char* configFileName) {
  int k;
  OFBool found = OFFalse;
  const char** aeTitles;
  ...
  if (config.aeTitlesForPeer(peerName, &aeTitles) <= 0) {
    DCMQRDB_ERROR("no AE titles defined (in: " << configFileName
                                               << ") for peer: " << peerName);
    return (OFFalse);
  }
...
OFBool DcmQueryRetrieveTelnetInitiator::findDBPeerTitles(
    const char* configFileName, TI_DBEntry* dbEntry, const char* peer) {
  ...
  for (i = 0; i < peerTitleCount; i++) {
    if (config.peerInAETitle(dbEntry->title, peerTitles[i], peer)) {
      ...
      if (dbEntry->peerTitles == NULL)
        ...
        dbEntry->peerTitles = (const char**)realloc(
            dbEntry->peerTitles,
            (dbEntry->peerTitleCount + 1) * sizeof(const char*));
    ...
    if (!TI_dbReadable(ctnTitles[i]))
      ...
      if (dbEntry->peerTitleCount > 0) {
        ...
        if (dbEntries == NULL)
          ...
          dbEntries = (TI_DBEntry**)realloc(
              dbEntries, (dbCount + 1) * sizeof(TI_DBEntry*));
  ...
  for (i = 0; i < remoteDBTitlesCount; i++) {
    ...
    if (config.peerForAETitle(remoteDBTitles[i], &peerName, &portNumber)) {
      ...
      if (dbEntries == NULL)
        ...
        dbEntries = (TI_DBEntry**)realloc(dbEntries,
                                          (dbCount + 1) * sizeof(TI_DBEntry*));
      ...
      for (j = 0; j < peerNamesCount; j++) {
        ...
        if (peerTitleCount <= 0)
          DCMQRDB_ERROR("no AE titles defined (in: "
                        << configFileName << ") for peer: " << peerNames[j]);
