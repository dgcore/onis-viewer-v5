...
/** this class describes configuration settings for a list of symbolic hosts or
 * vendors
 */
...
public:
   ...
   *     0 - not found
   */
  int peerForAETitle(const char* AETitle, const char** HostName,
   ...
   *     0 -- no
   */
  int peerInAETitle(const char* calledAETitle, const char* callingAETitle,
   ...
   *      0 if no entries found.
   */
  int aeTitlesForSymbolicName(const char* symbolicName,
...
private:
  ...
  DcmQueryRetrieveConfigConfiguration
      CNF_Config;                              /* configuration file contents */
