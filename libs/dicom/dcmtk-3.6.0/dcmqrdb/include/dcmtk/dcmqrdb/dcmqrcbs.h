...
public:
   ...
   *  @param ff pointer to DcmFileformat object to be used for storing the
   * dataset
   *  @param correctuidpadding flag indicating whether space padded UIDs should
   * be silently corrected
   */
  DcmQueryRetrieveStoreContext(DcmQueryRetrieveDatabaseHandle& handle,
                               const DcmQueryRetrieveOptions& options, DIC_US s,
                               DcmFileFormat* ff, OFBool correctuidpadding)
      : dbHandle(handle),
        options_(options),
        status(s),
        fileName(NULL),
   ...
   */
  void callbackHandler(T_DIMSE_StoreProgress* progress, T_DIMSE_C_StoreRQ* req,
...
private:
  ...
  void saveImageToDB(T_DIMSE_C_StoreRQ* req, /* original store request */
                     const char* imageFileName,
                     /* out */
                     T_DIMSE_C_StoreRSP* rsp, /* final store response */
                     DcmDataset** stDetail);

  void writeToFile(DcmFileFormat* ff, const char* fname,
