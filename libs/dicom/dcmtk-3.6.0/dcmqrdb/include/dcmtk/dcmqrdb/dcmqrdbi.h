...
enum DVIFhierarchyStatus {
  ...
  DVIF_objectIsNew,
  /// object (study or series) in the database is not new but contains new
  /// subobjects
  DVIF_objectContainsNewSubobjects
};
...
/** This class maintains database handles based on the classical "index.dat"
 * file. A database handle maintains a connection to a database and encapsulates
 * database support for store, find and move/get operations.
 */
class DcmQueryRetrieveIndexDatabaseHandle
    : public DcmQueryRetrieveDatabaseHandle {
public:
   ...
   *  @param maxStudiesPerStorageArea maximum number of studies for this storage
   * area, needed to correctly parse the index file.
   *  @param maxBytesPerStudy maximum number of bytes per study, for quota
   * mechanism
   *  @param result upon successful initialization of the database handle,
   *    EC_Normal is returned in this parameter, otherwise an error code is
   * returned.
   */
  DcmQueryRetrieveIndexDatabaseHandle(const char* storageArea,
   ...
   *    Memory must be provided by the caller and should be at least
   * MAXPATHLEN+1 characters. The file name generated should be an absolute file
   * name.
   *  @return EC_Normal upon normal completion, or some other OFCondition code
   * upon failure.
   */
  OFCondition makeNewStoreFileName(const char* SOPClassUID,
                                   const char* SOPInstanceUID,
                                   char* newImageFileName);

  /** the given DICOM object, which has been received through a C-STORE
   ...
   *  @return EC_Normal upon normal completion, or some other OFCondition code
   upon failure.
   */
  OFCondition storeRequest(const char* SOPClassUID, const char* SOPInstanceUID,
   ...
   *  @param findRequestIdentifiers dataset containing request identifiers
   * (i.e., the query) The caller retains responsibility for destroying the
   *    findRequestIdentifiers when no longer needed.
   *  @param status pointer to DB status object in which a DIMSE status code
   *    suitable for use with the C-FIND-RSP message is set. Status will be
   *    PENDING if any FIND responses will be generated or SUCCESS if no FIND
   * responses will be generated (SUCCESS indicates the completion of a
   * operation), or another status code upon failure.
   *  @return EC_Normal upon normal completion, or some other OFCondition code
   * upon failure.
   */
  OFCondition startFindRequest(const char* SOPClassUID,
   ...
   *  @param moveRequestIdentifiers dataset containing request identifiers
   * (i.e., the query) The caller retains responsibility for destroying the
   *    moveRequestIdentifiers when no longer needed.
   *  @param status pointer to DB status object in which a DIMSE status code
   *    suitable for use with the C-MOVE-RSP message is set. Status will be
   *    PENDING if any MOVE responses will be generated or SUCCESS if no MOVE
   * responses will be generated (SUCCESS indicates the completion of a
   * operation), or another status code upon failure.
   *  @return EC_Normal upon normal completion, or some other OFCondition code
   * upon failure.
   */
  OFCondition startMoveRequest(const char* SOPClassUID,
   ...
   *    which the SOP class UID for the next DICOM object to be transferred is
   * copied.
   *  @param SOPInstanceUID pointer to string of at least 65 characters into
   *    which the SOP instance UID for the next DICOM object to be transferred
   * is copied.
   *  @param imageFileName pointer to string of at least MAXPATHLEN+1 characters
   * into which the file path for the next DICOM object to be transferred is
   * copied.
   *  @param numberOfRemainingSubOperations On return, this parameter will
   * contain the number of suboperations still remaining for the request (this
   * number is needed by move responses with PENDING status).
   *  @param status pointer to DB status object in which a DIMSE status code
   *    suitable for use with the C-MOVE-RSP message is set. Status will be
   ...
   * upon failure.
   */
  OFCondition nextMoveResponse(char* SOPClassUID, char* SOPInstanceUID,
                               char* imageFileName,
   ...
   *  to be deleted if certain boundaries (number of studies, bytes per study)
   * are exceeded.
   */
  ...
  /** Get next Index record that is in use (i.e. references a non-empty a
   * filename)
...
private:
  OFCondition removeDuplicateImage(const char* SOPInstanceUID,
                                   const char* StudyInstanceUID,
                                   StudyDescRecord* pStudyDesc,
                                   const char* newImageFileName);
  int deleteOldestStudy(StudyDescRecord* pStudyDesc);
  OFCondition deleteOldestImages(StudyDescRecord* pStudyDesc, int StudyNum,
                                 char* StudyUID, long RequiredSize);
  ...
  int matchStudyUIDInStudyDesc(StudyDescRecord* pStudyDesc, char* StudyUID,
                               int maxStudiesAllowed);
  OFCondition checkupinStudyDesc(StudyDescRecord* pStudyDesc, char* StudyUID,
                                 long imageSize);

  OFCondition hierarchicalCompare(DB_Private_Handle* phandle, IdxRecord* idxRec,
                                  DB_LEVEL level, DB_LEVEL infLevel,
                                  int* match);

  OFCondition testFindRequestList(DB_ElementList* findRequestList,
                                  DB_LEVEL queryLevel, DB_LEVEL infLevel,
                                  DB_LEVEL lowestLevel);

  OFCondition testMoveRequestList(DB_ElementList* findRequestList,
                                  DB_LEVEL queryLevel, DB_LEVEL infLevel,
                                  DB_LEVEL lowestLevel);
  ...
  /// flag indicating whether or not the check function for FIND requests is
  /// enabled
  OFBool doCheckFindIdentifier;

  /// flag indicating whether or not the check function for MOVE requests is
  /// enabled
...
/** Index database factory class. Instances of this class are able to create
 * database handles for a given called application entity title.
 */
class DcmQueryRetrieveIndexDatabaseHandleFactory
    : public DcmQueryRetrieveDatabaseHandleFactory {
public:
  /** constructor
   ...
   *  @return pointer to database object, must not be NULL if result is
   * EC_Normal.
   */
