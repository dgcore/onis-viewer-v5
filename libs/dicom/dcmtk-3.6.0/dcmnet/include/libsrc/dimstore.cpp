...
static void privateUserCallback(void* callbackData, unsigned long bytes) {
...
OFCondition DIMSE_storeUser(
    ...
    T_DIMSE_C_StoreRQ* request, const char* imageFileName,
    DcmDataset* imageDataSet, DIMSE_StoreUserCallback callback,
    void* callbackData, T_DIMSE_BlockingMode blockMode, int timeout,
    T_DIMSE_C_StoreRSP* response, DcmDataset** statusDetail,
    T_DIMSE_DetectedCancelParameters* checkForCancelParams,
    long imageFileTotalBytes)
/*
 * This function transmits data from a file or a dataset to an SCP. The
 * transmission is conducted via network and using DIMSE C-STORE messages.
 * Additionally, this function evaluates C-STORE-Response messages which were
 * received from the SCP.
 *
 * Parameters:
 *   assoc                - [in] The association (network connection to SCP).
 *   presId               - [in] The ID of the presentation context which shall
 * be used request              - [in] Represents a DIMSE C-Store Request
 * Message. Contains corresponding information, e.g. message ID, affected SOP
 * class UID, etc. imageFileName        - [in] The name of the file which is
 * currently processed. imageDataSet         - [in] The data set which is
 * currently processed. callback             - [in] Pointer to a function which
 * shall be called to indicate progress. callbackData         - [in] Pointer to
 * data which shall be passed to the progress indicating function blockMode -
 * [in] The blocking mode for receiving data (either DIMSE_BLOCKING or
 * DIMSE_NONBLOCKING) timeout              - [in] Timeout interval for receiving
 * data. If the blocking mode is DIMSE_NONBLOCKING response             - [out]
 * Represents a DIMSE C-Store Response Message. Contains corresponding
 *                                information, e.g. message ID being responded
 * to, affected SOP class UID, etc. This variable contains in the end the
 * C-STORE-RSP command which was received as a response to the C-STORE-RQ which
 * was sent. statusDetail         - [out] If a non-NULL value is passed this
 * variable will in the end contain detailed information with regard to the
 * status information which is captured in the status element (0000,0900) of the
 * response message. Note that the value for element (0000,0900) is not
 * contained in this return value but in response. checkForCancelParams - [out]
 * Indicates, if a C-Cancel (Request) Message was encountered. Contains
 * corresponding information, e.g. a boolean value if a corresponding message
 * was encountered and the C-Cancel (Request) Message itself (in case it
 * actually was encountered). imageFileTotalBytes  - [in] The size of the file
 * which is currently processed in bytes.
 */
{
  ...
  /* initialize the variables which represent DIMSE C-STORE request and DIMSE
   * C-STORE response messages */
  ...
  if (callback != NULL) {
    ...
    if (imageFileTotalBytes > 0)
      ...
      if (imageFileName != NULL)
        ...
        progress.totalBytes =
            dcmGuessModalityBytes(request->AffectedSOPClassUID);
    }
  ...
  if (imageFileName != NULL) {
    cond = DIMSE_sendMessageUsingFileData(
        assoc, presId, &req, NULL, imageFileName, privCallback, &callbackCtx);
  } else {
    cond = DIMSE_sendMessageUsingMemoryData(
        assoc, presId, &req, NULL, imageDataSet, privCallback, &callbackCtx);
  }
  ...
  do {
    ...
    cond = DIMSE_receiveCommand(assoc, blockMode, timeout, &thisPresId, &rsp,
                                statusDetail);
    if (cond != EC_Normal)
      return cond;

    /* if everything was successful so far, the rsp variable contains the
     * command which */
    /* was received check if we encountered a C-CANCEL-RQ; if so, set some
     * variables */
    if (checkForCancelParams != NULL && rsp.CommandField == DIMSE_C_CANCEL_RQ) {
      ...
      if (rsp.CommandField != DIMSE_C_STORE_RSP) {
        char buf[256];
        sprintf(buf, "DIMSE: Unexpected Response Command Field: 0x%x",
                (unsigned)rsp.CommandField);
        return makeDcmnetCondition(DIMSEC_UNEXPECTEDRESPONSE, OF_error, buf);
      }

      /* if we get to here, we received a C-STORE-RSP; store this message in the
       * reference parameter */
      *response = rsp.msg.CStoreRSP;  // BoundsChecker warning !?

      /* check if the response relates to the request which was sent earlier; if
       * not, return an error */
      if (response->MessageIDBeingRespondedTo != request->MessageID) {
        char buf2[256];
        sprintf(buf2, "DIMSE: Unexpected Response MsgId: %d (expected: %d)",
                response->MessageIDBeingRespondedTo, request->MessageID);
        return makeDcmnetCondition(DIMSEC_UNEXPECTEDRESPONSE, OF_error, buf2);
      }
    }
  } while (checkForCancelParams != NULL &&
           rsp.CommandField == DIMSE_C_CANCEL_RQ);

  /* return result value */
  return EC_Normal;
}

OFCondition DIMSE_sendStoreResponse(T_ASC_Association* assoc,
                                    T_ASC_PresentationContextID presID,
                                    T_DIMSE_C_StoreRQ* request,
                                    T_DIMSE_C_StoreRSP* response,
                                    DcmDataset* statusDetail)
/*
 * This function takes care of sending a C-STORE-RSP message over the network to
 * the DICOM application this application is connected with.
 *
 * Parameters:
 ...
 * was specified in the PDV which contained the DIMSE C-STORE-RQ command.
 *   request      - [in] The DIMSE C-STORE-RQ command which was received
 * earlier. response     - [inout] The C-STORE-RSP command which shall be sent.
 * Might be modified. statusDetail - [in] Detailed information with regard to
 * the status information which is captured in the status element (0000,0900).
 * Note that the value for element (0000,0900) is contained in this variable.
 */
{
  OFCondition cond = EC_Normal;
  T_DIMSE_Message rsp;
...
static void privateProviderCallback(void* callbackData, unsigned long bytes) {
...
OFCondition DIMSE_storeProvider(
    T_ASC_Association* assoc, T_ASC_PresentationContextID presIdCmd,
    T_DIMSE_C_StoreRQ* request, const char* imageFileName, int writeMetaheader,
    DcmDataset** imageDataSet, DIMSE_StoreProviderCallback callback,
    void* callbackData, T_DIMSE_BlockingMode blockMode, int timeout)
/*
 * This function receives a data set over the network and either stores this
 * data in a file (exactly as it was received) or it stores this data in memory.
 * Before, during and after the process of receiving data, the callback function
 * which was provided by the caller (if it was provided) will be called to
 * indicate progress.
 *
 * Parameters:
 *   assoc           - [in] The association (network connection to another DICOM
 * application). presIDCmd       - [in] The ID of the presentation context which
 * was specified in the PDV which contained the DIMSE command. request         -
 * [in] The DIMSE C-STORE-RQ message that was received. imageFileName   - [in]
 * If this variable does not equal NULL, the information (which was received
 * over the network) will be written to a file exactly as it was received over
 * the network. In such a case, this this variable contains the name of the file
 * the information shall be written to. writeMetaheader - [in] Specifies if the
 * resulting file shall only contain the dataset which was received (OFFalse) or
 * if it shall contain both metaheader and dataset information (OFTrue) (i.e if
 * the file will be written according to the DICOM file format). imageDataSet -
 * [inout] If this variable does not equal NULL, and at the same time
 * imageFileName equals NULL, this variable will in the end contain the
 * information which was received over the network. Note that this function
 * assumes that either imageFileName or imageDataSet does not equal NULL.
 *   callback        - [in] Pointer to a function which shall be called to
 * indicate progress. callbackData    - [in] Pointer to data which shall be
 * passed to the progress indicating function blockMode       - [in] The
 * blocking mode for receiving data (either DIMSE_BLOCKING or DIMSE_NONBLOCKING)
 *   timeout         - [in] Timeout interval for receiving data (if the blocking
 * mode is DIMSE_NONBLOCKING).
 */
{
  OFCondition cond = EC_Normal;
  ...
  if (callback != NULL) {
    ...
    callbackCtx.callback = callback;
    /* execute initial callback */
    callback(callbackData, &progress, request, (char*)imageFileName,
             imageDataSet, &response, &statusDetail);
  } else {
    privCallback = NULL;
  }

  /* in the following, we want to receive data over the network and do something
   * with this data. If the */
  /* imageFileName does not equal NULL, the caller required that the data shall
   * be written to a file */
  /* exactly the way it was received over the network. Hence, a filestream will
   * be created and the data */
  /* set will be received and written to the file through the call to
   * DIMSE_receiveDataSetInFile(...).*/
  /* If the imageFileName does equal NULL but at the same time imageDataSet does
   * not equal NULL, the */
  /* data shall be received and stored in memory. This will be handled through
   * the call to function */
  /* DIMSE_receiveDataSetInMemory(...). The case in which both variables are
   * NULL is considered to */
  /* be an error and will be handled correspondingly. */
  if (imageFileName != NULL) {
    /* create filestream */
    DcmOutputFileStream* filestream = NULL;
    cond = DIMSE_createFilestream(imageFileName, request, assoc, presIdCmd,
                                  writeMetaheader, &filestream);
    if (cond.bad()) {
      /* We cannot create the filestream, so ignore the incoming dataset and
       * return an out-of-resources error to the SCU */
      DIC_UL bytesRead = 0;
      DIC_UL pdvCount = 0;
      cond =
          DIMSE_ignoreDataSet(assoc, blockMode, timeout, &bytesRead, &pdvCount);
      if (cond.good()) {
      ...
      cond = DIMSE_receiveDataSetInFile(assoc, blockMode, timeout, &presIdData,
                                        filestream, privCallback, &callbackCtx);
      delete filestream;
      if (cond != EC_Normal) {
    ...
    cond =
        DIMSE_receiveDataSetInMemory(assoc, blockMode, timeout, &presIdData,
                                     imageDataSet, privCallback, &callbackCtx);
  } else {
    /* if both variables are set to NULL, report an error */
    return DIMSE_BADDATA;
  }

  /* check if presentation context IDs of the command (which was received
   * earlier) and of the data */
  /* set (which was received just now) differ from each other. If this is the
   * case, return an error. */
  if (cond.good() && (presIdData != presIdCmd)) {
    cond = makeDcmnetCondition(
        DIMSEC_INVALIDPRESENTATIONCONTEXTID, OF_error,
        "DIMSE: Presentation Contexts of Command and Data Differ");
  }

  /* depending on the error status, set the success indicating flag in the
   * response message */
  if (cond == EC_Normal) {
   ...
   */
  OFCondition cond2 = DIMSE_sendStoreResponse(assoc, presIdCmd, request,
