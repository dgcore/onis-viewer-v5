...
static OFCondition parseSubItem(DUL_SUBITEM* subItem, unsigned char* buf,
                                unsigned long* itemLength,
                                unsigned long availData);
static OFCondition parsePresentationContext(
    unsigned char type, PRV_PRESENTATIONCONTEXTITEM* context,
    unsigned char* buf, unsigned long* itemLength, unsigned long availData);
static OFCondition parseUserInfo(DUL_USERINFO* userInfo, unsigned char* buf,
                                 unsigned long* itemLength,
                                 unsigned char typeRQorAC,
                                 unsigned long availData);
static OFCondition parseMaxPDU(DUL_MAXLENGTH* max, unsigned char* buf,
                               unsigned long* itemLength,
                               unsigned long availData);
static OFCondition parseDummy(unsigned char* buf, unsigned long* itemLength,
                              unsigned long availData);
static OFCondition parseSCUSCPRole(PRV_SCUSCPROLE* role, unsigned char* buf,
                                   unsigned long* length,
                                   unsigned long availData);
static void trim_trailing_spaces(char* s);

static OFCondition parseExtNeg(SOPClassExtendedNegotiationSubItem* extNeg,
                               unsigned char* buf, unsigned long* length,
                               unsigned long availData);

static OFCondition makeLengthError(const char* pdu, unsigned long bufSize,
...
OFCondition parseAssociate(unsigned char* buf, unsigned long pduLength,
  ...
  *context;
  ...
  if ((assoc->protocol & DUL_PROTOCOL) == 0) {
    char buffer[256];
    sprintf(buffer, "DUL Unsupported peer protocol %04x; expected %04x in %s",
            assoc->protocol, DUL_PROTOCOL, "parseAssociate");
  ...
  if (DCM_dcmnetGetLogger().isEnabledFor(OFLogger::DEBUG_LOG_LEVEL)) {
    ...
    */
    DCMNET_TRACE("PDU type: "
                 ...
                 << STD_NAMESPACE dec << " (" << s
                 << "), PDU Length: " << assoc->length << OFendl
                 << "DICOM Protocol: " << STD_NAMESPACE hex << assoc->protocol
  ...
  while ((cond.good()) && (pduLength > 0)) {
    type = *buf;
    DCMNET_TRACE("Parsing remaining "
                 << pduLength << " bytes of A-ASSOCIATE PDU" << OFendl
                 << "Next item type: " << STD_NAMESPACE hex
                 << STD_NAMESPACE setfill('0') << STD_NAMESPACE setw(2)
                 << ((unsigned int)type));
    switch (type) {
      ...
      case DUL_TYPEPRESENTATIONCONTEXTAC:
        context = (PRV_PRESENTATIONCONTEXTITEM*)malloc(
...
static OFCondition parseSubItem(DUL_SUBITEM* subItem, unsigned char* buf,
  ...
  if (subItem->length > DICOM_UI_LENGTH) {
    char buffer[256];
    sprintf(buffer,
            "DUL illegal subitem length %d. Maximum allowed size is %d.",
  ...
  *itemLength = 2 + 2 + subItem->length;
  ...
  DCMNET_TRACE("Subitem parse: Type "
               ...
               << STD_NAMESPACE setw(2) << ((unsigned int)subItem->type)
               << STD_NAMESPACE dec << ", Length " << STD_NAMESPACE setw(4)
               << (int)subItem->length << ", Content: " << subItem->data);
  return EC_Normal;
}
...
static OFCondition parsePresentationContext(
  ...
  *itemLength = 2 + 2 + length;
  ...
  DCMNET_TRACE("Parsing Presentation Context: ("
               << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
               << STD_NAMESPACE setw(2) << (unsigned int)context->type
               << STD_NAMESPACE dec
               << "), Length: " << (unsigned long)context->length << OFendl
               << "Presentation Context ID: " << STD_NAMESPACE hex
               << STD_NAMESPACE setfill('0') << STD_NAMESPACE setw(2)
               << (unsigned int)context->contextID << STD_NAMESPACE dec);
  ...
  if (!((type == DUL_TYPEPRESENTATIONCONTEXTAC) &&
    ...
    while (presentationLength > 0) {
      DCMNET_TRACE("Parsing remaining "
                   << presentationLength << " bytes of Presentation Context"
                   << OFendl << "Next item type: " << STD_NAMESPACE hex
                   << STD_NAMESPACE setfill('0') << STD_NAMESPACE setw(2)
                   << (unsigned int)*buf);
      switch (*buf) {
...
static OFCondition parseUserInfo(DUL_USERINFO* userInfo, unsigned char* buf,
  ...
  *itemLength = userLength + 4;
  ...
  DCMNET_TRACE("Parsing user info field ("
               << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
               << STD_NAMESPACE setw(2) << (unsigned int)userInfo->type
               << STD_NAMESPACE dec
               << "), Length: " << (unsigned long)userInfo->length);
  while (userLength > 0) {
    DCMNET_TRACE("Parsing remaining "
                 << (long)userLength << " bytes of User Information" << OFendl
                 << "Next item type: " << STD_NAMESPACE hex
                 << STD_NAMESPACE setfill('0') << STD_NAMESPACE setw(2)
                 << (unsigned int)*buf);
    switch (*buf) {
      ...
      case DUL_TYPEIMPLEMENTATIONCLASSUID:
        cond = parseSubItem(&userInfo->implementationClassUID, buf, &length,
                            userLength);
        if (cond.bad())
          return cond;
        buf += length;
      ...
      case DUL_TYPEIMPLEMENTATIONVERSIONNAME:
        cond = parseSubItem(&userInfo->implementationVersionName, buf, &length,
                            userLength);
        if (cond.bad())
          return cond;
        buf += length;
      ...
      case DUL_TYPENEGOTIATIONOFUSERIDENTITY:
        ...
        if (usrIdent == NULL)
          return EC_MemoryExhausted;
        cond =
            usrIdent->parseFromBuffer(buf, length /*return value*/, userLength);
        if (cond.bad()) {
          delete usrIdent;
          return cond;
        }
...
static OFCondition parseMaxPDU(DUL_MAXLENGTH* max, unsigned char* buf,
  ...
  *itemLength = 2 + 2 + max->length;
  ...
  if (max->length != 4)
    DCMNET_WARN("Invalid length (" << max->length
                                   << ") for maximum length item, must be 4");
...
static OFCondition parseSCUSCPRole(PRV_SCUSCPROLE* role, unsigned char* buf,
  ...
  *length = 2 + 2 + role->length;
  ...
  DCMNET_TRACE("Subitem parse: Type "
               << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
               << STD_NAMESPACE setw(2) << (unsigned int)role->type
               << STD_NAMESPACE dec << ", Length " << STD_NAMESPACE setw(4)
               << (int)role->length << ", Content: " << role->SOPClassUID << " "
               << (int)role->SCURole << " " << (int)role->SCPRole);
  return EC_Normal;
}
...
static OFCondition parseExtNeg(SOPClassExtendedNegotiationSubItem* extNeg,
  ...
  if (availData - 4 < extNeg->itemLength)
    return makeLengthError("extended negotiation", availData, 0,
                           extNeg->itemLength);
  ...
  if (extNeg->itemLength - 2 < extNeg->sopClassUIDLength)
    return makeLengthError("extended negotiation item", extNeg->itemLength, 0,
  ...
  *length = 2 + 2 + extNeg->itemLength;
  ...
  if (DCM_dcmnetGetLogger().isEnabledFor(OFLogger::TRACE_LOG_LEVEL)) {
    DCMNET_TRACE("ExtNeg Subitem parse: Type "
                 ...
                 << OFstatic_cast(unsigned int, extNeg->itemType)
                 << STD_NAMESPACE dec << ", Length " << STD_NAMESPACE setw(4)
                 << (int)extNeg->itemLength
    ...
    for (int j = 0; j < extNeg->serviceClassAppInfoLength; j++) {
      str << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
          << STD_NAMESPACE setw(2) << extNeg->serviceClassAppInfo[j]
          << STD_NAMESPACE dec << " ";
    }
...
static OFCondition makeLengthError(const char* pdu, unsigned long bufSize,
  ...
  if (length != 0)
    stream << " with a length field of " << length
           << " (data before length field is not included in length field)";
  if (minSize != 0)
