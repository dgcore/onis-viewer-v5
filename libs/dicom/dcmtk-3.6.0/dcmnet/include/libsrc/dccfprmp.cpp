...
const char* DcmProfileEntry::getRoleSelectionKey() const {
  if (roleSelectionGroup_.size() == 0)
    return NULL;
  else
    return roleSelectionGroup_.c_str();
}

const char* DcmProfileEntry::getExtendedNegotiationKey() const {
  if (extendedNegotiationGroup_.size() == 0)
    return NULL;
  else
    return extendedNegotiationGroup_.c_str();
}

/* ========================================================= */

DcmProfileMap::DcmProfileMap() : map_() {}
...
OFCondition DcmProfileMap::add(const char* key,
  ...
  OFString roleKey;
  if (roleSelectionKey)
    roleKey = roleSelectionKey;
  OFString extnegKey;
  if (extendedNegotiationKey)
    extnegKey = extendedNegotiationKey;

  OFString skey(key);
  DcmProfileEntry* const* value =
      OFconst_cast(DcmProfileEntry* const*, map_.lookup(skey));
  if (value == NULL) {
    DcmProfileEntry* newentry =
        new DcmProfileEntry(presKey, roleKey, extnegKey);
...
const char* DcmProfileMap::getPresentationContextKey(const char* key) const {
  if (key) {
    DcmProfileEntry* const* entry =
        OFconst_cast(DcmProfileEntry* const*, map_.lookup(OFString(key)));
    if (entry) {
