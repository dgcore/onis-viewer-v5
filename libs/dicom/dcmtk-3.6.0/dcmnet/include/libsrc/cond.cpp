...
const OFConditionConst ASCE_BADPRESENTATIONCONTEXTID(
    OFM_dcmnet, ASCC_BADPRESENTATIONCONTEXTID, OF_error,
    "ASC Bad presentation context ID");
const OFConditionConst ASCE_MISSINGTRANSFERSYNTAX(
    OFM_dcmnet, ASCC_MISSINGTRANSFERSYNTAX, OF_error,
    "ASC Missing transfer syntax");
const OFConditionConst ASCE_NULLKEY(OFM_dcmnet, ASCC_NULLKEY, OF_error,
                                    "ASC Caller passed in a NULL key");
const OFConditionConst ASCE_SHUTDOWNAPPLICATION(
    OFM_dcmnet, ASCC_SHUTDOWNAPPLICATION, OF_error,
    "ASC Application shutdown requested");
const OFConditionConst ASCE_USERIDENTIFICATIONFAILED(
    OFM_dcmnet, ASCC_USERIDENTIFICATIONFAILED, OF_error,
    "ASC User Identification Negotiation failed");
const OFConditionConst ASCE_SCPSCUROLESELECTIONFAILED(
    OFM_dcmnet, ASCC_SCPSCUROLESELECTIONFAILED, OF_error,
...
const OFConditionConst DIMSEE_BADCOMMANDTYPE(OFM_dcmnet, DIMSEC_BADCOMMANDTYPE,
                                             OF_error,
                                             "DIMSE Bad command type");
const OFConditionConst DIMSEE_BADDATA(OFM_dcmnet, DIMSEC_BADDATA, OF_error,
                                      "DIMSE Inappropriate data for message");
const OFConditionConst DIMSEE_BADMESSAGE(OFM_dcmnet, DIMSEC_BADMESSAGE,
                                         OF_error,
                                         "DIMSE Badly formed message");
const OFConditionConst DIMSEE_ILLEGALASSOCIATION(
    OFM_dcmnet, DIMSEC_ILLEGALASSOCIATION, OF_error,
    "DIMSE Caller passed in an illegal association");
const OFConditionConst DIMSEE_NODATAAVAILABLE(
    OFM_dcmnet, DIMSEC_NODATAAVAILABLE, OF_error,
    "DIMSE No data available (timeout in non-blocking mode)");
const OFConditionConst DIMSEE_NOVALIDPRESENTATIONCONTEXTID(
    OFM_dcmnet, DIMSEC_NOVALIDPRESENTATIONCONTEXTID, OF_error,
    "DIMSE No valid Presentation Context ID");
const OFConditionConst DIMSEE_NULLKEY(OFM_dcmnet, DIMSEC_NULLKEY, OF_error,
                                      "DIMSE Caller passed in a NULL key");
const OFConditionConst DIMSEE_OUTOFRESOURCES(OFM_dcmnet, DIMSEC_OUTOFRESOURCES,
                                             OF_error,
                                             "DIMSE Out of resources");
const OFConditionConst DIMSEE_PARSEFAILED(
    OFM_dcmnet, DIMSEC_PARSEFAILED, OF_error,
    "DIMSE Failed to parse received message");
const OFConditionConst DIMSEE_READPDVFAILED(OFM_dcmnet, DIMSEC_READPDVFAILED,
                                            OF_error, "DIMSE Read PDV failed");
const OFConditionConst DIMSEE_RECEIVEFAILED(OFM_dcmnet, DIMSEC_RECEIVEFAILED,
                                            OF_error,
                                            "DIMSE Failed to receive message");
const OFConditionConst DIMSEE_SENDFAILED(OFM_dcmnet, DIMSEC_SENDFAILED,
                                         OF_error,
                                         "DIMSE Failed to send message");
const OFConditionConst DIMSEE_UNEXPECTEDPDVTYPE(OFM_dcmnet,
                                                DIMSEC_UNEXPECTEDPDVTYPE,
                                                OF_error,
                                                "DIMSE Unexpected PDV type");
const OFConditionConst DIMSEE_NODATADICT(OFM_dcmnet, DIMSEC_NODATADICT,
                                         OF_error, "Data dictionary missing");
...
const OFCondition DIMSE_NOVALIDPRESENTATIONCONTEXTID(
    DIMSEE_NOVALIDPRESENTATIONCONTEXTID);
...
const OFConditionConst DULE_ASSOCIATIONREJECTED(OFM_dcmnet,
                                                DULC_ASSOCIATIONREJECTED,
                                                OF_error,
                                                "DUL Association Rejected");
const OFConditionConst DULE_ILLEGALACCEPT(OFM_dcmnet, DULC_ILLEGALACCEPT,
                                          OF_error,
                                          "Attempt to accept by requestor");
const OFConditionConst DULE_ILLEGALKEY(OFM_dcmnet, DULC_ILLEGALKEY, OF_error,
                                       "Illegal key");
const OFConditionConst DULE_ILLEGALPDU(OFM_dcmnet, DULC_ILLEGALPDU, OF_error,
                                       "DUL Illegal or ill-formed PDU");
const OFConditionConst DULE_ILLEGALPDULENGTH(OFM_dcmnet, DULC_ILLEGALPDULENGTH,
                                             OF_error,
                                             "DUL Illegal PDU Length");
const OFConditionConst DULE_ILLEGALREQUEST(OFM_dcmnet, DULC_ILLEGALREQUEST,
                                           OF_error,
                                           "Attempt to request by acceptor");
const OFConditionConst DULE_INCORRECTBUFFERLENGTH(
    OFM_dcmnet, DULC_INCORRECTBUFFERLENGTH, OF_error,
    "DUL Incorrect buffer space allocated for data");
const OFConditionConst DULE_INSUFFICIENTBUFFERLENGTH(
    OFM_dcmnet, DULC_INSUFFICIENTBUFFERLENGTH, OF_error,
    "DUL Insufficient buffer space allocated for data");
const OFConditionConst DULE_LISTERROR(OFM_dcmnet, DULC_LISTERROR, OF_error,
                                      "DUL List error");
const OFConditionConst DULE_NETWORKCLOSED(OFM_dcmnet, DULC_NETWORKCLOSED,
                                          OF_error, "DUL network closed");
const OFConditionConst DULE_NOASSOCIATIONREQUEST(
    OFM_dcmnet, DULC_NOASSOCIATIONREQUEST, OF_error,
    "No requests for associations for this server");
const OFConditionConst DULE_NOPDVS(OFM_dcmnet, DULC_NOPDVS, OF_error,
                                   "DUL No PDVs available in current buffer");
const OFConditionConst DULE_NULLKEY(OFM_dcmnet, DULC_NULLKEY, OF_error,
                                    "NULL key passed to routine");
const OFConditionConst DULE_PCTRANSLATIONFAILURE(
    OFM_dcmnet, DULC_PCTRANSLATIONFAILURE, OF_error,
    "DUL Presentation Context translation failure");
const OFConditionConst DULE_PDATAPDUARRIVED(OFM_dcmnet, DULC_PDATAPDUARRIVED,
                                            OF_error, "DUL P-Data PDU arrived");
const OFConditionConst DULE_PEERABORTEDASSOCIATION(
    OFM_dcmnet, DULC_PEERABORTEDASSOCIATION, OF_error,
    "Peer aborted Association (or never connected)");
const OFConditionConst DULE_PEERREQUESTEDRELEASE(OFM_dcmnet,
                                                 DULC_PEERREQUESTEDRELEASE,
                                                 OF_error,
                                                 "DUL Peer Requested Release");
const OFConditionConst DULE_READTIMEOUT(OFM_dcmnet, DULC_READTIMEOUT, OF_error,
                                        "DUL network read timeout");
const OFConditionConst DULE_REQUESTASSOCIATIONFAILED(
    OFM_dcmnet, DULC_REQUESTASSOCIATIONFAILED, OF_error,
    "Failed to establish association");
const OFConditionConst DULE_UNEXPECTEDPDU(OFM_dcmnet, DULC_UNEXPECTEDPDU,
                                          OF_error, "Received unexpected PDU");
const OFConditionConst DULE_UNSUPPORTEDPEERPROTOCOL(
    OFM_dcmnet, DULC_UNSUPPORTEDPEERPROTOCOL, OF_error,
    "DUL Unsupported peer protocol");
const OFConditionConst DULE_WRONGDATATYPE(
    OFM_dcmnet, DULC_WRONGDATATYPE, OF_error,
...
OFCondition DimseCondition::push(unsigned short aModule, unsigned short aCode,
                                 OFStatus aStatus, const char* aText,
                                 OFCondition subCondition) {
  OFOStringStream os;
  /* declare variable outside the block structure of the OFSTRINGSTREAM_xxx
   * macros */
...
OFCondition DimseCondition::push(OFCondition newCondition,
                                 OFCondition subCondition) {
  return DimseCondition::push(newCondition.module(), newCondition.code(),
                              newCondition.status(), newCondition.text(),
                              subCondition);
}
 ...
 * condition stack.  This is a major change, caveat emptor!
 *
 *
 */
