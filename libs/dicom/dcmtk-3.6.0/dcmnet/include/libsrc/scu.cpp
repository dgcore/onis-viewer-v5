...
DcmSCU::DcmSCU()
  ...
  WSAStartup(winSockVersionNeeded,
             &winSockData);  // TODO: check with multiple SCU instances whether
                             // this is harmful
#endif
}
...
#ifdef HAVE_WINSOCK_H
  WSACleanup();  // TODO: check with multiple SCU instances whether this is
                 // harmful
#endif
}
...
OFCondition DcmSCU::initNetwork() {
  ...
  OFCondition cond =
      ASC_initializeNetwork(NET_REQUESTOR, 0, m_acseTimeout, &m_net);
  if (cond.bad()) {
  ...
  /* sets this application's title and the called application's title in the
   * params */
  ...
  if ((m_peer.length() + 5 /* max 65535 */) + 1 /* for ":" */ > 63) {
    DCMNET_ERROR("Maximum length of peer host name '"
                 << m_peer << "' is longer than maximum of 57 characters");
    return EC_IllegalCall;  // TODO: need to find better error code
  }
  if (strlen(localHost) + 1 > 63) {
    DCMNET_ERROR("Maximum length of local host name '"
                 << localHost << " is longer than maximum of 62 characters");
    return EC_IllegalCall;  // TODO: need to find better error code
  }
  ...
  if (m_assocConfigFilename.length() != 0) {
    ...
    result = DcmAssociationConfigurationFile::initialize(
        assocConfig, m_assocConfigFilename.c_str());
    if (result.bad()) {
      DCMNET_WARN("Unable to parse association configuration file "
                  << m_assocConfigFilename << " (ignored): " << result.text());
      return result;
    } else {
      /* perform name mangling for config file key */
      OFString profileName;
      const unsigned char* c = OFreinterpret_cast(const unsigned char*,
                                                  m_assocConfigProfile.c_str());
      while (*c) {
        if (!isspace(*c))
      ...
      result =
          assocConfig.setAssociationParameters(profileName.c_str(), *m_params);
      if (result.bad()) {
        DCMNET_WARN("Unable to apply association configuration file"
                    << m_assocConfigFilename
                    << " (ignored): " << result.text());
        return result;
      }
  ...
  if (numContexts <= 127) {
    ...
    nextFreePresID =
        2 * numContexts + 1; /* add 1 to point to the next free ID*/
  }
  // Print warning if number of overall presenation contexts exceeds 128
  if ((numContexts + m_presContexts.size()) > 128) {
    DCMNET_WARN("Number of presentation contexts exceeds 128 ("
                << numContexts + m_presContexts.size()
                << "). Some contexts will not be negotiated");
  } else {
    DCMNET_TRACE("Configured " << numContexts
                               << " presentation contexts from config file");
    if (m_presContexts.size() > 0)
      DCMNET_TRACE("Adding another "
                   << m_presContexts.size()
                   << " presentation contexts configured for SCU");
  }
  ...
  DCMNET_DEBUG("Configured a total of " << numContexts
                                        << " presentation contexts for SCU");
...
OFCondition DcmSCU::negotiateAssociation() {
  ...
  if (m_verbosePCMode)
    DCMNET_INFO("Request Parameters:"
                << OFendl
                << ASC_dumpParameters(tempStr, m_params, ASC_ASSOC_RQ));
  else
  ...
  if (cond.bad()) {
    if (cond == DUL_ASSOCIATIONREJECTED) {
      ...
      DCMNET_DEBUG("Association Rejected:"
                   << OFendl << ASC_printRejectParameters(tempStr, &rej));
      return cond;
    } else {
      DCMNET_DEBUG("Association Request Failed: " << DimseCondition::dump(
                       tempStr, cond));
      return cond;
    }
  }
  ...
  /* dump general information concerning the establishment of the network
   * connection if required */
  DCMNET_INFO("Association Accepted (Max Send PDV: "
              << OFstatic_cast(unsigned long, m_assoc->sendPDVLength) << ")");
  return EC_Normal;
}
...
OFCondition DcmSCU::useSecureConnection(DcmTransportLayer* tlayer) {
  OFCondition cond = ASC_setTransportLayer(
      m_net, tlayer, OFFalse /* do not take over ownership */);
  if (cond.good())
...
T_ASC_PresentationContextID DcmSCU::findPresentationContextID(
  ...
  while (pc && !found) {
    ...
    if (!transferSyntax.empty())  // ignore transfer syntax if not specified
      found &=
          (strcmp(pc->acceptedTransferSyntax, transferSyntax.c_str()) == 0);
    if (!found)
...
void DcmSCU::closeAssociation(const DcmCloseAssociationType closeType) {
  ...
  switch (closeType) {
    case DCMSCU_RELEASE_ASSOCIATION:
      ...
      if (cond.bad()) {
        DCMNET_ERROR("Association Release Failed: " << DimseCondition::dump(
                         tempStr, cond));
    ...
    case DCMSCU_ABORT_ASSOCIATION:
      ...
      if (cond.bad()) {
        DCMNET_ERROR("Association Abort Failed: " << DimseCondition::dump(
                         tempStr, cond));
      }
      break;
   ...
   * This */
  /* call is the counterpart of ASC_requestAssociation(...) which was called
   * above. */
  cond = ASC_destroyAssociation(&m_assoc);
  if (cond.bad()) {
    DCMNET_ERROR("Unable to clean up internal association structures: "
                 << DimseCondition::dump(tempStr, cond));
  }
  ...
  if (cond.bad()) {
    DCMNET_ERROR("Unable to clean up internal network structures: "
                 << DimseCondition::dump(tempStr, cond));
  }
}
...
OFCondition DcmSCU::sendECHORequest(const T_ASC_PresentationContextID presID) {
  ...
  if (pcid == 0)
    pcid = findPresentationContextID(UID_VerificationSOPClass,
  ...
  if (pcid == 0) {
    DCMNET_ERROR(
        "No presentation context found for sending C-ECHO with SOP Class / "
        "Transfer Syntax: "
  ...
  Uint16 status;
  cond = DIMSE_echoUser(m_assoc, nextMessageID(), m_blockMode, m_dimseTimeout,
                        &status, NULL);
  if (cond.bad()) {
    OFString tempStr;
    DCMNET_ERROR("Failed sending C-ECHO request or receiving response: "
                 << DimseCondition::dump(tempStr, cond));
    return cond;
  } else {
    if (status == STATUS_Success)
      ...
      return makeOFCondition(
          OFM_dcmnet, 22, OF_error,
          "SCP returned non-success status in C-ECHO response");
    }
  }
  return EC_Normal;
}
...
OFCondition DcmSCU::sendSTORERequest(
    const T_ASC_PresentationContextID presID, const OFString& dicomFile,
    DcmDataset* dset, DcmDataset*& /* rspCommandSet */,  // TODO
  ...
  E_TransferSyntax transferSyntax =
      EXS_Unknown;  // Initialized in getDatasetInfo()
  ...
  if ((dset == NULL) || sopClass.empty() || sopInstance.empty() ||
  ...
  /* If necessary, find appropriate presentation context */
  if (pcid == 0)
    pcid = findPresentationContextID(sopClass,
                                     DcmXfer(transferSyntax).getXferID());
  if (pcid == 0) {
    OFString sopname = dcmFindNameOfUID(sopClass.c_str(), sopClass.c_str());
    OFString tsname = DcmXfer(transferSyntax).getXferName();
    DCMNET_ERROR(
        "No presentation context found for sending C-STORE with SOP Class / "
        "Transfer Syntax: "
        << sopname << "/"
  ...
  cond = sendDIMSEMessage(pcid, &msg, dset, NULL /* callback */,
                          NULL /* callbackContext */);
  delete dcmff;
  dcmff = NULL;
  if (cond.bad()) {
    DCMNET_ERROR("Failed sending C-STORE request: " << DimseCondition::dump(
                     tempStr, cond));
    return cond;
  }
  ...
  T_DIMSE_Message rsp;
  cond = receiveDIMSECommand(&pcid, &rsp, &statusDetail,
                             NULL /* not interested in the command set */);
  if (cond.bad()) {
    DCMNET_ERROR("Failed receiving DIMSE response: " << DimseCondition::dump(
                     tempStr, cond));
    return cond;
  }
  ...
  if (statusDetail != NULL) {
    DCMNET_DEBUG("Response has status detail:"
                 << OFendl << DcmObject::PrintHelper(*statusDetail));
    delete statusDetail;
  }
...
OFCondition DcmSCU::sendFINDRequest(const T_ASC_PresentationContextID presID,
  ...
  if (abstractSyntax.empty() || transferSyntax.empty())
    return DIMSE_NOVALIDPRESENTATIONCONTEXTID;
  OFStandard::strlcpy(req->AffectedSOPClassUID, abstractSyntax.c_str(),
                      sizeof(req->AffectedSOPClassUID));

  DCMNET_INFO("Send C-FIND Request");
  DCMNET_DEBUG(DIMSE_dumpMessage(tempStr, msg, DIMSE_OUTGOING, NULL, pcid));
  cond = sendDIMSEMessage(pcid, &msg, queryKeys, NULL /* callback */,
                          NULL /* callbackContext */);
  if (cond.bad()) {
    DCMNET_ERROR("Failed sending C-FIND request: " << DimseCondition::dump(
                     tempStr, cond));
    return cond;
  }
  ...
  while (waitForNextResponse) {
    ...
    findrsp->m_messageIDRespondedTo =
        rsp.msg.CFindRSP.MessageIDBeingRespondedTo;
    ...
    if (DICOM_PENDING_STATUS(findrsp->m_status)) {
      ...
      if (rsp.msg.CFindRSP.DataSetType == DIMSE_DATASET_NULL) {
        DCMNET_ERROR(
            "Received C-FIND response with PENDING status but no dataset "
            "announced, aborting");
        delete findrsp;
      ...
      // Receive dataset
      cond = receiveDIMSEDataset(&pcid, &rspDataset, NULL /* callback */,
                                 NULL /* callbackContext */);
      if (cond.bad()) {
      ...
      DCMNET_DEBUG("Received dataset on presentation context "
                   << OFstatic_cast(unsigned int, pcid));
    ...
    if (cond.bad()) {
      DCMNET_WARN("Unable to handle C-FIND response correctly: "
                  << cond.text() << " (ignored)");
      delete findrsp;
      cond = EC_Normal;
...
OFCondition DcmSCU::handleFINDResponse(Uint16 /* presContextID */,
  ...
  switch (response->m_status) {
    ...
    case STATUS_Success:
      ...
      DCMNET_DEBUG(
          "Received final C-FIND response, no more C-FIND responses expected");
      break;
    default:
...
OFCondition DcmSCU::sendACTIONRequest(const T_ASC_PresentationContextID presID,
  ...
  if (abstractSyntax.empty() || transferSyntax.empty())
    return DIMSE_NOVALIDPRESENTATIONCONTEXTID;
  OFStandard::strlcpy(actionReq.RequestedSOPClassUID, abstractSyntax.c_str(),
                      sizeof(actionReq.RequestedSOPClassUID));
  OFStandard::strlcpy(actionReq.RequestedSOPInstanceUID, sopInstanceUID.c_str(),
                      sizeof(actionReq.RequestedSOPInstanceUID));
  ...
  if (DCM_dcmnetGetLogger().isEnabledFor(OFLogger::TRACE_LOG_LEVEL))
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, request, DIMSE_OUTGOING, reqDataset, pcid));
  else
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, request, DIMSE_OUTGOING, NULL, pcid));
  cond = sendDIMSEMessage(pcid, &request, reqDataset, NULL /* callback */,
                          NULL /* callbackContext */);
  if (cond.bad()) {
    DCMNET_ERROR("Failed sending N-ACTION request: " << DimseCondition::dump(
                     tempStr, cond));
    return cond;
  }
  ...
  cond = receiveDIMSECommand(&pcid, &response, &statusDetail,
                             NULL /* commandSet */);
  if (cond.bad()) {
    DCMNET_ERROR("Failed receiving DIMSE response: " << DimseCondition::dump(
                     tempStr, cond));
  ...
  if (response.CommandField == DIMSE_N_ACTION_RSP) {
    ...
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, response, DIMSE_INCOMING, NULL, pcid));
  } else {
  ...
  if (actionRsp.DataSetType == DIMSE_DATASET_PRESENT) {
    ...
    cond = receiveDIMSEDataset(&tempID, &tempDataset, NULL /* callback */,
                               NULL /* callbackContext */);
    if (cond.good()) {
      DCMNET_WARN(
          "Received unexpected dataset after N-ACTION response, ignoring");
      delete tempDataset;
    } else {
      DCMNET_ERROR(
          "Failed receiving unexpected dataset after N-ACTION response: "
          << DimseCondition::dump(tempStr, cond));
      return DIMSE_BADDATA;
    }
  }
  if (actionRsp.MessageIDBeingRespondedTo != actionReq.MessageID) {
    DCMNET_ERROR("Received response with wrong message ID ("
                 << actionRsp.MessageIDBeingRespondedTo << " instead of "
                 << actionReq.MessageID << ")");
    return DIMSE_BADMESSAGE;
  }
...
OFCondition DcmSCU::handleEVENTREPORTRequest(DcmDataset*& reqDataset,
  ...
  if (timeout > 0)
    DCMNET_DEBUG("Handle N-EVENT-REPORT request, waiting up to "
                 << timeout << " seconds (only for N-EVENT-REPORT message)");
    ...
    DCMNET_DEBUG(
        "Handle N-EVENT-REPORT request, waiting an unlimited period of time");

  // Receive request, use specific timeout (if defined)
  cond = receiveDIMSECommand(&presID, &request, &statusDetail,
                             NULL /* commandSet */, timeout);
  if (cond.bad()) {
    if (cond != DIMSE_NODATAAVAILABLE)
      DCMNET_ERROR("Failed receiving DIMSE request: " << DimseCondition::dump(
                       tempStr, cond));
    return cond;
  }
  ...
  if (request.CommandField == DIMSE_N_EVENT_REPORT_RQ) {
    ...
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, request, DIMSE_INCOMING, NULL, presID));
  ...
  if (statusDetail != NULL) {
    DCMNET_DEBUG("Request has status detail:"
                 << OFendl << DcmObject::PrintHelper(*statusDetail));
    delete statusDetail;
  }
  ...
  if (DCM_dcmnetGetLogger().isEnabledFor(OFLogger::TRACE_LOG_LEVEL))
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, request, DIMSE_INCOMING, dataset, presID));
  else
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, request, DIMSE_INCOMING, NULL, presID));
...
Uint16 DcmSCU::checkEVENTREPORTRequest(
    T_DIMSE_N_EventReportRQ& /*eventReportReq*/, DcmDataset* /*reqDataset*/) {
...
OFCondition DcmSCU::getDatasetInfo(DcmDataset* dataset, OFString& sopClassUID,
