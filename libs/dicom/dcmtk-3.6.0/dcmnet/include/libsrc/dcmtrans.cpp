...
OFBool DcmTransportConnection::safeSelectReadableAssociation(
    DcmTransportConnection* connections[], int connCount, int timeout) {
  int numberOfRounds = timeout + 1;
  if (numberOfRounds < 0)
    numberOfRounds = 0xFFFF; /* a long time */
  ...
  while ((numberOfRounds > 0) && (!found)) {
    ...
    if (timeToWait == 1)
      return OFFalse; /* all entries NULL */
    numberOfRounds--;
  } /* while */
...
OFBool DcmTransportConnection::fastSelectReadableAssociation(
    DcmTransportConnection* connections[], int connCount, int timeout) {
  int socketfd = -1;
  int maxsocketfd = -1;
  int i = 0;
...
#endif
  if (nfound <= 0)
    return OFFalse; /* none available for reading */
...
OFBool DcmTransportConnection::selectReadableAssociation(
    DcmTransportConnection* connections[], int connCount, int timeout) {
  OFBool canUseFastMode = OFTrue;
  for (int i = 0; i < connCount; i++) {
    if ((connections[i]) && (!connections[i]->isTransparentConnection()))
      canUseFastMode = OFFalse;
  }
  if (canUseFastMode)
    return fastSelectReadableAssociation(connections, connCount, timeout);
...
DcmTransportLayerStatus DcmTCPConnection::renegotiate(
    const char* /* newSuite */) {
  return TCS_ok;
}
...
unsigned long DcmTCPConnection::getPeerCertificate(void* /* buf */,
...
OFBool DcmTCPConnection::networkDataAvailable(int timeout) {
  ...
  if (nfound <= 0)
    ...
    if (FD_ISSET(getSocket(), &fdset))
      return OFTrue;
    else
      return OFFalse; /* This should not really happen */
  }
}
 ...
 *  Fixed issue with missing invalidation of socket variable during close
 * method. Please note that this is only required if the connection objects
 * exists after the TCP/IP connection has been closed (which is currently not
 * the case).
 *
