...
OFBool DU_findSOPClassAndInstanceInDataSet(DcmItem* obj, char* sopClass,
                                           char* sopInstance,
  ...
  if (tolerateSpacePaddedUIDs) {
    ...
    if ((0 < (slength = strlen(sopInstance))) &&
        (sopInstance[slength - 1] == ' '))
      sopInstance[slength - 1] = 0;
  }
  return result;
}

OFBool DU_findSOPClassAndInstanceInFile(const char* fname, char* sopClass,
                                        char* sopInstance,
                                        OFBool tolerateSpacePaddedUIDs) {
  DcmFileFormat ff;
