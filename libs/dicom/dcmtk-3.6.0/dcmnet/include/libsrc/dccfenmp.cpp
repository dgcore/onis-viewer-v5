...
DcmExtendedNegotiationItem::DcmExtendedNegotiationItem(
    const DcmUIDHandler& abstractSyntax, const unsigned char* data,
    Uint32 length)
    : raw_(NULL), length_(0), uid_(abstractSyntax) {
  if (length && data) {
    length_ = length;
...
DcmExtendedNegotiationItem::DcmExtendedNegotiationItem(
    const DcmExtendedNegotiationItem& arg)
    : raw_(NULL), length_(arg.length_), uid_(arg.uid_) {
  if (length_) {
    raw_ = new unsigned char[length_];
...
OFBool DcmExtendedNegotiationItem::operator==(
    const DcmExtendedNegotiationItem& arg) const {
  return (uid_ == arg.uid_) && (length_ == arg.length_) &&
         ((length_ == 0) ||
          (memcmp(raw_, arg.raw_, OFstatic_cast(size_t, length_)) == 0));
}

/* ========================================================= */

DcmExtendedNegotiationMap::DcmExtendedNegotiationMap() : map_() {}

DcmExtendedNegotiationMap::~DcmExtendedNegotiationMap() {
  OFListIterator(DcmKeyValuePair<DcmExtendedNegotiationList*>*) first =
      map_.begin();
  OFListIterator(DcmKeyValuePair<DcmExtendedNegotiationList*>*) last =
      map_.end();
  while (first != last) {
    delete (*first)->value();
    ++first;
...
OFCondition DcmExtendedNegotiationMap::add(const char* key,
                                           ...
                                           Uint32 length) {
  if ((!key) || (!abstractSyntaxUID) || (!rawData) || (!length))
  ...
  OFString skey(key);
  DcmExtendedNegotiationList* const* value =
      OFconst_cast(DcmExtendedNegotiationList* const*, map_.lookup(skey));
  if (value == NULL) {
...
OFBool DcmExtendedNegotiationMap::isKnownKey(const char* key) const {
  if (!key)
    return OFFalse;
  if (map_.lookup(OFString(key)))
    return OFTrue;
  return OFFalse;
}

OFCondition DcmExtendedNegotiationMap::checkConsistency(
    const char* key, const DcmPresentationContextMap& pclist,
    const char* pckey) const {
  if ((!key) || (!pckey))
    return EC_IllegalCall;

  DcmExtendedNegotiationList* const* entry = OFconst_cast(
      DcmExtendedNegotiationList* const*, map_.lookup(OFString(key)));
  if (!entry) {
    // error: key undefined
...
const DcmExtendedNegotiationList*
DcmExtendedNegotiationMap::getExtendedNegotiationList(const char* key) const {
  const DcmExtendedNegotiationList* result = NULL;
  if (key) {
    DcmExtendedNegotiationList* const* value = OFconst_cast(
        DcmExtendedNegotiationList* const*, map_.lookup(OFString(key)));
    if (value)
      result = *value;
  }
