...
static int selectReadable(T_ASC_Association* assoc, T_ASC_Network* net,
...
OFCondition DIMSE_moveUser(
    ...
    /* out */
    T_DIMSE_C_MoveRSP* response, DcmDataset** statusDetail, DcmDataset** rspIds,
      ...
      case STATUS_Pending:
        ...
        if (response->DataSetType != DIMSE_DATASET_NULL) {
          ...
          if (!ignorePendingDatasets) {
            // Some systems send an (illegal) dataset following C-MOVE-RSP
            // messages with pending status, which is a protocol violation, but
            // we need to handle this nevertheless. The MV300 has been reported
            // to exhibit this behavior.
            DCMNET_WARN(DIMSE_warn_str(assoc)
                        << "Reading but ignoring response identifier set");
            DcmDataset* tempset = NULL;
            cond = DIMSE_receiveDataSetInMemory(assoc, blockMode, timeout,
                                                &presID, &tempset, NULL, NULL);
            delete tempset;
            if (cond != EC_Normal) {
              return cond;
            }
  ...
  switch (response->DimseStatus) {
    ...
    default:
      ...
      if (rspIds == NULL)
        ...
        rsp.msg.CMoveRSP.DataSetType = DIMSE_DATASET_PRESENT;
      break;
  }
...
OFCondition DIMSE_moveProvider(
  ...
  cond = DIMSE_receiveDataSetInMemory(assoc, blockMode, timeout, &presIdData,
                                      &reqIds, NULL, NULL);
    ...
    if (presIdData != presIdCmd) {
      ...
      while (cond == EC_Normal && rsp.DimseStatus == STATUS_Pending && normal) {
        ...
        if (normal) {
          ...
          cond = DIMSE_sendMoveResponse(assoc, presIdCmd, request, &rsp, rspIds,
                                        statusDetail);
