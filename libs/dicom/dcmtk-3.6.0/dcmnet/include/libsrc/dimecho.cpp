...
OFCondition DIMSE_echoUser(
  ...
  if (presID == 0) {
    ...
    return makeDcmnetCondition(DIMSEC_NOVALIDPRESENTATIONCONTEXTID, OF_error,
                               buf);
  }
  ...
  OFCondition cond = DIMSE_sendMessageUsingMemoryData(assoc, presID, &req, NULL,
                                                      NULL, NULL, NULL);
  if (cond.bad())
    return cond;
  ...
  if (rsp.msg.CEchoRSP.MessageIDBeingRespondedTo != msgId) {
    char buf1[256];
    sprintf(buf1, "DIMSE: Unexpected Response MsgId: %d (expected: %d)",
...
OFCondition DIMSE_sendEchoResponse(T_ASC_Association* assoc,
                                   T_ASC_PresentationContextID presID,
                                   T_DIMSE_C_EchoRQ* req, DIC_US status,
  ...
  strcpy(rsp.msg.CEchoRSP.AffectedSOPClassUID, req->AffectedSOPClassUID);
