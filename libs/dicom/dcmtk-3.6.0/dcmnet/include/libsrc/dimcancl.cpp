...
OFCondition DIMSE_sendCancelRequest(T_ASC_Association* assoc,
  ...
  return DIMSE_sendMessageUsingMemoryData(assoc, presId, &req, NULL, NULL, NULL,
                                          NULL);
}
...
OFCondition DIMSE_checkForCancelRQ(T_ASC_Association* assoc,
  ...
  OFCondition cond =
      DIMSE_receiveCommand(assoc, DIMSE_NONBLOCKING, 0, &presIdCmd, &msg, NULL);
  ...
  if (cond.good()) /* could be DIMSE_NODATAAVAILABLE or some error condition */
  {
    ...
    if (msg.CommandField != DIMSE_C_CANCEL_RQ) {
      char buf1[256];
      sprintf(buf1, "DIMSE: Checking for C-CANCEL-RQ, Protocol Error: Cmd=0x%x",
              msg.CommandField);
      return makeDcmnetCondition(DIMSEC_UNEXPECTEDREQUEST, OF_error, buf1);
    }
    if (msg.msg.CCancelRQ.MessageIDBeingRespondedTo != msgId) {
      char buf2[256];
      sprintf(buf2, "DIMSE: Checking for C-CANCEL-RQ, Protocol Error: msgId=%d",
