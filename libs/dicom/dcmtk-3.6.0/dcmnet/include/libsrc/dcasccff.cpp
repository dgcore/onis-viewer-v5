...
OFCondition DcmAssociationConfigurationFile::initialize(
    DcmAssociationConfiguration& cfg, const char* filename) {
  if (!filename)
...
OFCondition DcmAssociationConfigurationFile::parseTransferSyntaxes(
    DcmAssociationConfiguration& cfg, OFConfigFile& config) {
...
OFCondition DcmAssociationConfigurationFile::parsePresentationContexts(
  ...
  while (config.section_valid(1)) {
    ...
    while (found) {
      ...
      value = config.get_entry(buf);
      if (value) {
        // value must consist of abstract syntax and transfer syntax, separated
        // by '\\'
        ...
        while (++i < len) {
          ...
          if (!isspace(c))
            transferSyntaxKey += (char)(toupper(c));
        }

        // finally cut abstract syntax name
        abstractSyntaxUID.erase(separator);

        result = cfg.addPresentationContext(key, abstractSyntaxUID.c_str(),
                                            transferSyntaxKey.c_str());
        if (result.bad())
          return result;
...
OFCondition DcmAssociationConfigurationFile::parseRoleSelectionItems(
  ...
  if (!config.section_valid(2))
    return result;  // SCP/SCU role selection is optional, may be absent
    ...
    while (found) {
      ...
      if (value) {
        ...
        if (roleKey == "SCU") {
          ...
          result =
              cfg.addRole(key, abstractSyntaxUID.c_str(), ASC_SC_ROLE_SCUSCP);
        } else {
...
OFCondition DcmAssociationConfigurationFile::parseExtendedNegotiationItems(
  ...
  if (!config.section_valid(2))
    return result;  // extended negotiation is optional, may be absent
  ...
  while (config.section_valid(1)) {
    ...
    while (found) {
      ...
      value = config.get_entry(buf);
      if (value) {
        // value must consist of abstract syntax and extended negotiation data,
        // separated by '\\'
        abstractSyntaxUID = value;
        len = abstractSyntaxUID.size();
        separator = abstractSyntaxUID.find("\\");
        ...
        if (len - separator > 2048) {
          OFString s(
              "extended negotiation data too long, cannot handle > 1024 bytes "
              "in entry ");
          s += buf;
          s += " in config file";
        ...
        result = cfg.addExtendedNegotiation(key, abstractSyntaxUID.c_str(), raw,
                                            (Uint32)rawSize);
        if (result.bad())
          return result;
...
OFCondition DcmAssociationConfigurationFile::parseProfiles(
  ...
  while (config.section_valid(1)) {
    ...
    scontext.clear();
    while (*c) {
      if (!isspace(TO_UCHAR(*c)))
        scontext += (char)(toupper(TO_UCHAR(*c)));
      ++c;
    }
    ...
    if (extneg) {
      ...
      while (*c) {
        if (!isspace(TO_UCHAR(*c)))
          sextneg += (char)(toupper(TO_UCHAR(*c)));
        ++c;
      }
