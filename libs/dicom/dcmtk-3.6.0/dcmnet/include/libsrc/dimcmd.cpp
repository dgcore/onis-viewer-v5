...
static OFCondition buildErrorWithMsg(const char* msg, DcmTagKey t) {
  DcmTag tag(t);
  char buf[1024];
  sprintf(buf, "DIMSE: Command Build Failed: %s: Element: (%04x,%04x) %s", msg,
          t.getGroup(), t.getElement(), tag.getTagName());
  return makeDcmnetCondition(DIMSEC_BUILDFAILED, OF_error, buf);
}

static OFCondition parseError(DcmTagKey t) {
  DcmTag tag(t);
  char buf[1024];
...
static OFCondition parseErrorWithMsg(const char* msg, DcmTagKey t) {
  DcmTag tag(t);
  char buf[1024];
...
static OFCondition deleteElem(DcmDataset* obj, DcmTagKey t) {
  DcmTag tag(t);
  DcmElement* e = NULL;
  OFBool ok = OFTrue;
  ...
  return (ok) ? (EC_Normal)
              : (parseErrorWithMsg("dimcmd:deleteElem: Cannot delete element",
                                   t));
}

static OFCondition addString(DcmDataset* obj, DcmTagKey t, char* s,
                             OFBool keepPadding) {
...
static OFCondition getString(DcmDataset* obj, DcmTagKey t, char* s, int maxlen,
                             OFBool* spacePadded) {
  DcmElement* elem;
  DcmStack stack;
  OFCondition ec = EC_Normal;
...
static OFCondition getAndDeleteString(DcmDataset* obj, DcmTagKey t, char* s,
                                      int maxlen, OFBool* spacePadded) {
  OFCondition cond = getString(obj, t, s, maxlen, spacePadded);
  PRET(cond, t);
  cond = deleteElem(obj, t);
  return cond;
}

static OFCondition getAndDeleteStringOpt(DcmDataset* obj, DcmTagKey t, char* s,
                                         int maxlen, OFBool* spacePadded) {
  OFCondition cond = getString(obj, t, s, maxlen, spacePadded);
  RET(cond);
  cond = deleteElem(obj, t);
...
static OFCondition addUS(DcmDataset* obj, DcmTagKey t, Uint16 us) {
  OFCondition ec = EC_Normal;
  DcmElement* e = NULL;
  DcmTag tag(t);
...
static OFCondition getUS(DcmDataset* obj, DcmTagKey t, Uint16* us) {
  DcmElement* elem;
  DcmStack stack;
  OFCondition ec = EC_Normal;
...
static OFCondition getAndDeleteUS(DcmDataset* obj, DcmTagKey t, Uint16* us) {
  OFCondition cond = getUS(obj, t, us);
  PRET(cond, t);
  cond = deleteElem(obj, t);
  return cond;
}

static OFCondition getAndDeleteUSOpt(DcmDataset* obj, DcmTagKey t, Uint16* us) {
  OFCondition cond = getUS(obj, t, us);
  RET(cond);
  cond = deleteElem(obj, t);
  return cond;
...
static OFCondition addUL(DcmDataset* obj, DcmTagKey t, Uint32 ul) {
  OFCondition ec = EC_Normal;
  DcmElement* e = NULL;
  DcmTag tag(t);
...
static OFCondition getUL(DcmDataset* obj, DcmTagKey t, Uint32* ul) {
  DcmElement* elem;
  DcmStack stack;
  OFCondition ec = EC_Normal;
...
static OFCondition getAndDeleteULOpt(DcmDataset* obj, DcmTagKey t, Uint32* ul) {
  OFCondition cond = getUL(obj, t, ul);
  RET(cond);
  cond = deleteElem(obj, t);
  return cond;
}

static OFCondition addAttributeList(DcmDataset* obj, DcmTagKey t, Uint16* lst,
                                    int listCount) {
  ...
  */
  ...
  if ((listCount % 2) != 0) {
    return buildErrorWithMsg("dimcmd:addAttributeList: Error: Uneven listCount",
                             t);
  }
  ...
  return (ec == EC_Normal)
             ? (EC_Normal)
             : (buildErrorWithMsg("dimcmd:addAttributeList: Cannot add list",
                                  t));
}

static OFCondition getAttributeList(DcmDataset* obj, DcmTagKey t, Uint16** lst,
                                    int* listCount) {
  DcmElement* elem;
  DcmStack stack;
  OFCondition ec = EC_Normal;
 ...
 */
static OFCondition getAndDeleteAttributeListOpt(DcmDataset* obj, DcmTagKey t,
                                                Uint16** lst, int* listCount) {
  OFCondition cond = getAttributeList(obj, t, lst, listCount);
  RET(cond);
  cond = deleteElem(obj, t);
  return cond;
}
...
static OFCondition buildCommonRQ(DcmDataset* obj, Uint16 command,
                                 Uint16 messageID, Uint16 dataSetType) {
  // insert group length but calculate later
  OFCondition cond = addUL(obj, DCM_CommandGroupLength, 0);
  RET(cond);
  cond = addUS(obj, DCM_CommandField, command);
  RET(cond);
  cond = addUS(obj, DCM_MessageID, messageID);
  RET(cond);
  cond = addUS(obj, DCM_DataSetType, dataSetType);
  RET(cond);

  return EC_Normal;
}

static OFCondition parseCommonRQ(DcmDataset* obj, Uint16* command,
                                 Uint16* messageID, Uint16* dataSetType) {
  OFCondition cond = getAndDeleteUS(obj, DCM_CommandField, command);
  RET(cond);
  cond = getAndDeleteUS(obj, DCM_MessageID, messageID);
  RET(cond);
  cond = getAndDeleteUS(obj, DCM_DataSetType, dataSetType);
  RET(cond);

  return EC_Normal;
}

static OFCondition buildCommonRSP(DcmDataset* obj, Uint16 command,
  ...
  cond = addUS(obj, DCM_CommandField, command);
  RET(cond);
  cond = addUS(obj, DCM_MessageIDBeingRespondedTo, messageIDBeingRespondedTo);
  RET(cond);
  cond = addUS(obj, DCM_DataSetType, dataSetType);
  RET(cond);
  cond = addUS(obj, DCM_Status, status);
  RET(cond);

  return EC_Normal;
}

static OFCondition parseCommonRSP(DcmDataset* obj, Uint16* command,
                                  Uint16* messageIDBeingRespondedTo,
                                  Uint16* dataSetType, Uint16* status) {
  OFCondition cond = getAndDeleteUS(obj, DCM_CommandField, command);
  RET(cond);
  ...
  cond = getAndDeleteUS(obj, DCM_DataSetType, dataSetType);
  RET(cond);
  cond = getAndDeleteUS(obj, DCM_Status, status);
  RET(cond);
...
static OFCondition buildCEchoRQ(T_DIMSE_C_EchoRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_C_ECHO_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  cond =
      addString(obj, DCM_AffectedSOPClassUID, e->AffectedSOPClassUID, OFFalse);
  RET(cond);

  return cond;
}

static OFCondition parseCEchoRQ(T_DIMSE_C_EchoRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;

  OFCondition cond = parseCommonRQ(obj, &cmd, &msgid, &dtype);
  RET(cond);
  e->MessageID = msgid;
...
static OFCondition buildCEchoRSP(T_DIMSE_C_EchoRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_C_ECHO_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);

  /* build optional items */
  if (e->opts & O_ECHO_AFFECTEDSOPCLASSUID) {
    cond = addString(obj, DCM_AffectedSOPClassUID, e->AffectedSOPClassUID,
                     OFFalse);
    RET(cond);
  }
  return cond;
...
static OFCondition parseCEchoRSP(T_DIMSE_C_EchoRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;

  OFCondition cond = parseCommonRSP(obj, &cmd, &msgid, &dtype, &status);
  RET(cond);
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_ECHO_AFFECTEDSOPCLASSUID;
...
static OFCondition buildCStoreRQ(T_DIMSE_C_StoreRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_C_STORE_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  ...
  cond =
      addString(obj, DCM_AffectedSOPClassUID, e->AffectedSOPClassUID, OFFalse);
  RET(cond);
  cond = addString(obj, DCM_AffectedSOPInstanceUID, e->AffectedSOPInstanceUID,
                   OFFalse);
  RET(cond);
  cond = addUS(obj, DCM_Priority, e->Priority);
  RET(cond);

  /* build optional items */
  if (e->opts & O_STORE_MOVEORIGINATORAETITLE) {
    cond = addString(obj, DCM_MoveOriginatorApplicationEntityTitle,
                     e->MoveOriginatorApplicationEntityTitle, OFFalse);
    RET(cond);
  }
  if (e->opts & O_STORE_MOVEORIGINATORID) {
    cond = addUS(obj, DCM_MoveOriginatorMessageID, e->MoveOriginatorID);
    RET(cond);
  }
  return cond;
}

static OFCondition parseCStoreRQ(T_DIMSE_C_StoreRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 priority;
  ...
  e->Priority = (T_DIMSE_Priority)priority;
  RET(cond);

  /* parse optional items */
  e->opts = 0;
  cond = getAndDeleteStringOpt(obj, DCM_MoveOriginatorApplicationEntityTitle,
                               e->MoveOriginatorApplicationEntityTitle,
                               DIC_AE_LEN, NULL);
  if (cond.good())
    e->opts |= O_STORE_MOVEORIGINATORAETITLE;

  cond =
      getAndDeleteUSOpt(obj, DCM_MoveOriginatorMessageID, &e->MoveOriginatorID);
  if (cond.good())
    e->opts |= O_STORE_MOVEORIGINATORID;
  if (uidSpacePadded)
    e->opts |= O_STORE_RQ_BLANK_PADDING;
...
static OFCondition buildCStoreRSP(T_DIMSE_C_StoreRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_C_STORE_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
  ...
  if (e->opts & O_STORE_AFFECTEDSOPINSTANCEUID) {
    ...
    cond = addString(obj, DCM_AffectedSOPInstanceUID, instanceuid, OFTrue);
    RET(cond);
  }
  return cond;
}

static OFCondition parseCStoreRSP(T_DIMSE_C_StoreRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
...
static OFCondition buildCFindRQ(T_DIMSE_C_FindRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_C_FIND_RQ, e->MessageID, e->DataSetType);
  RET(cond);
...
static OFCondition parseCFindRQ(T_DIMSE_C_FindRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 priority;
...
static OFCondition buildCFindRSP(T_DIMSE_C_FindRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_C_FIND_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseCFindRSP(T_DIMSE_C_FindRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_FIND_AFFECTEDSOPCLASSUID;
...
static OFCondition buildCGetRQ(T_DIMSE_C_GetRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_C_GET_RQ, e->MessageID, e->DataSetType);
  RET(cond);
...
static OFCondition parseCGetRQ(T_DIMSE_C_GetRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 priority;
...
static OFCondition buildCGetRSP(T_DIMSE_C_GetRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_C_GET_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseCGetRSP(T_DIMSE_C_GetRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
...
static OFCondition buildCMoveRQ(T_DIMSE_C_MoveRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_C_MOVE_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  ...
  cond = addUS(obj, DCM_Priority, e->Priority);
  RET(cond);
  cond = addString(obj, DCM_MoveDestination, e->MoveDestination, OFFalse);
  RET(cond);

  return cond;
}

static OFCondition parseCMoveRQ(T_DIMSE_C_MoveRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 priority;
  ...
  e->Priority = (T_DIMSE_Priority)priority;
  RET(cond);
  cond = getAndDeleteString(obj, DCM_MoveDestination, e->MoveDestination,
                            DIC_AE_LEN, NULL);
  RET(cond);
...
static OFCondition buildCMoveRSP(T_DIMSE_C_MoveRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_C_MOVE_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseCMoveRSP(T_DIMSE_C_MoveRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_MOVE_AFFECTEDSOPCLASSUID;

  cond = getAndDeleteUSOpt(obj, DCM_NumberOfRemainingSubOperations,
                           &e->NumberOfRemainingSubOperations);
  if (cond.good())
    e->opts |= O_MOVE_NUMBEROFREMAININGSUBOPERATIONS;

  cond = getAndDeleteUSOpt(obj, DCM_NumberOfCompletedSubOperations,
                           &e->NumberOfCompletedSubOperations);
  if (cond.good())
    e->opts |= O_MOVE_NUMBEROFCOMPLETEDSUBOPERATIONS;

  cond = getAndDeleteUSOpt(obj, DCM_NumberOfFailedSubOperations,
                           &e->NumberOfFailedSubOperations);
  if (cond.good())
    e->opts |= O_MOVE_NUMBEROFFAILEDSUBOPERATIONS;

  cond = getAndDeleteUSOpt(obj, DCM_NumberOfWarningSubOperations,
                           &e->NumberOfWarningSubOperations);
  if (cond.good())
    e->opts |= O_MOVE_NUMBEROFWARNINGSUBOPERATIONS;
...
static OFCondition buildCCancelRQ(T_DIMSE_C_CancelRQ* e, DcmDataset* obj) {
  OFCondition cond = addUS(obj, DCM_CommandField, DIMSE_C_CANCEL_RQ);
  RET(cond);
  cond =
      addUS(obj, DCM_MessageIDBeingRespondedTo, e->MessageIDBeingRespondedTo);
  RET(cond);
  cond = addUS(obj, DCM_DataSetType, e->DataSetType);
  RET(cond);

  return EC_Normal;
}

static OFCondition parseCCancelRQ(T_DIMSE_C_CancelRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;

  OFCondition cond = getAndDeleteUS(obj, DCM_CommandField, &cmd);
  RET(cond);
  cond = getAndDeleteUS(obj, DCM_MessageIDBeingRespondedTo, &msgid);
  RET(cond);
  cond = getAndDeleteUS(obj, DCM_DataSetType, &dtype);
  RET(cond);
...
static OFCondition buildNEventReportRQ(T_DIMSE_N_EventReportRQ* e,
                                       DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_N_EVENT_REPORT_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  cond =
      addString(obj, DCM_AffectedSOPClassUID, e->AffectedSOPClassUID, OFFalse);
  RET(cond);
  cond = addString(obj, DCM_AffectedSOPInstanceUID, e->AffectedSOPInstanceUID,
                   OFFalse);
  RET(cond);
  cond = addUS(obj, DCM_EventTypeID, e->EventTypeID);
  RET(cond);

  return cond;
}

static OFCondition parseNEventReportRQ(T_DIMSE_N_EventReportRQ* e,
                                       DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  ...
  cond = getAndDeleteString(obj, DCM_AffectedSOPInstanceUID,
                            e->AffectedSOPInstanceUID, DIC_UI_LEN, NULL);
  RET(cond);
  cond = getAndDeleteUS(obj, DCM_EventTypeID, &e->EventTypeID);
  RET(cond);
  return cond;
}
...
static OFCondition buildNEventReportRSP(T_DIMSE_N_EventReportRSP* e,
                                        DcmDataset* obj) {
...
static OFCondition parseNEventReportRSP(T_DIMSE_N_EventReportRSP* e,
                                        DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NEVENTREPORT_AFFECTEDSOPCLASSUID;

  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPInstanceUID,
                               e->AffectedSOPInstanceUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NEVENTREPORT_AFFECTEDSOPINSTANCEUID;

  cond = getAndDeleteUSOpt(obj, DCM_EventTypeID, &e->EventTypeID);
  if (cond.good())
    e->opts |= O_NEVENTREPORT_EVENTTYPEID;
...
static OFCondition buildNGetRQ(T_DIMSE_N_GetRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_N_GET_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  cond = addString(obj, DCM_RequestedSOPClassUID, e->RequestedSOPClassUID,
                   OFFalse);
  RET(cond);
  cond = addString(obj, DCM_RequestedSOPInstanceUID, e->RequestedSOPInstanceUID,
                   OFFalse);
  RET(cond);
...
static OFCondition parseNGetRQ(T_DIMSE_N_GetRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  ...
  cond = getAndDeleteString(obj, DCM_RequestedSOPClassUID,
                            e->RequestedSOPClassUID, DIC_UI_LEN, NULL);
  RET(cond);
  cond = getAndDeleteString(obj, DCM_RequestedSOPInstanceUID,
                            e->RequestedSOPInstanceUID, DIC_UI_LEN, NULL);
  RET(cond);
...
static OFCondition buildNGetRSP(T_DIMSE_N_GetRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_N_GET_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseNGetRSP(T_DIMSE_N_GetRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NGET_AFFECTEDSOPCLASSUID;

  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPInstanceUID,
                               e->AffectedSOPInstanceUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NGET_AFFECTEDSOPINSTANCEUID;
...
static OFCondition buildNSetRQ(T_DIMSE_N_SetRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_N_SET_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  cond = addString(obj, DCM_RequestedSOPClassUID, e->RequestedSOPClassUID,
                   OFFalse);
  RET(cond);
...
static OFCondition parseNSetRQ(T_DIMSE_N_SetRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
...
static OFCondition buildNSetRSP(T_DIMSE_N_SetRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_N_SET_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseNSetRSP(T_DIMSE_N_SetRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NSET_AFFECTEDSOPCLASSUID;

  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPInstanceUID,
                               e->AffectedSOPInstanceUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NSET_AFFECTEDSOPINSTANCEUID;
...
static OFCondition buildNActionRQ(T_DIMSE_N_ActionRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_N_ACTION_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  cond = addString(obj, DCM_RequestedSOPClassUID, e->RequestedSOPClassUID,
                   OFFalse);
  RET(cond);
  cond = addString(obj, DCM_RequestedSOPInstanceUID, e->RequestedSOPInstanceUID,
                   OFFalse);
  RET(cond);
  cond = addUS(obj, DCM_ActionTypeID, e->ActionTypeID);
  RET(cond);

  return cond;
}

static OFCondition parseNActionRQ(T_DIMSE_N_ActionRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  ...
  cond = getAndDeleteString(obj, DCM_RequestedSOPInstanceUID,
                            e->RequestedSOPInstanceUID, DIC_UI_LEN, NULL);
  RET(cond);
  cond = getAndDeleteUS(obj, DCM_ActionTypeID, &e->ActionTypeID);
  RET(cond);
  return cond;
}
...
static OFCondition buildNActionRSP(T_DIMSE_N_ActionRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_N_ACTION_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseNActionRSP(T_DIMSE_N_ActionRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NACTION_AFFECTEDSOPCLASSUID;

  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPInstanceUID,
                               e->AffectedSOPInstanceUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NACTION_AFFECTEDSOPINSTANCEUID;

  cond = getAndDeleteUSOpt(obj, DCM_ActionTypeID, &e->ActionTypeID);
  if (cond.good())
    e->opts |= O_NACTION_ACTIONTYPEID;
...
static OFCondition buildNCreateRQ(T_DIMSE_N_CreateRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_N_CREATE_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  cond =
      addString(obj, DCM_AffectedSOPClassUID, e->AffectedSOPClassUID, OFFalse);
  RET(cond);
...
static OFCondition parseNCreateRQ(T_DIMSE_N_CreateRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPInstanceUID,
                               e->AffectedSOPInstanceUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NCREATE_AFFECTEDSOPINSTANCEUID;
...
static OFCondition buildNCreateRSP(T_DIMSE_N_CreateRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_N_CREATE_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseNCreateRSP(T_DIMSE_N_CreateRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NCREATE_AFFECTEDSOPCLASSUID;
...
static OFCondition buildNDeleteRQ(T_DIMSE_N_DeleteRQ* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRQ(obj, DIMSE_N_DELETE_RQ, e->MessageID, e->DataSetType);
  RET(cond);
  cond = addString(obj, DCM_RequestedSOPClassUID, e->RequestedSOPClassUID,
                   OFFalse);
  RET(cond);
...
static OFCondition parseNDeleteRQ(T_DIMSE_N_DeleteRQ* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
...
static OFCondition buildNDeleteRSP(T_DIMSE_N_DeleteRSP* e, DcmDataset* obj) {
  OFCondition cond =
      buildCommonRSP(obj, DIMSE_N_DELETE_RSP, e->MessageIDBeingRespondedTo,
                     e->DataSetType, e->DimseStatus);
  RET(cond);
...
static OFCondition parseNDeleteRSP(T_DIMSE_N_DeleteRSP* e, DcmDataset* obj) {
  Uint16 cmd;
  Uint16 msgid;
  Uint16 dtype;
  Uint16 status;
  ...
  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPClassUID,
                               e->AffectedSOPClassUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NDELETE_AFFECTEDSOPCLASSUID;

  cond = getAndDeleteStringOpt(obj, DCM_AffectedSOPInstanceUID,
                               e->AffectedSOPInstanceUID, DIC_UI_LEN, NULL);
  if (cond.good())
    e->opts |= O_NDELETE_AFFECTEDSOPINSTANCEUID;

  return EC_Normal;
}

OFCondition DIMSE_buildCmdObject(T_DIMSE_Message* msg, DcmDataset** obj)
/*
 * This function creates a DcmDataSet object ("command object") based on the
 * information in the DIMSE message variable (remember that all DICOM command
 * messages are - in the end - particular data sets). This newly created object
 * will contain all the information that is needed for a particular DIMSE
 * command that will be sent over the network, i.e. also command group length,
 * command field, data set type etc. Note that a group length element will
 * already be added to the created object but that the value in this element
 * will not yet be set. (It has to be set before the command is sent over the
 * network though.)
 *
 * Parameters:
 *   msg - [in] Structure that represents a certain DIMSE command which shall be
 * sent. obj - [out] The DcmDataSet object which was created from msg.
 */
{
  ...
  /* depending on the type of DIMSE command, create a corresponding DcmDataSet
   * object */
  switch (msg->CommandField) {
    ...
    default: {
      char buf[256];
      sprintf(buf, "DIMSE_buildCmdObject: Invalid Command Message: 0x%x",
              msg->CommandField);
...
OFCondition DIMSE_parseCmdObject(T_DIMSE_Message* msg, DcmDataset* obj)
/*
 * This function parses the information in the DcmDataset object which was
 * passed and creates a corresponding T_DIMSE_Message structure which represents
 * the DIMSE message which is contained in obj.
 *
 * Parameters:
 *   msg - [out] Contains in the end the DIMSE message which is contained in
 * obj. obj - [in] The DcmDataset object which shall be parsed.
 */
{
  ...
  if (cond.bad()) {
    return makeDcmnetCondition(DIMSEC_PARSEFAILED, OF_error,
  ...
  switch (cmd) {
    ...
    default: {
      char buf[256];
      sprintf(buf, "DIMSE_parseCmdObject: Invalid Command Message: 0x%x",
              msg->CommandField);
      cond = makeDcmnetCondition(DIMSEC_BADCOMMANDTYPE, OF_error, buf);
    } break;
  }
...
OFBool DIMSE_isDataSetPresent(T_DIMSE_Message* msg) {
...
unsigned long DIMSE_countElements(DcmDataset* obj) {
  unsigned long n = 0;
