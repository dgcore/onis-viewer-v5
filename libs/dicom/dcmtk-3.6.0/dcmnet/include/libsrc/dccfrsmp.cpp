...
OFCondition DcmRoleSelectionMap::add(const char* key,
  ...
  OFString skey(key);
  DcmRoleSelectionList* const* value =
      OFconst_cast(DcmRoleSelectionList* const*, map_.lookup(skey));
  if (value == NULL) {
...
OFCondition DcmRoleSelectionMap::checkConsistency(
  ...
  DcmRoleSelectionList* const* entry =
      OFconst_cast(DcmRoleSelectionList* const*, map_.lookup(OFString(key)));
  if (!entry) {
    // error: key undefined
...
const DcmRoleSelectionList* DcmRoleSelectionMap::getRoleSelectionList(
