...
#ifdef macintosh
/*
** The WinSock header on Macintosh does not declare the WORD type nor the
*MAKEWORD
...
/* it is in the libraries but we have no prototype */
/* Note: Modern systems already have setsockopt, so we don't declare it */
/* BEGIN_EXTERN_C
...
END_EXTERN_C */ /* Modern systems already have select() */
#else
/* don't know how to emulate */
#endif
#endif
...
/* int access(const char* path, int amode); */ /* Modern systems already have
                                                  access() */
#else                                          /* HAVE_ACCESS */
...
#define X_OK \
  00   /* execute permission has no meaning under windows, treat as existance */
#endif /* R_OK */
#endif /* _WIN32 */
