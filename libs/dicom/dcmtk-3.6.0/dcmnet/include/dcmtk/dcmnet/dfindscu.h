...
public:
   ...
   */
  virtual void callback(T_DIMSE_C_FindRQ* request, int responseCount,
...
protected: /* the two member variables are protected and can be accessed from
              derived classes */
  /// pointer to current association. Will contain valid value when callback()
  /// is called.
  T_ASC_Association* assoc_;

  /// current presentation context ID. Will contain valid value when callback()
  /// is called.
  T_ASC_PresentationContextID presId_;
};

/** Default implementation of FindSCU callback class. This implementation is
 *  used when no explicit callback is passed by the user, e.g. in the findscu
 * tool.
 */
class DcmFindSCUDefaultCallback : public DcmFindSCUCallback {
public:
  /** constructor
   *  @param extractResponsesToFile if true, C-FIND-RSP datasets will be stored
   * as DICOM files
   *  @param cancelAfterNResponses if non-negative, a C-FIND-CANCEL will be
   * issued after the given number of incoming C-FIND-RSP messages
   */
  DcmFindSCUDefaultCallback(OFBool extractResponsesToFile,
...
private:
  ...
  /// if non-negative, a C-FIND-CANCEL will be issued after the given number of
  /// incoming C-FIND-RSP messages
  int cancelAfterNResponses_;
};

/** This class implements a complete DICOM C-FIND SCU, including association
 * set-up, execution of the C-FIND transaction including processing of results,
 * and closing of the association. By default, incoming C-FIND-RSP messages will
 * be displayed on console and, optionally, also stored in files. By providing a
 * user-defined callback, other types of processing are possible.
 */
class DcmFindSCU {
public:
   ...
   *  @param preferredTransferSyntax. May be Unknown, Implicit Little Endian, or
   * any of the two uncompressed explicit VR transfer syntaxes. By default
   * (unknown), local endian explicit VR is proposed first, followed by opposite
   * endian explicit VR, followed by implicit VR. This behaviour can be modified
   * by explicitly specifying the preferred explicit VR transfer syntax. With
   * Little Endian Implicit, only Implicit VR is proposed.
   *  @param blockMode DIMSE blocking mode
   *  @param dimse_timeout timeout for DIMSE operations (in seconds)
   *  @param maxReceivePDULength limit the maximum PDU size for incoming PDUs to
   * the given value. This value should be less than or equal to
   * ASC_DEFAULTMAXPDU, and is usually identical to ASC_DEFAULTMAXPDU (other
   * values are only useful for debugging purposes).
   *  @param secureConnection this flag, if true, requests a secure TLS
   * connection to be used instead of a normal connection. This will only work
   * if DCMTK has been compiled with OpenSSL support (WITH_OPENSSL) and if a
   * transport layer object supporting secure connections has been set with
   * setTransportLayer() prior to this call.
   *  @param abortAssociation abort association instead of releasing it (for
   * debugging purposes)
   *  @param repeatCount number of times this query should be repeated
   *    (for debugging purposes, works only with default callback)
   *  @param extractResponsesToFile if true, extract incoming response messages
   * to file (works only with default callback)
   *  @param cancelAfterNResponses issue C-FIND-CANCEL after given number of
   * responses (works only with default callback)
   *  @param overrideKeys list of keys/paths that override those in the query
   * files, if any. Either the list of query files or override keys or both
   * should be non-empty, because the query dataset will be empty otherwise. For
   * path syntax see DcmPath.
   *  @param callback user-provided non-default callback handler object.
   *    For default callback, pass NULL.
   *  @param fileNameList list of query files. Each file is expected to be a
   * DICOM file containing a dataset that is used as a query, possibly modified
   * by override keys, if any. This parameter, if non-NULL, points to a list of
   * filenames (paths).
   *  @return EC_Normal if successful, an error code otherwise
   */
  OFCondition performQuery(
      const char* peer, unsigned int port, const char* ourTitle,
      const char* peerTitle, const char* abstractSyntax,
      E_TransferSyntax preferredTransferSyntax, T_DIMSE_BlockingMode blockMode,
      int dimse_timeout, Uint32 maxReceivePDULength, OFBool secureConnection,
      OFBool abortAssociation, unsigned int repeatCount,
      OFBool extractResponsesToFile, int cancelAfterNResponses,
      OFList<OFString>* overrideKeys, DcmFindSCUCallback* callback = NULL,
   ...
   *  This method produces a temporary copy of the dataset and should,
   * therefore, not be used with very large datasets.
...
private:
  /** add presentation context for given abstract syntax and given preferred
   * transfer syntax to the ACSE parameter struct.
   ...
   *  @param fname file name for the query file to be used with this query. May
   * be NULL.
   *  @param repeatCount number of times this query should be repeated
   *    (for debugging purposes, works only with default callback)
   *  @param abstractSyntax SOP Class UID or Meta SOP Class UID of service
   ...
   *  @param cancelAfterNResponses issue C-FIND-CANCEL after given number of
   * responses (works only with default callback)
   *  @param overrideKeys dataset with keys that override those in the query
   * file, if any. Either query file or override keys or both should be
   * non-empty, because the query dataset will be empty otherwise.
   *  @param callback user-provided non-default callback handler object.
   *    For default callback, pass NULL.
   *  @return EC_Normal if successful, an error code otherwise
   */
  OFCondition findSCU(T_ASC_Association* assoc, const char* fname,
                      int repeatCount, const char* abstractSyntax,
                      T_DIMSE_BlockingMode blockMode, int dimse_timeout,
                      OFBool extractResponsesToFile, int cancelAfterNResponses,
 ...
 * Refactored findscu code into class DcmFindSCU, which is now part of the
 * dcmnet library, and a short command line tool that only evaluates command
 * line parameters and then makes use of this class. This facilitates re-use of
 * the findscu code in other applications.
 *
 *
 */
