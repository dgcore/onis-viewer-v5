...
extern const OFCondition
    ASC_BADPRESENTATIONCONTEXTID; /* Bad presentation context ID */
extern const OFCondition
    ASC_MISSINGTRANSFERSYNTAX;        /* Missing transfer syntax */
extern const OFCondition ASC_NULLKEY; /* Caller passed in a NULL key */
extern const OFCondition
    ASC_SHUTDOWNAPPLICATION; /* Peer requested application shutdown */
extern const OFCondition
    ASC_USERIDENTIFICATIONFAILED; /* User Identity Negotiation failed */
extern const OFCondition
    ASC_SCPSCUROLESELECTIONFAILED; /* SCP/SCU role selection failed */
...
extern const OFCondition
    DIMSE_BADDATA; /* Inappropriate data for message (send) */
extern const OFCondition DIMSE_BADMESSAGE; /* Badly formed message (send) */
extern const OFCondition
    DIMSE_ILLEGALASSOCIATION; /* Caller passed in an illegal association */
extern const OFCondition
    DIMSE_NODATAAVAILABLE; /* no data (timeout in non-blocking mode) */
extern const OFCondition
    DIMSE_NOVALIDPRESENTATIONCONTEXTID; /* no valid presentation context ID */
...
extern const OFCondition
    DIMSE_PARSEFAILED; /* Failed to parse received message*/
...
// 0x311 was NETWORKINITIALIZED (Network already initialized) in previous
// releases
...
extern const OFCondition
    DUL_INCORRECTBUFFERLENGTH; /* DUL Incorrect buffer space allocated for data
                                */
extern const OFCondition
    DUL_INSUFFICIENTBUFFERLENGTH; /* DUL Insufficient buffer space allocated for
                                     data */
...
extern const OFCondition
    DUL_PCTRANSLATIONFAILURE; /* DUL Presentation Context translation failure */
extern const OFCondition DUL_PDATAPDUARRIVED; /* DUL P-Data PDU arrived */
extern const OFCondition DUL_PEERABORTEDASSOCIATION; /* Peer aborted Association
                                                        (or never connected) */
extern const OFCondition
...
extern const OFCondition
    DUL_UNSUPPORTEDPEERPROTOCOL; /* DUL Unsupported peer protocol */
extern const OFCondition
...
// This macro creates a condition with given code, severity, text, and
// subcondition. This method is intended as a replacement for
// COND_PushCondition(). Instead of maintaining a global condition stack (which
// is difficult in multi-threaded applications), the error text of the
// sub-condition is appended to the newly created condition. Making this a macro
// instead of a function saves the creation of a temporary.
#define makeDcmnetSubCondition(A, B, C, D) \
...
public:
   ...
   */
  static OFCondition push(unsigned short aModule, unsigned short aCode,
   ...
   * sub-condition. This method is intended as a replacement for
   * COND_PushCondition(). Instead of maintaining a global condition stack
   * (which is difficult in multi-threaded applications), the error text of the
   * sub-condition is appended to the newly created condition.
   */
