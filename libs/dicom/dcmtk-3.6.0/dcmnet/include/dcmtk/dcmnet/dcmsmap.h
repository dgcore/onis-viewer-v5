 ...
 */
template <class T>
class DcmKeyValuePair {
public:
  /** constructor
   *  @param k key
   *  @param v value
   */
  DcmKeyValuePair(const OFString& k, const T& v) : key_(k), value_(v) {}

  /// copy constructor
  DcmKeyValuePair(const DcmKeyValuePair& arg)
      : key_(arg.key_), value_(arg.value_) {}
...
public:
  ...
  const T* lookup(const OFString& key) const {
    OFLIST_TYPENAME OFListConstIterator(DcmKeyValuePair<T>*)
        first(list_.begin());
