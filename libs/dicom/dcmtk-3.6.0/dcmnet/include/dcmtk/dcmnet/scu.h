...
/** Base class for implementing DICOM Service Class User functionality. The
 * class offers support for negotiating associations and sending and receiving
 * arbitrary DIMSE messages on that connection. DcmSCU has built-in C-ECHO
 * support so derived classes do not have to implement that capability on their
 * own.
...
public:
   ...
   *  @param xferSyntaxes   [in] List of transfer syntaxes to be added for the
  ...
  /** Negotiate association by using presentation contexts and parameters as
   * defined by earlier function calls. If negotiation fails, there is no need
   * to close the association or to do anything else with this class.
   *  @return EC_Normal if negotiation was successful, otherwise error code
   */
  virtual OFCondition negotiateAssociation();

  /** After negotiation association, this call returns the first usable
   * presentation context given the desired abstract syntax and transfer syntax
   ...
   *  @return Adequate Presentation context ID that can be used. 0 if none
   * found.
   */
  T_ASC_PresentationContextID findPresentationContextID(
      const OFString& abstractSyntax, const OFString& transferSyntax);

  /** This function sends a C-ECHO command via network to another DICOM
   * application
   *  @param presID [in] Presentation context ID to use. A value of 0 lets SCP
   * class tries to choose one on its own.
  ...
  /** This function sends a C-STORE request on the currently opened association
   * and receives the corresponding response then
   *  @param presID          [in]  Contains in the end the ID of the
   * presentation context which was specified in the DIMSE command. If 0 is
   * given, the function tries to find an approriate presentation context
   *                               itself.
   *  @param dicomFile       [in]  The filename of the DICOM file to be sent.
   * Alternatively, a dataset can be given in the next parameter. If both are
   *                               given the dataset from the file name is used.
   *  @param dset            [in]  The dataset to be sent. Alternatively, a
   * filename can be specified in the previous parameter. If both are given the
   *                               dataset from the file name is used.
   *  @param rspCommandSet   [out] If this parameter is not NULL it will return
   * a copy of the DIMSE command which was received from the other DICOM
   *                               application.
   *  @param rspStatusDetail [out] If a non-NULL value is passed this variable
   * will in the end contain detailed information with regard to the status
   ...
   *  @return EC_Normal if request could be issued and response was received
   * successfully, error code otherwise. That means that if the receiver sends a
   * response denoting failure of the storage request, EC_Normal will be
   * returned.
   */
  ...
  /** Sends a C-FIND Request on given presentation context and receives list of
   * responses. The function receives the first response and then calls the
   * function handleFINDResponse which gets the relevant presentation context
   * together with the response dataset and status information. Then it waits
   * again for the next response, if there are more to come (i.e. response
   * status is PENDING). In the end, after receiving all responses, the full
   * list of responses is returned to the caller. If he is not interested, he
   * just sets responses=NULL when calling the function. This function can be
   * overwritten by actual SCU implementations but just should work fine for
   * most people.
   *  @param TODO
   *  @return TODO
   */
  ...
  /** This is the standard handler for C-FIND message responses: It just adds up
   * all responses it receives and prints a DEBUG message. Therefore, it is
   * called by for each response received in sendFINDRequest(). The idea is of
   * course to overwrite this function in a derived, actual SCU implementation
   * if required. Thus, after each response, the caller of sendFINDRequest() can
   * decide on its own whether he wants to cancel the C-FIND session, terminate
   * the association, do something useful or whatever. That way this is a more
  ...
  /** Send C-FIND-CANCEL and, therefore, ends the C-FIND session, i.e.\ no
   ...
   * valid C-FIND-enabled SOP class and usually only, if the last command send
   * on that presentation context was a C-FIND message.
   *  @param TODO
   *  @return TODO
   */
  virtual OFCondition sendCANCELRequest(Uint16 presContextID);

  /** This function sends a N-ACTION request on the currently opened association
   * and receives the corresponding response then
   *  @param presID         [in]  The ID of the presentation context to be used
   * for sending the request message. Should not be 0.
   ...
   * success, others can be found in the DICOM standard.
   *  @return EC_Normal if request could be issued and response was received
   * successfully, an error code otherwise. That means that if the receiver
   * sends a response denoting failure of the storage request, EC_Normal will be
   * returned.
   */
  virtual OFCondition sendACTIONRequest(
      const T_ASC_PresentationContextID presID, const OFString& sopInstanceUID,
      const Uint16 actionTypeID, DcmDataset* reqDataset, Uint16& rspStatusCode);
   ...
   *  @param timeout     [in]  Optional timeout in seconds for receiving data.
   * This value (if not 0) overwrites the standard DIMSE timeout and also
   *                           enables the non-blocking mode for receiving the
   * request.
  ...
  /** Closes the association created by this SCU. Also resets the current
   * association.
  ...
  /** Set SCP's host (hostname or IP address) to talk to in association
   * negotiation
   ...
   *  @param dimseTimeout [in] DIMSE Timeout in seconds for receiving data. If
   * the blocking mode is DIMSE_NONBLOCKING and we are trying to read data from
   *                           the incoming socket stream and no data has been
   * received.
   */
  void setDIMSETimeout(const Uint32 dimseTimeout);

  /** Set timeout for receiving ACSE messages
   *  @param acseTimeout [in] ACSE Timeout in seconds used by timer for message
   * timeouts during association negotiation
   */
  ...
  /** Returns the DIMSE timeout configured defining how long SCU will wait for
   * DIMSE responses
   *  @return The DIMSE timeout configured
   */
  Uint32 getDIMSETimeout() const;

  /** Returns the timeout configured defining how long SCU will wait for
   * messages during ACSE messaging (association negotiation)
...
protected:
  ...
  /** Returns SOP Class UID, SOP Instance UID and original transfer syntax for a
   * given dataset
   *  @param dataset        [in]  The dataset to read from
   *  @param sopClassUID    [out] The value of attribute SOP Class UID if
   * present
   *  @param sopInstanceUID [out] The value of attribute SOP Instance UID if
   * present
   *  @param transferSyntax [out] The value of transfer syntax that originally
   * was read from disk
   *  @return Error if dataset is NULL, otherwise EC_Normal. All return
   * parameters will be empty if not found (transferSyntax will be set to
   * EXS_Unknown) in the file.
   */
  OFCondition getDatasetInfo(DcmDataset* dataset, OFString& sopClassUID,
                             OFString& sopInstanceUID,
                             E_TransferSyntax& transferSyntax);

  /** Tells DcmSCU to use a secure TLS connection described by the given TLS
   * layer
  ...
  /** After negotiation association, this call returns the presentation context
   * belonging to the given presentation context ID
   *  @param presID         [in]  The presentation context ID to look for
   *  @param abstractSyntax [out] The abstract syntax (UID) for that ID.
   *                              Empty, if such a presentation context does not
   * exist.
