 ...
 * associations. If disabled, the numerical IP address instead of the symbolic
 * hostname is stored in the callingPresentationAddress field of the association
 * parameters structure.  Most DICOM applications (except imagectn) don't need
 * the symbolic hostname anyway, and the reverse DNS lookup can cause a long
 * timeout.
 */
 ...
 *  user data item with appropriate values (default). This applies to
 * association acceptors only.
 */
extern OFGlobal<OFBool> dcmStrictRoleSelection; /* default OFFalse */

/**  Global timeout (seconds) for connecting to remote hosts.
 *   Default value is -1 which selects infinite timeout, i.e. blocking
 * connect().
 */
 ...
 * which will be used by dcmnet the next time receiveTransportConnectionTCP() is
 * called. Useful for use with proxy applications, but inherently thread unsafe!
 */
...
/* Global option flag for compatibility with DCMTK releases prior to
 * version 3.0. Default (0) is automatic handling, which should work in most
 * cases.
 */
...
typedef enum { DUL_K_MAX_PDV_XMIT } DUL_ASSOCIATION_PARAMETER;
...
OFCondition DUL_AcknowledgeAssociationRQ(DUL_ASSOCIATIONKEY** association,
                                         DUL_ASSOCIATESERVICEPARAMETERS* params,
                                         int activatePDUStorage);

OFCondition DUL_InitializeNetwork(const char* mode, void* param, int timeout,
                                  unsigned long options,
                                  DUL_NETWORKKEY** network);

OFCondition DUL_ReceiveAssociationRQ(DUL_NETWORKKEY** net, DUL_BLOCKOPTIONS blk,
                                     int timeout,
                                     DUL_ASSOCIATESERVICEPARAMETERS* parameters,
                                     DUL_ASSOCIATIONKEY** association,
                                     int activatePDUStorage);

OFCondition DUL_RejectAssociationRQ(DUL_ASSOCIATIONKEY** association,
                                    DUL_ABORTITEMS* params,
                                    int activatePDUStorage);

OFCondition DUL_RequestAssociation(DUL_NETWORKKEY** network,
                                   DUL_ASSOCIATESERVICEPARAMETERS* params,
                                   DUL_ASSOCIATIONKEY** association,
                                   int activatePDUStorage);
 ...
 */
OFCondition DUL_ReadPDVs(DUL_ASSOCIATIONKEY** association, DUL_PDVLIST* pdvList,
                         DUL_BLOCKOPTIONS block, int timeout);
OFCondition DUL_WritePDVs(DUL_ASSOCIATIONKEY** association,
                          DUL_PDVLIST* pdvList);
OFCondition DUL_NextPDV(DUL_ASSOCIATIONKEY** association, DUL_PDV* pdv);

/* Miscellaneous functions.
 */
OFCondition DUL_AssociationParameter(DUL_ASSOCIATIONKEY** association,
                                     DUL_ASSOCIATION_PARAMETER param,
                                     DUL_DATA_TYPE type, void* address,
                                     size_t length);
OFCondition DUL_MakePresentationCtx(DUL_PRESENTATIONCONTEXT** ctx,
                                    DUL_SC_ROLE proposedSCRole,
                                    DUL_SC_ROLE acceptedSCRole,
...
OFString& dumpExtNegList(OFString& str,
...
OFBool DUL_dataWaiting(DUL_ASSOCIATIONKEY* callerAssociation, int timeout);
int DUL_networkSocket(DUL_NETWORKKEY* callerNet);
OFBool DUL_associationWaiting(DUL_NETWORKKEY* callerNet, int timeout);
...
DcmTransportConnection* DUL_getTransportConnection(
 ...
 *  DICOM network layer during receipt of a TCP transport connection, i.e.
 *  in receiveTransportConnectionTCP(). The call is not reversible - use with
 * care.
 */
void DUL_markProcessAsForkedChild();

/** this function marks the current process as a multi-process server and
 * enables the creation of child processes for each incoming TCP transport
 * connection in receiveTransportConnectionTCP().
...
/// @deprecated Use OFString& DUL_DumpParams(OFString&,
/// DUL_ASSOCIATESERVICEPARAMETERS) instead.
void DUL_DumpParams(DUL_ASSOCIATESERVICEPARAMETERS* params);
/// @deprecated Use OFString& DUL_DumpConnectionParameters(OFString&,
/// DUL_ASSOCIATIONKEY*) instead.
void DUL_DumpConnectionParameters(DUL_ASSOCIATIONKEY* association,
                                  STD_NAMESPACE ostream& outstream);
/// @deprecated Use OFString& dumpExtNegList(OFString&,
/// SOPClassExtendedNegotiationSubItemList&) instead.
