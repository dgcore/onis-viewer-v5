...
public:
   ...
   *  item type.
   *  @param readBuffer - [in/out] The buffer to read from. The pointer to the
   * buffer gets incremented by "bytesRead" bytes.
...
public:
   ...
   *  item type.
   *  @param readBuffer - [in] The buffer to read from. The pointer to the
   * buffer gets incremented by "bytesRead" bytes.
   *  @param bytesRead  - [out] Number of bytes read by this function
   *  @param availData  - [in] Size of the buffer.
   *  @return EC_Normal if successful, error code otherwise
   ...
   * @return none
   */
  UserIdentityNegotiationSubItemRQ& operator=(
...
private:
  /// User Identity Type: 1 (username), 2 (username/password), 3 (kerberos), 4
  /// (SAML)
...
public:
   ...
   * @return none
   */
  UserIdentityNegotiationSubItemAC& operator=(
