...
static OFCondition AE_1_TransportConnect(PRIVATE_NETWORKKEY** network,
                                         PRIVATE_ASSOCIATIONKEY** association,
                                         int nextState, void* params);
static OFCondition AE_2_SendAssociateRQPDU(PRIVATE_NETWORKKEY** network,
                                           PRIVATE_ASSOCIATIONKEY** association,
                                           int nextState, void* params);
static OFCondition AE_3_AssociateConfirmationAccept(
    PRIVATE_NETWORKKEY** network, PRIVATE_ASSOCIATIONKEY** association,
    int nextState, void* params);
static OFCondition AE_4_AssociateConfirmationReject(
    PRIVATE_NETWORKKEY** network, PRIVATE_ASSOCIATIONKEY** association,
    int nextState, void* params);
static OFCondition AE_5_TransportConnectResponse(
    PRIVATE_NETWORKKEY** network, PRIVATE_ASSOCIATIONKEY** association,
    int nextState, void* params);
static OFCondition AE_6_ExamineAssociateRequest(
    PRIVATE_NETWORKKEY** network, PRIVATE_ASSOCIATIONKEY** association,
    int nextState, void* params);
static OFCondition AE_7_SendAssociateAC(PRIVATE_NETWORKKEY** network,
                                        PRIVATE_ASSOCIATIONKEY** association,
                                        int nextState, void* params);
static OFCondition AE_8_SendAssociateRJ(PRIVATE_NETWORKKEY** network,
                                        PRIVATE_ASSOCIATIONKEY** association,
                                        int nextState, void* params);

static OFCondition DT_1_SendPData(PRIVATE_NETWORKKEY** network,
                                  PRIVATE_ASSOCIATIONKEY** associatin,
                                  int nextState, void* params);
static OFCondition DT_2_IndicatePData(PRIVATE_NETWORKKEY** network,
                                      PRIVATE_ASSOCIATIONKEY** association,
                                      int nextState, void* params);

static OFCondition AA_1_SendAAbort(PRIVATE_NETWORKKEY** network,
                                   PRIVATE_ASSOCIATIONKEY** associatin,
                                   int nextState, void* params);
static OFCondition AA_2_CloseTransport(PRIVATE_NETWORKKEY** network,
                                       PRIVATE_ASSOCIATIONKEY** association,
                                       int nextState, void* params);
static OFCondition AA_2_CloseTimeout(PRIVATE_NETWORKKEY** network,
                                     PRIVATE_ASSOCIATIONKEY** association,
                                     int nextState, void* params);
static OFCondition AA_3_IndicatePeerAborted(
    PRIVATE_NETWORKKEY** network, PRIVATE_ASSOCIATIONKEY** association,
    int nextState, void* params);
static OFCondition AA_4_IndicateAPAbort(PRIVATE_NETWORKKEY** network,
                                        PRIVATE_ASSOCIATIONKEY** association,
                                        int nextState, void* params);
static OFCondition AA_5_StopARTIMtimer(PRIVATE_NETWORKKEY** network,
                                       PRIVATE_ASSOCIATIONKEY** association,
                                       int nextState, void* params);
static OFCondition AA_6_IgnorePDU(PRIVATE_NETWORKKEY** network,
                                  PRIVATE_ASSOCIATIONKEY** association,
                                  int nextState, void* params);
static OFCondition AA_7_State13SendAbort(PRIVATE_NETWORKKEY** network,
                                         PRIVATE_ASSOCIATIONKEY** association,
                                         int nextState, void* params);
static OFCondition AA_8_UnrecognizedPDUSendAbort(
    PRIVATE_NETWORKKEY** network, PRIVATE_ASSOCIATIONKEY** association,
    int nextState, void* params);

static OFCondition AR_1_SendReleaseRQ(PRIVATE_NETWORKKEY** network,
                                      PRIVATE_ASSOCIATIONKEY** association,
                                      int nextState, void* params);
static OFCondition AR_2_IndicateRelease(PRIVATE_NETWORKKEY** network,
                                        PRIVATE_ASSOCIATIONKEY** association,
                                        int nextState, void* params);
static OFCondition AR_3_ConfirmRelease(PRIVATE_NETWORKKEY** network,
                                       PRIVATE_ASSOCIATIONKEY** association,
                                       int nextState, void* params);
static OFCondition AR_4_SendReleaseRP(PRIVATE_NETWORKKEY** network,
                                      PRIVATE_ASSOCIATIONKEY** association,
                                      int nextState, void* params);
static OFCondition AR_5_StopARTIMtimer(PRIVATE_NETWORKKEY** network,
                                       PRIVATE_ASSOCIATIONKEY** association,
                                       int nextState, void* params);
static OFCondition AR_6_IndicatePData(PRIVATE_NETWORKKEY** network,
                                      PRIVATE_ASSOCIATIONKEY** association,
                                      int nextState, void* params);
static OFCondition AR_7_SendPDATA(PRIVATE_NETWORKKEY** network,
                                  PRIVATE_ASSOCIATIONKEY** association,
                                  int nextState, void* params);
static OFCondition AR_8_IndicateARelease(PRIVATE_NETWORKKEY** network,
                                         PRIVATE_ASSOCIATIONKEY** association,
                                         int nextState, void* params);
static OFCondition AR_9_SendAReleaseRP(PRIVATE_NETWORKKEY** network,
                                       PRIVATE_ASSOCIATIONKEY** association,
                                       int nextState, void* params);
static OFCondition AR_10_ConfirmRelease(PRIVATE_NETWORKKEY** network,
                                        PRIVATE_ASSOCIATIONKEY** association,
                                        int nextState, void* params);

static OFCondition requestAssociationTCP(PRIVATE_NETWORKKEY** network,
                                         DUL_ASSOCIATESERVICEPARAMETERS* params,
                                         PRIVATE_ASSOCIATIONKEY** association);
static OFCondition sendAssociationRQTCP(PRIVATE_NETWORKKEY** network,
                                        DUL_ASSOCIATESERVICEPARAMETERS* params,
                                        PRIVATE_ASSOCIATIONKEY** association);
static OFCondition sendAssociationACTCP(PRIVATE_NETWORKKEY** network,
                                        DUL_ASSOCIATESERVICEPARAMETERS* params,
                                        PRIVATE_ASSOCIATIONKEY** association);
static OFCondition sendAssociationRJTCP(PRIVATE_NETWORKKEY** network,
                                        DUL_ABORTITEMS* abortItems,
                                        PRIVATE_ASSOCIATIONKEY** association);
static OFCondition sendAbortTCP(DUL_ABORTITEMS* abortItems,
                                PRIVATE_ASSOCIATIONKEY** association);
...
static OFCondition sendPDataTCP(PRIVATE_ASSOCIATIONKEY** association,
                                DUL_PDVLIST* pdvList);
static OFCondition writeDataPDU(PRIVATE_ASSOCIATIONKEY** association,
                                DUL_DATAPDU* pdu);
...
static OFCondition readPDUHead(PRIVATE_ASSOCIATIONKEY** association,
                               unsigned char* buffer, unsigned long maxlength,
                               DUL_BLOCKOPTIONS block, int timeout,
                               unsigned char* PDUtype,
                               unsigned char* PDUreserved,
                               unsigned long* PDULength);
static OFCondition readPDU(PRIVATE_ASSOCIATIONKEY** association,
                           DUL_BLOCKOPTIONS block, int timeout,
                           unsigned char** buffer, unsigned char* pduType,
                           unsigned char* pduReserved,
                           unsigned long* pduLength);
static OFCondition readPDUBody(PRIVATE_ASSOCIATIONKEY** association,
                               DUL_BLOCKOPTIONS block, int timeout,
                               unsigned char* buffer, unsigned long maxLength,
                               unsigned char* pduType,
                               unsigned char* pduReserved,
                               unsigned long* pduLength);
static OFCondition readPDUHeadTCP(PRIVATE_ASSOCIATIONKEY** association,
                                  ...
                                  unsigned char* PDUreserved,
                                  unsigned long* PDULength);
static OFCondition readPDUBodyTCP(
    PRIVATE_ASSOCIATIONKEY** association, DUL_BLOCKOPTIONS block, int timeout,
    unsigned char* buffer, unsigned long maxLength, unsigned char* pduType,
    unsigned char* pduReserved, unsigned long* pduLength);
static OFCondition defragmentTCP(DcmTransportConnection* connection,
                                 DUL_BLOCKOPTIONS block, time_t timerStart,
                                 int timeout, void* b, unsigned long l,
...
OFCondition translatePresentationContextList(LST_HEAD** internalList,
                                             LST_HEAD** SCUSCPRoleList,
                                             LST_HEAD** userContextList);
DUL_PRESENTATIONCONTEXT* findPresentationCtx(
    LST_HEAD** lst, DUL_PRESENTATIONCONTEXTID contextID);

PRV_SCUSCPROLE* findSCUSCPRole(LST_HEAD** lst, char* abstractSyntax);
...
*/
OFCondition DUL_InitializeFSM() {
  unsigned long l_index, idx2;
  FSM_ENTRY
  *stateEntries;

  stateEntries = (FSM_ENTRY*)StateTable;
  for (l_index = 0; l_index < DUL_NUMBER_OF_EVENTS * DUL_NUMBER_OF_STATES;
       l_index++) {
    if (stateEntries[l_index].action != NOACTION) {
      for (idx2 = 0; idx2 < DIM_OF(FSM_FunctionTable) &&
                     stateEntries[l_index].actionFunction == NULL;
           idx2++)
    ...
    for (idx2 = 0; idx2 < DIM_OF(Event_Table) &&
                   strlen(stateEntries[l_index].eventName) == 0;
         idx2++) {
...
**      Description of the algorithm (optional) and any other notes.
*/
OFCondition PRV_StateMachine(PRIVATE_NETWORKKEY** network,
                             PRIVATE_ASSOCIATIONKEY** association, int event,
                             int state, void* params) {
  FSM_ENTRY
  *entry;
  ...
  if (event < 0 || event >= DUL_NUMBER_OF_EVENTS) {
    char buf1[256];
    sprintf(buf1,
            "DUL Finite State Machine Error: Bad event, state %d event %d",
            state, event);
  ...
  if (state < 1 || state > DUL_NUMBER_OF_STATES) {
    char buf1[256];
    sprintf(buf1,
            "DUL Finite State Machine Error: Bad state, state %d event %d",
            state, event);
    return makeDcmnetCondition(DULC_FSMERROR, OF_error, buf1);
  }

  /* depending on the given event and state, determine the state table's entry
   * (the state */
  /* table implements the state transition table of DICOM's Upper Layer State
   * Machine which */
  /* in turn implements the DICOM upper layer protocol) (see DICOM standard
   * (year 2000) part */
  /* 8, section 9) (or the corresponding section in a later version of the
   * standard) */
  entry = &StateTable[event][state - 1];

  /* dump information if required */
  DCMNET_TRACE("DUL  FSM Table: State: "
               << state << " Event: " << event << OFendl
               << "DUL  Event:  " << entry->eventName << OFendl
               << "DUL  Action: " << entry->actionName);

  /* if the state table's entry specifies an action function, execute this
   * function and return */
  /* it's result value. If there is no action function defined, return a
   * corresponding error. */
  if (entry->actionFunction != NULL)
    ...
    sprintf(
        buf1,
        "DUL Finite State Machine Error: No action defined, state %d event %d",
        state, event);
    return makeDcmnetCondition(DULC_FSMERROR, OF_error, buf1);
  }
}
...
static OFCondition AE_3_AssociateConfirmationAccept(
  ...
  DUL_ASSOCIATESERVICEPARAMETERS
  *service;
  unsigned char *buffer = NULL, pduType, pduReserve;
  unsigned long pduLength;
  PRV_ASSOCIATEPDU
  assoc;
  ...
  *userPresentationCtx, *requestedPresentationCtx;
  DUL_SUBITEM
  *subItem;
  PRV_SCUSCPROLE
  *scuscpRole;
  ...
  service = (DUL_ASSOCIATESERVICEPARAMETERS*)params;
  OFCondition cond = readPDU(association, DUL_BLOCK, 0, &buffer, &pduType,
                             &pduReserve, &pduLength);
  ...
  if (pduType == DUL_TYPEASSOCIATEAC) {
    ...
    free(buffer);
    if (cond.bad())
      return makeDcmnetSubCondition(DULC_ILLEGALPDU, OF_error,
                                    "DUL Illegal or ill-formed PDU", cond);
    ...
    prvCtx =
        (PRV_PRESENTATIONCONTEXTITEM*)LST_Head(&assoc.presentationContextList);
    if (prvCtx != NULL)
      (void)LST_Position(&assoc.presentationContextList, (LST_NODE*)prvCtx);
    while (prvCtx != NULL) {
      userPresentationCtx =
          (DUL_PRESENTATIONCONTEXT*)malloc(sizeof(DUL_PRESENTATIONCONTEXT));
      ...
      if (prvCtx->transferSyntaxList == NULL) {
        ...
        sprintf(buf1,
                ...
                0);
        return makeDcmnetCondition(DULC_PEERILLEGALXFERSYNTAXCOUNT, OF_error,
                                   buf1);
      }
      ...
      if ((prvCtx->result == DUL_PRESENTATION_ACCEPT) &&
        ...
        char buf2[256];
        sprintf(buf2,
                "DUL Peer supplied illegal number of transfer syntaxes (%ld)",
      ...
      if (subItem != NULL)
        (void)strcpy(userPresentationCtx->acceptedTransferSyntax,
                     subItem->data);
      cond = LST_Enqueue(&service->acceptedPresentationContext,
                         (LST_NODE*)userPresentationCtx);
      if (cond.bad())
        return cond;

      prvCtx = (PRV_PRESENTATIONCONTEXTITEM*)LST_Next(
          &assoc.presentationContextList);
    }
    ...
    if (assoc.userInfo.usrIdent != NULL) {
      service->ackUserIdentNeg =
          new UserIdentityNegotiationSubItemAC(*(OFstatic_cast(
              UserIdentityNegotiationSubItemAC*, assoc.userInfo.usrIdent)));
    ...
    (*association)->maxPDVAcceptor = assoc.userInfo.maxLength.maxLength;
...
static OFCondition AE_4_AssociateConfirmationReject(
  ...
  DUL_ASSOCIATESERVICEPARAMETERS
  *service;
  unsigned char buffer[128], pduType, pduReserve;
  unsigned long pduLength;

  service = (DUL_ASSOCIATESERVICEPARAMETERS*)params;
  OFCondition cond =
      readPDUBody(association, DUL_BLOCK, 0, buffer, sizeof(buffer), &pduType,
                  &pduReserve, &pduLength);
  if (cond.bad())
    return cond;
  ...
  *service;
  ...
  if (pduType == DUL_TYPEASSOCIATERQ) {
    ...
    if (translatePresentationContextList(&assoc.presentationContextList,
                                         ...
                                         &service->requestedPresentationContext)
            .bad()) {
    ...
    /* user identity negotiation: Remember request values in association
     * parameters (copy)*/
    if (assoc.userInfo.usrIdent != NULL) {
      service->reqUserIdentNeg = new UserIdentityNegotiationSubItemRQ();
      if (service->reqUserIdentNeg == NULL)
        return EC_MemoryExhausted;
      *(service->reqUserIdentNeg) = *(OFstatic_cast(
          UserIdentityNegotiationSubItemRQ*, assoc.userInfo.usrIdent));
    }

    service->peerMaxPDU = assoc.userInfo.maxLength.maxLength;
    (*association)->maxPDV = assoc.userInfo.maxLength.maxLength;
    (*association)->maxPDVRequestor = assoc.userInfo.maxLength.maxLength;
...
static OFCondition DT_1_SendPData(PRIVATE_NETWORKKEY** /*network*/,
  ...
  *pdvList;
  ...
  /* Remember that params used to be a variable of type DUL_PDVLIST * and
...
static OFCondition DT_2_IndicatePData(PRIVATE_NETWORKKEY** /*network*/,
                                      PRIVATE_ASSOCIATIONKEY** association,
                                      int nextState, void* /*params*/) {
  unsigned char pduType, pduReserved;
  unsigned long pduLength, pdvLength, pdvCount;
  long length;
  unsigned char* p;
  ...
  /* read PDU body information from the incoming socket stream. In case the
   * incoming */
  /* PDU's header information has not yet been read, also read this information.
   */
  OFCondition cond = readPDUBody(
      association, DUL_BLOCK, 0, (*association)->fragmentBuffer,
      (*association)->fragmentBufferLength, &pduType, &pduReserved, &pduLength);
  ...
  p = (*association)->fragmentBuffer;  // set p to the buffer which contains the
                                       // PDU's PDVs
  while (length >=
         4) {  // as long as length is at least 4 (= a length field can be read)
    EXTRACT_LONG_BIG(p, pdvLength);  // determine the length of the current PDV
                                     // (the PDV p points to)
    p += 4 + pdvLength;  // move p so that it points to the next PDV (move p 4
                         // bytes over the length field plus the amount of bytes
                         // which is captured in the PDV's length field (over
                         // presentation context.Id, message information header
                         // and data fragment))
    length -= 4 + pdvLength;  // update length (i.e. determine the length of the
                              // buffer which has not been evaluated yet.)
  ...
  if (length != 0) {
    char buf[256];
    sprintf(buf,
            "PDV lengths don't add up correctly: %d PDVs. This probably "
            "indicates a malformed P-DATA PDU. PDU type is %02x.",
            (int)pdvCount, (unsigned int)pduType);
  ...
  /* if at least one PDV is contained in the PDU, the association's pdvIndex has
   * to be set to 0 */
  if (pdvCount > 0)
    (*association)->pdvIndex = 0;
  else {
    /* if there is no PDV contained in the PDU, the association's pdvIndex has
     * to be set to -1 */
    (*association)->pdvIndex = -1;

    /* bugfix by wilkens 01/10/12: return error if PDU does not contain any
     * PDVs: */
    /* Additionally, it is not allowed to have a PDU that does not contain any
     * PDVs. */
    /* Hence, return an error (see DICOM standard (year 2000) part 8,
     * section 9.3.1, */
    /* figure 9-2) (or the corresponding section in a later version of the
     * standard) */
    char buf[256];
    sprintf(buf,
            "PDU without any PDVs encountered. In DT_2_IndicatePData.  This "
            "probably indicates a  malformed P DATA PDU.");
    return makeDcmnetCondition(DULC_ILLEGALPDU, OF_error, buf);
  }

  /* at this point we need to set the association's currentPDV variable so that
   * it contains the data */
  /* of the next unprocessed PDV (currentPDV shall always contain the next
   * unprocessed PDV's data.) */

  /* set the fragment buffer (the buffer which contains all PDVs of the current
   * PDU) to a local variable */
  ...
  /* set the data fragment length in the association's currentPDV.fragmentLength
   * variable */
  /* (we start now overwriting all the entries in (*association)->currentPDV).
   * The actual */
  /* length of the data fragment of the next (unprocessed) PDV equals the above
   * determined */
  /* length minus 1 byte (for the presentation context ID) and minus another
   * byte (for the */
  /* message control header). */
  (*association)->currentPDV.fragmentLength = pdvLength - 2;

  /* set the presentation context ID value in the association's
   * currentPDV.presentationContextID */
  /* variable. This value is 1 byte wide and contained in the 5th byte of p (the
   * first four bytes */
  /* contain the PDV length value, the fifth byte the presentation context ID
   * value) */
  (*association)->currentPDV.presentationContextID = p[4];

  /* now determine if the next (unprocessed) PDV contains the last fragment of a
   * data set or DIMSE */
  /* command and if the next (unprocessed) PDV is a data set PDV or command PDV.
   * This information */
  /* is captured in the 6th byte of p: */
  unsigned char u = p[5];
  if (u & 2)
    (*association)->currentPDV.lastPDV =
        OFTrue;  // if bit 1 of the message control header is 1, this fragment
                 // does contain the last fragment of a data set or command
  else
    (*association)->currentPDV.lastPDV =
        OFFalse;  // if bit 1 of the message control header is 0, this fragment
                  // does not contain the last fragment of a data set or command

  if (u & 1)
    (*association)->currentPDV.pdvType =
        DUL_COMMANDPDV;  // if bit 0 of the message control header is 1, this is
                         // a command PDV
  else
    (*association)->currentPDV.pdvType =
        DUL_DATASETPDV;  // if bit 0 of the message control header is 0, this is
                         // a data set PDV

  /* now assign the data fragment of the next (unprocessed) PDV to the
   * association's */
  /* currentPDV.data variable. The fragment starts 6 bytes to the right of the
   * address */
...
static OFCondition AR_2_IndicateRelease(PRIVATE_NETWORKKEY** /*network*/,
  ...
  if (pduLength == 4) {
    ...
    if ((*association)->modeCallback &&
        !((mode & DUL_MAXPDUCOMPAT) ^ DUL_DULCOMPAT)) {
...
static OFCondition AA_3_IndicatePeerAborted(
  ...
  if (pduLength == 4) {
    unsigned long mode =
        pduReserve << 24 | buffer[0] << 16 | buffer[1] << 8 | buffer[3];
    if ((*association)->modeCallback &&
        !((mode & DUL_MAXPDUCOMPAT) ^ DUL_DULCOMPAT)) {
      (*association)->modeCallback->callback(mode);
    }
...
static OFCondition AA_6_IgnorePDU(PRIVATE_NETWORKKEY** /*network*/,
                                  PRIVATE_ASSOCIATIONKEY** association,
                                  int nextState, void* /*params*/) {
  unsigned char buffer[1024], PDUType, PDUReserved;
  long PDULength;
  unsigned long l;

  (*association)->protocolState = nextState;
  OFCondition cond =
      readPDUHead(association, buffer, sizeof(buffer), DUL_NOBLOCK,
  ...
  while (PDULength > 0 && cond.good()) {
    cond = defragmentTCP((*association)->connection, DUL_NOBLOCK,
                         (*association)->timerStart, (*association)->timeout,
...
static OFCondition requestAssociationTCP(PRIVATE_NETWORKKEY** network,
  ...
  if (sscanf(params->calledPresentationAddress, "%[^:]:%d", node, &port) != 2) {
    char buf[1024];
    sprintf(buf, "Illegal service parameter: %s",
...
#endif
  {
    ...
    if (rc == 0) {
...
#else
      // some platforms (e.g. OSF1 4.0) prefer size_t, but modern systems use
      // socklen_t
      socklen_t socketErrorLen = sizeof(socketError);
#endif
      ...
      getsockopt(s, SOL_SOCKET, SO_ERROR, (char*)(&socketError),
                 &socketErrorLen);
      if (socketError) {
    ...
    // The connect() returned without using the select(), reset the socket if
    // needed
    if (connectTimeout >= 0) {
      // reset socket to blocking mode
#ifdef HAVE_WINSOCK_H
  ...
  if (rc < 0) {
    ...
    if (network && (*network) && ((*network)->networkSpecific.TCP.tLayer)) {
      (*association)->connection =
          ((*network)->networkSpecific.TCP.tLayer)
              ->createConnection(s, params->useSecureLayer);
    } else
      (*association)->connection = NULL;
     ...
     * This provides a 2-4 times performance improvement (WinNT4/SP4, 100Mbit/s
     * Ethernet). Effects on other environments are unknown. The code below
     * allows the Nagle algorithm to be enabled by setting the TCP_NODELAY
     * environment variable to have value 0.
     */
    ...
    if ((tcpNoDelayString = getenv("TCP_NODELAY")) != NULL) {
      if (sscanf(tcpNoDelayString, "%d", &tcpNoDelay) != 1) {
        DCMNET_WARN("DULFSM: cannot parse environment variable TCP_NODELAY="
                    << tcpNoDelayString);
      }
    }
    if (tcpNoDelay) {
      if (setsockopt(s, IPPROTO_TCP, TCP_NODELAY, (char*)&tcpNoDelay,
                     sizeof(tcpNoDelay)) < 0) {
        char buf[256];
        OFString msg = "TCP Initialization Error: ";
        msg += OFStandard::strerror(errno, buf, sizeof(buf));
    ...
    if (TCS_ok !=
        (tcsStatus = (*association)->connection->clientSideHandshake())) {
...
static OFCondition sendAssociationRQTCP(PRIVATE_NETWORKKEY** /*network*/,
  ...
  PRV_ASSOCIATEPDU
  associateRequest;
  unsigned char buffer[4096], *b;
  unsigned long length;
  int nbytes;

  OFBitmanipTemplate<char>::zeroMem(
      (char*)&associateRequest, sizeof(PRV_ASSOCIATEPDU));  // initialize PDU
  // associateRequest.presentationContextList = NULL;
  OFCondition cond =
      constructAssociatePDU(params, DUL_TYPEASSOCIATERQ, &associateRequest);
  if (cond.bad()) {
  ...
  cond = streamAssociatePDU(&associateRequest, b, associateRequest.length + 6,
                            &length);
  ...
  do {
    nbytes = (*association)->connection
                 ? (*association)
                       ->connection->write((char*)b,
...
static OFCondition sendAssociationACTCP(PRIVATE_NETWORKKEY** /*network*/,
  ...
  OFBitmanipTemplate<char>::zeroMem(
      (char*)&associateReply, sizeof(PRV_ASSOCIATEPDU));  // initialize PDU
  ...
  if (associateReply.length + 10 <= sizeof(buffer))
    b = buffer;
  else {
  ...
  cond = streamAssociatePDU(&associateReply, b, associateReply.length + 10,
                            &length);
...
static OFCondition sendAssociationRJTCP(PRIVATE_NETWORKKEY** /*network*/,
  ...
  OFCondition cond = constructAssociateRejectPDU(
      (unsigned char)abortItems->result, (unsigned char)abortItems->source,
      (unsigned char)abortItems->reason, &pdu);
  ...
  if (cond.good()) {
    do {
      nbytes = (*association)->connection
                   ? (*association)
                         ->connection->write((char*)b, size_t(pdu.length + 6))
                   : 0;
    } while (nbytes == -1 && errno == EINTR);
...
static OFCondition sendAbortTCP(DUL_ABORTITEMS* abortItems,
  ...
  OFCondition cond = constructAbortPDU(abortItems->source, abortItems->reason,
                                       &pdu, (*association)->compatibilityMode);
  if (cond.bad())
    return cond;
...
static OFCondition sendReleaseRQTCP(PRIVATE_ASSOCIATIONKEY** association) {
  ...
  OFCondition cond =
      constructReleaseRQPDU(&pdu, (*association)->compatibilityMode);
  if (cond.bad())
    return cond;
...
static OFCondition sendPDataTCP(PRIVATE_ASSOCIATIONKEY** association,
                                DUL_PDVLIST* pdvList) {
  DUL_PDV* pdv;
  unsigned long count, length, pdvLength, maxLength;
  ...
  /* assign the amount of PDVs in the array and the PDV array itself to local
   * variables */
  count = pdvList->count;
  pdv = pdvList->pdv;

  /* determine the maximum size (length) of a PDU which can be sent over the
   * network. */
  /* Note that the name "maxPDV" here is misleading. This field contains the
   * maxPDU */
  ...
  if (maxLength == 0)
    maxLength = ASC_MAXIMUMPDUSIZE - 12;
  else if (maxLength < 14) {
    char buf[256];
    sprintf(buf,
            "DUL Cannot send P-DATA PDU because receiver's max PDU size of %lu "
            "is illegal (must be > 12)",
            maxLength);
  ...
  while (cond.good() && count-- > 0) {
    ...
    while ((firstTrip || (length > 0)) && (cond.good())) {
      ...
      /* adjust the pointer to the data, so that he points to data which still
       * has to be sent */
      p += pdvLength;
...
static OFCondition writeDataPDU(PRIVATE_ASSOCIATIONKEY** association,
  ...
  do {
    nbytes =
        (*association)->connection
            ? (*association)->connection->write((char*)head, size_t(length))
            : 0;
  } while (nbytes == -1 && errno == EINTR);
  ...
  /* send the PDU's PDV data (note that our representation of a PDU can only
   * contain one PDV.) */
  do {
    nbytes = (*association)->connection
                 ? (*association)
...
OFCondition PRV_NextPDUType(PRIVATE_ASSOCIATIONKEY** association,
                            DUL_BLOCKOPTIONS block, int timeout,
                            unsigned char* pduType) {
  OFCondition cond = EC_Normal;
  ...
  if ((*association)->inputPDU == NO_PDU) {
    ...
    cond = readPDUHead(
        association, (*association)->pduHead, sizeof((*association)->pduHead),
        block, timeout, &(*association)->nextPDUType,
        &(*association)->nextPDUReserved, &(*association)->nextPDULength);
  ...
  *buffer = (unsigned char*)malloc(size_t(maxLength));
  if (*buffer) {
    ...
    cond = readPDUBody(association, block, timeout,
                       ...
                       maxLength - sizeof((*association)->pduHead), pduType,
                       pduReserved, pduLength);
  } else
...
static OFCondition readPDUHead(PRIVATE_ASSOCIATIONKEY** association,
  ...
  if (cond.good()) {
    ...
    *PDULength = (*association)->nextPDULength;
     ...
     */
    /* the lenght of this PDU must not be greater than the specified maximum
     * length. */
    /* (bugfix - thanks to B. Gorissen, Philips Medical Systems) */
    if ((*PDUType == DUL_TYPEDATA) &&
        (*PDULength > (*association)->maxPDVInput)) {
      char buf1[256];
      sprintf(buf1, "DUL Illegal PDU Length %ld.  Max expected %ld", *PDULength,
              (*association)->maxPDVInput);
...
static OFCondition readPDUBody(PRIVATE_ASSOCIATIONKEY** association,
  ...
  cond = readPDUBodyTCP(association, block, timeout, buffer, maxLength, pduType,
                        pduReserved, pduLength);
...
static OFCondition readPDUHeadTCP(PRIVATE_ASSOCIATIONKEY** association,
                                  ...
                                  unsigned long maxLength,
                                  DUL_BLOCKOPTIONS block, int timeout,
                                  unsigned char* type, unsigned char* reserved,
                                  unsigned long* pduLength) {
  unsigned long length;
  static unsigned char legalPDUTypes[] = {
      DUL_TYPEASSOCIATERQ, DUL_TYPEASSOCIATEAC, DUL_TYPEASSOCIATERJ,
      DUL_TYPEDATA,        DUL_TYPERELEASERQ,   DUL_TYPERELEASERP,
      DUL_TYPEABORT};
  int found;
  unsigned long idx;

  /* check if the buffer is too small to capture the PDU head we are about to
   * receive */
  if (maxLength < 6) {
    return makeDcmnetCondition(
        DULC_CODINGERROR, OF_error,
        "Coding Error in DUL routine: buffer too small in readPDUTCPHead");
  }
  ...
  OFCondition cond =
      defragmentTCP((*association)->connection, block,
  ...
  DCMNET_TRACE("Read PDU HEAD TCP:"
  ...
  /* determine PDU type (captured in byte 0 of buffer) and assign it to
   * reference parameter */
  *type = *buffer++;
  ...
  *pduLength = length;
  ...
  DCMNET_TRACE("Read PDU HEAD TCP: type: "
               << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
               << STD_NAMESPACE setw(2) << (unsigned short)(*type)
               << ", length: " << STD_NAMESPACE dec << (*pduLength) << " ("
               << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
               << STD_NAMESPACE setw(2) << (unsigned int)*pduLength << ")");
...
static OFCondition readPDUBodyTCP(
  ...
  if ((*association)->inputPDU == NO_PDU) {
    /* If it does not, we need to try to receive PDU head information over the
     * network */
    cond = readPDUHead(
        association, (*association)->pduHead, sizeof((*association)->pduHead),
        block, timeout, &(*association)->nextPDUType,
  ...
  /* determine PDU type and its values in the reserved field and in the PDU
   * length field */
  *pduType = (*association)->nextPDUType;
  *pduReserved = (*association)->nextPDUReserved;
  *pduLength = (*association)->nextPDULength;
  ...
  if (*pduLength > maxLength) {
    ...
    /* if it does, read the current PDU's PDVs from the incoming socket stream.
     * (Note that the */
    /* PDVs of the current PDU are (*association)->nextPDULength bytes long.
     * Hence, in detail */
    /* we want to try to receive (*association)->nextPDULength bytes of data on
     * the network) */
    /* The information that was received will be available through the buffer
     * variable. */
    cond = defragmentTCP((*association)->connection, block,
                         (*association)->timerStart, timeout, buffer,
                         (*association)->nextPDULength, &length);
  }
...
static OFCondition defragmentTCP(DcmTransportConnection* connection,
                                 DUL_BLOCKOPTIONS block, time_t timerStart,
                                 int timeout, void* p, unsigned long l,
                                 unsigned long* rtnLen) {
  unsigned char* b;
  int bytesRead;
  ...
  if (block == DUL_NOBLOCK) {
    /* figure out how long we want to wait: if timerStart equals 0 we want to
     * wait exactly */
    /* timeout seconds starting from the call to select(...) within the below
     * called function; */
    /* if timerStart does not equal 0 we want to substract the time which has
     * already passed */
    /* after the timer was started from timeout and wait the resulting amount of
     * seconds */
  ...
  while (l > 0) {
    ...
    do {
      /* if DUL_NOBLOCK is specified as a blocking option, we only want to wait
       * a certain time for receiving data over the network. If no data was
       * received during that time, DUL_READTIMEOUT shall be returned. Note that
       * if DUL_BLOCK is specified the application will not stop waiting until
       * data is actually received over the network.
       */
      if (block == DUL_NOBLOCK) {
        /* determine remaining time to wait */
        timeToWait = timeout - (int)(time(NULL) - timerStart);

        /* go ahead and see if within timeout seconds data will be received over
         * the network. */
        /* if not, return DUL_READTIMEOUT, if yes, stay in this function. */
        if (!connection->networkDataAvailable(timeToWait))
          return DUL_READTIMEOUT;
      }
    ...
    /* if we actually received data, move the buffer pointer to its own end,
     * update the variable */
    /* that determines the end of the first loop, and update the reference
    ...
    if (bytesRead > 0) {
      ...
      /* in case we did not receive any data, an error must have occured; return
...
static OFString dump_pdu(const char* type, void* buffer, unsigned long length) {
  ...
  str << "PDU Type: " << type << ", PDU Length: " << length - 6
      << " + 6 bytes PDU header" << OFendl;
  if (length > 512) {
  ...
  while (length-- > 0) {
    str << "  " << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
        << STD_NAMESPACE setw(2) << ((unsigned int)(*p++)) << STD_NAMESPACE dec;
    if ((++position) % 16 == 0)
      str << OFendl;
  }
...
static void setTCPBufferLength(int sock) {
   ...
   * Other environments, particularly slower networks may require different
   * values for optimal performance.
   */
#ifdef HAVE_GUSI_H
  /* GUSI always returns an error for setsockopt(...) */
#else
  bufLen = 32768;  // a socket buffer size of 32K gives best throughput for
                   // image transmission
  if ((TCPBufferLength = getenv("TCP_BUFFER_LENGTH")) != NULL) {
    if (sscanf(TCPBufferLength, "%d", &bufLen) != 1) {
      DCMNET_WARN("DULFSM: cannot parse environment variable TCP_BUFFER_LENGTH="
                  << TCPBufferLength);
    }
  }
...
OFCondition translatePresentationContextList(LST_HEAD** internalList,
  ...
  *scuscpRole;
  ...
  while (context != NULL) {
    ...
    scuscpRole = findSCUSCPRole(SCUSCPRoleList, userContext->abstractSyntax);
    if (scuscpRole != NULL) {
      if (scuscpRole->SCURole == scuscpRole->SCPRole) {
        userContext->proposedSCRole = DUL_SC_ROLE_SCUSCP;
        if (scuscpRole->SCURole == 0)
          DCMNET_WARN(
              "DULFSM: both role fields are 0 in SCP/SCU role selection "
              "sub-item");
      } else if (scuscpRole->SCURole == 1)
    ...
    if (subItem == NULL) {
      char buf1[256];
      sprintf(buf1,
              "DUL Peer supplied illegal number of transfer syntaxes (%d)", 0);
      return makeDcmnetCondition(DULC_PEERILLEGALXFERSYNTAXCOUNT, OF_error,
                                 buf1);
    }
    ...
    while (subItem != NULL) {
      ...
      cond = LST_Enqueue(&userContext->proposedTransferSyntax,
                         (LST_NODE*)transfer);
      if (cond.bad())
        return cond;
...
**   failures on systems with high CPU load if an association timout was
*specified
...
** Added global option flag for compatibility with very old DCMTK releases in
*the
**   DICOM upper layer and ACSE code. Default is automatic handling, which
*should
**   work in most cases.
**
...
** Fixed bugs related to association negotiation in the DICOM upper layer
*module.
