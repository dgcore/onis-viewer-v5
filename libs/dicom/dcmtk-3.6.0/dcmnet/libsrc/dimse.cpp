...
static void saveDimseFragment(DcmDataset* dset, OFBool isCommand,
                              OFBool isReceive)
/*
 ...
 *   isCommand - [in] Specifies if dset's information refers to a DIMSE command
 * (as for example C-STORE) (OFTrue) or if it refers to instance data (OFFalse).
 *   isReceive - [in] Specifies if this application received (OFTrue) or sent
 * (OFFalse) the information in dset.
 */
{
  /* depending on if the information in dset refers to a DIMSE command or not,
  ...
  /* create a string which will contain the name of the file we are about to
   * write to */
  /* filenames are supposed to look like "dimse-cmd-[rcv|snd]-[counter].dcm" */
  const char* transmission = (isReceive ? "rcv" : "snd");
  char filename[2048];
  if (isCommand) {
    sprintf(filename, "dimse-cmd-%s-%04lu.dcm", transmission,
            g_dimse_commandCounter);
  } else {
    if (g_dimse_dataCounter < 2) {
      sprintf(filename, "dimse-dat-%s-%04lu.dcm", transmission,
              g_dimse_commandCounter);
    } else {
      sprintf(filename, "dimse-dat-%s-%04lu-%02lu.dcm", transmission,
              g_dimse_commandCounter, g_dimse_dataCounter);
    }
  }

  /* write data to file */
  dset->saveFile(filename, EXS_LittleEndianImplicit, EET_ExplicitLength,
                 EGL_recalcGL, EPD_withoutPadding);
  return;
}

static OFBool isDataDictPresent() {
...
static OFCondition DIMSE_readNextPDV(T_ASC_Association* assoc,
                                     T_DIMSE_BlockingMode blocking, int timeout,
                                     DUL_PDV* pdv)
/*
 * This function returns the next PDV which was (earlier or just now) received
 * on the incoming socket stream. If there are no PDVs (which were already
 * received earlier) waiting to be picked up, this function will go ahead and
 * read a new PDU (containing one or more new PDVs) from the incoming socket
 * stream.
 *
 * Parameters:
 *   assoc    - [in] The association (network connection to another DICOM
 * application). blocking - [in] The blocking mode for reading data (either
 * DIMSE_BLOCKING or DIMSE_NONBLOCKING) timeout  - [in] Timeout interval for
 * receiving data. If the blocking mode is DIMSE_NONBLOCKING and we are trying
 * to read data from the incoming socket stream and no data has been received
 * after timeout seconds, an error will be reported. pdv      - [out] Contains
 * in the end the next PDV which was (earlier or just now) received on the
 ...
 */
{
  ...
  /* get the next PDV from the association, in case there are still some PDVs
  ...
  if (cond.bad()) {
    ...
    /* if the blocking mode is DIMSE_NONBLOCKING and there is no data waiting
     * after timeout seconds, report an error */
    if (blocking == DIMSE_NONBLOCKING) {
      if (!ASC_dataWaiting(assoc, timeout))
        return DIMSE_NODATAAVAILABLE;
    }

    /* try to receive new PDVs on the incoming socket stream (in detail, try to
     * receive one PDU) */
    cond = DUL_ReadPDVs(&assoc->DULassociation, NULL, blk, timeout);

    /* check return value, if it is different from DUL_PDATAPDUARRIVED, an error
     * occured */
    if (cond != DUL_PDATAPDUARRIVED) {
      if (cond == DUL_NULLKEY || cond == DUL_ILLEGALKEY)
        ...
        return makeDcmnetSubCondition(DIMSEC_READPDVFAILED, OF_error,
...
static OFCondition getTransferSyntax(T_ASC_Association* assoc,
                                     T_ASC_PresentationContextID pid,
                                     E_TransferSyntax* xferSyntax)
/*
 * This function checks if the presentation context id which was passed refers
 * to a valid presentation context. If this is the case, this function
 * determines the transfer syntax the presentation context ID refers to (will be
 * returned to the user) and also checks if dcmtk supports this transfer syntax.
 *
 * Parameters:
 *   assoc      - [in] The association (network connection to another DICOM
 * application). pid        - [in] The id of the presentation context which
 * shall be checked regarding validity. xferSyntax - [out] If pid refers to a
 * valuid presentation context, this variable contains in the end the transfer
 * syntax which is specified in the presentation context.
 */
{
  ...
  OFCondition cond =
      ASC_findAcceptedPresentationContext(assoc->params, pid, &pc);
  if (cond.bad()) {
    return makeDcmnetSubCondition(DIMSEC_RECEIVEFAILED, OF_error,
                                  "DIMSE Failed to receive message", cond);
  }

  /* determine the transfer syntax which is specified in the presentation
   * context */
  ts = pc.acceptedTransferSyntax;

  /* create a DcmXfer object on the basis of the transfer syntax which was
   * determined above */
  DcmXfer xfer(ts);
  ...
  *xferSyntax = xfer.getXfer();
  switch (*xferSyntax) {
    ...
    case EXS_DeflatedLittleEndianExplicit:
      ...
      break;
    default:
      /* all other transfer syntaxes are not supported; hence, set the error
       * indicator variable */
      {
        char buf[256];
        ...
        cond =
            makeDcmnetSubCondition(DIMSEC_RECEIVEFAILED, OF_error,
...
static OFCondition checkPresentationContextForMessage(
    T_ASC_Association* assoc, T_DIMSE_Message* msg,
    T_ASC_PresentationContextID presID, E_TransferSyntax* xferSyntax)
/*
 * This function checks if the presentation context id refers to a valid
 * presentation context and determines the transfer syntax which is specified
 * for this presentation context. Additionally, the function checks if the
 * specified transfer syntax is supported at all. If any of the checks returns
 * an error, return a corresponding error..
 *
 * Parameters:
 *   assoc                - [in] The association (network connection to another
 * DICOM application). msg                  - [in] Structure that represents a
 * certain DIMSE message. presID               - [in] The ID of the presentation
 * context which shall be used. xferSyntax           - [out] Contains in the end
 * the transfer syntax which is specified in the presentation context with the
 * corresponding ID.
 */
{
  ...
  /* try to find the accepted presentation context on the basis of the ID which
   * was passed */
  OFCondition cond =
  ...
  /* if the accepted presentation context was found, check if the message type
   * is supported */
  if (cond.good()) {
...
static OFCondition validateMessage(T_ASC_Association* assoc,
                                   T_DIMSE_Message* msg)
/*
 * This function checks if the information which is contained in msg meets
 * certain conditions. For example, if msg represents a C-ECHO-RQ, then there is
 * not supposed to be a corresponding data set. If the specified conditions are
 * not met, return an error.
 *
 * Parameters:
 *   assoc - [in] The association (network connection to another DICOM
 * application). msg   - [in] Structure that represents a certain DIMSE message.
 */
{
  ...
  switch (msg->CommandField) {
    ...
    case DIMSE_C_STORE_RQ:
      ...
      if (!IN_RANGE(strlen(msg->msg.CStoreRQ.AffectedSOPInstanceUID), 1,
                    DIC_UI_LEN)) {
        DCMNET_WARN(DIMSE_warn_str(assoc)
                    << "C-Store RQ: AffectedSOPInstanceUID: bad size");
        cond = DIMSE_BADMESSAGE;
      }
      break;
    case DIMSE_C_STORE_RSP:
      if (msg->msg.CStoreRSP.DataSetType != DIMSE_DATASET_NULL) {
        DCMNET_WARN(DIMSE_warn_str(assoc)
                    << "C-Store RSP: DataSetType != NULL");
        cond = DIMSE_BADMESSAGE;
      }
      if ((msg->msg.CStoreRSP.opts & O_STORE_AFFECTEDSOPINSTANCEUID) &&
          (!IN_RANGE(strlen(msg->msg.CStoreRSP.AffectedSOPInstanceUID), 1,
                     DIC_UI_LEN))) {
        DCMNET_WARN(DIMSE_warn_str(assoc)
                    << "C-Store RSP: AffectedSOPInstanceUID: bad size");
        cond = DIMSE_BADMESSAGE;
      }
      break;
    case DIMSE_C_GET_RQ:
...
static OFCondition sendDcmDataset(T_ASC_Association* assoc, DcmDataset* obj,
                                  T_ASC_PresentationContextID presID,
                                  E_TransferSyntax xferSyntax,
                                  DUL_DATAPDV pdvType,
                                  DIMSE_ProgressCallback callback,
                                  void* callbackContext)
/*
 * This function sends all information which is included in a DcmDataset object
 * over the network which is provided in assoc.
 *
 * Parameters:
 *   assoc           - [in] The association (network connection to another DICOM
 * application). obj             - [in] Contains the information which shall be
 * sent over the network. presId          - [in] The ID of the presentation
 * context which shall be used xferSyntax      - [in] The transfer syntax which
 * shall be used. pdvType         - [in] Specifies if the information in this
 * DcmDataset object belongs to a DIMSE command (as for example C-STORE)
 * (DUL_COMMANDPDV) or if the information is actual instance information
 * (DUL_DATASETPDV). callback        - [in] Pointer to a function which shall be
 * called to indicate progress. callbackContext - []
 */
{
  OFCondition dulCond = EC_Normal;
  ...
  /* initialize some local variables (we want to use the association's send
   * buffer */
  ...
  /* on the basis of the association's buffer, create a buffer variable that we
   * can write to */
  ...
  /* initialize two more variables: groupLength_encoding specifies what will be
   * done concerning */
  /* group length tags, sequenceType_encoding specifies how sequences will be
   * handled */
  ...
  /* start a loop: in each iteration information from the DcmDataset object
   * (i.e. infor- */
  /* mation which shall be sent) will be set in the buffer (we need more than
   * one itera- */
  /* tion if there is more information than the buffer can take at a time), a
   * PDV object */
  /* with the buffer's data will be created and assigned to a list, and finally
   * the */
  /* list's information will be sent over the network to the other DICOM
   * application. */
  while (!last) {
    /* write data values which are contained in the DcmDataSet variable to the
     * above created */
    /* buffer. Mind the transfer syntax, the sequence type encoding, the group
     * length encoding */
    /* and remove all padding data elements. Depending on whether all
     * information has been written */
    /* to the buffer, update the variable that determines the end of the while
     * loop. (Note that */
    /* DcmDataset stores information about what of its content has already been
     * sent to the buffer.) */
    if (!written) {
      ...
      if (econd == EC_Normal) /* all contents have been written to the buffer */
      {
        written = OFTrue;
                 ...
                 EC_StreamNotifyClient) /* no more space in buffer, _not_ all
                                           elements have been written to it */
      {
        // nothing to do
      } else /* some error has occurred */
      {
        DCMNET_WARN(DIMSE_warn_str(assoc)
                    << "writeBlock Failed (" << econd.text() << ")");
    ...
    if (rtnLength > 0) {
      ...
      DCMNET_TRACE("DIMSE sendDcmDataset: sending " << pdv.fragmentLength
                                                    << " bytes");

      /* send information over the network to the other DICOM application */
      dulCond = DUL_WritePDVs(&assoc->DULassociation, &pdvList);
      if (dulCond.bad())
        return makeDcmnetSubCondition(DIMSEC_SENDFAILED, OF_error,
                                      "DIMSE Failed to send message", dulCond);
...
static OFCondition DIMSE_sendMessage(
    T_ASC_Association* assoc, T_ASC_PresentationContextID presID,
    T_DIMSE_Message* msg, DcmDataset* statusDetail, DcmDataset* dataObject,
    const char* dataFileName, DIMSE_ProgressCallback callback,
    void* callbackContext, DcmDataset** commandSet)
/*
 * This function sends a DIMSE command and possibly also instance data from a
 * file or from a given data object via network to another DICOM application.
 *
 * Parameters:
 *   assoc           - [in] The association (network connection to another DICOM
 * application). presId          - [in] The ID of the presentation context which
 * shall be used msg             - [in] Structure that represents a certain
 * DIMSE command which shall be sent. statusDetail    - [in] Detailed
 * information with regard to the status information which is captured in the
 * status element (0000,0900). Note that the value for element (0000,0900) is
 * contained in this variable. dataObject      - [in] The instance data which
 * shall be sent to the other DICOM application, NULL, if there is none
 *   dataFileName    - [in] The name of the file that contains the instance data
 * which shall be sent to the other DICOM application, NULL; if there is none.
 *   callback        - [in] Pointer to a function which shall be called to
 * indicate progress. callbackContext - [] commandSet      - [out] [optional
 * parameter, default = NULL] If this parameter is not NULL it will return a
 * copy of the DIMSE command which is sent to the other DICOM application.
 */
{
  E_TransferSyntax xferSyntax;
   ...
   */
  /* C-ECHO-RQ, then there is not supposed to be a corresponding data set. If
   * the */
  /* specified conditions are not met, return an error. */
  if (EC_Normal != (cond = validateMessage(assoc, msg)))
    return cond;

  /* check if the presentation context id refers to a valid presentation context
   * and determine the */
  /* transfer syntax which is specified for this presentation context.
   * Additionally, check if the specified */
  /* transfer syntax is supported at all. If any of the checks returns an error,
   * return this error. */
  if (EC_Normal != (cond = checkPresentationContextForMessage(
                        assoc, msg, presID, &xferSyntax)))
    return cond;

  /* create a DcmDataset object ("command object") based on the information in
   * the DIMSE command */
  /* variable (remember that all DICOM commands are - in the end - particular
   * data sets). The */
  /* information which will shortly be set in this object will be sent over the
   * network later. */
  cond = DIMSE_buildCmdObject(msg, &cmdObj);

  /* if the command object has been created successfully and there is status
   * detail */
  ...
  /* if the command object has been created successfully and the data set is
   * present */
  /* according to the information in the DIMSE command information variable, do
   * something */
  if (cond.good() && DIMSE_isDataSetPresent(msg)) {
    ...
    if ((dataObject != NULL) && (dataFileName != NULL)) {
      DCMNET_WARN(
          DIMSE_warn_str(assoc)
          << "sendData: both object and file specified (sending object only)");
    }
    /* if there is no data object but a file name, we need to read data from the
       specified file */
     ...
     */
      ...
      if (!dcmff.loadFile(dataFileName, EXS_Unknown).good()) {
        char buf[256];
        DCMNET_WARN(DIMSE_warn_str(assoc)
                    << "sendMessage: cannot open DICOM file (" << dataFileName
    ...
    /* if we have a data object now, check if we can write the data object's
     * elements in the required  */
    /* transfer syntax. In detail, every single item of the data object will be
     * checked. */
    if (dataObject) {
      if (dataObject->isEmpty()) {
        /* if dataset is empty, i.e. it contains no data elements, create a
         * warning. */
        DCMNET_WARN(DIMSE_warn_str(assoc) << "sendMessage: dataset is empty");
        cond = DIMSE_SENDFAILED;
      } else if (!dataObject->canWriteXfer(xferSyntax)) {
        /* if we cannot write all elements in the required transfer syntax,
         * create a warning. */
        DcmXfer writeXferSyntax(xferSyntax);
        DcmXfer originalXferSyntax(dataObject->getOriginalXfer());
        if (fromFile && dataFileName) {
          DCMNET_WARN(DIMSE_warn_str(assoc)
                      << "sendMessage: unable to convert DICOM file '"
                      << dataFileName << "' from '"
                      << originalXferSyntax.getXferName()
                      << "' transfer syntax to '"
                      << writeXferSyntax.getXferName() << "'");
        } else {
          DCMNET_WARN(DIMSE_warn_str(assoc)
                      << "sendMessage: unable to convert dataset from '"
                      << originalXferSyntax.getXferName()
                      << "' transfer syntax to '"
                      << writeXferSyntax.getXferName() << "'");
        }
      ...
      /* if there is neither a data object nor a file name, create a warning,
       * since */
  ...
  if (cond.good()) {
    ...
    DCMNET_TRACE("DIMSE Command to be sent on Presentation Context ID: "
                 << OFstatic_cast(Uint16, presID));
    DCMNET_TRACE("DIMSE Command to send:" << OFendl
                                          << DcmObject::PrintHelper(*cmdObj));

    /* Send the DIMSE command. DIMSE commands are always little endian implicit.
     */
    cond = sendDcmDataset(assoc, cmdObj, presID, EXS_LittleEndianImplicit,
                          DUL_COMMANDPDV, NULL, NULL);
  }

  /* Then we still have to send the actual instance data if the DIMSE command
   * information variable */
  /* says that instance data is present and there actually is a corresponding
   * data object */
  if (cond.good() && DIMSE_isDataSetPresent(msg) && (dataObject)) {
    /* again, if the global variable says so, we want to save the instance data
     * to a file */
    if (g_dimse_save_dimse_data)
...
OFCondition DIMSE_sendMessageUsingFileData(
    T_ASC_Association* assoc, T_ASC_PresentationContextID presID,
    T_DIMSE_Message* msg, DcmDataset* statusDetail, const char* dataFileName,
    DIMSE_ProgressCallback callback, void* callbackContext,
    DcmDataset** commandSet)
/*
 * This function sends a DIMSE command and possibly also instance data from a
 * file via network to another DICOM application.
 *
 * Parameters:
 *   assoc           - [in] The association (network connection to another DICOM
 * application). presId          - [in] The ID of the presentation context which
 ...
 * information with regard to the status information which is captured in the
 * status element (0000,0900). Note that the value for element (0000,0900) is
 * contained in this variable. dataFileName    - [in] The name of the file that
 * contains the instance data which shall be sent to the other DICOM
 * application, NULL; if there is none. callback        - [in] Pointer to a
 * function which shall be called to indicate progress. callbackContext - []
 *   commandSet      - [out] [optional parameter, default = NULL] If this
 * parameter is not NULL it will return a copy of the DIMSE command which is
 * sent to the other DICOM application.
 */
{
  /* simply call DIMSE_sendMessage to accomplish this task */
  return DIMSE_sendMessage(assoc, presID, msg, statusDetail, NULL, dataFileName,
                           callback, callbackContext, commandSet);
}

OFCondition DIMSE_sendMessageUsingMemoryData(
    T_ASC_Association* assoc, T_ASC_PresentationContextID presID,
    T_DIMSE_Message* msg, DcmDataset* statusDetail, DcmDataset* dataObject,
    DIMSE_ProgressCallback callback, void* callbackContext,
    DcmDataset** commandSet)
 ...
 * status element (0000,0900). Note that the value for element (0000,0900) is
 * contained in this variable. dataObject      - [in] The instance data which
 * shall be sent to the other DICOM application, NULL, if there is none callback
 * - [in] Pointer to a function which shall be called to indicate progress.
 *   callbackContext - []
 *   commandSet      - [out] [optional parameter, default = NULL] If this
 * parameter is not NULL it will return a copy of the DIMSE command which is
 * sent to the other DICOM application.
...
{
  /* simply call DIMSE_sendMessage to accomplish this task */
  return DIMSE_sendMessage(assoc, presID, msg, statusDetail, dataObject, NULL,
                           callback, callbackContext, commandSet);
}
...
OFCondition DIMSE_ignoreDataSet(T_ASC_Association* assoc,
                                T_DIMSE_BlockingMode blocking, int timeout,
...
OFCondition DIMSE_receiveCommand(T_ASC_Association* assoc,
                                 T_DIMSE_BlockingMode blocking, int timeout,
                                 T_ASC_PresentationContextID* presID,
                                 T_DIMSE_Message* msg,
                                 ...
                                 DcmDataset** commandSet)
/*
 * This function revceives a DIMSE command via network from another DICOM
 * application.
 *
 * Parameters:
 *   assoc        - [in] The association (network connection to another DICOM
 ...
 * receiving data. If the blocking mode is DIMSE_NONBLOCKING and we are trying
 * to read data from the incoming socket stream and no data has been received
 * after timeout seconds, an error will be reported. presId       - [out]
 * Contains in the end the ID of the presentation context which was specified in
 * the DIMSE command. msg          - [out] Contains in the end information which
 * represents a certain DIMSE command which was received. statusDetail - [out]
 * If a non-NULL value is passed this variable will in the end contain detailed
 *                        information with regard to the status information
 * which is captured in the status element (0000,0900). Note that the value for
 * element (0000,0900) is not contained in this return value but in msg. For
 * details on the structure of this object, see DICOM standard (year 2000) part
 * 7, annex C) (or the corresponding section in a later version of the
 * standard.) commandSet   - [out] [optional parameter, default = NULL] If this
 * parameter is not NULL it will return a copy of the DIMSE command which was
 * received from the other DICOM application.
 */
{
  OFCondition cond = EC_Normal;
  ...
  /* create a new DcmDataset variable to capture the DIMSE command which we are
   * about to receive */
  ...
  /* create a buffer variable which can be used to store the received
   * information */
  ...
  /* start a loop in which we want to read a DIMSE command from the incoming
   * socket stream. */
  /* Since the command could stretch over more than one PDU, the use of a loop
   * is mandatory. */
  for (last = OFFalse, bytesRead = 0, type = DUL_COMMANDPDV;
    ...
    if (cond.bad() || (cond == DUL_PEERREQUESTEDRELEASE)) {
      ...
      if (cond == DIMSE_READPDVFAILED)
        ...
        return cond; /* it was an abort or release request */
    }

    /* if this is the first loop iteration, get the presentation context ID
     * which is captured in the */
    /* current PDV. If this is not the first loop iteration, check if the
     * presentation context IDs in */
    /* the current PDV and in the last PDV are identical. If they are not,
     * return an error. */
    if (pdvCount == 0) {
      ...
      delete cmdSet;
      char buf1[256];
      sprintf(buf1, "DIMSE: Different PresIDs inside Command Set: %d != %d",
              pid, pdv.presentationContextID);
      OFCondition subCond = makeDcmnetCondition(
          DIMSEC_INVALIDPRESENTATIONCONTEXTID, OF_error, buf1);
      return makeDcmnetSubCondition(DIMSEC_RECEIVEFAILED, OF_error,
                                    "DIMSE Failed to receive message", subCond);
    }
    ...
    /* if information is contained the PDVs fragment, we want to insert this
     * information into the buffer */
    if (pdv.fragmentLength > 0) {
      cmdBuf.setBuffer(pdv.data, pdv.fragmentLength);
    }

    /* if this fragment contains the last fragment of the DIMSE command, set the
     * end of the stream */
    if (pdv.lastPDV) {
      cmdBuf.setEos();
    }

    /* insert the information which is contained in the buffer into the
     * DcmDataset */
    /* variable. Mind that DIMSE commands are always specified in the little
     * endian */
    /* implicit transfer syntax. Additionally, we want to remove group length
     * tags. */
    econd = cmdSet->read(cmdBuf, EXS_LittleEndianImplicit, EGL_withoutGL);
    if (econd != EC_Normal && econd != EC_StreamNotifyClient) {
      delete cmdSet;
      return makeDcmnetSubCondition(
          DIMSEC_RECEIVEFAILED, OF_error,
          "DIMSE: receiveCommand: cmdSet->read() Failed", econd);
    }

    /* update the counter that counts how many bytes were read from the incoming
     * socket */
    /* stream. This variable will only be used for dumpimg general information.
     */
    bytesRead += pdv.fragmentLength;

    /* update the following variables which will be evaluated at the beginning
     * of each loop iteration. */
    last = pdv.lastPDV;
    type = pdv.pdvType;

    /* update the counter that counts how many PDVs were received on the
     * incoming */
  ...
  /* dump information if required */
  DCMNET_TRACE("DIMSE receiveCommand: " << pdvCount << " PDVs (" << bytesRead
                                        << " bytes), PresID=" << (int)pid);
  ...
  if (type != DUL_COMMANDPDV) {
    delete cmdSet;
    OFCondition subCond = makeDcmnetCondition(
        DIMSEC_UNEXPECTEDPDVTYPE, OF_error, "DIMSE: Command PDV Expected");
    return makeDcmnetSubCondition(DIMSEC_RECEIVEFAILED, OF_error,
                                  "DIMSE Failed to receive message", subCond);
  }
  ...
  /* DIMSE command's information to a file */
  if (g_dimse_save_dimse_data)
    saveDimseFragment(cmdSet, OFTrue, OFTrue);

  /* return a copy of the DIMSE command if required */
  if (commandSet)
    *commandSet = new DcmDataset(*cmdSet);

  /* dump some more information if required */
  DCMNET_TRACE("DIMSE Command Received:" << OFendl
                                         << DcmObject::PrintHelper(*cmdSet));
  ...
  /* if the T_DIMSE_Message structure was created successfully, validate the
   * message, i.e. */
...
OFCondition DIMSE_createFilestream(const char* filename,
  ...
  cond = ASC_findAcceptedPresentationContext(assoc->params, presIdCmd,
                                             &presentationContext);
  if (cond.bad())
    return cond;
  ...
  if (writeMetaheader) {
    ...
    if (NULL !=
        (elem = new DcmOtherByteOtherWord(fileMetaInformationVersion))) {
      metainfo->insert(elem, OFTrue);
      Uint8 version[2] = {0, 1};
    ...
    if (NULL != (elem = new DcmShortString(implementationVersionName))) {
      ...
      cond = EC_MemoryExhausted;
    if (NULL !=
        (elem = new DcmApplicationEntity(sourceApplicationEntityTitle))) {
      metainfo->insert(elem, OFTrue);
      const char* aet = assoc->params->DULparams.callingAPTitle;
      if (aet)
        ((DcmApplicationEntity*)elem)->putString(aet);
    } else
      cond = EC_MemoryExhausted;
    ...
    cond = metainfo->computeGroupLengthAndPadding(
  ...
  *filestream = new DcmOutputFileStream(filename);
  ...
  if (metainfo) {
    ...
    if (EC_Normal != metainfo->write(**filestream,
                                     META_HEADER_DEFAULT_TRANSFERSYNTAX,
                                     EET_ExplicitLength, NULL)) {
      char buf2[4096];  // file names could be long!
      sprintf(buf2,
              "DIMSE createFilestream: cannot write metaheader to file '%s'",
              filename);
...
OFCondition DIMSE_receiveDataSetInFile(
    T_ASC_Association* assoc, T_DIMSE_BlockingMode blocking, int timeout,
    T_ASC_PresentationContextID* presID, DcmOutputStream* filestream,
  ...
  while (!last) {
    ...
    if (!last) {
      if (pdvCount == 0) {
        ...
        char buf1[256];
        sprintf(buf1, "DIMSE: Different PresIDs inside data set: %d != %d", pid,
    ...
    if (!last) {
      ...
      if ((!filestream->good()) || (written != (Uint32)(pdv.fragmentLength))) {
        cond = DIMSE_ignoreDataSet(assoc, blocking, timeout, &bytesRead,
                                   &pdvCount);
        if (cond == EC_Normal) {
          cond = makeDcmnetCondition(
              DIMSEC_OUTOFRESOURCES, OF_error,
              "DIMSE receiveDataSetInFile: Cannot write to file");
        }
        last = OFTrue;  // terminate loop
      }
    }
    ...
    if (!last) {
      ...
      DCMNET_TRACE("DIMSE receiveDataSetInFile: "
                   << pdv.fragmentLength << " bytes read (last: "
                   << ((last) ? ("YES") : ("NO")) << ")");
...
OFCondition DIMSE_receiveDataSetInMemory(
    T_ASC_Association* assoc, T_DIMSE_BlockingMode blocking, int timeout,
    T_ASC_PresentationContextID* presID, DcmDataset** dataObject,
    DIMSE_ProgressCallback callback, void* callbackData)
/*
 * This function receives one data set (of instance data) via network from
 * another DICOM application.
 *
 * Parameters:
 *   assoc           - [in] The association (network connection to another DICOM
 * application). blocking        - [in] The blocking mode for receiving data
 * (either DIMSE_BLOCKING or DIMSE_NONBLOCKING) timeout         - [in] Timeout
 * interval for receiving data (if the blocking mode is DIMSE_NONBLOCKING).
 *   presID          - [out] Contains in the end the ID of the presentation
 * context which was used in the PDVs that were received on the network. If the
 * PDVs show different presentation context IDs, this function will return an
 * error. dataObject      - [out] Contains in the end the information which was
 * received over the network. Note that this function assumes that either
 * imageFileName or imageDataSet does not equal NULL. callback        - [in]
 * Pointer to a function which shall be called to indicate progress.
 *   callbackData    - [in] Pointer to data which shall be passed to the
 ...
 */
{
  ...
  /* check if the caller provided an address where the data set can be stored.
  ...
  /* check if there is still no DcmDataset object which can be used to store the
   * data set. */
  if (dset == NULL) {
    /* if this is the case, just go ahead an receive data, but do not store it
     * anywhere */
    cond = DIMSE_ignoreDataSet(assoc, blocking, timeout, &bytesRead, &pdvCount);

    /* if receiving was successful, let the caller know though that no
     * DcmDataset variable could be created */
    if (cond == EC_Normal) {
      return makeDcmnetCondition(
          DIMSEC_OUTOFRESOURCES, OF_error,
          "DIMSE receiveDataSetInMemory: Cannot create DcmDataset");
    }
  ...
  /* start a loop in which we want to read the data set from the incoming socket
   * stream. */
  /* Since the data set could stretch over more than one PDU, the use of a loop
   * is mandatory. */
  while (!last && cond == EC_Normal) {
    ...
    /* we are expecting to see a data set PDV; if the PDV which was received
     * does not refer */
    /* to a data set, set the error indicating variable correspondingly and end
     * the loop. */
    if (!last) {
      if (pdv.pdvType != DUL_DATASETPDV) {
        cond = DIMSE_UNEXPECTEDPDVTYPE;
    ...
    /* if this is the first loop iteration, get the presentation context ID
     * which is captured */
    /* in the current PDV and determine what transfer syntax the presentation
     * context ID refers */
    /* to (this is also a check concerning the question if the presentation
     * context ID is valid). */
    /* If this is not the first loop iteration, check if the presentation
     * context IDs in the */
    /* current PDV and in the last PDV are identical. If they are not, return an
     * error. */
    if (!last) {
      if (pdvCount == 0) {
        pid = pdv.presentationContextID;
    ...
    if (!last) {
      ...
      /* if this fragment contains the last fragment of the data set, set the
       * end of the stream */
      if (pdv.lastPDV) {
        dataBuf.setEos();
      }
      ...
      /* insert the information which is contained in the buffer into the
       * DcmDataset variable. Mind the */
      /* transfer syntax which was specified through the presentation context ID
       * of the first PDV. */
      econd = dset->read(dataBuf, xferSyntax);
      if (econd != EC_Normal && econd != EC_StreamNotifyClient) {
        DCMNET_WARN(DIMSE_warn_str(assoc)
                    << "DIMSE receiveDataSetInMemory: dset->read() Failed ("
                    << econd.text() << ")");
    ...
    if (!last) {
      /* update the counter that counts how many bytes were read from the
       * incoming socket */
      /* stream. This variable will only be used for dumpimg general
       * information. */
      bytesRead += pdv.fragmentLength;
      ...
      /* update the variable which will be evaluated at the beginning of each
       * loop iteration. */
      last = pdv.lastPDV;

      /* dump information if required */
      DCMNET_TRACE("DIMSE receiveDataSetInMemory: "
                   << pdv.fragmentLength << " bytes read (last: "
                   << ((last) ? ("YES") : ("NO")) << ")");
  ...
  /* DIMSE command's information to a file */
  if (g_dimse_save_dimse_data)
    saveDimseFragment(dset, OFFalse, OFTrue);
...
**   large files (2 GBytes or more) if supported by compiler and operating
*system.
**
...
**   by an OFCondition based implementation which eliminates the global
*condition
