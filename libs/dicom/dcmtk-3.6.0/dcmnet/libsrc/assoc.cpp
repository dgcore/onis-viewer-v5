...
OFCondition ASC_initializeNetwork(T_ASC_NetworkRole role, int acceptorPort,
                                  int timeout, T_ASC_Network** network,
  ...
  OFCondition cond = DUL_InitializeNetwork(
      mode, &acceptorPort, timeout, DUL_ORDERBIGENDIAN | options, &netkey);
  if (cond.bad())
    return cond;
...
OFCondition ASC_createAssociationParameters(T_ASC_Parameters** params,
  ...
  *params = (T_ASC_Parameters*)malloc(sizeof(**params));
  ...
  strncpy((*params)->ourImplementationClassUID, OFFIS_IMPLEMENTATION_CLASS_UID,
  ...
  ASC_setAPTitles(*params, "calling AP Title", "called AP Title",
                  "resp. AP Title");

  /* make sure max pdv length is even */
  if ((maxReceivePDUSize % 2) != 0) {
    DCMNET_WARN("ASSOC: PDV receive length " << maxReceivePDUSize
                                             << " is odd (using "
                                             << (maxReceivePDUSize - 1) << ")");
    maxReceivePDUSize--;
  }
  if (maxReceivePDUSize < ASC_MINIMUMPDUSIZE) {
    DCMNET_WARN("ASC_createAssociationParameters: maxReceivePDUSize "
                << maxReceivePDUSize << " too small (using "
                << ASC_MINIMUMPDUSIZE << ")");
  ...
  ASC_setPresentationAddresses(*params, "calling Presentation Address",
...
static void destroyPresentationContextList(LST_HEAD** lst) {
  ...
  while ((pc = (DUL_PRESENTATIONCONTEXT*)LST_Dequeue(lst)) != NULL) {
    if (pc->proposedTransferSyntax != NULL) {
      while ((ts = (DUL_TRANSFERSYNTAX*)LST_Dequeue(
                  &pc->proposedTransferSyntax)) != NULL) {
        free(ts);
      }
...
OFCondition ASC_destroyAssociationParameters(T_ASC_Parameters** params)
/*
...
OFCondition ASC_setAPTitles(T_ASC_Parameters* params,
...
OFCondition ASC_getAPTitles(T_ASC_Parameters* params, char* callingAPTitle,
                            char* calledAPTitle, char* respondingAPTitle)
/*
...
OFCondition ASC_getApplicationContextName(T_ASC_Parameters* params,
 ...
 */
{
  if (applicationContextName)
    strcpy(applicationContextName, params->DULparams.applicationContextName);
  return EC_Normal;
}

OFCondition ASC_setPresentationAddresses(T_ASC_Parameters* params,
...
OFCondition ASC_getPresentationAddresses(T_ASC_Parameters* params,
...
OFCondition ASC_getRejectParameters(T_ASC_Parameters* params,
...
OFString& ASC_printRejectParameters(OFString& str,
  ...
  switch (rej->source) {
    ...
    case ASC_SOURCE_SERVICEPROVIDER_ACSE_RELATED:
      source = "Service Provider (ACSE Related)";
      break;
    case ASC_SOURCE_SERVICEPROVIDER_PRESENTATION_RELATED:
      source = "Service Provider (Presentation Related)";
      break;
    default:
      source = "UNKNOWN";
      break;
...
static T_ASC_SC_ROLE dulRole2ascRole(DUL_SC_ROLE role) {
  ...
  switch (role) {
    ...
    default:
      ar = ASC_SC_ROLE_DEFAULT;
      break;
  }
  return ar;
}

static DUL_SC_ROLE ascRole2dulRole(T_ASC_SC_ROLE role) {
  ...
  switch (role) {
    ...
    default:
      dr = DUL_SC_ROLE_DEFAULT;
      break;
  }
  return dr;
}

static const char* ascRole2String(T_ASC_SC_ROLE role) {
  const char* s = NULL;
  switch (role) {
    case ASC_SC_ROLE_SCU:
      s = "SCU";
...
static DUL_PRESENTATIONCONTEXT* findPresentationContextID(
    LST_HEAD* head, T_ASC_PresentationContextID presentationContextID) {
  DUL_PRESENTATIONCONTEXT* pc;
  LST_HEAD** l;
...
OFCondition ASC_addPresentationContext(
    T_ASC_Parameters* params, T_ASC_PresentationContextID presentationContextID,
    const char* abstractSyntax, const char* transferSyntaxList[],
    int transferSyntaxListCount, T_ASC_SC_ROLE proposedRole)
/*
 ...
 */
{
  ...
  pc = findPresentationContextID(params->DULparams.requestedPresentationContext,
                                 presentationContextID);
  if (pc) {
    char buf[256];
    sprintf(buf, "ASC Duplicate presentation context ID: %d",
            presentationContextID);
    return makeDcmnetCondition(ASCC_DUPLICATEPRESENTATIONCONTEXTID, OF_error,
                               buf);
  }
...
int ASC_countPresentationContexts(T_ASC_Parameters* params)
/*
...
int ASC_countAcceptedPresentationContexts(T_ASC_Parameters* params)
...
OFCondition ASC_getPresentationContext(
    T_ASC_Parameters* params, int listPosition,
 ...
 */
{
  ...
  if (params->DULparams.requestedPresentationContext == NULL) {
    ...
    return makeDcmnetCondition(ASCC_BADPRESENTATIONCONTEXTPOSITION, OF_error,
                               buf);
  }
  l = &(params->DULparams.requestedPresentationContext);
  if (*l == NULL) {
  ...
  while (transfer != NULL) {
    if (count >= DICOM_MAXTRANSFERSYNTAXES) {
      return makeDcmnetCondition(
          ASCC_CODINGERROR, OF_error,
          "ASC Coding error in ASC_getPresentationContext: too many transfer "
          "syntaxes");
    }
...
OFCondition ASC_acceptPresentationContext(
    T_ASC_Parameters* params, T_ASC_PresentationContextID presentationContextID,
    const char* transferSyntax, T_ASC_SC_ROLE acceptedRole)
/*
 ...
 */
{
  ...
  proposedContext = findPresentationContextID(
      params->DULparams.requestedPresentationContext, presentationContextID);
  ...
  if (dcmStrictRoleSelection.get()) {
    if (proposedContext->proposedSCRole != proposedContext->acceptedSCRole) {
      if (((proposedContext->proposedSCRole == DUL_SC_ROLE_DEFAULT) &&
           (proposedContext->acceptedSCRole != DUL_SC_ROLE_SCU)) ||
          ((proposedContext->proposedSCRole == DUL_SC_ROLE_SCU) &&
           (proposedContext->acceptedSCRole != DUL_SC_ROLE_DEFAULT)) ||
          ((proposedContext->proposedSCRole != DUL_SC_ROLE_SCUSCP) &&
           (proposedContext->acceptedSCRole != DUL_SC_ROLE_SCUSCP))) {
        proposedContext->result = ASC_P_NOREASON;
        DCMNET_ERROR(
            "ASSOC: SCP/SCU role selection failed, proposed ("
            << ascRole2String(dulRole2ascRole(proposedContext->proposedSCRole))
            << ") and accepted role (" << ascRole2String(acceptedRole)
            << ") are incompatible");
  ...
  acceptedContext = findPresentationContextID(
      params->DULparams.acceptedPresentationContext, presentationContextID);

  if (acceptedContext != NULL) {
    /* it is already in the list, mark it as accepted */
    acceptedContext->result = ASC_P_ACCEPTANCE;
    strcpy(acceptedContext->abstractSyntax, proposedContext->abstractSyntax);
    ...
    cond = DUL_MakePresentationCtx(
        &acceptedContext, proposedContext->proposedSCRole,
        ascRole2dulRole(acceptedRole), presentationContextID, ASC_P_ACCEPTANCE,
        proposedContext->abstractSyntax, (char*)transferSyntax, NULL);
    if (cond.bad())
      return cond;
...
OFCondition ASC_refusePresentationContext(
    T_ASC_Parameters* params, T_ASC_PresentationContextID presentationContextID,
 ...
 */
{
  ...
  if (acceptedContext != NULL) {
    ...
    cond = DUL_MakePresentationCtx(
        &acceptedContext, DUL_SC_ROLE_DEFAULT, DUL_SC_ROLE_DEFAULT,
        presentationContextID, resultReason, proposedContext->abstractSyntax,
...
OFCondition ASC_findAcceptedPresentationContext(
    T_ASC_Parameters* params, T_ASC_PresentationContextID presentationContextID,
    T_ASC_PresentationContext* presentationContext)
/*
  ...
  */
  ...
  while (transfer != NULL) {
    if (count >= DICOM_MAXTRANSFERSYNTAXES) {
      return makeDcmnetCondition(
          ASCC_CODINGERROR, OF_error,
          "ASC Coding error in ASC_findAcceptedPresentationContext: too many "
          "transfer syntaxes");
    }
    OFStandard::strlcpy(presentationContext->proposedTransferSyntaxes[count],
                        transfer->transferSyntax, sizeof(DIC_UI));
    count++;
    transfer = (DUL_TRANSFERSYNTAX*)LST_Next(l);
  }

  OFStandard::strlcpy(presentationContext->abstractSyntax, pc->abstractSyntax,
                      sizeof(DIC_UI));
  presentationContext->presentationContextID = pc->presentationContextID;
  presentationContext->resultReason = (T_ASC_P_ResultReason)pc->result;
  presentationContext->proposedRole = dulRole2ascRole(pc->proposedSCRole);
  ...
  OFStandard::strlcpy(presentationContext->acceptedTransferSyntax,
                      pc->acceptedTransferSyntax, sizeof(DIC_UI));

  return EC_Normal;
}

T_ASC_PresentationContextID ASC_findAcceptedPresentationContextID(
    T_ASC_Association* assoc, const char* abstractSyntax) {
...
T_ASC_PresentationContextID ASC_findAcceptedPresentationContextID(
  ...
  while (pc && !found) {
    found = (strcmp(pc->abstractSyntax, abstractSyntax) == 0) &&
            (pc->result == ASC_P_ACCEPTANCE) &&
            (strcmp(pc->acceptedTransferSyntax, transferSyntax) == 0);
    if (!found)
      pc = (DUL_PRESENTATIONCONTEXT*)LST_Next(l);
  }
  if (found)
  ...
  while (pc && !found) {
    found = (strcmp(pc->abstractSyntax, abstractSyntax) == 0) &&
            (pc->result == ASC_P_ACCEPTANCE) &&
            ((strcmp(pc->acceptedTransferSyntax,
                     UID_LittleEndianExplicitTransferSyntax) == 0) ||
             (strcmp(pc->acceptedTransferSyntax,
                     UID_BigEndianExplicitTransferSyntax) == 0));
    if (!found)
      pc = (DUL_PRESENTATIONCONTEXT*)LST_Next(l);
  }
  if (found)
  ...
  while (pc && !found) {
    found = (strcmp(pc->abstractSyntax, abstractSyntax) == 0) &&
            (pc->result == ASC_P_ACCEPTANCE) &&
            (strcmp(pc->acceptedTransferSyntax,
                    UID_LittleEndianImplicitTransferSyntax) == 0);
    if (!found)
      pc = (DUL_PRESENTATIONCONTEXT*)LST_Next(l);
  }
  if (found)
...
OFCondition ASC_acceptContextsWithTransferSyntax(T_ASC_Parameters* params,
                                                 const char* transferSyntax,
 ...
 */
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (accepted) {
      cond = ASC_acceptPresentationContext(params, pc.presentationContextID,
      ...
      if (cond == ASC_SCPSCUROLESELECTIONFAILED) {
        cond = ASC_refusePresentationContext(params, pc.presentationContextID,
                                             ASC_P_NOREASON);
      }
      if (cond.bad())
        return cond;
...
OFCondition ASC_acceptContextsWithPreferredTransferSyntaxes(
    T_ASC_Parameters* params, const char* abstractSyntaxes[],
    int abstractSyntaxCount, const char* transferSyntaxes[],
    int transferSyntaxCount, T_ASC_SC_ROLE acceptedRole) {
  int i;
  OFCondition cond = EC_Normal;
  /*
  ...
  */
  for (i = transferSyntaxCount - 1; i >= 0; i--) {
    cond = ASC_acceptContextsWithTransferSyntax(params, transferSyntaxes[i],
                                                abstractSyntaxCount,
                                                abstractSyntaxes, acceptedRole);
    if (cond.bad())
      return cond;
  }
  return cond;
...
void ASC_getUserIdentRQ(T_ASC_Parameters* params,
                        UserIdentityNegotiationSubItemRQ** usrIdentRQ) {
  *usrIdentRQ = params->DULparams.reqUserIdentNeg;
}

void ASC_getUserIdentAC(T_ASC_Parameters* params,
                        UserIdentityNegotiationSubItemAC** usrIdentAC) {
  *usrIdentAC = params->DULparams.ackUserIdentNeg;
}

OFCondition ASC_setIdentRQUserPassword(T_ASC_Parameters* params,
  ...
  if ((userName.length() > 65535) || (password.length() > 65535) ||
      (userName.length() + password.length() > 65535))
...
OFCondition ASC_setIdentRQUserOnly(T_ASC_Parameters* params,
                                   const OFString& userName,
                                   const OFBool& requestRsp) {
  if (params == NULL)
    return ASC_NULLKEY;
...
OFCondition ASC_setIdentRQKerberos(T_ASC_Parameters* params,
                                   const char* kerbTicket, const Uint16& length,
                                   const OFBool& requestRsp) {
  if (params == NULL)
    return ASC_NULLKEY;
  UserIdentityNegotiationSubItemRQ* rq = params->DULparams.reqUserIdentNeg;
...
OFCondition ASC_setIdentRQSaml(T_ASC_Parameters* params, const char* saml,
                               const Uint16& length, const OFBool& requestRsp) {
  if (params == NULL)
    return ASC_NULLKEY;
  UserIdentityNegotiationSubItemRQ* rq = params->DULparams.reqUserIdentNeg;
  if (rq == NULL)
...
void ASC_getCopyOfIdentResponse(T_ASC_Parameters* params, char*& buffer,
...
OFCondition ASC_setIdentAC(T_ASC_Parameters* params, const char* response,
                           const Uint16& length) {
  if (params == NULL)
    return ASC_NULLKEY;
 ...
 */
{
  ...
  outstream << "    Proposed SCP/SCU Role: " << ascRole2String(p->proposedRole)
            << OFendl;
  ...
  if (p->resultReason == ASC_P_ACCEPTANCE) {
    ...
    if (ts) {
      ...
      outstream << "    Accepted Transfer Syntax: " << p->acceptedTransferSyntax
                << OFendl;
    }
  }
...
OFString& ASC_dumpParameters(OFString& str, T_ASC_Parameters* params,
                             ASC_associateType dir)
/*
 ...
 */
{
  ...
  outstream << "====================== BEGIN A-ASSOCIATE-" << str_dir
            << " =====================" << OFendl
  ...
  outstream << "Our Max PDU Receive Size:    " << params->ourMaxPDUReceiveSize
            << OFendl
            << "Their Max PDU Receive Size:  " << params->theirMaxPDUReceiveSize
            << OFendl;
...
OFString& ASC_dumpConnectionParameters(OFString& str,
...
extern int DUL_networkSocket(DUL_NETWORKKEY* callerNet);

typedef DcmTransportConnection* P_DcmTransportConnection;

OFBool ASC_selectReadableAssociation(T_ASC_Association* assocs[],
                                     int assocCount, int timeout) {
  if (assocCount <= 0)
    return OFFalse;

  P_DcmTransportConnection* connections =
      new P_DcmTransportConnection[assocCount];
  if (connections == NULL)
    return OFFalse;
  ...
  for (i = 0; i < assocCount; i++) {
    if (assocs[i])
      connections[i] = DUL_getTransportConnection(assocs[i]->DULassociation);
    else
      connections[i] = NULL;
  }
  OFBool result = DcmTransportConnection::selectReadableAssociation(
      connections, assocCount, timeout);
  if (result) {
    for (i = 0; i < assocCount; i++) {
...
OFBool ASC_dataWaiting(T_ASC_Association* association, int timeout) {
  return DUL_dataWaiting(association->DULassociation, timeout);
}

OFBool ASC_associationWaiting(T_ASC_Network* network, int timeout) {
  int s;
  struct timeval t;
  fd_set fdset;
  int nfound;
...
OFCondition ASC_destroyAssociation(T_ASC_Association** association) {
  OFCondition cond = EC_Normal;
...
OFCondition ASC_receiveAssociation(T_ASC_Network* network,
                                   ...
                                   long maxReceivePDUSize, void** associatePDU,
  ...
  if (associatePDU && associatePDUlength)
    retrieveRawPDU = 1;

  OFCondition cond =
      ASC_createAssociationParameters(&params, maxReceivePDUSize);
  if (cond.bad())
    return cond;
  ...
  *assoc = (T_ASC_Association*)malloc(sizeof(**assoc));
  ...
  if (retrieveRawPDU && DULassociation) {
    DUL_returnAssociatePDUStorage((*assoc)->DULassociation, *associatePDU,
                                  *associatePDUlength);
  }
...
static OFCondition updateRequestedPCFromAcceptedPC(
    DUL_PRESENTATIONCONTEXT* apc, DUL_PRESENTATIONCONTEXT* rpc) {
  if (apc == NULL || rpc == NULL)
    return EC_Normal;

  if (rpc->presentationContextID != apc->presentationContextID) {
    return makeDcmnetCondition(
        ASCC_CODINGERROR, OF_error,
        "ASC Coding error in updateRequestedPCFromAcceptedPC: "
        "presentationContextIDs differ");
  }
...
static OFCondition updateRequestedPCListFromAcceptedPCList(
  ...
  if (*acceptedList != NULL) {
    ...
    while (acceptedPc) {
      requestedPc =
          findPresentationContextID(dulParams->requestedPresentationContext,
...
OFCondition ASC_requestAssociation(T_ASC_Network* network,
                                   T_ASC_Parameters* params,
                                   T_ASC_Association** assoc,
                                   void** associatePDU,
                                   unsigned long* associatePDUlength) {
  ...
  if (ASC_countPresentationContexts(params) == 0) {
    return makeDcmnetCondition(ASCC_CODINGERROR, OF_error,
                               "ASC Coding error in ASC_requestAssociation: "
                               "missing presentation contexts");
  }
  ...
  *assoc = (T_ASC_Association*)malloc(sizeof(**assoc));
  ...
  cond = DUL_RequestAssociation(&network->network, &(*assoc)->params->DULparams,
                                &(*assoc)->DULassociation, retrieveRawPDU);
  ...
  if (cond.good()) {
    ...
    if (!((params->theirMaxPDUReceiveSize & DUL_MAXPDUCOMPAT) ^
          DUL_DULCOMPAT)) {
      /* activate compatibility with DCMTK releases prior to 3.0 */
      DUL_activateCompatibilityMode((*assoc)->DULassociation,
                                    dcmEnableBackwardCompatibility.get() |
                                        DUL_DULCOMPAT | DUL_DIMSECOMPAT);
      if (params->modeCallback)
        params->modeCallback->callback(params->theirMaxPDUReceiveSize);
    }
    ...
    /* make sure max pdv length is even */
    if ((sendLen % 2) != 0) {
      DCMNET_WARN("ASSOC: PDV send length " << sendLen << " is odd (using "
                                            << (sendLen - 1) << ")");
      sendLen--;
    }
    ...
    if (sendLen < 12) {
       ...
       */
      DCMNET_WARN(
          "ASSOC: PDV send length too small, using DUL to split larger PDVs.");
      sendLen = ASC_MINIMUMPDUSIZE - 12;
    }
...
OFCondition ASC_acknowledgeAssociation(T_ASC_Association* assoc,
                                       void** associatePDU,
                                       unsigned long* associatePDUlength) {
  long sendLen;
  ...
  if (!((assoc->params->theirMaxPDUReceiveSize & DUL_MAXPDUCOMPAT) ^
        DUL_DULCOMPAT)) {
    assoc->params->DULparams.maxPDU =
        dcmEnableBackwardCompatibility.get() | DUL_DULCOMPAT | DUL_DIMSECOMPAT;
  }
  ...
  OFCondition cond = DUL_AcknowledgeAssociationRQ(
      &assoc->DULassociation, &assoc->params->DULparams, retrieveRawPDU);
...
OFCondition ASC_rejectAssociation(T_ASC_Association* association,
                                  T_ASC_RejectParameters* rejectParameters,
                                  void** associatePDU,
                                  unsigned long* associatePDUlength) {
  ...
  OFCondition cond = DUL_RejectAssociationRQ(&association->DULassociation,
                                             &l_abort, retrieveRawPDU);

  if (retrieveRawPDU && (association->DULassociation)) {
    DUL_returnAssociatePDUStorage(association->DULassociation, *associatePDU,
                                  *associatePDUlength);
  }
...
OFCondition ASC_releaseAssociation(T_ASC_Association* association) {
  if (association == NULL)
    return ASC_NULLKEY;
  if (association->DULassociation == NULL)
    return ASC_NULLKEY;
...
OFCondition ASC_abortAssociation(T_ASC_Association* association) {
  if (association == NULL)
    return ASC_NULLKEY;
  if (association->DULassociation == NULL)
    return ASC_NULLKEY;
...
OFCondition ASC_setTransportLayerType(T_ASC_Parameters* params,
                                      OFBool useSecureLayer) {
  if (params == NULL)
    return ASC_NULLKEY;
...
OFCondition ASC_setTransportLayer(T_ASC_Network* network,
...
unsigned long ASC_getPeerCertificate(T_ASC_Association* assoc, void* buf,
...
void ASC_printRejectParameters(STD_NAMESPACE ostream& out,
                               T_ASC_RejectParameters* rej) {
  OFString str;
  ASC_printRejectParameters(str, rej);
  out << str << OFendl;
...
void ASC_dumpParameters(T_ASC_Parameters* params,
...
void ASC_dumpPresentationContext(T_ASC_PresentationContext* presentationContext,
                                 STD_NAMESPACE ostream& outstream) {
  outstream << ASC_dumpPresentationContext(presentationContext) << OFendl;
}

void ASC_dumpConnectionParameters(T_ASC_Association* association,
                                  STD_NAMESPACE ostream& outstream) {
  OFString str;
...
**   if a client sent an A-ASSOCIATE request with a longer
...
**   accepted presentation context that matches both abstract and transfer
*syntax.
**
