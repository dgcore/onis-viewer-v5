...
typedef struct user_info {
  ...
  PRV_ASYNCOPERATIONS
      asyncOperations;  // 53H: async operations (not yet implemented!)
  ...
  SOPClassExtendedNegotiationSubItemList*
      extNegList;  // 56H: extended negotiation
                   // 57H: SOP CLASS COMMON EXTENDED NEGOTIATION (not
                   // implemented)
  UserIdentityNegotiationSubItem*
      usrIdent;  // 58H: user identity negotiation RQ or AC
} DUL_USERINFO;
...
#define EXTRACT_LONG_BIG(A, B)                                               \
  {                                                                          \
    (B) = (unsigned long)(A)[3] | (((unsigned long)(A)[2]) << 8) |           \
          (((unsigned long)(A)[1]) << 16) | (((unsigned long)(A)[0]) << 24); \
  }

#define EXTRACT_SHORT_BIG(A, B)                                     \
  {                                                                 \
    (B) = (unsigned short)(A)[1] | (((unsigned short)(A)[0]) << 8); \
  }
