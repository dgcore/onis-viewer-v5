...
UserIdentityNegotiationSubItem::UserIdentityNegotiationSubItem()
    : m_itemType(DUL_TYPENEGOTIATIONOFUSERIDENTITY), m_reserved(0) {}
...
void UserIdentityNegotiationSubItemRQ::setIdentityType(
...
unsigned short UserIdentityNegotiationSubItemRQ::getSecField(
    char*& resultBuf, Uint16& resultLen) const {
...
void UserIdentityNegotiationSubItemRQ::setReqPosResponse(
    const OFBool& reqPosRsp) {
  m_posRspRequested = reqPosRsp ? 1 : 0;
}

// Stream the package into a byte stream for network transmission
OFCondition UserIdentityNegotiationSubItemRQ::stream(
    unsigned char* targetBuffer, unsigned long& lengthWritten) const {
  ...
  *targetBuffer++ = getReserved();
  ...
  COPY_SHORT_BIG(OFstatic_cast(unsigned short, lengthWritten - 4),
                 targetBuffer);
  targetBuffer += 2;
...
// Calculates length values and returns total length of the user item if
// streamed
OFCondition UserIdentityNegotiationSubItemRQ::streamedLength(
    unsigned long& length) const {
  // content length = userIdentityType + positive Response Requested + primary
  // field length field + primary field length + 2nd field length (2) (+ 2nd
  // field)
  ...
  // sum up length of item (item type + reserved + item length field + content
  // length) to get total length
  length += 1 + 1 + 2;

  return EC_Normal;
}

// Parse User Identity request from read buffer. Buffer must start with item
// type field.
OFCondition UserIdentityNegotiationSubItemRQ::parseFromBuffer(
    unsigned char* readBuffer, unsigned long& bytesRead,
  ...
  if (availData < 10) {
    char buffer[256];
    sprintf(buffer, "DUL user identity rq length %ld. Need at least 10 bytes",
            availData);
  ...
  if (availData - 4 < itemLength) {
    ...
    sprintf(
        buffer,
        "DUL illegal user identify rq length %ld. Info claims to be %hd bytes.",
        availData, itemLength);
    return makeDcmnetCondition(DULC_ILLEGALPDULENGTH, OF_error, buffer);
  }

  // Do some length checking
  if (itemLength < 6)  // at least 6 bytes are mandatory: identity type(1) pos.
                       // response(1) primLength(2), secfieldlength(2)
  ...
  if ((identType < 1) || (identType > 4))
    ...
    m_userIdentityType =
        OFstatic_cast(T_ASC_UserIdentityNegotiationMode, identType);
  readBuffer++;
  ...
  if (itemLength - 4 - 2 < m_primFieldLength) {
    ...
    sprintf(
        buffer,
        "DUL illegal user identify rq length %ld. Info claims to be %hd bytes. "
        "Primary field has %hd bytes.",
        availData, itemLength, m_primFieldLength);
    return makeDcmnetCondition(DULC_ILLEGALPDULENGTH, OF_error, buffer);
  }
  ...
  // 6 + m_primField bytes read so far, trying to read m_secFieldLength more
  // bytes
  ...
  // only user identity type 2 (Username + password authentication) requires
  // second value field
  if ((m_userIdentityType == ASC_USER_IDENTITY_USER_PASSWORD) &&
      (m_secFieldLength > 0)) {
...
void UserIdentityNegotiationSubItemRQ::dump(
    STD_NAMESPACE ostream& outstream) const {
  switch (m_userIdentityType) {
    ...
    case ASC_USER_IDENTITY_USER:
      ...
      if (m_primFieldLength > 0) {
        ...
        strncpy(buf, OFstatic_cast(const char*, m_primField),
                m_primFieldLength);
    ...
    case ASC_USER_IDENTITY_KERBEROS:
      outstream << "  Authentication mode 3: Kerberos" << OFendl
                << "  Kerberos Service Ticket (not dumped) length: "
                << m_primFieldLength << OFendl;
      break;
    case ASC_USER_IDENTITY_SAML:
      outstream << "  Authentication mode 4: SAML" << OFendl
                << "  SAML Assertion (not dumped) length: " << m_primFieldLength
                << OFendl;
      break;
    default:
      outstream << "  Authentication mode: Unknown " << OFendl
                << "  First value (not dumped), length: " << m_primFieldLength
                << OFendl
                << "  Second Value (not dumped), length: " << m_secFieldLength
                << OFendl;
  }
  outstream << "  Positive Response requested: "
            << ((m_posRspRequested != 0) ? "Yes" : "No") << OFendl
            << OFStringStream_ends;
}

// Assignment operator
UserIdentityNegotiationSubItemRQ& UserIdentityNegotiationSubItemRQ::operator=(
    const UserIdentityNegotiationSubItemRQ& rhs) {
  this->clear();
  m_userIdentityType = rhs.m_userIdentityType;
  m_posRspRequested = rhs.m_posRspRequested;
  if (rhs.m_primFieldLength > 0) {
    if (rhs.getPrimField(m_primField, m_primFieldLength) !=
        rhs.m_primFieldLength) {
      m_primField = NULL;
      m_primFieldLength = 0;
    }
  } else
    m_primFieldLength = 0;
  if ((rhs.m_secFieldLength > 0) &&
      (rhs.m_userIdentityType == ASC_USER_IDENTITY_USER_PASSWORD)) {
...
Uint16 UserIdentityNegotiationSubItemAC::getServerResponse(
    char*& targetBuffer, Uint16& resultLen) const {
...
OFCondition UserIdentityNegotiationSubItemAC::streamedLength(
    unsigned long& length) const {
  // content length: server response length field (2) + length of server
  // response field
  length = 2 + m_rspLength;
  // total length: (item type + reserved + item length field + item content
  // length)
  length = 1 + 1 + 2 + length;
  return EC_Normal;
}
...
OFCondition UserIdentityNegotiationSubItemAC::stream(
    unsigned char* targetBuffer, unsigned long& lengthWritten) const {
  if (OFstatic_cast(unsigned long, m_rspLength) + 2 >
      (unsigned long)65535 /* for response length field */) {
    char errbuf[500];
    sprintf(errbuf,
            "Length of User Identity response (%lu bytes) exceeds upper limit "
            "of 65535 bytes",
            (unsigned long)m_rspLength + 2);
  ...
  *targetBuffer++ = getItemType();
  *targetBuffer++ = getReserved();
  // value for item length field is = length of response value + length of
  // length field (2)
...
// Parase User Identity sub item from buffer. Buffer has to start with item type
// field.
OFCondition UserIdentityNegotiationSubItemAC::parseFromBuffer(
  ...
  if (availData < 6) {
    char buffer[256];
    sprintf(buffer,
            "DUL illegal user identify ac length %ld. Need at least 6 bytes for"
            " user identify ac",
            availData);
    return makeDcmnetCondition(DULC_ILLEGALPDULENGTH, OF_error, buffer);
  }
  ...
  // end of any buffer
  if (availData - 4 < itemLength || itemLength < 2 ||
      itemLength - 2 < m_rspLength) {
    char buffer[256];
    sprintf(
        buffer,
        "DUL illegal user identify ac length %ld. Info claims to be %hd bytes. "
        "Response claims to be %hd bytes.",
        availData, itemLength, m_rspLength);
    return makeDcmnetCondition(DULC_ILLEGALPDULENGTH, OF_error, buffer);
  }
...
void UserIdentityNegotiationSubItemAC::dump(
    STD_NAMESPACE ostream& outstream) const {
  outstream << "  Server Response (not dumped) length: " << m_rspLength
            << OFendl;
}

// Assignment operator
UserIdentityNegotiationSubItemAC& UserIdentityNegotiationSubItemAC::operator=(
    const UserIdentityNegotiationSubItemAC& rhs) {
  this->clear();
