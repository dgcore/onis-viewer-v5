...
OFCondition DIMSE_sendGetResponse(T_ASC_Association* assoc,
  ...
  opts =
      (O_GET_NUMBEROFREMAININGSUBOPERATIONS |
       ...
       O_GET_NUMBEROFFAILEDSUBOPERATIONS | O_GET_NUMBEROFWARNINGSUBOPERATIONS);
  ...
  return DIMSE_sendMessageUsingMemoryData(assoc, presID, &rsp, statusDetail,
                                          rspIds, NULL, NULL);
}
    ...
    if (presIdData != presIdCmd) {
      ...
      while (cond == EC_Normal && rsp.DimseStatus == STATUS_Pending && normal) {
        ...
        if (normal) {
          ...
          cond = DIMSE_sendGetResponse(assoc, presIdCmd, request, &rsp, rspIds,
                                       statusDetail);
