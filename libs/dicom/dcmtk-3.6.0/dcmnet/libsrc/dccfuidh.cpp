...
DcmUIDHandler::DcmUIDHandler() : uid_() {}

DcmUIDHandler::DcmUIDHandler(const OFString& arg) : uid_(arg) {
  lookupSymbolicUID();
}

DcmUIDHandler::DcmUIDHandler(const char* arg) : uid_() {
  if (arg)
    uid_ = arg;
  lookupSymbolicUID();
}

DcmUIDHandler::DcmUIDHandler(const DcmUIDHandler& arg) : uid_(arg.uid_) {}
...
void DcmUIDHandler::lookupSymbolicUID() {
  if (uid_.length() > 0) {
    ...
    if ((*c < '0') || (*c > '9')) {
      ...
      if (*c == '=')
        ++c;  // skip leading '=' as used in dump2dcm etc.
      ...
      if (0 == strcmp(SymbolicUID_LocalEndianExplicit, c)) {
        if (gLocalByteOrder == EBO_BigEndian)
          uid_ = UID_BigEndianExplicitTransferSyntax;
        else
          uid_ = UID_LittleEndianExplicitTransferSyntax;
        return;
      }
