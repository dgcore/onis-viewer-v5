...
DcmPresentationContextItem::DcmPresentationContextItem(
    const DcmUIDHandler& abstractSyntax, const OFString& xferSyntaxGroup)
    : uid_(abstractSyntax), xferSyntaxGroup_(xferSyntaxGroup) {}

DcmPresentationContextItem::DcmPresentationContextItem(
    const DcmPresentationContextItem& arg)
    : uid_(arg.uid_), xferSyntaxGroup_(arg.xferSyntaxGroup_) {}

DcmPresentationContextItem::~DcmPresentationContextItem() {}

/* ========================================================= */

DcmPresentationContextMap::DcmPresentationContextMap() : map_() {}

DcmPresentationContextMap::~DcmPresentationContextMap() {
  OFListIterator(DcmKeyValuePair<DcmPresentationContextList*>*) first =
      map_.begin();
  OFListIterator(DcmKeyValuePair<DcmPresentationContextList*>*) last =
      map_.end();
  while (first != last) {
    delete (*first)->value();
    ++first;
...
OFCondition DcmPresentationContextMap::add(const char* key,
  ...
  OFString skey(key);
  DcmPresentationContextList* const* value =
      OFconst_cast(DcmPresentationContextList* const*, map_.lookup(skey));
  if (value == NULL) {
  ...
  (*value)->push_back(
      DcmPresentationContextItem(uid, OFString(transferSyntaxKey)));
  return EC_Normal;
}
...
OFBool DcmPresentationContextMap::isKnownAbstractSyntax(
    const char* key, const DcmUIDHandler& abstractSyntax) const {
  if (!key)
    return OFFalse;
  ...
  if (value) {
    ...
    while (first != last) {
      if ((*first).matches(abstractSyntax))
        return OFTrue;
      ++first;
    }
  }
...
const DcmPresentationContextList*
DcmPresentationContextMap::getPresentationContextList(const char* key) const {
  const DcmPresentationContextList* result = NULL;
  if (key) {
    DcmPresentationContextList* const* value = OFconst_cast(
        DcmPresentationContextList* const*, map_.lookup(OFString(key)));
    if (value)
      result = *value;
  }
  return result;
