...
static void DIMSE_dumpMessage_end(OFString& str, DcmItem* dataset = NULL) {
  ...
  if (dataset) {
    ...
    stream << DcmObject::PrintHelper(*dataset,
...
static void DIMSE_printNStatusString(STD_NAMESPACE ostream& dumpStream,
                                     int status) {
  switch (status) {
    case STATUS_Success:
    ...
    case STATUS_N_PRINT_BFS_BFB_Fail_PositionCollision:
      dumpStream
          << "0xC604: Basic film session/box failure - Position collision";
      break;
    ...
    default:
      dumpStream << "0x" << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
                 << STD_NAMESPACE setw(4) << status << ": Unknown Status Code";
      break;
  }
}

static void DIMSE_printCStoreStatusString(STD_NAMESPACE ostream& dumpStream,
                                          int status) {
  dumpStream << "0x" << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
  ...
  if ((status & 0xff00) == STATUS_STORE_Refused_OutOfResources) {
    ...
    dumpStream << ": Error: Refused - SOP Class not supported";
             ...
             STATUS_STORE_Error_DataSetDoesNotMatchSOPClass) {
...
static void DIMSE_printCFindStatusString(STD_NAMESPACE ostream& dumpStream,
                                         int status) {
  dumpStream << "0x" << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
             << STD_NAMESPACE setw(4) << status;
...
static void DIMSE_printCGetMoveStatusString(STD_NAMESPACE ostream& dumpStream,
                                            int status) {
  dumpStream << "0x" << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
             << STD_NAMESPACE setw(4) << status;
...
static void DIMSE_printCEchoStatusString(STD_NAMESPACE ostream& dumpStream,
                                         int status) {
  dumpStream << "0x" << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
             << STD_NAMESPACE setw(4) << status;
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_C_EchoRQ& msg,
                            enum DIMSE_direction dir, DcmItem* dataset,
                            T_ASC_PresentationContextID presID) {
  OFOStringStream stream;
  DIMSE_dumpMessage_start(str, dir);
  stream << "Message Type                  : C-ECHO RQ" << OFendl;
  if (presID > 0) {
    stream << "Presentation Context ID       : " << OFstatic_cast(int, presID)
           << OFendl;
  }
  stream << "Message ID                    : " << msg.MessageID << OFendl
         << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present");
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_C_EchoRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_ECHO_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  stream << "Message Type                  : C-ECHO RSP" << OFendl;
  if (presID > 0) {
    stream << "Presentation Context ID       : " << OFstatic_cast(int, presID)
           << OFendl;
  }
  stream << "Message ID Being Responded To : " << msg.MessageIDBeingRespondedTo
         << OFendl << "Affected SOP Class UID        : ";
  if (msg.opts & O_ECHO_AFFECTEDSOPCLASSUID)
    stream << (uid ? uid : msg.AffectedSOPClassUID) << OFendl;
  else
    stream << "none" << OFendl;
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_C_StoreRQ& msg,
  ...
  stream << "Message ID                    : " << msg.MessageID << OFendl
         ...
         << (uid ? uid : msg.AffectedSOPClassUID) << OFendl
         << "Affected SOP Instance UID     : " << msg.AffectedSOPInstanceUID
         << OFendl << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present")
         << OFendl << "Priority                      : ";
  switch (msg.Priority) {
  ...
  if (msg.opts & O_STORE_MOVEORIGINATORAETITLE) {
    stream << OFendl << "Move Originator AE Title      : "
           << msg.MoveOriginatorApplicationEntityTitle;
  }
  if (msg.opts & O_STORE_MOVEORIGINATORID) {
    stream << OFendl
           << "Move Originator ID            : " << msg.MoveOriginatorID;
  }
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_C_StoreRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_STORE_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_STORE_AFFECTEDSOPINSTANCEUID)
    stream << msg.AffectedSOPInstanceUID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present")
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_C_GetRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_GET_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_GET_NUMBEROFREMAININGSUBOPERATIONS)
    stream << msg.NumberOfRemainingSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Completed Suboperations       : ";
  if (msg.opts & O_GET_NUMBEROFCOMPLETEDSUBOPERATIONS)
    stream << msg.NumberOfCompletedSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Failed Suboperations          : ";
  if (msg.opts & O_GET_NUMBEROFFAILEDSUBOPERATIONS)
    stream << msg.NumberOfFailedSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Warning Suboperations         : ";
  if (msg.opts & O_GET_NUMBEROFWARNINGSUBOPERATIONS)
    stream << msg.NumberOfWarningSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present")
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_C_MoveRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_MOVE_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_MOVE_AFFECTEDSOPCLASSUID)
    ...
    stream << "none" << OFendl;
  stream << "Remaining Suboperations       : ";
  if (msg.opts & O_MOVE_NUMBEROFREMAININGSUBOPERATIONS)
    stream << msg.NumberOfRemainingSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Completed Suboperations       : ";
  if (msg.opts & O_MOVE_NUMBEROFCOMPLETEDSUBOPERATIONS)
    stream << msg.NumberOfCompletedSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Failed Suboperations          : ";
  if (msg.opts & O_MOVE_NUMBEROFFAILEDSUBOPERATIONS)
    stream << msg.NumberOfFailedSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Warning Suboperations         : ";
  if (msg.opts & O_MOVE_NUMBEROFWARNINGSUBOPERATIONS)
    stream << msg.NumberOfWarningSubOperations << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_C_FindRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_FIND_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_EventReportRQ& msg,
  ...
  stream << "Message ID                    : " << msg.MessageID << OFendl
         ...
         << OFendl << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present")
         << OFendl << "Event Type ID                 : " << msg.EventTypeID;
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_EventReportRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_NEVENTREPORT_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_NEVENTREPORT_AFFECTEDSOPCLASSUID)
    ...
    stream << "none" << OFendl;
  stream << "Affected SOP Instance UID     : ";
  if (msg.opts & O_NEVENTREPORT_AFFECTEDSOPINSTANCEUID)
    stream << msg.AffectedSOPInstanceUID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present")
         << OFendl << "Event Type ID                 : ";
  if (msg.opts & O_NEVENTREPORT_EVENTTYPEID)
    stream << msg.EventTypeID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "DIMSE Status                  : ";
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_GetRQ& msg,
  ...
  stream << "Message ID                    : " << msg.MessageID << OFendl
         ...
         << (uid ? uid : msg.RequestedSOPClassUID) << OFendl
         << "Requested SOP Instance UID    : " << msg.RequestedSOPInstanceUID
         << OFendl << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present")
         << OFendl << "Attribute Identifier List     : ";
  if (msg.ListCount == 0)
    stream << "none";
  else
    for (int cList = 0; cList < msg.ListCount; cList += 2) {
      stream << "(" << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
             << STD_NAMESPACE setw(4) << msg.AttributeIdentifierList[cList]
             << "," << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
             << STD_NAMESPACE setw(4) << msg.AttributeIdentifierList[cList + 1]
             << ") ";
    }
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_GetRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_NGET_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_NGET_AFFECTEDSOPCLASSUID)
    ...
    stream << "none" << OFendl;
  stream << "Affected SOP Instance UID     : ";
  if (msg.opts & O_NGET_AFFECTEDSOPINSTANCEUID)
    stream << msg.AffectedSOPInstanceUID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_SetRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_NSET_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_NSET_AFFECTEDSOPCLASSUID)
    ...
    stream << "none" << OFendl;
  stream << "Affected SOP Instance UID     : ";
  if (msg.opts & O_NSET_AFFECTEDSOPINSTANCEUID)
    stream << msg.AffectedSOPInstanceUID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_ActionRQ& msg,
  ...
  stream << "Message ID                    : " << msg.MessageID << OFendl
         ...
         << (uid ? uid : msg.RequestedSOPClassUID) << OFendl
         << "Requested SOP Instance UID    : " << msg.RequestedSOPInstanceUID
         << OFendl << "Action Type ID                : " << msg.ActionTypeID
         << OFendl << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present");
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_ActionRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_NACTION_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_NACTION_AFFECTEDSOPCLASSUID)
    ...
    stream << "none" << OFendl;
  stream << "Affected SOP Instance UID     : ";
  if (msg.opts & O_NACTION_AFFECTEDSOPINSTANCEUID)
    stream << msg.AffectedSOPInstanceUID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
         << ((msg.DataSetType == DIMSE_DATASET_NULL) ? "none" : "present")
         << OFendl << "Action Type ID                : ";
  if (msg.opts & O_NACTION_ACTIONTYPEID)
    stream << msg.ActionTypeID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "DIMSE Status                  : ";
  DIMSE_printNStatusString(stream, msg.DimseStatus);
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_CreateRQ& msg,
  ...
  if (msg.opts & O_NCREATE_AFFECTEDSOPINSTANCEUID)
    stream << msg.AffectedSOPInstanceUID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_CreateRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_NCREATE_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
...
OFString& DIMSE_dumpMessage(OFString& str, T_DIMSE_N_DeleteRSP& msg,
  ...
  DIMSE_dumpMessage_start(str, dir);
  if (msg.opts & O_NDELETE_AFFECTEDSOPCLASSUID)
    uid = dcmFindNameOfUID(msg.AffectedSOPClassUID);
  ...
  if (msg.opts & O_NDELETE_AFFECTEDSOPCLASSUID)
    ...
    stream << "none" << OFendl;
  stream << "Affected SOP Instance UID     : ";
  if (msg.opts & O_NDELETE_AFFECTEDSOPINSTANCEUID)
    stream << msg.AffectedSOPInstanceUID << OFendl;
  else
    stream << "none" << OFendl;
  stream << "Data Set                      : "
