...
OFCondition DcmAssociationConfiguration::addExtendedNegotiation(
    const char* key, const char* abstractSyntaxUID,
    const unsigned char* rawData, Uint32 length) {
  return extneg_.add(key, abstractSyntaxUID, rawData, length);
}

OFCondition DcmAssociationConfiguration::addProfile(
    const char* key, const char* presentationContextKey,
    const char* roleSelectionKey, const char* extendedNegotiationKey) {
  if ((!key) || (!presentationContextKey))
    return EC_IllegalCall;
  ...
  if (roleSelectionKey) {
    // check that role selection list exists and matches presentation context
    // list
    status = roleselection_.checkConsistency(roleSelectionKey, contexts_,
  ...
  if (extendedNegotiationKey) {
    // check that extended negotiation list exists and matches presentation
    // context list
    status = extneg_.checkConsistency(extendedNegotiationKey, contexts_,
                                      presentationContextKey);
    if (status.bad())
      return status;
  }

  return profiles_.add(key, presentationContextKey, roleSelectionKey,
...
OFBool DcmAssociationConfiguration::isValidSCPProfile(const char* key) const {
  ...
  const DcmPresentationContextList* contextList =
      contexts_.getPresentationContextList(contextKey);
  if (contextList) {
    // now loop through all presentation contexts
    OFListConstIterator(DcmPresentationContextItem) outerfirst =
        contextList->begin();
    ...
    while (outerfirst != last) {
      ...
      while (innerfirst != last) {
        if (outerAbstractSyntax == (*innerfirst).getAbstractSyntax()) {
          // we have found two instances of the same abstract syntax in this
          // list, so this is not a valid SCP profile.
          return OFFalse;
        }
        ++innerfirst;
      }
...
OFCondition DcmAssociationConfiguration::setAssociationParameters(
    const char* profile, T_ASC_Parameters& params) const {
  if (!profile)
    return EC_IllegalCall;
  ...
  while (first != last) {
    ...
    if (transferSyntaxList->size() > 128) {
      // error: key undefined
      OFString s(
          "transfer syntax list too long, cannot handle more than 128 "
          "entries: ");
      s += transferSyntaxKey;
    ...
    result =
        ASC_addPresentationContext(&params, cid, abstractSyntax, xferSyntaxes,
                                   numXferSyntaxes, proposedRole);
    if (result.bad())
      return result;
  ...
  if (extendedNegotiationList) {
    ...
    SOPClassExtendedNegotiationSubItemList* enlist =
        new SOPClassExtendedNegotiationSubItemList();
    ASC_setRequestedExtNegList(&params, enlist);
    SOPClassExtendedNegotiationSubItem* currentItem = NULL;

    // for each entry in our list
    OFListConstIterator(DcmExtendedNegotiationItem) enfirst =
        extendedNegotiationList->begin();
    OFListConstIterator(DcmExtendedNegotiationItem) enlast =
        extendedNegotiationList->end();
    while (enfirst != enlast) {
      ...
      currentItem->serviceClassAppInfoLength =
          OFstatic_cast(unsigned short, (*enfirst).getLength());
      currentItem->serviceClassAppInfo =
          new unsigned char[currentItem->serviceClassAppInfoLength];
      memcpy(currentItem->serviceClassAppInfo, (*enfirst).getRaw(),
             currentItem->serviceClassAppInfoLength);
...
OFCondition DcmAssociationConfiguration::evaluateAssociationParameters(
    const char* profile, T_ASC_Association& assoc) const {
  if (!profile)
    return EC_IllegalCall;
  ...
  for (int i = 0; i < numContexts; ++i) {
    ...
    if (!found) {
      ...
      result =
          ASC_refusePresentationContext(assoc.params, pc.presentationContextID,
      ...
      transferSyntaxKey = (*first).getTransferSyntaxKey();
      transferSyntaxList =
          xferSyntaxes_.getTransferSyntaxList(transferSyntaxKey);
      if (!transferSyntaxList) {
        // error: key undefined
        OFString s("transfer syntax key undefined: ");
        s += transferSyntaxKey;
      ...
      // try to accept the presentation context with the most preferred transfer
      // syntax
      tsfirst = transferSyntaxList->begin();
      tslast = transferSyntaxList->end();
      found = OFFalse;

      // loop through our list of preferred transfer syntaxes for this abstract
      // syntax, starting with the most preferred one
          ...
          if ((*tsfirst).str() == pc.proposedTransferSyntaxes[j]) {
            ...
            if (result == ASC_SCPSCUROLESELECTIONFAILED) {
              result = ASC_refusePresentationContext(
                  assoc.params, pc.presentationContextID, ASC_P_NOREASON);
            }
            if (result.bad())
              return result;
          }
  ...
  if (extendedNegotiationList) {
    ...
    OFListConstIterator(DcmExtendedNegotiationItem) enlast =
        extendedNegotiationList->end();
    while (enfirst != enlast) {
      // check if we have an accepted presentation context for this abstract
      // syntax
      if (0 != ASC_findAcceptedPresentationContextID(
                   &assoc, (*enfirst).getAbstractSyntaxC())) {
        // create and populate SOPClassExtendedNegotiationSubItem object
        currentItem = new SOPClassExtendedNegotiationSubItem();
        currentItem->sopClassUID = (*enfirst).getAbstractSyntaxC();
        currentItem->serviceClassAppInfoLength =
