...
static void progressCallback(void* callbackData, T_DIMSE_C_FindRQ* request,
                             int responseCount, T_DIMSE_C_FindRSP* rsp,
                             DcmDataset* responseIdentifiers) {
  DcmFindSCUCallback* callback =
      OFreinterpret_cast(DcmFindSCUCallback*, callbackData);
  if (callback)
    callback->callback(request, responseCount, rsp, responseIdentifiers);
}

/* ---------------- class DcmFindSCUCallback ---------------- */

DcmFindSCUCallback::DcmFindSCUCallback() : assoc_(NULL), presId_(0) {}

void DcmFindSCUCallback::setAssociation(T_ASC_Association* assoc) {
  assoc_ = assoc;
}

void DcmFindSCUCallback::setPresentationContextID(
...
void DcmFindSCUDefaultCallback::callback(T_DIMSE_C_FindRQ* request,
                                         int responseCount,
  ...
  DCMNET_WARN("Find Response: " << responseCount << " ("
  ...
  /* in case extractResponsesToFile is set the responses shall be extracted to a
   * certain file */
  ...
  if (cancelAfterNResponses_ == responseCount) {
    ...
    if (cond.bad()) {
      OFString temp_str;
      DCMNET_ERROR(
          "Cancel Request Failed: " << DimseCondition::dump(temp_str, cond));
    }
  }
}
...
DcmFindSCU::DcmFindSCU() : net_(NULL) {}
...
OFCondition DcmFindSCU::performQuery(
    ...
    OFBool extractResponsesToFile, int cancelAfterNResponses,
    OFList<OFString>* overrideKeys, DcmFindSCUCallback* callback,
   ...
   * T_ASC_Parameters*. */
  OFCondition cond =
      ASC_createAssociationParameters(&params, maxReceivePDULength);
  if (cond.bad())
    return cond;
  ...
  cond =
      addPresentationContext(params, abstractSyntax, preferredTransferSyntax);
  ...
  DCMNET_DEBUG("Request Parameters:"
               << OFendl << ASC_dumpParameters(temp_str, params, ASC_ASSOC_RQ));
  ...
  if (cond.bad()) {
    if (cond == DUL_ASSOCIATIONREJECTED) {
      ...
      DCMNET_ERROR("Association Request Failed: " << DimseCondition::dump(
                       temp_str, cond));
      return cond;
    }
  }
  ...
  DCMNET_DEBUG("Association Parameters Negotiated:"
               << OFendl << ASC_dumpParameters(temp_str, params, ASC_ASSOC_AC));
  ...
  /* do the real work, i.e. for all files which were specified in the command
   * line, send a */
  /* C-FIND-RQ to the other DICOM application and receive corresponding response
   * messages. */
  cond = EC_Normal;
  if ((fileNameList == NULL) || fileNameList->empty()) {
    /* no files provided on command line */
    cond = findSCU(assoc, NULL, repeatCount, abstractSyntax, blockMode,
                   dimse_timeout, extractResponsesToFile, cancelAfterNResponses,
                   overrideKeys, callback);
  } else {
    OFListIterator(OFString) iter = fileNameList->begin();
    OFListIterator(OFString) enditer = fileNameList->end();
    while ((iter != enditer) && cond.good()) {
      cond = findSCU(assoc, (*iter).c_str(), repeatCount, abstractSyntax,
                     blockMode, dimse_timeout, extractResponsesToFile,
                     cancelAfterNResponses, overrideKeys, callback);
      ++iter;
    }
  }
...
OFCondition DcmFindSCU::addPresentationContext(
    T_ASC_Parameters* params, const char* abstractSyntax,
  ...
  */
  ...
  return ASC_addPresentationContext(params, 1, abstractSyntax, transferSyntaxes,
                                    numTransferSyntaxes);
}
...
OFCondition DcmFindSCU::findSCU(
    T_ASC_Association* assoc, const char* fname, int repeatCount,
    const char* abstractSyntax, T_DIMSE_BlockingMode blockMode,
    int dimse_timeout, OFBool extractResponsesToFile, int cancelAfterNResponses,
    OFList<OFString>* overrideKeys, DcmFindSCUCallback* callback) const
/*
 ...
 * application). fname - [in] Name of the file which shall be processed.
 */
{
  T_ASC_PresentationContextID presId;
  ...
  OFCondition cond;
  if (fname != NULL) {
    /* read information from file (this information specifies a search mask).
     * After the */
    /* call to DcmFileFormat::read(...) the information which is encapsulated in
     * the file */
    /* will be available through the DcmFileFormat object. In detail, it will be
     * available */
    /* through calls to DcmFileFormat::getMetaInfo() (for meta header
     * information) and */
  ...
  DcmFindSCUDefaultCallback defaultCallback(extractResponsesToFile,
  ...
  while (cond.good() && n--) {
    ...
    DCMNET_INFO("Find SCU Request Identifiers:"
                << OFendl << DcmObject::PrintHelper(*dset));

    /* finally conduct transmission of data */
    cond = DIMSE_findUser(assoc, presId, &req, dset, progressCallback, callback,
                          blockMode, dimse_timeout, &rsp, &statusDetail);
    ...
    if (cond.good()) {
      if (DCM_dcmnetGetLogger().isEnabledFor(OFLogger::INFO_LOG_LEVEL)) {
        ...
        if (rsp.DimseStatus != STATUS_Success) {
          DCMNET_ERROR(
              "Find Response: " << DU_cfindStatusString(rsp.DimseStatus));
        }
      }
    } else {
      if (fname) {
    ...
    if (statusDetail != NULL) {
      DCMNET_WARN("Status Detail:" << OFendl
 ...
 * - Fixed bug in addOverrideKey() that caused  problems when parsing a value in
 * a tag-value combination if the value contained whitespace characters.
 *
