...
static void getPresentationContextInfo(const T_ASC_Association* assoc,
  ...
  DUL_PRESENTATIONCONTEXT* pc = findPresentationContextID(
      assoc->params->DULparams.acceptedPresentationContext, presID);
  if (pc != NULL) {
...
DcmSCP::DcmSCP()
  ...
  WSAStartup(winSockVersionNeeded,
             &winSockData);  // TODO: check with multiple SCP instances whether
                             // this is harmful
#endif
}
...
DcmSCP::~DcmSCP() {
  ...
  // If there is an open association, drop it and free memory (just to be
  // sure...)
  if (m_assoc) {
...
#ifdef HAVE_WINSOCK_H
  WSACleanup();  // TODO: check with multiple SCP instances whether this is
                 // harmful
#endif
}
...
OFCondition DcmSCP::enableMultiProcessMode(int argc, char* argv[]) {
  if (m_assoc != NULL)
...
OFCondition DcmSCP::listen() {
  ...
  if (m_port < 1024 && geteuid() != 0) {
    DCMNET_ERROR(
        "No privileges to open this network port (choose port below 1024?)");
    return EC_IllegalCall;  // TODO: need to find better error code
  }
#endif
  ...
  cond = ASC_initializeNetwork(NET_ACCEPTOR, OFstatic_cast(int, m_port),
                               m_acseTimeout, &m_net);
  if (cond.bad())
    return cond;
  ...
  // If we get to this point, the entire initialization process has been
  // completed successfully. Now, we want to start handling all incoming
  // requests. Since this activity is supposed to represent a server process, we
  // do not want to terminate this activity (unless indicated by the
  // stopAfterCurrentAssociation() method). Hence, create an inifinite
  // while-loop.
  while (cond.good() && !stopAfterCurrentAssociation()) {
    ...
    // Clean up any child processes if the execution is not limited to a single
    // process. (On Windows platform, childs are not handled via the process
    // table, so there's no need to clean up children)
#ifdef HAVE_FORK
    if (!m_singleProcess)
      cleanChildren();
#elif _WIN32
    // if running in multi-process mode, always terminate child after one
    // association for unix, this is done in WaitForAssociation() with exit()
...
void DcmSCP::refuseAssociation(DcmRefuseReasonType reason) {
  if (m_assoc == NULL) {
    DCMNET_WARN(
        "DcmSCP::refuseAssociation() called but actually no association "
        "running, ignoring");
    return;
  }
  ...
  switch (reason) {
    ...
    case DCMSCP_NO_IMPLEMENTATION_CLASS_UID:
      DCMNET_INFO(
          "Refusing Association (no implementation class UID provided)");
      break;
...
OFCondition DcmSCP::waitForAssociation(T_ASC_Network* network) {
  ...
  OFCondition cond = ASC_receiveAssociation(
      network, &m_assoc, m_maxReceivePDULength, NULL, NULL, OFFalse,
      DUL_NOBLOCK, OFstatic_cast(int, timeout));
  ...
  // if error occurs and we're not in single process mode, close association and
  // return
  ...
  // Now we have to figure out if we might have to refuse the association
  // request. This is the case if at least one of five conditions is met:

  // Condition 1: if option "--refuse" is set we want to refuse the association
  // request.
  ...
  // Condition 2: determine the application context name. If an error occurred
  ...
  if (m_respondWithCalledAETitle)
    ASC_setAPTitles(m_assoc->params, NULL, NULL,
                    m_assoc->params->DULparams.calledAPTitle);
  else
  ...
  // Dump some debug information
  OFString tempStr;
  DCMNET_INFO("Association Acknowledged (Max Send PDV: "
              << OFstatic_cast(Uint32, m_assoc->sendPDVLength) << ")");
  if (m_verbosePCMode)
  ...
  // Depending on if this execution shall be limited to one process or not,
  // spawn a sub- process to handle the association or don't. (Note: For Windows
  // dcmnet is handling the creation for a new subprocess, so we can call
  // HandleAssociation directly, too.)
  if (m_singleProcess || m_forkedChild) {
    // Go ahead and handle the association (i.e. handle the callers requests) in
    // this process
    handleAssociation();
  }
#ifdef HAVE_FORK
    ...
    // Spawn a sub-process to handle the association (i.e. handle the callers
    // requests)
...
OFCondition DcmSCP::negotiateAssociation() {
  ...
  if (m_assocConfig == NULL) {
    DCMNET_ERROR(
        "Cannot negotiate association: Missing association configuration");
    return EC_IllegalCall;  // TODO: need to find better error code
  }
  ...
  OFCondition result;
  if (m_assocConfig)
    result = m_assocConfig->evaluateAssociationParameters(
        m_assocCfgProfileName.c_str(), *m_assoc);
  else
    return EC_IllegalCall;  // TODO: need to find better error code
  if (result.bad()) {
    OFString tempStr;
...
void DcmSCP::handleAssociation() {
  if (m_assoc == NULL) {
    DCMNET_WARN(
        "DcmSCP::handleAssociation() called but SCP actually has no "
        "association running, ignoring");
    return;
  }

  // Receive a DIMSE command and perform all the necessary actions. (Note that
  // ReceiveAndHandleCommands() will always return a value 'cond' for which
  // 'cond.bad()' will be true. This value indicates that either some kind of
  // error occurred, or that the peer aborted the association
  // (DUL_PEERABORTEDASSOCIATION), or that the peer requested the release of the
  // association (DUL_PEERREQUESTEDRELEASE).) (Also note that
  // ReceiveAndHandleCommands() will never return EC_Normal.)
  OFCondition cond = EC_Normal;
  T_DIMSE_Message msg;
  ...
  while (cond.good()) {
    ...
    cond =
        DIMSE_receiveCommand(m_assoc, DIMSE_BLOCKING, 0, &presID, &msg, NULL);
...
OFCondition DcmSCP::handleIncomingCommand(
    T_DIMSE_Message* msg, const DcmPresentationContextInfo& info) {
  OFCondition cond;
  if (msg->CommandField == DIMSE_C_ECHO_RQ) {
    // Process C-ECHO request
    cond = handleECHORequest(msg->msg.CEchoRQ, info.presentationContextID);
  } else {
    // We cannot handle this kind of message. Note that the condition will be
    // returned and that the caller is responsible to end the association if
    // desired.
    OFString tempStr;
    DCMNET_ERROR("Cannot handle this kind of DIMSE command (0x"
                 << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
                 << STD_NAMESPACE setw(4)
...
OFCondition DcmSCP::handleECHORequest(T_DIMSE_C_EchoRQ& reqMessage,
  ...
  DCMNET_DEBUG(
      DIMSE_dumpMessage(tempStr, reqMessage, DIMSE_INCOMING, NULL, presID));
  DCMNET_INFO("Sending C-ECHO Response");

  // Send an echo response
  OFCondition cond = DIMSE_sendEchoResponse(m_assoc, presID, &reqMessage,
                                            STATUS_Success, NULL);
  if (cond.bad())
    DCMNET_ERROR(
        "Cannot send C-ECHO Response: " << DimseCondition::dump(tempStr, cond));
  else
...
OFCondition DcmSCP::sendSTOREResponse(T_ASC_PresentationContextID presID,
  ...
  return DIMSE_sendStoreResponse(m_assoc, presID, &reqMessage, &rspMessage,
                                 statusDetail);
}

// ----------------------------------------------------------------------------

OFCondition DcmSCP::handleEVENTREPORTRequest(
  ...
  //  DcmDataset *statusDetail = NULL; // TODO: do we need this and if so, how
  //  do we get it?
  Uint16 statusCode = 0;
  ...
  if (reqMessage.DataSetType == DIMSE_DATASET_NULL) {
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, reqMessage, DIMSE_INCOMING, NULL, presID));
    DCMNET_ERROR(
        "Received N-EVENT-REPORT request but no dataset announced, aborting");
    return DIMSE_BADMESSAGE;
  }

  // Receive dataset
  cond = receiveDIMSEDataset(&presIDdset, &dataset, NULL /* callback */,
                             NULL /* callbackContext */);
  if (cond.bad()) {
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, reqMessage, DIMSE_INCOMING, NULL, presID));
    DCMNET_ERROR(
        "Unable to receive N-EVENT-REPORT dataset on presentation context "
  ...
  if (DCM_dcmnetGetLogger().isEnabledFor(OFLogger::TRACE_LOG_LEVEL))
    DCMNET_DEBUG(DIMSE_dumpMessage(tempStr, reqMessage, DIMSE_INCOMING, dataset,
                                   presID));
  else
    DCMNET_DEBUG(
        DIMSE_dumpMessage(tempStr, reqMessage, DIMSE_INCOMING, NULL, presID));
  ...
  if (presIDdset != presID) {
    DCMNET_ERROR("Presentation Context ID of command ("
                 << OFstatic_cast(unsigned int, presID) << ") and data set ("
  ...
  DCMNET_DEBUG(
      DIMSE_dumpMessage(tempStr, response, DIMSE_OUTGOING, NULL, presID));
  cond = sendDIMSEMessage(presID, &response, NULL /* dataObject */,
                          NULL /* callback */, NULL /* callbackContext */);
  if (cond.bad()) {
    DCMNET_ERROR("Failed sending N-EVENT-REPORT response: "
...
// Sends a DIMSE command and possibly also instance data to the configured peer
// DICOM application
OFCondition DcmSCP::sendDIMSEMessage(const T_ASC_PresentationContextID presID,
  ...
  cond = DIMSE_sendMessageUsingMemoryData(
      m_assoc, presID, msg, NULL /*statusDetail*/, dataObject, callback,
      callbackContext, commandSet);
  return cond;
}
...
OFCondition DcmSCP::receiveDIMSECommand(T_ASC_PresentationContextID* presID,
  ...
  OFCondition cond;
  if (timeout > 0) {
    /* call the corresponding DIMSE function to receive the command (use
     * specified timeout) */
    cond = DIMSE_receiveCommand(m_assoc, DIMSE_NONBLOCKING, timeout, presID,
                                msg, statusDetail, commandSet);
  } else {
    /* call the corresponding DIMSE function to receive the command (use default
     * timeout) */
...
OFCondition DcmSCP::receiveDIMSEDataset(T_ASC_PresentationContextID* presID,
  ...
  cond =
      DIMSE_receiveDataSetInMemory(m_assoc, m_blockMode, m_dimseTimeout, presID,
...
void DcmSCP::removeProcessFromTable(int pid) {
  ...
  DCMNET_WARN("Could not find and delete process with ID "
              << pid << " from internal process table");
}
...
void DcmSCP::cleanChildren() {
  ...
  while (child > 0) {
    ...
    if (child == 0) {
      ...
      if (errno == ECHILD) {
        ...
        DCMNET_WARN(
            "Wait for child failed while cleaning child process (ignored)");
      }
    } else if (child > 0) {
      // dump some debug information
  ...
  DCMNET_WARN(
      "Don't know how to wait for child processes on this platform (ignoring)");
#endif
}
...
//      {
//        DCMNET_WARN("Value inside given LO attribute too large for status
//        detail (max " << OFstatic_cast(Uint32, vr.getMaxValueLength()) << ")
//        for " << vr.getVRName());
//      }
...
//      {
//        DCMNET_WARN("Value inside given AT attribute too large for status
//        detail (max " << OFstatic_cast(Uint32, vr.getMaxValueLength()) << ")
//        for " << vr.getVRName());
//      }
...
//      DCMNET_ERROR("Cannot add status detail, unsupported detail attribute
//      type: " << vr.getVRName()); break;
//  }
//}
...
OFCondition DcmSCP::loadAssociationCfgFile(const OFString& assocFile) {
  ...
  OFCondition result = DcmAssociationConfigurationFile::initialize(
      *m_assocConfig, assocFile.c_str());
  if (result.bad()) {
    DCMNET_ERROR("DcmSCP: Unable to parse association configuration file "
                 << assocFile << ": " << result.text());
    delete m_assocConfig;
    m_assocConfig = NULL;
  }
  return result;
...
OFCondition DcmSCP::setAndCheckAssociationProfile(const OFString& profileName) {
  ...
  const unsigned char* c =
      OFreinterpret_cast(const unsigned char*, profileName.c_str());
  while (*c) {
    if (!isspace(*c))
      mangledName += OFstatic_cast(char, toupper(*c));
    ++c;
  }
  /* check profile */
  if (result.good() && !m_assocConfig->isKnownProfile(mangledName.c_str())) {
    DCMNET_ERROR("No association profile named \""
                 << profileName << "\" in association configuration");
  ...
  if (result.good() && !m_assocConfig->isValidSCPProfile(mangledName.c_str())) {
    DCMNET_ERROR("The association profile named \""
                 << profileName << "\" is not a valid SCP association profile");
    result = EC_IllegalParameter;  // TODO: need to find better error code
  }
  if (result.good())
...
OFCondition DcmSCP::addPresentationContext(const OFString& abstractSyntax,
  ...
  if (result.good()) {
    result = m_assocConfig->addPresentationContext(
        DCMSCP_PC_KEY, abstractSyntax.c_str(), DCMSCP_TS_KEY);
  }
  if (result.good()) {
    result =
        m_assocConfig->addRole(DCMSCP_RO_KEY, abstractSyntax.c_str(), role);
  }
  /* perform name mangling for config file key */
  const unsigned char* c =
      OFreinterpret_cast(const unsigned char*, profile.c_str());
  OFString mangledName;
  while (*c) {
    if (!isspace(*c))
      mangledName += OFstatic_cast(char, toupper(*c));
  ...
  if (result.good() && !m_assocConfig->isKnownProfile(mangledName.c_str())) {
    result = m_assocConfig->addProfile(mangledName.c_str(), DCMSCP_PC_KEY,
                                       DCMSCP_RO_KEY);
  }
...
void DcmSCP::notifyAssociationRequest(const T_ASC_Parameters& params,
  ...
  // Dump some information if required
  DCMNET_INFO("Association Received "
  ...
  OFString tempStr;
  if (m_verbosePCMode)
    DCMNET_INFO("Incoming Association Request:"
                << OFendl
...
void DcmSCP::notifyDIMSEError(const OFCondition& cond) {
  OFString tempStr;
  DCMNET_DEBUG("DIMSE Error, detail (if available): " << DimseCondition::dump(
                   tempStr, cond));
}
...
** Added check on whether the presentation context ID of command and data set
*are
