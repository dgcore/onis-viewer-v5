...
#include <sys/socket.h>  /* for getpeername(), accept() */
...
#endif
#ifdef HAVE_SIGNAL_H
// On Solaris with Sun Workshop 11, <signal.h> declares signal() but <csignal>
// does not
#include <signal.h>
#endif
END_EXTERN_C
...
static OFCondition createNetworkKey(const char* mode, int timeout,
                                    unsigned long opt, PRIVATE_NETWORKKEY** k);
static OFCondition createAssociationKey(PRIVATE_NETWORKKEY** net,
                                        const char* node, unsigned long maxPDU,
                                        PRIVATE_ASSOCIATIONKEY** assoc);
static OFCondition initializeNetworkTCP(PRIVATE_NETWORKKEY** k, void* p);
static OFCondition receiveTransportConnection(
    PRIVATE_NETWORKKEY** network, DUL_BLOCKOPTIONS block, int timeout,
    DUL_ASSOCIATESERVICEPARAMETERS* params,
    PRIVATE_ASSOCIATIONKEY** association);
static OFCondition receiveTransportConnectionTCP(
    PRIVATE_NETWORKKEY** network, DUL_BLOCKOPTIONS block, int timeout,
    DUL_ASSOCIATESERVICEPARAMETERS* params,
    PRIVATE_ASSOCIATIONKEY** association);
...
#endif
static OFCondition get_association_parameter(
    void* paramAddress, DUL_DATA_TYPE paramType, size_t paramLength,
...
void DUL_returnAssociatePDUStorage(DUL_ASSOCIATIONKEY* dulassoc, void*& pdu,
...
unsigned long DUL_getPeerCertificate(DUL_ASSOCIATIONKEY* dulassoc, void* buf,
...
OFCondition DUL_InitializeNetwork(const char* mode, void* networkParameter,
                                  int timeout, unsigned long opt,
                                  DUL_NETWORKKEY** networkKey) {
  // default return value if something goes wrong
  *networkKey = NULL;

  // a few initializations must only be done the first time this function is
  // called.
...
OFCondition DUL_DropNetwork(DUL_NETWORKKEY** callerNetworkKey) {
  PRIVATE_NETWORKKEY
  **networkKey;
...
OFCondition DUL_RequestAssociation(DUL_NETWORKKEY** callerNetworkKey,
                                   DUL_ASSOCIATESERVICEPARAMETERS* params,
  ...
  **network;
  ...
  if (params->maxPDU < MIN_PDU_LENGTH || params->maxPDU > MAX_PDU_LENGTH) {
    return makeDcmnetCondition(
        DULC_ILLEGALPARAMETER, OF_error,
  ...
  if (activatePDUStorage)
    DUL_activateAssociatePDUStorage(*association);

  /* send a request primitive */
  cond = PRV_StateMachine(network, association, A_ASSOCIATE_REQ_LOCAL_USER,
                          (*network)->protocolState, params);
  ...
  if (cond.bad()) {
     ...
     */
    OFCondition cond2 =
        PRV_StateMachine(network, association, TRANS_CONN_CLOSED,
                         (*association)->protocolState, NULL);
    destroyAssociationKey(association);
    return makeDcmnetSubCondition(DULC_REQUESTASSOCIATIONFAILED, OF_error,
                                  "Failed to establish association",
                                  DimseCondition::push(cond2, cond));
  }
  cond = PRV_StateMachine(network, association, TRANS_CONN_CONFIRM_LOCAL_USER,
...
OFCondition DUL_ReceiveAssociationRQ(DUL_NETWORKKEY** callerNetworkKey,
  ...
  **association;
  ...
  if (params->maxPDU < MIN_PDU_LENGTH || params->maxPDU > MAX_PDU_LENGTH)
    return makeDcmnetCondition(
        DULC_ILLEGALPARAMETER, OF_error,
        "DUL Illegal parameter (maxPDU) in function DUL_ReceiveAssociationRQ");
  ...
  cond =
      receiveTransportConnection(network, block, timeout, params, association);
  ...
  cond = PRV_StateMachine(network, association, TRANS_CONN_INDICATION,
                          (*network)->protocolState, params);
  if (cond.bad())
    return cond;
  ...
  if (cond == DUL_UNSUPPORTEDPEERPROTOCOL) {
    ...
    (void)PRV_StateMachine(NULL, association, A_ASSOCIATE_RESPONSE_REJECT,
...
OFCondition DUL_AcknowledgeAssociationRQ(DUL_ASSOCIATIONKEY** callerAssociation,
                                         DUL_ASSOCIATESERVICEPARAMETERS* params,
                                         int activatePDUStorage) {
  PRIVATE_ASSOCIATIONKEY
  **association;
  ...
  cond = PRV_StateMachine(NULL, association, A_ASSOCIATE_RESPONSE_ACCEPT,
                          (*association)->protocolState, params);
...
OFCondition DUL_RejectAssociationRQ(DUL_ASSOCIATIONKEY** callerAssociation,
  ...
  **association;
  ...
  localParams.source = 0x01;
  {
    ...
    if (!found) {
      ...
      return makeDcmnetCondition(DULC_ILLEGALREJECTREASON, OF_error,
                                 msg.c_str());
    }
  }
  {
    ...
    if (!found) {
      ...
      OFSTRINGSTREAM_GETOFSTRING(stream, msg)
      return makeDcmnetCondition(DULC_ILLEGALREJECTRESULT, OF_error,
                                 msg.c_str());
    }
  }
  cond = PRV_StateMachine(NULL, association, A_ASSOCIATE_RESPONSE_REJECT,
                          (*association)->protocolState, &localParams);
...
OFCondition DUL_DropAssociation(DUL_ASSOCIATIONKEY** callerAssociation) {
  PRIVATE_ASSOCIATIONKEY
  **association;
...
OFCondition DUL_ReleaseAssociation(DUL_ASSOCIATIONKEY** callerAssociation) {
  PRIVATE_ASSOCIATIONKEY
  **association;
  unsigned char pduType;
  int event;
  ...
  cond = PRV_StateMachine(NULL, association, A_RELEASE_REQ,
                          (*association)->protocolState, NULL);
  if (cond.bad())
    return cond;

  cond =
      PRV_NextPDUType(association, DUL_NOBLOCK, PRV_DEFAULTTIMEOUT, &pduType);
  if (cond == DUL_NETWORKCLOSED)
    event = TRANS_CONN_CLOSED;
  else if (cond == DUL_READTIMEOUT)
    event = ARTIM_TIMER_EXPIRED;
  ...
  return PRV_StateMachine(NULL, association, event,
                          (*association)->protocolState, NULL);
}
...
OFCondition DUL_AcknowledgeRelease(DUL_ASSOCIATIONKEY** callerAssociation) {
  PRIVATE_ASSOCIATIONKEY
  **association;
  ...
  cond = PRV_StateMachine(NULL, association, A_RELEASE_RESP,
                          (*association)->protocolState, NULL);
  if (cond.bad())
    return cond;
...
OFCondition DUL_AbortAssociation(DUL_ASSOCIATIONKEY** callerAssociation) {
  DUL_ABORTITEMS abortItems = {0, DUL_SCU_INITIATED_ABORT, 0};
  int event = 0;
  unsigned char pduType = 0;

  PRIVATE_ASSOCIATIONKEY** association =
      (PRIVATE_ASSOCIATIONKEY**)callerAssociation;
  OFCondition cond = checkAssociation(association);
  if (cond.bad())
    return cond;
  ...
  OFBool done = OFFalse;
  while (!done) {
    cond = PRV_NextPDUType(association, DUL_NOBLOCK, PRV_DEFAULTTIMEOUT,
                           &pduType);  // may return DUL_NETWORKCLOSED.
...
OFCondition DUL_WritePDVs(DUL_ASSOCIATIONKEY** callerAssociation,
                          DUL_PDVLIST* pdvList) {
  PRIVATE_ASSOCIATIONKEY
  **association;
  ...
  /* call the finite state machine to invoke an action function given the
   * current */
...
OFCondition DUL_ReadPDVs(DUL_ASSOCIATIONKEY** callerAssociation,
                         DUL_PDVLIST* pdvList, DUL_BLOCKOPTIONS block,
                         int timeout) {
  PRIVATE_ASSOCIATIONKEY
  **association;
  unsigned char pduType;
...
OFCondition DUL_AssociationParameter(DUL_ASSOCIATIONKEY** callerAssociation,
                                     DUL_ASSOCIATION_PARAMETER param,
                                     DUL_DATA_TYPE type, void* address,
                                     size_t length) {
  PRIVATE_ASSOCIATIONKEY
  **association;
    ...
    case DUL_K_MAX_PDV_XMIT:
      if ((*association)->applicationFunction == DICOM_APPLICATION_ACCEPTOR) {
        cond = get_association_parameter(
            &((*association)->maxPDVRequestor), DUL_K_INTEGER,
            sizeof((*association)->maxPDVRequestor), type, address, length);
      } else {
        cond = get_association_parameter(
            &((*association)->maxPDVAcceptor), DUL_K_INTEGER,
            sizeof((*association)->maxPDVAcceptor), type, address, length);
      }
      if (cond.bad())
        return cond;
      break;
...
OFCondition DUL_NextPDV(DUL_ASSOCIATIONKEY** callerAssociation, DUL_PDV* pdv) {
  PRIVATE_ASSOCIATIONKEY
  **association;
  unsigned char* p;
  unsigned long pdvLength;
  ...
  *pdv = (*association)->currentPDV;
  ...
  /* indicate that the current PDV was read by increasing the association's PDV
   * index variable */
  (*association)->pdvIndex++;

  /* if the association's pdvIndex does now show a value equal to or greater */
  /* than pdvCount there are no more unprocessed PDVs contained in the
   * association. */
  ...
  if ((*association)->pdvIndex >= (*association)->pdvCount)
    ...
    /* if the association still contains unprocessed PDVs, we need to reset the
     * association's */
    /* currentPDV variable so that it contains the data of the next unprocessed
     * PDV */
    /* (currentPDV shall always contain the next unprocessed PDV.) */

    /* The variable (*association)->pdvPointer always points to the buffer
     * address */
    /* where the information of the PDV which is represented by the
     * association's */
    /* currentPDV variable can be found. Remember this address in a local
     * variable. */
    p = (*association)->pdvPointer;

    /* move this pointer to the next (unprocessed) PDV which is contained in the
     * association. */
    /* (We need to move the pointer 4 bytes to the right (over the PDV length
     * field of the */
    /* current PDV), another 2 bytes to the right (over the presentation context
     * ID and */
    /* message control header field of the current PDV) and another
     * currentPDV.fragmentLength */
    ...
    /* set the data fragment length in the association's
     * currentPDV.fragmentLength variable */
    /* (we start now overwriting all the entries in (*association)->currentPDV).
     * The actual */
    /* length of the data fragment of the next (unprocessed) PDV equals the
     * above determined */
    ...
    /* variable. This value is 1 byte wide and contained in the 5th byte of p
    ...
    /* now determine if the next (unprocessed) PDV contains the last fragment of
     * a data set or DIMSE */
    /* command and if the next (unprocessed) PDV is a data set PDV or command
     * PDV. This information */
    ...
    if (u & 2)
      ...
      (*association)->currentPDV.lastPDV =
          OFFalse;  // if bit 1 of the message control header is 0, this
                    // fragment does not contain the last fragment of a data set
                    // or command

    if (u & 1)
      (*association)->currentPDV.pdvType =
          DUL_COMMANDPDV;  // if bit 0 of the message control header is 1, this
                           // is a command PDV
    else
      (*association)->currentPDV.pdvType =
          DUL_DATASETPDV;  // if bit 0 of the message control header is 0, this
                           // is a data set PDV
    ...
    /* and finally, after the association's currentPDV variable has been
     * completely set to the next */
    /* (unprocessed) PDV, we need to update the association's pdvPointer, so
     * that it again points */
    /* to the address of the PDV which is represented by the association's
     * currentPDV variable. */
...
OFCondition DUL_ClearServiceParameters(DUL_ASSOCIATESERVICEPARAMETERS* params) {
  ...
  delete params->reqUserIdentNeg;
  params->reqUserIdentNeg = NULL;
  delete params->ackUserIdentNeg;
  params->ackUserIdentNeg = NULL;
  return EC_Normal;
}
...
static OFCondition receiveTransportConnection(
    ...
    DUL_ASSOCIATESERVICEPARAMETERS* params,
    PRIVATE_ASSOCIATIONKEY** association) {
  return receiveTransportConnectionTCP(network, block, timeout, params,
                                       association);
}
...
static OFCondition receiveTransportConnectionTCP(
  ...
  if (sock > 0) {
    ...
    if (getpeername(sock, &from, &len)) {
      char buf[256];
      OFOStringStream stream;
      stream << "TCP Initialization Error: "
             << OFStandard::strerror(errno, buf, sizeof(buf))
             << ", getpeername failed on socket " << sock
             << OFStringStream_ends;
      OFSTRINGSTREAM_GETOFSTRING(stream, msg)
    ...
    if (block == DUL_NOBLOCK) {
      ...
      FD_SET((unsigned int)((*network)->networkSpecific.TCP.listenSocket),
             &fdset);
#else
...
#else
      nfound = select((*network)->networkSpecific.TCP.listenSocket + 1, &fdset,
  ...
  if (shouldFork) {
    ...
    if (pid > 0) {
      ...
      OFOStringStream stream;
      stream << "New child process started with pid " << pid
             << OFStringStream_ends;
      OFSTRINGSTREAM_GETOFSTRING(stream, msg)
  ...
  if (shouldFork && (command_argc > 0) && command_argv) {
    ...
    if (!CreatePipe(&hChildStdInRead, &hChildStdInWrite, &sa, 0)) {
      OFOStringStream stream;
      stream << "Multi-Process Error: Creating anonymous pipe failed with "
                "error code "
    ...
    if (!DuplicateHandle(GetCurrentProcess(), hChildStdInWrite,
      ...
      return makeDcmnetCondition(
          DULC_CANNOTFORK, OF_error,
          "Multi-Process Error: Duplicating handle failed");
    }
    ...
    if (!CreateProcess(NULL, OFconst_cast(char*, cmdLine.c_str()), NULL, NULL,
      ...
      if (DuplicateHandle(hParentProcessHandle, (HANDLE)sock, pi.hProcess,
        ...
        if (!WriteFile(hChildStdInWriteDup, buf, strlen(buf) + 1, &bytesWritten,
                       NULL)) {
          CloseHandle(hChildStdInWriteDup);
          return makeDcmnetCondition(
              DULC_CANNOTFORK, OF_error,
              "Multi-Process Error: Writing to anonymous pipe failed");
        }
        ...
        stream << "New child process started with pid "
               ...
               << OFreinterpret_cast(int, childSocketHandle)
               << OFStringStream_ends;
        OFSTRINGSTREAM_GETOFSTRING(stream, msg)
        return makeDcmnetCondition(DULC_FORKEDCHILD, OF_ok, msg.c_str());
      } else {
        ...
        return makeDcmnetCondition(
            DULC_CANNOTFORK, OF_error,
            "Multi-Process Error: Duplicating socket handle failed");
      }
    }
  }
#endif
  ...
  DCMNET_DEBUG("setting network receive timeout to " << recvTimeout
                                                     << " seconds");
#ifdef HAVE_WINSOCK_H
  // for Windows, specify receive timeout in milliseconds
  int timeoutVal = recvTimeout * 1000;
  if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, (char*)&timeoutVal,
                 sizeof(timeoutVal)) < 0)
#else
  ...
  if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeoutVal,
                 sizeof(timeoutVal)) < 0)
#endif
  {
    // according to MSDN: available in the Microsoft implementation of Windows
    // Sockets 2, so we are reporting a warning message but are not returning
    // with an error code; this also applies to all other systems where the call
    // to this function might fail
    DCMNET_WARN("cannot set network receive timeout to " << recvTimeout
                                                         << " seconds");
  }
#endif
#endif
   ...
   * Ethernet). Effects on other environments are unknown. The code below allows
   * the Nagle algorithm to be enabled by setting the TCP_NODELAY environment
  ...
  if ((tcpNoDelayString = getenv("TCP_NODELAY")) != NULL) {
    if (sscanf(tcpNoDelayString, "%d", &tcpNoDelay) != 1) {
      DCMNET_WARN("DUL: cannot parse environment variable TCP_NODELAY="
                  << tcpNoDelayString);
    }
  }
  if (tcpNoDelay) {
    if (setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char*)&tcpNoDelay,
                   sizeof(tcpNoDelay)) < 0) {
      char buf[256];
      OFString msg = "TCP Initialization Error: ";
      msg += OFStandard::strerror(errno, buf, sizeof(buf));
  ...
  sprintf(client_ip_address,
          "%-d.%-d.%-d.%-d",  // this code is ugly but thread safe
          ((int)from.sa_data[2]) & 0xff, ((int)from.sa_data[3]) & 0xff,
          ((int)from.sa_data[4]) & 0xff, ((int)from.sa_data[5]) & 0xff);
  ...
  if (remote == NULL) {
    ...
    OFStandard::strlcpy((*association)->remoteNode, client_ip_address,
    ...
    OFStandard::strlcpy((*association)->remoteNode, node,
                        sizeof((*association)->remoteNode));
    OFStandard::strlcpy(params->callingPresentationAddress, node,
                        sizeof(params->callingPresentationAddress));
  }
  ...
  if (daemon) {
    ...
    if (client_dns_name.empty())
      client_dns_name = STRING_UNKNOWN;
  ...
  if ((*network)->networkSpecific.TCP.tLayer) {
    (*association)->connection =
        ((*network)->networkSpecific.TCP.tLayer)
            ->createConnection(sock, params->useSecureLayer);
  } else
    (*association)->connection = NULL;
  ...
  if (TCS_ok !=
      (tcsStatus = (*association)->connection->serverSideHandshake())) {
...
static OFCondition createNetworkKey(const char* mode, int timeout,
                                    unsigned long opt,
                                    PRIVATE_NETWORKKEY** key) {
  if (strcmp(mode, AE_REQUESTOR) != 0 && strcmp(mode, AE_ACCEPTOR) != 0 &&
...
**      Description of the algorithm (optional) and any other notes.
*/
static OFCondition initializeNetworkTCP(PRIVATE_NETWORKKEY** key,
                                        void* parameter) {
  struct linger sockarg;
  int reuse = 1;
  ...
  if ((dcmExternalSocketHandle.get() < 0) &&
    ...
    /* GUSI always returns an error for setsockopt(...) */
#else
    if (setsockopt((*key)->networkSpecific.TCP.listenSocket, SOL_SOCKET,
    ...
    sockarg.l_onoff = 0;
    if (setsockopt((*key)->networkSpecific.TCP.listenSocket, SOL_SOCKET,
                   SO_LINGER, (char*)&sockarg, sizeof(sockarg)) < 0) {
      char buf[256];
      OFString msg = "TCP Initialization Error: ";
      msg += OFStandard::strerror(errno, buf, sizeof(buf));
      return makeDcmnetCondition(DULC_TCPINITERROR, OF_error, msg.c_str());
  ...
  (*key)->networkSpecific.TCP.tLayer =
      new DcmTransportLayer((*key)->applicationFunction);
...
**      Description of the algorithm (optional) and any other notes.
*/
static OFCondition createAssociationKey(
    PRIVATE_NETWORKKEY** networkKey, const char* remoteNode,
    unsigned long maxPDU, PRIVATE_ASSOCIATIONKEY** associationKey) {
...
static OFCondition get_association_parameter(
  ...
  if (paramType != outputType)
    return DUL_WRONGDATATYPE;
  if ((paramType == DUL_K_INTEGER) && (outputLength != paramLength))
    return DUL_INCORRECTBUFFERLENGTH;
  if ((paramType == DUL_K_STRING) &&
      (outputLength < strlen((char*)paramAddress)))
    return DUL_INSUFFICIENTBUFFERLENGTH;
...
static void setTCPBufferLength(int sock) {
  ...
  if ((TCPBufferLength = getenv("TCP_BUFFER_LENGTH")) != NULL) {
    if (sscanf(TCPBufferLength, "%d", &bufLen) != 1) {
      DCMNET_WARN("DUL: cannot parse environment variable TCP_BUFFER_LENGTH="
                  << TCPBufferLength);
    }
  }
...
**      Description of the algorithm (optional) and any other notes.
*/
OFString& DUL_DumpParams(OFString& ret_str,
                         DUL_ASSOCIATESERVICEPARAMETERS* params) {
  OFOStringStream str;
  OFString temp_str;
...
static SC_MAP scMap[] = {
    {DUL_SC_ROLE_NONE, "None"},     {DUL_SC_ROLE_DEFAULT, "Default"},
    {DUL_SC_ROLE_SCU, "SCU"},       {DUL_SC_ROLE_SCP, "SCP"},
...
OFString& dumpExtNegList(OFString& ret,
  ...
  while (i != lst.end()) {
    ...
    const char* uidName =
        dcmFindNameOfUID(extNeg->sopClassUID.c_str(), "Unknown-UID");
    str << "  =" << uidName << " (" << extNeg->sopClassUID.c_str() << ")"
        << OFendl << "    [";
    for (int k = 0; k < (int)extNeg->serviceClassAppInfoLength; k++) {
      str << "0x";
      str << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
          << STD_NAMESPACE setw(2) << (int)(extNeg->serviceClassAppInfo[k]);
...
static void clearPresentationContext(LST_HEAD** l) {
  ...
  *transfer;
  ...
  while ((ctx = (DUL_PRESENTATIONCONTEXT*)LST_Pop(l)) != NULL) {
    if (ctx->proposedTransferSyntax != NULL) {
      while ((transfer = (DUL_TRANSFERSYNTAX*)LST_Pop(
                  &ctx->proposedTransferSyntax)) != NULL)
        free(transfer);
...
OFCondition DUL_setTransportLayer(DUL_NETWORKKEY* callerNetworkKey,
  ...
  if (callerNetworkKey && newLayer) {
    ...
    if ((key->networkSpecific.TCP.tLayerOwned) &&
        (key->networkSpecific.TCP.tLayer))
      delete key->networkSpecific.TCP.tLayer;
...
OFString& DUL_DumpConnectionParameters(OFString& str,
...
void DUL_DumpConnectionParameters(DUL_ASSOCIATIONKEY* association,
...
** Fixed infinite loop in DUL_AbortAssociation that could occur on Win2K
*systems.
**
...
**   parameter if this type is defined. Avoids warning on recent Linux
*distributions.
**
