...
static OFCondition constructSubItem(char* name, unsigned char type,
                                    DUL_SUBITEM* applicationContext,
                                    unsigned long* rtnlen);
static OFCondition constructPresentationContext(
    unsigned char associateType, unsigned char contextID, unsigned char reason,
    char* abstractSyntax, LST_HEAD** proposedTransferSyntax,
    char* acceptedTransferSyntax, PRV_PRESENTATIONCONTEXTITEM* context,
    unsigned long* rtnLength);
static OFCondition constructUserInfo(unsigned char type,
                                     DUL_ASSOCIATESERVICEPARAMETERS* params,
                                     DUL_USERINFO* userInfo,
                                     unsigned long* rtnLen);
static OFCondition constructMaxLength(unsigned long maxPDU, DUL_MAXLENGTH* max,
                                      unsigned long* rtnLen);
static OFCondition constructSCUSCPRoles(unsigned char type,
                                        DUL_ASSOCIATESERVICEPARAMETERS* params,
                                        LST_HEAD** lst,
                                        unsigned long* rtnLength);
static OFCondition constructSCUSCPSubItem(char* name, unsigned char type,
                                          unsigned char scuRole,
                                          unsigned char scpRole,
                                          PRV_SCUSCPROLE* scuscpItem,
                                          unsigned long* length);
static OFCondition streamSubItem(DUL_SUBITEM* item, unsigned char* b,
                                 unsigned long* len);
static OFCondition streamPresentationContext(LST_HEAD** presentationContextList,
                                             unsigned char* b,
                                             unsigned long* length);
static OFCondition streamUserInfo(DUL_USERINFO* userInfo, unsigned char* b,
                                  unsigned long* length);
static OFCondition streamMaxLength(DUL_MAXLENGTH* max, unsigned char* b,
                                   unsigned long* length);
static OFCondition streamSCUSCPList(LST_HEAD** lst, unsigned char* b,
                                    unsigned long* length);
static OFCondition streamSCUSCPRole(PRV_SCUSCPROLE* scuscpRole,
                                    unsigned char* b, unsigned long* len);
static OFCondition constructExtNeg(unsigned char type,
                                   DUL_ASSOCIATESERVICEPARAMETERS* params,
                                   SOPClassExtendedNegotiationSubItemList** lst,
                                   unsigned long* rtnLength);

static OFCondition streamExtNegList(SOPClassExtendedNegotiationSubItemList* lst,
                                    unsigned char* b, unsigned long* length);

static OFCondition streamExtNeg(SOPClassExtendedNegotiationSubItem* extNeg,
                                unsigned char* b, unsigned long* len);
...
OFCondition constructAssociatePDU(DUL_ASSOCIATESERVICEPARAMETERS* params,
                                  unsigned char type, PRV_ASSOCIATEPDU* pdu) {
  unsigned long itemLength; /* Holds length of an item computed by a
                             * lower level routine. */
  ...
  *contextItem; /* A presentation context item which is
  ...
  if (strlen(params->callingAPTitle) < 1 || strlen(params->callingAPTitle) > 16)
    return makeDcmnetCondition(DULC_ILLEGALSERVICEPARAMETER, OF_error,
                               "Illegal service parameter: Calling AP Title");
  ...
  OFCondition cond = constructSubItem(params->applicationContextName,
                                      DUL_TYPEAPPLICATIONCONTEXT,
                                      &pdu->applicationContext, &itemLength);
  if (cond.bad())
    return cond;
  ...
  if (type == DUL_TYPEASSOCIATERQ) {
    ...
    presentationCtx = (DUL_PRESENTATIONCONTEXT*)LST_Head(
        &params->requestedPresentationContext);
    (void)LST_Position(&params->requestedPresentationContext,
                       (LST_NODE*)presentationCtx);
    while (presentationCtx != NULL && cond.good()) {
      contextItem = (PRV_PRESENTATIONCONTEXTITEM*)malloc(
          sizeof(PRV_PRESENTATIONCONTEXTITEM));
      if (contextItem == NULL)
        return EC_MemoryExhausted;

      cond = constructPresentationContext(
          type, presentationCtx->presentationContextID, presentationCtx->result,
          presentationCtx->abstractSyntax,
          &presentationCtx->proposedTransferSyntax, NULL, contextItem,
          &itemLength);
      OFCondition cond2 =
      ...
      pdu->length += itemLength;
      presentationCtx = (DUL_PRESENTATIONCONTEXT*)LST_Next(
          &params->requestedPresentationContext);
    }
  } else {
    ...
    if (params->acceptedPresentationContext != NULL) {
      presentationCtx = (DUL_PRESENTATIONCONTEXT*)LST_Head(
          &params->acceptedPresentationContext);
      if (presentationCtx != NULL)
        (void)LST_Position(&params->acceptedPresentationContext,
                           (LST_NODE*)presentationCtx);
      while (presentationCtx != NULL && cond.good()) {
        contextItem =
            (PRV_PRESENTATIONCONTEXTITEM*)malloc(sizeof(*contextItem));
        if (contextItem == NULL)
          return EC_MemoryExhausted;

        cond = constructPresentationContext(
            type, presentationCtx->presentationContextID,
            presentationCtx->result, presentationCtx->abstractSyntax, NULL,
            presentationCtx->acceptedTransferSyntax, contextItem, &itemLength);
...
OFCondition constructAssociateRejectPDU(unsigned char result,
...
**  Description of the algorithm (optional) and any other notes.
*/
OFCondition constructAbortPDU(unsigned char src, unsigned char reason,
                              DUL_REJECTRELEASEABORTPDU* pdu,
                              unsigned long mode) {
...
OFCondition constructDataPDU(void* buf, unsigned long length, DUL_DATAPDV type,
...
OFCondition streamAssociatePDU(PRV_ASSOCIATEPDU* assoc, unsigned char* b,
  ...
  *b++ = assoc->rsv2[1];
  ...
  b += 16;
  (void)strncpy((char*)b, assoc->callingAPTitle, strlen(assoc->callingAPTitle));
  b += 16;
  (void)memset(b, 0, 32);
  b += 32;
  ...
  *rtnLen += subLength;
  cond =
      streamPresentationContext(&assoc->presentationContextList, b, &subLength);
...
OFCondition streamRejectReleaseAbortPDU(DUL_REJECTRELEASEABORTPDU* pdu,
                                        unsigned char* b,
                                        unsigned long maxLength,
                                        unsigned long* rtnLen) {
  if (maxLength < 2 + 2 + 4) {
    return makeDcmnetCondition(DULC_CODINGERROR, OF_error,
                               "Coding Error in DUL routine: buffer too short "
                               "in streamRejectReleaseAbortPDU");
  }
...
OFCondition streamDataPDUHead(DUL_DATAPDU* pdu, unsigned char* buf,
  ...
  if (maxLength < 18) {
    return makeDcmnetCondition(
        ...
        "Coding Error in DUL routine: buffer too short in streamDataPDUHead");
  }
  ...
  *rtnLen += 8;
  ...
  if (associateType == DUL_TYPEASSOCIATERQ) {
    ...
    while (transfer != NULL) {
      ...
      cond = constructSubItem(transfer->transferSyntax, DUL_TYPETRANSFERSYNTAX,
                              subItem, &length);
      if (cond.bad())
        return cond;
    ...
    if (subItem == NULL)
      return EC_MemoryExhausted;
    cond = constructSubItem(acceptedTransferSyntax, DUL_TYPETRANSFERSYNTAX,
                            subItem, &length);
    if (cond.bad()) {
      free(subItem);
      return cond;
...
static OFCondition constructUserInfo(unsigned char type,
  ...
  *rtnLen = 4;
  ...
  OFCondition cond =
      constructMaxLength(params->maxPDU, &userInfo->maxLength, &length);
  if (cond.bad())
    return cond;
  ...
  *rtnLen += length;
  ...
  if (type == DUL_TYPEASSOCIATERQ)
    cond = constructSubItem(params->callingImplementationClassUID,
                            DUL_TYPEIMPLEMENTATIONCLASSUID,
                            &userInfo->implementationClassUID, &length);
  else
  ...
  if (type == DUL_TYPEASSOCIATERQ) {
    if (strlen(params->callingImplementationVersionName) != 0) {
      cond = constructSubItem(params->callingImplementationVersionName,
                              ...
                              &userInfo->implementationVersionName, &length);
      if (cond.bad())
        return cond;
      totalUserInfoLength += length;
      *rtnLen += length;
  ...
  *rtnLen += length;
  ...
  if (params->reqUserIdentNeg && (type == DUL_TYPEASSOCIATERQ)) {
    ...
    *(OFstatic_cast(UserIdentityNegotiationSubItemRQ*, userInfo->usrIdent)) =
        *(OFstatic_cast(UserIdentityNegotiationSubItemRQ*,
                        params->reqUserIdentNeg));
    totalUserInfoLength += length;
    *rtnLen += length;
    ...
    *(OFstatic_cast(UserIdentityNegotiationSubItemAC*, userInfo->usrIdent)) =
        *(OFstatic_cast(UserIdentityNegotiationSubItemAC*,
                        params->ackUserIdentNeg));
    totalUserInfoLength += length;
    *rtnLen += length;
  }
  if (totalUserInfoLength > 65535) {
    char errbuf[500];
    sprintf(errbuf,
            "Total length of user items (%lu bytes) exceeds upper limit of "
            "65535 bytes",
            totalUserInfoLength);
...
static OFCondition constructMaxLength(unsigned long maxPDU, DUL_MAXLENGTH* max,
  ...
  max->rsv1 = 0;
  max->length = 4;
  if (compatMode & 0x8000)
    max->maxLength = DUL_DULCOMPAT | DUL_DIMSECOMPAT | compatMode;
  else
  ...
  *rtnLength = 0;
  ...
  if (type == DUL_TYPEASSOCIATERQ) {
    presentationCtx = params->requestedPresentationContext != NULL
                          ? (DUL_PRESENTATIONCONTEXT*)LST_Head(
                                &params->requestedPresentationContext)
    ...
    while (presentationCtx != NULL) {
      ...
      if ((presentationCtx->proposedSCRole != DUL_SC_ROLE_DEFAULT) &&
        ...
        if ((scuRole == 0) && (scpRole == 0)) {
          ...
          DCMNET_WARN(
              "setting accepted SCP/SCU role to NONE, i.e. both role fields "
              "are 0 in SCP/SCU role selection sub-item");
        }
        cond = constructSCUSCPSubItem(presentationCtx->abstractSyntax,
                                      DUL_TYPESCUSCPROLE, scuRole, scpRole,
                                      scuscpItem, &length);
...
static OFCondition constructExtNeg(unsigned char type,
  ...
  *rtnLength = 0;
  ...
  } else if (type == DUL_TYPEASSOCIATEAC &&
  ...
  if (*lst != NULL) {
    ...
    while (i != (*lst)->end()) {
      ...
      extNeg->itemLength =
          2 + extNeg->sopClassUIDLength + extNeg->serviceClassAppInfoLength;
...
static OFCondition streamPresentationContext(LST_HEAD** presentationContextList,
  ...
  *length = 0;
  ...
  presentation =
      (PRV_PRESENTATIONCONTEXTITEM*)LST_Head(presentationContextList);
  ...
  while (presentation != NULL) {
    ...
    *length += 8;
    ...
    presentation =
        (PRV_PRESENTATIONCONTEXTITEM*)LST_Next(presentationContextList);
  }
  return EC_Normal;
}
...
static OFCondition streamExtNeg(SOPClassExtendedNegotiationSubItem* extNeg,
  ...
  *b++ = extNeg->reserved1;
  ...
  (void)memcpy(b, extNeg->serviceClassAppInfo,
               extNeg->serviceClassAppInfoLength);
