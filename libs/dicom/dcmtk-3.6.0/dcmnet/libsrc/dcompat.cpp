...
#include <fcntl.h>
#else
#include <fcntl.h> /* for flock structure */
#endif
...
int dcmtk_flock(int fd, int operation) {
  DCMNET_WARN("Unsupported flock(fd[" << fd << "],operation[0x" << hex
                                      << operation << "])");
  return 0;
}
 ...
 * Since _locking() does not implement shared/exclusive locks, the only
 * alternative would be to re-write the complete file access to Win32 API
 * functions. Have fun!
 */
...
int dcmtk_flock(int fd, int operation) {
  ...
  if (operation == LOCK_SH) {
    if (GetVersion() < 0x80000000)  // Windows NT
    {
      ...
      if (LockFileEx(handle, 0, 0, (DWORD)-1, (DWORD)-1, &overl) != 0)
        return 0;
      else
        return -1;
    } else {
      ...
      if (LockFile(handle, 0, 0, (DWORD)0x7FFFFFFF, 0) != 0)
        return 0;
      else
        return -1;
    }
    ...
    {
      // LockFileEx is only supported on Windows NT
      if (LockFileEx(handle, LOCKFILE_EXCLUSIVE_LOCK, 0, (DWORD)-1, (DWORD)-1,
                     &overl) != 0)
        return 0;
      else
        return -1;
    ...
    if (GetVersion() < 0x80000000)  // Windows NT
    {
      ...
      if (UnlockFileEx(handle, 0, (DWORD)-1, (DWORD)-1, &overl) != 0)
        return 0;
      else
        return -1;
    } else {
      // using UnlockFile on Win32s and Win95.
      if (UnlockFile(handle, 0, 0, (DWORD)0x7FFFFFFF, 0) != 0)
        return 0;
      else
        return -1;
    }
  } else
    return -1; /* unknown lock operation */
}
...
int dcmtk_flock(int fd, int operation) {
  ...
  if (operation & LOCK_SH)
    mode = _LK_LOCK; /* shared lock */
  if (operation & LOCK_EX)
    mode = _LK_LOCK; /* exclusive lock */
  if (operation & LOCK_UN)
    mode = _LK_UNLCK; /* unlock */
  if (operation & LOCK_NB)
    mode = _LK_NBLCK; /* non-blocking */
