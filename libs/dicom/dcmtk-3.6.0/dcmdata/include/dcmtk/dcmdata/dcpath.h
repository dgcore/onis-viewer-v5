...
public:
   ...
   *  @return none
   */
  static OFCondition separatePathNodes(const OFString& path,
...
public:
   ...
   *  @return EC_Normal if successful, error code otherwise.
   */
  OFCondition findOrCreatePath(DcmObject* obj, const OFString& path,
   ...
   */
  OFCondition findOrDeletePath(DcmObject* obj, const OFString& path,
                               Uint32& numDeleted);
   ...
   *  @param pathParam [in] the override key in path syntax (see class DcmPath).
   * Also the path can end with a value assignment, e. g.
   * "PatientName=Doe^John". An empty (or missing value) will not be ignored but
   * will be written as empty to the attribute (if not a sequence or item)
