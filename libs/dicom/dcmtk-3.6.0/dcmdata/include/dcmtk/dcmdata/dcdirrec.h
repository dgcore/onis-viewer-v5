...
public:
   ...
   *  @param sourceFileName path to referenced file in operating system specific
   * format
   */
  ...
  /** if this directory record references an MRDR (multi-reference directory
   * record), return pointer to the MRDR referenced by this object.
   *  @return pointer to MRDR referenced by this object or NULL of no MRDR
   * referenced
   */
   ...
   *  @param dirRec directory record to be inserted. Must be allocated on heap,
   * ownership is transferred to this object
   *  @param where index where to insert object
   *  @param before flag indicating whether to insert the record before or after
   * the element identified by where
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  The current position is stored internally in the 'lowerLevelList' member
   * variable.
   *  @param dirRec new child record to be inserted
   *  @param before flag indicating whether to insert the record before
   * (OFFalse) or after (OFTrue) the current position
  ...
  /** remove child directory record and delete file referenced by that record,
   * if any
   *  @param num index number of element, must be < cardSub()
   *  @return status, EC_Normal upon success, an error code otherwise
   */
  ...
  /// revert the list of child directory records to default constructed (empty)
  /// state
  ...
  /// get the filename from which this directory record was read from, NULL �f
  /// not set
...
private:
  /// string in which the filename (path) of the file from which this directory
  /// record was read is kept
  ...
  /// pointer to multi-referenced directory record (MRDR) if this record refers
  /// to one, NULL otherwise
  DcmDirectoryRecord* referencedMRDR;

  /// number of other directory records referring to this one; used for MRDR
  /// records
  Uint32 numberOfReferences;

  /// byte offset at which the start of this directory record resides in the
