...
public:
   ...
   *  @param oldFormat support old ACR/NEMA format if OFTrue (':' as a
   * separator)
   ...
   *  The DICOM TM format supported by this function is "HHMM[SS[.FFFFFF]]"
   * where the brackets enclose optional parts. If the current system time or
   * parts of it are unavailable the corresponding values are set to "0" and an
   * error code is returned.
   *  @param seconds add optional seconds ("SS") if OFTrue
   *  @param fraction add optional fractional part of a second (".FFFFFF") if
   * OFTrue (requires parameter 'seconds' to be also OFTrue)
   ...
   *  Please note that the element value is expected to be in valid DICOM TM
   * format
   *  ("[HH[MM[SS[.FFFFFF]]]]", "[HH[:MM[:SS[.FFFFFF]]]]" is also supported for
   * reasons of backward compatibility). Since there is no time zone for the
   * DICOM TM format local time is assumed (the time zone of 'timeValue' is set
   * automatically). If this function fails the result variable 'timeValue' is
   * cleared automatically.
   *  @param timeValue reference to OFTime variable where the result is stored
   *  @param pos index of the element component in case of value multiplicity
   * (0..vm-1)
   *  @param supportOldFormat if OFTrue support old (prior V3.0) time format
   * (see above)
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getOFTime(OFTime& timeValue, const unsigned long pos = 0,
   ...
   *  where the brackets enclose optional parts. Please note that the element
   * value is expected to be in valid DICOM TM format ("[HH[MM[SS[.FFFFFF]]]]",
   *  "[HH[:MM[:SS[.FFFFFF]]]]" is also supported for reasons of backward
   * compatibility). If this function fails the result variable 'formattedTime'
   * is cleared automatically. Please note that if the "Timezone Offset From
   * UTC" attribute (0008,0201) is present, it applies to all TM attributes in
   * the object. However, the time zone is not taken into account for the
   * creation of the ISO formatted time. See also "getTimeZoneFromString()"
   * below.
   ...
   *  @param fraction add optional fractional part of a second (".FFFFFF") if
   * OFTrue (requires parameter 'seconds' to be also OFTrue)
   *  @param createMissingPart if OFTrue create optional parts (seconds and/or
   * fractional part of a seconds) if absent in the element value
   *  @param supportOldFormat if OFTrue support old (prior V3.0) time format
   * (see above)
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   * automatically). If this function fails the result variable 'timeValue' is
   * cleared automatically.
   *  @param dicomTime string value in DICOM TM format to be converted to ISO
   * format
   *  @param timeValue reference to OFTime variable where the result is stored
   *  @param supportOldFormat if OFTrue support old (prior V3.0) time format
   * (see above)
  ...
  static OFCondition getOFTimeFromString(
      const OFString& dicomTime, OFTime& timeValue,
      const OFBool supportOldFormat = OFTrue);

  /** get the specified DICOM time value in ISO format.
   *  The ISO time format supported by this function is "HH:MM[:SS[.FFFFFF]]"
   *  where the brackets enclose optional parts. Please note that the specified
   * value is expected to be in valid DICOM TM format ("[HH[MM[SS[.FFFFFF]]]]",
   *  "[HH[:MM[:SS[.FFFFFF]]]]" is also supported for reasons of backward
   * compatibility). If this function fails the result variable 'formattedTime'
   ...
   * OFTrue (requires parameter 'seconds' to be also OFTrue)
   *  @param createMissingPart if OFTrue create optional parts (seconds and/or
   * fractional part of a seconds) if absent in the DICOM TM value
   *  @param supportOldFormat if OFTrue support old (prior V3.0) time format
   * (see above)
   *  @return EC_Normal upon success, an error code otherwise
   */
  static OFCondition getISOFormattedTimeFromString(
      const OFString& dicomTime, OFString& formattedTime,
      const OFBool seconds = OFTrue, const OFBool fraction = OFFalse,
      const OFBool createMissingPart = OFFalse,
      const OFBool supportOldFormat = OFTrue);

  /** get the specified DICOM time zone in number of hours format
   *  DICOM standard states that if the "Timezone Offset From UTC" attribute
   * (0008,0201) is present it applies to all TM attributes in the object. This
   * functions allows to convert the DICOM format ("&ZZZZ" where "&" is "+" or
   * "-" and "ZZZZ" hours and minutes) to a floating point value, e.g. "+1.0"
   * means plus one hour and "-2.5" minus two and a half hour, i.e. 2 hours and
   * 30 minutes. The resulting 'timeZone' value can be used in conjuction with a
   * OFTime object to convert the time to different time zones (e.g. to local
   * time or UTC).
   *  @param dicomTimeZone string value in DICOM format ("&ZZZZ") to be
   * converted
   *  @param timeZone reference to floating point variable where the resulting
   * UTC offset is stored
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   * checked for. (valid values: "1", "1-2", "1-3", "1-8", "1-99", "1-n", "2",
   * "2-n", "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32")
   *  @param oldFormat support old ACR/NEMA time format if OFTrue (i.e. with ":"
   * and "." delimiters)
   *  @return status of the check, EC_Normal if value is correct, an error code
   * otherwise
   */
