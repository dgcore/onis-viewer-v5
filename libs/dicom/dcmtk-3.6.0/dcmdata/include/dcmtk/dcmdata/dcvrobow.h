...
/** a class representing the DICOM value representations 'Other Byte String'
 * (OB) and 'Other Word String' (OW)
 */
...
public:
   ...
   *  @return OFTrue if transfer syntax can be changed to the new one, OFFalse
   * otherwise
   */
...
protected:
   ...
   * filename creation
   */
  void printPixel(STD_NAMESPACE ostream& out, const size_t flags,
                  const int level, const char* pixelFileName,
                  size_t* pixelCounter);

private:
  /** this flag is used during write operations and indicates that compact()
   * should be called once the write operation finishes.
   */
