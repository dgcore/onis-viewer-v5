...
public:
    ...
    if (!fail_)  // if fail_ is true, just ignore input
    {
      ...
      if (suspendInfo_ > 128) {
        ...
        if (compressedSize < nbytes) {
          ...
          suspendInfo_ =
              OFstatic_cast(unsigned char, nbytes - compressedSize - 1);
      ...
      while (compressedSize && (!fail_)) {
        ...
        if (ch & 0x80) {
          // replicate run
          if (compressedSize) {
            // DICOM packbit scheme uses 257 - nbytes to represent replicate
            // runs
  ...
  /** returns true if the RLE compressor has failed (out of memory or output
   * buffer too small).
   */
...
private:
   ...
   *  If suspended during a replicate run, contains control byte of repeat run
   * (> 128). If suspended during a literal run, contains number of remaining
   * bytes in literal run minus 1 (< 128).
   */
