 ...
 *  Default is enabled.
 */
extern OFGlobal<OFBool>
    dcmEnableAutomaticInputDataCorrection; /* default OFTrue */
 ...
 *  is enforced by adding a zero pad byte if
 * dcmEnableAutomaticInputDataCorrection is true. Otherwise the odd number of
 * bytes remains as read.
 *
 ...
 */
extern OFGlobal<OFBool>
    dcmAcceptUnexpectedImplicitEncoding; /* default OFFalse */
 ...
 */
extern OFGlobal<OFBool>
    dcmReadImplPrivAttribMaxLengthAsSQ; /* default OFFalse */
 ...
 */
extern OFGlobal<OFBool>
    dcmWriteOversizedSeqsAndItemsUndefined; /* default OFTrue */
 ...
 *  header length is ignored, the behavior is identical to the case when no
 * value is available (i.e. all elements are read as long as the group number is
 * 0x0002).
 */
extern OFGlobal<OFBool>
    dcmIgnoreFileMetaInformationGroupLength; /* default OFFalse */
...
public:
   ...
   */
  inline OFBool isaString() const {
    return Tag.getVR().isaString();
  }
   ...
   */
  inline E_TransferState transferState() const {
   ...
   *  @return const reference to the attribute tag for this object
   */
  inline const DcmTag& getTag() const {
    return Tag;
  }

  /** assign group tag (but not element tag) of the attribute tag for this
   * object. This is sometimes useful when creating repeating group elements.
   *  @param gtag new attribute group tag
   */
  inline void setGTag(Uint16 gtag) {
    Tag.setGroup(gtag);
  }
   ...
   *    while parsing and read later upon first access if the stream type
   * supports this.
   *  @return EC_Normal if successful, an error code otherwise
   */
   ...
   *  @param normalize normalize value before checking (ignore non-significant
   * characters)
   ...
   * otherwise
   */
  virtual OFCondition verify1(const OFBool autocorrect = OFFalse) = 0;
   ...
   */
  virtual OFCondition search(const DcmTagKey& xtag, DcmStack& resultStack,
  ...
  /** return the current value of the Length field (which is different from the
   * functionality of the public getLength method). Only needed for internal
   * purposes and for checker tools that verify values against the length field.
...
protected:
   ...
   *  @param level current level of nested items. Used for indentation.
   */
  void printNestingLevel(STD_NAMESPACE ostream& out, const size_t flags,
                         const int level);
   ...
   */
  void printInfoLineStart(STD_NAMESPACE ostream& out, const size_t flags,
                          const int level, DcmTag* tag = NULL);
   ...
   *  @param tag optional tag used to print the data element information
   */
  void printInfoLineEnd(
      STD_NAMESPACE ostream& out, const size_t flags,
   ...
   *  @param tag optional tag used to print the data element information
   *  @param isInfo optional flag indicating whether this text is really given
   * for informational purposes only. Used to choose the correct output color.
   */
  virtual void printInfoLine(STD_NAMESPACE ostream& out, const size_t flags,
                             const int level = 0, const char* info = NULL,
                             DcmTag* tag = NULL, const OFBool isInfo = OFTrue);
   ...
   *  @param writtenBytes number of bytes written to stream returned in this
   * parameter
   *  @return EC_Normal if successful, an error code otherwise
   */
   ...
   *  @return number of bytes, may be 8 or 12 depending on VR and transfer
   * syntax.
   */
  virtual Uint32 getTagAndLengthSize(const E_TransferSyntax oxfer) const;

  /** return the DICOM attribute tag name for this object. If not known yet,
   * will be looked up in the dictionary and cached. Therefore, method is not
   * const.
  ...
  /** return the current transfer state of this object during
  ...
  /** set the current transfer state of this object during
   * serialization/deserialization
   *  @param newState new transfer state of this object
   */
  void setTransferState(E_TransferState newState) {
    fTransferState = newState;
  }

  /** return the number of transferred bytes for this object during
   * serialization/deserialization
  ...
  /** set the number of transferred bytes for this object during
   * serialization/deserialization
   *  @param val number of transferred bytes
   */
  void setTransferredBytes(Uint32 val) {
    fTransferredBytes = val;
  }

  /** add to the number of transferred bytes for this object during
   * serialization/deserialization
 ...
 */
static inline STD_NAMESPACE ostream& operator<<(STD_NAMESPACE ostream& stream,
 ...
 *   stream, while it should not. The global variable
 * dcmEnableOldSignatureFormat and a corresponding command line option in
 * dcmsign allow to re-enable the old implementation.
 *
 ...
 * accepts odd length attributes or implements the old behaviour, i.e. assumes a
 * real length larger by one.
 *
