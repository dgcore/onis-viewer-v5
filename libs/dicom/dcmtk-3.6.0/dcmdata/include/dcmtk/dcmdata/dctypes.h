...
typedef enum {
  /// no change of group length values, WARNING: DO NOT USE THIS VALUE FOR WRITE
  /// OPERATIONS
  EGL_noChange = 0,
...
typedef enum {
  /// write as fileformat (update only missing information, this is the old
  /// behavior)
  EWM_fileformat = 0,
  /// write as dataset (without meta header)
  EWM_dataset = 1,
  /// write as fileformat and update required information (e.g. SOP
  /// Class/Instance UID)
  EWM_updateMeta = 2,
  /// write as fileformat and create new meta header (do not retain existing
  /// information)
  EWM_createNewMeta = 3,
...
public:
  ...
  /// do not map well-known UID numbers to UID names (e.g. Transfer Syntax and
  /// SOP Class)
