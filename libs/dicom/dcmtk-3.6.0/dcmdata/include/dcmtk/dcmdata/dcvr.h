...
/*
** VR Enumerations.
** NB: The order of entries has to conform to the order in DcmVRDict (see
*dcmvr.cc)!
**     If not an error message is reported and the program aborts (only in DEBUG
*mode).
*/
enum DcmEVR {
  ...
  /// used internally for elements with unknown VR (encoded with 4-byte length
  /// field in explicit VR)
  EVR_UNKNOWN,
  ...
  /// used internally for elements with unknown VR with 2-byte length field in
  /// explicit VR
  EVR_UNKNOWN2B
};
...
public:
  ...
  /** returns true if VR uses an extended length encoding for explicit transfer
   * syntaxes
   *  @return true if VR uses an extended length encoding for explicit transfer
   * syntaxes
   */
   ...
   *  and the other one is a possible standard VR to which the internal one
   * maps.
  ...
  */
  ...
  /** return minimum length of a value with this VR (in bytes), assuming single
   * byte characters
   *  @return minimum length of a value
   */
  Uint32 getMinValueLength() const;

  /** return maximum length of a value with this VR (in bytes), assuming single
   * byte characters
