...
public:
  ...
  friend STD_NAMESPACE ostream& operator<<(STD_NAMESPACE ostream& s,
                                           const DcmTagKey& k);
...
inline DcmTagKey::DcmTagKey() : group(0xffff), element(0xffff) {}

inline DcmTagKey::DcmTagKey(const DcmTagKey& key)
    : group(key.group), element(key.element) {}

inline DcmTagKey::DcmTagKey(Uint16 g, Uint16 e) : group(g), element(e) {}

inline DcmTagKey::~DcmTagKey() {}

/* access methods */

inline void DcmTagKey::set(const DcmTagKey& key) {
  group = key.group;
  element = key.element;
}

inline void DcmTagKey::set(Uint16 g, Uint16 e) {
  group = g;
  element = e;
}

inline void DcmTagKey::setGroup(Uint16 g) {
  group = g;
}

inline void DcmTagKey::setElement(Uint16 e) {
  element = e;
}

inline Uint16 DcmTagKey::getGroup() const {
  return group;
}

inline Uint16 DcmTagKey::getElement() const {
  return element;
}

inline OFBool DcmTagKey::isGroupLength() const {
  return (element == 0) && hasValidGroup();
}

inline OFBool DcmTagKey::isPrivate() const {
  return ((group & 1) != 0) && hasValidGroup();
}

inline OFBool DcmTagKey::isPrivateReservation() const {
  // private reservation has element number ranging from 0x0010 to 0x00FF
  return isPrivate() && (element >= 0x10) && (element <= 0xFF);
}

inline OFBool DcmTagKey::hasValidGroup() const {
...
inline DcmTagKey& DcmTagKey::operator=(const DcmTagKey& key) {
  set(key);
  return *this;
}
...
inline Uint32 DcmTagKey::hash() const {
...
inline int DcmTagKey::groupLT(const DcmTagKey& key) const {
  return (getGroup() < key.getGroup());
}

inline int DcmTagKey::groupGT(const DcmTagKey& key) const {
  return (getGroup() > key.getGroup());
}

inline int DcmTagKey::groupEQ(const DcmTagKey& key) const {
  return getGroup() == key.getGroup();
}

inline int DcmTagKey::elementLT(const DcmTagKey& key) const {
  return (getElement() < key.getElement());
}

inline int DcmTagKey::elementGT(const DcmTagKey& key) const {
  return (getElement() > key.getElement());
}

inline int DcmTagKey::elementEQ(const DcmTagKey& key) const {
  return getElement() == key.getElement();
}

inline int DcmTagKey::operator==(const DcmTagKey& key) const {
