...
public:
  ...
  /** remove all elements with an invalid group number from the meta information
   * header and dataset
   */
   ...
   *  @param pixelFileName optional filename used to write the raw pixel data
   * file
   *  @param pixelCounter optional counter used for automatic pixel data
   * filename creation
   */
  ...
  /** make sure that all data elements of the file meta information header are
   * existent in metainfo and contain correct values.
   *  @param oxfer the transfer syntax which shall be used
   *  @param writeMode flag indicating whether to update the file meta
   * information or not
   */
  virtual OFCondition validateMetaInfo(
   ...
   *  Please note that a new, initially empty dataset is also added to the
   * fileformat.
   ...
   *  The XML declaration (e.g. <?xml version="1.0"?>) is not written by this
   * function.
   ...
   *  This method supports DICOM objects stored as a file (with meta header) or
   * as a dataset (without meta header).  By default, the presence of a meta
   * header is detected automatically.
   *  @param fileName name of the file to load
   *  @param readXfer transfer syntax used to read the data (auto detection if
   * EXS_Unknown)
   *  @param groupLength flag, specifying how to handle the group length tags
   *  @param maxReadLength maximum number of bytes to be read for an element
   * value. Element values with a larger size are not loaded until their value
   * is retrieved (with getXXX()) or loadAllDataElements() is called.
   *  @param readMode read file with or without meta header, i.e. as a
   * fileformat or a dataset.  Use ERM_fileOnly in order to force the presence
   * of a meta header.
   *  @return status, EC_Normal if successful, an error code otherwise
   */
  virtual OFCondition loadFile(
      const char* fileName, const E_TransferSyntax readXfer = EXS_Unknown,
   ...
   *  @param writeXfer transfer syntax used to write the data (EXS_Unknown means
   * use current)
   ...
   *  @param padLength number of bytes used for the dataset padding (has to be
   * an even number)
   *  @param subPadLength number of bytes used for the item padding (has to be
   * an even number)
   *  @param writeMode write file with or without meta header. Also allows for
   * updating the information in the file meta information header.
   *  @return status, EC_Normal if successful, an error code otherwise
  ...
  virtual OFCondition saveFile(
      const char* fileName, const E_TransferSyntax writeXfer = EXS_Unknown,
      ...
      const Uint32 padLength = 0, const Uint32 subPadLength = 0,
      const E_FileWriteMode writeMode = EWM_fileformat);

  // methods for different pixel representations

  /** select a specific representation (compressed or uncompressed) of the
   * dataset and create the representation if needed. This may cause compression
   * or decompression to be applied to the pixel data in the dataset.
   *  @param repType desired transfer syntax
   *  @param repParam desired representation parameter (e.g. quality factor for
   * lossy compression)
  ...
  /** check if all PixelData elements in this dataset have a representation
   * conforming to the given transfer syntax and representation parameters (see
   * dcpixel.h for definition of "conforming").
   *  @param repType desired transfer syntax
   *  @param repParam desired representation parameter (e.g. quality factor for
   * lossy compression)
   *  @return true if all pixel elements have the desired representation, false
   * otherwise
   */
  ...
  /** get current file read mode.  This mode specifies whether a file is read as
   * a fileformat or dataset (without meta header).  In addition, the reading
   * can be restricted to DICOM files only.
  ...
  /** set current file read mode.  This mode specifies whether a file is read as
   * a fileformat or dataset (without meta header).  In addition, the reading
   * can be restricted to DICOM files only.
  ...
  /** method inherited from base class that shall not be used for instances of
   * this class. Method immediately returns with error code.
   *  @param item item
   *  @param where where
   *  @return always returns EC_IllegalCall.
   */
  virtual OFCondition insertItem(
      DcmItem* item, const unsigned long where = DCM_EndOfListIndex);

  /** method inherited from base class that shall not be used for instances of
   * this class. Method immediately returns.
   *  @param num num
...
private:
   ...
   * inserted.  Additionally, this function makes sure that the corresponding
   * data element will contain a correct value.
   ...
   *  @param obj       data object from metainfo which represents the data
   * element that shall be checked. Equals NULL if this data element is not
   * existent in the meta header information.
   *  @param oxfer     The transfer syntax which shall be used.
   *  @param writeMode flag indicating whether to update the file meta
   * information or not
   *  @return EC_Normal if successful, an error code otherwise
   ...
   *  @return E_TransferSyntax value for DCM_TransferSyntaxUID, EXS_Unknown if
