...
public:
   ...
   */
  virtual void print(STD_NAMESPACE ostream& out, const size_t flags = 0,
   ...
   * EC_SeqOrItemContentOverflow in case the item content (excluding tag header
   * etc.) is already too large.
   ...
   *  @param oldXfer transfer syntax in which the DICOM object was read or
   * created.
   *  @return true if object can be encoded in desired transfer syntax, false
   * otherwise.
   */
  ...
  /** returns true if the object contains an element with Unknown VR at any
   * nesting level
   *  @return true if the object contains an element with Unknown VR, false
   * otherwise
   */
  virtual OFBool containsUnknownVR() const;

  /** check if this object contains non-ASCII characters at any nesting level
   *  @param checkAllStrings if true, also check elements with string values not
   * affected by SpecificCharacterSet (0008,0005), default: only check PN, LO,
   * LT, SH, ST, UT
   *  @return true if object contains non-ASCII characters, false otherwise
   */
  virtual OFBool containsExtendedCharacters(
      const OFBool checkAllStrings = OFFalse);

  /** check if this object is affected by SpecificCharacterSet at any nesting
   * level. In detail, it is checked whether this object contains any data
   * elements that according to their VR are affected by the
   * SpecificCharacterSet (0008,0005) element. This is true for the following
   * VRs: PN, LO, LT, SH, ST and UT
   *  @return true if object is affected by SpecificCharacterSet, false
   * otherwise
   */
   ...
   *  @return status, EC_Normal if successful, an error code otherwise
   */
  virtual OFCondition insert(DcmElement* elem, OFBool replaceOld = OFFalse,
   ...
   *  returned to the caller who is responsible for further management of the
   *  DcmElement object.
   *  @param elem pointer to element (as type DcmObject *) to be removed from
   * list
   ...
   *  @return status, EC_Normal if value length is correct, an error code
   * otherwise
   */
  virtual OFCondition verify1(const OFBool autocorrect = OFFalse);
   ...
   *  hierarchically, from a starting position identified through a cursor
   * stack.
   *  @param xtag the DICOM attribute tag we are searching for
   *  @param resultStack Depending on the search mode (see below), this
   * parameter either serves as an input and output parameter, or as an output
   * parameter only (the latter being the default). When used as an input
   * parameter, the cursor stack defines the start position for the search
   * within a hierarchical DICOM dataset. Upon successful return, the stack
   * contains the position of the element found, in the form of a pointer to
   * each dataset, sequence, item and element from the main dataset down to the
   * found element.
   ...
   *  @param searchIntoSub if true, the search will be performed hierarchically
   * descending into the sequences and items of the dataset. If false, only the
   * current container (sequence or item) will be traversed.
   *  @return EC_Normal if found, EC_TagNotFound if not found, an error code is
   ...
   *  After a call to this method, the file from which a dataset was read may
   * safely be deleted or replaced. For large files, this method may obviously
  ...
  virtual OFCondition computeGroupLengthAndPadding(
      ...
      const Uint32 padlen = 0, const Uint32 subPadlen = 0,
   ...
   *  The result variable 'element' is automatically set to NULL if an error
   * occurs.
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param element variable in which the reference to (or copy of) the element
   * is stored
   *  @param searchIntoSub flag indicating whether to search into sequences or
   * not
   *  @param createCopy create a copy of the element if true, return a reference
   * otherwise
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetElement(const DcmTagKey& tagKey, DcmElement*& element,
                                const OFBool searchIntoSub = OFFalse,
                                const OFBool createCopy = OFFalse);

  /** find all elements matching a particular tag and return references to them
   * on a stack. This functions always performs a deep search (i.e. searches
   * into sequence of items).
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param resultStack stack where references to the elements are stored
   * (added to). If no element is found, the stack is not modified (e.g.
   * cleared).
   *  @return EC_Normal if at least one matching tag is found, an error code
   * otherwise.
   */
   ...
   *  Applicable to the following VRs: AE, AS, CS, DA, DS, DT, IS, LO, LT, PN,
   * SH, ST, TM, UI, UT Since the getString() routine is called internally the
   * resulting string reference represents the (possibly multi-valued) value as
   * stored in the dataset, i.e. no normalization is performed. The result
   * variable 'value' is automatically set to NULL if an error occurs.
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param value variable in which the reference to the element value is
   * stored (might be NULL)
   *  @param searchIntoSub flag indicating whether to search into sequences or
   * not
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetString(const DcmTagKey& tagKey, const char*& value,
                               const OFBool searchIntoSub = OFFalse);

  /** find element and get value as a C++ string (only one component).
   *  Applicable to the following VRs: AE, AS, AT, CS, DA, DS, DT, FL, FD, IS,
   * LO, LT, OB, OF, OW, PN, SH, SL, SS, ST, TM, UI, UL, US, UT Since the
   * getOFString() routine is called internally the resulting string is
   * normalized, i.e. leading and/or trailing spaces are removed according to
   * the associated value representation, or the element value is converted to a
   * character string (for non-string VRs) - see documentation in the
   * corresponding header file. In contrast to the above and below function only
   * the specified component (see parameter 'pos') is returned. The result
   * variable 'value' is automatically set to an empty string if an error
   *  occurs.
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   ...
   *  Applicable to the following VRs: AE, AS, AT, CS, DA, DS, DT, FL, FD, IS,
   * LO, LT, OB, OF, OW, PN, SH, SL, SS, ST, TM, UI, UL, US, UT Since the
   * getOFStringArray() routine is called internally the resulting string is
   * normalized, i.e. leading and/or trailing spaces are removed according to
   * the associated value representation or the element values are converted to
   * character strings (for non-string VRs) - see documentation in the
   * corresponding header file. The result variable 'value' is automatically set
   * to an empty string if an error occurs.
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param value variable in which the element value is stored
   *  @param searchIntoSub flag indicating whether to search into sequences or
   * not
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetOFStringArray(const DcmTagKey& tagKey, OFString& value,
                                      const OFBool searchIntoSub = OFFalse);

  /** find element and get value as an unsigned 8-bit integer.
   *  Applicable to the following VRs: OB
   *  The result variable 'value' is automatically set to zero if an error
   * occurs.
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param value variable in which the element value is stored
   *  @param pos index of the value in case of multi-valued elements (0..vm-1)
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetUint8(const DcmTagKey& tagKey, Uint8& value,
                              const unsigned long pos = 0,
                              const OFBool searchIntoSub = OFFalse);

  /** find element and get value as an array of unsigned 8-bit integers.
   *  Applicable to the following VRs: OB
   *  The result variable 'value' is automatically set to NULL if an error
   * occurs.
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param value variable in which the reference to the element value is
   * stored
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetUint8Array(const DcmTagKey& tagKey, const Uint8*& value,
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetUint16(const DcmTagKey& tagKey, Uint16& value,
                               const unsigned long pos = 0,
                               const OFBool searchIntoSub = OFFalse);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetSint16(const DcmTagKey& tagKey, Sint16& value,
                               const unsigned long pos = 0,
                               const OFBool searchIntoSub = OFFalse);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetUint32(const DcmTagKey& tagKey, Uint32& value,
                               const unsigned long pos = 0,
                               const OFBool searchIntoSub = OFFalse);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetSint32(const DcmTagKey& tagKey, Sint32& value,
                               const unsigned long pos = 0,
                               const OFBool searchIntoSub = OFFalse);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetLongInt(const DcmTagKey& tagKey, long int& value,
                                const unsigned long pos = 0,
                                const OFBool searchIntoSub = OFFalse);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetFloat32(const DcmTagKey& tagKey, Float32& value,
                                const unsigned long pos = 0,
                                const OFBool searchIntoSub = OFFalse);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition findAndGetFloat64(const DcmTagKey& tagKey, Float64& value,
                                const unsigned long pos = 0,
                                const OFBool searchIntoSub = OFFalse);
   ...
   *  The result variable 'sequence' is automatically set to NULL if an error
   * occurs (e.g. if 'seqTagKey' does not refer to a sequence attribute).
   *  @param seqTagKey DICOM tag specifying the sequence attribute to be
   * searched for
   *  @param sequence variable in which the reference to (or copy of) the
   * sequence is stored
   *  @param searchIntoSub flag indicating whether to search into sub-sequences
   * or not
   *  @param createCopy create a copy of the sequence if true, return a
   * reference otherwise
  ...
  /** looks up and returns a given sequence item, if it exists. Otherwise sets
   * 'item' to NULL and returns EC_TagNotFound (specified sequence does not
   * exist) or EC_IllegalParameter (specified item does not exist). Only the
   * top-most level of the dataset/item is examined (i.e. no deep-search is
   * performed). Applicable to the following VRs: SQ, (pixelSQ)
   *  @param seqTagKey DICOM tag specifying the sequence attribute to be
   * searched for
   *  @param item variable in which the reference to (or copy of) the item is
   * stored
   *  @param itemNum number of the item to be searched for (0..n-1, -1 for last)
   *  @param createCopy create a copy of the item if true, return a reference
   * otherwise
   *  @return EC_Normal upon success, an error otherwise.
   */
  OFCondition findAndGetSequenceItem(const DcmTagKey& seqTagKey, DcmItem*& item,
                                     const signed long itemNum = 0,
                                     const OFBool createCopy = OFFalse);

  /* --- findOrCreate functions: find an element or create a new one --- */

  /** looks up the given sequence in the current dataset and returns the given
   * item. If either the sequence or the item do not exist, they are created. If
   * necessary, multiple empty items are inserted. Only the top-most level of
   * the dataset/item is examined (i.e. no deep-search is performed). Applicable
   * to the following VRs: SQ, (pixelSQ)
   *  @param seqTag DICOM tag specifying the sequence attribute to be searched
   * for (or to be created)
   *  @param item variable in which the reference to the sequence item is stored
   *  @param itemNum number of the item to be searched for (0..n-1, -1 for last,
   *    -2 for append new)
   *  @return EC_Normal upon success, an error otherwise.
   */
  OFCondition findOrCreateSequenceItem(const DcmTag& seqTag, DcmItem*& item,
                                       const signed long itemNum = 0);

  /* --- findAndXXX functions: find an element and do something with it --- */

  /** find element, create a copy and insert it into the given destination
   * dataset. This functions never performs a deep search (i.e. does not search
   * into sequence of items). Empty elements are also copied. However, if the
   * given tag is not found in the current dataset, EC_TagNotFound is returned
   * and the destination dataset remains unchanged. Applicable to all DICOM
   * value representations (VR).
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param destItem destination dataset to which the copied element is
   * inserted
   *  @param replaceOld flag indicating whether to replace an existing element
   * or not
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  Applicable to all DICOM value representations (VR).
   *  @param tagKey DICOM tag specifying the attribute to be searched for
   *  @param allOccurrences flag indicating whether to delete all occurrences of
   * the attribute tag or the first one only (implies 'searchIntoSub' to be
   * true)
   *  @param searchIntoSub flag indicating whether to search into sequences or
   * not
   *  @return EC_Normal upon success, an error code otherwise.
  ...
  /** looks up the given sequence in the current dataset and deletes the given
   * item. Applicable to the following VRs: SQ, (pixelSQ)
   *  @param seqTagKey DICOM tag specifying the sequence attribute to be
   * searched for
  ...
  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: AE, AS, AT, CS, DA,
   * DS, DT, FL, FD, IS, LO, LT, OB, OF, OW, PN, SH, SL, SS, ST, TM, UI, UL, US,
   * UT
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertString(const DcmTag& tag, const char* value,
                                 const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: AE, AS, CS, DA, DS,
   * DT, IS, LO, LT, PN, SH, ST, TM, UI, UT
   *  @param tag DICOM tag specifying the attribute to be created
  ...
  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: OB, ox (polymorph
   * OB/OW or pixel data)
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element (might be NULL)
   *  @param count number of values (= bytes in this case) to be copied from
   * 'value'
   *  @param replaceOld flag indicating whether to replace an existing element
   * or not
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertUint8Array(const DcmTag& tag, const Uint8* value,
                                     const unsigned long count,
                                     const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: US, xs (US or SS)
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element
   *  @param pos index of the value to be set (0..vm). A value can be appended
   * to the end of or inserted within the existing value field.
   *  @param replaceOld flag indicating whether to replace an existing element
   * or not
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertUint16(const DcmTag& tag, const Uint16 value,
                                 const unsigned long pos = 0,
                                 const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: AT, OW, US, ox
   * (polymorph OB/OW or pixel data), xs (US or SS)
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element (might be NULL)
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertUint16Array(const DcmTag& tag, const Uint16* value,
                                      const unsigned long count,
                                      const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: SS, xs (US or SS)
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element
   *  @param pos index of the value to be set (0..vm). A value can be appended
   * to the end of or inserted within the existing value field.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertSint16(const DcmTag& tag, const Sint16 value,
                                 const unsigned long pos = 0,
                                 const OFBool replaceOld = OFTrue);
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertSint16Array(const DcmTag& tag, const Sint16* value,
                                      const unsigned long count,
                                      const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: UL
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element
   *  @param pos index of the value to be set (0..vm). A value can be appended
   * to the end of or inserted within the existing value field.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertUint32(const DcmTag& tag, const Uint32 value,
                                 const unsigned long pos = 0,
                                 const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: SL
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element
   *  @param pos index of the value to be set (0..vm). A value can be appended
   * to the end of or inserted within the existing value field.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertSint32(const DcmTag& tag, const Sint32 value,
                                 const unsigned long pos = 0,
                                 const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: FL, OF
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element
   *  @param pos index of the value to be set (0..vm). A value can be appended
   * to the end of or inserted within the existing value field.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertFloat32(const DcmTag& tag, const Float32 value,
                                  const unsigned long pos = 0,
                                  const OFBool replaceOld = OFTrue);

  /** create a new element, put specified value to it and insert the element
   * into the dataset/item. Applicable to the following VRs: FD
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param value value to be set for the new element
   *  @param pos index of the value to be set (0..vm). A value can be appended
   * to the end of or inserted within the existing value field.
   ...
   *  @return EC_Normal upon success, an error code otherwise.
   */
  OFCondition putAndInsertFloat64(const DcmTag& tag, const Float64 value,
                                  const unsigned long pos = 0,
                                  const OFBool replaceOld = OFTrue);
   ...
   *  Applicable to the following VRs: AE, AS, AT, CS, DA, DS, DT, FL, FD, IS,
   * LO, LT, OB, OF, OW, PN, SH, SL, SQ, SS, ST, TM, UI, UL, US, UT
   *  @param tag DICOM tag specifying the attribute to be created
   *  @param replaceOld flag indicating whether to replace an existing element
   * or not
   *  @return EC_Normal upon success, an error code otherwise.
  ...
  /** looks up the given sequence in the current dataset and inserts the given
   * item. If the sequence does not exist, it is created. If necessary, multiple
   * empty items are inserted before the specified item position. Only the
   * top-most level of the dataset/item is examined (i.e. no deep-search is
   * performed). Applicable to the following VRs: SQ, (pixelSQ)
   *  @param seqTag DICOM tag specifying the sequence attribute to be searched
   * for (or to be created)
   *  @param item item to be inserted into the sequence, must not be contained
   * in this or any other sequence. Will be deleted upon destruction of the
   * sequence object.
   *  @param itemNum position of the item (0..n-1, -1 = before last, -2 = after
   * last)
   *  @return EC_Normal upon success, an error otherwise (delete 'item'
   * manually!).
   */
  OFCondition insertSequenceItem(const DcmTag& seqTag, DcmItem* item,
...
protected:
   ...
   *  and b) Do the last 2 bytes which were read from the stream represent a
   * valid VR? In certain special cases, where the transfer syntax cannot be
   * determined without doubt, we want to guess the most likely transfer syntax
   * (see code).
  ...
  /** check whether the given tag requires some special handling regarding the
   * VR (i.e. in case it is undefined and multiple values are possible). If
   * required, the VR of the given element tag is then updated according to the
   * DICOM standard, e.g. the VR of PixelPaddingValue (if undefined) is set to
   * 'SS' or 'US' depending on the value of PixelRepresentation.
   *  @param item dataset or item that can be used to lookup other element
   * values
...
private:
  /** helper function for search(). May only be called if elementList is
   * non-empty. Performs hierarchical search for given tag and pushes pointer of
   * sub-element on result stack if found
   *  @param tag tag key to be searched
   *  @param resultStack upon successful return, pointer to element pushed onto
   * this stack
   *  @param searchIntoSub flag indicating whether recursive search is desired
   *  @return EC_Normal if tag found and stack modified, EC_TagNotFound if tag
   * not found and stack unmodified
   */
   ...
   *  @param atposition pointer to array of (at least) two bytes interpreted as
   * VR
...
/** helper function for DICOM parser. Creates new DICOM element from given
 * attribute tag
 ...
 *  @param privateCreatorCache cache object for private creator strings in the
 * current dataset
 *  @param readAsUN flag indicating whether parser is currently handling a UN
 * element that must be read in implicit VR little endian; updated upon return
 *  @return EC_Normal upon success, an error code otherwise
 */
 ...
 *  @param tag attribute tag of the element to be created
 *  @param length attribute value length of the element to be created
 *  @return pointer to newly created element returned in this parameter upon
 * success, NULL pointer otherwise
 */
DcmElement* newDicomElement(const DcmTag& tag, const Uint32 length = 0);

/** helper function for DcmElement::nextObject.
 *  hierarchically traverses all datasets/items after the position indicated by
 * the call stack
 *  @param st stack
 *  @return EC_Normal upon success, an error code otherwise
 */
