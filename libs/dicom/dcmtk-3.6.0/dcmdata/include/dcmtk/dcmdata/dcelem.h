...
public:
   ...
   *  @return true if value is present in memory, false if value still resides
   * in file
   */
  inline OFBool valueLoaded() const {
    return fValue != NULL || getLengthField() == 0;
  }
   ...
   *  (ERW_inWork) or if the writing of the bytes of this element's value has
   * not even begun yet (ERW_init). The member variable fTransferredBytes
   * indicates how many bytes (starting from byte 0) of this element's value
   * have already been written to the stream. This function will return
   * EC_Normal, if the entire value of this element has been written to the
   * stream, it will return EC_StreamNotifyClient, if there is no more space in
   * the buffer and _not_ all bytes of this element's value have been written,
   * and it will return some other (error) value if there was an error.
   *  @param outStream The stream the information will be written to.
   *  @param oxfer The transfer syntax which shall be used.
   *  @param enctype encoding types (undefined or explicit length) (actually
   * unused)
   ...
   * checked for. (valid values: "1", "1-2", "1-3", "1-8", "1-99", "1-n", "2",
   * "2-n", "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32"), interpreted
   * as cardinality (number of items) for sequence attributes
   *  @param oldFormat support old ACR/NEMA format for certain VRs (DA, TM, PN)
   * if OFTrue
  ...
  /** retrieve a single value of type Uint8. Requires element to be of
   * corresponding VR, otherwise an error is returned.
  ...
  /** retrieve a single value of type Sint16. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param val value returned in this parameter upon success
   *  @param pos position in multi-valued attribute, default 0
   *  @return EC_Normal upon success, an error code otherwise
  ...
  /** retrieve a single value of type Uint16. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param val value returned in this parameter upon success
   *  @param pos position in multi-valued attribute, default 0
   *  @return EC_Normal upon success, an error code otherwise
  ...
  /** retrieve a single value of type Sint32. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param val value returned in this parameter upon success
   *  @param pos position in multi-valued attribute, default 0
   *  @return EC_Normal upon success, an error code otherwise
  ...
  /** retrieve a single value of type Uint32. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param val value returned in this parameter upon success
   *  @param pos position in multi-valued attribute, default 0
   *  @return EC_Normal upon success, an error code otherwise
  ...
  /** retrieve a single value of type Float32. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param val value returned in this parameter upon success
   *  @param pos position in multi-valued attribute, default 0
   *  @return EC_Normal upon success, an error code otherwise
  ...
  /** retrieve a single value of type Float64. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param val value returned in this parameter upon success
   *  @param pos position in multi-valued attribute, default 0
   *  @return EC_Normal upon success, an error code otherwise
  ...
  /** retrieve a single value of type DcmTagKey. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param val value returned in this parameter upon success
   *  @param pos position in multi-valued attribute, default 0
   *  @return EC_Normal upon success, an error code otherwise
  ...
  /** retrieve a single value of type string. Requires element to be of
   * corresponding VR, otherwise an error is returned.
   *  @param str value returned in this parameter upon success
   *  @param pos position in multi-valued attribute
   *  @param normalize true if string is to be normalized (e.g. trailing space
   * characters to be trimmed)
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition getOFString(OFString& str, const unsigned long pos,
   ...
   *  Requires element to be of corresponding VR, otherwise an error is
   * returned. This method does not copy, but returns a pointer to the element
   * value, which remains under control of this object and is valid only until
   * the next read, write or put operation.
   ...
   * multi-valued). Requires element to be of corresponding VR, otherwise an
   * error is returned.
   ...
   *  a value can be appended to the end of the current element or inserted
   * within the existing value field.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putFloat32(const Float32 val,
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putTagVal(const DcmTagKey& attrTag,
                                const unsigned long pos = 0);

  /** replace the element value by a copy of the given Uint8 array (which is
   * possibly multi-valued). Requires element to be of corresponding VR,
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putSint16Array(const Sint16* vals,
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putUint16Array(const Uint16* vals,
                                     const unsigned long num);
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putSint32Array(const Sint32* vals,
                                     const unsigned long num);
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putUint32Array(const Uint32* vals,
                                     const unsigned long num);
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putFloat32Array(const Float32* vals,
                                      const unsigned long num);
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition putFloat64Array(const Float64* vals,
                                      const unsigned long num);
   ...
   *  The temporary file is not opened/loaded until the attribute value is
   * accessed, very much like large attributes that remain in file during a read
   * operation.
   ...
   *  multi-frame object. The frame is copied into the buffer passed by the
   * caller which must be large enough to contain a complete frame.
   ...
   *  @return EC_Normal if successful, an error code otherwise
   */
  virtual OFCondition getUncompressedFrame(DcmItem* dataset, Uint32 frameNo,
                                           Uint32& startFragment, void* buffer,
                                           Uint32 bufSize,
   ...
   */
  static int scanValue(const OFString& value, const OFString& vr,
                       const size_t pos = 0, const size_t num = OFString_npos);

  // START MODIFY DIGITALCORE
  uint64_t GetfLoadValueoffset();
...
protected:
  ...
  /** performs clean-up functions after loading an attribute value into main
   * memory. In particular, if dcmEnableAutomaticInputDataCorrection is enabled,
   * silently fixes odd length elements.
   */
   ...
   */
  virtual void writeXMLStartTag(STD_NAMESPACE ostream& out, const size_t flags,
   ...
   *    For empty values (vmNum=0), the status of the check is always EC_Normal
   * (i.e. no error).
   *  @param vmStr value multiplicity (according to the data dictionary) to be
   * checked for. (valid values: "1", "1-2", "1-3", "1-8", "1-99", "1-n", "2",
   * "2-n", "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32")
