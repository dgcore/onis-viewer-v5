...
public:
  /** default constructor
   *  @param doPad if true, RLE codec will pad output data to even number of
   * bytes
   */
  ...
  inline void add(unsigned char ch) {
    if (!fail_)  // if fail_ is true, just ignore input
    {
      ...
      if (OFstatic_cast(int, ch) == RLE_prev_)
        RLE_pcount_++;
      else {
          ...
          default:
            ...
            for (; RLE_pcount_ > 0; RLE_pcount_ -= 128) {
              ...
              if (RLE_pcount_ > 128)
                ...
                RLE_buff_[0] = OFstatic_cast(unsigned char, 257 - RLE_pcount_);
              move(2);
            }
  ...
  inline size_t size() const {
    ...
    if (pad_ && (result & 1))
      result++;  // enforce even number of bytes
    return result;
  }
  ...
  inline void write(void* target) const {
    if ((!fail_) && target) {
      ...
      if (pad_ &&
          ((blockList_.size() * DcmRLEEncoder_BLOCKSIZE + offset_) & 1)) {
        target8 += offset_;
        *target8 = 0;
      }
    }
...
private:
  ...
  /** this member contains a list of memory blocks of size
   * DcmRLEEncoder_BLOCKSIZE which already have been filled with encoded RLE
   * data. The current block (pointed to by currentBlock_) is not contained in
   * this list.
   */
   ...
   *  may temporarily become negative, guaranteed to be >= 0 between method
   * calls.
   */
  int RLE_pcount_;
