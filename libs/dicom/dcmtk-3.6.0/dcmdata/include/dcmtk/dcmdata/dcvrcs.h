...
public:
  ...
  /** check whether given value conforms to value representation CS (Code
   * String). Valid characters are: A-Z, 0-9, _ and ' ' (space).  The maximum
   * length is 16. NB: This method is only used by the DicomDirInterface class
   * and might be replaced by the following method (checkStringValue) in the
   * future.
   ...
   *  @param checkLength check maximum length if OFTrue, ignore length if
   * OFFalse
   *  @return OFTrue if value is valid, OFFalse otherwise
   */
  static OFBool checkVR(const OFString& value, size_t* pos = NULL,
