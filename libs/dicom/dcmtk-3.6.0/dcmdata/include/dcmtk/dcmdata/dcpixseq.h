...
/** this class implements a sequence of pixel items, i.e. the data structure
 * DICOM is using to store compressed pixel data. The object behaves very much
 * like a sequence, but uses the pixel data tag (7FE0,0010) and OB value
 * representation with undefined length, and the "items" contained within the
 * sequence are in fact pixel items (class DcmPixelItem) that contain no list of
 * DICOM elements but raw compressed pixel data.
 */
...
public:
  ...
  /** insert the given pixel item at the given position within the list
   * maintained by this object. Ownership of the pixel item, which must be
   * allocated on the heap, is transferred to the pixel sequence.
   *  @param item pointer to DcmPixelItem instance allocated on the heap, must
   * not be NULL.
  ...
  /** access a pixel item from the pixel sequence. This method returns a pointer
   * to one of the pixel items in the list, and not a copy.
   *  @param item upon success, a pointer to the selected pixel item is returned
   * in this parameter
   ...
   *  DcmPixelItem object.
   *  @param item upon success, a pointer to the removed pixel item is returned
   * in this parameter
   ...
   *  the caller is responsible for further management of the DcmPixelItem
   * object.
   ...
   *  This only works if no transfer syntax was defined so far, or if the new
   * and the old one are identical.
   *  @param newXfer
   ...
   *    Upon success, an entry is appended to the list. The offset values are
   * always even, so it is expected that odd length pixel items are padded later
   * during writing.
   ...
   *  @param fragmentSize maximum fragment size (in kbytes) for compression, 0
   * for unlimited.
   *  @return EC_Normal if successful, an error code otherwise
   */
...
protected:
   ...
   *  @param newObject upon success, a pointer to the newly created object is
...
private:
  /** the transfer syntax in which the compressed pixel data maintained by this
   * object is encoded. This may very well differ from the transfer syntax of
   * the main dataset if this object was created by a compression codec in
   * memory.
   */
