...
/** this class implements support for DICOMDIR files, which are special DICOM
 * files containing a list of directory records, with a logical tree structure
 * being implemented through references between records as byte offsets in file.
 */
class DcmDicomDir {
public:
  /// default constructor
  ...
  /// returns container in which all MRDR (multi-reference directory records)
  /// are stored
  virtual DcmSequenceOfItems& getMRDRSequence();

  /** look up directory record for the given referenced file ID (not OS file
   * path)
   ...
   * otherwise
   */
  virtual OFCondition verify1(OFBool autocorrect = OFFalse);
...
private:
  ...
  /** flag indicating whether or not this DICOMDIR has been modified after being
   * read from file. If true, the destructor of this class will write the
   * modified DICOMDIR back to file
   */
  OFBool modified;

  /// flag indicating whether the DICOM file in DirFile needs to be re-created
  /// upon writing
  ...
  /// container in which all MRDR (multi-reference directory records) for this
  /// DICOMDIR are kept
