...
typedef enum {
  ...
  /// JPEG Spectral Selection, Non-Hierarchical (lossy, 8/12 bit), arithmetic
  /// coding
  ...
  /// JPEG Full Progression, Non-Hierarchical (lossy, 8/12 bit), arithmetic
  /// coding
  ...
  /// JPEG Extended Sequential, Hierarchical (lossy, 8/12 bit), arithmetic
  /// coding
...
/** a class that allows for a lookup of Transfer Syntax properties and readable
 * descriptions
 */
class DcmXfer {
public:
  ...
  /// return true if transfer syntax is native (non-encapsulated), false
  /// otherwise
   ...
   *  Lossy JPEG transfer syntaxes support two alternative JPEG encoding
   * processes - 8 and 12 bits. When called for a non-JPEG transfer syntax,
   * returns 0.
 ...
 *  because of "fat" binaries that can be executed on multiple CPU types (e.g.
 * NeXTStep)
 */
