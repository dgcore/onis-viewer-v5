...
public:
  /** constructor.
   *  @param pCreateSOPInstanceUID true if a new SOP instance UID should be
   * assigned upon compression/decompression
   ...
   *  @param pReverseDecompressionByteOrder flag indicating whether the byte
   * order should be reversed upon decompression. Needed to correctly decode
   * some incorrectly encoded images with more than one byte per sample.
   */
  DcmRLECodecParameter(OFBool pCreateSOPInstanceUID = OFFalse,
