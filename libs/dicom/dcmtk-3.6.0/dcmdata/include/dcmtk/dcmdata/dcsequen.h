 ...
 *  This class is derived from class DcmElement (and not from DcmObject) despite
 * the fact that sequences have no value field as such, they maintain a list of
 * items. However, all APIs in class DcmItem and class DcmDataset accept
 * DcmElements. This is ugly and causes some DcmElement API methods to be
 * useless with DcmSequence.
 */
...
public:
  ...
  inline OFCondition error() const {
    return errorFlag;
  }
   ...
   *    (valid values: "1", "1-2", "1-3", "1-8", "1-99", "1-n", "2", "2-n",
   * "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32"), parameter used to
   * specify the value multiplicity for non-sequence attributes
   *  @param oldFormat parameter not used for this VR (only for DA, TM, PN)
   *  @return status of the check, EC_Normal if value is correct, an error code
   * otherwise
   */
   ...
   * EC_SeqOrItemContentOverflow in case the sequence content (excluding tag
   * header etc.) is already too large.
  ...
  /** insert the given item at the start of the item list maintained by this
   * sequence. Ownership of the item, which must be allocated on the heap, is
   * transferred to the sequence.
   *  @param item pointer to DcmItem instance allocated on the heap, must not be
   * NULL.
   *  @return EC_Normal if successful, an error code otherwise
   */
  virtual OFCondition prepend(DcmItem* item);

  /** insert the given item at the given position within the item list
   * maintained by this sequence. Ownership of the item, which must be allocated
   * on the heap, is transferred to the sequence.
   *  @param item pointer to DcmItem instance allocated on the heap, must not be
   * NULL.
   *  @param where index of the item after or before which the new item is to be
   * inserted. Value must be < card() or equal to DCM_EndOfListIndex.
   *  @param before indicates whether the new item should be inserted before or
   * after the item identified by "where"
   *  @return EC_Normal if successful, an error code otherwise
   */
  ...
  /** insert the given item at the end of the item list maintained by this
   * sequence. Ownership of the item, which must be allocated on the heap, is
   * transferred to the sequence.
   *  @param item pointer to DcmItem instance allocated on the heap, must not be
   * NULL.
   ...
   *  The current position is stored internally in the 'itemList' member
   * variable.
   *  @param item new item to be inserted
   *  @param before flag indicating whether to insert the item before (OFFalse)
   * or after (OFTrue) the current position
  ...
  /** Copy numBytes bytes of data from the attribute value in byteOrder byte
   * order to targetBuffer, starting at byte offset offset of the attribute
   * value. This method does not cause the complete attribute value to be read
   * into main memory. Subsequent calls for the same partial value may cause
   ...
   *  @param cache file cache object that may be passed to multiple subsequent
   * calls to this method for the same file; the file cache will then keep a
   * file handle open, thus improving performance. Optional, may be NULL
   *  @param byteOrder byte order desired byte order of attribute value in
   * memory buffer. Default is the local byte order of the machine.
   *  @return EC_Normal upon success, an error code otherwise
   */
  virtual OFCondition getPartialValue(void* targetBuffer, const Uint32 offset,
                                      Uint32 numBytes,
...
protected:
  ...
  /** helper function for read(). Create sub-object (item, item delimiter or
   * sequence delimiter) of the appropriate type depending on the tag.
   *  @param subObject upon success, a pointer to the newly created object is
   * returned in this parameter
   *  @param newTag tag of the sub-object to be created
   *  @param newLength length of the sub-object to be created
   *  @return status, EC_Normal if successful, an error code otherwise
   */
  virtual OFCondition makeSubObject(DcmObject*& subObject, const DcmTag& newTag,
   ...
   *  @return status, EC_Normal if successful, an error code otherwise
   */
  OFCondition readSubItem(
      DcmInputStream& inStream,                         // inout
  ...
  /** helper function for search(). May only be called if item list is
