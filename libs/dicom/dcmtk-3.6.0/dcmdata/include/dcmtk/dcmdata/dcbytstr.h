...
public:
   ...
   *  @param enctype flag, specifying the encoding with undefined or explicit
   * length
  ...
  /** write data element to a stream as required for the creation of digital
   * signatures
   *  @param outStream output stream
   *  @param oxfer transfer syntax used to write the data
   *  @param enctype flag, specifying the encoding with undefined or explicit
   ...
   *  @param normalize not used since string normalization depends on value
   * representation
   *  @return status, EC_Normal if successful, an error code otherwise
   */
   ...
   *  Checks every string component for the maximum length specified for the
   * particular value representation.
...
protected:
  ...
  /** set non-significant characters used to determine whether the value is
   * empty
   *  @param characters non-significant characters used to determine whether the
   * value is empty
   */
  void setNonSignificantChars(const OFString& characters) {
    nonSignificantChars = characters;
  }
   ...
   * "2-n", "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32" or "" for no
   * check)
   *  @param vr two-character identifier of the VR to be checked (lower case)
   *  @param vrID expected numeric identifier of the VR
   *  @param maxLen maximum number of characters allowed for a single value (0 =
   * no check)
   *  @return status of the check, EC_Normal if value is correct, an error code
   * otherwise
   */
  static OFCondition checkStringValue(const OFString& value, const OFString& vm,
                                      const OFString& vr, const int vrID,
 ...
 *  These flags can be used with normalizeString() to specify the extent of
 * normalization.
 */
//@{
 ...
 *  @return status, EC_Normal if successful, an error code otherwise
 */
OFCondition getStringPart(OFString& result, const char* orgStr,
 ...
 */
void normalizeString(OFString& string, const OFBool multiPart,
                     const OFBool leading, const OFBool trailing);
