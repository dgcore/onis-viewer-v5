...
public:
   ...
   */
  virtual OFBool scaleData(const Uint8* srcData, const unsigned int srcWidth,
                           const unsigned int srcHeight, Uint8* dstData,
                           const unsigned int dstWidth,
                           const unsigned int dstHeight) const = 0;

  /** scale image (from DICOM dataset) to specified size
   *  @param dataset DICOM dataset where the pixel data is stored (original
   * image)
   ...
   *  @return OFTrue if successful, OFFalse otherwise
   */
  virtual OFBool scaleImage(DcmItem* dataset, Uint8* pixel,
                            const unsigned long count,
                            const unsigned long frame, const unsigned int width,
...
public:
  ...
  bool mb_CheckMandatoryAttribute;
  void disableCheckForMandatoryAttribute(bool pb_disable);
   ...
   *  This function replaces any previously existing DICOMDIR file with the
   * specified 'filename'.  If the backup mode (see 'enableBackupMode') is
   * enabled a backup copy ('filename' + ".BAK") is created from the existing
   * file and automatically deleted after the new file has been written without
   * any errors.
   *  @param profile media storage application profile to be used for the
   * DICOMDIR
   *  @param filename name of the DICOMDIR file to be created (default:
   * 'DICOMDIR'). The filename may include a fully qualified pathname.
   *  @param filesetID value of the attribute FileSetID (default:
   * 'DCMTK_MEDIA_DEMO')
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition createNewDicomDir(
      const E_ApplicationProfile profile = AP_GeneralPurpose,
   ...
   *  This function can be used to append new entries to an existing DICOMDIR
   * file. If the backup mode (see 'enableBackupMode') is enabled a backup copy
   * ('filename'
   *  + ".BAK") is created from the existing file and automatically deleted
   * after the new file has been written without any errors.
   *  @param profile media storage application profile to be used for the
   * DICOMDIR. NB: The same profile should be used as for the creation of the
   * DICOMDIR file.
   *  @param filename name of the DICOMDIR file to be appended.  The filename
   * may include a fully qualified pathname.
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  This function can be used to append new entries to and update existing
   * entries in an existing DICOMDIR file. If the backup mode (see
   * 'enableBackupMode') is enabled a backup copy ('filename'
   *  + ".BAK") is created from the existing file and automatically deleted
   * after the new file has been written without any errors.
   *  @param profile media storage application profile to be used for the
   * DICOMDIR
   ...
   *  NB: The filename has already been specified for the object creation (see
   * above).
   *  @param encodingType flag, specifying the encoding with undefined or
   * explicit length
   *  @param groupLength flag, specifying how to handle the group length tags
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition writeDicomDir(
      const E_EncodingType encodingType = EET_UndefinedLength,
      const E_GrpLenEncoding groupLength = EGL_withoutGL);

  /** check whether specified filename is valid. i.e. conforms to the DICOM
   * standard requirements (length, number of components and proper characters).
   * This function is called automatically for the following methods:
   * checkDicomFile(), addDicomFile() and setFilesetDescriptor().  So usually
   * there's no need to call it manually (especially not in addition to the
   ...
   *  Valid character sets are (see DICOM PS3.3 for details): ISO_IR 100, ISO_IR
   * 101, ISO_IR 109, ISO_IR 110, ISO_IR 144, ISO_IR 127, ISO_IR 126, ISO_IR
   * 138, ISO_IR 148, ISO_IR 166, ISO_IR 13, ISO_IR 192.
  ...
  /** check whether specified DICOM file is suitable to be included into the
   * DICOMDIR. This method loads the given file and checks whether it conforms
   * to the current application profile.  Since this check is also performed by
   * addDicomFile() there is usually no need to call this method directly.
   *  @param filename name of the DICOM file to be checked
   *  @param directory directory where the DICOM file is stored (optional).
   *    This parameter might be useful in cases where the DICOM file is not
   * (yet) stored in the final directory (i.e. "relative" to the DICOMDIR
   * location).
   *  @return EC_Normal upon success, an error code otherwise
   */
   ...
   *  This method loads the given file, checks whether it conforms to the
   * current application profile and finally adds it to the DICOMDIR (in case of
   * conformance).
  ...
  // BEGIN DIGITALCORE
  OFCondition addDicomFile1(const char* path, const char* target);
  // END DIGITALCORE

  /** set the fileset descriptor file ID and character set.
   *  Prior to any internal modification both 'filename' and 'charset' are
   * checked using the above checking routines.  Existence of 'filename' is not
   * checked. NB: Requires a DICOMDIR to exist (see createNewDicomDir and
   * appendToDicomDir).
   ...
   *    default: ISO Latin 1 ("ISO_IR 100"), use NULL or empty string to omit
   * value.
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition setFilesetDescriptor(
      const char* filename, const char* charset = DEFAULT_DESCRIPTOR_CHARSET);
   ...
   *  externally stored PGM (portable gray map, 8 bit binary) file instead of
   * the DICOM image file.  The PGM filename is: 'prefix' + 'dicom_filename'.
   * The image does not need to have the correct size as it is scaled
   * automatically.
   ...
   *  DICOM file, respectively) a default icon (8 bit binary PGM) can be
   * specified. If this image also fails to load a black icon (filled with
   * zeros) is used.
   ...
   *  If this mode is disabled no backup file of an existing DICOMDIR is
   * created. However, when appending new files to an existing DICOMDIR a
   * _temporary_ backup file "<dicomdir>.$$$" is always created. Default: on,
   * create a backup file "<dicomdir>.BAK"
   ...
   *  If this mode is disabled the spatial resolution is not check for
   * compliance with the selected application profile.  Please use this switch
   * with care since the resulting DICOMDIR will probably violate the rules for
   * the selected application profile. Default: on, check spatial resolution
   *  @param newMode disable check if OFFalse, enable if OFTrue
   *  @return previously stored value
   */
   ...
   *  already existing ones is not checked (see
   * 'warnAboutInconsistentAttributes' for details). Default: on, perform
   * consistency check
   *  @param newMode disable check if OFFalse, enable if OFTrue
   *  @return previously stored value
   */
...
protected:
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition loadAndCheckDicomFile(const char* filename, const char* directory,
                                    DcmFileFormat& fileformat);

  // BEGIN DIGITALCORE
  OFCondition loadAndCheckDicomFile1(const char* path, const char* target,
                                     DcmFileFormat& fileformat);
  // END DIGITALCORE
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition checkSOPClassAndXfer(DcmMetaInfo* metainfo, DcmItem* dataset,
                                   const char* filename);
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition checkCTandMRAttributes(DcmItem* dataset, const OFString& sopClass,
                                     const char* filename);
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition checkMandatoryAttributes(DcmMetaInfo* metainfo, DcmItem* dataset,
                                       const char* filename);
   ...
   *  @return pointer to new or updated record, NULL if an error occurred
   */
  DcmDirectoryRecord* buildRTStructureSetRecord(
  ...
  /** create or update key object doc record and copy required values from
   * dataset
   *  @param record record to be updated, use NULL to create a new one
   *  @param dataset DICOM dataset of the current file
   *  @param referencedFileID value of the Referenced File ID attribute
   ...
   *  @return pointer to new or updated record, NULL if an error occurred
   */
  DcmDirectoryRecord* buildHangingProtocolRecord(
      DcmDirectoryRecord* record, DcmItem* dataset,
      const OFString& referencedFileID, const OFString& sourceFilename);
  ...
  /** create or update implant group record and copy required values from
   * dataset
   *  @param record record to be updated, use NULL to create a new one
   *  @param dataset DICOM dataset of the current file
   *  @param referencedFileID value of the Referenced File ID attribute
   ...
   *  @param pixel pointer to memory buffer where the pixel data are to be
   * stored
   ...
   *  @return OFTrue if successful, OFFalse otherwise
   */
  OFBool getIconFromFile(const OFString& filename, Uint8* pixel,
                         const unsigned long count, const unsigned int width,
                         const unsigned int height);
   ...
   *  @return OFTrue if successful, OFFalse otherwise
   */
  OFBool getIconFromDataset(DcmItem* dataset, Uint8* pixel,
                            const unsigned long count, const unsigned int width,
                            const unsigned int height);
   ...
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition addIconImage(DcmDirectoryRecord* record, DcmItem* dataset,
  ...
  DcmDirectoryRecord* addRecord(DcmDirectoryRecord* parent,
                                const E_DirRecType recordType, DcmItem* dataset,
                                const OFString& referencedFileID,
                                const OFString& sourceFilename);
  ...
  /** invent missing type 1 attributes for all child records (from patient
   * level)
   *  @param parent invent missing attributes for all children of this record
   * (root)
   *  @param recurse invent missing attributes also for lower level records if
   * OFTrue. Only invent attributes for the top (patient) level if OFFalse.
   */
  ...
  /** invent missing type 1 attributes for all child records (from instance
   * level)
   *  @param parent invent missing attributes for all children of this record
   */
   ...
   *  The output format is: "{Error|Warning}: attribute \<key.tagName()\>
   * \<key\> has other value than expected[ in file: \<filename\>]"
   ...
   *  The output format is: "Error: <error.text()>: required attribute
   * \<key.tagName()\>
   ...
   *  The output format is: "Error: \<error.text()\>: [cannot \<operation\>
   * ]\<key.tagName()\>
   *  \<key\>"
   *  @param key affected tag
   *  @param error status to be reported (only if ".bad()")
   *  @param operation name of the operation that failed (optional, might be
   * NULL)
   */
  ...
  /** see if all the attributes in record match the values in dataset and warn
   * if not
   *  @param record directory record to be checked
   *  @param dataset DICOM dataset to be compared with the directory record
   *  @param sourceFilename name of the source DICOM file
   *  @param abortCheck flag indicating whether to abort on the first
   * inconsistent record
   ...
   */
  OFBool checkExists(DcmItem* dataset, const DcmTagKey& key,
                     const char* filename = NULL);
   ...
   */
  OFBool checkExistsWithValue(DcmItem* dataset, const DcmTagKey& key,
                              const char* filename = NULL);

  /** check whether given tag exists in the DICOM dataset and has the expected
   * string value
   *  @param dataset DICOM dataset to be checked
   *  @param key tag to be searched for
   ...
   */
  OFBool checkExistsWithStringValue(DcmItem* dataset, const DcmTagKey& key,
   ...
   *  @return OFTrue if tag exists with given string value, OFFalse otherwise
   */
  OFBool checkExistsWithIntegerValue(DcmItem* dataset, const DcmTagKey& key,
                                     const long value,
                                     const char* filename = NULL,
                                     const OFBool reject = OFTrue);

  /** check whether given tag exists in the DICOM dataset and has an integer
   ...
   *  @return OFTrue if tag exists with given string value, OFFalse otherwise
   */
  OFBool checkExistsWithMinMaxValue(DcmItem* dataset, const DcmTagKey& key,
   ...
   */
  OFString& getStringFromDataset(DcmItem* dataset, const DcmTagKey& key,
                                 OFString& result,
   ...
   *  @param key tag of the attribute to be retrieved
   *  @param result string variable in which the resulting value is stored
   *  @param pos index of the component in case of multi-valued elements
   * (0..vm-1)
   *  @param searchIntoSub flag indicating whether to do a deep search or not
   *  @return reference to the resulting string value (parameter 'result')
   */
   ...
   *  @param filename name of the file from which the string value is to be
   * retrieved
   *  @param key tag of the attribute to be retrieved
   *  @param result string variable in which the resulting value is stored
   *  @param searchIntoSub flag indicating whether to do a deep search or not
   *  @return reference to the resulting string value (parameter 'result')
   */
  OFString& getStringFromFile(const char* filename, const DcmTagKey& key,
                              OFString& result, OFBool searchIntoSub = OFFalse);
   ...
   *  @param sourceFilename name of the source DICOM file
   *  @param optional flag indicating whether the element is optional or
   * required
   *  @param copyEmpty flag indicating whether to copy an empty element (no
   * value)
   */
  void copyElement(DcmItem* dataset, const DcmTagKey& key,
                   DcmDirectoryRecord* record, const OFString& sourceFilename,
   ...
   *  @param sourceFilename name of the source DICOM file
   */
  void copyElementType1(DcmItem* dataset, const DcmTagKey& key,
                        DcmDirectoryRecord* record,
                        const OFString& sourceFilename) {
    copyElement(dataset, key, record, sourceFilename, OFFalse /*optional*/,
                OFFalse /*copyEmpty*/);
  }
   ...
   *  @param sourceFilename name of the source DICOM file
   */
  void copyElementType1C(DcmItem* dataset, const DcmTagKey& key,
                         DcmDirectoryRecord* record,
                         const OFString& sourceFilename) {
    copyElement(dataset, key, record, sourceFilename, OFTrue /*optional*/,
                OFFalse /*copyEmpty*/);
  }
   ...
   *  @param sourceFilename name of the source DICOM file
   */
  void copyElementType2(DcmItem* dataset, const DcmTagKey& key,
                        DcmDirectoryRecord* record,
                        const OFString& sourceFilename) {
    copyElement(dataset, key, record, sourceFilename, OFFalse /*optional*/,
                OFTrue /*copyEmpty*/);
   ...
   *  @param sourceFilename name of the source DICOM file
   */
  void copyElementType3(DcmItem* dataset, const DcmTagKey& key,
                        DcmDirectoryRecord* record,
                        const OFString& sourceFilename) {
    copyElement(dataset, key, record, sourceFilename, OFTrue /*optional*/,
                OFTrue /*copyEmpty*/);
   ...
   *  @param sourceFilename name of the source DICOM file
   *  @param defaultValue default string value used in case the element is
   * missing
   *  @param printWarning print warning message if element does not exist (with
   * a value) and no default value is given
   */
  void copyStringWithDefault(DcmItem* dataset, const DcmTagKey& key,
                             DcmDirectoryRecord* record,
                             const OFString& sourceFilename,
   ...
   */
  OFBool compareStringAttributes(DcmItem* dataset, const DcmTagKey& datKey,
                                 DcmDirectoryRecord* record,
                                 const DcmTagKey& recKey,
                                 const OFString& sourceFilename,
                                 const OFBool errorMsg = OFFalse);

  /** compare sequence attribute from dataset and record and report any
   * deviation
   ...
   */
  OFBool compareSequenceAttributes(DcmItem* dataset, DcmTagKey& key,
                                   DcmDirectoryRecord* record,
                                   const OFString& sourceFilename);
   ...
   */
  void setDefaultValue(DcmDirectoryRecord* record, const DcmTagKey& key,
                       const unsigned long number, const char* prefix = NULL);
