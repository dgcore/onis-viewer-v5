...
public:
  ...
  /// copy assignment operator
  DcmDictEntryListConstIterator& operator=(
      const DcmDictEntryListConstIterator& i) {
...
public:
  /// default constructor
  DcmHashDictIterator() : dict(NULL), hindex(0), iterating(OFFalse), iter() {
    init(NULL);
  }
...
private:
   ...
   *  @param e new element to add, will be deleted upon destruction of the hash
   * table
   ...
   */
  DcmDictEntry* removeInList(DcmDictEntryList& lst, const DcmTagKey& k,
   ...
   */
  DcmDictEntry* findInList(DcmDictEntryList& lst, const DcmTagKey& k,
  ...
  /// index of highest bucket for which the DcmDictEntryList has been
  /// initialized
  int highestBucket;
...
** Initial Version - Interface for hash table data structure for data
*dictionary.
**
*/
