...
public:
   ...
   */
  virtual OFCondition getOFString(OFString& stringVal, const unsigned long pos,
                                  OFBool normalize = OFTrue);

  /** get name components from the element value.
   *  The DICOM PN consists of up to three component groups separated by a "=".
   * The supported format is "[CG0][=CG1][=CG2]" where the brackets enclose
   * optional parts and CG0 is a single-byte character representation, CG1 an
   * ideographic representation, and CG2 a phonetic representation of the name.
   *  Each component group may consist of up to five components separated by a
   * "^". The format is
   * "[lastName[^firstName[^middleName[^namePrefix[^nameSuffix]]]]"; each
   * component might be empty. If this function fails the result variables are
   * cleared automatically. If the format is valid but does not comply with the
   * above described scheme ("=" and "^") the full person name is returned in
   * the 'lastName' variable.
   *  @param lastName reference to string variable where the "last name" is
   * stored
   *  @param firstName reference to string variable where the "first name" is
   * stored
   *  @param middleName reference to string variable where the "middle name" is
   * stored
   *  @param namePrefix reference to string variable where the "name prefix" is
   * stored
   *  @param nameSuffix reference to string variable where the "name suffix" is
   * stored
   *  @param pos index of the element component in case of value multiplicity
   * (0..vm-1)
   *  @param componentGroup index of the component group (0..2) to be used, see
   * above
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getNameComponents(OFString& lastName, OFString& firstName,
                                OFString& middleName, OFString& namePrefix,
                                OFString& nameSuffix,
                                const unsigned long pos = 0,
                                const unsigned int componentGroup = 0);

  /** get current element value as a formatted/readable name.
   *  The current element value is expected to be in DICOM PN format as
   * described above. The output format is "[namePrefix][ firstName][
   * middleName][ lastName][, nameSuffix]"; the delimiters (" " and ", ") are
   * only inserted if required. If this function fails the result variable
   * 'formattedName' is cleared automatically.
   *  @param formattedName reference to string variable where the result is
   * stored
   *  @param pos index of the element component in case of value multiplicity
   * (0..vm-1)
   *  @param componentGroup index of the component group (0..2) to be used, see
   ...
   * "[lastName[^firstName[^middleName[^namePrefix[^nameSuffix]]]]]", i.e. a
   * DICOM Person Name (PN). Component groups are not (yet) supported. If this
   * function fails the currently stored value is not modified.
   *  @param lastName reference to string variable where the "last name" is
   * stored
   *  @param firstName reference to string variable where the "first name" is
   * stored
   ...
   * above described scheme ("=" and "^") the full person name is returned in
   * the 'lastName' variable.
   *  @param dicomName string value in DICOM PN format to be split into
   * components
   *  @param lastName reference to string variable where the "last name" is
   * stored
   *  @param firstName reference to string variable where the "first name" is
  ...
  static OFCondition getNameComponentsFromString(
      const OFString& dicomName, OFString& lastName, OFString& firstName,
      OFString& middleName, OFString& namePrefix, OFString& nameSuffix,
      const unsigned int componentGroup = 0);

  /** get specified DICOM person name as a formatted/readable name.
   *  The specified 'dicomName' is expected to be in DICOM PN format as
   * described above. The output format is "[namePrefix][ firstName][
   * middleName][ lastName][, nameSuffix]"; the delimiters (" " and ", ") are
   * only inserted if required. If this function fails the result variable
   * 'formattedName' is cleared automatically.
   *  @param dicomName string value in DICOM PN format to be converted to
   * readable format
   *  @param formattedName reference to string variable where the result is
   * stored
   *  @param componentGroup index of the component group (0..2) to be used, see
  ...
  static OFCondition getFormattedNameFromString(
      const OFString& dicomName, OFString& formattedName,
      const unsigned int componentGroup = 0);

  /** get formatted/readable name from specified name components.
   *  The output format is "[namePrefix][ firstName][ middleName][ lastName][,
   * nameSuffix]"; the delimiters (" " and ", ") are only inserted if required.
   *  If this function fails the result variable 'formattedName' is cleared
   * automatically.
   *  @param lastName reference to string variable where the "last name" is
   * stored
   *  @param firstName reference to string variable where the "first name" is
   ...
   * "[lastName[^firstName[^middleName[^namePrefix[^nameSuffix]]]]]". Component
   * groups are not (yet) supported. If this function fails the result variable
   * 'dicomName' is cleared automatically.
   *  @param lastName reference to string variable where the "last name" is
   * stored
   *  @param firstName reference to string variable where the "first name" is
   * stored
   ...
   * checked for. (valid values: "1", "1-2", "1-3", "1-8", "1-99", "1-n", "2",
   * "2-n", "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32")
   *  @param oldFormat support old ACR/NEMA name format if OFTrue (i.e. without
   * "^" delimiters)
   *  @return status of the check, EC_Normal if value is correct, an error code
   * otherwise
   */
