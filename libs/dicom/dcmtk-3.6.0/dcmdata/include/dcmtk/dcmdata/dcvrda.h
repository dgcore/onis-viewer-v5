...
public:
   ...
   *  The DICOM DA format supported by this function is "YYYYMMDD". If the
   * current system date is unavailable the date is set to "19000101" and an
   ...
   *  Please note that the element value is expected to be in valid DICOM DA
   * format
   *  ("YYYYMMDD", "YYYY.MM.DD" is also supported for reasons of backward
   * compatibility). If this function fails the result variable 'dateValue' is
   ...
   * (0..vm-1)
   *  @param supportOldFormat if OFTrue support old (prior V3.0) date format
   * (see above)
   *  @return EC_Normal upon success, an error code otherwise
   */
  OFCondition getOFDate(OFDate& dateValue, const unsigned long pos = 0,
                        const OFBool supportOldFormat = OFTrue);

  /** get the current element value in ISO date format.
   *  The ISO date format supported by this function is "YYYY-MM-DD". Please
   * note that the element value is expected to be in valid DICOM DA format
   * ("YYYYMMDD", "YYYY.MM.DD" is also supported for reasons of backward
   * compatibility). If this function fails the result variable 'formattedDate'
   * is cleared automatically.
   *  @param formattedDate reference to string variable where the result is
   * stored
   *  @param pos index of the element component in case of value multiplicity
   * (0..vm-1)
   *  @param supportOldFormat if OFTrue support old (prior V3.0) date format
   ...
   *  The DICOM DA format supported by this function is "YYYYMMDD". If the
   * specified date is invalid the date is set to "19000101" and an error code
   * is returned.
   ...
   *  Please note that the specified value is expected to be in valid DICOM DA
   * format
   *  ("YYYYMMDD", "YYYY.MM.DD" is also supported for reasons of backward
   * compatibility). If this function fails the result variable 'dateValue' is
   * cleared automatically.
   *  @param dicomDate string value in DICOM DA format to be converted to ISO
   * format
   *  @param dateValue reference to OFDate variable where the result is stored
   *  @param supportOldFormat if OFTrue support old (prior V3.0) date format
   * (see above)
  ...
  static OFCondition getOFDateFromString(
      const OFString& dicomDate, OFDate& dateValue,
      const OFBool supportOldFormat = OFTrue);

  /** get the specified DICOM date value in ISO format.
   *  The ISO date format supported by this function is "YYYY-MM-DD". Please
   * note that the specified value is expected to be in valid DICOM DA format
   * ("YYYYMMDD", "YYYY.MM.DD" is also supported for reasons of backward
   * compatibility). If this function fails the result variable 'formattedDate'
   * is cleared automatically.
   *  @param dicomDate string value in DICOM DA format to be converted to ISO
   ...
   * checked for. (valid values: "1", "1-2", "1-3", "1-8", "1-99", "1-n", "2",
   * "2-n", "2-2n", "3", "3-n", "3-3n", "4", "6", "9", "16", "32")
   *  @param oldFormat support old ACR/NEMA date format if OFTrue (i.e. with "."
   * delimiters)
   *  @return status of the check, EC_Normal if value is correct, an error code
   * otherwise
   */
