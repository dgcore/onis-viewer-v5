 ...
 *  elements that reside in file into an output stream.
 * DcmElement::getPartialValue is used to fill the buffer maintained by this
 * class, and the buffer content is then copied to the output stream. The
 * intermediate buffer is necessary because both DcmElement::getPartialValue and
 * DcmOutputStream::write expect a buffer to write to and read from,
 * respectively.
 */
class DcmWriteCache {
public:
  /// default constructor. Construction is cheap (no allocation of memory
  /// block).
  DcmWriteCache()
      : fcache_(),
        buf_(NULL),
  ...
  /** initialize the buffer maintained by this class. Can safely be called
   * multiple times.
   ...
   *  @param bytesTransferred number of bytes of user data in DICOM element that
   * have already been transferred
   *  @param byteOrder byteOrder desired byte order in buffer
   */
  void init(void* owner, Uint32 fieldLength, Uint32 bytesTransferred,
   ...
   *  This method uses DcmElement::getPartialValue to fill the buffer from the
   * given DICOM element at the given offset (which is updated to reflect the
