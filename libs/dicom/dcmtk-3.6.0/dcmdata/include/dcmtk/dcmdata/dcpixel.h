 ...
 *  A codec parameter set subclass is implemented for each codec and passed to
 * the codec by the encode() and decode() routines. It is supposed to contain
 * data that may vary between compressed image representations using the same
 * transfer syntax, for example the quality factor or compression factor for
 * lossy compression.
 */
...
class DcmRepresentationEntry {
   ...
   *     will be deleted when this object destructs.
   */
  DcmRepresentationEntry(const E_TransferSyntax rt,
...
typedef OFListConstIterator(DcmRepresentationEntry*)
    DcmRepresentationListConstIterator;
...
private:
  ...
  OFCondition findConformingEncapsulatedRepresentation(
      const DcmXfer& repType, const DcmRepresentationParameter* repParam,
   ...
   *  operation!
   */
  OFCondition findRepresentationEntry(const DcmRepresentationEntry& findEntry,
                                      DcmRepresentationListIterator& result);
   ...
   */
  OFCondition decode(const DcmXfer& fromType,
                     const DcmRepresentationParameter* fromParam,
                     DcmPixelSequence* fromPixSeq, DcmStack& pixelStack);

  /** encode to encapsulated format
   */
  OFCondition encode(const DcmXfer& fromType,
                     const DcmRepresentationParameter* fromParam,
                     DcmPixelSequence* fromPixSeq, const DcmXfer& toType,
   ...
   */
  void recalcVR() {
    if (current == repList.end())
      setTagVR(unencapsulatedVR);
    else
      setTagVR(EVR_OB);
  }
...
public:
  ...
  /** set/change the current value representation of the uncompressed image
   * representation, if any
   *  @param vr new value representation to be set.  All VRs except for OW
   * (Other Word String) are treated as 8 bit data (OB).  This is particularily
   * useful for unknown (UN) or unsupported VRs.
   ...
   */
  OFBool canChooseRepresentation(const E_TransferSyntax repType,
   ...
   */
  void putOriginalRepresentation(const E_TransferSyntax repType,
                                 const DcmRepresentationParameter* repParam,
                                 DcmPixelSequence* pixSeq);
  ...
  OFCondition getEncapsulatedRepresentation(
      ...
      const DcmRepresentationParameter* repParam, DcmPixelSequence*& pixSeq);
   ...
   */
  void getCurrentRepresentationKey(E_TransferSyntax& repType,
                                   const DcmRepresentationParameter*& repParam);
   ...
   */
  OFCondition removeRepresentation(const E_TransferSyntax repType,
                                   const DcmRepresentationParameter* repParam);
   ...
   *  This flag should never be enabled for pixel data elements on the main
   * dataset level, only for pixel data elements within the icon image sequence
