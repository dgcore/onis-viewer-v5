...
public:
  ...
  /** remove all elements with an invalid group number, i.e. everything but
   * 0x0002
   */
...
private:
   ...
   *  @param headerLen output parameter; length of meta-header as encoded in
   * group length element
   *  @param bytesRead output parameter; number of bytes read when reading group
   * length (for counting the remaining number of meta-header bytes)
   *  @param maxReadLength max read length for elements
   *  @return EC_Normal if successful, an error code otherwise
   */
  OFCondition readGroupLength(
