...
public:
   ...
   *  If the string starts with a "=" the subsequent characters are interpreted
   * as a UID name and mapped to the corresponding UID number (using
   * "dcmFindUIDFromName()") if possible. Otherwise the leading "=" is removed.
  ...
  /** check whether given string value conforms to the VR "UI" (Unique
   * Identifier) and to the specified VM.
...
protected:
   ...
   *  It removes any leading, embedded and trailing space character and
   * recomputes the string length. This manipulation attempts to correct
   * problems with incorrectly encoded UIDs which have been observed in some
   * images.
   *  @return status, EC_Normal if successful, an error code otherwise
   */
