 ...
 *  The result of this function is allocated with new[] and must be delete[]d by
 * the caller. The original string pointed to by s is not modified.
 *  @param s points to start of string; updated to point to start of next value
 * in string
 *  @return first value in string, zero-terminated. char array allocated on
 * heap, must be delete[]d by caller
 */
