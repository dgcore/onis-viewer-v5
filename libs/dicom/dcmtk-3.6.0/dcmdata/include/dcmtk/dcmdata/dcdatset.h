...
public:
   ...
   *  @param padenc         Encoding type for padding. Specifies what will be
   ...
   * length used when computing the padding; this parameter is for instance used
   * to pass the length of the file meta header from the DcmFileFormat to the
   * DcmDataset object.
   *  @return status, EC_Normal if successful, an error code otherwise
   */
  virtual OFCondition write(DcmOutputStream& outStream,
   ...
   *  This method only supports DICOM objects stored as a dataset, i.e. without
   * meta header. Use DcmFileFormat::loadFile() to load files with meta header.
   ...
   *  This method only supports DICOM objects stored as a dataset, i.e. without
   * meta header. Use DcmFileFormat::saveFile() to save files with meta header.
