...
public:
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  virtual OFCondition decodeFrame(const DcmRepresentationParameter* fromParam,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  virtual OFCondition encode(const Uint16* pixelData, const Uint32 length,
  ...
  // static helper methods that have proven useful in codec classes derived from
  // DcmCodec

  /** helper function that inserts a string attribute with a given value into a
   * dataset if missing in the dataset.
   ...
   *  @return EC_Normal if successful, an error code otherwise
   */
  static OFCondition insertStringIfMissing(DcmItem* dataset,
                                           const DcmTagKey& tag,
                                           const char* val);
   ...
   *  by Secondary Capture. It does not, however, change an existing SOP
   * Instance UID.
   *  @param dataset dataset to insert to, must not be NULL.
   *  @return EC_Normal if successful, an error code otherwise
   */
   ...
   *  @param purposeOfReferenceCodingScheme coding scheme designator for purpose
   * of reference code sequence
   *  @param purposeOfReferenceCodeValue code value for purpose of reference
   * code sequence
   *  @param purposeOfReferenceCodeMeaning code meaning for purpose of reference
   * code sequence
   *  @return EC_Normal if successful, an error code otherwise
   */
  static OFCondition newInstance(
      DcmItem* dataset, const char* purposeOfReferenceCodingScheme = NULL,
   ...
   *  @param codingSchemeDesignator coding scheme designator for the sequence
   * item
   ...
   */
  static OFCondition insertCodeSequence(DcmItem* dataset,
                                        const DcmTagKey& tagKey,
                                        const char* codingSchemeDesignator,
                                        const char* codeValue,
                                        const char* codeMeaning);

  /** determine the index number (starting with zero) of the compressed pixel
   * data fragment corresponding to the given frame (also starting with zero)
   *  @param frameNo frame number
   *  @param numberOfFrames number of frames of this image
   *  @param fromPixSeq compressed pixel sequence
   *  @param currentItem index of compressed pixel data fragment returned in
   * this parameter on success
   *  @return EC_Normal if successful, an error code otherwise
   */
  static OFCondition determineStartFragment(Uint32 frameNo,
...
public:
   ...
   *    Must remain unmodified and valid until the codec has been deregistered
   * or the parameter has been replaced by a call to updateCodecParameter()
   *  @return EC_Normal if successful, an error code otherwise
   */
  ...
  /** updates the codec parameters object for a codec that has been registered
   * before. This call is safe in multi-thread operations.
   *  @param aCodec pointer to codec object that has been registered before
   *  @param aCodecParameter codec parameter.
   *    Must remain unmodified and valid until the codec has been deregistered
   * or the parameter has been replaced by another call to
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  static OFCondition decode(const DcmXfer& fromType,
  ...
  /** looks for a codec that is able to decode from the given transfer syntax
   *  and calls the decodeFrame() method of the codec.  A read lock on the list
   * of codecs is acquired until this method returns.
   *  @param fromType transfer syntax to decode from
   *  @param fromParam representation parameter of current compressed
   *    representation, may be NULL.
   *  @param fromPixSeq compressed pixel sequence
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  static OFCondition decodeFrame(const DcmXfer& fromType,
                                 const DcmRepresentationParameter* fromParam,
   ...
   *  @return EC_Normal if successful, an error code otherwise.
   */
  static OFCondition encode(const E_TransferSyntax fromRepType,
                            const Uint16* pixelData, const Uint32 length,
                            const E_TransferSyntax toRepType,
                            const DcmRepresentationParameter* toRepParam,
                            DcmPixelSequence*& pixSeq, DcmStack& pixelStack);
   ...
   *  codecs is acquired until this method returns.
   *  @param fromRepType current transfer syntax of the compressed image
   *  @param fromParam current representation parameter of compressed data, may
   * be NULL
   *  @param fromPixSeq compressed pixel sequence
   *  @param toRepType transfer syntax to compress to
   *  @param toRepParam representation parameter describing the desired
  ...
  static OFCondition encode(const E_TransferSyntax fromRepType,
                            ...
                            const E_TransferSyntax toRepType,
                            const DcmRepresentationParameter* toRepParam,
                            DcmPixelSequence*& toPixSeq, DcmStack& pixelStack);
   ...
   */
  static OFBool canChangeCoding(const E_TransferSyntax fromRepType,
                                const E_TransferSyntax toRepType);
...
private:
   ...
   *  @param aCodecParameter codec parameter.
   */
  DcmCodecList(const DcmCodec* aCodec,
               const DcmRepresentationParameter* aDefaultRepParam,
               const DcmCodecParameter* aCodecParameter);
