...
DcmDate::DcmDate(const DcmDate& old) : DcmByteString(old) {}
...
OFCondition DcmDate::checkValue(const OFString& vm, const OFBool oldFormat) {
  OFString strVal;
...
OFCondition DcmDate::getOFString(OFString& stringVal, const unsigned long pos,
                                 OFBool normalize) {
...
OFCondition DcmDate::getOFDate(OFDate& dateValue, const unsigned long pos,
...
OFCondition DcmDate::getOFDateFromString(const OFString& dicomDate,
  ...
  else if (supportOldFormat && (dicomDate.length() == 10) &&
           (dicomDate[4] == '.') && (dicomDate[7] == '.')) {
    unsigned int year, month, day;
    /* extract components from date string */
    if (sscanf(dicomDate.c_str(), "%04u.%02u.%02u", &year, &month, &day) == 3) {
      if (dateValue.setDate(year, month, day))
...
OFCondition DcmDate::getISOFormattedDateFromString(
    const OFString& dicomDate, OFString& formattedDate,
    const OFBool supportOldFormat) {
  OFDate dateValue;
  /* convert string to OFDate */
  OFCondition l_error =
      getOFDateFromString(dicomDate, dateValue, supportOldFormat);
  if (l_error.good()) {
...
OFCondition DcmDate::checkStringValue(const OFString& value, const OFString& vm,
