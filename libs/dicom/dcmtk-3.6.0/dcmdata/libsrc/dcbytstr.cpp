...
DcmByteString::DcmByteString(const DcmTag& tag, const Uint32 len)
    : DcmElement(tag, len),
      paddingChar(' '),
...
Uint32 DcmByteString::getRealLength() {
  ...
  if (fStringMode != DCM_MachineString) {
    /* strips non-significant trailing spaces (padding) and determines
     * 'realLength' */
...
void DcmByteString::print(STD_NAMESPACE ostream& out, const size_t flags,
  ...
  if (valueLoaded()) {
    ...
    if (stringVal) {
      ...
      if (flags & DCMTypes::PF_convertToMarkup)
        OFStandard::convertToMarkupString(stringVal, outString, OFTrue,
                                          OFStandard::MM_XML, OFFalse);
      else
        outString = stringVal;

      unsigned long printedLength =
          outString.length() + 2 /* for enclosing brackets */;
  ...
  if (value != NULL) {
    ...
    if (dcmEnableAutomaticInputDataCorrection.get()) {
      ...
      */
      if (realLength > 0) {
        size_t i = 0;
        for (i = OFstatic_cast(size_t, realLength);
             (i > 0) && (value[i - 1] == paddingChar); i--)
          value[i - 1] = '\0';
...
Uint8* DcmByteString::newValueField() {
  ...
  if (lengthField & 1) {
    if (lengthField == DCM_UndefinedLength) {
       ...
       */
      DCMDATA_WARN("DcmByteString: "
                   << getTagName() << " " << getTag().getXTag()
                   << " has odd, maximum length (" << DCM_UndefinedLength
                   << ") and therefore is not loaded");
      errorFlag = EC_CorruptedData;
      return NULL;
    }
    /* allocate space for extra padding character (required for the DICOM
...
OFCondition DcmByteString::verify1(const OFBool autocorrect) {
  char* value = NULL;
  /* get string data */
  errorFlag = getString(value);
...
void normalizeString(OFString& string, const OFBool multiPart,
  ...
  if (!string.empty()) {
    ...
    while (partindex < len) {
      ...
      if (leading) {
        offset = 0;
        while ((partindex + offset < len) &&
               (string[partindex + offset] == ' '))
          offset++;
        if (offset > 0)
