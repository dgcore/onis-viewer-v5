...
DcmPolymorphOBOW::DcmPolymorphOBOW(const DcmTag& tag, const Uint32 len)
    : DcmOtherByteOtherWord(tag, len), changeVR(OFFalse), currentVR(EVR_OW) {
  if (getTag().getEVR() == EVR_ox || getTag().getEVR() == EVR_lt)
    setTagVR(EVR_OW);
}
...
OFCondition DcmPolymorphOBOW::getUint8Array(Uint8*& bytes) {
  errorFlag = EC_Normal;
...
OFCondition DcmPolymorphOBOW::getUint16Array(Uint16*& words) {
  errorFlag = EC_Normal;
  OFBool bchangeVR = OFFalse;
  if (currentVR == EVR_OB) {
    setByteOrder(EBO_LittleEndian);
...
OFCondition DcmPolymorphOBOW::createUint8Array(const Uint32 numBytes,
                                               Uint8*& bytes) {
  currentVR = EVR_OB;
  setTagVR(EVR_OB);
...
OFCondition DcmPolymorphOBOW::createUint16Array(const Uint32 numWords,
                                                Uint16*& words) {
  currentVR = EVR_OW;
  setTagVR(EVR_OW);
...
OFCondition DcmPolymorphOBOW::putUint8Array(const Uint8* byteValue,
...
OFCondition DcmPolymorphOBOW::putUint16Array(const Uint16* wordValue,
  ...
  if (numWords) {
    if (wordValue) {
      ...
      if (errorFlag == EC_Normal && getTag().getEVR() == EVR_OB &&
          getByteOrder() == EBO_BigEndian) {
        swapValueField(sizeof(Uint16));
        setByteOrder(EBO_LittleEndian);
      }
    } else
...
OFCondition DcmPolymorphOBOW::read(DcmInputStream& inStream,
...
void DcmPolymorphOBOW::transferEnd() {
  changeVR = OFFalse;
  DcmOtherByteOtherWord::transferEnd();
}

void DcmPolymorphOBOW::transferInit() {
  changeVR = OFFalse;
  DcmOtherByteOtherWord::transferInit();
}

OFCondition DcmPolymorphOBOW::write(DcmOutputStream& outStream,
...
OFCondition DcmPolymorphOBOW::writeSignatureFormat(DcmOutputStream& outStream,
                                                   const E_TransferSyntax oxfer,
                                                   const E_EncodingType enctype,
                                                   DcmWriteCache* wcache) {
  DcmXfer oXferSyn(oxfer);
  ...
  errorFlag = DcmOtherByteOtherWord::writeSignatureFormat(outStream, oxfer,
                                                          enctype, wcache);
  if (getTransferState() == ERW_ready && changeVR) {
    // VR must be OB again. No Swapping is needed since the written
    // transfer syntax was LittleEndianImplicit and so no swapping
