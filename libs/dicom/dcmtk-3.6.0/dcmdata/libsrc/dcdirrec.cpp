...
static const short DIM_OF_DRTypeNames =
    (sizeof(DRTypeNames) / sizeof(DRTypeNames[0]));
...
DcmDirectoryRecord::DcmDirectoryRecord(const DcmTag& tag, const Uint32 len)
    : DcmItem(tag, len),
      recordsOriginFile(NULL),
      lowerLevelList(new DcmSequenceOfItems(DCM_DirectoryRecordSequence)),
      DirRecordType(ERT_Private),
...
DcmDirectoryRecord& DcmDirectoryRecord::operator=(
    const DcmDirectoryRecord& obj) {
  if (this != &obj) {
...
E_DirRecType DcmDirectoryRecord::recordNameToType(const char* recordTypeName) {
  ...
  if (recordTypeName != NULL) {
    ...
    DCMDATA_TRACE("DcmDirectoryRecord::recordNameToType() input char*=\""
                  << recordTypeName << "\" output enum=" << recType);
  }
  return recType;
}
...
char* DcmDirectoryRecord::buildFileName(const char* origName, char* destName) {
  ...
  if (!elementList->empty()) {
    ...
    if (search(DCM_DirectoryRecordType, stack, ESM_fromHere, OFFalse).good()) {
      if (stack.top()->ident() == EVR_CS) {
        ...
        recType->verify1(OFTrue);  // force dealignment
        recType->getString(recName);
        localType = recordNameToType(recName);

        DCMDATA_TRACE(
            "DcmDirectoryRecord::lookForRecordType() RecordType Element ("
...
OFCondition DcmDirectoryRecord::setReferencedFileID(
    const char* referencedFileID) {
  OFCondition l_error = EC_Normal;
...
const char* DcmDirectoryRecord::lookForReferencedFileID() {
  ...
  if (!elementList->empty()) {
    ...
    if (search(DCM_ReferencedFileID, stack, ESM_fromHere, OFFalse).good()) {
      if (stack.top()->ident() == EVR_CS) {
        ...
        refFile->verify1(OFTrue);  // force dealignment
  ...
  DCMDATA_TRACE(
      "DcmDirectoryRecord::lookForReferencedFileID() ReferencedFileID = "
      << ((localFile) ? localFile : ""));
...
DcmDirectoryRecord* DcmDirectoryRecord::lookForReferencedMRDR() {
  ...
  if (!elementList->empty()) {
    DcmStack stack;
    if (search(DCM_RETIRED_MRDRDirectoryRecordOffset, stack, ESM_fromHere,
               OFFalse)
            .good()) {
      if (stack.top()->ident() == EVR_up) {
        ...
        DCMDATA_TRACE(
            "DcmDirectoryRecord::lookForReferencedMRDR() MRDR Offset Element ("
            << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
  ...
  if (localMRDR == NULL)
    DCMDATA_TRACE(
        "DcmDirectoryRecord::lookForReferencedMRDR() no ReferencedMRDR found");
  ...
  if (!elementList->empty()) {
    ...
    if (search(DCM_RecordInUseFlag, stack, ESM_fromHere, OFFalse).good()) {
      if (stack.top()->ident() == EVR_US)
        errorFlag =
            OFstatic_cast(DcmUnsignedShort*, stack.top())->getUint16(localFlag);
    }
  }
  return localFlag;
}
...
OFCondition DcmDirectoryRecord::setNumberOfReferences(Uint32 newRefNum) {
  ...
  if (DirRecordType == ERT_Mrdr) {
    ...
    DCMDATA_ERROR(
        "illegal usage of DcmDirectoryRecord::setNumberOfReferences() - "
        "RecordType must be MRDR");
  }
  return l_error;
}
...
Uint32 DcmDirectoryRecord::lookForNumberOfReferences() {
  ...
  if (!elementList->empty()) {
    DcmStack stack;
    if (search(DCM_RETIRED_NumberOfReferences, stack, ESM_fromHere, OFFalse)
            .good()) {
      if (stack.top()->ident() == EVR_UL)
        errorFlag = OFstatic_cast(DcmUnsignedLong*, stack.top())
...
Uint32 DcmDirectoryRecord::increaseRefNum() {
  if (DirRecordType == ERT_Mrdr) {
    ...
    errorFlag = EC_IllegalCall;
    DCMDATA_ERROR(
        "illegal usage of DcmDirectoryRecord::increaseRefNum() - RecordType "
        "must be MRDR");
  }
...
Uint32 DcmDirectoryRecord::decreaseRefNum() {
  if (DirRecordType == ERT_Mrdr) {
    if (numberOfReferences > 0) {
      ...
      errorFlag = EC_IllegalCall;
      DCMDATA_WARN(
          "DcmDirectoryRecord::decreaseRefNum() attempt to decrease value "
          "lower than zero");
    }
  } else {
    errorFlag = EC_IllegalCall;
    DCMDATA_ERROR(
        "illegal usage of DcmDirectoryRecord::decreaseRefNum() - RecordType "
        "must be MRDR");
  }
  return numberOfReferences;
}
...
OFCondition DcmDirectoryRecord::fillElementsAndReadSOP(
    const char* referencedFileID, const char* sourceFileName) {
  OFCondition l_error = EC_Normal;
  char* fileName = NULL;
  ...
  if (referencedFileID != NULL && *referencedFileID != '\0')
    directFromFile = OFTrue;
           ...
           referencedMRDR != NULL)  // illegal for MRDR
  {
    indirectViaMRDR = OFTrue;
  ...
  if (referencedFileID != NULL && *referencedFileID != '\0') {
    ...
    if (DirRecordType != ERT_Mrdr) {
      ...
      if (l_error.bad()) {
        DCMDATA_ERROR(
            "DcmDirectoryRecord::readSOPandFileElements(): DicomFile \""
            << fileName << "\" not found");
  ...
  if (DirRecordType != ERT_Mrdr && (directFromFile || indirectViaMRDR)) {
    if (refFile == NULL) {
      DCMDATA_ERROR(
          "Internal ERROR in DcmDirectoryRecord::fillElementsAndReadSOP()");
    }
    ...
    if (refFile->search(DCM_SOPInstanceUID, stack).good() ||
        refFile->search(DCM_MediaStorageSOPInstanceUID, stack).good()) {
      char* uid = NULL;
      OFstatic_cast(DcmUniqueIdentifier*, stack.top())->getString(uid);
      uiP->putString(uid);
    } else {
      DCMDATA_ERROR("DcmDirectoryRecord::fillElementsAndReadSOP(): "
                    << "can't find SOPInstanceUID neither in Dataset or "
                       "MetaInfo of file: "
                    << fileName);
      l_error = EC_CorruptedData;
    }
    ...
    if (refFile->search(DCM_TransferSyntaxUID, stack).good()) {
      ...
      DCMDATA_ERROR("DcmDirectoryRecord::fillElementsAndReadSOP(): "
                    << "can't find TransferSyntaxUID in MetaInfo of file: "
                    << fileName);
      l_error = EC_CorruptedData;
    }
    insert(uiP, OFTrue);
...
OFCondition DcmDirectoryRecord::purgeReferencedFile() {
  ...
  if (DirRecordType != ERT_root) {
    ...
    DCMDATA_DEBUG(
        "DcmDirectoryRecord::purgeReferencedFile() trying to purge file "
        << localFileName << " from file system");

    if (localFileName != NULL) {  // filename exists
...
void DcmDirectoryRecord::print(STD_NAMESPACE ostream& out, const size_t flags,
                               const int level, const char* pixelFileName,
                               size_t* pixelCounter) {
  if (flags & DCMTypes::PF_showTreeStructure) {
    /* print record line */
    OFOStringStream oss;
    oss << "\"Directory Record\" (offset=$" << getFileOffset() << ")"
        << OFStringStream_ends;
    ...
    if (getLengthField() == DCM_UndefinedLength)
      printInfoLine(out, flags, level, "\"ItemDelimitationItem\"",
                    &delimItemTag);
    else
      printInfoLine(out, flags, level,
                    "\"ItemDelimitationItem for re-encoding\"", &delimItemTag);
  }
}
...
OFCondition DcmDirectoryRecord::read(DcmInputStream& inStream,
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    if (getTransferState() != ERW_ready) {
      ...
      */
      offsetInFile = OFstatic_cast(Uint32, fStartPosition) -
                     xferSyn.sizeofTagHeader(getTag().getEVR());
    }

    if (getTransferState() == ERW_ready &&
        DirRecordType == ERT_Private)  // minimizes multiple evaluation
    {
...
OFCondition DcmDirectoryRecord::verify1(const OFBool autocorrect) {
  ...
  err1 = DcmItem::verify1(autocorrect);
  err2 = lowerLevelList->verify1(autocorrect);
...
OFCondition DcmDirectoryRecord::search(const DcmTagKey& tag,
                                       DcmStack& resultStack, E_SearchMode mode,
...
OFCondition DcmDirectoryRecord::assignToSOPFile(const char* referencedFileID,
  ...
  if (DirRecordType != ERT_root) {
    DCMDATA_DEBUG(
        "DcmDirectoryRecord::assignToSOPFile() old Referenced File ID was "
...
OFCondition DcmDirectoryRecord::assignToMRDR(DcmDirectoryRecord* mrdr) {
  errorFlag = EC_Normal;
  if (DirRecordType != ERT_root && mrdr != NULL  // new MRDR available
      && mrdr != referencedMRDR                  // old MRDR != new MRDR
  ) {
    DCMDATA_DEBUG(
        "DcmDirectoryRecord::assignToMRDR() old Referenced File ID was "
        << getReferencedFileName());
    DCMDATA_DEBUG("new Referenced File ID is "
                  << mrdr->lookForReferencedFileID());
...
OFCondition DcmDirectoryRecord::insertSub(DcmDirectoryRecord* dirRec,
                                          unsigned long where, OFBool before) {
  if (dirRec != NULL) {
...
OFCondition DcmDirectoryRecord::insertSubAtCurrentPos(
    DcmDirectoryRecord* dirRec, OFBool before) {
  if (dirRec != NULL) {
    if (checkHierarchy(DirRecordType, dirRec->DirRecordType).good())
...
DcmDirectoryRecord* DcmDirectoryRecord::getSub(const unsigned long num) {
  DcmDirectoryRecord* retRec =
      OFstatic_cast(DcmDirectoryRecord*, lowerLevelList->getItem(num));
...
DcmDirectoryRecord* DcmDirectoryRecord::nextSub(
    const DcmDirectoryRecord* dirRec) {
  DcmDirectoryRecord* retRec = OFstatic_cast(
      DcmDirectoryRecord*, lowerLevelList->nextInContainer(dirRec));
  errorFlag = lowerLevelList->error();
  return retRec;
}
...
DcmDirectoryRecord* DcmDirectoryRecord::removeSub(const unsigned long num) {
  DcmDirectoryRecord* retRec =
      OFstatic_cast(DcmDirectoryRecord*, lowerLevelList->remove(num));
  errorFlag = lowerLevelList->error();
  return retRec;
}
...
DcmDirectoryRecord* DcmDirectoryRecord::removeSub(DcmDirectoryRecord* dirRec) {
  DcmDirectoryRecord* retRec =
      OFstatic_cast(DcmDirectoryRecord*, lowerLevelList->remove(dirRec));
  errorFlag = lowerLevelList->error();
  return retRec;
}
...
OFCondition DcmDirectoryRecord::deleteSubAndPurgeFile(
