...
#define SWAPBUFFER_SIZE \
  16 /* sufficient for all DICOM VRs as per the 2007 edition */
...
DcmElement::DcmElement(const DcmElement& elem)
  ...
  if (elem.fValue) {
    DcmVR vr(elem.getVR());
    const unsigned short pad = (vr.isaString())
                                   ? OFstatic_cast(unsigned short, 1)
                                   : OFstatic_cast(unsigned short, 0);
    ...
    if (getLengthField() & 1) {
      ...
      // back to the caller.
      fValue =
          new (std::nothrow) Uint8[getLengthField() + 1 +
                                   pad];  // protocol error: odd value length
#else
      fValue = new Uint8[getLengthField() + 1 +
                         pad];  // protocol error: odd value length
#endif
      if (fValue)
...
OFBool DcmElement::canWriteXfer(const E_TransferSyntax newXfer,
  ...
  if (canWrite) {
    ...
    if (DcmXfer(newXfer).isExplicitVR() &&
        !DcmVR(getVR()).usesExtendedLengthEncoding()) {
      const Uint32 length = getLength(newXfer);
      if (length > 0xffff) {
        DCMDATA_DEBUG("DcmElement::canWriteXfer() Length of element "
                      << getTagName() << " " << getTag()
                      << " exceeds maximum of 16-bit length field (" << length
                      << " > 65535 bytes)");
        canWrite = OFFalse;
      }
    }
...
OFCondition DcmElement::getOFStringArray(OFString& value, OFBool normalize) {
  ...
  if (count > 0) {
    ...
    while ((i < count) &&
           (errorFlag = getOFString(string, i, normalize)).good()) {
      if (i > 0)
        value += '\\';
...
void* DcmElement::getValue(const E_ByteOrder newByteOrder) {
  ...
  if (newByteOrder == EBO_unknown)
    ...
    /* in case this call is not illegal, we need to do something. First of all,
     * set the error flag to ok */
    errorFlag = EC_Normal;
    /* do something only if the length of this element's value does not equal
     * (i.e. is greater than) 0 */
    if (getLengthField() != 0) {
      ...
      if (errorFlag.good()) {
        ...
        if (newByteOrder != fByteOrder) {
          swapIfNecessary(newByteOrder, fByteOrder, fValue, getLengthField(),
                          getTag().getVR().getValueWidth());
...
uint64_t DcmElement::GetfLoadValueoffset() {
  if (!fLoadValue)
    return 0;

  return fLoadValue->GetOffset();
}
...
OFCondition DcmElement::loadValue(DcmInputStream* inStream) {
  ...
  if (getLengthField() != 0) {
    ...
    if (readStream) {
      ...
      /* if we encountered the end of the stream, set the error flag
       * correspondingly */
      if (errorFlag.good() && readStream->eos())
        errorFlag = EC_EndOfStream;
      /* if we did not encounter the end of the stream and no error occured so
       * far, go ahead */
      else if (errorFlag.good()) {
        /* if the object which holds this element's value does not yet exist,
         * create it */
        if (!fValue)
          fValue = newValueField(); /* also set errorFlag in case of error */

        /* if object could be created  (i.e. we have an object which can be used
         * to capture this element's */
        ...
        if (fValue) {
          ...
          /* read a corresponding amount of bytes from the stream, store the
           * information in fvalue */
          /* increase the counter that counts how many bytes were actually read
           */
          incTransferredBytes(OFstatic_cast(
              Uint32,
              readStream->read(&fValue[getTransferredBytes()], readLength)));

          /* if we have read all the bytes which make up this element's value */
          if (getLengthField() == getTransferredBytes()) {
            /* call a function which performs certain operations on the
             * information which was read */
            postLoadValue();
            errorFlag = EC_Normal;
          }
          /* else set the return value correspondingly */
            ...
            DCMDATA_ERROR("DcmElement: "
                          << getTagName() << " " << getTag() << " larger ("
                          << getLengthField() << ") than remaining bytes ("
                          << getTransferredBytes()
...
Uint8* DcmElement::newValueField() {
  ...
  if (lengthField & 1) {
    if (lengthField == DCM_UndefinedLength) {
      /* Print an error message when private attribute states to have an odd
       * length equal to the maximum length, because we are not able then to
       * make this value even (+1) which would an overflow on some systems as
       * well as being illegal in DICOM
       */
      DCMDATA_ERROR("DcmElement: " << getTagName() << " " << getTag()
                                   << " has odd, maximum length ("
                                   << DCM_UndefinedLength
    ...
    value = new (std::nothrow)
        Uint8[lengthField + 1];  // protocol error: odd value length
#else
    value = new Uint8[lengthField + 1];  // protocol error: odd value length
#endif
    /* if creation was successful, set last byte to 0 (in order to initialize
     * this byte) */
  ...
  /* if this element's length is even, create a corresponding array of Lenght
     bytes */
  else
#ifdef HAVE_STD__NOTHROW
    // we want to use a non-throwing new here if available.
...
OFCondition DcmElement::changeValue(const void* value, const Uint32 position,
                                    const Uint32 num) {
  OFBool done = OFFalse;
  errorFlag = EC_Normal;
  if (position % num != 0 || getLengthField() % num != 0 ||
      position > getLengthField())
    errorFlag = EC_IllegalCall;
    ...
    if (getLengthField() == 0) {
      ...
      if (errorFlag.good()) {
        ...
        swapIfNecessary(gLocalByteOrder, fByteOrder, fValue, getLengthField(),
                        getTag().getVR().getValueWidth());
        fByteOrder = gLocalByteOrder;
        // copy old value in the beginning of new value
        memcpy(newValue, fValue, size_t(getLengthField()));
        // set parameter value in the extension
        memcpy(&newValue[getLengthField()], OFstatic_cast(const Uint8*, value),
               size_t(num));
#if defined(HAVE_STD__NOTHROW) && defined(HAVE_NOTHROW_DELETE)
        // if created with the nothrow version it must also be deleted with
        // the nothrow version else memory error.
...
OFCondition DcmElement::putValue(const void* newValue, const Uint32 length) {
  errorFlag = EC_Normal;
...
OFCondition DcmElement::createEmptyValue(const Uint32 length) {
  ...
  if (length != 0) {
    ...
    // initialize <length> bytes (which may be odd), not Length (which is always
    // even)
    if (fValue)
...
OFCondition DcmElement::read(DcmInputStream& inStream,
  ...
  /* if this element's transfer state shows ERW_notInitialized, this is an
   * illegal call */
  if (getTransferState() == ERW_notInitialized)
     ...
     * far, go ahead */
      ...
      if (getTransferState() == ERW_init) {
        /* if the Length of this element's value is greater than the amount of
         * bytes we */
        /* can read from the stream and if the stream has random access, we want
         * to create */
        /* a DcmInputStreamFactory object that enables us to read this element's
         * value later. */
        /* This new object will be stored (together with the position where we
         * have to start */
          ...
          if (fLoadValue) {
            ...
            if (skipped < getLengthField()) {
              ...
              if (dcmIgnoreParsingErrors.get())
                ...
                errorFlag = EC_StreamNotifyClient;
              /* Print an error message when too few bytes are available in the
               * file in order to distinguish this problem from any other
               * generic "InvalidStream" problem. */
              DCMDATA_ERROR("DcmElement: " << getTagName() << " " << getTag()
                                           << " larger (" << getLengthField()
                                           << ") than remaining bytes in file");
            }
          }
        }
...
OFCondition DcmElement::write(DcmOutputStream& outStream,
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    if (errorFlag.good()) {
       ...
       * write behaviour is active (i.e. everything loaded into memory upon
       * write)
       */
      Uint8* value = NULL;
      ...
      if (getLengthField() > 0) {
        if (valueLoaded()) {
          ...
          /* initialize cache object. This is safe even if the object was
           * already initialized */
          wcache->init(this, getLengthField(), getTransferredBytes(),
                       outXfer.getByteOrder());
      ...
      /* if this element's transfer state is ERW_init (i.e. it has not yet been
       * written to */
      /* the stream) and if the outstream provides enough space for tag and
       * length information */
      /* write tag and length information to it, do something */
      if (getTransferState() == ERW_init) {
        /* first compare with DCM_TagInfoLength (12). If there is not enough
         * space in the buffer, check if the buffer is still sufficient for the
         * requirements of this element, which may need only 8 instead of 12
         * bytes.
         */
        if ((outStream.avail() >= DCM_TagInfoLength) ||
          ...
          /* remember how many bytes have been written to the stream, currently
           * none so far */
          Uint32 writtenBytes = 0;

          /* write tag and length information (and possibly also data type
           * information) to the stream, */
          /* mind the transfer syntax and remember the amount of bytes that have
           * been written */
      ...
      if (accessPossible && getTransferState() == ERW_inWork) {
        Uint32 len = 0;
        if (valueLoaded()) {
          /* write as many bytes as possible to the stream starting at
           * value[getTransferredBytes()] */
          /* (note that the bytes value[0] to value[getTransferredBytes()-1]
           * have already been */
          /* written to the stream) */
          len = OFstatic_cast(
              Uint32,
              outStream.write(&value[getTransferredBytes()],
                              getLengthField() - getTransferredBytes()));

          /* increase the amount of bytes which have been transfered
           * correspondingly */
          ...
          while (!done) {
            ...
            // stop writing if something went wrong, we were unable to send all
            // of the buffer content (which indicates that the output stream
            // needs to be flushed, or everything was sent out.
            done = errorFlag.bad() || (len < buflen) ||
                   (getTransferredBytes() == getLengthField());
          }
        }

        /* if the amount of transferred bytes equals the length of the element's
         * value, the */
        /* entire value has been written to the stream. Thus, this element's
         * transfer state */
        /* has to be set to ERW_ready. If this is not the case but the error
         * flag still shows */
        /* an ok value, there was no more space in the stream and a
...
void DcmElement::writeXMLStartTag(STD_NAMESPACE ostream& out,
                                  const size_t flags, const char* attrText) {
  OFString xmlString;
  ...
  if (!(flags & DCMTypes::XF_omitDataElementName))
    out << " name=\""
        << OFStandard::convertToMarkupString(getTagName(), xmlString) << "\"";
...
OFCondition DcmElement::getPartialValue(void* targetBuffer, const Uint32 offset,
                                        Uint32 numBytes, DcmFileCache* cache,
  ...
  // handled above).
  if (offset >= getLengthField())
    return EC_InvalidOffset;

  // check if the caller is trying to read past the end of the value field
  if (numBytes > (getLengthField() - offset))
    return EC_TooManyBytesRequested;

  // check if the value is already in memory
  if (valueLoaded()) {
    // the attribute value is already in memory.
    // change internal byte order of the attribute value to the desired byte
    // order. This should only happen once for multiple calls to this method
    // since the caller will hopefully always request the same byte order.
    ...
    // the swap buffer should be large enough to keep one value of the current
    // VR
    unsigned char swapBuffer[SWAPBUFFER_SIZE];
    if (valueWidth > SWAPBUFFER_SIZE)
      return EC_IllegalCall;

    // seekoffset is the number of bytes we need to skip from the beginning of
    // the value field to the point where we will start reading. This is always
    // at the start of a new value of a multi-valued attribute.
    ...
    if (cache->isUser(this)) {
      ...
      // since we cannot seek back in the stream (only forward), check if the
      // stream is already past our needed start position
    ...
    offile_off_t remainingBytesToSkip =
        seekoffset - (readStream->tell() - cache->getOffset());
    offile_off_t skipResult;

    while (remainingBytesToSkip) {
      skipResult = readStream->skip(remainingBytesToSkip);
      if (skipResult == 0)
        return EC_InvalidStream;  // error while skipping
      remainingBytesToSkip -= skipResult;
    }

    // check if the first few bytes we want to read are "in the middle" of one
    // value of a multi-valued attribute. In that case we need to read the
    // complete value, swap it and then copy only the last bytes in desired byte
    // order.
    if (partialoffset > 0) {
      ...
      if (valueWidth !=
          OFstatic_cast(size_t, readStream->read(swapBuffer, valueWidth)))
        return EC_InvalidStream;

      // swap to desired byte order. fByteOrder contains the byte order in file.
      swapIfNecessary(byteOrder, fByteOrder, swapBuffer, valueWidth,
                      valueWidth);
      ...
      if (partialvalue > numBytes) {
        ...
        // Reset stream to position marked before, since we have not copied the
        // complete value
        readStream->putback();
      } else {
    ...
    if (bytesToRead > 0) {
      ...
      // swap to desired byte order. fByteOrder contains the byte order in file.
      swapIfNecessary(byteOrder, fByteOrder, targetBufferChar, bytesToRead,
                      valueWidth);
...
OFCondition DcmElement::getUncompressedFrameSize(DcmItem* dataset,
  ...
  if ((bitsAllocated % 8) == 0) {
    ...
    /* need to split calculation in order to avoid integer overflow for large
     * pixel data */
    const Uint32 v1 = rows * cols * samplesPerPixel;
    const Uint32 v2 = (bitsAllocated / 8) * v1;
    const Uint32 v3 = ((bitsAllocated % 8) * v1 + 7) / 8;
    //  # old code: frameSize = (bitsAllocated * rows * cols * samplesPerPixel +
    //  7) / 8;
    frameSize = v2 + v3;
  }
...
OFCondition DcmElement::getUncompressedFrame(
    ...
    OFString& /* decompressedColorModel */, DcmFileCache* /* cache */) {
  return EC_IllegalCall;
}

OFCondition DcmElement::getDecompressedColorModel(
...
int DcmElement::scanValue(const OFString& value, const OFString& vr,
