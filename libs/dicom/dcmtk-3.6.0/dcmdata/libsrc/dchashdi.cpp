...
DcmDictEntry* DcmDictEntryList::find(const DcmTagKey& k,
...
void DcmHashDictIterator::init(const DcmHashDict* d, OFBool atEnd) {
  dict = d;
  hindex = 0;
  iterating = OFFalse;
  if (dict != NULL) {
...
void DcmHashDictIterator::stepUp() {
  assert(dict != NULL);
...
void DcmHashDict::_init(int hashTabLen) {
...
DcmDictEntry* DcmHashDict::insertInList(DcmDictEntryList& l, DcmDictEntry* e) {
...
DcmDictEntry* DcmHashDict::findInList(DcmDictEntryList& l, const DcmTagKey& k,
                                      const char* privCreator) const {
  return l.find(k, privCreator);
}

const DcmDictEntry* DcmHashDict::get(const DcmTagKey& k,
                                     const char* privCreator) const {
  ...
  DcmDictEntryList* bucket = hashTab[idx];
  if (bucket)
    entry = findInList(*bucket, k, privCreator);

  if ((entry == NULL) && privCreator) {
    // As a second guess, we look for a private tag with flexible element
    // number.
    DcmTagKey tk(k.getGroup(),
                 OFstatic_cast(unsigned short, k.getElement() & 0xff));
    idx = hash(&tk);
    bucket = hashTab[idx];
    if (bucket)
      entry = findInList(*bucket, tk, privCreator);
  }

  return entry;
}

DcmDictEntry* DcmHashDict::removeInList(DcmDictEntryList& l, const DcmTagKey& k,
                                        const char* privCreator) {
...
void DcmHashDict::del(const DcmTagKey& k, const char* privCreator) {
  Uint32 idx = hash(&k);
...
STD_NAMESPACE ostream& DcmHashDict::loadSummary(STD_NAMESPACE ostream& out) {
  out << "DcmHashDict: size=" << hashTabLength << ", total entries=" << size()
      << OFendl;
