...
// MODIFY DIGITALCORE
OFGlobal<OFBool> dcmReadImplPrivAttribMaxLengthAsSQ(OFTrue);
// END MODIFY DIGITALCORE

OFGlobal<OFBool> dcmIgnoreParsingErrors(OFFalse);
OFGlobal<DcmTagKey> dcmStopParsingAfterElement(
    DCM_UndefinedTagKey);  // (0xffff,0xffff)
...
void DcmObject::printNestingLevel(STD_NAMESPACE ostream& out,
                                  const size_t flags, const int level) {
...
void DcmObject::printInfoLineStart(STD_NAMESPACE ostream& out,
  ...
  if (flags & DCMTypes::PF_showTreeStructure) {
    ...
    const signed long padLength =
        DCM_OptPrintAttributeNameLength - strlen(tag->getTagName()) - 2 * level;
    if (padLength > 0)
    ...
    /* print line start: VR */
    out << vr.getVRName() << " " << STD_NAMESPACE dec
        << STD_NAMESPACE setfill(' ');
  }
...
void DcmObject::printInfoLineEnd(STD_NAMESPACE ostream& out, const size_t flags,
  ...
  if (flags & DCMTypes::PF_showTreeStructure) {
    ...
    if (printedLength < DCM_OptPrintValueLength)
      out << OFString(
          OFstatic_cast(size_t, DCM_OptPrintValueLength - printedLength), ' ');
    if (flags & DCMTypes::PF_useANSIEscapeCodes)
...
void DcmObject::printInfoLine(STD_NAMESPACE ostream& out, const size_t flags,
                              const int level, const char* info, DcmTag* tag,
                              const OFBool isInfo) {
  /* print tag and VR */
  ...
  if (info != NULL) {
    ...
    if (printedLength > DCM_OptPrintValueLength) {
      ...
      if ((flags & DCMTypes::PF_shortenLongTagValues) &&
        ...
        OFStandard::strlcpy(output, info,
                            OFstatic_cast(size_t, DCM_OptPrintLineLength) -
                                3 /* for "..." */ + 1);
        OFStandard::strlcat(output, "...",
                            OFstatic_cast(size_t, DCM_OptPrintLineLength) + 1);
        out << output;
        printedLength = DCM_OptPrintLineLength;
      } else
        out << info;
...
/*
 * This function writes this DICOM object's tag and length information to the
 * stream. When writing information, the transfer syntax which was passed is
 * accounted for. If the transfer syntax shows an explicit value representation,
 * the data type of this object is also written to the stream. In general, this
 * function follows the rules which are specified in the DICOM standard (see
 * DICOM standard part 5, section 7) concerning the encoding of a data set which
 * shall be transmitted.
 *
 * Parameters:
 *   outStream    - [out] The stream that the information will be written to.
 *   oxfer        - [in] The transfer syntax which shall be used.
 *   writtenBytes - [out] Contains in the end the amount of bytes which have
 ...
 */
{
  ...
  if (l_error.bad()) {
    ...
    if (oxferSyn.isExplicitVR()) {
      ...
      /* getValidEVR() will convert datatype "UN" to "OB" if generation of "UN"
       * is disabled */
       ...
       * part 5, */
      /* section 7.1.2) we have to add 2 reserved bytes (set to a value of 00H)
       * to the */
      ...
      if (outvr.usesExtendedLengthEncoding()) {
        ...
        swapIfNecessary(oByteOrder, gLocalByteOrder, &valueLength, 4,
                        4);                // mind transfer syntax
        outStream.write(&valueLength, 4);  // write length, 4 bytes wide
        writtenBytes += 6;  // remember that 6 bytes were written in total
      }
      /* in case that we are dealing with a transfer syntax with explicit VR
         (see if above) and */
      /* the actual VR does not use extended length encoding (see DICOM standard
         part 5, section */
      /* 7.1.2) we do not have to add reserved bytes to the data type field and
         the actual length */
      ...
      /* the length really fits into the 2-byte field ... */
        ...
        Uint16 valueLength =
            OFstatic_cast(Uint16, Length);  // determine length (cast to 16 bit)
        swapIfNecessary(oByteOrder, gLocalByteOrder, &valueLength, 2,
                        2);                // mind transfer syntax
      ...
      /* ... if not, report an error message and return an error code. */
        ...
        DcmTag tag(Tag);
        DCMDATA_ERROR("DcmObject: Length of element "
                      << tag.getTagName() << " " << tag
                      << " exceeds maximum of 16-bit length field");
        l_error = EC_ElemLengthExceeds16BitField;
      }
    }
    /* if the transfer syntax is one with implicit value representation this
       value's data type */
    /* does not have to be written to the stream. Only the length information
       has to be written */
    /* to the stream. According to the DICOM standard the length field is in
       this case always 4 */
