...
DcmDataset::DcmDataset()
    : DcmItem(ItemTag, DCM_UndefinedLength), Xfer(EXS_Unknown) {}
...
DcmDataset::DcmDataset(const DcmDataset& old) : DcmItem(old), Xfer(old.Xfer) {}
...
void DcmDataset::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmDataset::read(DcmInputStream& inStream,
  ...
  /* state does not equal ERW_ready, go ahead and do something */
    ...
    /* if the transfer state is ERW_init, go ahead and check the transfer syntax
     * which was passed */
    if (getTransferState() == ERW_init) {
      if (dcmAutoDetectDatasetXfer.get()) {
        /* To support incorrectly encoded datasets detect the transfer syntax
         * from the stream.  */
        /* This is possible for given unknown and plain big or little endian
         * transfer syntaxes. */
        switch (xfer) {
          case EXS_Unknown:
          ...
          case EXS_BigEndianImplicit:
            ...
            if ((xfer != EXS_Unknown) && (Xfer != xfer))
              DCMDATA_WARN(
                  "DcmDataset: Wrong transfer syntax specified, detecting from "
                  "dataset");
            break;
          default:
            Xfer = xfer;
      ...
      } else /* default behaviour */
      {
         ...
         */
        /* determine the transfer syntax from the information in the stream
         * itself. */
  ...
  if (errorFlag.good() || errorFlag == EC_EndOfStream) {
    ...
    /* and set the transfer state to ERW_ready to indicate that the data set is
     * complete */
...
OFCondition DcmDataset::write(
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    /* check if the stream reported an error so far; if not, we can go ahead and
     * write some data to it */
    errorFlag = outStream.status();

    if (errorFlag.good() && getTransferState() != ERW_ready) {
      /* Determine the transfer syntax which shall be used. Either we use the
       * one which was passed, */
      /* or (if it's an unknown tranfer syntax) we use the one which is
       * contained in this->Xfer. */
      E_TransferSyntax newXfer = oxfer;
      if (newXfer == EXS_Unknown)
        newXfer = Xfer;

      /* if this function was called for the first time for the dataset object,
       * the transferState is still */
      /* set to ERW_init. In this case, we need to take care of group length and
       * padding elements according */
      /* to the strategies which are specified in glenc and padenc.
       * Additionally, we need to set the element */
      /* list pointer of this data set to the fist element and we need to set
       * the transfer state to ERW_inWork */
      /* so that this scenario will only be executed once for this data set
       * object. */
      if (getTransferState() == ERW_init) {
        /* Check stream compression for this transfer syntax */
        DcmXfer xf(newXfer);
        ...
        switch (sc) {
          case ESC_none:
            ...
            break;
          case ESC_unsupported:
            // stream compressed transfer syntax that we cannot create; bail
            // out.
            if (errorFlag.good())
              errorFlag = EC_UnsupportedEncoding;
            break;
        ...
        /* take care of group length and padding elements, according to what is
         * specified in glenc and padenc */
        computeGroupLengthAndPadding(glenc, padenc, newXfer, enctype, padlen,
       ...
       * information which */
      /* is included in this data set's element list into the buffer which was
       * passed. */
...
OFCondition DcmDataset::writeSignatureFormat(DcmOutputStream& outStream,
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    if (errorFlag.good() && getTransferState() != ERW_ready) {
      if (getTransferState() == ERW_init) {
        computeGroupLengthAndPadding(EGL_recalcGL, EPD_noChange, newXfer,
                                     enctype, 0, 0, 0);
        elementList->seek(ELP_first);
        setTransferState(ERW_inWork);
      }
      if (getTransferState() == ERW_inWork) {
        ...
        if (!elementList->empty() && (elementList->get() != NULL)) {
          ...
          do {
            dO = elementList->get();
            errorFlag =
                dO->writeSignatureFormat(outStream, newXfer, enctype, wcache);
          } while (errorFlag.good() && elementList->seek(ELP_next));
        }
        if (errorFlag.good())
          setTransferState(ERW_ready);
...
OFCondition DcmDataset::chooseRepresentation(
...
OFBool DcmDataset::hasRepresentation(
    const E_TransferSyntax repType,
    const DcmRepresentationParameter* repParam) {
