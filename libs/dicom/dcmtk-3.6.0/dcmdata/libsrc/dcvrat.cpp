...
void DcmAttributeTag::print(STD_NAMESPACE ostream& out, const size_t flags,
      ...
      const unsigned long printCount =
          ((expectedLength > DCM_OptPrintLineLength) &&
           (flags & DCMTypes::PF_shortenLongTagValues))
              ? (DCM_OptPrintLineLength - 3 /* for "..." */ + 1) /
                    (11 /* (gggg,eeee) */ + 1 /* for "\" */)
              : count;
...
OFCondition DcmAttributeTag::getTagVal(DcmTagKey& tagVal,
  ...
  if (errorFlag.good()) {
    if (uintValues == NULL)
      ...
      tagVal.set(uintValues[2 * pos] /*group*/,
                 uintValues[2 * pos + 1] /*element*/);
  }
...
OFCondition DcmAttributeTag::putTagVal(const DcmTagKey& tagVal,
  ...
  errorFlag =
      changeValue(uintVals, 2 * sizeof(Uint16) * OFstatic_cast(Uint32, pos),
                  2 * sizeof(Uint16));
  return errorFlag;
}
  ...
  if ((stringVal != NULL) && (strlen(stringVal) > 0)) {
    ...
    if (vm > 0) {
      ...
      for (unsigned long i = 0; (i < 2 * vm) && errorFlag.good(); i += 2) {
        ...
        if ((value == NULL) ||
            sscanf(value, "(%hx,%hx)", &field[i], &field[i + 1]) != 2)
          errorFlag = EC_CorruptedData;
        delete[] value;
      }
...
OFCondition DcmAttributeTag::verify1(const OFBool autocorrect) {
  ...
  if (getLengthField() % (2 * sizeof(Uint16)) != 0) {
    ...
    if (autocorrect) {
      ...
      setLengthField(getLengthField() -
                     (getLengthField() % (2 * sizeof(Uint16))));
    }
  } else
    errorFlag = EC_Normal;
  return errorFlag;
