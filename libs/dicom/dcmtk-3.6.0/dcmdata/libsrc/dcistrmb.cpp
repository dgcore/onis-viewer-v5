...
void DcmBufferProducer::releaseBuffer() {
  // releaseBuffer() might be called multiple times, so buffer_ could already be
  // NULL.
  if (status_.good() && buffer_) {
    // compute the least number of bytes that we have to store in the backup
    // buffer
    offile_off_t numBytes = bufSize_ - bufIndex_;
    ...
    if (numBytes > backupIndex_) {
      ...
      // if number of bytes is smaller than free space in backup buffer, make as
      // large as possible
      if (numBytes < backupIndex_) {
      ...
      // data in backup buffer to keep older data available for putback
      // operations
      if (numBytes < DCMBUFFERPRODUCER_BUFSIZE) {
        // move (DCMBUFFERPRODUCER_BUFSIZE - numBytes) bytes from end of backup
        // buffer to start of backup buffer. Everything else will be overwritten
        // from the user buffer.
        memmove(backup_, backup_ + numBytes,
                OFstatic_cast(size_t, DCMBUFFERPRODUCER_BUFSIZE - numBytes));

        // adjust backupStart_
        if (backupStart_ < numBytes)
          backupStart_ = 0;
        else
          backupStart_ -= numBytes;
      } else {
        // the backup buffer will be filled completely from the user buffer
        backupStart_ = 0;
      }

      // copy (numBytes) bytes from the end of the user buffer to the end of the
      // backup buffer
      memcpy(backup_ + DCMBUFFERPRODUCER_BUFSIZE - numBytes,
             buffer_ + bufSize_ - numBytes, OFstatic_cast(size_t, numBytes));

      // adjust backupIndex_
      if (backupIndex_ == DCMBUFFERPRODUCER_BUFSIZE) {
        // there was no unread data in the backup buffer before.
        // backupIndex_ only depends on the number of unread bytes in the user
        // buffer. we know that (bufSize_ - bufIndex_ <
        // DCMBUFFERPRODUCER_BUFSIZE) because this was tested before.
        backupIndex_ = DCMBUFFERPRODUCER_BUFSIZE + bufIndex_ - bufSize_;
      } else {
        // there was unread data in the backup buffer before.
        // This implies that all of the user buffer is unread and the complete
        // user buffer fits into the free space in the backup buffer, because
        // otherwise we would not have got this far. Adjust backupIndex_ by the
        // number of bytes we have moved the content of the backup buffer.
...
DcmInputBufferStream::DcmInputBufferStream()
    : DcmInputStream(
          &producer_)  // safe because DcmInputStream only stores pointer
      ,
      producer_() {}
