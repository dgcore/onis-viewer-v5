...
static OFBool compareSQAttributes(DcmSequenceOfItems* seq1,
                                  DcmSequenceOfItems* seq2, OFString& reason);
...
static OFBool locateInvalidFilenameChars(
    const OFString& filename, size_t& invalidChar, const OFBool mapFilenames,
    const char separator = PATH_SEPARATOR) {
  unsigned char c;
  size_t i = 0;
...
// massage filename into DICOM format (DOS conventions for path separators,
...
static OFString& hostToDicomFilename(const OFString& hostFilename,
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    if (c == PATH_SEPARATOR) {
      ...
      /* only characters, digits, underscore and backslash (path separator)
       * allowed */
      dicomFilename += c;
    }
  }
...
/* Massage filename into machine format (replace DOS conventions for path
 * separators) */
...
static OFBool compare(const OFString& string1, const OFString& string2) {
...
static OFString& constructTagName(DcmObject* object, OFString& tagName) {
  if (object != NULL) {
...
static OFBool compareAttributes(DcmElement* elem1, DcmElement* elem2,
                                DcmSequenceOfItems* fromSequence,
                                unsigned long itemNumber, OFString& reason) {
  reason.clear();
  OFString tmpString;
  ...
  if (elem1 != NULL) {
    if (elem2 != NULL) {
      ...
      if (elem1->getTag().getXTag() == elem2->getTag().getXTag()) {
        ...
        if (vr1.getEVR() == vr2.getEVR()) {
          ...
          if (length1 == length2) {
            ...
            if (length1 > 0) {
              if (vr1.getEVR() == EVR_SQ) {
                ...
                compareSQAttributes(OFstatic_cast(DcmSequenceOfItems*, elem1),
                                    OFstatic_cast(DcmSequenceOfItems*, elem2),
                                    reason);
              } else {
                /* everything else is regarded as a string, might be useful to
                 * check binary data separately though */
                OFString value1, value2;
                /* compare string value from dataset and record */
                if (elem1->getOFStringArray(value1).good() &&
                    elem2->getOFStringArray(value2).good()) {
                  if (!compare(value1, value2)) {
                    reason = "different values: ";
                    /* print differing values */
                    if ((value1.length() < MAX_PRINT_LENGTH) &&
                        (value2.length() < MAX_PRINT_LENGTH))
                      reason += "\"" + value1 + "\" != \"" + value2 + "\" ";
                    reason += "(" +
                              constructTagNameWithSQ(elem1, fromSequence,
                                                     itemNumber, tmpString) +
                              ")";
                  }
                }
              }
            ...
            reason = "different value lengths: " +
                     constructDifferentNumbersText(length1, length2, tmpString);
            reason += " (" +
          ...
          reason = "different VRs: " + OFString(vr1.getVRName()) +
                   " != " + vr2.getVRName() + " (" +
                   constructTagNameWithSQ(elem1, fromSequence, itemNumber,
                                          tmpString) +
                   ")";
        ...
        reason =
            "INTERNAL ERROR: different attributes: " +
...
static OFBool compareItems(DcmItem* item1, DcmItem* item2,
                           DcmSequenceOfItems* fromSequence,
                           const unsigned long itemNumber, OFString& reason) {
  reason.clear();
  OFString tmpString;
  ...
  if ((item1 != NULL) && (item2 != NULL)) {
    ...
    if (card1 == card2) {
      ...
      while (item1->nextObject(stack1, first).good() &&
             item2->nextObject(stack2, first).good()) {
        if (!compareAttributes(OFstatic_cast(DcmElement*, stack1.top()),
                               OFstatic_cast(DcmElement*, stack2.top()),
                               fromSequence, i++, reason))
          break;
        first = OFFalse;
      }
    } else {
      reason = "different number of attributes in item: " +
               constructDifferentNumbersText(card1, card2, tmpString);
      reason += " (" +
                constructTagNameWithSQ(NULL /*object*/, fromSequence,
                                       itemNumber, tmpString) +
                ")";
    }
  } else
    reason =
        "missing item: " + constructTagNameWithSQ(NULL /*object*/, fromSequence,
                                                  itemNumber, tmpString);
  /* non-empty reason variable indicates an error */
  return reason.empty();
}
    ...
    if (seq2 != NULL) {
      ...
      if (seq1->getTag().getXTag() == seq2->getTag().getXTag()) {
        ...
        if (card1 == card2) {
          ...
          while (seq1->nextObject(stack1, first).good() &&
                 seq2->nextObject(stack2, first).good()) {
            if (!compareItems(OFstatic_cast(DcmItem*, stack1.top()),
                              OFstatic_cast(DcmItem*, stack2.top()), seq1, i++,
                              reason))
              break;
            first = OFFalse;
          }
        } else {
          reason = "different number of items in sequence: " +
                   constructDifferentNumbersText(card1, card2, tmpString);
          reason += " (" + constructTagName(seq1, tmpString) + ")";
        }
      } else
        reason = "INTERNAL ERROR: different sequences: " +
                 constructTagName(seq1, tmpString) +
  ...
  if ((record != NULL) && (dataset != NULL)) {
    ...
    if (newSeq != NULL) {
      do {
        ...
        if (dataset->findAndGetSequenceItem(DCM_ContentSequence, ditem, i++)
                .good()) {
          /* check RelationshipType */
          if (ditem->findAndGetOFString(DCM_RelationshipType, tmpString)
                  .good() &&
      ...
      if ((newSeq->card() == 0) ||
          (record->insert(newSeq, OFTrue /*replaceOld*/).bad()))
        delete newSeq;
    }
  }
}
...
static void addBlendingSequence(DcmDirectoryRecord* record, DcmItem* dataset) {
  if ((record != NULL) && (dataset != NULL)) {
    ...
    if (dataset->tagExistsWithValue(DCM_BlendingSequence)) {
      ...
      if (newSeq != NULL) {
        do {
          /* get sequence item (not very efficient, but it works) */
          if (dataset->findAndGetSequenceItem(DCM_BlendingSequence, ditem, i++)
                  .good()) {
            DcmItem* newItem = new DcmItem();
            if (newItem != NULL) {
              if (newSeq->append(newItem).good()) {
                ditem->findAndInsertCopyOfElement(DCM_StudyInstanceUID,
                                                  newItem);
                ditem->findAndInsertCopyOfElement(DCM_ReferencedSeriesSequence,
                                                  newItem);
              } else
                delete newItem;
            }
...
// insert child record into the parent's list based on the numeric value of the
// criterionKey
  ...
  if ((parent != NULL) && (child != NULL)) {
    ...
    if (result.good()) {
      ...
      while (!found && ((record = parent->nextSub(record)) != NULL)) {
        ...
        if (record->findAndGetSint32(criterionKey, parentNumber).good() &&
  ...
  if ((parent != NULL) && (child != NULL)) {
    switch (child->getRecordType()) {
      ...
      case ERT_Overlay:
        ...
        result =
            insertWithISCriterion(parent, child, DCM_RETIRED_OverlayNumber);
        break;
      case ERT_Curve:
...
static OFString& alternativeStudyDate(DcmItem* dataset, OFString& result) {
  if (dataset != NULL) {
...
static OFString& alternativeStudyTime(DcmItem* dataset, OFString& result) {
  if (dataset != NULL) {
...
DicomDirInterface::DicomDirInterface()
  ...
  RLESupport =
      DcmCodecList::canChangeCoding(EXS_RLELossless, EXS_LittleEndianExplicit);
  JPEGSupport =
      DcmCodecList::canChangeCoding(EXS_JPEGProcess1TransferSyntax,
                                    EXS_LittleEndianExplicit) &&
      DcmCodecList::canChangeCoding(EXS_JPEGProcess14SV1TransferSyntax,
                                    EXS_LittleEndianExplicit);
  JP2KSupport =
      DcmCodecList::canChangeCoding(EXS_JPEG2000LosslessOnly,
                                    EXS_LittleEndianExplicit) &&
      DcmCodecList::canChangeCoding(EXS_JPEG2000, EXS_LittleEndianExplicit);

  // START MODFIY DIGITALCORE
  mb_CheckMandatoryAttribute = true;
...
// START MODFIY DIGITALCORE
void DicomDirInterface::disableCheckForMandatoryAttribute(bool pb_disable) {
  mb_CheckMandatoryAttribute = !pb_disable;
}
// END MODIFY DIGITALCORE
...
OFCondition DicomDirInterface::createNewDicomDir(
    const E_ApplicationProfile profile, const char* filename,
  ...
  if ((filename != NULL) && checkFilesetID(filesetID)) {
    ...
    if (result.good()) {
      DCMDATA_INFO("creating DICOMDIR file using "
                   << getProfileName(ApplicationProfile)
                   << " profile: " << filename);
...
OFCondition DicomDirInterface::appendToDicomDir(
    const E_ApplicationProfile profile, const char* filename) {
  OFCondition result = EC_IllegalParameter;
  if (filename != NULL) {
    FilesetUpdateMode = OFFalse;
    ...
    if (OFStandard::fileExists(filename)) {
      ...
      result = selectApplicationProfile(profile);
      if (result.good()) {
        DCMDATA_INFO("appending to DICOMDIR file using "
                     << getProfileName(ApplicationProfile)
                     << " profile: " << filename);
      ...
      /*  error code 18 is reserved for file read error messages (see
       * dcerror.cc) */
...
// create a DICOMDIR based on an existing one, i.e. append new and update
// existing entries
OFCondition DicomDirInterface::updateDicomDir(
    const E_ApplicationProfile profile, const char* filename) {
  OFCondition result = EC_IllegalParameter;
  if (filename != NULL) {
    ...
    if (OFStandard::fileExists(filename)) {
      ...
      result = selectApplicationProfile(profile);
      if (result.good()) {
        DCMDATA_INFO("updating DICOMDIR file using "
                     << getProfileName(ApplicationProfile)
                     << " profile: " << filename);
        /* finally, create a DICOMDIR object based on the existing file */
        DicomDir = new DcmDicomDir(filename);
...
OFCondition DicomDirInterface::writeDicomDir(
    const E_EncodingType encodingType, const E_GrpLenEncoding groupLength) {
  ...
  if (isDicomDirValid()) {
    ...
    result = DicomDir->write(DICOMDIR_DEFAULT_TRANSFERSYNTAX, encodingType,
                             groupLength);
    /* delete backup copy in case the new file could be written without any
     * errors */
    if (result.good())
      deleteDicomDirBackup();
    else {
      /* report an error */
      DCMDATA_ERROR(result.text()
                    << ": writing file: " << DicomDir->getDirFileName());
    }
  }
  return result;
}

// check whether the specified filename conforms to the DICOM standard
// requirements
OFBool DicomDirInterface::isFilenameValid(const char* filename,
  ...
  if ((filename == NULL) || (strlen(filename) == 0)) {
    ...
    if ((filename[0] == PATH_SEPARATOR) /* absolute path? */ ||
      ...
      DCMDATA_ERROR("invalid character(s) in filename: "
                    << filename << OFendl
    ...
    if (componentCount(filename) > MAX_FNAME_COMPONENTS) {
      DCMDATA_ERROR("too many path components (max "
                    << MAX_FNAME_COMPONENTS << ") in filename: " << filename);
      result = OFFalse;
    }
    /* ensure that each component is not too large */
    if (isComponentTooLarge(filename, MAX_FNAME_COMPONENT_SIZE,
                            MapFilenamesMode)) {
      /* create error message */
      DCMDATA_ERROR("component too large (max "
                    << MAX_FNAME_COMPONENT_SIZE
...
OFBool DicomDirInterface::isCharsetValid(const char* charset) {
  ...
  if ((charset != NULL) && (strlen(charset) > 0)) {
    ...
    if (!result)
      DCMDATA_ERROR(
          "unknown character set for fileset descriptor: " << charset);
  }
  return result;
}
...
OFCondition DicomDirInterface::checkSOPClassAndXfer(DcmMetaInfo* metainfo,
  ...
  if ((filename != NULL) && (metainfo != NULL) && (dataset != NULL)) {
    ...
    if (metainfo
            ->findAndGetOFStringArray(DCM_MediaStorageSOPClassUID,
                                      mediaSOPClassUID)
            .bad()) {
      DCMDATA_ERROR(
          "MediaStorageSOPClassUID missing in metainfo-header: " << filename);
      ...
      switch (ApplicationProfile) {
        case AP_MPEG2MPatMLDVD:
          ...
          found =
              compare(mediaSOPClassUID, UID_BreastTomosynthesisImageStorage) ||
        ...
        case AP_BasicCardiac:
          if (compare(mediaSOPClassUID, UID_XRayAngiographicImageStorage)) {
            ...
            found = compare(mediaSOPClassUID,
                            UID_RETIRED_DetachedPatientManagementSOPClass);
          }
          break;
        ...
        case AP_UltrasoundCCMF:
          /* transfer syntax needs to be checked later */
          found =
              compare(mediaSOPClassUID, UID_UltrasoundImageStorage) ||
              compare(mediaSOPClassUID, UID_UltrasoundMultiframeImageStorage);
          break;
        ...
        default: {
          ...
          if (!found) {
            found =
                compare(mediaSOPClassUID, UID_TwelveLeadECGWaveformStorage) ||
          ...
          if (!found) {
            found = compare(mediaSOPClassUID,
                            UID_SegmentationStorage) ||  // will be mapped to
                                                         // IMAGE record
          ...
          if (!found) {
            found =
                compare(mediaSOPClassUID, UID_LensometryMeasurementsStorage) ||
                compare(mediaSOPClassUID,
          ...
          if (!found) {
            found =
                compare(mediaSOPClassUID, UID_GenericImplantTemplateStorage) ||
          ...
          /* the following SOP classes have been retired with previous editions
      ...
      if (found)
        ...
        DCMDATA_ERROR("invalid SOP class ("
                      << sopClassName << ") for "
                      << getProfileName(ApplicationProfile)
                      << " profile: " << filename);
      ...
      if (result.good()) {
        ...
        if (metainfo
                ->findAndGetOFStringArray(DCM_TransferSyntaxUID, transferSyntax)
                .bad()) {
          DCMDATA_ERROR(
              "TransferSyntaxUID missing in metainfo-header: " << filename);
          result = EC_TagNotFound;
        }
        ...
        if (result.good()) {
          ...
          if (compare(transferSyntax, UID_JPEG2000LosslessOnlyTransferSyntax) ||
            ...
            if (!JP2KSupport && IconImageMode) {
              DCMDATA_ERROR(
                  "JPEG 2000 compression not supported: " << filename);
              result = EC_CannotChangeRepresentation;
            }
          }
        }
          ...
          switch (ApplicationProfile) {
            ...
            case AP_USBandFlash:
              ...
              found =
                  ...
                  compare(transferSyntax, UID_JPEG2000TransferSyntax);
              if (!found) {
                OFString xferName1 = dcmFindNameOfUID(
                    UID_LittleEndianExplicitTransferSyntax, "");
                OFString xferName2 =
                    dcmFindNameOfUID(UID_JPEGProcess14SV1TransferSyntax, "");
                OFString xferName3 =
                    dcmFindNameOfUID(UID_JPEGProcess1TransferSyntax, "");
                OFString xferName4 =
                    dcmFindNameOfUID(UID_JPEGProcess2_4TransferSyntax, "");
                OFString xferName5 = dcmFindNameOfUID(
                    UID_JPEG2000LosslessOnlyTransferSyntax, "");
                OFString xferName6 =
                    dcmFindNameOfUID(UID_JPEG2000TransferSyntax, "");
                /* create error message */
                OFOStringStream oss;
                oss << xferName1 << ", " << xferName2 << ", " << xferName3
                    << ", " << xferName4 << ", " << xferName5 << " or "
                    << xferName6 << " expected: " << filename
                    << OFStringStream_ends;
                OFSTRINGSTREAM_GETSTR(oss, tmpString)
            ...
            case AP_UltrasoundCCMF:
              ...
              if (!found) {
                OFString xferName1 = dcmFindNameOfUID(
                    UID_LittleEndianExplicitTransferSyntax, "");
                OFString xferName2 =
                    dcmFindNameOfUID(UID_RLELosslessTransferSyntax, "");
                OFString xferName3 =
                    dcmFindNameOfUID(UID_JPEGProcess1TransferSyntax, "");
                /* create error message */
                OFOStringStream oss;
            ...
            default: {
              ...
              if (!found) {
                OFString xferName =
                    dcmFindNameOfUID(expectedTransferSyntax.c_str(), "");
                if (xferName.empty())
                  xferName = expectedTransferSyntax;
                /* create error message */
                OFOStringStream oss;
                oss << xferName << " expected: " << filename
                    << OFStringStream_ends;
                OFSTRINGSTREAM_GETSTR(oss, tmpString)
                if (TransferSyntaxCheck) {
                  DCMDATA_ERROR(tmpString);
...
OFCondition DicomDirInterface::checkBasicCardiacAttributes(
    DcmItem* dataset, const char* filename) {
  OFCondition result = EC_Normal;
  if (!checkExistsWithStringValue(dataset, DCM_Modality, "XA", filename))
    result = EC_ApplicationProfileViolated;
  if (!checkExistsWithMinMaxValue(dataset, DCM_Rows, 1, 512, filename,
                                  ResolutionCheck))
    result = EC_ApplicationProfileViolated;
  if (!checkExistsWithMinMaxValue(dataset, DCM_Columns, 1, 512, filename,
                                  ResolutionCheck))
    result = EC_ApplicationProfileViolated;
  ...
  for (unsigned int grp = 0x6000; grp < 0x601f; grp += 2) {
    /* check minimum number of attributes required for an overlay plane to be
     * displayed */
    if (dataset->tagExistsWithValue(
            DcmTagKey(grp, DCM_OverlayRows.getElement())) &&
        dataset->tagExistsWithValue(
            DcmTagKey(grp, DCM_OverlayColumns.getElement())) &&
        dataset->tagExistsWithValue(
            DcmTagKey(grp, DCM_OverlayBitsAllocated.getElement())) &&
        dataset->tagExistsWithValue(
            DcmTagKey(grp, DCM_OverlayBitPosition.getElement())) &&
        !dataset->tagExistsWithValue(
            DcmTagKey(grp, DCM_OverlayData.getElement()))) {
      DCMDATA_ERROR("embedded overlay data present in group 0x"
                    << STD_NAMESPACE hex << grp << ", file: " << filename);
      result = EC_ApplicationProfileViolated;
    }
  }
  return result;
...
OFCondition DicomDirInterface::checkXrayAngiographicAttributes(
    DcmItem* dataset, const OFString& sopClass, const char* filename) {
  OFCondition result = EC_Normal;
  ...
  if (compare(sopClass, UID_XRayAngiographicImageStorage)) {
    ...
    if (!checkExistsWithStringValue(dataset, DCM_Modality, "XA", filename))
      result = EC_ApplicationProfileViolated;
    if (!checkExistsWithMinMaxValue(dataset, DCM_Rows, 1, 1024, filename,
                                    ResolutionCheck))
      result = EC_ApplicationProfileViolated;
    if (!checkExistsWithMinMaxValue(dataset, DCM_Columns, 1, 1024, filename,
                                    ResolutionCheck))
      result = EC_ApplicationProfileViolated;
    ...
    if (!checkExistsWithIntegerValue(dataset, DCM_SamplesPerPixel, 1, filename))
      result = EC_ApplicationProfileViolated;
    if (!checkExistsWithStringValue(dataset, DCM_PhotometricInterpretation,
                                    "MONOCHROME2", filename))
      result = EC_ApplicationProfileViolated;
    if (!checkExistsWithIntegerValue(dataset, DCM_BitsAllocated, 8, filename,
                                     EncodingCheck))
      result = EC_ApplicationProfileViolated;
    ...
    if (!checkExistsWithIntegerValue(dataset, DCM_HighBit, 7, filename,
                                     EncodingCheck))
      result = EC_ApplicationProfileViolated;
    if (!checkExistsWithIntegerValue(dataset, DCM_PixelRepresentation, 0,
                                     filename))
      result = EC_ApplicationProfileViolated;
...
OFCondition DicomDirInterface::checkDentalRadiographAttributes(
    DcmItem* dataset, const char* filename) {
  OFCondition result = EC_Normal;
  ...
  if (compare(sopClass, UID_CTImageStorage)) {
    ...
    if (!checkExists(dataset, DCM_PhotometricInterpretation, filename)) {
      ...
      if (compare(pi, "MONOCHROME2")) {
        ...
        if (!checkExistsWithIntegerValue(dataset, DCM_BitsAllocated, 8,
                                         filename))
          result = EC_ApplicationProfileViolated;
        if (!checkExistsWithIntegerValue(dataset, DCM_BitsStored, 8, filename))
          result = EC_ApplicationProfileViolated;
...
OFCondition DicomDirInterface::checkUltrasoundAttributes(
    DcmItem* dataset, const OFString& transferSyntax, const char* filename) {
  OFCondition result = EC_Normal;
  /* a US image */
  if (!checkExistsWithValue(dataset, DCM_PhotometricInterpretation, filename))
    ...
    const OFBool uncompressed =
        compare(transferSyntax, UID_LittleEndianExplicitTransferSyntax);
    const OFBool rle_lossless =
        compare(transferSyntax, UID_RLELosslessTransferSyntax);
    const OFBool jpeg_lossy =
        compare(transferSyntax, UID_JPEGProcess1TransferSyntax);
    OFBool valid =
        (compare(pi, "MONOCHROME2") && (uncompressed || rle_lossless)) ||
  ...
  if ((ApplicationProfile == AP_UltrasoundSCSF) ||
    ...
    while (ok && dataset
      ...
      if ((ApplicationProfile == AP_UltrasoundCCSF) ||
        ...
        if (checkExistsWithValue(ditem, DCM_PixelComponentOrganization,
                                 filename)) {
          ok &= checkExistsWithValue(ditem, DCM_PixelComponentPhysicalUnits,
                                     filename);
          ok &=
              checkExistsWithValue(ditem, DCM_PixelComponentDataType, filename);
          long pco;
          ...
          if (pco == 0) {
            ...
            ok &= checkExistsWithValue(ditem, DCM_NumberOfTableBreakPoints,
                                       filename);
            ok &=
                checkExistsWithValue(ditem, DCM_TableOfXBreakPoints, filename);
            ok &=
                checkExistsWithValue(ditem, DCM_TableOfYBreakPoints, filename);
          } else if (pco == 1) {
            /* pixel component organization: ranges */
            ok &= checkExistsWithValue(ditem, DCM_PixelComponentRangeStart,
                                       filename);
            ...
            ok &=
                checkExistsWithValue(ditem, DCM_NumberOfTableEntries, filename);
            ok &= checkExistsWithValue(ditem, DCM_TableOfPixelValues, filename);
            ok &= checkExistsWithValue(ditem, DCM_TableOfParameterValues,
                                       filename);
          } else {
            /* report an error */
            printUnexpectedValueMessage(DCM_PixelComponentOrganization,
                                        filename);
            ok = OFFalse;
          }
        } else
    ...
    if (ok) {
      if (i == 0) {
        ...
        printRequiredAttributeMessage(DCM_SequenceOfUltrasoundRegions,
                                      filename);
        result = EC_ApplicationProfileViolated;
      }
    } else {
      /* report any error within the 'while' loop as an violoation of the
       * application profile */
      result = EC_ApplicationProfileViolated;
    }
  }
...
OFCondition DicomDirInterface::checkMandatoryAttributes(DcmMetaInfo* metainfo,
  ...
  if ((filename != NULL) && (dataset != NULL)) {
    ...
    metainfo->findAndGetOFStringArray(DCM_MediaStorageSOPClassUID,
                                      mediaSOPClassUID);
    ...
    if (recordType == ERT_HangingProtocol) {
      ...
      if (!checkExistsWithValue(dataset, DCM_HangingProtocolName, filename))
        result = EC_InvalidTag;
      if (!checkExistsWithValue(dataset, DCM_HangingProtocolDescription,
                                filename))
        result = EC_InvalidTag;
      ...
      if (!checkExistsWithValue(dataset, DCM_HangingProtocolCreator, filename))
        result = EC_InvalidTag;
      if (!checkExistsWithValue(dataset, DCM_HangingProtocolCreationDateTime,
                                filename))
        result = EC_InvalidTag;
      if (!checkExistsWithValue(dataset, DCM_HangingProtocolDefinitionSequence,
                                filename))
        result = EC_InvalidTag;
      if (!checkExistsWithValue(dataset, DCM_NumberOfPriorsReferenced,
                                filename))
        result = EC_InvalidTag;
      ...
      /* check whether all type 1 elements are really present */
      if (!checkExistsWithValue(dataset, DCM_ImplantAssemblyTemplateName,
                                filename))
        result = EC_InvalidTag;
      if (!checkExistsWithValue(dataset, DCM_ImplantAssemblyTemplateIssuer,
                                filename))
        result = EC_InvalidTag;
      if (!checkExistsWithValue(dataset, DCM_ProcedureTypeCodeSequence,
                                filename))
        result = EC_InvalidTag;
    } else if (recordType == ERT_ImplantAssy) {
      /* check whether all type 1 elements are really present */
      if (!checkExistsWithValue(dataset, DCM_ImplantTemplateGroupName,
                                filename))
        result = EC_InvalidTag;
      if (!checkExistsWithValue(dataset, DCM_ImplantTemplateGroupIssuer,
                                filename))
        result = EC_InvalidTag;
    } else {
      ...
      */
      ...
      switch (recordType) {
        case ERT_Overlay:
          if (!InventMode) {
            if (!checkExistsWithValue(dataset, DCM_RETIRED_OverlayNumber,
                                      filename))
              result = EC_InvalidTag;
          }
          break;
        ...
        case ERT_Curve:
          if (!InventMode) {
            if (!checkExistsWithValue(dataset, DCM_RETIRED_CurveNumber,
                                      filename))
              result = EC_InvalidTag;
          }
          break;
        case ERT_SRDocument:
          ...
          if (!checkExistsWithValue(dataset, DCM_ContentTime, filename))
            result = EC_InvalidTag;
          if (!checkExistsWithValue(dataset, DCM_ConceptNameCodeSequence,
                                    filename))
            result = EC_InvalidTag;
          {
            OFString tmpString;
            if (compare(getStringFromDataset(dataset, DCM_VerificationFlag,
                                             tmpString),
                        "VERIFIED")) {
              /* VerificationDateTime is required if verification flag is
                 VERIFIED, retrieve most recent (= last) entry from
                 VerifyingObserverSequence */
              DcmItem* ditem = NULL;
              OFCondition l_status = dataset->findAndGetSequenceItem(
                  DCM_VerifyingObserverSequence, ditem, -1 /*last*/);
              if (l_status.good()) {
                if (!checkExistsWithValue(ditem, DCM_VerificationDateTime,
                                          filename))
                  result = EC_InvalidTag;
              } else
                result = l_status;
        ...
        case ERT_Presentation:
          ...
          if (!checkExistsWithValue(dataset, DCM_ContentLabel, filename))
            result = EC_InvalidTag;
          if (!checkExistsWithValue(dataset, DCM_PresentationCreationDate,
                                    filename))
            result = EC_InvalidTag;
          if (!checkExistsWithValue(dataset, DCM_PresentationCreationTime,
                                    filename))
            result = EC_InvalidTag;
          break;
        case ERT_Waveform:
        ...
        case ERT_EncapDoc:
          if (!checkExistsWithValue(dataset, DCM_InstanceNumber, filename))
            result = EC_InvalidTag;
          if (!checkExistsWithValue(dataset, DCM_MIMETypeOfEncapsulatedDocument,
                                    filename))
            result = EC_InvalidTag;
          break;
        case ERT_Stereometric:
        ...
        default: {
          ...
          if ((ApplicationProfile == AP_GeneralPurposeDVD) ||
            ...
            l_status = checkXrayAngiographicAttributes(
                dataset, mediaSOPClassUID, filename);
            ...
            l_status =
                checkCTandMRAttributes(dataset, mediaSOPClassUID, filename);
                   ...
                   (ApplicationProfile == AP_UltrasoundCCMF)) {
            l_status =
                checkUltrasoundAttributes(dataset, transferSyntax, filename);
          }
...
OFCondition DicomDirInterface::selectApplicationProfile(
...
// check whether DICOM file is suitable for a DICOMDIR of the specified
// application profile
...
// load DICOM file and check whether it is suitable for a DICOMDIR of the
// specified application profile
OFCondition DicomDirInterface::loadAndCheckDicomFile(
    const char* filename, const char* directory, DcmFileFormat& fileformat) {
  OFCondition result = EC_IllegalParameter;
  /* create fully qualified pathname of the DICOM file to be added */
  OFString pathname;
  OFStandard::combineDirAndFilename(pathname, OFSTRING_GUARD(directory),
                                    OFSTRING_GUARD(filename),
  ...
  if (isFilenameValid(filename)) {
    ...
    if (result.good()) {
      ...
      if ((metainfo == NULL) || (metainfo->card() == 0)) {
        DCMDATA_ERROR(
            "file not in part 10 format (no metainfo-header): " << filename);
...
// BEGIN DIGITALCORE
OFCondition DicomDirInterface::loadAndCheckDicomFile1(
    const char* path, const char* target, DcmFileFormat& fileformat) {
  OFCondition result = EC_IllegalParameter;
  /* create fully qualified pathname of the DICOM file to be added */
  // OFString pathname;
  // OFStandard::combineDirAndFilename(pathname, OFSTRING_GUARD(directory),
  // OFSTRING_GUARD(filename), OFTrue /*allowEmptyDirName*/);
  // DCMDATA_INFO("checking file: " << pathname);
  /* check filename */
  if (isFilenameValid(target)) {
    /* load DICOM file */
    result = fileformat.loadFile(path);
    if (result.good()) {
      /* check for correct part 10 file format */
      DcmMetaInfo* metainfo = fileformat.getMetaInfo();
      if ((metainfo == NULL) || (metainfo->card() == 0)) {
        DCMDATA_ERROR(
            "file not in part 10 format (no metainfo-header): " << target);
        result = EC_InvalidStream;
      }
      DcmDataset* dataset = fileformat.getDataset();
      if (dataset == NULL) {
        DCMDATA_ERROR("file contains no data (no dataset): " << target);
        result = EC_CorruptedData;
      }
      /* only proceed if previous checks have been passed */
      if (result.good()) {
        /* check for SOP class and transfer syntax */
        result = checkSOPClassAndXfer(metainfo, dataset, target);
        if (result.good()) {
          /* check for mandatory attributes */
          // START MODIFY DIGITALCORE
          if (mb_CheckMandatoryAttribute)
            // END MODIFY DIGITALCORE
            if (checkMandatoryAttributes(metainfo, dataset, target).bad())
              result = EC_ApplicationProfileViolated;
        }
      }
    } else {
    ...
    switch (record->getRecordType()) {
      case ERT_Patient:
        if (checkExistsWithValue(dataset, DCM_PatientID)) {
          ...
          result = compare(
              getStringFromDataset(record, DCM_PatientID, patientID),
              getStringFromDataset(dataset, DCM_PatientID, datasetString));
          /* optional: check whether PatientName also matches */
          if (result && !compare(getStringFromDataset(record, DCM_PatientName,
                                                      recordString),
                                 getStringFromDataset(dataset, DCM_PatientName,
                                                      datasetString))) {
            if (InventPatientIDMode) {
              DCMDATA_WARN(
                  "PatientName inconsistent for PatientID: " << patientID);
           ...
           * PatientName */
          result = compare(
              getStringFromDataset(record, DCM_PatientName, recordString),
      ...
      case ERT_Study:
        if (checkExistsWithValue(record, DCM_StudyInstanceUID)) {
          result = compare(
              getStringFromDataset(record, DCM_StudyInstanceUID, recordString),
              getStringFromDataset(dataset, DCM_StudyInstanceUID,
                                   datasetString));
        } else {
          /* the Study Instance UID can be in the referenced file instead */
          OFString refFilename;
          if (!getStringFromDataset(record, DCM_ReferencedFileID, refFilename)
                   .empty()) {
            OFString hostFilename;
            if (locateDicomFile(refFilename, hostFilename)) {
              result =
                  compare(getStringFromFile(hostFilename.c_str(),
                                            DCM_StudyInstanceUID, recordString),
                          getStringFromDataset(dataset, DCM_StudyInstanceUID,
                                               datasetString));
            } else
      ...
      case ERT_Series:
        result = compare(
            getStringFromDataset(record, DCM_SeriesInstanceUID, recordString),
            getStringFromDataset(dataset, DCM_SeriesInstanceUID,
                                 datasetString));
        break;
      case ERT_Image:
      case ERT_Overlay:
      ...
      case ERT_ImplantAssy:
         ...
         */
        result = compare(
            getStringFromDataset(record, DCM_ReferencedSOPInstanceUIDInFile,
                                 recordString),
...
DcmDirectoryRecord* DicomDirInterface::findExistingRecord(
    DcmDirectoryRecord* parent, const E_DirRecType recordType,
    DcmItem* dataset) {
  OFBool found = OFFalse;
  DcmDirectoryRecord* record = NULL;
  if (parent != NULL) {
...
DcmDirectoryRecord* DicomDirInterface::buildPatientRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
  ...
  if (record != NULL) {
    ...
    if (record->error().good()) {
      ...
      if ((ApplicationProfile == AP_GeneralPurposeDVD) ||
        ...
        copyElementType1C(dataset, DCM_PatientBirthDate, record,
                          sourceFilename);
...
DcmDirectoryRecord* DicomDirInterface::buildStudyRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& sourceFilename) {
  ...
  if (record != NULL) {
    ...
    if (record->error().good()) {
      ...
      copyStringWithDefault(dataset, DCM_StudyDate, record, sourceFilename,
                            alternativeStudyDate(dataset, tmpString).c_str(),
                            OFTrue /*printWarning*/);
      copyStringWithDefault(dataset, DCM_StudyTime, record, sourceFilename,
                            alternativeStudyTime(dataset, tmpString).c_str(),
                            OFTrue /*printWarning*/);
...
DcmDirectoryRecord* DicomDirInterface::buildSeriesRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& sourceFilename) {
    ...
    if (record->error().good()) {
      ...
      if ((ApplicationProfile == AP_GeneralPurposeDVD) ||
        ...
        copyElementType1C(dataset, DCM_InstitutionAddress, record,
                          sourceFilename);
        copyElementType1C(dataset, DCM_PerformingPhysicianName, record,
                          sourceFilename);
      } else if ((ApplicationProfile == AP_BasicCardiac) ||
                 (ApplicationProfile == AP_XrayAngiographic) ||
                 (ApplicationProfile == AP_XrayAngiographicDVD)) {
        /* additional type 2 keys specified by specific profiles (type 1C or 3
         * in file) */
        copyStringWithDefault(dataset, DCM_InstitutionName, record,
                              sourceFilename);
        copyStringWithDefault(dataset, DCM_InstitutionAddress, record,
                              sourceFilename);
        copyStringWithDefault(dataset, DCM_PerformingPhysicianName, record,
                              sourceFilename);
      }
    } else {
...
DcmDirectoryRecord* DicomDirInterface::buildOverlayRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new overlay record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Overlay, referencedFileID.c_str(),
...
DcmDirectoryRecord* DicomDirInterface::buildModalityLutRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new modality lut record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_ModalityLut, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildVoiLutRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new voi lut record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_VoiLut, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildCurveRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new curve record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Curve, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
// create or update structure reporting record and copy required values from
// dataset
DcmDirectoryRecord* DicomDirInterface::buildStructReportRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new struct report record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_SRDocument, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      ...
      if (compare(
              getStringFromDataset(dataset, DCM_VerificationFlag, tmpString),
              "VERIFIED")) {
        ...
        DcmItem* ditem = NULL;
        OFCondition status = dataset->findAndGetSequenceItem(
            DCM_VerifyingObserverSequence, ditem, -1 /*last*/);
        if (status.good())
          copyElementType1(ditem, DCM_VerificationDateTime, record,
                           sourceFilename);
        else
          printAttributeErrorMessage(DCM_VerifyingObserverSequence, status,
                                     "retrieve");
      }
...
// create or update presentation state record and copy required values from
// dataset
DcmDirectoryRecord* DicomDirInterface::buildPresentationRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new presentation record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Presentation, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      ...
      copyElementType1C(dataset, DCM_ReferencedSeriesSequence, record,
                        sourceFilename);
...
DcmDirectoryRecord* DicomDirInterface::buildWaveformRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new waveform record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Waveform, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildRTDoseRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new rt dose record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_RTDose, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
// create or update rt structure set record and copy required values from
// dataset
DcmDirectoryRecord* DicomDirInterface::buildRTStructureSetRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new rt structure set record */
  if (record == NULL)
    record = new DcmDirectoryRecord(
        ERT_RTStructureSet, referencedFileID.c_str(), sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildRTPlanRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new rt plan record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_RTPlan, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildRTTreatmentRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new rt treatment record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_RTTreatRecord, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildStoredPrintRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new stored print record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_StoredPrint, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildKeyObjectDocRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new key object doc record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_KeyObjectDoc, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildRegistrationRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new registration record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Registration, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildFiducialRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new fiducial record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Fiducial, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildRawDataRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new raw data record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_RawData, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildSpectroscopyRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new spectroscopy record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Spectroscopy, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      ...
      if ((ApplicationProfile == AP_GeneralPurposeDVD) ||
        ...
        copyElementType1C(dataset, DCM_FrameOfReferenceUID, record,
                          sourceFilename);
        copyElementType1C(dataset, DCM_SynchronizationFrameOfReferenceUID,
...
DcmDirectoryRecord* DicomDirInterface::buildEncapDocRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new encap doc record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_EncapDoc, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      ...
      copyElementType1C(dataset, DCM_HL7InstanceIdentifier, record,
                        sourceFilename);
...
DcmDirectoryRecord* DicomDirInterface::buildValueMapRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new value map record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_ValueMap, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
// create or update hanging protocol record and copy required values from
// dataset
DcmDirectoryRecord* DicomDirInterface::buildHangingProtocolRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new hanging protocol record */
  if (record == NULL)
    record = new DcmDirectoryRecord(
        ERT_HangingProtocol, referencedFileID.c_str(), sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      ...
      copyElementType1(dataset, DCM_NumberOfPriorsReferenced, record,
                       sourceFilename);
      copyElementType2(dataset,
                       DCM_HangingProtocolUserIdentificationCodeSequence,
                       record, sourceFilename);
    } else {
...
DcmDirectoryRecord* DicomDirInterface::buildStereometricRecord(
    DcmDirectoryRecord* record, DcmItem* /* dataset */,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new value map record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Stereometric, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildPaletteRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new palette record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Palette, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildSurfaceRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new surface record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Surface, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildMeasurementRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new measurement record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Measurement, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildImplantRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new implant record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Implant, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
...
DcmDirectoryRecord* DicomDirInterface::buildImplantGroupRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new implant group record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_ImplantGroup, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      /* copy attribute values from dataset to implant group record */
      copyElementType1(dataset, DCM_ImplantAssemblyTemplateName, record,
                       sourceFilename);
      copyElementType1(dataset, DCM_ImplantAssemblyTemplateIssuer, record,
                       sourceFilename);
      copyElementType1(dataset, DCM_ProcedureTypeCodeSequence, record,
                       sourceFilename);
    } else {
...
DcmDirectoryRecord* DicomDirInterface::buildImplantAssyRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new implant assy record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_ImplantAssy, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      /* copy attribute values from dataset to implant assy record */
      copyElementType1(dataset, DCM_ImplantTemplateGroupName, record,
                       sourceFilename);
      copyElementType3(dataset, DCM_ImplantTemplateGroupDescription, record,
                       sourceFilename);
      copyElementType1(dataset, DCM_ImplantTemplateGroupIssuer, record,
                       sourceFilename);
    } else {
...
DcmDirectoryRecord* DicomDirInterface::buildImageRecord(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  /* create new image record */
  if (record == NULL)
    record = new DcmDirectoryRecord(ERT_Image, referencedFileID.c_str(),
                                    sourceFilename.c_str());
  if (record != NULL) {
    /* check whether new record is ok */
    if (record->error().good()) {
      ...
      switch (ApplicationProfile) {
        ...
        case AP_BasicCardiac: {
          OFString tmpString;
          OFBool xaImage =
              compare(getStringFromDataset(dataset, DCM_SOPClassUID, tmpString),
                      UID_XRayAngiographicImageStorage);
          if (xaImage) {
            /* type 1C: required for XA images (type 1 for Basic Cardiac
             * Profile) */
            copyElementType1(dataset, DCM_ImageType, record, sourceFilename);
            /* type 1C: required if ImageType value 3 is "BIPLANE A" or "BIPLANE
             * B" */
            getStringComponentFromDataset(dataset, DCM_ImageType, tmpString, 2);
            if (compare(tmpString, "BIPLANE A") ||
                compare(tmpString, "BIPLANE B"))
              copyElementType1(dataset, DCM_ReferencedImageSequence, record,
                               sourceFilename);
          }
          /* additional type 2 keys specified by specific profiles (type 3 in
           * image IOD) */
      ...
      if (iconImage) {
        OFCondition status =
            addIconImage(record, dataset, iconSize, sourceFilename);
        if (status.bad()) {
      ...
      if ((fgets(line, maxline, file) != NULL) && (strcmp(line, "P5\n") == 0)) {
        ...
        if ((fgets(line, maxline, file) != NULL) && (line[0] != '\0')) {
          ...
          if (((*line != '#') || (fgets(line, maxline, file) != NULL)) &&
              ...
              (pgmWidth > 0) && (pgmHeight > 0)) {
            unsigned int pgmMax = 0;
            /* get maximum gray value */
            if ((fgets(line, maxline, file) != NULL) &&
                (sscanf(line, "%u", &pgmMax) > 0) && (pgmMax == 255)) {
              const unsigned long pgmSize = pgmWidth * pgmHeight;
              Uint8* pgmData = new Uint8[pgmSize];
              if (pgmData != NULL) {
                /* get pgm image data */
                if (fread(pgmData, sizeof(Uint8),
                          OFstatic_cast(size_t, pgmSize), file) == pgmSize) {
                  /* if already scaled, just copy the bitmap */
                  if ((width == pgmWidth) && (height == pgmHeight) &&
                      (count == pgmSize)) {
                    OFBitmanipTemplate<Uint8>::copyMem(pgmData, pixel, count);
                    result = OFTrue;
                  } else if (ImagePlugin != NULL) {
                    /* scale image to requested size */
                    result = ImagePlugin->scaleData(
                        pgmData, pgmWidth, pgmHeight, pixel, width, height);
                  }
                  if (!result)
                    DCMDATA_ERROR(
                        "cannot scale external icon, no image support "
                        "available");
                  corrupt = OFFalse;
                }
                /* free memory */
                ...
                DCMDATA_ERROR(EC_MemoryExhausted.text()
                              << ": cannot allocate memory for pgm pixel data");
        ...
        if (corrupt)
          DCMDATA_ERROR(
              "corrupt file format for external icon (not pgm binary)");
      } else
...
OFBool DicomDirInterface::getIconFromDataset(DcmItem* dataset, Uint8* pixel,
                                             const unsigned long count,
                                             const unsigned int width,
                                             const unsigned int height) {
  OFBool result = OFFalse;
  /* check parameters (incl. availability of image plugin) */
  if ((ImagePlugin != NULL) && (dataset != NULL) && (pixel != NULL) &&
      (count >= width * height)) {
    ...
    result = ImagePlugin->scaleImage(dataset, pixel, count,
                                     OFstatic_cast(unsigned long, frame), width,
                                     height, (fCount == 1) /*decompressAll*/);
  }
  return result;
}
    ...
    if (result.good()) {
      ...
      if (pixel != NULL) {
        ...
        if (!IconPrefix.empty()) {
          ...
          iconOk = getIconFromFile(IconPrefix + sourceFilename, pixel, count,
                                   width, height);
        } else {
...
DcmDirectoryRecord* DicomDirInterface::addRecord(
    DcmDirectoryRecord* parent, const E_DirRecType recordType, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  DcmDirectoryRecord* record = NULL;
  if (parent != NULL) {
    /* check whether record already exists */
    DcmDirectoryRecord* oldRecord = record =
        findExistingRecord(parent, recordType, dataset);
    if ((record == NULL) || FilesetUpdateMode) {
      ...
      if (record != NULL) {
        ...
        if ((recordType != ERT_Patient) && (recordType != ERT_Study) &&
            (recordType != ERT_Series)) {
          if (!checkReferencedSOPInstance(record, dataset, referencedFileID,
                                          sourceFilename))
            return NULL;
        }
      }
      ...
      switch (recordType) {
        ...
        case ERT_Series:
          ...
          break;
        case ERT_Overlay:
          record = buildOverlayRecord(record, dataset, referencedFileID,
                                      sourceFilename);
          break;
        case ERT_ModalityLut:
          record = buildModalityLutRecord(record, dataset, referencedFileID,
                                          sourceFilename);
          break;
        case ERT_VoiLut:
          record = buildVoiLutRecord(record, dataset, referencedFileID,
                                     sourceFilename);
          break;
        case ERT_Curve:
          record = buildCurveRecord(record, dataset, referencedFileID,
                                    sourceFilename);
          break;
        case ERT_SRDocument:
          record = buildStructReportRecord(record, dataset, referencedFileID,
                                           sourceFilename);
          break;
        case ERT_Presentation:
          record = buildPresentationRecord(record, dataset, referencedFileID,
                                           sourceFilename);
          break;
        case ERT_Waveform:
          record = buildWaveformRecord(record, dataset, referencedFileID,
                                       sourceFilename);
          break;
        case ERT_RTDose:
          record = buildRTDoseRecord(record, dataset, referencedFileID,
                                     sourceFilename);
          break;
        case ERT_RTStructureSet:
          record = buildRTStructureSetRecord(record, dataset, referencedFileID,
                                             sourceFilename);
          break;
        case ERT_RTPlan:
          record = buildRTPlanRecord(record, dataset, referencedFileID,
                                     sourceFilename);
          break;
        case ERT_RTTreatRecord:
          record = buildRTTreatmentRecord(record, dataset, referencedFileID,
                                          sourceFilename);
          break;
        case ERT_StoredPrint:
          record = buildStoredPrintRecord(record, dataset, referencedFileID,
                                          sourceFilename);
          break;
        case ERT_KeyObjectDoc:
          record = buildKeyObjectDocRecord(record, dataset, referencedFileID,
                                           sourceFilename);
          break;
        case ERT_Registration:
          record = buildRegistrationRecord(record, dataset, referencedFileID,
                                           sourceFilename);
          break;
        case ERT_Fiducial:
          record = buildFiducialRecord(record, dataset, referencedFileID,
                                       sourceFilename);
          break;
        case ERT_RawData:
          record = buildRawDataRecord(record, dataset, referencedFileID,
                                      sourceFilename);
          break;
        case ERT_Spectroscopy:
          record = buildSpectroscopyRecord(record, dataset, referencedFileID,
                                           sourceFilename);
          break;
        case ERT_EncapDoc:
          record = buildEncapDocRecord(record, dataset, referencedFileID,
                                       sourceFilename);
          break;
        case ERT_ValueMap:
          record = buildValueMapRecord(record, dataset, referencedFileID,
                                       sourceFilename);
          break;
        case ERT_HangingProtocol:
          record = buildHangingProtocolRecord(record, dataset, referencedFileID,
                                              sourceFilename);
          break;
        case ERT_Stereometric:
          record = buildStereometricRecord(record, dataset, referencedFileID,
                                           sourceFilename);
          break;
        case ERT_Palette:
          record = buildPaletteRecord(record, dataset, referencedFileID,
                                      sourceFilename);
          break;
        case ERT_Surface:
          record = buildSurfaceRecord(record, dataset, referencedFileID,
                                      sourceFilename);
          break;
        case ERT_Measurement:
          record = buildMeasurementRecord(record, dataset, referencedFileID,
                                          sourceFilename);
          break;
        case ERT_Implant:
          record = buildImplantRecord(record, dataset, referencedFileID,
                                      sourceFilename);
          break;
        case ERT_ImplantGroup:
          record = buildImplantGroupRecord(record, dataset, referencedFileID,
                                           sourceFilename);
          break;
        case ERT_ImplantAssy:
          record = buildImplantAssyRecord(record, dataset, referencedFileID,
                                          sourceFilename);
          break;
        default:
          /* it can only be an image */
          record = buildImageRecord(record, dataset, referencedFileID,
                                    sourceFilename);
      }
      if (record != NULL) {
        /* type 1C: required if an extended character set is used in one of the
         * record keys */
      ...
      if ((recordType != ERT_Patient) && (recordType != ERT_Study) &&
        ...
        DCMDATA_WARN(
            "file "
            << sourceFilename
            << ": directory record for this SOP instance already exists");
      }
      /* perform consistency check */
      if (ConsistencyCheck) {
        /* abort on any inconsistancy */
        if (warnAboutInconsistentAttributes(record, dataset, sourceFilename,
                                            AbortMode) &&
            AbortMode)
          return NULL;
      }
...
OFBool DicomDirInterface::checkReferencedSOPInstance(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& referencedFileID, const OFString& sourceFilename) {
  OFBool result = OFTrue;
  if ((record != NULL) && (dataset != NULL)) {
    ...
    if (record->findAndGetOFStringArray(DCM_ReferencedFileID, refFileID)
            .good() &&
        !compare(refFileID, referencedFileID)) {
      /* create error message */
      DCMDATA_ERROR("file " << sourceFilename
                            << ": SOP instance already referenced "
                            << "with different file ID (" << refFileID << ")");
      result = OFFalse;
    }
    /* check SOP class UID */
    if (!compareStringAttributes(dataset, DCM_SOPClassUID, record,
                                 DCM_ReferencedSOPClassUIDInFile,
                                 sourceFilename, OFTrue /*errorMsg*/))
      result = OFFalse;
  }
  return result;
}
    ...
    while ((record = parent->nextSub(record)) != NULL) {
      ...
      if (record->getRecordType() == ERT_Patient) {
        if (!record->tagExistsWithValue(DCM_PatientID))
          setDefaultValue(record, DCM_PatientID, AutoPatientNumber++,
                          AUTO_PATIENTID_PREFIX);
        if (recurse)
...
void DicomDirInterface::inventMissingStudyLevelAttributes(
    DcmDirectoryRecord* parent) {
  if (parent != NULL) {
    DcmDirectoryRecord* record = NULL;
    /* iterate over all child records */
    while ((record = parent->nextSub(record)) != NULL) {
      if (!record->tagExistsWithValue(DCM_StudyID))
        setDefaultValue(record, DCM_StudyID, AutoStudyNumber++,
                        AUTO_STUDYID_PREFIX);
...
void DicomDirInterface::inventMissingSeriesLevelAttributes(
    DcmDirectoryRecord* parent) {
  if (parent != NULL) {
    DcmDirectoryRecord* record = NULL;
    /* iterate over all child records */
...
void DicomDirInterface::inventMissingInstanceLevelAttributes(
    DcmDirectoryRecord* parent) {
  if (parent != NULL) {
    DcmDirectoryRecord* record = NULL;
    /* iterate over all child records */
...
OFCondition DicomDirInterface::addDicomFile(const char* filename,
  ...
  if (DicomDir != NULL) {
    ...
    if (result.good()) {
      ...
      DcmMetaInfo* metainfo = fileformat.getMetaInfo();
      DcmDataset* dataset = fileformat.getDataset();
      /* massage filename into DICOM format (DOS conventions for path
      ...
      /* if hanging protocol, palette or implant file then attach it to the root
       * record and stop */
      if (compare(sopClass, UID_HangingProtocolStorage)) {
        /* add a hanging protocol record below the root */
        if (addRecord(rootRecord, ERT_HangingProtocol, dataset, fileID,
                      pathname) == NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_ColorPaletteStorage)) {
        /* add a palette record below the root */
        if (addRecord(rootRecord, ERT_Palette, dataset, fileID, pathname) ==
            NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_GenericImplantTemplateStorage)) {
        /* add an implant record below the root */
        if (addRecord(rootRecord, ERT_Implant, dataset, fileID, pathname) ==
            NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_ImplantAssemblyTemplateStorage)) {
        /* add an implant group record below the root */
        if (addRecord(rootRecord, ERT_ImplantGroup, dataset, fileID,
                      pathname) == NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_ImplantTemplateGroupStorage)) {
        /* add an implant assy record below the root */
        if (addRecord(rootRecord, ERT_ImplantAssy, dataset, fileID, pathname) ==
            NULL)
          result = EC_CorruptedData;
      } else {
        /* add a patient record below the root */
        DcmDirectoryRecord* patientRecord =
            addRecord(rootRecord, ERT_Patient, dataset, fileID, pathname);
        if (patientRecord != NULL) {
          /* if patient management file then attach it to patient record and
           * stop */
          if (compare(sopClass,
                      UID_RETIRED_DetachedPatientManagementMetaSOPClass)) {
            result = patientRecord->assignToSOPFile(fileID.c_str(),
                                                    pathname.c_str());
            DCMDATA_ERROR(result.text()
            ...
            DcmDirectoryRecord* studyRecord =
                addRecord(patientRecord, ERT_Study, dataset, fileID, pathname);
            ;
            if (studyRecord != NULL) {
              /* add a series record below the current study record */
              DcmDirectoryRecord* seriesRecord =
                  addRecord(studyRecord, ERT_Series, dataset, fileID, pathname);
              ;
              if (seriesRecord != NULL) {
                /* add one of the instance record below the current series
                 * record */
                if (addRecord(seriesRecord, sopClassToRecordType(sopClass),
                              dataset, fileID, pathname) == NULL)
                  result = EC_CorruptedData;
              } else
                result = EC_CorruptedData;
            } else
...
// BEGIN DIGITALCORE
OFCondition DicomDirInterface::addDicomFile1(const char* path,
                                             const char* target) {
  OFCondition result = EC_IllegalParameter;
  /* first, make sure that a DICOMDIR object exists */
  if (DicomDir != NULL) {
    ...
    /* then check the file name, load the file and check the content */
    DcmFileFormat fileformat;
    result = loadAndCheckDicomFile1(path, target, fileformat);
    if (result.good()) {
      DCMDATA_INFO("adding file: " << path);
      /* start creating the DICOMDIR directory structure */
      DcmDirectoryRecord* rootRecord = &(DicomDir->getRootRecord());
      DcmMetaInfo* metainfo = fileformat.getMetaInfo();
      DcmDataset* dataset = fileformat.getDataset();
       ...
       * separators, uppercase) */
      OFString fileID;
      hostToDicomFilename(target, fileID);
      /* what kind of object (SOP Class) is stored in the file */
      OFString sopClass;
      metainfo->findAndGetOFString(DCM_MediaStorageSOPClassUID, sopClass);
      /* if hanging protocol, palette or implant file then attach it to the root
      ...
      if (compare(sopClass, UID_HangingProtocolStorage)) {
        /* add a hanging protocol record below the root */
        if (addRecord(rootRecord, ERT_HangingProtocol, dataset, fileID, path) ==
            NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_ColorPaletteStorage)) {
        /* add a palette record below the root */
        if (addRecord(rootRecord, ERT_Palette, dataset, fileID, path) == NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_GenericImplantTemplateStorage)) {
        /* add an implant record below the root */
        if (addRecord(rootRecord, ERT_Implant, dataset, fileID, path) == NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_ImplantAssemblyTemplateStorage)) {
        /* add an implant group record below the root */
        if (addRecord(rootRecord, ERT_ImplantGroup, dataset, fileID, path) ==
            NULL)
          result = EC_CorruptedData;
      } else if (compare(sopClass, UID_ImplantTemplateGroupStorage)) {
        /* add an implant assy record below the root */
        if (addRecord(rootRecord, ERT_ImplantAssy, dataset, fileID, path) ==
            NULL)
          result = EC_CorruptedData;
      } else {
        /* add a patient record below the root */
        DcmDirectoryRecord* patientRecord =
            addRecord(rootRecord, ERT_Patient, dataset, fileID, path);
        if (patientRecord != NULL) {
          /* if patient management file then attach it to patient record and
           * stop */
          if (compare(sopClass,
                      UID_RETIRED_DetachedPatientManagementMetaSOPClass)) {
            result = patientRecord->assignToSOPFile(fileID.c_str(), path);
            DCMDATA_ERROR(result.text()
                          << ": cannot assign patient record to file: "
                          << path);
          } else {
            /* add a study record below the current patient record */
            DcmDirectoryRecord* studyRecord =
                addRecord(patientRecord, ERT_Study, dataset, fileID, path);
            if (studyRecord != NULL) {
              /* add a series record below the current study record */
              DcmDirectoryRecord* seriesRecord =
                  addRecord(studyRecord, ERT_Series, dataset, fileID, path);
              if (seriesRecord != NULL) {
                /* add one of the instance record below the current series
                 * record */
                if (addRecord(seriesRecord, sopClassToRecordType(sopClass),
                              dataset, fileID, path) == NULL)
                  result = EC_CorruptedData;
              } else
                result = EC_CorruptedData;
            } else
...
OFCondition DicomDirInterface::setFilesetDescriptor(const char* filename,
  ...
  if (DicomDir != NULL) {
    ...
    if (isFilenameValid(filename, OFTrue /*allowEmpty*/) &&
      ...
      if (dataset != NULL) {
        ...
        if ((filename == NULL) || (strlen(filename) == 0)) {
          ...
          dataset->findAndDeleteElement(
              DCM_SpecificCharacterSetOfFileSetDescriptorFile);
          result = EC_Normal;
        } else {
          /* massage filename into DICOM format (DOS conventions for path
           * separators, uppercase) */
          ...
          result = dataset->putAndInsertString(DCM_FileSetDescriptorFileID,
                                               fileID.c_str());
          if (result.good()) {
            ...
            if ((charset == NULL) || (strlen(charset) == 0)) {
              ...
              result = dataset->putAndInsertString(
                  DCM_SpecificCharacterSetOfFileSetDescriptorFile, charset);
            }
          }
        }
      }
...
// enable/disable the abort mode, i.e. abort on first inconsistent file
// (otherwise warn)
...
// enable/disable filename mapping mode, i.e. whether non-conforming filename
...
// enable/disable invent mode, i.e. whether missing attribute values are
// invented
...
// enable/disable invent patient ID mode, i.e. whether missing patient ID is
// invented
...
// enable/disable retired SOP class support, i.e. whether retired SOP classes
// are supported
...
// enable/disable the backup mode, i.e. whether to create a backup of the
// DICOMDIR
...
// enable/disable pixel encoding check, i.e. whether the pixel encoding is
// checked for particular application profiles
...
// enable/disable resolution check, i.e. whether the spatial resolution is
// checked for particular application profiles
...
// enable/disable transfer syntax check, i.e. whether the transfer syntax is
// checked for particular application profiles
...
// enable/disable consistency check, i.e. whether the file is checked for
// consistency with the directory record
...
// print an error message to the console (stderr) that the value of the given
// tag is unexpected
...
// print an error message to the console (stderr) that a required attribute is
// missing/empty
void DicomDirInterface::printRequiredAttributeMessage(const DcmTagKey& key,
  ...
  DCMDATA_ERROR("required attribute " << DcmTag(key).getTagName() << " " << key
                                      << " " << (emptyMsg ? "empty" : "missing")
                                      << str);
}

// print an error message to the console (stderr) that something went wrong with
// an attribute
void DicomDirInterface::printAttributeErrorMessage(const DcmTagKey& key,
  ...
  if (error.bad()) {
    ...
    DCMDATA_ERROR(error.text()
                  << ": " << str << DcmTag(key).getTagName() << " " << key);
  }
}
...
void DicomDirInterface::printRecordErrorMessage(const OFCondition& error,
  ...
  if (error.bad()) {
    ...
    DCMDATA_ERROR(error.text() << ": " << str << recordTypeToName(recordType)
                               << " directory record");
  }
}

// return the name of the specified application profile
const char* DicomDirInterface::getProfileName(
    const E_ApplicationProfile profile) {
  const char* result = "";
  switch (profile) {
    case AP_GeneralPurpose:
      ...
      if (toFile != NULL) {
        ...
        while (result && ((c = getc(fromFile)) != EOF)) {
          ...
          if (putc(c, toFile) == EOF) {
            /* create error message */
            OFOStringStream oss;
            DCMDATA_ERROR("copying files: " << fromFilename << " to "
                                            << toFilename);
            /* abort loop */
            result = OFFalse;
          }
...
OFBool DicomDirInterface::warnAboutInconsistentAttributes(
    DcmDirectoryRecord* record, DcmItem* dataset,
    const OFString& sourceFilename, const OFBool abortCheck) {
  OFBool result = OFFalse;
  /* check parameters first */
  if ((record != NULL) && (dataset != NULL)) {
      ...
      if ((delem != NULL) && (delem->getLength() > 0)) {
        ...
        if (dataset->tagExistsWithValue(tag)) {
          if (delem->getTag().getEVR() == EVR_SQ) {
            /* do not check particular sequences (because they will always
             * deviate) */
            if ((delem->getTag() != DCM_ContentSequence) &&
                (delem->getTag() != DCM_BlendingSequence))
              result &= compareSequenceAttributes(dataset, tag, record,
                                                  sourceFilename);
          } else {
            /* everything else can be compared as a string */
            result &= compareStringAttributes(dataset, tag, record, tag,
                                              sourceFilename);
          }
        }
      }
...
OFBool DicomDirInterface::checkFilesetID(const OFString& filesetID) {
  ...
  if (!filesetID.empty()) {
    ...
    if (!DcmCodeString::checkVR(filesetID, &invalidChar,
                                OFFalse /*checkLength*/)) {
      /* create error message */
      DCMDATA_ERROR(
          "invalid character(s) in fileset ID: "
          << filesetID << OFendl
    ...
    if (isComponentTooLarge(
            filesetID, OFstatic_cast(size_t, DcmVR(EVR_CS).getMaxValueLength()),
            MapFilenamesMode)) {
...
OFBool DicomDirInterface::checkExists(DcmItem* dataset, const DcmTagKey& key,
                                      const char* filename) {
...
// check whether specified tag exists in the dataset and has the expected string
// value
...
// check whether specified tag exists in the dataset and has the expected
// integer value
...
// check whether specified tag exists in the dataset and has an integer value in
// the expected range
OFBool DicomDirInterface::checkExistsWithMinMaxValue(
    DcmItem* dataset, const DcmTagKey& key, const long min, const long max,
    const char* filename, const OFBool reject) {
  /* first, check whether tag exists, and report an error if not */
  OFBool result = checkExists(dataset, key, filename);
  if (result) {
    long i;
...
OFString& DicomDirInterface::getStringFromDataset(DcmItem* dataset,
  ...
  if (dataset != NULL) {
    ...
    OFCondition status =
        dataset->findAndGetOFStringArray(key, result, searchIntoSub);
...
OFString& DicomDirInterface::getStringComponentFromDataset(
    DcmItem* dataset, const DcmTagKey& key, OFString& result,
    const unsigned long pos, OFBool searchIntoSub) {
  result.clear();
  if (dataset != NULL) {
    /* get string value component from dataset and report if tag or component is
     * missing */
    OFCondition status =
        dataset->findAndGetOFString(key, result, pos, searchIntoSub);
    if (status.bad()) {
      DCMDATA_ERROR(status.text()
                    << ": cannot retrieve value " << (pos + 1) << " of "
...
void DicomDirInterface::copyElement(DcmItem* dataset, const DcmTagKey& key,
  ...
  if ((dataset != NULL) && (record != NULL)) {
    ...
    if (!optional || (copyEmpty && dataset->tagExists(key)) ||
        dataset->tagExistsWithValue(key)) {
      DcmElement* delem = NULL;
      /* get copy of element from source dataset */
      OFCondition status = dataset->findAndGetElement(
          key, delem, OFFalse /*searchIntoSub*/, OFTrue /*createCopy*/);
      if (status.good()) {
        ...
        if (status.good()) {
          ...
          if (delem->getVR() != tag.getEVR()) {
            ...
            DCMDATA_WARN("file " << sourceFilename << ": possibly wrong VR: "
                                 ...
                                 << DcmVR(delem->getVR()).getVRName()
                                 << " found, expected " << tag.getVRName()
                                 << " instead");
          }
        } else
          delete delem;
...
void DicomDirInterface::copyStringWithDefault(DcmItem* dataset,
  ...
  if ((dataset != NULL) && (record != NULL)) {
    ...
    if (dataset->tagExistsWithValue(key)) {
      ...
      /* retrieve string value from source dataset and put it into the
       * destination dataset */
      status = record->putAndInsertString(
          key, getStringFromDataset(dataset, key, stringValue).c_str());
    } else {
      if (printWarning && (defaultValue != NULL)) {
        /* create warning message */
        DCMDATA_WARN(
            "file " << sourceFilename << ": " << DcmTag(key).getTagName() << " "
...
OFBool DicomDirInterface::compareStringAttributes(
    DcmItem* dataset, const DcmTagKey& datKey, DcmDirectoryRecord* record,
    const DcmTagKey& recKey, const OFString& sourceFilename,
    const OFBool errorMsg) {
  OFBool result = OFFalse;
  /* check parameters first */
  if ((dataset != NULL) && (record != NULL)) {
    ...
    if (!result) {
      ...
      oss << "  " << recordTypeToName(record->getRecordType())
          << " Record [Key: " << DcmTag(uniqueKey).getTagName() << " "
          << uniqueKey << "=\"" << uniqueString << "\"]" << OFendl;
      oss << "    Existing Record (origin: " << originFilename
          << ") defines: " << OFendl;
      oss << "      " << DcmTag(recKey).getTagName() << " " << recKey << "=\""
          << recordString << "\"" << OFendl;
      oss << "    File (" << sourceFilename << ") defines:" << OFendl;
      oss << "      " << DcmTag(datKey).getTagName() << " " << datKey << "=\""
          << datasetString << "\"" << OFendl;
...
OFBool DicomDirInterface::compareSequenceAttributes(
    DcmItem* dataset, DcmTagKey& key, DcmDirectoryRecord* record,
    const OFString& sourceFilename) {
  OFBool result = OFFalse;
  /* check parameters first */
  if ((dataset != NULL) && (record != NULL)) {
    ...
    if (record->findAndGetSequence(key, seq1).good() &&
      ...
      if (!result) {
        OFString uniqueString;
        OFString originFilename =
            OFSTRING_GUARD(record->getRecordsOriginFile());
        const DcmTagKey uniqueKey = getRecordUniqueKey(record->getRecordType());
        getStringFromDataset(record, uniqueKey, uniqueString);
        if (originFilename.empty())
          originFilename = "<unknown>";
        /* create warning message */
        DCMDATA_WARN("file inconsistent with existing DICOMDIR record"
                     << OFendl << "  "
                     << recordTypeToName(record->getRecordType())
                     << " Record [Key: " << DcmTag(uniqueKey).getTagName()
                     << " " << uniqueKey << "=\"" << uniqueString << "\"]"
                     << OFendl << "    Reason: " << reason << OFendl
                     << "    Existing Record (origin: " << originFilename
                     << ") defines: " << OFendl
...
// set the specified default value (number or prefix and number) to the given
// tag
void DicomDirInterface::setDefaultValue(DcmDirectoryRecord* record,
  ...
  if (record != NULL) {
    ...
    DCMDATA_WARN(
        recordTypeToName(record->getRecordType())
        << " Record (origin: " << OFSTRING_GUARD(record->getRecordsOriginFile())
        << ") inventing " << DcmTag(key).getTagName() << ": " << buffer);
  }
}
 ...
 *  Fixed inconsistency in the list of attributes for the implant group
 * directory record (this has been corrected with the "FT2" version of
 * Supplement 131).
 *
 ...
 *  Further enhancements on BlendingSequence (e.g. check whether it exists
 * before trying to copy certain data elements from it).
 *
 ...
 *  directory record type MEASUREMENT. Also fixed issues with other record
 * types.
 *
 ...
 *  Compared names of SOP Class UIDs with 2009 edition of the DICOM standard.
 * The resulting name changes are mainly caused by the fact that the
 * corresponding SOP Class is now retired.
 *
 *  Revision 1.40  2010-08-10 11:59:31  uli
 *  Fixed some cases where dcmFindNameOfUID() returning NULL could cause
 * crashes.
 *
 ...
 *  Fixed issue with checking of DICOM input files (wrong handling of
 * OFCondition return value).
 *
 ...
 *  Added full support for X-Ray Radiation Dose SR documents and Real World
 * Value Mapping objects to DICOMDIR. Renamed ERT_StructReport to
 * ERT_SRDocument.
 *
