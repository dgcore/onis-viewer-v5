...
GlobalDcmDataDictionary dcmDataDict(OFTrue /*loadBuiltin*/,
                                    OFFalse /*loadExternal*/);
#else
// this is the default case, i.e. both the builtin and the external dictionaries
// are loaded
GlobalDcmDataDictionary dcmDataDict(OFTrue /*loadBuiltin*/,
                                    OFTrue /*loadExternal*/);
#endif
...
static DcmDictEntry* makeSkelEntry(Uint16 group, Uint16 element,
                                   Uint16 upperGroup, Uint16 upperElement,
                                   DcmEVR evr, const char* tagName, int vmMin,
                                   int vmMax, const char* standardVersion,
                                   DcmDictRangeRestriction groupRestriction,
                                   DcmDictRangeRestriction elementRestriction,
                                   const char* privCreator) {
  DcmDictEntry* e = NULL;
  e = new DcmDictEntry(group, element, upperGroup, upperElement, evr, tagName,
                       vmMin, vmMax, standardVersion, OFFalse, privCreator);
  if (e != NULL) {
...
OFBool DcmDataDictionary::loadSkeletonDictionary() {
  ...
  */
  DcmDictEntry* e = NULL;
  e = makeSkelEntry(0x0000, 0x0000, 0xffff, 0x0000, EVR_UL,
                    "GenericGroupLength", 1, 1, "GENERIC",
  ...
  */
  e = makeSkelEntry(0xfffe, 0xe000, 0xfffe, 0xe000, EVR_na, "Item", 1, 1,
                    "DICOM3", DcmDictRange_Unspecified,
                    DcmDictRange_Unspecified, NULL);
  addEntry(e);
  e = makeSkelEntry(0xfffe, 0xe00d, 0xfffe, 0xe00d, EVR_na,
                    "ItemDelimitationItem", 1, 1, "DICOM3",
                    DcmDictRange_Unspecified, DcmDictRange_Unspecified, NULL);
  addEntry(e);
  e = makeSkelEntry(0xfffe, 0xe0dd, 0xfffe, 0xe0dd, EVR_na,
                    "SequenceDelimitationItem", 1, 1, "DICOM3",
                    DcmDictRange_Unspecified, DcmDictRange_Unspecified, NULL);
  addEntry(e);
...
DcmDataDictionary::DcmDataDictionary(OFBool loadBuiltin, OFBool loadExternal)
    : hashDict(), repDict(), skeletonCount(0), dictionaryLoaded(OFFalse) {
...
static char* stripTrailingWhitespace(char* s) {
  int i, n;
...
static void stripLeadingWhitespace(char* s) {
  if (s) {
    unsigned char c;
    unsigned char* t;
    unsigned char* p;
...
static OFBool parseVMField(char* vmField, int& vmMin, int& vmMax) {
  OFBool ok = OFTrue;
  char c = 0;
  int dummy = 0;
...
static int splitFields(const char* line, char* fields[], int maxFields,
                       char splitChar) {
  const char* p;
  int foundFields = 0;
  int len;
...
static OFBool parseTagPart(char* s, unsigned int& l, unsigned int& h,
  ...
  if (sscanf(s, "%x-%c-%x", &l, &restrictor, &h) == 3) {
    switch (restrictor) {
      ...
      default:
        DCMDATA_ERROR(
            "DcmDataDictionary: Unknown range restrictor: " << restrictor);
        ok = OFFalse;
        break;
    }
...
static OFBool parseWholeTagField(char* s, DcmTagKey& key, DcmTagKey& upperKey,
                                 DcmDictRangeRestriction& groupRestriction,
                                 DcmDictRangeRestriction& elementRestriction,
                                 char*& privCreator) {
  ...
  if (s[i] == '\0')
    return OFFalse; /* element part missing */
  i++;              /* after the ',' */
  ...
  if (s[i] == '\"') /* private creator */
  {
    ...
    pc[pi] = '\0';
    if (s[i] == '\0')
      return OFFalse; /* closing quotation mark missing */
    i++;
    stripLeadingWhitespace(s + i);
    if (s[i] != ',')
      return OFFalse; /* element part missing */
  ...
  if (pi > 0) {
    ...
    if (privCreator)
      strcpy(privCreator, pc);
  }

  key.set(OFstatic_cast(unsigned short, gl), OFstatic_cast(unsigned short, el));
  upperKey.set(OFstatic_cast(unsigned short, gh),
               OFstatic_cast(unsigned short, eh));

  return OFTrue;
}

static OFBool onlyWhitespace(const char* s) {
  int len = strlen(s);
...
static char* getLine(char* line, int maxLineLen, FILE* f) {
  char* s;
...
static OFBool isaCommentLine(const char* s) {
  OFBool isComment = OFFalse; /* assumption */
  int len = strlen(s);
  int i = 0;
  for (i = 0; i < len && isspace(OFstatic_cast(unsigned char, s[i]));
       i++) /*loop*/
    ;
...
OFBool DcmDataDictionary::loadDictionary(const char* fileName,
  ...
  /* first, check whether 'fileName' really points to a file (and not to a
  ...
  while (getLine(lineBuf, DCM_MAXDICTLINESIZE, f)) {
    ...
    fieldsPresent = splitFields(lineBuf, lineFields, DCM_MAXDICTFIELDS,
    ...
    switch (fieldsPresent) {
      ...
      case 4:
        ...
        /* drop through to next case label */
      case 3:
        if (!parseWholeTagField(lineFields[0], key, upperKey, groupRestriction,
                                elementRestriction, privCreator)) {
          DCMDATA_ERROR("DcmDataDictionary: " << fileName << ": "
                                              << "bad Tag field (line "
                                              << lineNumber
                                              << "): " << lineFields[0]);
    ...
    if (!errorOnThisLine) {
      e = new DcmDictEntry(key.getGroup(), key.getElement(),
                           upperKey.getGroup(), upperKey.getElement(), vr,
                           tagName, vmMin, vmMax, standardVersion, OFTrue,
                           privCreator);
...
OFBool DcmDataDictionary::loadExternalDictionaries() {
  ...
  if ((env != NULL) && (strlen(env) != 0)) {
    ...
    if (sepCnt == 0) {
      ...
      int ndicts =
          splitFields(env, dictArray, sepCnt + 1, ENVIRONMENT_PATH_SEPARATOR);
...
void DcmDataDictionary::addEntry(DcmDictEntry* e) {
  if (e->isRepeating()) {
    /*
...
const DcmDictEntry* DcmDataDictionary::findEntry(
    const DcmDictEntry& entry) const {
...
const DcmDictEntry* DcmDataDictionary::findEntry(
    const DcmTagKey& key, const char* privCreator) const {
...
const DcmDictEntry* DcmDataDictionary::findEntry(const char* name) const {
  ...
  for (iter = hashDict.begin(); (e == NULL) && (iter != hashDict.end());
       ++iter) {
    if ((*iter)->contains(name)) {
      e = *iter;
      if (e->getGroup() % 2) {
        /* tag is a private tag - continue search to be sure to find non-private
         * keys first */
        if (!ePrivate)
          ePrivate = e;
        e = NULL;
