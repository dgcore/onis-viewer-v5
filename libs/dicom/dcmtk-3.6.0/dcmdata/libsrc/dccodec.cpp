...
OFCondition DcmCodec::insertStringIfMissing(DcmItem* dataset,
                                            const DcmTagKey& tag,
                                            const char* val) {
  DcmStack stack;
...
OFCondition DcmCodec::convertToSecondaryCapture(DcmItem* dataset) {
  ...
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_SOPInstanceUID, buf);

  // Type 1 attributes - insert with value if missing
  dcmGenerateUniqueIdentifier(buf, SITE_STUDY_UID_ROOT);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_StudyInstanceUID, buf);
  dcmGenerateUniqueIdentifier(buf, SITE_SERIES_UID_ROOT);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_SeriesInstanceUID, buf);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_ConversionType, "WSD");
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_Modality, "OT");

  // Type 2 attributes - insert without value if missing
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_PatientName, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_PatientID, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_PatientBirthDate, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_PatientSex, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_PatientOrientation, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_StudyDate, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_StudyTime, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_ReferringPhysicianName, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_StudyID, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_AccessionNumber, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_SeriesNumber, NULL);
  if (result.good())
    result = insertStringIfMissing(dataset, DCM_InstanceNumber, NULL);

  return result;
}

OFCondition DcmCodec::insertCodeSequence(DcmItem* dataset,
                                         const DcmTagKey& tagKey,
                                         const char* codingSchemeDesignator,
                                         const char* codeValue,
                                         const char* codeMeaning) {
  if (dataset == NULL || codingSchemeDesignator == NULL || codeValue == NULL ||
      codeMeaning == NULL)
    return EC_IllegalCall;
  ...
  if (dseq) {
    ...
    if (ditem) {
      dseq->insert(ditem);
      result = ditem->putAndInsertString(DCM_CodingSchemeDesignator,
                                         codingSchemeDesignator);
      if (result.good())
        result = ditem->putAndInsertString(DCM_CodeValue, codeValue);
      if (result.good())
        result = ditem->putAndInsertString(DCM_CodeMeaning, codeMeaning);
    } else
...
OFCondition DcmCodec::newInstance(DcmItem* dataset,
  ...
  if (tempResult.good())
    tempResult = dataset->findAndGetString(DCM_SOPInstanceUID, instanceUID);
    ...
    if (dseq) {
      ...
      if (ditem) {
        ...
        if (elem1) {
          ...
          if (result.good()) {
            DcmElement* elem2 =
                new DcmUniqueIdentifier(DCM_ReferencedSOPInstanceUID);
            if (elem2) {
        ...
        if (result.good() && purposeOfReferenceCodingScheme &&
          ...
          result = DcmCodec::insertCodeSequence(
              ditem, DCM_PurposeOfReferenceCodeSequence,
              purposeOfReferenceCodingScheme, purposeOfReferenceCodeValue,
              purposeOfReferenceCodeMeaning);
        }
      } else
        result = EC_MemoryExhausted;
      if (result.good())
...
OFCondition DcmCodec::updateImageType(DcmItem* dataset) {
  ...
  OFCondition status =
      dataset->search(DCM_ImageType, stack, ESM_fromHere, OFFalse);
  if (status.good()) {
...
OFCondition DcmCodec::determineStartFragment(Uint32 frameNo,
                                             Sint32 numberOfFrames,
                                             DcmPixelSequence* fromPixSeq,
                                             Uint32& currentItem) {
  Uint32 numberOfFragments = fromPixSeq->card();
  if (numberOfFrames < 1 ||
      numberOfFragments <= OFstatic_cast(Uint32, numberOfFrames) ||
  ...
  if (result.good()) {
    ...
    if (result.good()) {
      // check if the offset table has the right size: 4 bytes for each frame
      // (not fragment!)
      if (tableLength != 4 * OFstatic_cast(Uint32, numberOfFrames))
        return EC_IllegalCall;

      // byte swap offset table into local byte order. In file, the offset table
      // is always in little endian
      swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, rawOffsetTable,
                      tableLength, sizeof(Uint32));
      ...
      // OK, now let's look if we can find a fragment that actually corresponds
      // to that offset. In counter we compute the offset for each frame by
      // adding all fragment lenghts
      Uint32 counter = 0;
      // now iterate over all fragments except the index table. The start of the
      // first fragment is defined as zero.
      for (Uint32 idx = 1; idx < numberOfFragments; ++idx) {
        ...
        // add pixel item length plus 8 bytes overhead for the item tag and
        // length field
        counter += pixItem->getLength() + 8;
      }

      // bad luck. We have not found a fragment corresponding to the offset in
      // the offset table. Either we cannot correctly add numbers, or they
      // cannot :-)
...
OFCondition DcmCodecList::registerCodec(
    const DcmCodec* aCodec, const DcmRepresentationParameter* aDefaultRepParam,
    const DcmCodecParameter* aCodecParameter) {
  if ((aCodec == NULL) || (aCodecParameter == NULL))
    return EC_IllegalParameter;
#ifdef WITH_THREADS
  if (!codecLock.initialized())
    return EC_IllegalCall;  // should never happen
#endif

  // acquire write lock on codec list.  Will block if some codec is currently
  // active.
  OFCondition result = EC_Normal;
#ifdef WITH_THREADS
  ...
  if (0 == locker.wrlock()) {
#endif
    DcmCodecList* listEntry =
        new DcmCodecList(aCodec, aDefaultRepParam, aCodecParameter);
    if (listEntry) {
      ...
      if (result.good())
        registeredCodecs.push_back(listEntry);
      else
        delete listEntry;
    } else
      result = EC_MemoryExhausted;
...
OFCondition DcmCodecList::updateCodecParameter(
    const DcmCodec* aCodec, const DcmCodecParameter* aCodecParameter) {
  if ((aCodec == NULL) || (aCodecParameter == NULL))
    return EC_IllegalParameter;
#ifdef WITH_THREADS
  if (!codecLock.initialized())
  ...
  if (0 == locker.wrlock()) {
    ...
    while (first != last) {
      if ((*first)->codec == aCodec)
        (*first)->codecParameter = aCodecParameter;
      ++first;
    }
#ifdef WITH_THREADS
  } else
...
OFCondition DcmCodecList::decode(const DcmXfer& fromType,
  ...
  // acquire write lock on codec list.  Will block if some write lock is
  // currently active.
#ifdef WITH_THREADS
  OFReadWriteLocker locker(codecLock);
  if (0 == locker.rdlock()) {
    ...
    OFListIterator(DcmCodecList*) last = registeredCodecs.end();
    while (first != last) {
      if ((*first)->codec->canChangeCoding(fromXfer,
                                           EXS_LittleEndianExplicit)) {
        result = (*first)->codec->decode(fromParam, fromPixSeq,
...
OFCondition DcmCodecList::decodeFrame(
    const DcmXfer& fromType, const DcmRepresentationParameter* fromParam,
    DcmPixelSequence* fromPixSeq, DcmItem* dataset, Uint32 frameNo,
  ...
  if (0 == locker.rdlock()) {
    ...
    while (first != last) {
      if ((*first)->codec->canChangeCoding(fromXfer,
                                           EXS_LittleEndianExplicit)) {
        result = (*first)->codec->decodeFrame(
            fromParam, fromPixSeq, (*first)->codecParameter, dataset, frameNo,
            startFragment, buffer, bufSize, decompressedColorModel);
        first = last;
      } else
        ++first;
    }
...
OFCondition DcmCodecList::encode(const E_TransferSyntax fromRepType,
                                 const DcmRepresentationParameter* fromParam,
                                 DcmPixelSequence* fromPixSeq,
  ...
  if (0 == locker.rdlock()) {
    ...
    while (first != last) {
      if ((*first)->codec->canChangeCoding(fromRepType, toRepType)) {
        if (!toRepParam)
          toRepParam = (*first)->defaultRepParam;
    ...
    while (first != last) {
      if ((*first)->codec->canChangeCoding(fromRepType, toRepType)) {
        if (!toRepParam)
          toRepParam = (*first)->defaultRepParam;
        result =
            (*first)->codec->encode(pixelData, length, toRepParam, toPixSeq,
                                    (*first)->codecParameter, pixelStack);
        first = last;
      } else
        ++first;
...
OFBool DcmCodecList::canChangeCoding(const E_TransferSyntax fromRepType,
                                     const E_TransferSyntax toRepType) {
#ifdef WITH_THREADS
  if (!codecLock.initialized())
    return OFFalse;  // should never happen
#endif
...
OFCondition DcmCodecList::determineDecompressedColorModel(
    const DcmXfer& fromType, const DcmRepresentationParameter* fromParam,
    DcmPixelSequence* fromPixSeq, DcmItem* dataset,
    OFString& decompressedColorModel) {
#ifdef WITH_THREADS
  if (!codecLock.initialized())
    return EC_IllegalCall;  // should never happen
  ...
  if (0 == locker.rdlock()) {
    ...
    while (first != last) {
      if ((*first)->codec->canChangeCoding(fromXfer,
                                           EXS_LittleEndianExplicit)) {
        result = (*first)->codec->determineDecompressedColorModel(
            fromParam, fromPixSeq, (*first)->codecParameter, dataset,
