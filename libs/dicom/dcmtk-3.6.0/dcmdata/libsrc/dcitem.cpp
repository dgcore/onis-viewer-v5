...
E_TransferSyntax DcmItem::checkTransferSyntax(DcmInputStream& inStream) {
  ...
  const Uint16 t1 = OFstatic_cast(
      unsigned short,
      (c1 & 0xff) + ((c2 & 0xff) << 8));  // explicit little endian
  const Uint16 t2 = OFstatic_cast(
      unsigned short, (c3 & 0xff) + ((c4 & 0xff) << 8));  // conversion
  DcmTag taglittle(t1, t2);
  DcmTag tagbig(swapShort(t1), swapShort(t2));

  /* now we want to determine the transfer syntax which was used to code the
   * information in the stream. */
  /* The decision is based on two questions: a) Did we encounter a valid tag?
   * and b) Do the last 2 bytes */
  /* which were read from the stream represent a valid VR? In certain special
   * cases, where the transfer */
  /* cannot be determined without doubt, we want to guess the most probable
   * transfer syntax. */

  /* if both tag variables show an error, we encountered an invalid tag */
  if ((taglittle.error().bad()) && (tagbig.error().bad())) {
    /* in case we encounterd an invalid tag, we want to assume that the used
     * transfer syntax */
    /* is a little endian transfer syntax. Now we have to figure out, if it is
     * an implicit or */
    /* explicit transfer syntax. Hence, check if the last 2 bytes represent a
     * valid VR. */
  ...
  /* if at least one tag variable did not show an error, we encountered a valid
     tag */
  else {
    /* in case we encounterd a valid tag, we want to figure out, if it is an
     * implicit or */
    /* explicit transfer syntax. Hence, check if the last 2 bytes represent a
     * valid VR. */
    if (foundVR(&tagAndVR[4])) {
      ...
      if (taglittle.error().bad()) {
        /* if the litte endian tag was invalid, the transfer syntax is big
         * endian explicit */
        transferSyntax = EXS_BigEndianExplicit;
      } else if (tagbig.error().bad()) {
        /* if the big endian tag was invalid, the transfer syntax is little
         * endian explicit */
        transferSyntax = EXS_LittleEndianExplicit;
      } else {
        ...
        if ((taglittle.getGTag() > 0xff) && (tagbig.getGTag() <= 0xff))
          transferSyntax = EXS_BigEndianExplicit;
        else
          transferSyntax = EXS_LittleEndianExplicit;
      }
...
void DcmItem::checkAndUpdateVR(DcmItem& item, DcmTag& tag) {
  /* handle special cases where the VR can be determined by some other element
   * values */
  if (((tag == DCM_WaveformData) || (tag == DCM_WaveformPaddingValue)) &&
    ...
    if (item.findAndGetUint16(DCM_WaveformBitsAllocated, bitsAlloc).good()) {
      if (bitsAlloc == 8) {
        DCMDATA_DEBUG("setting undefined VR of "
                      << tag.getTagName() << " " << tag
                      << " to 'OB' because WaveformBitsAllocated "
                      << DCM_WaveformBitsAllocated << " has a value of 8");
        tag.setVR(EVR_OB);
      } else {
        DCMDATA_DEBUG("setting undefined VR of "
                      << tag.getTagName() << " " << tag
                      << " to 'OW' because WaveformBitsAllocated "
                      << DCM_WaveformBitsAllocated
                      << " has a value that is different from 8");
        tag.setVR(EVR_OW);
      }
    }
  } else if (((tag == DCM_PixelPaddingValue) ||
              (tag == DCM_PixelPaddingRangeLimit) ||
              (tag == DCM_HistogramFirstBinValue) ||
              (tag == DCM_HistogramLastBinValue)) &&
             (tag.getEVR() == EVR_xs)) {
    /* case 2 (PixelPaddingValue and others): see section C.7.5.1 and C.11.5 in
     * PS 3.3 */
    Uint16 pixelRep;
    if (item.findAndGetUint16(DCM_PixelRepresentation, pixelRep).good()) {
      if (pixelRep == 0x0001) {
        DCMDATA_DEBUG("setting undefined VR of "
                      << tag.getTagName() << " " << tag
                      << " to 'SS' because PixelRepresentation "
                      << DCM_PixelRepresentation << " has a value of 1");
        tag.setVR(EVR_SS);
      } else {
        DCMDATA_DEBUG("setting undefined VR of "
                      << tag.getTagName() << " " << tag
                      << " to 'US' because PixelRepresentation "
                      << DCM_PixelRepresentation
                      << " has a value that is different from 1");
        tag.setVR(EVR_US);
      }
    }
  } else if (((tag == DCM_OverlayData) || (tag == DCM_PixelData)) &&
             (tag.getEVR() == EVR_ox)) {
    /* case 3 (OverlayData and PixelData): see section 8.1.2 and 8.2 in PS 3.5
     */
    DCMDATA_DEBUG("setting undefined VR of " << tag.getTagName() << " " << tag
                                             << " to 'OW'");
    tag.setVR(EVR_OW);
...
void DcmItem::print(STD_NAMESPACE ostream& out, const size_t flags,
  ...
  if (flags & DCMTypes::PF_showTreeStructure) {
    ...
    if (getLengthField() == DCM_UndefinedLength)
      ...
      printInfoLine(out, flags, level, "(ItemDelimitationItem for re-encoding)",
                    &delimItemTag);
  }
}
...
OFCondition DcmItem::writeXML(STD_NAMESPACE ostream& out, const size_t flags) {
...
Uint32 DcmItem::calcElementLength(const E_TransferSyntax xfer,
  ...
  /* Since the item's total length can exceed the maximum length of 32 bit, it
   * is always necessary to check for overflows. The approach taken is not
   * elegant but should work...
   */
  if ((itemlen == DCM_UndefinedLength) ||
      OFStandard::check32BitAddOverflow(itemlen, headersize))
    return DCM_UndefinedLength;
  itemlen += xferSyn.sizeofTagHeader(getVR());
  if (enctype == EET_UndefinedLength)  // add bytes for closing item tag marker
                                       // if necessary
  {
...
Uint32 DcmItem::getLength(const E_TransferSyntax xfer,
  ...
  if (!elementList->empty()) {
    ...
    do {
      ...
      /* explicit length: be sure that total size of contained elements fits
         into item's 32 Bit length field. If not, switch encoding automatically
         to undefined length for this item. Nevertheless, any contained elements
         will be written with explicit length if possible.
       */
      if ((enctype == EET_ExplicitLength) &&
          OFStandard::check32BitAddOverflow(sublen, itemlen)) {
        if (dcmWriteOversizedSeqsAndItemsUndefined.get()) {
          DCMDATA_WARN(
              "DcmItem: Explicit length of item exceeds 32-Bit length field, "
...
OFCondition DcmItem::computeGroupLengthAndPadding(
    const E_GrpLenEncoding glenc, const E_PaddingEncoding padenc,
    const E_TransferSyntax xfer, const E_EncodingType enctype,
    const Uint32 padlen, const Uint32 subPadlen, Uint32 instanceLength) {
  ...
  /* if we get to this point, we need to do something. First of all, set the
   * error indicator to normal. */
  OFCondition l_error = EC_Normal;
  /* collects group length elements that cannot be calculated due to length
   * field overflows */
  ...
  if (!elementList->empty()) {
    ...
    /* determine the current seek mode and set the list pointer to the first
     * element */
    E_ListPos seekmode = ELP_next;
    elementList->seek(ELP_first);

    /* start a loop: we want to go through all elements as long as everything is
     * okay */
    do {
      /* set the seek mode to "next" again, in case it has been modified in the
       * last iteration */
      seekmode = ELP_next;
      ...
      /* if the current element is a sequence, compute group length and padding
       * for the sub sequence */
      if (dO->getVR() == EVR_SQ) {
        ...
        l_error =
            OFstatic_cast(DcmSequenceOfItems*, dO)
                ->computeGroupLengthAndPadding(glenc, padenc, xfer, enctype,
                                               subPadlen, subPadlen, templen);
      }

      /* if everything is ok so far */
      if (l_error.good()) {
        /* in case one of the following two conditions is met */
        /*  (i) the caller specified that we want to add or remove group length
         * elements and the current */
        /*      element's tag shows that it is a group length element (tag's
         * element number equals 0x0000) */
        /*  (ii) the caller specified that we want to add or remove padding
         * elements and the current */
        /*      element's tag shows that it is a padding element (tag is
         * (0xfffc,0xfffc) */
        ...
        if (((glenc == EGL_withGL || glenc == EGL_withoutGL) &&
          ...
          seekmode = ELP_atpos;  // remove advances 1 element forward -> make
                                 // next seek() work
          dO = NULL;
        }
        /* if the above mentioned conditions are not met but the caller
           specified that we want to add group */
        /* length tags for every group or that we want to recalculate values for
           existing group length tags */
        else if (glenc == EGL_withGL || glenc == EGL_recalcGL) {
          /* we need to determine the current element's group number */
          actGrp = dO->getGTag();

          /* and if the group number is different from the last remembered group
           * number or */
          /* if this id the very first element that is treated then we've found
           * a new group */
          if (actGrp != lastGrp || beginning)  // new Group found
          {
            ...
            /* if the current element is a group length element and its data
             * type */
            /* is not UL replace this element with one that has a UL datatype
             * since */
            ...
            if (dO->getETag() == 0x0000 && dO->ident() != EVR_UL) {
              ...
              dO = dUL;
              DCMDATA_WARN(
                  "DcmItem: Group Length with VR other than UL found, "
                  "corrected");
            }
            /* if the above mentioned condition is not met but the caller
               specified */
            /* that we want to add group length elements, we need to add such an
               element */
            ...
            /* in case we want to add padding elements and the current element
             * is a */
            /* padding element we want to remember the padding element so that
             * the */
            ...
            /* if actGLElem contains a valid pointer it was set in one of the
             * last iterations */
            /* to the group lenght element of the last group. We need to write
             * the current computed */
            /* group length value to this element. Exception: If group length
             * exceeds maximum possible */
            /* value, than remove group length element instead of setting it */
            if (actGLElem != NULL) {
              if (!groupLengthExceeded) {
                // do not use putUint32() in order to make sure that the
                // resulting VM is really 1
                actGLElem->putUint32Array(&grplen, 1);
                DCMDATA_DEBUG(
                    "DcmItem::computeGroupLengthAndPadding() Length of Group 0x"
                ...
                DCMDATA_WARN(
                    "DcmItem: Group length of group 0x"
                    << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
                    << STD_NAMESPACE setw(4) << actGLElem->getGTag()
            ...
            /* set the group length value to 0 since it is the beginning of the
             * new group */
            grplen = 0;

            /* if the current element is a group length element, remember its
             * address for later */
            /* (we need to assign the group length value to this element in a
             * subsequent iteration) */
            /* in case the current element (at the beginning of the group) is
             * not a group length */
          ...
          /* if this is not a new group, calculate the element's length and add
             it */
          /* to the currently computed group length value. If group length is
             larger */
          /* than group length field permits, set flag to not add group length
             for this group */
          else {
            sublen = dO->calcElementLength(xfer, enctype);
            // test for 32-bit overflow return value
            if ((sublen == DCM_UndefinedLength) ||
                OFStandard::check32BitAddOverflow(sublen, grplen)) {
          ...
          /* remember the current element's group number so that it is possible
           * to */
    ...
    /* if there was no error and the caller specified that we want to add or
     * recalculate */
    /* group length tags and if actGLElem has a valid value, we need to add the
     * above */
    /* computed group length value to the last group's group length element.
     * Exception: */
    /* If group length exceeds maximum possible value, remove group length
     * element and */
    /* i.e. do not write it for this group. */
    if (l_error.good() && (glenc == EGL_withGL || glenc == EGL_recalcGL) &&
        actGLElem) {
      if (groupLengthExceeded) {
        exceededGroupLengthElems.push_back(actGLElem);
      } else {
    ...
    if (padenc == EPD_withPadding && padlen) {
      /* calculate how much space the entire padding element is supposed to
       * occupy */
      Uint32 padding;
      ...
      /* if now padding does not equal padlen we need to create a padding
       * element. (if both values are equal */
      /* the element does have the exact required padlen length and does not
       * need a padding element.) */
      if (padding != padlen) {
         ...
         */
        /* need to increase padding (the value which specifies how much space
         * the */
        /* entire padding element is supposed to occupy) until it is no longer
         * smaller */
        ...
        /* set information in the above created padding element (size and actual
         * value) */
        ...
        /* finally we need to update the group length for the padding element if
         * it exists */
        if (paddingGL) {
          Uint32 len;
...
OFCondition DcmItem::readTagAndLength(DcmInputStream& inStream,
                                      const E_TransferSyntax xfer, DcmTag& tag,
  ...
  DCMDATA_TRACE("DcmItem::readTagAndLength() TransferSyntax=\""
                << xferSyn.getXferName() << "\"");
#endif
  ...
  /* check if either 4 (for implicit transfer syntaxes) or 6 (for explicit
   * transfer */
  /* syntaxes) bytes are available in (i.e. can be read from) inStream. if an
   * error */
  ...
  /* read tag information (4 bytes) from inStream and create a corresponding
   * DcmTag object */
  inStream.mark();
  ...
  /* if the transfer syntax which was passed is an explicit VR syntax and if the
   * current */
  /* item is not a delimitation item (note that delimitation items do not have a
   * VR), go */
  /* ahead and read 2 bytes from inStream. These 2 bytes contain this item's VR
   * value. */
  if (xferSyn.isExplicitVR() && newTag.getEVR() != EVR_na) {
    ...
    if (!vr.isStandard()) {
      ...
      oss << "DcmItem: Non-standard VR '"
          ...
          << "' (" << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
          << STD_NAMESPACE setw(2)
          << OFstatic_cast(unsigned int, vrstr[0] & 0xff) << "\\"
          << STD_NAMESPACE setw(2)
          << OFstatic_cast(unsigned int, vrstr[1] & 0xff)
          << ") encountered while parsing element " << newTag
          << OFStringStream_ends;
      OFSTRINGSTREAM_GETSTR(oss, tmpString)
      ...
      if (dcmAcceptUnexpectedImplicitEncoding.get()) {
        DCMDATA_WARN(tmpString
                     << ", trying again with Implicit VR Little Endian");
        /* put back read bytes to input stream ... */
        inStream.putback();
        bytesRead = 0;
        /* ... and retry with Implicit VR Little Endian transfer syntax */
        return readTagAndLength(inStream, EXS_LittleEndianImplicit, tag, length,
                                bytesRead);
      } else {
        DCMDATA_WARN(tmpString << ", assuming "
                               << (vr.usesExtendedLengthEncoding() ? "4" : "2")
                               << " byte length field");
      }
      OFSTRINGSTREAM_FREESTR(tmpString)
    }
  ...
  /* the next thing we want to do is read the value in the length field from
   * inStream. */
  /* determine if there is a corresponding amount of bytes (for the length
   * field) still */
  ...
  /* read the value in the length field. In some cases, it is 4 bytes wide, in
   * other */
  ...
  if (isPrivate && dcmReadImplPrivAttribMaxLengthAsSQ.get() &&
      (valueLength == DCM_UndefinedLength)) {
  ...
  if ((ident() == EVR_item /* e.g. meta info would have length 0 */) && (valueLengthItem != DCM_UndefinedLength /* this does not work in undefined length items */) &&
      (valueLength != DCM_UndefinedLength) /* Also, do not check sequences with
                                              undefined length 0xFFFFFFFF */
  ) {
    const offile_off_t remainingItemBytes =
        valueLengthItem - (inStream.tell() - fStartPosition);
    if (valueLength > remainingItemBytes) {
      DCMDATA_WARN("DcmItem: Element "
                   << newTag.getTagName() << " " << newTag << " larger ("
                   << valueLength
                   << ") than remaining bytes ("
                   /* need to cast remainingItemBytes to unsigned long because
                      VC6 cannot print offile_off_t (int64_t). */
...
OFCondition DcmItem::readSubElement(DcmInputStream& inStream, DcmTag& newTag,
  ...
  OFCondition l_error = newDicomElement(subElem, newTag, newLength,
                                        &privateCreatorCache, readAsUN);

  /* if no error occured and subElem does not equal NULL, go ahead */
  if (l_error.good() && subElem != NULL) {
    // inStream.UnsetPutbackMark(); // not needed anymore with new stream
    // architecture
    ...
    l_error =
        subElem->read(inStream, (readAsUN ? EXS_LittleEndianImplicit : xfer),
                      glenc, maxReadLength);
  ...
  /* else if an error occured, try to recover from this error */
    ...
    DCMDATA_WARN("DcmItem: Parse error in sequence item, found "
                 << newTag << " instead of an item delimiter");
  } else {
    // inStream.UnsetPutbackMark(); // not needed anymore with new stream
    // architecture
  }
...
OFCondition DcmItem::read(DcmInputStream& inStream, const E_TransferSyntax xfer,
                          const E_GrpLenEncoding glenc,
                          const Uint32 maxReadLength) {
  /* check if this is an illegal call; if so set the error flag and do nothing,
   * else go ahead */
  ...
  if (errorFlag.good() && inStream.eos())
    ...
    /* start a loop in order to read all elements (attributes) which are
     * contained in the inStream */
    while (inStream.good() &&
           (getTransferredBytes() < getLengthField() || !lastElementComplete) &&
           !readStopElem) {
      /* initialize variables */
      Uint32 newValueLength = 0;
      Uint32 bytes_tagAndLen = 0;
      /* if the reading of the last element was complete, go ahead and read the
       * next element */
      if (lastElementComplete) {
        ...
        if ((errorFlag == EC_ElemLengthLargerThanItem) &&
            dcmIgnoreParsingErrors.get()) {
          DCMDATA_WARN("DcmItem: Element "
                       << newTag.getTagName() << " " << newTag
                       << " too large, trying to skip over rest of item");
          /* we can call getLengthField because error does only occur for
        ...
        {
          /* if there was an error while we were reading from the stream,
           * terminate the while-loop */
          /* (note that if the last element had been read from the instream in
           * the last iteration, */
          /* another iteration will be started, and of course then
           * readTagAndLength(...) above will */
          /* return that it encountered the end of the stream. It is only then
           * (and here) when the */
          ...
          /* in case of implicit VR, check whether the "default VR" is really
           * appropriate */
          ...
          errorFlag = readSubElement(inStream, newTag, newValueLength, xfer,
                                     glenc, maxReadLength);
          /* if reading was successful, we read the entire data value
           * information */
        ...
        /* if lastElementComplete is false, we have only read the current
         * element's */
        /* tag and length (and possibly VR) information as well as maybe some
         * data */
        /* data value information. We need to continue reading the data value */
        /* information for this particular element. */
        errorFlag =
            elementList->get()->read(inStream, xfer, glenc, maxReadLength);
      ...
      setTransferredBytes(
          OFstatic_cast(Uint32, inStream.tell() - fStartPosition));
      if (errorFlag.good()) {
        ...
        if (lastElementComplete) {
          ...
          if ((dcmStopParsingAfterElement.get() != DCM_UndefinedTagKey) &&
              (dcmStopParsingAfterElement.get() ==
               elementList->get()->getTag()) &&
              ident() == EVR_dataset) {
            DCMDATA_WARN("DcmItem: Element "
                         << newTag.getTagName() << " " << newTag
    ...
    /* determine an appropriate result value; note that if the above called read
     * function */
    /* encountered the end of the stream before all information for this element
     * could be */
    /* read from the stream, the errorFlag has already been set to
     * EC_StreamNotifyClient. */
    if (errorFlag.good()) {
      // if stop element was read or end of stream occurs, tell parser end of
      // stream is reached
      if (readStopElem || inStream.eos())
        errorFlag = EC_EndOfStream;
      // if all bytes could be read or last element read could not be completed,
      // set to error
      else if ((getTransferredBytes() < getLengthField() ||
                !lastElementComplete))
        errorFlag = EC_StreamNotifyClient;
    }
  }  // else errorFlag
  /* modify the result value: two kinds of special error codes do not count as
   * an error */
      ...
      if (getTransferState() == ERW_inWork) {
        ...
        if (!elementList->empty() && (elementList->get() != NULL)) {
          ...
          do {
            ...
            if (dO->transferState() != ERW_ready)
              errorFlag =
                  dO->writeSignatureFormat(outStream, oxfer, enctype, wcache);
          } while (errorFlag.good() && elementList->seek(ELP_next));
        }
        if (errorFlag.good()) {
          setTransferState(ERW_ready);
          /* we don't write an item delimitation even if the item has undefined
           * length */
        }
      }
    }
...
OFCondition DcmItem::insert(DcmElement* elem, OFBool replaceOld,
  ...
  if (elem != NULL) {
    ...
    do {
      ...
      /* else if the current element and the new element show the same tag */
        ...
        if (elem != dE) {
          ...
          if (replaceOld) {
            ...
            if (remObj != NULL) {
              ...
              DCMDATA_TRACE("DcmItem::insert() Element "
                            << remObj->getTag() << " VR=\""
                            << DcmVR(remObj->getVR()).getVRName()
                            << "\" p=" << OFstatic_cast(void*, remObj)
                            << " removed and deleted");
              delete remObj;
            }
            ...
            DCMDATA_TRACE("DcmItem::insert() Element "
                          << elem->getTag() << " VR=\""
                          << DcmVR(elem->getVR()).getVRName()
                          << "\" p=" << OFstatic_cast(void*, elem)
                          << " replaced older one");
          }  // if (replaceOld)
          /* or else, i.e. the current element shall not be replaced by the new
             element */
          else {
        ...
        /* if the new and the current element are identical, the caller tries to
           insert */
...
DcmElement* DcmItem::remove(DcmObject* elem) {
  ...
  if (!elementList->empty() && elem != NULL) {
    ...
    do {
      ...
      if (dO == elem) {
        elementList
            ->remove();  // removes element from list but does not delete it
        errorFlag = EC_Normal;
        break;
      }
    } while (elementList->seek(ELP_next));
...
OFCondition DcmItem::verify1(const OFBool autocorrect) {
  DCMDATA_TRACE("DcmItem::verify1() Element "
                << getTag() << " VR=\"" << DcmVR(getVR()).getVRName() << "\"");
...
OFCondition DcmItem::search(const DcmTagKey& tag, DcmStack& resultStack,
                            E_SearchMode mode, OFBool searchIntoSub) {
  DcmObject* dO = NULL;
  OFCondition l_error = EC_TagNotFound;
  if (mode == ESM_afterStackTop && resultStack.top() == this) {
    ...
    if (mode == ESM_fromHere || resultStack.empty()) {
      ...
      if (dO == this)
        l_error = searchSubFromHere(tag, resultStack, searchIntoSub);
      else {  // gehe direkt zu Sub-Baum und suche dort weiter
        l_error = dO->search(tag, resultStack, mode, searchIntoSub);
        // The next two lines destroy the stack->so delete them
        //                if (l_error.bad()) // raeumt nur die oberste
        //                Stackebene
        //                    resultStack.pop();      // ab; der Rest ist
        //                    unveraendert
      }
      ...
      if (dO != this &&
          resultStack.card() > 0) {  // oberste Ebene steht nie in resultStack
      ...
      if (dO == this) {
        ...
        else                         //   siehe oben
        {
          ...
          do {
            ...
            if (!searchNode) {  // suche jetzt weiter
...
OFCondition newDicomElement(DcmElement*& newElement, const DcmTag& tag,
                            const Uint32 length) {
  DcmTag newTag(tag);
  OFBool readAsUN = OFFalse;
...
DcmElement* newDicomElement(const DcmTag& tag, const Uint32 length) {
...
OFCondition newDicomElement(DcmElement*& newElement, DcmTag& tag,
                            const Uint32 length,
  ...
  if ((evr == EVR_UN) && (length != DCM_UndefinedLength) &&
      dcmEnableUnknownVRConversion.get()) {
    /* look up VR in data dictionary */
    DcmTag newTag(tag.getGroup(), tag.getElement());

    /* special handling for private elements */
    if (privateCreatorCache && (newTag.getGroup() & 1) &&
        (newTag.getElement() >= 0x1000)) {
      const char* pc = privateCreatorCache->findPrivateCreator(newTag);
      if (pc != NULL) {
        // we have a private creator for this element
        newTag.setPrivateCreator(pc);
  ...
  switch (evr) {
    ...
    case EVR_ox:
      ...
      else if (((tag.getGTag() & 0xffe1) == 0x6000) &&
               (tag.getETag() == 0x3000))  // DCM_OverlayData
    ...
    case EVR_OW:
      if (tag == DCM_PixelData)
        newElement = new DcmPixelData(tag, length);
               ...
               (tag.getETag() == 0x3000))  // DCM_OverlayData
        newElement = new DcmOverlayData(tag, length);
      else if (length == DCM_UndefinedLength) {
    ...
    default:
      if (length == DCM_UndefinedLength) {
        ...
        // really a sequence so that we can catch the sequence delimitation
        // item.
        DcmVR sqVR(
            EVR_SQ);  // on writing we will handle this element as SQ, not UN
        DcmTag newTag(tag.getXTag(), sqVR);
        newElement =
            new DcmSequenceOfItems(newTag, length, dcmEnableCP246Support.get());
      } else {
...
/* --- findAndGet functions: find an element and get it or the value,
 * respectively --- */
...
OFCondition DcmItem::findAndGetUint8(const DcmTagKey& tagKey, Uint8& value,
                                     const unsigned long pos,
                                     const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetUint16(const DcmTagKey& tagKey, Uint16& value,
                                      const unsigned long pos,
                                      const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetSint16(const DcmTagKey& tagKey, Sint16& value,
                                      const unsigned long pos,
                                      const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetUint32(const DcmTagKey& tagKey, Uint32& value,
                                      const unsigned long pos,
                                      const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetSint32(const DcmTagKey& tagKey, Sint32& value,
                                      const unsigned long pos,
                                      const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetLongInt(const DcmTagKey& tagKey, long int& value,
                                       const unsigned long pos,
                                       const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetFloat32(const DcmTagKey& tagKey, Float32& value,
                                       const unsigned long pos,
                                       const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetFloat64(const DcmTagKey& tagKey, Float64& value,
                                       const unsigned long pos,
                                       const OFBool searchIntoSub) {
  DcmElement* elem;
  /* find the element */
...
OFCondition DcmItem::findAndGetSequenceItem(const DcmTagKey& seqTagKey,
  ...
  /* find sequence */
  OFCondition status =
      search(seqTagKey, stack, ESM_fromHere, OFFalse /*searchIntoSub*/);
  if (status.good()) {
    /* get element */
    DcmElement* delem = OFstatic_cast(DcmElement*, stack.top());
    if (delem != NULL) {
      ...
      if ((delem->ident() == EVR_SQ) || (delem->ident() == EVR_pixelSQ)) {
        DcmSequenceOfItems* sequence =
            OFstatic_cast(DcmSequenceOfItems*, delem);
        ...
        if (count > 0) {
          ...
          /* get specified item */
                   ...
                   (OFstatic_cast(unsigned long, itemNum) < count))
...
OFCondition DcmItem::findOrCreateSequenceItem(const DcmTag& seqTag,
  ...
  /* find sequence */
  OFCondition status =
      search(seqTag, stack, ESM_fromHere, OFFalse /*searchIntoSub*/);
  ...
  if (status.good()) {
    if (sequence != NULL) {
      const unsigned long count = sequence->card();
      /* existing item? */
      if ((count > 0) && (itemNum >= -1) &&
          (itemNum < OFstatic_cast(signed long, count))) {
        if (itemNum == -1) {
          /* get last item */
          item = sequence->getItem(count - 1);
        } else {
...
OFCondition DcmItem::findAndInsertCopyOfElement(const DcmTagKey& tagKey,
  ...
  if (destItem != NULL) {
    ...
    status = findAndGetElement(tagKey, delem, OFFalse /*searchIntoSub*/,
                               OFTrue /*createCopy*/);
    if (status.good()) {
...
OFCondition DcmItem::putAndInsertString(const DcmTag& tag, const char* value,
                                        const OFBool replaceOld) {
...
OFCondition DcmItem::putAndInsertUint16(const DcmTag& tag, const Uint16 value,
                                        const unsigned long pos,
                                        const OFBool replaceOld) {
  OFCondition status = EC_Normal;
  DcmElement* elem = NULL;
...
OFCondition DcmItem::putAndInsertSint16(const DcmTag& tag, const Sint16 value,
                                        const unsigned long pos,
                                        const OFBool replaceOld) {
  OFCondition status = EC_Normal;
  DcmElement* elem = NULL;
...
OFCondition DcmItem::putAndInsertUint32(const DcmTag& tag, const Uint32 value,
                                        const unsigned long pos,
                                        const OFBool replaceOld) {
...
OFCondition DcmItem::putAndInsertSint32(const DcmTag& tag, const Sint32 value,
                                        const unsigned long pos,
                                        const OFBool replaceOld) {
  OFCondition status = EC_IllegalCall;
  /* create new element */
...
OFCondition DcmItem::putAndInsertFloat32(const DcmTag& tag, const Float32 value,
                                         const unsigned long pos,
                                         const OFBool replaceOld) {
  OFCondition status = EC_IllegalCall;
  /* create new element */
...
OFCondition DcmItem::putAndInsertFloat64(const DcmTag& tag, const Float64 value,
                                         const unsigned long pos,
                                         const OFBool replaceOld) {
  OFCondition status = EC_IllegalCall;
  /* create new element */
...
OFCondition DcmItem::insertSequenceItem(const DcmTag& seqTag, DcmItem* item,
                                        const signed long itemNum) {
  OFCondition status = EC_IllegalParameter;
  if (item != NULL) {
    DcmStack stack;
      ...
      if (sequence != NULL) {
        ...
        if ((count > 0) && (itemNum >= -1) &&
          ...
          if (itemNum == -1) {
            ...
            status = sequence->insert(
                item, OFstatic_cast(unsigned long, itemNum), OFTrue /*before*/);
          }
          ...
          /* create empty trailing items if required */
          const unsigned long itemCount =
              (itemNum > OFstatic_cast(signed long, count)) ? (itemNum - count)
                                                            : 0;
          while ((i < itemCount) && status.good()) {
            newItem = new DcmItem();
            if (newItem != NULL) {
...
**   syntax, if the real VR is defined in the data dictionary. Default is
*OFFalse,
...
**   global boolean flag dcmAcceptOddAttributeLength, the parser now either
*accepts
...
**   of datasets containing attributes with value representation "ox" (= OB or
*OW)
...
**   truncates the original string (since it is not copied!). If you rely on
*this
