...
DcmStackNode::DcmStackNode(DcmObject* obj) : link(NULL), objNodeValue(obj) {}
...
DcmStack::DcmStack() : topNode_(NULL), cardinality_(0) {}

// ************************************************

DcmStack::DcmStack(const DcmStack& oldStack)
    : topNode_(NULL), cardinality_(oldStack.cardinality_) {
  if (cardinality_) {
...
OFBool DcmStack::operator<(const DcmStack& arg) const {
  if (cardinality_ < arg.cardinality_)
    return OFTrue;
  if (cardinality_ > arg.cardinality_)
    return OFFalse;
  ...
  while (thisPtr) {
    if (thisPtr->objNodeValue < argPtr->objNodeValue)
      return OFTrue;
    if (thisPtr->objNodeValue > argPtr->objNodeValue)
      return OFFalse;
