...
DcmRLECodecDecoder::DcmRLECodecDecoder() : DcmCodec() {}
...
OFCondition DcmRLECodecDecoder::decode(
  ...
  const DcmRLECodecParameter* djcp =
      OFstatic_cast(const DcmRLECodecParameter*, cp);
  ...
  if ((!dataset) ||
    ...
    if (result.good())
      result =
          ditem->findAndGetUint16(DCM_SamplesPerPixel, imageSamplesPerPixel);
    if (result.good())
    ...
    if (result.good() && (imageSamplesPerPixel > 1)) {
      result = ditem->findAndGetUint16(DCM_PlanarConfiguration,
    ...
    // present.
    if (result.good()) {
      if (ditem->findAndGetSint32(DCM_NumberOfFrames, imageFrames).good())
    ...
    if (result.good()) {
      ...
      if (rledecoder.fail())
        result = EC_MemoryExhausted;  // RLE decoder failed to initialize
      else {
        ...
        if (result.good()) {
          ...
          while ((currentFrame < imageFrames) && result.good()) {
            ...
            if (result.good()) {
              ...
              swapIfNecessary(gLocalByteOrder, EBO_LittleEndian, rleHeader,
                              16 * sizeof(Uint32), sizeof(Uint32));

              // determine number of stripes.
              numberOfStripes = rleHeader[0];

              // check that number of stripes in RLE header matches our
              // expectation
              if ((numberOfStripes < 1) || (numberOfStripes > 15) ||
                  (numberOfStripes !=
                   OFstatic_cast(Uint32, imageBytesAllocated) *
                       imageSamplesPerPixel))
                result = EC_CannotChangeRepresentation;
            }
            ...
            if (result.good()) {
              ...
              // this variable keeps the current position within the current
              // fragment
              Uint32 byteOffset = 0;
              ...
              for (i = 0; (i < numberOfStripes) && result.good(); ++i) {
                ...
                // adjust start point for RLE stripe, ignoring trailing garbage
                // from the last run
                ...
                if (byteOffset < fragmentOffset)
                  ...
                  byteOffset -=
                      fragmentOffset;  // now byteOffset is correct but may
                                       // point to next fragment
                  while ((byteOffset > fragmentLength) && result.good()) {
                    DCMDATA_DEBUG("RLE decoder processes pixel item "
                                  << currentItem);
                    result = pixSeq->getItem(pixItem, currentItem++);
                    if (result.good()) {
                ...
                if (i + 1 == numberOfStripes)
                  lastStripe = OFTrue;
                else
                  lastStripe = OFFalse;

                if (lastStripe) {
                  // the last stripe needs special handling because we cannot
                  // use the offset table to determine the number of bytes to
                  // feed to the codec if the RLE data is split in multiple
                  // fragments. We need to feed data fragment by fragment until
                  // the RLE codec has produced sufficient output.
                  while ((rledecoder.size() < bytesPerStripe) &&
                         result.good()) {
                    // feed complete remaining content of fragment to RLE codec
                    // and switch to next fragment
                    result = rledecoder.decompress(
                        rleData + byteOffset,
                        OFstatic_cast(size_t, fragmentLength - byteOffset));

                    // special handling for zero pad byte at the end of the RLE
                    // stream which results in an EC_StreamNotifyClient return
                    // code or trailing garbage data which results in
                    // EC_CorruptedData
                    if (rledecoder.size() == bytesPerStripe)
                      result = EC_Normal;

                    // Check if we're already done. If yes, don't change
                    // fragment
                    if (result.good() || result == EC_StreamNotifyClient) {
                      if (rledecoder.size() < bytesPerStripe) {
                        DCMDATA_WARN(
                            "RLE decoder is finished but has produced "
                            "insufficient data for this stripe, will continue "
                            "with next pixel item");
                        DCMDATA_DEBUG("RLE decoder processes pixel item "
                                      << currentItem);
                        result = pixSeq->getItem(pixItem, currentItem++);
                        if (result.good()) {
                  ...
                  // not the last stripe. We can use the offset table to
                  // determine the number of bytes to feed to the RLE codec.
                  inputBytes = rleHeader[i + 2];
                  if (inputBytes < rleHeader[i + 1])
                    result = EC_CannotChangeRepresentation;
                  else {
                    inputBytes -=
                        rleHeader[i + 1];  // number of bytes to feed to codec
                    while ((inputBytes > (fragmentLength - byteOffset)) &&
                           result.good()) {
                      // feed complete remaining content of fragment to RLE
                      // codec and switch to next fragment
                      result = rledecoder.decompress(
                          rleData + byteOffset,
                          OFstatic_cast(size_t, fragmentLength - byteOffset));
                    ...
                    result = rledecoder.decompress(
                        rleData + byteOffset,
                        OFstatic_cast(size_t, inputBytes));
                ...
                // make sure the RLE decoder has produced the right amount of
                // data
                if (result.good() && (rledecoder.size() != bytesPerStripe)) {
                  DCMDATA_ERROR(
                      "RLE decoder is finished but has produced insufficient "
                      "data for this stripe");
                  result = EC_CannotChangeRepresentation;
                }
                ...
                if (result.good()) {
                  ...
                  // raw buffer containing bytesPerStripe bytes of uncompressed
                  // data
                  outputBuffer =
                      OFstatic_cast(Uint8*, rledecoder.getOutputBuffer());

                  // compute byte offsets
                  if (imagePlanarConfiguration == 0) {
                    sampleOffset = sample * imageBytesAllocated;
                    offsetBetweenSamples =
                        imageSamplesPerPixel * imageBytesAllocated;
                  } else {
                    sampleOffset =
                  ...
                  if (enableReverseByteOrder) {
                    // assume incorrect LSB to MSB order of RLE segments as
                    // produced by some tools
                    pixelPointer = imageData8 + sampleOffset + byte;
                  } else {
                    pixelPointer = imageData8 + sampleOffset +
                                   imageBytesAllocated - byte - 1;
                  }
          ...
          swapIfNecessary(EBO_LittleEndian, gLocalByteOrder, imageData16,
                          totalSize, sizeof(Uint16));

          // Number of Frames might have changed in case the previous value was
          // wrong
          if (result.good() && (numberOfFramesPresent || (imageFrames > 1))) {
            char numBuf[20];
            sprintf(numBuf, "%ld", OFstatic_cast(long, imageFrames));
            result = OFstatic_cast(DcmItem*, dataset)
    ...
    if (dataset->ident() == EVR_dataset) {
      ...
      if (result.good() && djcp->getUIDCreation())
        result = DcmCodec::newInstance(OFstatic_cast(DcmItem*, dataset), NULL,
                                       NULL, NULL);
    }
  }
  return result;
...
OFCondition DcmRLECodecDecoder::decodeFrame(
    ...
    Uint32 frameNo, Uint32& startFragment, void* buffer, Uint32 bufSize,
  ...
  Uint32 i;
  Uint32 frameSize =
      imageBytesAllocated * imageRows * imageColumns * imageSamplesPerPixel;
  ...
  DcmRLEDecoder rledecoder(bytesPerStripe);
  if (rledecoder.fail())
    return EC_MemoryExhausted;  // RLE decoder failed to initialize
  ...
  if ((numberOfStripes < 1) || (numberOfStripes > 15) ||
      (numberOfStripes !=
       OFstatic_cast(Uint32, imageBytesAllocated) * imageSamplesPerPixel))
    return EC_CannotChangeRepresentation;
  ...
  for (i = 0; i < numberOfStripes; ++i) {
    ...
    if (lastStripe && (rledecoder.size() < bytesPerStripe)) {
      ...
      if (result == EC_StreamNotifyClient) {
        DCMDATA_WARN(
            "RLE decoder is finished but has produced insufficient data for "
            "this stripe, filling remaining pixels");
        result = EC_Normal;
      }
    } else if (rledecoder.size() != bytesPerStripe) {
      DCMDATA_ERROR(
          "RLE decoder is finished but has produced insufficient data for this "
          "stripe");
      return EC_CannotChangeRepresentation;
    }
    ...
    // initialize pointer to output data
    if (enableReverseByteOrder) {
      // assume incorrect LSB to MSB order of RLE segments as produced by some
      // tools
      pixelPointer = imageData8 + sampleOffset + byte;
    } else {
  ...
  // adjust byte order for uncompressed image to little endian
  swapIfNecessary(EBO_LittleEndian, gLocalByteOrder, imageData16, frameSize,
                  sizeof(Uint16));
