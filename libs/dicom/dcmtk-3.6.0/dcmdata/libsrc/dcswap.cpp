...
OFCondition swapIfNecessary(const E_ByteOrder newByteOrder,
                            const E_ByteOrder oldByteOrder, void* value,
                            const Uint32 byteLength, const size_t valWidth)
/*
 * This function swaps byteLength bytes in value if newByteOrder and
 * oldByteOrder differ from each other. In case bytes have to be swapped, these
 * bytes are seperated in valWidth elements which will be swapped seperately.
 *
 * Parameters:
 ...
 *   value        - [in] Array that contains the actual bytes which might have
 * to be swapped. byteLength   - [in] Length of the above array. valWidth     -
 * [in] Specifies how many bytes shall be treated together as one element.
 */
{
  ...
  if (oldByteOrder != EBO_unknown && newByteOrder != EBO_unknown) {
    ...
    if (oldByteOrder != newByteOrder && valWidth != 1) {
      /* in case the array length equals valWidth and only 2 or 4 bytes have to
       * be swapped */
      /* we can swiftly swap these bytes by calling the corresponding functions.
       * If this is */
...
void swapBytes(void* value, const Uint32 byteLength, const size_t valWidth)
/*
 ...
 */
{
  /* use (if available) to increase speed */
  Uint8 save;
