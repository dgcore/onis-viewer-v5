...
void DcmWriteCache::init(void* owner, Uint32 fieldLength,
                         Uint32 bytesTransferred, E_ByteOrder byteOrder) {
  if (!buf_) {
...
OFCondition DcmWriteCache::fillBuffer(DcmElement& elem) {
  ...
  if (buf_) {
    ...
    if (!numBytes_) {
      ...
      if (bytesToRead > capacity_)
        bytesToRead = capacity_;

      result = elem.getPartialValue(buf_, fieldOffset_, bytesToRead, &fcache_,
                                    byteOrder_);
