...
DcmZLibOutputFilter::~DcmZLibOutputFilter() {
  if (zstream_) {
    deflateEnd(zstream_);  // discards any unprocessed input and does not flush
                           // any pending output
    delete zstream_;
  }
  delete[] inputBuf_;
...
OFBool DcmZLibOutputFilter::isFlushed() const {
  if (status_.bad() || (current_ == NULL))
    return OFTrue;
  return (inputBufCount_ == 0) && (outputBufCount_ == 0) && flushed_ &&
         current_->isFlushed();
}
...
offile_off_t DcmZLibOutputFilter::avail() const {
  ...
  if (status_.good())
    return DCMZLIBOUTPUTFILTER_BUFSIZE - inputBufCount_;
  else
    return 0;
}
...
void DcmZLibOutputFilter::flushOutputBuffer() {
  if (outputBufCount_) {
    // flush from outputBufStart_ to end of data or end of buffer, whatever
    // comes first
    offile_off_t numBytes =
        (outputBufStart_ + outputBufCount_ > DCMZLIBOUTPUTFILTER_BUFSIZE)
            ? (DCMZLIBOUTPUTFILTER_BUFSIZE - outputBufStart_)
            : outputBufCount_;

    offile_off_t written =
        current_->write(outputBuf_ + outputBufStart_, numBytes);
...
offile_off_t DcmZLibOutputFilter::fillInputBuffer(const void* buf,
  ...
  if (buf && buflen && inputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) {
    ...
    if (inputBufStart_ + inputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) {
      ...
      memcpy(inputBuf_ + inputBufStart_ + inputBufCount_, data,
             OFstatic_cast(size_t, result));
    ...
    if (buflen && (inputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) &&
      ...
      memcpy(inputBuf_ + (inputBufStart_ + inputBufCount_ -
                          DCMZLIBOUTPUTFILTER_BUFSIZE),
             data, OFstatic_cast(size_t, len));
...
void DcmZLibOutputFilter::compressInputBuffer(OFBool finalize) {
  if (inputBufCount_ || finalize) {
    // flush from inputBufStart_ to end of data or end of buffer, whatever comes
    // first
    offile_off_t numBytes =
        (inputBufStart_ + inputBufCount_ > DCMZLIBOUTPUTFILTER_BUFSIZE)
            ? (DCMZLIBOUTPUTFILTER_BUFSIZE - inputBufStart_)
            : inputBufCount_;

    offile_off_t written =
        compress(inputBuf_ + inputBufStart_, numBytes, finalize);
...
offile_off_t DcmZLibOutputFilter::compress(const void* buf, offile_off_t buflen,
                                           OFBool finalize) {
  offile_off_t result = 0;
  if (outputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) {
    ...
    if (outputBufStart_ + outputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) {
      zstream_->next_out =
          OFstatic_cast(Bytef*, outputBuf_ + outputBufStart_ + outputBufCount_);
      zstream_->avail_out =
          OFstatic_cast(uInt, DCMZLIBOUTPUTFILTER_BUFSIZE -
                                  (outputBufStart_ + outputBufCount_));
      ...
      outputBufCount_ = DCMZLIBOUTPUTFILTER_BUFSIZE - outputBufStart_ -
                        OFstatic_cast(offile_off_t, zstream_->avail_out);
    }

    // use second part of output buffer
    if ((outputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) &&
        outputBufStart_ + outputBufCount_ >= DCMZLIBOUTPUTFILTER_BUFSIZE) {
      zstream_->next_out = OFstatic_cast(
          Bytef*, outputBuf_ + (outputBufStart_ + outputBufCount_ -
                                DCMZLIBOUTPUTFILTER_BUFSIZE));
      zstream_->avail_out =
          OFstatic_cast(uInt, DCMZLIBOUTPUTFILTER_BUFSIZE - outputBufCount_);
      zstatus = deflate(zstream_, (finalize ? Z_FINISH : 0));
      ...
      outputBufCount_ = DCMZLIBOUTPUTFILTER_BUFSIZE -
                        OFstatic_cast(offile_off_t, zstream_->avail_out);
    }
...
offile_off_t DcmZLibOutputFilter::write(const void* buf, offile_off_t buflen) {
  ...
  while (status_.good() && inputBufCount_ > 0 &&
         outputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) {
  ...
  if (inputBufCount_ == 0) {
    while (status_.good() && (buflen > result) &&
           outputBufCount_ < DCMZLIBOUTPUTFILTER_BUFSIZE) {
