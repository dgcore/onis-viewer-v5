...
DcmFloatingPointDouble::DcmFloatingPointDouble(
    const DcmFloatingPointDouble& old)
    : DcmElement(old) {}

DcmFloatingPointDouble::~DcmFloatingPointDouble() {}

DcmFloatingPointDouble& DcmFloatingPointDouble::operator=(
    const DcmFloatingPointDouble& obj) {
    ...
    if (doubleVals != NULL) {
      ...
      const unsigned long maxLength =
          (flags & DCMTypes::PF_shortenLongTagValues)
              ...
              : OFstatic_cast(unsigned long, -1) /*unlimited*/;
      ...
      for (unsigned int i = 0; i < count; i++, doubleVals++) {
        ...
        if (i == 0)
          OFStandard::ftoa(buffer, sizeof(buffer), *doubleVals, 0, 0,
                           17 /* DBL_DIG + 2 for DICOM FD */);
        else {
          buffer[0] = '\\';
          OFStandard::ftoa(buffer + 1, sizeof(buffer) - 1, *doubleVals, 0, 0,
                           17 /* DBL_DIG + 2 for DICOM FD */);
        }
...
OFCondition DcmFloatingPointDouble::getOFString(OFString& stringVal,
  ...
  if (errorFlag.good()) {
    ...
    char buffer[64];
    OFStandard::ftoa(buffer, sizeof(buffer), doubleVal, 0, 0,
                     17 /* DBL_DIG + 2 for DICOM FD */);
    /* assign result */
    stringVal = buffer;
  }
...
OFCondition DcmFloatingPointDouble::putFloat64Array(
    const Float64* doubleVals, const unsigned long numDoubles) {
  errorFlag = EC_Normal;
  if (numDoubles > 0) {
    /* check for valid data */
    if (doubleVals != NULL)
      errorFlag = putValue(doubleVals,
                           sizeof(Float64) * OFstatic_cast(Uint32, numDoubles));
    else
...
OFCondition DcmFloatingPointDouble::verify1(const OFBool autocorrect) {
