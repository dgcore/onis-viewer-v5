...
OFString DcmTagKey::toString() const {
  ...
  if (group == 0xffff && element == 0xffff) {
    strcpy(tagBuf, "(\?\?\?\?,\?\?\?\?)");  // prevent trigraph expansion in
                                            // string constant
  } else {
    sprintf(tagBuf, "(%04x,%04x)", OFstatic_cast(unsigned, group),
            OFstatic_cast(unsigned, element));
  }
  return tagBuf;
}
...
OFBool DcmTagKey::isSignableTag() const {
  ...
  if ((group == 0x0008) && (element == 0x0001))
    return OFFalse;
  ...
  if ((group == 0x4ffe) && (element == 0x0001))
    return OFFalse;
