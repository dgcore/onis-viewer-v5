...
DcmRepresentationEntry::DcmRepresentationEntry(
    const E_TransferSyntax rt, const DcmRepresentationParameter* rp,
    DcmPixelSequence* ps)
    : repType(rt), repParam(NULL), pixSeq(ps) {
  if (rp)
    repParam = rp->clone();
}
...
DcmRepresentationEntry::DcmRepresentationEntry(
    ...
    : repType(oldEntry.repType), repParam(NULL), pixSeq(NULL) {
...
OFBool DcmRepresentationEntry::operator==(
...
DcmPixelData::DcmPixelData(const DcmTag& tag, const Uint32 len)
  ...
  if (getTag().getEVR() == EVR_ox)
    setTagVR(EVR_OW);
  unencapsulatedVR = getTag().getEVR();
  recalcVR();
}

DcmPixelData::DcmPixelData(const DcmPixelData& oldPixelData)
...
DcmPixelData::~DcmPixelData() {
  for (DcmRepresentationListIterator it(repList.begin()); it != repListEnd;
       ++it) {
    delete *it;
    *it = NULL;
  }
...
Uint32 DcmPixelData::calcElementLength(const E_TransferSyntax xfer,
...
OFBool DcmPixelData::canChooseRepresentation(
  ...
  if ((!toType.isEncapsulated() && existUnencapsulated) ||
      (toType.isEncapsulated() && alwaysUnencapsulated &&
       existUnencapsulated) ||
    ...
    if (original == repListEnd) {
      result = DcmCodecList::canChangeCoding(EXS_LittleEndianExplicit,
                                             toType.getXfer());
      ...
      if (!result) {
        // direct transcoding is not possible. Check if we can decode and then
        // encode.
      ...
      result = DcmCodecList::canChangeCoding((*original)->repType,
...
OFBool DcmPixelData::canWriteXfer(const E_TransferSyntax newXfer,
                                  const E_TransferSyntax /*oldXfer*/) {
  DcmXfer newXferSyn(newXfer);
  DcmRepresentationListIterator found;
  OFBool result = existUnencapsulated &&
                  (!newXferSyn.isEncapsulated() || alwaysUnencapsulated);

  if (!result && newXferSyn.isEncapsulated())
    result = (findConformingEncapsulatedRepresentation(newXferSyn, NULL,
                                                       found) == EC_Normal);
  return result;
}

OFCondition DcmPixelData::chooseRepresentation(
    const E_TransferSyntax repType, const DcmRepresentationParameter* repParam,
    DcmStack& pixelStack) {
  ...
  if ((!toType.isEncapsulated() && existUnencapsulated) ||
      (toType.isEncapsulated() && existUnencapsulated &&
    ...
    if (original == repListEnd)
      l_error = encode(EXS_LittleEndianExplicit, NULL, NULL, toType, repParam,
                       pixelStack);
    else if (toType.isEncapsulated())
...
OFCondition DcmPixelData::decode(const DcmXfer& fromType,
  ...
  if (existUnencapsulated)
    return EC_Normal;
  OFCondition l_error =
      DcmCodecList::decode(fromType, fromParam, fromPixSeq, *this, pixelStack);
  if (l_error.good()) {
...
OFCondition DcmPixelData::encode(const DcmXfer& fromType,
                                 const DcmRepresentationParameter* fromParam,
                                 DcmPixelSequence* fromPixSeq,
  ...
  if (toType.isEncapsulated()) {
    ...
    if (fromType.isEncapsulated()) {
      l_error =
          DcmCodecList::encode(fromType.getXfer(), fromParam, fromPixSeq,
      ...
      if (l_error == EC_Normal) {
        l_error = DcmCodecList::encode(fromType.getXfer(), pixelData, length,
                                       toType.getXfer(), toParam, toPixSeq,
                                       pixelStack);
      }
    }
    ...
    if (l_error.bad() && fromType.isEncapsulated()) {
      ...
      if (l_error.good())
        l_error = encode(EXS_LittleEndianExplicit, NULL, NULL, toType, toParam,
                         pixelStack);
    }
  }
  return l_error;
...
OFCondition DcmPixelData::findRepresentationEntry(
    const DcmRepresentationEntry& findEntry,
    DcmRepresentationListIterator& result) {
  result = repList.begin();
  while (result != repListEnd && (*result)->repType < findEntry.repType)
    ++result;
...
OFCondition DcmPixelData::findConformingEncapsulatedRepresentation(
    const DcmXfer& repTypeSyn, const DcmRepresentationParameter* repParam,
    DcmRepresentationListIterator& result) {
  ...
  if (repTypeSyn.isEncapsulated()) {
    ...
    if ((current != repListEnd) && ((*current)->repType == repType) &&
        ((repParam == NULL) || (((*current)->repParam != NULL) &&
      ...
      while (!found && (it != repListEnd)) {
        if ((*it)->repType == repType) {
          if ((repParam == NULL) ||
              (((*it)->repParam != NULL) && (*(*it)->repParam == *repParam))) {
...
OFCondition DcmPixelData::getEncapsulatedRepresentation(
    const E_TransferSyntax repType, const DcmRepresentationParameter* repParam,
...
OFBool DcmPixelData::hasRepresentation(
    const E_TransferSyntax repType,
    const DcmRepresentationParameter* repParam) {
  ...
  if (!repTypeSyn.isEncapsulated() && existUnencapsulated)
    ...
    return findConformingEncapsulatedRepresentation(repTypeSyn, repParam, found)
        .good();
  return OFFalse;
}

Uint32 DcmPixelData::getLength(const E_TransferSyntax xfer,
                               const E_EncodingType enctype) {
  DcmXfer xferSyn(xfer);
  errorFlag = EC_Normal;
  ...
  if (xferSyn.isEncapsulated() && !alwaysUnencapsulated) {
    ...
    errorFlag =
        findConformingEncapsulatedRepresentation(xferSyn, NULL, foundEntry);
    if (errorFlag == EC_Normal)
...
void DcmPixelData::getCurrentRepresentationKey(
    E_TransferSyntax& repType, const DcmRepresentationParameter*& repParam) {
...
void DcmPixelData::getOriginalRepresentationKey(
    E_TransferSyntax& repType, const DcmRepresentationParameter*& repParam) {
  if (original != repListEnd) {
...
DcmRepresentationListIterator DcmPixelData::insertRepresentationEntry(
...
void DcmPixelData::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmPixelData::putUint8Array(const Uint8* byteValue,
...
OFCondition DcmPixelData::putUint16Array(const Uint16* wordValue,
                                         const unsigned long length) {
  // clear RepresentationList
  clearRepresentationList(repListEnd);
...
OFCondition DcmPixelData::createUint8Array(const Uint32 numBytes,
                                           Uint8*& bytes) {
...
OFCondition DcmPixelData::createUint16Array(const Uint32 numWords,
                                            Uint16*& words) {
...
OFCondition DcmPixelData::createValueFromTempFile(
    DcmInputStreamFactory* factory, const Uint32 length,
    const E_ByteOrder byteOrder) {
  OFCondition l_error =
      DcmPolymorphOBOW::createValueFromTempFile(factory, length, byteOrder);
  existUnencapsulated = OFTrue;
  return l_error;
}

void DcmPixelData::putOriginalRepresentation(
    const E_TransferSyntax repType, const DcmRepresentationParameter* repParam,
    DcmPixelSequence* pixSeq) {
...
OFCondition DcmPixelData::read(DcmInputStream& inStream,
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    /* if the transfer state is ERW_init, we need to prepare the reading of the
     * pixel */
    /* data from the stream: remove all representations from the representation
     * list. */
    ...
    /* determine if the pixel data is captured in native (uncompressed) or
     * encapsulated (compressed) format.  We only derive this information from
     * the length field which is set to undefined length for encapsulated data
     * because even in compressed transfer syntaxes the Icon Image Sequence may
     * contain an uncompressed image.
     */
    if (getLengthField() == DCM_UndefinedLength) {
      ...
      if (getTransferState() == ERW_init) {
        current = insertRepresentationEntry(new DcmRepresentationEntry(
      ...
      /* conduct the reading process */
      errorFlag = DcmPolymorphOBOW::read(inStream, ixfer, glenc, maxReadLength);
    }
  }
...
void DcmPixelData::removeAllButOriginalRepresentations() {
...
OFCondition DcmPixelData::removeOriginalRepresentation(
    const E_TransferSyntax repType,
    const DcmRepresentationParameter* repParam) {
...
OFCondition DcmPixelData::removeRepresentation(
    const E_TransferSyntax repType,
    const DcmRepresentationParameter* repParam) {
  OFCondition l_error = EC_Normal;
  DcmXfer repTypeSyn(repType);
...
OFCondition DcmPixelData::setCurrentRepresentationParameter(
    const DcmRepresentationParameter* repParam) {
  if (current != repListEnd) {
    if (repParam == NULL)
...
OFCondition DcmPixelData::setVR(DcmEVR vr) {
  unencapsulatedVR = vr;
  return DcmPolymorphOBOW::setVR(vr);
}

void DcmPixelData::transferEnd() {
...
void DcmPixelData::transferInit() {
...
OFCondition DcmPixelData::write(DcmOutputStream& outStream,
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    if (xferSyn.isEncapsulated() && (!alwaysUnencapsulated)) {
      if (getTransferState() == ERW_init) {
        DcmRepresentationListIterator found;
        errorFlag =
            findConformingEncapsulatedRepresentation(xferSyn, NULL, found);
        if (errorFlag == EC_Normal) {
          current = found;
          recalcVR();
      ...
      if (errorFlag == EC_Normal && pixelSeqForWrite)
        errorFlag = pixelSeqForWrite->write(outStream, oxfer, enctype, wcache);
      if (errorFlag == EC_Normal)
        setTransferState(ERW_ready);
    } else if (existUnencapsulated) {
      current = repListEnd;
...
OFCondition DcmPixelData::writeXML(STD_NAMESPACE ostream& out,
                                   const size_t flags) {
  if (current == repListEnd) {
...
OFCondition DcmPixelData::writeSignatureFormat(DcmOutputStream& outStream,
                                               const E_TransferSyntax oxfer,
                                               const E_EncodingType enctype,
                                               DcmWriteCache* wcache) {
  errorFlag = EC_Normal;
  if (getTransferState() == ERW_notInitialized)
    ...
    if (xferSyn.isEncapsulated() && (!alwaysUnencapsulated)) {
      ...
      if (errorFlag == EC_Normal && pixelSeqForWrite)
        errorFlag = pixelSeqForWrite->writeSignatureFormat(outStream, oxfer,
                                                           enctype, wcache);
      if (errorFlag == EC_Normal)
        setTransferState(ERW_ready);
    } else if (existUnencapsulated) {
      current = repListEnd;
      recalcVR();
      errorFlag = DcmPolymorphOBOW::writeSignatureFormat(outStream, oxfer,
                                                         enctype, wcache);
    } else if (getValue() == NULL) {
      errorFlag = DcmPolymorphOBOW::writeSignatureFormat(outStream, oxfer,
                                                         enctype, wcache);
...
OFCondition DcmPixelData::getUncompressedFrame(DcmItem* dataset, Uint32 frameNo,
  ...
  dataset->findAndGetSint32(DCM_NumberOfFrames,
                            numberOfFrames);  // don't fail if absent
  if (numberOfFrames < 1)
    numberOfFrames = 1;
  ...
  // determine the minimum buffer size, which may be frame size plus one pad
  // byte if frame size is odd. We need this extra byte, because the image might
  // be in a different endianness than our host cpu. In this case the decoder
  // will swap the data to the host byte order which could overflow the buffer.
  ...
  // check frame number
  if (frameNo >= OFstatic_cast(Uint32, numberOfFrames))
    return EC_IllegalCall;
  ...
  if (existUnencapsulated) {
    ...
    result = DcmCodecList::decodeFrame(
        ...
        dataset, frameNo, startFragment, buffer, bufSize,
        decompressedColorModel);
  }
  return result;
}
...
OFCondition DcmPixelData::getDecompressedColorModel(
  ...
  if (dataset != NULL) {
    if (existUnencapsulated) {
      // we already have an uncompressed version of the pixel data either in
      // memory or in file, so just retrieve the color model from the given
      // dataset
      result = dataset->findAndGetOFString(DCM_PhotometricInterpretation,
                                           decompressedColorModel);
    } else {
