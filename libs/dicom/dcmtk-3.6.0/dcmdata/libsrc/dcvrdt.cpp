...
DcmDateTime::DcmDateTime(const DcmDateTime& old) : DcmByteString(old) {}
...
OFCondition DcmDateTime::getISOFormattedDateTime(
    OFString& formattedDateTime, const unsigned long pos, const OFBool seconds,
    const OFBool fraction, const OFBool timeZone,
    const OFBool createMissingPart) {
  /* call the real function, required to make Sun CC 2.0.1 happy (see header
   * file) */
  return getISOFormattedDateTime(formattedDateTime, pos, seconds, fraction,
                                 timeZone, createMissingPart,
                                 " " /*dateTimeSeparator*/);
}
...
OFCondition DcmDateTime::getISOFormattedDateTime(
    OFString& formattedDateTime, const unsigned long pos, const OFBool seconds,
    const OFBool fraction, const OFBool timeZone,
    const OFBool createMissingPart, const OFString& dateTimeSeparator) {
  OFString dicomDateTime;
  OFCondition l_error = getOFString(dicomDateTime, pos);
  if (l_error.good()) {
    l_error = getISOFormattedDateTimeFromString(
        dicomDateTime, formattedDateTime, seconds, fraction, timeZone,
...
OFCondition DcmDateTime::setCurrentDateTime(const OFBool seconds,
  ...
  OFString dicomDateTime;
  OFCondition l_error =
      getCurrentDateTime(dicomDateTime, seconds, fraction, timeZone);
  if (l_error.good())
...
OFCondition DcmDateTime::setOFDateTime(const OFDateTime& dateTimeValue) {
  ...
  OFCondition l_error =
      getDicomDateTimeFromOFDateTime(dateTimeValue, dicomDateTime);
  if (l_error.good())
    l_error = putString(dicomDateTime.c_str());
  return l_error;
}
...
OFCondition DcmDateTime::getCurrentDateTime(OFString& dicomDateTime,
  ...
  if (dateTimeValue.setCurrentDateTime()) {
    ...
    if (dateTimeValue.getISOFormattedDateTime(dicomDateTime, seconds, fraction,
                                              timeZone,
                                              OFFalse /*showDelimiter*/))
      l_error = EC_Normal;
  }
...
OFCondition DcmDateTime::getDicomDateTimeFromOFDateTime(
    const OFDateTime& dateTimeValue, OFString& dicomDateTime,
    const OFBool seconds, const OFBool fraction, const OFBool timeZone) {
...
OFCondition DcmDateTime::getOFDateTimeFromString(const OFString& dicomDateTime,
  ...
  if (dicomDateTime.length() >= 8) {
     ...
     */
    if (DcmTime::getTimeZoneFromString(
            dicomDateTime.substr(dicomDateTime.length() - 5), timeZone)
            .good())
    ...
    /* extract remaining components from date/time string:
     * YYYYMMDDHHMM[SS[.FFFFFF]] */
    /* scan seconds using OFStandard::atof to avoid locale issues */
    if (sscanf(string.c_str(), "%04u%02u%02u%02u%02u", &year, &month, &day,
               &hour, &minute) >= 3) {
      if (string.length() > 12) {
        string.erase(0, 12);
        second = OFStandard::atof(string.c_str());
      }
      if (dateTimeValue.setDateTime(year, month, day, hour, minute, second,
                                    timeZone))
        l_error = EC_Normal;
    }
  }
...
OFCondition DcmDateTime::getISOFormattedDateTimeFromString(
    const OFString& dicomDateTime, OFString& formattedDateTime,
    const OFBool seconds, const OFBool fraction, const OFBool timeZone,
    const OFBool createMissingPart) {
  return getISOFormattedDateTimeFromString(
      dicomDateTime, formattedDateTime, seconds, fraction, timeZone,
      createMissingPart, " " /*dateTimeSeparator*/);
}
...
OFCondition DcmDateTime::getISOFormattedDateTimeFromString(
  ...
  if (length >= 8) {
    ...
    l_error = DcmDate::getOFDateFromString(
        dicomDateTime.substr(0, 8), dateValue, OFFalse /*supportOldFormat*/);
    if (l_error.good()) {
      ...
      OFString dicomTime = (posSign != OFString_npos)
                               ? dicomDateTime.substr(8, posSign - 8)
                               : dicomDateTime.substr(8);
      l_error = DcmTime::getISOFormattedTimeFromString(
          dicomTime, timeString, seconds, fraction, createMissingPart,
          OFFalse /*supportOldFormat*/);
      if (l_error.good()) {
...
OFCondition DcmDateTime::checkStringValue(const OFString& value,
  ...
  if (valLen > 0) {
    ...
    while (posStart != OFString_npos) {
      ...
      const size_t length =
          (posEnd == OFString_npos) ? valLen - posStart : posEnd - posStart;
