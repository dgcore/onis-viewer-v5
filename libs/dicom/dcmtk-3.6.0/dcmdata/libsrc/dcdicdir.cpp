...
DcmDataset& DcmDicomDir::getDataset() {
  ...
  if (localDataset == NULL) {
    ...
    DCMDATA_ERROR(
        "DcmDicomDir::getDataset() Missing Dataset in DICOMDIR File. Must "
...
DcmSequenceOfItems& DcmDicomDir::getDirRecSeq(DcmDataset& dset) {
  ...
  DcmStack stack;
  if (dset.search(DCM_DirectoryRecordSequence, stack, ESM_fromHere, OFFalse) ==
      EC_Normal) {
  ...
  if (localDirRecSeq == NULL) {
    ...
    if (!mustCreateNewDir) {
      DCMDATA_WARN(
          "DcmDicomDir::getDirRecSeq() Missing Directory Record Sequence. Must "
          "create new one.");
    }
...
DcmUnsignedLongOffset* DcmDicomDir::lookForOffsetElem(
    DcmObject* obj, const DcmTagKey& offsetTag) {
    ...
    if (obj->search(offsetTag, stack, ESM_fromHere, OFFalse) == EC_Normal) {
      if (stack.top()->ident() == EVR_up) {
        ...
        DCMDATA_TRACE("DcmDicomDir::lookForOffsetElem() Offset Element ("
                      ...
                      << STD_NAMESPACE setw(4) << offElem->getETag()
                      << ") offs=0x" << STD_NAMESPACE setw(8) << l_uint
...
OFCondition DcmDicomDir::resolveGivenOffsets(DcmObject* startPoint,
  ...
  if (startPoint != NULL) {
    DcmStack stack;
    Uint32 offset;
    while (startPoint->search(offsetTag, stack, ESM_afterStackTop, OFTrue) ==
           EC_Normal) {
      if (stack.top()->ident() != EVR_up)
        continue;
      DcmUnsignedLongOffset* offElem =
          OFstatic_cast(DcmUnsignedLongOffset*, stack.top());
      for (unsigned long i = 0; i < numOffsets; i++) {
        ...
        if (offset == itOffsets[i].fileOffset) {
          DCMDATA_DEBUG("DcmDicomDir::resolveGivenOffset() Offset Element ("
                        ...
                        << STD_NAMESPACE setw(4) << offElem->getGTag() << ","
                        << STD_NAMESPACE setw(4) << offElem->getETag()
                        << ") with offset 0x" << STD_NAMESPACE setw(8) << offset
                        << " found");
...
OFCondition DcmDicomDir::resolveAllOffsets(DcmDataset& dset)  // inout
{
  ...
  for (unsigned long i = 0; i < maxitems; i++) {
    ...
    DCMDATA_DEBUG("DcmDicomDir::resolveAllOffsets() Item Offset ["
                  << i << "] = 0x" << STD_NAMESPACE hex
                  << STD_NAMESPACE setfill('0') << STD_NAMESPACE setw(8)
                  << filePos);
  }
  resolveGivenOffsets(
      &dset, itOffsets, maxitems,
      DCM_OffsetOfTheFirstDirectoryRecordOfTheRootDirectoryEntity);
  resolveGivenOffsets(
      &dset, itOffsets, maxitems,
...
OFCondition DcmDicomDir::moveRecordToTree(DcmDirectoryRecord* startRec,
  ...
  if (toRecord == NULL)
    ...
    DcmUnsignedLongOffset* offElem;
    offElem = lookForOffsetElem(
        startRec, DCM_OffsetOfReferencedLowerLevelDirectoryEntity);
    if (offElem != NULL)
    ...
    DCMDATA_DEBUG("DcmDicomDir::moveRecordToTree() Record ("
                  ...
                  << " and next=" << OFstatic_cast(void*, nextRec)
                  << " Record");

    linkMRDRtoRecord(startRec);

    // use protected method for insertion without type check:
    if (toRecord->masterInsertSub(startRec) ==
        EC_Normal) {  // only works since friend class
      DcmItem* dit = fromDirSQ.remove(startRec);
      if (dit == NULL) {
        DCMDATA_ERROR(
            "DcmDicomDir::moveRecordToTree() DirRecord is part of unknown "
            "Sequence");
      }
    } else {
      DCMDATA_ERROR(
          "DcmDicomDir::moveRecordToTree() Cannot insert DirRecord (=NULL?)");
    }
...
OFCondition DcmDicomDir::convertLinearToTree() {
  ...
  DcmUnsignedLongOffset* offElem = lookForOffsetElem(
      &dset, DCM_OffsetOfTheFirstDirectoryRecordOfTheRootDirectoryEntity);
  if (offElem != NULL)
    firstRootRecord =
        OFstatic_cast(DcmDirectoryRecord*, offElem->getNextRecord());
  ...
  for (unsigned long i = localDirRecSeq.card(); i > 0; i--)
    linkMRDRtoRecord(
        OFstatic_cast(DcmDirectoryRecord*, localDirRecSeq.getItem(i - 1)));
...
Uint32 DcmDicomDir::lengthUntilSQ(DcmDataset& dset, E_TransferSyntax oxfer,
  ...
  for (unsigned long i = 0; i < num; i++) {
    ...
    if (sublength == DCM_UndefinedLength) {
      ...
      DCMDATA_WARN("DcmDicomDir::lengthUntilSQ() Sub element \""
                   << subvr.getVRName() << "\" has undefined Length");
    }
  ...
  DCMDATA_TRACE(
      "DcmDicomDir::lengthUntilSQ() Length of Dataset until SQ=" << templen);
...
Uint32 DcmDicomDir::lengthOfRecord(DcmItem* item, E_TransferSyntax oxfer,
                                   E_EncodingType enctype) {
  Uint32 templen = 0;
  if (item != NULL) {
                                           ...
                                           E_EncodingType enctype)  // in
{
  ...
  OFCondition e1 = convertGivenPointer(
      &dset, itOffsets, num,
      DCM_OffsetOfTheFirstDirectoryRecordOfTheRootDirectoryEntity);
  OFCondition e2 = convertGivenPointer(
      &dset, itOffsets, num,
      DCM_OffsetOfTheLastDirectoryRecordOfTheRootDirectoryEntity);
  ...
  OFCondition e3 = convertGivenPointer(&localDirRecSeq, itOffsets, num,
                                       DCM_OffsetOfTheNextDirectoryRecord);
  OFCondition e4 =
      convertGivenPointer(&localDirRecSeq, itOffsets, num,
                          DCM_OffsetOfReferencedLowerLevelDirectoryEntity);
  OFCondition e5 = convertGivenPointer(&localDirRecSeq, itOffsets, num,
                                       DCM_RETIRED_MRDRDirectoryRecordOffset);

  if (e1 == EC_InvalidVR || e2 == EC_InvalidVR || e3 == EC_InvalidVR ||
      e4 == EC_InvalidVR || e5 == EC_InvalidVR)
...
OFCondition DcmDicomDir::copyRecordPtrToSQ(DcmDirectoryRecord* record,
  ...
  if (record != NULL) {
    ...
    for (unsigned long i = lastIndex; i > 0; i--) {
      DCMDATA_DEBUG("DcmDicomDir::copyRecordPtrToSQ() Testing sub record no. "
                    << i << " of " << lastIndex);
...
OFCondition DcmDicomDir::convertTreeToLinear(Uint32 beginOfDataSet,
  ...
  for (unsigned long i = numUnresItems; i > 0; i--) {
    DCMDATA_DEBUG(
        "DcmDicomDir::convertTreeToLinear() Copy pointer of unresolved Record "
        "no. "
  ...
  copyRecordPtrToSQ(&getRootRecord(), localDirRecSeq, firstRootRecord,
                    lastRootRecord);
  ...
  offElem = lookForOffsetElem(
      &dset, DCM_OffsetOfTheLastDirectoryRecordOfTheRootDirectoryEntity);
  if (offElem != NULL)
  ...
  for (unsigned long j = numMRDRItems; j > 0; j--) {
    DCMDATA_DEBUG("DcmDicomDir::convertTreeToLinear() Copy pointer of MRDR no. "
                  << j << " of " << numUnresItems << " to localDirRecSeq:");
  ...
  if (convertAllPointer(dset, beginOfDataSet, oxfer, enctype) == EC_InvalidVR)
    if (convertAllPointer(dset, beginOfDataSet, oxfer, enctype) ==
        EC_InvalidVR) {
      DCMDATA_ERROR("DcmDicomDir: There are some incorrect Offsets in file "
                    << dicomDirFileName);
...
void DcmDicomDir::print(STD_NAMESPACE ostream& out, const size_t flags,
  ...
  getRootRecord().lowerLevelList->print(out, flags, 1, pixelFileName,
                                        pixelCounter);  // friend class
...
DcmDirectoryRecord* DcmDicomDir::matchFilename(const char* filename) {
  ...
  if (retRec == NULL) {
    DCMDATA_DEBUG(
        "DcmDicomDir::matchFilename() No Record with matching filename ["
        << filename << "] found");
  }
  return retRec;
}
...
DcmDirectoryRecord* DcmDicomDir::matchOrCreateMRDR(const char* filename) {
  ...
  if (matchRec != NULL) {
    if (matchRec->getRecordType() == ERT_Mrdr)
      ...
      if (newMRDR != NULL) {
        DCMDATA_DEBUG("DcmDicomDir::matchOrCreateMRDR() New MRDR p="
                      << OFstatic_cast(void*, newMRDR)
                      << " with matching filename [" << filename
                      << "] created, original Record p="
                      << OFstatic_cast(void*, matchRec)
                      << " with same filename modified");
      }
      modified = OFTrue;
    }
  }
  if (newMRDR == NULL)
    DCMDATA_WARN(
        "DcmDicomDir::matchOrCreateMRDR() No MRDR with matching filename ["
        << filename << "] found");
...
OFCondition DcmDicomDir::write(const E_TransferSyntax oxfer,
  ...
  if (oxfer != DICOMDIR_DEFAULT_TRANSFERSYNTAX) {
    DCMDATA_ERROR(
        "DcmDicomDir::write() Wrong TransferSyntax used, only "
        "LittleEndianExplicit allowed");
  }
  errorFlag = EC_Normal;
  ...
  if (pathsepposition == OFString_npos)
    ...
    tempfilename.erase(pathsepposition + 1);
  ...
  if (tempfilefd < 0) {
    char buf[256];
    DCMDATA_ERROR(
        "DcmDicomDir: Cannot create DICOMDIR temporary file: " << tempfile);
    delete[] tempfile;
  ...
  {
    // it is important that the cache object is destroyed before the file is
    // renamed! Therefore, the variable declaration is "encapsulated" in curly
    // brackets.
    ...
    errorFlag = convertTreeToLinear(beginOfDataset, outxfer, enctype, glenc,
                                    localUnresRecs);
  ...
  if (!mustCreateNewDir) {
    ...
    backupname =
        new char[1 + strlen(dicomDirFileName) + strlen(DICOMDIR_BACKUP_SUFFIX)];
...
OFCondition DcmDicomDir::checkMRDRRefCounter(DcmDirectoryRecord* startRec,
  ...
  if (refCounter == NULL)
    ...
    for (unsigned long i = 0; i < lastIndex; i++) {
      ...
      OFCondition err1 =
          checkMRDRRefCounter(subRecord, refCounter, numCounters);
...
OFCondition DcmDicomDir::verify1(OFBool autocorrect) {
  errorFlag = EC_Normal;
  ...
  for (unsigned long k = 0; k < maxMRDRs; k++) {
    DcmDirectoryRecord* refMRDR =
        OFstatic_cast(DcmDirectoryRecord*, refCounter[k].item);
    Uint32 refNum = refMRDR->lookForNumberOfReferences();  // friend
    if (refCounter[k].fileOffset != refNum) {
      DCMDATA_ERROR("DcmDicomDir::verify() Reference counter of MRDR p="
                    << OFstatic_cast(void*, refMRDR)
                    << " has incorrect value=" << refNum << " (must be "
                    << refCounter[k].fileOffset << ")");
  ...
  delete[] refCounter;
  OFCondition err1 = getDirFileFormat().verify1(autocorrect);
  OFCondition err2 =
      getRootRecord().verify1(OFFalse);  // no automatic correction
  if (errorFlag == EC_Normal &&
      (err1 != EC_Normal || err2 != EC_Normal || err3 != EC_Normal))
    errorFlag = EC_CorruptedData;
  return errorFlag;
}
