...
DcmFileConsumer::DcmFileConsumer(const char* filename)
    : DcmConsumer(), file_(), status_(EC_Normal) {
...
DcmFileConsumer::DcmFileConsumer(FILE* file)
    : DcmConsumer(), file_(file), status_(EC_Normal) {}
...
offile_off_t DcmFileConsumer::avail() const {
  // since we cannot report "unlimited", let's claim that we can still write
  // 2GB. Note that offile_off_t is a signed type.
  return 2147483647L;
}
...
offile_off_t DcmFileConsumer::write(const void* buf, offile_off_t buflen) {
  ...
  if (status_.good() && file_.open() && buf && buflen) {
    ...
    result = OFstatic_cast(offile_off_t,
                           file_.fwrite(buf, 1, OFstatic_cast(size_t, buflen)));
#else
    /* On Windows (at least for some versions of MSVC), calls to fwrite() for
     * more than 67,076,095 bytes (a bit less than 64 MByte) fail if we're
     * writing to a network share. See MSDN KB899149. As a workaround, we always
     * write in chunks of 32M which should hardly negatively affect performance.
     */
#define DcmFileConsumer_MAX_CHUNK_SIZE 33554432 /* 32 MByte */
    ...
    while (buflen > DcmFileConsumer_MAX_CHUNK_SIZE) {
      written = OFstatic_cast(
          offile_off_t, file_.fwrite(buf2, 1, DcmFileConsumer_MAX_CHUNK_SIZE));
      result += written;
      buf2 += written;

      // if we have not written a complete chunk, there is problem; bail out
      if (written == DcmFileConsumer_MAX_CHUNK_SIZE)
        buflen -= DcmFileConsumer_MAX_CHUNK_SIZE;
      else
        buflen = 0;
    }

    // last call to fwrite if the file size is not a multiple of
    // DcmFileConsumer_MAX_CHUNK_SIZE
    if (buflen) {
      written = OFstatic_cast(
          offile_off_t, file_.fwrite(buf2, 1, OFstatic_cast(size_t, buflen)));
      result += written;
    }
#endif
  }
...
DcmOutputFileStream::DcmOutputFileStream(const char* filename)
    : DcmOutputStream(
          &consumer_)  // safe because DcmOutputStream only stores pointer
      ,
      consumer_(filename) {}
