...
void DcmPixelItem::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmPixelItem::createOffsetTable(const DcmOffsetList& offsetList) {
  ...
  if (numEntries > 0) {
    ...
    if (array) {
      DCMDATA_DEBUG("DcmPixelItem: creating offset table with " << numEntries
                                                                << " entries");
      ...
      while ((first != last) && result.good()) {
        ...
        if (current & 1) {
          DCMDATA_WARN("DcmPixelItem: odd frame size ("
                       << current << ") found for frame #" << (idx + 1)
      ...
      if (result.good()) {
        result = swapIfNecessary(EBO_LittleEndian, gLocalByteOrder, array,
                                 numEntries * sizeof(Uint32), sizeof(Uint32));
        if (result.good())
          result = putUint8Array(OFreinterpret_cast(Uint8*, array),
                                 numEntries * sizeof(Uint32));
      }
      delete[] array;
    } else
...
OFCondition DcmPixelItem::writeXML(STD_NAMESPACE ostream& out,
  ...
  if (valueLoaded() && (flags & DCMTypes::XF_writeBinaryData)) {
    ...
    if (flags & DCMTypes::XF_encodeBase64) {
      /* pixel items always contain 8 bit data, therefore, byte swapping not
       * required */
      OFStandard::encodeBase64(out, OFstatic_cast(Uint8*, getValue()),
      ...
      if (getUint8Array(byteValues).good() && (byteValues != NULL)) {
        ...
        for (unsigned long i = 1; i < count; i++)
          out << "\\" << STD_NAMESPACE setw(2)
              << OFstatic_cast(int, *(byteValues++));
...
OFCondition DcmPixelItem::writeSignatureFormat(DcmOutputStream& outStream,
                                               const E_TransferSyntax oxfer,
                                               const E_EncodingType enctype,
                                               DcmWriteCache* wcache) {
  if (dcmEnableOldSignatureFormat.get()) {
    /* Old signature format as created by DCMTK releases previous to 3.5.4.
     * This is non-conformant because it includes the item length in pixel
     * items.
     */
    return DcmOtherByteOtherWord::writeSignatureFormat(outStream, oxfer,
                                                       enctype, wcache);
  } else {
    ...
    if (getTransferState() == ERW_notInitialized)
      ...
      if (errorFlag.good()) {
         ...
         * If the element is unaccessable (which would mean that the value
         * resides in file and access to the file fails), write an empty element
         * with zero length.
         */
        if (getLengthField() > 0) {
          if (valueLoaded()) {
        ...
        /* if this element's transfer state is ERW_init (i.e. it has not yet
         * been written to */
        ...
        if (getTransferState() == ERW_init) {
           ...
           * space in the buffer, check if the buffer is still sufficient for
           * the requirements of this element, which may need only 8 instead of
           * 12 bytes.
           */
          if (outStream.avail() >= 4) {
            ...
            /* mind the transfer syntax and remember the amount of bytes that
             * have been written */
        ...
        if (accessPossible && getTransferState() == ERW_inWork) {
          ...
          if (valueLoaded()) {
            ...
            while (!done) {
              ...
              // stop writing if something went wrong, we were unable to send
              // all of the buffer content (which indicates that the output
              // stream needs to be flushed, or everything was sent out.
              done = errorFlag.bad() || (len < buflen) ||
                     (getTransferredBytes() == getLengthField());
            }
          }

          /* if the amount of transferred bytes equals the length of the
           * element's value, the */
...
**   and a corresponding command line option in dcmsign allow to re-enable the
*old
**   implementation.
**
