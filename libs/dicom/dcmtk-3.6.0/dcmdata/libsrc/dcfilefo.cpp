...
DcmFileFormat::DcmFileFormat()
    : DcmSequenceOfItems(InternalUseTag), FileReadMode(ERM_autoDetect) {
...
DcmFileFormat::DcmFileFormat(const DcmFileFormat& old)
    : DcmSequenceOfItems(old), FileReadMode(old.FileReadMode) {}
...
void DcmFileFormat::print(STD_NAMESPACE ostream& out, const size_t flags,
...
/*
 * This function checks if a particular data element of the file meta
 * information header is existent.  If the element is not existent, it will be
 * inserted.  Additionally, this function makes sure that the corresponding data
 * element will contain a correct value.
 *
 * Parameters:
 ...
 *   obj       - [in] Data object from metainfo which represents the data
 * element that shall be checked. Equals NULL, if this data element is not
 * existent in the meta header information. oxfer     - [in] The transfer syntax
 * which shall be used. writeMode - [in] Flag indicating whether to update the
 * file meta information or not.
 */
{
  /* initialize result value */
  OFCondition l_error = EC_Normal;

  /* if there is meta header information and also data set information, do
   * something */
  if (metainfo != NULL && dataset != NULL) {
    ...
    if (xtag == DCM_FileMetaInformationGroupLength)  // (0002,0000)
    {
      ...
      // the calculation of actual group length value is contained in
      // validateMetaInfo()
    } else if (xtag == DCM_FileMetaInformationVersion)  // (0002,0001)
    {
      ...
      Uint8* currVers = NULL;
      l_error =
          OFstatic_cast(DcmOtherByteOtherWord*, elem)->getUint8Array(currVers);
      if (l_error.good() && (currVers != NULL)) {
        if (((currVers[0] & version[0] & 0xff) == version[0]) &&
            ((currVers[1] & version[1] & 0xff) == version[1])) {
          DCMDATA_DEBUG(
              "DcmFileFormat::checkMetaHeaderValue() Version of MetaHeader is "
              "ok: 0x"
              << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
              << STD_NAMESPACE setw(2) << OFstatic_cast(int, currVers[1])
              << STD_NAMESPACE setw(2) << OFstatic_cast(int, currVers[0]));
        } else {
          currVers[0] = OFstatic_cast(
              Uint8, currVers[0] | version[0]);  // direct manipulation
          currVers[1] =
              OFstatic_cast(Uint8, currVers[1] | version[1]);  // of data
          DCMDATA_WARN(
              "DcmFileFormat: Unknown Version of MetaHeader detected: 0x"
              << STD_NAMESPACE hex << STD_NAMESPACE setfill('0')
              << STD_NAMESPACE setw(2) << OFstatic_cast(int, currVers[1])
              << STD_NAMESPACE setw(2) << OFstatic_cast(int, currVers[0])
              << " supported: 0x" << STD_NAMESPACE setw(2)
              << OFstatic_cast(int, version[1]) << STD_NAMESPACE setw(2)
              << OFstatic_cast(int, version[0]));
        }
      } else {
    ...
    } else if (xtag == DCM_MediaStorageSOPClassUID)  // (0002,0002)
    {
      ...
      if (((writeMode == EWM_updateMeta) || (elem->getLength() == 0)) &&
        ...
        if (dataset->search(DCM_SOPClassUID, stack).good()) {
          ...
          DCMDATA_DEBUG(
              "DcmFileFormat::checkMetaHeaderValue() use SOPClassUID [" << uid
                                                                        << "]");
        } else if (elem->getLength() == 0) {
          OFstatic_cast(DcmUniqueIdentifier*, elem)
              ->putString(UID_PrivateGenericFileSOPClass);
          DCMDATA_DEBUG(
              "DcmFileFormat::checkMetaHeaderValue() no SOPClassUID in "
              "Dataset, using PrivateGenericFileSOPClass");
        }
      }
    } else if (xtag == DCM_MediaStorageSOPInstanceUID)  // (0002,0003)
    {
      ...
      if (((writeMode == EWM_updateMeta) || (elem->getLength() == 0)) &&
        ...
        if (dataset->search(DCM_SOPInstanceUID, stack).good()) {
          ...
          OFstatic_cast(DcmUniqueIdentifier*, elem)->putString(uid);
          DCMDATA_DEBUG(
              "DcmFileFormat::checkMetaHeaderValue() use SOPInstanceUID ["
              << uid << "] from Dataset");
        } else if (elem->getLength() == 0) {
          char uid[128];
          ...
          OFstatic_cast(DcmUniqueIdentifier*, elem)->putString(uid);
          DCMDATA_DEBUG(
              "DcmFileFormat::checkMetaHeaderValue() use new generated "
              "SOPInstanceUID ["
              << uid << "]");
        }
      }
    } else if (xtag == DCM_TransferSyntaxUID)  // (0002,0010)
    {
      ...
      if (elem->ident() == EVR_UI) {
        ...
        if (uidtmp != NULL)
          DCMDATA_DEBUG(
              "DcmFileFormat::checkMetaHeaderValue() found old "
              "TransferSyntaxUID ["
              << uidtmp << "]");
#endif
        ...
        elem->putString(uid);
        DCMDATA_DEBUG(
            "DcmFileFormat::checkMetaHeaderValue() use new TransferSyntaxUID ["
    ...
    } else if (xtag == DCM_SourceApplicationEntityTitle)  // (0002,0016)
    {
      ...
      DCMDATA_ERROR(
          "DcmFileFormat: Don't know how to handle "
    ...
    } else if (xtag == DCM_PrivateInformation)  // (0002,0102)
    {
      ...
      DCMDATA_WARN(
          "DcmFileFormat: Don't know how to handle PrivateInformation");
    } else {
      DCMDATA_WARN("DcmFileFormat: Don't know how to handle "
                   << tag.getTagName());
    }
    ...
    /* (i.e. there is either no meta header information or no data set
     * information */
...
OFCondition DcmFileFormat::validateMetaInfo(const E_TransferSyntax oxfer,
                                            const E_FileWriteMode writeMode)
/*
 * This function makes sure that all data elements of the meta header
 * information are existent in metainfo and contain correct values.
 *
 * Parameters:
 *   oxfer     - [in] The transfer syntax which shall be used.
 *   writeMode - [in] Flag indicating whether to update the file meta
 * information or not.
 */
{
  ...
  if (metinf != NULL && datset != NULL) {
    if (writeMode == EWM_dontUpdateMeta) {
      DCMDATA_WARN(
          "DcmFileFormat::validateMetaInfo(): Meta Information Header is not "
          "updated!");
    } else {
      ...
      metinf->search(DCM_FileMetaInformationGroupLength, stack, ESM_fromHere,
                     OFFalse);
      checkMetaHeaderValue(metinf, datset, DCM_FileMetaInformationGroupLength,
                           stack.top(), oxfer, writeMode);

      /* DCM_FileMetaInformationVersion */
      metinf->search(DCM_FileMetaInformationVersion, stack, ESM_fromHere,
                     OFFalse);
      checkMetaHeaderValue(metinf, datset, DCM_FileMetaInformationVersion,
                           stack.top(), oxfer, writeMode);

      /* DCM_MediaStorageSOPClassUID */
      metinf->search(DCM_MediaStorageSOPClassUID, stack, ESM_fromHere, OFFalse);
      checkMetaHeaderValue(metinf, datset, DCM_MediaStorageSOPClassUID,
                           stack.top(), oxfer, writeMode);

      /* DCM_MediaStorageSOPInstanceUID */
      metinf->search(DCM_MediaStorageSOPInstanceUID, stack, ESM_fromHere,
                     OFFalse);
      checkMetaHeaderValue(metinf, datset, DCM_MediaStorageSOPInstanceUID,
                           stack.top(), oxfer, writeMode);

      /* DCM_TransferSyntaxUID */
      metinf->search(DCM_TransferSyntaxUID, stack, ESM_fromHere, OFFalse);
      checkMetaHeaderValue(metinf, datset, DCM_TransferSyntaxUID, stack.top(),
                           oxfer, writeMode);

      /* DCM_ImplementationClassUID */
      metinf->search(DCM_ImplementationClassUID, stack, ESM_fromHere, OFFalse);
      checkMetaHeaderValue(metinf, datset, DCM_ImplementationClassUID,
                           stack.top(), oxfer, writeMode);

      /* DCM_ImplementationVersionName */
      metinf->search(DCM_ImplementationVersionName, stack, ESM_fromHere,
                     OFFalse);
      checkMetaHeaderValue(metinf, datset, DCM_ImplementationVersionName,
                           stack.top(), oxfer, writeMode);

      /* dump some information if required */
      DCMDATA_DEBUG("DcmFileFormat: Found "
                    << metinf->card() << " Elements in DcmMetaInfo 'metinf'");

      /* calculate new GroupLength for meta header */
      if (metinf
              ->computeGroupLengthAndPadding(EGL_withGL, EPD_noChange,
                                             META_HEADER_DEFAULT_TRANSFERSYNTAX,
                                             EET_UndefinedLength)
              .bad()) {
        DCMDATA_ERROR(
            "DcmFileFormat::validateMetaInfo() Group length of Meta "
            "Information Header not adapted");
      }
    }
  } else {
    /* (i.e. there is either no meta header information or no data set
     * information, or both are missing) */
...
OFCondition DcmFileFormat::write(
    DcmOutputStream& outStream, const E_TransferSyntax oxfer,
    const E_EncodingType enctype, DcmWriteCache* wcache,
    ...
    const E_FileWriteMode writeMode)
/*
 * This function writes data values which are contained in this to the stream
 * which is passed as first argument. With regard to the writing of information,
 * the other parameters which are passed are accounted for. The function will
 * return EC_Normal, if the information from all elements of this data set has
 * been written to the buffer, and it will return some other (error) value if
 * there was an error.
 *
 * Parameters:
 *   outStream      - [inout] The stream that the information will be written
 * to. oxfer          - [in] The transfer syntax which shall be used. enctype -
 * [in] Encoding type for sequences. Specifies how sequences will be handled.
 *   glenc          - [in] Encoding type for group length. Specifies what will
 * be done with group length tags. padenc         - [in] Encoding type for
 * padding. Specifies what will be done with padding tags. padlen         - [in]
 * The length up to which the dataset shall be padded, if padding is desired.
 *   subPadlen      - [in] For sequences (i.e. sub elements), the length up to
 * which item shall be padded, if padding is desired. instanceLength - [in]
 * Number of extra bytes added to the item/dataset length used when computing
 * the padding. This parameter is for instance used to pass the length of the
 * file meta header from the DcmFileFormat to the DcmDataset object. writeMode
 * - [in] Write file with or without meta header. Also allows for updating the
 ...
 */
{
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    DcmDataset* dataset = getDataset();
    DcmMetaInfo* metainfo = getMetaInfo();
    /* Determine the transfer syntax which shall be used. Either we use the one
     * which was passed, */
    /* or (if it's an unknown tranfer syntax) we use the data set's original
     * transfer syntax. */
    ...
    /* check if we can actually write data to the stream; in certain cases we
     * cannot. */
    if (outxfer == EXS_Unknown || outxfer == EXS_BigEndianImplicit)
      ...
      /* if this function was called for the first time for the dataset object,
       * the transferState is */
      /* still set to ERW_init. In this case, we need to validate the meta
       * header information, set the */
      /* item list pointer to the fist element and we need to set the transfer
       * state to ERW_inWork. */
      if (getTransferState() == ERW_init) {
      ...
      if (getTransferState() == ERW_inWork) {
        ...
        if (errorFlag.good())
          errorFlag = dataset->write(outStream, outxfer, enctype, wcache, glenc,
    ...
    if (outxfer == EXS_BigEndianImplicit)
      DCMDATA_ERROR(
          "DcmFileFormat::write() Illegal TransferSyntax (BigEndianImplicit) "
          "used");
  }
  /* return result value */
  return errorFlag;
...
OFCondition DcmFileFormat::loadFile(const char* fileName,
  ...
  if (readMode == ERM_dataset)
    return getDataset()->loadFile(fileName, readXfer, groupLength,
                                  maxReadLength);
...
OFCondition DcmFileFormat::saveFile(
    const char* fileName, const E_TransferSyntax writeXfer,
    const E_EncodingType encodingType, const E_GrpLenEncoding groupLength,
    const E_PaddingEncoding padEncoding, const Uint32 padLength,
    const Uint32 subPadLength, const E_FileWriteMode writeMode) {
  if (writeMode == EWM_dataset) {
    return getDataset()->saveFile(fileName, writeXfer, encodingType,
                                  groupLength, padEncoding, padLength,
                                  subPadLength);
  }
  OFCondition l_error = EC_IllegalParameter;
  /* check parameters first */
...
OFCondition DcmFileFormat::insertItem(DcmItem* /*item*/,
  ...
  DCMDATA_WARN(
      "Illegal call of DcmFileFormat::insert(DcmItem *, unsigned long)");
  errorFlag = EC_IllegalCall;
  return errorFlag;
}
...
** add method getAndRemoveDataset to remove connection of dataset from
*fileformat
**
