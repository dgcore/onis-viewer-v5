...
void DcmUnsignedLong::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmUnsignedLong::putUint32Array(const Uint32* uintVals,
  ...
  if (numUints > 0) {
    ...
    if (uintVals != NULL)
      errorFlag =
          putValue(uintVals, sizeof(Uint32) * OFstatic_cast(Uint32, numUints));
    else
...
OFCondition DcmUnsignedLong::verify1(const OFBool autocorrect) {
