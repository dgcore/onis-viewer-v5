...
DcmUniqueIdentifier::DcmUniqueIdentifier(const DcmTag& tag, const Uint32 len)
...
DcmUniqueIdentifier& DcmUniqueIdentifier::operator=(
    const DcmUniqueIdentifier& obj) {
...
void DcmUniqueIdentifier::print(STD_NAMESPACE ostream& out, const size_t flags,
    ...
    if (stringVal != NULL) {
      ...
      if ((symbol != NULL) && (strlen(symbol) > 0)) {
        ...
        if (buffer != NULL) {
          ...
          printInfoLine(out, flags, level, buffer, NULL /*tag*/,
                        OFFalse /*isInfo*/);
