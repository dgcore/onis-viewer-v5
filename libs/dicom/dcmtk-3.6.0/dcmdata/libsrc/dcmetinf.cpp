...
void DcmMetaInfo::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmMetaInfo::writeXML(STD_NAMESPACE ostream& out,
  ...
  out << " name=\""
      << OFStandard::convertToMarkupString(xfer.getXferName(), xmlString)
      << "\">" << OFendl;
  if (!elementList->empty()) {
...
OFBool DcmMetaInfo::checkAndReadPreamble(DcmInputStream& inStream,
  ...
  if (fPreambleTransferState == ERW_inWork) {
    ...
    if (readLen > 0)
      incTransferredBytes(OFstatic_cast(
          Uint32,
          inStream.read(&filePreamble[getTransferredBytes()], readLen)));
    if (inStream.eos() &&
        getTransferredBytes() != preambleLen) {  // file too short, no preamble
      inStream.putback();
      DCMDATA_TRACE(
          "DcmMetaInfo::checkAndReadPreamble() No Preamble available: File too "
          "short ("
    ...
    } else if (getTransferredBytes() ==
               preambleLen)  // check Preamble and Dicom Prefix
  ...
  if (fPreambleTransferState == ERW_ready) {
    ...
    if ((tmpxferSyn.isExplicitVR() && xferSyn.isImplicitVR()) ||
      ...
      if (xferSyn.getXfer() != EXS_Unknown)
        DCMDATA_WARN(
            "DcmMetaInfo: TransferSyntax of MetaInfo is other than expected");
    } else
  ...
  if (retval == OFTrue) {
    DCMDATA_TRACE("DcmMetaInfo::checkAndReadPreamble() Preamble = 0x"
                  ...
                  << OFstatic_cast(Uint32, *filePreamble));
  } else
...
Uint32 DcmMetaInfo::calcElementLength(const E_TransferSyntax /*xfer*/,
  ...
  Uint32 metaLength =
      DcmItem::getLength(META_HEADER_DEFAULT_TRANSFERSYNTAX, enctype);
...
OFCondition DcmMetaInfo::readGroupLength(DcmInputStream& inStream,
                                         ...
                                         Uint32& headerLen, Uint32& bytesRead,
  ...
  if (nextTagIsMeta(inStream)) {
    ...
    l_error = DcmItem::readTagAndLength(inStream, newxfer, newTag,
                                        newValueLength, bytes_tagAndLen);
    bytesRead += bytes_tagAndLen;
    if (l_error.good() && !inStream.good())
      l_error = inStream.status();
    if (l_error.good()) {
      l_error = DcmItem::readSubElement(inStream, newTag, newValueLength,
                                        newxfer, glenc, maxReadLength);
      bytesRead += newValueLength;
      if (l_error.good() && newTag.getXTag() == xtag &&
          elementList->get() != NULL && newValueLength > 0) {
        l_error = (OFstatic_cast(DcmUnsignedLong*, elementList->get()))
                      ->getUint32(headerLen);
        DCMDATA_TRACE(
            "DcmMetaInfo::readGroupLength() Group Length of File Meta Header = "
            << headerLen + bytesRead);
      } else {
        l_error = EC_CorruptedData;
        DCMDATA_WARN(
            "DcmMetaInfo: No Group Length available in Meta Information "
            "Header");
      }
    }
  }
  ...
  DCMDATA_TRACE(
      "DcmMetaInfo::readGroupLength() returns error = " << l_error.text());
  return l_error;
}
...
OFCondition DcmMetaInfo::read(DcmInputStream& inStream,
  ...
  if (fPreambleTransferState == ERW_notInitialized ||
      getTransferState() == ERW_notInitialized)
    ...
    if (errorFlag.good() && inStream.eos())
      ...
      if (getTransferState() == ERW_inWork && getLengthField() == 0) {
        if (inStream.avail() <
          ...
          errorFlag = readGroupLength(inStream, newxfer,
                                      DCM_FileMetaInformationGroupLength, glenc,
                                      headerLength, bytesRead, maxReadLength);

          setTransferredBytes(bytesRead);

          if (errorFlag.good()) {
            /* FileMetaInformationGroupLength (0002,0000) is present but should
             * be ignored  */
            if (dcmIgnoreFileMetaInformationGroupLength.get()) {
              DCMDATA_WARN(
                  "DcmMetaInfo: Ignoring Group Length of Meta Information "
                  "Header");
      ...
      // this is the old behaviour up to DCMTK 3.5.3: fail with EC_CorruptedData
      // error code if the file meta header group length (0002,0000) is absent.
      if (getTransferState() == ERW_inWork && getLengthField() != 0 &&
          errorFlag.good()) {
#else
      // new behaviour: accept file without meta header group length, determine
      // end of meta header based on heuristic that checks for group 0002 tags.
      if (getTransferState() == ERW_inWork && getLengthField() != 0 &&
          (errorFlag.good() || ((errorFlag == EC_CorruptedData) &&
                                (getLengthField() == DCM_UndefinedLength)))) {
        /* start with "no error" in order to handle meta-header with only one
         * data element */
        errorFlag = EC_Normal;
#endif
        while (inStream.good() && !inStream.eos() &&
          ...
          if (lastElementComplete) {
            errorFlag = DcmItem::readTagAndLength(
                inStream, newxfer, newTag, newValueLength, bytes_tagAndLen);
            incTransferredBytes(bytes_tagAndLen);
            if (errorFlag != EC_Normal)
              break;  // terminate while loop
            lastElementComplete = OFFalse;
            errorFlag =
                DcmItem::readSubElement(inStream, newTag, newValueLength,
                                        newxfer, glenc, maxReadLength);
            if (errorFlag.good())
              lastElementComplete = OFTrue;
            /* check for valid meta-header elements */
            if (newTag.getGroup() != 0x0002)
              DCMDATA_WARN("DcmMetaInfo: Invalid Element "
                           << newTag << " found in Meta Information Header");
          } else {
            errorFlag =
      ...
      if (errorFlag.good()) {
        if (getLengthField() != DCM_UndefinedLength &&
            getTransferredBytes() != getLengthField())
          DCMDATA_WARN(
              "DcmMetaInfo: Group Length of Meta Information Header has "
              "incorrect value");
...
/*
 * This function writes all data elements which make up the meta header to the
 * stream. For a specification of the elements that make up the meta header see
 * DICOM standard (year 2000) part 10, section 7.1)) or the corresponding
 * section in a later version of the standard).
 *
 * Parameters:
 *   outStream - [inout] The stream that the information will be written to.
 *   oxfer     - [in] The transfer syntax which shall be used. (is not necessary
 * since the meta header shall always be encoded in the explicit VR little
 * endian transfer syntax) enctype   - [in] Encoding type for sequences.
 * Specifies how sequences will be handled.
 */
{
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    /* determine the (default) transfer syntax which shall be used (always
     * explicit VR little endian) */
    E_TransferSyntax outxfer = META_HEADER_DEFAULT_TRANSFERSYNTAX;
    /* check if the stream reported an error so far */
    errorFlag = outStream.status();
    /* if the stream did not report any error and the transfer state is
     * ERW_ready, */
    /* go ahead and write the meta header information to the out stream */
    if (errorFlag.good() && getTransferState() != ERW_ready) {
      /* if some particular conditions are met we need to write the file
       * preamble (128 byte wide) and */
      /* the DICOM prefix "DICM" to the stream. Always check if there is enough
       * space in the stream and */
      /* set the transfer state of certain elements to indicate that they have
      ...
      if (getTransferState() == ERW_init) {
        if (preambleUsed || !elementList->empty()) {
          if (fPreambleTransferState == ERW_init) {
            incTransferredBytes(OFstatic_cast(
                Uint32,
                outStream.write(&filePreamble[getTransferredBytes()],
      ...
      if (!elementList->empty() && (getTransferState() == ERW_inWork) &&
          (elementList->get() != NULL)) {
        DcmObject* dO;
      ...
      /* if the error flag equals ok and the transfer state equals ERW_inWork,
       * all data elements of the meta */
      /* header have been written to the stream. Indicate this by setting the
       * transfer state to ERW_ready */
