...
DcmFileProducer::DcmFileProducer(const char* filename, offile_off_t offset)
    : DcmProducer(), file_(), status_(EC_Normal), size_(0) {
...
DcmInputFileStreamFactory::DcmInputFileStreamFactory(const char* filename,
                                                     offile_off_t offset)
    : DcmInputStreamFactory(), filename_(), offset_(offset) {
  if (filename)
    filename_ = filename;
}

DcmInputFileStreamFactory::DcmInputFileStreamFactory(
    const DcmInputFileStreamFactory& arg)
...
offile_off_t DcmInputFileStreamFactory::GetOffset() const {
  return offset_;
}
...
DcmInputTempFileStreamFactory::DcmInputTempFileStreamFactory(
    DcmTempFileHandler* handler)
    : DcmInputStreamFactory(), fileHandler_(handler) {
  fileHandler_->increaseRefCount();
}

DcmInputTempFileStreamFactory::DcmInputTempFileStreamFactory(
    const DcmInputTempFileStreamFactory& arg)
    : DcmInputStreamFactory(arg), fileHandler_(arg.fileHandler_) {
  fileHandler_->increaseRefCount();
}
...
offile_off_t DcmInputTempFileStreamFactory::GetOffset() const {
  return 0;
}
 ...
 *   temporary file and are needed for DcmElement temporary file extensions to
 * come.
 *
