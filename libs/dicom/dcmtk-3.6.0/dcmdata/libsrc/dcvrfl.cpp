...
DcmFloatingPointSingle::DcmFloatingPointSingle(
    const DcmFloatingPointSingle& old)
    : DcmElement(old) {}

DcmFloatingPointSingle::~DcmFloatingPointSingle() {}

DcmFloatingPointSingle& DcmFloatingPointSingle::operator=(
    const DcmFloatingPointSingle& obj) {
...
OFCondition DcmFloatingPointSingle::checkValue(const OFString& vm,
  ...
  /* check VM only, further checks on the floating point values could be added
   * later */
  ...
  if (valueLoaded()) {
    ...
    if (floatVals != NULL) {
      const unsigned long count =
          getLengthField() / sizeof(Float32) /* do not use getVM()! */;
      const unsigned long maxLength =
          (flags & DCMTypes::PF_shortenLongTagValues)
              ? DCM_OptPrintLineLength
              : OFstatic_cast(unsigned long, -1);
      ...
      for (unsigned int i = 0; i < count; i++, floatVals++) {
        ...
        if (i == 0)
          OFStandard::ftoa(buffer, sizeof(buffer), *floatVals, 0, 0,
                           8 /* FLT_DIG + 2 for DICOM FL */);
        else {
          buffer[0] = '\\';
          OFStandard::ftoa(buffer + 1, sizeof(buffer) - 1, *floatVals, 0, 0,
                           8 /* FLT_DIG + 2 for DICOM FL */);
        }
        /* check whether current value sticks to the length limit */
        newLength = printedLength + strlen(buffer);
        if ((newLength <= maxLength) &&
            ((i + 1 == count) || (newLength + 3 <= maxLength))) {
          out << buffer;
...
OFCondition DcmFloatingPointSingle::getOFString(OFString& value,
  ...
  if (errorFlag.good()) {
    ...
    char buffer[64];
    OFStandard::ftoa(buffer, sizeof(buffer), floatVal, 0, 0,
                     8 /* FLT_DIG + 2 for DICOM FL */);
    /* assign result */
    value = buffer;
  }
...
OFCondition DcmFloatingPointSingle::putFloat32Array(
    const Float32* floatVals, const unsigned long numFloats) {
  errorFlag = EC_Normal;
  if (numFloats > 0) {
    /* check for valid float data */
    if (floatVals != NULL)
      errorFlag = putValue(floatVals,
                           sizeof(Float32) * OFstatic_cast(Uint32, numFloats));
    else
...
OFCondition DcmFloatingPointSingle::verify1(const OFBool autocorrect) {
...
** Increased output precision to 8 (FLT_DIG+2) when converting an FL element to
*string.
**
