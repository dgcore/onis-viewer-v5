...
DcmRLECodecEncoder::DcmRLECodecEncoder() : DcmCodec() {}
...
OFCondition DcmRLECodecEncoder::encode(
  ...
  Uint8* pixelData8 =
      OFreinterpret_cast(Uint8*, OFconst_cast(Uint16*, pixelData));
  ...
  if ((!dataset) ||
    ...
    if (result.good())
      result = ditem->findAndGetUint16(DCM_SamplesPerPixel, samplesPerPixel);
    if (result.good())
      result = ditem->findAndGetUint16(DCM_Columns, columns);
    if (result.good())
      result = ditem->findAndGetUint16(DCM_Rows, rows);
    if (result.good()) {
    ...
    if (result.good() && (samplesPerPixel > 1)) {
      result =
          ditem->findAndGetUint16(DCM_PlanarConfiguration, planarConfiguration);
    }

    if (result.good()) {
      // check if bitsAllocated is a multiple of 8 - we don't handle anything
      // else
      ...
      // an RLE stripe set can have at most 15 stripes, i.e. 15 bytes allocated
      // per pixel
      numberOfStripes = bytesAllocated * samplesPerPixel;
      if (numberOfStripes > 15)
        result = EC_CannotChangeRepresentation;

      // make sure that we have at least as many bytes of pixel data as we
      // expect
    ...
    if (result.good()) {
      ...
      const Uint32 frameSize =
          columns * rows * samplesPerPixel * bytesAllocated;
      ...
      if (planarConfiguration == 0)
        ...
        offsetBetweenSamples = bytesAllocated;

      // loop through all frames of the image
      for (Uint32 currentFrame = 0;
           ((currentFrame < OFstatic_cast(Uint32, numberOfFrames)) &&
            result.good());
           currentFrame++) {
        ...
        for (sample = 0; sample < samplesPerPixel; sample++) {
          ...
          if (planarConfiguration == 0)
            ...
            sampleOffset = sample * bytesAllocated * columns * rows;
          ...
          for (byte = 0; byte < bytesAllocated; byte++) {
            ...
            if (rleEncoder) {
              ...
              for (pixel = 0; pixel < bytesPerStripe; ++pixel) {
                ...
                // enforce DICOM rule that "Each row of the image shall be
                // encoded separately and not cross a row boundary." (see DICOM
                // part 5 section G.3.1)
                if (--columnCounter == 0) {
                  rleEncoder->flush();
                  columnCounter = columns;
                }
        ...
        if (result.good() && (rleEncoderList.size() > 0) &&
            (rleEncoderList.size() < 16)) {
          ...
          if (rleData) {
            ...
            swapIfNecessary(EBO_LittleEndian, gLocalByteOrder, rleHeader,
            ...
            result = pixelSequence->storeCompressedFrame(
                offsetList, rleData, rleSize, djcp->getFragmentSize());
    ...
    if (dataset->ident() == EVR_dataset) {
      if (result.good()) {
        // create new UID if mode is true or if we're converting to Secondary
        // Capture
        if (djcp->getConvertToSC() || djcp->getUIDCreation()) {
          result =
              DcmCodec::newInstance(OFstatic_cast(DcmItem*, dataset), "DCM",
                                    "121320", "Uncompressed predecessor");

          // set image type to DERIVED
          if (result.good())
            result = updateImageType(OFstatic_cast(DcmItem*, dataset));
          ...
          if (result.good()) {
            ...
            if (compressedSize > 0)
              compressionRatio =
                  (OFstatic_cast(double,
                                 columns* rows* bitsAllocated* OFstatic_cast(
                                     Uint32, numberOfFrames) *
                                     samplesPerPixel) /
                   8.0) /
                  compressedSize;
            result = updateDerivationDescription(
                OFstatic_cast(DcmItem*, dataset), compressionRatio);
          }
        }
      }
      ...
      if (result.good() && djcp->getConvertToSC())
        result = DcmCodec::convertToSecondaryCapture(
            OFstatic_cast(DcmItem*, dataset));
    }
  }
...
OFCondition DcmRLECodecEncoder::updateDerivationDescription(DcmItem* dataset,
                                                            double ratio) {
  char buf[32];

  // create new Derivation Description
  OFString derivationDescription =
      "Lossless RLE compression, compression ratio ";
  ...
  return dataset->putAndInsertString(DCM_DerivationDescription,
                                     derivationDescription.c_str());
}
