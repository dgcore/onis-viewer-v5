...
DcmOtherByteOtherWord::DcmOtherByteOtherWord(const DcmTag& tag,
                                             const Uint32 len)
    : DcmElement(tag, len), compactAfterTransfer(OFFalse) {}

DcmOtherByteOtherWord::DcmOtherByteOtherWord(const DcmOtherByteOtherWord& old)
    : DcmElement(old), compactAfterTransfer(old.compactAfterTransfer) {}

DcmOtherByteOtherWord::~DcmOtherByteOtherWord() {}

DcmOtherByteOtherWord& DcmOtherByteOtherWord::operator=(
    const DcmOtherByteOtherWord& obj) {
...
void DcmOtherByteOtherWord::printPixel(STD_NAMESPACE ostream& out,
  ...
  if (pixelFileName != NULL) {
    ...
    OFString str = "=";
    str += fname;
    printInfoLine(out, flags, level, str.c_str(), NULL /*tag*/,
                  OFFalse /*isInfo*/);
    ...
    if (!OFStandard::fileExists(fname)) {
      ...
      if (file != NULL) {
        if (getTag().getEVR() == EVR_OW || getTag().getEVR() == EVR_lt) {
          ...
          getUint16Array(data);
          if (data != NULL) {
            swapIfNecessary(EBO_LittleEndian, gLocalByteOrder, data,
                            getLengthField(), sizeof(Uint16));
            setByteOrder(EBO_LittleEndian);
            fwrite(data, sizeof(Uint16),
                   OFstatic_cast(size_t, getLengthField() / sizeof(Uint16)),
                   file);
          }
        } else {
          Uint8* data = NULL;
          getUint8Array(data);
          if (data != NULL)
            fwrite(data, sizeof(Uint8), OFstatic_cast(size_t, getLengthField()),
                   file);
        }
        fclose(file);
      } else {
        DCMDATA_WARN(
            "DcmOtherByteOtherWord: Can't open output file for pixel data: "
            << fname);
      }
    } else {
      DCMDATA_WARN(
          "DcmOtherByteOtherWord: Output file for pixel data already exists, "
          "skipping: "
          << fname);
    }
  } else
    DcmOtherByteOtherWord::print(out, flags, level, pixelFileName,
                                 pixelCounter);
}
...
OFCondition DcmOtherByteOtherWord::alignValue() {
  errorFlag = EC_Normal;
  if ((getTag().getEVR() != EVR_OW && getTag().getEVR() != EVR_lt) &&
      (getLengthField() > 0) && ((getLengthField() & 1) != 0)) {
    // We have an odd number of bytes. This should never happen and is certainly
    // not allowed in DICOM. To fix this problem, we will add a zero pad byte at
    // the end of the value field. This requires us to load the value field into
    // memory, which may very well be a problem if this is part of a very large
    // multi-frame object.
...
OFCondition DcmOtherByteOtherWord::putUint8Array(const Uint8* byteValue,
  ...
  if (numBytes > 0) {
    ...
    if ((byteValue != NULL) &&
        (getTag().getEVR() != EVR_OW && getTag().getEVR() != EVR_lt)) {
      errorFlag =
          putValue(byteValue, sizeof(Uint8) * OFstatic_cast(Uint32, numBytes));
      alignValue();
    } else
...
OFCondition DcmOtherByteOtherWord::putUint16Array(
    const Uint16* wordValue, const unsigned long numWords) {
  errorFlag = EC_Normal;
  if (numWords > 0) {
    ...
    if ((wordValue != NULL) &&
        (getTag().getEVR() == EVR_OW || getTag().getEVR() == EVR_lt))
      errorFlag =
          putValue(wordValue, sizeof(Uint16) * OFstatic_cast(Uint32, numWords));
    else
      errorFlag = EC_CorruptedData;
  } else
...
OFCondition DcmOtherByteOtherWord::createUint8Array(const Uint32 numBytes,
  ...
  if ((getTag().getEVR() != EVR_OW) && (getTag().getEVR() != EVR_lt))
    errorFlag =
        createEmptyValue(sizeof(Uint8) * OFstatic_cast(Uint32, numBytes));
  else
    errorFlag = EC_CorruptedData;
  if (errorFlag.good())
...
OFCondition DcmOtherByteOtherWord::createUint16Array(const Uint32 numWords,
  ...
  if ((getTag().getEVR() == EVR_OW) || (getTag().getEVR() == EVR_lt))
    errorFlag =
        createEmptyValue(sizeof(Uint16) * OFstatic_cast(Uint32, numWords));
  else
    errorFlag = EC_CorruptedData;
  if (errorFlag.good())
...
OFCondition DcmOtherByteOtherWord::getOFStringArray(OFString& stringVal,
  ...
  if (getTag().getEVR() == EVR_OW || getTag().getEVR() == EVR_lt) {
    ...
    if ((uint8Vals != NULL) && (count > 0)) {
      ...
      /* for all other array elements ... */
      for (size_t i = 1; i < count; i++)
        stream << "\\" << STD_NAMESPACE setw(2)
               << OFstatic_cast(int, *(uint8Vals++));
      stream << OFStringStream_ends;
      /* convert string stream into a character string */
      OFSTRINGSTREAM_GETSTR(stream, buffer_str)
      stringVal.assign(buffer_str);
...
OFCondition DcmOtherByteOtherWord::verify1(const OFBool autocorrect) {
  errorFlag = EC_Normal;
  if (autocorrect)
...
OFCondition DcmOtherByteOtherWord::write(DcmOutputStream& outStream,
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    if (getTransferState() == ERW_init) {
      // if the attribute value is in file, we should call compact() if the
      // write operation causes the value to be loaded into main memory, which
      // can happen for odd length attributes.
  ...
  if (errorFlag.good() && compactAfterTransfer)
    compact();
...
OFCondition DcmOtherByteOtherWord::writeSignatureFormat(
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    // call inherited method
    errorFlag =
        DcmElement::writeSignatureFormat(outStream, oxfer, enctype, wcache);
  }
  ...
  if (valueLoaded() && (flags & DCMTypes::XF_writeBinaryData)) {
    ...
    if (flags & DCMTypes::XF_encodeBase64) {
      ...
      if ((evr == EVR_OW) || (evr == EVR_lt)) {
        ...
        swapIfNecessary(EBO_BigEndian, gLocalByteOrder, byteValues,
                        getLengthField(), sizeof(Uint16));
        /* update the byte order indicator variable correspondingly */
        setByteOrder(EBO_BigEndian);
      }
      OFStandard::encodeBase64(out, byteValues,
