...
DcmPixelSequence::DcmPixelSequence(const DcmTag& tag, const Uint32 len)
    : DcmSequenceOfItems(tag, len), Xfer(EXS_Unknown) {
  setTagVR(EVR_OB);
  setLengthField(
      DCM_UndefinedLength);  // pixel sequences always use undefined length
}

DcmPixelSequence::DcmPixelSequence(const DcmPixelSequence& old)
    : DcmSequenceOfItems(old), Xfer(old.Xfer) {
...
void DcmPixelSequence::print(STD_NAMESPACE ostream& out, const size_t flags,
...
Uint32 DcmPixelSequence::calcElementLength(const E_TransferSyntax xfer,
  ...
  // add 8 bytes for Sequence Delimitation Tag which always exists for Pixel
  // Sequences
...
OFCondition DcmPixelSequence::insert(DcmPixelItem* item, unsigned long where) {
  errorFlag = EC_Normal;
  if (item != NULL) {
    itemList->seek_to(where);
    itemList->insert(item);
    if (where < itemList->card())
      DCMDATA_TRACE("DcmPixelSequence::insert() Item at position "
                    << where << " inserted");
    if (where >= itemList->card())
      DCMDATA_TRACE(
          "DcmPixelSequence::insert() Item at last position inserted");
  } else
...
OFCondition DcmPixelSequence::getItem(DcmPixelItem*& item,
  ...
  errorFlag = EC_Normal;
  item = OFstatic_cast(DcmPixelItem*,
                       itemList->seek_to(num));  // read item from list
  if (item == NULL)
    errorFlag = EC_IllegalCall;
  return errorFlag;
}
...
OFCondition DcmPixelSequence::writeSignatureFormat(
  ...
  OFCondition l_error = changeXfer(oxfer);
  if (l_error.good())
    return DcmSequenceOfItems::writeSignatureFormat(
        outStream, oxfer, EET_UndefinedLength, wcache);
...
OFCondition DcmPixelSequence::storeCompressedFrame(DcmOffsetList& offsetList,
  ...
  while ((offset < compressedLen) && (result.good())) {
    ...
    if (fragment == NULL)
      ...
      // if currentSize is odd this will be fixed during
  ...
  currentSize =
      offset + (numFragments << 3);  // 8 bytes extra for each item header
  // odd frame size requires padding, i.e. last fragment uses odd length pixel
  // item
  if (currentSize & 1)
    currentSize++;
