...
DcmSignedLong::DcmSignedLong(const DcmSignedLong& old) : DcmElement(old) {}
...
void DcmSignedLong::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmSignedLong::getSint32(Sint32& sintVal, const unsigned long pos) {
  /* get signed integer data */
...
OFCondition DcmSignedLong::putSint32Array(const Sint32* sintVals,
  ...
  if (numSints > 0) {
    ...
    if (sintVals != NULL)
      errorFlag =
          putValue(sintVals, sizeof(Sint32) * OFstatic_cast(Uint32, numSints));
    else
...
OFCondition DcmSignedLong::verify1(const OFBool autocorrect) {
