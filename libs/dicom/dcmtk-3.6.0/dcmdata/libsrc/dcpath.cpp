...
DcmPath::DcmPath(const OFList<DcmPathNode*>& currentPath) : m_path() {
                                         ...
                                         OFBool& wasWildcard)  // out
{
  ...
  if ((closePos != OFString_npos) && (path[0] == '[')) {
    long int parsedNo;
    // try parsing item number; parsing for %lu would cause overflows in case of
    // negative numbers
    int parsed = sscanf(path.c_str(), "[%ld]", &parsedNo);
    if (parsed == 1) {
      if (parsedNo < 0) {
        OFString errMsg =
            "Negative item number (not permitted) at beginning of path: ";
        errMsg += path;
        return makeOFCondition(OFM_dcmdata, 25, OF_error, errMsg.c_str());
      }
      itemNo = OFstatic_cast(Uint32, parsedNo);
      if (closePos + 1 <
          path.length())  // if end of path not reached, cut off "."
        closePos++;
                                      ...
                                      DcmTag& tag)     // out
{
  ...
  if (path[0] == '(') {
    ...
    if (pos != OFString_npos)
      result = DcmTag::findTagFromName(
          path.substr(1, pos - 1).c_str() /* "gggg,eeee" */, tag);
    else {
  ...
  // otherwise we could have a dictionary name
    ...
    if (pos == OFString_npos)
      ...
      result = DcmTag::findTagFromName(path.substr(0, pos).c_str(),
                                       tag);  // parse path up to "[" char
  }
  // construct error message if necessary and return
  if (result.bad()) {
    OFString errMsg(
        "Unable to parse tag/dictionary name at beginning of path: ");
    errMsg += path;
    return makeOFCondition(OFM_dcmdata, 25, OF_error, errMsg.c_str());
  }
...
OFCondition DcmPathProcessor::findOrCreatePath(DcmObject* obj,
  ...
  if (!m_itemWildcardsEnabled) {
    if (path.find('*') != OFString_npos) {
      return makeOFCondition(
          OFM_dcmdata, 25, OF_error,
          "Item wildcard '*' found in path but wildcards disabled");
    }
  }
  clear();
  ...
  if ((obj->ident() == EVR_item) || (obj->ident() == EVR_dataset))
    ...
    return findOrCreateSequencePath(OFstatic_cast(DcmSequenceOfItems*, obj),
                                    pathCopy);
  else
    return EC_IllegalParameter;
}
...
OFCondition DcmPathProcessor::findOrDeletePath(DcmObject* obj,
  ...
  if ((obj->ident() == EVR_item) || (obj->ident() == EVR_dataset))
    ...
    result = findOrCreateSequencePath(OFstatic_cast(DcmSequenceOfItems*, obj),
                                      pathCopy);
  else
    return EC_IllegalParameter;
  if (result.bad())
...
OFCondition DcmPathProcessor::applyPathWithValue(DcmDataset* dataset,
  ...
  // create path
  OFCondition result =
      findOrCreatePath(dataset, path, OFTrue /* create if necessary */);
  if (result.bad())
    return result;
  ...
  if (!last->m_obj->isLeaf()) {
    if (value.empty())
      ...
      return makeOFCondition(
          OFM_dcmdata, 25, OF_error,
          "Cannot insert value into path ending with item or sequence");
  }
...
// Resets status (including results) of DcmPathProcessor and frees corresponding
// memory
...
// Helper function that deletes last DICOM element from a path from the DICOM
// hierarchy
OFCondition DcmPathProcessor::deleteLastElemFromPath(DcmObject* objSearchedIn,
  ...
  if (path->size() == 1) {
    ...
    if ((objSearchedIn->ident() != EVR_item) &&
        (objSearchedIn->ident() != EVR_dataset))
      return makeOFCondition(
          OFM_dcmdata, 25, OF_error,
          "Cannot search leaf element in object being not an item");
  ...
  if (containingItem == NULL)
    return EC_IllegalCall;
  OFCondition result = containingItem->findAndDeleteElement(
      toDelete->m_obj->getTag(), OFFalse, OFFalse);
  return result;
}

// Helper function that deletes last DICOM item from a path from the DICOM
// hierarchy
OFCondition DcmPathProcessor::deleteLastItemFromPath(DcmObject* objSearchedIn,
  ...
  if (path->size() == 1) {
    ...
    if (objSearchedIn->ident() != EVR_SQ)
      return makeOFCondition(
          OFM_dcmdata, 25, OF_error,
          "Cannot search item in object being not a sequence");
  ...
  if (containingSeq == NULL)
    return EC_IllegalCall;
  DcmItem* item2BDeleted =
      containingSeq->remove(OFstatic_cast(DcmItem*, toDelete->m_obj));
...
OFCondition DcmPathProcessor::findOrCreateItemPath(DcmItem* item,
  ...
  if (tag.getEVR() == EVR_SQ) {
    ...
    if (!seq)
      ...
      // if sequence could be inserted and there is nothing more to do: add
      // current path to results and return success
      if (restPath.empty()) {
  ...
  } else if (restPath.empty())  // we inserted a leaf element: path must be
                                // completed
  {
    ...
    status = makeOFCondition(
        OFM_dcmdata, 25, OF_error,
        "Invalid Path: Non-sequence tag found with rest path following");

  // in case of errors: delete result path copy and delete DICOM element if it
  // was newly created
  if (status.bad() && (elem != NULL)) {
    ...
    if (newlyCreated)  // only delete from this dataset and memory if newly
                       // created ("undo")
    {
...
OFCondition DcmPathProcessor::findOrCreateSequencePath(DcmSequenceOfItems* seq,
  ...
  Uint32 itemNo = 0;
  Uint32 newlyCreated =
      0;  // number of items created (appended) (only non-wildcard mode)
  ...
  if (isWildcard) {
    ...
    if (numItems == 0) {
      if (!m_createIfNecessary)
        ...
        return makeOFCondition(OFM_dcmdata, 25, OF_error,
                               "Cannot insert unspecified number (wildcard) of "
                               "items into sequence");
    }
    ...
    for (itemNo = 0; itemNo < numItems; itemNo++) {
       ...
       */
      if (oneItem != NULL) {
        // if the item was the last thing to parse, add list to results and
        // return
        if (restPath.empty()) {
  ...
  // item does not exist and should not be created newly, return "path not
  // found"
  else
    return EC_TagNotFound;
  ...
  // finding/creating the path was successful. now check whether there is more
  // to do
  if (!restPath.empty()) {
    ...
    delete itemNode;
    itemNode = NULL;
    // in case of no success, delete any items that were newly created and
    // return error
    if (status.bad()) {
...
DcmTagKey DcmPathProcessor::calcPrivateReservationTag(
    const DcmTagKey& privateKey) {
...
OFCondition DcmPathProcessor::checkPrivateTagReservation(
    DcmItem* item /* in */, DcmTag& tag /* in/out */) {
  OFCondition result;
  ...
  if (m_checkPrivateReservations && !tag.isPrivateReservation()) {
    ...
    if (reservationKey == DCM_UndefinedTagKey) {
      OFString msg(
          "Path evaluation error: Unable to compute private reservation for "
          "tag: ");
      msg += tag.toString();
      return makeOFCondition(OFM_dcmdata, 25, OF_error, msg.c_str());
    }
    if (!item->tagExists(reservationKey)) {
      OFString msg("Invalid path: No private reservation found for tag: ");
      msg += tag.toString();
      return makeOFCondition(OFM_dcmdata, 25, OF_error, msg.c_str());
    } else {
      ...
      if (result.bad() || (privCreator.empty())) {
        privCreator = "Invalid or empty private creator tag: ";
        privCreator += reservationKey.toString();
