...
DcmListNode::DcmListNode(DcmObject* obj)
    : nextNode(NULL), prevNode(NULL), objNodeValue(obj) {}
...
DcmList::DcmList()
    : firstNode(NULL), lastNode(NULL), currentNode(NULL), cardinality(0) {}
...
DcmObject* DcmList::seek_to(unsigned long absolute_position) {
  const unsigned long tmppos =
      absolute_position < cardinality ? absolute_position : cardinality;
  seek(ELP_first);
 ...
 * Introduced new function to delete all elements (including memory
 * de-allocation) from DcmList.
 *
