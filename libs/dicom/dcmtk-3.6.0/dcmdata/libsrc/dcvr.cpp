...
static const DcmVREntry DcmVRDict[] = {
    ...
    {EVR_OB, "OB", sizeof(Uint8), DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},
    {EVR_OF, "OF", sizeof(Float32), DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},
    {EVR_OW, "OW", sizeof(Uint16), DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},
    ...
    {EVR_SQ, "SQ", 0, DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},
    ...
    {EVR_UT, "UT", sizeof(char),
     DCMVR_PROP_ISASTRING | DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},
    {EVR_ox, "ox", sizeof(Uint8),
     DCMVR_PROP_NONSTANDARD | DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},
    {EVR_xs, "xs", sizeof(Uint16), DCMVR_PROP_NONSTANDARD, 2, 2},
    {EVR_lt, "lt", sizeof(Uint16),
     DCMVR_PROP_NONSTANDARD | DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},
    ...
    {EVR_item, "it_EVR_item", 0, DCMVR_PROP_NONSTANDARD | DCMVR_PROP_INTERNAL,
     0, 0},
    ...
    /* Moved from internal use to non standard only: necessary to distinguish
       from "normal" OB */
    {EVR_pixelItem, "pi", sizeof(Uint8), DCMVR_PROP_NONSTANDARD, 0,
     DCM_UndefinedLength},

    {EVR_UNKNOWN, "??", sizeof(Uint8), /* EVR_UNKNOWN (i.e. "future" VRs) should
                                          be mapped to UN or OB */
     DCMVR_PROP_NONSTANDARD | DCMVR_PROP_INTERNAL |
         DCMVR_PROP_EXTENDEDLENGTHENCODING,
     0, DCM_UndefinedLength},

    /* Unknown Value Representation - Supplement 14 */
    {EVR_UN, "UN", sizeof(Uint8), DCMVR_PROP_EXTENDEDLENGTHENCODING, 0,
     DCM_UndefinedLength},

    /* Pixel Data - only used in ident() */
    {EVR_PixelData, "PixelData", 0, DCMVR_PROP_INTERNAL, 0,
     DCM_UndefinedLength},
    /* Overlay Data - only used in ident() */
    {EVR_OverlayData, "OverlayData", 0, DCMVR_PROP_INTERNAL, 0,
     DCM_UndefinedLength},

    {EVR_UNKNOWN2B, "??",
     sizeof(Uint8), /* illegal VRs, we assume no extended length coding */
     DCMVR_PROP_NONSTANDARD | DCMVR_PROP_INTERNAL, 0, DCM_UndefinedLength},
...
DcmVRDict_checker::DcmVRDict_checker() : error_found(OFFalse) {
  for (int i = 0; i < DcmVRDict_DIM; i++) {
    if (DcmVRDict[i].vr != i) {
      ...
      DCMDATA_FATAL("DcmVRDict: Internal ERROR: inconsistent indexing: "
                    << DcmVRDict[i].vrName);
      abort();
    }
  }
...
void DcmVR::setVR(DcmEVR evr) {
  if ((OFstatic_cast(int, evr) >= 0) &&
      (OFstatic_cast(int, evr) < DcmVRDict_DIM)) {
    vr = evr;
  } else {
    vr = EVR_UNKNOWN;
  }
...
size_t DcmVR::getValueWidth(void) const {
...
OFBool DcmVR::isStandard() const {
  return (DcmVRDict[vr].propertyFlags & DCMVR_PROP_NONSTANDARD) ? OFFalse
                                                                : OFTrue;
}
...
OFBool DcmVR::isaString() const {
  return (DcmVRDict[vr].propertyFlags & DCMVR_PROP_ISASTRING) ? OFTrue
                                                              : OFFalse;
}
 ...
 * OFFalse, i.e. to retain the previous behavior.
 *
