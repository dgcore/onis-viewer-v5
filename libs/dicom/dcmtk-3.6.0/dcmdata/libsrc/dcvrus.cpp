...
void DcmUnsignedShort::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmUnsignedShort::putUint16Array(const Uint16* uintVals,
  ...
  if (numUints > 0) {
    ...
    if (uintVals != NULL)
      errorFlag =
          putValue(uintVals, sizeof(Uint16) * OFstatic_cast(Uint32, numUints));
    else
...
OFCondition DcmUnsignedShort::verify1(const OFBool autocorrect) {
