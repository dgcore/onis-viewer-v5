...
DcmZLibInputFilter::DcmZLibInputFilter()
  ...
  if (zstream_ && inputBuf_ && outputBuf_) {
    ...
    if (dcmZlibExpectRFC1950Encoding.get()) {
       ...
       * return Z_STREAM_END.
       */
      if (Z_OK == inflateInit2(zstream_, -MAX_WBITS))
        status_ = EC_Normal;
      else {
        OFString etext = "ZLib Error: ";
        if (zstream_->msg)
...
DcmZLibInputFilter::~DcmZLibInputFilter() {
  if (zstream_) {
    inflateEnd(zstream_);  // discards any unprocessed input and does not flush
                           // any pending output
    delete zstream_;
  }
  delete[] inputBuf_;
...
offile_off_t DcmZLibInputFilter::read(void* buf, offile_off_t buflen) {
  ...
  do {
    ...
    if (outputBufCount_) {
      ...
      if (offset >= DCMZLIBINPUTFILTER_BUFSIZE)
        offset -= DCMZLIBINPUTFILTER_BUFSIZE;

      availBytes = outputBufCount_;
      if (offset + availBytes > DCMZLIBINPUTFILTER_BUFSIZE)
        availBytes = DCMZLIBINPUTFILTER_BUFSIZE - offset;
      if (availBytes > buflen)
        availBytes = buflen;

      if (availBytes)
        memcpy(target, outputBuf_ + offset, OFstatic_cast(size_t, availBytes));
      target += availBytes;
      result += availBytes;
      buflen -= availBytes;
      ...
      if (outputBufPutback_ > DCMZLIBINPUTFILTER_PUTBACKSIZE) {
        ...
        if (outputBufStart_ >= DCMZLIBINPUTFILTER_BUFSIZE)
          outputBufStart_ -= DCMZLIBINPUTFILTER_BUFSIZE;
  ...
  // we're either done or the output buffer is empty because of producer
  // suspension
  return result;
}
...
offile_off_t DcmZLibInputFilter::fillInputBuffer() {
  ...
  if (status_.good() && current_ &&
      (inputBufCount_ < DCMZLIBINPUTFILTER_BUFSIZE)) {
    // use first part of input buffer
    if (inputBufStart_ + inputBufCount_ < DCMZLIBINPUTFILTER_BUFSIZE) {
      result = current_->read(
          inputBuf_ + inputBufStart_ + inputBufCount_,
    ...
    if (inputBufCount_ < DCMZLIBINPUTFILTER_BUFSIZE &&
      ...
      offile_off_t result2 = current_->read(
          inputBuf_ +
              (inputBufStart_ + inputBufCount_ - DCMZLIBINPUTFILTER_BUFSIZE),
...
offile_off_t DcmZLibInputFilter::decompress(const void* buf,
  ...
  // decompress from inputBufStart_ to end of data or end of buffer, whatever
  // comes first
  offile_off_t numBytes =
      (inputBufStart_ + inputBufCount_ > DCMZLIBINPUTFILTER_BUFSIZE)
          ? (DCMZLIBINPUTFILTER_BUFSIZE - inputBufStart_)
          : inputBufCount_;
  ...
  if (numBytes || buflen) {
    ...
    if (astatus == Z_OK || astatus == Z_BUF_ERROR) { /* everything OK */
      ...
      if (!eos_) {
        offile_off_t count =
            inputBufCount_ -
            (numBytes - OFstatic_cast(offile_off_t, zstream_->avail_in));
        if (count > 2) {
           ...
           */
          DCMDATA_WARN("zlib: " << OFstatic_cast(ulong, count - 1)
                                << " pending input bytes in buffer.");
        }
      }
#endif
      eos_ = OFTrue;
    ...
    if (inputBufStart_ == DCMZLIBINPUTFILTER_BUFSIZE) {
      ...
      if (inputBufCount_ && (zstream_->avail_out > 0)) {
        ...
        if (astatus == Z_OK || astatus == Z_BUF_ERROR) { /* everything OK */
          ...
          if (!eos_) {
            offile_off_t count =
                OFstatic_cast(offile_off_t, zstream_->avail_in);
            if (count > 2) {
              /* we silently ignore up to two trailing bytes after the end of
               * the deflated stream. One byte has been added by ourselves to
               * make sure zlib detects eos, another one might be the padding
               * necessary for odd length zlib streams transmitted through a
               * DICOM network (where PDVs always have even length). Everything
               * else generates a warning.
               */
              DCMDATA_WARN("zlib: " << OFstatic_cast(ulong, count - 1)
                                    << " pending input bytes in buffer.");
            }
...
void DcmZLibInputFilter::fillOutputBuffer() {
  ...
  do {
    ...
    availBytes =
        DCMZLIBINPUTFILTER_BUFSIZE - (outputBufPutback_ + outputBufCount_);
    if (offset + availBytes > DCMZLIBINPUTFILTER_BUFSIZE)
      availBytes = DCMZLIBINPUTFILTER_BUFSIZE - offset;
