...
DcmSequenceOfItems& DcmSequenceOfItems::operator=(
    const DcmSequenceOfItems& obj) {
  if (this != &obj) {
          ...
          if (!obj.itemList->empty()) {
            ...
            do {
              ...
              switch (oldDO->ident()) {
                ...
                case EVR_pixelItem:
                  newDO =
                      new DcmPixelItem(*OFstatic_cast(DcmPixelItem*, oldDO));
                  break;
                case EVR_metainfo:
...
void DcmSequenceOfItems::print(STD_NAMESPACE ostream& out, const size_t flags,
  ...
  if (flags & DCMTypes::PF_showTreeStructure) {
    ...
    if (getLengthField() == DCM_UndefinedLength)
      printInfoLine(out, flags, level, "(SequenceDelimitationItem)",
                    &delimItemTag);
    else
      printInfoLine(out, flags, level,
                    "(SequenceDelimitationItem for re-encod.)", &delimItemTag);
  }
}
...
Uint32 DcmSequenceOfItems::calcElementLength(const E_TransferSyntax xfer,
  ...
  /* Get length of sequence header + sequence content (will call
   * DcmSequenceOfItems::getLength()) */
  ...
  if (!itemList->empty()) {
    ...
    do {
      ...
      /* explicit length: be sure that total size of contained elements fits
         into sequence's 32 Bit length field. If not, switch encoding
         automatically to undefined length for this sequence. Nevertheless, any
         contained items will be written with explicit length if possible.
       */
      if ((enctype == EET_ExplicitLength) &&
          OFStandard::check32BitAddOverflow(seqlen, sublen)) {
        if (dcmWriteOversizedSeqsAndItemsUndefined.get()) {
          DCMDATA_WARN("DcmSequenceOfItems: Explicit length of sequence "
                       << getTagName() << " " << getTag()
                       << " exceeds 32-bit length field, trying to treat it as "
                          "undefined length instead");
        } else {
          DCMDATA_WARN("DcmSequenceOfItems: Explicit length of sequence "
                       << getTagName() << " " << getTag()
                       << " exceeds 32-bit length field, writing with explicit "
...
OFCondition DcmSequenceOfItems::readTagAndLength(DcmInputStream& inStream,
  ...
  if (l_error.bad())
    DCMDATA_TRACE("DcmSequenceOfItems::readTagAndLength() returns error = "
                  << l_error.text());
  return l_error;
}
...
OFCondition DcmSequenceOfItems::readSubItem(DcmInputStream& inStream,
  ...
  if (l_error.good() && (subObject != NULL)) {
    ...
    l_error =
        subObject->read(inStream, xfer, glenc, maxReadLength);  // read sub-item
  ...
  } else if (l_error == EC_InvalidTag)  // try to recover parsing
  {
    ...
    DCMDATA_DEBUG(
        "DcmSequenceOfItems::readSubItem(): parse error occurred: " << newTag);
...
OFCondition DcmSequenceOfItems::read(DcmInputStream& inStream,
  ...
  if (getTransferState() == ERW_notInitialized)
    ...
    if (errorFlag.good() && inStream.eos())
      ...
      while (inStream.good() && ((getTransferredBytes() < getLengthField()) ||
                                 !lastItemComplete)) {
        DcmTag newTag;
        ...
        if (lastItemComplete) {
          errorFlag =
              readTagAndLength(inStream, readxfer, newTag, newValueLength);
          ...
          errorFlag = readSubItem(inStream, newTag, newValueLength, readxfer,
                                  glenc, maxReadLength);
          if (errorFlag.good())
            lastItemComplete = OFTrue;
        } else {
          errorFlag =
              itemList->get()->read(inStream, readxfer, glenc, maxReadLength);
          if (errorFlag.good())
            lastItemComplete = OFTrue;
        }
        setTransferredBytes(
      ...
      if (((getTransferredBytes() < getLengthField()) || !lastItemComplete) &&
          errorFlag.good())
...
OFCondition DcmSequenceOfItems::writeTagAndVR(DcmOutputStream& outStream,
  ...
  if (l_error.good()) {
    ...
    if (oxferSyn.isExplicitVR()) {
      ...
      /* create another data type object on the basis of the above created
       * object */
      DcmVR outvr(myvr.getValidEVR());
      /* in case we are dealing with a transfer syntax with explicit VR (see if
       * above) */
      /* and the actual VR uses extended length encoding (see DICOM standard
       * (year 2000) */
      /* part 5, section 7.1.2) (or the corresponding section in a later version
       * of the */
      /* standard) we have to add 2 reserved bytes (set to a value of 00H) to
       * the data */
      /* type field and the actual length field is 4 bytes wide. Write the
       * corresponding */
...
OFCondition DcmSequenceOfItems::insert(DcmItem* item, unsigned long where,
                                       OFBool before) {
  errorFlag = EC_Normal;
  if (item != NULL) {
...
DcmItem* DcmSequenceOfItems::getItem(const unsigned long num) {
  ...
  DcmItem* item;
  item =
      OFstatic_cast(DcmItem*, itemList->seek_to(num));  // read item from list
  if (item == NULL)
    errorFlag = EC_IllegalCall;
  return item;
}
  ...
  if (!obj)
    ...
    if (itemList->get() != obj) {
      for (DcmObject* search_obj = itemList->seek(ELP_first);
           ...
           search_obj = itemList->seek(ELP_next)) { /* do nothing */
      }
    }
...
OFCondition DcmSequenceOfItems::verify1(const OFBool autocorrect) {
  errorFlag = EC_Normal;
  if (!itemList->empty()) {
    DcmObject* dO;
    itemList->seek(ELP_first);
 ...
 * continue search: push pointer to sub-element onto resultStack and start
 * sub-search
 */
...
OFCondition DcmSequenceOfItems::search(const DcmTagKey& tag,
  ...
  if ((mode == ESM_afterStackTop) && (resultStack.top() == this)) {
    ...
    if ((mode == ESM_fromHere) || resultStack.empty()) {
      ...
      if (dO == this)
        l_error = searchSubFromHere(tag, resultStack, searchIntoSub);
      else {  // continue directly in sub-tree
      ...
      if ((dO != this) &&
          (resultStack.card() > 0)) {  // highest level is never in resultStack
...
OFCondition DcmSequenceOfItems::getPartialValue(void* /* targetBuffer */,
  ...
  // cannot use getPartialValue() with class DcmSequenceOfItems or derived
  // classes
  return EC_IllegalCall;
}
