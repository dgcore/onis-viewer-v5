...
DcmTime::DcmTime(const DcmTime& old) : DcmByteString(old) {}
...
OFCondition DcmTime::checkValue(const OFString& vm, const OFBool oldFormat) {
  OFString strVal;
...
OFCondition DcmTime::getOFString(OFString& stringVal, const unsigned long pos,
                                 OFBool normalize) {
...
OFCondition DcmTime::getOFTime(OFTime& timeValue, const unsigned long pos,
...
OFCondition DcmTime::getISOFormattedTime(OFString& formattedTime,
  ...
  OFCondition l_error = getOFString(dicomTime, pos);
  if (l_error.good())
    l_error = getISOFormattedTimeFromString(dicomTime, formattedTime, seconds,
                                            fraction, createMissingPart,
                                            supportOldFormat);
  else
    formattedTime.clear();
  return l_error;
...
OFCondition DcmTime::getCurrentTime(OFString& dicomTime, const OFBool seconds,
                                    const OFBool fraction) {
  ...
  if (timeValue.setCurrentTime()) {
    ...
    if (timeValue.getISOFormattedTime(dicomTime, seconds, fraction,
                                      OFFalse /*timeZone*/,
  ...
  if (l_error.bad()) {
    /* if the current system time cannot be retrieved create a valid default
     * time */
    if (seconds) {
      if (fraction) {
...
OFCondition DcmTime::getISOFormattedTimeFromString(
    const OFString& dicomTime, OFString& formattedTime, const OFBool seconds,
    const OFBool fraction, const OFBool createMissingPart,
    const OFBool supportOldFormat) {
  ...
  if (supportOldFormat || (dicomTime.find(":") == OFString_npos)) {
    ...
    const size_t minPos =
        (supportOldFormat && (length > 2) && (dicomTime[2] == ':')) ? 3 : 2;
    const size_t secPos = (supportOldFormat && (length > minPos + 2) &&
                           (dicomTime[minPos + 2] == ':'))
                              ? minPos + 3
                              : minPos + 2;
...
OFCondition DcmTime::getTimeZoneFromString(const OFString& dicomTimeZone,
  ...
  if ((dicomTimeZone.length() == 5) &&
      ((dicomTimeZone[0] == '+') || (dicomTimeZone[0] == '-'))) {
    signed int hour;
    unsigned int minute;
...
OFCondition DcmTime::checkStringValue(const OFString& value, const OFString& vm,
