...
DcmSignedShort::DcmSignedShort(const DcmSignedShort& old) : DcmElement(old) {}
...
void DcmSignedShort::print(STD_NAMESPACE ostream& out, const size_t flags,
...
OFCondition DcmSignedShort::putSint16Array(const Sint16* sintVals,
  ...
  if (numSints > 0) {
    ...
    if (sintVals != NULL)
      errorFlag =
          putValue(sintVals, sizeof(Sint16) * OFstatic_cast(Uint32, numSints));
    else
...
OFCondition DcmSignedShort::verify1(const OFBool autocorrect) {
