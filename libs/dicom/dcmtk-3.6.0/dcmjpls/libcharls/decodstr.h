...
// Implements encoding to stream of bits. In encoding mode JpegLsCodec inherits
// from EncoderStrategy
...
public:
  ...
  virtual size_t DecodeScan(void* outputData, const JlsRect& size,
                            const void* compressedData, size_t byteCount,
                            bool bCheck) = 0;
  ...
  void OnLineBegin(LONG /*cpixel*/, void* /*ptypeBuffer*/,
                   LONG /*pixelStride*/) {}
  ...
  inlinehint bool OptimizedRead() {
    // Easy & fast: if there is no 0xFF byte in sight, we can read without
    // bitstuffing
