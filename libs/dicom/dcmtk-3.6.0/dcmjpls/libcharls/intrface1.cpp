...
JLS_ERROR CheckInput(const void* compressedData, size_t compressedLength,
                     const void* uncompressedData, size_t uncompressedLength,
  ...
  int bytesperline = pparams->bytesperline < 0 ? -pparams->bytesperline
                                               : pparams->bytesperline;
...
CHARLS_IMEXPORT(JLS_ERROR)
JpegLsEncode(void* compressedData, size_t compressedLength,
             size_t* pcbyteWritten, const void* uncompressedData,
             size_t uncompressedLength, struct JlsParameters* pparams) {
  ...
  JLS_ERROR parameterError =
      CheckInput(compressedData, compressedLength, uncompressedData,
                 uncompressedLength, &info);
  ...
  if (info.ilv == ILV_NONE) {
    ...
    for (LONG component = 0; component < info.components; ++component) {
      const BYTE* compareData =
          static_cast<const BYTE*>(uncompressedData) + component * cbyteComp;
...
CHARLS_IMEXPORT(JLS_ERROR)
JpegLsDecode(void* uncompressedData, size_t uncompressedLength,
...
CHARLS_IMEXPORT(JLS_ERROR)
JpegLsDecodeRect(void* uncompressedData, size_t uncompressedLength,
                 const void* compressedData, size_t compressedLength,
                 JlsRect roi, JlsParameters* info) {
  JLSInputStream reader((BYTE*)compressedData, compressedLength);
...
CHARLS_IMEXPORT(JLS_ERROR)
JpegLsVerifyEncode(const void* uncompressedData, size_t uncompressedLength,
                   const void* compressedData, size_t compressedLength) {
  ...
  error = CheckInput(compressedData, compressedLength, uncompressedData,
                     uncompressedLength, &info);
...
CHARLS_IMEXPORT(JLS_ERROR)
JpegLsReadHeader(const void* compressedData, size_t compressedLength,
                 JlsParameters* pparams) {
  try {
    JLSInputStream reader((BYTE*)compressedData, compressedLength);
    reader.ReadHeader();
