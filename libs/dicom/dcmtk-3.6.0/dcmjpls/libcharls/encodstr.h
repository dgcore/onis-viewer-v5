...
public:
  ...
  virtual size_t EncodeScan(const void* pvoid, void* pvoidOut, size_t byteCount,
                            void* pvoidCompare) = 0;
...
protected:
  ...
  void AppendToBitStream(LONG value, LONG length) {
    ...
    ASSERT((_qdecoder.get() == NULL) || (length == 0 && value == 0) ||
           (_qdecoder->ReadLongValue(length) == value));
