...
enum interleavemode { ILV_NONE = 0, ILV_LINE = 1, ILV_SAMPLE = 2 };
...
struct JfifParameters {
  ...
  short Xthumb;
  short Ythumb;
  void* pdataThumbnail;  // user must set buffer which size is
                         // Xthumb*Ythumb*3(RGB) before JpegLsDecode()
};
...
struct JlsParameters {
  ...
  int height;
  int bitspersample;
  int bytesperline;  // for [source (at encoding)][decoded (at decoding)] pixel
                     // image in user buffer
  int components;
