...
JLS_ERROR CheckParameterCoherent(const JlsParameters* pparams) {
  ...
  switch (pparams->components) {
    case 4:
      return pparams->ilv == ILV_SAMPLE ? ParameterValueNotSupported : OK;
    case 3:
      return OK;
    case 1:
      return pparams->ilv != ILV_NONE ? ParameterValueNotSupported : OK;
    case 0:
...
//
JpegSegment* CreateMarkerStartOfFrame(Size size, LONG bitsPerSample,
                                      LONG ccomp) {
  OFVector<BYTE> vec;
...
void JLSInputStream::ReadPixels(void* pvoid, LONG cbyteAvailable) {
  ...
  unsigned long availableSamples =
      cbyteAvailable / (unsigned long)(bytesPerSample * _info.components);
...
JpegMarkerSegment* EncodeStartOfScan(const JlsParameters* pparams,
                                     LONG icomponent) {
  BYTE itable = 0;
...
void JLSInputStream::ReadJfif() {
  ...
  if (_info.jfif.Xthumb > 0 && _info.jfif.pdataThumbnail) {
    OFVector<char> tempbuff((char*)_info.jfif.pdataThumbnail,
                            (char*)_info.jfif.pdataThumbnail +
                                3 * _info.jfif.Xthumb * _info.jfif.Ythumb);
...
JpegMarkerSegment* CreateJFIF(const JfifParameters* jfif) {
  ...
  if (jfif->Xthumb > 0) {
    ...
    rgbyte.insert(
        rgbyte.end(), (BYTE*)jfif->pdataThumbnail,
...
void JLSInputStream::ReadScan(void* pvout) {
  OFauto_ptr<DecoderStrategy> qcodec =
      JlsCodecFactory<DecoderStrategy>().GetCodec(_info, _info.custom);

  _cbyteOffset += qcodec->DecodeScan(pvout, _rect, _pdata + _cbyteOffset,
                                     _cbyteLength - _cbyteOffset, _bCompare);
}

class JpegImageDataSegment : public JpegSegment {
public:
  JpegImageDataSegment(const void* pvoidRaw, const JlsParameters& info,
                       LONG icompStart, int ccompScan)
  ...
  void Write(JLSOutputStream* pstream) {
    ...
    OFauto_ptr<EncoderStrategy> qcodec =
        JlsCodecFactory<EncoderStrategy>().GetCodec(info, _info.custom);
    size_t cbyteWritten = qcodec->EncodeScan(
        (BYTE*)_pvoidRaw, pstream->GetPos(), pstream->GetLength(),
        pstream->_bCompare ? pstream->GetPos() : NULL);
...
void JLSOutputStream::AddScan(const void* compareData,
  ...
  _icompLast += 1;
  _segments.push_back(
      EncodeStartOfScan(pparams, pparams->ilv == ILV_NONE ? _icompLast : -1));

  //	Size size = Size(pparams->width, pparams->height);
  int ccomp = pparams->ilv == ILV_NONE ? 1 : pparams->components;
  _segments.push_back(
      new JpegImageDataSegment(compareData, *pparams, _icompLast, ccomp));
}
