...
// internal line buffers. Color transforms work best for computer generated
// images.
//
...
struct TransformHp2 {
  ...
  inlinehint Triplet<SAMPLE> operator()(int R, int G, int B) {
    return Triplet<SAMPLE>(R - G + RANGE / 2, G,
                           B - ((R + G) >> 1) - RANGE / 2);
  }
};
...
// Transform class that shifts bits towards the high bit when bitcount is not 8
// or 16 needed to make the HP color transforms work correctly.
...
struct TransformShifted {
  ...
  struct INVERSE {
    ...
    inlinehint Triplet<SAMPLE> operator()(int v1, int v2, int v3) {
      Triplet<SAMPLE> result =
          _inverseTransform(v1 << _shift, v2 << _shift, v3 << _shift);

      return Triplet<SAMPLE>(result.R >> _shift, result.G >> _shift,
                             result.B >> _shift);
    }
    ...
    inlinehint Quad<SAMPLE> operator()(int v1, int v2, int v3, int v4) {
      ...
      return Quad<SAMPLE>(result.R >> _shift, result.G >> _shift,
                          result.B >> _shift, v4);
    }
  ...
  TransformShifted(int shift) : _shift(shift) {}

  inlinehint Triplet<SAMPLE> operator()(int R, int G, int B) {
    Triplet<SAMPLE> result =
        _colortransform(R << _shift, G << _shift, B << _shift);
