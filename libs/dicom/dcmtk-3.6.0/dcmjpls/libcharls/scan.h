...
// Two alternatives for GetPredictedValue() (second is slightly faster due to
// reduced branching)
...
public:
  ...
  Triplet<SAMPLE> EncodeRIPixel(Triplet<SAMPLE> x, Triplet<SAMPLE> Ra,
                                Triplet<SAMPLE> Rb);
  ...
  SAMPLE DoRegular(LONG Qs, LONG x, LONG pred, EncoderStrategy*) {
    ...
    EncodeMappedValue(
        k, GetMappedErrVal(ctx.GetErrorCorrection(k | traits.NEAR) ^ ErrVal),
        traits.LIMIT);
    ctx.UpdateVariables(ErrVal, traits.NEAR, traits.RESET);
    ASSERT(traits.IsNear(
        traits.ComputeReconstructedSample(Px, ApplySign(ErrVal, sign)), x));
    return static_cast<SAMPLE>(
...
public:
  ...
  size_t EncodeScan(const void* rawData, void* pvoidOut,
                    size_t compressedLength, void* pvoidCompare);
  size_t DecodeScan(void* rawData, const JlsRect& size,
...
inlinehint OFPair<LONG, LONG> CreateEncodedValue(LONG k, LONG mappedError) {
  ...
  return OFMake_pair<LONG, LONG>(
      highbits + k + 1, (LONG(1) << k) | (mappedError & ((LONG(1) << k) - 1)));
}
...
template <class TRAITS, class STRATEGY>
inlinehint void JlsCodec<TRAITS, STRATEGY>::EncodeMappedValue(LONG k,
                                                              LONG mappedError,
                                                              LONG limit) {
  LONG highbits = mappedError >> k;
  ...
  STRATEGY::AppendToBitStream((mappedError - 1) & ((1 << traits.qbpp) - 1),
                              traits.qbpp);
}
...
void JlsCodec<TRAITS, STRATEGY>::InitQuantizationLUT() {
  // for lossless mode with default parameters, we have precomputed te luts for
  // bitcounts 8,10,12 and 16
...
template <class TRAITS, class STRATEGY>
void JlsCodec<TRAITS, STRATEGY>::EncodeRIError(CContextRunMode& ctx,
                                               LONG Errval) {
  LONG k = ctx.GetGolomb();
...
// JlsCodec<TRAITS,STRATEGY>::DecodeRIPixel(Triplet<SAMPLE> Ra, Triplet<SAMPLE>
// Rb)
Triplet<typename TRAITS::SAMPLE> JlsCodec<TRAITS, STRATEGY>::DecodeRIPixel(
    Triplet<typename TRAITS::SAMPLE> Ra, Triplet<typename TRAITS::SAMPLE> Rb) {
  ...
  return Triplet<SAMPLE>(
      traits.ComputeReconstructedSample(Rb.v1, Errval1 * Sign(Rb.v1 - Ra.v1)),
...
template <class TRAITS, class STRATEGY>
// Triplet<OFTypename TRAITS::SAMPLE>
// JlsCodec<TRAITS,STRATEGY>::EncodeRIPixel(Triplet<SAMPLE> x, Triplet<SAMPLE>
// Ra, Triplet<SAMPLE> Rb)
Triplet<typename TRAITS::SAMPLE> JlsCodec<TRAITS, STRATEGY>::EncodeRIPixel(
    Triplet<typename TRAITS::SAMPLE> x, Triplet<typename TRAITS::SAMPLE> Ra,
    Triplet<typename TRAITS::SAMPLE> Rb) {
...
template <class TRAITS, class STRATEGY>
void JlsCodec<TRAITS, STRATEGY>::EncodeRunPixels(LONG runLength,
                                                 bool endOfLine) {
  ...
  if (endOfLine) {
    ...
    STRATEGY::AppendToBitStream(
        runLength, J[_RUNindex] + 1);  // leading 0 + actual remaining length
  }
}

template <class TRAITS, class STRATEGY>
LONG JlsCodec<TRAITS, STRATEGY>::DecodeRunPixels(PIXEL Ra, PIXEL* startPos,
                                                 LONG cpixelMac) {
  LONG index = 0;
...
LONG JlsCodec<TRAITS, STRATEGY>::DoRunMode(LONG index, EncoderStrategy*) {
  ...
  ptypeCurX[runLength] =
      EncodeRIPixel(ptypeCurX[runLength], Ra, ptypePrevX[runLength]);
  DecrementRunIndex();
  return runLength + 1;
}
...
LONG JlsCodec<TRAITS, STRATEGY>::DoRunMode(LONG startIndex, DecoderStrategy*) {
  ...
  LONG runLength =
      DecodeRunPixels(Ra, _currentLine + startIndex, _width - startIndex);
...
void JlsCodec<TRAITS, STRATEGY>::DoLine(SAMPLE*) {
  ...
  while (index < _width) {
    ...
    LONG Qs =
        ComputeContextID(QuantizeGratient(Rd - Rb), QuantizeGratient(Rb - Rc),
                         QuantizeGratient(Rc - Ra));

    if (Qs != 0) {
      _currentLine[index] =
          DoRegular(Qs, _currentLine[index], GetPredictedValue(Ra, Rb, Rc),
                    (STRATEGY*)(NULL));
      index++;
    } else {
...
void JlsCodec<TRAITS, STRATEGY>::DoLine(Triplet<SAMPLE>*) {
  ...
  while (index < _width) {
    ...
    LONG Qs1 = ComputeContextID(QuantizeGratient(Rd.v1 - Rb.v1),
    ...
    if (Qs1 == 0 && Qs2 == 0 && Qs3 == 0) {
      ...
      Triplet<SAMPLE> Rx;
      Rx.v1 =
          DoRegular(Qs1, _currentLine[index].v1,
                    GetPredictedValue(Ra.v1, Rb.v1, Rc.v1), (STRATEGY*)(NULL));
      Rx.v2 =
          DoRegular(Qs2, _currentLine[index].v2,
                    GetPredictedValue(Ra.v2, Rb.v2, Rc.v2), (STRATEGY*)(NULL));
      Rx.v3 =
          DoRegular(Qs3, _currentLine[index].v3,
...
template <class TRAITS, class STRATEGY>
void JlsCodec<TRAITS, STRATEGY>::DoScan(BYTE* compressedBytes,
  ...
  for (LONG line = 0; line < Info().height; ++line) {
    ...
    if (_rect.Y <= line && line < _rect.Y + _rect.Height) {
      STRATEGY::OnLineEnd(_rect.Width,
                          _currentLine + _rect.X - (components * pixelstride),
                          pixelstride);
    }
  }
...
// Factory function for ProcessLine objects to copy/transform unencoded pixels
// to/from our scanline buffers.
...
ProcessLine* JlsCodec<TRAITS, STRATEGY>::CreateProcess(void* pvoidOut) {
  ...
  if (Info().colorTransform == 0)
    ...
    // return new ProcessTransformed<TransformNone<OFTypename TRAITS::SAMPLE>
    // >(pvoidOut, Info(), TransformNone<SAMPLE>());
    return new ProcessTransformed<TransformNone<typename TRAITS::SAMPLE>>(
        pvoidOut, Info(), TransformNone<SAMPLE>());
  ...
  if ((Info().bitspersample == sizeof(SAMPLE) * 8)) {
    switch (Info().colorTransform) {
      case COLORXFORM_HP1:
        return new ProcessTransformed<TransformHp1<SAMPLE>>(
            pvoidOut, Info(), TransformHp1<SAMPLE>());
        break;
      case COLORXFORM_HP2:
        return new ProcessTransformed<TransformHp2<SAMPLE>>(
            pvoidOut, Info(), TransformHp2<SAMPLE>());
        break;
      case COLORXFORM_HP3:
        return new ProcessTransformed<TransformHp3<SAMPLE>>(
            pvoidOut, Info(), TransformHp3<SAMPLE>());
        break;
      default:
        throw JlsException(UnsupportedColorTransform);
    }
  } else if (Info().bitspersample > 8) {
    int shift = 16 - Info().bitspersample;
    switch (Info().colorTransform) {
      case COLORXFORM_HP1:
        return new ProcessTransformed<TransformShifted<TransformHp1<USHORT>>>(
            pvoidOut, Info(), TransformShifted<TransformHp1<USHORT>>(shift));
        break;
      case COLORXFORM_HP2:
        return new ProcessTransformed<TransformShifted<TransformHp2<USHORT>>>(
            pvoidOut, Info(), TransformShifted<TransformHp2<USHORT>>(shift));
        break;
      case COLORXFORM_HP3:
        return new ProcessTransformed<TransformShifted<TransformHp3<USHORT>>>(
            pvoidOut, Info(), TransformShifted<TransformHp3<USHORT>>(shift));
        break;
      default:
        throw JlsException(UnsupportedColorTransform);
    }
...
template <class TRAITS, class STRATEGY>
size_t JlsCodec<TRAITS, STRATEGY>::EncodeScan(const void* rawData,
                                              void* compressedData,
                                              size_t compressedLength,
                                              void* pvoidCompare) {
  STRATEGY::_processLine =
      OFauto_ptr<ProcessLine>(CreateProcess(const_cast<void*>(rawData)));

  BYTE* compressedBytes = static_cast<BYTE*>(compressedData);

  if (pvoidCompare != NULL) {
    STRATEGY::_qdecoder = OFauto_ptr<DecoderStrategy>(
        new JlsCodec<TRAITS, DecoderStrategy>(traits, Info()));
...
template <class TRAITS, class STRATEGY>
size_t JlsCodec<TRAITS, STRATEGY>::DecodeScan(void* rawData,
                                              const JlsRect& rect,
  ...
  BYTE* compressedBytes =
      const_cast<BYTE*>(static_cast<const BYTE*>(compressedData));
  _bCompare = bCompare;
...
// Initialize the codec data structures. Depends on JPEG-LS parameters like
// T1-T3.

template <class TRAITS, class STRATEGY>
void JlsCodec<TRAITS, STRATEGY>::InitParams(LONG t1, LONG t2, LONG t3,
                                            LONG nReset) {
  T1 = t1;
  T2 = t2;
  T3 = t3;
