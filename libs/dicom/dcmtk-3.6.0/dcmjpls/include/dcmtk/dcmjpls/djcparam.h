...
public:
   ...
   */
  enum interleaveMode {
    /// Use same interleave mode as the DICOM header says in
    /// planarConfiguration, if possible. The cooked encoder will force
    /// interleaveLine.
    interleaveDefault,
   ...
   *  @param jpls_optionsEnabled       enable/disable use of all five JPEG-LS
   * parameters
   *  @param jpls_t1                   JPEG-LS parameter "Threshold 1" (used for
   * quantization)
   *  @param jpls_t2                   JPEG-LS parameter "Threshold 2"
   *  @param jpls_t3                   JPEG-LS parameter "Threshold 3"
   *  @param jpls_reset                JPEG-LS parameter "RESET", i.e., value at
   * which the counters A, B, and N are halved.
  ...
  DJLSCodecParameter(
      ...
      Uint16 jpls_t2 = 7, Uint16 jpls_t3 = 21, Uint16 jpls_reset = 64,
      Uint16 jpls_limit = 0, OFBool preferCookedEncoding = OFTrue,
  ...
  /** returns true if JPEG-LS parameters T1-T3, RESET and LIMIT are enabled,
   * false otherwise
   *  @return true if JPEG-LS parameters T1-T3, RESET and LIMIT are enabled,
   * false otherwise
   */
...
private:
  ...
  /// JPEG-LS parameter "RESET", i.e., value at which the counters A, B, and N
  /// are halved.
  Uint16 jpls_reset_;
