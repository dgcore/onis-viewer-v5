...
DJLSEncoderBase::DJLSEncoderBase() : DcmCodec() {}
...
OFCondition DJLSEncoderBase::encode(
  ...
  const DJLSCodecParameter* djcp =
      OFreinterpret_cast(const DJLSCodecParameter*, cp);
  const DJLSRepresentationParameter* djrp =
      OFreinterpret_cast(const DJLSRepresentationParameter*, toRepParam);
  double compressionRatio = 0.0;

  if (supportedTransferSyntax() == EXS_JPEGLSLossless ||
      djrp->useLosslessProcess()) {
    if (djcp->cookedEncodingPreferred())
      result = losslessCookedEncode(pixelData, length, dataset, djrp, pixSeq,
                                    djcp, compressionRatio, 0);
    else
...
OFCondition DJLSEncoderBase::updateDerivationDescription(
    DcmItem* dataset, const DJLSRepresentationParameter* djrp,
    double ratio) const {
  ...
  sprintf(buf, " (NEAR=%lu)",
          OFstatic_cast(unsigned long, djrp->getnearlosslessDeviation()));
  derivationDescription += buf;
...
OFCondition DJLSEncoderBase::losslessRawEncode(
    const Uint16* pixelData, const Uint32 length, DcmItem* dataset,
    const DJLSRepresentationParameter* djrp, DcmPixelSequence*& pixSeq,
    const DJLSCodecParameter* djcp, double& compressionRatio) const {
  ...
  if (result.good()) {
    ...
    if (bitsAllocated == 8) {
      ...
      if (photometricInterpretation == "MONOCHROME1" ||
        ...
        // to cooked encoder.
        return losslessCookedEncode(pixelData, length, dataset, djrp, pixSeq,
                                    djcp, compressionRatio, 0);
      }
  ...
  if (result.good()) {
    ...
    for (unsigned long i = 0; (i < frameCount) && (result.good()); ++i) {
      ...
      result = compressRawFrame(framePointer, bitsAllocated, columns, rows,
                                ...
                                photometricInterpretation, pixelSequence,
                                offsetList, compressedFrameSize, djcp);
...
OFCondition DJLSEncoderBase::compressRawFrame(
    const Uint8* framePointer, Uint16 bitsAllocated, Uint16 width,
    ...
    unsigned long& compressedSize, const DJLSCodecParameter* djcp) const {
  OFCondition result = EC_Normal;
  ...
  // Special case: one component images are always ILV_NONE (Standard requires
  // this)
  ...
  if ((jls_params.ilv == ILV_NONE && (ilv == ILV_SAMPLE || ilv == ILV_LINE)) ||
      (ilv == ILV_NONE &&
       (jls_params.ilv == ILV_SAMPLE || jls_params.ilv == ILV_LINE))) {
    DCMJPLS_INFO(
        "Converting image from "
        << (ilv == ILV_NONE ? "color-by-plane" : "color-by-pixel") << " to "
        << (jls_params.ilv == ILV_NONE ? "color-by-plane" : "color-by-pixel"));

    frameBuffer = new Uint8[frameSize];
    if (jls_params.ilv == ILV_NONE)
      result =
          convertToUninterleaved(frameBuffer, framePointer, samplesPerPixel,
                                 width, height, bitsAllocated);
    else
      /* For CharLS, sample-interleaved and line-interleaved is both expected to
       * be color-by-pixel.
       */
      result =
          convertToSampleInterleaved(frameBuffer, framePointer, samplesPerPixel,
                                     width, height, bitsAllocated);
  ...
  if (result.good()) {
    ...
    // way to find out, so we just allocate a buffer large enough for the raw
    // data plus a little more for JPEG metadata. Yes, this is way too much for
    // just a little JPEG metadata, but some test-images showed that the buffer
    // previously was too small. Plus, at some places charls fails to do proper
    // bounds checking and writes behind the end of the buffer (sometimes way
    // behind its end...).
    size_t size = frameSize + 1024;
    Uint8* buffer = new Uint8[size];

    JLS_ERROR err =
        JpegLsEncode(buffer, size, &size, framePointer, frameSize, &jls_params);
    result = DJLSError::convert(err);

    if (result.good()) {
      // 'size' now contains the size of the compressed data in buffer
      compressedSize = size;
      result = pixelSequence->storeCompressedFrame(offsetList, buffer, size,
                                                   fragmentSize);
    }
...
OFCondition DJLSEncoderBase::losslessCookedEncode(
    const Uint16* pixelData, const Uint32 length, DcmItem* dataset,
    const DJLSRepresentationParameter* djrp, DcmPixelSequence*& pixSeq,
    const DJLSCodecParameter* djcp, double& compressionRatio,
  ...
  if (photometricInterpretation != "MONOCHROME1" &&
    ...
    if (nearLosslessDeviation > 0)
      return EC_JLSUnsupportedPhotometricInterpretation;
    else
  ...
  if (pixelRepresentation > 0) {
    // in near lossless mode we cannot handle signed images because JPEG-LS
    // assumes unsigned pixels
    if (nearLosslessDeviation > 0)
      return EC_JLSUnsupportedPixelRepresentation;

    // The cooked encoder only handles unsigned color images
    if (photometricInterpretation == "RGB" ||
        photometricInterpretation == "YBR_FULL") {
      // prevent a loop - only call lossless raw encoder if bitsAllocated is OK
      // for the raw encoder
  ...
  if (result.good() && (bitsPerSample > 16))
    result = EC_JLSUnsupportedBitDepth;
  ...
  if (result.good()) {
    ...
    for (unsigned long i = 0; (i < frameCount) && (result.good()); ++i) {
      // compress frame
      DCMJPLS_INFO("Encoding frame " << (i + 1) << " of " << frameCount);
      result = compressCookedFrame(
          pixelSequence, dimage, photometricInterpretation, offsetList,
          compressedFrameSize, djcp, i, nearLosslessDeviation);
...
OFCondition DJLSEncoderBase::compressCookedFrame(
    ...
    const OFString& /* photometricInterpretation */, DcmOffsetList& offsetList,
    unsigned long& compressedSize, const DJLSCodecParameter* djcp, Uint32 frame,
    Uint16 nearLosslessDeviation) const {
  if (dimage == NULL)
    return EC_IllegalCall;
  ...
  switch (dinter->getRepresentation()) {
    ...
    case EPR_Sint8: {
      ...
      if (samplesPerPixel == 1) {
        const Uint8* yv =
            OFreinterpret_cast(const Uint8*, planes[0]) + framesize * frame;
        ...
        const Uint8* rv =
            OFreinterpret_cast(const Uint8*, planes[0]) + framesize * frame;
        const Uint8* gv =
            OFreinterpret_cast(const Uint8*, planes[1]) + framesize * frame;
        const Uint8* bv =
            OFreinterpret_cast(const Uint8*, planes[2]) + framesize * frame;
    ...
    case EPR_Sint16: {
      ...
      if (samplesPerPixel == 1) {
        const Uint16* yv =
            OFreinterpret_cast(const Uint16*, planes[0]) + framesize * frame;
        ...
        const Uint16* rv =
            OFreinterpret_cast(const Uint16*, planes[0]) + framesize * frame;
        const Uint16* gv =
            OFreinterpret_cast(const Uint16*, planes[1]) + framesize * frame;
        const Uint16* bv =
            OFreinterpret_cast(const Uint16*, planes[2]) + framesize * frame;
  ...
  if (jls_params.ilv == ILV_NONE && jls_params.components != 1) {
    ...
    framePointer = frameBuffer;
    result = convertToUninterleaved(frameBuffer, buffer, samplesPerPixel, width,
                                    height, jls_params.bitspersample);
  }
  ...
  JLS_ERROR err = JpegLsEncode(compressed_buffer, compressed_buffer_size,
                               &compressed_buffer_size, framePointer,
                               buffer_size, &jls_params);
  result = DJLSError::convert(err);

  if (result.good()) {
    // 'compressed_buffer_size' now contains the size of the compressed data in
    // buffer
    compressedSize = compressed_buffer_size;
    result = pixelSequence->storeCompressedFrame(
        offsetList, compressed_buffer, compressed_buffer_size, fragmentSize);
  }
...
OFCondition DJLSEncoderBase::convertToUninterleaved(
    Uint8* target, const Uint8* source, Uint16 components, Uint32 width,
...
OFCondition DJLSEncoderBase::convertToSampleInterleaved(
  ...
  for (Uint32 pos = 0; pos < width * height; pos++) {
    for (int i = 0; i < components; i++) {
      memcpy(target, &source[i * planeSize + pos * bytesAllocated],
             bytesAllocated);
