...
DJLSDecoderBase::DJLSDecoderBase() : DcmCodec() {}
...
OFCondition DJLSDecoderBase::decode(
  ...
  if (result.good()) {
    ...
    if ((dataset->ident() == EVR_dataset) &&
        (djcp->getUIDCreation() == EJLSUC_always)) {
...
OFCondition DJLSDecoderBase::decodeFrame(
  ...
  if (result.good()) {
    ...
    DCMJPLS_DEBUG("Starting to decode frame " << frameNo << " with fragment "
                                              << currentItem);
...
OFCondition DJLSDecoderBase::decodeFrame(
    DcmPixelSequence* fromPixSeq, const DJLSCodecParameter* cp,
  ...
  // compute the number of JPEG-LS fragments we need in order to decode the next
  // frame
  ...
  if (fragmentsForThisFrame == 0)
    result = EC_JLSCannotComputeNumberOfFragments;
  ...
  if (result.good()) {
    ...
    if (result.good()) {
      if (params.width != imageColumns)
        result = EC_JLSImageDataMismatch;
      else if (params.height != imageRows)
        result = EC_JLSImageDataMismatch;
      else if (params.components != imageSamplesPerPixel)
        result = EC_JLSImageDataMismatch;
      else if ((bytesPerSample == 1) && (params.bitspersample > 8))
        result = EC_JLSImageDataMismatch;
      else if ((bytesPerSample == 2) && (params.bitspersample <= 8))
        result = EC_JLSImageDataMismatch;
    }
    ...
    if (!result.good()) {
      ...
      if (result.good() && imageSamplesPerPixel == 3) {
        if (imagePlanarConfiguration == 1 && params.ilv != ILV_NONE) {
          ...
          // it isn't -> convert it.
          DCMJPLS_WARN(
              "different planar configuration in JPEG stream, converting to "
              "\"1\"");
          if (bytesPerSample == 1)
            result = createPlanarConfiguration1Byte(
                OFreinterpret_cast(Uint8*, buffer), imageColumns, imageRows);
          else
            result = createPlanarConfiguration1Word(
                OFreinterpret_cast(Uint16*, buffer), imageColumns, imageRows);
        } else if (imagePlanarConfiguration == 0 && params.ilv != ILV_SAMPLE &&
                   params.ilv != ILV_LINE) {
          ...
          DCMJPLS_WARN(
              ...
              "\"0\"");
          if (bytesPerSample == 1)
            result = createPlanarConfiguration0Byte(
                OFreinterpret_cast(Uint8*, buffer), imageColumns, imageRows);
          else
            result = createPlanarConfiguration0Word(
                OFreinterpret_cast(Uint16*, buffer), imageColumns, imageRows);
        }
      }
...
Uint16 DJLSDecoderBase::determinePlanarConfiguration(
    const OFString& sopClassUID, const OFString& photometricInterpretation) {
  // Hardcopy Color Image always requires color-by-plane
  if (sopClassUID == UID_RETIRED_HardcopyColorImageStorage)
    return 1;
...
Uint32 DJLSDecoderBase::computeNumberOfFragments(Sint32 numberOfFrames,
                                                 Uint32 currentFrame,
                                                 Uint32 startItem,
    ...
    if (result.good() && pixItem) {
      ...
      if (result.good() && fragmentData && (pixItem->getLength() > 3)) {
        if (isJPEGLSStartOfImage(fragmentData)) {
          // found a JPEG-LS SOI marker. Assume that this is the start of the
          // next frame.
...
OFBool DJLSDecoderBase::isJPEGLSStartOfImage(Uint8* fragmentData) {
  // A valid JPEG-LS bitstream will always start with an SOI marker FFD8,
  // followed by either an SOF55 (FFF7), COM (FFFE) or APPn (FFE0-FFEF) marker.
  if ((*fragmentData++) != 0xFF)
    return OFFalse;
  if ((*fragmentData++) != 0xD8)
    return OFFalse;
  if ((*fragmentData++) != 0xFF)
    return OFFalse;
...
OFCondition DJLSDecoderBase::createPlanarConfiguration1Byte(Uint8* imageFrame,
                                                            Uint16 columns,
                                                            Uint16 rows) {
  if (imageFrame == NULL)
    return EC_IllegalCall;
...
OFCondition DJLSDecoderBase::createPlanarConfiguration1Word(Uint16* imageFrame,
                                                            Uint16 columns,
                                                            Uint16 rows) {
  if (imageFrame == NULL)
    return EC_IllegalCall;
...
OFCondition DJLSDecoderBase::createPlanarConfiguration0Byte(Uint8* imageFrame,
                                                            Uint16 columns,
                                                            Uint16 rows) {
  if (imageFrame == NULL)
    return EC_IllegalCall;
...
OFCondition DJLSDecoderBase::createPlanarConfiguration0Word(Uint16* imageFrame,
                                                            Uint16 columns,
                                                            Uint16 rows) {
  if (imageFrame == NULL)
    return EC_IllegalCall;
