 ...
 *
 *  Purpose: singleton class that registers decoders for all supported JPEG-LS
 * processes.
 *
...
void DJLSDecoderRegistration::registerCodecs(
    JLS_UIDCreation uidcreation, JLS_PlanarConfiguration planarconfig,
  ...
  if (!registered_) {
    ...
    if (cp_) {
      ...
      if (nearlosslessdecoder_)
        DcmCodecList::registerCodec(nearlosslessdecoder_, NULL, cp_);
      registered_ = OFTrue;
    }
  }
}
