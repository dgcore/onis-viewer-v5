...
unsigned int DicomImageClass::rangeToBits(double minvalue, double maxvalue) {
  /* assertion: min < max ! */
...
int DicomImageClass::isRepresentationSigned(EP_Representation repres) {
  ...
  return (repres == EPR_Sint8) || (repres == EPR_Sint16) ||
         (repres == EPR_Sint32);
}
...
EP_Representation DicomImageClass::determineRepresentation(double minvalue,
  ...
  if (minvalue < 0) /* signed */
  {
    ...
    if (-minvalue > maxval(MAX_BITS - 1, 0)) {
      DCMIMGLE_WARN("minimum pixel value ("
                    << minvalue << ") exceeds signed " << MAX_BITS << " bit "
                    << "representation after modality transformation");
    }
    if (maxvalue > maxval(MAX_BITS - 1)) {
      DCMIMGLE_WARN("maximum pixel value ("
                    << maxvalue << ") exceeds signed " << MAX_BITS << " bit "
                    << "representation after modality transformation");
    }
#endif
    return EPR_Sint32;
  ...
  if (maxvalue > maxval(MAX_BITS)) {
    DCMIMGLE_WARN("maximum pixel value ("
                  << maxvalue << ") exceeds unsigned " << MAX_BITS << " bit "
                  << "representation after modality transformation");
  }
#endif
  return EPR_Uint32;
