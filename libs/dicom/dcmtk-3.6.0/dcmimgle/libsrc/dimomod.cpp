...
DiMonoModality::DiMonoModality(const DiDocument* docu, DiInputPixel* pixel)
  ...
  if (Init(docu, pixel)) {
    if (!(docu->getFlags() &
          CIF_UsePresentationState) &&  // ignore modality LUT and rescaling
        !(docu->getFlags() & CIF_IgnoreModalityTransformation)) {
      const char* sopClassUID =
          NULL;  // check for XA and XRF image (ignore MLUT)
      if ((docu->getValue(DCM_SOPClassUID, sopClassUID) == 0) ||
          (sopClassUID == NULL) ||
          ((strcmp(sopClassUID, UID_XRayAngiographicImageStorage) != 0) &&
           (strcmp(sopClassUID, UID_XRayRadiofluoroscopicImageStorage) != 0) &&
           (strcmp(sopClassUID,
                   UID_RETIRED_XRayAngiographicBiPlaneImageStorage) != 0))) {
        ...
        TableData =
            new DiLookupTable(docu, DCM_ModalityLUTSequence, DCM_LUTDescriptor,
                              DCM_LUTData, DCM_LUTExplanation, descMode);
        checkTable();
        Rescaling =
            (docu->getValue(DCM_RescaleIntercept, RescaleIntercept) > 0);
        Rescaling &= (docu->getValue(DCM_RescaleSlope, RescaleSlope) > 0);
        checkRescaling(pixel);
      } else {
        DCMIMGLE_INFO(
            "processing XA or XRF image ... ignoring possible modality "
            "transform");
      }
    } else {
      DCMIMGLE_INFO(
          "configuration flag set ... ignoring possible modality transform");
    }
...
int DiMonoModality::Init(const DiDocument* docu, DiInputPixel* pixel) {
...
void DiMonoModality::checkRescaling(const DiInputPixel* pixel) {
  if (Rescaling) {
    if (LookupTable) {
      DCMIMGLE_WARN(
          "redundant values for 'RescaleSlope/Intercept' ... using modality "
          "LUT transformation");
      Rescaling = 0;
    } else {
      if (RescaleSlope == 0) {
        DCMIMGLE_WARN("invalid value for 'RescaleSlope' ("
                      << RescaleSlope
...
void DiMonoModality::determineRepresentation(const DiDocument* docu) {
  ...
  if ((docu != NULL) && (docu->getFlags() & CIF_UseAbsolutePixelRange))
    Representation =
        DicomImageClass::determineRepresentation(AbsMinimum, AbsMaximum);
  else
    Representation =
        DicomImageClass::determineRepresentation(MinValue, MaxValue);
  DCMIMGLE_TRACE(
      "internal representation for monochrome images: "
      << DicomImageClass::getRepresentationBits(Representation) << " bits ("
      << (DicomImageClass::isRepresentationSigned(Representation) ? "signed"
                                                                  : "unsigned")
      << ")");
}
