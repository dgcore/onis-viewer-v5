...
DiImage::DiImage(const DiDocument* docu, const EI_Status status, const int spp)
    : ImageStatus(status),
      Document(docu),
      FirstFrame(0),
      NumberOfFrames(0),
  ...
  if ((Document != NULL) && (ImageStatus == EIS_Normal)) {
    ...
    if (Document->getValue(DCM_NumberOfFrames, sl)) {
      if (sl < 1) {
        DCMIMGLE_WARN("invalid value for 'NumberOfFrames' ("
                      << sl << ") ... assuming 1");
        NumberOfFrames = 1;
      } else
    ...
    if (Document->getValue(DCM_RepresentativeFrameNumber, us)) {
      if (us <= FirstFrame) {
        DCMIMGLE_WARN("invalid value for 'RepresentativeFrameNumber' ("
                      << us << ") ... assuming first frame");
        RepresentativeFrame = FirstFrame;
      } else if (us > NumberOfFrames) {
        DCMIMGLE_WARN("invalid value for 'RepresentativeFrameNumber' ("
                      << us << ") ... assuming last frame");
        RepresentativeFrame = NumberOfFrames - 1;
      } else
        RepresentativeFrame = us - 1;
    }
    FirstFrame = (docu->getFrameStart() < NumberOfFrames)
                     ? docu->getFrameStart()
                     : NumberOfFrames - 1;
    ...
    /* start from first processed frame (might still exceed number of loaded
     * frames) */
    RepresentativeFrame -= FirstFrame;
    ...
    if (!ok || ((Rows > 0) && (Columns > 0))) {
      ...
      if (((Document->getValue(DCM_HighBit, HighBit) == 0) ||
           ((HighBit == 0) && (BitsStored > 1))) &&
          ok) {
      ...
      if ((us != 0) && (us != 1))
        DCMIMGLE_WARN("invalid value for 'PixelRepresentation' ("
                      << us << ") ... assuming 'unsigned' (0)");
      if (!(Document->getFlags() & CIF_UsePresentationState)) {
        hasPixelSpacing =
            (Document->getValue(DCM_PixelSpacing, PixelHeight, 0) > 0);
        if (hasPixelSpacing) {
          if (Document->getValue(DCM_PixelSpacing, PixelWidth, 1) < 2)
            DCMIMGLE_WARN(
                "missing second value for 'PixelSpacing' ... assuming 'Width' "
                "= "
                << PixelWidth);
        } else {
          hasImagerPixelSpacing =
              (Document->getValue(DCM_ImagerPixelSpacing, PixelHeight, 0) > 0);
          if (hasImagerPixelSpacing) {
            if (Document->getValue(DCM_ImagerPixelSpacing, PixelWidth, 1) < 2)
              DCMIMGLE_WARN(
                  "missing second value for 'ImagerPixelSpacing' ... assuming "
                  "'Width' = "
                  << PixelWidth);
          } else {
            hasNominalScannedPixelSpacing =
                (Document->getValue(DCM_NominalScannedPixelSpacing, PixelHeight,
                                    0) > 0);
            if (hasNominalScannedPixelSpacing) {
              if (Document->getValue(DCM_NominalScannedPixelSpacing, PixelWidth,
                                     1) < 2)
                DCMIMGLE_WARN(
                    "missing second value for 'NominalScannedPixelSpacing' ... "
                    "assuming 'Width' = "
                    << PixelWidth);
            } else {
              Sint32 sl2;
              hasPixelAspectRatio =
                  (Document->getValue(DCM_PixelAspectRatio, sl2, 0) > 0);
              if (hasPixelAspectRatio) {
                PixelHeight = sl2;
                if (Document->getValue(DCM_PixelAspectRatio, sl2, 1) < 2)
                  DCMIMGLE_WARN(
                      "missing second value for 'PixelAspectRatio' ... "
                      "assuming 'Width' = "
                      << PixelWidth);
                else
                  PixelWidth = sl2;
      ...
      if (ok && (Document->getPixelData() != NULL)) {
        ...
        DCMIMGLE_ERROR(
            "one or more mandatory attributes are missing in image pixel "
            "module");
      }
    } else {
      ImageStatus = EIS_InvalidValue;
      DCMIMGLE_ERROR("invalid value for 'Rows' ("
                     << Rows << ") and/or 'Columns' (" << Columns << ")");
    }
  } else {
...
DiImage::DiImage(const DiImage* image, const unsigned long fstart,
...
DiImage::DiImage(const DiImage* image, const Uint16 columns, const Uint16 rows,
  ...
  const double xfactor =
      OFstatic_cast(double, Columns) / OFstatic_cast(double, image->Columns);
  const double yfactor =
      OFstatic_cast(double, Rows) / OFstatic_cast(double, image->Rows);
  ...
  if (image->hasPixelSpacing) {
    ...
    PixelWidth = image->PixelWidth / xfactor;
    PixelHeight = image->PixelHeight / yfactor;
  } else if (image->hasPixelAspectRatio &&
             !aspect /*recognize pixel aspect ratio*/) {
    hasPixelAspectRatio = image->hasPixelAspectRatio;
    PixelWidth = image->PixelWidth * xfactor;
    PixelHeight = image->PixelHeight * yfactor;
...
DiImage::DiImage(const DiImage* image, const int degree)
    : ImageStatus(image->ImageStatus),
      Document(image->Document),
      FirstFrame(image->FirstFrame),
      NumberOfFrames(image->NumberOfFrames),
      ...
      Columns(((degree == 90) || (degree == 270)) ? image->Rows
                                                  : image->Columns),
      PixelWidth(((degree == 90) || (degree == 270)) ? image->PixelHeight
                                                     : image->PixelWidth),
      PixelHeight(((degree == 90) || (degree == 270)) ? image->PixelWidth
                                                      : image->PixelHeight),
      BitsAllocated(image->BitsAllocated),
      BitsStored(image->BitsStored),
      HighBit(image->HighBit),
...
DiImage::DiImage(const DiImage* image, const unsigned long frame,
...
int DiImage::processNextFrames(const unsigned long fcount) {
  if ((ImageStatus == EIS_Normal) && (Document != NULL) && isOriginal) {
    if ((Document->getFlags() & CIF_UsePartialAccessToPixelData) &&
        (Document->getPixelData() != NULL)) {
...
void DiImage::checkPixelExtension() {
  if (hasPixelSpacing || hasImagerPixelSpacing ||
      hasNominalScannedPixelSpacing || hasPixelAspectRatio) {
    if (PixelHeight == 0) {
      DCMIMGLE_WARN("invalid value for 'PixelHeight' (" << PixelHeight
                                                        << ") ... assuming 1");
      /* according to the standard, a value of 0 would be valid in case number
       * of rows is 1 */
      PixelHeight = 1;
    } else if (PixelHeight < 0) {
      DCMIMGLE_WARN("negative value for 'PixelHeight' ("
                    << PixelHeight << ") ... assuming " << -PixelHeight);
    ...
    if (PixelWidth == 0) {
      ...
      DCMIMGLE_WARN("negative value for 'PixelWidth' ("
                    << PixelWidth << ") ... assuming " << -PixelWidth);
      PixelHeight = -PixelHeight;
    }
  }
}
...
void DiImage::convertPixelData() {
  ...
  DCMIMGLE_TRACE("Pixel Data VR: "
                 << DcmVR(evr).getVRName() << ", "
                 << (compressed ? "Compressed" : "Uncompressed") << " ("
                 << ((xferName != NULL) ? xferName : xferUID) << ")");
  DCMIMGLE_TRACE("First frame: "
                 << FirstFrame << ", Number of frames: " << NumberOfFrames
                 << ", Total number of frames: " << TotalNumberOfFrames);
  DCMIMGLE_TRACE("Columns: " << Columns << ", Rows: " << Rows
                             << ", Samples per Pixel: " << SamplesPerPixel
                             << ", Photometric Interpretation: "
                             << Document->getPhotometricInterpretation());
  DCMIMGLE_TRACE("Bits Allocated: "
                 << BitsAllocated << ", Bits Stored: " << BitsStored
                 << ", High Bit: " << HighBit << ", "
                 << (hasSignedRepresentation ? "Signed" : "Unsigned")
                 << " integer");
  /* check for valid/supported pixel data encoding */
  if ((evr == EVR_OW) ||
      ((evr == EVR_OB) && (compressed || (BitsAllocated <= 16)))) {
    ...
    if ((BitsAllocated < 1) || (BitsStored < 1) ||
      ...
      if (!compressed && (BitsAllocated > 8))
        DCMIMGLE_WARN("invalid value for 'BitsAllocated' ("
                      << BitsAllocated
                      << "), > 8 for OB encoded uncompressed 'PixelData'");
      if (hasSignedRepresentation)
        InputData = new DiInputPixelTemplate<Uint8, Sint8>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
      else
        InputData = new DiInputPixelTemplate<Uint8, Uint8>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
      ...
      if (!compressed && (BitsAllocated > 8))
        DCMIMGLE_WARN("invalid value for 'BitsAllocated' ("
                      ...
                      << "), > 8 for OB encoded uncompressed 'PixelData'");
      if (hasSignedRepresentation)
        InputData = new DiInputPixelTemplate<Uint8, Sint16>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
      else
        InputData = new DiInputPixelTemplate<Uint8, Uint16>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
    } else if ((evr == EVR_OB) && compressed && (BitsStored <= 32)) {
      if (hasSignedRepresentation)
        InputData = new DiInputPixelTemplate<Uint8, Sint32>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
      else
        InputData = new DiInputPixelTemplate<Uint8, Uint32>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
    } else if (BitsStored <= 8) {
      if (hasSignedRepresentation)
        InputData = new DiInputPixelTemplate<Uint16, Sint8>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
      else
        InputData = new DiInputPixelTemplate<Uint16, Uint8>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
      ...
      if (hasSignedRepresentation)
        ...
        InputData = new DiInputPixelTemplate<Uint16, Uint16>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
    } else if (BitsStored <= 32) {
      if (hasSignedRepresentation)
        InputData = new DiInputPixelTemplate<Uint16, Sint32>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
      else
        InputData = new DiInputPixelTemplate<Uint16, Uint32>(
            Document, BitsAllocated, BitsStored, HighBit, FirstFrame,
            NumberOfFrames, fsize, &FileCache, CurrentFragment);
    } else /* BitsStored > 32 !! */
    {
      ImageStatus = EIS_NotSupportedValue;
      DCMIMGLE_ERROR("invalid value for 'BitsStored' ("
                     << BitsStored << ") ... exceeds " << MAX_BITS << " bit");
      return;
    }
    if (InputData == NULL) {
      ...
      DCMIMGLE_ERROR(
          "can't convert input pixel data, probably unsupported compression");
    } else if (InputData->getPixelStart() >= InputData->getCount()) {
      ImageStatus = EIS_InvalidValue;
      DCMIMGLE_ERROR("start offset (" << InputData->getPixelStart()
                                      << ") exceeds number of pixels stored ("
                                      << InputData->getCount() << ")");
    }
  } else {
    ImageStatus = EIS_NotSupportedValue;
    DCMIMGLE_ERROR(
        "'PixelData' has an other value representation than OB (with "
        "'BitsAllocated' <= 16) or OW");
  }
}
...
int DiImage::detachPixelData() {
  if ((Document != NULL) && isOriginal) {
    if (Document->getFlags() & CIF_MayDetachPixelData) {
      ...
      if (!(Document->getFlags() & CIF_UsePartialAccessToPixelData) ||
          (FirstFrame + NumberOfFrames >= TotalNumberOfFrames)) {
        DcmPixelData* pixel = Document->getPixelData();
        if (pixel != NULL) {
...
void DiImage::updateImagePixelModuleAttributes(DcmItem& dataset) {
  ...
  */
  ...
  strcat(buffer, "\\");
  OFStandard::ftoa(strchr(buffer, 0), 15, PixelWidth,
                   OFStandard::ftoa_format_f);
...
int DiImage::writeFrameToDataset(DcmItem& dataset, const unsigned long frame,
                                 const int bits, const int planar) {
  int result = 0;
  ...
  if (pixel != NULL) {
    ...
    if ((getInternalColorModel() == EPI_Monochrome1) ||
        (getInternalColorModel() == EPI_Monochrome2)) {
      /* monochrome image */
      count = OFstatic_cast(unsigned long, Columns) *
      ...
      /* color image */
      count = OFstatic_cast(unsigned long, Columns) *
              OFstatic_cast(unsigned long, Rows) * 3 /*samples per pixel*/;
    ...
    if (bitsStored <= 8)
      dataset.putAndInsertUint8Array(
          DCM_PixelData, OFstatic_cast(Uint8*, OFconst_cast(void*, pixel)),
          count);
    else if (bitsStored <= 16)
      dataset.putAndInsertUint16Array(
          DCM_PixelData, OFstatic_cast(Uint16*, OFconst_cast(void*, pixel)),
          count);
    else
      dataset.putAndInsertUint16Array(
          DCM_PixelData, OFstatic_cast(Uint16*, OFconst_cast(void*, pixel)),
...
int DiImage::writeBMP(FILE* stream, const unsigned long frame, const int bits) {
  int result = 0;
  if ((stream != NULL) && ((bits == 8) || (bits == 24) || (bits == 32))) {
    ...
    const unsigned long bytes =
        createDIB(data, 0, frame, bits, 1 /*upsideDown*/);
    if ((data != NULL) && (bytes > 0)) {
      /* number of bytes */
      SB_BitmapFileHeader fileHeader;
      SB_BitmapInfoHeader infoHeader;
      Uint32* palette =
          (bits == 8) ? new Uint32[256] : OFstatic_cast(Uint32*, NULL);
      /* fill bitmap file header with data */
      fileHeader.bfType[0] = 'B';
      fileHeader.bfType[1] = 'M';
      fileHeader.bfSize = 14 /*sizeof(SB_BitmapFileHeader)*/ +
                          40 /*sizeof(SB_BitmapInfoHeader)*/ + bytes;
      fileHeader.bfReserved1 = 0;
      fileHeader.bfReserved2 = 0;
      fileHeader.bfOffBits = 14 /*sizeof(SB_BitmapFileHeader)*/ +
                             40 /*sizeof(SB_BitmapInfoHeader)*/;
      ...
      if (gLocalByteOrder != EBO_LittleEndian) {
        /* other data elements are always '0' and, therefore, can remain as they
         * are */
        ...
        if (palette != NULL)
          swapBytes(OFreinterpret_cast(Uint8*, palette), 256 * 4 /*byteLength*/,
                    4 /*valWidth*/);
      }
      /* write bitmap file header: do not write the struct because of 32-bit
       * alignment */
      fwrite(&fileHeader.bfType, sizeof(fileHeader.bfType), 1, stream);
      fwrite(&fileHeader.bfSize, sizeof(fileHeader.bfSize), 1, stream);
      fwrite(&fileHeader.bfReserved1, sizeof(fileHeader.bfReserved1), 1,
             stream);
      fwrite(&fileHeader.bfReserved2, sizeof(fileHeader.bfReserved2), 1,
             stream);
      fwrite(&fileHeader.bfOffBits, sizeof(fileHeader.bfOffBits), 1, stream);
      /* write bitmap info header: do not write the struct because of 32-bit
       * alignment  */
      ...
      fwrite(&infoHeader.biCompression, sizeof(infoHeader.biCompression), 1,
             stream);
      fwrite(&infoHeader.biSizeImage, sizeof(infoHeader.biSizeImage), 1,
             stream);
      fwrite(&infoHeader.biXPelsPerMeter, sizeof(infoHeader.biXPelsPerMeter), 1,
             stream);
      fwrite(&infoHeader.biYPelsPerMeter, sizeof(infoHeader.biYPelsPerMeter), 1,
             stream);
      fwrite(&infoHeader.biClrUsed, sizeof(infoHeader.biClrUsed), 1, stream);
      fwrite(&infoHeader.biClrImportant, sizeof(infoHeader.biClrImportant), 1,
             stream);
    ...
    /* delete pixel data */
    delete OFstatic_cast(char*, data);  // type cast necessary to avoid compiler
                                        // warnings using gcc >2.95
  }
  return result;
}
