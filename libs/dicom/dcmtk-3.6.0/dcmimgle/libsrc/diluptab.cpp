...
DiLookupTable::DiLookupTable(const DiDocument* docu,
                             const DcmTagKey& descriptor, const DcmTagKey& data,
                             const DcmTagKey& explanation,
                             const EL_BitsPerTableEntry descripMode,
                             EI_Status* status)
    : DiBaseLUT(), OriginalBitsAllocated(16), OriginalData(NULL) {
  if (docu != NULL)
    Init(docu, NULL, descriptor, data, explanation, descripMode, status);
}

DiLookupTable::DiLookupTable(const DiDocument* docu, const DcmTagKey& sequence,
                             const DcmTagKey& descriptor, const DcmTagKey& data,
                             const DcmTagKey& explanation,
                             const EL_BitsPerTableEntry descripMode,
                             const unsigned long pos, unsigned long* card)
    : DiBaseLUT(), OriginalBitsAllocated(16), OriginalData(NULL) {
  if (docu != NULL) {
...
DiLookupTable::DiLookupTable(const DcmUnsignedShort& data,
                             ...
                             const EL_BitsPerTableEntry descripMode,
                             const signed long first, EI_Status* status)
    : DiBaseLUT(), OriginalBitsAllocated(16), OriginalData(NULL) {
  Uint16 us = 0;
  const DcmElement* descElem =
      OFreinterpret_cast(const DcmElement*, &descriptor);
  if (DiDocument::getElemValue(descElem, us, 0, OFTrue /*allowSigned*/) >=
      3)  // number of LUT entries
  {
    Count = (us == 0) ? MAX_TABLE_ENTRY_COUNT
                      : us;  // see DICOM supplement 5: "0" => 65536
    DiDocument::getElemValue(
        descElem, FirstEntry, 1,
        OFTrue /*allowSigned*/);  // can be SS or US (will be type casted later)
    if ((first >= 0) && (FirstEntry != OFstatic_cast(Uint16, first))) {
      DCMIMGLE_WARN(
          "invalid value for 'FirstInputValueMapped' in lookup table ("
          << FirstEntry << ") ... assuming " << first);
      FirstEntry = OFstatic_cast(Uint16, first);
    }
    DiDocument::getElemValue(
        descElem, us, 2,
        OFTrue /*allowSigned*/);  // bits per entry (only informational)
    unsigned long count = DiDocument::getElemValue(
        OFreinterpret_cast(const DcmElement*, &data), Data);
    OriginalData = OFstatic_cast(
        void*, OFconst_cast(Uint16*, Data));  // store pointer to original data
    if (explanation != NULL)
      DiDocument::getElemValue(
          OFreinterpret_cast(const DcmElement*, explanation),
          Explanation);  // explanation (free form text)
    ...
    if (status != NULL) {
      *status = EIS_MissingAttribute;
      ...
      DCMIMGLE_WARN("incomplete or missing 'LookupTableDescriptor' "
                    << descriptor.getTag() << " ... ignoring LUT");
    }
  }
}

DiLookupTable::DiLookupTable(Uint16* buffer, const Uint32 count,
                             const Uint16 bits)
...
void DiLookupTable::Init(const DiDocument* docu, DcmObject* obj,
                         const DcmTagKey& descriptor, const DcmTagKey& data,
                         const DcmTagKey& explanation,
                         const EL_BitsPerTableEntry descripMode,
                         EI_Status* status) {
  Uint16 us = 0;
  if (docu->getValue(descriptor, us, 0, obj, OFTrue /*allowSigned*/) >=
      3)  // number of LUT entries
  {
    Count = (us == 0) ? MAX_TABLE_ENTRY_COUNT
                      : us;  // see DICOM supplement 5: "0" => 65536
    docu->getValue(
        descriptor, FirstEntry, 1, obj,
        OFTrue /*allowSigned*/);  // can be SS or US (will be type casted later)
    docu->getValue(
        descriptor, us, 2, obj,
        OFTrue /*allowSigned*/);  // bits per entry (only informational)
    ...
    if (explanation != DCM_UndefinedTagKey)
      docu->getValue(explanation, Explanation, 0 /*vm pos*/,
                     obj);  // explanation (free form text)
    checkTable(count, us, descripMode, status);
  } else {
    if (status != NULL) {
      *status = EIS_MissingAttribute;
      ...
      DCMIMGLE_WARN("incomplete or missing 'LookupTableDescriptor' "
                    << descriptor << " ... ignoring LUT");
    }
  }
}

void DiLookupTable::checkTable(unsigned long count, Uint16 bits,
                               const EL_BitsPerTableEntry descripMode,
                               EI_Status* status) {
  if (count > 0)  // valid LUT
  {
    ...
    if (count != Count)  // length of LUT differs from number of LUT entries
    {
      if (count == ((Count + 1) >> 1))  // bits allocated 8, ignore padding
      {
        ...
        DCMIMGLE_DEBUG(
            "lookup table uses 8 bits allocated ... converting to 16 bits");
        DataBuffer = new Uint16[Count];  // create new LUT
        if ((DataBuffer != NULL) && (Data != NULL)) {
          const Uint8* p = OFreinterpret_cast(const Uint8*, Data);
          Uint16* q = DataBuffer;
          if (gLocalByteOrder ==
              EBO_BigEndian)  // local machine has big endian byte ordering
          {
            DCMIMGLE_DEBUG(
                "local machine has big endian byte ordering ... swapping 8 bit "
                "LUT entries");
            for (i = count; i != 0;
                 --i)  // copy 8 bit entries to new 16 bit LUT (swap hi/lo byte)
            {
              *(q++) = *(p + 1);  // copy low byte ...
              *(q++) = *p;        // ... and then high byte
              p += 2;             // jump to next hi/lo byte pair
            }
          } else {  // local machine has little endian byte ordering (or
                    // unknown)
            for (i = Count; i != 0; --i)
        ...
        DCMIMGLE_WARN("invalid value for 'NumberOfTableEntries' ("
                      << Count << ") ... assuming " << count);
        Count = count;
      }
    }
    MinValue = OFstatic_cast(
        Uint16, DicomImageClass::maxval(
                    MAX_TABLE_ENTRY_SIZE));  // set minimum to maximum value
    const Uint16* p = Data;
    Uint16 value;
    if (DataBuffer != NULL)  // LUT entries have been copied 8 -> 16 bits
    {
      ...
      for (i = Count; i != 0; --i) {
        ...
        if (((value >> 8) != 0) &&
            (value & 0xff) !=
                (value >> 8))  // lo-byte not equal to hi-byte and ...
          cmp = 1;
        if (value < MinValue)  // get global minimum
          MinValue = value;
        if (value > MaxValue)  // get global maximum
      ...
      if (cmp == 0)  // lo-byte is always equal to hi-byte
        checkBits(bits, MIN_TABLE_ENTRY_SIZE, MAX_TABLE_ENTRY_SIZE,
                  descripMode);  // set 'Bits'
      else
        checkBits(bits, MAX_TABLE_ENTRY_SIZE, MIN_TABLE_ENTRY_SIZE,
                  descripMode);
    }
    Uint16 mask = OFstatic_cast(
        Uint16,
        DicomImageClass::maxval(Bits));  // mask lo-byte (8) or full word (16)
    if (((MinValue & mask) != MinValue) ||
        ((MaxValue & mask) !=
         MaxValue)) {  // mask table entries and copy them to new LUT
      MinValue &= mask;
      MaxValue &= mask;
      if (DataBuffer == NULL)
        DataBuffer = new Uint16[Count];  // create new LUT
...
void DiLookupTable::checkBits(const Uint16 bits, const Uint16 rightBits,
                              const Uint16 wrongBits,
                              const EL_BitsPerTableEntry descripMode) {
  /* is stored bit depth out of range? */
  if ((descripMode == ELM_IgnoreValue) || (bits < MIN_TABLE_ENTRY_SIZE) ||
    ...
    if (bits != Bits) {
      if (descripMode == ELM_IgnoreValue) {
        DCMIMGLE_INFO("ignoring value for 'BitsPerTableEntry' ("
                      << bits << ") ... using " << Bits << " instead");
      } else {
        DCMIMGLE_WARN("unsuitable value for 'BitsPerTableEntry' ("
                      << bits << ") ... valid range " << MIN_TABLE_ENTRY_SIZE
                      << "-" << MAX_TABLE_ENTRY_SIZE << ", using " << Bits);
      }
    }
  } else if ((descripMode == ELM_CheckValue) && (bits == wrongBits)) {
    DCMIMGLE_WARN("unsuitable value for 'BitsPerTableEntry' ("
                  << bits << ") ... assuming " << rightBits);
    Bits = rightBits;
  } else {
    ...
    if (flag & 0x2) {
      if (OriginalData != NULL) {
        if (OriginalBitsAllocated == 8) {
          if (Bits <= 8) {
            const Uint8* p =
                OFconst_cast(const Uint8*, OFstatic_cast(Uint8*, OriginalData));
            Uint8* q = OFstatic_cast(Uint8*, OriginalData);
            const Uint8 max =
                OFstatic_cast(Uint8, DicomImageClass::maxval(Bits));
            for (i = Count; i != 0; --i)
              *(q++) = max - *(p++);
            result |= 0x2;
          }
          ...
          const Uint16* p =
              OFconst_cast(const Uint16*, OFstatic_cast(Uint16*, OriginalData));
          Uint16* q = OFstatic_cast(Uint16*, OriginalData);
          const Uint16 max =
              OFstatic_cast(Uint16, DicomImageClass::maxval(Bits));
          for (i = Count; i != 0; --i)
            *(q++) = max - *(p++);
          result |= 0x2;
        }
      ...
      while (i < count)  // fill gaps with valid values
      {
        if (valid[i])  // skip valid values
          ...
          if (valid[last])  // check for starting conditions
          {
            ...
            while (i < mid) {  // fill first half with 'left' value
              data[i] = data[last];
              ++i;
            }
          }
