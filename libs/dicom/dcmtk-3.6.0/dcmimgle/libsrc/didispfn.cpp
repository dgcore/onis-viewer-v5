...
DiDisplayFunction::DiDisplayFunction(const char* filename,
  ...
  if (readConfigFile(filename)) {
    ...
    if (ord >= 0)
      Order = ord;
    Valid = createSortedTable(DDLValue, LODValue) && calculateMinMax() &&
            interpolateValues();
  }
}
...
DiDisplayFunction::DiDisplayFunction(const Uint16* ddl_tab,  // UNTESTED !!
  ...
  if (ValueCount <= MAX_TABLE_ENTRY_COUNT)
    Valid = createSortedTable(ddl_tab, val_tab) && calculateMinMax() &&
            interpolateValues();
}

DiDisplayFunction::DiDisplayFunction(const double val_min, const double val_max,
                                     const unsigned long count,
                                     const E_DeviceType deviceType,
                                     const signed int ord)
    : Valid(0),
  ...
  OFBitmanipTemplate<DiDisplayLUT*>::zeroMem(LookupTable, MAX_NUMBER_OF_TABLES);
  /* check parameters */
  if ((ValueCount > 1) && (ValueCount <= MAX_TABLE_ENTRY_COUNT) &&
      (MinValue < MaxValue)) {
    /* create value tables */
    ...
    if ((DDLValue != NULL) && (LODValue != NULL)) {
      Uint16 i;
      const double min =
          ((DeviceType == EDT_Printer) || (DeviceType == EDT_Scanner))
              ? val_max
              : val_min;
      const double max =
          ((DeviceType == EDT_Printer) || (DeviceType == EDT_Scanner))
...
const DiDisplayLUT* DiDisplayFunction::getLookupTable(const int bits,
  ...
  if (Valid && (bits >= MinBits) && (bits <= MaxBits)) {
    ...
    if ((LookupTable[idx] != NULL) &&
        ((count != LookupTable[idx]->getCount()) ||
        ...
        if (c == '#')  // comment character
        {
          while (file.get(c) && (c != '\n') && (c != '\r'))
            ;  // skip comments
                       ...
                       OFstatic_cast(unsigned char, c)))  // skip whitespaces
        {
          file.putback(c);
          if (MaxDDLValue == 0)  // read maxvalue
          {
            ...
            if (strcmp(str, "max") == 0)  // check for key word: max
            {
              ...
              if (MaxDDLValue > 0) {
                DDLValue =
                    new Uint16[OFstatic_cast(unsigned long, MaxDDLValue) + 1];
                LODValue =
                    new double[OFstatic_cast(unsigned long, MaxDDLValue) + 1];
                if ((DDLValue == NULL) || (LODValue == NULL))
                  return 0;
              } else {
                DCMIMGLE_ERROR(
                    "invalid or missing value for maximum DDL value in DISPLAY "
                    "file");
                return 0;  // abort
              }
            } else {
              DCMIMGLE_ERROR(
                  "missing keyword 'max' for maximum DDL value in DISPLAY "
                  "file");
              return 0;  // abort
            }
          } else if ((AmbientLight == 0.0) &&
                     (c == 'a'))  // read ambient light value (optional)
          {
            char str[4];
            file.get(str, sizeof(str));
            if (strcmp(str, "amb") == 0)  // check for key word: amb
            {
              ...
              if (AmbientLight < 0) {
                DCMIMGLE_WARN(
                    "invalid value for ambient light in DISPLAY file ... "
                    "ignoring");
                AmbientLight = 0;
              }
            } else {
              ...
              return 0;  // abort
                     ...
                     (c == 'l'))  // read ambient light value (optional)
          {
            char str[4];
            file.get(str, sizeof(str));
            if (strcmp(str, "lum") == 0)  // check for key word: lum
            {
              ...
              if (Illumination < 0) {
                DCMIMGLE_WARN(
                    "invalid value for illumination in DISPLAY file ... "
                    "ignoring");
                Illumination = 0;
              }
            } else {
              ...
              return 0;  // abort
                     ...
                     (c == 'o'))  // read polynomial order (optional)
          {
            char str[4];
            file.get(str, sizeof(str));
            if (strcmp(str, "ord") == 0)  // check for key word: ord
            {
              ...
              if (Order < 0) {
                DCMIMGLE_WARN(
                    "invalid value for polynomial order in DISPLAY file ... "
                    "ignoring");
                Order = 0;
              }
            } else {
            ...
            if (ValueCount <= OFstatic_cast(unsigned long, MaxDDLValue)) {
              ...
              if (file.fail()) {
                DCMIMGLE_WARN(
                    "missing luminance/OD value in DISPLAY file ... ignoring "
                    "last entry");
              } else if (DDLValue[ValueCount] > MaxDDLValue) {
                DCMIMGLE_WARN("DDL value ("
                              << DDLValue[ValueCount]
                              << ") exceeds maximum value (" << MaxDDLValue
                              << ") in DISPLAY file ... ignoring value");
              } else
                ++ValueCount;
            } else {
              DCMIMGLE_WARN(
                  "too many values in DISPLAY file ... ignoring last line(s)");
              return 2;
            }
          }
        }
...
int DiDisplayFunction::createSortedTable(const Uint16* ddl_tab,
  ...
  if ((ValueCount > 0) && (ddl_tab != NULL) && (val_tab != NULL)) {
    ...
    if ((DDLValue != NULL) && (LODValue != NULL) && (sort_tab != NULL)) {
      OFBitmanipTemplate<Sint32>::setMem(sort_tab, -1,
                                         count);  // initialize array
      unsigned long i;
      ...
      for (i = 0; i <= MaxDDLValue; ++i)  // sort ascending
      {
        if (sort_tab[i] >= 0) {
          ...
          LODValue[ValueCount] =
              (val_tab[sort_tab[i]] > 0) ? val_tab[sort_tab[i]] : 0;
      ...
      if ((DeviceType == EDT_Printer) || (DeviceType == EDT_Scanner)) {
        ...
        if (i < ValueCount) {
          DCMIMGLE_WARN(
              "OD values (ordered by DDLs) don't descend monotonously");
        }
      } else {
        ...
        if (i < ValueCount) {
          DCMIMGLE_WARN(
              "luminance values (ordered by DDLs) don't ascend monotonously");
        }
      }
      status = (ValueCount > 0);
    }
...
int DiDisplayFunction::interpolateValues() {
  if (ValueCount <=
      OFstatic_cast(unsigned long, MaxDDLValue))  // interpolation necessary ?
  {
    int status = 0;
    if (Order > 0) {
      ...
      if ((coeff != NULL) &&
          DiCurveFitting<Uint16, double>::calculateCoefficients(
              DDLValue, LODValue, OFstatic_cast(unsigned int, ValueCount),
              Order, coeff)) {
        ...
        if ((DDLValue != NULL) && (LODValue != NULL)) {
          ...
          status = DiCurveFitting<Uint16, double>::calculateValues(
              0, MaxDDLValue, LODValue, OFstatic_cast(unsigned int, ValueCount),
              Order, coeff);
        }
      }
      delete[] coeff;
      ...
      if ((spline != NULL) &&
          DiCubicSpline<Uint16, double>::Function(
              DDLValue, LODValue, OFstatic_cast(unsigned int, ValueCount),
              spline)) {
        /* save old values */
        ...
        if ((DDLValue != NULL) && (LODValue != NULL)) {
          ...
          for (i = 0; i <= MaxDDLValue; ++i)
            DDLValue[i] = OFstatic_cast(Uint16, i);
          /* compute new y values */
          status = DiCubicSpline<Uint16, double>::Interpolation(
              old_ddl, old_val, spline, OFstatic_cast(unsigned int, count),
              DDLValue, LODValue, OFstatic_cast(unsigned int, ValueCount));
        }
        /* delete old data arrays */
        delete[] old_ddl;
...
int DiDisplayFunction::checkMinMaxDensity() const {
  if ((MinDensity >= 0) && (MaxDensity >= 0) && (MinDensity >= MaxDensity)) {
    DCMIMGLE_WARN("invalid optical density range (Dmin = "
                  << MinDensity << ", Dmax = " << MaxDensity << ")");
    return 0;
  }
  return 1;
}
    ...
    if (lum_tab != NULL) {
      ...
      if (useAmb) {
        for (i = 0; i < count; ++i)
          lum_tab[i] =
              AmbientLight +
              Illumination * pow(OFstatic_cast(double, 10), -od_tab[i]);
      } else {
...
double DiDisplayFunction::convertODtoLum(const double value,
  ...
  return (value >= 0) && (ambient >= 0) && (illum >= 0)
             ? ambient + illum * pow(OFstatic_cast(double, 10), -value)
             : -1 /*invalid*/;
}
