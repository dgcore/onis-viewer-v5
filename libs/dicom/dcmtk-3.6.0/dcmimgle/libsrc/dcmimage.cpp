...
DicomImage::DicomImage(const char* filename, const unsigned long flags,
  ...
  {
    Document = new DiDocument(filename, flags | CIF_MayDetachPixelData, fstart,
                              fcount);
    Init();
  }
}
...
DicomImage::DicomImage(DcmObject* object, const E_TransferSyntax xfer,
                       const unsigned long flags, const unsigned long fstart,
...
// --- create 'DicomImage' from valid 'DicomObject' with given rescale 'slope'
// and 'intercept'
...
// --- create 'DicomImage' from valid 'DicomObject' with given modality LUT
// (specified by 'data' and 'descriptor')
...
// --- protected: create 'DicomImage' from source with different image data and
// photometric interpretation

DicomImage::DicomImage(const DicomImage* dicom, DiImage* image,
...
// --- initialize 'DicomImage' object (same procedure for every 'real'
// constructor)
...
void DicomImage::Init() {
  if ((Document != NULL) && (Document->good())) {
    ...
    if (hasSOPclassUID(UID_RETIRED_StandaloneOverlayStorage)) {
      PhotometricInterpretation =
          EPI_Monochrome2;  // standalone overlays are handled like monochrome
      Image = new DiOverlayImage(Document,
                                 ImageStatus);  // images without pixel data
      ...
      if (cstr != NULL) {
        ...
        *q = '\0';  // end of C string
        ...
        DCMIMGLE_WARN(
            "can't create filtered version of 'PhotometricInterpretation' ("
            << str << ")");
        cstr = OFconst_cast(char*, str);  // just reference the original string
        while ((pin->DefinedTerm != NULL) &&
               (strcmp(pin->DefinedTerm, cstr) != 0))
          ++pin;
      }
      ...
      switch (PhotometricInterpretation) {
        ...
        default:  // unknown or unsupported color model
          if (DiRegisterBase::Pointer != NULL)
            Image = DiRegisterBase::Pointer->createImage(
                Document, ImageStatus, PhotometricInterpretation);
          if (Image == NULL) {
            if (PhotometricInterpretation == EPI_Unknown) {
              ImageStatus = EIS_InvalidValue;
              DCMIMGLE_ERROR("invalid value for 'PhotometricInterpretation' ("
                             << str << ")");
            } else {
              ImageStatus = EIS_NotSupportedValue;
              DCMIMGLE_ERROR(
                  "unsupported value for 'PhotometricInterpretation' (" << str
                                                                        << ")");
            }
          }
      }
    } else if (Document->getFlags() &
               CIF_AcrNemaCompatibility)  // ACR-NEMA has no 'photometric
                                          // interpretation'
    {
      ...
      if (Document->getPixelData() != NULL)
        DCMIMGLE_ERROR(
            "mandatory attribute 'PhotometricInterpretation' is missing or "
            "can't be determined");
    }
  } else
...
// --- return 'true' (1) if 'Document' has the same 'SOPclassUID' as given in
// parameter 'uid'
...
// --- create scaled to given size ('width' and 'height') image, memory isn't
// handled internally !
// --- if one dimension ist '0' the other is automatically adjusted (with
// respect to pixel aspect ratio)

DicomImage* DicomImage::createScaledImage(const unsigned long width,
                                          const unsigned long height,
                                          const int interpolate,
                                          int aspect) const {
  return createScaledImage(0, 0, getWidth(), getHeight(), width, height,
                           interpolate, aspect);
}

// --- create scaled with given factors ('xfactor' and 'yfactor') image, memory
// isn't handled internally !

DicomImage* DicomImage::createScaledImage(const double xfactor,
                                          const double yfactor,
                                          const int interpolate,
                                          const int aspect) const {
  return createScaledImage(0, 0, getWidth(), getHeight(),
                           OFstatic_cast(unsigned long, xfactor* getWidth()),
                           OFstatic_cast(unsigned long, yfactor* getHeight()),
                           interpolate, aspect);
}
...
DicomImage* DicomImage::createScaledImage(
    ...
    unsigned long scale_width, unsigned long scale_height,
    const int interpolate, int aspect, const Uint16 pvalue) const {
  const unsigned long gw = getWidth();
  const unsigned long gh = getHeight();
  if ((Image != NULL) && (gw > 0) && (gh > 0)) {
    if ((clip_width == 0) &&
        (left_pos < OFstatic_cast(signed long,
                                  gw)))  // set 'width' if parameter is missing
      clip_width = gw - left_pos;
    if ((clip_height == 0) &&
        (top_pos < OFstatic_cast(signed long, gh)))  // same for 'height'
    ...
    if ((scale_width == 0) && (scale_height == 0)) {
      ...
      {
        if (scale_width == 0)
          scale_width = OFstatic_cast(
              unsigned long,
              getWidthHeightRatio() *
                  OFstatic_cast(double, scale_height* clip_width) /
                  clip_height);
        else if (scale_height == 0)
          scale_height = OFstatic_cast(
              unsigned long,
              getHeightWidthRatio() *
                  OFstatic_cast(double, scale_width* clip_height) / clip_width);
        else
      ...
      } else           // ignore pixel aspect ratio
      {
        if (scale_width == 0)
          scale_width = OFstatic_cast(
              unsigned long,
              OFstatic_cast(double, scale_height* clip_width) / clip_height);
        else if (scale_height == 0)
          scale_height = OFstatic_cast(
              unsigned long,
              OFstatic_cast(double, scale_width* clip_height) / clip_width);
    ...
    if (((left_pos < 0) ||
         (OFstatic_cast(unsigned long, left_pos + clip_width) > gw) ||
         (top_pos < 0) ||
         (OFstatic_cast(unsigned long, top_pos + clip_height) > gh)) &&
        ((clip_width != scale_width) || (clip_height != scale_height))) {
      DCMIMGLE_ERROR(
          "combined clipping & scaling outside image boundaries not yet "
          "supported");
    } else if ((scale_width > 0) && (scale_height > 0)) {
      DiImage* image = Image->createScale(
          left_pos, top_pos, clip_width, clip_height, scale_width, scale_height,
...
DicomImage* DicomImage::createScaledImage(
    const signed long left_pos, const signed long top_pos, unsigned long width,
    unsigned long height, const double xfactor, const double yfactor,
  ...
  if ((xfactor >= 0) && (yfactor >= 0)) {
    ...
    if ((width == 0) &&
        (left_pos <
         OFstatic_cast(signed long,
                       gw)))  // set 'width' if parameter is missing (0)
      width = gw - left_pos;
    if ((height == 0) &&
        (top_pos < OFstatic_cast(signed long, gh)))  // same for 'height'
      height = gh - top_pos;
    return createScaledImage(left_pos, top_pos, width, height,
                             OFstatic_cast(unsigned long, xfactor* width),
                             OFstatic_cast(unsigned long, yfactor* height),
                             interpolate, aspect, pvalue);
  }
  return NULL;
}

// --- create clipped to given box ('left_pos', 'top_pos' and 'width', 'height')
// image,
...
// --- create flipped image (horizontal: x > 1 and/or vertical y > 1), memory
// isn't handled internally !

DicomImage* DicomImage::createFlippedImage(int horz, int vert) const {
  if ((Image != NULL) && (horz || vert)) {
...
// --- create color-image to mono-image with given 'red', 'green' and 'blue'
// coefficients converted image, memory ... !
...
DicomImage* DicomImage::createMonochromeImage(const double red,
  ...
  if (Image != NULL) {
    DiImage* image =
        Image->createMono(red, green, blue);  // create monochrome image data
    if (image != NULL) {
...
int DicomImage::writePPM(const char* filename, const int bits,
                         const unsigned long frame) {
  if ((filename != NULL) && (Image != NULL)) {
    char fname[FILENAME_MAX + 1];
    if (sprintf(fname, filename, frame) >=
        0)  // replace '%d' etc. with frame number
      filename = fname;
...
int DicomImage::writePPM(STD_NAMESPACE ostream& stream, const int bits,
                         const unsigned long frame) {
...
int DicomImage::writePPM(FILE* stream, const int bits,
                         const unsigned long frame) {
...
int DicomImage::writeRawPPM(const char* filename, const int bits,
                            const unsigned long frame) {
  if ((filename != NULL) && (Image != NULL) &&
      (Image->getBits(bits) <= MAX_RAWPPM_BITS)) {
    char fname[FILENAME_MAX + 1];
    if (sprintf(fname, filename, frame) >=
        0)  // replace '%d' etc. with frame number
      filename = fname;
...
int DicomImage::writeRawPPM(FILE* stream, const int bits,
                            const unsigned long frame) {
  if ((stream != NULL) && (Image != NULL))
...
int DicomImage::writeBMP(const char* filename, const int bits,
                         const unsigned long frame) {
  if ((filename != NULL) && (Image != NULL) &&
      ((bits == 0) || ((bits == 8) && isMonochrome()) || (bits == 24) ||
       (bits == 32))) {
    char fname[FILENAME_MAX + 1];
    if (sprintf(fname, filename, frame) >=
        0)  // replace '%d' etc. with frame number
...
int DicomImage::writeBMP(FILE* stream, const int bits,
                         const unsigned long frame) {
  if ((stream != NULL) && (Image != NULL) &&
      ((bits == 0) || ((bits == 8) && isMonochrome()) || (bits == 24) ||
       (bits == 32))) {
...
int DicomImage::writePluginFormat(const DiPluginFormat* plugin, FILE* stream,
                                  const unsigned long frame) {
