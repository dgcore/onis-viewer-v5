...
DiDocument::DiDocument(DcmObject* object, const E_TransferSyntax xfer,
  ...
  if (object != NULL) {
    if (object->ident() == EVR_fileFormat) {
      // store reference to DICOM file format to be deleted on object
      // destruction
...
void DiDocument::convertPixelData() {
  DcmStack pstack;
  DcmXfer xfer(Xfer);
  DCMIMGLE_DEBUG("transfer syntax of DICOM dataset: "
                 << xfer.getXferName() << " (" << xfer.getXferID() << ")");
  if (search(DCM_PixelData, pstack)) {
    ...
    if (PixelData != NULL) {
      ...
      if ((Flags & CIF_DecompressCompletePixelData) ||
          !(Flags & CIF_UsePartialAccessToPixelData)) {
        pstack.clear();
        // push reference to DICOM dataset on the stack (required for
        // decompression process)
        pstack.push(Object);
        // dummy stack entry
        pstack.push(PixelData);
        if (PixelData
                ->chooseRepresentation(EXS_LittleEndianExplicit, NULL, pstack)
                .good()) {
          // set transfer syntax to unencapsulated/uncompressed
          if (DcmXfer(Xfer).isEncapsulated()) {
            Xfer = EXS_LittleEndianExplicit;
            DCMIMGLE_DEBUG("decompressed complete pixel data in memory: "
                           << PixelData->getLength(Xfer) << " bytes");
          }
        } else
          DCMIMGLE_ERROR(
      ...
      OFCondition status = PixelData->getDecompressedColorModel(
          OFstatic_cast(DcmDataset*, Object), PhotometricInterpretation);
      if (status.bad()) {
        DCMIMGLE_ERROR(
            "can't determine 'PhotometricInterpretation' of decompressed "
            "image");
        DCMIMGLE_DEBUG("DcmPixelData::getDecompressedColorModel() returned: "
                       << status.text());
      }
    } else
...
DcmElement* DiDocument::search(const DcmTagKey& tag, DcmObject* obj) const {
  DcmStack stack;
  if (obj == NULL)
    obj = Object;
  if ((obj != NULL) &&
      (obj->search(tag, stack, ESM_fromHere, OFFalse /* searchIntoSub */) ==
       EC_Normal) &&
...
int DiDocument::search(const DcmTagKey& tag, DcmStack& pstack) const {
  ...
  if ((obj != NULL) &&
      (obj->search(tag, pstack, ESM_fromHere, OFFalse /* searchIntoSub */) ==
       EC_Normal) &&
...
unsigned long DiDocument::getValue(const DcmTagKey& tag, Uint16& returnVal,
                                   const unsigned long pos, DcmObject* item,
                                   const OFBool allowSigned) const {
  return getElemValue(search(tag, item), returnVal, pos, allowSigned);
}

unsigned long DiDocument::getValue(const DcmTagKey& tag, Sint16& returnVal,
...
unsigned long DiDocument::getValue(const DcmTagKey& tag, Uint32& returnVal,
                                   const unsigned long pos) const {
  DcmElement* elem = search(tag);
  if (elem != NULL) {
...
unsigned long DiDocument::getValue(const DcmTagKey& tag, Sint32& returnVal,
                                   const unsigned long pos) const {
  DcmElement* elem = search(tag);
  if (elem != NULL) {
...
unsigned long DiDocument::getValue(const DcmTagKey& tag, double& returnVal,
                                   const unsigned long pos) const {
  DcmElement* elem = search(tag);
  if (elem != NULL) {
...
unsigned long DiDocument::getValue(const DcmTagKey& tag, const char*& returnVal,
                                   DcmObject* item) const {
  return getElemValue(search(tag, item), returnVal);
}

unsigned long DiDocument::getValue(const DcmTagKey& tag, OFString& returnVal,
  ...
  if (elem != NULL) {
    ...
    if (OFconst_cast(DcmElement*, elem)->getUint16(returnVal, pos).good())
      ...
      if (OFconst_cast(DcmElement*, elem)->getSint16(value, pos).good()) {
        ...
        DCMIMGLE_TRACE(
            "retrieved signed value ("
            << value << ") at position " << pos << " from element "
            << OFconst_cast(DcmElement*, elem)->getTag() << ", VR="
            << DcmVR(OFconst_cast(DcmElement*, elem)->getVR()).getVRName()
  ...
  if (elem != NULL) {
    ...
    if (OFconst_cast(DcmElement*, elem)->getUint16Array(val).good()) {
      ...
      if ((vr == EVR_OW) || (vr == EVR_lt))
        return OFconst_cast(DcmElement*, elem)->getLength(/*Xfer*/) /
               sizeof(Uint16);
      return OFconst_cast(DcmElement*, elem)->getVM();
    }
  }
