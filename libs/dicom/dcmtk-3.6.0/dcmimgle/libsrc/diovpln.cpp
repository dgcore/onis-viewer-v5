...
DiOverlayPlane::DiOverlayPlane(const DiDocument* docu, const unsigned int group,
                               Uint16 alloc, const Uint16 stored,
                               const Uint16 high)
    : NumberOfFrames(0),
      ImageFrameOrigin(0),
      Top(0),
  ...
  if (docu != NULL) {
    ...
    const char* str;
    if ((docu->getValue(tag, str) > 0) && (str != NULL) &&
        (strcmp(str, "R") == 0))
    ...
    if (Valid) {
      DCMIMGLE_DEBUG("processing overlay plane in group 0x" << STD_NAMESPACE hex
                                                            << group);
      if (docu->getValue(tag, Top, 1) < 2)
        DCMIMGLE_WARN(
            "missing second value for 'OverlayOrigin' ... assuming 'Top' = "
            << Top);
    }
#else
    ...
    if (Valid) {
      ...
      if (docu->getValue(tag, Left, 1) < 2)
        DCMIMGLE_WARN(
            "missing second value for 'OverlayOrigin' ... assuming 'Left' = "
            << Left);
    }
#endif
    ...
    if (Valid) {
        ...
        if (!(docu->getFlags() & CIF_NeverAccessEmbeddedOverlays)) {
          if (!docu->isCompressed()) {
            ...
            if (pixelData != NULL) {
              ...
              if (pixelData->getUint16Array(OFconst_cast(Uint16*&, Data))
                      .good()) {
                length = pixelData->getLength(docu->getTransferSyntax());
                EmbeddedData = (Data != NULL);
                if (!loaded)
                  DCMIMGLE_DEBUG(
                      "loaded complete pixel data into memory for embedded "
                      "overlay data: "
                      << length << " bytes");
              }
            }
            ...
            DCMIMGLE_ERROR(
                "embedded overlay data cannot be accessed since pixel data is "
                "still compressed");
        } else
          DCMIMGLE_WARN(
              "ignoring possibly embedded overlay data by configuration");
      } else
      ...
      {
        DCMIMGLE_WARN("invalid value for 'OverlayBitsAllocated' ("
                      << BitsAllocated << ") ... assuming " << alloc);
        BitsAllocated = alloc;
      }
      /* check for correct value of BitPosition */
      if (BitPosition >= BitsAllocated) {
        DCMIMGLE_WARN("invalid value for 'OverlayBitPosition' ("
                      << BitPosition << ") ... assuming "
                      << (BitsAllocated - 1));
        BitPosition = BitsAllocated - 1;
      }
      if (EmbeddedData && (BitPosition <= high) &&
          (BitPosition + stored > high)) {
        DCMIMGLE_WARN("invalid value for 'OverlayBitPosition' ("
                      << BitPosition
...
DiOverlayPlane::DiOverlayPlane(const unsigned int group, const Sint16 left_pos,
                               const Sint16 top_pos, const Uint16 columns,
                               const Uint16 rows, const DcmOverlayData& data,
    ...
    : NumberOfFrames(1),
      ...
      StartPtr(NULL),
      Data(NULL) {
  DiDocument::getElemValue(OFreinterpret_cast(const DcmElement*, &label),
                           Label);
  DiDocument::getElemValue(OFreinterpret_cast(const DcmElement*, &description),
                           Description);
  if ((Columns > 0) && (Rows > 0)) {
    const unsigned long length =
        DiDocument::getElemValue(OFreinterpret_cast(const DcmElement*, &data),
                                 Data) *
        2 /* Bytes */;
    /* expected length of overlay data */
    const unsigned long expLen = (OFstatic_cast(unsigned long, Rows) *
                                      OFstatic_cast(unsigned long, Columns) +
                                  7) /
                                 8;
...
DiOverlayPlane::DiOverlayPlane(DiOverlayPlane* plane, const unsigned int bit,
                               Uint16* data, Uint16* temp, const Uint16 width,
                               const Uint16 height, const Uint16 columns,
                               const Uint16 rows)
  ...
  if (temp != NULL) {
    ...
    const unsigned long skip_f =
        OFstatic_cast(unsigned long, height - plane->Rows) *
        OFstatic_cast(unsigned long, width);
...
void* DiOverlayPlane::getData(const unsigned long frame, const Uint16 xmin,
                              const Uint16 ymin, const Uint16 xmax,
                              const Uint16 ymax, const int bits,
                              const Uint16 fore, const Uint16 back) {
  const unsigned long count = OFstatic_cast(unsigned long, xmax - xmin) *
                              OFstatic_cast(unsigned long, ymax - ymin);
  if (Valid && (count > 0)) {
    const Uint16 mask = OFstatic_cast(Uint16, DicomImageClass::maxval(bits));
    if (bits == 1) {
      const unsigned long count8 =
          (count + 7) / 8;  // round value: 8 bit padding
...
unsigned long DiOverlayPlane::create6xxx3000Data(Uint8*& buffer,
  ...
  const unsigned long count = OFstatic_cast(unsigned long, Width) *
                              OFstatic_cast(unsigned long, Height) *
                              NumberOfFrames;
  if (Valid && (count > 0)) {
    const unsigned long count8 =
        ((count + 15) / 16) * 2;  // round value: 16 bit padding
...
void DiOverlayPlane::show(const double fore, const double thresh,
                          const EM_Overlay mode) {
...
void DiOverlayPlane::setScaling(const double xfactor, const double yfactor) {
...
void DiOverlayPlane::setFlipping(const int horz, const int vert,
                                 const signed long columns,
                                 const signed long rows) {
  if (horz) {
    Left = OFstatic_cast(Sint16, columns - Width - Left);
    StartLeft = OFstatic_cast(
        Uint16, OFstatic_cast(signed long, Columns) - Width - StartLeft);
  }
  if (vert) {
    Top = OFstatic_cast(Sint16, rows - Height - Top);
    StartTop = OFstatic_cast(
        Uint16, OFstatic_cast(signed long, Rows) - Height - StartTop);
  }
}

void DiOverlayPlane::setRotation(const int degree, const signed long left_pos,
  ...
  if (degree == 180)  // equal to v/h flip
    ...
    /*
       ...
       -> already done in the constructor ! Rows = Columns; Columns = us;
    */
    if (degree == 90)  // rotate right
    {
      ...
      us = StartLeft;
      Left = OFstatic_cast(
          Sint16, OFstatic_cast(signed long, columns) - Width - Top + top_pos);
      StartLeft = OFstatic_cast(
          Uint16, OFstatic_cast(signed long, Columns) - Width - StartTop);
      ...
      StartLeft = StartTop;
      Top = OFstatic_cast(
          Sint16, OFstatic_cast(signed long, rows) - Height - ss + left_pos);
      StartTop =
          OFstatic_cast(Uint16, OFstatic_cast(signed long, Rows) - Height - us);
    }
  }
}
