...
DiGSDFLUT::DiGSDFLUT(const unsigned long count, const Uint16 max,
                     const Uint16* ddl_tab, const double* val_tab,
                     const unsigned long ddl_cnt, const double* gsdf_tab,
                     const double* gsdf_spl, const unsigned int gsdf_cnt,
                     const double jnd_min, const double jnd_max,
                     const double lum_min, const double lum_max,
                     const double amb, const double illum, const OFBool inverse,
                     STD_NAMESPACE ostream* stream, const OFBool printMode)
    : DiDisplayLUT(count, max, amb, illum) {
  if ((Count > 0) && (Bits > 0)) {
    DCMIMGLE_DEBUG("new GSDF LUT with " << Bits << " bits output and " << Count
                                        << " entries created");
    if (jnd_min >= jnd_max) {
    ...
    Valid = createLUT(ddl_tab, val_tab, ddl_cnt, gsdf_tab, gsdf_spl, gsdf_cnt,
                      jnd_min, jnd_max, lum_min, lum_max, inverse, stream,
                      printMode);
  }
}
...
int DiGSDFLUT::createLUT(const Uint16* ddl_tab, const double* val_tab,
                         const unsigned long ddl_cnt, const double* gsdf_tab,
                         const double* gsdf_spl, const unsigned int gsdf_cnt,
                         const double jnd_min, const double jnd_max,
                         const double lum_min, const double lum_max,
  ...
  if ((ddl_tab != NULL) && (val_tab != NULL) && (ddl_cnt > 0) &&
      (gsdf_tab != NULL) && (gsdf_spl != NULL) && (gsdf_cnt > 0) &&
      (jnd_min < jnd_max)) {
    int status = 0;
    const unsigned long gin_ctn =
        (inverse) ? ddl_cnt : Count;  // number of points to be interpolated
    double* jidx = new double[gin_ctn];
    if (jidx != NULL) {
      const double dist =
          (jnd_max - jnd_min) / (gin_ctn - 1);  // distance between two entries
      unsigned long i;
      double* s = jidx;
      ...
      *s = jnd_max;  // last value is fixed !
      ...
      if (jnd_idx != NULL) {
        ...
        if (gsdf != NULL) {
          if (DiCubicSpline<double, double>::Interpolation(
                  jnd_idx, gsdf_tab, gsdf_spl, gsdf_cnt, jidx, gsdf,
            ...
            if (DataBuffer != NULL) {
              ...
              if (inverse) {
                ...
                for (i = 0; i < Count; ++i) {
                  ...
                  while ((j + 1 < ddl_cnt) &&
                         (gsdf[j] <
                          v))  // search for closest index, assuming monotony
                    ++j;
                ...
                for (i = Count; i != 0; --i, ++r) {
                  while ((j < ddl_max) &&
                    ...
                    ++j;
                  if ((j > 0) && (fabs(val_tab[j - 1] + amb - *r) <
                                  fabs(val_tab[j] + amb - *r)))
                    --j;
                  *(q++) = ddl_tab[j];
                }
              }
              ...
              {
                if (Count == ddl_cnt)  // check whether GSDF LUT fits exactly to
                                       // DISPLAY file
                {
                  for (i = 0; i < ddl_cnt; ++i) {
                  ...
                  DCMIMGLE_WARN(
                      "can't write curve data, wrong DISPLAY file or GSDF LUT");
                }
              }
              status = 1;
            }
