...
DiOverlay::DiOverlay(const DiDocument* docu, const Uint16 alloc,
                     const Uint16 stored, const Uint16 high)
    : Left(0),
      Top(0),
      Width(0),
      Height(0),
  ...
  Data = new DiOverlayData(
      MaxOverlayCount);  // can't determine number of overlays :-(
  if ((docu != NULL) && (Data != NULL) && (Data->Planes != NULL)) {
    unsigned int i;
    for (i = 0; i < MaxOverlayCount; ++i) {
      Data->Planes[Data->Count] = new DiOverlayPlane(
          docu, convertToGroupNumber(i), alloc, stored, high);
...
DiOverlay::DiOverlay(const DiOverlay* overlay, const signed long left_pos,
                     const signed long top_pos, const double xfactor,
  ...
  if (temp != NULL) {
    ...
    DiScaleTemplate<Uint16> scale(1, overlay->Width, overlay->Height, Width,
                                  Height, Frames);
    scale.scaleData(OFconst_cast(const Uint16**, &temp), &(Data->DataBuffer),
                    0);
...
DiOverlay::DiOverlay(const DiOverlay* overlay, const int horz, const int vert,
  ...
  if (temp != NULL) {
    ...
    flip.flipData(OFconst_cast(const Uint16**, &temp), &(Data->DataBuffer),
                  horz, vert);
    if (temp != overlay->Data->DataBuffer)
      delete[] temp;
    unsigned int i;
    for (i = 0; i < Data->ArrayEntries; ++i) {
      if (Data->Planes[i] != NULL) {
        Data->Planes[i]->setFlipping(
            horz, vert, OFstatic_cast(signed long, columns) + overlay->Left,
...
DiOverlay::DiOverlay(
    const DiOverlay* overlay, const int degree,
    const Uint16 columns,  // width of surrounding image (already rotated)
    const Uint16 rows)
    : Left(0),
      Top(0),
      Width(((degree == 90) || (degree == 270)) ? overlay->Height
                                                : overlay->Width),
      Height(((degree == 90) || (degree == 270)) ? overlay->Width
                                                 : overlay->Height),
      Frames(overlay->Frames),
      AdditionalPlanes(overlay->AdditionalPlanes),
      Data(NULL) {
  Uint16* temp = Init(overlay);
  if (temp != NULL) {
    DiRotateTemplate<Uint16> rotate(1, overlay->Width, overlay->Height, Width,
                                    Height, Frames);
    rotate.rotateData(OFconst_cast(const Uint16**, &temp), &(Data->DataBuffer),
                      degree);
    if (temp != overlay->Data->DataBuffer)
      delete[] temp;
    unsigned int i;
    for (i = 0; i < Data->ArrayEntries; ++i) {
      if (Data->Planes[i] != NULL)
        Data->Planes[i]->setRotation(degree, overlay->Left, overlay->Top,
                                     columns, rows);
    }
  }
}
...
Uint16* DiOverlay::Init(const DiOverlay* overlay) {
  if ((overlay != NULL) && (overlay->Data != NULL) &&
    ...
    if (AdditionalPlanes)
      Data = new DiOverlayData(
          overlay->Data->ArrayEntries);  // use same array size
    else
      Data = new DiOverlayData(
          overlay->Data->Count);  // shrink array size to minimal size
    const unsigned long count = OFstatic_cast(unsigned long, overlay->Width) *
                                OFstatic_cast(unsigned long, overlay->Height) *
                                overlay->Frames;
    if ((Data != NULL) && (Data->Planes != NULL) && (count > 0)) {
      unsigned int i;
      Data->DataBuffer =
          new Uint16[OFstatic_cast(unsigned long, Width) *
      ...
      if (Data->DataBuffer != NULL) {
        ...
        for (i = 0; i < Data->ArrayEntries; ++i) {
          if ((overlay->Data->Planes[i] !=
               NULL) /*&& (overlay->Data->Planes[i]->isValid())*/) {
            Data->Planes[i] = new DiOverlayPlane(
                overlay->Data->Planes[i], i, Data->DataBuffer, temp,
        ...
        {
          DCMIMGLE_WARN(
              "different number of overlay planes for scaled and unscaled "
              "image");
        }
...
int DiOverlay::convertToPlaneNumber(unsigned int& plane, const int mode) const {
  if ((Data != NULL) && (Data->Planes != NULL)) {
    if (isValidGroupNumber(plane)) {
      if (AdditionalPlanes) {
        plane =
            (plane - FirstOverlayGroup) >> 1;  // plane = (group - 0x6000) / 2
        ...
        for (i = 0; i < Data->Count; ++i) {
          if ((Data->Planes[i] != NULL) &&
              (Data->Planes[i]->getGroupNumber() == plane)) {
            plane = i;  // plane number
            return 2;
          }
        }
      }
    } else if (!mode && (plane < Data->Count) &&
               (Data->Planes[plane] != NULL))  // valid plane number?
      return 3;
  }
  return 0;
}
...
int DiOverlay::isValidGroupNumber(const unsigned int group) const {
  return (group >= convertToGroupNumber(0)) &&
         (group <= convertToGroupNumber(MaxOverlayCount - 1)) && !(group & 1);
}

int DiOverlay::checkPlane(const unsigned int plane, const int mode) {
  if ((Data != NULL) && (Data->Planes != NULL) && (plane < MaxOverlayCount) &&
    ...
    if (Data->Planes[plane]->isValid()) {
      ...
      if (Data->Planes[plane]->getHeight() >
          Height)  // determine maximum height
        Height = Data->Planes[plane]->getHeight();
      if (mode && (Data->Planes[plane]->getNumberOfFrames() >
                   Frames))  // determine maximum frames
...
int DiOverlay::showPlane(unsigned int plane, const double fore,
                         const double tresh, const EM_Overlay mode) {
  if (convertToPlaneNumber(plane, AdditionalPlanes) > 1) {
...
int DiOverlay::showPlane(unsigned int plane, const Uint16 pvalue) {
  if (convertToPlaneNumber(plane, AdditionalPlanes) > 1)
...
int DiOverlay::showAllPlanes(const double fore, const double tresh,
...
int DiOverlay::placePlane(unsigned int plane, const signed int left_pos,
                          const signed int top_pos) {
  if (convertToPlaneNumber(plane, AdditionalPlanes) > 1) {
    if ((Data->Planes[plane]->getLeft() == left_pos) &&
        (Data->Planes[plane]->getTop() == top_pos))
      return 2;
...
int DiOverlay::addPlane(const unsigned int group, const signed int left_pos,
  ...
  if (AdditionalPlanes && isValidGroupNumber(group)) {
    ...
    if ((status != 0) && (plane < Data->ArrayEntries)) {
      ...
      Data->Planes[plane] =
          new DiOverlayPlane(group, left_pos, top_pos, columns, rows, data,
                             label, description, mode);
      if (checkPlane(plane, 0)) {
        if (Data->Planes[plane]->getNumberOfFrames() >
            Frames)  // set maximum number of frames
          Frames = Data->Planes[plane]->getNumberOfFrames();
      } else {
...
void* DiOverlay::getPlaneData(const unsigned long frame, unsigned int plane,
                              unsigned int& left_pos, unsigned int& top_pos,
                              unsigned int& width, unsigned int& height,
                              EM_Overlay& mode, const Uint16 columns,
                              const Uint16 rows, const int bits,
  ...
  if (convertToPlaneNumber(plane, AdditionalPlanes) > 1)  // plane does exist
  {
    ...
    if ((op != NULL) && op->isValid()) {
      ...
      const Uint16 xmax =
          (op->getRight(Left) < columns) ? op->getRight(Left) : columns;
      const Uint16 ymax =
          (op->getBottom(Top) < rows) ? op->getBottom(Top) : rows;
      left_pos = xmin;
      top_pos = ymin;
      width = xmax - xmin;
      height = ymax - ymin;
...
void* DiOverlay::getFullPlaneData(const unsigned long frame, unsigned int plane,
                                  unsigned int& width, unsigned int& height,
                                  const int bits, const Uint16 fore,
                                  const Uint16 back) {
  if (convertToPlaneNumber(plane, AdditionalPlanes) > 1)  // plane does exist
  {
    DiOverlayPlane* op = Data->Planes[plane];
