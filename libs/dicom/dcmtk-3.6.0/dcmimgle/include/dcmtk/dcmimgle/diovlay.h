...
public:
  ...
  DiOverlay(const DiDocument* docu = NULL, const Uint16 alloc = 0,
            const Uint16 stored = 0, const Uint16 high = 0);

  /** constructor, clip/scale
   *
   ** @param  overlay   pointer to reference overlay object
   *  @param  left_pos  x-coordinate of new overlay origin (offset for all
   *planes)
   *  @param  top_pos   y-coordinate of new overlay origin (offset for all
   *planes)
   *  @param  xfactor   scaling factor in x-direction
   *  @param  yfactor   scaling factor in y-direction
   */
  DiOverlay(const DiOverlay* overlay, const signed long left_pos,
   ...
   */
  DiOverlay(const DiOverlay* overlay, const int horz, const int vert,
   ...
   *  @param  rows     height of the surrounding image
   */
  DiOverlay(const DiOverlay* overlay, const int degree, const Uint16 columns,
            const Uint16 rows);
   ...
   *
   ** @param  plane  index of plane (starting from 0) or group number
   *(0x6000-0x60ff)
   *
   ...
   ** @return status, true if successful, false otherwise
   */
  int showPlane(unsigned int plane, const double fore, const double thresh,
   ...
   ** @return status, true if successful, false otherwise
   */
  int showAllPlanes(const double fore, const double thresh,
                    const EM_Overlay mode);
   ...
   ** @return status, true if successful, false otherwise
   */
  int placePlane(unsigned int plane, const signed int left_pos,
   ...
   *                       If group number already exists in the list of
   *additional planes the old one is replaced. If the number exists in the list
   *of planes stored in the image dataset the new plane hides it.
   ...
   ** @return status, true if successful, false otherwise
   */
  int addPlane(const unsigned int group, const signed int left_pos,
  ...
  inline DiOverlayPlane* getPlane(const unsigned int plane) const {
    return ((Data != NULL) && (Data->Planes != NULL) && (plane < Data->Count))
               ? Data->Planes[plane]
               : OFstatic_cast(DiOverlayPlane*, NULL);
  }
   ...
   ** @param  plane    index of plane (starting from 0) or group number
   *(0x6000-0x60ff)
   *  @param  visible  flag indicating whether plane should be visible or not
   *(default: 0 = ignore)
   *
   ** @return status, true if such a plane exists, false otherwise
   */
  inline int hasPlane(unsigned int plane, const int visible = 0) const {
    return (convertToPlaneNumber(plane, AdditionalPlanes) > 1) &&
           (!visible || Data->Planes[plane]->isVisible());
  }

  /** get overlay plane data as an array of 1/8/16 bit values.
   *  Memory isn't handled internally and must therefore be deleted from calling
   *program.
   *
   ...
   *8
   *  @param  fore      foreground color used for the plane (default: 0xff =
   *white, for 8 bits)
   *  @param  back      transparent background color (default: 0x00 = black)
   *
   ** @return pointer to pixel data if successful, NULL otherwise
   */
  void* getPlaneData(const unsigned long frame, unsigned int plane,
                     unsigned int& left_pos, unsigned int& top_pos,
                     unsigned int& width, unsigned int& height,
                     EM_Overlay& mode, const Uint16 columns, const Uint16 rows,
   ...
   ** @return pointer to pixel data if successful, NULL otherwise
   */
  void* getFullPlaneData(const unsigned long frame, unsigned int plane,
                         unsigned int& width, unsigned int& height,
                         const int bits = 8, const Uint16 fore = 0xff,
                         const Uint16 back = 0x0);
   ...
   *otherwise
   */
  unsigned long create6xxx3000PlaneData(Uint8*& buffer, unsigned int plane,
                                        unsigned int& width,
                                        unsigned int& height,
...
protected:
   ...
   *
   ** @return status, true if successful, false otherwise.
   *          (1 = additional plane with the specified group number would be
   *new, 2 = additional plane with the specified group number already exists, 3
   *= 'plane' is the number of a valid plane - only if 'mode' us false)
   */
  ...
  /** check whether the specified plane is valid. and determine maximum
   *resolution and number of frames which are common for all overlay planes so
   *far.
   *
   ** @param  plane  index of plane (0..15)
   *  @param  mode   if true the maximum number of frames is determined,
   *otherwise not
   *
   ** @return status, true if successful, false otherwise
   */
