...
/** Template class to convert monochrome pixel data to intermediate
 * representation
 */
template <class T1, class T2, class T3>
class DiMonoInputPixelTemplate : public DiMonoPixelTemplate<T3> {
public:
  /** constructor
   *
  ...
  DiMonoInputPixelTemplate(DiInputPixel* pixel, DiMonoModality* modality)
    ...
    if ((pixel != NULL) && (this->Count > 0)) {
      ...
      if ((this->Modality != NULL) && this->Modality->hasLookupTable() &&
          (bitsof(T1) <= MAX_TABLE_ENTRY_SIZE)) {
        modlut(pixel);
        // ignore modality LUT min/max values since the image does not
        // necessarily have to use all LUT entries
        ...
        this->determineMinMax(OFstatic_cast(T3, this->Modality->getMinValue()),
                              OFstatic_cast(T3, this->Modality->getMaxValue()));
      } else {
...
private:
   ...
   *  @param  ocnt  number of LUT entries (will be check as optimization
   *criteria)
   *
  ...
  inline int initOptimizationLUT(T3*& lut, const unsigned long ocnt) {
    int result = 0;
    if ((sizeof(T1) <= 2) &&
        (this->InputCount > 3 * ocnt))  // optimization criteria
  ...
  void modlut(DiInputPixel* input) {
    ...
    if ((pixel != NULL) && (this->Modality != NULL)) {
      ...
      if (mlut != NULL) {
        const int useInputBuffer =
            (sizeof(T1) == sizeof(T3)) && (this->Count <= input->getCount());
        ...
        if (this->Data != NULL) {
          DCMIMGLE_DEBUG("applying modality tranformation with LUT ("
                         << mlut->getCount() << " entries)");
          T2 value = 0;
          const T2 firstentry =
              mlut->getFirstEntry(value);  // choose signed/unsigned method
          ...
          const unsigned long ocnt = OFstatic_cast(
              unsigned long, input->getAbsMaxRange());  // number of LUT entries
   ...
   */
  void rescale(DiInputPixel* input, const double slope = 1.0,
               const double intercept = 0.0) {
    const T1* pixel = OFstatic_cast(const T1*, input->getData());
    if (pixel != NULL) {
      const int useInputBuffer = (sizeof(T1) == sizeof(T3)) &&
                                 (this->Count <= input->getCount()) &&
                                 (input->getPixelStart() == 0);
      if (useInputBuffer) {  // do not copy pixel data, reference them!
        DCMIMGLE_DEBUG("re-using input buffer, do not copy pixel data");
        this->Data = OFstatic_cast(T3*, input->getDataPtr());
        input->removeDataReference();  // avoid double deletion
      ...
      if (this->Data != NULL) {
        ...
        if ((slope == 1.0) && (intercept == 0.0)) {
          if (!useInputBuffer) {
            const T1* p = pixel + input->getPixelStart();
            for (i = this->InputCount; i != 0;
                 --i)  // copy pixel data: can't use copyMem because T1 isn't
                       // always equal to T3
              *(q++) = OFstatic_cast(T3, *(p++));
          }
        } else {
          DCMIMGLE_DEBUG(
              "applying modality transformation with rescale slope = "
          ...
          if (initOptimizationLUT(lut, ocnt)) {  // use LUT for optimization
            ...
            if (slope == 1.0) {
              for (i = 0; i < ocnt; ++i)  // calculating LUT entries
                *(q++) = OFstatic_cast(
                    T3, OFstatic_cast(double, i) + absmin + intercept);
            } else {
              if (intercept == 0.0) {
                for (i = 0; i < ocnt; ++i)
                  *(q++) = OFstatic_cast(
                      T3, (OFstatic_cast(double, i) + absmin) * slope);
              } else {
                for (i = 0; i < ocnt; ++i)
                  *(q++) = OFstatic_cast(
                      T3,
                      (OFstatic_cast(double, i) + absmin) * slope + intercept);
              }
            }
            const T3* lut0 =
                lut - OFstatic_cast(T2, absmin);  // points to 'zero' entry
          ...
          if (lut == NULL)  // use "normal" transformation
          {
            if (slope == 1.0) {
              for (i = this->InputCount; i != 0; --i)
                *(q++) = OFstatic_cast(
                    T3, OFstatic_cast(double, *(p++)) + intercept);
            } else {
              if (intercept == 0.0) {
                for (i = this->InputCount; i != 0; --i)
                  *(q++) =
                      OFstatic_cast(T3, OFstatic_cast(double, *(p++)) * slope);
              } else {
                for (i = this->InputCount; i != 0; --i)
                  *(q++) = OFstatic_cast(
                      T3, OFstatic_cast(double, *(p++)) * slope + intercept);
              }
            }
          }
          delete[] lut;
