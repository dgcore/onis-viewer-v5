...
public:
   ...
   */
  enum E_DeviceType {
    /// monitor (softcopy output device), values expected to be in luminance
    /// (cd/m^2)
    EDT_Monitor,
    /// camera (softcopy input device), values expected to be in luminance
    /// (cd/m^2)
    EDT_Camera,
    /// printer (hardcopy output device), values expected to be in optical
    /// density (OD)
    EDT_Printer,
    /// scanner (hardcopy input device), values expected to be in optical
    /// density (OD)
    EDT_Scanner
  };

  /** constructor, read device characteristics file.
   *  Keywords: "max" for maximum DDL (device driving level, required at first
   *position) "amb" for ambient light and "lum" for illumination (both optional)
   *            "ord" for the order of the polynomial curve fitting algorithm
   *used to interpolate the given base points (0 or absent = use cubic spline
   *interpolation)
   *
   ** @param  filename    name of the characteristics file (luminance/OD for
   *each DDL)
   *  @param  deviceType  type of the output device (default: monitor)
   *  @param  ord         order of the polynomial curve fitting algorithm used
   *to interpolate the given base points (-1 = use file setting, 0 = cubic
   *spline)
   */
   ...
   *  Values must be sorted and complete (i.e. there must be an entry for each
   *DDL). The given arrays are copied internally.
   *
   ** @param  val_tab     pointer to array with luminance/OD values
   *  @param  count       number of array elements (should be equal to 'max +
   *1')
   *  @param  max         maximum DDL (device driving level)
   *  @param  deviceType  type of the output device (default: monitor)
   *  @param  ord         order of the polynomial curve fitting algorithm used
   *to interpolate the given base points (0 or negative = use cubic spline
   *interpolation)
   */
  DiDisplayFunction(const double* val_tab, const unsigned long count,
                    const Uint16 max = 255,
                    const E_DeviceType deviceType = EDT_Monitor,
                    const signed int ord = 0);

  /** constructor, use given array of DDL and luminance/OD values. UNTESTED
   *  Values will be automatically sorted and missing values will be
   *interpolated. The given arrays are copied internally.
   *
   ** @param  ddl_tab     pointer to array with DDL values (must be with the
   *interval 0..max)
   *  @param  val_tab     pointer to array with luminance/OD values
   ...
   *interpolation)
   */
  DiDisplayFunction(const Uint16* ddl_tab, const double* val_tab,
                    const unsigned long count, const Uint16 max = 255,
                    const E_DeviceType deviceType = EDT_Monitor,
                    const signed int ord = 0);

  /** constructor, compute luminance/OD values automatically within the
   *specified range. Initial values for hardcopy: "lum" = 2000, "amb" = 10, for
   *softcopy: "amb" = 0.
   *
   ...
   *interpolation)
   */
  DiDisplayFunction(const double val_min, const double val_max,
   ...
   *
   ** @param  filename  name of the text file to which the data should be
   *written
   ...
   *
   ** @return status, true if successful (1 = Dmin set, 2 = Dmin unset), false
   *otherwise
   */
   ...
   *
   ** @return status, true if successful (1 = Dmax set, 2 = Dmax unset), false
   *otherwise
   */
   ...
   *
   ** @return polynomial order (0 = use cubic spline interpolation, -1 = not
   *specified)
   */
   ...
   *  This function uses the currently set ambient light and illumination
   *values.
   *
   ...
   ** @return luminance value if successful, -1 otherwise
   */
  static double convertODtoLum(const double value, const double ambient,
                               const double illum);
...
protected:
   ...
   *  Uses the currently set ambient light and illumination value.  The
   *resulting luminance table has to be deleted by the caller.
   *
   ...
   */
  double* convertODtoLumTable(const double* od_tab, const unsigned long count,
                              const OFBool useAmb);

  /** interpolate device characteristic curve by means of a cubic spline
   * interpolation
   */
  ...
  /// constant defining maximum value for number of bits for LUT input (here:
  /// 16)
