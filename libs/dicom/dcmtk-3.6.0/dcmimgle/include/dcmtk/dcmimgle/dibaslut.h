...
public:
   ...
   *  ... or the previous pixel transformation requires an unsigned LUT input
   *value.
   *
   ...
   *  ... or the previous pixel transformation requires a signed LUT input
   *value.
   *
   ** (#)param  dummy (used to distinguish between signed and unsigned methods)
   *
   ...
   *  The maximum value which could be stored with the specified bit depth is
   *calculated.
   *
  ...
  inline const char* getExplanation() const {
    return (Explanation.empty()) ? OFstatic_cast(const char*, NULL)
                                 : Explanation.c_str();
  }
