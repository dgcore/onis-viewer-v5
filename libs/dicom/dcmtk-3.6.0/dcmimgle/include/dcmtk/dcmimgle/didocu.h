...
public:
   ...
   */
  DiDocument(const char* filename, const unsigned long flags = 0,
   ...
   *  @param  fcount  number of frames (only stored for later use)
   */
  DiDocument(DcmObject* object, const E_TransferSyntax xfer,
   ...
   *  Please note that this is the color model of the decompressed image which
   *might deviate from the color model of the original compressed image.
   *
   ...
   *
   ** @return true if pixel data is compressed, false if an uncompressed version
   *exists
   */
   ...
   *  @param  obj  element in the dataset where the search should start
   *(default: root)
   *
   ...
   *  @param  item         element in the dataset where the search should start
   *(default: root)
   *  @param  allowSigned  also allow signed value (Sint16) if true
   *
   ** @return VM if successful, 0 otherwise
   */
  unsigned long getValue(const DcmTagKey& tag, Uint16& returnVal,
                         const unsigned long pos = 0, DcmObject* item = NULL,
   ...
   ** @return VM if successful, 0 otherwise
   */
  unsigned long getValue(const DcmTagKey& tag, Sint16& returnVal,
   ...
   ** @return VM if successful, 0 otherwise
   */
  unsigned long getValue(const DcmTagKey& tag, Uint32& returnVal,
                         const unsigned long pos = 0) const;
   ...
   ** @return VM if successful, 0 otherwise
   */
  unsigned long getValue(const DcmTagKey& tag, Sint32& returnVal,
                         const unsigned long pos = 0) const;
   ...
   ** @return VM if successful, 0 otherwise
   */
  unsigned long getValue(const DcmTagKey& tag, double& returnVal,
                         const unsigned long pos = 0) const;
   ...
   */
  unsigned long getValue(const DcmTagKey& tag, const Uint16*& returnVal,
   ...
   ** @return VM if successful, 0 otherwise
   */
  unsigned long getValue(const DcmTagKey& tag, const char*& returnVal,
                         DcmObject* item = NULL) const;
   ...
   ** @return VM if successful, 0 otherwise
   */
  unsigned long getValue(const DcmTagKey& tag, OFString& returnVal,
   ...
   ** @return VM if successful, 0 otherwise
   */
  static unsigned long getElemValue(const DcmElement* elem, Uint16& returnVal,
                                    const unsigned long pos = 0,
   ...
   ** @return VM if successful, 0 otherwise
   */
  static unsigned long getElemValue(const DcmElement* elem, OFString& returnVal,
                                    const unsigned long pos = 0);
