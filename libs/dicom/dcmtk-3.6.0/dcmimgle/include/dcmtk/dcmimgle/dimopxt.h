 ...
 */
template <class T>
class DiMonoPixelTemplate : public DiMonoPixel,
...
public:
  ...
  DiMonoPixelTemplate(const DiInputPixel* pixel, DiMonoModality* modality)
      : DiMonoPixel(pixel, modality), Data(NULL) {
    MinValue[0] = 0;
    MinValue[1] = 0;
    MaxValue[0] = 0;
    MaxValue[1] = 0;
  ...
  /** constructor
   *
   ** @param  pixel     pointer to output pixel data used for intermediate
   *representation
   *  @param  modality  pointer to object managing modality transform
   */
  ...
  inline int getMinMaxWindow(const int idx, double& center, double& width) {
    ...
    if ((idx >= 0) && (idx <= 1)) {
      ...
      */
      center = (OFstatic_cast(double, MinValue[idx]) +
                OFstatic_cast(double, MaxValue[idx]) + 1) /
               2;  // type cast to avoid overflows !
      width = OFstatic_cast(double, MaxValue[idx]) -
              OFstatic_cast(double, MinValue[idx]) + 1;
  ...
  virtual int getRoiWindow(const unsigned long left_pos,
                           ...
                           const unsigned long rows, const unsigned long frame,
                           double& voiCenter, double& voiWidth) {
    int result = 0;
    if ((Data != NULL) && (left_pos < columns) && (top_pos < rows)) {
      T* p = Data + (columns * rows * frame) + (top_pos * columns) + left_pos;
      const unsigned long right_pos =
          (left_pos + width < columns) ? left_pos + width : columns;
      const unsigned long bottom =
          (top_pos + height < rows) ? top_pos + height : rows;
      ...
      */
      voiCenter =
          (OFstatic_cast(double, min) + OFstatic_cast(double, max) + 1) /
          2;  // type cast to avoid overflows !
   ...
   *
   ** @param  thresh  ignore certain percentage of pixels at lower and upper
   *boundaries
   *  @param  center  reference to storage area for window center value
   *  @param  width   reference to storage area for window width value
   *
   ** @return status, true if successful, false otherwise
   */
  int getHistogramWindow(
      const double thresh,  // could be optimized if necessary (see diinpxt.h)!
    ...
    if ((Data != NULL) && (MinValue[0] < MaxValue[0])) {
      ...
      if (quant != NULL) {
        ...
        for (i = 0; i < Count; ++i) {
          if ((Data[i] >= MinValue[0]) &&
              (Data[i] <= MaxValue[0]))  // only for stability !
            ++quant[OFstatic_cast(Uint32,
                                  Data[i] - MinValue[0])];  // count values
#ifdef DEBUG
          else
            DCMIMGLE_WARN(
                "invalid value ("
        ...
        const Uint32 threshvalue =
            OFstatic_cast(Uint32, thresh * OFstatic_cast(double, Count));
        Uint32 t = 0;
        i = 0;
        ...
        if (minvalue < maxvalue) {
          /* suppl. 33: "A Window Center of 2^n-1 and a Window Width of 2^n
                         selects the range of input values from 0 to 2^n-1."
          */
          center = (OFstatic_cast(double, minvalue) +
                    OFstatic_cast(double, maxvalue) + 1) /
                   2;
          width = OFstatic_cast(double, maxvalue) -
...
protected:
  /** constructor
   *
   ** @param  pixel     pointer to intermediate pixel data (not necessarily
   *monochrome)
   *  @param  modality  pointer to object managing modality transform
   */
  ...
  DiMonoPixelTemplate(const DiMonoPixel* pixel, const unsigned long count)
      : DiMonoPixel(pixel, count), Data(NULL) {
    MinValue[0] = 0;
    MinValue[1] = 0;
    MaxValue[0] = 0;
    MaxValue[1] = 0;
    ...
    if (Data != NULL) {
      if (mode & 0x1) {
        if ((minvalue == 0) && (maxvalue == 0)) {
          DCMIMGLE_DEBUG(
              "determining global minimum and maximum pixel values for "
              "monochrome image");
          T* p = Data;
          T value = *p;
          unsigned long i;
          ...
          maxvalue = value;
          for (i = Count; i > 1;
               --i)  // could be optimized if necessary (see diinpxt.h) !
          {
            value = *(++p);
            if (value < minvalue)
              minvalue = value;
      ...
      if (mode & 0x2) {
        DCMIMGLE_DEBUG(
            "determining next minimum and maximum pixel values for monochrome "
            "image");
        T* p = Data;
        T value;
        int firstmin = 1;
