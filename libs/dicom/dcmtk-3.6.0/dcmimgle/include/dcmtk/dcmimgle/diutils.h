...
/// take responsibility for the given external DICOM dataset, i.e. delete it on
/// destruction
...
/// ignore third value of the modality LUT descriptor, determine bit depth
/// automatically
const unsigned long CIF_IgnoreModalityLutBitDepth = 0x0000080;

/// check third value of the LUT descriptor, compare with with expected bit
/// depth based on LUT data
const unsigned long CIF_CheckLutBitDepth = 0x0000100;

/// use absolute (possible) pixel range for determining the internal
/// representation (monochrome only)
const unsigned long CIF_UseAbsolutePixelRange = 0x0000200;

/// use partial access to pixel data, i.e. without decompressing or loading a
/// complete multi-frame image
const unsigned long CIF_UsePartialAccessToPixelData = 0x0000400;

/// always decompress complete pixel data when processing an image, i.e. even if
/// partial access is used
const unsigned long CIF_DecompressCompletePixelData = 0x0000800;

/// never access embedded overlays since this requires to load and uncompress
/// the complete pixel data
...
struct SB_BitmapInfoHeader {
  ...
  Uint16 biPlanes;
  /// bits per pixel, supported values: 8 = color palette with 256 entries, 24 =
  /// true color
  Uint16 biBitCount;
  /// type of compression, support value: 0 = BI_RGB, no compression
  Uint32 biCompression;
  /// size of the image data (in bytes), might be set to '0' if image is
  /// uncompressed
  Uint32 biSizeImage;
  ...
  /// number of actually used colors, if '0' the number of colors is calculated
  /// using 'biBitCount'
  Uint32 biClrUsed;
...
enum EP_Representation {
  ...
  EPR_Uint8,
  EPR_MinUnsigned = EPR_Uint8,
  /// signed 8 bit integer
  EPR_Sint8,
  EPR_MinSigned = EPR_Sint8,
  ...
  EPR_Uint32,
  EPR_MaxUnsigned = EPR_Uint32,
  /// signed 32 bit integer
  EPR_Sint32,
  EPR_MaxSigned = EPR_Sint32
};
...
const SP_Interpretation PhotometricInterpretationNames[] = {
    ...
    {"PALETTECOLOR", "PALETTE COLOR",
     EPI_PaletteColor},  // space deleted to simplify detection
    {"RGB", "RGB", EPI_RGB},
    {"HSV", "HSV", EPI_HSV},
    {"ARGB", "ARGB", EPI_ARGB},
    {"CMYK", "CMYK", EPI_CMYK},
    {"YBRFULL", "YBR_FULL",
     EPI_YBR_Full},  // underscore deleted to simplify detection
    {"YBRFULL422", "YBR_FULL_422",
     EPI_YBR_Full_422},  // underscores deleted to simplify detection
    {"YBRPARTIAL422", "YBR_PARTIAL_422",
     EPI_YBR_Partial_422},  // underscores deleted to simplify detection
...
public:
  /** calculate maximum value which could be stored in the specified number of
   *bits
   *
  ...
  /** determine integer representation which is necessary to store values in the
   *specified range
   *
   ** @param  minvalue  minimum value to be stored
   *  @param  maxvalue  maximum value to be stored
