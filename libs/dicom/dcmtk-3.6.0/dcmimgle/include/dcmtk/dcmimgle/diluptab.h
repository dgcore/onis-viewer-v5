 ...
 */
class DiLookupTable : public DiBaseLUT, public DiObjectCounter {
public:
  /** constructor
   *
   ...
   */
  DiLookupTable(const DiDocument* docu, const DcmTagKey& descriptor,
                const DcmTagKey& data, const DcmTagKey& explanation,
   ...
   *  @param  descripMode  mode specifying the use of the bits per table entry
   *value
   *  @param  pos          position in the LUT sequence which should be used
   *(optional)
   *  @param  card         pointer to storage area where the number of sequence
   *entries should be stored (optional)
   */
  DiLookupTable(const DiDocument* docu, const DcmTagKey& sequence,
                const DcmTagKey& descriptor, const DcmTagKey& data,
                const DcmTagKey& explanation,
                const EL_BitsPerTableEntry descripMode = ELM_UseValue,
                const unsigned long pos = 0, unsigned long* card = NULL);
   ...
   *  @param  descripMode  mode specifying the use of the bits per table entry
   *value
   *  @param  first        expected value for "first input value mapped"
   *(optional)
   *  @param  status       pointer to image status variable (optional)
   */
  DiLookupTable(const DcmUnsignedShort& data,
                ...
                const EL_BitsPerTableEntry descripMode = ELM_UseValue,
                const signed long first = -1, EI_Status* status = NULL);
   ...
   *
   ** @param  flag  if bit 0 is set (0x1, default) the internal copy of the LUT
   *data is modified or a copy of the original data is created (if not already
   *existing), if bit 1 is set (0x2) the original LUT data is modified, a
   *combination of both modes is also supported.
   *
   ** @return status true if successful (0x1 if internal data modified, 0x2 if
   *original data, 0x3 if both), false otherwise
   */
...
protected:
   ...
   *
   ** @param  docu         pointer to dataset (encapsulated)
   *  @param  obj          pointer to item in dataset where to start (maybe
   *NULL)
   *  @param  descriptor   tag key containing the LUT descriptor
   *  @param  data         tag key containing the LUT data
   *  @param  explanation  tag key containing the LUT explanation
   ...
   *  @param  status       pointer to image status variable (optional)
   */
  void Init(const DiDocument* docu, DcmObject* obj, const DcmTagKey& descriptor,
            const DcmTagKey& data, const DcmTagKey& explanation,
            const EL_BitsPerTableEntry descripMode = ELM_UseValue,
            EI_Status* status = NULL);
   ...
   *  @param  status       pointer to image status variable (optional)
   */
  void checkTable(unsigned long count, Uint16 bits,
                  const EL_BitsPerTableEntry descripMode = ELM_UseValue,
                  EI_Status* status = NULL);
   ...
   *value
   */
  void checkBits(const Uint16 bits, const Uint16 rightBits,
