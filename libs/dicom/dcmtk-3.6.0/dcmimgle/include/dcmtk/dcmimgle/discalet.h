...
static inline void setScaleValues(Uint16 data[], const Uint16 min,
                                  const Uint16 max) {
  ...
  if (remainder > OFstatic_cast(Uint16, min / 2)) {
    ...
    ++step1;
  const double count =
      OFstatic_cast(double, min) / (OFstatic_cast(double, remainder) + 1);
  Uint16 i;
  double c = count;
  for (i = 0; i < min; ++i) {
...
static inline double cubicValue(const double v1, const double v2,
                                const double v3, const double v4,
                                const double dD, const double minVal,
                                const double maxVal) {
  double dVal =
      0.5 *
      ((((-v1 + 3 * v2 - 3 * v3 + v4) * dD + (2 * v1 - 5 * v2 + 4 * v3 - v4)) *
            dD +
        (-v1 + v3)) *
           dD +
       (v2 + v2));
...
public:
  ...
  DiScaleTemplate(const int planes,
                  ...
                  const int bits = 0)
      : DiTransTemplate<T>(planes, src_cols, src_rows, dest_cols, dest_rows,
                           frames, bits),
        Left(left_pos),
        Top(top_pos),
        Columns(columns),
   ...
   *  @param  interpolate  preferred interpolation algorithm (0 = no
   *interpolation, 1 = pbmplus algorithm, 2 = c't algorithm, 3 = bilinear
   *magnification, 4 = bicubic magnification)
   *  @param  value        value to be set outside the image boundaries (used
   *for clipping, default: 0)
   */
  void scaleData(const T* src[], T* dest[], const int interpolate,
                 const T value = 0) {
    if ((src != NULL) && (dest != NULL)) {
      DCMIMGLE_TRACE("Col/Rows: "
                     << Columns << " " << Rows << OFendl << "Left/Top: " << Left
                     << " " << Top << OFendl << "Src  X/Y: " << this->Src_X
                     << " " << this->Src_Y << OFendl
                     << "Dest X/Y: " << this->Dest_X << " " << this->Dest_Y);
      if ((Left + OFstatic_cast(signed long, this->Src_X) <= 0) ||
          (Top + OFstatic_cast(signed long, this->Src_Y) <= 0) ||
          (Left >= OFstatic_cast(signed long, Columns)) ||
          (Top >=
           OFstatic_cast(signed long, Rows))) {  // no image to be displayed
        DCMIMGLE_DEBUG("clipping area is fully outside the image boundaries");
        this->fillPixel(dest, value);  // ... fill bitmap
      } else if ((this->Src_X == this->Dest_X) &&
                 (this->Src_Y == this->Dest_Y))  // no scaling
      {
        if ((Left == 0) && (Top == 0) && (Columns == this->Src_X) &&
            (Rows == this->Src_Y))
          this->copyPixel(src, dest);  // copying
        else if ((Left >= 0) &&
                 (OFstatic_cast(Uint16, Left + this->Src_X) <= Columns) &&
                 (Top >= 0) &&
                 (OFstatic_cast(Uint16, Top + this->Src_Y) <= Rows))
      ...
      else if ((interpolate == 4) && (this->Dest_X >= this->Src_X) &&
               (this->Dest_Y >= this->Src_Y) && (this->Src_X >= 3) &&
               (this->Src_Y >= 3))
        bicubicPixel(src, dest);  // bicubic magnification
      else if ((interpolate >= 3) && (this->Dest_X >= this->Src_X) &&
               (this->Dest_Y >= this->Src_Y) && (this->Src_X >= 2) &&
               (this->Src_Y >= 2))
        bilinearPixel(src, dest);  // bilinear magnification
      else if ((interpolate >= 1) && (this->Dest_X >= this->Src_X) &&
               (this->Dest_Y >= this->Src_Y))
        expandPixel(src, dest);  // interpolated expansion (c't)
      else if ((interpolate >= 1) && (this->Src_X >= this->Dest_X) &&
               (this->Src_Y >= this->Dest_Y))
        reducePixel(src, dest);  // interpolated reduction (c't)
      else if ((interpolate >= 1) && (this->Bits <= MAX_INTERPOLATION_BITS))
        interpolatePixel(src, dest);  // interpolation (pbmplus), fallback
      else if ((this->Dest_X % this->Src_X == 0) &&
               (this->Dest_Y % this->Src_Y == 0))
        replicatePixel(src, dest);  // replication
      else if ((this->Src_X % this->Dest_X == 0) &&
               (this->Src_Y % this->Dest_Y == 0))
...
private:
  ...
  void clipPixel(const T* src[], T* dest[]) {
    ...
    const unsigned long y_feed =
        OFstatic_cast(unsigned long, Rows - this->Src_Y) *
    ...
    for (int j = 0; j < this->Planes; ++j) {
      p = src[j] +
          OFstatic_cast(unsigned long, Top) *
              OFstatic_cast(unsigned long, Columns) +
          Left;
      q = dest[j];
  ...
  void clipBorderPixel(const T* src[], T* dest[], const T value) {
    ...
    const Uint16 d_right = (OFstatic_cast(unsigned long, this->Src_X) +
                                OFstatic_cast(unsigned long, s_left) <
                            OFstatic_cast(unsigned long, Columns) +
                                OFstatic_cast(unsigned long, d_left))
                               ? (this->Src_X - 1)
                               : (Columns + d_left - s_left - 1);
    const Uint16 d_bottom = (OFstatic_cast(unsigned long, this->Src_Y) +
                                 OFstatic_cast(unsigned long, s_top) <
                             OFstatic_cast(unsigned long, Rows) +
                                 OFstatic_cast(unsigned long, d_top))
                                ? (this->Src_Y - 1)
                                : (Rows + d_top - s_top - 1);
    const Uint16 x_count = d_right - d_left + 1;
    const Uint16 y_count = d_bottom - d_top + 1;
    const unsigned long s_start = OFstatic_cast(unsigned long, s_top) *
                                      OFstatic_cast(unsigned long, Columns) +
                                  s_left;
    const unsigned long x_feed = Columns - x_count;
    const unsigned long y_feed =
        OFstatic_cast(unsigned long, Rows - y_count) * Columns;
    const unsigned long t_feed = OFstatic_cast(unsigned long, d_top) *
                                 OFstatic_cast(unsigned long, this->Src_X);
    const unsigned long b_feed =
        OFstatic_cast(unsigned long, this->Src_Y - d_bottom - 1) *
        OFstatic_cast(unsigned long, this->Src_X);

    /*
     *  The approach is to divide the destination image in up to four areas
     * outside the source image plus one area for the source image. The for and
     * while loops are scanning linearly over the destination image and setting
     * the appropriate value depending on the current area. This is different
     * from most of the other algorithms in this toolkit where the source image
     * is scanned linearly.
     */
    Uint16 x;
    Uint16 y;
  ...
  void suppressPixel(const T* src[], T* dest[]) {
    ...
    const unsigned long x_feed =
        OFstatic_cast(unsigned long, this->Src_Y / this->Dest_Y) *
            OFstatic_cast(unsigned long, Columns) -
        this->Src_X;
    const unsigned long y_feed =
        OFstatic_cast(unsigned long, Rows - this->Src_Y) *
  ...
  void scalePixel(const T* src[], T* dest[]) {
    ...
    const Uint16 xmin = (this->Dest_X < this->Src_X)
                            ? this->Dest_X
                            : this->Src_X;  // minimum width
    const Uint16 ymin = (this->Dest_Y < this->Src_Y)
                            ? this->Dest_Y
                            : this->Src_Y;  // minimum height
    ...
    /*
     *  Approach: If one pixel line has to be added or removed it is taken from
     * the middle of the image (1/2). For two lines it is at 1/3 and 2/3 of the
     * image and so on. It sounds easy but it was a hard job ;-)
     */

    if ((x_step != NULL) && (y_step != NULL) && (x_fact != NULL) &&
        (y_fact != NULL)) {
      Uint16 x;
      Uint16 y;
      ...
      if (this->Dest_X <= this->Src_X)
        OFBitmanipTemplate<Uint16>::setMem(
            x_fact, 1, xmin);  // initialize with default values
      if (this->Dest_X >= this->Src_X)
        OFBitmanipTemplate<Uint16>::setMem(
            x_step, 1, xmin);  // initialize with default values
      ...
      if (this->Dest_Y <= this->Src_Y)
        OFBitmanipTemplate<Uint16>::setMem(
            y_fact, 1, ymin);  // initialize with default values
      if (this->Dest_Y >= this->Src_Y)
        OFBitmanipTemplate<Uint16>::setMem(
            y_step, 1, ymin);                  // initialize with default values
      ...
      for (int j = 0; j < this->Planes; ++j) {
        ...
        for (unsigned long f = 0; f < this->Frames; ++f) {
          for (y = 0; y < ymin; ++y) {
            ...
            sp += OFstatic_cast(unsigned long, y_step[y]) *
                  OFstatic_cast(unsigned long, Columns);
          }
        }
      }
  ...
  void interpolatePixel(const T* src[], T* dest[]) {
    ...
    if ((this->Src_X != Columns) || (this->Src_Y != Rows)) {
      DCMIMGLE_ERROR(
          "interpolated scaling and clipping at the same time not implemented "
          "... ignoring clipping region");
      this->Src_X = Columns;  // temporarily removed 'const' for 'Src_X' in
                              // class 'DiTransTemplate'
      this->Src_Y = Rows;     //                             ... 'Src_Y' ...
    }

    /*
     *   based on scaling algorithm from "Extended Portable Bitmap Toolkit"
     * (pbmplus10dec91) (adapted to be used with signed pixel representation,
     * inverse images - mono1, various bit depths, multi-frame and
     * multi-plane/color images)
     */
    ...
    const unsigned long sxscale =
        OFstatic_cast(unsigned long, (OFstatic_cast(double, this->Dest_X) /
                                      OFstatic_cast(double, this->Src_X)) *
                                         SCALE_FACTOR);
    const unsigned long syscale =
        OFstatic_cast(unsigned long, (OFstatic_cast(double, this->Dest_Y) /
                                      OFstatic_cast(double, this->Src_Y)) *
                                         SCALE_FACTOR);
    const signed long maxvalue =
        DicomImageClass::maxval(this->Bits - isSigned());
  ...
  void expandPixel(const T* src[], T* dest[]) {
    DCMIMGLE_DEBUG(
        "using expand pixel scaling algorithm with interpolation from c't "
        "magazine");
    const double x_factor = OFstatic_cast(double, this->Src_X) /
                            OFstatic_cast(double, this->Dest_X);
    const double y_factor = OFstatic_cast(double, this->Src_Y) /
                            OFstatic_cast(double, this->Dest_Y);
    const unsigned long f_size = OFstatic_cast(unsigned long, Rows) *
                                 OFstatic_cast(unsigned long, Columns);
    const T* sp;
    double bx, ex;
    double by, ey;
    ...
    /*
     *   based on scaling algorithm from "c't - Magazin fuer Computertechnik"
     * (c't 11/94) (adapted to be used with signed pixel representation, inverse
     * images - mono1, various bit depths, multi-frame and multi-plane/color
     * images, combined clipping/scaling)
     */
      ...
      for (unsigned long f = 0; f < this->Frames; ++f) {
        for (y = 0; y < this->Dest_Y; ++y) {
          ...
          for (x = 0; x < this->Dest_X; ++x) {
            ...
            offset = OFstatic_cast(unsigned long, byi) *
                     OFstatic_cast(unsigned long, Columns);
  ...
  void reducePixel(const T* src[], T* dest[]) {
    DCMIMGLE_DEBUG(
        "using reduce pixel scaling algorithm with interpolation from c't "
        "magazine");
    const double x_factor = OFstatic_cast(double, this->Src_X) /
                            OFstatic_cast(double, this->Dest_X);
    const double y_factor = OFstatic_cast(double, this->Src_Y) /
  ...
  void bilinearPixel(const T* src[], T* dest[]) {
    DCMIMGLE_DEBUG(
        "using magnification algorithm with bilinear interpolation contributed "
        "by Eduard Stanescu");
    const double x_factor = OFstatic_cast(double, this->Src_X) /
                            OFstatic_cast(double, this->Dest_X);
    const double y_factor = OFstatic_cast(double, this->Src_Y) /
    ...
    const unsigned long f_size = OFstatic_cast(unsigned long, Rows) *
                                 OFstatic_cast(unsigned long, Columns);
    const unsigned long l_offset =
        OFstatic_cast(unsigned long, this->Src_Y - 1) *
        OFstatic_cast(unsigned long, this->Dest_X);
    Uint16 x;
    Uint16 y;
    T* pD;
    T* pCurrTemp;
    ...
    T* pTemp = new T[OFstatic_cast(unsigned long, this->Src_Y) *
                     OFstatic_cast(unsigned long, this->Dest_X)];
    if (pTemp == NULL) {
      DCMIMGLE_ERROR(
       ...
       *   (adapted to be used with signed pixel representation, inverse images
       * - mono1, various bit depths, multi-frame multi-plane/color images,
       * combined clipping/scaling)
       */
        ...
        for (unsigned long f = this->Frames; f != 0; --f) {
          ...
          for (x = 1; x < this->Dest_X - 1; ++x) {
            ...
            for (y = 0; y < this->Src_Y; ++y) {
              *(pCurrTemp) = OFstatic_cast(
                  T, *(pCurrSrc) + (*(pCurrSrc + 1) - *(pCurrSrc)) * dOff);
              pCurrSrc += Columns;
              pCurrTemp += this->Dest_X;
            }
            // don't go beyond the source data
            if ((nSrcIndex < this->Src_X - 2) &&
                (x * x_factor >= nSrcIndex + 1)) {
              pS++;
              nSrcIndex++;
            }
            pCurrSrc = pS;
          ...
          // now the columns are interpolated in temp buffer, so interpolate the
          // lines
          pT = pCurrTemp = pTemp;
          ...
          for (y = 1; y < this->Dest_Y - 1; ++y) {
            ...
            for (x = this->Dest_X; x != 0; --x) {
              *(pD++) = OFstatic_cast(
                  T, *(pCurrTemp) +
                         (*(pCurrTemp + this->Dest_X) - *(pCurrTemp)) * dOff);
              pCurrTemp++;
            }
            // don't go beyond the source data
            if ((nSrcIndex < this->Src_Y - 2) &&
                (y * y_factor >= nSrcIndex + 1)) {
              pT += this->Dest_X;
              nSrcIndex++;
            }
            pCurrTemp = pT;
  ...
  void bicubicPixel(const T* src[], T* dest[]) {
    ...
    const double minVal =
        (isSigned())
            ? -OFstatic_cast(double, DicomImageClass::maxval(this->Bits - 1, 0))
            : 0.0;
    const double maxVal =
        OFstatic_cast(double, DicomImageClass::maxval(this->Bits - isSigned()));
    const double x_factor = OFstatic_cast(double, this->Src_X) /
                            OFstatic_cast(double, this->Dest_X);
    const double y_factor = OFstatic_cast(double, this->Src_Y) /
                            OFstatic_cast(double, this->Dest_Y);
    ...
    // buffer used for storing temporarily the interpolated lines
    T* pTemp = pT = pCurrTemp =
        new T[OFstatic_cast(unsigned long, this->Src_Y) *
              OFstatic_cast(unsigned long, this->Dest_X)];
    if (pTemp == NULL) {
      DCMIMGLE_ERROR(
          "can't allocate temporary buffer for interpolation scaling");
      ...
      for (int j = 0; j < this->Planes; ++j) {
        ...
        for (unsigned long f = this->Frames; f != 0; --f) {
          ...
          for (x = xDelta + 1; x < this->Dest_X - 2 * xDelta; ++x) {
            ...
            for (y = this->Src_Y; y != 0; --y) {
              *(pCurrTemp) = OFstatic_cast(
                  T, cubicValue(*(pCurrSrc - 1), *(pCurrSrc), *(pCurrSrc + 1),
                                *(pCurrSrc + 2), dOff, minVal, maxVal));
              pCurrSrc += Columns;
              pCurrTemp += this->Dest_X;
            }
            // don't go beyond the source data
            if ((nSrcIndex < this->Src_X - 3) &&
                (x * x_factor >= nSrcIndex + 1)) {
              pS++;
              nSrcIndex++;
            }
          ...
          for (x = this->Dest_X; x != 0; --x)
            *(pD++) = *(pCurrTemp++);
          // for the next few lines, linear interpolation between line 0 and 1
          // of the temp buffer
          ...
          for (y = yDelta + 1; y < this->Dest_Y - yDelta - 1; ++y) {
            ...
            for (x = this->Dest_X; x != 0; --x) {
              *(pD++) = OFstatic_cast(
                  T, cubicValue(*(pCurrTemp - this->Dest_X), *(pCurrTemp),
                                *(pCurrTemp + this->Dest_X),
                                *(pCurrTemp + this->Dest_X + this->Dest_X),
                                dOff, minVal, maxVal));
              pCurrTemp++;
            }
            // don't go beyond the source data
            if ((nSrcIndex < this->Src_Y - 3) &&
                (y * y_factor >= nSrcIndex + 1)) {
              pT += this->Dest_X;
              nSrcIndex++;
            }
          ...
          // the last few lines except the very last one, linear interpolation
          // in between the second last and the last lines
          pCurrTemp = pT =
              pTemp + OFstatic_cast(unsigned long, this->Src_Y - 2) *
                          OFstatic_cast(unsigned long, this->Dest_X);
