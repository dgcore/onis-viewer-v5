...
class DiMonoOutputPixelTemplate : public DiMonoOutputPixel,
                                  public DiPixelRepresentationTemplate<T3> {
public:
  /** constructor
   *
   ** @param  buffer    storage area for the output pixel data (optional, maybe
   *NULL)
   ...
   * (#)param frames    total number of frames present in intermediate
   *representation
   *  @param  pastel    flag indicating whether to use not only 'real' grayscale
   *values (optional, experimental)
   */
  DiMonoOutputPixelTemplate(void* buffer, const DiMonoPixel* pixel,
                            DiOverlay* overlays[2], const DiLookupTable* vlut,
                            const DiLookupTable* plut, DiDisplayFunction* disp,
                            const EF_VoiLutFunction vfunc, const double center,
                            const double width, const Uint32 low,
                            const Uint32 high, const Uint16 columns,
                            const Uint16 rows, const unsigned long frame,
#ifdef PASTEL_COLOR_OUTPUT
      ...
      : DiMonoOutputPixel(
            ...
            frame,
            OFstatic_cast(unsigned long,
                          fabs(OFstatic_cast(double, high - low)))),
        Data(NULL),
    ...
    if ((pixel != NULL) && (Count > 0) && (FrameSize >= Count)) {
      if (pastel)
        ...
        DCMIMGLE_TRACE("monochrome output image - columns: "
                       << columns << ", rows: " << rows
                       << ", frame: " << frame);
        DCMIMGLE_TRACE("monochrome output values - low: "
                       << OFstatic_cast(unsigned long, low)
                       << ", high: " << OFstatic_cast(unsigned long, high)
                       << ((low > high) ? " (inverted)" : ""));
        Data = OFstatic_cast(T3*, buffer);
        if ((vlut != NULL) && (vlut->isValid()))  // valid VOI LUT ?
          voilut(pixel, frame * FrameSize, vlut, plut, disp,
                 OFstatic_cast(T3, low), OFstatic_cast(T3, high));
        else {
          if (width < 1)  // no valid window according to supplement 33
            nowindow(pixel, frame * FrameSize, plut, disp,
                     OFstatic_cast(T3, low), OFstatic_cast(T3, high));
          else if (vfunc == EFV_Sigmoid)
            sigmoid(pixel, frame * FrameSize, plut, disp, center, width,
                    OFstatic_cast(T3, low), OFstatic_cast(T3, high));
          else  // linear
            window(pixel, frame * FrameSize, plut, disp, center, width,
                   OFstatic_cast(T3, low), OFstatic_cast(T3, high));
        }
        overlay(overlays, disp, columns, rows,
                frame);  // add (visible) overlay planes to output bitmap
      }
    }
  }
  ...
  inline const void* getData() const {
    return (ColorData != NULL) ? ColorData->getData()
                               : OFstatic_cast(const void*, Data);
  }
  ...
  virtual void* getDataPtr() {
    return (ColorData != NULL) ? ColorData->getDataPtr()
                               : OFstatic_cast(void*, Data);
  }
  ...
  inline int writePPM(STD_NAMESPACE ostream& stream) const {
    if (Data != NULL) {
      ...
      for (i = 0; i < FrameSize; ++i)
        stream << OFstatic_cast(unsigned long, Data[i])
               << " ";  // typecast to resolve problems with 'char'
      return 1;
    }
    if (ColorData != NULL)
...
protected:
  ...
  inline void determineUsedValues() {
    if ((UsedValues == NULL) && (MaxValue > 0) &&
        (MaxValue < MAX_TABLE_ENTRY_COUNT)) {
      UsedValues = new Uint8[MaxValue + 1];
      if (UsedValues != NULL) {
        OFBitmanipTemplate<Uint8>::zeroMem(UsedValues,
                                           MaxValue + 1);  // initialize array
        const T3* p = Data;
        Uint8* q = UsedValues;
        unsigned long i;
        for (i = Count; i != 0; --i)
...
private:
   ...
   *
   ** @param  dlut  reference to storage area where the display LUT should be
   *stored
  ...
  inline void createDisplayLUT(const DiDisplayLUT*& dlut,
                               DiDisplayFunction* disp, const int bits) {
    if ((disp != NULL) && (disp->isValid())) {  // create Display LUT
   ...
   *
   ** @param  lut   reference to storage area where the optimization LUT should
   *be stored
   *  @param  ocnt  number of entries for the optimization LUT
   */
  inline int initOptimizationLUT(T3*& lut, const unsigned long ocnt) {
    int result = 0;
  ...
  void color(void* buffer,  // create true color pastel image
             const DiMonoPixel* inter, const unsigned long frame,
             const unsigned long frames) {
    ColorData = new DiMonoColorOutputPixelTemplate<T1, T3>(buffer, inter, frame,
                                                           frames);
    if (ColorData != NULL)
   ...
   *  @param  high   highest pixel value for the output data (e.g. 255)
   */
  void voilut(const DiMonoPixel* inter, const Uint32 start,
              const DiLookupTable* vlut, const DiLookupTable* plut,
              DiDisplayFunction* disp, const T3 low, const T3 high) {
    const T1* pixel = OFstatic_cast(const T1*, inter->getData());
    if ((pixel != NULL) && (vlut != NULL)) {
      if (Data == NULL)
        Data = new T3[FrameSize];
      if (Data != NULL) {
        DCMIMGLE_DEBUG("applying VOI transformation with LUT ("
                       << vlut->getCount() << " entries)");
        const DiDisplayLUT* dlut = NULL;
        const double minvalue = vlut->getMinValue();
        const double outrange =
            OFstatic_cast(double, high) - OFstatic_cast(double, low) + 1;
        unsigned long i;
        if (minvalue == vlut->getMaxValue())  // LUT has only one entry or all
                                              // entries are equal
        {
          T3 value;
          if ((plut != NULL) && (plut->isValid()))  // has presentation LUT
          {
            ...
            const Uint32 value2 = OFstatic_cast(
                Uint32,
                (minvalue / OFstatic_cast(double, vlut->getAbsMaxRange())) *
                    plut->getCount());
            if (dlut != NULL)  // perform display transformation
            {
              ...
              if (low > high)  // invers
                value = OFstatic_cast(
                    T3, dlut->getValue(OFstatic_cast(
                            Uint16, plut->getAbsMaxRange() -
                                        plut->getValue(value2) - 1)));
              else  // normal
                value = OFstatic_cast(T3, dlut->getValue(OFstatic_cast(
                                              Uint16, plut->getValue(value2))));
            } else {  // don't use display: invalid or absent
              DCMIMGLE_TRACE("monochrome rendering: VOI LUT #2");
              value = OFstatic_cast(
                  T3, OFstatic_cast(double, low) +
                          OFstatic_cast(double, plut->getValue(value2)) *
                              outrange /
                              OFstatic_cast(double, plut->getAbsMaxRange()));
            }
            ...
            if (dlut != NULL)  // perform display transformation
            {
              ...
              if (low > high)  // invers
                value = OFstatic_cast(
                    T3, dlut->getValue(OFstatic_cast(
                            Uint16, vlut->getAbsMaxRange() - minvalue - 1)));
              else  // normal
                value = OFstatic_cast(
                    T3, dlut->getValue(OFstatic_cast(Uint16, minvalue)));
            } else {  // don't use display: invalid or absent
              DCMIMGLE_TRACE("monochrome rendering: VOI LUT #4");
              value = OFstatic_cast(
                  T3, OFstatic_cast(double, low) +
                          (minvalue /
                           OFstatic_cast(double, vlut->getAbsMaxRange())) *
                              outrange);
            }
          }
          OFBitmanipTemplate<T3>::setMem(
              Data, value, Count);  // set output pixels to LUT value
        } else {
          T2 value = 0;
          const T2 absmin = OFstatic_cast(T2, inter->getAbsMinimum());
          const T2 firstentry =
              vlut->getFirstEntry(value);  // choose signed/unsigned method
          const T2 lastentry = vlut->getLastEntry(value);
          const unsigned long ocnt = OFstatic_cast(
              unsigned long, inter->getAbsMaxRange());  // number of LUT entries
          ...
          if ((plut != NULL) && (plut->isValid()))  // has presentation LUT
          {
            ...
            const double gradient1 =
                OFstatic_cast(double, pcnt) /
                OFstatic_cast(double, vlut->getAbsMaxRange());
            const Uint32 firstvalue = OFstatic_cast(
                Uint32,
                OFstatic_cast(double, vlut->getFirstValue()) * gradient1);
            const Uint32 lastvalue = OFstatic_cast(
                Uint32,
                OFstatic_cast(double, vlut->getLastValue()) * gradient1);
            if (initOptimizationLUT(lut, ocnt)) {  // use LUT for optimization
              q = lut;
              if (dlut != NULL)  // perform display transformation
              {
                ...
                if (low > high)  // inverse
                {
                  const Uint16 maxvalue =
                      OFstatic_cast(Uint16, plut->getAbsMaxRange() - 1);
                  for (i = 0; i < ocnt; ++i) {
                    ...
                    if (value <= firstentry)
                      ...
                      value2 = OFstatic_cast(
                          Uint32, OFstatic_cast(double, vlut->getValue(value)) *
                                      gradient1);
                    *(q++) = OFstatic_cast(
                        T3, dlut->getValue(OFstatic_cast(
                                Uint16, maxvalue - plut->getValue(value2))));
                  }
                } else {  // normal
                  for (i = 0; i < ocnt; ++i) {
                    value = OFstatic_cast(T2, i) + absmin;
                    if (value <= firstentry)
                      ...
                      value2 = OFstatic_cast(
                          Uint32, OFstatic_cast(double, vlut->getValue(value)) *
                                      gradient1);
                    *(q++) = OFstatic_cast(
                        T3, dlut->getValue(plut->getValue(value2)));
                  }
                }
              } else {  // don't use display: invalid or absent
                DCMIMGLE_TRACE("monochrome rendering: VOI LUT #6");
                const double gradient2 =
                    outrange / OFstatic_cast(double, plut->getAbsMaxRange());
                for (i = 0; i < ocnt; ++i) {
                  value = OFstatic_cast(T2, i) + absmin;
                  if (value <= firstentry)
                    value2 = firstvalue;
              ...
              const T3* lut0 =
                  lut -
                  OFstatic_cast(
                      T2, inter->getAbsMinimum());  // points to 'zero' entry
              q = Data;
            ...
            if (lut == NULL)  // use "normal" transformation
            {
              if (dlut != NULL)  // perform display transformation
              {
                ...
                {
                  const Uint16 maxvalue =
                      OFstatic_cast(Uint16, vlut->getAbsMaxRange() - 1);
                  for (i = Count; i != 0; --i) {
            ...
            const double gradient =
                outrange / OFstatic_cast(double, vlut->getAbsMaxRange());
            const T3 firstvalue = OFstatic_cast(
                T3,
                OFstatic_cast(double, low) +
                    OFstatic_cast(double, vlut->getFirstValue()) * gradient);
            const T3 lastvalue = OFstatic_cast(
                T3, OFstatic_cast(double, low) +
                        OFstatic_cast(double, vlut->getLastValue()) * gradient);
            if (initOptimizationLUT(lut, ocnt)) {  // use LUT for optimization
              q = lut;
              if (dlut != NULL)  // perform display transformation
              {
                ...
                if (low > high)  // inverse
                {
                  ...
                  for (i = 0; i < ocnt; ++i) {
                    ...
                    *(q++) = OFstatic_cast(
                        T3, dlut->getValue(OFstatic_cast(
                                Uint16, maxvalue - vlut->getValue(value))));
                  }
                } else {  // normal
                  for (i = 0; i < ocnt; ++i) {
                    value = OFstatic_cast(T2, i) + absmin;
                    if (value < firstentry)
                      ...
                      value = lastentry;
                    *(q++) = OFstatic_cast(
                        T3, dlut->getValue(vlut->getValue(value)));
                  }
                }
              } else {  // don't use display: invalid or absent
                ...
                for (i = 0; i < ocnt; ++i)  // calculating LUT entries
                {
                  ...
                  if (value <= firstentry)
                    ...
                    *(q++) = OFstatic_cast(
                        T3, OFstatic_cast(double, low) +
                                OFstatic_cast(double, vlut->getValue(value)) *
                                    gradient);
                }
              }
              const T3* lut0 =
        ...
        if (Count < FrameSize)
          OFBitmanipTemplate<T3>::zeroMem(
              Data + Count,
              FrameSize - Count);  // set remaining pixels of frame to zero
      }
    } else
      Data = NULL;
  }
   ...
   *  @param  high   highest pixel value for the output data (e.g. 255)
   */
  void nowindow(const DiMonoPixel* inter, const Uint32 start,
                const DiLookupTable* plut, DiDisplayFunction* disp,
                const T3 low, const T3 high) {
    const DiDisplayLUT* dlut = NULL;
    const T1* pixel = OFstatic_cast(const T1*, inter->getData());
    if (pixel != NULL) {
      ...
      if (Data != NULL) {
        ...
        if ((plut != NULL) && (plut->isValid()))  // has presentation LUT
        {
          ...
          const double gradient1 =
              OFstatic_cast(double, plut->getCount()) / inter->getAbsMaxRange();
          const double gradient2 =
              outrange / OFstatic_cast(double, plut->getAbsMaxRange());
          if (initOptimizationLUT(lut, ocnt)) {  // use LUT for optimization
            q = lut;
            if (dlut != NULL)  // perform display transformation
            {
              ...
              if (low > high)  // inverse
              {
                const Uint16 maxvalue =
                    OFstatic_cast(Uint16, plut->getAbsMaxRange() - 1);
                for (i = 0; i < ocnt; ++i) {
                  value = OFstatic_cast(Uint32,
                                        OFstatic_cast(double, i) * gradient1);
                  *(q++) = OFstatic_cast(
                      T3, dlut->getValue(OFstatic_cast(
                              Uint16, maxvalue - plut->getValue(value))));
                }
              } else {  // normal
                for (i = 0; i < ocnt; ++i) {
                  value = OFstatic_cast(Uint32,
                                        OFstatic_cast(double, i) * gradient1);
                  *(q++) =
                      OFstatic_cast(T3, dlut->getValue(plut->getValue(value)));
                }
              }
            } else {  // don't use display: invalid or absent
              ...
              for (i = 0; i < ocnt; ++i) {
                value =
                    OFstatic_cast(Uint32, OFstatic_cast(double, i) * gradient1);
                *(q++) = OFstatic_cast(
                    T3, OFstatic_cast(double, low) +
                            OFstatic_cast(double, plut->getValue(value)) *
                                gradient2);
              }
            }
            const T3* lut0 =
            ...
            if (dlut != NULL)  // perform display transformation
            {
              ...
              if (low > high)  // inverse
              {
                ...
                for (i = Count; i != 0; --i) {
                  value = OFstatic_cast(
                      Uint32,
                      (OFstatic_cast(double, *(p++)) - absmin) * gradient1);
                  *(q++) = OFstatic_cast(
                      T3, dlut->getValue(OFstatic_cast(
                              Uint16, maxvalue - plut->getValue(value))));
                }
            ...
            if (dlut != NULL)  // perform display transformation
            {
              ...
              if (low > high)  // inverse
              {
                for (i = ocnt; i != 0; --i)  // calculating LUT entries
                  *(q++) = OFstatic_cast(
                      T3, dlut->getValue(OFstatic_cast(Uint16, i - 1)));
              } else {                      // normal
                for (i = 0; i < ocnt; ++i)  // calculating LUT entries
                  *(q++) = OFstatic_cast(
                      T3, dlut->getValue(OFstatic_cast(Uint16, i)));
              }
            } else {  // don't use display: invalid or absent
              DCMIMGLE_TRACE("monochrome rendering: VOI NONE #6");
              for (i = 0; i < ocnt; ++i)  // calculating LUT entries
                *(q++) =
                    OFstatic_cast(T3, OFstatic_cast(double, low) +
                                          OFstatic_cast(double, i) * gradient);
            }
            const T3* lut0 =
                lut -
                OFstatic_cast(
            ...
            if (dlut != NULL)  // perform display transformation
            {
              ...
              if (low > high)  // inverse
              {
                for (i = Count; i != 0; --i)
                  *(q++) = OFstatic_cast(
                      T3, dlut->getValue(OFstatic_cast(
                              Uint16, absmax - (OFstatic_cast(double, *(p++)) -
                                                absmin))));
              } else {  // normal
                for (i = Count; i != 0; --i)
                  *(q++) = OFstatic_cast(
                      T3, dlut->getValue(OFstatic_cast(
                              Uint16, OFstatic_cast(double, *(p++)) - absmin)));
              }
            } else {  // don't use display: invalid or absent
              ...
              for (i = Count; i != 0; --i)
                *(q++) = OFstatic_cast(
                    T3,
                    OFstatic_cast(double, low) +
                        (OFstatic_cast(double, *(p++)) - absmin) * gradient);
            }
          }
        }
        delete[] lut;
   ...
   *  @param  high    highest pixel value for the output data (e.g. 255)
   */
  void sigmoid(const DiMonoPixel* inter, const Uint32 start,
               const DiLookupTable* plut, DiDisplayFunction* disp,
               const double center, const double width, const T3 low,
               const T3 high) {
    const T1* pixel = OFstatic_cast(const T1*, inter->getData());
    if (pixel != NULL) {
      if (Data == NULL)
      ...
      if (Data != NULL) {
        DCMIMGLE_DEBUG(
            "applying sigmoid VOI transformation with window center = "
            << center << ", width = " << width);
        const DiDisplayLUT* dlut = NULL;
        const double absmin = inter->getAbsMinimum();
        const double outrange = OFstatic_cast(double, high) -
                                OFstatic_cast(double, low);  // output range
        const unsigned long ocnt = OFstatic_cast(
            unsigned long, inter->getAbsMaxRange());  // number of LUT entries
        const T1* p = pixel + start;
        T3* q = Data;
        ...
        if ((plut != NULL) && (plut->isValid()))  // has presentation LUT
        {
          ...
          const double plutmax_1 =
              OFstatic_cast(double, plut->getAbsMaxRange() - 1);
          if (initOptimizationLUT(lut, ocnt)) {  // use LUT for optimization
            q = lut;
            if (dlut != NULL)  // perform display transformation
            {
              ...
              const double maxvalue =
                  OFstatic_cast(double, dlut->getCount() - 1);
              const double offset = (low > high) ? maxvalue : 0;
              const double gradient = (low > high) ? (-maxvalue / plutmax_1)
                                                   : (maxvalue / plutmax_1);
              for (i = 0; i < ocnt; ++i) {
                value = OFstatic_cast(double, i) + absmin;
                value2 = OFstatic_cast(
                    Uint32,
                    plutcnt_1 / (1 + exp(-4 * (value - center) / width)));
                *(q++) = OFstatic_cast(
                    T3,
                    dlut->getValue(OFstatic_cast(
                        Uint16,
                        offset + OFstatic_cast(double, plut->getValue(value2)) *
                                     gradient)));
              }
            } else {  // don't use display: invalid or absent
            ...
            const T3* lut0 =
                lut - OFstatic_cast(T2, absmin);  // points to 'zero' entry
            q = Data;
            for (i = Count; i != 0; --i)  // apply LUT
              *(q++) = *(lut0 + (*(p++)));
          }
            ...
            if (dlut != NULL)  // perform display transformation
            {
              ...
              for (i = 0; i < ocnt; ++i)  // calculating LUT entries
              {
                ...
                *(q++) = OFstatic_cast(
                    ...
                    dlut->getValue(OFstatic_cast(
                        Uint16,
                        offset + outrange2 / (1 + exp(-4 * (value - center) /
                                                      width)))));
              }
            } else {  // don't use display: invalid or absent
              ...
              for (i = 0; i < ocnt; ++i)  // calculating LUT entries
              {
                value = OFstatic_cast(double, i) + absmin;
                *(q++) = OFstatic_cast(
                    T3, outrange / (1 + exp(-4 * (value - center) / width)));
              }
            }
            const T3* lut0 =
                lut - OFstatic_cast(T2, absmin);  // points to 'zero' entry
   ...
   *  @param  high    highest pixel value for the output data (e.g. 255)
   */
  void window(const DiMonoPixel* inter, const Uint32 start,
              const DiLookupTable* plut, DiDisplayFunction* disp,
              const double center, const double width, const T3 low,
              const T3 high) {
    const T1* pixel = OFstatic_cast(const T1*, inter->getData());
    if (pixel != NULL) {
      ...
      if (Data != NULL) {
        DCMIMGLE_DEBUG(
            "applying linear VOI transformation with window center = "
            << center << ", width = " << width);
        const DiDisplayLUT* dlut = NULL;
        const double absmin = inter->getAbsMinimum();
        ...
        const double leftBorder =
            center - 0.5 -
            width_1 / 2;  // window borders, according to supplement 33
        ...
        if ((plut != NULL) && (plut->isValid()))  // has presentation LUT
        {
          ...
          const double plutmax_1 =
              OFstatic_cast(double, plut->getAbsMaxRange()) - 1;
          const double gradient1 =
              (width_1 == 0) ? 0 : OFstatic_cast(double, pcnt - 1) / width_1;
          if (initOptimizationLUT(lut, ocnt)) {  // use LUT for optimization
            q = lut;
            if (dlut != NULL)  // perform display transformation
            {
              ...
              const double maxvalue =
                  OFstatic_cast(double, dlut->getCount() - 1);
              const double offset = (low > high) ? maxvalue : 0;
              const double gradient2 = (low > high) ? (-maxvalue / plutmax_1)
                                                    : (maxvalue / plutmax_1);
              for (i = 0; i < ocnt; ++i) {
                ...
                if (value <= leftBorder)
                  ...
                  value2 =
                      OFstatic_cast(Uint32, (value - leftBorder) * gradient1);
                *(q++) = OFstatic_cast(
                    T3,
                    dlut->getValue(OFstatic_cast(
                        Uint16,
            ...
            if (dlut != NULL)  // perform display transformation
            {
              ...
              for (i = 0; i < ocnt; ++i)  // calculating LUT entries
              {
                ...
                *(q++) = OFstatic_cast(
                    T3, dlut->getValue(OFstatic_cast(
                            Uint16,
                            offset + value * gradient)));  // calculate value
              }
            } else {  // don't use display: invalid or absent
              ...
              for (i = 0; i < ocnt; ++i)  // calculating LUT entries
              {
                ...
                if (value <= leftBorder)
                  ...
                  *(q++) = OFstatic_cast(
                      T3, offset + value * gradient);  // gray value
              }
            }
            const T3* lut0 =
                lut - OFstatic_cast(T2, absmin);  // points to 'zero' entry
   ...
   */
  void overlay(DiOverlay* overlays[2], DiDisplayFunction* disp,
      ...
      for (unsigned int j = 0; j < 2; ++j) {
        if (overlays[j] != NULL) {
          if (overlays[j]->getCount() > 0)
            DCMIMGLE_DEBUG("applying " << ((j == 0) ? "built-in" : "additional")
                                       << " overlay planes");
          ...
          for (unsigned int i = 0; i < overlays[j]->getCount(); ++i) {
            ...
            if ((plane != NULL) && plane->isVisible() && plane->reset(frame)) {
              ...
              Uint16 y;
              const Uint16 xmin =
                  (plane->getLeft(left_pos) > 0) ? plane->getLeft(left_pos) : 0;
              const Uint16 ymin =
                  (plane->getTop(top_pos) > 0) ? plane->getTop(top_pos) : 0;
              const Uint16 xmax = (plane->getRight(left_pos) < columns)
                                      ? plane->getRight(left_pos)
                                      : columns;
              const Uint16 ymax = (plane->getBottom(top_pos) < rows)
                                      ? plane->getBottom(top_pos)
                                      : rows;
              const T3 maxvalue =
                  OFstatic_cast(T3, DicomImageClass::maxval(bitsof(T3)));
              switch (plane->getMode()) {
                case EMO_Replace: {
                  DCMIMGLE_DEBUG("applying overlay plane "
                                 << (i + 1) << " with 'replace' mode");
                  const T3 fore =
                      OFstatic_cast(T3, plane->getForeground() * maxvalue);
                  for (y = ymin; y < ymax; ++y) {
                    plane->setStart(OFstatic_cast(Uint16, left_pos + xmin),
                                    OFstatic_cast(Uint16, top_pos + y));
                    q = Data +
                        OFstatic_cast(unsigned long, y) *
                            OFstatic_cast(unsigned long, columns) +
                        OFstatic_cast(unsigned long, xmin);
                ...
                case EMO_ThresholdReplace: {
                  ...
                  const T3 fore =
                      OFstatic_cast(T3, plane->getForeground() * maxvalue);
                  const T3 thresh =
                      OFstatic_cast(T3, plane->getThreshold() * maxvalue);
                  for (y = ymin; y < ymax; ++y) {
                    plane->setStart(OFstatic_cast(Uint16, left_pos + xmin),
                                    OFstatic_cast(Uint16, top_pos + y));
                    q = Data +
                ...
                case EMO_Complement: {
                  DCMIMGLE_DEBUG("applying overlay plane "
                                 << (i + 1) << " with 'complement' mode");
                  const T3 thresh = OFstatic_cast(
                      T3, DicomImageClass::maxval(bitsof(T3) / 2));
                  for (y = ymin; y < ymax; ++y) {
                    plane->setStart(OFstatic_cast(Uint16, left_pos + xmin),
                                    OFstatic_cast(Uint16, top_pos + y));
                    q = Data +
                ...
                case EMO_BitmapShutter: {
                  DCMIMGLE_DEBUG("applying overlay plane "
                                 << (i + 1) << " with 'bitmap shutter' mode");
                  T3 fore = OFstatic_cast(
                      T3, OFstatic_cast(double, maxvalue) *
                              OFstatic_cast(double, plane->getPValue()) /
                              OFstatic_cast(double, DicomImageClass::maxval(
                                                        WIDTH_OF_PVALUES)));
                  if ((disp != NULL) && (disp->isValid())) {
                    const DiDisplayLUT* dlut =
                        disp->getLookupTable(WIDTH_OF_PVALUES);
                    if ((dlut != NULL) && (dlut->isValid()))
                      fore =
                          OFstatic_cast(T3, dlut->getValue(plane->getPValue()));
                  }
                  for (y = ymin; y < ymax; ++y) {
                    plane->setStart(OFstatic_cast(Uint16, left_pos + xmin),
                                    OFstatic_cast(Uint16, top_pos + y));
                ...
                default: /* e.g. EMO_Default */
                  DCMIMGLE_WARN("unhandled overlay mode ("
                                << OFstatic_cast(int, plane->getMode()) << ")");
              }
            }
          }
        }
  ...
  T3* Data;
  /// flag indicating whether the output data buffer should be deleted in the
  /// destructor
  int DeleteData;
