...
protected:
   ...
   */
  DiTransTemplate(const int planes, const Uint16 src_x, const Uint16 src_y,
      ...
      : Planes(planes),
        ...
        Dest_Y(dest_y),
        Frames(frames),
        Bits(((bits < 1) || (bits > OFstatic_cast(int, bitsof(T))))
                 ? OFstatic_cast(int, bitsof(T))
                 : bits) {}
  ...
  inline void copyPixel(const T* src[], T* dest[]) {
    const unsigned long count = OFstatic_cast(unsigned long, Dest_X) *
                                OFstatic_cast(unsigned long, Dest_Y) * Frames;
   ...
   */
  inline void fillPixel(T* dest[], const T value) {
    const unsigned long count = OFstatic_cast(unsigned long, Dest_X) *
                                OFstatic_cast(unsigned long, Dest_Y) * Frames;
    for (int j = 0; j < Planes; ++j)
  ...
  /*const*/ Uint16 Src_X;  // add 'const' when interpolated scaling with
                           // clipping is fully implemented
