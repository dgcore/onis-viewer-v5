 ...
 */
template <class T>
class DiFlipTemplate : public DiTransTemplate<T> {
public:
  /** constructor.
   *  This method is used to flip an image and store the result in the same
   *storage area.
   *
   ...
   */
  DiFlipTemplate(DiPixel* pixel, const Uint16 columns, const Uint16 rows,
                 const Uint32 frames, const int horz, const int vert)
      : DiTransTemplate<T>(0, columns, rows, columns, rows, frames) {
    if (pixel != NULL) {
      this->Planes = pixel->getPlanes();
      if ((pixel->getCount() > 0) && (this->Planes > 0) &&
          (pixel->getCount() == OFstatic_cast(unsigned long, columns) *
  ...
  /** constructor.
   *  This method is used to perform only the preparation and to start flipping
   *later with method 'flipData()'
   *
   ...
   *  @param  frames   number of frames
   */
  DiFlipTemplate(const int planes, const Uint16 columns, const Uint16 rows,
                 const Uint32 frames)
   ...
   *  @param  vert  flags indicating whether to flip vertically or not
   */
  inline void flipData(const T* src[], T* dest[], const int horz,
                       const int vert) {
...
protected:
   ...
   *  @param  dest  array of pointers to destination image pixels
   */
  inline void flipHorz(const T* src[], T* dest[]) {
    if ((src != NULL) && (dest != NULL)) {
      Uint16 x;
      Uint16 y;
      const T* p;
   ...
   *  @param  dest  array of pointers to destination image pixels
   */
  inline void flipVert(const T* src[], T* dest[]) {
    if ((src != NULL) && (dest != NULL)) {
      Uint16 x;
      Uint16 y;
      const T* p;
  ...
  /** flip source image horizontally and vertically and store result in
   *destination image
   *
   ** @param  src   array of pointers to source image pixels
   *  @param  dest  array of pointers to destination image pixels
   */
  inline void flipHorzVert(const T* src[], T* dest[]) {
    if ((src != NULL) && (dest != NULL)) {
      unsigned long i;
      const T* p;
      T* q;
...
private:
  ...
  /** flip image horizontally and vertically and store result in the same
   *storage area
   *
   ** @param  data  array of pointers to source/destination image pixels
   */
