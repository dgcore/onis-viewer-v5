...
public:
   ...
   *  @param  explanation  element containing the modality LUT explanation
   *(optional)
   */
  DiMonoImage(const DiDocument* docu, const EI_Status status,
   ...
   *problems with different number ranges, limited to monochrome images
   *
  ...
  inline int deleteDisplayLUT(const int bits) {
    return (DisplayFunction != NULL) ? DisplayFunction->deleteLookupTable(bits)
                                     : 0;
  }
   ...
   *Interest (ROI). The ROI is specified by means of a rectangle (left_pos,
   *top_pos, width, height). Possibly active VOI LUT is implicitly disabled.
   *
   ...
   *          false otherwise
   */
  int setRoiWindow(const unsigned long left_pos, const unsigned long top_pos,
   ...
   *stored in image file). possibly active VOI LUT is implicitly disabled.
   *
  ...
  inline const char* getModalityLutExplanation() const {
    return (InterData != NULL) ? InterData->getModalityLutExplanation()
   ...
   */
  int setHardcopyParameters(const unsigned int min, const unsigned int max,
   ...
   *                 'default' means that the output data is always created with
   *0 for black and maxvalue for white (i.e. monochrome2 data is created for
   *output).
   *
  ...
  inline const char* getPresentationLutExplanation() const {
    return (PresLutData != NULL) ? PresLutData->getExplanation()
                                 : OFstatic_cast(const char*, NULL);
  }
   ...
   */
  int addOverlay(const unsigned int group, const signed int left_pos,
                 const signed int top_pos, const unsigned int columns,
                 const unsigned int rows, const DcmOverlayData& data,
                 const DcmLongString& label, const DcmLongString& description,
   ...
   *  If the optional parameter is specified the value will be checked and in
   *any case a valid value will be returned.
   *
   ** @param  bits  value to be returned (if less than 1 or greater than the
   *maximum (32) the default value will be used which is equal to the bits per
   *sample value stored in the DICOM dataset)
   *
   ** @return status, true if successful, false otherwise
   */
  virtual int getBits(const int bits = 0) const {
    return (((bits < 1) || (bits > MAX_BITS)) && (bits != MI_PastelColor))
               ? BitsPerSample
               : bits;
  }
   ...
   */
  virtual const void* getOutputData(const unsigned long frame, const int bits,
   ...
   ** @return status, true if successful, false otherwise
   */
  virtual int getOutputData(void* buffer, const unsigned long size,
   ...
   *  @param  idx       index of overlay group (0 = dataset, 1 = additional, 2 =
   *'1' plane hides '0' plane)
  ...
  const void* getOverlayData(const unsigned long frame,
                             const unsigned int plane, unsigned int& left_pos,
                             unsigned int& top_pos, unsigned int& width,
                             unsigned int& height, EM_Overlay& mode,
                             const unsigned int idx, const int bits = 8,
  ...
  const void* getFullOverlayData(const unsigned long frame,
                                 const unsigned int plane, unsigned int& width,
                                 unsigned int& height, const unsigned int idx,
   ...
   *is 0, data is 16 bit padded - even length) memory is not handled internally
   *- must be deleted from calling program.
   *
   ...
   *  @param  width   returns width of overlay plane (in pixels)
   *  @param  height  returns height of overlay plane (in pixels)
   *  @param  frames  returns number of frames (multiple overlay frames
   *possible!)
   ...
   *otherwise
   */
  unsigned long create6xxx3000OverlayData(
      Uint8*& buffer, const unsigned int plane, unsigned int& width,
      unsigned int& height, unsigned long& frames, const unsigned int idx = 0);
   ...
   *(8, 24 or 32)
   *  @param  upsideDown  specifies the order of lines in the images (0 =
   *top-down, bottom-up otherwise)
   ...
   */
  unsigned long createDIB(void*& data, const unsigned long size,
                          const unsigned long frame, const int bits,
                          const int upsideDown, const int padding = 1);

  /** create true color (32 bit) or palette (8 bit) bitmap for Java (AWT default
   *format). Memory is not handled internally - must be deleted from calling
   *program.
   *
   ...
   *  @param  alloc   number of bits allocated for the pixels stored in the
   *buffer (e.g. 16)
   *  @param  stored  number of bits stored for the pixels stored in the buffer
   *(e.g. 12)
   *
   ** @return pointer to memory buffer containing the packed output bitmap data
   *(NULL if an error occurred)
   */
  static void* createPackedBitmap(const void* buffer, const unsigned long size,
                                  const unsigned long count, const int alloc,
                                  const int stored);
   ...
   *values, 1 = determine value of BitsStored from 'possible' pixel values
   ...
   ** @return true if successful, false otherwise
   */
  int writePPM(STD_NAMESPACE ostream& stream, const unsigned long frame,
               const int bits);
...
protected:
   ...
   */
  DiMonoImage(const DiDocument* image, const EI_Status status,
              const char dummy);
   ...
   */
  DiMonoImage(const DiMonoImage* image, const unsigned long fstart,
   ...
   ** @return pointer to new DiImage object (NULL if an error occurred)
   */
  DiMonoImage(const DiColorImage* image, const double red, const double green,
              const double blue);
   ...
   *(0..65535)
   */
  DiMonoImage(const DiMonoImage* image, const signed long left_pos,
              const signed long top_pos, const Uint16 src_cols,
              const Uint16 src_rows, const Uint16 dest_cols,
              const Uint16 dest_rows, const int interpolate, const int aspect,
              const Uint16 pvalue);
   ...
   */
  DiMonoImage(const DiMonoImage* image, DiMonoOutputPixel* pixel,
              const unsigned long frame, const int stored, const int alloc);

  /** initialize internal data structures and member variables
   *
   ** @param  modality  pointer to object handling the modality transform
   *  @param  reuse     reuse particular information determined in a previous
   *call
   */
   ...
   *  (memory is handled externally)
   *
   ** @param  buffer    untyped pointer to the externally allocated memory
   *buffer
   *  @param  size      size of the memory buffer in bytes (will be checked)
   *  @param  frame     number of frame to be rendered
   *  @param  bits      number of bits for the output pixel data (depth)
   ...
   ** @return untyped pointer to the pixel data if successful, NULL otherwise
   */
  const void* getData(void* buffer, const unsigned long size,
                      const unsigned long frame, int bits, const int planar,
                      const int negative);
   ...
   */
  void getDataUint8(void* buffer, DiDisplayFunction* disp, const int samples,
                    const unsigned long frame, const int bits, const Uint32 low,
                    const Uint32 high);
   ...
   *  @param  high     output pixel value to which 2^bits-1 is mapped (max)
   */
  void getDataSint8(void* buffer, DiDisplayFunction* disp, const int samples,
                    const unsigned long frame, const int bits, const Uint32 low,
                    const Uint32 high);
   ...
   *  @param  high     output pixel value to which 2^bits-1 is mapped (max)
   */
  void getDataUint16(void* buffer, DiDisplayFunction* disp, const int samples,
                     const unsigned long frame, const int bits,
                     const Uint32 low, const Uint32 high);
   ...
   *  @param  high     output pixel value to which 2^bits-1 is mapped (max)
   */
  void getDataSint16(void* buffer, DiDisplayFunction* disp, const int samples,
                     const unsigned long frame, const int bits,
                     const Uint32 low, const Uint32 high);
   ...
   *  @param  high     output pixel value to which 2^bits-1 is mapped (max)
   */
  void getDataUint32(void* buffer, DiDisplayFunction* disp, const int samples,
                     const unsigned long frame, const int bits,
                     const Uint32 low, const Uint32 high);
   ...
   *  @param  high     output pixel value to which 2^bits-1 is mapped (max)
   */
  void getDataSint32(void* buffer, DiDisplayFunction* disp, const int samples,
                     const unsigned long frame, const int bits,
                     const Uint32 low, const Uint32 high);

  /** create a presentation look-up table converting the pixel data which is
   *linear to Optical Density to DDLs of the softcopy device (used to display
