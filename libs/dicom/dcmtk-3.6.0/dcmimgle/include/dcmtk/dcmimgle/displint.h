...
public:
   ...
   *  @param  y2   array used to store the resulting spline function (used for
   ...
   */
  static int Function(const T1* x, const T2* y, const unsigned int n, T3_* y2,
                      const T3_ yp1 = 1.0e30, const T3_ ypn = 1.0e30) {
      ...
      if (u != NULL) {
        ...
        if (yp1 > 0.99e30)  // ignore value for first derivative at point 1
          ...
          u[0] = (3.0 / (OFstatic_cast(T3_, x[1]) - OFstatic_cast(T3_, x[0]))) *
                 ((OFstatic_cast(T3_, y[1]) - OFstatic_cast(T3_, y[0])) /
                      (OFstatic_cast(T3_, x[1]) - OFstatic_cast(T3_, x[0])) -
                  yp1);
        }
        for (i = 1; i < n - 1; ++i) {
        ...
        if (ypn > 0.99e30)  // ignore value for first derivative at point 1
          ...
          un = (3.0 /
               ...
               (ypn -
                (OFstatic_cast(T3_, y[n - 1]) - OFstatic_cast(T3_, y[n - 2])) /
                    (OFstatic_cast(T3_, x[n - 1]) -
                     OFstatic_cast(T3_, x[n - 2])));
        }
   ...
   *  @param  y2a  array used to store the resulting spline function (calculated
   *by CubicSplineFunction)
   ...
   ** @return true if successful, false otherwise
   */
  static int Interpolation(const T1* xa, const T2* ya, const T3_* y2a,
                           const unsigned int na, const T1* x, T2* y,
                           const unsigned int n) {
    if ((xa != NULL) && (ya != NULL) && (y2a != NULL) && (na > 0) &&
        (x != NULL) && (y != NULL) && (n > 0)) {
      unsigned int k, i;
      unsigned int klo = 0;
      ...
      for (i = 0; i < n; ++i) {
        ...
        if (xa[khi] == x[i])  // optimization: use known values
          ...
          y[i] = OFstatic_cast(T2, a * OFstatic_cast(T3_, ya[klo]) +
                                       b * OFstatic_cast(T3_, ya[khi]) +
                                       ((a * a * a - a) * y2a[klo] +
                                        (b * b * b - b) * y2a[khi]) *
                                           (h * h) / 6.0);
        }
