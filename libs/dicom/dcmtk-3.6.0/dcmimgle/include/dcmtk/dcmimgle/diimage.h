...
public:
   ...
   *  Possible values are: EPI_Monochrome1, EPI_Monochrome2, EPI_RGB and
   *EPI_YBR_Full
   *
   ...
   *  @param  bits    number of bits for the output pixel data (depth)
   *  @param  planar  flag, whether the output data (for multi-planar images)
   *should be planar or not
   *
   ...
   *
   ** (#)param  idx  index of overlay group (here: not used, since only
   *applicable for monochrome images)
   *
   ...
   ** @return pointer to new DiImage object (NULL if an error occurred)
   */
  virtual DiImage* createScale(
      const signed long left_pos, const signed long top_pos,
      const unsigned long clip_width, const unsigned long clip_height,
      const unsigned long scale_width, const unsigned long scale_height,
      const int interpolate, const int aspect, const Uint16 pvalue) const = 0;
   ...
   ** @return pointer to new DiImage object (NULL if an error occurred)
   */
  virtual DiImage* createMono(const double red, const double green,
   ...
   */
  virtual unsigned long createDIB(void*& data, const unsigned long size,
                                  const unsigned long frame, const int bits,
  ...
  /** render pixel data of given frame and write image related attributes to
   *DICOM dataset.
   *
   ...
   ** @return true if successful, false otherwise
   */
  int writeFrameToDataset(DcmItem& dataset, const unsigned long frame = 0,
                          const int bits = 0, const int planar = 0);

  /** write current image and related attributes to DICOM dataset.
   *
   ** @param  dataset  reference to DICOM dataset where the image attributes are
   *stored
   *  @param  mode     determine value of BitsStored from 'used' or 'possible'
   *pixel values
   *  @param  planar   flag, whether the output data (for multi-planar images)
   *should be planar or not
   *
   ** @return true if successful, false otherwise
   */
  virtual int writeImageToDataset(DcmItem& dataset, const int mode = 0,
   ...
   ** @return true if successful, false otherwise
   */
  virtual int writePPM(STD_NAMESPACE ostream& stream, const unsigned long frame,
                       const int bits) = 0;
   ...
   ** @return true if successful, false otherwise
   */
  virtual int writePPM(FILE* stream, const unsigned long frame,
                       const int bits) = 0;
   ...
   ** @return true if successful, false otherwise
   */
  virtual int writeRawPPM(FILE* stream, const unsigned long frame,
                          const int bits) = 0;
...
protected:
   ...
   */
  DiImage(const DiImage* image, const unsigned long fstart,
   ...
   *  @param  aspect  flag indicating whether pixel aspect ratio should be used
   *or not
   */
  DiImage(const DiImage* image, const Uint16 width, const Uint16 height,
          const int aspect = 0);
   ...
   */
  DiImage(const DiImage* image, const unsigned long frame, const int stored,
          const int alloc);
   ...
   *  Removes smallest/largest pixel value and updates pixel aspect ratio as
   *well as pixel spacing (if required). Used in writeXXXToDataset() routines.
   *
