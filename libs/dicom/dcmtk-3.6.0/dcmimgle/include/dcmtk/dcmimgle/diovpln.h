...
public:
   ...
   */
  DiOverlayPlane(const unsigned int group, const Sint16 left_pos,
                 const Sint16 top_pos, const Uint16 columns, const Uint16 rows,
                 const DcmOverlayData& data, const DcmLongString& label,
                 const DcmLongString& description, const EM_Overlay mode);
   ...
   *  @param  rows     height of the overlay plane
   */
  DiOverlayPlane(DiOverlayPlane* plane, const unsigned int bit, Uint16* data,
                 Uint16* temp, const Uint16 width, const Uint16 height,
   ...
   *
   ** @param  left_pos  offset to be subtracted from the actual value
   *(optional).
   *
   ** @return right border of overlay plane origin. Negative values are set to
   *0.
   */
  inline Uint16 getRight(const Sint32 left_pos = 0) const {
    return (OFstatic_cast(Sint32, Left) + OFstatic_cast(Sint32, Width) -
                left_pos >
            0)
               ? OFstatic_cast(Uint16, OFstatic_cast(Sint32, Left) +
                                           OFstatic_cast(Sint32, Width) -
                                           left_pos)
               : 0;
  }
   ...
   ** @param  top_pos  offset to be subtracted from the actual value (optional).
   *
   ** @return bottom border of overlay plane origin. Negative values are set to
   *0.
   */
  inline Uint16 getBottom(const Sint32 top_pos = 0) const {
    return (OFstatic_cast(Sint32, Top) + OFstatic_cast(Sint32, Height) -
                top_pos >
            0)
               ? OFstatic_cast(Uint16, OFstatic_cast(Sint32, Top) +
                                           OFstatic_cast(Sint32, Height) -
                                           top_pos)
               : 0;
  }
   ...
   */
  void setFlipping(const int horz, const int vert, const signed long columns,
   ...
   */
  void setRotation(const int degree, const signed long left_pos,
                   const signed long top_pos, const Uint16 columns,
                   const Uint16 rows);
  ...
  const char* getDescription() const {
    return (Description.empty()) ? OFstatic_cast(const char*, NULL)
                                 : Description.c_str();
  }
   ...
   *  Overlay plane is clipped to the area specified by the four min/max
   *coordinates. Memory isn't handled internally and must therefore be deleted
   ...
   */
  void* getData(const unsigned long frame, const Uint16 xmin, const Uint16 ymin,
                const Uint16 xmax, const Uint16 ymax, const int bits,
   ...
   *otherwise
   */
  unsigned long create6xxx3000Data(Uint8*& buffer, unsigned int& width,
                                   unsigned int& height, unsigned long& frames);
  ...
  /** get value of the current overlay plane bit and move 'cursor' to the next
   *position
   *
   ...
   */
  inline void setStart(const Uint16 x, const Uint16 y);
...
inline int DiOverlayPlane::reset(const unsigned long frame) {
  int result = 0;
  if (Valid && (Data != NULL) && (frame >= ImageFrameOrigin) &&
      (frame < ImageFrameOrigin + NumberOfFrames)) {
    const unsigned long bits = (OFstatic_cast(unsigned long, StartLeft) +
                                OFstatic_cast(unsigned long, StartTop) *
                                    OFstatic_cast(unsigned long, Columns) +
                                frame * OFstatic_cast(unsigned long, Rows) *
                                    OFstatic_cast(unsigned long, Columns)) *
...
inline void DiOverlayPlane::setStart(const Uint16 x, const Uint16 y) {
  if (BitsAllocated == 16)
    Ptr = StartPtr +
          OFstatic_cast(unsigned long, y - Top) *
              OFstatic_cast(unsigned long, Columns) +
          OFstatic_cast(unsigned long, x - Left);
  else
    BitPos = StartBitPos + (OFstatic_cast(unsigned long, y - Top) *
                                OFstatic_cast(unsigned long, Columns) +
                            OFstatic_cast(unsigned long, x - Left)) *
                               OFstatic_cast(unsigned long, BitsAllocated);
}
