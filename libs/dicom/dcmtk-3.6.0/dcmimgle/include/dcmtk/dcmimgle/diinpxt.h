...
static inline Uint16 expandSign(const Uint16 Value, const Uint16,
                                const Uint16) {
  return Value;
}

static inline Uint32 expandSign(const Uint32 Value, const Uint32,
                                const Uint32) {
  return Value;
}

static inline Sint8 expandSign(const Sint8 Value, const Sint8 SignBit,
                               const Sint8 SignMask) {
  return (Value & SignBit) ? (Value | SignMask) : Value;
}

static inline Sint16 expandSign(const Sint16 Value, const Sint16 SignBit,
                                const Sint16 SignMask) {
  return (Value & SignBit) ? (Value | SignMask) : Value;
}

static inline Sint32 expandSign(const Sint32 Value, const Sint32 SignBit,
...
class DiInputPixelTemplate : public DiInputPixel,
...
public:
   ...
   *  @param  fragment   current pixel item fragment (for encapsulated pixel
   *data)
   */
  DiInputPixelTemplate(const DiDocument* document, const Uint16 alloc,
                       const Uint16 stored, const Uint16 high,
                       const unsigned long first, const unsigned long number,
                       const unsigned long fsize, DcmFileCache* fileCache,
                       Uint32& fragment)
      : DiInputPixel(stored, first, number, fsize), Data(NULL) {
    MinValue[0] = 0;
    MinValue[1] = 0;
    MaxValue[0] = 0;
    MaxValue[1] = 0;
    ...
    if ((PixelCount == 0) ||
        (PixelStart + PixelCount > Count))  // check for corrupt pixel length
    {
      ...
      if ((sizeof(T2) <= 2) && (Count > 3 * ocnt))  // optimization criteria
      {
        ...
        if (lut != NULL) {
          ...
          for (i = 0; i < ocnt; ++i)  // search for minimum
          {
            if (*(q++) != 0) {
              MinValue[0] =
                  OFstatic_cast(T2, OFstatic_cast(double, i) + getAbsMinimum());
              break;
            }
          }
          q = lut + ocnt;
          for (i = ocnt; i != 0; --i)  // search for maximum
          {
            if (*(--q) != 0) {
              MaxValue[0] = OFstatic_cast(
                  T2, OFstatic_cast(double, i - 1) + getAbsMinimum());
              break;
            }
          }
          if (Count >= PixelCount)  // use global min/max value
          {
            ...
            {
              if (*(q++) != 0) {
                MinValue[1] = OFstatic_cast(
                    T2, OFstatic_cast(double, i) + getAbsMinimum());
                break;
              }
            }
            q = lut + ocnt;
   ...
   *               the minimum of the selected pixel range (1, see
   *PixelStart/Range)
   *
   ** @return minimum pixel value
   */
  inline double getMinValue(const int idx) const {
    return (idx == 0) ? OFstatic_cast(double, MinValue[0])
                      : OFstatic_cast(double, MinValue[1]);
  }

  /** get maximum pixel value
   *
   ** @param  idx  specifies whether to return the global maximum (0) or
   *               the maximum of the selected pixel range (1, see
   *PixelStart/Range)
   *
   ** @return maximum pixel value
   */
  inline double getMaxValue(const int idx) const {
    return (idx == 0) ? OFstatic_cast(double, MaxValue[0])
                      : OFstatic_cast(double, MaxValue[1]);
  }
...
private:
   ...
   *data)
   */
  void convert(const DiDocument* document, const Uint16 bitsAllocated,
               const Uint16 bitsStored, const Uint16 highBit,
               DcmFileCache* fileCache, Uint32& fragment) {
    T1* pixel = NULL;
    ...
    const OFBool uncompressed =
        pixelData->canWriteXfer(EXS_LittleEndianExplicit, EXS_Unknown);
    /* check whether to use partial read */
    if ((document->getFlags() & CIF_UsePartialAccessToPixelData) &&
        (PixelCount > 0) && (!uncompressed || !pixelData->valueLoaded()) &&
        (bitsAllocated % 8 == 0)) {
      /* Bits Allocated is always a multiple of 8 (see above), same for bits of
       * T1 */
      const Uint32 byteFactor = bitsAllocated / 8;
      const Uint32 bytes_T1 = bitsof_T1 / 8;
      const Uint32 count_T1 =
          (byteFactor == bytes_T1)
              ? PixelCount
              : (PixelCount * byteFactor + bytes_T1 - 1) / bytes_T1;
#ifdef DEBUG
      DCMIMGLE_TRACE("PixelCount: " << PixelCount << ", byteFactor: "
                                    << byteFactor << ", bytes_T1: " << bytes_T1
                                    << ", count_T1: " << count_T1);
#endif
      ...
      if (pixel != NULL) {
        if (uncompressed) {
          ...
          const OFCondition status =
              pixelData->getPartialValue(pixel, offset, bufSize, fileCache);
          if (status.good()) {
            PixelStart = 0;
            lengthBytes = bufSize;
          } else {
            DCMIMGLE_ERROR("can't access partial value from byte offset "
                           << offset << " to " << (offset + bufSize - 1) << ": "
                           << status.text());
          }
        } else {
          ...
          for (Uint32 frame = 0; frame < NumberOfFrames; ++frame) {
            /* make sure that the buffer always has an even number of bytes as
             * required for getUncompressedFrame() */
            const Uint32 bufSize = (fsize & 1) ? fsize + 1 : fsize;
            status = pixelData->getUncompressedFrame(
                document->getDataset(), FirstFrame + frame, fragment,
                OFreinterpret_cast(Uint8*, pixel) + lengthBytes, bufSize,
                decompressedColorModel, fileCache);
            if (status.good()) {
              DCMIMGLE_TRACE("successfully decompressed frame "
                             << FirstFrame + frame);
              lengthBytes += fsize;
            } else {
              DCMIMGLE_ERROR("can't decompress frame "
                             << FirstFrame + frame << ": " << status.text());
              break;
            }
          }
          if (status.good())
          ...
          if (!decompressedColorModel.empty() &&
              (decompressedColorModel !=
               document->getPhotometricInterpretation())) {
            DCMIMGLE_WARN(
                "Photometric Interpretation of decompressed pixel data "
                "deviates from original image: "
                << decompressedColorModel);
          }
        }
        deletePixel = OFTrue;
    ...
    if ((pixel != NULL) && (lengthBytes > 0)) {
      ...
      /* need to split 'length' in order to avoid integer overflow for large
       * pixel data */
      ...
      //          bitsAllocated;
      Count =
          8 * length_B1 + (8 * length_B2 + bitsAllocated - 1) / bitsAllocated;
      unsigned long i;
      Data = new T2[Count];
      if (Data != NULL) {
        DCMIMGLE_TRACE("Input length: "
                       << lengthBytes << " bytes, Pixel count: " << Count
                       << " (" << PixelCount << "), In: " << bitsof_T1
                       << " bits, Out: " << bitsof_T2 << " bits ("
          ...
          } else /* bitsStored < bitsAllocated */
          {
            ...
            if (shift == 0) {
              ...
              for (i = length_T1; i != 0; --i)
                *(q++) =
                    expandSign(OFstatic_cast(T2, *(p++) & mask), sign, smask);
            } else /* shift > 0 */
            {
              DCMIMGLE_DEBUG(
                  "convert input pixel data: case 1c (shift & mask & sign)");
              for (i = length_T1; i != 0; --i)
                *(q++) = expandSign(OFstatic_cast(T2, (*(p++) >> shift) & mask),
                                    sign, smask);
            }
          }
        } else if ((bitsof_T1 > bitsAllocated) &&
                   (bitsof_T1 % bitsAllocated ==
                    0))  // case 2: divisor of 8/16 bit
        {
          ...
          if ((bitsStored == bitsAllocated) && (bitsStored == bitsof_T2)) {
            ...
            for (i = length_T1; i != 0; --i) {
              ...
              for (j = times; j != 0; --j) {
                *(q++) =
                    expandSign(OFstatic_cast(T2, value & mask), sign, smask);
                value >>= bitsAllocated;
              }
            }
          }
        } else if ((bitsof_T1 < bitsAllocated) &&
                   (bitsAllocated % bitsof_T1 ==
                    0)  // case 3: multiplicant of 8/16
        ...
        } else  // case 4: anything else
        {
          ...
          while (i < length_T1) {
            if (skip < bitsof_T1) {
              ...
              {
                value |= (OFstatic_cast(
                              T2, (*p >> skip) & mask[bitsStored - bits - 1])
                          << bits);
              ...
              {
                value |= (OFstatic_cast(
                              T2, (*p >> skip) & mask[bitsof_T1 - skip - 1])
                          << bits);
