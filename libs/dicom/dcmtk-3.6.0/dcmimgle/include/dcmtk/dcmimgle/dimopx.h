...
public:
  ...
  virtual int getRoiWindow(const unsigned long left_pos,
                           ...
                           double& voiCenter, double& voiWidth) = 0;
   ...
   ** @return status, true if successful, false otherwise
   */
  virtual int getHistogramWindow(const double thresh, double& center,
                                 double& width) = 0;
  ...
  inline const char* getModalityLutExplanation() const {
    return (Modality != NULL) ? Modality->getExplanation()
                              : OFstatic_cast(const char*, NULL);
  }

  /** check whether pixel data is 'potentially' signed.
   *  This check is necessary to interpret possibly folowing LUT descriptors
   *correctly (see supplement 33)
   *
