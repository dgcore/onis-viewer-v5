...
public:
   ...
   */
  static int calculateCoefficients(const T1* x, const T2* y,
                                   const unsigned int n, const unsigned int o,
                                   T3_* c) {
    int result = 0;
   ...
   *  @param  n   number of entries in array (= points)
   *  @param  o   order of polynomial function
   *  @param  c   array of coefficients computed by the above method (o+1
   *entries !)
   *
   ** @return true if successful, false otherwise
   */
  static int calculateValues(const T1 xs, const T1 xe, T2* y,
                             const unsigned int n, const unsigned int o,
                             const T3_* c) {
    int result = 0;
    if ((y != NULL) && (c != NULL) && (n > 0) && (xe > xs)) {
      ...
      const T3_ xi = OFstatic_cast(
          T3_, (OFstatic_cast(T3_, xe) - OFstatic_cast(T3_, xs)) / (n - 1));
      for (i = 0; i < n; ++i) {
...
private:
  ...
  static void convertValue(const T3_ input, Uint8& output) {
    output =
        (input <= 0) ? 0 : ((input >= 255) ? 255 : OFstatic_cast(Uint8, input));
  }
  ...
  static void convertValue(const T3_ input, Sint8& output) {
    output = (input <= -128)
                 ? -128
                 : ((input >= 127) ? 127 : OFstatic_cast(Sint8, input));
  }
  ...
  static void convertValue(const T3_ input, Uint16& output) {
    output = (input <= 0)
                 ? 0
                 : ((input >= 65535) ? 65535 : OFstatic_cast(Uint16, input));
  }
  ...
  static void convertValue(const T3_ input, Sint16& output) {
    output = (input <= -32768)
                 ? -32768
                 : ((input >= 32767) ? 32767 : OFstatic_cast(Sint16, input));
  }
 ...
 * Changed static template functions to methods. Required for xlC 1.0 on
 * AIX 3.2.
 *
