...
public:
   ...
   *  Opens specified file and reads image related data, creates internal
   *representation of image data.  Use getStatus() to obtain detailed
   *information about any errors.
   *
   ** @param  filename  the DICOM file
   *  @param  flags     configuration flags (see diutils.h,
   *CIF_MayDetachPixelData is set automatically)
   *  @param  fstart    first frame to be processed (optional, 0 = 1st frame),
   *all subsequent use of parameters labeled 'frame' in this class refers to
   *this start frame.
   *  @param  fcount    number of frames (optional, 0 = all frames)
   */
  DicomImage(const char* filename, const unsigned long flags = 0,
   ...
   *                  (do not delete while referenced, i.e. while this image
   *object or any descendant exists; not deleted within dcmimage unless
   *configuration flag CIF_TakeOverExternalDataset is set - in this case do not
   *delete it at all)
   *  @param  xfer    transfer syntax
   *  @param  flags   configuration flags (CIF_xxx, see diutils.h)
   *  @param  fstart  first frame to be processed (optional, 0 = 1st frame), all
   *subsequent use of parameters labeled 'frame' in this class refers to this
   *start frame.
   *  @param  fcount  number of frames (optional, 0 = all frames)
   */
  DicomImage(DcmObject* object, const E_TransferSyntax xfer,
             const unsigned long flags = 0, const unsigned long fstart = 0,
             const unsigned long fcount = 0);

  /** constructor, use a given DcmObject with specified rescale/slope.
   *  NB: This constructor ignores the Photometric Interpretation stored in the
   *DICOM dataset and always creates a MONOCHROME2 image - useful in combination
   ...
   *  @param  fcount     number of frames (optional, 0 = all frames)
   */
  DicomImage(DcmObject* object, const E_TransferSyntax xfer, const double slope,
             const double intercept, const unsigned long flags = 0,
             const unsigned long fstart = 0, const unsigned long fcount = 0);
   ...
   *
   ** @param  object       pointer to DICOM data structures
   *                       (do not delete while referenced, i.e. while this
   *image object or any descendant exists; not deleted within dcmimage unless
   *configuration flag CIF_TakeOverExternalDataset is set - in this case do not
   *delete it at all)
   *  @param  xfer         transfer syntax
   ...
   *  @param  flags        configuration flags (CIF_xxx, see diutils.h)
   *  @param  fstart       first frame to be processed (optional, 0 = 1st
   *frame), all subsequent use of parameters labeled 'frame' in this class
   *refers to this start frame.
   *  @param  fcount       number of frames (optional, 0 = all frames)
   */
  DicomImage(DcmObject* object, E_TransferSyntax xfer,
             const DcmUnsignedShort& data, const DcmUnsignedShort& descriptor,
  ...
  /** process next couple of frames.  If the image object has been created with
   *less than the number of frames stored in the DICOM image, this function
   *allows for accessing the subsequent frames. Multiple calls to this function
   *allow for successively processing all frames stored in the file or dataset.
   *See parameters 'fstart' and 'fcount' of the constructor for how to initially
   *  create an instance of this class.
   *  NB: Only "original" images can be processed in this way, i.e. images that
   *were created by one of the above constructors.  Images that were created by
   *one of the createXXX() methods can not be processed since they are derived
   *from original image data in a certain way, e.g. scaled.
   *
   ** @param  fcount  number of frames to be processed (0 = same number as
   *before)
   *
   ** @return status, true if successful, false otherwise
   */
  inline int processNextFrames(const unsigned long fcount = 0) {
    return (Image != NULL) ? Image->processNextFrames(fcount) : 0;
  }
  ...
  /** convert photometric interpretation code to interpretation string (defined
   *term)
   *
   ...
   *  Please note that this function does not return the number of frames stored
   *in the DICOM file/dataset.  It rather refers to the number of frames
   *processed by this class (see constructors for details).
   *
   ...
   *  This functions returns the index of the first frame processed by this
   *class (see constructors for details).
   *
   ** @return index of first frame (0..n-1)
   */
  inline unsigned long getFirstFrame() const {
    return (Image != NULL) ? Image->getFirstFrame() : 0;
  }
  ...
  inline unsigned long getRepresentativeFrame() const {
    return (Image != NULL) ? Image->getRepresentativeFrame() : 0;
  }
   ...
   *  the resulting pixel values are stored in 'double' variables to avoid
   *problems with different number ranges, limited to monochrome images. Please
   *note that the min/max values refer to the full pixel data (i.e. including
   *  all possible present frames as specified in the constructor of this
   *class).
   *
   ...
   */
  inline int getMinMaxValues(double& min, double& max,
                             const int mode = 0) const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getMinMaxValues(min, max, mode)
               : 0;
  }

  /** get width height ratio (pixel aspect ratio: x/y).
   *  If present in the dataset and not overwritten using setWidthHeightRatio(),
   *the following attributes are supported in order to determine this value:
  ...
  inline int setWidthHeightRatio(const double ratio) const {
    return (Image != NULL) ? Image->setColumnRowRatio(ratio) : 0;
  }
  ...
  inline int setHeightWidthRatio(const double ratio) const {
    return (Image != NULL) ? Image->setRowColumnRatio(ratio) : 0;
  }
   ...
   *
   ** @return status, true if unused (1 = within output range, 2 = out of
   *range), false otherwise
   */
  inline int isOutputValueUnused(const unsigned long value) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->isValueUnused(value)
               : 0;
  }
   ...
   *  This function determines the size of a rendered frame as created by
   *getOutputData(). Therefore, it can be used to allocate a sufficiently large
   *memory buffer and pass its size to the second variant of getOutputData().
   *
   ** @param  bits  number of bits per sample used to render the pixel data
   *                (image depth, 1..MAX_BITS, 0 means 'bits stored' in the
   *image) (MI_PastelColor = -1 for true color pastel mode, EXPERIMENTAL)
   *
   ** @return number of bytes if successful, 0 otherwise
   */
  inline unsigned long getOutputDataSize(const int bits = 0) const {
    return (Image != NULL) ? Image->getOutputDataSize(Image->getBits(bits)) : 0;
  }

  /** render pixel data and return pointer to internal memory buffer.
   *  apply VOI/PLUT transformation and (visible) overlay planes.
   *  internal memory buffer will be delete for the next getBitmap/Output
   *operation. output data is always padded to 8, 16, 32, ... bits (bits
   *allocated). Supported output color models: Monochrome 2, RGB (and YCbCr_Full
   *if flag CIF_KeepYCbCrColorModel set).  The rendered pixel data is alway
   *unsigned.
   *
   ** @param  bits    number of bits per sample used to render the pixel data
   *                  (image depth, 1..MAX_BITS, 0 means 'bits stored' in the
   ...
   *                  (only applicable to multi-planar/color images, otherwise
   *ignored)
   *
  ...
  inline const void* getOutputData(const int bits = 0,
                                   ...
                                   const int planar = 0) {
    return (Image != NULL)
               ? Image->getOutputData(frame, Image->getBits(bits), planar)
               : NULL;
  }
   ...
   *  @param  size    size of memory buffer (will be checked whether it is
   *sufficient)
   *  @param  bits    number of bits per sample used to render the pixel data
   *                  (image depth, 1..MAX_BITS, 0 means 'bits stored' in the
   *image) (MI_PastelColor = -1 for true color pastel mode, EXPERIMENTAL)
   ...
   ** @return status code (true if successful)
   */
  inline int getOutputData(void* buffer, const unsigned long size,
                           const int bits = 0, const unsigned long frame = 0,
                           const int planar = 0) {
    return (Image != NULL) ? Image->getOutputData(buffer, size, frame,
                                                  Image->getBits(bits), planar)
                           : 0;
  }

  /** render pixel data and return pointer to given plane (internal memory
   *buffer). apply VOI/PLUT transformation and (visible) overlay planes internal
   *memory buffer will be delete for the next getBitmap/Output operation.
   *  Supported output color models: Monochrome 2, RGB (and YCbCr_Full if flag
   *  CIF_KeepYCbCrColorModel set).  The rendered pixel data is alway unsigned.
   *
  ...
  inline int isMonochrome() const {
    return (PhotometricInterpretation == EPI_Monochrome1) ||
   ...
   *  This function allows to access the pixel data after they have been
   *extracted from the DICOM data element and the modality transformation has
   *been applied (if present and not disabled).  Please note that for monochrome
   *images the internal representation might be signed whereas color images are
   *automatically converted to unsigned RGB format.  Pixels are aligned to 8, 16
   *or 32 bits. Always use DiPixel::getRepresentation() in order to determine
   *the integer representation of the internally stored pixel data.
   *
   ** @return pointer to intermediate pixel data representation
   */
  inline const DiPixel* getInterData() const {
    return (Image != NULL) ? Image->getInterData() : NULL;
  }
  ...
  inline DiDisplayFunction* getDisplayFunction() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getDisplayFunction()
               : OFstatic_cast(DiDisplayFunction*, NULL);
  }

  /** set display function
   *
   ** @param  display  object describing the output device characteristic (only
   *referenced!)
   *
  ...
  inline int setDisplayFunction(DiDisplayFunction* display) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setDisplayFunction(display)
               : 0;
  }
  ...
  inline int setNoDisplayFunction() {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setNoDisplayFunction()
               : 0;
  }
  ...
  inline int deleteDisplayLUT(const int bits = 0) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->deleteDisplayLUT(bits)
               : 0;
  }
   ...
   *          false otherwise
   */
  inline int convertPValueToDDL(const Uint16 pvalue, Uint16& ddl,
                                const int bits = 8) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->convertPValueToDDL(pvalue, ddl, bits)
               : 0;
  }
  ...
  inline int setNoVoiTransformation() {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setNoVoiTransformation()
               : 0;
  }

  /** set automatically calculated minimum/maximum window.
   *  possibly active VOI LUT is implicitly disabled.
   *  Please note that the min/max values refer to the full pixel data (i.e.
   *including all possible present frames as specified in the constructor of
   *this class).
   *
  ...
  inline int setMinMaxWindow(const int idx = 0) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setMinMaxWindow(idx)
               : 0;
  }
   ...
   *  possibly active VOI LUT is implicitly disabled.
   *
   ** @param  thresh  threshhold value specifying percentage of histogram border
   *which shall be ignored (defaut: 5%).
   *
   ** @return true if successful, false otherwise
   */
  inline int setHistogramWindow(const double thresh = 0.05) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setHistogramWindow(thresh)
               : 0;
  }

  /** set automatically calculated VOI window for the specified Region of
   *Interest (ROI). The ROI is specified by means of a rectangle (left, top,
   *width, height).  Only the part of the ROI that overlaps with the image is
   *regarded - if the overlapping area is empty this method returns false (0).
   *  Possibly active VOI LUT is implicitly disabled.
   *
   ** @param  left_pos  x-coordinate of the top left-hand corner of the ROI
   *(starting from 0)
   *  @param  top_pos   y-coordinate of the top left-hand corner of the ROI
   *(starting from 0)
   ...
   *  @param  frame     index of the frame to be used for calculation (default:
   *0 = first)
   *
   ** @return true if sucessful (1 = window has changed,
   *                             2 = new window is the same as previous one),
  ...
  inline int setRoiWindow(const unsigned long left_pos,
                          ...
                          const unsigned long frame = 0) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setRoiWindow(left_pos, top_pos,
                                                        width, height, frame)
               : 0;
  }

  /** set specified window (given by index to window width/center sequence
   *stored in image file). possibly active VOI LUT is implicitly disabled. NB:
   *This function does nothing if the flag CIF_UsePresentationState is set.
   *
   ** @param  window  index to window width/center sequence (0..n-1)
   *
   ** @return true if successful, false otherwise (none monochrome or invalid
   *index)
   */
  inline int setWindow(const unsigned long window) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setWindow(window)
               : 0;
  }
  ...
  inline int getWindow(double& center, double& width) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getWindow(center, width)
               : 0;
  }

  /** get number of VOI windows (stored in image file).
   *  NB: This function does nothing if the flag CIF_UsePresentationState is
   *set.
   *
  ...
  inline unsigned long getWindowCount() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getWindowCount()
               : 0;
  }

  /** set VOI LUT function.
   *  NB: Implementation of sigmoid transformation in "dimoopxt.h" is still
   *missing.
   *
  ...
  inline int setVoiLutFunction(const EF_VoiLutFunction function) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setVoiLutFunction(function)
               : 0;
  }
  ...
  inline EF_VoiLutFunction getVoiLutFunction() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getVoiLutFunction()
               : EFV_Default;
  }

  /** set VOI LUT (given by dcmdata elements).
   *  possibly active window/center is implicitly disabled.
   *  Given data is only referenced and not copied! Make sure that the
   *corresponding DcmXXX objects exist as long as the VOI LUT is set.
   *
   ...
   *  @param  descripMode  mode specifying the use of the bits per table entry
   *value
   *
   ** @return true if successful, false otherwise
   */
  inline int setVoiLut(const DcmUnsignedShort& data,
    ...
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setVoiLut(data, descriptor,
                                                     explanation, descripMode)
               : 0;
  }
  ...
  inline int setVoiLut(const unsigned long table,
                       const EL_BitsPerTableEntry descripMode = ELM_UseValue) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setVoiLut(table, descripMode)
               : 0;
  }
  ...
  inline unsigned long getVoiLutCount() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getVoiLutCount()
               : 0;
  }
  ...
  inline const char* getVoiTransformationExplanation() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getVoiTransformationExplanation()
               : OFstatic_cast(const char*, NULL);
  }
  ...
  inline const char* getVoiWindowExplanation(const unsigned long window,
    ...
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getVoiWindowExplanation(window,
                                                                   explanation)
               : OFstatic_cast(const char*, NULL);
  }
  ...
  inline const char* getVoiLutExplanation(const unsigned long table,
                                          OFString& explanation) const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getVoiLutExplanation(table,
                                                                explanation)
               : OFstatic_cast(const char*, NULL);
  }
  ...
  inline const char* getModalityLutExplanation() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getModalityLutExplanation()
               : OFstatic_cast(const char*, NULL);
  }
  ...
  inline EP_Polarity getPolarity() const {
    return (Image != NULL) ? Image->getPolarity() : EPP_Normal;
  }
  ...
  inline int setPolarity(const EP_Polarity polarity) {
    return (Image != NULL) ? Image->setPolarity(polarity) : 0;
  }

  /** set hardcopy parameters. only applicable to monochrome images.
   *  used to display LinOD images
   *
   ** @param  min      minimum density of the print-out (in hundreds of Optical
   *Density, e.g. 150 means 1.5 OD)
   *  @param  max      maximum density of the print-out (ditto)
   *  @param  reflect  reflected ambient light (in candela per square meter -
   *cd/m^2)
   *  @param  illumin  illumination (ditto)
   *
   ** @return true if successful (1 = at least one of the parameters has
   *changed, 2 = no parameter has changed), false otherwise
   */
  inline int setHardcopyParameters(const unsigned int min,
    ...
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setHardcopyParameters(
                     min, max, reflect, illumin)
               : 0;
  }
  ...
  inline ES_PresentationLut getPresentationLutShape() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getPresentationLutShape()
               : ESP_Default;
  }
   ...
   *
   ** @param  shape  presentation LUT shape (default, identity, inverse or lin
   *OD). 'default' means that the output data is always created with 0 for black
   *                 and maxvalue for white (i.e. monochrome2 data is created
   *for output).
   *
  ...
  inline int setPresentationLutShape(const ES_PresentationLut shape) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setPresentationLutShape(shape)
               : 0;
  }
   ...
   *  possibly active presentation LUT is implicitly disabled.
   *  Given data is only referenced and not copied! Make sure that the
   *corresponding DcmXXX objects exist as long as the presentation LUT is set.
   *
   ** @param  data         contains LUT data
   *  @param  descriptor   describes LUT structure
   ...
   ** @return true if successful, false otherwise
   */
  inline int setPresentationLut(
      const DcmUnsignedShort& data, const DcmUnsignedShort& descriptor,
      const DcmLongString* explanation = NULL,
      const EL_BitsPerTableEntry descripMode = ELM_UseValue) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setPresentationLut(
                     data, descriptor, explanation, descripMode)
               : 0;
  }
  ...
  inline const char* getPresentationLutExplanation() const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getPresentationLutExplanation()
               : OFstatic_cast(const char*, NULL);
  }
   ...
   ** @return true if successful, false otherwise
   */
  inline int setInversePresentationLut(
      const DcmUnsignedShort& data, const DcmUnsignedShort& descriptor,
      const EL_BitsPerTableEntry descripMode = ELM_UseValue) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->setInversePresentationLut(
                     data, descriptor, descripMode)
               : 0;
  }
   ...
   *  @param  left_pos     x coordinate of plane orgin (referring to image
   *origin)
   ...
   *
   ** @return false (0) if an error occurred, true otherwise (1 = added new
   *plane, 2 = replaced existing plane)
   */
  inline int addOverlay(const unsigned int group, const signed int left_pos,
                        const signed int top_pos, const unsigned int width,
                        const unsigned int height, const DcmOverlayData& data,
                        const DcmLongString& label,
                        const DcmLongString& description,
                        const EM_Overlay mode = EMO_Default) {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->addOverlay(group, left_pos, top_pos,
                                                      width, height, data,
   ...
   ** @param  group  group number (0x60nn) of overlay plane
   *
   ** @return false (0) if an error occurred (e.g. plane doesn't exist), true
   *otherwise
   */
  inline int removeOverlay(const unsigned int group) {
    return ((Image != NULL) && (Image->getOverlayPtr(1) != NULL))
               ? Image->getOverlayPtr(1)->removePlane(group)
               : 0;
  }

  /** remove all additional overlay planes
   *
   ** @return false (0) if an error occurred, true otherwise (1 = all planes
   *deleted, 2 = no planes to be deleted)
   */
  inline int removeAllOverlays() {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->removeAllOverlays()
               : 0;
  }
  ...
  inline int isOverlayVisible(const unsigned int plane,
    ...
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->isPlaneVisible(plane)
               : 0;
  }
   ...
   *default: 0
   *
   ** @return false (0) if an error occurred, true otherwise (1 = plane
   *successfully activated, 2 = plane has already been visible)
   */
  inline int showOverlay(const unsigned int plane, const unsigned int idx = 0) {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->showPlane(plane)
               : 0;
  }
   ...
   *  @param  thresh  treshhold value (in percent, default: 0.5), only for
   *EMO_TreshholdReplace
   *  @param  idx     index of overlay group (0 = dataset, 1 = additional),
   *default: 0
   *
   ** @return false (0) if an error occurred, true otherwise
   */
  inline int showOverlay(const unsigned int plane, const EM_Overlay mode,
                         const double fore = 1.0, const double thresh = 0.5,
                         const unsigned int idx = 0) {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->showPlane(plane, fore, thresh, mode)
               : 0;
  }

  /** activate specified overlay plane and change 'pvalue' (only for bitmap
   *shutters)
   *
   ** @param  plane   number (0..15) or group number (0x60nn) of overlay plane
   ...
   ** @return false (0) if an error occurred, true otherwise
   */
  inline int showOverlay(const unsigned int plane, const Uint16 pvalue) {
    return ((Image != NULL) && (Image->getOverlayPtr(1) != NULL))
               ? Image->getOverlayPtr(1)->showPlane(plane, pvalue)
               : 0;
  }
   ...
   *default: 0
   *
   ** @return false (0) if an error occurred, true otherwise (1 = planes have
   *been successfully activated, 2 = no planes to be activated)
   */
  inline int showAllOverlays(const unsigned int idx = 0) {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->showAllPlanes()
               : 0;
  }
   ...
   *been successfully activated, 2 = no planes to be activated)
   */
  inline int showAllOverlays(const EM_Overlay mode, const double fore = 1,
                             const double thresh = 0.5,
                             const unsigned int idx = 0) {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->showAllPlanes(fore, thresh, mode)
               : 0;
  }
   ...
   *
   ** @return false (0) if an error occurred, true otherwise (1 = plane
   *successfully deactivated, 2 = plane has already been invisible)
   */
  inline int hideOverlay(const unsigned int plane, const unsigned int idx = 0) {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->hidePlane(plane)
               : 0;
  }
   ...
   *
   ** @return false (0) if an error occurred, true otherwise (1 = planes have
   *been successfully deactivated, 2 = no planes to be deactivated)
   */
  inline int hideAllOverlays(const unsigned int idx = 0) {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->hideAllPlanes()
               : 0;
  }
   ...
   *default: 0
   *
   ** @return false (0) if an error occurred, true otherwise (1 = plane has been
   *successfully moved, 2 = old and new position are equal, nothing to do)
   */
  inline int placeOverlay(const unsigned int plane, const signed int left_pos,
                          const signed int top_pos,
                          const unsigned int idx = 0) {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->placePlane(plane, left_pos, top_pos)
               : 0;
  }
  ...
  inline unsigned int getOverlayCount(const unsigned int idx = 0) const {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->getCount()
               : 0;
  }
   ...
   *default: 0
   *
   ** @return group number of given overlay plane if successful, false (0)
   *otherwise
   */
  inline unsigned int getOverlayGroupNumber(const unsigned int plane,
                                            const unsigned int idx = 0) const {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->getPlaneGroupNumber(plane)
               : 0;
  }
   ...
   *default: 0
   *
   ** @return pointer to overlay plane label if successful, false (NULL)
   *otherwise
   */
  inline const char* getOverlayLabel(const unsigned int plane,
                                     const unsigned int idx = 0) const {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->getPlaneLabel(plane)
               : OFstatic_cast(const char*, NULL);
  }
   ...
   *default: 0
   *
   ** @return pointer to overlay plane description if successful, false (NULL)
   *otherwise
   */
  inline const char* getOverlayDescription(const unsigned int plane,
                                           const unsigned int idx = 0) const {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->getPlaneDescription(plane)
               : OFstatic_cast(const char*, NULL);
  }
  ...
  inline EM_Overlay getOverlayMode(const unsigned int plane,
                                   const unsigned int idx = 0) const {
    return ((Image != NULL) && (Image->getOverlayPtr(idx) != NULL))
               ? Image->getOverlayPtr(idx)->getPlaneMode(plane)
               : EMO_Default;
  }

  /** create bitmap for specified overlay plane.
   *  (up to 16 bits per pixel with two values: 'fore' and 'back')
   *  Please note that the current origin of the overlay plane is used.
   *Furthermore, the size of the overlay plane is restricted to the size of the
   *surrounding image.  Use the method getFullOverlayData() if the complete
   *bitmap data is required regardless of its position.
   *
   ** @param  plane     number (0..15) or group number (0x60nn) of overlay plane
   ...
   *  @param  bits      number of bits (stored) in the resulting array, default:
   *8, range: 1..16 Used to mask the values for foreground and background color.
   *The resulting array is always padded to 8 or 16 bits with 1, 8 or 16 bits
   *allocated depending on the value of 'bits'.
   *  @param  fore      foreground color to be set in bitmap, default: 255,
   *range: 0..2^bits-1
   *  @param  back      background color to be set in bitmap (transparent),
   *default: 0, range: 0..2^bits-1
   *  @param  idx       index of overlay group (0 = dataset, planes stored in
   *the image dataset; 1 = additional, planes added by addOverlay(); 2 =
   *'additional' overlay planes hide 'dataset' planes when the overlay group
   *number exists in both), default: 2
   *
   ** @return pointer to overlay plane data (internal memory buffer)
   */
  const void* getOverlayData(const unsigned int plane, unsigned int& left_pos,
                             unsigned int& top_pos, unsigned int& width,
                             unsigned int& height, EM_Overlay& mode,
                             const unsigned long frame = 0, const int bits = 8,
                             const Uint16 fore = 0xff, const Uint16 back = 0x0,
                             const unsigned int idx = 2) const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getOverlayData(
                     frame, plane, left_pos, top_pos, width, height, mode, idx,
                     bits, fore, back)
               : NULL;
  }
  ...
  /** create bitmap for specified overlay plane.
   *  (up to 16 bits per pixel with two values: 'fore' and 'back')
   *  In contrast to the previous method the full bitmap data is always
   *returned.
   *
   ** @param  plane   number (0..15) or group number (0x60nn) of overlay plane
   *  @param  width   returns width of overlay plane (in pixels)
   ...
   *The resulting array is always padded to 8 or 16 bits with 1, 8 or 16 bits
   *allocated depending on the value of 'bits'.
   *  @param  fore    foreground color to be set in bitmap, default: 255, range:
   *0..2^bits-1
   *  @param  back    background color to be set in bitmap (transparent),
   *default: 0, range: 0..2^bits-1
   *  @param  idx     index of overlay group (0 = dataset, planes stored in the
   *image dataset; 1 = additional, planes added by addOverlay()), default: 0
   *
   ** @return pointer to overlay plane data (internal memory buffer)
   */
  const void* getFullOverlayData(const unsigned int plane, unsigned int& width,
                                 unsigned int& height,
                                 const unsigned long frame = 0,
                                 const int bits = 8, const Uint16 fore = 0xff,
                                 const Uint16 back = 0x0,
                                 const unsigned int idx = 0) const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->getFullOverlayData(
                     frame, plane, width, height, idx, bits, fore, back)
               : NULL;
  }
  ...
  /** create bitmap for specified overlay plane and store it in (6xxx,3000)
   *format. (1 bit allocated and stored, foreground color is 1, background color
   *is 0, data is 16 bit padded - even length) memory is allocated but not
   *handled internally - must be deleted from calling program.
   *
   ** @param  buffer  stores pointer to overlay data (memory is allocated
   *internally)
   *  @param  plane   number (0..15) or group number (0x60nn) of overlay plane
   *  @param  width   returns width of overlay plane (in pixels)
   *  @param  height  returns height of overlay plane (in pixels)
   ...
   *image dataset; 1 = additional, planes added by addOverlay()), default: 0
   *
   ** @return number of bytes allocated for the 'buffer' if successful, 0
   *otherwise
   */
  unsigned long create6xxx3000OverlayData(Uint8*& buffer,
                                          ...
                                          const unsigned int idx = 0) const {
    return ((Image != NULL) && (Image->getMonoImagePtr() != NULL))
               ? Image->getMonoImagePtr()->create6xxx3000OverlayData(
                     buffer, plane, width, height, frames, idx)
               : 0;
  }

  // --- create...Image: return pointer to new 'DicomImage' object, memory isn't
  // handled internally !
   ...
   *  @param  interpolate  specifies whether scaling algorithm should use
   *interpolation (if necessary). default: no interpolation (0), preferred
   *interpolation algorithm (if applicable): 1 = pbmplus algorithm, 2 = c't
   *algorithm, 3 = bilinear magnification, 4 = bicubic magnification
   *  @param  aspect       specifies whether pixel aspect ratio should be taken
   *into consideration (if true, width OR height should be 0, i.e. this
   *component will be calculated automatically)
   *
   ** @return pointer to new DicomImage object (NULL if an error occurred)
   */
   ...
   *  memory is not handled internally - must be deleted from calling program.
   *
   ** @param  xfactor      width of new image is multiplied with this factor (>
   *0)
   *  @param  yfactor      height of new image is multiplied with this factor (>
   *0)
   *  @param  interpolate  specifies whether scaling algorithm should use
   *interpolation (if necessary). default: no interpolation (0), preferred
   *interpolation algorithm (if applicable): 1 = pbmplus algorithm, 2 = c't
   ...
   ** @return pointer to new DicomImage object (NULL if an error occurred)
   */
  DicomImage* createScaledImage(const double xfactor, const double yfactor = 0,
                                const int interpolate = 0,
                                const int aspect = 0) const;

  /** create scaled copy of specified (clipping) area of the current image
   *object. memory is not handled internally - must be deleted from calling
   *program. NB: Clipping and interpolated scaling at the same moment is not yet
   *fully implemented!
   *
   ** @param  left_pos      x coordinate of top left corner of area to be scaled
   *                        (referring to image origin, negative values create a
   ...
   *into consideration (if true, width OR height should be 0, i.e. this
   *component will be calculated automatically)
   *  @param  pvalue        P-value used for the border outside the image
   *(0..65535)
   *
   ** @return pointer to new DicomImage object (NULL if an error occurred)
   */
  DicomImage* createScaledImage(
      const signed long left_pos, const signed long top_pos,
      unsigned long clip_width, unsigned long clip_height,
      unsigned long scale_width = 0, unsigned long scale_height = 0,
      const int interpolate = 0, int aspect = 0, const Uint16 pvalue = 0) const;
   ...
   *
   ** @param  left_pos     x coordinate of top left corner of area to be scaled
   *                       (referring to image orgin, negative values create a
   *border around the image)
   *  @param  top_pos      y coordinate of top left corner of area to be scaled
  ...
  DicomImage* createScaledImage(const signed long left_pos,
                                const signed long top_pos, unsigned long width,
                                unsigned long height, const double xfactor,
                                const double yfactor = 0,
                                const int interpolate = 0, const int aspect = 0,
   ...
   *                              2 = not flipped, because of image resolution -
   *width and/or height equal to 1), false otherwise
   */
   ...
   *
   ** @param  degree  angle by which the image shall be rotated (-360, -270,
   *-180, -90, 0, 90, 180, 270, 360)
   *
   ** @return true if successful (1 = rotated by at least 90 degrees,
   *                              2 = not rotated, because of image resolution
   *or angle), false otherwise
   */
   ...
   *  memory is not handled internally - must be deleted from calling program.
   *  This method does not work if original YCbCr color model is retained (see
   ...
   *  8 bit images require an appropriate color palette (256 entries, values: 0
   *to 255) and are only applicable to monochrome images, the beginning of a
   *each line starts on a 32-bit address (if 'padding' is true); 24 bit images
   *store 24 bits per pixel (BGR) and do align each line to a 32-bit address (if
   *'padding' is true); 32 bit images store 32 bits per pixel (ARGB), but only
   *  use the lower 24 bits (-RGB).
   *  The memory buffer can be allocated both externally (from the calling
   *program) and internally (inside this class/module).  If the 'data' parameter
   *is not NULL and the 'size' parameter, which describes the size (in bytes) of
   *the allocated buffer, is suffiently large, the bitmap is stored in this
   *buffer.  Otherwise (i.e. 'data' is NULL) the memory is allocated internally.
   *Please note that in both cases the memory is not handled internally after
   *this method has finished and, therefore, must be deleted from the calling
   *program. This method does not work if original YCbCr color model is retained
   *(see CIF_KeepYCbCrColorModel).
   *
   ** @param  data        untyped pointer memory buffer (set to NULL if not
   *allocated externally)
   *  @param  size        size of the memory buffer in bytes (if 0 'data' is set
   *to NULL)
   *  @param  frame       index of frame to be converted (default: 0 = first
   *frame)
   *  @param  bits        number of bits per pixel used for the output bitmap
   *(8, 24 or 32, default: 24)
   *  @param  upsideDown  flag indicating whether the first line stored is the
   *top-most (default: 0) or the bottom-most of the source image (as required by
   *the BMP file format)
   ...
   */
  unsigned long createWindowsDIB(void*& data, const unsigned long size,
                                 const unsigned long frame = 0,
                                 const int bits = 24, const int upsideDown = 0,
                                 const int padding = 1) {
    return (Image != NULL)
               ? Image->createDIB(data, size, frame, bits, upsideDown, padding)
               : 0;
  }

  /** create true color (32 bit) or palette (8 bit) bitmap for Java (AWT default
   *format). 32 bit images allocate 32 bits per pixel (RGB), but only use the
   *upper 24 bits.  The sample order for color images is: Red, Green, Blue.
   *  Memory is not handled internally - must be deleted from calling program.
   *  This method does not work if original YCbCr color model is retained (see
   *CIF_KeepYCbCrColorModel).
   *
   ** @param  data   resulting pointer to bitmap data (set to NULL if an error
   *occurred)
   *  @param  frame  index of frame to be converted (default: 0 = first frame)
   *  @param  bits   number of bits per pixel used for the output bitmap (8 or
   *32, default: 32)
   *
   ** @return number of bytes allocated by the bitmap, or 0 if an error occured
   */
  unsigned long createJavaAWTBitmap(void*& data, const unsigned long frame = 0,
                                    const int bits = 32) {
    return (Image != NULL) ? Image->createAWTBitmap(data, frame, bits) : 0;
  }

  /** create 12 bit packed (monochrome) bitmap for DICOM printers.
   *  Memory is not handled internally - must be deleted from calling program.
   *
   ** @param  buffer  pointer to input memory buffer (16 bits allocated, 12 bits
   *stored)
   *  @param  size    size of memory buffer (will be checked whether it is
   *sufficient)
  ...
  /** render pixel data of the given frame and write image related attributes to
   *DICOM dataset. Applies VOI/PLUT transformation and (visible) overlay planes,
   *output data is always padded to 8, 16, 32, ... bits (bits allocated).
   *Replaces any modality transformation in the dataset by a linear
   *rescale/slope since the modality transformation is rendered into the pixel
   *data.  Replaces the VOI transformations in the dataset by a "max range" VOI
   *window.  Removes all Overlay Plane Module attributes for embedded overlay
   *planes from the dataset. Writes the following DICOM attributes (from Image
   *Pixel Module):
   ...
   *  Updates the following DICOM attributes (if present in the original image
   *dataset):
   *    - Pixel Spacing and/or Pixel Aspect Ratio
   *  Supported output color models: Monochrome 2, RGB (and YCbCr_Full if flag
   *  CIF_KeepYCbCrColorModel set).
   *
   ** @param  dataset  reference to DICOM dataset where the image attributes are
   *stored
   ...
   ** @return true if successful, false otherwise
   */
  inline int writeFrameToDataset(DcmItem& dataset, const int bits = 0,
                                 const unsigned long frame = 0,
                                 const int planar = 0) {
    return (Image != NULL)
               ? Image->writeFrameToDataset(dataset, frame, bits, planar)
               : 0;
  }

  /** write current image and related attributes to DICOM dataset.
   *  Uses the internal representation of the pixel data, therefore the output
   *data is always padded to 8, 16, 32, ... bits (bits allocated).  Replaces any
   *modality transformation in the dataset by a linear rescale/slope since the
   *modality transformation is rendered into the pixel data.  Removes all
   *Overlay Plane Module attributes for embedded overlay planes from the
   *dataset. Writes the following DICOM attributes (from Image Pixel Module):
   *    - Photometric Interpretation, Samples per Pixel
   *    - Columns, Rows, Number of Frames
   *    - Bits Allocated, Bits Stored, High Bit
   *    - Planar Configuration (only if "Samples per Pixel" is greater than 1)
   ...
   ** @param  dataset  reference to DICOM dataset where the image attributes are
   *stored
   *  @param  mode     0 = determine value of BitsStored from 'used' pixel
   *values, 1 = determine value of BitsStored from 'possible' pixel values (used
   *for monochrome images only)
   *  @param  planar   0 = color-by-pixel (R1G1B1...R2G2B2...R3G3B3...),
   *                   1 = color-by-plane (R1R2R3...G1G2G3...B1B2B3...),
   *                   2 = same as original DICOM image (i.e. color-by-pixel or
   *color-by-plane) (only applicable to multi-planar/color images, otherwise
   *ignored)
   *
   ** @return true if successful, false otherwise
   */
  inline int writeImageToDataset(DcmItem& dataset, const int mode = 0,
                                 const int planar = 2) {
    return (Image != NULL) ? Image->writeImageToDataset(dataset, mode, planar)
                           : 0;
  }
   ...
   *  (see CIF_KeepYCbCrColorModel).
   *
   ** @param  filename  name of output file (%d is replaced by frame number if
   *present)
   *  @param  bits      number of bits used for output of pixel data
   *                    (default: full resolution, max: 32;
   *                     MI_PastelColor = -1 for true color pastel mode,
   *EXPERIMENTAL)
   ...
   ** @return true if successful, false otherwise
   */
  int writePPM(const char* filename, const int bits = 0,
               const unsigned long frame = 0);
   ...
   ** @return true if successful, false otherwise
   */
  int writePPM(STD_NAMESPACE ostream& stream, const int bits = 0,
               const unsigned long frame = 0);
   ...
   ** @return true if successful, false otherwise
   */
  int writeRawPPM(const char* filename, const int bits = 0,
                  const unsigned long frame = 0);
   ...
   ** @return true if successful, false otherwise
   */
  int writeRawPPM(FILE* stream, const int bits = 0,
                  const unsigned long frame = 0);
   ...
   ** @param  stream  open C output stream (binary mode required!)
   *  @param  bits    number of bits used for output of pixel data
   *                  (8, 24 or 32, default (0): 8 for monochrome and 24 for
   *color images)
   *  @param  frame   index of frame used for output (default: first frame = 0)
   *
   ** @return true if successful, false otherwise
   ...
   ** @return true if successful, false otherwise
   */
  int writeBMP(const char* filename, const int bits = 0,
               const unsigned long frame = 0);

  /** write pixel data to plugable image format file (specified by open C
   *stream). Format specific parameters may be set directly in the instantiated
   *'plugin' class.
   *
   ** @param  plugin  pointer to image format plugin (derived from abstract
   *class DiPluginFormat)
   *  @param  stream  open C output stream (binary mode required!)
   *  @param  frame   index of frame used for output (default: first frame = 0)
   *
   ** @return true if successful, false otherwise
   */
  int writePluginFormat(const DiPluginFormat* plugin, FILE* stream,
                        const unsigned long frame = 0);

  /** write pixel data to plugable image format file (specified by filename).
   *  Format specific parameters may be set directly in the instantiated
   *'plugin' class.
   *
   ** @param  plugin    pointer to image format plugin (derived from abstract
   *class DiPluginFormat)
   ...
   ** @return true if successful, false otherwise
   */
  int writePluginFormat(const DiPluginFormat* plugin, const char* filename,
                        const unsigned long frame = 0);

protected:
  /** constructor, create copy of given image object with different image data
   *and photometric interpretation
   *
   ** @param  dicom      source object to be copied
   *  @param  image      new image data
   *  @param  interpret  new photometric interpretation
   */
  DicomImage(const DicomImage* dicom, DiImage* image,
             const EP_Interpretation interpret = EPI_Unknown);

  /** initialize object.
   *  create internal image object depending on color model. is used for all
   * 'real' constructors.
   */
  void Init();
   ...
   *  negative value are mapped to positive range (-360 -> 0, -270 -> 90, -180
   *-> 180, -90 -> 270), 360 is set to 0, all other values are rejected
   *
