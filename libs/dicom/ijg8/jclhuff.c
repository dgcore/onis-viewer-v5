  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    if (gather_statistics) {
      ...
      if (entropy->count_ptrs[dctbl] == NULL)
        entropy->count_ptrs[dctbl] = (long*)(*cinfo->mem->alloc_small)(
...
/* Emit some bits; return TRUE if successful, FALSE if must suspend */
{
  ...
  put_buffer &=
      (((IJG_INT32)1) << size) - 1; /* mask off any extra bits in code */
...
jinit_lhuff_encoder(j_compress_ptr cinfo) {
  ...
  entropy = (lhuff_entropy_ptr)(*cinfo->mem->alloc_small)(
      (j_common_ptr)cinfo, JPOOL_IMAGE, SIZEOF(lhuff_entropy_encoder));
