...
METHODDEF(void)
simple_upscale(j_decompress_ptr cinfo, JDIFFROW diff_buf, JSAMPROW output_buf,
               JDIMENSION width) {
...
METHODDEF(void)
simple_downscale(j_decompress_ptr cinfo, JDIFFROW diff_buf, JSAMPROW output_buf,
                 JDIMENSION width) {
  j_lossless_d_ptr losslsd = (j_lossless_d_ptr)cinfo->codec;
  scaler_ptr scale = (scaler_ptr)losslsd->scaler_private;
  int scale_factor = scale->scale_factor;
...
METHODDEF(void)
noscale(j_decompress_ptr cinfo, JDIFFROW diff_buf, JSAMPROW output_buf,
        JDIMENSION width) {
  unsigned int xindex;
...
jinit_d_scaler(j_decompress_ptr cinfo) {
  ...
  scale = (scaler_ptr)(*cinfo->mem->alloc_small)((j_common_ptr)cinfo,
                                                 JPOOL_IMAGE, SIZEOF(scaler));
