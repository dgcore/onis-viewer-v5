...
// #define TAG_CHANNEL_MINIMUM_VALUE 0x003A0216UL #define
// TAG_CHANNEL_MAXIMUM_VALUE 0x003A0217UL
...
#define TAG_PERFORMED_PROCEDURE_STEP_DISCONTINUATION_REASON_CODE_SEQUENCE \
  0x00400281UL
...
#define TAG_RESULTING_GENERAL_PURPOSE_PERFORMED_PROCEDURE_STEPS_SEQUENCE \
  0x00404015UL
#define TAG_REFERENCED_GENERAL_PURPOSE_SCHEDULED_PROCEDURE_STEP_SEQUENCE \
  0x00404016UL
...
#define TAG_REFERENCED_GENERAL_PURPOSE_SCHEDULED_PROCEDURE_STEP_TRANSACTION_UID \
  0x00404023UL
...
#define IMGINFO_RESCALE 1
#define IMGINFO_PLANAR 2
#define IMGINFO_BITS 4
#define IMGINFO_DIMENSIONS 8
#define IMGINFO_PIXRATIO 16
#define IMGINFO_PHOTOINTERP 32
#define IMGINFO_PIXEL_PADDING 64

namespace onis {

class frame_region;
typedef std::shared_ptr<frame_region> frame_region_ptr;
typedef std::weak_ptr<frame_region> frame_region_wptr;
typedef std::vector<frame_region_ptr> frame_region_list;

enum dcm_encoding_type { explicit_length = 0, undefined_length = 1 };

enum dcm_group_len_encoding {
  no_change_in_gl = 0,  // no change of GL values, WARNING: DO NOT USE FOR WRITE
  without_gl = 1,       // remove group length tags
  with_gl = 2,          // add group length tags for every group
  recalc_gl = 3         // recalculate values for existing group length tags
};

enum dcm_padding_encoding {
  no_change_in_padding = 0,  // no change of padding tags
  without_padding = 1,       // remove all padding tags
  with_padding = 2           // add padding tags
};
...
const std::int32_t charset_default = 0;
const std::int32_t charset_latin1 = 1;
const std::int32_t charset_latin2 = 2;
const std::int32_t charset_latin3 = 3;
const std::int32_t charset_latin4 = 4;
const std::int32_t charset_latin5 = 5;
const std::int32_t charset_cyrillic = 6;
const std::int32_t charset_arabic = 7;
const std::int32_t charset_greek = 8;
const std::int32_t charset_hebrew = 9;
const std::int32_t charset_japanese = 10;
const std::int32_t charset_thai = 11;
const std::int32_t charset_korean = 12;
const std::int32_t charset_utf8 = 13;

class dicom_charset_info {
public:
  dicom_charset_info() = default;
  virtual ~dicom_charset_info() = default;

  bool single_byte{true};
  std::string esc_g0;
  std::string esc_g1;
  std::string defined_term;
  std::string no_extention_term;
  std::string code_page;
};

typedef std::vector<dicom_charset_info*> dicom_charset_info_list;

class dicom_charset {
public:
  dicom_charset() = default;
  virtual ~dicom_charset() {
    for (auto& info : info) {
      delete info;
    }
  }
  std::string code;
  std::string name;
  dicom_charset_info_list info;
};
typedef std::vector<const dicom_charset*> dicom_charset_list;

///////////////////////////////////////////////////////////////////////
// dicom_tag_info
///////////////////////////////////////////////////////////////////////

struct dicom_tag_info {
  std::int32_t tag;
  std::string vr;
};
...
class dicom_frame_offsets {
public:
  dicom_frame_offsets() = default;
  ~dicom_frame_offsets() {
    delete[] offsets;
  }
...
///////////////////////////////////////////////////////////////////////
// dicom_secondary_capture_info
///////////////////////////////////////////////////////////////////////

class dicom_secondary_capture_info {
public:
  // constructor:
  dicom_secondary_capture_info() = default;

  // destructor:
  virtual ~dicom_secondary_capture_info() {
    for (auto& tag : tags_to_copy) {
      delete tag;
    }
  }

  // operators:
  void add_tag_to_copy(std::int32_t tag, const std::string& vr) {
    dicom_tag_info* info = new dicom_tag_info;
    info->tag = tag;
    info->vr = vr;
    tags_to_copy.push_back(info);
  }

  void remove_tag_to_copy(std::int32_t tag) {
    for (std::list<dicom_tag_info*>::iterator it = tags_to_copy.begin();
         it != tags_to_copy.end(); it++)
      if ((*it)->tag == tag) {
        delete *it;
  ...
  std::string study_description;         // type 3

  // general series:
  std::string modality;                   // type 1
  ...
  std::string patient_position;           // type 2C

  // equipment:
  std::string conversion_type;                 // type 1
  std::string device_id;                       // type 3
  std::string device_manufacturer;             // type 3
  std::string device_manufacturer_model_name;  // type 3
...
public:
  ...
  dicom_hard_copy_info() {
    ...
    add_tag_to_copy(TAG_PATIENT_ID, "LO");
    add_tag_to_copy(TAG_PATIENT_NAME, "PN");
    add_tag_to_copy(TAG_PATIENT_SEX, "CS");
    add_tag_to_copy(TAG_PATIENT_AGE, "AS");
    add_tag_to_copy(TAG_PATIENT_SIZE, "DS");
    add_tag_to_copy(TAG_PATIENT_WEIGHT, "DS");
    add_tag_to_copy(TAG_PATIENT_BIRTH_DATE, "DA");
    add_tag_to_copy(TAG_PATIENT_BIRTH_TIME, "TM");
  }

  // destructor:
  virtual ~dicom_hard_copy_info() {
    for (auto& tag : tags_to_copy) {
      delete tag;
    }
  }
  ...
  void remove_tag_to_copy(std::int32_t tag) {
    for (auto it = tags_to_copy.begin(); it != tags_to_copy.end(); it++) {
      if ((*it)->tag == tag) {
        delete *it;
        tags_to_copy.erase(it);
        break;
  ...
  std::string device_software_version;
...
struct dicom_image_info {
  dicom_image_info() {
    pixels = nullptr;
    length = 0;
    red_palette_data_count = 0;
    green_palette_data_count = 0;
    blue_palette_data_count = 0;
    red_palette_data = nullptr;
    green_palette_data = nullptr;
    blue_palette_data = nullptr;
  }

  ~dicom_image_info() {
    delete[] pixels;
    delete[] red_palette_data;
    delete[] green_palette_data;
    delete[] blue_palette_data;
  }
  ...
  std::int32_t width;
  std::int32_t height;
  std::string photo_interpretation;
  std::int32_t bits_alloc;
  std::int32_t bits_stored;
  std::int32_t high_bit;
  std::int32_t pixel_representation;
  std::int32_t sample_per_pixel;
  std::int32_t pixel_ratio[2];
  std::string planar_configuration;
  std::string rescale_intercept;
  std::string rescale_slope;
  std::string rescale_type;
  std::string pixel_padding_value;
  std::int32_t frame_count;
  std::uint8_t* pixels;
  std::int32_t length;

  std::string red_palette_desc;
  std::string green_palette_desc;
  std::string blue_palette_desc;
  std::int32_t red_palette_data_count;
  std::uint8_t* red_palette_data;
  std::int32_t green_palette_data_count;
  std::uint8_t* green_palette_data;
  std::int32_t blue_palette_data_count;
  std::uint8_t* blue_palette_data;
};
...
class dicom_sequence_item;
typedef std::shared_ptr<dicom_sequence_item> dicom_sequence_item_ptr;
typedef std::weak_ptr<dicom_sequence_item> dicom_sequence_item_wptr;

class dicom_base {
public:
  // prevent copy and assignment:
  dicom_base(const dicom_base&) = delete;
  dicom_base& operator=(const dicom_base&) = delete;
  dicom_base(dicom_base&&) = delete;
  dicom_base& operator=(dicom_base&&) = delete;
  ...
  virtual void* get_next_element(std::int32_t target, void* elem,
                                 ...
                                 std::int32_t* vm) const = 0;
  virtual bool get_string_from_element(std::string& output,
                                       void* elem) const = 0;
  virtual bool get_string_from_element(std::string& output, void* elem,
                                       std::string& charset) const = 0;
  virtual bool get_string_element(std::string& output, std::int32_t tag,
                                  const std::string& type) const = 0;
  virtual bool get_string_element(
      std::string& output, std::int32_t tag, const std::string& type,
      const std::string& charset,
      onis::dicom_charset_info_list* used_charset_infos = nullptr) const = 0;
  virtual bool get_us_element(std::int32_t tag, std::uint16_t* value) const = 0;
  ...
  // set elements:
  virtual bool set_string_element(std::int32_t tag, const std::string& type,
                                  const std::string& value, bool create) = 0;
  virtual std::int32_t set_string_element(
      std::int32_t tag, const std::string& type, const std::string& utf8_value,
      std::list<const onis::dicom_charset*>* compatible_charsets,
      bool create) = 0;
  ...
  virtual bool set_time_element(std::int32_t tag, std::int32_t hour,
                                ...
                                std::int32_t fraction, bool create = true) = 0;
  virtual bool set_binary_value(std::int32_t tag, const std::string& type,
                                std::int32_t length, std::uint8_t* data,
                                bool create = true) = 0;
  // virtual bool set_binary_value(std::int32_t tag, const std::string &type,
  // std::int32_t len, std::uint8_t *data, const std::string &transfer_syntax,
  // bool create = OSTRUE) = 0;

  // remove elements:
  virtual bool remove_element(std::int32_t tag) = 0;
  virtual bool remove_pixel_data() = 0;

  // regions:
  virtual void get_regions(onis::frame_region_list& list) const = 0;

  // transfer
  virtual void transfer_init() = 0;
  ...
  virtual std::int32_t read(
      ...
      dcm_group_len_encoding group_encoding = no_change_in_gl,
      std::uint32_t max_element_read_len = 4096) = 0;
  virtual std::int32_t write(
      std::uint8_t* buffer, std::int32_t length, std::string transfer_syntax,
      std::int32_t* write_out, dcm_encoding_type encoding_type,
      dcm_group_len_encoding group_encoding,
      dcm_padding_encoding padding = no_change_in_padding) = 0;

  // vr:
  virtual bool get_vr(std::int32_t tag, std::string& vr) = 0;
...
typedef std::weak_ptr<dicom_base> dicom_base_wptr;

///////////////////////////////////////////////////////////////////////
// dicom_dataset (dicom file without meta header)
///////////////////////////////////////////////////////////////////////

class dicom_dataset : public virtual dicom_base,
                      public std::enable_shared_from_this<dicom_dataset> {
public:
protected:
  // constructor:
  dicom_dataset() : dicom_base() {}

  // destructor:
  virtual ~dicom_dataset() {}

  // prevent copy and assignment:
  dicom_dataset(const dicom_dataset&) = delete;
  dicom_dataset& operator=(const dicom_dataset&) = delete;
  dicom_dataset(dicom_dataset&&) = delete;
  dicom_dataset& operator=(dicom_dataset&&) = delete;
};

typedef std::shared_ptr<dicom_dataset> dicom_dataset_ptr;
typedef std::weak_ptr<dicom_dataset> dicom_dataset_wptr;

///////////////////////////////////////////////////////////////////////
// dicom_file (dicom file with meta header)
///////////////////////////////////////////////////////////////////////

class dicom_file : public virtual dicom_base,
                   public std::enable_shared_from_this<dicom_file> {
public:
  // prevent copy and assignment:
  dicom_file(const dicom_file&) = delete;
  dicom_file& operator=(const dicom_file&) = delete;
  dicom_file(dicom_file&&) = delete;
  dicom_file& operator=(dicom_file&&) = delete;

  // loading:
  virtual bool load_file(const std::string& path,
                         std::int32_t retry_interval = 0,
                         std::int32_t limit = 0) = 0;
  ...
  // properties:
  virtual bool is_temporary_file() const = 0;
  virtual void set_temporary_file(bool temp) = 0;
  virtual std::string get_file_path() const = 0;

  // saving:
  // virtual bool save_file(const std::string &utf8_path, const std::string
  // &transfer_syntax="", bool set_file_path = OSFALSE) = 0;
  virtual bool save_file(const std::string& path,
                         const std::string& transfer_syntax = "",
                         bool set_file_path = false) = 0;

  // palette:
  virtual onis::dicom_raw_palette* get_raw_palette(
      std::int32_t channel) const = 0;

  // frames:
  virtual dicom_frame_ptr extract_frame(std::int32_t index) = 0;

  // pixels:
  virtual bool get_pixel_data_positions(std::uint64_t* start,
                                        std::uint64_t* end) = 0;
  virtual dicom_frame_offsets* get_pixel_data_positions(
      std::int32_t& count) = 0;
  virtual bool remove_private_tags_with_pixel_data() = 0;

  // frames:
  virtual bool set_image(onis::dicom_image_info* info) = 0;
  ...
  virtual bool add_streaming_data(std::uint8_t* data, std::uint32_t len,
                                  std::uint64_t total_expected = -1) = 0;
  virtual bool is_streaming() = 0;
  virtual bool streaming_is_complete() = 0;
  virtual std::uint64_t get_stream_data_len() = 0;
  virtual std::uint64_t get_stream_data_expected_len() = 0;
  virtual std::uint32_t get_stream_bit_rate() = 0;
  virtual void set_stream_bit_rate(std::uint32_t rate) = 0;
  virtual onis::file_ptr get_streaming_file() = 0;
  virtual void get_streaming_status() = 0;

  // mpeg frame:
  virtual bool is_mpeg_frame() = 0;
  /*virtual onis::bitmap_ptr update_mpeg_frame(const onis::bitmap_ptr& bmp,
                                             bool can_retain) = 0;
  virtual onis::bitmap_ptr get_mpeg_frame(bool copy = OSTRUE) = 0;*/

protected:
  // constructor:
  dicom_file() : dicom_base() {}

  // destructor:
  virtual ~dicom_file() {}
};

typedef std::shared_ptr<dicom_file> dicom_file_ptr;
typedef std::weak_ptr<dicom_file> dicom_file_wptr;
...
class dicom_sequence_item
    : public virtual dicom_base,
      public std::enable_shared_from_this<dicom_sequence_item> {
protected:
  // constructor:
  dicom_sequence_item() : dicom_base() {}

  // destructor:
  virtual ~dicom_sequence_item() {}
};
...
typedef std::shared_ptr<dicom_palette> dicom_palette_ptr;
typedef std::weak_ptr<dicom_palette> dicom_palette_wptr;
...
public:
  // constructor:
  dicom_overlay() = default;

  // destructor:
  virtual ~dicom_overlay() {
    delete[] data;
  }
...
// unit types:
const std::int32_t os_unit_invalid = 0xFFFFFF;
const std::int32_t os_unit_none = 0x00;
const std::int32_t os_unit_percent = 0x01;
const std::int32_t os_unit_decibel = 0x02;
const std::int32_t os_unit_cm = 0x03;
const std::int32_t os_unit_second = 0x04;
const std::int32_t os_unit_hertz = 0x05;
const std::int32_t os_unit_decibel_per_second = 0x06;
const std::int32_t os_unit_cm_per_second = 0x07;
const std::int32_t os_unit_cm2 = 0x08;
const std::int32_t os_unit_cm2_per_second = 0x09;
const std::int32_t os_unit_cm3 = 0x0A;
const std::int32_t os_unit_cm3_per_second = 0x0B;
const std::int32_t os_unit_degrees = 0x0C;

// region spatial format:
const std::int32_t os_rsf_none = 0x00;
const std::int32_t os_rsf_2d = 0x01;
const std::int32_t os_rsf_mmode = 0x02;
const std::int32_t os_rsf_spectral = 0x03;
const std::int32_t os_rsf_wave_form = 0x04;
const std::int32_t os_rsf_graphics = 0x05;

// region data type:
const std::int32_t os_rdt_none = 0x00;
const std::int32_t os_rdt_tissue = 0x01;
const std::int32_t os_rdt_color_flow = 0x02;
const std::int32_t os_rdt_pw_spectral_doppler = 0x03;
const std::int32_t os_rdt_cw_spectral_doppler = 0x04;
const std::int32_t os_rdt_doppler_mean_trace = 0x05;
const std::int32_t os_rdt_doppler_mode_trace = 0x06;
const std::int32_t os_rdt_doppler_max_trace = 0x07;
const std::int32_t os_rdt_volume_trace = 0x08;
const std::int32_t os_rdt_dvolume_per_dt = 0x09;
const std::int32_t os_rdt_ecg_trace = 0xA0;
const std::int32_t os_rdt_pulse_trace = 0xB0;
const std::int32_t os_rdt_phonocardiogram_trace = 0xC0;
const std::int32_t os_rdt_gray_bar = 0xD0;
const std::int32_t os_rdt_color_bar = 0xE0;
const std::int32_t os_rdt_integrated_backscatter = 0xF0;
const std::int32_t os_rdt_area_trace = 0x10;
const std::int32_t os_rdt_darea_per_dt = 0x11;

class frame_region {
public:
  // static constructor:
  static frame_region_ptr create();

  // constructor:
  frame_region() = default;

  // destructor:
  virtual ~frame_region() = default;

  // prevent copy and assignment:
  frame_region(const frame_region&) = delete;
  frame_region& operator=(const frame_region&) = delete;
  frame_region(frame_region&&) = delete;
  frame_region& operator=(frame_region&&) = delete;

  // operators:
  bool operator==(const frame_region& other) const;
  bool operator!=(const frame_region& other) const;

  // clone:
  frame_region_ptr clone() const;

  // properties:
  std::int32_t spatial_format{os_rsf_2d};
  std::int32_t data_type{os_rdt_none};
  double original_spacing[2]{1.0, 1.0};
  std::int32_t original_unit[2]{os_unit_none, os_unit_none};
  double calibrated_spacing[2]{1.0, 1.0};
  std::int32_t calibrated_unit[2]{os_unit_none, os_unit_none};
  std::int32_t x0{0}, x1{0};
...
class frame_region_info;
typedef std::shared_ptr<frame_region_info> frame_region_info_ptr;
typedef std::weak_ptr<frame_region_info> frame_region_info_wptr;
typedef std::vector<frame_region_info_ptr> frame_region_info_list;

class frame_region_info {
public:
  // static constructor:
  static frame_region_info_ptr create();

  // constructor:
  frame_region_info() = default;

  // destructor:
  virtual ~frame_region_info() = default;

  // clone:
  frame_region_info_ptr clone() const;

  // properties:
  std::int32_t dimensions[2]{0, 0};
  frame_region_list regions;
};
...
class dicom_frame : public std::enable_shared_from_this<dicom_frame> {
public:
  // prevent copy and assignment:
  dicom_frame(const dicom_frame&) = delete;
  dicom_frame& operator=(const dicom_frame&) = delete;
  dicom_frame(dicom_frame&&) = delete;
  ...
  // voi lut:
  virtual void set_voi_lut_function(std::int32_t mode) = 0;
  ...
  // min-max values:
  virtual bool get_min_max_values(double* min_val, double* max_val,
                                  bool intermediate) const = 0;
  virtual bool set_min_max_values(double min_val, double max_val,
                                  bool intermediate) = 0;

  // rescale/intercept:
  virtual bool get_rescale_and_intercept(double* rescale,
                                         double* intercept) = 0;
  virtual bool set_rescale_and_intercept(double rescale, double intercept) = 0;
...
class dicom_dir : public std::enable_shared_from_this<dicom_dir> {
public:
  // prevent copy and assignment:
  dicom_dir(const dicom_dir&) = delete;
  dicom_dir& operator=(const dicom_dir&) = delete;
  dicom_dir(dicom_dir&&) = delete;
  dicom_dir& operator=(dicom_dir&&) = delete;

  // properties:
  virtual std::string get_full_path() const = 0;
  virtual std::string get_dir_path() const = 0;

  // operations:
  virtual bool create(const std::string& path) = 0;
  // virtual bool add_file(const std::string& relative_path) = 0;
  virtual bool add_file(const std::string& full_path,
...
protected:
  ...
  // destructor:
  virtual ~dicom_dir() = default;
};

typedef std::shared_ptr<dicom_dir> dicom_dir_ptr;
typedef std::weak_ptr<dicom_dir> dicom_dir_wptr;

///////////////////////////////////////////////////////////////////////
// dicom_manager
///////////////////////////////////////////////////////////////////////

class dicom_manager : public std::enable_shared_from_this<dicom_manager> {
public:
  // prevent copy and assignment:
  dicom_manager(const dicom_manager&) = delete;
  dicom_manager& operator=(const dicom_manager&) = delete;
  dicom_manager(dicom_manager&&) = delete;
  dicom_manager& operator=(dicom_manager&&) = delete;
  ...
  virtual const dicom_charset* find_character_set_by_escape(
      const std::string& escape, const onis::dicom_charset_info** info,
      bool* g0) const = 0;
  virtual const dicom_charset* find_character_set_by_iso_number(
      const std::string& number,
      const onis::dicom_charset_info** info) const = 0;
  virtual const dicom_charset* find_character_set_by_info(
      const onis::dicom_charset_info* info) const = 0;
  virtual const dicom_charset_list* get_character_set_list() const = 0;
  virtual const dicom_charset* get_default_character_set() const = 0;

  // utilities:
  virtual void create_instance_uid(std::int32_t level,
                                   std::string& uid) const = 0;

protected:
  // constructor:
  dicom_manager() = default;
